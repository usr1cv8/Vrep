	...
	If StructuralUnitPayee.StructuralUnitType = Enums.StructuralUnitsTypes.Retail
	 OR StructuralUnitPayee.StructuralUnitType = Enums.StructuralUnitsTypes.RetailAccrualAccounting Then
		...
		While SelectionOfQueryResult.Next() Do
			...
			MessageText = NStr("en='For products and services %ProductsAndServicesPresentation% in string %LineNumber% of the ""Inventory"" list the retail price is not set!';ru='Для номенклатуры %ПредставлениеНоменклатуры% в строке %НомерСтроки% списка ""Запасы"" не установлена розничная цена!';vi='Đối với mặt hàng %ProductsAndServicesPresentation% tại dòng %LineNumber% trong danh sách ""Vật tư"" chưa đặt đơn giá bán lẻ!'");
...
Procedure Filling(FillingData, StandardProcessing) Export
	...
	ElsIf TypeOf(FillingData) = Type("DocumentRef.InventoryAssembly") 
		...
		|		END AS Reserve
		|	) AS Products,
		|	InventoryAssembly.CustomerOrderPosition AS CustomerOrderPosition
		|FROM
	...
	ElsIf TypeOf(FillingData) = Type("DocumentRef.ProductionOrder")
		...
		|////////////////////////////////////////////////////////////////////////////////
		|SELECT
		|	T.ProductsAndServices AS ProductsAndServices,
		|	T.Characteristic AS Characteristic,
		|	SUM(T.Quantity) AS Quantity,
		|	T.MeasurementUnit AS MeasurementUnit,
		|	SUM(T.Reserve) AS Reserve,
		|	T.CustomerOrder AS CustomerOrder
		|FROM
		|	(SELECT
		...
		|		ProductionOrderInventory.CustomerOrder AS CustomerOrder
		|	FROM
		...
		|		-InventoryTransferInventory.Quantity,
		...
		|	WHERE
		|		InventoryTransferInventory.Ref.Posted = TRUE
		|		AND InventoryTransferInventory.Ref.BasisDocument = &BasisDocument) AS T
		|
		|GROUP BY
		...
		HeaderData = QueryResult[0];
		If Not HeaderData.IsEmpty() Then
			
			QueryResultSelection = HeaderData.Select();
			QueryResultSelection.Next();
			
			FillPropertyValues(ThisObject, QueryResultSelection);
		EndIf;
	
		InventoryData = QueryResult[1];
		If Not InventoryData.IsEmpty() Then
			
			SelectionInventory = InventoryData.Select();
			While SelectionInventory.Next() Do
				NewRow = Inventory.Add();
				FillPropertyValues(NewRow, SelectionInventory);
			EndDo;
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
		...
		For Each StringInventory IN Inventory Do
			...
			If StringInventory.Reserve > StringInventory.Quantity Then
				...
				MessageText = NStr("en='In row No. %Number% of the ""Inventory"" tabular section, the quantity of items transferred to reserve exceeds the total inventory quantity.';
					|ru='В строке №%Номер% табл. части ""Запасы"" количество передаваемых в резерв позиций превышает общее количество запасов.';
					|vi='Tại dòng số %Number% của phần bảng ""Vật tư"" số lượng mặt hàng chuyển giao vào dự phòng vượt quá tổng số lượng vật tư.""'");
	...
	If Not OperationKind = Enums.OperationKindsInventoryTransfer.Move Then
...
Procedure FillStructuralUnitsTypes() Export
	...
	StructuralUnitPayeeType = CommonUse.ObjectAttributeValue(StructuralUnitPayee, "StructuralUnitType");
