	 ...
	 OR ReturnFromRetailAccrualAccounting Then
		...
		If AmountMarkup <> 0 Then
			...
			If TableRow <> Undefined Then
				...
				RowTableManagerial.Content = NStr("en='Markup';ru='Торговая наценка';vi='Phụ thu thương mại'");
...
Procedure GenerateTableRetailAmountAccounting(DocumentRefInventoryTransfer, StructureAdditionalProperties)
	...
	Query.SetParameter("RetailTransfer", NStr("en='Move to retail';ru='Перемещение в розницу';vi='Điều chuyển trong bán lẻ'"));
	Query.SetParameter("RetailTransfer", NStr("en='Movement in retail';ru='Перемещение в рознице';vi='Chuyển tiền vào bán lẻ'"));
	Query.SetParameter("ReturnAndRetail", NStr("en='Return from retail';ru='Возврат из розницы';vi='Nhận hàng bán lẻ bị trả lại'"));
	Query.SetParameter("ExchangeDifference", NStr("en='Exchange rate difference';ru='Курсовая разница';vi='Chênh lệch tỷ giá'"));
...
Procedure InitializeDocumentData(DocumentRefInventoryTransfer, StructureAdditionalProperties) Export
	...
	|	InventoryTransferInventory.CountryOfOrigin AS CountryOfOrigin,
	|	InventoryTransferInventory.CCDNo AS CCDNo,
	|	CASE
	|		WHEN InventoryTransferInventory.Ref.OperationKind = VALUE(Enum.OperationKindsInventoryTransfer.Move)
	|				OR InventoryTransferInventory.Ref.OperationKind = VALUE(Enum.OperationKindsInventoryTransfer.ReturnFromExploitation)
	...
	ExpenseRecord = Not(DocumentRefInventoryTransfer.OperationKind = Enums.OperationKindsInventoryTransfer.ReturnFromExploitation);
	SmallBusinessServer.GenerateTableInventoryByCCD(DocumentRefInventoryTransfer, StructureAdditionalProperties, ExpenseRecord);
	...
	If StructureTemporaryTables.RegisterRecordsInventoryInWarehousesChange
	 ...
	 OR StructureTemporaryTables.RegisterRecordsSerialNumbersChange Then
		...
		If Not ResultsArray[4].IsEmpty() Then
			...
			SmallBusinessServer.ShowMessageAboutPostingToInventoryByCCDRegisterErrors(DocumentObjectInventoryTransfer, QueryResultSelection, Cancel);
		EndIf;
	...
	For Each CurrentDocument IN ObjectsArray Do
		...
		If TemplateName = "InventoryTransfer" Then
			...
			TemplateArea.Parameters.HeaderText =
				NStr("en='Inventory transfer #';vi='Phiếu điều chuyển vật tư số ';")
			  + DocumentNumber
		...
		ElsIf TemplateName = "MerchandiseFillingFormSender" Then
			...
			TemplateArea.Parameters.PrintingTime =
				NStr("en='Date and time of printing: ';vi='Ngày và thời gian in';")
			  + CurrentDate()
		...
		ElsIf TemplateName = "WarehouseOutputSlipAndInternalTransportation" Then
			...
			|	InventoryTransfer.StructuralUnitPayee AS StructuralUnitPayee,
			|	InventoryTransfer.StructuralUnit.FRP AS OutputWarehouseMan,
			|	InventoryTransfer.StructuralUnitPayee.FRP AS InputWarehouseMan,
			|	InventoryTransfer.Author AS Author,
			|	InventoryTransfer.Comment AS Comment,
			|	InventoryTransfer.Inventory.(
			|		LineNumber AS LineNumber,
			|		CASE
			|			WHEN (CAST(InventoryTransfer.Inventory.ProductsAndServices.DescriptionFull AS STRING(1000))) = """"
			|				THEN InventoryTransfer.Inventory.ProductsAndServices.Description
			|			ELSE CAST(InventoryTransfer.Inventory.ProductsAndServices.DescriptionFull AS STRING(1000))
			|		END AS Product,
			|		ProductsAndServices.SKU AS SKU,
			...
			SpreadsheetDocument.PrintParametersName = "PRINT_PARAMETERS_PF_MXL_WarehouseOutputSlipAndInternalTransportation";
			
			Template = PrintManagement.GetTemplate("Document.InventoryTransfer.PF_MXL_WarehouseOutputSlipAndInternalTransportation");
			...
			FillStructureSection.Insert("TIN", 		InfoAboutCompany.TIN);
...
Procedure Print(ObjectsArray, PrintParameters, PrintFormsCollection, PrintObjects, OutputParameters) Export
	...
	// 1C - CHIENTN - 18/02/2019
	If PrintManagement.NeedToPrintTemplate(PrintFormsCollection, "WarehouseOutputSlipAndInternalTransportation") Then
		
		PrintManagement.OutputSpreadsheetDocumentToCollection(PrintFormsCollection, "WarehouseOutputSlipAndInternalTransportation", "Warehouse output slip and internal transportation", PrintForm(ObjectsArray, PrintObjects, "WarehouseOutputSlipAndInternalTransportation"));
...
Procedure AddPrintCommands(PrintCommands) Export
	...
	PrintCommand.ID = "WarehouseOutputSlipAndInternalTransportation";
	PrintCommand.Presentation = NStr("en='Warehouse output slip and internal transportation';ru='Warehouse output slip and internal transportation';vi='Phiếu xuất kho kiêm vận chuyển nội bộ'");
	...
	PrintCommand.Presentation = NStr("en='Inventory movement';ru='Перемещение запасов';vi='Điều chuyển vật tư'");
	PrintCommand.FormsList = "DocumentForm,ListForm";
	PrintCommand.CheckPostingBeforePrint = False;
	...
	PrintCommand.Presentation = NStr("en='Goods content form (Sender)';ru='Бланк товарного наполнения (Отправитель)';vi='Mẫu bổ sung hàng hóa (Người gửi)'");
	PrintCommand.FormsList = "DocumentForm,ListForm";
	PrintCommand.CheckPostingBeforePrint = False;
	...
	PrintCommand.Presentation = NStr("en='Goods content form (Recipient)';ru='Бланк товарного наполнения (Получатель)';vi='Mẫu bổ sung hàng hóa (Người nhận)'");
	PrintCommand.FormsList = "DocumentForm,ListForm";
	PrintCommand.CheckPostingBeforePrint = False;
