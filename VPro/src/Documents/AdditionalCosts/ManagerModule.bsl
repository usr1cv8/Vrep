...
Procedure GenerateTableManagerial(DocumentRefAdditionalCosts, StructureAdditionalProperties)
	...
	Query.SetParameter("InventoryReceipt", NStr("en='Add. expense capitalization';ru='Оприходование доп. расходов';vi='Ghi tăng chi phí bổ sung'"));
...
Procedure InitializeDocumentData(DocumentRefAdditionalCosts, StructureAdditionalProperties) Export
	...
	"SELECT
	|	CASE
	|		WHEN AdditionalCosts.Contract.IsServiceContract
	|		AND
	|			AdditionalCosts.Contract.ServiceContractBusinessActivity <> VALUE(Catalog.BusinessActivities.EmptyRef)
	|			THEN TRUE
	|		ELSE FALSE
	|	END AS KeepAccountingExpensesByServiceContracts,
	|	AdditionalCosts.Contract.ServiceContractBusinessActivity AS BusinessActivity
	...
	|		ELSE AdditionalCostsExpenses.ProductsAndServices.BusinessActivity
