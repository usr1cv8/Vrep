...
Procedure DistributeExpensesByQuantity(Command)
	...
	If Object.Inventory.Count() = 0 Then
		SmallBusinessClient.ShowMessageAboutError(Object, NStr("en='No records in tabular section ""Inventory"".';ru='В табличной части ""Запасы"" нет записей!';vi='Trong phần bảng ""Vật tư"" không có bản ghi!'"));
		Return;
	EndIf;
	
	If Object.Expenses.Count() = 0 Then
		SmallBusinessClient.ShowMessageAboutError(Object, NStr("en='There are no records in the tabular section ""Services"".';ru='В табличной части ""Услуги"" нет записей!';vi='Trong phần bảng ""Dịch vụ"" không có bản ghi!'"));
		Return;
	EndIf;
...
Procedure PurchaseOrderOnChange(Item)
	...
	If Object.Prepayment.Count() > 0
	   ...
	   AND CounterpartyDoSettlementsByOrders Then
		...
		ShowQueryBox(New NotifyDescription("PurchaseOrderOnChangeEnd", ThisObject), NStr("en='Prepayment setoff will be cleared, continue?';ru='Зачет предоплаты будет очищен, продолжить?';vi='Sẽ hủy việc khấu trừ khoản trả trước, tiếp tục?'"), Mode, 0);
		Return;
	EndIf;
