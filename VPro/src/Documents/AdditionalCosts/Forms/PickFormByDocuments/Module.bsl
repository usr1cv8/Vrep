...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	For Each ReceiptDocument IN ArrayOfSelectedDocuments Do
		...
		If ArrayOfFoundDocuments.Count() > 0 Then
			...
			MessageText = NStr("en='The %DocumentPerformance% document is already present in the selected document list.';ru='Документ %DocumentPerformance% уже присутствует в списке выбранных документов.';vi='Chứng từ %DocumentPerformance% đã có trong danh sách chứng từ được chọn.'");
...
Procedure DocumentsMultiplePick(Command)
	...
	ListOfDocumentTypes.Add("ExpenseReport", NStr("en='Expense report';ru='Авансовый отчет';vi='Giấy thanh toán tiền tạm ứng'"));
	ListOfDocumentTypes.Add("SupplierInvoice", NStr("en='Supplier invoice';ru='Приходная накладная';vi='Hóa đơn nhận hàng'"));
	
	Notification = New NotifyDescription("MultiplePickupDocumentsCompletion",ThisForm);
	ListOfDocumentTypes.ShowChooseItem(Notification, NStr("en='Select the document type for add';ru='Выберите тип документа для добавления';vi='Chọn loại tài liệu để thêm'"));
...
Procedure FillByFilteredDocuments(Command)
	...
	If FilteredInventory.Count() > 0 Then
		...
		QuestionText = NStr("en='Tabular section will be cleared and filled in again. Continue?';ru='Табличная часть будит очищена и повторно заполнена. Продолжить?';vi='Phần bảng sẽ bị xóa và điền lại. Tiếp tục?'");
...
Procedure FillByCurrentDocument(Command)
	...
	If CurrentRowOfReceiptDocuments = Undefined Then
		...
		CommonUseClientServer.MessageToUser(
			NStr("en='Select a line with document and try to fill in again';ru='Выделите строку с документом и повторите попытку заполнить';vi='Hãy chọn dòng có chứng từ và thử điền lại'"),
			,
			"FilteredDocuments",
			,
			);
