...
Procedure Calculate(Command)
	...
	If Object.Posted Then
		ShowMessageBox(Undefined,NStr("en='Depreciation can be calculated only in non-posted document.';ru='Рассчет амортизации возможен только в непроведенном документе.';vi='Chỉ có thể tính khấu hao trong chứng từ chưa kết chuyển.'"));
		Return;
	EndIf;
	
	ShowQueryBox(New NotifyDescription("CalculateEnd", ThisObject), NStr("en='Entered data will be recalculated. Continue?';ru='Введенные данные будут пересчитаны! Продолжить?';vi='Dữ liệu đã nhập sẽ được tính lại. Tiếp tục?'"),QuestionDialogMode.YesNo);
