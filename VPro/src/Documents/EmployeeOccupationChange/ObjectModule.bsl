...
Procedure RunControl(AdditionalProperties, Cancel) 
	...
	While QueryResultSelection.Next() Do
		If Not ValueIsFilled(QueryResultSelection.StructuralUnit) Then
		    MessageText = NStr("en='In row No.%Number% of the ""Employees"" tabular section, it is indicated that the %Employee% employee is not hired to the %Company% company.';ru='В строке №%Номер% табл. части ""Сотрудники"" сотрудник %Сотрудник% не принят на работу в организацию %Организация%.';vi='Tại dòng số %Number% của phần bảng ""Người lao động"", người lao động %Employees% chưa được tiếp nhận vào làm việc trong doanh nghiệp %Company%.'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
