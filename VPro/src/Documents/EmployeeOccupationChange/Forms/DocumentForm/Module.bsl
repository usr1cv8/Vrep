...
Procedure FillCurrentEmployeesChoiceList()
	...
	For Each RowEmployee IN Object.Employees Do
		...
		RowPresentation = String(RowEmployee.Employee) + NStr("en=', employee code: ';ru=', ТН: ';vi='. mã nhân viên:'") + String(RowEmployee.Employee.Code);
...
Procedure FillAccrualsDeductions(Command)	
	...
	If TabularSectionRow = Undefined Then
		...
		Message.Text = NStr("en='The ""Employees"" row is not selected in the tabular section.';ru='Не выбрана строка табличной части ""Сотрудники""!';vi='Chưa chọn dòng của phần bảng ""Người lao động""!'");
		Message.Message();	
		Return;
	...
	If Object.AccrualsDeductions.FindRows(New Structure("ConnectionKey", TabularSectionRow.ConnectionKey)).Count() > 0 Then
		...
		ShowQueryBox(New NotifyDescription("FillDeductionAccrualsEnd", ThisObject, New Structure("TabularSectionRow", TabularSectionRow)), NStr("en='The ""Accruals and deductions"" tabular section will be cleared. Continue?';ru='Табличная часть ""Начисления и удержания"" будет очищена! Продолжить?';vi='Phần bảng ""Tính lương và giữ lại từ lương"" sẽ bị xóa! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
        Return; 
	EndIf;
