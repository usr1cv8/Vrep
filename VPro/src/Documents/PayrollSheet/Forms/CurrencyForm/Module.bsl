...
Procedure CheckFillOfFormAttributes(Cancel)
	...
	If Not ValueIsFilled(DocumentCurrency) Then
		...
		Message.Text = NStr("en='Currency for population is not selected.';ru='Не выбрана валюта для заполнения!';vi='Chưa chọn tiền tệ để điền!'");
	...
	If Not ValueIsFilled(SettlementsCurrency) Then
		Message = New UserMessage();
		Message.Text = NStr("en='Settlement currency for population is not selected.';ru='Не выбрана валюта расчетов для заполнения!';vi='Chưa chọn tiền tệ hạch toán để điền!'");
