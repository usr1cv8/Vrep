...
Procedure RunControl(AdditionalProperties, Cancel) 
	...
	While QueryResultSelection.Next() Do
		MessageText = NStr("en='In row No. %Number% of the ""Employees"" tabular section, there are personnel register records for employee %Employee% within %Period% after dismissal date.';ru='В строке №%Номер% табл. части ""Сотрудники"" по сотруднику %Сотрудник% есть кадровые движения %Период% после даты увольнения.';vi='Tại dòng số %Number% của phần bảng ""Người lao động"" theo người lao động %Employee% có điều chuyển nhân sự %Period% sau ngày thôi việc.'");
		MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
		MessageText = StrReplace(MessageText, "%Employee%", QueryResultSelection.Employee); 
