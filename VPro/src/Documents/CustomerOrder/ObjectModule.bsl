...
Procedure Filling(FillingData, StandardProcessing)
	...
	If TypeOf(FillingData)=Type("Structure")
		And FillingData.Property("Event")
		And ValueIsFilled(FillingData.Event)
		And FillingData.Event.EventType = Enums.EventTypes.Record Then
		РесурсыПредприятияСобытие = FillingData.Event.EnterpriseResources;
		For Each ResourcesRow In РесурсыПредприятияСобытие Do
			NewRow = EnterpriseResources.Add();
			FillPropertyValues(NewRow, ResourcesRow);
		EndDo;
		Start = FillingData.Event.EventBegin;
		Finish = FillingData.Event.EventEnding;
	EndIf;
...
Procedure Posting(Cancel, PostingMode)
	...
	// Company resources
	If GetFunctionalOption("PlanCompanyResourcesLoading") 
		And ThisObject.OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder Then
		...
		If ValueIsFilled(Event) And Not Event.DeletionMark 
			And Not Event.State = Catalogs.EventStates.FindByDescription("Canceled") Then
			ResourcePlanningCM.CreateRecordsEmployeeCalendarByResources(Event,, Cancel);
			ResourcePlanningCM.WriteScheduleResourceLoading(Event,,Cancel)
		EndIf;
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Inventory.Total("Reserve") > 0 Then
		...
		For Each StringInventory IN Inventory Do
			...
			AND Not ValueIsFilled(StructuralUnitReserve) Then
				...
				MessageText = NStr("en='Reserve warehouse is required.';ru='Не заполнен слад резерва';vi='Chưa điền kho dự phòng.'");
		...
		If OperationKind = Enums.OperationKindsCustomerOrder.OrderForSale Then
			...
			For Each StringInventory IN Inventory Do
				...
				If StringInventory.Reserve > StringInventory.Quantity Then
					...
					MessageText = NStr("en='The number of reserved positions exceeds the total inventory quantity in row No. %Number% of the ""Goods, services"" tabular section.';ru='В строке №%Номер% табл. части ""Товары, услуги"" количество резервируемых позиций превышает общее количество запасов.';vi='Tại dòng số %Number% của phần bảng ""Hàng hóa, dịch vụ"" số lượng mặt hàng dự phòng vượt quá tổng số vật tư.'");
		...
		ElsIf OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder Then
			...
			For Each StringInventory IN Inventory Do
				...
				If OrderState.OrderStatus = Enums.OrderStatuses.Completed
					...
					MessageText = NStr("en='The number of items written off from reserve exceeds the total inventory quantity in row No. %Number% of the Goods tabular section.';ru='В строке №%Номер% табл. части ""Товары"" количество списываемых позиций из резерва превышает общее количество запасов.';vi='Tại dòng số %Number% của phần bảng ""Hàng hóa"" số lượng mặt hàng được ghi giảm từ dự phòng vượt quá tổng số lượng vật tư.'");
					MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
					SmallBusinessServer.ShowMessageAboutError(
						ThisObject,
						MessageText,
				...
				ElsIf StringInventory.Reserve > StringInventory.Quantity Then	
					...
					MessageText = NStr("en='The number of reserved positions exceeds the total inventory quantity in row No. %Number% of the Goods tabular section.';ru='В строке №%Номер% табл. части ""Товары"" количество резервируемых позиций превышает общее количество запасов.';vi='Tại dòng số %Number% của phần bảng ""Hàng hóa"" số lượng mặt hàng dự phòng vượt quá tổng số lượng vật tư.'");
					MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
					SmallBusinessServer.ShowMessageAboutError(
						ThisObject,
						MessageText,
			...
			For Each StringInventory IN Materials Do
				...
				If OrderState.OrderStatus = Enums.OrderStatuses.Completed
					...
					MessageText = NStr("en='Quantity of items written off from reserve exceeds the total inventory quantity in row No. %Number% of the ""Materials"" tabular section.';ru='В строке №%Номер% табл. части ""Материалы"" количество списываемых позиций из резерва превышает общее количество запасов.';vi='Tại dòng số %Number% của phần bảng ""Nguyên vật liệu"" số lượng ghi giảm từ dự phòng lớn hơn tổng số lượng vật tư.'");
					MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
					SmallBusinessServer.ShowMessageAboutError(
						ThisObject,
						MessageText,
				...
				ElsIf StringInventory.Reserve > StringInventory.Quantity Then
					...
					MessageText = NStr("en='The number of reserved positions exceeds the total inventory quantity in row No. %Number% of the ""Materials"" tabular section.';ru='В строке №%Номер% табл. части ""Материалы"" количество резервируемых позиций превышает общее количество запасов.';vi='Tại dòng số %Number% của phần bảng ""Nguyên vật liệu"" số lượng dự phòng lớn hơn tổng số lượng vật tư.'");
					MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
					SmallBusinessServer.ShowMessageAboutError(
						ThisObject,
						MessageText,
	...
	If ThereAreManualDiscounts OR ThereAreAutomaticDiscounts Then
		For Each StringInventory IN Inventory Do
			...
			If StringInventory.DiscountMarkupPercent <> 100 AND CurAmountDiscounts < CurAmount
				...
				MessageText = NStr("en='The ""Amount"" column is not filled out in the %Number% row of the ""Goods, works, services"" list.';ru='Не заполнена колонка ""Сумма"" в строке %Номер% списка ""Товары, работы, услуги"".';vi='Chưa điền cột ""Thành tiền"" trong dòng %Number% danh sách ""Hàng hóa, dịch vụ"".'");
				MessageText = StrReplace(MessageText, "%Number%", StringInventory.LineNumber);
				SmallBusinessServer.ShowMessageAboutError(
					ThisObject,
					MessageText,
	...
	If ThereAreManualDiscounts Then
		For Each WorkRow IN Works Do
			...
			If WorkRow.DiscountMarkupPercent <> 100 AND CurAmountDiscounts < CurAmount
				...
				MessageText = NStr("en='The ""Amount"" column is not filled out in the %Number% row of the ""Works"" list.';ru='Не заполнена колонка ""Сумма"" в строке %Номер% списка ""Работы"".';vi='Chưa điền cột ""Thành tiền"" trong dòng %Number% danh sách ""Công việc"".'");
	...
	WorkOrder = (OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder);
	If WorkOrder Then
		
		TabularSectionNames = New Array;
		TabularSectionNames.Add("Inventory");
		TabularSectionNames.Add("Materials");
		
		CargoCustomsDeclarationsServer.OnProcessingFillingCheck(cancel, ThisObject, TabularSectionNames);
		
	EndIf;
	
		// Billing
	If OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder
		And GetFunctionalOption("UseBilling")
		And Contract.IsServiceContract Then
		
		For Each Str In Works Do
			If Not SmallBusinessServer.SoldProductsAndServicesByServiceContractAllowed(Contract, Str.ProductsAndServices) Then
				CommonUseClientServer.MessageToUser(
				NStr("en='Запрещено проводить незапланированные работы/услуги по текущему договору обслуживания!';ru='Запрещено проводить незапланированные работы/услуги по текущему договору обслуживания!';vi='Không được thực hiện các công việc / dịch vụ ngoài kế hoạch theo hợp đồng dịch vụ hiện hành!'"),
				Contract.ServiceContractTariffPlan,
				CommonUseClientServer.PathToTabularSection("Works", Str.LineNumber, "ProductsAndServices"),,
				Cancel
				);
			EndIf;
		EndDo;
		For Each Str In Inventory Do
			If Not SmallBusinessServer.SoldProductsAndServicesByServiceContractAllowed(Contract, Str.ProductsAndServices, Str.CHARACTERISTIC) Then
				CommonUseClientServer.MessageToUser(
				NStr("en='Запрещено проводить незапланированные товары по текущему договору обслуживания!';ru='Запрещено проводить незапланированные товары по текущему договору обслуживания!';vi='Không được phép mang hàng ngoài kế hoạch theo hợp đồng dịch vụ hiện hành!'"),
				Contract.ServiceContractTariffPlan,
				CommonUseClientServer.PathToTabularSection("Works", Str.LineNumber, "ProductsAndServices"),,
				Cancel
				);
...
Procedure BeforeWrite(Cancel, WriteMode, PostingMode)
	...
	If WriteMode=DocumentWriteMode.Posting
		And OperationKind=Enums.OperationKindsCustomerOrder.OrderForSale Then
		ProductionFormulasServer.CalculateParametricSpecifications(ThisObject, "Inventory", Cancel);
	EndIf; 
...
Procedure FillConsumerMaterialsBySpecification(NodesSpecificationStack, NodesTable = Undefined) Export
	...
	If NodesTable = Undefined Then
		...
		TableProduction = Inventory.Unload();
		Array = New Array();
		...
		For Each ProductsRow In TableProduction Do
			If ValueIsFilled(ProductsRow.MeasurementUnit)
				...
				ProductsRow.Factor = CommonUse.ObjectAttributeValue(ProductsRow.MeasurementUnit, "Factor");
			Else
	...
	While Selection.Next() Do
		If Selection.ContentRowType = Enums.SpecificationContentRowTypes.Node Then
			...
			If Not NodesSpecificationStack.Find(Selection.Specification) = Undefined Then
				TextOfMessage = NStr("en='При попытке заполнить табличную часть ""Материалы заказчика"" по спецификации,"
"обнаружено рекурсивное вхождение элемента';ru='При попытке заполнить табличную часть ""Материалы заказчика"" по спецификации,"
"обнаружено рекурсивное вхождение элемента';vi='Khi cố gắng điền vào phần bảng ""Nguyên vật liệu khách hàng"" theo bảng chi tiết nguyên vật liệu, đã tìm thấy sự xuất hiện đệ quy của một yếu tố đã tìm thấy sự xuất hiện đệ quy của một yếu tố'")+" "+Selection.ProductsAndServices+" "+NStr("en='в спецификации';ru='в спецификации';vi='trong bảng kê chi tiết'")+" "+Selection.ProductionSpecification+"
			...
			FillConsumerMaterialsBySpecification(NodesSpecificationStack, NodesTable);
		Else
	...
	ConsumerMaterials.GroupBy("ProductsAndServices, Characteristic, MeasurementUnit", "Quantity");
