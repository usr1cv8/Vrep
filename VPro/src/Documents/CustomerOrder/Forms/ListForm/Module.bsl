...
Procedure SetFilterCurrentWorks()
	...
	If Parameters.Property("PastPerformance") Then
		FormHeaderText = NStr("en='Customer orders: fulfillment is expired';ru='Заказы покупателей: просрочено выполнение';vi='Đơn hàng của khách: đã quá hạn thực hiện'");
		SmallBusinessClientServer.SetListFilterItem(List, "PastPerformance", True);
	EndIf;
	
	If Parameters.Property("OverduePayment") Then
		FormHeaderText = NStr("en='Customer orders: payment is overdue';ru='Заказы покупателей: просрочена оплата';vi='Đơn hàng của khách: đã quá hạn thanh toán'");
		SmallBusinessClientServer.SetListFilterItem(List, "OverduePayment", True);
	EndIf;
	
	If Parameters.Property("ForToday") Then
		FormHeaderText = NStr("en='Customer orders: for today';ru='Заказы покупателей: на сегодня';vi='Đơn hàng của khách: hôm nay'");
	...
	If Parameters.Property("AreNew") Then
		...
		FormHeaderText = NStr("en='Customer orders: new';ru='Заказы покупателей: новые';vi='Đơn hàng của khách: mới'");
		If UseStatuses Then
	...
	If Parameters.Property("InProcess") Then
		FormHeaderText = NStr("en='Customer orders: in progress';ru='Заказы покупателей: в работе';vi='Đơn hàng của khách: đang làm việc'");
