...
Procedure CounterpartyOnChangeFragment1(Val SettlementsCurrencyBeforeChange, Val ContractBeforeChange, Val StructureData)
    ...
    If (ValueIsFilled(Object.Contract)
        ...
        ShowMessageBox(New NotifyDescription("CounterpartyOnChangeEnd", ThisObject, New Structure("SettlementsCurrencyBeforeChange, StructureData", SettlementsCurrencyBeforeChange, StructureData)), NStr("en='Currency of settlements by contract with the counterparty changed. Check document currency.';ru='Изменилась валюта расчетов по договору с контрагентом! Необходимо проверить валюту документа!';vi='Đã thay đổi tiền tệ hạch toán theo hợp đồng với đối tác! Cần kiểm tra tiền tệ trên chứng từ!'"));
        Return;
