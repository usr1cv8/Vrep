...
Procedure SetFilterCurrentWorks()
	...
	If Parameters.Property("PastPerformance") Then
		FormHeaderText = NStr("en='Work orders: execution is overdue';ru='Заказ-наряды: просрочено выполнение';vi='Đơn hàng trọn gói: đã quá hạn thực hiện'");
		SmallBusinessClientServer.SetListFilterItem(List, "PastPerformance", True);
	EndIf;
	
	If Parameters.Property("OverduePayment") Then
		FormHeaderText = NStr("en='Work orders: payment is overdue';ru='Заказ-наряды: просрочена оплата';vi='Đơn hàng trọn gói: quá hạn thanh toán'");
		SmallBusinessClientServer.SetListFilterItem(List, "OverduePayment", True);
	EndIf;
	
	If Parameters.Property("ForToday") Then
		FormHeaderText = NStr("en='Work orders: for today';ru='Заказ-наряды: на сегодня';vi='Đơn hàng trọn gói: vào hôm nay'");
	...
	If Parameters.Property("New") Then
		...
		FormHeaderText = NStr("en='Work orders: new';ru='Заказ-наряды: новые';vi='Đơn hàng trọn gói: mới'");
		If UseStatuses Then
	...
	If Parameters.Property("InProcess") Then
		FormHeaderText = NStr("en='Work orders: in progess';ru='Заказ-наряды: в работе';vi='Đơn hàng trọn gói: đang làm việc'");
	...
	If Parameters.Property("Responsible") Then
		...
		EndIf;
		FormHeaderText = FormHeaderText + ", " + NStr("en='responsible';ru='ответственный';vi='chịu trách nhiệm'") + " " + Parameters.Responsible.Initials;
	EndIf;
