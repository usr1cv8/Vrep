...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	DataImportFromExternalSources.OnCreateAtServer(Metadata.Documents.CustomerOrder.TabularSections.Inventory, DataLoadSettings, ThisObject);
	...
	AdditionalParameters = PropertiesManagementOverridable.FillAdditionalParameters(Object, "AdditionalAttributes", , "GroupGlobalCommands");
	...
	If Object.Ref.IsEmpty() Then
		...
		UseAdditionalAttributesAndInformation = GetFunctionalOption("UseAdditionalAttributesAndInformation");
	EndIf; 
	
	SetFormConditionalAppearance();
	
	FormManagement();
	UpdateSpecificationParametersColumnVisibility(ThisObject);
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	ProductionFormulasServer.CheckSpecificationsAdditAttributesFilling(Object, "Inventory", Cancel);
...
Procedure InventorySelection(Item, SelectedRow, Field, StandardProcessing)
	...
	TabularSectionRow = Object.Inventory.FindByID(SelectedRow);
	
	// ParametricSpecifications
	If Not ReadOnly 
		And TabularSectionRow.HasSpecificationAdditionalAttributes
		And Item.CurrentItem = Items.InventorySpecificationParameters Then
		
		StandardProcessing = False;
		OpenSpecificationParametersEditForm();
...
Procedure InventoryBeforeDeleteRow(Item, Cancel)
	...
	// ParametricSpecifications
	SmallBusinessClientServer.DeleteRowsByConnectionKey(Object.ParametricSpecificationsAttributes, CurrentData); 
	// End ParametricSpecifications
...
Procedure InventoryOnStartEdit(Item, NewRow, Copy)
	...
	If NewRow Then
		ObjectsFillingCMClient.FillRowByHeader(TabularSectionRow, Object, "ShipmentDate", "ShipmentDatePosition");
	EndIf; 
...
Procedure InventoryProductsAndServicesOnChange(Item)
	...
	If TabularSectionRow = Undefined Then Return EndIf;
	...
	If Object.OperationKind = PredefinedValue("Enum.OperationKindsCustomerOrder.OrderForSale")
		Then
		StructureData.Insert("WithTemplates", True);
	EndIf;
	...
	TabularSectionRow.IsSpecificationTemplate = StructureData.IsSpecificationTemplate;
	TabularSectionRow.HasSpecificationAdditionalAttributes = StructureData.HasSpecificationAdditionalAttributes;
	If TabularSectionRow.HasSpecificationAdditionalAttributes Then
		If TabularSectionRow.ConnectionKey<>0 Then
			SmallBusinessClientServer.DeleteRowsByConnectionKey(Object.ParametricSpecificationsAttributes, TabularSectionRow);
		EndIf; 
		FilAdditionalAttributesByDefault(TabularSectionRow.GetID());
		UpdateAdditionalAttributesDescription(ThisObject, TabularSectionRow.ConnectionKey);
	EndIf; 
...
Procedure InventorySpecificationOnChange(Item)
	...
	StructureData = New Structure;
	StructureData.Insert("Specification", TabularSectionRow.Specification);
	
	StructureData = GetSpecificationDataOnChange(StructureData);
...
&AtClient
Procedure InventorySpecificationParametersStartChoice(Item, ChoiceData, StandardProcessing)
...
&AtClient
Procedure OpenSpecificationParametersEditForm()
	
	CurrentRow = Items.Inventory.CurrentData;
	
	If Not ValueIsFilled(CurrentRow.Specification) Then
		Return;
	EndIf;
	...
	OpeningStructure.Insert("ConnectionKey", CurrentRow.ConnectionKey);
	RowsAttributes = Object.ParametricSpecificationsAttributes.FindRows(OpeningStructure);
	OpeningStructure.Insert("Specification", CurrentRow.Specification);
	OpeningStructure.Insert("Attributes", New Array);
	For Each TabularSectionRow In RowsAttributes Do
		StringStructure = New Structure("Property, Value");
		FillPropertyValues(StringStructure, TabularSectionRow);
		OpeningStructure.Attributes.Add(StringStructure);
	EndDo;
	Notification = New NotifyDescription("InventorySpecificationParametersChoiceCompletion", ThisObject);
	
	OpenForm("Catalog.Specifications.Form.SpecificationAdditionalAttributes", OpeningStructure, , , , , Notification, FormWindowOpeningMode.LockOwnerWindow);
 
EndProcedure

&AtClient
Procedure InventorySpecificationParametersChoiceCompletion(SelectedValue, AdditionalParameters) Export
	
	If SelectedValue=Undefined Then
		Return;
	EndIf; 	
	
	SmallBusinessClientServer.DeleteRowsByConnectionKey(Object.ParametricSpecificationsAttributes, SelectedValue); 
	For Each AdditionalAttributesData In SelectedValue.Attributes Do
		NewRow = Object.ParametricSpecificationsAttributes.Add();
		FillPropertyValues(NewRow, AdditionalAttributesData);
		NewRow.ConnectionKey = SelectedValue.ConnectionKey;
	EndDo;
	
	UpdateAdditionalAttributesDescription(ThisObject, SelectedValue.ConnectionKey);
...
Procedure ShipmentDateOnChange(Item)
	...
	For Each TabularSectionRow In Object.Inventory Do
		If TypeOf(TabularSectionRow.ProductsAndServices)<>Type("CatalogRef.ProductsAndServices") Then
			Continue;
		EndIf; 
		TabularSectionRow.ShipmentDate = Object.ShipmentDate;
	EndDo; 
...
Procedure ChangeReserveFillByBalances(Command)
	...
	If Object.Inventory.Count() = 0 Then
		Message = New UserMessage;
		Message.Text = NStr("en='The ""Goods, services"" tabular section is not filled in.';ru='Табличная часть ""Товары, услуги"" не заполнена!';vi='Phần bảng ""Hàng hóa, dịch vụ"" chưa điền!'");
		Message.Message();
		Return;
	EndIf;
...
Procedure FillBySpecification(Command)
	...
	If Object.ConsumerMaterials.Count() <> 0 Then
		...
		ShowQueryBox(New NotifyDescription("FillBySpecificationEnd", ThisObject), NStr("en='Табличная часть ""Материалы заказчика"" будет перезаполнена! Продолжить выполнение операции?';ru='Табличная часть ""Материалы заказчика"" будет перезаполнена! Продолжить выполнение операции?';vi='Phần bảng ""Nguyên vật liệu khách hàng"" sẽ được điền lại! Tiếp tục thực hiện?'"), 
...
EndProcedure // ЗаполнитьПоСпецификации()
...
Procedure FillBySpecificationsAtServer()
	...
	Document.FillConsumerMaterialsBySpecification(NodesSpecificationStack);
	ValueToFormAttribute(Document, "Object");

	ActionsAfterReadObject();
	
EndProcedure // ЗаполнитьПоСпецификацииНаСервере()
...
Procedure ProcessPricesKindAndSettlementsCurrencyChange(DocumentParameters)
	...
	If OpenFormPricesAndCurrencies Then
		...
		If PriceKindChanged OR DiscountKindChanged Then
			...
			WarningText = NStr("en='The price and discount conditions in the contract with counterparty"
"differ from price and discount in the document! "
"Perhaps you have to refill prices.';ru='Договор с контрагентом предусматривает условия цен и скидок, "
"отличные от установленных в документе! "
"Возможно, необходимо перезаполнить цены.';vi='Hợp đồng với đối tác có đặt điều kiện đơn giá và chiết khấu,"
"khác với các điều kiện đã đặt trong chứng từ!"
"Có thể là cần điền lại đơn giá.'") + Chars.LF + Chars.LF;
...
Function GetDataProductsAndServicesOnChange(StructureData)
	...
	ProductsAndServicesData = CommonUse.ObjectAttributesValues(StructureData.ProductsAndServices,
		"MeasurementUnit, ProductsAndServicesType, VATRate, CountryOfOrigin, FixedCost");
	
	StructureData.Insert("MeasurementUnit", StructureData.ProductsAndServices.MeasurementUnit);
	
	StructureData.Insert("IsService", ProductsAndServicesData.ProductsAndServicesType = PredefinedValue("Enum.ProductsAndServicesTypes.Service"));
	StructureData.Insert("IsInventoryItem", ProductsAndServicesData.ProductsAndServicesType = PredefinedValue("Enum.ProductsAndServicesTypes.InventoryItem"));
	...
	If StructureData.Property("PriceKind") Then
		...
		If StructureData.Property("WorkKind") Then
			...
			If ProductsAndServicesData.FixedCost Then
	...
	WithTemplates = (StructureData.Property("WithTemplates") And StructureData.WithTemplates);
	If StructureData.Property("Characteristic") Then
		StructureData.Insert("Specification", SmallBusinessServer.GetDefaultSpecification(StructureData.ProductsAndServices, StructureData.Characteristic, WithTemplates));
	Else
		StructureData.Insert("Specification", SmallBusinessServer.GetDefaultSpecification(StructureData.ProductsAndServices, , WithTemplates));
	EndIf;
	...
	For Each CurBarcode IN StructureData.BarcodesArray Do
		...
		Else
			...
			If TSRowsArray.Count() = 0 Then
				...
				ObjectsFillingCMClient.FillRowByHeader(NewRow, Object, "ShipmentDate", "ShipmentDatePosition");
				...
				NewRow.IsSpecificationTemplate = BarcodeData.StructureProductsAndServicesData.IsSpecificationTemplate;
				NewRow.HasSpecificationAdditionalAttributes = BarcodeData.StructureProductsAndServicesData.HasSpecificationAdditionalAttributes;
				If NewRow.HasSpecificationAdditionalAttributes Then
					If NewRow.ConnectionKey<>0 Then
						SmallBusinessClientServer.DeleteRowsByConnectionKey(Object.ParametricSpecificationsAttributes, NewRow);
					EndIf; 
					FilAdditionalAttributesByDefault(NewRow.GetID());
					UpdateAdditionalAttributesDescription(ThisObject, NewRow.ConnectionKey);
				EndIf; 
	...
	If TypeOf(ClosingResult) = Type("Structure") 
		...
		ElsIf ValueIsFilled(ClosingResult.DiscountCard) AND ValueIsFilled(ClosingResult.Counterparty) AND Object.Counterparty.IsEmpty() Then
			...
			ShowUserNotification(
				...
				StringFunctionsClientServer.SubstituteParametersInString(NStr("en='The counterparty is filled out in the document and discount card %1 is read';ru='В документе заполнен контрагент и считана дисконтная карта %1';vi='Trong chứng từ đã điền đối tác và đọc thẻ ưu đãi %1'"), Object.DiscountCard),
				PictureLib.Information32);
		Else
			Object.DiscountCard = ClosingResult.DiscountCard;
			Object.DiscountPercentByDiscountCard = ClosingResult.DiscountPercentByDiscountCard;
...
Procedure FormManagement()
	...
	// ParametricSpecifications
	If IsOrderForProcessing And Not ReadOnly Then
		For Each TabularSectionRow In Object.Inventory Do
			If TabularSectionRow.IsSpecificationTemplate Then
				TabularSectionRow.Specification = Undefined;
				TabularSectionRow.IsSpecificationTemplate = False;
				TabularSectionRow.HasSpecificationAdditionalAttributes = False;
			EndIf; 
		EndDo;
		Object.ParametricSpecificationsAttributes.Clear();
	EndIf; 
	// End ParametricSpecifications
...
Procedure SetFormConditionalAppearance()
	...
	WorkWithForm.AddDataCompositionFilterItem(NewConditionalAppearance.Filter, "Object.Inventory.ProductsAndServicesTypeInventory", False);
	WorkWithForm.AddDataCompositionFilterItem(NewConditionalAppearance.Filter, "Object.Inventory.IsSplitter", False);
	WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Items.InventoryReserve.Name);
	WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "TextColor", StyleColors.MinorInscriptionText);
	WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "Text", NStr("en='<Для товаров>';ru='<Для товаров>';vi='<Đối với hàng hóa>'"));
	WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "Enabled", False);
	
	// 3.
	NewConditionalAppearance = ConditionalAppearance.Items.Add();
	WorkWithForm.AddDataCompositionFilterItem(NewConditionalAppearance.Filter, "Object.Inventory.IsSplitter", True);
	WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Items.InventoryQuantity.Name);
	WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Items.InventoryVATRate.Name);
	WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Items.InventoryPrice.Name);
	WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Items.InventoryAmount.Name);
	WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Items.InventoryMeasurementUnit.Name);
	WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Items.InventoryShipmentDate.Name);
	WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "MarkIncomplete", False);
	
	// 4.
	NewConditionalAppearance = ConditionalAppearance.Items.Add();
	WorkWithForm.AddDataCompositionFilterItem(NewConditionalAppearance.Filter, "Object.Inventory.IsSplitter", True);
	WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Items.InventoryProductsAndServices.Name);
	WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Items.InventoryAmount.Name);
	WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Items.InventoryVATAmount.Name);
	WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "Font", New Font(Items.InventoryProductsAndServices.Font,,, True));
	
	// 5.
	NewConditionalAppearance = ConditionalAppearance.Items.Add();
	WorkWithForm.AddDataCompositionFilterItem(NewConditionalAppearance.Filter, "Object.Inventory.IsSplitter", True);
	For Each Item In Items.Inventory.ChildItems Do
		If TypeOf(Item) = Type("FormGroup") Then
			For Each SubordinateEl In Item.ChildItems Do
				WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, SubordinateEl.Name);
			EndDo;
		ElsIf TypeOf(Item) = Type("FormField") Then
			WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Item.Name);
		EndIf;
	EndDo;
	WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "BackColor", StyleColors.FilterOptionsPanelBackground);
	...
	For Each Item In Items.Inventory.ChildItems Do
		If TypeOf(Item) <> Type("FormField") Then
			Continue;
		EndIf;
		If Item.Name = Items.InventoryProductsAndServices.Name Then
			Continue;
		EndIf;
		WorkWithForm.AddConditionalAppearanceField(NewConditionalAppearance, Item.Name);
	EndDo;
	...
	If UseAutomaticDiscounts Then
		NewConditionalAppearance = ConditionalAppearance.Items.Add();
		WorkWithForm.AddDataCompositionFilterItem(NewConditionalAppearance.Filter, "Object.Inventory.AutomaticDiscountsPercent", 0, DataCompositionComparisonType.Equal);
		WorkWithForm.AddConditionalAppearanceFields(NewConditionalAppearance, "InventoryAutomaticDiscountPercent");
		WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "Text", "%");
		WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "TextColor", StyleColors.InaccessibleDataColor);
		
		NewConditionalAppearance = ConditionalAppearance.Items.Add();
		WorkWithForm.AddDataCompositionFilterItem(NewConditionalAppearance.Filter, "Object.Inventory.AutomaticDiscountAmount", 0, DataCompositionComparisonType.Equal);
		WorkWithForm.AddConditionalAppearanceFields(NewConditionalAppearance, "InventoryAutomaticDiscountAmount");
		WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "Text", "Amount");
		WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "TextColor", StyleColors.InaccessibleDataColor);
	EndIf;
	// Конец Скидки
	...
	// ParametricSpecifications
	NewConditionalAppearance = ConditionalAppearance.Items.Add();
	WorkWithForm.AddDataCompositionFilterItem(NewConditionalAppearance.Filter, "Object.Inventory.IsSpecificationTemplate", True);
	WorkWithForm.AddConditionalAppearanceFields(NewConditionalAppearance, "InventorySpecification");
	WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "TextColor", StyleColors.RedTextOfRows);
	
	NewConditionalAppearance = ConditionalAppearance.Items.Add();
	WorkWithForm.AddDataCompositionFilterItem(NewConditionalAppearance.Filter, "Object.Inventory.HasSpecificationAdditionalAttributes", False);
	WorkWithForm.AddConditionalAppearanceFields(NewConditionalAppearance, "InventorySpecificationParameters");
	WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "ReadOnly", True);
	WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "TextColor", StyleColors.InaccessibleDataColor);
	WorkWithForm.AddConditionalAppearanceElement(NewConditionalAppearance, "Text", NStr("en='<Не используются>';ru='<Не используются>';vi='<Không sử dụng>'"));
	// End ParametricSpecifications

EndProcedure

&AtServer
Procedure ActionsAfterReadObject()
	
	// ParametricSpecifications
	UpdateParametricSpecificationSigns(Object.Inventory);
	UpdateAdditionalAttributesDescription(ThisObject);
	// End ParametricSpecifications
	
EndProcedure

&AtServerNoContext
Function GetSpecificationDataOnChange(StructureData)
	
	// ParametricSpecifications
	If ValueIsFilled(StructureData.Specification)
		And TypeOf(StructureData.Specification)=Type("CatalogRef.Specifications") Then
		StructureData.Insert("IsSpecificationTemplate", CommonUse.ObjectAttributeValue(StructureData.Specification, "IsTemplate"));
		StructureData.Insert("HasSpecificationAdditionalAttributes", ProductionServer.HasSpecificationAdditionalAttributes(StructureData.Specification));
	Else
		StructureData.Insert("IsSpecificationTemplate", False);
		StructureData.Insert("HasSpecificationAdditionalAttributes", False);
	EndIf; 
	// End ParametricSpecifications
...
Procedure GetInventoryFromStorage(InventoryAddressInStorage, TabularSectionName, AreCharacteristics, AreBatches)
	...
	If Not (TypeOf(TableForImport) = Type("ValueTable")
		...
		EventLogMonitorErrorText = "Mismatch type переданного In document from selection [" + TypeOf(TableForImport) + "].
				|Address inventories In storage: " + TrimAll(InventoryAddressInStorage) + "
				|Name table parts: " + TrimAll(TabularSectionName);
	...
	For Each ImportRow In TableForImport Do
		...
		If NewRow.Property("Check") Then
			NewRow.Check = True;
		EndIf;
		If TabularSectionName="Inventory" Then
			ObjectFillingSB.FillRowByHeader(NewRow, Object, "ShipmentDate", "ShipmentDatePosition");
		EndIf; 
		
		StructureProductsAndServicesData = New Structure();
		StructureProductsAndServicesData.Insert("ProductsAndServices", ImportRow.ProductsAndServices);
		StructureProductsAndServicesData.Insert("Characteristic", ImportRow.Characteristic);
		StructureProductsAndServicesData.Insert("ProcessingDate", Object.Date);
		StructureProductsAndServicesData.Insert("Company", Object.Company);
		
		If Object.OperationKind = PredefinedValue("Enum.OperationKindsCustomerOrder.OrderForSale")
			Then
			BatchStatus = New ValueList;
			BatchStatus.Add(PredefinedValue("Enum.BatchStatuses.OwnInventory"));
			BatchStatus.Add(PredefinedValue("Enum.BatchStatuses.ProductsOnCommission"));
			
			StructureProductsAndServicesData.Insert("BatchStatus", BatchStatus);
		EndIf;
		...
		If TabularSectionName = "Works" Then
			...
			NewRow.ConnectionKey = SmallBusinessServer.CreateNewLinkKey(ThisObject);
			...
			If ValueIsFilled(ImportRow.ProductsAndServices) Then
				...
				StructureData = GetDataProductsAndServicesOnChange(StructureProductsAndServicesData);
				
			EndIf;
			
		ElsIf TabularSectionName = "Inventory" Then
			
			StructureProductsAndServicesData.Insert("Batch", ImportRow.Batch);
			
			If ValueIsFilled(ImportRow.ProductsAndServices) Then
				
				AttributeValues = CommonUse.ObjectAttributesValues(ImportRow.ProductsAndServices, "ProductsAndServicesType");
				
				StructureProductsAndServicesData.Insert("Company", Object.Company);
				StructureProductsAndServicesData.Insert("ProductsAndServicesType", AttributeValues.ProductsAndServicesType);
				StructureProductsAndServicesData.Insert("MeasurementUnit", ImportRow.MeasurementUnit);
				StructureProductsAndServicesData.Insert("VATTaxation", Object.VATTaxation);
				StructureProductsAndServicesData.Insert("ProcessingDate", Object.Date);
				
				NewRow.ProductsAndServicesTypeInventory = ((AttributeValues.ProductsAndServicesType = PredefinedValue("Enum.ProductsAndServicesTypes.InventoryItem"))
												Or (AttributeValues.ProductsAndServicesType = PredefinedValue("Enum.ProductsAndServicesTypes.GiftCertificate")));
												
				StructureData = GetDataProductsAndServicesOnChange(StructureProductsAndServicesData);
					
				//Batches
//				NewRow.UseBatches = StructureData.UseBatches;
//				NewRow.ПроверятьЗаполнениеПартий = StructureData.ПроверятьЗаполнениеПартий;
				
				NewRow.Batch = ?(ValueIsFilled(ImportRow.Batch), ImportRow.Batch, StructureData.Batch);
				// End Batches
				
			EndIf;
			
			If Object.OperationKind = Enums.OperationKindsCustomerOrder.OrderForProcessing Then
		...
		// Characteristic
		If ValueIsFilled(ImportRow.ProductsAndServices)	Then
			NewRow.Characteristic = StructureData.Characteristic;
		EndIf;
		// End Characteristic
		...
		If NewRow.Property("Specification") Then 
			...
			NewRow.IsSpecificationTemplate = StructureData.IsSpecificationTemplate;
			NewRow.HasSpecificationAdditionalAttributes = StructureData.HasSpecificationAdditionalAttributes;
			If NewRow.HasSpecificationAdditionalAttributes Then
				If NewRow.ConnectionKey<>0 Then
					SmallBusinessClientServer.DeleteRowsByConnectionKey(Object.ParametricSpecificationsAttributes, NewRow);
				EndIf; 
...
Procedure LoadFromFileInventory(Command)
	...
	DataLoadSettings.Insert("TabularSectionFullName",	"CustomerOrder.Inventory");
...
#Region ParametricSpecifications

&AtServerNoContext
Procedure UpdateParametricSpecificationSigns(Inventory)

	If Not GetFunctionalOption("UseParametricSpecifications")
		Or Not AccessRight("Read", Metadata.Catalogs.Specifications) Then
		Return;
	EndIf; 
	
	SpecificationsArray = New Array;
	For Each Str In Inventory Do
		If ValueIsFilled(Str.Specification) And SpecificationsArray.Find(Str.Specification)=Undefined Then
			SpecificationsArray.Add(Str.Specification);
		EndIf;
	EndDo;
	
	If SpecificationsArray.Count()>0 Then
		Query = New Query;
		Query.SetParameter("SpecificationsArray", SpecificationsArray);
		Query.Text =
		"SELECT
		...
		|WHERE
		|	Specifications.Ref IN(&SpecificationsArray)
		|
		|GROUP BY
		|	CASE
		|		WHEN AdditionalAttributesAndInformation.Ref IS NULL
		...
		SpecificationParameters = New Map;
		While Selection.Next() Do
			ParametersStructure = New Structure("IsTemplate, AreAdditAttributes");
			FillPropertyValues(ParametersStructure, Selection);
			SpecificationParameters.Insert(Selection.Ref, ParametersStructure);	
		EndDo; 
		
		For Each Str In Inventory Do
			ParametersStructure = SpecificationParameters.Get(Str.Specification);
			If Not ValueIsFilled(Str.Specification) Or ParametersStructure=Undefined Then
				Str.IsSpecificationTemplate = False;
				Str.HasSpecificationAdditionalAttributes = False;
			Else
				Str.IsSpecificationTemplate = ParametersStructure.IsTemplate;
				Str.HasSpecificationAdditionalAttributes = ParametersStructure.AreAdditAttributes;
			EndIf;
		EndDo; 
	EndIf; 
...
&AtClientAtServerNoContext
Procedure UpdateSpecificationParametersColumnVisibility(Form)
	
	Inventory = Form.Object.Inventory;
	Items = Form.Items;
	
	If Not Form.UseParametricSpecifications
		Or Not Form.UseAdditionalAttributesAndInformation
		Or Form.Object.OperationKind<>PredefinedValue("Enum.OperationKindsCustomerOrder.OrderForSale")  Then
		CommonUseClientServer.SetFormItemProperty(Items, "InventorySpecificationParameters", "Visible", False); 
		Return;
	EndIf; 
	
	ColumnVisibility = False;
	For Each TabularSectionRow In Inventory Do
		If TabularSectionRow.IsSpecificationTemplate And TabularSectionRow.HasSpecificationAdditionalAttributes Then
			ColumnVisibility = True;
		EndIf; 
	EndDo;
	
	CommonUseClientServer.SetFormItemProperty(Items, "InventorySpecificationParameters", "Visible", ColumnVisibility); 
	
EndProcedure

&AtClientAtServerNoContext
Procedure UpdateAdditionalAttributesDescription(Form, ConnectionKey = Undefined)
	
	Inventory = Form.Object.Inventory;
	Attributes = Form.Object.ParametricSpecificationsAttributes;
	Items = Form.Items;
	...
	If Not Form.UseParametricSpecifications
		...
		Or Form.Object.OperationKind<>PredefinedValue("Enum.OperationKindsCustomerOrder.OrderForSale")  Then
		For Each TabularSectionRow In Inventory Do
			TabularSectionRow.SpecificationParameters = "";
		EndDo; 
		Return;
	EndIf; 
	
	For Each TabularSectionRow In Inventory Do
		If Not ValueIsFilled(TabularSectionRow.Specification) 
			Or Not TabularSectionRow.HasSpecificationAdditionalAttributes Then
			Continue;
		EndIf;
		If ConnectionKey<>Undefined And TabularSectionRow.ConnectionKey<>ConnectionKey Then
			Continue;
		EndIf; 
		FilterStructure = New Structure;
		FilterStructure.Insert("ConnectionKey", TabularSectionRow.ConnectionKey);
		AttributeRows = Attributes.FindRows(FilterStructure);
		LongDescItems = New Array;
		For Each AttributeString In AttributeRows Do
			If Not ValueIsFilled(AttributeString.Value) Then
				Continue;
			EndIf;
			PropertyName = String(AttributeString.Property);
			Position = StrFind(PropertyName, "(");
			If Position>0 Then
				PropertyName = Left(PropertyName, Position - 2);
			EndIf; 
			If TypeOf(AttributeString.Value)=Type("Number") Then
				ValueDescription = Format(AttributeString.Value, "NDS=.; NZ=0; NG=0");
			Else
				ValueDescription = String(AttributeString.Value);
			EndIf; 
			LongDescItems.Add(PropertyName + ": " + ValueDescription);
		EndDo;
		TabularSectionRow.SpecificationParameters = StrConcat(LongDescItems, "; ");
	EndDo; 
 
EndProcedure

&AtServer
Procedure FilAdditionalAttributesByDefault(ID)
	
	TabularSectionRow = Object.Inventory.FindByID(ID);
	If TabularSectionRow=Undefined 
		Or Not ValueIsFilled(TabularSectionRow.Specification)
		Or Not TabularSectionRow.HasSpecificationAdditionalAttributes Then
		Return;
	EndIf;
	...
	If TabularSectionRow.ConnectionKey=0 Then
		TabularSectionName = "Inventory";
		TabularSectionRow.ConnectionKey = SmallBusinessServer.CreateNewLinkKey(ThisObject);
	EndIf;
	
	Query = New Query;
	Query.SetParameter("Specification", TabularSectionRow.Specification);
	Query.Text = 
	"SELECT
	|	SpecificationsAdditionalAttributes.Property AS Property,
	...
	|WHERE
	|	SpecificationsAdditionalAttributes.Ref = &Specification";
	Selection = Query.Execute().Select();
	While Selection.Next() Do
