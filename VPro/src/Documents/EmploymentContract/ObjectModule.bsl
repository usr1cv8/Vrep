...
Procedure RunPreliminaryControl(Cancel)
	...
	For Each AccrualDetentionRow IN TableAccrualsDeductions Do
		...
		Else
			...
			MessageText = NStr("en='Invalid link condition in row No. %Number% of the ""Accruals and deductions"" tabular section.';ru='Не верное условие связи в строке №%Номер% табл. части ""Начислений и удержаний"".';vi='Sai điều kiện liên kết trong dòng số %Number% phần bảng ""Trích hoặc giữ lại từ lương"".'");
	...
	If Not ResultsArray[2].IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No. %Number% of the ""Employees"" tabular section, the order validity contradicts the ""%PersonnelOrder%"" personnel order.';ru='В строке №%Номер% табл. части ""Сотрудники"" период действия приказа противоречит кадровому приказу ""%КадровыйПриказ%"".';vi='Tại dòng số %Number%của phần bảng ""Người lao động"" kỳ thực hiện quyết định mâu thuẫn với quyết định bổ nhiệm cán bộ ""%PersonnelOrder%"".'");
	...
	If Not ResultsArray[3].IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No. %Number% of the ""Accruals and deductions"" tabular section, the order validity contradicts the ""%PersonnelOrder%"" personnel order.';ru='В строке №%Номер% табл. части ""Начисления и удержания"" период действия приказа противоречит кадровому приказу ""%КадровыйПриказ%"".';vi='Tại dòng số %Number% của phần bảng ""Tính lương và giữ lại từ lương"" kỳ thực hiện quyết định mâu thuẫn với quyết định bổ nhiệm cán bộ ""%PersonnelOrder%"".'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
			MessageText = StrReplace(MessageText, "%RegularOrder%", QueryResultSelection.Recorder);
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
	...
	If Not ResultsArray[4].IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No. %Number% of the ""Employees"" tabular section, the employee is specified again.';ru='В строке №%Номер% табл. части ""Сотрудники"" сотрудник указывается повторно.';vi='Tại dòng số %Number% của phần bảng ""Người lao động"" có chỉ ra lặp lại người lao động.'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber);
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
				MessageText,
...
Procedure RunControl(AdditionalProperties, Cancel)
	...
	If Not ResultsArray[0].IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No. %Number% of the ""Employees"" tabular section, the %Employee% employee already works in the %StructuralUnit% department.';ru='В строке №%Номер% табл. части ""Сотрудники"" сотрудник %Сотрудник% уже работает в подразделении %СтруктурнаяЕдиница%.';vi='Tại dòng số %Number% của phần bảng ""Người lao động"", người lao động %Employee% đã làm việc ở bộ phận %StructuralUnit%.'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
	...
	If Not ResultsArray[1].IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No. %Number% of the ""Employees"" tabular section for the %Individual% individual, the %Employee% employee is already hired to a primary place of employment in the %StructuralUnit% department.';ru='В строке №%Номер% табл. части ""Сотрудники"" для физлица %Физлицо% уже принят на основное место работы сотрудник %Сотрудник% в подразделение %СтруктурнаяЕдиница%.';vi='Tại dòng số %Number% của phần bảng ""Người lao động"" đối với cá nhân %Individual% đã tiếp nhận vào vị trí làm việc chính của người lao động %Employee% trong bộ phận %StructuralUnit%.'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
	...
	If Not ResultsArray[2].IsEmpty() Then
		QueryResultSelection = ResultsArray[2].Select();
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No. %NumberDouble% of the ""Employees"" tabular section for the %Individual% individual, the %Employee% employee is rehired to a primary place of employment. Individual is already specified in row No. %Number%.';ru='В строке №%НомерДубль% табл. части ""Сотрудники"" для физлица %Физлицо% повторно принимается на основное место работы сотрудник %Сотрудник%. Физлицо уже указано в строке №%Номер%.';vi='Tại dòng số %NumberDouble% của phần bảng ""Người lao động"" đối với cá nhân %Individual%, người lao động %Employee% được tiếp nhận lại vào chỗ làm việc chính. Cá nhân đã được chỉ ra tại dòng số %Number%.'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
	...
	If Not ResultsArray[3].IsEmpty() Then
		QueryResultSelection = ResultsArray[3].Select();
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='In row No. %Number% of the ""Employee"" tabular section: the employee %Employee% worked in the company earlier. To hire the employee again, create a new employee.';ru='В строке №%Номер% табл. части ""Сотрудники"": сотрудник %Сотрудник% уже работал в компании. Для повторного приема на работу необходимо создать нового сотрудника.';vi='Tại dòng số %Number% của phần bảng ""Người lao động"": người lao động %Employee% đã làm việc người lao động mới.'");
			MessageText = StrReplace(MessageText, "%Number%", QueryResultSelection.LineNumber); 
			MessageText = StrReplace(MessageText, "%Employee%", QueryResultSelection.Employee);
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
...
Procedure RunControlStaffSchedule(AdditionalProperties, Cancel) 
	...
	While Selection.Next() Do
		If Selection.FontsContradiction Then
			MessageText = NStr("en='Row No. %Number% of the ""Employees"" tabular section: employment rates are not provided in the staff list.';ru='Строка №%Номер% табл. части ""Сотрудники"": в штатном расписании не предусмотрены ставки для приема сотрудника!';vi='Dòng №%Number% của phần bảng ""Người lao động"": trong biên chế chưa xem xét mức lương để tiếp nhận người lao động!'");
