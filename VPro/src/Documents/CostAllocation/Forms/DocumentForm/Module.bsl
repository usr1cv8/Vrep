...
Procedure InventoryFillByStandards(Command)
	...
	If Object.Inventory.Count() <> 0 Then
		...
		QuestionText = NStr("en='The ""Inventory"" tabular section will be filled in again.';ru='Табличная часть ""Запасы"" будет перезаполнена!';vi='Phần bảng ""Vật tư"" sẽ được điền lại!'") + Chars.LF;
		If Object.InventoryDistribution.Count() <> 0 Then
			QuestionText = QuestionText + NStr("en='The ""Inventory allocation"" tabular section will be cleared.';ru='Табличная часть ""Распределение запасов"" будет очищена!';vi='Phần bảng ""Phân bổ vật tư"" sẽ bị xóa!'") + Chars.LF;
		EndIf;	
		QuestionText = QuestionText + NStr("en='Continue?';ru='Продолжить?';vi='Tiếp tục?'");
		Response = Undefined;
...
Procedure InventoryDistributeByStandards(Command)
	...
	If Object.InventoryDistribution.Count() <> 0 Then
		...
		ShowQueryBox(New NotifyDescription("InventoryDistributeByStandardsEnd", ThisObject), NStr("en='The ""Inventory allocation"" tabular section will be filled in again. Continue?';ru='Табличная часть ""Распределение запасов"" будет перезаполнена! Продолжить?';vi='Phần bảng ""Phân bổ vật tư"" sẽ điền lại! Tiếp tục?'"), 
...
Procedure InventoryDistributeByQuantity(Command)
	...
	If Object.InventoryDistribution.Count() <> 0 Then
		...
		ShowQueryBox(New NotifyDescription("InventoryDistributeByQuantityEnd", ThisObject), NStr("en='The ""Inventory allocation"" tabular section will be filled in again. Continue?';ru='Табличная часть ""Распределение запасов"" будет перезаполнена! Продолжить?';vi='Phần bảng ""Phân bổ vật tư"" sẽ điền lại! Tiếp tục?'"), 
							QuestionDialogMode.YesNo, 0);
        Return;
...
Procedure CostsFillByBalance(Command)
	...
	If Object.Costs.Count() <> 0 Then
		...
		QuestionText = NStr("en='The ""Expenses"" tabular section will be filled in again.';ru='Табличная часть ""Затраты"" будет перезаполнена!';vi='Phần bảng ""Chi phí"" sẽ được điền lại!'") + Chars.LF;
		QuestionText = QuestionText + NStr("en='The ""Expense allocation"" tabular section will be filled in again.';ru='Табличная часть ""Распределение расходов"" будет очищена!';vi='Phần bảng ""Phân bổ chi phí"" sẽ bị xóa!'") + Chars.LF;
		QuestionText = QuestionText + NStr("en='Continue?';ru='Продолжить?';vi='Tiếp tục?'");
		Response = Undefined;
...
Procedure CostsDistributeByQuantity(Command)
	...
	If Object.CostAllocation.Count() <> 0 Then
		...
		ShowQueryBox(New NotifyDescription("AllocateCostsByQuantityEnd", ThisObject), NStr("en='The ""Expense allocation"" tabular section will be filled in again. Continue?';ru='Табличная часть ""Распределение расходов"" будет перезаполнена! Продолжить?';vi='Phần bảng ""Phân bổ chi phí"" sẽ điền lại! Tiếp tục?'"), 
							QuestionDialogMode.YesNo, 0);
        Return;
...
Procedure ProductsFillByOutput(Command)
	...
	If Object.Products.Count() <> 0 Then
		...
		ShowQueryBox(New NotifyDescription("ProductsFillByOutputEnd", ThisObject), NStr("en='The ""Products"" tabular section will be filled in again. Continue?';ru='Табличная часть ""Продукция"" будет перезаполнена! Продолжить?';vi='Phần bảng ""Thành phẩm"" sẽ điền lại! Tiếp tục?'"), 
							QuestionDialogMode.YesNo, 0);
        Return;
