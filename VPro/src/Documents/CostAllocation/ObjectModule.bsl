...
Procedure RunInventoryFillingByBalance() Export
	...
	Query.Text =
    "SELECT
    |	SUM(WorkInProgressInventory.Quantity) AS Quantity,
    |	SUM(CASE
    |			WHEN WorkInProgressInventory.CustomerOrder <> VALUE(Document.CustomerOrder.EmptyRef)
    |				THEN WorkInProgressInventory.Quantity
    |			ELSE 0
    |		END) AS Reserve,
    |	ISNULL(WorkInProgressInventory.ProductsAndServices, VALUE(Catalog.ProductsAndServices.EmptyRef)) AS ProductsAndServices,
    |	WorkInProgressInventory.Characteristic AS Characteristic,
    |	WorkInProgressInventory.Batch AS Batch,
    |	WorkInProgressInventory.CustomerOrder AS CustomerOrder
    |INTO WorkInProgressInventory
    |FROM
    |	Document.WorkInProgressCosts.Inventory AS WorkInProgressInventory
    |WHERE
    |	WorkInProgressInventory.Quantity > 0
    |	AND WorkInProgressInventory.ProductsAndServices <> VALUE(Catalog.ProductsAndServices.EmptyRef)
    |	AND WorkInProgressInventory.Ref.Company = &Company
    |	AND WorkInProgressInventory.Ref.StructuralUnit = &StructuralUnit
    |	AND WorkInProgressInventory.CustomerOrder IN(&CustomerOrder)
    |	AND WorkInProgressInventory.Ref.Posted
    |	AND WorkInProgressInventory.Ref.Date BETWEEN &PeriodOpenDate AND &ProcessingDate
    |
    |GROUP BY
    |	WorkInProgressInventory.Batch,
    |	WorkInProgressInventory.ProductsAndServices,
    |	WorkInProgressInventory.Characteristic,
    |	WorkInProgressInventory.CustomerOrder
    |;
    |
    |////////////////////////////////////////////////////////////////////////////////
    |SELECT
    |	SUM(InventoryBalances.QuantityBalance - ISNULL(WorkInProgressInventory.Quantity, 0)) AS Quantity,
    |	SUM(CASE
    |			WHEN InventoryBalances.CustomerOrder <> VALUE(Document.CustomerOrder.EmptyRef)
    |				THEN InventoryBalances.QuantityBalance - ISNULL(WorkInProgressInventory.Reserve, 0)
    |			ELSE 0
    |		END) AS Reserve,
    ...
    |		LEFT JOIN WorkInProgressInventory AS WorkInProgressInventory
    |		ON InventoryBalances.ProductsAndServices = WorkInProgressInventory.ProductsAndServices
    |			AND InventoryBalances.Characteristic = WorkInProgressInventory.Characteristic
    |			AND InventoryBalances.Batch = WorkInProgressInventory.Batch
    |			AND InventoryBalances.CustomerOrder = WorkInProgressInventory.CustomerOrder
    |WHERE
	...
	Query.SetParameter("ProcessingDate", EndOfDay(Date));
	Query.SetParameter("PeriodOpenDate", BegOfDay(PeriodOpenDate));
...
Procedure RunInventoryDistributionByStandards() Export
	...
	TableInventoryByBom = TemporaryTableDistribution.Скопировать(,"ProductsAndServices, Characteristic, MeasurementUnit, CustomerOrder, Specification, Quantity");
	TableInventoryByBom.Свернуть("ProductsAndServices, Characteristic, MeasurementUnit,CustomerOrder, Specification", "Quantity");
	TemporaryTableDistribution.Columns.Add("TotalQuantity",New TypeDescription("Number",
																		New NumberQualifiers(15, 3, AllowedSign.Nonnegative)));
	
	//Get the percentage of each line of shares in the total number of standards
	For Each TabularSectionRow In TableInventoryByBom Do
		FilterFields = New Structure;
		FilterFields.Insert("ProductsAndServices", 		TabularSectionRow.ProductsAndServices);
		FilterFields.Insert("Characteristic", 		TabularSectionRow.Characteristic);
		FilterFields.Insert("MeasurementUnit", 	TabularSectionRow.MeasurementUnit);
		FilterFields.Insert("CustomerOrder", 	TabularSectionRow.CustomerOrder);
		FilterFields.Insert("Specification", 	TabularSectionRow.Specification);
		
		
		RowArray = TemporaryTableDistribution.FindRows(FilterFields);
		For Each ArrayRow In RowArray Do
			ArrayRow.TotalQuantity		= TabularSectionRow.Quantity;
		EndDo;
	EndDo;
	...
	For Each TabularSectionRow IN Inventory Do
		...
		QuantityDistribution = TabularSectionRow.Quantity;
		...
		For Each ArrayRow IN SearchingArray Do
			...
			If QuantityDistribution > 0 Then
				
				ArrayRowQuantity = Мин(Окр(TabularSectionRow.Quantity*ArrayRow.Quantity/ArrayRow.TotalQuantity,3), QuantityDistribution);
				QuantityDistribution = QuantityDistribution - ArrayRowQuantity;
				
				StringReserve = min(ArrayRowQuantity, ReserveAllocation);
				ReserveAllocation = ReserveAllocation - StringReserve;
				
				NewRow = InventoryDistribution.Add();
				NewRow.Quantity 		= ArrayRowQuantity;
		...
		If SearchingArray.Count()=0 Then
			
			ProductionTotal = TemporaryTableProduction.Total("Quantity");
			
			For Each ArrayRow In TemporaryTableProduction Do
				
				If QuantityDistribution > 0 Then
					
					ArrayRowQuantity = Мин(Окр(TabularSectionRow.Quantity*ArrayRow.Quantity/ProductionTotal,3), QuantityDistribution);
					QuantityDistribution = QuantityDistribution - ArrayRowQuantity;
					
					ReserveRow = МИН(ArrayRowQuantity, ReserveAllocation);
					ReserveAllocation = ReserveAllocation - ReserveRow;
					
					NewRow = InventoryDistribution.Добавить();
					NewRow.Quantity 		= ArrayRowQuantity;
					NewRow.Reserve 			= ReserveRow;
					NewRow.ProductsAndServices	= ArrayRow.ProductsAndServices;
					NewRow.Characteristic 	= ArrayRow.Characteristic;
					NewRow.Batch 			= ArrayRow.Batch;
					NewRow.CustomerOrder	= ArrayRow.CustomerOrder;
					NewRow.Specification	= ArrayRow.Specification;
					NewRow.ConnectionKey		= TabularSectionRow.ConnectionKey;
		...
		If QuantityDistribution<>0 И NewRow <> Undefined Then
			NewRow.Quantity = NewRow.Quantity + QuantityDistribution;
		EndIf; 
...
Procedure RunExpenseFillingByBalance() Export
	...
	Query.Text =
    "SELECT
    |	SUM(WorkInProgressCosts.Amount) AS Amount,
    |	WorkInProgressCosts.GLExpenseAccount AS GLExpenseAccount,
    |	WorkInProgressCosts.CustomerOrder AS CustomerOrder
    |INTO WorkInProgressCosts
    |FROM
    |	Document.WorkInProgressCosts.Costs AS WorkInProgressCosts
    |WHERE
    |	WorkInProgressCosts.Amount > 0
    |	AND WorkInProgressCosts.Ref.Company = &Company
    |	AND WorkInProgressCosts.Ref.StructuralUnit = &StructuralUnit
    |	AND WorkInProgressCosts.GLExpenseAccount.TypeOfAccount = VALUE(Enum.GLAccountsTypes.UnfinishedProduction)
    |	AND WorkInProgressCosts.CustomerOrder IN(&CustomerOrder)
    |	AND WorkInProgressCosts.Ref.Posted
    |	AND WorkInProgressCosts.Ref.Date BETWEEN &PeriodOpenDate AND &ProcessingDate
    |
    |GROUP BY
    |	WorkInProgressCosts.GLExpenseAccount,
    |	WorkInProgressCosts.CustomerOrder
    |;
    |
    |////////////////////////////////////////////////////////////////////////////////
    |SELECT
    |	SUM(InventoryBalances.AmountBalance - ISNULL(WorkInProgressCosts.Amount, 0)) AS Amount,
    ...
    |		LEFT JOIN WorkInProgressCosts AS WorkInProgressCosts
    |		ON InventoryBalances.GLAccount = WorkInProgressCosts.GLExpenseAccount
    |			AND InventoryBalances.CustomerOrder = WorkInProgressCosts.CustomerOrder
    |WHERE
...
Procedure Filling(FillingData, StandardProcessing) Export
	...
	If TypeOf(FillingData) = Type("DocumentRef.InventoryAssembly") Then
		...
		If QueryResultSelection.Next() Then
			...
			While SelectionProducts.Next() Do
				...
				NewRow.CustomerOrder		= SelectionProducts.CustomerOrder;
			EndDo;
	...
	ElsIf TypeOf(FillingData) = Type("DocumentRef.CustomerOrder")
		...
		Raise NStr("en='Cannot enter Cost allocation based on the work order.';ru='Нельзя ввести Распределение затрат на основании заказ-наряда!';vi='Không thể nhập Chi phí được phân bổ trên cơ sở đơn hàng trọn gói!'");
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Inventory.Total("Quantity") <> InventoryDistribution.Total("Quantity") Then
		...
		MessageText = NStr("en='Quantity of inventory does not match those of the allocation.';ru='Количество запасов не соответствует количеству распределения!';vi='Số lượng vật tư không tương ứng với số lượng được phân bổ!'");
	...
	If Costs.Total("Amount") <> CostAllocation.Total("Amount") Then
		...
		MessageText = NStr("en='Amount of expenses does not correspond to the allocation amount.';ru='Сумма расходов не соответствует сумме распределения!';vi='Số tiền chi phí không tương ứng với số tiền phân bổ!'");
	...
	If Constants.FunctionalOptionInventoryReservation.Get() Then
		...
		For Each StringInventoryDistribution IN InventoryDistribution Do
			...
			If StringInventoryDistribution.Reserve > StringInventoryDistribution.Quantity Then
				...
				MessageText = NStr("en='In row No. %Number% of the ""Inventory allocation"" tabular section, the number of items for write-off from reserve exceeds the total inventory quantity.';ru='В строке №%Номер% табл. части ""Распределение запасов"" количество позиций к списанию из резерва превышает общее количество запасов.';vi='Tại dòng số %Number% của phần bảng ""Phân bổ vật tư"" số lượng mặt hàng chờ ghi giảm từ dự phòng vượt quá tổng số lượng vật tư.'");
