...
Procedure GenerateAccountingSectionsList()
	...
	If Constants.FunctionalOptionUseSubsystemPayroll.Get()
		OR Object.AccountingSection = Enums.AccountingSections.PersonnelSettlements Then
		Items.AccountingSection.ChoiceList.Add(Enums.AccountingSections.PersonnelSettlements);
	EndIf;
	
	// FD Use Belongings.
	If Constants.FunctionalOptionAccountingFixedAssets.Get()
		OR Object.AccountingSection = Enums.AccountingSections.Assets Then
		Items.AccountingSection.ChoiceList.Add(Enums.AccountingSections.Assets);
	EndIf;
	
	// Other.
	Items.AccountingSection.ChoiceList.Add(Enums.AccountingSections.OtherSections);
...
Function GetPageName(AccountingSection)
	...
	Map = New Map;
	Map.Insert(PredefinedValue("Enum.AccountingSections.Assets"), "FolderFixedAssets");
	Map.Insert(PredefinedValue("Enum.AccountingSections.Inventory"), "GroupInventory");
	Map.Insert(PredefinedValue("Enum.AccountingSections.CashAssets"), "FolderBanking");
	Map.Insert(PredefinedValue("Enum.AccountingSections.SettlementsWithSuppliersAndCustomers"), "GroupSettlementsWithCounterparties");
	Map.Insert(PredefinedValue("Enum.AccountingSections.TaxSettlements"), "FolderTaxesSettlements");
	Map.Insert(PredefinedValue("Enum.AccountingSections.PersonnelSettlements"), "GroupSettlementsWithHPersonnel");
	Map.Insert(PredefinedValue("Enum.AccountingSections.SettlementsWithAdvanceHolders"), "GroupAdvanceHolderPayments");
	Map.Insert(PredefinedValue("Enum.AccountingSections.OtherSections"), "GroupOtherSections");
...
Procedure SetItemsVisibleEnabled()
	...
	If Object.AccountingSection = Enums.AccountingSections.SettlementsWithSuppliersAndCustomers
		OR Object.AccountingSection = Enums.AccountingSections.SettlementsWithAdvanceHolders Then
...
Procedure FixedAssetsVolumeProductsWorksForDepreciationCalculationOnChange(Item)
	...
	If Not StructureData.MethodOfDepreciationProportionallyProductsAmount Then
		ShowMessageBox(Undefined,NStr("en='""Product (work) volume for calculating depreciation"" cannot be filled in for the specified depreciation accrual method.';ru='""Объем продукции (работ) для исчисления амортизации"" не может быть заполнен для указанного способа начисления амортизации!';vi='Không thể điền ""Khối lượng sản phẩm (công việc) để tính khấu hao"" đối với phương pháp tính khấu hao đã chỉ ra!'"));
...
Procedure FixedAssetsUsagePeriodForDepreciationCalculationOnChange(Item)
	...
	If StructureData.MethodOfDepreciationProportionallyProductsAmount Then
		ShowMessageBox(Undefined,NStr("en='Cannot fill in ""Useful life for calculating depreciation"" for the specified method of depreciation accrual.';ru='""Срок использования для вычисления амортизации"" не может быть заполнен для указанного способа начисления амортизации!';vi='Không thể điền ""Thời hạn sử dụng để tính khấu hao"" đối với phương pháp tính khấu hao đã chỉ ra!'"));
...
Procedure OtherSectionsCurrencyStartChoice(Item, ChoiceData, StandardProcessing)
	...
	If Not StructureData.Currency Then
		...
		If ValueIsFilled(CurrentRow.Account) Then
			ShowMessageBox(Undefined,NStr("en='Currency flag is not set for the selected account.';ru='У выбранного счета не установлен признак валютный!';vi='Ở tài khoản đã chọn chưa thiết lập dấu hiệu tiền tệ!'"));
		Else
			ShowMessageBox(Undefined,NStr("en='Specify the account first.';ru='Укажите в начале счет!';vi='Trước tiên, hãy chỉ ra tài khoản!'"));
		EndIf;
	EndIf;
...
Procedure InventoryByCCDDStartRMNumberChoice(Item, ChoiceData, StandardProcessing)
	...
	If DataCurrentRows <> Undefined Then
		...
		If Not ValueIsFilled(DataCurrentRows.CountryOfOrigin) Then
			...
			MessageText = NStr("en='Country of origin is required.';ru='Не заполнена страна происхождения!';vi='Chưa điền nước xuất xứ!'");
		...
		If ValueIsFilled(DataCurrentRows.CountryOfOrigin)
		   AND DataCurrentRows.CountryOfOrigin = PredefinedValue("Catalog.WorldCountries.Russia") Then
			...
			MessageText = NStr("en='CCD accounting for the domestic goods is not kept.';ru='Учет ГТД для отечественных товаров не ведется!';vi='Không ghi nhận Tờ khai hải quan đối với hàng hóa trong nước!'");
			CommonUseClientServer.MessageToUser(MessageText);
		...
		If (ValueIsFilled(DataCurrentRows.ProductsAndServices)
			...
			MessageText = NStr("en='CCD accounting in the application is kept only for products and services with the ""Inventory"" type.';ru='Учет ГТД в программе ведеться только для номенклатуры с типом ""Запас"".';vi='Chỉ ghi nhận Tờ khai hải quan trong chương trình đối với mặt hàng có kiểu ""Vật tư"".'");
			CommonUseClientServer.MessageToUser(MessageText);
			StandardProcessing = False;
