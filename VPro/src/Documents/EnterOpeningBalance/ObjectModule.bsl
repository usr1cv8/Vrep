...
Procedure FillByInventoryReconciliation(FillingData)
	...
	AccountingSection = Enums.AccountingSections.Inventory;
  ...
  OR Not Autogeneration Then
	...
	For Each String IN AccountsReceivable Do
		If Not ValueIsFilled(String.Document)
			...
			StringComment = NStr("en='It is generated automatically by the ""Entering the opening balances"" document No.%Number% from %Date%';ru='Сформирован автоматически документом Ввод начальных остатков № %Номер% от %Дата%';vi='Được lập tự động bởi chứng từ Nhập số dư đầu số %Number% ngày %Date%'");
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	For Each TSRow IN OtherSections Do
		...
		AND Not ValueIsFilled(TSRow.Currency) Then
			MessageText = NStr("en='The ""Currency"" column is not filled in for the currency account in row No. %RowNumber% of the ""Other sections"" list.';ru='Не заполнена колонка ""Валюта"" для валютного счета в строке %НомерСтроки% списка ""Прочие разделы"".';vi='Chưa điền cột ""Tiền tệ"" cho tài khoản tiền tệ tại dòng %LineNumber% của danh sách ""Phần hành khác"".'");
	...
	For Each TSRow IN AccountsReceivable Do
		...
		AND Not ValueIsFilled(TSRow.Document) Then
			MessageText = NStr("en='The ""Settlement document"" column is not populated in the %LineNumber% line of the ""Settlements with customers"" list.';ru='Не заполнена колонка ""Документ расчетов"" в строке %НомерСтроки% списка ""Расчеты с покупателями"".';vi='Chưa điền cột ""Chứng từ thanh toán"" tại dòng %LineNumber% của danh sách ""Hạch toán với người mua"".'");
			MessageText = StrReplace(MessageText, "%LineNumber%", String(TSRow.LineNumber));
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
				MessageText,
