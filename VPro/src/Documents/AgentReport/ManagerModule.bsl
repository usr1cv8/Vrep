...
Procedure GenerateTableInventory(DocumentRefAgentReport, StructureAdditionalProperties)
	...
	Query.SetParameter("AgentReport", NStr("en='Agent report';ru='Отчет комиссионера';vi='Bảng kê hàng bán của người nhận bán hộ'"));
...
Procedure GenerateTableInventoryByCCD(DocumentRefAgentReport, StructureAdditionalProperties)
...
Procedure InitializeDocumentData(DocumentRefAgentReport, StructureAdditionalProperties) Export
	...
	|	AgentReportInventory.CCDNo AS CCDNo,
	|	AgentReportInventory.CountryOfOrigin AS CountryOfOrigin,
	|	CASE
	...
	GenerateTableInventoryByCCD(DocumentRefAgentReport, StructureAdditionalProperties);
...
//
Procedure RunControl(DocumentObjectAgentReport, AdditionalProperties, Cancel, PostingDelete = False) Export
	...
	//||	If StructureTemporaryTables.RegisterRecordsInventoryByCCDChange Then
		...
		If Not ResultsArray[3].IsEmpty() Then
			...
			SmallBusinessServer.ShowMessageAboutPostingToInventoryByCCDRegisterErrors(DocumentObjectAgentReport, QueryResultSelection, Cancel);
		EndIf;
