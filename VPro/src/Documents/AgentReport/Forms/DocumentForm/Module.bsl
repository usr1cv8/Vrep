...
//
&AtServer
Procedure CalculateAmountInTabularSectionLineAtServer(TabularSectionRow = Undefined)
...
//tabular section,
&AtServer
Procedure CalculateCommissionRemunerationAtServer(TabularSectionRow)
...
Function GenerateLabelPricesAndCurrency(LabelStructure)
	...
	If ValueIsFilled(LabelStructure.PriceKind) Then
		If IsBlankString(LabelText) Then
			LabelText = LabelText + NStr("en='%PriceKind%';ru='%PriceKind%';vi='%PriceKind%'");
		Else	
			LabelText = LabelText + NStr("en=' • %PriceKind%';ru=' • %PriceKind%';vi=' • %PriceKind%'");
		EndIf;	
...
Procedure GetInventoryTransferredFromStorage(AddressInventoryTransferredInStorage)
	...
	For Each TabularSectionRow IN InventoryTransferred Do
		...
		CalculateAmountInTabularSectionLineAtServer(NewRow);
		CalculateCommissionRemunerationAtServer(NewRow);
		//-- 1CVN } 
	EndDo;
