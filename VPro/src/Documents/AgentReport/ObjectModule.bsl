	...
	For Each String IN TablePrepayment Do
		...
		If FoundStringInventory = Undefined
		   ...
		   AND Counterparty.DoOperationsByOrders Then
			MessageText = NStr("en='Advance of order that is different form specified in the ""Inventory"" tabular section cannot be set off!';ru='Нельзя зачесть аванс по заказу отличному от указанных в табличной части ""Запасы""!';vi='Không thể tính khoản ứng trước theo đơn hàng khác với đơn hàng mà đã chỉ ra trong phần bảng ""Vật tư""!'");
...
Procedure Posting(Cancel, PostingMode)
	...
	Documents.AgentReport.RunControl(ThisObject, AdditionalProperties, Cancel);
...
Procedure UndoPosting(Cancel)
	...
	// Control of occurrence of a negative balance.
	Documents.AgentReport.RunControl(ThisObject, AdditionalProperties, Cancel, True);
