...
Procedure InitializeDocumentDataAssembly(DocumentRefProductionOrder, StructureAdditionalProperties) Export
	...
	Query.Text = 
	"SELECT
	|	OrderProducts.LineNumber AS LineNumber,
	|	OrderProducts.Ref AS Ref,
	|	OrderProducts.Ref.StructuralUnit AS StructuralUnit,
	|	OrderProducts.Ref.StructuralUnit.StructuralUnitType AS StructuralUnitType,
	|	OrderProducts.Ref.Date AS Date,
	|	BEGINOFPERIOD(OrderProducts.Ref.Finish, DAY) AS Finish,
	|	&Company AS Company,
	|	OrderProducts.CustomerOrder AS CustomerOrder,
	|	OrderProducts.ProductsAndServices AS ProductsAndServices,
	|	OrderProducts.ProductsAndServices.ProductsAndServicesType AS ProductsAndServicesType,
	|	CASE
	|		WHEN &UseCharacteristics
	|			THEN OrderProducts.Characteristic
	|		ELSE VALUE(Catalog.ProductsAndServicesCharacteristics.ПустаяСсылка)
	|	END AS Characteristic,
	|	CASE
	|		WHEN &UseBatches
	|			THEN OrderProducts.Batch
	|		ELSE VALUE(Catalog.ProductsAndServicesBatches.ПустаяСсылка)
	|	END AS Batch,
	|	OrderProducts.Specification AS Specification,
	|	CASE
	|		WHEN VALUETYPE(OrderProducts.MeasurementUnit) = TYPE(Catalog.UOMClassifier)
	|			THEN OrderProducts.Quantity
	|		ELSE OrderProducts.Quantity * OrderProducts.MeasurementUnit.Factor
	|	END AS Quantity,
	|	OrderProducts.CompletiveStageDepartment AS CompletiveStageDepartment
	|INTO TableProducts
	|FROM
	|	Document.ProductionOrder.Products AS OrderProducts
	|WHERE
	|	OrderProducts.Ref = &Ref
	|	AND OrderProducts.Ref.OrderState.OrderStatus = VALUE(Enum.OrderStatuses.InProcess)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT
	|	OrderInventory.Ref AS Ref,
	|	OrderInventory.LineNumber AS LineNumber,
	|	OrderInventory.Ref.Start AS Start,
	|	OrderInventory.Ref.Finish AS Finish,
	|	OrderInventory.Ref.Date AS Date,
	|	&Company AS Company,
	|	OrderInventory.StructuralUnit AS StructuralUnit,
	|	OrderInventory.ProductsAndServices.InventoryGLAccount AS InventoryGLAccount,
	|	OrderInventory.ProductsAndServices AS ProductsAndServices,
	|	CASE
	|		WHEN &UseCharacteristics
	|			THEN OrderInventory.Characteristic
	|		ELSE VALUE(Catalog.ProductsAndServicesCharacteristics.ПустаяСсылка)
	|	END AS Characteristic,
	|	CASE
	|		WHEN &UseBatches
	|			THEN OrderInventory.Batch
	|		ELSE VALUE(Catalog.ProductsAndServicesBatches.ПустаяСсылка)
	|	END AS Batch,
	|	OrderInventory.CustomerOrder AS CustomerOrder,
	|	CASE
	|		WHEN VALUETYPE(OrderInventory.MeasurementUnit) = TYPE(Catalog.UOMClassifier)
	|			THEN OrderInventory.Quantity
	|		ELSE OrderInventory.Quantity * OrderInventory.MeasurementUnit.Factor
	|	END AS Quantity,
	|	CASE
	|		WHEN VALUETYPE(OrderInventory.MeasurementUnit) = TYPE(Catalog.UOMClassifier)
	|			THEN OrderInventory.Reserve
	|		ELSE OrderInventory.Reserve * OrderInventory.MeasurementUnit.Factor
	|	END AS Reserve
	|INTO TempTableInventory
	|FROM
	|	Document.ProductionOrder.Inventory AS OrderInventory
	|WHERE
	|	OrderInventory.Ref = &Ref
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT
	...
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT
	|	Products.Date AS Period,
	|	Products.Company AS Company,
	|	CASE
	|		WHEN Products.CustomerOrder = VALUE(Документ.CustomerOrder.ПустаяСсылка)
	|			THEN Products.Ref
	|		ELSE Products.CustomerOrder
	|	END AS Order,
	...
	|	ISNULL(ProductionKindsStages.Stage, VALUE(Catalog.ProductionStages.ProductionComplete)) AS Stage,
	|	Products.Quantity AS QuantityPlan,
	|	0 AS QuantityFact,
	|	Products.CompletiveStageDepartment AS StructuralUnit
	|FROM
	|	TableProducts AS Products
	...
	|WHERE
	|	&UseProductionStages
	|	AND Products.Specification <> VALUE(Catalog.Specifications.ПустаяСсылка)
	|	AND CAST(Products.Specification AS Catalog.Specifications).ProductionKind <> VALUE(Catalog.ProductionKinds.ПустаяСсылка)
	|	AND Products.StructuralUnitType = VALUE(Enum.StructuralUnitsTypes.Department)
	|
	|UNION ALL
	|
	|SELECT
	...
	|	0,
	|	CASE
	|		WHEN Products.CompletiveStageDepartment = VALUE(Catalog.StructuralUnits.ПустаяСсылка)
	|			THEN Products.StructuralUnit
	|		ELSE Products.CompletiveStageDepartment
	|	END
	|FROM
	|	TableProducts AS Products
	|WHERE
	...
	Query.SetParameter("InventoryReservation", NStr("ru = 'Резервирование запасов';
													|vi = 'Lập dự phòng vật tư';
													|en = 'Inventory reservation'"));
	Query.SetParameter("UseProductionStages", StructureAdditionalProperties.AccountingPolicy.UseProductionStages);

	Result = Query.ExecuteBatch();

	StructureAdditionalProperties.TableForRegisterRecords.Insert("TableInventoryTransferSchedule", Result[2].Unload());
	...
	StructureAdditionalProperties.TableForRegisterRecords.Insert("TableOrdersPlacement", Result[5].Unload());
	StructureAdditionalProperties.TableForRegisterRecords.Insert("TableInventory", Result[7].Unload());
	StructureAdditionalProperties.TableForRegisterRecords.Insert("TableProductRelease", Result[8].Unload());
	
	StructureAdditionalProperties.TableForRegisterRecords.Insert("TableProductionStages", Result[9].Unload());
	
	//Resources
	If GetFunctionalOption("PlanCompanyResourcesLoading") Then
		ResourcesRecordsTable = ResourcePlanningCM.DocumentsRecordsTable(DocumentRefProductionOrder);
...
Procedure VerifyEnteringAbilityByProductionOrder(FillingData, AttributeValues) Export
	...
	If AttributeValues.Property("Closed") Then
		If AttributeValues.Closed Then
			ErrorText = NStr("ru = 'Документ %Документ% закрыт (выполнен). Ввод на основании закрытого заказа запрещен.';
							|vi = 'Chứng từ %Document% đã đóng (đã thực hiện). Cấm nhập trên cơ sở đơn hàng đã đóng.';
							|en = 'The %Document% document is closed (completed). Cannot enter on the basis of the closed order.'");
			ErrorText = StrReplace(ErrorText, "%Document%", FillingData);
			Raise ErrorText;
		EndIf;
	EndIf;
	...
	If AttributeValues.Property("OrderState") Then
		If AttributeValues.OrderState.OrderStatus = Enums.OrderStatuses.Open Then
			ErrorText = NStr("ru = 'Документ %Документ% в состоянии %СостояниеЗаказа%. Ввод на основании запрещен.';
							|vi = 'Chứng từ %Document% trong trạng thái %OrderState%. Cấm nhập trên cơ sở.';
							|en = 'The %Document% document in the %OrderState% state. Cannot enter on the basis.'");
			ErrorText = StrReplace(ErrorText, "%Document%", FillingData);
...
Function PrintForm(ObjectsArray, PrintObjects, TemplateName = "")
	...
	Query.SetParameter("Characteristic", NStr("ru = 'Характеристика:';
												|vi = 'Đặc tính:';
												|en = 'Characteristic:'"));
	Query.Text = 
	"SELECT
	...
	While Header.Next() Do
		...
		TemplateArea.Parameters.HeaderText = NStr("en='Request to warehouse No. ';vi='Đơn hàng sản xuất số ';")
												+ DocumentNumber
												+ NStr("en=' from ';vi=' ';")
...
Procedure Print(ObjectsArray, PrintParameters, PrintFormsCollection, PrintObjects, OutputParameters) Export
	...
	If PrintManagement.NeedToPrintTemplate(PrintFormsCollection, "RequestToWarehouse") Then
		...
		PrintManagement.OutputSpreadsheetDocumentToCollection(PrintFormsCollection, "RequestToWarehouse", NStr("en='Request to warehouse';vi='Đơn hàng sản xuất';"), PrintForm(ObjectsArray, PrintObjects));
...
Procedure AddPrintCommands(PrintCommands) Export
	...
	PrintCommand.Presentation = NStr("ru = 'Требование на склад';
									|vi = 'Phiếu yêu cầu vật tư';
									|en = 'Demand to the warehouse'");
