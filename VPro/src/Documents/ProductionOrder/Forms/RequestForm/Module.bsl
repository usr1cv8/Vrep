	...
	If ValueSelected = OperationKinds.Disassembly Then
		...
		For Each StringProducts IN Object.Products Do
			...
			If Not StringProducts.ProductsAndServicesTypeInventory Then
				...
				MessageText = NStr("en='Disassembling operation is invalid for works and services!""""The %ProductsAndServicesPresentation% products and services could be a work(service) in the %Number% string of the tabular section ""Products""';ru='Операция разборки не выполняется для работ и услуг!""""В строке №%Номер% табличной части ""Продукция"" номенклатура ""%НоменклатураПредставление%"" является работой (услугой)';vi='Không thực hiện giao dịch tách bộ cho công việc và dịch vụ!""""Tại dòng số %Number% của phần bảng ""Sản phẩm"", mặt hàng ""%ProductsAndServicesPresentation%"" là công việc (dịch vụ)'");
