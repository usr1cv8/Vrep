...
Procedure RecalculateAmounts()
	...
	If AmountShortChange > CashReceived Then
		...
		Message.Text = NStr("en='Deal amount exceeds payment in cash amount. It is required to reduce the payment amount or return on card ';vi='Số tiền giao dịch vượt quá thanh toán bằng tiền mặt. Yêu cầu giảm số tiền thanh toán hoặc trả lại thẻ'")+(AmountShortChange - CashReceived)+" "+DocumentCurrency;
		Message.Message();
	EndIf;
...
Procedure AddCardOnServer(CurrentStringIdentifier = Undefined)
	...
	NewGroupCardAmount.Group = ChildFormItemsGroup.HorizontalIfPossible;
	...
	NewGroupCardAttributesPart1.Group = ChildFormItemsGroup.HorizontalIfPossible;
	...
	NewGroupCardAttributesPart2.Group = ChildFormItemsGroup.HorizontalIfPossible;
...
Procedure AddDigit(EnteredDigitByString)
	...
	If TemporaryCardsTable.Count() = 0 AND Items.GroupPayment.CurrentPage = Items.GroupCashlessPayment Then
		Message = New UserMessage;
		Message.Text = NStr("en='Add card.';vi='Thêm thẻ.'");
		Message.Message();
	...
	Else
		If Object.POSTerminal.IsEmpty() Then
			Message = New UserMessage;
			Message.Text = NStr("en='Select the POS terminal';vi='Chọn quầy giao dịch'");
			Message.Message();
	...
	For Each CurrentRow IN TemporaryCardsTable Do
		If CurrentRow.Amount > 0 Then
			If Not ValueIsFilled(CurrentRow.ChargeCardKind) Then
				Cancel = True;
				Message = New UserMessage;
				Message.Text = NStr("en='Fill card kind';vi='Điền dạng thẻ'");
				Message.Message();
			EndIf;
			If Not ValueIsFilled(CurrentRow.ChargeCardNo) Then
				Cancel = True;
				Message = New UserMessage;
				Message.Text = NStr("en='Fill card number';vi='Điền số thẻ'");
				Message.Message();
			EndIf;
		EndIf;
	EndDo;
...
"""%ErrorDescription%""."
"Chưa ithực hiện giao dịch theo thẻ.'");
...
Procedure DeletePaymentByCardAfterCardSelection(CurrentData)
	...
	If CurrentData.Amount = 0 Then
		CommonUseClientServer.MessageToUser(NStr("en='Amount in the selected line = 0.';ru='Сумма в выбранной строке = 0.';vi='Số tiền trong dòng đã chọn = 0.'"));
		Return;
	EndIf;
...
Procedure ChooseCashToDeletePayment(Command)
	...
	Else
		...
		Message = New UserMessage;
		Message.Text = NStr("en='Error: delete command is not detected!';vi='Lỗi: lệnh xóa không được phát hiện!'");
		Message.Message();
