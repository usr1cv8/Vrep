...
Procedure GenerateToolTipsToAttributes()
	...
	TitleAmountCheque = NStr("en='Receipt amount (';ru='Сумма чека (';vi='Số tiền trên phiếu ('") + "%Currency%" + ")";
	...
	TitleReceivedCash = NStr("en='Received in cash (';ru='Получено наличными (';vi='Đã nhận tiền mặt ('") + "%Currency%" + ")";
	...
	TitlePaymentWithPaymentCards = NStr("en='By payment cards (';ru='Платежными картами (';vi='Bằng thẻ thanh toán ('") + "%Currency%" + ")";
	...
	TitleAmountPutting = NStr("en='Change (';ru='Сдача (';vi='Cấp ('") + "%Currency%" + ")";
...
Procedure SetModeReadOnly()
	...
	If CashCRUseWithoutEquipmentConnection Then
		Items.IssueReceipt.Title = NStr("en='Cancel issuing';ru='Отменить пробитие';vi='Hủy bỏ đánh phiếu tính tiền'");
...
Procedure CancelModeViewOnly()
	...
	Items.IssueReceipt.Title = NStr("en='Issue cash receipt';ru='Пробить чек';vi='Đánh phiếu'");
	ReadOnly = False;
...
"""%ErrorDescription%""."
"Chưa thực hiện giao dịch theo thẻ.'");
...
"Операция по карте не была выполнена.';vi='Khi kết nối thiết bị"
...
"""%ErrorDescription%""."
"Chưa thực hiện hủy bỏ theo thẻ'"
            );
            MessageText = StrReplace(
            MessageText,
            "%ErrorDescription%",
...
"Операция по карте была отменена.';vi='Khi in phiếu cuối''"
"đã xảy ra lỗi: ""%ErrorDescription%""."
"Đã hủy bỏ giao dịch theo thẻ.'"
            );
...
Procedure DeletePaymentByCard(Command)
	...
	If CurrentData = Undefined Then
		CommonUseClientServer.MessageToUser(NStr("en='Select a row of deleted payment by card';ru='Выберите строку удаляемой оплаты картой.';vi='Hãy chọn dòng thanh toán bằng thẻ cần xóa.'"));
		Return;
	EndIf;
...
"Возможно для данного сеанса еще не выполнялась эквайринговая операция.';vi='Không có phiếu cuối."
"Có thể là đối với phiên làm việc này vẫn chưa thực hiện giao dịch thẻ thanh toán.'"));
				Return;
			EndIf;
...
"возникла ошибка: ""%ОписаниеОшибки%"".';vi='Khi in phiếu cuối"
"đã xảy ra lỗi: ""%ErrorDescription%"".'");
						MessageText = StrReplace(MessageText,
													 "%ErrorDescription%",
													 Output_Parameters[1]);
				...
				Else
					MessageText = NStr("en='An error occurred while connecting"
"the fiscal register: ""%ErrorDescription%"".';ru='При подключении фискального регистратора произошла ошибка: ""%ОписаниеОшибки%"".';vi='Khi kết nối máy ghi nhận tiền mặt, đã xảy ra lỗi: ""%ErrorDescription%"".'");
					MessageText = StrReplace(MessageText, "%ErrorDescription%", ErrorDescription);
					CommonUseClientServer.MessageToUser(MessageText);
				EndIf;
			EndIf;
...
Function PaymentFormOnChangeAtServer(ErrorDescription = "")
	...
	If Object.PaymentForm = Enums.CashAssetTypes.Cash Then
		...
		If Object.PaymentWithPaymentCards.Count() > 0 Then
			...
			ErrorDescription = NStr("en='Paid with payment cards. Cannot set the ""In cash"" payment method';ru='Проведена оплата платежными картами! Установить форму оплаты ""Наличными"" невозможно';vi='Đã tiến hành thanh toán bằng thẻ thanh toán! Không thể đặt biểu mẫu thanh toán ""Bằng tiền mặt""'");
			Return False;
