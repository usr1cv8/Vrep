...
Procedure GenerateTitle(StructureStateCashCRSession)
	...
	If StructureStateCashCRSession.SessionIsOpen Then
		MessageText = NStr("en='%Outlet%, Session No %NumberOfSession% %ModifiedAt%';ru='%Outlet%, Смена № %NumberOfSession%  %ModifiedAt%';vi='%Outlet%, Phiên làm việc số %NumberOfSession%  %ModifiedAt%'");
	...
	Else
		MessageText = NStr("en='%Outlet%';ru='%Outlet%';vi='%Outlet%'");
					...
					If Result Then
						...
						If Result Then
							...
							Except
								...
								FillInDetailsForTSInventoryAtClient();
								ShowMessageBox(Undefined, NStr("en='Failed to post document';ru='Не удалось выполнить проведение документа';vi='Không thể thực hiện kết chuyển chứng từ'")); // Asynchronous method!
								Return;
							EndTry;
...
"bổ sung: %ДополнительноеОписание%'"
				...
				Else
					...
					MessageText = NStr("en='Fiscal data recorder is not selected';ru='Не выбран фискальный регистратор';vi='Chưa chọn máy ghi nhận tiền mặt'");
					CommonUseClientServer.MessageToUser(MessageText);
...
Procedure IssueReceiptExecute(Command, GenerateSalesReceipt = False)
	...
	If Object.DeletionMark Then
		...
		ErrorText = NStr("en='The document is marked for deletion';ru='Документ помечен на удаление';vi='Chứng từ đã bị đánh dấu xóa'");
	...
	If Object.DocumentAmount > Object.CashReceived + Object.PaymentWithPaymentCards.Total("Amount") Then
		...
		ErrorText = NStr("en='The payment amount is less than the receipt amount';ru='Сумма оплаты меньше суммы чека';vi='Số tiền thanh toán nhỏ hơn số tiền trên phiếu tính tiền'");
	...
	If Object.DocumentAmount < Object.PaymentWithPaymentCards.Total("Amount") Then
		...
		ErrorText = NStr("en='The amount of payment by payment cards exceeds the total of a receipt';ru='Сумма оплаты платежными картами превышает сумму чека';vi='Số tiền thanh toán bằng thẻ thanh toán vượt quá số tiền của phiếu tính tiền'");
...
Procedure ReportPrintingWithoutBlankingExecute()
	...
	If EquipmentManagerClient.RefreshClientWorkplace() Then
		...
		EquipmentManagerClient.OfferSelectDevice(NOTifyDescription, 
																	 "FiscalRegister",
																	 NStr("en='Select a fiscal data recorder';ru='Выберите фискальный регистратор';vi='Hãy chọn máy ghi nhận tiền mặt'"), 
																	 NStr("en='Fiscal data recorder is not connected';ru='Фискальный регистратор не подключен';vi='Máy ghi nhận tiền mặt chưa kết nối'")
																	 );
		If Not IsBlankString(MessageText) Then
			MessageText = NStr("en='Print X report';ru='Печать X-отчета';vi='In báo cáo giữa phiên thu ngân'") + MessageText;
			CommonUseClientServer.MessageToUser(MessageText);
		EndIf;
...
"Дополнительное"
"описание: %ДополнительноеОписание%';vi='Khi kết nối thiết bị đã xảy ra lỗi."
"Chưa in báo cáo và phiên thu ngân chưa bị đóng trên máy ghi nhận tiền mặt."
"Mô tả"
"bổ sung: %AdditionalDetails%'"
				);
				MessageText = StrReplace(MessageText, "%AdditionalDetails%", ErrorDescription);
...
"Дополнительное"
"описание: %ДополнительноеОписание%';vi='Khi hủy bỏ phiếu tính tiền đã xảy ra lỗi. Chưa hủy bỏ phiếu tính tiền trên máy ghi nhận tiền mặt."
"Mô tả"
"bổ sung: %AdditionalDetails%'");
					MessageText = StrReplace(MessageText,
												 "%AdditionalDetails%",
...
"Дополнительное"
"описание: %ДополнительноеОписание%';vi='Khi kết nối thiết bị đã xảy ra lỗi. Chưa hủy phiếu tính tiền trên máy ghi nhận tiền mặt."
"Mô tả"
"bổ sung: %AdditionalDetails%"
"'");
				MessageText = StrReplace(MessageText, "%AdditionalDetails%", ErrorDescription);
...
Procedure PrintCopyOnFiscalRegistrar(Command)
	...
	If Not UsePeripherals Then
		...
		MessageText = NStr("en='Cannot print the sales slip. Peripherals are not used.';ru='Слип-чек не может быть напечатан. Подключаемое оборудование не используется.';vi='Không thể in phiếu cuối. Chưa sử dụng thiết bị kết nối.'");
		CommonUseClientServer.MessageToUser(MessageText);
	...
	If EquipmentManagerClient.RefreshClientWorkplace() Then
		...
		EquipmentManagerClient.OfferSelectDevice(NOTifyDescription, 
																	 ...
																	 NStr("en='Select a fiscal data recorder';ru='Выберите фискальный регистратор';vi='Hãy chọn máy ghi nhận tiền mặt'"), 
																	 NStr("en='Fiscal data recorder is not connected';ru='Фискальный регистратор не подключен';vi='Máy ghi nhận tiền mặt chưa kết nối'")
																	 //, NStr("en='Fiscal data recorder is not selected';ru='Фискальный регистратор не выбран';vi='Chưa chọn máy ghi nhận tiền mặt'"),
																	 //True,
																	 //MessageText
																	 );
		If Not IsBlankString(MessageText) Then
			MessageText = NStr("en='Print the last sales slip';ru='Напечатать последний слип чек';vi='In séc cuối gần đây nhất'") + MessageText;
			CommonUseClientServer.MessageToUser(MessageText);
		EndIf;
		
	Else
		MessageText = NStr("en='Fiscal data recorder is not connected';ru='Фискальный регистратор не подключен';vi='Máy ghi nhận tiền mặt chưa kết nối'");
		CommonUseClientServer.MessageToUser(MessageText);
	EndIf;
		...
		If ResultFR Then
			If Not IsBlankString(glPeripherals.LastSlipReceipt) Then
				...
				If Not ResultFR Then
					MessageText = NStr("en='An error occurred when printing a sales slip: ""%ErrorDescription%"".';ru='При печати слип-чека возникла ошибка: ""%ОписаниеОшибки%"".';vi='Khi in cạnh phiếu cuối, đã xảy ra lỗi: ""%ErrorDescription%"".'"); 
					MessageText = StrReplace(MessageText,
					                             "%ErrorDescription%",
					                             Output_Parameters[1]);
					CommonUseClientServer.MessageToUser(MessageText);
...
"Слип-чек не напечатан.';vi='Khi kết nối máy ghi nhận tiền mặt, đã xảy ra lỗi: ""%ErrorDescription%""."
"Chưa in được phiếu cuối.'");
	...
	For Each CurrentSettingCommandButtons IN CWPSetting.LowerBarButtons Do
		...
		Except
			...
			Message.Text = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Error is occurred when button and menu command setting. %1';ru='Произошла ошибка при настройке кнопок и команд меню.';vi='Đã xảy ra lỗi khi tùy chỉnh nút và lệnh trên menu."
"'"), ErrorDescription());
			Message.Message();
		EndTry;
	EndDo;
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(CashCR) Then
		Message = New UserMessage();
		Message.Text = NStr("en='Cash register is not determined for the user.';ru='Для пользователя не определена Касса ККМ!';vi='Không được xác định quầy thu ngân cho người sử dụng!'");
		Message.Message();
		Cancel = True;
		Return;
	EndIf;
...
Procedure UpdateLabelVisibleTimedOutOver24Hours(StructureStateCashCRSession = Undefined)
	...
	If StructureStateCashCRSession.SessionIsOpen Then
		MessageText = NStr("en='Register shift is opened';ru='Кассовая смена открыта';vi='Đã mở quầy thu ngân'");
...
Procedure QuickProductsSettings(Command)
	...
	Else
		Message = New UserMessage;
		Message.Text = NStr("en='CWP setting is not selected';vi='Chưa chọn tùy chỉnh quầy thu ngân'");
		Message.Message();
	EndIf;
...
Procedure CreateDebitInvoiceForReturn(Command)
	...
	Else
		...
		Else
			Message = New UserMessage;
			Message.Text = NStr("en='Receipt CR is not selected!';vi='Phiếu tính tiền trả lại chưa chọn'");
...
Procedure CreateReceiptCRForReturn(Command)
	...
	If Not ReceiptCRForReturn.IsEmpty() Then
		Message = New UserMessage;
		Message.Text = NStr("en='Receipt for return is already created.';vi='Phiếu tính tiền trả lại đã được tạo.'");
...
Procedure CreateCPVBasedOnReceipt(Command)
	...
	If SupplierInvoiceForReturn.IsEmpty() Then
		Message = New UserMessage;
		Message.Text = NStr("en='First of all it is necessary to create the supplier invoice for return.';vi='Trước hết, cần thiết tạo hóa đơn nhà cung cấp để trả lại.'");
...
Procedure Pay(Command)
	...
	If Object.Inventory.Count() = 0 Or Object.Inventory.Total("Amount") = 0 Then
		Message = New UserMessage;
		Message.Text = NStr("en='Amount to payment = 0!';vi='Số tiền thanh toán = 0!'");
	...
	If Not ControlAtWarehouseDisabled Then
		If Not ReserveAtServer() Then
			CommonUseClientServer.MessageToUser(NStr("en='It wasn't succeeded to execute reservation!';vi='';"));
			Return;
		Else
			FillInDetailsForTSInventoryAtClient();
		EndIf;
	...
	For Each CurrentRow IN Object.Inventory Do
		If CurrentRow.Price = 0 Then
			Message = New UserMessage;
			Message.Text = NStr("en='In string price is not filled';vi='Dòng giá chưa điền'");
		...
		If CurrentRow.Quantity = 0 Then
			Message = New UserMessage;
			Message.Text = NStr("en='In string quantity isn't filled';vi='';");
		...
		If CurrentRow.ProductsAndServices.IsEmpty() Then
			Message = New UserMessage;
			Message.Text = NStr("en='In string ProductsAndServices is not selected';vi='Dòng ProductsAndServices không được chọn'");
...
Procedure PrintSalesReceipt(Command)
	...
	If ThereAreRetailSaleReports Then
		Message = New UserMessage;
		Message.Text = NStr("en='For retail sale reports sales receipt isn't formed.';vi='';");
		Message.Field = "List";
		Message.Message();
	EndIf;
...
Procedure Dependencies(Command)
	...
	Else
		Message = New UserMessage;
		Message.Text = NStr("en='Select document!';vi='Chọn chứng từ!'");
		Message.Field = "ReceiptCRList";
		Message.Message();
	EndIf;
...
Function ReserveAtServer(CancelReservation = False)
	...
	Else
		...
		Except
			...
			Message = New UserMessage;
			Message.Text = NStr("en='It wasn't succeeded to execute document post!';vi='';");
			Message.Message();
		EndTry;
...
Procedure ReceiptIsNotShownOnChangeAtServer()
	...
	If ReceiptIsNotShown Then
		...
		Items.PagesReceiptCRList_and_ReceiptCRContent_and_PageWithLabel.CurrentPage = Items.PageWithEmptyLabel;
	Else
		Items.DecorationTitleReceiptsCR.Title = "Choose reason for return";
		
		Items.PagesReceiptCRList_and_ReceiptCRContent_and_PageWithLabel.CurrentPage = Items.PageReceiptCRList_and_ReceiptCRContent;
	EndIf;
...
Procedure CashCROnChange(Item)
	...
	If CashCR.IsEmpty() Then
		Message = New UserMessage;
		Message.Text = NStr("en='Cash register can't be empty';vi='';");
...
Procedure GroupQuantity(Command)
	...
	Else
		Message = New UserMessage;
		Message.Text = NStr("en='String is not selected!';vi='Dòng chưa được chọn'");
...
Procedure CreateSupplierInvoiceListForReturn(Command)
	...
	If CurrentData <> Undefined Then
		If TypeOf(CurrentData.Ref) = Type("DocumentRef.ReceiptCR") Then
			OpenForm("Document.SupplierInvoice.ObjectForm", New Structure("Basis, CWP", CurrentData.Ref, True), ThisObject, UUID);
		Else
			MessageText = NStr("en='Receipt CR for return is not allowed to enter based on kind documents ""';vi='Phiếu tính tiền để trả lại không được phép nhập dựa trên cơ sở chứng từ ""'")+TypeOf(CurrentData.Ref)+""".";
		EndIf;
	Else
		MessageText = NStr("en='Receipt CR is not selected!';vi='Phiếu tính tiền trả lại chưa chọn'");
	EndIf;
...
"Go to ""Applied discounts"" is possible only after the selection in list.';vi='Chứng từ không được chọn."
"Chỉ có thể đi đến ""Áp dụng chiết khấu"" sau khi lựa chọn trong danh sách.'");
		Message = New UserMessage;
		Message.Text = MessageText;
		Message.Field = "List";
		Message.Message();
	...
	Else
		MessageText = NStr("en='Select receipt CR or receipt CR for return';vi='Chọn phiếu hoặc phiếu trả lại'");
		Message = New UserMessage;
		Message.Text = MessageText;
		Message.Field = "List";
		Message.Message();
...
Procedure DisplayInformationOnCustomerDisplay()
	...
	DPText = ?(
	  ...
	  + Chars.LF
	  + NStr("en='Total: ';ru='Итого: ';vi='Tổng số:'")
...
"Дополнительное"
"описание: %ДополнительноеОписание%';vi='Khi sử dụng màn hình khách hàng đã xảy ra lỗi."
"Mô tả"
"bổ sung: %AdditionalDetails%'"
			);
			MessageText = StrReplace(
...
Procedure SearchByBarcode(Command)
	...
	ShowInputValue(NOTifyDescription, CurBarcode, NStr("en='Enter barcode';ru='Введите штрихкод';vi='Hãy nhập mã vạch'"));
...
Procedure SetPeriodAtServer(PeriodKindCWP, ListName, ParameterStandardPeriod = Undefined)
	...
	If PeriodKindCWP = ForCurrentShift Then
		...
		If ListName = "List" Then
			...
			MagazinePeriodKind = NStr("en='For current shift';ru='За текущую смену';vi='Cho lần thay đổi hiện tại'");
		ElsIf ListName = "ReceiptCRListForReturn" Then
			ReceiptCRPeriodKindForReturn = NStr("en='For current shift';ru='За текущую смену';vi='Cho lần thay đổi hiện tại'");
		ElsIf ListName = "ReceiptCRList" Then
			ReceiptCRPeriodKind = NStr("en='For current shift';ru='За текущую смену';vi='Cho lần thay đổi hiện tại'");
		EndIf;
	...
	ElsIf PeriodKindCWP = ForPreviousShift Then
		...
		If ListName = "List" Then
			Items.Date.Visible = False;
			MagazinePeriodKind = NStr("en='For last shift';ru='За прошлую смену';vi='Cho lần thay đổi gần nhất'");
		ElsIf ListName = "ReceiptCRListForReturn" Then
			ReceiptCRPeriodKindForReturn = NStr("en='For last shift';ru='За прошлую смену';vi='Cho lần thay đổi gần nhất'");
		ElsIf ListName = "ReceiptCRList" Then
			ReceiptCRPeriodKind = NStr("en='For last shift';ru='За прошлую смену';vi='Cho lần thay đổi gần nhất'");
		EndIf;
	...
	ElsIf PeriodKindCWP = ForYesterday Then
		...
		If ListName = "List" Then
			Items.Date.Visible = False;
			MagazinePeriodKind = NStr("en='For yesterday';ru='За вчерашний день';vi='Cho ngày hôm qua'");
		ElsIf ListName = "ReceiptCRListForReturn" Then
			ReceiptCRPeriodKindForReturn = NStr("en='For yesterday';ru='За вчерашний день';vi='Cho ngày hôm qua'");
		ElsIf ListName = "ReceiptCRList" Then
			ReceiptCRPeriodKind = NStr("en='For yesterday';ru='За вчерашний день';vi='Cho ngày hôm qua'");
		EndIf;
	...
	ElsIf PeriodKindCWP = ForEntirePeriod Then
		...
		If ListName = "List" Then
			Items.Date.Visible = True;
			MagazinePeriodKind = NStr("en='During all the time';ru='За всё время';vi='Trong suốt thời gian'");
		ElsIf ListName = "ReceiptCRListForReturn" Then
			ReceiptCRPeriodKindForReturn = NStr("en='During all the time';ru='За всё время';vi='Trong suốt thời gian'");
		ElsIf ListName = "ReceiptCRList" Then
			ReceiptCRPeriodKind = NStr("en='During all the time';ru='За всё время';vi='Trong suốt thời gian'");
		EndIf;
...
Function GetPeriodPresentation(StandardPeriod, Delimiter)
	...
	If Not ValueIsFilled(StartDate) AND Not ValueIsFilled(EndDate) Then
		Return NStr("en='During all the time';ru='За всё время';vi='Trong suốt thời gian'");;
...
Procedure FillFastGoods(OnOpen = False)
	...
	For Each QuickProduct IN ResultTable Do
		...
		If CurAcc = 1 OR (CurAcc-1) % ColumnQuantity = 0 Then
			...
			NewFolder.Group = ChildFormItemsGroup.HorizontalIfPossible;
		EndIf;
