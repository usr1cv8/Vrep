...
Procedure GenerateTableInventory(DocumentRefFixedAssetsDepreciation, StructureAdditionalProperties)
	...
	Query.SetParameter("DepreciationAccrual", NStr("en='Depreciation accrual';ru='Начисление амортизации';vi='Tính khấu hao'"));
...
Procedure GenerateTableMonthEndErrors(DocumentRefFixedAssetsDepreciation, StructureAdditionalProperties)
	...
	Query.SetParameter("DepreciationAccrual", NStr("en='Depreciation accrual';ru='Начисление амортизации';vi='Tính khấu hao'"));
	Query.SetParameter("DepreciationEqualsZero", NStr("en='calculated depreciation is equal to 0.';ru='расчитанная амортизация равна 0!';vi='khấu hao đã tính bằng 0!'"));
	Query.SetParameter("Company", StructureAdditionalProperties.ForPosting.Company);
	...
	For Each CurRow IN ResultTable Do
		ErrorText = NStr("en='For the %FixedAssetPresentation% (%Code%) property %MessageAboutDepreciationAccrualError%.';ru='Для имущества %ВнеоборотныйАктивПредставление% (%Код%) %СообщениеОбОшибкеНачисленияАмортизации%.';vi='Đối với tài sản %FixedAssetPresentation% (%Code%) %MessageAboutDepreciationAccrualError%.'");
...
Procedure InitializeDocumentData(DocumentRefFixedAssetsDepreciation, StructureAdditionalProperties) Export
	...
	Query.SetParameter("Message1",	   NStr("en='depreciation has already been accrued this month.';ru='амортизация в этом месяце уже начислялась!';vi='Đã tính khấu hao trong tháng này!'"));
	Query.SetParameter("Message2",	   NStr("en='depreciation accrual method is not specified.';ru='не указан способ начисления амортизации!';vi='chưa chỉ ra phương pháp tính khấu hao!'"));
	Query.SetParameter("Message3",	   NStr("en='cost is equal to 0.';ru='стоимость равна 0!';vi='giá trị bằng 0!'"));
	Query.SetParameter("Message4",	   NStr("en='useful life is 0.';ru='срок использования равен 0!';vi='thời hạn sử dụng bằng 0!'"));
	Query.SetParameter("Message5",	   NStr("en='the scope of work to calculate depreciation is not filled in.';ru='объем продукции работ для вычисления амортизации не заполнен!';vi='chưa điền khối lượng sản phẩm công việc để tính khấu hao!'"));
	Query.SetParameter("Message6",	   NStr("en='the initial cost is 0.';ru='первоначальная стоимость равна 0!';vi='giá trị ban đầu bằng 0!'"));
