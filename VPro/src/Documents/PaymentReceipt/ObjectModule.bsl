...
//
Procedure FillPaymentDetails(Invoice = Undefined) Export
	...
	|		AND CASE
	|				WHEN &DocumentFilter
	|					THEN DocumentRegisterRecordsAccountsReceivable.Document = &Document
	|				ELSE TRUE
	|			END
	...
	Query.SetParameter("DocumentFilter", ValueIsFilled(Invoice));
...
Procedure FillByTaxAccrual(BasisDocument)
	...
	If BasisDocument.OperationKind <> Enums.OperationKindsTaxAccrual.Reimbursement Then
		Raise NStr("en='You can enter receipt to account only based on the tax refund but not on the accrual.';ru='Поступление на счет можно ввести только на основании возмещения налогов, а не начисления.';vi='Thu vào tài khoản chỉ có thể tiến hành trên cơ sở đền bù thuế, chứ không phải tính lương.'");
	EndIf;
...
Procedure FillBySalesInvoice(BasisDocument)
	...
	If BasisDocument.OperationKind = Enums.OperationKindsCustomerInvoice.TransferToProcessing Then
		ErrorMessage = NStr("en='Cannot enter the document based on the operation - ""%OperationKind"".';ru='Невозможен ввод документа на основании операции - ""%ВидОперации""!';vi='Không thể nhập chứng từ trên cơ sở giao dịch - ""%OperationKind""!'");
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	ElsIf OperationKind = Enums.OperationKindsPaymentReceipt.LoanRepaymentByEmployee Then
	...
	ElsIf OperationKind = Enums.OperationKindsPaymentReceipt.LoanSettlements Then
...
Procedure FillByLoanContract(DocRefLoanContract, Amount = Undefined) Export
	...
	If Amount <> Undefined Then
		...
		|			THEN VALUE(Enum.OperationKindsPaymentReceipt.LoanSettlements)
		|		ELSE VALUE(Enum.OperationKindsPaymentReceipt.LoanRepaymentByEmployee)
		|	END AS OperationKind,
		|	&Ref AS BasisDocument,
		|	DocumentTable.Company AS Company,
...
Procedure FillByAccrualsForLoans(FillingData) Export
	...
	"SELECT
	|	REFPRESENTATION(&Ref) AS Basis,
	|	VALUE(Enum.OperationKindsPaymentReceipt.LoanRepaymentByEmployee) AS OperationKind,
	|	&Ref AS BasisDocument,
	|	DocumentTable.Ref.Company AS Company,
	|	DocumentTable.SettlementsCurrency AS CashCurrency,
	|	DocumentTable.Employee AS AdvanceHolder,
