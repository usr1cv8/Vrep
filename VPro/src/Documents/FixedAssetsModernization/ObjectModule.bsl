...
"'"
	...
	For Each RowOfFixedAssets IN FixedAssets Do
		...
		If ArrayVAStatus.Find(RowOfFixedAssets.FixedAsset) = Undefined Then
			MessageText = NStr("en='States are not registered for the %FixedAsset% property in the %LineNumber% line of the """"Property"""" list.';ru='Для имущества ""%ВнеоборотныйАктив%"" указанного в строке %НомерСтроки% списка """"Имущество"""", не зарегистрированы состояния.';vi='Đối với tài sản ""%FixedAsset%"" đã chỉ ra tại dòng %LineNumber% của danh sách """"Tài sản cố định"""" chưa ghi nhận trạng thái.'"
			);
		...
		ElsIf ArrayVAAcceptedForAccounting.Find(RowOfFixedAssets.FixedAsset) = Undefined Then
			MessageText = NStr("en='The current state for the %FixedAsset% property in line No. %LineNumber% of the ""Property"" list is ""Taken off the books"".';ru='Для имущества ""%ВнеоборотныйАктив%"" указанного в строке %НомерСтроки% списка ""Имущество"", текущее состояние ""Снят с учета"".';vi='Đối với tài sản cố định ""%FixedAsset%"" mà đã chỉ ra tại dòng %LineNumber% trong danh sách ""Tài sản cố định"", trạng thái hiện tại ""Loại bỏ khỏi kế toán"".'"
			);
			MessageText = StrReplace(MessageText, "%FixedAsset%", TrimAll(String(RowOfFixedAssets.FixedAsset))); 
			MessageText = StrReplace(MessageText, "%LineNumber%",String(RowOfFixedAssets.LineNumber));
			SmallBusinessServer.ShowMessageAboutError(
	...
	For Each RowOfFixedAssets IN FixedAssets Do
		...
		If RowOfFixedAssets.FixedAsset.DepreciationMethod = Enums.FixedAssetsDepreciationMethods.ProportionallyToProductsVolume
		   AND RowOfFixedAssets.AmountOfProductsServicesForDepreciationCalculation = 0 Then
			MessageText = NStr("en='For fixed assets ""%FixedAsset%"" indicated in row No.%LineNumber% of the ""Property"" list, the ""Product (work) volume for depreciation calculation in physical terms"" should be filled."".';ru='Для имущества ""%ВнеоборотныйАктив%"" указанного в строке %НомерСтроки% списка ""Имущество"", должен быть заполнен ""Объем продукции (работ) для исчисления амортизации в натуральных ед."".';vi='Đối với tài sản cố định ""%FixedAsset%""  mà đã chỉ ra tại dòng %LineNumber% trong danh sách ""Tài sản cố định"", cần điền ""Khối lượng sản phẩm (công việc) để trích khấu hao theo đơn vị tự nhiên"".'"
			);
			MessageText = StrReplace(MessageText, "%FixedAsset%", TrimAll(String(RowOfFixedAssets.FixedAsset))); 
			MessageText = StrReplace(MessageText, "%LineNumber%",String(RowOfFixedAssets.LineNumber));
			SmallBusinessServer.ShowMessageAboutError(
