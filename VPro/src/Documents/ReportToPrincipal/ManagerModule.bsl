...
Procedure GenerateTableManagerial(DocumentRefReportToCommissioner, StructureAdditionalProperties)
	...
	Query.SetParameter("ComitentDebt", NStr("en='Debt to principal';ru='Задолженность комитенту';vi='Công nợ phải trả cho người đặt ký gửi'"));
...
Procedure GenerateTableInventoryReceived(DocumentRefReportToCommissioner, StructureAdditionalProperties)
	...
	Query.SetParameter("InventoryreceptionPostponedIncome", NStr("en='Inventory receipt';ru='Прием запасов';vi='Tiếp nhận vật tư'"));
...
Procedure GenerateTableAccountsPayable(DocumentRefReportToCommissioner, StructureAdditionalProperties)
	...
	Query.SetParameter("AppearenceOfLiabilityToVendor", NStr("en='Incurrence of liabilities to supplier';ru='Возникновение обязательств перед поставщикт';vi='Phát sinh công nợ phải trả nhà cung cấp'"));
...
Procedure InitializeDocumentData(DocumentRefReportToCommissioner, StructureAdditionalProperties) Export
	...
	|	ReportToPrincipalInventory.CountryOfOrigin AS CountryOfOrigin,
	|	ReportToPrincipalInventory.CCDNo AS CCDNo
	...
	// Inventory by CCD
	SmallBusinessServer.GenerateTableInventoryByCCD(DocumentRefReportToCommissioner, StructureAdditionalProperties, True);
...
Procedure RunControl(DocumentRefReportToCommissioner, AdditionalProperties, Cancel, PostingDelete = False) Export
	...
	If StructureTemporaryTables.RegisterRecordsInventoryReceivedChange
		...
		If Not ResultsArray[2].IsEmpty() Then
			...
			SmallBusinessServer.ShowMessageAboutPostingToInventoryByCCDRegisterErrors(DocumentObjectReportToPrincipal, QueryResultSelection, Cancel);
		EndIf;
...
Function PrintCertificate(ObjectsArray, PrintObjects)
	...
	While Header.Next() Do
		...
		TemplateArea.Parameters.HeaderText			= NStr("en='Acceptance certificate';ru='Акт выполненных работ';vi='Biên bản cung cấp dịch vụ'");
...
Function ReportToPrincipalPrinting(ObjectsArray, PrintObjects)
	...
	While Header.Next() Do
		...
		TemplateArea.Parameters.HeaderText = NStr("en='Report to principal';ru='Отчет комитенту';vi='Bảng kê hàng bán dành cho người đặt ký gửi'");
		SpreadsheetDocument.Put(TemplateArea);
...
Procedure AddPrintCommands(PrintCommands) Export
	...
	PrintCommand.Presentation = NStr("en='Report to principal';ru='Отчет комитенту';vi='Bảng kê hàng bán dành cho người đặt ký gửi'");
	PrintCommand.FormsList = "DocumentForm,ListForm";
	PrintCommand.CheckPostingBeforePrint = False;
