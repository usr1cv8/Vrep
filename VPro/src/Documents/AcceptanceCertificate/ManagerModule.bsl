...
Procedure GenerateTableIncomeAndExpensesRetained(DocumentRefAcceptanceCertificate, StructureAdditionalProperties)
	...
	|	AND NOT TemporaryTableHeader.KeepAccountingExpensesByServiceContracts
...
Procedure InitializeDocumentData(DocumentRefAcceptanceCertificate, StructureAdditionalProperties) Export
	...
	"SELECT
	|	CASE
	|		WHEN AcceptanceCertificate.Contract.IsServiceContract
	|				AND AcceptanceCertificate.Contract.ServiceContractBusinessActivity <> VALUE(Catalog.BusinessActivities.EmptyRef)
	|			THEN TRUE
	|		ELSE FALSE
	|	END AS KeepAccountingExpensesByServiceContracts,
	|	AcceptanceCertificate.Contract.ServiceContractBusinessActivity AS BusinessActivity,
	|	AcceptanceCertificate.Contract.SettlementsCurrency AS ContractSettlementsCurrency,
	|	AcceptanceCertificate.Contract.SettlementsInStandardUnits AS ContractSettlementsInStandardUnits,
	|	AcceptanceCertificate.Counterparty AS Counterparty,
	|	AcceptanceCertificate.DocumentCurrency AS DocumentCurrency,
	|	AcceptanceCertificate.Multiplicity AS Multiplicity,
	|	AcceptanceCertificate.ExchangeRate AS ExchangeRate,
	|	AcceptanceCertificate.Contract AS Contract,
	|	AcceptanceCertificate.Date AS Date,
	|	AcceptanceCertificate.DiscountMarkupKind AS DiscountMarkupKind,
	|	AcceptanceCertificate.Date AS Period,
	|	AcceptanceCertificate.CustomerOrder AS CustomerOrder,
	|	AcceptanceCertificate.Ref AS Ref,
	|	AcceptanceCertificate.CustomerOrderPosition AS CustomerOrderPosition,
	|	AcceptanceCertificate.Counterparty.DoOperationsByDocuments AS DoOperationsByDocuments,
	|	AcceptanceCertificate.Counterparty.DoOperationsByOrders AS DoOperationsByOrders
	...
	|		ELSE AcceptanceCertificateWorksAndServices.ProductsAndServices.BusinessActivity
	|	END AS BusinessActivitySales,
	|	CASE
	|		WHEN TemporaryTableHeader.KeepAccountingExpensesByServiceContracts
	|			THEN TemporaryTableHeader.BusinessActivity.GLAccountRevenueFromSales
	|		ELSE AcceptanceCertificateWorksAndServices.ProductsAndServices.BusinessActivity.GLAccountRevenueFromSales
	|	END AS AccountStatementSales,
	|	CASE
	|		WHEN TemporaryTableHeader.KeepAccountingExpensesByServiceContracts
	|			THEN TemporaryTableHeader.BusinessActivity.GLAccountCostOfSales
	|		ELSE AcceptanceCertificateWorksAndServices.ProductsAndServices.BusinessActivity.GLAccountCostOfSales
...
Procedure GenerateAct(ObjectsArray, PrintObjects, SpreadsheetDocument)
	...
	While Header.Next() Do
		...
		If VATAmount = 0 Then
			TemplateArea.Parameters.VAT = NStr("en='Without tax (VAT)';vi='Không có thuế GTGT';");
