...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If ThereAreManualDiscounts OR ThereAreAutomaticDiscounts Then
		For Each StringWorksAndServices IN WorksAndServices Do
			...
			If StringWorksAndServices.DiscountMarkupPercent <> 100 AND CurAmountDiscounts < CurAmount
				...
				MessageText = NStr("en='The ""Amount"" column is not filled out in the %Number% row of the ""Works and services"" list.';ru='Не заполнена колонка ""Сумма"" в строке %Номер% списка ""Работы и услуги"".';vi='Chưa điền cột ""Thành tiền"" trong dòng %Number% danh sách ""Công việc và dịch vụ"".'");
	...
	For Each String IN TablePrepayment Do
		...
		If FoundStringWorksAndServices = Undefined
		   ...
		   AND Counterparty.DoOperationsByOrders Then
			MessageText = NStr("en='Advance of order that is different from the one specified in the ""Works and services"" tabular section cannot be set off.';ru='Нельзя зачесть аванс по заказу отличному от указанных в табличной части ""Работы и услуги""!';vi='Không thể tính khoản ứng trước theo đơn hàng khác với các đơn hàng mà đã chỉ ra trong phần bảng ""Công việc và dịch vụ""!'");
			SmallBusinessServer.ShowMessageAboutError(
				,
				MessageText,
				Undefined,
	...
	If GetFunctionalOption("UseBilling") And Contract.IsServiceContract Then
		For Each Str In WorksAndServices Do
			If Not SmallBusinessServer.SoldProductsAndServicesByServiceContractAllowed(Contract, Str.ProductsAndServices, Str.CHARACTERISTIC) Then
				CommonUseClientServer.MessageToUser(
					NStr("en='Запрещено проводить незапланированные услуги/работы по текущему договору обслуживания!';ru='Запрещено проводить незапланированные услуги/работы по текущему договору обслуживания!';vi='Không được thực hiện các dịch vụ / công việc ngoài kế hoạch theo hợp đồng dịch vụ hiện hành!'"),
