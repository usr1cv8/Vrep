...
Procedure CreateAcceptanceCertificate(Command)
	...
	If Items.ListCustomerOrders.CurrentData = Undefined Then
		...
		WarningText = NStr("en='Command cannot be executed for the specified object.';ru='Команда не может быть выполнена для указанного объекта!';vi='Lệnh không thể điền đối với đối tượng đã chỉ ra!'");
	...
	Else
		...
		If DataStructure.GenerateFewOrders Then
			...
			MessageText = NStr("en='The orders have different data (%DataPresentation%) in document headers. Create multiple acceptance certificates?';ru='Заказы отличаются данными (%ПредставлениеДанных%) шапки документов! Сформировать несколько актов выполненных работ?';vi='Đơn hàng khác nhau bởi dữ liệu (%DataPresentation%) của phần đầu chứng từ!"
"Lập nhiều biên bản bàn giao công việc?'");
...
Procedure CreateAcceptanceCertificateEnd(Result, AdditionalParameters) Export
    ...
    If Response = DialogReturnCode.Yes Then
        ...
        Text = NStr("en='Created:';ru='Создание:';vi='Tạo mới:'");
