...
Procedure CustomerOrderOnChange(Item)
	...
	If Object.Prepayment.Count() > 0
	   ...
	   AND CounterpartyDoSettlementsByOrders Then
		...
		ShowQueryBox(New NotifyDescription("CustomerOrderOnChangeEnd", ThisObject), NStr("en='Prepayment setoff will be cleared, continue?';ru='Зачет предоплаты будет очищен, продолжить?';vi='Sẽ hủy việc khấu trừ khoản trả trước, tiếp tục?'"), Mode, 0);
        Return;
	EndIf;
...
Procedure AddAdditionalAttribute(Command)
	...
	FormParameters = New Structure;
	FormParameters.Insert("CurrentSetOfProperties", PredefinedValue("Catalog.AdditionalAttributesAndInformationSets.Document_AcceptanceCertificate"));
