...
Procedure FillPaymentDetails(Invoice = Undefined) Export
	...
	ContractKindsList = Catalogs.CounterpartyContracts.GetContractKindsListForDocument(Ref, OperationKind);
	If NeedFilterByContracts
	   AND Counterparty.DoOperationsByContracts Then
		Query.Text = StrReplace(Query.Text, "// TextOfContractSelection", "And Contract.ContractKind IN (&ContractKindsList)");
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	ElsIf OperationKind = Enums.OperationKindsCashReceipt.LoanRepaymentByEmployee Then
	...
	ElsIf OperationKind = Enums.OperationKindsCashReceipt.LoanSettlements Then
...
Procedure FillByLoanContract(DocRefLoanContract, Amount = Undefined) Export
	...
	If Amount <> Undefined Then
		...
		|			THEN VALUE(Enum.OperationKindsCashReceipt.LoanSettlements)
		|		ELSE VALUE(Enum.OperationKindsCashReceipt.LoanRepaymentByEmployee)
		|	END AS OperationKind,
		|	&Ref AS BasisDocument,
		|	DocumentTable.Company AS Company,
		...
		|		LEFT JOIN InformationRegister.CurrencyRates.SliceLast(&Date, ) AS SettlementsCurrencyRates
		|		ON DocumentTable.SettlementsCurrency = SettlementsCurrencyRates.Currency
		|WHERE
		|	DocumentTable.Ref = &Ref";
...
Procedure FillByAccrualsForLoans(FillingData) Export
	...
	"SELECT
	|	REFPRESENTATION(&Ref) AS Basis,
	|	VALUE(Enum.OperationKindsCashReceipt.LoanRepaymentByEmployee) AS OperationKind,
	|	&Ref AS BasisDocument,
	|	DocumentTable.Ref.Company AS Company,
	|	DocumentTable.SettlementsCurrency AS CashCurrency,
	|	DocumentTable.Employee AS AdvanceHolder,
