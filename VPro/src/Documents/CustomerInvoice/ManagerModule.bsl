...
Procedure GenerateTableIncomeAndExpenses(DocumentRefSalesInvoice, StructureAdditionalProperties)
	...
	Query.SetParameter("Income", NStr("en='Revenue from sale';ru='Выручка от продажи';vi='Doanh thu bán hàng'"));
...
Procedure GenerateTableManagerial(DocumentRefSalesInvoice, StructureAdditionalProperties)
	...
	Query.SetParameter("ReversingSupplies", NStr("en='Delivery reversal';ru='Сторнирование поставки';vi='Hủy giao hàng'"));
...
Procedure GenerateTableServiceContractExecution(DocumentRefCustomerInvoice, StructureAdditionalProperties)
	...
	|	TemporaryTableInventory.ProductsAndServices AS ServiceContractObject,
	|	TemporaryTableInventory.CHARACTERISTIC,
	|	TRUE AS CostSpecified,
	|	TemporaryTableInventory.Quantity,
	|	TemporaryTableInventory.AmountCur AS Amount
	|FROM
...
Procedure InitializeDocumentData(DocumentRefSalesInvoice, StructureAdditionalProperties) Export
	...
	Query.Text =
	"SELECT
	|	CustomerInvoice.Contract.IsServiceContract AS IsServiceContract,
	|	CASE
	|		WHEN CustomerInvoice.Contract.IsServiceContract
	|				AND CustomerInvoice.Contract.ServiceContractBusinessActivity <> VALUE(Catalog.BusinessActivities.EmptyRef)
	|			THEN TRUE
	|		ELSE FALSE
	|	END AS KeepAccountingExpensesByServiceContracts,
	|	CustomerInvoice.Contract.ServiceContractBusinessActivity AS BusinessActivity,
	|	CustomerInvoice.Contract.SettlementsCurrency AS ContractSettlementsCurrency,
	|	CustomerInvoice.Contract.SettlementsInStandardUnits AS ContractSettlementsInStandardUnits,
	...
	|	CustomerInvoice.Contract AS Contract,
	|	CustomerInvoice.Date AS Date,
	|	CustomerInvoice.DiscountMarkupKind AS DiscountMarkupKind,
	|	CustomerInvoice.Date AS Period,
	|	CustomerInvoice.Ref AS Ref,
	|	CustomerInvoice.CustomerOrderPosition AS CustomerOrderPosition,
	|	CustomerInvoice.Counterparty.DoOperationsByDocuments AS DoOperationsByDocuments,
	|	CustomerInvoice.Counterparty.DoOperationsByOrders AS DoOperationsByOrders,
	|	CustomerInvoice.OperationKind AS OperationKind
	|INTO TemporaryTableHeader
	|FROM
	|	Document.CustomerInvoice AS CustomerInvoice
	|WHERE
	|	CustomerInvoice.Ref = &Ref
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT
	...
	|		ELSE CustomerInvoiceInventory.ProductsAndServices.BusinessActivity
	|	END AS BusinessActivitySales,
	|	CASE
	|		WHEN TemporaryTableHeader.KeepAccountingExpensesByServiceContracts
	|			THEN TemporaryTableHeader.BusinessActivity.GLAccountRevenueFromSales
	|		ELSE CustomerInvoiceInventory.ProductsAndServices.BusinessActivity.GLAccountRevenueFromSales
	|	END AS AccountStatementSales,
	|	CASE
	|		WHEN TemporaryTableHeader.KeepAccountingExpensesByServiceContracts
	|			THEN TemporaryTableHeader.BusinessActivity.GLAccountCostOfSales
	|		ELSE CustomerInvoiceInventory.ProductsAndServices.BusinessActivity.GLAccountCostOfSales
	...
	|	CustomerInvoiceInventory.CountryOfOrigin AS CountryOfOrigin,
	|	CustomerInvoiceInventory.CCDNo AS CCDNo,
	|	CASE
	|		WHEN CustomerInvoiceInventory.Ref.OperationKind = VALUE(Enum.OperationKindsCustomerInvoice.TransferForCommission)
	|				OR CustomerInvoiceInventory.Ref.OperationKind = VALUE(Enum.OperationKindsCustomerInvoice.TransferToProcessing)
	|				OR CustomerInvoiceInventory.Ref.OperationKind = VALUE(Enum.OperationKindsCustomerInvoice.TransferForSafeCustody)
	...
	SmallBusinessServer.GenerateTableInventoryByCCD(DocumentRefSalesInvoice, StructureAdditionalProperties, True);
	...
	GenerateTableManagerial(DocumentRefSalesInvoice, StructureAdditionalProperties);
		...
		// Billing
	GenerateTableServiceContractExecution(DocumentRefSalesInvoice, StructureAdditionalProperties);
	...
	If StructureTemporaryTables.RegisterRecordsInventoryChange
	 ...
	 OR StructureTemporaryTables.RegisterRecordsAccountsReceivableChange Then
		...
		If Not ResultsArray[0].IsEmpty()
			...
			Then DocumentObjectSalesInvoice = DocumentRefSalesInvoice.GetObject()
		EndIf;
		...
		// Negative balance InventoryByCCD
		If Not ResultsArray[8].IsEmpty() Then
			QueryResultSelection = ResultsArray[8].Select();
			SmallBusinessServer.ShowMessageAboutPostingToInventoryByCCDRegisterErrors(DocumentObjectSalesInvoice, QueryResultSelection, Cancel);
		EndIf;
...
// Procedure of generating printed form CustomerInvoice
...
Procedure GenerateWarehouseOutputSlip(SpreadsheetDocument, CurrentDocument)
	...
	|	CustomerInvoice.Company AS Company,
	|	CustomerInvoice.Company.Prefix AS Prefix,
	|	CustomerInvoice.Department AS Department,
	|	CustomerInvoice.StructuralUnit AS Warehouse,
	|	CustomerInvoice.Counterparty AS Receiver,
	|	CustomerInvoice.Author AS Author,
	|	CustomerInvoice.DocumentCurrency AS DocumentCurrency,
	|	CustomerInvoice.AmountIncludesVAT AS AmountIncludesVAT,
	|	CustomerInvoice.Comment AS Comment,
	|	CustomerInvoice.Inventory.(
	|		LineNumber AS LineNumber,
	|		CASE
	...
	InfoAboutCounterparty 	= SmallBusinessServer.InfoAboutLegalEntityIndividual(Selection.Receiver, Selection.DocumentDate, ,);
	...
	FillStructureSection.Insert("Address", 	InfoAboutCounterparty.LegalAddress);
