...
Procedure CreateCustomerInvoice(Command)
	...
	Else
		...
		If DataStructure.GenerateFewOrders Then
			...
			MessageText = NStr("en='The orders have different data (%DataPresentation%) in document headers. Create multiple invoices?';ru='Заказы отличаются данными (%ПредставлениеДанных%) шапки документов! Сформировать несколько расходных накладных?';vi='Đơn hàng khác nhau bởi dữ liệu (%DataPresentation%) của phần đầu chứng từ!"
"Lập nhiều hóa đơn giao hàng?'");
