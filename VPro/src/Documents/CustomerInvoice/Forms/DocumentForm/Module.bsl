...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	DataImportFromExternalSources.OnCreateAtServer(Metadata.Documents.CustomerInvoice.TabularSections.Inventory, DataLoadSettings, ThisObject);
...
Procedure OnClose()
	...
	If UseAutomaticDiscounts AND DiscountsCalculatedBeforeWrite Then
		ShowUserNotification(NStr("en='Change:';ru='Изменение:';vi='Thay đổi:'"), 
									GetURL(Object.Ref), 
									String(Object.Ref) + NStr("en='. Automatic discounts (markups) are calculated!';ru='. Автоматические скидки (наценки) рассчитаны!';vi='. Đã tự động tính chiết khấu (phụ thu)!'"), 
...
Procedure OrderOnChange(Item)
	...
	If Object.Prepayment.Count() > 0
	   ...
	   AND Object.OperationKind = PredefinedValue("Enum.OperationKindsCustomerInvoice.SaleToCustomer") Then
		...
		ShowQueryBox(New NotifyDescription("OrderOnChangeEnd", ThisObject), NStr("en='Prepayment setoff will be cleared, continue?';ru='Зачет предоплаты будет очищен, продолжить?';vi='Sẽ hủy việc khấu trừ khoản trả trước, tiếp tục?'"), Mode, 0);
        Return;
	EndIf;
...
Procedure AddAdditionalAttribute(Command)
	...
	FormParameters.Insert("CurrentSetOfProperties", PredefinedValue("Catalog.AdditionalAttributesAndInformationSets.Document_CustomerInvoice"));
...
Procedure ChangeReserveFillByReserves(Command)
	...
	If Object.Inventory.Count() = 0 Then
		Message = New UserMessage;
		Message.Text = NStr("en='The ""Inventory and services"" tabular section is not filled in.';ru='Табличная часть ""Запасы и услуги"" не заполнена!';vi='Phần bảng ""Vật tư và dịch vụ"" chưa điền!'");
		Message.Message();
		Return;
	EndIf;
...
Procedure ProcessContractConditionsChange(ContractData, ContractBeforeChange)
	...
	Object.StampBase = NStr("en='Contract: ';ru='Договор: ';vi='Hợp đồng:'") + String(Object.Contract);
...
Procedure SetVisibleAndEnabled(ChangedTypeOperations = False)
	...
	ElsIf Object.OperationKind = PredefinedValue("Enum.OperationKindsCustomerInvoice.TransferForSafeCustody") Then
		Items.Counterparty.Title = NStr("en='Recipient';ru='Получатель';vi='Người nhận'");
	Else	
		Items.Counterparty.Title = NStr("en='Supplier';ru='Поставщик';vi='Đối tác'");
	EndIf;
...
Procedure DataLoadFromFile(Command)
	...
	ExportParameters.Title = NStr("en='Import inventory from file';ru='Загрузка запасов из файла';vi='Kết nhập vật tư từ tệp'");
...
Procedure LoadFromFileInventory(Command)
	...
	DataLoadSettings.Insert("TabularSectionFullName",	"CustomerInvoice.Inventory");
	DataLoadSettings.Insert("Title",					NStr("en='Import inventory from file';ru='Загрузка запасов из файла';vi='Kết nhập vật tư từ tệp'"));
	DataLoadSettings.Insert("OrderPositionInDocument", 	Object.CustomerOrderPosition);
...
Procedure CCDMode(Command)
	...
	CommonUseClientServer.SetFormItemProperty(Items, "CCDModeContext", "Check", Not Items.CCDModeContext.Check);
...
Procedure EnableCCDOperationsMode()
	...
	UnchangeableAttributes.Add("InventoryCheck");
...
&AtClient
Procedure CCDNumbersFillByActualBalances(Command)
	
	CCDNumbersFillByActualBalancesAtServer(True);

EndProcedure

&AtServer
Procedure CCDNumbersFillByActualBalancesAtServer(IsContextCall)
	...
	SelectionParameters.Insert("IndexOfCurrentRow", ?(ValueIsFilled(Items.Inventory.CurrentRow), Object.Inventory.IndexOf(Object.Inventory.FindByID(Items.Inventory.CurrentRow)), 0));
	...
	FieldsNamesArray.Add("Amount");
	FieldsNamesArray.Add("VATAmount");
	FieldsNamesArray.Add("Total");
	...
	CargoCustomsDeclarationsServer.TransferCCDNumberBalancesToFormTable(Object.Inventory, SelectionParameters);
	
	If SelectionParameters.IndexOfCurrentRow <> -1 Then
		
		CollectionRow = Object.Inventory.Get(SelectionParameters.IndexOfCurrentRow);
		Items.Inventory.CurrentRow = CollectionRow.GetID();
