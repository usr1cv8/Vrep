...
Procedure FillByStructure(FillingData) Export
	...
	If FillingData.Property("Basis")
		...
		Then
		Raise NStr("en='Cannot enter invoice on the basis of work order.';ru='Нельзя ввести Расходную накладную на основании заказ-наряда!';vi='Không thể nhập Hóa đơn nhận hàng trên cơ sở đơn hàng trọn gói!'");
	EndIf;
	
	If FillingData.Property("ArrayOfCustomerOrders") Then
	...
	If Operation = "Sale" Then
		...
		Else
			TextExclusion = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Can not enter the operation ""Sale to customer"" on the basis of the operation - ""%1""!';ru='Невозможен ввод операции ""Продажа покупателю"" на основании операции - ""%1""!';vi='Không thể nhập giao dịch ""Bán hàng cho khách"" trên cơ sở giao dịch - ""%1""!'"),
	...
	ElsIf Operation = "Return" Then
		...
		Else
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Cannot input the ""Return"" operation based on the ""%1"" operation.';ru='Невозможен ввод операции ""Возврат"" на основании операции - ""%1""!';vi='Không thể nhập giao dịch ""Trả lại"" trên cơ sở giao dịch - ""%1""!'"),
				FillingData.OperationKind);
			Raise ErrorMessage;
		EndIf;			
	EndIf;
...
Procedure FillByCustomerOrder(FillingData) Export
	...
	If Not ValueIsFilled(StructuralUnit) Then
		...
		If Not ValueIsFilled(SettingValue) Then
			StructuralUnit = CommonUseClientServer.PredefinedItem("Catalog.StructuralUnits.MainWarehouse");
		EndIf;
	EndIf;
	...
	For Each String IN TablePrepayment Do
		...
		If FoundStringInventory = Undefined
		   ...
		   AND Counterparty.DoOperationsByOrders Then
			MessageText = NStr("en='Advance of order different from the one specified in tabular sections ""inventory"" or ""Expenses"" cannot be set off.';ru='Нельзя зачесть аванс по заказу отличному от указанных в табличных частях ""Запасы"" или ""Расходы""!';vi='Không thể tính khoản ứng trước theo đơn hàng khác với các đơn hàng mà đã chỉ ra trong phần bảng ""Vật tư"" hoặc ""Chi phí""!'");
	...
	If Constants.FunctionalOptionInventoryReservation.Get()
		...
		For Each StringInventory IN Inventory Do
			...
			If StringInventory.Reserve > StringInventory.Quantity Then
				...
				MessageText = NStr("en='In row No. %Number% of the ""Inventory and services"" tabular section, the quantity of items shipped from the reserve exceeds the total quantity of inventory.';ru='В строке №%Номер% табл. части ""Запасы и услуги"" количество отгружаемых позиций из резерва превышает общее количество запасов.';vi='Tại dòng số %Number% của phần bảng ""Vật tư và dịch vụ"" số lượng mặt hàng xuất bán từ dự phòng cao hơn tổng số lượng vật tư.'");
