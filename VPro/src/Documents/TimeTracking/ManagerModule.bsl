...
Procedure InitializeDocumentData(DocumentRefTimeTracking, StructureAdditionalProperties) Export
	...
	For Counter = 0 To 6 Do
		...
		PutInTempTable = ?(Counter = 0, "INTO TemporaryTableWorks", "");
		...
		|&Company AS Company, 
		|DATEADD(TimeTrackingOperations.Ref.DateFrom, MINUTE, Hour(TimeTrackingOperations." + Prefix + "BeginTime) * 60 + MINUTE (TimeTrackingOperations." + Prefix + "BeginTime) + 1440 * " + Counter + ") AS BeginTime, 
	...
	GenerateWorkOrderTable(DocumentRefTimeTracking, StructureAdditionalProperties);
	
	// Billing
	GenerateTableServiceContractExecution(DocumentRefTimeTracking, StructureAdditionalProperties);
	
EndProcedure // DocumentDataInitialization()

#Region InternalProceduresAndFunctions

Procedure GenerateWorkOrderTable(DocumentRefWorkOrder, StructureAdditionalProperties)
	
	Query = New Query;
	Query.TempTablesManager = StructureAdditionalProperties.ForPosting.StructureTemporaryTables.TempTablesManager;
	Query.Text =
	"SELECT
	|	TemporaryTableWorks.LineNumber,
	|	TemporaryTableWorks.Period,
	|	TemporaryTableWorks.Counterparty,
	|	TemporaryTableWorks.Contract,
	|	TemporaryTableWorks.CustomerOrder,
	|	TemporaryTableWorks.Employee,
	|	TemporaryTableWorks.ProductsAndServices,
	|	TemporaryTableWorks.CHARACTERISTIC,
	|	TemporaryTableWorks.WorkKind,
	|	TemporaryTableWorks.ImportFact,
	|	TemporaryTableWorks.AmountFact,
	|	TemporaryTableWorks.StructuralUnit,
	|	TemporaryTableWorks.Company,
	|	TemporaryTableWorks.BeginTime,
	|	TemporaryTableWorks.EndTime,
	|	TemporaryTableWorks.Comment
	|FROM
...
EndProcedure // СФормироватьТаблицаЗаданияНаРаботу()

Procedure GenerateTableServiceContractExecution(DocumentRefWorkOrder, StructureAdditionalProperties)
	...
	|	TemporaryTableWorks.CHARACTERISTIC,
	|	CASE
	|		WHEN TemporaryTableWorks.AmountFact = 0
	|			THEN FALSE
	|		ELSE TRUE
	|	END AS CostSpecified,
	|	TemporaryTableWorks.ImportFact AS Quantity,
	|	TemporaryTableWorks.AmountFact AS Amount
	|FROM
	|	TemporaryTableWorks AS TemporaryTableWorks
	|		INNER JOIN Catalog.CounterpartyContracts AS CounterpartyContracts
	|		ON TemporaryTableWorks.Contract = CounterpartyContracts.Ref
	|			AND (CounterpartyContracts.IsServiceContract)
	|WHERE
	|	TemporaryTableWorks.Contract <> VALUE(Catalog.CounterpartyContracts.EmptyRef)";
	
	QueryResult = Query.Execute();
	
	StructureAdditionalProperties.TableForRegisterRecords.Insert("TableServiceContractExecution", QueryResult.Unload());
	
EndProcedure // СформироватьТаблицаВыполнениеДоговораОбслуживания()
...
Function PrintForm(ObjectsArray, PrintObjects)
	...
	While Header.Next() Do
		...
		TemplateArea.Parameters.HeaderText = NStr("en='Time tracking No ';vi='Theo dõi thời gian số ';")
												+ DocumentNumber
												+ NStr("en=' from ';vi='  '")
...
Procedure Print(ObjectsArray, PrintParameters, PrintFormsCollection, PrintObjects, OutputParameters) Export
	...
	If PrintManagement.NeedToPrintTemplate(PrintFormsCollection, "TimeTracking") Then
		...
		PrintManagement.OutputSpreadsheetDocumentToCollection(PrintFormsCollection, "TimeTracking", NStr("en='Time tracking';vi='Theo dõi thời gian';"), PrintForm(ObjectsArray, PrintObjects));
...
Procedure AddPrintCommands(PrintCommands) Export
	...
	PrintCommand.Presentation = NStr("en='Time-keeping';ru='Учет времени';vi='Ghi nhận thời gian'");
