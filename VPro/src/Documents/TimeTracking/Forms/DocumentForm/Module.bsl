...
Procedure SetColumnHeaders()
	...
	Items.OperationsMonDuration.Title = NStr("en='Mo ';ru='Пн ';vi='Thứ hai '") + Format(Object.DateFrom, "DF=dd.MM");
	Items.OperationsTuDuration.Title = NStr("en='Tu ';ru='Вт ';vi='Thứ ba '") + Format(Object.DateFrom + 86400, "DF=dd.MM");
	Items.OperationsAverageDuration.Title = NStr("en='We ';ru='Ср ';vi='Thứ tư '") + Format(Object.DateFrom + 86400*2, "DF=dd.MM");
	Items.OperationsThDuration.Title = NStr("en='Th ';ru='Чт ';vi='Thứ năm '") + Format(Object.DateFrom + 86400*3, "DF=dd.MM");
	Items.OperationsFrDuration.Title = NStr("en='Fr ';ru='Пт ';vi='Thứ sáu '") + Format(Object.DateFrom + 86400*4, "DF=dd.MM");
	Items.OperationsSaDuration.Title = NStr("en='Sa ';ru='Сб ';vi='Thứ bảy '") + Format(Object.DateFrom + 86400*5, "DF=dd.MM");
	Items.OperationsSuDuration.Title = NStr("en='Su ';ru='Вс ';vi='Chủ nhật'") + Format(Object.DateFrom + 86400*6, "DF=dd.MM");
...
Procedure FillByPlan(Command)
	...
	If Not ValueIsFilled(Object.Company) Then
		...
		Message.Text = NStr("en='Company is not populated. Population is canceled.';ru='Не заполнена организация! Заполнение отменено.';vi='Chưa điền doanh nghiệp! Thao tác điền bị hủy bỏ.'");
	...
	If Not ValueIsFilled(Object.StructuralUnit) Then
        Message = New UserMessage();
		Message.Text = NStr("en='Department is not populated. Population is canceled.';ru='Не заполнено подразделение! Заполнение отменено.';vi='Chưa điền bộ phận! Thao tác điền bị hủy bỏ.'");
	...
	If Not ValueIsFilled(Object.Employee) Then
        Message = New UserMessage();
		Message.Text = NStr("en='Employee is not selected. Population is canceled.';ru='Не выбран сотрудник! Заполнение отменено.';vi='Chưa chọn người lao động! Thao tác điền bị hủy bỏ.'");
	...
	If Not ValueIsFilled(Object.DateFrom) Then
        Message = New UserMessage();
		Message.Text = NStr("en='Week start is not selected. Population is canceled.';ru='Не выбрано начало недели! Заполнение отменено.';vi='Chưa chọn đầu tuần! Thao tác điền bị hủy bỏ.'");
	...
	If Not ValueIsFilled(Object.DateTo) Then
        Message = New UserMessage();
		Message.Text = NStr("en='Week end is not selected. Population is canceled.';ru='Не выбрано окончание недели! Заполнение отменено.';vi='Chưa chọn cuối tuần! Thao tác điền bị hủy bỏ.'");
	...
	If Object.Operations.Count() > 0 Then
		...
		ShowQueryBox(New NotifyDescription("FillInByPlanEnd", ThisObject), NStr("en='Tabular section of the document will be cleared. Continue?';ru='Табличная часть документа будет очищена! Продолжить?';vi='Phần bảng chứng từ sẽ bị xóa! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
        Return; 
	EndIf;
