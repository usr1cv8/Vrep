...
Procedure GenerateTableSettlementsWithOtherCounterparties(DocumentRefOtherExpenses, StructureAdditionalProperties)
	...
	Query.SetParameter("CommentReceipt",				NStr("en='Increase in counterparty debt';ru='Увеличение долга контрагента';vi='Tăng nợ của đối tác'", Metadata.DefaultLanguage.LanguageCode));
	Query.SetParameter("CommentExpense",				NStr("en='Decrease in counterparty debt';ru='Уменьшение долга контрагента';vi='Giảm nợ của đối tác'", Metadata.DefaultLanguage.LanguageCode));
...
Procedure InitializeDocumentData(DocumentRefOtherExpenses, StructureAdditionalProperties) Export
	...
	Query.SetParameter("RevenueIncomes", NStr("en='Other income';ru='Прочие доходы';vi='Thu nhập khác'"));
	Query.SetParameter("OtherIncome", NStr("en='Other costs (expenses)';ru='Прочих затраты (расходы)';vi='Chi phí khác'"));
