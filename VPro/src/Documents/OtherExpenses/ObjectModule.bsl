		...
		For Each CurrentRowExpenses In Expenses Do
			If CurrentRowExpenses.GLExpenseAccount.TypeOfAccount = Enums.GLAccountsTypes.Debitors 
				...
				If CurrentRowExpenses.Counterparty.IsEmpty() Then
					MessageText = NStr("en='Specify the counterparty in the line %LineNumber% of the list ""Expenses""';ru='Укажите контрагента в строке %LineNumber% списка ""Расходы"".';vi='Hãy chỉ ra đối tác tại dòng %LineNumber% trong danh sách ""Chi phí"".'");
				...
				ElsIf CurrentRowExpenses.Counterparty.DoOperationsByContracts And CurrentRowExpenses.Contract.IsEmpty() Then
					MessageText = NStr("en='Specify the contract in the line %LineNumber% of the list ""Expenses""';ru='Укажите договор в строке %LineNumber% списка ""Расходы"".';vi='Hãy chỉ ra hợp đồng tại dòng %LineNumber% trong danh sách ""Chi phí"".'");
					MessageText = StrReplace(MessageText, "%LineNumber%", CurrentRowExpenses.LineNumber);
					SmallBusinessServer.ShowMessageAboutError(
						ThisObject,
						MessageText,
