...
Procedure FillByTaxAccrual(BasisDocument)
	...
	If BasisDocument.OperationKind <> Enums.OperationKindsTaxAccrual.Accrual Then
		Raise NStr("en='Cash fund expense can be entered only if taxes are accrued, not compensated.';ru='Расход из кассы можно ввести только на основании начисления налогов, а не возмещения.';vi='Chỉ có thể chi tiền mặt trên cơ sở tính thuế, chứ không phải từ bồi thường.'");
	EndIf;
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If OperationKind = Enums.OperationKindsCashPayment.Vendor
	 OR OperationKind = Enums.OperationKindsCashPayment.ToCustomer Then
		...
		If PaymentAmount <> DocumentAmount Then
			MessageText = NStr("en='Document amount: %DocumentAmount% %CashCurrency% does not correspond to the posted payments in the tabular section: %PaymentAmount% %CashCurrency%!';ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!';vi='Số tiền chứng từ: %DocumentAmount% %CashCurrency%, không tương ứng với số tiền của các khoản thanh toán đã liệt kê trong phần bảng: %PaymentAmount% %CashCurrency%!'");
	...
	ElsIf OperationKind = Enums.OperationKindsCashPayment.IssueLoanToEmployee Then
	...
	ElsIf OperationKind = Enums.OperationKindsCashPayment.LoanSettlements Then
	...
	If GetFunctionalOption("UseBilling")
		...
		And (OperationKind = Enums.OperationKindsCashPayment.Other) Then
...
Procedure FillByLoanContract(DocRefLoanContract) Export
	...
	|			THEN VALUE(Enum.OperationKindsCashPayment.LoanSettlements)
	|		ELSE VALUE(Enum.OperationKindsCashPayment.IssueLoanToEmployee)
	|	END AS OperationKind,
	|	&Ref AS BasisDocument,
	|	DocumentTable.Company AS Company,
	...
	VATTaxation = Enums.VATTaxationTypes.NotTaxableByVAT;
...
Procedure FillByAccrualsForLoans(FillingData) Export
	...
	"SELECT
	|	REFPRESENTATION(&Ref) AS Basis,
	|	VALUE(Enum.OperationKindsCashPayment.LoanSettlements) AS OperationKind,
	|	&Ref AS BasisDocument,
	|	DocumentTable.Ref.Company AS Company,
	|	DocumentTable.SettlementsCurrency AS CashCurrency,
