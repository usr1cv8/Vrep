...
Procedure InitializeDocumentDataCashAssetsForecast(DocumentRefBudget, StructureAdditionalProperties)
	...
	Query.SetParameter("IncomeContent", NStr("en='Forecast of funds receipt';ru='Прогноз поступления денежных средств';vi='Dự tính thu tiền'"));
	Query.SetParameter("ExpenceContent", NStr("en='Forecast of funds outflow';ru='Прогноз выбытия денежных средств';vi='Dự tính chi tiền'"));
...
Procedure InitializeDocumentDataIncomeAndExpensesForecast(DocumentRefBudget, StructureAdditionalProperties)
	...
	Query.SetParameter("Ref", DocumentRefBudget);
	Query.SetParameter("Company", StructureAdditionalProperties.ForPosting.Company);
	Query.SetParameter("IncomeContent", NStr("en='Income forecast';ru='Прогноз доходов';vi='Dự tính thu nhập'"));
	Query.SetParameter("ExpenceContent", NStr("en='Expense forecast';ru='Прогноз расходов';vi='Dự tính chi phí'"));
...
Procedure InitializeDocumentDataBalances(DocumentRefBudget, StructureAdditionalProperties)
	...
	Query.SetParameter("Company", StructureAdditionalProperties.ForPosting.Company);
	Query.SetParameter("AccountingCurrency", Constants.AccountingCurrency.Get());
	Query.SetParameter("Content", NStr("en='Opening balance forecast';ru='Прогноз начальных остатков';vi='Dự tính số dư đầu'"));
	...
	CreditAccountTypes.Add(Enums.GLAccountsTypes.Loans);
						...
						If BypassByGLAccounts.AmountIncome <> 0 
						 OR BypassByGLAccounts.AmountExpenses <> 0 Then
							...
							If SumWasDistributed = 0 Then
								...
								MessageText = NStr("en='Financial result calculation: The ""%FinancialAccount%""  GL account has no distribution base.';ru='Расчет финансового результата: Счет учета ""%СчетУчета%"", не имеет базы распределения!';vi='Tính kết quả kinh doanh: Tài khoản kế toán ""%FinancialAccount%"", không có cơ sở phân bổ!'");
