...
//
Procedure FillPaymentDetails(SuplierInvoice = Undefined) Export
	...
	|						ELSE TRUE
	|					END) AS AccountsPayableBalances
	|	
	|	UNION ALL
	|	
	|	SELECT
	...
	|		AND CASE
	|				WHEN &DocumentFilter
	|					THEN DocumentRegisterRecordsVendorSettlements.Document = &Document
	|				ELSE TRUE
	|			END) AS AccountsPayableBalances
	...
	Query.SetParameter("Document", SuplierInvoice);
	Query.SetParameter("DocumentFilter", ValueIsFilled(SuplierInvoice));
...
Procedure FillByCashOutflowPlan(BasisDocument, Amount = Undefined)
	...
	If BasisDocument.PaymentConfirmationStatus = Enums.PaymentApprovalStatuses.NotApproved Then
		Raise NStr("en='You cannot enter money outflow based on the unapproved request.';ru='Нельзя ввести расход денег на основании неутвержденной заявки!';vi='Không thể nhập chi phí tiền mặt trên cở sở giấy đề nghị không được phê duyệt!'");
	EndIf;
	If BasisDocument.CashAssetsType = Enums.CashAssetTypes.Cash Then
		Raise NStr("en='You cannot enter cash outflow. Invalid payment method (funds type) is specified in the request.';ru='Нельзя ввести расход денег. В заявке указан не верный способ оплаты (тип денежных средств)!';vi='Không thể nhập chi phí tiền mặt. Đã chỉ ra sai phương pháp thanh toán (kiểu vốn bằng tiền) trong giấy đề nghị!'");
	EndIf;
	...
	If Not QueryResult.IsEmpty() Then
		...
		BasisDoc = CommonUse.ObjectAttributeValue(BasisDocument, "BasisDocument");
		
		FillPaymentDetails(BasisDoc);
...
Procedure FillByTaxAccrual(BasisDocument)
	...
	If BasisDocument.OperationKind <> Enums.OperationKindsTaxAccrual.Accrual Then
		Raise NStr("en='You can enter expense from account only if taxes are accrued, not compensated.';ru='Расход со счета можно ввести только на основании начисления налогов, а не возмещения.';vi='Chỉ có thể chi từ tài khoản trên cơ sở tính thuế, chứ không phải từ bồi thường.'");
	EndIf;
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If OperationKind = Enums.OperationKindsPaymentExpense.Vendor
	 OR OperationKind = Enums.OperationKindsPaymentExpense.ToCustomer Then
		...
		SmallBusinessServer.DeleteAttributeBeingChecked(CheckedAttributes, "LoanContract");
		SmallBusinessServer.DeleteAttributeBeingChecked(CheckedAttributes, "PaymentDetails.TypeOfAmount");
				   ...
				   AND Not RowPaymentDetails.AdvanceFlag)) Then
					If PaymentDetails.Count() = 1 Then
						If OperationKind = Enums.OperationKindsPaymentExpense.Vendor Then
							MessageText = NStr("en='Specify the shipment document or payment advance.';ru='Укажите документ отгрузки или признак аванса платежа.';vi='Hãy chỉ ra chứng từ giao hàng hoặc dấu hiệu thanh toán ứng trước.'");
						Else
							MessageText = NStr("en='Specify document of settlements.';ru='Укажите документ расчетов.';vi='Hãy chỉ ra chứng từ hạch toán'");
						EndIf;
					Else
						If OperationKind = Enums.OperationKindsPaymentExpense.Vendor Then
							MessageText = NStr("en='Specify the shipment document or payment flag in the %LineNumber% line of the ""Payment explanation"" list.';ru='Укажите документ отгрузки или признак оплаты в строке %НомерСтроки% списка ""Расшифровка платежа"".';vi='Hãy chỉ ra chứng từ giao hàng hoặc dấu hiệu thanh toán trong dòng %LineNumber% danh sách ""Diễn giải thanh toán"".'");
						Else
							MessageText = NStr("en='Specify document of settlements in the row %LineNumber% of the list ""Payment details"".';ru='Укажите документ расчетов в строке %НомерСтроки% списка ""Расшифровка платежа"".';vi='Hãy chỉ ra chứng từ hạch toán trong dòng %LineNumber% danh sách ""Diễn giải thanh toán"".'");
						EndIf;
		...
		If PaymentAmount <> DocumentAmount Then
			MessageText = NStr("en='Document amount: %DocumentAmount% %CashAssetsCurrency% does not correspond to the posted payments in the tabular section: %PaymentAmount% %CashAssetsCurrency%!';ru='Сумма документа: %СуммаДокумента% %ВалютаДенежныхСредств%, не соответствует сумме разнесенных платежей в табличной части: %СуммаПлатежа% %ВалютаДенежныхСредств%!';vi='Số tiền chứng từ: %DocumentAmount% %CashAssetsCurrency%, không tương ứng với số tiền của các khoản thanh toán đã liệt kê trong phần bảng: %PaymentAmount% %CashAssetsCurrency%!'");
	...
	ElsIf OperationKind = Enums.OperationKindsPaymentExpense.OtherSettlements Then
		...
		PaymentAmount = PaymentDetails.Total("PaymentAmount");
		If PaymentAmount <> DocumentAmount Then
			MessageText = NStr("en='Document amount: %DocumentAmount% %CashCurrency% does not match with the posted payments in the tabular section:  %PaymentAmount% %CashCurrency%!';ru='Сумма документа: %DocumentAmount% %CashCurrency%, не соответствует сумме разнесенных платежей в табличной части: %PaymentAmount% %CashCurrency%!';vi='Số tiền chứng từ: %DocumentAmount% %CashCurrency%, không tương ứng với số tiền thanh toán tách riêng trong phần bảng: %PaymentAmount% %CashCurrency%!'");
			MessageText = StrReplace(MessageText, "%DocumentAmount%", String(DocumentAmount));
			MessageText = StrReplace(MessageText, "%PaymentAmount%", String(PaymentAmount));
	...
	ElsIf OperationKind = Enums.OperationKindsPaymentExpense.IssueLoanToEmployee Then
		...
		If AdvanceHolder.IsEmpty() Then
			MessageText = НСтр("en='The ""Employee"" field is required';ru='Поле ""Сотрудник"" не заполнено';vi='Chưa điền trường ""Nhân viên""'");
			SmallBusinessServer.ShowMessageAboutError(
				ThisObject,
				MessageText,
				,
	...
	ElsIf OperationKind = Enums.OperationKindsPaymentExpense.LoanSettlements Then
		...
		PaymentAmount = PaymentDetails.Total("PaymentAmount");
		If PaymentAmount <> DocumentAmount Then
			MessageText = NStr("en='Document amount: %DocumentAmount% %CashCurrency% does not match with the posted payments in the tabular section:  %PaymentAmount% %CashCurrency%!';ru='Сумма документа: %DocumentAmount% %CashCurrency%, не соответствует сумме разнесенных платежей в табличной части: %PaymentAmount% %CashCurrency%!';vi='Số tiền chứng từ: %DocumentAmount% %CashCurrency% không tương ứng với số tiền thanh toán đã đưa vào phần bảng: %PaymentAmount% %CashCurrency%!'");
			MessageText = StrReplace(MessageText, "%DocumentAmount%", String(DocumentAmount));
			MessageText = StrReplace(MessageText, "%PaymentAmount%", String(PaymentAmount));
			MessageText = StrReplace(MessageText, "%CashCurrency%", String(Строка(CashCurrency)));
			SmallBusinessServer.ShowMessageAboutError(
	...
	// Billing
	If GetFunctionalOption("UseBilling")
		And Constants.BillingKeepExpensesAccountingByServiceContracts.Get()
		And ValueIsFilled(BusinessActivity)
		And (OperationKind = Enums.OperationKindsPaymentExpense.Other) Then
		...
		|	AND CounterpartyContracts.ServiceContractBusinessActivity = &BusinessActivity";
		Query.SetParameter("BusinessActivity", BusinessActivity);
		SELECTION = Query.Execute().Select();
		
		If SELECTION.Next() Then
			If Not SmallBusinessServer.SoldProductsAndServicesByServiceContractAllowed(SELECTION.Ref, Correspondence) Then
				CommonUseClientServer.MessageToUser(
					NStr("en='Запрещено проводить незапланированные расходы по текущему договору обслуживания!';ru='Запрещено проводить незапланированные расходы по текущему договору обслуживания!';vi='Không được chi các khoản ngoài kế hoạch theo hợp đồng dịch vụ hiện hành!'"),,
					"Correspondence",,
					Cancel
				);
			EndIf;
...
Procedure FillByLoanContract(DocRefLoanContract, Amount = Undefined) Export
	...
	If Amount <> Undefined Then
		...
		|			THEN VALUE(Enum.OperationKindsPaymentExpense.LoanSettlements)
		|		ELSE VALUE(Enum.OperationKindsPaymentExpense.IssueLoanToEmployee)
		|	END AS OperationKind,
		|	&Ref AS BasisDocument,
		|	DocumentTable.Company AS Company,
		...
		|	&Amount AS PaymentAmount,
		|	DocumentTable.Company.DefaultVATRate AS VATRate,
		|	ISNULL(CurrencyRates.ExchangeRate, 1) AS ExchangeRate,
		|	ISNULL(CurrencyRates.Multiplicity, 1) AS Multiplicity,
		|	CAST(&Amount AS NUMBER(15, 2)) AS SettlementsAmount,
		|	CAST(&Amount * (1 - 1 / ((ISNULL(DocumentTable.Company.DefaultVATRate.Rate, 0) + 100) / 100)) AS NUMBER(15, 2)) AS VATAmount
		|FROM
		|	Document.LoanContract AS DocumentTable
		|		LEFT JOIN InformationRegister.CurrencyRates.SliceLast(&Date, ) AS CurrencyRates
		|		ON DocumentTable.SettlementsCurrency = CurrencyRates.Currency
		|WHERE
		|	DocumentTable.Ref = &Ref";
	...
	Else
		...
		|	CAST(DocumentTable.Total * (1 - 1 / ((ISNULL(DocumentTable.Company.DefaultVATRate.Rate, 0) + 100) / 100)) AS NUMBER(15, 2)) AS VATAmount
		|FROM
		|	Document.LoanContract AS DocumentTable
		|		LEFT JOIN InformationRegister.CurrencyRates.SliceLast(&Date, ) AS CurrencyRates
		|		ON DocumentTable.SettlementsCurrency = CurrencyRates.Currency
	...
	If NOT QueryResult.IsEmpty() Then
		...
		PettyCash = Catalogs.PettyCashes.GetPettyCashByDefault(Company);
...
Procedure FillByAccrualsForLoans(FillingData) Export
	...
	"SELECT
	|	REFPRESENTATION(&Ref) AS Basis,
	|	VALUE(Enum.OperationKindsPaymentExpense.LoanSettlements) AS OperationKind,
	|	&Ref AS BasisDocument,
	|	DocumentTable.Ref.Company AS Company,
	|	DocumentTable.SettlementsCurrency AS CashCurrency,
	...
	|	DocumentTable.Total AS PaymentAmount,
	|	DocumentTable.Ref.Company.DefaultVATRate AS VATRate,
	|	ISNULL(CurrencyRates.ExchangeRate, 1) AS ExchangeRate,
	|	ISNULL(CurrencyRates.Multiplicity, 1) AS Multiplicity,
	|	CAST(DocumentTable.Total AS NUMBER(15, 2)) AS SettlementsAmount,
	|	CAST(DocumentTable.Total * (1 - 1 / ((ISNULL(DocumentTable.Ref.Company.DefaultVATRate.Rate, 0) + 100) / 100)) AS NUMBER(15, 2)) AS VATAmount,
