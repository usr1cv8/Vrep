...
Procedure SetFormFrom2014Year()
	...
	ElsIf Object.OperationKind = Enums.OperationKindsPaymentOrder.Payment Then
		...
		Items.PaymentIdentifier.WarningOnEdit = NStr("en='Payment ID is used only for payments to the budget until 03/31/2014';ru='До 31.03.2014 идентификатор платежа используется только для платежей в бюджет';vi='Trước ngày 31.03.2014, mã thanh toán được sử dụng chỉ để nộp tiền vào ngân sách'");
	EndIf;
...
Procedure FillByDocumentAndSetEnabled()
	...
	ShowQueryBox(New NotifyDescription("FillInByDocumentAndSetAvailableEnd", ThisObject), NStr("en='The document will be cleared and filled out under the ""Basis"". Continue?';ru='Документ будет очищен и заполнен по ""Основанию""! Продолжить?';vi='Chứng từ sẽ bị xóa và điền theo ""Cơ sở""! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
...
Procedure GeneratePaymentDestination(UpdateAmount = False)
	...
	Else
		If IsBlankString(PaymentDestination)
			...
			PaymentDestination = NStr("en='Payment against invoice No. %AccountNumber%';ru='Оплата по счету N %AccountNumber%';vi='Thanh toán theo tài khoản số %AccountNumber%'");
			...
			If ValueIsFilled(IncomingDocumentDate) Then
				PaymentDestination = PaymentDestination + NStr("en=' dated ';vi='  '") + TrimAll(String(Format(IncomingDocumentDate, "DF=dd MMMM yyyy'"))) + " g.";
			EndIf;
		EndIf;
		TextDestination = PaymentDestination;
	EndIf;
	...
	AND Not WithoutTaxVAT Then
		TextVAT = NStr("en='VAT(%VATRate%) %VATAmount%';ru='НДС(%VATRate%) %VATAmount%';vi='Thuế GTGT(%VATRate%) %VATAmount%'");
	...
	AND Not ValueIsFilled(Object.VATRate) Then
		TextVAT = NStr("en='VAT %VATAmount%';ru='НДС %СуммаНДС%';vi='Thuế GTGT %VATAmount%'");
		TextVAT = StrReplace(TextVAT, "%VATAmount%", String(Format(Object.VATAmount, "ND=15; NFD=2; NDS=-; NZ=0-00; NG=")));
	EndIf;
	
	TextPaymentDestination = NStr("en='%TextDestination% Amount %TextAmount% %VATRateValue% %TextVAT%';ru='%TextDestination% Сумма %TextAmount% %VATRateValue% %TextVAT%';vi='%TextDestination% Số tiền %TextAmount% %VATRateValue% %TextVAT%'"
	);
	...
	TextPaymentDestination = StrReplace(TextPaymentDestination, "%VATRateValue%", ?(WithoutTaxVAT OR (NOT ValueIsFilled(Object.VATAmount) AND Not ValueIsFilled(Object.VATRate)), NStr("en='Without tax (VAT)';ru='Без налога (НДС)';vi='Không thuế (GTGT)'"), NStr("en='including';ru='В т.ч.';vi='Bao gồm'")));
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	LegalEntityIndividual = CommonUse.ObjectAttributeValue(Object.Company, "LegalEntityIndividual");
	ThisIsInd = (LegalEntityIndividual = Enums.CounterpartyKinds.Individual);
