...
Procedure FillPaymentDestination()
	...
	If IsBlankString(PaymentText)
		...
		PaymentText = NStr("en='Payment against invoice No. %AccountNumber%';ru='Оплата по счету N %AccountNumber%';vi='Thanh toán theo tài khoản số %AccountNumber%'");
		...
		If ValueIsFilled(BasisDocument.IncomingDocumentDate) Then
			PaymentText = PaymentText + NStr("en=' dated ';vi='  '") + TrimAll(String(Format(BasisDocument.IncomingDocumentDate, "DF=dd MMMM yyyy'"))) + " g.";
		EndIf;
	EndIf;
	...
	TextPaymentDestination = StrReplace(TextPaymentDestination, "%VATRateValue%", ?(WithoutTaxVAT, NStr("en='Without tax (VAT)';ru='Без налога (НДС)';vi='Không thuế (GTGT)'"), NStr("en='including';ru='В т.ч.';vi='Bao gồm'")));
...
Procedure FillByCashOutflowPlan(BasisDocument);
	...
	If BasisDocument.PaymentConfirmationStatus = Enums.PaymentApprovalStatuses.NotApproved Then
		Raise NStr("en='You cannot enter a payment order based on the unapproved request.';ru='Нельзя ввести платежное поручение на основании не утвержденной заявки!';vi='Không thể nhập ủy nhiệm chi trên cơ sở giấy đề nghị chưa được phê duyệt!'");
	EndIf;
	If BasisDocument.CashAssetsType = Enums.CashAssetTypes.Cash Then
		Raise NStr("en='Cannot enter a payment order. Invalid payment method is specified in the request (funds type).';ru='Нельзя ввести платежное поручение. В заявке указан не верный способ оплаты (тип денежных средств)!';vi='Không thể nhập ủy nhiệm chi. Trong giấy đề nghị đã chỉ ra sai phương pháp thanh toán (kiểu vốn bằng tiền)!'");
	EndIf;
