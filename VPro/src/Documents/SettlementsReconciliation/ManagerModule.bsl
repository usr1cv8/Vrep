...
Function SetStatus(DocumentArray, StatusValue) Export
	...
	While Selection.Next() Do
		...
		Except
			...
			ErrorText = NStr("en='Failed to lock %Document%. %ErrorDescription%';ru='Не удалось заблокировать %Документ%. %ОписаниеОшибки%';vi='Không thể phong tỏa %Document%. %ErrorDescription%'");
		...
		Except
			...
			ErrorText = NStr("en='Unable to write %Document%. %ErrorDescription%';ru='Unable to write %Document%. %ErrorDescription%';vi='Không thể ghi %Document%. %ErrorDescription%'");
			ErrorText = StrReplace(ErrorText, "%Document%",       Selection.Ref);
			ErrorText = StrReplace(ErrorText, "%ErrorDescription%", BriefErrorDescription(ErrorInfo()));
			Raise ErrorText;
...
Function GetDocumentPresentationsForCounterparties()
	...
	DocumentKindsCounterparty.Insert("ExpenseReport",					NStr("en='Credit slip';ru='Приходный кассовый ордер';vi='Phiếu thu'"));
	DocumentKindsCounterparty.Insert("AcceptanceCertificate",				NStr("en='Receipt (goods, services)';ru='Поступление (товаров, услуг)';vi='Tiếp nhận (hàng hóa, dịch vụ)'"));
	DocumentKindsCounterparty.Insert("Netting",						NStr("en='Debt setoff';ru='Взаимозачет задолженности';vi='Đối trừ công nợ'"));
	DocumentKindsCounterparty.Insert("CustomerOrder",					NStr("en='Order';ru='Заказ';vi='Đơn hàng'"));
	DocumentKindsCounterparty.Insert("PurchaseOrder", 					NStr("en='Order';ru='Заказ';vi='Đơn hàng'"));
	DocumentKindsCounterparty.Insert("RegistersCorrection",			NStr("en='Debt adjustment';ru='Корректировка долга';vi='Điều chỉnh công nợ'"));
	DocumentKindsCounterparty.Insert("AgentReport",					NStr("en='Report to principal';ru='Отчет комитенту';vi='Bảng kê hàng bán dành cho người đặt ký gửi'"));
	DocumentKindsCounterparty.Insert("ReportToPrincipal",					NStr("en='Agent report';ru='Отчет комиссионера';vi='Bảng kê hàng bán của người nhận bán hộ'"));
	DocumentKindsCounterparty.Insert("PaymentReceipt", 				NStr("en='Expense from account';ru='Расход со счета';vi='Chi từ tài khoản'"));
	DocumentKindsCounterparty.Insert("CashReceipt",					NStr("en='Debit slip';ru='Расходный кассовый ордер';vi='Phiếu chi'"));
	DocumentKindsCounterparty.Insert("PaymentExpense",						NStr("en='Receipts to account';ru='Поступления на счет';vi='Thu vào tài khoản'"));
	DocumentKindsCounterparty.Insert("CashPayment",						NStr("en='Credit slip';ru='Приходный кассовый ордер';vi='Phiếu thu'"));
	DocumentKindsCounterparty.Insert("SupplierInvoice",				NStr("en='Sale (goods, services)';ru='Реализация (товаров, услуг)';vi='Giao (hàng, dịch vụ)'"));
	DocumentKindsCounterparty.Insert("CustomerInvoice", 				NStr("en='Receipt (goods, services)';ru='Поступление (товаров, услуг)';vi='Tiếp nhận (hàng hóa, dịch vụ)'"));
	DocumentKindsCounterparty.Insert("InvoiceForPayment", 						NStr("en='Proforma invoice';ru='Счет на оплату';vi='Hóa đơn thanh toán'"));
...
Function CompanyAccountingDocumentDescription(DocumentRef, DocumentNumber = "" , Val DocumentDate, CompanyToPresentation = False) Export
	...
	If DocumentRef = Undefined Then
		...
		DescriptionString =  NStr("en='Settlement document No. %1 dated %2';ru='Расчетный документ № %1 от %2 г.';vi='Chứng từ thanh toán số %1 từ năm %2 .'");
		DescriptionString = StringFunctionsClientServer.SubstituteParametersInString(DescriptionString, 
			?(IsBlankString(DocumentNumber), NStr("en='_______';ru='_______';vi='_______'"), ObjectPrefixationClientServer.GetNumberForPrinting(DocumentNumber, False, True)), 
			?(ValueIsFilled(DocumentDate), Format(DocumentDate, "DLF=D"), NStr("en='___.___.________';ru='___.___.________';vi='___.___.________'"))
			);
	...
	If TypeOf(DocumentRef) = Type("DocumentRef.CustomerOrder") 
		...
		DocumentDescription = NStr("en='Operating procedure';ru='Порядок работы';vi='Trình tự làm việc'");
	...
	If CompanyToPresentation Then
		...
		SetCompanyPresentation = NStr("en=' (';ru=' (';vi=' ('") + DocumentRef.Company.DescriptionFull + NStr("en=')';ru=')';vi=')'");
		
	EndIf;
	
	Return DocumentDescription + AddToDescriptionFull 
		+ NStr("en=' No ';ru=' No ';vi=' Số '") + DocumentNumber 
		+ NStr("en=' from ';ru=' от ';vi=' ngày '") + DocumentDate + NStr("en=' y.';ru=' г.';vi=' năm'") + SetCompanyPresentation;
...
Function CounterpartyAccountingDocumentDescription(DocumentRef, Val DocumentNumber, Val DocumentDate, CompanyToPresentation = False) Export
	...
	DocumentNumber	= ?(IsBlankString(DocumentNumber), NStr("en='_______';ru='_______';vi='_______'"), ObjectPrefixationClientServer.GetNumberForPrinting(DocumentNumber, False, True));
	DocumentDate	= ?(ValueIsFilled(DocumentDate), Format(DocumentDate, "DLF=D"), NStr("en='___.___._______';ru='___.___._______';vi='___.___._______'"));
	...
	If TypeOf(DocumentRef) = Type("DocumentRef.CustomerOrder") 
		...
		DocumentDescription = NStr("en='Receipt (goods, services)';ru='Поступление (товаров, услуг)';vi='Tiếp nhận (hàng hóa, dịch vụ)'");
	...
	If IsBlankString(DocumentDescription) Then
		...
		DocumentDescription = NStr("en='Payment document';ru='Расчетный документ';vi='Chứng từ thanh toán'");
...
Procedure ReplaceEmptyParametersWithUnderscores(TemplateParameters)
	...
	Underline = NStr("en='__________________';ru='__________________';vi='__________________'");
...
Function GeneratePrintFormCertificateWithoutDifferences(DocumentPrint, PrintObjects, OutputFacsimile = False)
	...
	While DocumentsSelection.Next() Do
		...
		DrawingUpDatePresentation = NStr("en='Document is generated %1';ru='Документ сформирован %1';vi='Đã lập chứng từ %1'");
		...
		TemplateParameters.Insert("CompanyPresentation", NStr("en='1. ';ru='1. ';vi='1. '") + SmallBusinessServer.CompaniesDescriptionFull(InfoAboutCompany, "FullDescr, TIN, LegalAddress, PhoneNumbers, Fax"));
		...
		TemplateParameters.Insert("PresentationOfCounterparty", NStr("en='2. ';ru='2. ';vi='2. '") + SmallBusinessServer.CompaniesDescriptionFull(InfoAboutCounterparty, "FullDescr, TIN, LegalAddress, PhoneNumbers, Fax"));
		...
		If ValueIsFilled(Header.BeginOfPeriod) Then
			...
			PeriodPresentation = NStr("en='from %1 to %2';ru='в период с %1 по %2';vi='trong kỳ từ %1 đến %2'");
		...
		Else
			...
			PeriodPresentation = NStr("en='as of %1';ru='по состоянию на %1';vi='theo trạng thái trên %1'");
		...
		For RowCounter = 0 To MaxIndex Do
			...
			If Header.SortByContracts Then
				...
				If DataRow.Contract <> Contract Then
					...
					TemplateParameters.Insert("PresentationTreaty", NStr("en='Under contract: ';ru='По договору: ';vi='Theo hợp đồng:'") + Contract);
...
Procedure AddPrintCommands(PrintCommands) Export
	...
	PrintCommand.Presentation = NStr("en='Mutual settlements reconciliation statement (without variances)';ru='Акт сверки взаиморасчетов (без расхождений)';vi='Biên bản đối chiếu công nợ (không có chênh lệch)'");
	PrintCommand.FormsList = "DocumentForm,ListForm,DocumentsListForm";
	PrintCommand.CheckPostingBeforePrint = False;
	PrintCommand.Order = 4;
	
	//
	// Definition Commands printing NStr("en='Mutual settlements reconciliation statement (without variances with facsimile)';ru='Акт сверки взаиморасчетов (без расхождений с факсимиле)';vi='Biên bản đối chiếu công nợ (không có chênh lệch với bản sao chụp)'");
	//
	
	PrintCommand = PrintCommands.Add();
	PrintCommand.ID = "CertificateWithCounterpartyData";
	PrintCommand.Presentation = NStr("en='Mutual settlements reconciliation statement (with counterparty data)';ru='Акт сверки взаимных расчетов (с данными контрагента)';vi='Biên bản đối chiếu hạch toán (với dữ liệu của đối tác)'");
	PrintCommand.FormsList = "DocumentForm,ListForm";
	PrintCommand.CheckPostingBeforePrint = False;
	PrintCommand.Order = 10;
	
	//
	// Definition Commands printing NStr("en='Mutual settlements reconciliation statement (with counterparty data and facsimile)';ru='Акт сверки взаиморасчетов (с данными контрагента и с факсимиле)';vi='Biên bản đối chiếu công nợ (với dữ liệu đối tác và với bản sao chụp)'");
	//
