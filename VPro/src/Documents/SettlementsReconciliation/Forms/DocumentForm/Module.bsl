...
Procedure FillTitleBySelectedCatalogs()
	...
	If Object.CounterpartyContracts.Count() = 0 Then
		...
		LabelText = NStr("en='Contract list is empty';ru='Список договоров пуст';vi='Danh sách hợp đồng trống.'");
	...
	ElsIf ArrayMarked.Count() = 0 Then
		...
		LabelText = NStr("en='Contracts are not selected';ru='Договоры не выбраны';vi='Chưa chọn các chứng từ'");
...
Procedure GetCounterpartyData(CounterpartyData)
	...
	If FillData Then
		...
		Object.DescriptionContractsSelection = NStr("en='All counterparty contracts are selected';ru='Выбраны все договоры контрагента';vi='Đã chọn tất cả hợp đồng đối tác'");
...
"Для привязки необходимо включить видимость соответствующей колонки и указать документ самостоятельно.';vi='Dòng không được gắn vào chứng từ hạch toán."
"Để gắn cần bật phần hiện hình của cột tương ứng và chỉ ra chứng từ một cách độc lập.'");
...
Procedure StatusChoiceProcessing(Item, ValueSelected, StandardProcessing)
	...
	If ValueSelected = PredefinedValue("Enum.SettlementsReconciliationStatuses.Verified") Then
		...
		If Not ValueIsFilled(Object.Responsible) Then
			...
			MessageText = NStr("en='Company responsible person is filled in incorrectly.';ru='Неверно заполнено ответственное лицо организации.';vi='Điền sai người chịu trách nhiệm của doanh nghiệp.'");
		...
		If Not ValueIsFilled(Object.CounterpartyRepresentative) Then
			...
			MessageText = NStr("en='Counterparty representative is filled in incorrectly.';ru='Неверно заполнен представитель контрагента.';vi='Điền sai đại diện đối tác.'");
...
Procedure CounterpartyOnChange(Item)
	...
	Else
		...
		Object.CounterpartyContracts.Clear();
		Object.DescriptionContractsSelection = NStr("en='Contract list is empty';ru='Список договоров пуст';vi='Danh sách hợp đồng trống.'");
...
Procedure FillByAccountingDocumentsDates(Command)
	...
	If Object.CounterpartyData.Count() < 1 Then
		...
		MessageText = NStr("en='Tabular section of mutual settlements by counterparty data is empty.';ru='Табличная часть взаиморасчетов по данным контрагента пуста.';vi='Phần bảng hạch toán theo dữ liệu của đối tác rỗng.'");
		CommonUseClientServer.MessageToUser(MessageText);
		Return;
...
Procedure FillAccordingToAccounting(Command)
	...
	If Not ValueIsFilled(Object.EndOfPeriod) Then
		...
		MessageText	= NStr("en='The period end date is filled in incorrectly.';ru='Неверно заполнена дата окончания периода.';vi='Điền sai ngày kết thúc kỳ.'");
	...
	If CallProcedureToFillTableParts Then
		...
		If Object.CompanyData.Count() > 0 Then
			...
			QuestionText	= NStr("en='Tabular section will be cleared and filled in again. Continue?';ru='Табличная часть будит очищена и заполнена повторно. Продолжить?';vi='Phần bảng sẽ bị xóa và điền lại. Tiếp tục?'");
...
Procedure TransferFromCompanyData(Command)
	...
	If Object.CompanyData.Count() < 1 Then
		...
		MessageText = NStr("en='Tabular section with company data is not filled in.';ru='Не заполнена табличная часть с данными организации.';vi='Chưa điền phần bảng có dữ liệu của doanh nghiệp.'");
		CommonUseClientServer.MessageToUser(MessageText);
		Return;
