...
Procedure GenerateTableRetailAmountAccounting(DocumentRefRetailRevaluation, StructureAdditionalProperties)
	...
	Query.SetParameter("RetailRevaluation", NStr("en='Revaluation in retail';ru='Переоценка в рознице';vi='Đánh giá lại trong bán lẻ'"));
...
Procedure GenerateTableManagerial(DocumentRefRetailRevaluation, StructureAdditionalProperties)
	...
	Query.SetParameter("TradeMarkup", NStr("en='Markup';ru='Торговая наценка';vi='Phụ thu thương mại'"));
	Query.SetParameter("ExchangeDifference", NStr("en='Exchange rate difference';ru='Курсовая разница';vi='Chênh lệch tỷ giá'"));
	Query.SetParameter("IncomeReflection", NStr("en='Record income';ru='Отражение доходов';vi='Phản ánh thu nhập'"));
	Query.SetParameter("CostsReflection", NStr("en='Record expenses';ru='Отражение расходов';vi='Phản ánh chi phí'"));
