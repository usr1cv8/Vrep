...
Function GenerateErrorDescriptionCostAllocation(GLAccount, MethodOfDistribution, FilterByOrder, Amount)
	...
	ErrorDescription = NStr("en='The ""%GLAccount%"" cost in the %Amount% amount allocated for production release by %AllocationMethod% can not be allocated as in the calculated period there was no %AdditionalDetails%.';ru='Затрата ""%GLAccount%"" в сумме %Amount%, распределяемая на выпуск продукции по %AllocationMethod% не может быть распределена, т.к. в рассчитываемом периоде не было %AdditionalDetails%.';vi='Chi phí ""%GLAccount%"" trong số tiền %Amount%, dùng để phân bổ cho xuất xưởng sản phẩm theo %AllocationMethod% không thể phân bổ được, bởi vì trong kỳ tính không có %AdditionalDetails%.'"
	);
	...
	ErrorDescription = StrReplace(
		...
		?(MethodOfDistribution = Enums.CostingBases.ProductionVolume,
			NStr("en='release volume';ru='объему выпуска';vi='cho khối lượng xuất xưởng'"),
			NStr("en='direct costs';ru='прямым затратам';vi='đối với chi phí trực tiếp'")
		)
	);
	...
	ErrorDescription = StrReplace(
		...
		"%AdditionalDetails%",
		?(MethodOfDistribution = Enums.CostingBases.ProductionVolume,
			NStr("en='production release%Order%';ru='выпусков продукции%Order%';vi='xuất xưởng thành phẩm%Order%'"),
			NStr("en='allocation of direct costs%Order% specified in the allocation setting';ru='распределения прямых затрат%Order%, указанных в настройке распределения';vi='phân bổ chi phí trực tiếp%Order% mà được chỉ ra trong tùy chỉnh phân bổ'")
		)
	);
	...
	ErrorDescription = StrReplace(
		...
		?(ValueIsFilled(FilterByOrder),
			NStr("en=' to ';ru=' по ';vi=' đến '") + String(FilterByOrder),
			""
		)
	);
...
Function GenerateErrorDescriptionExpensesDistribution(GLAccount, MethodOfDistribution, Amount)
	...
	If MethodOfDistribution = Enums.CostingBases.SalesVolume Then
		TextMethodOfDistribution = NStr("en='sales volume';ru='объему продаж';vi='đối với khối lượng bán hàng'");
	ElsIf MethodOfDistribution = Enums.CostingBases.SalesRevenue Then
		TextMethodOfDistribution = NStr("en='sales revenue';ru='выручке от продаж';vi='đối với doanh thu bán hàng'");
	ElsIf MethodOfDistribution = Enums.CostingBases.CostOfGoodsSold Then
		TextMethodOfDistribution = NStr("en='sales primecost';ru='себестоимости продаж';vi='giá vốn hàng bán'");
	ElsIf MethodOfDistribution = Enums.CostingBases.GrossProfit Then
		TextMethodOfDistribution = NStr("en='gross profit';ru='валовой прибыли';vi='lãi gộp'");
	EndIf;
	...
	If MethodOfDistribution = Enums.CostingBases.SalesVolume Then
		TextAdditionalDetails = NStr("en='sales';ru='продаж';vi='bán hàng'");
	ElsIf MethodOfDistribution = Enums.CostingBases.SalesRevenue Then
		TextAdditionalDetails = NStr("en='sales revenue';ru='выручки от продаж';vi='của doanh thu bán hàng'");
	ElsIf MethodOfDistribution = Enums.CostingBases.CostOfGoodsSold Then
		TextAdditionalDetails = NStr("en='sales primecost';ru='себестоимости продаж';vi='giá vốn hàng bán'");
	ElsIf MethodOfDistribution = Enums.CostingBases.GrossProfit Then
		TextAdditionalDetails = NStr("en='gross profit';ru='валовой прибыли';vi='lãi gộp'");
	EndIf;
...
Function MakeRegisterRecordsByRegisterWriteOffCostsCorrectionNodes(Cancel)
	...
	|	CostAccountingForCalculations.ProductionOrder AS ProductionOrder,
	|	CASE
	...
	|		CostAccounting.ProductionOrder AS ProductionOrder
	|	FROM
	...
	|			CostAccounting.ProductionOrder,
	|			CASE
	|				WHEN CostAccounting.RecordType = VALUE(AccumulationRecordType.Receipt)
	|					THEN CostAccounting.Quantity
	...
	|			AccountingCostBalance.ProductionOrder,
	...
	|			AND CostAccountingForCalculations.ProductionOrder = NestedSelect.ProductionOrder
	|WHERE
	...
	|	CostAccountingForCalculations.ProductionOrder";
	Query.Execute();
	...
	|	AccountingCostBalance.ProductionOrder AS ProductionOrder,
	|	CASE
	...
	|			AND (BalanceTableBatches.ProductionOrder = CostAccounting.ProductionOrder)
	...
	|	PeriodsOfBatches.ProductionOrder AS ProductionOrder,
	|	SUM(CASE
	|			WHEN CostAccounting.RecordType = VALUE(AccumulationRecordType.Receipt)
	|				THEN CostAccounting.Quantity
	|			ELSE -CostAccounting.Quantity
	...
	|			AND PeriodsOfBatches.ProductionOrder = CostAccounting.ProductionOrder
	...
	|	PeriodsOfBatches.ProductionOrder,
	|	PeriodsOfBatches.Period
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	BalanceTableBatches.ProductionOrder AS ProductionOrder,
	|	CASE
	...
	|			AND BalanceTableBatches.ProductionOrder = ExternalReceiptForPeriod.ProductionOrder
	|			AND (BalanceTableBatches.QuantityBalance <> 0)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	TableOfCurrentBatchesBalances.ProductionOrder AS ProductionOrder,
	...
	|	TableNodsOfCorrectionOfCostWriteOffs.ProductionOrder AS ProductionOrder,
	...
	|			AND TableNodsOfCorrectionOfCostWriteOffs.ProductionOrder = BalanceTableBatches.ProductionOrder
	|
	|UNION ALL
	|
	|SELECT
...
Function SolveLES()
	...
	|	InventoryAndCostAccounting.ProductionOrder AS ProductionOrder,
	...
	|	InventoryAndCostAccounting.ProductionOrder,
	...
	|	CostAccountingReturnsCurPeriod.ProductionOrder AS ProductionOrder,
	...
	|			AND CostAccountingReturnsCurPeriod.CustomerOrder <> Value(Document.CustomerOrder.EmptyRef)
	|			AND CostAccountingReturnsCurPeriod.ProductionOrder = Value(Document.ProductionOrder.EmptyRef)
	...
	|			AND (InventoryAndCostAccounting.ProductionOrder = WriteOffCostsCorrectionNodes.ProductionOrder)
	|Where
	|			InventoryAndCostAccounting.CustomerOrder <> VALUE(Document.CustomerOrder.EmptyRef)
	|			AND InventoryAndCostAccounting.ProductionOrder = VALUE(Document.ProductionOrder.EmptyRef)	
	|GROUP BY
	...
	|	CostAccountingReturnsCurPeriod.ProductionOrder,
	|	WriteOffCostsCorrectionNodes.NodeNo,
	...
	|Where
	|			InventoryAndCostAccounting.CustomerOrder = VALUE(Document.CustomerOrder.EmptyRef)
	|			И InventoryAndCostAccounting.ProductionOrder = VALUE(Document.ProductionOrder.EmptyRef)
	|
	|
	|GROUP BY
	|	CostAccountingReturnsCurPeriod.Company,
	...
	|	CostAccountingReturns.ProductionOrder AS ProductionOrder,
	...
	|		CostAccountingReturns.ProductionOrder,
	...
	|	ReturnsTable.ProductionOrder AS ProductionOrder,
	...
	|	InventoryAndCostAccounting.ProductionCorrOrder AS ProductionOrder,
	|	WriteOffCostsCorrectionNodes.NodeNo AS NodeNo,
	|	SUM(CASE
	...
	|	InventoryAndCostAccounting.ProductionCorrOrder,
	|	WriteOffCostsCorrectionNodes.NodeNo
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|		CostAccountingNetOfRefunds.ProductionOrder AS ProductionOrder,
	...
	While (CurrentVariance > RequiredPrecision * RequiredPrecision) AND (IterationsQuantity < 100) Do
		...
		|			AND WriteOffCostsCorrectionNodes.ProductionOrder = CostAccounting.ProductionOrder
		|WHERE
		|	WriteOffCostsCorrectionNodes.Recorder = &Recorder
		|
		|GROUP BY
...
Procedure GenerateRegisterRecordsByExpensesRegister(RecordSet, RecordSetManagerial, RegisterRecordRow, Amount, FixedCost, ContentOfAccountingRecord = Undefined)
	...
	If ContentOfAccountingRecord = Undefined Then
		If RegisterRecordRow.GLAccountGLAccountType = Enums.GLAccountsTypes.Inventory Then
			ContentOfAccountingRecord = NStr("en='Write off warehouse inventory';ru='Списание запасов со склада';vi='Ghi giảm vật tư từ kho'");
		Else
			If ValueIsFilled(RegisterRecordRow.ProductsAndServices) Then
				ContentOfAccountingRecord = NStr("en='Expense write-off';ru='Списание расходов';vi='Ghi giảm chi phí'");
			Else
				ContentOfAccountingRecord = NStr("en='Expense write-off';ru='Списание расходов';vi='Ghi giảm chi phí'");
			EndIf;
		EndIf;
	...
	If RegisterRecordRow.CorrAccountFinancialAccountType = Enums.GLAccountsTypes.Inventory Then
		ContentOfAccountingRecord = NStr("en='Inventory capitalization to warehouse';ru='Оприходование запасов на склад';vi='Ghi tăng vật tư vào kho'");
	Else
		If ValueIsFilled(RegisterRecordRow.ProductsAndServices) Then
			ContentOfAccountingRecord = NStr("en='Expense receipt';ru='Поступление расходов';vi='Tiếp nhận chi phí'");
		Else
			ContentOfAccountingRecord = NStr("en='Expense receipt';ru='Поступление расходов';vi='Tiếp nhận chi phí'");
		EndIf;
	EndIf;
	...
	NewRow.ProductionOrder = RegisterRecordRow.ProductionCorrOrder;
	...
	NewRow.ProductionCorrOrder = RegisterRecordRow.ProductionOrder;
	NewRow.FixedCost = FixedCost;
	NewRow.Amount = Amount;
	NewRow.ContentOfAccountingRecord = ContentOfAccountingRecord;
...
Procedure GenerateCorrectiveRegisterRecordsByExpensesRegister()
	...
	|	CostAccounting.ProductionCorrOrder AS ProductionCorrOrder,
	|	SUM(CASE
	|			WHEN CostAccounting.RecordType = VALUE(AccumulationRecordType.Expense)
	|				THEN CostAccounting.Quantity
	|			ELSE -CostAccounting.Quantity
	...
	|	CostAccounting.ProductionCorrOrder,
	...
	|	WriteOffCostsCorrectionNodes.ProductionOrder AS ProductionOrder,
	|	WriteOffCostsCorrectionNodes.NodeNo AS NodeNo,
	|	CostAccounting.Specification AS Specification,
	|	CostAccounting.SpecificationCorr AS SpecificationCorr,
			...
			If SelectionDetailRecords.CorrGLAccount = AdditionalProperties.ForPosting.EmptyAccount Then
				...
				If ValueIsFilled(SelectionDetailRecords.SalesDocument)
				  ...
				  OR TypeOf(SelectionDetailRecords.SalesDocument) = Type("DocumentRef.ProcessingReport")) Then
					...
					NewRow.ContentOfAccountingRecord = NStr("en='Record expenses';ru='Отражение расходов';vi='Phản ánh chi phí'");
					...
					NewRow.Content = NStr("en='Record expenses';ru='Отражение расходов';vi='Phản ánh chi phí'");
				...
				ElsIf ValueIsFilled(SelectionDetailRecords.SalesDocument)
					...
					If CorrectionAmount < 0 Then
						...
						NewRow.AmountExpense = CorrectionAmount;
						NewRow.ContentOfAccountingRecord = NStr("en='Other costs (expenses)';ru='Прочих затраты (расходы)';vi='Chi phí khác'");
					Else
						NewRow.GLAccount = ChartsOfAccounts.Managerial.OtherIncome;
						NewRow.AmountIncome = CorrectionAmount;
						NewRow.ContentOfAccountingRecord = NStr("en='Other income';ru='Прочие доходы';vi='Thu nhập khác'");
					EndIf;
					...
					If CorrectionAmount < 0 Then
						...
						NewRow.AccountCr = SelectionDetailRecords.GLAccount;
						NewRow.Amount = - CorrectionAmount;
						NewRow.Content = NStr("en='Other costs (expenses)';ru='Прочих затраты (расходы)';vi='Chi phí khác'");
					Else
						NewRow.AccountDr = SelectionDetailRecords.GLAccount;
						NewRow.AccountCr = ChartsOfAccounts.Managerial.OtherIncome;
						NewRow.Amount = CorrectionAmount;
						NewRow.Content = NStr("en='Other income';ru='Прочие доходы';vi='Thu nhập khác'");
					EndIf;
				...
				ElsIf SelectionDetailRecords.RetailTransferAccrualAccounting Then
					...
					NewRow.ContentOfAccountingRecord = NStr("en='Move to retail';ru='Перемещение в розницу';vi='Điều chuyển trong bán lẻ'");
					NewRow.Cost = CorrectionAmount;
					...
					NewRow.AccountCr = SelectionDetailRecords.GLAccount;
					NewRow.Content = NStr("en='Move to retail';ru='Перемещение в розницу';vi='Điều chuyển trong bán lẻ'");
					NewRow.Amount = CorrectionAmount; 
				...
				Else
					...
					NewRow.AccountDr = SelectionDetailRecords.GLAccountWriteOff;
					NewRow.AccountCr = SelectionDetailRecords.GLAccount;
					NewRow.Content = NStr("en='Inventory write-off to arbitrary account';ru='Списание запасов на произвольный счет';vi='Ghi giảm vật tư vào tài khoản tùy ý'");
					NewRow.Amount = CorrectionAmount;
...
Procedure DistributeAmountsWithoutQuantity(OperationKind, ErrorsTable)
	...
	|	NestedSelect.ProductionCorrOrder AS ProductionCorrOrder,
	...
	|				AND NestedSelect.ProductionCorrOrder = AccountingCostBalanceCorr.ProductionOrder
	|			LEFT JOIN InformationRegister.WriteOffCostsCorrectionNodes AS WriteOffCostsCorrectionNodes
	...
	|				AND NestedSelect.ProductionCorrOrder = WriteOffCostsCorrectionNodes.ProductionOrder
	...
	|			AND AccountingCostBalance.ProductionOrder = NestedSelect.ProductionOrder
	...
	|			AND AccountingCostBalance.ProductionOrder = CostAdjustmentsNodesWriteOffSource.ProductionOrder
	|WHERE
	...
	While Not Result.IsEmpty() Do
		...
		If IterationsQuantity > 60 Then
			ErrorDescription = NStr("en='Cannot adjust cost balance values.';ru='Не удалось скорректировать суммовые остатки по затратам.';vi='Không thể điều chỉnh số dư chi phí theo giá trị.'");
			...
			If SelectionDetailRecords.CorrGLAccount = AdditionalProperties.ForPosting.EmptyAccount Then
				...
				If ValueIsFilled(SelectionDetailRecords.SalesDocument)
				  ...
				  OR TypeOf(SelectionDetailRecords.SalesDocument) = Type("DocumentRef.ProcessingReport")) Then
					...
					NewRow.GLAccount = SelectionDetailRecords.BusinessActivitySalesGLAccountOfSalesCost;
					NewRow.AmountExpense = CorrectionAmount;
					NewRow.ContentOfAccountingRecord = NStr("en='Record sale expenses';ru='Отражение расходов по продаже';vi='Phản ánh chi phí theo bán hàng'");
					...
					NewRow.AccountDr = SelectionDetailRecords.BusinessActivitySalesGLAccountOfSalesCost;
					NewRow.AccountCr = SelectionDetailRecords.GLAccount;
					NewRow.Content = NStr("en='Record sale expenses';ru='Отражение расходов по продаже';vi='Phản ánh chi phí theo bán hàng'");
					NewRow.Amount = CorrectionAmount;
				...
				ElsIf ValueIsFilled(SelectionDetailRecords.SalesDocument)
					...
					If CorrectionAmount < 0 Then
						NewRow.GLAccount = ChartsOfAccounts.Managerial.OtherExpenses;
						NewRow.AmountExpense = CorrectionAmount;
						NewRow.ContentOfAccountingRecord = NStr("en='Other expenses on return to the supplier';ru='Прочие расходы по возврату поставщику';vi='Chi phí khác theo hàng trả lại nhà cung cấp'");
					Else
						NewRow.GLAccount = ChartsOfAccounts.Managerial.OtherIncome;
						NewRow.AmountIncome = CorrectionAmount;
						NewRow.ContentOfAccountingRecord = NStr("en='Other income on return to the supplier';ru='Прочие доходы по возврату поставщику';vi='Thu nhập khác theo hàng trả lại nhà cung cấp'");
					EndIf;
					...
					If CorrectionAmount < 0 Then
						...
						NewRow.AccountCr = SelectionDetailRecords.GLAccount;
						NewRow.Amount = - CorrectionAmount;
						NewRow.Content = NStr("en='Other expenses on return to the supplier';ru='Прочие расходы по возврату поставщику';vi='Chi phí khác theo hàng trả lại nhà cung cấp'");
					Else
						NewRow.AccountDr = SelectionDetailRecords.GLAccount;
						NewRow.AccountCr = ChartsOfAccounts.Managerial.OtherIncome;
						NewRow.Amount = CorrectionAmount;
						NewRow.Content = NStr("en='Other income on return to the supplier';ru='Прочие доходы по возврату поставщику';vi='Thu nhập khác theo hàng trả lại nhà cung cấp'");
					EndIf;
...
Procedure CalculateCostOfReturns()
	...
	|	Inventory.ProductionOrder AS ProductionOrder,
	...
	While SelectionDetailRecords.Next() Do
		...
		If (NOT Round(CorrectionAmount, 2) = 0) Then
			...
			GenerateRegisterRecordsByExpensesRegister(
				...
				NStr("en='Cost of return from customer';ru='Себестоимость возврата от покупателя';vi='Giá vốn hàng bán bị trả lại'")
			);
						...
						For Each DistributionDirection IN BaseTable Do
							...
							If CostAmount <> 0 Then
								...
								If BypassByGLAccounts.GLAccountGLAccountType = Enums.GLAccountsTypes.IndirectExpenses Then // the indirect ones are allocated via the closing account
									...
									RegisterRecordRow.Insert("ProductionOrder"       , BypassByGLAccounts.ProductionOrder);
									...
									RegisterRecordRow.Insert("ProductionCorrOrder", Documents.ProductionOrder.EmptyRef());
									...
									If ValueIsFilled(DistributionDirection.ProductsAndServices) Then
										...
										RegisterRecordRow.Insert("ProductionOrder"       , Documents.ProductionOrder.EmptyRef());
										RegisterRecordRow.Insert("StructuralUnitCorr", DistributionDirection.StructuralUnit);
...
Procedure CalculateCostPriceInRetailAccrualAccounting(Cancel, ErrorsTable)
	...
	While SelectionDetailRecords.Next() Do
		...
		NewRow.ContentOfAccountingRecord = NStr("en='Cost';ru='Себестоимость';vi='Giá vốn'");
		...
		NewRow.Content = NStr("en='Cost';ru='Себестоимость';vi='Giá vốn'");
		...
		NewRow.Content = NStr("en='Markup';ru='Наценка';vi='Phụ thu'");
		NewRow.Amount = - SelectionDetailRecords.CorrectionAmount;
...
Procedure CalculateExchangeDifferences(Cancel, ErrorsTable)
	...
	While SelectionDetailRecords.Next() Do
		...
		NewRow.Section = NStr("en='Cash assets';vi='Tiền mặt';");
	...
	While SelectionDetailRecords.Next() Do
		...
		NewRow.Section = NStr("en='Cash in cash registers';vi='Tiền gửi';");
	...
	While SelectionDetailRecords.Next() Do
		...
		NewRow.Section = NStr("en='Personnel settlements';vi='Người lao động';");
	...
	While SelectionDetailRecords.Next() Do
		...
		NewRow.Section = NStr("en='Settlements with advance holders';vi='Hạch toán với người nhận tạm ứng';");
	...
	While SelectionDetailRecords.Next() Do
		...
		NewRow.Section = NStr("en='Accounts receivable';vi='Các khoản phải thu';");
	...
	While SelectionDetailRecords.Next() Do
		...
		NewRow.Analytics =
		  ...
		  + " / "
		  + SelectionDetailRecords.Order;
		NewRow.Section = NStr("en='Accounts payable';vi='Các khoản phải trả';");
...
Procedure Posting(Cancel, PostingMode)
	...
	If ErrorsTable.Count() > 0 Then
		MessageText = NStr("en='Warnings were generated on month-end closing. For more information, see the month-end closing report.';ru='При закрытии месяца были сформированы предупреждения! Подробнее см. в отчете о закрытии месяца.';vi='Khi đóng tháng đã xuất hiện cảnh báo! Chi tiết xem trong báo cáo đóng tháng.'");
