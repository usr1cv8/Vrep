...
Procedure CancelPayment(Command)
	...
	If CurrentData = Undefined Then
		CommonUseClientServer.MessageToUser(NStr("en='Select the line of canceled payment by card.';ru='Выберите строку отменяемой оплаты картой.';vi='Hãy chọn dòng thanh toán bằng thẻ bị hủy bỏ.'"));
		Return;
	EndIf;
	
	If CurrentData.PaymentCanceled Then
		CommonUseClientServer.MessageToUser(NStr("en='This payment has already been canceled.';ru='Данная оплата уже отменена.';vi='Khoản thanh toán này đã bị hủy.'"));
		Return;
	EndIf;
