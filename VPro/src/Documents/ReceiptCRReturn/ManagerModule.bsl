...
Function GeneratePrintFormOfTheBill(ObjectsArray, PrintObjects)
	...
	For Each ReceiptCR IN ObjectsArray Do
		...
		TemplateArea.Parameters.HeaderText = NStr("en='Receipt CR on return No ';ru='Чек ККМ на возврат № ';vi='Phiếu tính tiền cần trả lại Số'")
												+ DocumentNumber
												+ NStr("en=' from ';ru=' от ';vi='ngày'")
		...
		While LinesSelectionInventory.Next() Do
			...
			If Not ValueIsFilled(LinesSelectionInventory.ProductsAndServices) Then
				Message(NStr("en='Products and services value is not filled in in one of the rows - String during printing is skipped.';ru='В одной из строк не заполнено значение номенклатуры - строка при печати пропущена.';vi='Trong một trong số các dòng chưa điền giá trị mặt hàng - dòng kbhi in bị bỏ qua.'"), MessageStatus.Important);
				Continue;
			EndIf;
		...
		TemplateArea.Parameters.TotalRow = NStr("en='Total titles ';ru='Всего наименований ';vi='Tất cả '")
													+ String(LinesSelectionInventory.Count())
													+ NStr("en=', in the amount of ';ru=', на сумму ';vi=', với số tiền là '")
