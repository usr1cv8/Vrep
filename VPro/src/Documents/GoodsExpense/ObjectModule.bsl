	...
	If AttributeValues.Property("OperationKind") Then
		If ValueIsFilled(AttributeValues.OperationKind)
			AND Not FillingData.OperationKind = Enums.OperationKindsCustomerOrder.WorkOrder Then
				ErrorMessage = NStr("en='You can enter issue slip only based on the work order.';ru='Расходный ордер можно ввести только на основании заказ-наряда!';vi='Chỉ có thể nhập phiếu xuất kho trên cơ sở đơn hàng trọn gói!'");
				Raise ErrorMessage;
		EndIf;
	EndIf;
...
"Документ ""%ДокументСсылка"" не имеет ордерного склада!';vi='Không thể nhập giao dịch ""Tiếp nhận vào kho 2 pha""."
"Chứng từ ""%ThamChiếuChứngTừ"" không có kho 2 pha!'");
