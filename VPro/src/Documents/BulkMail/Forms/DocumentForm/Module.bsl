...
"Настройка осуществляется в разделе Настройки - Органайзер - Настройка отправки SMS.';vi='Để gửi SMS cần tùy chỉnh các tham số gửi."
"Thực hiện tùy chỉnh trong phần hành Tùy chỉnh - Doanh nghiệp - Tùy chỉnh gửi SMS.'");
...
"Để thực hiện tùy chỉnh, hãy liên hệ với người quản trị.'");
		...
		NotificationText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Successfully sent: %1 messages';ru='Успешно отправлено: %1 сообщений';vi='Đã gửi thành công: %1 thông báo'"), SuccessfullySent);
...
Procedure CheckEmailAddressCorrectness(Cancel)
	...
	For Each RecipientRow IN Object.Recipients Do
		...
		Except
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Recipient email is specified incorrectly: %1, due to: %2';ru='Некорректно указан E-mail получателя: %1, по причине: %2';vi='Đã chỉ ra sai E-mail người nhận: %1, bởi vì: %2'"),
...
Function SendEmailMailing()
	...
	For Each RecipientRow IN Object.Recipients Do
		...
		Except
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Cannot send Email to recipient: %1, due to: %2';ru='Не удалось отправить E-mail получателю: %1, по причине: %2';vi='Không thể gửi E-mail cho người nhận: %1, bởi vì: %2'"),
				RecipientRow.Contact,
				BriefErrorDescription(ErrorInfo()),
				);
...
Function SendSMSMailing()
	...
	For Each RecipientRow IN Object.Recipients Do
		...
		If IsBlankString(SendingResult.ErrorDescription) Then
			...
			SuccessfullySent = SuccessfullySent + 1;
		Else
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Cannot send SMS to recipient: %1, due to: %2';ru='Не удалось отправить SMS получателю: %1, по причине: %2';vi='Không thể gửi SMS cho người nhận: %1, bởi vì: %2'"),
				RecipientRow.Contact,
...
Function GetSpreadsheetDocumentByBinaryData(Val BinaryData)
	...
	Except
		WriteLogEvent(NStr("en='Receive spreadsheet document';ru='Получение табличного документа';vi='Nhận văn bản dạng bảng'", CommonUseClientServer.MainLanguageCode()), EventLogLevel.Error, , , 
