...
Function PrintForm(ObjectsArray, PrintObjects, TemplateName)
	...
	For Each CurrentDocument IN ObjectsArray Do
		...
		If TemplateName = "MerchandiseFillingForm" Then
			...
			TemplateArea.Parameters.HeaderText = NStr("en='Inventory survey No';ru='Инвентаризация запасов № ';vi='Kiểm kê vật tư số'") + 
												 DocumentNumber + 
												 NStr("en=' dated ';ru=' от ';vi='ngày'") + 
			...
			TemplateArea.Parameters.PrintingTime = 	NStr("en='Date and time of printing: ';ru='Дата и время печати: ';vi='Ngày và giờ in:'") +
													CurrentDate() + 
													NStr("en='. User: ';ru='. Пользователь: ';vi='. Người sử dụng: '") + 
													Users.CurrentUser();
			SpreadsheetDocument.Put(TemplateArea);
		...
		ElsIf TemplateName = "InventoryReconciliation" Then
			...
			TemplateArea = Template.GetArea("Title");
			TemplateArea.Parameters.HeaderText = NStr("en='Inventory survey No';ru='Инвентаризация запасов № ';vi='Kiểm kê vật tư số'") + DocumentNumber + NStr("en=' dated ';ru=' от ';vi='ngày'") + Format(Header.DocumentDate, "DLF=DD");
			SpreadsheetDocument.Put(TemplateArea);
...
Procedure AddPrintCommands(PrintCommands) Export
	...
	PrintCommand.Presentation = NStr("en='Physical inventory';ru='Инвентаризации запасов';vi='Kiểm kê vật tư'");
