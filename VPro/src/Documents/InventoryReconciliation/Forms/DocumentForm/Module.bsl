...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	DataImportFromExternalSources.OnCreateAtServer(Metadata.Documents.InventoryReconciliation.TabularSections.Inventory, DataLoadSettings, ThisObject);
...
Function GenerateFilterHeaderFromList(ItemList)
	...
	If FilterHeaderString = "" Then
		FilterHeaderString = NStr("en='Filter not set';ru='Отбор не установлен';vi='Chưa thiết lập phễu lọc'");
	EndIf;
...
Procedure ClearFilterConditionByProductsAndServices()
	...
	Items.SetFilterByProductsAndServices.Title = NStr("en='Filter not set';ru='Отбор не установлен';vi='Chưa thiết lập phễu lọc'");
...
Procedure ClearFilterCriteriaByProductAndServicesGroups()
	...
	Items.SetFilterByProductsAndServicesGroups.Title = NStr("en='Filter not set';ru='Отбор не установлен';vi='Chưa thiết lập phễu lọc'");
...
Procedure ClearFilterCriteriaByProductsAndServicesCategories()
	...
	Items.SetFilterByProductsAndServicesCategories.Title = NStr("en='Filter not set';ru='Отбор не установлен';vi='Chưa thiết lập phễu lọc'");
...
Procedure CommandFillByBalanceAtWarehouse()
	...
	If Object.Inventory.Count() > 0 Then
		...
		ShowQueryBox(New NotifyDescription("FillCommandByBalanceOnWarehouseEnd", ThisObject), NStr("en='Tabular section will be cleared. Continue?';ru='Табличная часть будет очищена! Продолжить выполнение операции?';vi='Phần bảng sẽ bị xóa! Tiếp tục thực hiện thao tác?'"), QuestionDialogMode.YesNo, 0);
        Return; 
	EndIf;
...
Procedure CommandFillOnlyAccountingData()
	...
	If Object.Inventory.Count() > 0 Then
		...
		ShowQueryBox(New NotifyDescription("CommandFillOnlyAccountingDataEnd", ThisObject), NStr("en='Accounting data will be cleared. Continue?';ru='Учетные данные будут очищены! Продолжить?';vi='Dữ liệu kế toán sẽ bị xóa! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
        Return; 
	Else
		Return;
	EndIf;
...
Procedure CommandFillByPriceKind(Command)
	...
	ShowQueryBox(New NotifyDescription("CommandFillByPriceKindEnd1", ThisObject), NStr("en='Prices will be refilled. Continue?';ru='Цены будут перезаполнены! Продолжить?';vi='Sẽ điền lại đơn giá! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
...
Procedure CommandZeroQuantityAndTheAmount(Command)
	...
	If Object.Inventory.Count() > 0 Then
		...
		ShowQueryBox(New NotifyDescription("CommandZeroOutQuantityAndAmountEnd", ThisObject), NStr("en='The ""Quantity"" and ""Amount"" columns will be cleared. Continue?';ru='Колонки ""Количество"" и ""Сумма"" будут очищены! Продолжить?';vi='Cột ""Số lượng"" và ""Số tiền"" sẽ bị xóa! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
        Return;
	Else
		Return;
	EndIf;
...
Procedure LoadFromFileInventory(Command)
	...
	DataLoadSettings.Insert("TabularSectionFullName",	"InventoryReconciliation.Inventory");
