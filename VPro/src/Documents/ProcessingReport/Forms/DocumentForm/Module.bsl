...
Procedure ChangeReserveFillByReserves(Command)
	...
	If Object.Products.Count() = 0 Then
		Message = New UserMessage;
		Message.Text = NStr("en='The ""Products"" tabular section is not filled in.';ru='Табличная часть ""Продукция"" не заполнена!';vi='Phần bảng ""Thành phẩm"" chưa điền!'");
		Message.Message();
		Return;
	EndIf;
...
Procedure CommandFillBySpecification(Command)
	...
	If Object.Inventory.Count() <> 0 Then
		...
		ShowQueryBox(New NotifyDescription("CommandToFillBySpecificationEnd", ThisObject), NStr("en='Tabular section ""Materials"" will be filled in again. Continue?';ru='Табличная часть ""Материалы"" будет перезаполнена! Продолжить?';vi='Phần bảng ""Nguyên vật liệu"" sẽ được điền lại! Tiếp tục?'"), 
