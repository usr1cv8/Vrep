...
Procedure InitializeDocumentData(DocumentRefReportAboutRecycling, StructureAdditionalProperties) Export
	...
	"SELECT
	|	CASE
	|		WHEN ProcessingReport.Contract.IsServiceContract
	|				AND ProcessingReport.Contract.ServiceContractBusinessActivity <> VALUE(Catalog.BusinessActivities.EmptyRef)
	|			THEN TRUE
	|		ELSE FALSE
	|	END AS KeepAccountingExpensesByServiceContracts,
	|	ProcessingReport.Contract.ServiceContractBusinessActivity AS BusinessActivity,
	|	ProcessingReport.DiscountMarkupKind AS DiscountMarkupKind
	...
	|		WHEN TemporaryTableHeader.KeepAccountingExpensesByServiceContracts
	|			THEN TemporaryTableHeader.BusinessActivity
	|		ELSE ProcessingReportProducts.ProductsAndServices.BusinessActivity
	|	END AS BusinessActivitySales,
	|	CASE
	|		WHEN TemporaryTableHeader.KeepAccountingExpensesByServiceContracts
	|			THEN TemporaryTableHeader.BusinessActivity.GLAccountRevenueFromSales
	|		ELSE ProcessingReportProducts.ProductsAndServices.BusinessActivity.GLAccountRevenueFromSales
	|	END AS AccountStatementSales,
	|	CASE
	|		WHEN TemporaryTableHeader.KeepAccountingExpensesByServiceContracts
	|			THEN TemporaryTableHeader.BusinessActivity.GLAccountCostOfSales
	|		ELSE ProcessingReportProducts.ProductsAndServices.BusinessActivity.GLAccountCostOfSales
	...
	For Each CurrentDocument IN ObjectsArray Do
		...
		If TemplateName = "Act" Then
			...
			While Header.Next() Do
				...
				If VATAmount = 0 Then
					TemplateArea.Parameters.VAT = NStr("en='Without tax (VAT)';vi='Tổng số không có thuế GTGT';");
					TemplateArea.Parameters.TotalVAT = "-";
				Else
					TemplateArea.Parameters.VAT = ?(Header.AmountIncludesVAT, NStr("en='Including VAT:';vi='Thuế GTGT:';"), NStr("en='VAT Amount:';vi='Thuế GTGT:';"));
				...
				TemplateArea.Parameters.TotalRow = NStr("en='Total titles ';vi='Tổng số ';")
				                                        + String(Quantity)
				                                        + NStr("en=', in the amount of ';vi=', thành tiền là ';")
		...
		ElsIf TemplateName = "MerchandiseFillingForm" Then
			...
			TemplateArea = Template.GetArea("Title");
			TemplateArea.Parameters.HeaderText = NStr("en='Processing report # ';vi='Bảng kê hàng hóa nhận gia công số ';") 
													+ DocumentNumber
													+ NStr("en=' from ';vi=' '")
													+ Format(Header.DocumentDate, "DLF=DD");
			...
			TemplateArea.Parameters.PrintingTime = NStr("en='Date and time of printing: ';vi='Ngày va giờ in: ';") 
												 	+ CurrentDate()
