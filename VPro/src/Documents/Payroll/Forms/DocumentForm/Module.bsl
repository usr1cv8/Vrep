...
Function FillIndicators(AccrualDeductionKind)
	...
	For Each ParameterStructures IN ParametersStructure Do
		...
		If Not ValueIsFilled(CalculationParameter) Then
			...
			Message.Text = NStr("en='Parameter is not found ';ru='Не найден параметр ';vi='Không tìm thấy tham số'") + CalculationParameter + NStr("en=' for accrual (deduction) formula ';ru=' для формулы начисления (удержания) ';vi=' đối với công thức tính (giữ lại)'") + AccrualDeductionKind;
			Message.Message();
			Continue;
...
Procedure FillByDepartment()
	...
	For Each TabularSectionRow IN Object.AccrualsDeductions Do
		...
		If RepetitionsArray.Count() > 1 AND RepetitionsArray[0].TotalForPeriod Then
			...
			MessageText = NStr("en='%Employee%, %AccrualKind%: Working hours data has been entered consolidated. Time calculation for each accrual (deduction) kind is not possible!';ru='%Employee%, %AccrualKind%: Данные об отработанном времени введены сводно. Расчет времени по каждому виду начисления (удержания) невозможен!';vi='%Employee%, %AccrualKind%: Đã nhập tổng hợp dữ liệu về thời gian làm việc. Việc tính toán thời gian theo mỗi dạng tính lương (giữ lại) là không thể!'");
		...
		For Each ParameterStructures IN ParametersStructure Do
			...
			CalculationParameter = Catalogs.CalculationsParameters.FindByAttribute("ID", ParameterStructures.Key);
		 	If Not ValueIsFilled(CalculationParameter) Then		
				Message = New UserMessage();
				Message.Text = NStr("en='Parameter is not found ';ru='Не найден параметр ';vi='Không tìm thấy tham số'") + CalculationParameter + NStr("en=' for the employee in row No. ';ru=' для сотрудника в строке № ';vi=' đối với người lao động tại dòng số '") + (Object.AccrualsDeductions.IndexOf(TabularSectionRow) + 1);
				Message.Message();
		    EndIf; 
		...
		// 5. Indicator calculation
			...
			TabularSectionRow["Value" + Counter] = SmallBusinessServer.CalculateParameterValue(StructureOfSelections, CalculationParameter, NStr("en=' for the employee in row No.';ru=' для сотрудника в строке №';vi=' đối với người lao động tại dòng số '") + (Object.AccrualsDeductions.IndexOf(TabularSectionRow) + 1));
...
Procedure CalculateByFormulas()
	...
	For Each AccrualsRow IN Object.AccrualsDeductions Do
		...
		Except
			MessageText = NStr("en='Cannot calculate the accrual amount in the line No. %LineNumber%. The formula may contain an error, or indicators are not filled in.';ru='Не удалось рассчитать сумму начисления в строке №%НомерСтроки%. Возможно, формула содержит ошибку или не заполнены показатели.';vi='Không thể tính số tiền lương tại dòng số %LineNumber%. Có thể là công thức bị lỗi hoặc chưa điền các chỉ số.'");
...
Procedure Fill(Command)
	...
	If Not ValueIsFilled(Object.StructuralUnit) Then
		...
		Message = New UserMessage();
		Message.Text = NStr("en='Department is not populated. Document population is canceled.';ru='Не заполнено подразделение! Заполнение документа отменено.';vi='Chưa điền bộ phận! Đã hủy bỏ việc điền chứng từ.'");
	...
	If Object.AccrualsDeductions.Count() > 0 AND Object.IncomeTaxes.Count() > 0 Then
		...
		ShowQueryBox(New NotifyDescription("FillEnd1", ThisObject), NStr("en='Document tabular sections will be cleared. Continue?';ru='Табличные части документа будут очищены! Продолжить?';vi='Phần bảng chứng từ sẽ bị xóa! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
        Return;
		
	ElsIf Object.AccrualsDeductions.Count() > 0 OR Object.IncomeTaxes.Count() > 0 Then
		
		ShowQueryBox(New NotifyDescription("FillEnd", ThisObject), NStr("en='Tabular section of the document will be cleared. Continue?';ru='Табличная часть документа будет очищена! Продолжить?';vi='Phần bảng chứng từ sẽ bị xóa! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
        Return; 
...
Procedure DocumentCurrencyOnChange(Item)
	...
	If Object.AccrualsDeductions.Count() > 0 Then
		...
		ShowQueryBox(New NotifyDescription("DocumentCurrencyOnChangeEnd", ThisObject), NStr("en='Tabular section will be cleared. Continue?';ru='Табличная часть будет очищена! Продолжить выполнение операции?';vi='Phần bảng sẽ bị xóa! Tiếp tục thực hiện thao tác?'"), Mode, 0);
		Return;
