...
Procedure Filling(FillingData, StandardProcessing) Export
	...
	If TypeOf(FillingData) = Type("DocumentRef.InventoryReconciliation") AND ValueIsFilled(FillingData) Then
		...
		If Not Constants.FunctionalOptionUseSubsystemProduction.Get()
			...
			Raise NStr("en='Cannot enter Inventory capitalization basing on the physical inventory, as the Production activity kind is not available';ru='Нельзя ввести Оприходование запасов на основании инвентеризации запасов, т.к. недоступен вид деятельности Производство!';vi='Không thể nhập Ghi tăng vật tư trên cơ sở kiểm kê vật tư, bởi vì không sử dụng được dạng hoạt động Sản xuất!'");
		EndIf;
		...
		If Inventory.Count() = 0 Then
			...
			Raise NStr("en='No data for capitalization registration.';ru='Нет данных для оформления оприходования!';vi='Không có dữ liệu để lập chứng từ ghi tăng!'");
