...
Procedure FillByBasis(Command)
	...
	If Not ValueIsFilled(Object.BasisDocument) Then
		...
		MessageText	= NStr("en='Specify the basis document.';ru='Требуется заполнить документ-основание.';vi='Cần điền chứng từ-cơ sở'");
		CommonUseClientServer.MessageToUser(MessageText, , "Object.BasisDocument");
		Return;
		
	EndIf;
	
	QuestionText 	= NStr("en='The  document will be fully filled out according to the ""Basis"". Continue?';ru='Документ будет полностью перезаполнен по ""Основанию""! Продолжить?';vi='Chứng từ sẽ được điền lại toàn bộ theo ""Cơ sở""! Tiếp tục?'");
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	AdditionalParameters = PropertiesManagementOverridable.FillAdditionalParameters(Object,
		"AdditionalAttributesGroup");
...
Procedure AddAdditionalAttribute(Command)
	...
	FormParameters.Insert("CurrentSetOfProperties",
		PredefinedValue("Catalog.AdditionalAttributesAndInformationSets.Document_CashOutflowPlan"));
