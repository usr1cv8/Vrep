...
Procedure RunPreliminaryControl(Cancel)
	...
	If Not QueryResult.IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='The ""%FixedAsset%"" property in the %LineNumber% line of the ""Property"" list is specified again.';ru='Имущество ""%ВнеоборотныйАктив%"" указанное в строке %НомерСтроки% списка ""Имущество"", указано повторно.';vi='Tài sản cố định ""%FixedAsset%"" mà đã chỉ ra tại dòng %LineNumber% trong danh sách ""Tài sản cố định"" đã bị lặp lại."
"'"
			);
	...
	If TotalOriginalCost <> Amount Then
		MessageText = NStr("en='Products and services amount: %Amount% does not correspond to the amount of initial property costs: %TotalInitialCost%';ru='Стоимость номенклатуры: %Amount% , не соответствует сумме первоначальных стоимостей имущества: %TotalInitialCost%';vi='Giá trị của mặt hàng: %Amount% , không tương ứng với số tiền theo giá trị ban đầu của tài sản cố định: %TotalInitialCost%'"
		);
	...
	For Each RowOfFixedAssets IN FixedAssets Do
		...
		If ArrayVAStatus.Find(RowOfFixedAssets.FixedAsset) <> Undefined Then
			MessageText = NStr("en='The current state of the ""%FixedAsset%"" property specified in the %LineNumber% line of the ""Property"" list is ""Entered in the books"".';ru='Для имущества ""%ВнеоборотныйАктив%"" указанного в строке %НомерСтроки% списка ""Имущество"", текущее состояние ""Принят к учету"".';vi='Đối với tài sản cố định ""%FixedAsset%"" mà đã chỉ ra tại dòng %LineNumber% trong danh sách ""Tài sản cố định"", trạng thái hiện tại ""Đã tiếp nhận vào kế toán"".'"
			);
	...
	For Each RowOfFixedAssets IN FixedAssets Do
		...
		If RowOfFixedAssets.FixedAsset.DepreciationMethod = Enums.FixedAssetsDepreciationMethods.ProportionallyToProductsVolume
		   AND RowOfFixedAssets.AmountOfProductsServicesForDepreciationCalculation = 0 Then
			MessageText = NStr("en='For property ""%FixedAsset%"" indicated in string %LineNumber% of list ""Property"" should be filled with ""Product (work) volume for depreciation calculation in physical terms."".';ru='Для имущества ""%ВнеоборотныйАктив%"" указанного в строке %НомерСтроки% списка ""Имущество"", должен быть заполнен ""Объем продукции (работ) для исчисления амортизации в натуральных ед."".';vi='Đối với tài sản cố định ""%FixedAsset%""  mà đã chỉ ra tại dòng %LineNumber% trong danh sách ""Tài sản cố định"", cần điền ""Khối lượng sản phẩm (công việc) để trích khấu hao bằng đơn vị tự nhiên"".'"
			);
			MessageText = StrReplace(MessageText, "%FixedAsset%", TrimAll(String(RowOfFixedAssets.FixedAsset))); 
			MessageText = StrReplace(MessageText, "%LineNumber%",String(RowOfFixedAssets.LineNumber));
			SmallBusinessServer.ShowMessageAboutError(
