	...
	For Each RowTaxes IN Taxes Do
		...
		If Constants.FunctionalOptionAccountingByMultipleDepartments.Get()
		 ...
		 AND Not ValueIsFilled(RowTaxes.Department) Then
			MessageText = NStr("en='The ""Department"" attribute should be filled in for the ""%Mail%"" costs account specified in the %LineNumber% line of the ""Taxes"" list.';ru='Для счета затрат ""%Корреспонденция%"" указанного в строке %НомерСтроки% списка ""Налоги"", должен быть заполнен реквизит ""Подразделение"".';vi='Đối với tài khoản chi phí ""%Mail%"" mà đã chỉ ra tại dòng %LineNumber% trong danh sách ""Thuế"", cần điền mục tin ""Bộ phận"".'"
			);
