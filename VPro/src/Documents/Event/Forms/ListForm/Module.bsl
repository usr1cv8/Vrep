...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ThisForm.WindowOpeningMode = ?(Parameters.ChoiceMode,FormWindowOpeningMode.LockOwnerWindow, FormWindowOpeningMode.Independent);
...
Procedure PaintList()
	...
	While SelectionEventStates.Next() Do
		...
		ConditionalAppearanceItem.Presentation = NStr("en='By event state ';ru='По состоянию события ';vi='Theo trạng thái sự kiện'") + SelectionEventStates.Description;
...
Procedure SetFilterCurrentWorks()
	...
	AutoTitle	= False;
	Title		= NStr("en='Events';ru='События';vi='Sự kiện'");
	...
	If Parameters.Property("PastPerformance") Then
		...
		Title = Title + ": " + NStr("en='expired';ru='просроченные';vi='đã quá hạn'");
	...
	ElsIf Parameters.Property("ForToday") Then
		...
		Title = Title + ": " + NStr("en='for today';ru='на сегодня';vi='hôm nay'");
		SmallBusinessClientServer.SetListFilterItem(
			List, 
			"EventBegin", 
	...
	ElsIf Parameters.Property("InProcess") Then
		...
		Title = Title + ": " + NStr("en='in process';ru='в работе';vi='đang làm việc'");
		
	EndIf;
	
	If Parameters.Property("Responsible") Then
		Title = Title + ", " + NStr("en='responsible';ru='ответственный';vi='chịu trách nhiệm'") + " " + Parameters.Responsible.Initials;
	EndIf;
...
Procedure SetFilterEventType()
	...
	If Parameters.EventType = Enums.EventTypes.PhoneCall Then
		Title = NStr("en='Events: phone calls';ru='События: телефонные звонки';vi='Sự kiện: cuộc gọi'");
	ElsIf Parameters.EventType = Enums.EventTypes.Email Then
		Title = NStr("en='Events: emails';ru='События: электронные письма';vi='Sự kiện: E-mail'");
	ElsIf Parameters.EventType = Enums.EventTypes.SMS Then
		Title = NStr("en='Events: SMS';ru='События: сообщения SMS';vi='Sự kiện: SMS'");
	ElsIf Parameters.EventType = Enums.EventTypes.PersonalMeeting Then
		Title = NStr("en='Events: personal meetings';ru='События: личные встречи';vi='Sự kiện: cuộc gặp'");
	ElsIf Parameters.EventType = Enums.EventTypes.Other Then
		Title = NStr("en='Events: other';ru='События: прочие';vi='Sự kiện: khác'");
	ElsIf Parameters.EventType = Enums.EventTypes.Record Then
		Title = NStr("en='Events: records';ru='События: запись';vi='Sự kiện: ghi lại'");
	EndIf;
