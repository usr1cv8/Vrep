...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		...
		AutoTitle = False;
		Title = StrTemplate(
		NStr("en='Event: %1 (create)';ru='Событие: %1 (создание)';vi='Sự kiện: %1 (tạo mới)'"),
		Object.EventType);
...
Procedure ContactsContactStartChoice(Item, ChoiceData, StandardProcessing)
	...
	If Not ValueIsFilled(Counterparty) Then
		CommonUseClientServer.MessageToUser(NStr("en='Select a counterparty.';ru='Необходимо выбрать контрагента.';vi='Cần chọn đối tác.'"), , "Counterparty");
		Return;
	EndIf;
...
Procedure FillByBasis(Command)
	...
	If Not ValueIsFilled(Object.BasisDocument) Then
		ShowMessageBox(Undefined, NStr("en='The basis for filling is not specified.';ru='Не указано основание для заполнения.';vi='Chưa chỉ ra cơ sở để điền.'"));
		Return;
	EndIf;
	
	ShowQueryBox(New NotifyDescription("FillByBasisEnd", ThisObject),
		NStr("en='The  document will be fully filled out according to the ""Basis"". Continue?';ru='Документ будет полностью перезаполнен по ""Основанию""! Продолжить?';vi='Chứng từ sẽ được điền lại toàn bộ theo ""Cơ sở""! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
...
Procedure FillByCounterparty(Command)
	...
	If Contacts.Count() > 0 Then
		ShowQueryBox(New NotifyDescription("FillByCounterpartyEnd", ThisObject),
			NStr("en='Contacts will be completely refilled according to the counterparty. Continue?';ru='Контакты будут полностью перезаполнены по контрагенту! Продолжить?';vi='Người liên hệ sẽ được điền lại toàn bộ theo đối tác! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
	Else
...
Function GetSubjectChoiceList(val SearchString, val SubjectRowHistory)
	...
	For Each ItemOfList IN SubjectSelectionData Do
		ListChoiceOfTopics.Add(ItemOfList.Value, New FormattedString(ItemOfList.Presentation, NStr("en=' (event subject)';ru=' (тема события)';vi='(chủ đề sự kiện)'")));
	EndDo;
...
Procedure FormDurationPresentation(Form)
	...
	CaptionDays = SmallBusinessClientServer.PluralForm(
		NStr("en='Day';ru='день';vi='Ngày'"),
		NStr("en='day';ru='дня';vi='ngày'"),
		NStr("en='days';ru='дней';vi='ngày'"),
		Days
	);
	
	Hours = Int((DurationSec - Days * 86400) / 3600);
	CaptionHours = SmallBusinessClientServer.PluralForm(
		NStr("en='hour';ru='час';vi='giờ'"),
		NStr("en='hours';ru='часа';vi='giờ'"),
		NStr("en='hours';ru='часов';vi='giờ'"),
		Hours
	);
	
	Minutes = Int((DurationSec - Days * 86400 - Hours * 3600) / 60);
	CaptionMinutes = SmallBusinessClientServer.PluralForm(
		NStr("en='minute';ru='минута';vi='phút'"),
		NStr("en='minutes';ru='минуты';vi='phút'"),
		NStr("en='minutes';ru='минут';vi='phút'"),
		Minutes
	);
