...
Procedure Send(Command)
	...
	If Not ValueIsFilled(Object.UserAccount) Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Select an account to send emails.';ru='Выберите учетную запись для отправки почты.';vi='Hãy chọn trương mục E-mail để gửi E-mail.'"), ,
			"Object.UserAccount");
		Return;
	EndIf;
...
Procedure ContinueSendingEmailsWithPassword(Password = Undefined)
	...
	If EmailParameters = Undefined Then
		CommonUseClientServer.MessageToUser(NStr("en='An error occurred while generating email message parameters';ru='Ошибка формирования параметров почтового сообщения';vi='Lỗi tạo tham số thông điệp'"));
		Return;
	EndIf;
	...
	If Successfully Then
		...
		Write();
		ShowUserNotification(NStr("en='Message is sent successfully';ru='Сообщение успешно отправлено';vi='Thông điệp đã gửi thành công'"), GetURL(Object.Ref), String(Object.Ref), PictureLib.Information32);
		Close(Successfully);
	EndIf;
...
Procedure SetBasisDocumentHyperlinkTitle()
	...
	Items.OpenBasisDocuments.Title = ?(Result = "", NStr("en='Basis documents list';ru='Список документов оснований';vi='Danh sách chứng từ cơ sở'"), Result);
