...
Procedure BeforeWrite(Cancel, WriteMode, PostingMode)
	...
	If ЭтоЗапись() And Not Cancel Then
		ResourcesRecordsTable = ResourcePlanningCM.DocumentsRecordsTable(ThisObject);
		AdditionalProperties.Insert("ResourcesRecordsTable", ResourcesRecordsTable);
	EndIf;
...
Procedure OnCopy(CopiedObject)
	...
	If EventType = Enums.EventTypes.Email
		...
		Raise NStr("en='You can not copy an incoming message.';ru='Копирование входящего письма невозможно.';vi='Không thể sao chép E-mail đến.'");
	EndIf;
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If EventEnding < EventBegin Then
		CommonUseClientServer.MessageToUser(
			NStr("en='The end date cannot be earlier than the start date.';ru='Дата окончания не может быть меньше даты начала.';vi='Ngày kết thúc không thể nhỏ hơn ngày bắt đầu.'"),
			ThisObject,
			"EventEnding",
			,
			Cancel
...
Procedure FillingHandler(FillingData) Export
	...
	If FillingData.EventType = Enums.EventTypes.PhoneCall
		...
		ThisObject.Content = StrTemplate(NStr("en='Call from number:%1.';ru='Звонок с номера: %1.';vi='Cuộc gọi từ số: %1.'"), FillingData.PhoneNumber);
	EndIf;
...
Procedure ЗапланироватьРесурсыПредприятия(Cancel)
	
	If Cancel Then
		Return;
	EndIf;
	
	If Not ЭтоЗапись() Then
		Return;
	EndIf;
	
	ResourcesRecordsTable = Undefined;
	
	If Not DeletionMark
		And State <> Catalogs.EventStates.FindByDescription("Canceled")
		And Not ЕстьПодчиненныеЗаказНаряды() Then
		AdditionalProperties.Property("ResourcesRecordsTable", ResourcesRecordsTable);
	EndIf;
	
	ResourcePlanningCM.CreateRecordsEmployeeCalendarByResources(Ref, ResourcesRecordsTable, Cancel);
	ResourcePlanningCM.WriteScheduleResourceLoading(Ref, ResourcesRecordsTable, Cancel);
	
EndProcedure

Function ЭтоЗапись()
	
	Return EventType = Enums.EventTypes.Record;
...
Procedure OnWrite(Cancel)
	...
	//	InformationRegisters.УдалениеСобытий.ОтметитьКакУдалено(Ref);
	//Else
	//	InformationRegisters.УдалениеСобытий.DeleteRecord(Ref);
	//EndIf;
	
	ЗапланироватьРесурсыПредприятия(Cancel);

EndProcedure

Function ЕстьПодчиненныеЗаказНаряды()
	
	Query = New Query;
	
	Query.SetParameter("Event", Ref);
...
Процедура ЗаполнитьВремяСобытия()
	...
	ResourcesRecordsTable = Неопределено;
	ДополнительныеСвойства.Свойство("ResourcesRecordsTable", ResourcesRecordsTable);
	
	Если ЭтоЗапись() И ResourcesRecordsTable <> Неопределено Тогда
		СтруктураИнтервалов = ResourcePlanningCM.MaxIntervalBoarders(ResourcesRecordsTable, "Start", "Finish");
		Если СтруктураИнтервалов <> Неопределено Тогда
			EventBegin = СтруктураИнтервалов.IntervalBegin;
			EventEnding = СтруктураИнтервалов.IntervalEnd;
		Иначе
			EventBegin = Неопределено;
			EventEnding = Неопределено;
		КонецЕсли;
	КонецЕсли;
