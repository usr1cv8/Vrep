...
Procedure SetFilterCurrentWorks()
	...
	If Parameters.Property("PastPerformance") Then
		FormHeaderText = NStr("en='Purchase orders: execution is overdue';ru='Заказы поставщикам: просрочено выполнение';vi='Đơn hàng đặt nhà cung cấp: đã quá hạn thực hiện'");
		SmallBusinessClientServer.SetListFilterItem(List, "PastPerformance", True);
	EndIf;
	
	If Parameters.Property("OverduePayment") Then
		FormHeaderText = NStr("en='Purchase orders: payment is overdue';ru='Заказы поставщикам: просрочена оплата';vi='Đơn hàng đặt nhà cung cấp: đã quá hạn thanh toán'");
		SmallBusinessClientServer.SetListFilterItem(List, "OverduePayment", True);
	EndIf;
	
	If Parameters.Property("ForToday") Then
		FormHeaderText = NStr("en='Purchase orders: for today';ru='Заказы поставщикам: на сегодня';vi='Đơn hàng đặt nhà cung cấp: vào hôm nay'");
		SmallBusinessClientServer.SetListFilterItem(List, "ForToday", True);
	EndIf;
	
	If Parameters.Property("InProcess") Then
		FormHeaderText = NStr("en='Purchase orders: in progess';ru='Заказы поставщикам: в работе';vi='Đơn hàng đặt nhà cung cấp: đang làm việc'");
