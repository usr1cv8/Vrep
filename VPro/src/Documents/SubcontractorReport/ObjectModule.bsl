...
Procedure Filling(FillingData, StandardProcessing) Export
	...
	ElsIf TypeOf(FillingData) = Type("DocumentRef.CustomerInvoice") Then
		If FillingData.OperationKind <> Enums.OperationKindsCustomerInvoice.TransferToProcessing Then
			Raise NStr("en='Processing report is displayed only based on the transfer to processing.';ru='Отчет о переработке вводится только на основании передачи в переработку!';vi='Bảng kê hàng hóa nhận gia công sẽ chỉ được nhập trên cơ sở chuyển đi gia công!'");;
		EndIf;
