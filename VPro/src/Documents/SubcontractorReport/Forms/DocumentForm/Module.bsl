...
"Пересчитать документ в соответствии с договором?';vi='Hợp đồng với đối tác"
"xem xét dạng giá đối tác mà khác với dạng giá đã thiết lập trong chứng từ!"
"Tính lại chứng từ phù hợp với hợp đồng?"
"'") + Chars.LF + Chars.LF;
...
Procedure BasisDocumentOnChange(Item)
	...
	If Object.Prepayment.Count() > 0
	   ...
	   AND CounterpartyDoSettlementsByOrders Then
		...
		ShowQueryBox(New NotifyDescription("BaseDocumentOnChangeEnd", ThisObject), NStr("en='Prepayment setoff will be cleared, continue?';ru='Зачет предоплаты будет очищен, продолжить?';vi='Sẽ hủy việc khấu trừ khoản trả trước, tiếp tục?'"), Mode, 0);
        Return;
	EndIf;
...
Procedure CommandFillBySpecification(Command)
	...
	If Object.Inventory.Count() <> 0 Then
		...
		ShowQueryBox(New NotifyDescription("CommandToFillBySpecificationEnd", ThisObject), NStr("en='The ""Inventory"" tabular section will be filled in again. Continue?';ru='Табличная часть ""Запасы"" будет перезаполнена! Продолжить?';vi='Phần bảng ""Hàng hóa"" sẽ được điền lại! Tiếp tục?'"), 
