...
Procedure GenerateTableInventoryReceived(DocumentRefReportOnRetailSales, StructureAdditionalProperties)
	...
	Query.SetParameter("InventoryReceiptProductsOnCommission", NStr("en='Commission goods sales';ru='Реализация комиссионных товаров';vi='Bán hàng hóa được ký gửi'"));
...
Procedure InitializeDocumentData(DocumentRefReportOnRetailSales, StructureAdditionalProperties) Export
	...
	|	RetailSalesReportInventory.Ref.Date AS Period,
	...
	|	RetailSalesReportInventory.CountryOfOrigin AS CountryOfOrigin,
	|	RetailSalesReportInventory.CCDNo AS CCDNo,
	...
	|	RetailSalesReportDiscountsMarkups.ProductsAndServices AS ProductsAndServices,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT
	|	RetailReportSerialNumbers.ConnectionKey AS ConnectionKey,
	|	RetailReportSerialNumbers.SerialNumber AS SerialNumber
	...
	// Inventory by CCD
	SmallBusinessServer.GenerateTableInventoryByCCD(DocumentRefReportOnRetailSales, StructureAdditionalProperties);
	...
	If StructureTemporaryTables.RegisterRecordsInventoryInWarehousesChange
	 ...
	 OR StructureTemporaryTables.RegisterRecordsSerialNumbersChange Then
		...
		If Not ResultsArray[4].IsEmpty() Then
			...
			SmallBusinessServer.ShowMessageAboutPostingToInventoryByCCDRegisterErrors(DocumentRetailReport, QueryResultSelection, Cancel);
		EndIf;
...
Function CashCRSessionOpen(CashCR, ErrorDescription = "") Export
	...
	If StructureStateCashCRSession.CashCRSessionStatus = Enums.CashCRSessionStatuses.IsOpen Then
		...
		Else
			...
			ErrorDescription = NStr("en='More than 24 hours have passed since the register shift was opened. Close the register shift.';ru='С момента открытия кассовой смены истекло более 24 часов. Необходимо выполнить закрытие кассовой смены.';vi='Đã quá 24 giờ kể từ khi mở phiên làm việc lần trước. Cần đóng phiên thu ngân.'");
	...
	Else
		...
		Else
			CompletedSuccessfully = False;
			ErrorDescription = NStr("en='Check settings of the retail warehouse and cash register.';ru='Проверьте настройки розничного склада и кассы ККМ.';vi='Hãy kiểm tra tùy chỉnh kho bán lẻ và quầy thu ngân.'");
		EndIf;
...
"Закрытие кассовой смены не выполнено.';vi='Khi lập báo cáo bán lẻ đã xảy ra lỗi."
"Chưa thực hiện đóng phiên thu ngân.'"
		);
...
"Дополнительное"
"описание: %ДополнительноеОписание%';vi='Khi xóa bỏ phiếu tính tiền đang hoãn đã xảy ra lỗi."
"Mô tả"
...
"Дополнительное"
"описание: %ДополнительноеОписание%';vi='Khi lưu trữ phiếu tính tiền đã xảy ra lỗi."
"Chưa thực hiện lưu trữ phiếu tính tiền."
"Mô tả"
"bổ sung: %AdditionalDetails%'"
		);
		ErrorDescription = StrReplace(ErrorDescription, "%AdditionalDetails%", ErrorInfo().Definition);
	...
	If Selection.Next() Then
		...
		ElsIf ValueIsFilled(Selection.CashCRSessionStatus) Then
			...
			If Selection.CashCRSessionEnd >= Date AND Selection.CashCRSessionStart <= Date Then
				SessionIsOpen = True;
			Else
				ErrorDescription = NStr("en='Shift is not opened';ru='Смена не открыта';vi='Chưa mở phiên làm việc'");
				SessionIsOpen = False;
			EndIf;
...
Function GeneratePrintFormOfReportAboutRetailSales(ObjectsArray, PrintObjects)
	...
	For Each ReceiptCR IN ObjectsArray Do
		...
		If VATAmount = 0 Then
			...
			DataStructure.Insert("VAT", NStr("en='Without tax (VAT)';vi='Không bao gồm thuế GTGT '"));
			DataStructure.Insert("VATAmount", "-");
			
		Else
			
			DataStructure.Insert("VAT", ?(Header.AmountIncludesVAT, NStr("en='Including VAT:';vi='Gồm thuế GTGT:';"), NStr("en='VAT Amount:';vi='Thuế GTGT:';")));
		...
		TemplateArea.Parameters.TotalRow = NStr("en='Total titles ';vi='Tổng số ';")
													...
													+ NStr("en=', in the amount of ';vi=', với số tiền';")
...
Procedure AddPrintCommands(PrintCommands) Export
	...
	PrintCommand.Presentation = NStr("en='Retail sales report';ru='Отчет о розничных продажах';vi='Bảng kê bán lẻ hàng hóa'");
