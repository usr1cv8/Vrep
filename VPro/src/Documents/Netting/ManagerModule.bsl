...
Procedure GenerateTableIncomeAndExpenses(DocumentRefNetting, StructureAdditionalProperties)
	...
	Query.SetParameter("PointInTime", New Boundary(StructureAdditionalProperties.ForPosting.PointInTime, BoundaryType.Including));
	Query.SetParameter("ExchangeDifference", NStr("en='Exchange rate difference';ru='Курсовая разница';vi='Chênh lệch tỷ giá'"));
	Query.SetParameter("DebtAdjustment", NStr("en='Debt adjustment';ru='Корректировка долга';vi='Điều chỉnh công nợ'"));
...
Procedure InitializeDocumentData(DocumentRefNetting, StructureAdditionalProperties) Export
	...
	Query.SetParameter("Ref", DocumentRefNetting);
	Query.SetParameter("PointInTime", New Boundary(StructureAdditionalProperties.ForPosting.PointInTime, BoundaryType.Including));
	Query.SetParameter("Netting", NStr("en='Setoff';ru='Взаимозачет';vi='Đối trừ'"));
	Query.SetParameter("Novation", NStr("en='Debt assignment';ru='Переуступка долга';vi='Nhượng lại công nợ'"));
	Query.SetParameter("DebtAdjustment", NStr("en='Debt adjustment';ru='Корректировка долга';vi='Điều chỉnh công nợ'"));
...
"Остаток задолженности покупателя: %RemainingDebtAmount% %CurrencyPresentation%.';vi='%CounterpartyPresentation% - số dư công nợ của người mua theo chứng từ hạch toán nhỏ hơn số tiền đã ghi giảm.."
"Số tiền ghi giảm: %SumCurOnWrite% %CurrencyPresentation%."
"Số nợ còn lại của khách hàng: %RemainingDebtAmount% %CurrencyPresentation%.'"
					);
				EndIf;
				If QueryResultSelection.SettlementsType = Enums.SettlementsTypes.Advance Then
					If QueryResultSelection.AmountOfOutstandingAdvances = 0 Then
						MessageText = NStr("en='%PresentationOfCounterparty% - perhaps the advances of the customer have not been received or they have been completely set off in the trade documents.';ru='%PresentationOfCounterparty% - возможно, авансов от покупателя не было или они уже полностью зачтены в товарных документах.';vi='%PresentationOfCounterparty% - có thể, chưa có khoản ứng trước của khách hàng hoặc đã khấu trừ hết khoản ứng trước đó trong các chứng tù hàng hóa.'"
						);
					Else
...
"Остаток незачтенных авансов: %UnpaidAdvancesAmount% %CurrencyPresentation%.';vi='%CounterpartyPresentation% - khoản ứng trước đã nhận từ khách hàng đã được khấu trừ một phần trong các chứng từ hàng hóa."
"Số còn lại của khoản ứng trước chưa được khấu trừ: %UnpaidAdvancesAmount% %CurrencyPresentation%.'"
						);
...
"Списываемая сумма: %SumCurOnWrite% %CurrencyPresentation%."
"Остаток задолженности перед поставщиком: %SumCurOnWrite% %CurrencyPresentation%.';vi='%CounterpartyPresentation% - số dư công nợ với nhà cung cấp theo chứng từ hạch toán nhỏ hơn số tiền đã ghi giảm."
"Số tiền ghi giảm: %SumCurOnWrite% %CurrencyPresentation%."
"Công nợ còn lại với nhà cung cấp: %SumCurOnWrite% %CurrencyPresentation%."
"'"
					);
				EndIf;
				If QueryResultSelection.SettlementsType = Enums.SettlementsTypes.Advance Then
					If QueryResultSelection.AmountOfOutstandingAdvances = 0 Then
						MessageText = NStr("en=""%CounterpartyPresentation% - perhaps the vendor didn't get the advances or they have been completely set off in the trade documents ."";ru='%PresentationOfCounterparty% - возможно, авансов поставщику не было или они уже полностью зачтены в товарных документах.';vi='%PresentationOfCounterparty% - có thể là không có khoản ứng trước cho nhà cung cấp hoặc đã khấu trừ toàn bộ khoản ứng trước trong các chứng từ bán hàng.'"
						);
					Else
...
"Остаток незачтенных авансов: %RemainingDebtAmount% %ВалютаПредставление%.';vi='%CounterpartyPresentation% - Khoản ứng trước cho nhà cung cấp đã được khấu trừ một phần trong các chứng từ hàng hóa."
"Số còn lại của khoản tạm ứng chưa được khấu trừ: %RemainingDebtAmount%"
