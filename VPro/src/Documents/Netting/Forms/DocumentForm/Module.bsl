...
Procedure SetVisibleAndEnabled()
	...
	If Object.OperationKind = PredefinedValue("Enum.OperationKindsNetting.Netting") Then
		...
		Items.CounterpartySource.Title = NStr("en='Customer';ru='Клиент';vi='Khách hàng'");
		Items.CounterpartyRecipient.Title = NStr("en='Vendor';ru='Поставщик';vi='Nhà cung cấp'");
	...
	ElsIf Object.OperationKind = PredefinedValue("Enum.OperationKindsNetting.CustomerDebtAssignment") Then
		...
		Items.CounterpartyRecipient.Visible = True;
		Items.CounterpartySource.Title = NStr("en='Customer';ru='Клиент';vi='Khách hàng'");
		Items.CounterpartyRecipient.Title = NStr("en='Customer-recipient';ru='Клиент-получатель';vi='Khách hàng-người nhận'");
		Items.Pages.PagesRepresentation = FormPagesRepresentation.TabsOnTop;
	...
	ElsIf Object.OperationKind = PredefinedValue("Enum.OperationKindsNetting.DebtAssignmentToVendor") Then
		...
		Items.CounterpartySource.Visible = True;
		Items.CounterpartyRecipient.Visible = True;
		Items.CounterpartySource.Title = NStr("en='Vendor';ru='Поставщик';vi='Nhà cung cấp'");
		Items.CounterpartyRecipient.Title = NStr("en='Vendor-recipient';ru='Поставщик-получатель';vi='Nhà cung cấp-người nhận'");
		Items.Pages.PagesRepresentation = FormPagesRepresentation.TabsOnTop;
...
""
"Продолжить запись документа?';vi='Chứng từ lập ở nhiều dạng tiền tệ sẽ không được đưa vào ""Kế toán doanh nghiệp""."
""
"Tiếp tục ghi lại chứng từ?'");
...
""
"Продолжить запись документа?';vi='Chứng từ mà có số tiền ứng trước trong phần bảng"
"""Hạch toán với khách hàng"" không tương ứng với số tiền ứng trước trong phần bảng ""Hạch toán với nhà cung cấp"" không được chuyển sang ""Kế toán doanh nghiệp""."
""
"Tiếp tục ghi lại chứng từ?'");
...
Procedure PickAccountsReceivable(Command)
	...
	If Not ValueIsFilled(Object.CounterpartySource) Then
		ShowMessageBox(, NStr("en='Specify the customer first.';ru='Укажите вначале покупателя!';vi='Trước tiên, hãy chỉ ra khách hàng!'"));
		Return;
	EndIf;
...
Procedure PickVendorSettlements(Command)
	...
	If Not ValueIsFilled(Object.Counterparty) Then
		ShowMessageBox(, NStr("en='Specify the supplier first.';ru='Укажите вначале поставщика!';vi='Trước tiên, hãy chỉ ra nhà cung cấp!'"));
		Return;
	EndIf;
...
Procedure FillByDocumentBase(Command)
	...
	If Not ValueIsFilled(Object.BasisDocument) Then
		...
		Message.Text			= NStr("en='Select basis document.';ru='Выберите документ основание.';vi='Hãy chọn chứng từ cơ sở.'");
	...
	QuestionText 	= NStr("en='Document will be cleared and filled in according to the basis document. Continue?';ru='Документ будит очищен и заполнен по документу-основанию. Продолжить?';vi='Chứng từ sẽ được xóa và điền theo chứng từ-cơ sở. Tiếp tục?'");
	Response = Undefined;
