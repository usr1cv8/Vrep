...
Procedure FillByCounterpartyContract(FillingData) Export
	...
	|	ISNULL(CounterpartyContracts.PriceKind.PriceIncludesVAT, FALSE) AS AmountIncludesVAT,
	|	CounterpartyContracts.DiscountMarkupKind AS DiscountMarkupKind,
	|	CounterpartyContracts.IsServiceContract AS IsServiceContract,
	...
	FillPropertyValues(ThisObject, HeaderSelection);
	...
	If HeaderSelection.IsServiceContract Then
		Documents.InvoiceForPayment.FillBillingDetails(ThisObject);
	EndIf;
	
EndProcedure // ЗаполнитьПоДоговоруКонтрагента()
...
Procedure FillingHandler(FillingData) Export
	...
	If Contract.IsServiceContract Then
		Documents.InvoiceForPayment.FillBillingDetails(ThisObject);
	EndIf;
...
Procedure Filling(FillingData, StandardProcessing) Export
	...
	FillingStrategy[Type("DocumentRef.CustomerOrder")] = "FillingHandler";
	FillingStrategy[Type("DocumentRef.AcceptanceCertificate")] = "FillingHandler";
	FillingStrategy[Type("DocumentRef.ProcessingReport")] = "FillingHandler";
	FillingStrategy[Type("DocumentRef.CustomerInvoice")] = "FillingHandler";
	FillingStrategy[Type("DocumentRef.Event")] = "FillingHandler";
	FillingStrategy[Type("CatalogRef.Counterparties")] = "FillingHandler";
	FillingStrategy[Type("CatalogRef.CounterpartyContracts")] = "FillByCounterpartyContract";
	FillingStrategy[Type("Structure")] = "FillingHandler";
...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	// Billing
	If GetFunctionalOption("UseBilling") And Contract.IsServiceContract Then
		For Each Str In Inventory Do
			If Not SmallBusinessServer.SoldProductsAndServicesByServiceContractAllowed(Contract, Str.ProductsAndServices, Str.CHARACTERISTIC) Then
				CommonUseClientServer.MessageToUser(
					NStr("en='It is prohibited to post unplanned products/services by the current service contract!';ru='Запрещено проводить незапланированные запасы/услуги по текущему договору обслуживания!';vi='Cấm ghi nhận hàng hóa/dịch vụ ngoài kế hoạch theo hợp đồng dịch vụ hiện tại!'"),
					Contract.ServiceContractTariffPlan,
					CommonUseClientServer.PathToTabularSection("WorksAndServices", Str.LineNumber, "ProductsAndServices"),,
					Cancel
				);
			EndIf;
		EndDo;
