...
Function GetDataCounterpartyOnChange(Date, DocumentCurrency, Counterparty, Company)
	...
	// Billing
	If IsServiceContract And Not ContractByDefault.IsServiceContract
		Or Not IsServiceContract And ContractByDefault.IsServiceContract Then
		
		IsServiceContract = ContractByDefault.IsServiceContract;
...
Function GetDataContractOnChange(Date, DocumentCurrency, Contract)
	...
	// Billing
	If IsServiceContract And Not Contract.IsServiceContract
		Or Not IsServiceContract And Contract.IsServiceContract Then
		
		IsServiceContract = Contract.IsServiceContract;
		UpdateBillingDetails();
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	IsServiceContract		= Object.Contract.IsServiceContract;
	...
	If GetFunctionalOption("UseBilling") Then
		If Constants.BillingKeepExpensesAccountingByServiceContracts.Get() Then
			...
			DetailsBillingSetConditionalAppearance();
			InitializeBillingDetails();
		Else
			// В ТЧ РасшифровкаБиллинга могут быть только позиции номенклатуры.
			Items.BillingDetailsServiceContractObject.TypeRestriction = New TypeDescription("CatalogRef.ProductsAndServices");
			Items.BillingDetailsServiceContractObject.Title = NStr("en='Products and services';ru='Номенклатура';vi='Mặt hàng'");
		EndIf;
	EndIf;
...
Procedure FillExecute()
	...
	ShowQueryBox(New NotifyDescription("FillInExecuteEnd", ThisObject), NStr("en='The  document will be fully filled out according to the ""Basis"". Continue?';ru='Документ будет полностью перезаполнен по ""Основанию""! Продолжить?';vi='Chứng từ sẽ được điền lại toàn bộ theo ""Cơ sở""! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
...
Procedure AddAdditionalAttribute(Command)
	...
	FormParameters = New Structure;
	FormParameters.Insert("CurrentSetOfProperties", PredefinedValue("Catalog.AdditionalAttributesAndInformationSets.Document_InvoiceForPayment"));
...
Procedure InventoryBeforeAddRow(Item, Cancel, Clone, Parent, IsFolder, Parameter)
	...
	Cancel = IsServiceContract;
	
EndProcedure

&AtClient
Procedure InventoryBeforeRowChange(Item, Cancel)
...
#Region FormTableItemsEventHandlersBillingDetails

&AtClient
Procedure RefreshFormFooter()
	
//	DeliveryCostWithVAT = Object.ShippingCost + ?(Object.AmountIncludesVAT, 0, Object.DeliveryVATAmount);
//	//Ручные скидки
//	DiscountsMarkupsClientServer.CalculateAmountAndDiscountPercent(ThisObject);
//	//Конец Ручные скидки
//	TotalAmount = Object.Inventory.Total("Total") + DeliveryCostWithVAT;
//	TotalVATAmount = Object.Inventory.Total("VATAmount") + Object.DeliveryVATAmount;
	
EndProcedure

&AtClient
Procedure BillingDetailsOnChange(Item)
...
&AtClient
Procedure BillingDetailsServiceContractObjectOnChange(Item)
	
	TabularSectionRow = Items.BillingDetails.CurrentData;
	
	FillingData = New Structure(
		"ServiceContractObject,CHARACTERISTIC,PresentationInAccount,Quantity,MeasurementUnit,Price,Amount",
		TabularSectionRow.ServiceContractObject,TabularSectionRow.CHARACTERISTIC
	);
	BillingDetailsOnChangeServiceContractObject(FillingData);
	FillPropertyValues(TabularSectionRow, FillingData);
	BillingDetailsCalculateAmountInTabularSectionRow(TabularSectionRow);
	TabularSectionRow.ThisIsChartOfAccounts = Not TypeOf(TabularSectionRow.ServiceContractObject) = Type("CatalogRef.ProductsAndServices");
	
	RefillDataByBillingDetails();
	RefreshFormFooter();
	
	// Характеристики
	TabularSectionRow.UseCharacteristics = False;
	TabularSectionRow.CheckCharacteristicFilling = False;
	TabularSectionRow.FillingCharacteristicChecked = True;
	
	If TypeOf(TabularSectionRow.ServiceContractObject) = Type("CatalogRef.ProductsAndServices")
		Then	
		StructureData = New Structure();
		StructureData.Insert("Company", 	 Object.Company);
		StructureData.Insert("ProcessingDate",	 Object.Date);
		...
		StructureData.Insert("DocumentCurrency",	 Object.DocumentCurrency);
		StructureData.Insert("AmountIncludesVAT", Object.AmountIncludesVAT);
		StructureData.Insert("ProductsAndServices",	 TabularSectionRow.ServiceContractObject);
		StructureData.Insert("CHARACTERISTIC",	 TabularSectionRow.CHARACTERISTIC);
		StructureData.Insert("Factor",		 1);
		StructureData.Insert("VATTaxation", Object.VATTaxation);
		StructureData.Insert("DiscountMarkupKind", Object.DiscountMarkupKind);
		...
		TabularSectionRow.UseCharacteristics = StructureData.UseCharacteristics;
		TabularSectionRow.CheckCharacteristicFilling = StructureData.CheckCharacteristicFilling;
		
		If StructureData.UseCharacteristics
			Then
			TabularSectionRow.CHARACTERISTIC = StructureData.CHARACTERISTIC;
		EndIf;
	EndIf;
...
&AtClient
Procedure BillingDetailsCHARACTERISTICOnChange(Item)
...
&AtClient
Procedure BillingDetailsQuantityOnChange(Item)
...
&AtClient
Procedure BillingDetailsMeasurementUnitChoiceProcessing(Item, SelectedValue, StandardProcessing)
	
	TabularSectionRow = Items.BillingDetails.CurrentData;
	
	If TabularSectionRow.MeasurementUnit = SelectedValue 
		Or TabularSectionRow.Price = 0 Then
		
		Return;
	EndIf;
	
	FillingData = New Structure("Quantity,Price,Amount", TabularSectionRow.Quantity, TabularSectionRow.Price, TabularSectionRow.Amount);
	BillingDetailsOnChangeMeasurementUnit(TabularSectionRow.MeasurementUnit, SelectedValue, FillingData);
...
&AtClient
Procedure BillingDetailsPriceOnChange(Item)
...
&AtClient
Procedure BillingDetailsAmountOnChange(Item)
...
&AtClient
Procedure BillingDetailsPresentationInAccountOnChange(Item)
	
	CurrentRow = Items.BillingDetails.CurrentData;
	CurrentRow.Content = "";
...
&AtClient
Procedure BillingDetailsContentOnChange(Item)
...
&AtServer
Procedure DetailsBillingSetConditionalAppearance()
	...
	Filter.Use  = True;
	Filter.LeftValue  = New DataCompositionField(Items.BillingDetailsThisIsChartOfAccounts.DataPath);
	Filter.RightValue = True;
	
	MadeOutField                = NewConditionalAppearance.Fields.Items.Add();
	MadeOutField.Field           = New DataCompositionField(Items.BillingDetailsCHARACTERISTIC.Name);
	MadeOutField.Use  = True;
	
	MadeOutField                = NewConditionalAppearance.Fields.Items.Add();
	MadeOutField.Field           = New DataCompositionField(Items.BillingDetailsMeasurementUnit.Name);
	MadeOutField.Use  = True;
	...
	Filter.Use  = True;
	Filter.LeftValue  = New DataCompositionField(Items.BillingDetailsServiceContractObject.DataPath);
	
	MadeOutField                = NewConditionalAppearance.Fields.Items.Add();
	MadeOutField.Field           = New DataCompositionField(Items.BillingDetailsPresentationInAccount.Name);
	MadeOutField.Use  = True;
...
&AtServer
Procedure InitializeBillingDetails()
	
	For Each Str In Object.BillingDetails Do
		
		Str.ThisIsChartOfAccounts = TypeOf(Str.ServiceContractObject) = Type("ChartOfAccountsRef.Managerial");
...
//
&AtServer
Procedure SetBillingDetailsVisibility()
	
	Items.GroupInventoryCommandBar.Enabled = Not IsServiceContract;
	Items.InventoryContextMenuGroupChange.Enabled = Not IsServiceContract;
//	Items.GroupInventoryRowsChange.Enabled = Not IsServiceContract;
	
	Items.GroupBillingDetails.Visible = IsServiceContract;
	
EndProcedure

&AtServer
Procedure RefillDataByBillingDetails()
	
	Documents.InvoiceForPayment.RefillInventoryByBillingDetails(Object);
	
EndProcedure

&AtServer
Procedure UpdateBillingDetails()
	
	If IsServiceContract Then
		Documents.InvoiceForPayment.FillBillingDetails(Object);
	Else
		Object.BillingDetails.Clear();
	EndIf;
...
&AtServer
Procedure BillingDetailsOnChangeServiceContractObject(FillingData)
	
	StructureData = Undefined;
	If TypeOf(FillingData.ServiceContractObject) = Type("CatalogRef.ProductsAndServices") Then
		StructureData = New Structure();
		...
		StructureData.Insert("DocumentCurrency",  Object.DocumentCurrency);
		StructureData.Insert("AmountIncludesVAT", Object.AmountIncludesVAT);
		StructureData.Insert("ProductsAndServices",     FillingData.ServiceContractObject);
		StructureData.Insert("CHARACTERISTIC",   FillingData.CHARACTERISTIC);
		StructureData.Insert("Factor",      1);
		StructureData.Insert("VATTaxation", Object.VATTaxation);
		StructureData.Insert("DiscountMarkupKind", Object.DiscountMarkupKind);
	...
	FillingData.Quantity       = 1;
	FillingData.MeasurementUnit = ?(StructureData = Undefined, Undefined, StructureData.MeasurementUnit);
	FillingData.Price             = ?(StructureData = Undefined, 0, StructureData.Price);
	FillingData.PresentationInAccount = 
		Catalogs.ServiceContractsTariffPlans.PresentationInAccount(Object.Contract.ServiceContractTariffPlan, FillingData.ServiceContractObject);
	
	ParameterArray = New Array;
	If TypeOf(FillingData.ServiceContractObject) = Type("ChartOfAccountsRef.Managerial") Then
		NewParameter = New ChoiceParameter("Filter.ProductsAndServicesType", Enums.ProductsAndServicesTypes.Service);
		ParameterArray.Add(NewParameter);
	EndIf;
	
	Items.BillingDetailsPresentationInAccount.ChoiceParameters = New FixedArray(ParameterArray);
	
EndProcedure

&AtServer
Procedure BillingDetailsOnChangeMeasurementUnit(PreviousValue, SelectedValue, FillingData)
	
	CurrentFactor = 0;
	If TypeOf(PreviousValue) = Type("CatalogRef.UOMClassifier") Then
		CurrentFactor = 1;
	EndIf;
	
	Factor = 0;
	If TypeOf(SelectedValue) = Type("CatalogRef.UOMClassifier") Then
		Factor = 1;
	EndIf;
	
	If CurrentFactor = 0 And Factor = 0 Then
		StructureData = GetDataMeasurementUnitOnChange(PreviousValue, SelectedValue);
	ElsIf CurrentFactor = 0 Then
		StructureData = GetDataMeasurementUnitOnChange(PreviousValue);
	ElsIf Factor = 0 Then
		StructureData = GetDataMeasurementUnitOnChange(Undefined, SelectedValue);
	ElsIf CurrentFactor = 1 And Factor = 1 Then
		StructureData = New Structure("CurrentFactor, Factor", 1, 1);
	EndIf;
	
	If StructureData.CurrentFactor <> 0 Then
		FillingData.Price = FillingData.Price * StructureData.Factor / StructureData.CurrentFactor;
		FillingData.Amount = FillingData.Quantity * FillingData.Price;
	EndIf;
	
EndProcedure

&AtClient
Procedure BillingDetailsCalculateAmountInTabularSectionRow(FillingData)
