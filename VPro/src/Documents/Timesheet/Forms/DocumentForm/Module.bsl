...
Procedure Fill(Command)
	...
	If Object.WorkedTimeByDays.Count() > 0
	 OR Object.WorkedTimePerPeriod.Count() > 0 Then
		...
		ShowQueryBox(New NotifyDescription("FillEnd", ThisObject), NStr("en='Tabular section will be cleared. Continue?';ru='Табличная часть будет очищена! Продолжить выполнение операции?';vi='Phần bảng sẽ bị xóa! Tiếp tục thực hiện thao tác?'"), Mode, 0);
	Else
		FillTimesheet();
	EndIf;
