...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(Object.Ref) Then
		...
		If Object.TypeOfAccount = PredefinedValue("Enum.GLAccountsTypes.DepreciationFixedAssets")
		 ...
		 OR Object.TypeOfAccount = PredefinedValue("Enum.GLAccountsTypes.Loans")
...
Procedure FormManagement()
	...
	If Object.TypeOfAccount = Enums.GLAccountsTypes.IndirectExpenses Then
		...
		Items.ClosingAccount.ToolTip = ?(
			UseBudgeting,
			NStr("en='Auto closing account on month closing and budgeting';ru='Счет автоматического закрытия при закрытии месяца и бюджетировании';vi='Tài khoản tự động đóng khi đóng tháng và lập ngân sách'"),
			NStr("en='Auto closing account on month closing';ru='Счет автоматического закрытия при закрытии месяца';vi='Tài khoản đóng tự động khi đóng tháng'")
		);
		Items.MethodOfDistribution.ToolTip = NStr("en='Method of automatic allocation to the cost of released products on month-end closing';ru='Способ автоматического распределения на себестоимость выпущенной продукции при закрытии месяца';vi='Phương pháp tự động phân bổ cho giá thành thành phẩm khi đóng tháng'"
		);
	ElsIf Object.TypeOfAccount =  Enums.GLAccountsTypes.UnfinishedProduction Then
		...
		Items.MethodOfDistribution.ToolTip = NStr("en='Method of automatic allocation to the cost of released products at month-end closing for intangible costs';ru='Способ автоматического распределения на себестоимость выпущенной продукции при закрытии месяца для нематериальных затрат';vi='Phương pháp tự động phân bổ cho giá thành thành phẩm khi đóng tháng đối với chi phí vô hình'"
		);
	ElsIf (TypeOfAccount <>  Enums.GLAccountsTypes.OtherIncome
		...
		Items.MethodOfDistribution.ChoiceList.Add(Enums.CostingBases.DoNotDistribute, NStr("en='Direct allocation';vi='Phân bổ trực tiếp'"));
		Items.MethodOfDistribution.ToolTip = ?(
			UseBudgeting,
			NStr("en='Method of automatic allocation to the financial result on month-end closing and budgeting';ru='Способ автоматического распределения на финансовый результат при закрытии месяца и бюджетировании';vi='Phương pháp tự động phân bổ theo kết quả kinh doanh khi đóng tháng và quản lý ngân sách'"),
			NStr("en='Method of automatic allocation to the financial result on month-end closing';ru='Способ автоматического распределения на финансовый результат при закрытии месяца';vi='Phương pháp phân bổ tự động đối với kết quả tài chính khi đóng tháng"
"'")
		);
	Else
