		...
		While SelectionByOrder.Next() Do
			...
			ElsIf SelectionByOrder.Order = 2 Then
				...
				While SelectionOfQueryResult.Next() Do
					...
					Else
						TemplateArea.Parameters.Description = NStr("en='<Name is not specified>';vi='<Tên không được chỉ định>';");
					EndIf;
					...
					Else
						TemplateArea.Parameters.Definition = NStr("en='<Description is not specified>';vi='<Mô tả không được chỉ định>';");
					EndIf;
					
					TemplateArea.Parameters.Counterparty = ?(ValueIsFilled(SelectionOfQueryResult.CustomerPresentation), SelectionOfQueryResult.CustomerPresentation, NStr("en='<Not specified>';vi='<Không được chỉ định>';"));
