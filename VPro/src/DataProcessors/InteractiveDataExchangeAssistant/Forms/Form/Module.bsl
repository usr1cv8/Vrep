...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ElsIf Parameters.Property("CodeOfInfobaseNode", CodeOfInfobaseNode) Then
		...
		If Object.InfobaseNode.IsEmpty() Then
			...
			DataExchangeServer.ShowMessageAboutError(NStr("en='Data exchange setting is not found.';ru='Настройка обмена данными не найдена.';vi='Chưa tìm thấy tùy chỉnh trao đổi dữ liệu.'"), Cancel);
			Return;
	...
	Else
		...
		DataExchangeServer.ShowMessageAboutError(NStr("en='Wizard cannot be opened.';ru='Непосредственное открытие помощника не предусмотрено.';vi='Mở trực tiếp trợ giúp không được xem xét.'"), Cancel);
		Return;
	...
	If Not DataExchangeReUse.IsUniversalDataExchangeNode(Object.InfobaseNode) Then
		...
		DataExchangeServer.ShowMessageAboutError(
			NStr("en='Data exchange execution with the setting is not required for the selected node.';ru='Для выбранного узла выполнение обмена данными с настройкой не предусмотрено.';vi='Đối với nút được chọn thực hiện trao đổi dữ liệu có tùy chỉnh không được xem xét.'"), Cancel);
		Return;
	...
	If NodesArray.Find(Object.InfobaseNode) = Undefined Then
		...
		CommonUseClientServer.MessageToUser(NStr("en='Data mapping is not required for the selected node.';ru='Для выбранного узла сопоставление данных не предусмотрено.';vi='Đối với nút được chọn so sánh dữ liệu không được xem xét.'"),,,, Cancel);
		Return;
...
Procedure OnOpen(Cancel)
	...
	If ChangeTitle Then
		Items.PageTitle.Title = NStr("en='Connection parameters';ru='Параметры подключения';vi='Tham số kết nối'");
	EndIf;
...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	If CurrentPage = Items.BeginningPage Then
		ConfirmationText = NStr("en='Cancel data synchronization?';ru='Отменить синхронизацию данных?';vi='Hủy bỏ đồng bộ hóa dữ liệu?'");
		
	Else
		ConfirmationText = NStr("en='Stop data synchronization?';ru='Прервать синхронизацию данных?';vi='Ngắt đồng bộ hóa dữ liệu?'");
...
Procedure ExecuteDataImportForRow(Command)
	...
	If SelectedRows.Count() = 0 Then
		NString = NStr("en='Select a table name in the statistical information field.';ru='Выберите имя таблицы в поле статистической информации.';vi='Hãy chọn tên bảng trong trường thông tin thống kê.'");
...
"При загрузке данных будут созданы дубли несопоставленных объектов. Продолжить?';vi='Có các đối tượng không tương ứng."
"Khi kết nhập dữ liệu, sẽ tạo ra trùng thêm các đối tượng không tương ứng. Tiếp tục?'");
...
Procedure OpenMappingForm(Command)
	...
	If Not CurrentData.UsePreview Then
		ShowMessageBox(, NStr("en='Cannot map objects for the data type.';ru='Для типа данных нельзя выполнить сопоставление объектов.';vi='Đối với kiểu dữ liệu không nên tiến hành so sánh đối tượng.'"));
		Return;
	EndIf;
...
Procedure ExportAdditionGeneralFilterClearing(Command)
	...
	QuestionText   = NStr("en='Clear common filter?';ru='Очистить общий отбор?';vi='Xóa lọc chung?'");
...
Procedure ExportAdditionCleaningDetailedFilter(Command)
	HeaderText = NStr("en='Confirmation';ru='Подтверждение';vi='Xác nhận'");
	QuestionText   = NStr("en='Clear detailed filter?';ru='Очистить детальный отбор?';vi='Xóa lọc chi tiết?'");
...
Procedure ExportAdditionFilterHistory(Command)
	...
	Text = NStr("en='Saving the current configuration...';ru='Сохранить текущую настройку...';vi='Lưu tùy chỉnh hiện tại...'");
...
Procedure ExportAdditionFilterHistorySelectionFromMenu(Val SelectedItem, Val AdditionalParameters) Export
	...
	If TypeOf(SettingRepresentation)=Type("String") Then
		...
		NStr("en='Restore settings ""%1""?';ru='Восстановить настройки ""%1""?';vi='Phục hồi tùy chỉnh ""%1""?'"), SettingRepresentation);
...
Procedure ImportDataForStringQuestionContinuation(Val SelectedRows) 
	...
	Status(NStr("en='Data import in progress...';ru='Выполняется загрузка данных...';vi='Đang thực hiện kết nhập dữ liệu...'"));
...
"Chuyển đến nhật ký sự kiện?'");
	...
	Status(NStr("en='Data import is complete.';ru='Загрузка данных завершена.';vi='Đã hoàn thành kết nhập dữ liệu.'"));
EndProcedure
...
Procedure OpenNodeInformationExchangeDirectory()
	...
	If IsBlankString(DirectoryName) Then
		ShowMessageBox(, NStr("en='Information exchange directory is not specified.';ru='Не задан каталог обмена информацией.';vi='Chưa chỉ ra thư mục trao đổi thông tin.'"));
		Return;
	EndIf;
...
Procedure AfterWorksWithFilesExpansionCheck(Result, AdditionalParameters) Export
	...
	Else
		WarningText = NStr("en='File operation extension is not installed. Directory cannot be opened.';ru='Расширение для работы с файлами не установлено, открытие каталога не возможно.';vi='Chưa thiết lập phần mở rộng để làm việc với tệp, không thể mở thư mục.'");
...
Procedure AfterFileExistenceCheck(Exist, AdditionalParameters) Export
	...
	Else
		WarningText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Directory ""%1"" does not exist or cannot be accessed.';ru='Каталог ""%1"" не существует или к нему нет доступа.';vi='Thư mục ""%1"" không tồn tại hoặc không thể truy cập đến thư mục đó.'"),
...
Procedure DetermineIsDirectory(IsDirectory, AdditionalParameters) Export
	...
	Else
		WarningText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='""%1"" is a file, not a catalog.';ru='""%1"" является файлом, а не каталогом.';vi='""%1"" là tệp mà không phải là thư mục.'"),
			AdditionalParameters.DirectoryName
		);
		ShowMessageBox(, WarningText);
	EndIf;
...
Procedure DeleteTemporaryDirectoryOfExchangeMessages(TempDirectoryName)
	...
	If Not IsBlankString(TempDirectoryName) Then
		...
		Try
			...
			TempDirectoryName = "";
		Except
			WriteLogEvent(NStr("en='Data exchange.Temporary file deletion';ru='Обмен данными.Удаление временных файлов';vi='Trao đổi dữ liệu.Xóa tệp tạm thời'", CommonUseClientServer.MainLanguageCode()),
...
Procedure RefreshTransportSettingsPages()
	...
	Else
		...
		TransportChoiceList.Add(Undefined, NStr("en='Connection is not configured';ru='подключение не настроено';vi='chưa thiết lập kết nối'") );
...
Procedure RefreshRepresentationOfDataExchangeItemsState()
	...
	If Items.PagesOfDataImportStatus.CurrentPage=Items.ImportStateUndefined Then
		Items.GoToDataImportEventsEventLogMonitor.Title = NStr("en='Data has not been imported';ru='Загрузка данных не произведена';vi='Chưa thực hiện kết nhập dữ liệu'");
	Else
	...
	If Items.PagesOfDataDumpStatus.CurrentPage=Items.ExportStateUndefined Then
		Items.GoToDataExportEventsEventLogMonitor.Title = NStr("en='Data is not exported';ru='Выгрузка данных не произведена';vi='Chưa tiến hành kết xuất dữ liệu'");
	Else
...
Function Attachable_BeginningPage_OnGoingNext(Cancel)
	...
	If Object.InfobaseNode.IsEmpty() Then
		...
		NString = NStr("en='Specify an infobase node';ru='Укажите узел информационной базы';vi='Hãy chỉ ra nút cơ sở thông tin'");
	...
	ElsIf Object.ExchangeMessageTransportKind.IsEmpty() Then
		...
		NString = NStr("en='Specify connection option';ru='Укажите вариант подключения';vi='Hãy chỉ ra phương án kết nối'");
	...
	ElsIf ExchangeOverWebService AND IsBlankString(WSPassword) Then
		...
		NString = NStr("en='Password is not specified.';ru='Не указан пароль.';vi='Chưa chỉ ra mật khẩu.'");
...
Procedure TransportMessageExchange()
	...
	Except
		...
		WriteErrorInEventLogMonitor(DetailErrorDescription(ErrorInfo()),
			NStr("en='Interactive data exchange wizard.Exchange message transport';ru='Помощник интерактивного обмена данными.Транспорт сообщения обмена';vi='Trợ giúp trao đổi trực tác dữ liệu. Truyền thông báo dữ liệu'")
		);
		Return;
	EndTry;
...
&AtClient
Function Attachable_DataAnalysisWaitPageLongOperation_LongOperationProcessing(Cancel, GoToNext)
...
&AtClient
Function Attachable_DataAnalysisWaitPageLongOperationEnd_LongOperationProcessing(Cancel, GoToNext)
...
Procedure DataAnalysis()
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Analysis of exchange message data';ru='Анализ данных сообщения обмена';vi='Phân tích dữ liệu thông báo trao đổi'")
		);
	...
	Except
		...
		IgnoreDataGet = True;
		WriteErrorInEventLogMonitor(DetailErrorDescription(ErrorInfo()),
			NStr("en='Interactive data exchange wizard.Data analysis';ru='Помощник интерактивного обмена данными.Анализ данных';vi='Trợ giúp trao đổi trực tác dữ liệu. Phân tích dữ liệu'"));
...
Function Attachable_StatisticsPage_OnGoingNext(Cancel)
	...
	Buttons.Add(DialogReturnCode.Yes,  NStr("en='Continue';ru='Продолжить';vi='Tiếp tục'"));
...
Procedure DataImport()
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Import data from the exchange message';ru='Загрузка данных из сообщения обмена';vi='Kết nhập dữ liệu từ thông báo trao đổi'"));
	...
	Except
		...
		DeleteTemporaryDirectoryOfExchangeMessages(Object.TemporaryExchangeMessagesDirectoryName);
		WriteErrorInEventLogMonitor(DetailErrorDescription(ErrorInfo()),
			NStr("en='Interactive data exchange wizard.Data import';ru='Помощник интерактивного обмена данными.Загрузка данных';vi='Trợ giúp trao đổi trực tác dữ liệu. Kết nhập dữ liệu'"));
		Return;
	EndTry;
...
Procedure SetCommonFilterAdditionDescription()
	...
	If FilterAbsent Then
		Text = NStr("en='All documents';ru='Все документы';vi='Tất cả chứng từ'");
	EndIf;
...
Procedure SetAdditionDescriptionInDetails()
	...
	FilterAbsent = IsBlankString(Text);
	If FilterAbsent Then
		Text = NStr("en='Additional data is not selected';ru='Дополнительные данные не выбраны';vi='Chưa chọn dữ liệu bổ sung'");
	EndIf;
...
Procedure GetDataThroughUsualChannelsLinks()
	...
	// Begin
	GoToTableNewRow(1, "BeginningPage", "NavigationPageStart",, "BeginningPage_OnGoingNext");
	
	// Receive data (exchange message transport).
	TransitionsTableNewStringLongOperation(2, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisWaitPage_LongOperationProcessing");
	TransitionsTableNewStringLongOperation(3, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisWaitPageLongOperation_LongOperationProcessing");
	TransitionsTableNewStringLongOperation(4, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisWaitPageLongOperationEnd_LongOperationProcessing");
	...
	TransitionsTableNewStringLongOperation(7, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisEndLongOperation_LongOperationProcessing");
	
	// Data match by a user.
	GoToTableNewRow(8, "StatisticsPage", "NavigationPageContinuation", "StatisticsPage_OnOpen", "StatisticsPage_OnGoingNext");
	...
	TransitionsTableNewStringLongOperation(11, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataImportLongOperationEnd_LongOperationProcessing");
...
Procedure DataGetThroughExternalConnectionOrWebService()
	...
	// Receive data (exchange message transport).
	TransitionsTableNewStringLongOperation(3, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisWaitPage_LongOperationProcessing");
	TransitionsTableNewStringLongOperation(4, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisWaitPageLongOperation_LongOperationProcessing");
	TransitionsTableNewStringLongOperation(5, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisWaitPageLongOperationEnd_LongOperationProcessing");
	...
	TransitionsTableNewStringLongOperation(7, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisLongOperation_LongOperationProcessing");
	TransitionsTableNewStringLongOperation(8, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisLongOperationEnd_LongOperationProcessing");
	
	// Data match by a user.
	GoToTableNewRow(9, "StatisticsPage", "NavigationPageContinuation", "StatisticsPage_OnOpen", "StatisticsPage_OnGoingNext");
	...
	TransitionsTableNewStringLongOperation(12, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataImportLongOperationEnd_LongOperationProcessing");
...
Procedure SendDataThroughUsualChannelsLinks()
	...
	// Data export setting
	DataExportResult = "";
	GoToTableNewRow(2, "PageIsQuestionOfCompositionOfDischarge", "NavigationPageContinuation", "PageIsQuestionOfCompositionOfDischarge_OnOpen");
	
	// Data export
	TransitionsTableNewStringLongOperation(3, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataExport_LongOperationProcessing");
...
Procedure SendDataThroughExternalConnectionOrWebService()
	...
	// Data export setting
	DataExportResult = "";
	GoToTableNewRow(3, "PageIsQuestionOfCompositionOfDischarge",  "NavigationPageContinuation", "PageIsQuestionOfCompositionOfDischarge_OnOpen");
	
	// Data export
	TransitionsTableNewStringLongOperation(4, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataImport_LongOperationProcessing");
...
Procedure GetAndSendDataThroughUsualChannelsLinks()
	...
	// analysis Automatic data match.
	TransitionsTableNewStringLongOperation(5, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysis_LongOperationProcessing");
	TransitionsTableNewStringLongOperation(6, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisLongOperation_LongOperationProcessing");
	TransitionsTableNewStringLongOperation(7, "DataAnalysisWaitPage", "NavigationPageWait", True, "DataAnalysisLongOperationEnd_LongOperationProcessing");
	...
	// Data export setting
	DataExportResult = "";
	GoToTableNewRow(12, "PageIsQuestionOfCompositionOfDischarge",  "NavigationPageContinuation", "PageIsQuestionOfCompositionOfDischarge_OnOpen");
	
	// Data export
	TransitionsTableNewStringLongOperation(13, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataExportIdlePage_LongOperationProcessing");
	TransitionsTableNewStringLongOperation(14, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataExportIdlePageLongOperation_LongOperationProcessing");
	TransitionsTableNewStringLongOperation(15, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataExportWaitingPageLongOperationEnd_LongOperationProcessing");
...
Procedure GetAndSendDataThroughExternalConnectionOrWebService()
	...
	// Data export setting
	DataExportResult = "";
	GoToTableNewRow(13, "PageIsQuestionOfCompositionOfDischarge",  "NavigationPageContinuation", "PageIsQuestionOfCompositionOfDischarge_OnOpen");
	
	// Data export
	TransitionsTableNewStringLongOperation(14, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataExport_LongOperationProcessing");
	TransitionsTableNewStringLongOperation(15, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataExportLongOperation_LongOperationProcessing");
...
Procedure CompanyFilterClean(Command)
	...
	HeaderText = NStr("en='Confirmation';ru='Подтверждение';vi='Xác nhận'");
	QuestionText   = NStr("en='Clear filter by companies?';ru='Очистить отбор по организациям?';vi='Xóa lọc theo doanh nghiệp?'");
...
Procedure InitializeScriptTransfer(ExchangeOverConnectionToCorrespondent)
	...
	If ExchangeOverConnectionToCorrespondent Then
		...
		Else
			...
			Raise NStr("en='This data synchronization scenario is not supported.';ru='Заданный сценарий синхронизации данных не поддерживается.';vi='Chưa hỗ trợ kịch bản đồng bộ dữ liệu đã xác định.'");
...
Procedure ScriptExchangeOnlineSynchronisationThroughExternalConnectionOrWebService()
	...
	TransitionsTableNewStringLongOperation(11, "DataExportWaitPageSimplified", "NavigationPageWait", True, "DataImportLongOperationEnd_LongOperationProcessing");
	...
	// Data export
	TransitionsTableNewStringLongOperation(14, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataImportLongOperation_LongOperationProcessing");
...
Procedure ScriptExchangeOnlineSynchronisationThroughUsualChannelsLinks()
	...
	TransitionsTableNewStringLongOperation(10, "DataExportWaitPageSimplified", "NavigationPageWait",True, "DataImportLongOperationEnd_LongOperationProcessing");
	...
	// Data export
	TransitionsTableNewStringLongOperation(13, "WaitPageSynchronizationData", "NavigationPageWait", True, "DataImportLongOperation_LongOperationProcessing");
...
Procedure UpdateFilterByCompanies(AddressOfObject="")
	...
	If Not CompaniesSelected Then
		Text = NStr("en='Select companies ';ru='Выбрать организации ';vi='Chọn doanh nghiệp'");
	Else
		Text = NStr("en='All companies ';ru='Все организации ';vi='Tất cả doanh nghiệp'");
	EndIf;
...
Function FilterPresentation(Period, Filter)
	...
	ExportAdditionObject = FormAttributeToValue("ExportAddition");
	DetailsOfEmptySelection = NStr("en='All documents';ru='Все документы';vi='Tất cả chứng từ'");
