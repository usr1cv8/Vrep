...
Procedure EndSession(Command)
	...
	If SelectedRowsQuantity = 0 Then
		...
		ShowMessageBox(,NStr("en='Users for ending sessions are not selected.';ru='Не выбраны пользователи для завершения сеансов.';vi='Chưa chọn người sử dụng để kết thúc phiên làm việc.'"));
		Return;
	...
	If ClientWorkParameters.DataSeparationEnabled AND ClientWorkParameters.CanUseSeparatedData Then
		...
		If EndingSession = InfobaseSessionNumber Then
			ShowMessageBox(,NStr("en='It is impossible to end the current session. To exit the application, close the main application window.';ru='Невозможно завершить текущий сеанс. Для выхода из программы можно закрыть главное окно программы.';vi='Không thể kết thúc phiên làm việc này. Để thoát khỏi chương trình có thể đóng cửa sổ chính của chương trình.'"));
			Return;
		EndIf;
	...
	Else
		...
		If RequestInfobaseAdministrationParameters Then
			...
			FormTitle = NStr("en='End session';ru='Завершить сеанс';vi='Kết thúc phiên làm việc'");
...
"необходимо ввести параметры администрирования кластера серверов';vi='Để kết thúc phiên làm việc"
"cần nhập tham số quản trị cụm Server'");
...
Procedure OpenEventLogMonitor()
	...
	If SelectedRows.Count() = 0 Then
		ShowMessageBox(,NStr("en='Select users to view the event log.';ru='Выберите пользователей для просмотра журнала регистрации.';vi='Hãy chọn người sử dụng để xem nhật ký sự kiện.'"));
		Return;
	EndIf;
...
Procedure AfterSessionEnd(Result, AdditionalParameters) Export
	...
	If Result = DialogReturnCode.OK Then
		...
		If AdditionalParameters.Property("SessionNumbers")
			...
			NotificationText = NStr("en='Sessions %1 are ended.';ru='Сеансы %1 завершены.';vi='Phiên làm việc %1 đã kết thúc'");
			...
			NotificationText = StringFunctionsClientServer.SubstituteParametersInString(NotificationText,
				SessionNumbers);
			ShowUserNotification(NStr("en='End sessions';ru='Завершение сеансов';vi='Kết thúc phiên làm việc'"),, NotificationText);
			
		Else
			
			NotificationText = NStr("en='Session %1 is ended.';ru='Сеанс %1 завершен.';vi='Đã hoàn tất phiên làm việc %1.'");
			NotificationText = StringFunctionsClientServer.SubstituteParametersInString(NotificationText,
			AdditionalParameters.SessionNumbers);
			ShowUserNotification(NStr("en='End session';ru='Завершить сеанс';vi='Kết thúc phiên làm việc'"),, NotificationText);
