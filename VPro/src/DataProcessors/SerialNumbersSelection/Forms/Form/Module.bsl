...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If NOT Parameters.Property("Inventory") OR NOT ValueIsFilled(Parameters.Inventory.ProductsAndServices) Then
		MessageText = NStr("en='Products and services are not filled in.';ru='Не заполнена номенклатура!';vi='Chưa điền mặt hàng!'");
	ElsIf NOT Parameters.Inventory.ProductsAndServices.UseSerialNumbers Then
		MessageText = NStr("en='No account by serial numbers for this products!';ru='Для номенклатуры не ведется учет по серийным номерам!';vi='Đối với mặt hàng chưa tiến hành kế toán theo số sê-ri!'");
	EndIf;
	...
	If GetFunctionalOption("SerialNumbersBalanceControl") Then
		...
		If Parameters.Property("StructuralUnit") Then
			...
			If ValueIsFilled(Parameters.StructuralUnit) Then
				Items.SeriesBalancesSerie.Title = NStr("en='Available in ';ru='В наличии на ';vi='Có trong'") + Parameters.StructuralUnit;
			EndIf;
		Else
	...
	Items.SerialNumbersUpdatePeriodAction.ChoiceList.Add(NStr("en='Generate numbers in order';ru='Сгенерировать номера в порядке очереди';vi='Tạo số sê-ri theo thứ tự'"));
	Items.SerialNumbersUpdatePeriodAction.ChoiceList.Add(NStr("en='Fill with numbers frome the range';ru='Заполнить номерами из диапазона';vi='Tạo số sê-ri trong dải'"));
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("en='New';ru='Новый';vi='Mới'"));
	...
	Item.Appearance.SetParameterValue("Text", NStr("en='Registered';ru='Зарегистрированный';vi='Ghi nhận'"));
...
Function FillByBarcodesData(BarcodesData)
	...
	For Each CurBarcode In StructureData.BarcodesArray Do
		...
		ElsIf NOT (BarcodeData.ProductsAndServices = ProductsAndServices AND BarcodeData.Characteristic = Characteristic 
			...
			MessageString = NStr("en='Read barcode associated with other products and services: %1% %2% %3% %4%';ru='Считанный штрихкод привязан к другой номенклатуре: %1% %2% %3% %4%';vi='Mã vạch đã đọc được gắn vào mặt hàng khác: %1% %2% %3% %4%'");
...
Procedure ReceivedIncorrectBarcodesType(IncorrectBarcodesType) Export
	...
	For Each CurhInvalidBarcode In IncorrectBarcodesType Do
		...
		MessageString = NStr("en='Product %2% founded by barcode %1% have type %3% which is not suitable for this table section';ru='Найденная по штрихкоду %1% номенклатура -%2%- имеет тип %3%, который не подходит для этой табличной части';vi='Đã tìm thấy mặt hàng - %2% theo mã vạch %1%-có kiểu %3% mà không phù hợp với phần bảng này'");
