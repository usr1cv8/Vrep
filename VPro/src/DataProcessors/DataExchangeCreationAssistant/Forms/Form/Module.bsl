...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	FilterSettingsAtNode    = DataExchangeServer.FilterSettingsAtNode(Object.ExchangePlanName, ConfigurationVersionCorrespondent, NodeConfigurationForm, Object.ExchangeSettingsVariant);
	...
	NodeFiltersSettingsAvailable    = FilterSettingsAtNode.Count() > 0
	...
	DataTransferRestrictionsDescriptionFull = DataExchangeServer.DataTransferRestrictionsDescriptionFull(Object.ExchangePlanName, FilterSettingsAtNode, ConfigurationVersionCorrespondent, Object.ExchangeSettingsVariant);
	...
	If IsContinuedInDIBSubordinateNodeSetup Then
		...
		Items.AssistantRunModeChoiceSwitchGroup.Title = NStr("en='Continue to configure data synchronization with the main node';ru='Продолжение настройки синхронизации данных с главным узлом';vi='Tiếp tục tùy chỉnh đồng bộ hóa dữ liệu với nút chính'");
...
Procedure OnOpen(Cancel)
	...
	If CommonUseClient.OfferToCreateBackups() Then
		...
		Text = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='We recommend that you make a <a href = %1 >data backup</a> before configuring data synchronization.';ru='Перед настройкой синхронизации рекомендуется сделать <a href = %1 >резервную копию данных</a>.';vi='Trước khi tùy chỉnh đồng bộ nên tiến hành <a href = %1 >sao lưu dự phòng</a>.'"),
			"CreateBackup");
...
"Работа помощника не может быть завершена.';vi='Đang thực hiện tạo mới đồng bộ hóa dữ liệu."
"Công việc với bộ trợ giúp có thể sẽ kết thúc.'"));
		Cancel = True;
		Return;
	EndIf;
...
"Отказаться от настройки и использовать значения по умолчанию?';vi='Đang thực hiện tùy chỉnh nút cơ sở thông tin phân tán đã kết nối."
"Từ chối tùy chỉnh và sử dụng giá trị theo mặc định?'");
	...
	WarningText = NStr("en='Cancel synchronization customizing and quit the wizard?';ru='Отменить настройку синхронизации и выйти из помощника?';vi='Hủy bỏ tùy chỉnh đồng bộ hóa và thoát khỏi trợ giúp?'");
...
Procedure NotificationProcessing(EventName, Parameter, Source)
	...
	If EventName = "ClosingObjectMappingForm" Then
		...
		Status(NStr("en='Collecting mapping information...';ru='Выполняется сбор информации сопоставления...';vi='Thực hiện thu thập thông tin so sánh...'"));
		
		RefreshDataOfMappingStatisticsAtServer(Cancel, Parameter);
		
		If Cancel Then
			ShowMessageBox(, NStr("en='Errors occurred when receiving statistics information.';ru='При получении информации статистики возникли ошибки.';vi='Khi nhận thông tin thống kê có xảy ra lỗi.'"));
		Else
			
			ExpandTreeOfInformationStatistics(Parameter.UniqueKey);
			
			Status(NStr("en='Information collection is complete';ru='Сбор информации завершен';vi='Đã hoàn thành thu thập thông tin'"));
		EndIf;
...
Procedure DataExchangeSettingsForImportFileNameStartChoice(Item, ChoiceData, StandardProcessing)
	...
	DialogSettings.Insert("Filter", NStr("en='Data synchronization setting file (*.xml)';ru='Файл настроек синхронизации данных (*.xml)';vi='Tệp tùy chỉnh đồng bộ hóa dữ liệu (*xml)'") + "|*.xml" );
...
Procedure DataRegistrationRestrictionsSetting(Command)
	...
	FormParameters = New Structure;
	FormParameters.Insert("CorrespondentVersion",   ConfigurationVersionCorrespondent);
	FormParameters.Insert("FilterSettingsAtNode", FilterSettingsAtNode);
...
Procedure DataRegistrationRestrictionsSetupEnd(Result, AdditionalParameters) Export
	...
	If Result <> Undefined Then
		...
		For Each FilterSettings IN FilterSettingsAtNode Do
			...
			FilterSettingsAtNode[FilterSettings.Key] = Result[FilterSettings.Key];
			
		EndDo;
		
		// server call
		GetDataTransferRestrictionsDescription(FilterSettingsAtNode);
...
Procedure SaveDataExchangeSettingsFile(Command)
	...
	If Cancel Then
		...
		ShowMessageBox(, NStr("en='When saving file of data synchronization settings, errors occurred.';ru='При сохранении файла настроек синхронизации данных возникли ошибки.';vi='Khi tạo mới tệp tùy chỉnh đồng bộ hóa dữ liệu, đã phát sinh lỗi.'"));
...
Procedure AfterWorksWithFilesExpansionCheck(Result, AdditionalParameters) Export
	...
	If Result Then
		...
		Dialog.Title      = NStr("en='Specify a name of the data synchronization setting file';ru='Укажите имя файла настроек синхронизации данных';vi='Hãy chỉ ra tên tệp tùy chỉnh đồng bộ hóa dữ liệu'");
...
Procedure CheckCOMConnectionEnd(Result, AdditionalParameters) Export
	...
	If Result = DialogReturnCode.OK Then
		...
		If IsBlankString(MessageText) Then
			MessageText = NStr("en='Connection check is completed successfully.';ru='Проверка подключения успешно завершена.';vi='Đã hoàn tất thành công kiểm tra kết nối.'");
		EndIf;
...
Procedure CheckWSConnectionEnd(Result, AdditionalParameters) Export
	...
	If Result = DialogReturnCode.OK Then
		...
		If Not Cancel Then
			...
			ShowMessageBox(, NStr("en='Connection has been successfully established.';ru='Подключение успешно установлено.';vi='Kết nối đã thiết lập thành công.'"));
...
"Перейти в журнал регистрации?';vi='Khi tạo mới đồng bộ hóa dữ liệu, đã phát sinh lỗi."
...
&AtServer
Procedure ConfigureNewDataExchangeAtServer(Cancel, FilterSettingsAtNode, DefaultValuesAtNode)
	
	Object.AssistantOperationOption = AssistantOperationOption;
	
	DataProcessorObject = FormAttributeToValue("Object");
	
	DataProcessorObject.ExecuteNewDataExchangeConfigureActions(Cancel, FilterSettingsAtNode, DefaultValuesAtNode);
...
Procedure SetupNewDataExchangeOverExternalConnectionAtServer(Cancel, CorrespondentInfobaseNodeFilterSetup, CorrespondentInfobaseNodeDefaultValues)
	...
	DataProcessorObject.SetUpNewDataExchangeOverExternalConnection(Cancel, FilterSettingsAtNode, DefaultValuesAtNode, CorrespondentInfobaseNodeFilterSetup, CorrespondentInfobaseNodeDefaultValues);
...
Procedure SetVisibleAtServer()
	...
	If Object.ThisIsSettingOfDistributedInformationBase Then
		...
		Items.AssistantRunModeChoiceSwitchGroup.Title = NStr("en='Create initial image of DIB subordinate node';ru='Создание начального образа подчиненного узла РИБ';vi='Tạo mới ảnh ban đầu của nút trực thuộc CSTT phân tán'");
...
"к приложению в сервисе: %1';vi='Tham số kết nối"
"đến ứng dụng trong dịch vụ: %1'");
	...
	Else
		...
		Result = String(Object.ExchangeMessageTransportKind)
			+ NStr("en=', parameters:';ru=', параметры:';vi=', tham số:'") + Chars.LF
...
&AtServer
Procedure GetDataTransferRestrictionsDescription(FilterSettingsAtNode)
...
Function GetDescriptionOfSettingsOfExchangeTransport()
	...
	For Each Item IN SettingsPresentation Do
		...
		If AssistantRunMode = "ExchangeOverExternalConnection" Then
			...
			If Item.Key = "COMInfobaseOperationMode" Then
				...
				SettingValue = ?(Object[Item.Key] = 0, NStr("en='File';ru='Файловый';vi='File-server'"), NStr("en='Client-server';ru='Клиент-серверный';vi='Client-server'"));
		...
		ElsIf  Not ValueType(SettingValue, "Number")
			...
			SettingValue = NStr("en='<empty>';ru='<пусто>';vi='<rỗng>'");
	...
	If IsBlankString(Result) Then
		...
		Result = NStr("en='Connection settings are not set.';ru='Настройки подключения не заданы.';vi='Chưa đặt tùy chỉnh kết nối.'");
...
Procedure CheckConnection(TransportKind)
	...
	If Not Cancel Then
		...
		If TransportKind = PredefinedValue("Enum.ExchangeMessagesTransportKinds.FILE") Then
			WarningText = NStr("en='Writing data to the specified directory is permitted.';ru='Запись данных в указанный каталог разрешена.';vi='Cho phép ghi dữ liệu vào thư mục đã chỉ ra.'");
		Else
			WarningText = NStr("en='Connection has been successfully established.';ru='Подключение успешно установлено.';vi='Kết nối đã thiết lập thành công.'");
		EndIf;
...
Procedure CheckWSConnectionAtServer(Cancel, ExtendedCheck, IsSuggestOpenEventLogMonitor)
	...
	If ExtendedCheck Then
		...
		If Not TargetParameters.ExchangePlanExists Then
			...
			Message = NStr("en='Another application is not for synchronization with the current one.';ru='Другая программа не предназначена для синхронизации с текущей.';vi='Chương trình thứ hai không được dùng để đồng bộ hóa với chương trình này'");
...
Procedure CheckWSConnectionAtClient(Cancel, ExtendedCheck = False)
	...
	If IsBlankString(Object.WSURLWebService) Then
		...
		NString = NStr("en='Specify user name.';ru='Укажите имя пользователя.';vi='Hãy chỉ ra tên người sử dụng.'");
		CommonUseClientServer.MessageToUser(NString,, "Object.WSURLWebService",, Cancel);
		
	ElsIf IsBlankString(Object.WSUserName) Then
		
		NString = NStr("en='Specify user password.';ru='Укажите пароль пользователя.';vi='Hãy chỉ ra mật khẩu người sử dụng.'");
...
"Перейти в журнал регистрации?';vi='Lỗi thiết lập kết nối."
...
Procedure CheckAttributeFillingOnForm(Cancel, FormToCheckName, FormParameters, FormAttributeName)
	...
	If Not SettingsForm.CheckFilling() Then
		...
		CommonUseClientServer.MessageToUser(NStr("en='Configure required settings.';ru='Необходимо задать обязательные настройки.';vi='Không thể chỉ ra tùy chỉnh bắt buộc.'"),,, FormAttributeName, Cancel);
...
Procedure CorrespondentInfobaseRegistrationRestrictionSetup(ConnectionType)
	...
	FormParameters.Insert("FilterSettingsAtNode",      CorrespondentInfobaseNodeFilterSetup);
	FormParameters.Insert("SettingID",      Object.ExchangeSettingsVariant);
...
Procedure FinishFirstExchangeOverOrdinaryCommunicationChannelsSetupStage(Cancel)
	...
	If Not Object.ThisIsSettingOfDistributedInformationBase
		...
		NString = NStr("en='Save file with settings for another application';ru='Сохраните файл с настройками для другой программы';vi='Hãy lưu lại tệp có tùy chỉnh đối với chương trình thứ hai'");
	...
	Else
		...
		If ExecuteDataExchangeNow Then
			...
			Status(NStr("en='Sending data...';ru='Выполняется отправка данных...';vi='Đang gửi dữ liệu...'"));
			InitializationOfDataExchangeAtClient(Cancel, Object.InfobaseNode, Object.ExchangeMessageTransportKind);
			Status(NStr("en='Data is sent';ru='Отправка данных завершена';vi='Đã hoàn tất gửi dữ liệu'"));
			
			If Cancel Then
				
				ShowMessageBox(, NStr("en='Errors occurred when sending the data (see the event log).';ru='Во время отправки данных возникли ошибки(см. журнал регистрации).';vi='Trong thời gian gửi dữ liệu nảy sinh lỗi (xem nhật ký sự kiện).'"));
...
Procedure FinishSecondExchangeOverOrdinaryCommunicationChannelsSetupStage(Cancel, Val OpenAfterClosingCurrent)
	...
	Status(NStr("en='Creating data synchronization setting';ru='Выполняется создание настройки синхронизации данных';vi='Đang thực hiện tạo mới đồng bộ hóa dữ liệu'"));
	
	ConfigureNewDataExchangeAtServer(Cancel, FilterSettingsAtNode, DefaultValuesAtNode);
	
	If Cancel Then
		
		ShowMessageBox(, NStr("en='When creating data synchronization setting, errors occurred.';ru='При создании настройки синхронизации данных возникли ошибки.';vi='Khi tạo mới tùy chỉnh đồng bộ hóa dữ liệu đã xuất hiện lỗi.'"));
		Return;
	EndIf;
...
Procedure OpenMappingForm()
	...
	If Not CurrentData.UsePreview Then
		ShowMessageBox(, NStr("en='Cannot map this data.';ru='Для этих данных нельзя выполнить сопоставление.';vi='Đối với dữ liệu này không thể thực hiện so sánh.'"));
		Return;
	EndIf;
	...
	Except
		...
		WriteErrorInEventLogMonitor(
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='An error occurred while executing handler OnConnectingToCorrespondent:%1%2';ru='При выполнении обработчика ПриПодключенииККорреспонденту произошла ошибка:%1%2';vi='Khi thực hiện hàm sự kiện OnConnectingToCorrespondent đã xuất hiện lỗi:%1%2'"),
				Chars.LF,
...
Function LabelNextFTP()
	...
	Return NStr("en='Click Next to set connection via FTP resource.';ru='Нажмите кнопку ""Далее"" для настройки подключения через FTP-ресурс.';vi='Hãy nhấn nút ""Tiếp theo"" để tùy chỉnh kết nối thông qua nguồn FTP.'");
...
Function LabelNextEMAIL()
	...
	Return NStr("en='Click Next to set email connection.';ru='Нажмите кнопку ""Далее"" для настройки подключения по почте.';vi='Hãy nhấn nút ""Tiếp theo"" để tùy chỉnh kết nối qua E-mail.'");
...
Function LabelNextSettings()
	...
	Return NStr("en='Click Next to set additional parameters of data synchronization.';ru='Нажмите кнопку ""Далее"" для настройки дополнительных параметров синхронизации данных.';vi='Hãy nhấn nút ""Tiếp theo"" để tùy chỉnh tham số bổ sung đồng bộ hóa dữ liệu.'");
...
Function Attachable_AssistantPageParameterSetup_OnGoingNext(Cancel)
	...
	If IsBlankString(Object.ThisInfobaseDescription) Then
		...
		NString = NStr("en='Specify this application name.';ru='Укажите наименование этой программы.';vi='Hãy chỉ ra tên gọi của chương trình này.'");
	...
	If IsBlankString(Object.SecondInfobaseDescription) Then
		...
		NString = NStr("en='Specify a different application name.';ru='Укажите наименование другой программы.';vi='Hãy chỉ ra tên gọi chương trình thứ hai.'");
	...
	If IsBlankString(Object.TargetInfobasePrefix) Then
		...
		NString = NStr("en='Specify an existing or required prefix of the second infobase.';ru='Укажите существующий или желаемый префикс второй информационной базы.';vi='Hãy chỉ ra tiền tố hiện có hoặc mong muốn của cơ sở thông tin thứ hai.'");
	...
	If TrimAll(Object.SourceInfobasePrefix) = TrimAll(Object.TargetInfobasePrefix) Then
		...
		NString = NStr("en='Infobase prefixes should be different.';ru='Префиксы информационных баз должны быть различными.';vi='Tiền tố cơ sở thông tin phải khác nhau.'");
		CommonUseClientServer.MessageToUser(NString,, "Object.TargetInfobasePrefix",, Cancel);
...
Function Attachable_AssistantPageFirstInfobaseExternalConnectionParameterSetup_OnGoingNext(Cancel)
	...
	If IsBlankString(Object.ThisInfobaseDescription) Then
		...
		NString = NStr("en='Specify an infobase name.';ru='Укажите наименование информационной базы.';vi='Hãy chỉ ra tên gọi cơ sở thông tin.'");
		CommonUseClientServer.MessageToUser(NString,, "Object.ThisInfobaseDescription",, Cancel);
		Return Undefined;
	EndIf;
...
Function Attachable_AssistantPageStart_OnGoingNext(Cancel)
	...
	If IsBlankString(Object.DataExchangeSettingsForImportFileName) Then
		ErrorText = NStr("en='Select a file with data synchronization settings';ru='Выберите файл с настройками синхронизации данных';vi='Hãy chọn tệp có tùy chỉnh đồng bộ hóa dữ liệu'");
	...
	WarningText = NStr("en='To transfer the file of data synchronization settings, install extension for web client 1C:Enterprise.';ru='Для передачи файла настроек синхронизации данных необходимо установить расширение для веб-клиента 1С:Предприятие.';vi='Để chuyển tệp tùy chỉnh đồng bộ hóa dữ liệu, cần thiết lập đuôi tệp cho Web-client 1C:DOANH NGHIỆP.'");
...
Procedure EndSelectingDataExchangeSettingsForImportFile(Val FilesPlacingResult, Val AdditionalParameters) Export
	...
	PlacedFileAddress = FilesPlacingResult.Storing;
	...
	If IsBlankString(ErrorText) Then
		...
		If AssistantParametersImportError Then
			ErrorText = NStr("en='Invalid data synchronization setting file is specified. Specify a correct file.';ru='Указан неправильный файл настроек синхронизации данных. Укажите корректный файл.';vi='Đã chỉ ra sai tệp tùy chỉnh đồng bộ hóa dữ liệu. Hãy chỉ ra tệp đúng.'");
		Else
...
"Следует настроить хотя бы один вариант подключения.';vi='Chưa chỉ ra tham số kết nối để đồng bộ hóa dữ liệu."
"Cần thiết lập ít nhất một phương án kết nối.'");
		//
...
"этой информационной базы: %4 Префикс второй информационной базы: %5';vi='%1%2%3Tiền tố"
"của cơ sở thông tin này: %4 Tiền tố của cơ sở thông tin thứ hai: %5'");
	...
	Else
		...
		// Display of exchange setup result.
		MessageString = NStr("en='%1%2%3Prefix of this infobase: %4';ru='%1%2%3Префикс этой информационной базы: %4';vi='%1%2%3Tiền tố của cơ sở thông tin này: %4'");
...
Function Attachable_AssistantPageExchangeSetupResults_OnOpen_ExternalConnection(Cancel, SkipPage, IsGoNext)
	...
	If ExchangeWithServiceSetup Then
		...
		MessageString = NStr("en='%1 Settings for this infobase: ======================================================== %2%3Prefix of the infobase: %4 Settings for the application located in the service: ======================================================== %5%6Prefix of the application: %7';ru='%1 Настройки для этой информационной базы: ======================================================== %2%3Префикс информационной базы: %4 Настройки для приложения, расположенного в сервисе: ======================================================== %5%6Префикс приложения: %7';vi='%1 Tùy chỉnh đối với cơ sở thông tin này: ======================================================== %2%3Tiền tố cơ sở thông tin: %4 Tùy chỉnh đối với ứng dụng mà sắp đặt trong dịch vụ: ======================================================== %5%6Tiền tố ứng dụng: %'");
		
	Else
		
		MessageString = NStr("en='%1 Parameters of data synchronization for this application: ======================================================== %2%3Prefix of the infobase: %4 Parameters of data synchronization for another application:: ================================================ %5%6Prefix of the infobase: %7';ru='%1 Параметры синхронизации данных для этой программы: ======================================================== %2%3Префикс информационной базы: %4 Параметры синхронизации данных для другой программы:: ======================================================== %5%6Префикс информационной базы: %7';vi='%1 Tham số đồng bộ hóa dữ liệu đối với chương trình này: ======================================================== %2%3Tiền tố cơ sở thông tin: %4 Tham số đồng bộ hóa dữ liệu đối với chương trình khác:  ======================================================== %5%6Tiền tố cơ sở thông tin: %7'");
...
"Для решения проблем воспользуйтесь журналом регистрации.';vi='Đã xuất hiện lỗi trong giai đoạn tạo mới tùy chỉnh đồng bộ hóa dữ liệu."
"Để giải quyết vấn đề, hãy sử dụng nhật ký sự kiện.'"));
...
"Для решения проблем воспользуйтесь журналом регистрации.';vi='Đã xuất hiện lỗi trong giai đoạn phân tích dữ liệu."
"Để giải quyết vấn đề, hãy sử dụng nhật ký sự kiện.'"));
...
Function Attachable_AssistantPageWaitForDataAnalysisAutomaticMapping_LongOperationProcessing(Cancel, GoToNext)
	...
	If Cancel Then
		...
		ShowMessageBox(, NStr("en='Errors occurred when analyzing data.';ru='Возникли ошибки на этапе анализа данных.';vi='Xảy ra lỗi ở giai đoạn phân tích dữ liệu.'"));
...
"Продолжить?';vi='Không phải tất cả dữ liệu được so sánh. Việc có"
"dữ liệu chưa so sánh có thể dẫn đến có một số phần tử danh mục giống nhau (bản sao)."
"Tiếp tục?'");
...
Procedure AssistantPageWaitForCatalogSynchronizationImport_LongOperationProcessing()
	...
	Result = LongActions.ExecuteInBackground(
		...
		MethodParameters,
		NStr("en='Import catalogs from exchange message';ru='Загрузка справочников из сообщения обмена';vi='Kết nhập danh mục từ thông điệp trao đổi'"));
...
"Для решения проблем воспользуйтесь журналом регистрации.';vi='Đã xuất hiện lỗi ở giai đoạn đồng bộ hóa danh mục."
"Để giải quyết vấn đề, hãy sử dụng nhật ký sự kiện.'"));
...
"Для решения проблем воспользуйтесь журналом регистрации.';vi='Đã xuất hiện lỗi ở giai đoạn lưu tùy chỉnh.'"));
...
"Для решения проблем воспользуйтесь журналом регистрации.';vi='Xuất hiện lỗi trong giai đoạn đồng bộ hóa dữ liệu."
"Để giải quyết vấn đề, hãy sử dụng nhật ký sự kiện.'"));
...
Procedure CheckAccountingSettingsAtServer(
	...
	If Not SystemAccountingSettingsAreSet Then
		...
		If IsBlankString(ErrorInfo) Then
			ErrorInfo = NStr("en='Accounting parameters in this application are not specified.';ru='Не заданы параметры учета в этой программе.';vi='Chưa chỉ ra tham số kế toán trong chương trình này.'");
		EndIf;
	...
	If Not CorrespondentAccountingSettingsAreSet Then
		...
		If IsBlankString(CorrespondentErrorMessage) Then
			CorrespondentErrorMessage = NStr("en='Accounting parameters are not specified for the online application.';ru='Не заданы параметры учета в приложении, расположенном в Интернете.';vi='Chưa xác định tham số kế toán trong ứng dụng trên Internet.'");
		EndIf;
...
Procedure CheckJobSettingsForFirstInfobase(Cancel, ConnectionType = "WebService")
	...
	If IsBlankString(Object.SecondInfobaseDescription) Then
		...
		NString = NStr("en='Specify the online application name.';ru='Укажите наименование приложения в Интернете.';vi='Hãy chỉ ra tên gọi ứng dụng trên Internet.'");
		CommonUseClientServer.MessageToUser(NString,, "Object.SecondInfobaseDescription",, Cancel);
...
Procedure CheckJobSettingsForSecondInfobase(Cancel, ConnectionType)
	...
	If IsBlankString(Object.SecondInfobaseDescription) Then
		...
		NString = NStr("en='Specify application name.';ru='Укажите наименование программы.';vi='Hãy chỉ ra tên chương trình.'");
		CommonUseClientServer.MessageToUser(NString,, "Object.SecondInfobaseDescription",, Cancel);
...
"Измените значение префикса или используйте существующую синхронизацию.';vi='Giá trị tiền tố của cơ sở thông tin thứ hai không phải là duy nhất."
"Trong hệ thống, đã tồn tại đồng bộ hóa dữ liệu cho cơ sở thông tin với tiền tố đã chỉ ra."
"Hãy thay đổi giá trị tiền tố hoặc sử dụng đồng bộ hóa dữ liệu hiện tại.'");
		//
		CommonUseClientServer.MessageToUser(NString,, "Object.TargetInfobasePrefix",, Cancel);
...
Procedure AssistantPageWaitForCheckExternalConnectionConnected_LongOperationProcessorOnServer(Cancel)
	...
	If Object.COMInfobaseOperationMode = 0 Then
		...
		If IsBlankString(Object.COMInfobaseDirectory) Then
			...
			NString = NStr("en='Specify an infobase name.';ru='Укажите имя информационной базы.';vi='Hãy chỉ ra tên cơ sở thông tin.'");
	...
	Else
		...
		If IsBlankString(Object.COMServerName1CEnterprise) Then
			...
			NString = NStr("en='Specify a server cluster name.';ru='Укажите имя кластера серверов.';vi='Hãy chỉ ra tên Cluster-server.'");
	...
	If Not ExchangePlanExists Then
		...
		Message = NStr("en='Data synchronization with the specified application is not applicable.';ru='Синхронизация данных с указанной программой не предусмотрена.';vi='Không xem xét đồng bộ hóa dữ liệu với chương trình đã chỉ ra.'");
		CommonUseClientServer.MessageToUser(Message,,,, Cancel);
		Return;
	...
	If Lower(InfobaseConnectionString()) = Lower(ExternalConnection.InfobaseConnectionString()) Then
		...
		Message = NStr("en='Connection settings for this infobase are configured.';ru='Заданы настройки подключения к этой информационной базе.';vi='Đã xác định tùy chỉnh kết nối đến cơ sở thông tin này.'");
		CommonUseClientServer.MessageToUser(Message,,,, Cancel);
		Return;
...
Procedure CheckWhetherDataExchangeWithSecondBaseExists(Cancel)
	...
	If Not IsBlankString(NodeCode)
		...
		Message = NStr("en='Data synchronization between applications has been configured earlier.';ru='Синхронизация данных между программами уже была настроена ранее.';vi='Đã tùy chỉnh từ trước việc đồng bộ hóa dữ liệu giữa các chương trình.'");
		CommonUseClientServer.MessageToUser(Message,,,, Cancel);
...
Procedure FirstExchangeSetupStageGoToTable()
	...
	NewTransition.NavigationPageName = "NavigationPageContinuation";
	NewTransition.GoNextHandlerName = "AssistantPageSetTransportParametersFILE_OnGoingNext";
	...
	NewTransition.OnOpenHandlerName = "AssistantPageParameterSetup_OnOpen";
...
Procedure ExchangeOverWebServiceSetupGoToTable()
	...
	NewTransition.NavigationPageName = "NavigationPageWait";
	NewTransition.LongOperation = True;
	NewTransition.LongOperationHandlerName = "AssistantPageWaitingWebServiceConnectionCheck_LongOperationProcessing";
