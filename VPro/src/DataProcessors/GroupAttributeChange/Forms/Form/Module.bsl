...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		Title = NStr("en='Bulk attribute editing';ru='Групповое изменение реквизитов';vi='Thay đổi mục tin theo nhóm'");
	EndIf;
...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	If Upper(ChoiceSource.FormName) = Upper("DataProcessor.GroupAttributeChange.Form.AdditionalParameters") Then
		...
		If TypeOf(ValueSelected) = Type("Structure") Then
			...
			Object.AbortOnError  = ValueSelected.AbortIfError;
			...
			TPAboutPortionSetting          = ValueSelected.BatchSetting;
			TOObjectsPercentInPortion   = ValueSelected.ObjectPercentageInBatch;
			TONumberOfObjectsInPortion     = ValueSelected.ObjectNumberInBatch;
...
Procedure Change(Command)
	...
	If ButtonFunction = "Change" Then
		If ObjectCountForDataProcessors = 0 Then
			ShowMessageBox(, NStr("en='Items for change are not specified';ru='Не указаны элементы для изменения';vi='Chưa chỉ ra phần tử để thay đổi'"));
			Return;
		EndIf;
		...
		Else
			QuestionText = NStr("en='Filter is not set. Change all items?';ru='Отбор не задан. Изменить все элементы?';vi='Chưa chọn phễu lọc. Thay đổi tất cả phần tử?'");
			NotifyDescription = New NotifyDescription("PerformChangingSelectionCheckIsDone", ThisObject);
			ShowQueryBox(NOTifyDescription, QuestionText, QuestionDialogMode.OKCancel, , , NStr("en='Change items';ru='Изменение элементов';vi='Thay đổi phần tử'"));
		EndIf;
...
Procedure PerformChangingSelectionCheckIsDone(QuestionResult = Undefined, AdditionalParameters = Undefined) Export
	...
	If Not ThereAreConfiguredChanges() Then
		QuestionText = NStr("en='Changes are not set. Rewrite the items without changes?';ru='Изменения не настроены. Выполнить перезапись элементов без изменений?';vi='Chưa tùy chỉnh thay đổi. Thực hiện ghi lại các phần tử mà không thay đổi?'");
...
Procedure FulfillActionsAtContextOpening()
	...
	For Each TransferedObject IN Parameters.ObjectsArray Do
		If TypeOf(TransferedObject) <> TransferedObjectType Then
			Raise NStr("en='Bulk edit is only for items of one type.';ru='Групповое изменение возможно только для элементов одного типа.';vi='Chỉ có thể thay đổi theo nhóm đối với các phần tử cùng kiểu.'");
		EndIf;
	EndDo;
	
	ObjectCount    = Parameters.ObjectsArray.Count();
	CaptionPattern       = NStr("en='Change selected items ""%1"" (%2)';ru='Изменение выделенных элементов ""%1"" (%2)';vi='Thay đổi phần tử đã chọn ""%1"" (%2)'");
	Title = SubstituteParametersInString(CaptionPattern, TransferedObjectType, ObjectCount);
...
Procedure QuestionGoToToAttributeUnblocking()
	...
	QuestionText = NStr("en='The attribute has been locked.  Unlock attributes?';ru='Реквизит заблокирован, перейти к разблокированию реквизитов?';vi='Mục tin bị phong tỏa, chuyển sang bỏ phong tỏa mục tin?'");
	
	Buttons = New ValueList;
	Buttons.Add(DialogReturnCode.Yes, NStr("en='Yes';ru='Да';vi='Đồng ý'"));
	...
	ShowQueryBox(NOTifyDescription, QuestionText, Buttons, , DialogReturnCode.Yes, NStr("en='Attribute is locked';ru='Реквизит заблокирован';vi='Mục tin bị phong tỏa'"));
...
Procedure SetButtonsDuringModification(BeginChanges)
	...
	If BeginChanges Then
		Items.FormChange.Title = NStr("en='Abort';ru='Прервать';vi='Ngừng'");
	Else
		If ObjectsFailedToChange.Count() > 0 Then
			Items.FormChange.Title = NStr("en='Change again';ru='Повторить изменение';vi='Lặp lại thay đổi'");
		Else
			Items.FormChange.Title = NStr("en='Close';ru='Закрыть';vi='Đóng'");
		EndIf;
	EndIf;
...
Procedure ChangeObjects()
	...
	If Object.ChangeInTransaction Then
		...
		If TPAboutPortionSetting = 1 Then // processing by one call
			...
			ShowUserNotification(NStr("en='Change selected items';ru='Изменение выделенных элементов';vi='Thay đổi phần tử đã chọn'"), ,NStr("en='Please wait. Data processor may take some time...';ru='Пожалуйста подождите, обработка может занять некоторое время...';vi='Xin vui lòng đợi, xử lý có thể mất nhiều thời gian...'"));
	...
	Else
		...
		Status(NStr("en='Processing items...';ru='Обрабатываются элементы...';vi='Xử lý các phần tử ...'"), 0, NStr("en='Change selected items';ru='Изменение выделенных элементов';vi='Thay đổi phần tử đã chọn'"));
...
Procedure ProcessChangeResult(ResultOfChange = Undefined, ContinueProcessing = Undefined)
	...
	While ContinueProcessing Do
		...
		If Object.ChangeInTransaction Then
			WarningText = NStr("en='Errors occurred while editing the items. The changes were canceled.';ru='При изменении элементов обнаружены ошибки - изменения отменены.';vi='Khi thay đổi phẩn tử đã phát hiện lỗi - hủy bỏ thay đổi.'");
...
"';vi='Khi thay đổi phần tử (nhóm phần tử) đã xảy ra lỗi."
"Ngừng thay đổi phần tử và chuyển đến xem lỗi?'");
		Buttons = New ValueList;
		Buttons.Add(DialogReturnCode.Abort, NStr("en='Abort';ru='Прервать';vi='Ngừng'"));
		Buttons.Add(DialogReturnCode.Ignore, NStr("en='Continue';ru='Продолжить';vi='Tiếp tục'"));
		Buttons.Add(DialogReturnCode.No, NStr("en='do not ask again';ru='больше не спрашивать';vi='không hỏi nữa'"));
		
		NotifyDescription = New NotifyDescription("ProcessChangeResultResponseReceived", ThisObject, ResultOfChange);
		ShowQueryBox(NOTifyDescription, QuestionText, Buttons, , DialogReturnCode.Abort, NStr("en='Errors occurred when changing items';ru='Ошибки при изменении элементов';vi='Lỗi khi thay đổi phần tử'"));
		Return;
	EndDo;
	...
	If CurrentStateChanges.ShowPercentOfProcessed Then
		...
		Status(NStr("en='Processing items...';ru='Обрабатываются элементы...';vi='Xử lý các phần tử ...'"), CurrentPercent, NStr("en='Change selected items';ru='Изменение выделенных элементов';vi='Thay đổi phần tử đã chọn'"));
	EndIf;
...
Procedure FinishChangeOfObjects()
	...
	If DataProcessorCompleted Then
		ShowUserNotification(NStr("en='Change item attributes';ru='Изменение реквизитов элементов';vi='Thay đổi mục tin phần tử'"), , 
			SubstituteParametersInString(NStr("en='Items are changed (%1).';ru='Изменены элементы (%1).';vi='Đã thay đổi các phần tử (%1).'"), CurrentStateChanges.CountOfChanged));
		GoToPageAllDone();
		Return;
	EndIf;
	...
	If DataProcessorCompleted Then
		MessagePattern = NStr("en='Changes are made in all selected items (%2).';ru='Изменения выполнены во всех выбранных элементах (%2).';vi='Đã thực hiện thay đổi trong tất cả các phần tử được chọn (%2).'");
	Else
		If Object.ChangeInTransaction Or CurrentStateChanges.CountOfChanged = 0 Then
			MessagePattern = NStr("en='Not changed.';ru='Изменения не выполнены.';vi='Chưa thực hiện thay đổi.'");
		Else
			MessagePattern = NStr("en='Partly modified."
"Modified: %1; Failed to change: %3';ru='Изменения выполнены частично."
"Изменено: %1; Не удалось изменить: %3';vi='Đã thực hiện thay đổi từng phần.'");
		EndIf;
	EndIf;
	...
	If Object.ChangeInTransaction AND Not DataProcessorCompleted Then
		...
		If SkippedQuantity > 0 AND Not CurrentStateChanges.InterruptChange Then
			...
			TableRow.Object = SubstituteParametersInString(NStr("en='... and other items (%1)';ru='... и другие элементы (%1)';vi='...và phần tử khác (%1)'"), SkippedQuantity);
			TableRow.Cause = NStr("en='Skipped as one or more items were not changed.';ru='Пропущены, так как не были изменены один или более элементов.';vi='Bị bỏ qua, bởi vì đã không thay đổi ít nhất một phần tử.'");
		EndIf;
	EndIf;
	
	Items.LabelDataProcessorResults.Title = SubstituteParametersInString(
		MessagePattern,
...
Procedure BackServer()
	...
	ObjectsFailedToChange.Clear();
	Items.FormBack.Visible = False;
	Items.FormChange.Title = NStr("en='Change attributes';ru='Изменить реквизиты';vi='Thay đổi mục tin'");
...
Procedure GoToPageAllDone()
	...
	Items.LabelSuccessfullyCompleted.Title = SubstituteParametersInString(
		NStr("en='Attributes of selected items were changed."
"Total modified items:%1';ru='Реквизиты выбранных элементов были изменены."
"Всего изменено элементов: %1';vi='Mục tin của các phần tử được chọn đã bị thay đổi."
"Tổng số phần tử đã bị thay đổi: %1'"), CurrentStateChanges.CountOfChanged);
	Items.FormChange.Title = NStr("en='Finish';ru='Готово';vi='Kết thúc'");
	Items.FormBack.Visible = True;
...
Function ChangeAtServer(Val StopChangingAtError)
	...
	If Not Object.ChangeInTransaction Or Not SubsystemExists("StandardSubsystems.BasicFunctionality") Then
			...
			BackgroundJobResult = New Structure("JobCompleted, StorageAddress", True, StorageAddress);
	Else
		JobDescription = NStr("en='Bulk edit of items';ru='Групповое изменение элементов';vi='Thay đổi phần tử theo nhóm'");
...
Procedure FillObjectTableParts(BlockedAttributes, NonEditableAttributes, FilteredAttributes)
	...
	For Each ObjectTable IN ObjectTables Do
		...
		FormTable.SetAction("BeforeRowChange", "Attachable_BeforeChangeStart");
...
Function AttributeTableColumsDescription()
	...
	ColumnDetails.Type = New TypeDescription("String");
	ColumnDetails.Presentation = NStr("en='Attribute';ru='Реквизит';vi='Mục tin'");
	...
	ColumnDetails.Presentation = NStr("en='New value';ru='Новое значение';vi='Giá trị mới'");
	ColumnDetails.FieldKind = FormFieldType.InputField;
...
Procedure GenerateConfiguredChangesExplanation()
	...
	If ChangeableAttributes.Count() = 1 Then
		Explanation = NStr("en='attributes';ru='атрибуты';vi='thuộc tính'") + " " + Explanation;
	ElsIf ChangeableAttributes.Count() > 1 Then
		Explanation = NStr("en='attributes';ru='атрибуты';vi='thuộc tính'") + " " + Explanation;
	EndIf;
	
	If Not IsBlankString(Explanation) Then
		Explanation = Explanation + " " + NStr("en='in the selected items';ru='в выбранных элементах';vi='trong các phần tử đã chọn'");
	EndIf;
	...
	For Each TabularSection IN ChangeableTableParts Do
		...
		If ChangeableAttributes.Count() > 3 Then
			If Not IsBlankString(Explanation) Then
				Explanation = Explanation + ", ";
			EndIf;
			Explanation = Explanation + SubstituteParametersInString(
				NStr("en='attributes (%1)';ru='реквизиты (%1)';vi='mục tin (%1)'"), ChangeableAttributes.Count());
		Else
			For Each Attribute IN ChangeableAttributes Do
				If Not IsBlankString(Explanation) Then
					Explanation = Explanation + ", ";
				...
				If ChangeableAttributes.Find(Attribute) = 0 Then
					If ChangeableAttributes.Count() = 1 Then
						Explanation = Explanation + NStr("en='Attribute';ru='Реквизит';vi='Mục tin'") + " ";
					ElsIf ChangeableAttributes.Count() > 1 Then
						Explanation =  Explanation + NStr("en='attributes';ru='атрибуты';vi='thuộc tính'") + " ";
					EndIf;
				EndIf;
				Explanation = Explanation + """" + Attribute + """";
			EndDo;
		EndIf;
		Explanation = Explanation + " " 
			+ SubstituteParametersInString(NStr("en='in the ""%1"" tabular section';ru='в табличной части ""%1""';vi='trong bảng ""%1""'"), TabularSection.Key);
	EndDo;
	
	If Not IsBlankString(Explanation) Then
		If ChangeableTableParts.Count() > 0 Then
			If FilterByStrings Then 
				Explanation = Explanation + " " + NStr("en='in all lines of the selected items';ru='во всех строках выбранных элементов';vi='trong tất cả các dòng của phần tử đã chọn'")
			Else
				Explanation = Explanation + " " + NStr("en='in all lines of the selected items';ru='во всех строках выбранных элементов';vi='trong tất cả các dòng của phần tử đã chọn'")
			EndIf;
		EndIf;
	...
	If ThereAreSelectedObjects Then
		If Not IsBlankString(Explanation) Then
			Explanation = NStr("en='Rewrite the selected items.';ru='Выполнить перезапись выбранных элементов.';vi='Thực hiện ghi lại các phần tử đã chọn.'") + " " + Explanation + ".";
		Else
			Explanation = NStr("en='Rewrite the selected items.';ru='Выполнить перезапись выбранных элементов.';vi='Thực hiện ghi lại các phần tử đã chọn.'");
		EndIf;
	Else
		Explanation = NStr("en='Items which attributes shall be changed are not selected.';ru='Не выбраны элементы, реквизиты которых необходимо изменить.';vi='Chưa chọn các phần tử mà cần thay đổi các mục tin của chúng.'");
	EndIf;
...
Procedure UpdateLabelSelectedCount()
	...
	If ThereAreCustomizedFilters() Then
		...
		If IsBlankString(ErrorMessageText) Then
			LabelText = SubstituteParametersInString(
				NStr("en='Selected items: %1';ru='Выбрано элементов: %1';vi='Đã chọn %1 phần tử'"), SelectedObjectsCount);
		Else
			ThereAreErrorsInSelection = True;
			LabelText = SubstituteParametersInString(
				NStr("en='Items are not selected. %1';ru='Элементы не выбраны. %1';vi='Các phần tử chưa được chọn. %1'"), ErrorMessageText);
		EndIf;
	Else
		LabelText = NStr("en='Change all items';ru='Изменять все элементы';vi='Thay đổi tất cả phần tử'");
	EndIf;
...
Procedure ConfigureChangeSetting(Val Setting)
	...
	If IsBlocked Then
		ShowMessageBox(, NStr("en='Some attributes are locked for editing. Changes were not defined.';ru='Некоторые реквизиты заблокированы для изменения, изменения не установлены.';vi='Một số mục tin bị khóa để thay đổi, việc thay đổi chưa được thực hiện.'"));
	EndIf;
...
Function ObjectManagerByFullName(DescriptionFull)
	...
	ElsIf Upper(MOClass) = "CALCULATIONREGISTER" Then
		...
		Else
			...
			Else
				Raise SubstituteParametersInString(
					NStr("en='Unknown metadata object type ""%1""';ru='Неизвестный тип объекта метаданных ""%1""';vi='Sai kiểu đối tượng metadata ""%1""'"), DescriptionFull);
			EndIf;
		EndIf;
...
Function SubstituteParametersInString(Val LookupString,
	...
	If UseAlternativeAlgorithm Then
		LookupString = SubstituteParametersInStringAlternativeAlgorithm(LookupString, Parameter1,
...
&AtClientAtServerNoContext
Function SubstituteParametersInStringAlternativeAlgorithm(Val LookupString,
	Val Parameter1, Val Parameter2 = Undefined, Val Parameter3 = Undefined)
...
Function ValidateVersionAndPlatformCompatibilityMode()
	...
	If Not (Left(Information.AppVersion, 3) = "8.3"
		...
		Raise NStr("en='DataProcessor is used to start on 1C:Enterprise 8.3 platform version (or above) with compatibility mode off';ru='Обработка предназначена для запуска на версии платформы 1С:Предприятие 8.3 (или выше) с отключенным режимом совместимости';vi='Bộ xử lý được dùng để khởi động sang phiên bản của nền tảng 1C:DOANH NGHIỆP 8.3 (hoặc cao hơn) có ngắt chế độ tương thích'");
