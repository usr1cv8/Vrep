...
"также проконтролировать из раздела ""Информация"" на рабочем столе, команда ""Описание изменений программы"".';vi='Quá trình xử lý dữ liệu của phiên bản chương trình có thể"
"được kiểm soát từ phần hành ""Thông tin"" trên bàn làm việc, lệnh ""Mô tả thay đổi chương trình"".'");
	...
	Else
		TitleRefreshCompleted = NStr("en='The application version was successfully updated to version %1';ru='Версия программы успешно обновлена на версию %1';vi='Đã cập nhật thành công phiên bản chương trình sang phiên bản %1'");
		Items.InformationRefreshEnabledCompleted.Title = StringFunctionsClientServer.SubstituteParametersInString(
	...
	Else
		...
		DataAboutUpdate = InfobaseUpdateService.DataOnUpdatingInformationBase();
		CaptionPattern = NStr("en='Additional data processor procedures are completed %1 at %2';ru='Дополнительные процедуры обработки данных завершены %1 в %2';vi='Đã kết thúc thủ tục bổ sung xử lý dữ liệu %1 sang %2'");
...
"на версию %1 Работа с этими данными временно ограничена';vi='Đang thực hiện thủ tục bổ sung xử lý dữ liệu"
"sang phiên bản %1 Làm việc với các dữ liệu này tạm thời bị hạn chế'"), Metadata.Version);
...
Function MessageAboutUpdateResult(DataAboutUpdate)
	...
	If TotalHandlers = SuccessfullyCompletedHandlers Then
		...
		Else
			MessageText = NStr("en='All update procedures are completed successfully (%1)';ru='Все процедуры обновления выполнены успешно (%1)';vi='Đã thực hiện thành công tất cả thủ tục cập nhật (%1)'");
		EndIf;
		Items.PictureInformation1.Picture = PictureLib.Successfully32;
	Else
		MessageText = NStr("en='Not all procedures were executed (%1 is executed out of %2)';ru='Не все процедуры удалось выполнить (выполнено %1 из %2)';vi='Đã thực hiện không phải tất cả thủ tục (đã thực hiện %1 trong %2)'");
...
Procedure RefreshInformationAboutUpdate(DataAboutUpdate, RefreshEnabledCompleted = False)
	...
	Items.InformationStateUpdate.Title = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Completed: %1 out of %2';ru='Выполнено: %1 из %2';vi='Đã thực hiện: %1 từ %2'"),
		CompletedHandlers,
		TotalHandlers);
...
Procedure ChangeScheduleAfterScheduleSetup(Schedule, AdditionalParameters) Export
	...
	If Schedule <> Undefined Then
		If Schedule.RepeatPeriodInDay = 0 Then
			...
			QuestionButtons.Add("ConfigureSchedule", NStr("en='Configure schedule';ru='Настроить расписание';vi='Tùy chỉnh lịch biểu'"));
			QuestionButtons.Add("RecommendedSettings", NStr("en='Set recommended settings';ru='Установить рекомендуемые настройки';vi='Đặt tùy chỉnh được đề xuất'"));
...
"и заполнить поле """"Повторять через"""".';vi='Thủ tục bổ sung xử lý dữ liệu được thực hiện bởi nhiều phần,"
"vì vậy để các phần này làm việc chính xác, cần bắt buộc chỉ ra khoảng lặp sau khi hoàn tất."
""
"Để thực hiện điều này, trong cửa sổ tùy chỉnh lịch biểu, cần chuyển sang thẻ """"theo ngày"""""
"và điền trường """"Lặp lại sau"""".'");
