...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If OnlyApplicationWorkParametersUpdate Then
		Title = NStr("en='Update the application parameters';ru='Обновление параметров работы программы';vi='Cập nhật tham số làm việc của chương trình'");
	...
	ElsIf DataUpdateMode = "InitialFilling" Then
		Title = NStr("en='Initial data population';ru='Начальное заполнение данных';vi='Điền ban đầu dữ liệu'");
		Items.RunMode.CurrentPage = Items.InitialFilling;
		
	ElsIf DataUpdateMode = "TransitionFromAnotherApplication" Then
		Title = NStr("en='Migration from another application';ru='Переход с другой программы';vi='Chuyển từ chương trình khác'");
	...
	Else
		Title = NStr("en='Application version update';ru='Обновление версии программы';vi='Cập nhật phiên bản chương trình'");
...
Function ImportUpdateApplicationWorkInBackgroundParameters()
	...
	Try
		Result = LongActions.ExecuteInBackground(
			...
			ExecuteParameters,
			NStr("en='Background update of the application parameters';ru='Фоновое обновление параметров работы программы';vi='Cập nhật nền tham số làm việc của chương trình'"));
...
"от фонового задания при обновлении параметров работы программы.';vi='Lỗi nhận kết quả"
"từ nhiệm vụ nền khi cập nhật tham số làm việc của chương trình.'");
