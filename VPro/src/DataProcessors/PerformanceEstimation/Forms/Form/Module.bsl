...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If OverallSystemPerformance.IsEmpty() Then
		Object.OverallSystemPerformance = NStr("en='Overall application performance';ru='Общая производительность системы';vi='Tổng hiệu suất của hệ thống'");
	Else
	...
	Except
		MessageText = NStr("en='Cannot import settings.';ru='Не удалось загрузить настройки.';vi='Không thể kết nhập tùy chỉnh.'");
...
"Рассчитать целевое время невозможно.';vi='Không có việc đo hiệu suất."
"Không thể tính toán thời gian đích.'"));
		Return;
	EndIf;
	
	Notification = New NotifyDescription("SpecifyEndAPDEX", ThisObject);
	ToolTip = NStr("en='Enter required APDEX value';ru='Введите желаемое значение APDEX';vi='Hãy nhập giá trị mong muốn APDEX'"); 
	APDEX = 0;
...
"Допустимые значения от 0 до 1.';vi='Nhập sai chỉ tiêu APDEX."
...
Procedure RefreshIndicators(FilterValues = Undefined)
	...
	If TableOfKeyOperations = Undefined Then
		CommonUseClientServer.MessageToUser(NStr("en='Period was set up incorrectly.';ru='Период установлен не верно.';vi='Kỳ thiết lập sai.'"));
		Return;
	EndIf;
...
Procedure RefreshChart(TableOfKeyOperations)
	...
	HeaderText = NStr("en='Performance chart from %1 to %2 - step: %3';ru='Диаграмма производительности с %1 по %2 - шаг: %3';vi='Biểu đồ hiệu suất từ %1 đến %2 - bước: %3'");
...
Function CalculateDateTimeSegment(StartDate, EndDate, IndexPeriod)
	...
	If NumberOfSteps <= IndexPeriod Then
		Raise NStr("en='Number of steps cannot be less than index.';ru='Количество шагов не может быть меньше индекса.';vi='Số lượng bước không thể nhỏ hơn chỉ mục.'");
	EndIf;
	...
	For Each TSRow IN Object.Performance Do
		...
		Then
			...
			CommonUseClientServer.MessageToUser(
				NStr("en='Target time must be filled in.';ru='Целевое время обязательно должно быть заполнено.';vi='Thời gian đích bắt buộc phải điền.'"),
				,
...
Procedure SelectFileAskedExport(FileOperationsExtensionConnected, AdditionalParameters) Export
	...
	Status(NStr("en='Exporting data...';ru='Экспорт данных...';vi='Kết xuất dữ liệu...'"));
