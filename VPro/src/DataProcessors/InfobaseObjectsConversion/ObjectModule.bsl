...
Function ObjectRegistrationRules(ExchangePlanNode)
	...
	If TypeOf(ObjectChangeRecordRulesField) <> Type("ValueTable") Then
		...
		ObjectChangeRecordRulesField = ObjectRegistrationRules.Copy(Filter, "MetadataObjectName, FlagAttributeName");
...
Procedure ExecuteDataExport(DataProcessorForDataImport = Undefined) Export
	...
	If IsExchangeOverExternalConnection() Then
		...
		If DataProcessorForDataImport().ErrorFlag() Then
			...
			MessageString = NStr("en='EXTERNAL CONNECTION: %1';ru='ВНЕШНЕЕ СОЕДИНЕНИЕ: %1';vi='KẾT NỐI NGOÀI: %1'");
	...
	Else
		...
		XMLRulesOL = OLStirngXMLRules();
		ExchangeFile.WriteLine(XMLRulesOL);
...
Function OLStirngXMLRules()
	
	OLRules = XMLRules;
	IDMap = XRIDMap();
	For Each KeyValue In IDMap Do
		
		RepKeyStart = "<" + KeyValue.Key + " ";
		RepKeyEnd = "</" + KeyValue.Key + ">";
		
		If Not StrFind(OLRules, RepKeyStart) = 0
			and Not StrFind(OLRules, RepKeyEnd) = 0 Then
			
			
			RepValStart = "<" + KeyValue.Value + " ";
			RepValEnd = "</" + KeyValue.Value + ">";
			
			OLRules = StrReplace(OLRules, RepKeyStart, RepValStart);
			OLRules = StrReplace(OLRules, RepKeyEnd, RepValEnd);
			
		ElsIf Not StrFind(OLRules, "<" + KeyValue.Key ) = 0 
			And Not StrFind(OLRules, RepKeyEnd) = 0 Then
			
			RepValEnd = "</" + KeyValue.Value + ">";
			
			OLRules = StrReplace(OLRules, "<" + KeyValue.Key, "<" + KeyValue.Value);
			OLRules = StrReplace(OLRules, RepKeyEnd, RepValEnd);
			
		Else
			
			RepKey = "<" + KeyValue.Key + "/>";
			RepVal = "<" + KeyValue.Value + "/>";
			OLRules = StrReplace(OLRules, RepKey, RepVal);
			
		EndIf
		
	EndDo;
	
	For Each KeyValue In AttrMap() Do
...
Function XRIDMap()
	...
	Result.Insert("SearchFields", "ПоляПоиска");
	Result.Insert("TableFields", "ПоляТаблицы");
	Result.Insert("SearchInTabularSections", "ПоискПоТабличнымЧастям");
	Result.Insert("Queries", "Запросы");
	Result.Insert("Algorithms", "Алгоритмы");
	Result.Insert("Algorithm", "Алгоритм");
	Result.Insert("Text", "Текст");
	Result.Insert("DataClearingRules", "ПравилаОчисткиДанных");
	Result.Insert("ExchangeRules", "ПравилаОбмена");
	...
	Result.Insert("BeforeGetChangedObjects", "ПередПолучениемИзмененныхОбъектов");
	Result.Insert("AfterGetExchangeNodeDetails", "ПослеПолученияИнформацииОбУзлахОбмена");
	Result.Insert("BeforeDataExport", "ПередВыгрузкойДанных");
	Result.Insert("BeforeObjectImport", "ПередЗагрузкойДанных");
	Result.Insert("OnGetDeletionInfo", "ПриПолученииИнформацииОбУдалении");
	Result.Insert("Parameters", "Параметры");
	Result.Insert("Parameter", "Параметр");
	
	Result.Insert("DataProcessors", "Обработки");
	Result.Insert("DataProcessor", "Обработкa");
	Result.Insert("ObjectConversionRules", "ПравилаКонвертацииОбъектов");
	Result.Insert("Group", "Группа");
	
	Result.Insert("Code", "Код");
	Result.Insert("Order", "Порядок");
	Result.Insert("Rule", "Правило");
	Result.Insert("Receiver", "Источник");
	Result.Insert("Properties", "Свойства");
	
	Result.Insert("SynchronizeByID", "СинхронизироватьПоИдентификатору");
	Result.Insert("ConversionRuleCode", "КодПравилаКонвертации");
	Result.Insert("BeforeExport", "ПередВыгрузкой");
	Result.Insert("ParameterForTransferName", "ИмяПараметраДляПередачи");
	Result.Insert("BeforeProcessExport", "ПередОбработкойВыгрузки");
	
	Return Result;
	
	//Result.Insert("Name=", "Имя=");
	//Result.Insert("Kind=", "Вид=");
...
Function AttrMap()
	...
	Result.Insert("Kind=", "Вид=");
	Result.Insert("Type=", "Тип=");
	Result.Insert("UsedOnImport=", "ИспользуетсяПриЗагрузке=");
...
Procedure ExecuteDataImport() Export
	...
	Try
		...
		Try
			...
			If ErrorFlag() Then
				Raise NStr("en='Errors occurred when importing data.';ru='Возникли ошибки при загрузке данных.';vi='Đã xuất hiện lỗi khi kết nhập dữ liệu.'");
			EndIf;
...
Procedure ExecuteDataImportToInformationBase(TableToImport) Export
	...
	MessageString = NStr("en='Data exchange process start for node: %1';ru='Начало процесса обмена данными для узла: %1';vi='Bắt đầu quá trình trao đổi dữ liệu đối với nút: %1'", CommonUseClientServer.MainLanguageCode());
	...
	// Record in the events log monitor.
	MessageString = NStr("en='%1, %2; %3 objects are processed';ru='%1, %2; Обработано %3 объектов';vi='%1, %2; Đã xử lý %3 đối tượng'", CommonUseClientServer.MainLanguageCode());
...
Procedure RunExchangeMessageAnalysis(AnalysisParameters = Undefined) Export
	...
	Try
		...
		Try
			...
			If ErrorFlag() Then
				Raise NStr("en='Errors occurred when analyzing data.';ru='Возникли ошибки при анализе данных.';vi='Đã xuất hiện lỗi khi phân tích dữ liệu.'");
			EndIf;
...
Procedure ExternalConnectionConversionHandlerBeforeDataImport(Cancel) Export
	...
	If Not IsBlankString(Conversion.BeforeDataImport) Then
		...
		Except
			WriteInformationAboutErrorConversionHandlers(22, ErrorDescription(), NStr("en='BeforeDataImport (Conversion)';ru='ПередЗагрузкойДанных (конвертация)';vi='BeforeDataImport (chuyển đổi)'"));
			Cancel = True;
		EndTry;
...
Procedure ExternalConnectionAfterDataImport() Export
	...
	If Not ErrorFlag() Then
		...
		If Not IsBlankString(Conversion.AfterDataImport) Then
			...
			Except
				WriteInformationAboutErrorConversionHandlers(23, ErrorDescription(), NStr("en='AfterDataImport (conversion)';ru='ПослеЗагрузкиДанных (конвертация)';vi='AfterDataImport (chuyển đổi)'"));
			EndTry;
...
Procedure WriteToFileObjectDeletion(Ref, Val ReceiverType, Val SourceType) Export
	...
	Receiver = CreateNode(OLDes("ObjectDeletion"));
	
	SetAttribute(Receiver, OLDes("ReceiverType"), ReceiverType);
	SetAttribute(Receiver, OLDes("SourceType"), SourceType);
	
	SetAttribute(Receiver, OLDes("UUID"), Ref.UUID());
...
Procedure WriteObjectToIB(Object, Type, WriteObject = False, Val SendBack = False) Export
	...
	If CommonUseReUse.DataSeparationEnabled()
		...
		ErrorMessageString = NStr("en='Attempting to change shared data (%1) in split session.';ru='Попытка изменения неразделенных данных (%1) в разделенном сеансе.';vi='Thử thay đổi dữ liệu chưa chia tách (%1) trong phiên làm việc đã chia tách.'");
...
Function DumpByRule(Source					= Undefined,
	...
	If CommentObjectProcessingFlag Then
		...
		NameActions = ?(GetRefNodeOnly, NStr("en='Conversion of reference to object';ru='Конвертация ссылки на объект';vi='Chuyển đổi tham chiếu đến đối tượng'"), NStr("en='Object conversion';ru='Конвертация объекта';vi='Chuyển đổi đối tượng'"));
		
		MessageText = NStr("en='[NameActions]: [Object]([ObjectType]), OCR: [OCR](OCRDescription)';ru='[ИмяДействия]: [Объект]([ТипОбъекта]), ПКО: [ПКО](НаименованиеПКО)';vi='[NameActions]: [Object]([ObjectType]), OCR: [OCR](OCRDescription)'");
	...
	If HasBeforeConvertObjectGlobalHandler Then
		...
		Except
			WriteInformationAboutOCRHandlerErrorDump(64, ErrorDescription(), OCR, Source, NStr("en='BeforeObjectConversion (global)';ru='ПередКонвертациейОбъекта (глобальный)';vi='BeforeObjectConversion (toàn cục)'"));
		EndTry;
...
"Если подходящего значения приемника нет, то указать пустое значение.';vi='Trong các quy tắc chuyển đổi giá trị (QT3) cần so sánh giá trị Nguồn với giá trị Đích."
"Nếu không có giá trị đích thích hợp, thì chỉ ra giá trị trống.'");
	...
	If DontExportByValueMap Then
		...
		If OCR.SearchProperties.Count() > 0 
			...
			//	Generate ref node
			Referencenode = CreateNode(OLDes("Ref"));
						
			If MustRememberObject Then
				
				If IsRuleWithGlobalObjectExport Then
					SetAttribute(Referencenode, OLDes("GSn"), NPP);
				Else
					SetAttribute(Referencenode, OLDes("NPP"), NPP);
				EndIf;
				
			EndIf;
			
			If DoNotCreateIfNotFound Then
				SetAttribute(Referencenode, OLDes("DoNotCreateIfNotFound"), DoNotCreateIfNotFound);
			EndIf;
			
			If OCR.SearchBySearchFieldsIfNotFoundByID Then
				SetAttribute(Referencenode, OLDes("ContinueSearch"), True);
			EndIf;
			
			If RecordObjectChangeAtSenderNode Then
				SetAttribute(Referencenode, OLDes("RecordObjectChangeAtSenderNode"), RecordObjectChangeAtSenderNode);
			EndIf;
			
			WriteExchangeObjectsPriority(ExchangeObjectPriority, Referencenode);
			
			If DontReplaceCreatedInTargetObject Then
				SetAttribute(Referencenode, OLDes("DontReplaceCreatedInTargetObject"), DontReplaceCreatedInTargetObject);				
			EndIf;
	...
	If Receiver = Undefined Then
		...
		Receiver = CreateNode(OLDes("Object"));
		
		If Not ExportRegisterRecordSetRow Then
			
			If IsRuleWithGlobalObjectExport Then
				SetAttribute(Receiver, OLDes("GSn"), NPP);
			Else
				SetAttribute(Receiver, OLDes("NPP"), NPP);
			EndIf;
			
			SetAttribute(Receiver, OLDes("Type"), 			OCR.Receiver);
			SetAttribute(Receiver, OLDes("Rulename"),	OCR.Name);
			...
			If DoNotReplaceObjectOnImport Then
				SetAttribute(Receiver, OLDes("Donotreplace"), "True");
			EndIf;
			
			If Not IsBlankString(AutonumerationPrefix) Then
				SetAttribute(Receiver, OLDes("AutonumerationPrefix"), AutonumerationPrefix);
			EndIf;
			
			If Not IsBlankString(WriteMode) Then
				
				SetAttribute(Receiver, OLDes("WriteMode"), WriteMode);
				If Not IsBlankString(PostingMode) Then
					SetAttribute(Receiver, OLDes("PostingMode"), PostingMode);
				EndIf;
...
Procedure DumpRegister(RecordSetForExport, 
	...
	Receiver = CreateNode(OLDes("RegisterRecordSet"));
	...
	SetAttribute(Receiver, OLDes("NPP"), NPP);
	SetAttribute(Receiver, OLDes("Type"), StrReplace(Rule.Receiver, "InformationRegisterRecord.", OLDes("InformationRegisterRecordSet")+"."));
	SetAttribute(Receiver, OLDes("Rulename"), Rule.Name);
	
	WriteExchangeObjectsPriority(ExchangeObjectPriority, Receiver);
	
	ExportingEmptySet = RegisterRecordCount = 0;
	If ExportingEmptySet Then
		SetAttribute(Receiver, OLDes("EmptySet"), True);
	EndIf;
	
	Receiver.WriteStartElement(OLDes("Filter"));
	...
	Receiver.WriteStartElement(OLDes("RecordSetRows"));
...
"File name %1."
"Error description: %2';ru='Ошибка при открытии файла для записи сообщения обмена."
"Имя файла %1."
"Описание ошибки: %2';vi='Lỗi khi mở tệp để ghi lại thông báo trao đổi."
"Tên tệp %1."
"Mô tả lỗi: %2'"),
	...
	LF = TypeOf(NodeForExchange) = Type("ExchangePlanRef.ExchangeCompanyManagementAccounting");
	
	TempXMLWriter.WriteStartElement(OLDes("ExchangeFile"));
	
	SetAttribute(TempXMLWriter, OLDes("FormatVersion"), 	VersionOfExchangeEventFormat());
	SetAttribute(TempXMLWriter, OLDes("ExportDate"), 		CurrentSessionDate());
	SetAttribute(TempXMLWriter, OLDes("SourceConfigurationName"), 	Conversion.Source);
	SetAttribute(TempXMLWriter, OLDes("SourceConfigurationVersion"), 	Conversion.SourceConfigurationVersion);
	SetAttribute(TempXMLWriter, OLDes( "TargetConfigurationName"), 		Conversion.Receiver);
	SetAttribute(TempXMLWriter, OLDes("ConversionRuleIDs"), Conversion.ID);
...
Procedure CloseFile()
	...
	LF = Upper(ExchangeFileName) = Upper("ExchangeCompanyManagementAccounting");

	ExchangeFile.WriteLine("</" + OLDes("ExchangeFile") + ">");
	ExchangeFile.Close();
...
Procedure ExchangeProtocolInitialization()
	...
	Except
		DataLogFile = Undefined;
		MessageString = NStr("en='An error occurred when attempting to write to data protocol file: %1. Error description: %2';ru='Ошибка при попытке записи в файл протокола данных: %1. Описание ошибки: %2';vi='Lỗi khi thử ghi ra tệp giao thức dữ liệu: %1. Mô tả lỗi: %2'",
...
Procedure ImportPCR( ExchangeRules,
	...
	While ExchangeRules.Read() Do
		...
		ElsIf NodeName = "CastToLength" Or NodeName = "ПриводитьКДлине" Then
...
"Требуется выполнить загрузку правил обмена повторно.';vi='Phiên bản định dạng lưu quy tắc trao đổi không như phiên bản dự tính."
"Cần thực hiện kết nhập lại quy tắc trao đổi.'"
	...
	HasAfterObjectImportGlobalHandler     = Not IsBlankString(Conversion.AfterObjectImport);
...
Procedure StartMessageReader(MessageReader, DataAnalysis = False)
	...
	If ExchangeFile.NodeType <> XMLNodeType.StartElement Then
		Raise NStr("en='Error in exchange message format.';ru='Ошибка формата сообщения обмена.';vi='Lỗi định dạng thông báo trao đổi.'");
	EndIf;
	
	If Not (ExchangeFile.LocalName = "ExchangeFile" Or ExchangeFile.LocalName = "ФайлОбмена") Then
		Raise NStr("en='Error in exchange message format.';ru='Ошибка формата сообщения обмена.';vi='Lỗi định dạng thông báo trao đổi.'");
	EndIf;
	
	LF = (ExchangeFile.LocalName = "ФайлОбмена");
	IncomingExchangeMessageFormatVersionField = deAttribute(ExchangeFile, StringType, ?(LF, "ВерсияФормата", "FormatVersion"));
	
	SourceConfigurationVersion = "";
	Conversion.Property("SourceConfigurationVersion", SourceConfigurationVersion);
	SourceVersionFromRules = deAttribute(ExchangeFile, StringType, ?(LF, "ВерсияКонфигурацииИсточника","SourceConfigurationVersion"));
	MessageText = "";
	...
	If Not(ExchangeFile.LocalName = "ExchangeRules" Or ExchangeFile.LocalName = "ПравилаОбмена") Then
		Raise NStr("en='Error in exchange message format.';ru='Ошибка формата сообщения обмена.';vi='Lỗi định dạng thông báo trao đổi.'");
	EndIf;
	...
	If ConversionRulesTable.Count() = 0 Then
		...
		If ErrorFlag() Then
			Raise NStr("en='Errors occurred when importing data exchange rules.';ru='При загрузке правил обмена данными возникли ошибки.';vi='Khi kết nhập quy tắc trao đổi dữ liệu đã nảy sinh lỗi.'");
		EndIf;
	Else
	...
	If Not IsBlankString(Conversion.BeforeDataImport) Then
		...
		Except
			Raise WriteInformationAboutErrorConversionHandlers(22, ErrorDescription(), NStr("en='BeforeDataImport (Conversion)';ru='ПередЗагрузкойДанных (конвертация)';vi='BeforeDataImport (chuyển đổi)'"));
		EndTry;
		
		If Cancel Then
			Raise NStr("en='Cancel importing exchange message in the BeforeDataImport handler (conversion).';ru='Отказ от загрузки сообщения обмена в обработчике ПередЗагрузкойДанных (конвертация).';vi='Từ chối kết nhập thông điệp trao đổi trong hàm sự kiện BeforeDataImport (chuyển đổi)."
"'");
		EndIf;
	...
	If ExchangeFile.LocalName = "CustomSearchSetup" Or ExchangeFile.LocalName = "НастройкаПользовательскогоПоиска" Then
		ImportInformationAboutCustomSearchFields();
		ExchangeFile.Read();
	EndIf;
	
	// InformationAboutDataTypes (optional)
	If ExchangeFile.LocalName = "DataTypeInfo" Or ExchangeFile.LocalName = "ИнформацияОТипахДанных" Then
		...
		Else
			...
			If ErrorFlag() Then
				Raise NStr("en='Errors occurred when importing information on data types.';ru='При загрузке информации о типах данных возникли ошибки.';vi='Khi kết nhập thông tin về kiểu dữ liệu, đã nảy sinh lỗi.'");
			EndIf;
		EndIf;
		ExchangeFile.Read();
	EndIf;
	...
	If ExchangeFile.LocalName = "ParameterValue" Or ExchangeFile.LocalName = "ЗначениеПараметра" Then
	...
	If ExchangeFile.LocalName = "AfterParameterExportAlgorithm" Or ExchangeFile.LocalName = "АлгоритмПослеЗагрузкиПараметров" Then
	...
	If Not (ExchangeFile.LocalName = "ExchangeData" Or ExchangeFile.LocalName = "ДанныеПоОбмену") Then
		Raise NStr("en='Error in exchange message format.';ru='Ошибка формата сообщения обмена.';vi='Lỗi định dạng thông báo trao đổi.'");
	EndIf;
	...
	If TransactionActive() Then
		Raise NStr("en='Lock of data receipt cannot be set in an active transaction.';ru='Блокировка получения данных не может быть установлена в активной транзакции.';vi='Không thể khóa việc nhận dữ liệu ở giao tác hiện hoạt.'");
	EndIf;
...
""
"Details: %1';ru='Ошибка установки блокировки на обмен данными."
"Возможно, обмен данными выполняется другим сеансом."
""
"Подробности: %1';vi='Lỗi cài đặt phong tỏa trao đổi dữ liệu."
...
Procedure FinishMessageReading(Val MessageReader)
	...
	If Not IsBlankString(Conversion.AfterDataImport) Then
		...
		Except
			Raise WriteInformationAboutErrorConversionHandlers(23, ErrorDescription(), NStr("en='AfterDataImport (conversion)';ru='ПослеЗагрузкиДанных (конвертация)';vi='AfterDataImport (chuyển đổi)'"));
		EndTry;
	...
	Try
		...
		If Cancel = True Then
			...
			If Not IsBlankString(CancelReason) Then
				...
				MessageString = NStr("en='Decline exchange message import in handler AfterParametersImport (conversion) due to: %1';ru='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация) по причине: %1';vi='Từ chối kết nhập thông báo trao đổi trong hàm sự kiện AfterParametersImport (chuyển đổi) theo nguyên nhân: %1'");
				MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString, CancelReason);
				Raise MessageString;
			Else
				Raise NStr("en='Cancel importing exchange message in the AfterImportOfParameters handler (conversion).';ru='Отказ от загрузки сообщения обмена в обработчике ПослеЗагрузкиПараметров (конвертация).';vi='Từ chối kết nhập thông báo trao đổi trong hàm sự kiện AfterImportOfParameters (chuyển đổi).'");
			EndIf;
...
Procedure DeleteDocumentRegisterRecords(DocumentObject)
	...
	For Each RegisterRecordRow IN RegisterRecordTable Do
		...
		If Not AccessRight("Update", Set.Metadata()) Then
			...
			ErrorMessage = NStr("en='Access right violation: %1';ru='Нарушение прав доступа: %1';vi='Vi phạm quyền truy cập: %1'");
	...
	For Each RegisterRecordRow IN RecordTableRowToProcessArray Do		
		...
		Except
			...
			ErrorMessage = NStr("en='Operation is not executed: %1 %2';ru='Операция не выполнена: %1 %2';vi='Đã thực hiện giao dịch: %1 %2'");
...
Function CreateNewObject(Type, SearchProperties, Object = Undefined, 
	...
	If TypeName = "Catalog"
		...
		IsFolder = SearchProperties[OLDES("IsFolder")];
...
Function ReadProperty(Type, DontCreateObjectIfNotFound = False, PropertyNotFoundByRef = False, OCRName = "")
	...
	LF = StringFunctionsClientServer.OnlyLatinInString(ExchangeFile.LocalName);
	
	While ExchangeFile.Read() Do
		
		NodeName = ExchangeFile.LocalName;
				
		If NodeName = OLDes("Value") Then
			
			SearchByProperty = deAttribute(ExchangeFile, StringType, OLDes("Property"));
			Value         = deItemValue(ExchangeFile, Type, SearchByProperty, False);
			PropertyExistence = True;
			
		ElsIf NodeName = OLDes("Ref") Then
		...
		ElsIf (NodeName = OLDes("Property") OR NodeName = OLDes( "ParameterValue")) AND (ExchangeFile.NodeType = XMLNodeTypeEndElement) Then
		...
		ElsIf NodeName = OLDes( "Expression") Then
			
			Value = Eval(deItemValue(ExchangeFile, StringType, , False));
			PropertyExistence = True;
			
		ElsIf NodeName = OLDes( "Empty") Then
...
Procedure SetObjectSearchAttributes(FoundObject, SearchProperties, SearchPropertiesDontReplace, 
	...
	For Each Property IN SearchProperties Do
		...
		TempName = StrReplace(Name, "{", "");
		TempName = StrReplace(TempName, "}", "");
		LF = StringFunctionsClientServer.OnlyLatinInString(TempName);
		
		If Name = OLDes("IsFolder")
			OR Name = OLDes("{UUID}")
			OR Name = OLDes("{PredefinedItemName}")
			OR Name = OLDes("{SearchKeyInIBSource}")
			OR Name = OLDes("{SearchKeyInIBReceiver}")
			OR Name = OLDes("{TypeNameInIBSource}")
			OR Name = OLDes( "{TypeNameInIBReceiver}") Then
						
			Continue;
						
		ElsIf Name = OLDes("DeletionMark") Then
...
Function GetPropertyType()
	...
	LF = StringFunctionsClientServer.OnlyLatinInString(ExchangeFile.LocalName);
	PropertyTypeString = ELDes(deAttribute(ExchangeFile, StringType, OLDes("Type")));
...
Procedure ReadSearchPropertiesFromFile(SearchProperties, SearchPropertiesDontReplace, TypeInformation,
	...
	While ExchangeFile.Read() Do
		...
		If    NodeName = OLDes("Property")
			OR NodeName = OLDes( "ParameterValue") Then
			
			IsParameter = (NodeName = OLDes("ParameterValue"));
			
			Name = deAttribute(ExchangeFile, StringType, OLDes("Name"));
			
			SourceTypeAsString = deAttribute(ExchangeFile, StringType, OLDes("ReceiverType"));
			ReceiverTypeAsString = deAttribute(ExchangeFile, StringType, OLDes("SourceType"));
			
			UUIDProperty = (Name = OLDes("{UUID}"));
			
			If UUIDProperty Then
				
				PropertyType = StringType;
				
			ElsIf Name = OLDes("{PredefinedItemName}")
				  OR Name = OLDes("{SearchKeyInIBSource}")
				  OR Name = OLDes("{SearchKeyInIBReceiver}")
				  OR Name = OLDes("{TypeNameInIBSource}")
				  OR Name = OLDes("{TypeNameInIBReceiver}") Then
			...
			DontReplaceProperty = deAttribute(ExchangeFile, BooleanType, OLDes("Donotreplace"));
			
			SearchByEqualDate = SearchByEqualDate 
						OR deAttribute(ExchangeFile, BooleanType, OLDes("SearchByEqualDate"));
			//
			OCRName = deAttribute(ExchangeFile, StringType, OLDes("OCRName"));
			...
			If (Name = OLDes("IsFolder")) AND (PropertyValue <> True) Then
		...
		ElsIf (NodeName = OLDes("Ref")) AND (ExchangeFile.NodeType = XMLNodeTypeEndElement) Then
...
Function FindItemUsingQuery(PropertyStructure, SearchProperties, ObjectType = Undefined, 
	...
	For Each Property IN SearchProperties Do
		...
		TempParameterName = StrReplace(ParameterName, "{","");
		TempParameterName = StrReplace(TempParameterName, "}","");
		
		LF = StringFunctionsClientServer.OnlyLatinInString(TempParameterName);
		
		If ParameterName = OLDes("{UUID}") Or ParameterName = OLDes("{PredefinedItemName}") Then
			Continue;
		EndIf;
...
Function FindObjectByRef(ObjectType, 
	...
	LF = (ExchangeFile.LocalName = "Ссылка");
	
	GlobalRefNPP = deAttribute(ExchangeFile, NumberType, OLDes("GSn"));
	RefNPP           = deAttribute(ExchangeFile, NumberType, OLDes("NPP"));
	
	// Shows that object should be registered for export for node-sender (sending back).
	RecordObjectChangeAtSenderNode = deAttribute(ExchangeFile, BooleanType, OLDes( "RecordObjectChangeAtSenderNode"));
	
	FlagDontCreateObjectIfNotFound = deAttribute(ExchangeFile, BooleanType, OLDes("DoNotCreateIfNotFound"));
	...
	DontReplaceCreatedInTargetObjectFlag = deAttribute(ExchangeFile, BooleanType, OLDes("DontReplaceCreatedInTargetObject"));
	...
	SearchBySearchFieldsIfNotFoundByID = deAttribute(ExchangeFile, BooleanType, OLDes("ContinueSearch"));
	...
	UUIDProperty = SearchProperties[OLDes("{UUID}")];
	PredefinedNameProperty    = SearchProperties[OLDes("{PredefinedItemName}")];
	...
	While Variantsearchnumber <= 10
		...
		Except
			...
			WriteInformationAboutOCRHandlerErrorImport(73, ErrorDescription(), "", "", 
				ObjectType, Undefined, NStr("en='Sequence of search fields';ru='Последовательность полей поиска';vi='Chuỗi nối tiếp các trường tìm kiếm'"));
	...
	If MainObjectSearchMode
		...
		AND (ObjectTypeName = OLDes("Document")
		OR ObjectTypeName = OLDes("Task")
		OR ObjectTypeName = OLDes("BusinessProcess")) Then
...
Procedure SetExchangeFileCollectionProperties(Object, ExchangeFileCollection, TypeInformation,
	...
	BranchName = TabularSectionName + OLDes("TabularSection");
	...
	While ExchangeFile.Read() Do
		...
		NodeName = ExchangeFile.LocalName;
		LF = StringFunctionsClientServer.OnlyLatinInString(NodeName);
		
		If NodeName = OLDes("Property") OR
			 NodeName = OLDes("ParameterValue") Then
		...
		ElsIf NodeName = OLDes("ExtDimensionDr") OR NodeName = OLDes("ExtDimensionCr") Then
			
			deIgnore(ExchangeFile);
				
		ElsIf (NodeName = OLDes("Record")) AND (ExchangeFile.NodeType = XMLNodeTypeEndElement) Then
...
Procedure ImportTabularSection(Object, TabularSectionName, DocumentTypeCommonInformation, ObjectParameters, OCR)
	...
	Except
		...
		Text = NStr("en='Tabular section name: %1';ru='Имя табличной части: %1';vi='Tên phần bảng: %1'");
...
Procedure FillExchangeFileCollection(Object, ExchangeFileCollection, TabularSectionName, DocumentTypeCommonInformation, ObjectParameters, KeySearchFieldArray, OrderFieldName)
	...
	While ExchangeFile.Read() Do
		...
		ElsIf (NodeName = OLDes("TabularSection")) AND (ExchangeFile.NodeType = XMLNodeTypeEndElement) Then
...
Procedure CommentsToObjectImport(NPP, Rulename, Source, ObjectType, GNPP = 0)
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = NStr("en='Import object No. %1';ru='Загрузка объекта № %1';vi='Kết nhập đối tượng số %1'");
...
Function ReadInformationAboutObjectRegistration()
	...
	UniqueReceiverHandle = deAttribute(ExchangeFile, StringType, OLDes("UniqueSourceHandle"));
	UniqueSourceHandle = deAttribute(ExchangeFile, StringType,  OLDes("UniqueReceiverHandle"));
	ReceiverType                     = deAttribute(ExchangeFile, StringType,  OLDes("SourceType"));
	SourceType                     = deAttribute(ExchangeFile, StringType,  OLDes("ReceiverType"));
	EmptySet                      = deAttribute(ExchangeFile, BooleanType,  OLDes("EmptySet"));
	...
	deIgnore(ExchangeFile, OLDes("ObjectChangeRecordData"));
...
Function ReadRegisterRecordSet()
	...
	NPP						= deAttribute(ExchangeFile, NumberType, OLDES("NPP"));
	Rulename				= deAttribute(ExchangeFile, StringType, OLDES("Rulename"));
	ObjectTypeAsString       = deAttribute(ExchangeFile, StringType, OLDES("Type"));
	ExchangeObjectPriority  = GetExchangeObjectPriority(ExchangeFile);
	
	IsEmptySet			= deAttribute(ExchangeFile, BooleanType, OLDES("EmptySet"));
	...
	RegisterRowTypeName = StrReplace(ObjectTypeAsString, OLDes("InformationRegisterRecordSet") +".", OLDes("InformationRegisterRecord") +".");
	RegisterName = StrReplace(ObjectTypeAsString, OLDes("InformationRegisterRecordSet") + ".", "");
	...
	If HasBeforeObjectImportGlobalHandler Then
		...
		Except
			...
			ObjectType, Undefined, NStr("en='BeforeObjectImport (global)';ru='ПередЗагрузкойОбъекта (глобальный)';vi='BeforeObjectImport (toàn cục)'"));
			
		EndTry;
		
		If Cancel Then	//	Denial of the object import
			
			deIgnore(ExchangeFile, OLDes("Object"));
			Return Undefined;
	...
	While ExchangeFile.Read() Do
		...
		If NodeName = "Filter" OR NodeName = "Отбор" Then
		...
		ElsIf NodeName = "Property" Or NodeName = "Свойство"
			OR NodeName = "ParameterValue" OR NodeName = "ЗначениеПараметра" Then
			
			IsParameterForObject = (NodeName = "ParameterValue" Or NodeName = "ЗначениеПараметра");
			...
			If IsParameterForObject Then
				...
				Else
					...
					AddComplexParameterIfNeeded(RecordSetParameters, OLDES("Rows"), RecNo, Name, Value);
				EndIf;
		...
		ElsIf NodeName = "RecordSetRows" Or NodeName = "СтрокиНабораЗаписей" Then
		...
		ElsIf NodeName = "Object" Or NodeName = "Объект" Then
		...
		ElsIf NodeName = "RegisterRecordSet" Or NodeName = "НаборЗаписейРегистра" AND ExchangeFile.NodeType = XMLNodeTypeEndElement Then
...
Function ELDes(ParameterID)
	
	
	ResultMap = New Map;
	ResultMap.Insert("ПланОбменаСсылка.ОбменУправлениеНебольшойФирмойБухгалтерия", "ExchangePlanRef.ExchangeCompanyManagementAccounting");
	ResultMap.Insert("ДатаНачалаВыгрузкиДокументов", "DocumentsDumpStartDate");
	ResultMap.Insert("РежимВыгрузкиПриНеобходимости", "ExportModeOnDemand");
	ResultMap.Insert("ПеречислениеСсылка.РежимыВыгрузкиОбъектовОбмена", "EnumRef.ExchangeObjectsExportModes");
	ResultMap.Insert("ВыгружатьПриНеобходимости", "ExportIfNecessary");
	ResultMap.Insert("ЗагружатьДокументыИзБухгалтерииПредприятия", "ImportDocumentsFromEnterprisesAccounting");
	ResultMap.Insert("РучнойОбмен", "ManualExchange");
	ResultMap.Insert("ИспользоватьОтборПоОрганизациям", "UseCompaniesFilter");
	ResultMap.Insert("СтатьяЗатрат", "ExpenseItem");
	ResultMap.Insert("СтатьяПрочихДоходовРасходов", "DebitItem");
	ResultMap.Insert("РегистрироватьИзменения", "RegisterChanges");
	
	Result = ResultMap.Get(ParameterID);
	If Result = Undefined Then
		Result = ParameterID;
	EndIf;
...
Function ReadObject(UUIDString = "")
	...
	LF = (ExchangeFile.LocalName = "Объект");
	
	NPP						= deAttribute(ExchangeFile, NumberType, OLDes("NPP"));
	GNPP					= deAttribute(ExchangeFile, NumberType, OLDes("GSn"));
	Source				= deAttribute(ExchangeFile, StringType, OLDes("Source"));
	Rulename				= deAttribute(ExchangeFile, StringType, OLDes("Rulename"));
	Donotreplaceobject 		= deAttribute(ExchangeFile, BooleanType, OLDes("Donotreplace"));
	AutonumerationPrefix	= deAttribute(ExchangeFile, StringType, OLDes("AutonumerationPrefix"));
	ExchangeObjectPriority  = GetExchangeObjectPriority(ExchangeFile);
	
	ObjectTypeAsString       = deAttribute(ExchangeFile, StringType, OLDes("Type"));
	ObjectType 				= Type(ELDes(ObjectTypeAsString));
	TypeInformation = MapOfDataTypesForImport()[ObjectType];
	...
	If ObjectTypeName = OLDes("Document") Then
		
		WriteMode     = deAttribute(ExchangeFile, StringType, OLDes("WriteMode"));
		PostingMode = deAttribute(ExchangeFile, StringType, OLDes("PostingMode"));
	...
	While ExchangeFile.Read() Do
		...
		If NodeName = OLDes("Property")
			...
			IsParameterForObject = (NodeName = OLDes("ParameterValue"));
			...
			If Object = Undefined Then
				...
				If Object = Undefined Then
					...
					If ObjectTypeName = OLDes("Constants") Then
			...
			Name                = ELDes(deAttribute(ExchangeFile, StringType, OLDes("Name")));
			DontReplaceProperty = deAttribute(ExchangeFile, BooleanType, OLDes("Donotreplace"));
			OCRName             = deAttribute(ExchangeFile, StringType, OLDes("OCRName"));
		...
		ElsIf NodeName = OLDes("Ref") Then
			...
			If ObjectTypeName = OLDes("Enum") Then
		...
		ElsIf NodeName = OLDes("TabularSection")
			  OR NodeName = OLDes("RecordSet") Then
			//
			
			If DataImportToValueTableMode()
				AND ObjectTypeName <> OLDes("ExchangePlan") Then
				deIgnore(ExchangeFile, NodeName);
				Continue;
			EndIf;
			...
			Name                = deAttribute(ExchangeFile, StringType, OLDes("Name"));
			DontReplaceProperty = deAttribute(ExchangeFile, BooleanType, OLDes("Donotreplace"));
			Donotclear          = deAttribute(ExchangeFile, BooleanType, OLDes("Donotclear"));
		...
		ElsIf (NodeName = OLDes("Object")) AND (ExchangeFile.NodeType = XMLNodeTypeEndElement) Then
			...
			If HasAfterObjectImportGlobalHandler Then
				...
				Except
					...
					WriteInformationAboutOCRHandlerErrorImport(54, ErrorDescription(), Rulename, Source,
						ObjectType, Object, NStr("en='AftertObjectImport (global)';ru='ПослеЗагрузкиОбъекта (глобальный)';vi='AftertObjectImport (toàn cục)'"));
			...
			If ObjectTypeName = OLDes("Document") Then
				...
				If WriteMode = OLDes("Posting") Then
					
					WriteMode = DocumentWriteMode.Posting;
					
				ElsIf WriteMode = OLDes("UndoPosting") Then
				...
				PostingMode = ?(PostingMode = OLDes("RealTime"), DocumentPostingMode.RealTime, DocumentPostingMode.Regular);
				...
				If DataImportToInformationBaseMode() Then
					...
					Except
						...
						MessageString = NStr("en='An error occurred while writing document: %1 Error description: %2';ru='Ошибка при записи документа: %1. Описание ошибки: %2';vi='Lỗi khi ghi chứng từ: %1. Mô tả lỗi: %2'");
			...
			ElsIf ObjectTypeName <> OLDes("Enum") Then
				
				If ObjectTypeName = OLDes("InformationRegister") Then
				...
				If DataImportToInformationBaseMode() Then
					...
					If Not (ObjectTypeName = OLDes("InformationRegister")
						 OR ObjectTypeName = OLDes("Constants")) Then
			...
			IsReferenceTypeObject = Not(ObjectTypeName = OLDes("InformationRegister")
										OR ObjectTypeName = OLDes("Constants"));
			
			Break;
			
		ElsIf NodeName = OLDes("SequenceRecordSet") Then
			
			deIgnore(ExchangeFile);
			
		ElsIf NodeName = OLDes("Types") Then
...
Function OLDes(ParamentID)
	
	LF = TypeOf(NodeForExchange) = Type("ExchangePlanRef.ExchangeCompanyManagementAccounting") Or TypeOf(ExchangeNodeForDataImport) = Type("ExchangePlanRef.ExchangeCompanyManagementAccounting");
	Result = ParamentID;
	If LF Then
		ParametersMap = DataExchangeReUse.ParametersMap();
		Result = ParametersMap.Get(ParamentID);
		If Result = Undefined Then
			Result = ParamentID;
		EndIf;
	EndIf;
...
Procedure DumpGroupOfProperties(Source, Receiver, IncomingData, OutgoingData, OCR, PGCR, PropertyCollectionNode, 
	...
	If TargetKind = "TabularSection" Then
		...
		CreateObjectsForRecordingDataToXML(PropertyNodeStructure, ObjectCollectionNode, TRUE, PGCR.Receiver, OLDes(MasterNodeName));
	...
	ElsIf Find(TargetKind, "RegisterRecordSet") > 0 Then
		...
		If Donotreplace Then
			...
			AddAttributeForRecordToXML(PropertyNodeStructure, ObjectCollectionNode, OLDes("Donotreplace"), "true");
						
		EndIf;
		
		If Donotclear Then
			
			AddAttributeForRecordToXML(PropertyNodeStructure, ObjectCollectionNode, OLDes("Donotclear"), "true");
...
"Имя файла ""%1""."
"Описание"
"ошибки: %2';vi='Lỗi khi tạo tệp tạm thời để kết xuất dữ liệu."
"Tên tệp ""%1""."
"Mô tả"
"lỗi: %2'"),
	...
	For Each CollectionObject IN ObjectsCollection Do
		...
		If PGCR.XMLNodeRequiredOnExport OR ExportGroupToFile Then
			CollectionObjectNode = CreateNode(OLDes("Record"));
		Else
			ObjectCollectionNode.WriteStartElement(OLDes("Record"));
	...
	If ExportGroupToFile Then
		TempRecordFile.WriteLine("</" + OLDes(MasterNodeName) + ">"); // close node
...
Procedure DefineOpportunityOfValueAssign(Value, ValueType, ReceiverType, PropertySet, TypeRequired)
	...
	If ValueType = StringType Then
		...
		ElsIf ReceiverType = "Number" Or ReceiverType = "Число" Then
					
			Value = Number(Value);
					
		ElsIf ReceiverType = "Boolean" Or ReceiverType = "Булево" Then
					
			Value = Boolean(Value);
					
		ElsIf ReceiverType = "Date" Or ReceiverType = "Дата" Then
					
			Value = Date(Value);
					
		ElsIf ReceiverType = "ValueStorage" Or ReceiverType = "ХранилищеЗначения" Then
					
			Value = New ValueStorage(Value);
					
		ElsIf ReceiverType = "UUID" Or ReceiverType = "УникальныйИдентификатор" Then
	...
	ElsIf ValueType = NumberType Then
		...
		If ReceiverType = "Number" Or ReceiverType = "Число" Or ReceiverType = "Строка"
	...
	ElsIf ValueType = DateType Then
		...
		If ReceiverType = "Date" Or ReceiverType = "Дата" Then
		ElsIf ReceiverType = "String" Or ReceiverType = "Строка" Then
	...
	ElsIf ValueType = BooleanType Then
		...
		If ReceiverType = "Boolean" OR ReceiverType = "Булево" Then
		ElsIf ReceiverType = "Number"  OR ReceiverType = "Число" Then
	...
	ElsIf ValueType = UUIDType Then
		...
		If ReceiverType = "UUID" Or ReceiverType = "УникальныйИдентификатор" Then
		ElsIf ReceiverType = "String" Or ReceiverType = "Строка" Then
...
Procedure ExecuteStructureRecordToXML(DataStructure, PropertyCollectionNode, IsOrdinaryProperty = True)
	...
	PropertyCollectionNode.WriteStartElement(?(IsOrdinaryProperty, OLDes("Property"), OLDes("ParameterValue")));
	
	For Each CollectionItem IN DataStructure Do
		
		If CollectionItem.Key = "Expression" Or CollectionItem.Key = "Выражение"
			OR CollectionItem.Key = "Value" Or CollectionItem.Key = "Значение"
			OR CollectionItem.Key = "NPP" Or CollectionItem.Key = "Нпп"
			OR CollectionItem.Key = "GSn" Or CollectionItem.Key = "ГНпп" Then
			
			deWriteItem(PropertyCollectionNode, CollectionItem.Key, CollectionItem.Value);
			
		ElsIf CollectionItem.Key = "Ref" Or CollectionItem.Key = "Ссылка" Then
...
Procedure CreateComplexInformationForRecordingToXML(DataStructure, Propirtiesnode, XMLNodeRequired, TargetName, ParameterName)
	...
	If IsBlankString(ParameterName) Then
		...
		CreateObjectsForRecordingDataToXML(DataStructure, Propirtiesnode, XMLNodeRequired, TargetName, OLDes("Property"));
		
	Else
		
		CreateObjectsForRecordingDataToXML(DataStructure, Propirtiesnode, XMLNodeRequired, ParameterName, OLDes("ParameterValue"));
...
Procedure CreateObjectsForRecordingDataToXML(DataStructure, Propirtiesnode, XMLNodeRequired, NodeName, XMLNodeDescription = "Property")
	...
	If XMLNodeRequired Then
		...
		SetAttribute(Propirtiesnode, OLDes("Name"), NodeName);
		
	Else
		
		DataStructure = New Structure(OLDes("Name"), NodeName);
...
Procedure DumpProperties(Source, 
	...
	If IsRefExport Then
		...
		If PredefinedItemName <> Undefined Then
			...
			PropertyCollectionNode.WriteStartElement(OLDes("Property"));
			SetAttribute(PropertyCollectionNode, OLDes("Name"), OLDes("{PredefinedItemName}"));
			deWriteItem(PropertyCollectionNode, OLDes("Value"), PredefinedItemName);
			PropertyCollectionNode.WriteEndElement();
	...
	For Each PCR IN PCRCollection Do
		...
		If PCR.SimplifiedPropertyExport Then
			...
			PropertyCollectionNode.WriteStartElement(OLDes("Property"));
			SetAttribute(PropertyCollectionNode, OLDes("Name"), PCR.Receiver);
			
			If Not IsBlankString(PCR.ReceiverType) Then
				
				SetAttribute(PropertyCollectionNode, OLDes("Type"), PCR.ReceiverType);
				
			EndIf;
			
			If PCR.Donotreplace Then
				
				SetAttribute(PropertyCollectionNode, OLDes("Donotreplace"), "true");
				
			EndIf;
			
			If PCR.SearchByEqualDate  Then
				
				SetAttribute(PropertyCollectionNode, OLDes("SearchByEqualDate"), "true");
			...
			deWriteItem(PropertyCollectionNode, OLDes("Value"), Value);
		...
		ElsIf PCR.Name = "{UUID}" Then
			...
			PropertyCollectionNode.WriteStartElement(OLDes("Property"));
			SetAttribute(PropertyCollectionNode, OLDes("Name"), OLDes("{UUID}"));
			SetAttribute(PropertyCollectionNode, OLDes("Type"), "String");
			SetAttribute(PropertyCollectionNode, OLDes("SourceType"), TypeForReceiver(OCR.SourceType));
			SetAttribute(PropertyCollectionNode, OLDes("ReceiverType"), TypeForReceiver(OCR.ReceiverType));
			deWriteItem(PropertyCollectionNode, OLDes("Value"), UUID);
			PropertyCollectionNode.WriteEndElement();
		...
		// Create property node
		PropertyNodeStructure = Undefined;
		Propertiesnode = Undefined;
		
		CreateComplexInformationForRecordingToXML(PropertyNodeStructure, Propertiesnode, PCR.XMLNodeRequiredOnExport, OLDes(PCR.Receiver), PCR.ParameterForTransferName);
							
		If Donotreplace Then
			
			AddAttributeForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("Donotreplace"), "true");
			
		EndIf;
		
		If PCR.SearchByEqualDate  Then
			
			AddAttributeForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("SearchByEqualDate"), "true");
		...
		If Not IsBlankString(ReceiverType) Then
			...
			AddAttributeForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("Type"), ReceiverType);
		...
		If Not IsBlankString(OCRName)
			...
			AddAttributeForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("OCRName"), OCRName);
		...
		If Expression <> Undefined Then
			...
			AddValueForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("Expression"), Expression);
			
			WriteDataIntoHeadNode(PropertyCollectionNode, PropertyNodeStructure, Propertiesnode, IsOrdinaryProperty);
			Continue;
		...
		If PCR.HasOnExportHandler Then
			...
			Try
				...
				If DebuggingExportHandlers Then
					...
					ExecuteHandler_PCR_OnExportProperty(ExchangeFile, Source, Receiver, IncomingData, OutgoingData,
																PCR, OCR, CollectionObject, Cancel, Value, KeyAndValue, ExtDimensionType,
																ExtDimension, Empty, OCRName, OCRProperties,Propertiesnode, PropertyCollectionNode,
																OCRNameextdimensiontype, ExportObject);
		...
		If OCRProperties = Undefined
			...
			If PropertySet Then
				...
				AddValueForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("Value"), Value);
		...
		If (OCRProperties <> Undefined) 
			...
			If RefNodeType = StringType Then
				...
				If Find(Referencenode, "<" + OLDes("Ref")) > 0 Then
								
					AddArbitraryDataForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("Ref"), Referencenode);
											
				Else
					
					AddValueForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("Value"), Referencenode);
			...
			ElsIf RefNodeType = NumberType Then
				...
				If RuleWithGlobalExport Then
					AddValueForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("GSn"), Referencenode);
				Else
					AddValueForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("NPP"), Referencenode);
				EndIf;
			...
			Else
				...
				AddArbitraryDataForRecordToXML(PropertyNodeStructure, Propertiesnode, OLDes("Ref"), InformationToWriteToFile);
		...
		If PCR.HasAfterExportHandler Then
			...
			Try
				...
				If DebuggingExportHandlers Then
					...
					ExecuteHandler_PCR_AfterExportProperty(ExchangeFile, Source, Receiver, IncomingData, OutgoingData,
																  PCR, OCR, CollectionObject, Cancel, Value, KeyAndValue, ExtDimensionType,
																  ExtDimension, OCRName, OCRNameextdimensiontype, OCRProperties, Propertiesnode,
																  Referencenode, PropertyCollectionNode, Nodeextdimension);
...
Function TypeForReceiver(Type)
	
	DotPos = StrFind(Type, ".");
	MDType = Left(Type, DotPos-1);
	
	MDName = Mid(Type, DotPos, StrLen(Type));
	
	OLMDType = OLDes(MDType);
	
	Return OLMDType+ MDName;
	
EndFunction

Procedure DumpInfobasesObjectsCorrespondenceRecord(SetRow, EmptySet)
	
	Receiver = CreateNode(OLDes("ObjectChangeRecordData"));
	
	SetAttribute(Receiver, OLDes("UniqueSourceHandle"), String(SetRow.UniqueSourceHandle.UUID()));
	SetAttribute(Receiver, OLDes("UniqueReceiverHandle"),        SetRow.UniqueReceiverHandle);
	SetAttribute(Receiver, OLDes("SourceType"),                            SetRow.SourceType);
	SetAttribute(Receiver, OLDes("ReceiverType"),                            SetRow.ReceiverType);
	
	SetAttribute(Receiver, OLDes("EmptySet"), EmptySet);
...
Procedure CallEventsBeforeObjectExport(Object, Rule, Properties=Undefined, IncomingData=Undefined, 
	...
	If CommentObjectProcessingFlag Then
		...
		EventName = NStr("en='ObjectExport: %1';ru='ВыгрузкаОбъекта: %1';vi='ВыгрузкаОбъекта: %1'");
	...
	If HasBeforeObjectExportGlobalHandler Then
		...
		Except
			WriteInformationAboutErrorDDRHandlers(65, ErrorDescription(), Rule.Name, NStr("en='BeforeExportSelectionObject (global)';ru='ПередВыгрузкойОбъектаВыборки (глобальный)';vi='BeforeExportSelectionObject (toàn cục)'"), Object);
		EndTry;
...
Procedure CallEventsAfterObjectExport(Object, Rule, Properties=Undefined, IncomingData=Undefined, 
	...
	If HasAfterObjectExportGlobalHandler Then
		...
		Except
			WriteInformationAboutErrorDDRHandlers(69, ErrorDescription(), Rule.Name, NStr("en='AfterSelectionObjectExport (Global)';ru='ПослеВыгрузкиОбъектаВыборки (глобальный)';vi='AfterSelectionObjectExport (toàn cục)'"), Object);
		EndTry;
	EndIf;
...
Procedure DumpDataByRule(Rule) Export
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = NStr("en='DATA EXPORT RULE: %1 (%2)';ru='ПРАВИЛО ВЫГРУЗКИ ДАННЫХ: %1 (%2)';vi='QUY TẮC KẾT XUẤT DỮ LIỆU: %1 (%2)'");
...
Procedure WorkThroughObjectDeletion(ObjectDeletionData, ErrorMessageString = "")
	...
	If Conversion.Property("BeforeSendDeletionInfo", EventText) Then
		...
		If Not IsBlankString(EventText) Then
			...
			Except
				ErrorMessageString = WriteInformationAboutErrorConversionHandlers(76, ErrorDescription(), NStr("en='BeforeSendingInformationAboutDeletion (conversion)';ru='ПередОтправкойИнформацииОбУдалении (конвертация)';vi='BeforeSendingInformationAboutDeletion (chuyển đổi)'"));
...
Function GetRegistrationAttributesStructure(PCRTable)
	...
	For Each PCR IN PCRRowArray Do
		...
		Except
			WriteLogEvent(NStr("en='Data exchange.Conversion rule import';ru='Обмен данными.Загрузка правил конвертации';vi='Trao đổi dữ liệu.Kết nhập quy tắc chuyển đổi'", CommonUseClientServer.MainLanguageCode()),
...
Procedure RunObjectRegistrationAttributesCheck(ChangeRecordAttributeTable)
	...
	For Each TableRow IN ChangeRecordAttributeTable Do
		...
		Except
			...
			MessageString = NStr("en='Object type is undefined: %1';ru='Тип объекта не определен: %1';vi='Chưa xác định kiểu đối tượng: %1'");
			...
			For Each Attribute IN TableRow.ChangeRecordAttributeStructure Do
				...
				If CommonUse.ThisIsTask(MDObject) Then
					...
					If Not (MDObject.Attributes.Find(Attribute.Key) <> Undefined
						...
						MessageString = NStr("en='Attributes of ""%1"" object header are specified incorrectly. Attribute ""%2"" does not exist.';ru='Неправильно указаны реквизиты шапки объекта ""%1"". Реквизит ""%2"" не существует.';vi='Chỉ ra chưa chính xác mục tin phần đầu của đối tượng ""%1"". Mục tin ""%2'' không tồn tại.'");
		...
		Else
			...
			If MetaTable = Undefined Then
				WriteInExecutionProtocol(StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Tabular section (standard tabular section, movements) %1 of object %2 does not exist.';ru='Табличная часть (стандартная табличная часть, движения) ""%1"" объекта ""%2"" не существует.';vi='Phần bảng (phần bảng chuẩn, luân chuyển) ""%1"" của đối tượng ""%2"" không tồn tại.'"),
			...
			For Each Attribute IN TableRow.ChangeRecordAttributeStructure Do
				...
				If Not AttributeFoundInObjectsRegistrationAttributesTabularSection(MetaTable, Attribute.Key) Then
					WriteInExecutionProtocol(StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='Attribute ""%3"" does not exist in the table section (standard table section, motion) ""%1"" of object ""%2"".';ru='Реквизит ""%3"" не существует в табличной части (стандартной табличной части, движениях) ""%1"" объекта ""%2"".';vi='Mục tin ""%3"" không tồn tại trong phần bảng (phần bảng chuẩn, luân chuyển) ""%1"" của đối tượng ""%2"".'"),
...
Function GetInformationAboutRules(AreCorrespondentRules = False) Export
	...
	If AreCorrespondentRules Then
		InfoString = NStr("en='Conversion rules of correspondent (%1) from %2';ru='Правила конвертации корреспондента (%1) от %2';vi='Quy tắc chuyển đổi nơi (%1) từ %2'");
	Else
		InfoString = NStr("en='Conversion rules of this infobase (%1) from %2';ru='Правила конвертации этой информационной базы (%1) от %2';vi='Quy tắc chuyển đổi của cơ sở thông tin này (%1) từ %2'");
	EndIf;
...
Procedure ImportInformationAboutDataTypes()
	...
	While ExchangeFile.Read() Do
		...
		If NodeName = "DataType" Or NodeName = "ТипДанных" Then
			
			FL = (NodeName = "ТипДанных");
			TypeName = deAttribute(ExchangeFile, StringType, ?(FL, "Имя","Name"));
		...
		ElsIf (NodeName = "DataTypeInfo" Or NodeName = "ИнформацияОТипахДанных") AND (ExchangeFile.NodeType = XMLNodeTypeEndElement) Then
...
Procedure ImportInformationAboutCustomSearchFields()
	...
	While ExchangeFile.Read() Do
		...
		If NodeName = "Rulename" Or NodeName = "ИмяПравила" Then
			
			Rulename = deItemValue(ExchangeFile, StringType);
			
		ElsIf NodeName = "SearchSetup" Or NodeName = "НастройкаПоиска" Then
			
			SearchSetup = deItemValue(ExchangeFile, StringType);
			CustomSearchFieldInfoOnDataImport.Insert(Rulename, SearchSetup);	
			
		ElsIf (NodeName = "CustomSearchSetup" Or NodeName = "НастройкаПользовательскогоПоиска") AND (ExchangeFile.NodeType = XMLNodeTypeEndElement) Then
...
Procedure ImportExchangeRules(Source="",
	...
	//LF = TypeOf(NodeForExchange) = Type("ExchangePlanRef.ExchangeDriveAccounting30");
	...
	While ExchangeRules.Read() Do
		...
		ElsIf NodeName = "AftertObjectImport" Then
			...
			HasAfterObjectImportGlobalHandler = Not IsBlankString(Conversion.AftertObjectImport);
	...
	If ExchangeMode <> "Import" AND Conversion.Property("AfterExchangeRuleImport", AfterExchangeRuleImportEventText)
		...
		Except
			ErrorMessageString = WriteInformationAboutErrorConversionHandlers(75, ErrorDescription(), NStr("en='AfterImportOfExchangeRules (conversion)';ru='ПослеЗагрузкиПравилОбмена (конвертация)';vi='AfterImportOfExchangeRules (chuyển đổi)'"));
			Cancel = True;
...
Procedure DeleteObjectByRef(Ref, ErrorMessageString)
	...
	If Not IsBlankString(Conversion.OnGetDeletionInfo) Then
		...
		Except
			ErrorMessageString = WriteInformationAboutErrorConversionHandlers(77, ErrorDescription(), NStr("en='WhenGettingInformationAboutDeleting (Conversion)';ru='ПриПолученииИнформацииОбУдалении (конвертация)';vi='WhenGettingInformationAboutDeleting (chuyển đổi)'"));
			Cancel = True;
...
Procedure ReadObjectDeletion(ErrorMessageString)
	...
	ALN = (ExchangeFile.Name = "УдалениеОбъекта");
	
	SourceTypeAsString = deAttribute(ExchangeFile, StringType, ?(ALN,"ТипПриемника","ReceiverType"));
	ReceiverTypeAsString = deAttribute(ExchangeFile, StringType, ?(ALN,"ТипИсточника","SourceType"));
	
	UUIDString = deAttribute(ExchangeFile, StringType, ?(ALN,"УникальныйИдентификатор","UUID"));
...
Procedure ReadData(MessageReader)
	...
	While ExchangeFile.Read() Do
		...
		If NodeName = "Object" OR NodeName = "Объект" Then
			
			LastImportObject = ReadObject();
			
			ProcessNewItemReadEnding(LastImportObject);
			
		ElsIf NodeName = "RegisterRecordSet" Or NodeName = "НаборЗаписейРегистра" Then
		...
		ElsIf NodeName = "ObjectDeletion" Or NodeName = "УдалениеОбъекта" Then
			
			// Process object removal from the infobase.
			ReadObjectDeletion(ErrorMessageString);
			
			ALN = NodeName = "УдалениеОбъекта";
			deIgnore(ExchangeFile, ?(ALN, "УдалениеОбъекта", "ObjectDeletion"));
			
			ProcessNewItemReadEnding();
			
		ElsIf NodeName = "ObjectChangeRecordData" Or NodeName = "ИнформацияОРегистрацииОбъекта" Then
		...
		ElsIf NodeName = "ObjectChangeRecordDataAdjustment" Or NodeName = "КорректировкаИнформацииОРегистрацииОбъекта" Then
		...
		ElsIf NodeName = "CommonNodeData" Or NodeName = "ОбщиеДанныеУзлов" Then
			
			ReadCommonNodeData(MessageReader);
			
			deIgnore(ExchangeFile, NodeName);
			
		ElsIf (NodeName = "ExchangeFile" Or NodeName = "ФайлОбмена" ) AND (ExchangeFile.NodeType = XMLNodeTypeEndElement) Then
					...
					If TableRow = Undefined Then
						...
						For Each Column IN ExchangeMessageDataTable.Columns Do
							...
							If Column.ValueType.ContainsType(StringType) Then
								...
								TableRow[Column.Name] = NStr("en='Deleting object';ru='Удаление объекта';vi='Xóa đối tượng'");
				...
				Try
					...
					If Cancel = True Then
						...
						If Not IsBlankString(CancelReason) Then
							...
							MessageString = NStr("en='Data import was canceled as: %1';ru='Загрузка данных отменена по причине: %1';vi='Đã hủy bỏ kết nhập dữ liệu theo nguyên nhân: %1'");
							MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString, CancelReason);
							Raise MessageString;
						Else
							Raise NStr("en='Data import is canceled';ru='Загрузка данных отменена';vi='Đã hủy bỏ kết nhập dữ liệu'");
						EndIf;
...
Procedure ReadExchangeData(MessageReader, DataAnalysis)
	...
	LF = (ExchangeFile.LocalName ="ДанныеПоОбмену");
	
	ExchangePlanNameField           = deAttribute(ExchangeFile, StringType, ?(LF, "ПланОбмена","ExchangePlan"));
	FromWhomCode                    = deAttribute(ExchangeFile, StringType, ?(LF, "ОтКого","FromWhom"));
	MessageNumberField           = deAttribute(ExchangeFile, NumberType, ?(LF, "НомерИсходящегоСообщения", "OutboundMessageNumber"));
	ReceivedMessageNumberField  = deAttribute(ExchangeFile, NumberType, ?(LF, "НомерВходящегоСообщения", "IncomingMessageNumber"));
	DeleteChangeRecords  = deAttribute(ExchangeFile, BooleanType, ?(LF, "УдалитьРегистрациюИзменений", "DeleteChangeRecords"));
	SenderVersion            = deAttribute(ExchangeFile, StringType, ?(LF, "ВерсияОтправителя", "SenderVersion"));
	
	ExchangePlanNameField = ExchangePlanDecs();
	...
	If Not ValueIsFilled(ExchangeNodeRecipient)
		...
		MessageString = NStr("en='Exchange node for data import is not found. Exchange plan: %1, Code: %2';ru='Не найден узел обмена для загрузки данных. План обмена: %1, Код: %2';vi='Chưa tìm thấy nút trao đổi để kết nhập dữ liệu. Sơ đồ trao đổi: %1, Mã: %2'");
	...
	If DataImportToInformationBaseMode() Then
		...
		If ReceivedNo >= MessageReader.MessageNo Then // Message number is either smaller or equals to the previously accepted one.
			...
			MessageReader = New FixedStructure(MessageReadingTemporary);
 			...
 			Raise NStr("en='Exchange message was previously received';ru='Сообщение обмена было принято ранее';vi='Đã tiếp nhận thông điệp trao đổi trước đây'");
		EndIf;
		...
		If DeleteChangeRecords Then // Delete registration of changes.
			...
			If TransactionActive() Then
				Raise NStr("en='Deletion of the data change registration cannot be completed in an active transaction.';ru='Удаление регистрации изменений данных не может быть выполнено в активной транзакции.';vi='Không thể thực hiện xóa nhật ký thay đổi dữ liệu ở giao tác hiện thời.'");
			EndIf;
	...
	If Not IsBlankString(Conversion.AfterGetExchangeNodeDetails) Then
		...
		Except
			Raise WriteInformationAboutErrorConversionHandlers(176, ErrorDescription(), NStr("en='AfterReceivingInformationAboutExchangeNodes (conversion)';ru='ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';vi='AfterReceivingInformationAboutExchangeNodes (конвертация)'"));
		EndTry;
...
Function ExchangePlanDecs()
	
	ExchangePlanDecs = ExchangePlanNameField;
	
	If Upper(ExchangePlanNameField) = Upper("ОбменУправлениеНебольшойФирмойБухгалтерия")
		Or Upper(ExchangePlanNameField) = Upper("ОбменУправлениеНебольшойФирмойБухгалтерия30") Then
		ExchangePlanDecs = "ExchangeCompanyManagementAccounting"
	EndIf;
	
	Return ExchangePlanDecs;
	...
	For Each MappingObject IN ObjectsForPostponedRecording() Do
		...
		Try
			...
			Else
				...
				ErrorDescription = NStr("en='An error occurred when checking the attribute population';ru='Ошибка проверки заполнения реквизитов';vi='Lỗi kiểm tra việc điền mục tin'");
...
Procedure WriteInformationAboutDataExchangeViaExchangePlans(Val SentNo)
	...
	Receiver = CreateNode(OLDes("ExchangeData"));
	
	ExchangePlanName = OLDes(ExchangePlanName());
	If NodeForExchange.SettingVariant = "Accounting3" Then
		ExchangePlanName = "ОбменУправлениеНебольшойФирмойБухгалтерия30"
	EndIF;
		
	SetAttribute(Receiver, OLDes("ExchangePlan"), ExchangePlanName);
	SetAttribute(Receiver, OLDes("Whom"),   NodeForExchange.Code);
	SetAttribute(Receiver, OLDes("FromWhom"), ExchangePlans[ExchangePlanName()].ThisNode().Code);
	
	// Attributes of exchange message handshaking mechanism.
	SetAttribute(Receiver, OLDes("OutboundMessageNumber"), SentNo);
	SetAttribute(Receiver, OLDes("IncomingMessageNumber"),  NodeForExchange.ReceivedNo);
	SetAttribute(Receiver, OLDes("DeleteChangeRecords"), True);
	
	SetAttribute(Receiver, OLDes("SenderVersion"), TrimAll(Metadata.Version));
...
Procedure ExportCommonNodeData(Val SentNo)
	...
	PCR.SourceKind = OLDes("Property");
	PCR.TargetKind = OLDes("Property");
	PCR.Source = OLDes("Code");
	PCR.Receiver = OLDes("Code");
	...
	XMLNode = CreateNode(OLDes("CommonNodeData"));
...
Procedure PassInformationAboutTypesToReceiver()
	...
	If Not IsBlankString(TypesForTargetString) Then
		...
		OLTypesForTarget = TypesForTargetStringOL();
		
		WriteToFile(OLTypesForTarget);
		
	EndIf;
		
EndProcedure

Function TypesForTargetStringOL()
	
	OLTypesForTerget = TypesForTargetString;
	IDMap = TTMap();
	
	ReplaceID(OLTypesForTerget, IDMap);
	
	Return OLTypesForTerget;
	
EndFunction

Procedure ReplaceID(OLTypesForTerget, IDMap)
	
	For Each ListVal In IDMap Do
		
		RepKeyValArr = StrSplit(ListVal.Value,",");
		RepKey = TrimAll(RepKeyValArr[0]);
		RepVal = TrimAll(RepKeyValArr[1]);
		OLTypesForTerget = StrReplace(OLTypesForTerget, RepKey, RepVal);
	EndDo;
...
Function TTMap()
	...
	Result.Add("DataTypeInfo,ИнформацияОТипахДанных", 1);
	Result.Add("Name=,Имя=", 2);
	Result.Add("TabularSection>,ТабличнаяЧасть>", 3);
	Result.Add("DataType,ТипДанных", 4);
...
Procedure RunExport(ErrorMessageString = "")
	...
	If Not IsBlankString(Conversion.BeforeDataExport) Then
		...
		Except
			WriteInformationAboutErrorConversionHandlers(62, ErrorDescription(), NStr("en='BeforeDataExport (conversion)';ru='ПередВыгрузкойДанных (конвертация)';vi='BeforeDataExport (chuyển đổi)'"));
			Cancel = True;
		EndTry; 
	...
	If Conversion.Property(OLDes("AfterParametersImport"), EventTextAfterParameterImport)
		AND Not IsBlankString(EventTextAfterParameterImport) Then
		
		WritingEvent = New XMLWriter;
		WritingEvent.SetString();
		deWriteItem(WritingEvent, OLDes("AfterParameterExportAlgorithm"), EventTextAfterParameterImport);
	...
	If Not Cancel AND Not IsBlankString(Conversion.AfterDataExport) Then
		...
		Except
			WriteInformationAboutErrorConversionHandlers(63, ErrorDescription(), NStr("en='AfterDataExport (conversion)';ru='ПослеВыгрузкиДанных (конвертация)';vi='AfterDataExport (chuyển đổi)'"));
		EndTry;
...
Procedure RunDumpOfRegisteredData(WriteMessage, ErrorMessageString, UsedUnloadRulesTable)
	...
	If Not IsBlankString(Conversion.BeforeGetChangedObjects) Then
		...
		Except
			WriteInformationAboutErrorConversionHandlers(175, ErrorDescription(), NStr("en='BeforeReceivingChangedObjects (conversion)';ru='ПередПолучениемИзмененныхОбъектов (конвертация)';vi='BeforeReceivingChangedObjects (chuyển đổi)'"));
			Return;
		EndTry;
...
Procedure MessagesInitialization()
	...
	ErrorMessages.Insert(2,  NStr("en='Link to a non-existing metadata object in the exchange rules';ru='Ссылка на несуществующий объект метаданных в правилах обмена';vi='Tham chiếu đến đối tượng Metadata không tồn tại trong các quy tắc trao đổi'"));
	ErrorMessages.Insert(3,  NStr("en='File name with exchange rules is not specified (Rule file name)';ru='Не указано имя файла с правилами обмена (Имя файла правил)';vi='Chưa chỉ ra tên tệp với các quy tắc trao đổi (Tên tệp quy tắc)'"));
	...
	ErrorMessages.Insert(5,  NStr("en='An error occurred when receiving a value of the object property (by the name of the source property)';ru='Ошибка получения значения свойства объекта (по имени свойства источника)';vi='Lỗi nhận giá trị thuộc tính đối tượng (theo tên thuộc tính nguồn)'"));
	ErrorMessages.Insert(6,  NStr("en='An error occurred when receiving the object property value (by the target property name)';ru='Ошибка получения значения свойства объекта (по имени свойства приемника)';vi='Lỗi nhận giá trị thuộc tính đối tượng (theo tên thuộc tính đích)'"));
	...
	ErrorMessages.Insert(8,  NStr("en='File name for data import is not specified (File name for import)';ru='Не указано имя файла для загрузки данных (Имя файла для загрузки)';vi='Chưa chỉ ra tên tệp để kết nhập dữ liệu (Tên tệp để kết nhập)'")); // not used
	...
	ErrorMessages.Insert(10, NStr("en='An error occurred while receiving value of subobject property (by source property name)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';vi='Lỗi nhận giá trị thuộc tính đối tượng trực thuộc (theo tên thuộc tính nguồn)'"));
	ErrorMessages.Insert(11, NStr("en='An error occurred while receiving value of subobject property (by target property name)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';vi='Lỗi nhận giá trị thuộc tính đối tượng trực thuộc (theo tên thuộc tính đích)'"));
	ErrorMessages.Insert(12, NStr("en='An error occurred when creating a data processor with the handler code';ru='Ошибка при создании обработки с кодом обработчиков';vi='Lỗi khi tạo mới bộ xử lý có mã hàm sự kiện'"));
			
	ErrorMessages.Insert(13, NStr("en='An error occurred in event handler OnObjectImport';ru='Ошибка в обработчике события ПриЗагрузкеОбъекта';vi='Lỗi trong hàm sự kiện OnObjectImport'"));
	ErrorMessages.Insert(14, NStr("en='An error occurred in event handler AfterObjectImport';ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта';vi='Lỗi trong hàm sự kiện AfterObjectImport'"));
	
	ErrorMessages.Insert(15, NStr("en='An error occurred in event handler AfterDataImport (conversion)';ru='Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)';vi='Lỗi trong hàm sự kiện AfterDataImport (chuyển đổi)'"));
			
	ErrorMessages.Insert(16, NStr("en='An error occurred when writing the document';ru='Ошибка при записи документа';vi='Lỗi khi ghi chứng từ'"));
	ErrorMessages.Insert(17, NStr("en='An error occurred when writing the object';ru='Ошибка записи объекта';vi='Lỗi ghi đối tượng'"));
	ErrorMessages.Insert(18, NStr("en='An error occurred in event handler BeforeProcessClearingRule';ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки';vi='Lỗi trong hàm sự kiện BeforeProcessClearingRule'"));
	ErrorMessages.Insert(19, NStr("en='An error occurred in event handler AfterClearingRuleProcessing';ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки';vi='Lỗi trong hàm sự kiện AfterClearingRuleProcessing'"));
	ErrorMessages.Insert(20, NStr("en='An error occurred in event handler BeforeDeleteObject';ru='Ошибка в обработчике события ПередУдалениемОбъекта';vi='Lỗi trong hàm sự kiện BeforeDeleteObject'"));
	...
	ErrorMessages.Insert(22, NStr("en='An error occurred in event handler BeforeProcessExportRule';ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';vi='Lỗi trong hàm sự kiện BeforeProcessExportRule'"));
	ErrorMessages.Insert(23, NStr("en='An error occurred in event handler AfterDumpRuleProcessing';ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';vi='Lỗi trong hàm sự kiện AfterDumpRuleProcessing'"));
	ErrorMessages.Insert(24, NStr("en='An error occurred in event handler BeforeObjectExport';ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';vi='Lỗi trong hàm sự kiện BeforeObjectExport'"));
	ErrorMessages.Insert(25, NStr("en='An error occurred in event handler AfterObjectExport';ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';vi='Lỗi trong hàm sự kiện AfterObjectExport'"));
	ErrorMessages.Insert(26, NStr("en='An error occurred when writing the object';ru='Ошибка записи объекта';vi='Lỗi ghi đối tượng'"));
	ErrorMessages.Insert(27, NStr("en='An error occurred in event handler BeforeObjectExport';ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';vi='Lỗi trong hàm sự kiện BeforeObjectExport'"));
	ErrorMessages.Insert(28, NStr("en='An error occurred in event handler OnObjectExport';ru='Ошибка в обработчике события ПриВыгрузкеОбъекта';vi='Lỗi trong hàm sự kiện OnObjectExport'"));
	ErrorMessages.Insert(29, NStr("en='An error occurred in event handler AfterObjectExport';ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';vi='Lỗi trong hàm sự kiện AfterObjectExport'"));
	
	ErrorMessages.Insert(31, NStr("en='Object conversion rule is not found';ru='Не найдено правило конвертации объектов';vi='Không tìm thấy quy tắc chuyển đổi đối tượng'"));
	...
	ErrorMessages.Insert(33, NStr("en='An error occurred in event handler BeforeExportProcessor of the property group';ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';vi='Lỗi trong hàm sự kiện BeforeExportProcessor nhóm thuộc tính'"));
	ErrorMessages.Insert(34, NStr("en='An error occurred in event handler AfterExportProcessor of the property group';ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';vi='Lỗi trong hàm sự kiện AfterExportProcessor nhóm thuộc tính'"));
			
	ErrorMessages.Insert(41, NStr("en='Error in event handler OnExport (of collection object)';ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';vi='Lỗi trong hàm sự kiện OnExport (đối tượng tập hợp)'"));
	ErrorMessages.Insert(42, NStr("en='Error in event handler AfterExport (of collection object)';ru='Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)';vi='Lỗi trong hàm sự kiện AfterExport (đối tượng tập hợp)'"));
	ErrorMessages.Insert(43, NStr("en='An error occurred in global event handler BeforeObjectImporting (conversion)';ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';vi='Lỗi trong hàm sự kiện toàn cục BeforeObjectImporting (chuyển đổi)'"));
			
	ErrorMessages.Insert(45, NStr("en='An error occurred in event handler BeforeExport (properties)';ru='Ошибка в обработчике события ПередВыгрузкой (свойства)';vi='Lỗi trong hàm sự kiện BeforeExport (thuộc tính)'"));
		
	ErrorMessages.Insert(48, NStr("en='An error occurred in event handler AfterExport (properties)';ru='Ошибка в обработчике события ПослеВыгрузки (свойства)';vi='Lỗi trong hàm sự kiện AfterExport (thuộc tính)'"));
	...
	ErrorMessages.Insert(50, NStr("en='An error occurred in event handler BeforeDataExport (conversion)';ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';vi='Lỗi trong hàm sự kiện BeforeDataExport (chuyển đổi)'"));
	ErrorMessages.Insert(51, NStr("en='An error occurred in event handler AfterDataExport (conversion)';ru='Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)';vi='Lỗi trong hàm sự kiện AfterDataExport (chuyển đổi)'"));
	ErrorMessages.Insert(52, NStr("en='An error occurred in global event handler BeforeObjectConversion (conversion)';ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';vi='Lỗi trong hàm sự kiện toàn cục BeforeObjectConversion (chuyển đổi)'"));
	ErrorMessages.Insert(53, NStr("en='An error occurred in global event handler BeforeObjectExport (conversion)';ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';vi='Lỗi trong hàm sự kiện toàn cục BeforeObjectExport (chuyển đổi)'"));
	ErrorMessages.Insert(54, NStr("en='An error occurred when receiving a subordinate object collection from the incoming data';ru='Ошибка получения коллекции подчиненных объектов из входящих данных';vi='Lỗi nhận tập hợp đối tượng trực thuộc từ dữ liệu đến'"));
	ErrorMessages.Insert(55, NStr("en='An error occurred when receiving the subordinate object properties from the incoming data';ru='Ошибка получения свойства подчиненного объекта из входящих данных';vi='Lỗi nhận thuộc tính đối tượng trực thuộc từ dữ liệu đến'"));
	ErrorMessages.Insert(56, NStr("en='An error occurred when receiving the object properties from the incoming data';ru='Ошибка получения свойства объекта из входящих данных';vi='Lỗi nhận thuộc tính đối tượng từ dữ liệu đến'"));
	...
	ErrorMessages.Insert(63, NStr("en='Match for the Source value is not found';ru='Не найдено соответствие для значения Источника';vi='Không tìm thấy sự tương ứng đối với giá trị Nguồn'"));
	...
	ErrorMessages.Insert(65, NStr("en='An error occurred when exporting data for the exchange plan node';ru='Ошибка при выгрузке данных для узла плана обмена';vi='Lỗi khi kết xuất dữ liệu đối với nút của sơ đồ trao đổi'"));
	...
	ErrorMessages.Insert(67, NStr("en='An error occurred in event handler SearchFieldsSequence';ru='Ошибка в обработчике события ПоследовательностьПолейПоиска';vi='Lỗi trong hàm sự kiện SearchFieldsSequence'"));
	ErrorMessages.Insert(68, NStr("en='Import exchange rules for data export again.';ru='Необходимо перезагрузить правила обмена для выгрузки данных.';vi='Cần kết nhập lại quy tắc trao đổi để kết xuất dữ liệu.'"));
	
	ErrorMessages.Insert(69, NStr("en='An error occurred in event handler AfterImportOfExchangeRules (conversion)';ru='Ошибка в обработчике события ПослеЗагрузкиПравилОбмена (конвертация)';vi='Lỗi trong hàm sự kiện AfterImportOfExchangeRules (chuyển đổi)'"));
	
	ErrorMessages.Insert(71, NStr("en='An error occurred in event handler OnObtainingInformationAboutDeletion (conversion)';ru='Ошибка в обработчике события ПриПолученииИнформацииОбУдалении (конвертация)';vi='Lỗi trong hàm sự kiện OnObtainingInformationAboutDeletion (chuyển đổi)'"));
	
	ErrorMessages.Insert(72, NStr("en='An error occurred when executing the algorithm after import of the parameter values';ru='Ошибка при выполнении алгоритма после загрузки значений параметров';vi='Lỗi khi thực hiện thuật toán sau khi kết nhập giá trị tham số'"));
	
	ErrorMessages.Insert(73, NStr("en='An error occurred in event handler AfterObjectExportToFile';ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';vi='Lỗi trong hàm sự kiện AfterObjectExportToFile'"));
	...
	ErrorMessages.Insert(78, NStr("en='Collision of change closing dates.';ru='Коллизия дат запрета изменения.';vi='Mâu thuẫn ngày cấm thay đổi.'"));
...
"Không nên đánh dấu xóa phần tử định trước. Việc đánh dấu xóa cho đối tượng không được thiết lập.'"));
	...
	ErrorMessages.Insert(173, NStr("en='An error of the exchange node lock occurred. Maybe, data synchronization is already in progress';ru='Ошибка блокировки узла обмена. Возможно синхронизация данных уже выполняется';vi='Lỗi phong tỏa nút trao đổi. Có thể, đã thực hiện đồng bộ hóa dữ liệu'"));
	...
	ErrorMessages.Insert(175, NStr("en='An error occurred in event handler BeforeModifiedObjectsReceiving (conversion)';ru='Ошибка в обработчике события ПередПолучениемИзмененныхОбъектов (конвертация)';vi='Lỗi trong hàm sự kiện BeforeModifiedObjectsReceiving (chuyển đổi)'"));
	ErrorMessages.Insert(176, NStr("en='An error occurred in event handler AfterGettingInformationAboutExchangeNodes (conversion)';ru='Ошибка в обработчике события ПослеПолученияИнформацииОбУзлахОбмена (конвертация)';vi='Lỗi trong hàm sự kiện AfterGettingInformationAboutExchangeNodes (chuyển đổi)'"));
