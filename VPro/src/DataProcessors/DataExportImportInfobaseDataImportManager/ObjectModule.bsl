...
Function SortImportedTypes(Val ImportedTypes)
	...
	For Each MetadataObject IN ImportedTypes Do
		...
		ElsIf Metadata.Sequences.Contains(MetadataObject) Then
			String.Priority = 4;
		Else
			TextPattern = NStr("en='Metadata object export is not supported %1';ru='Выгрузка объекта метаданных не поддерживается %1';vi='Hỗ trợ kết xuất đối tượng metadata %1'");
			MessageText = ServiceTechnologyIntegrationWithSSL.SubstituteParametersInString(TextPattern, MetadataObject.FullName());
			Raise(MessageText);
		EndIf;
...
Procedure ExecuteDataImport()
	...
	For Each MetadataObject IN CurrentImportedTypes Do
		...
		Else
			...
			WriteLogEvent(
				NStr("en='DataExportImport.ObjectExportSkipped';ru='ВыгрузкаЗагрузкаДанных.ЗагрузкаОбъектаПропущена';vi='DataExportImport.ObjectExportSkipped'", Metadata.DefaultLanguage.LanguageCode),
...
"список объектов метаданных, исключаемых из выгрузки и загрузки данных';vi='Việc kết xuất dữ liệu của đối tượng metadata %1 bị bỏ qua, bởi vì đối tượng có trong"
"danh sách đối tượng metadata mà được loại bỏ khỏi kết xuất và kết nhập dữ liệu'", Metadata.DefaultLanguage.LanguageCode),
