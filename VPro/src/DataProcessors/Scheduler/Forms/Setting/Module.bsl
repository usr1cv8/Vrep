...
Procedure TimeLimitToOnChange(Item)
	...
	If TimeLimitTo <= TimeLimitFrom 
		AND TimeLimitTo <> '00010101000000'
		AND TimeLimitFrom <> '00010101000000' Then
		Message(NStr("en='End time cannot be less or equal to start time.';ru='Время окончания не может быть меньше или равно времени начала.';vi='Thời gian kết thúc không thể nhỏ hơn hoặc bằng thời gian bắt đầu.'"));
		TimeLimitTo = TimeLimitFrom + 1800;
		Return;
	EndIf;
