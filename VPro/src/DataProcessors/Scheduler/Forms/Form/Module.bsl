...
Procedure GenerateScheduledWorksPeriod()
	...
	If WorksScheduleRadioButton = "Week" Then
		...
		If Month(CalendarDateBegin) = Month(CalendarDateEnd) Then
			...
			MonthOfScheduleEnd = Format(CalendarDateEnd, NStr("en = 'L=en_US; DF=MMM'; ru = 'L=ru_Ru; DF=MMM'; vi = 'L=vi; DF=MMM'"));
		...
		Else
			...
			DayOfScheduleBegin = Format(CalendarDateBegin, "DF=dd");
			MonthOfScheduleBegin = Format(CalendarDateBegin, NStr("en = 'L=en_US; DF=MMM'; ru = 'L=ru_Ru; DF=MMM'; vi = 'L=vi; DF=MMM'"));
			DayOfScheduleEnd = Format(CalendarDateEnd, "DF=dd");
			MonthOfScheduleEnd = Format(CalendarDateEnd, NStr("en = 'L=en_US; DF=MMM'; ru = 'L=ru_Ru; DF=MMM'; vi = 'L=vi; DF=MMM'"));
	...
	ElsIf WorksScheduleRadioButton = "Month" Then
		...
		MonthOfSchedule = Format(CalendarDateBegin, NStr("en = 'L=en_US; DF=MMM'; ru = 'L=ru_Ru; DF=MMM'; vi = 'L=vi; DF=MMM'"));
		YearOfSchedule = Format(Year(CalendarDateBegin), "NG=0");
...
Procedure UpdateScheduleChart()
	...
	While Selection.Next() Do
		...
		DotValue = Format(Selection.Start, "DF=""HH:mm""") + " - "
							+ Format(Selection.Finish, "DF=""HH:mm""") + " (# "
							+ ObjectPrefixationClientServer.GetNumberForPrinting(Selection.Number, True, True) + NStr("en=' dated ';vi=' ngày '") 
...
Procedure UpdateCalendarDayPeriod(Spreadsheet, ResourcesList, Period4Days = False)
	...
	Spreadsheet.Area(FirstColumnCoordinates).Text = Format(CalendarDateBegin, NStr("en = 'L=en_US; DF=""dd MMMM yyyy dddd""';ru = 'L=ru_RU; DF=""dd MMMM yyyy dddd""';vi = 'L=vi; DF=""dd MMMM yyyy dddd""'"));
...
Procedure RefreshChartOfResources()
	...
	While SelectionResource.Next() Do
		...
		While Selection.Next() Do
			...
			Point.Text = Format(Selection.Start, "DF=""HH:mm""") + " - "
							+ Format(Selection.Finish, "DF=""HH:mm""") + NStr("en = ' (# '; ru = ' (№ '; vi = ' (số '")
							+ ObjectPrefixationClientServer.GetNumberForPrinting(Selection.Number, True, True) + NStr("en = ' dated '; ru = ' от '; vi = ' ngày '") 
							+ Format(Selection.Date, "DF=dd.MM.yy") + ") ";
			Point.Details = Selection.Ref;
