...
"в справочнике найдены не уникальные записи.';vi='Trước khi kết nhập tên đối tượng metadata"
"trong danh mục đã tìm thấy các bản ghi không đơn trị.'"));
			EndIf;
...
"%1';vi='Chưa thực hiện kết nhập các thay đổi quan trọng."
"Khi kết nhập phần tử định trước, chưa tìm thấy bản ghi đơn trị."
"Không thể tiếp tục theo các nguyên nhân sau."
"%1'"),
					DenialDescription);
			EndIf;
			...
			If ValueIsFilled(PredefinedItemDoubles) Then
				WriteLogEvent(
					NStr("en='Predefined items.Non-unique entries found.';ru='Предопределенные элементы.Найдены не уникальные записи.';vi='Phần tử định trước. Đã tìm thấy các bản ghi không đơn trị.'",
...
"%1';vi='Khi kết nhập các phần tử định trước, chưa tìm thấy các bản ghi không đơn trị."
"%1'"),
						PredefinedItemDoubles));
			EndIf;
...
Procedure AddPredefinedItemDoubleDescription(WrittenObject, PredefinedItemDoubles, Cancel, DenialDescription)
	...
	While Selection.Next() Do
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NonUniqueRecordErrorTemplate(),
				NStr("en='When importing predefined items, non-unique records were found.';ru='При загрузке предопределенных элементов найдены не уникальные записи.';vi='Khi kết nhập các phần tử định trước, chưa tìm thấy các bản ghi không đơn trị.'"));
		EndIf;
	...
	If WriteInJournal Then
		If DoubleNumber = 1 Then
			Pattern = NStr("en='(imported reference: %1, duplicate reference: %2)';ru='(загружаемая ссылка: %1, ссылка дубля: %2)';vi='(tham chiếu đã kết nhập: %1, tham chiếu lặp lại; %2)'");
		Else
			Pattern = NStr("en='(imported reference: %1, duplicate references %2)';ru='(загружаемая ссылка: %1, ссылки дублей: %2)';vi='(tham chiếu đã kết nhập: %1, tham chiếu lặp lại: %2)'");
		EndIf;
...
"   - нажмите Выполнить."
"3. После этого запустите 1С:Предприятие и выполните повторную синхронизацию данных.';vi='Chưa thực hiện kết nhập các thay đổi quan trọng."
"%1"
"Cần chỉnh sửa cơ sở thông tin."
"1. Hãy mở bộ thiết kế, hãy chuyển đến menu Quản trị,"
"   chọn mục ""Thử nghiệm và sửa chữa ...""."
"2. Trong biểu mẫu vừa mở"
"   - hãy bật chỉ mục ""Kiểm tra tính toàn vẹn của cơ sở thông tin"";"
"   - hãy chọn phương án ""Thử nghiệm và sửa chữa"" mà không phải ""Chỉ thử nghiệm"";"
"   - hãy nhấn Thực hiện."
"3. Sau đó, hãy khởi động 1С:DOANH NGHIỆP và hãy thực hiện đồng bộ hóa lại dữ liệu.'");
EndFunction
...
Function ErrorOfOpeningOfExchangeMessageFile()
	...
	Return NStr("en='An error occurred when opening the exchange message file';ru='Ошибка открытия файла сообщения обмена';vi='Lỗi mở tệp thông điệp trao đổi'", CommonUseClientServer.MainLanguageCode());
	
EndFunction

Function ErrorOfStartReadOfExchangeMessageFile()
	
	Return NStr("en='An error occurred when starting reading the exchange message file';ru='Ошибка при начале чтения файла сообщения обмена';vi='Lỗi khi bắt đầu đọc tệp thông điệp trao đổi'", CommonUseClientServer.MainLanguageCode());
	
EndFunction

Function ErrorBeginningRecordsExchangeMessageFile()
	
	Return NStr("en='An error occurred when starting writing the exchange message file';ru='Ошибка при начале записи файла сообщения обмена';vi='Lỗi khi bắt đầu ghi lại tệp thông điệp trao đổi'", CommonUseClientServer.MainLanguageCode());
	
EndFunction

Function ErrorOfReadingOFExchangeMessageFile()
	
	Return NStr("en='An error occurred when reading an exchange message file';ru='Ошибка чтения файла сообщения обмена';vi='Lỗi đọc tệp thông điệp trao đổi'", CommonUseClientServer.MainLanguageCode());
	
EndFunction

Function ErrorOfExchangeMessageFileWrite()
	
	Return NStr("en='An error occurred when writing data to the exchange message file';ru='Ошибка записи данных в файл сообщения обмена';vi='Lỗi ghi dữ liệu vào tệp thông điệp trao đổi'");
	
EndFunction

Function ErrorOfDataExchangeKind()
	
	Return NStr("en='Exchange not according to conversion rules is not supported';ru='Обмен не по правилам конвертации не поддерживается';vi='Không hỗ trợ trao đổi không theo quy tắc chuyển đổi'", CommonUseClientServer.MainLanguageCode());
