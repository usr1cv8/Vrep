...
Function GetInformationAboutRules() Export
	...
	InfoString = NStr("en='Object registration rules of this infobase (%1) from %2';ru='Правила регистрации объектов этой информационной базы (%1) от %2';vi='Quy tắc ghi nhận đối tượng của cơ sở thông tin này (%1) từ %2'");
...
Procedure RunExchangePlanPresenceCheckup()
	...
	If Registration.ExchangePlanName <> ExchangePlanImportName Then
		...
		ErrorDescription = NStr("en='Exchange plan %1 is specified in the registration rules, and import in progress is for exchange plan %2';ru='В правилах регистрации указан план обмена %1, а загрузка выполняется для плана обмена %2';vi='Trong các quy tắc ghi nhận đã chỉ ra sơ đồ trao đổi %1, còn việc kết nhập được thực hiện đối với sơ đồ trao đổi %2'");
...
Function MessagesInitialization()
	...
	Messages.Insert(0, NStr("en='Internal error';ru='Внутренняя ошибка';vi='Lỗi bên trong'", MainLanguageCode));
	Messages.Insert(1, NStr("en='An error occurred when opening the rule file';ru='Ошибка открытия файла правил';vi='Lỗi mở tệp quy tắc'", MainLanguageCode));
	Messages.Insert(2, NStr("en='An error occurred when importing rules';ru='Ошибка при загрузке правил';vi='Lỗi khi kết nhập quy tắc'", MainLanguageCode));
	Messages.Insert(3, NStr("en='Rule format error';ru='Ошибка формата правил';vi='Lỗi định dạng quy tắc'", MainLanguageCode));
	Messages.Insert(4, NStr("en='An error occurred when receiving a reading rule file';ru='Ошибка при получении файла правил для чтения';vi='Lỗi khi nhận tệp quy tắc để đọc'", MainLanguageCode));
	Messages.Insert(5, NStr("en='Imported registration rules are not for the current exchange plan.';ru='Загружаемые правила регистрации не предназначены для текущего плана обмена.';vi='Quy tắc ghi nhận đã kết nhập không được dùng cho sơ đồ trao đổi hiện tại.'", MainLanguageCode));
