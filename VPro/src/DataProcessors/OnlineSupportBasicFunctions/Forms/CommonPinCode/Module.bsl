...
Function FieldsAreFilledCorrectly()
	...
	If IsBlankString(PinCode) Then
		...
		Message = New UserMessage;
		Message.Text = NStr("en='PIN code is not filled in';ru='Не заполнено поле ""Пинкод""';vi='Chưa điền trường ""Mã pin""'");
	...
	If StrLen(PinCodePage) <> 16 Then
		...
		Message = New UserMessage;
		Message.Text = NStr("en='PIN code should contain 16 digits.';ru='Пин-код должен состоять из 16 цифр.';vi='Pin code gồm 15 chữ số.'");
		Message.Field  = "PinCode";
		Message.Message();
...
Function MessageParametersToTechicalSupport()
	...
	Result.Insert("Subject", NStr("en='Online support. Product registration.';ru='Интернет-поддержка. Регистрация продукта.';vi='Hỗ trợ qua Internet. Ghi nhận sản phẩm.'"));
...
"Tôi không thể đăng ký sản phẩm phần mềm"
"để kết nối với bộ phận hỗ trợ qua Internet. Hãy giúp tôi giải quyết vấn đề này."
""
"Tên đăng nhập: %1."
"Số đăng ký: %2."
"Mã pin: %3."
