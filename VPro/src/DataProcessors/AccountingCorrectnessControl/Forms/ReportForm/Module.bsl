...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.ReportKind = "MutualSettlements" Then
		ThisForm.Title = NStr("en='Accounting assistant: settlements';vi='Trợ lý kế toán: định cư'");
		OutputReportSettlements();
	ElsIf Parameters.ReportKind = "WriteOffsInconsistenciesToSpecifications" Then
		ThisForm.Title = NStr("en='Accounting assistant: mismatches of actual writeoffs to specifications';vi='Trợ lý kế toán: sự không phù hợp của việc xóa sổ thực tế với thông số kỹ thuật'");
		OutputMismatchesWriteoffsToSpecifications();
	ElsIf Parameters.ReportKind = "SuggestedProductionSpecifications" Then
		ThisForm.Title = NStr("en='Accounting assistant: offered specifications to set to tab. Products part';vi='Trợ lý kế toán: cung cấp thông số kỹ thuật để đặt thành tab. Phần sản phẩm'");
		OutputOfferedSpecification();
	ElsIf Parameters.ReportKind = "PurchasePricesAnalysis" Then
		ThisForm.Title = NStr("en='Accounting assistant: purchase prices analysis';vi='Trợ lý kế toán: phân tích giá mua'");
		OutputPurchasePricesAnalysis();
	ElsIf Parameters.ReportKind = "ExchangeDifferences" Then
		ThisForm.Title = NStr("en='Accounting assistant: exchange rates differences';vi='Trợ lý kế toán: chênh lệch tỷ giá'");
		OutputCurrencyRatesDifferences();
	ElsIf Parameters.ReportKind = "CashFlowItems" Then
		ThisForm.Title = NStr("en='Accounting assistant: document list over a period';vi='Trợ lý kế toán: danh sách tài liệu trong một khoảng thời gian'");
		OutputCashFlowItems();
	EndIf;
