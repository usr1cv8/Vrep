	...
	If ProcedureNameToExecuteOnClient <> "" Then
		...
		Else			
			...
			Except
				...
				ErrorsDescriptionArray.Add(NStr("en='Unable to execute the procedure ';vi='Không thể thực hiện thủ tục'") + ProcedureNameToExecuteOnClient);
...
Procedure FillInitialData()
	...
	If Parameters.MonthEndContext Then
		...
		Items.PeriodCompanyLabel.Title = NStr("en='Per ';vi='Kỳ'") + Lower(PeriodPresentation) + NStr("en=', for companies ""';vi=', cho doanh nghiệp ""'") + TrimAll(Company.Description) + """";
...
Procedure FillSectionTree()
	...
	NewRow_Level0.AccountingSectionPresentation = NStr("en='ACCOUNTING SECTIONS';vi='HƯỚNG DẪN KẾ TOÁN'");
	...
	NewRow_Level1.AccountingSectionPresentation = NStr("en='Accounts payable';vi='Tài khoản phải trả'");
	...
	NewRow_Level1.AccountingSectionPresentation = NStr("en='Accounts receivable';vi='Tài khoản phải thu'");
	NewRow_Level1.Level = 1;
	SetExecutionDateToRow(NewRow_Level1, DataStructure);
	TreeRowsNumeration(NewRow_Level1, RowsCounter);
	...
	NewRow_Level1.AccountingSectionPresentation = NStr("en='Exchange rate differences';vi='Chênh lệch tỷ giá'");
	NewRow_Level1.Level = 1;
	SetExecutionDateToRow(NewRow_Level1, DataStructure);
	TreeRowsNumeration(NewRow_Level1, RowsCounter);
	...
	NewRow_Level1.AccountingSectionPresentation = NStr("en='Inventory-list without specifications';vi='Danh sách hàng tồn kho không có thành phần chi tiết'");
	NewRow_Level1.Level = 1;
	SetExecutionDateToRow(NewRow_Level1, DataStructure);
	TreeRowsNumeration(NewRow_Level1, RowsCounter);
	...
	NewRow_Level2.AccountingSectionPresentation = NStr("en='Reports of processing companies without specifications';vi='Báo cáo của các công ty chế biến không có thông số kỹ thuật'");
	...
	NewRow_Level3.AccountingSectionPresentation = NStr("en='Processor reports - discrepancies between write-offs and specifications';vi='Báo cáo bộ xử lý - sự khác biệt giữa xóa sổ và thông số kỹ thuật'");
	...
	NewRow_Level2.AccountingSectionPresentation = NStr("en='The Production documents without specifications.';vi='Các tài liệu sản xuất không có bảng kê'");
	NewRow_Level2.Level = 2;
	SetExecutionDateToRow(NewRow_Level2, DataStructure);
	TreeRowsNumeration(NewRow_Level2, RowsCounter);
	...
	NewRow_Level3.AccountingSectionPresentation = NStr("en='The Production documents - mismatch of descriptions to specifications';vi='Tài liệu sản xuất - không khớp với các mô tả về bảng kê'");
	NewRow_Level3.Level = 3;
	SetExecutionDateToRow(NewRow_Level3, DataStructure);
	TreeRowsNumeration(NewRow_Level3, RowsCounter);
	...
	NewRow_Level1.AccountingSectionPresentation = NStr("en='Analysis of procurement prices';vi='Phân tích giá mua sắm'");
	NewRow_Level1.Level = 1;
	SetExecutionDateToRow(NewRow_Level1, DataStructure);
	TreeRowsNumeration(NewRow_Level1, RowsCounter);
	...
	NewRow_Level1.AccountingSectionPresentation = NStr("en='Control of organizations and contracts in documents';vi='Kiểm soát các tổ chức và hợp đồng trong các tài liệu'");
	NewRow_Level1.Level = 1;
	SetExecutionDateToRow(NewRow_Level1, DataStructure);
	TreeRowsNumeration(NewRow_Level1, RowsCounter);
	...
	NewRow_Level1.AccountingSectionPresentation = NStr("en='Cash flow items';vi='Dạng lưu chuyển tiền tệ';ru='Статьи движения денежных средств'");
	NewRow_Level1.Level = 1;
	SetExecutionDateToRow(NewRow_Level1, DataStructure);
	TreeRowsNumeration(NewRow_Level1, RowsCounter);
	...
	NewRow_Level0.AccountingSectionPresentation = NStr("en='Final report';vi='Báo cáo tài chính'");
	NewRow_Level0.Level = 0;
	TreeRowsNumeration(NewRow_Level0, RowsCounter);
...
Procedure OutputNavigationUpdateData()
	...
	TemplateArea.Parameters.DetailRunDataAnalysis = "ExecuteProcedureOnClient#UpdateData(PredefineVariant, DataBeenChanged)";
...
//
&AtClient
Procedure UpdateData(PredefineVariant, DataBeenChanged)
...
Procedure UpdateDataByGeneratedSection(CurArea, PredefineVariant, DataBeenChanged)
	...
	Else
		...
		Message.Text = NStr("en='Handler of the data update from section is not found';vi='Không tìm thấy trình xử lý cập nhật dữ liệu từ phần'") + CurrentAccountingSectionPresentation + """";
...
Procedure OutputDataBySection()
	...
	Except
		ErrorsDescriptionArray = New Array;
		ErrorsDescriptionArray.Add(NStr("en='Unable to execute the procedure ';vi='Không thể thực hiện thủ tục'") + HandlerProcedureName);
		ErrorsDescriptionArray.Add(ErrorDescription());
		OutputErrorRow(ErrorsDescriptionArray);
	EndTry;
...
Procedure OutputTransferToNextSectionRow(TemplateOutput)
	...
	If CurrentSectionNumber = AllSectionsAvailable Then
		...
		TemplateArea.Parameters.FinalReport = NStr("en='Final report';vi='Báo cáo tài chính'");
...
Procedure CellClickProcessor(AreaName, PredefineVariant, ProcedureNameToExecuteOnClient, IsPicture = False, DataBeenChanged)
	...
	If TypeOf(AreaDecryption) = Type("String") Then
		...
		If Left(AreaDecryption, 17) = "ExecuteProcedure#" Then
			...
			Except
				ErrorsDescriptionArray = New Array;
				ErrorsDescriptionArray.Add(NStr("en='Unable to execute the procedure ';vi='Không thể thực hiện thủ tục'") + HighlightProcedureNameServer(AreaDecryption));
				ErrorsDescriptionArray.Add(ErrorDescription());
				OutputErrorRow(ErrorsDescriptionArray);
			EndTry;
...
Procedure OutputDataBySectionAccountsPayable(TemplateOutput)
	...
	For Each TabularSectionRow IN AccountsPayable Do
		...
		If TabularSectionRow.DataProcessorExecuted Then
			...
			TemplateArea.Parameters.ResultAnalysis = NStr("en='Advances were credited';vi='Các khoản nhận ứng trước'");
		...
		Else			
			...
			ElsIf TabularSectionRow.ThereAreTurnoversForPeriod Then
				...
				TemplateArea.Parameters.ExecuteAction = NStr("en='Execute expense offset';vi='Thực hiện bù đắp chi phí'");
			...
			TemplateArea.Parameters.ResultAnalysis = NStr("en='Unaccounted advances are found';vi='Không tìm thấy ứng trước'");
...
Procedure OutputDataBySectionExchangeDifferences(TemplateOutput)
	...
	Else
		...
		TemplateArea.Parameters.ExecuteAction = NStr("en='Fill reposting';vi='Điền kết chuyển'");
...
Procedure OutputDataBySectionProductsAndServicesWithoutSpecifications(TemplateOutput)
	...
	TemplateArea.Parameters.Comment = NStr("en='Records of the Products and services catalog were found in the accounting system with specifications created for them. Default specification is not specified for some products and services positions.';vi='Các bản ghi của danh mục Sản phẩm và dịch vụ đã được tìm thấy trong hệ thống kế toán với các thông số kỹ thuật được tạo cho chúng. Đặc điểm kỹ thuật mặc định không được chỉ định cho một số vị trí sản phẩm và dịch vụ.'");
	ResultDocument.Put(TemplateArea);
	...
	For Each TableRow IN ProductsAndServicesWithoutSpecifications Do
		...
		If TableRow.DataProcessorExecuted Then
			...
			TemplateArea.Parameters.ResultAnalysis = NStr("en='Open specification';vi='Mở bảng kê chi tiết'");
		...
		Else	
			...
			If TableRow.SpecificationsAmount = 1 Then
				TemplateArea.Parameters.ResultAnalysis = NStr("en='Open specification';vi='Mở bảng kê chi tiết'");
				TemplateArea.Parameters.AnalysisResultDecrypt = TableRow.Specification;
				TemplateArea.Parameters.ExecuteAction = NStr("en='Set default specification';vi='Đặt bảng kê chi tiết mặc định'");
			...
			Else
				TemplateArea.Parameters.ResultAnalysis = NStr("en='Several specifications are found';vi='Một số bảng kê được tìm thấy'");
				...
				TemplateArea.Parameters.ExecuteAction = NStr("en='Select and set specification';vi='Chọn và đặt thông số'");
...
Procedure OutputDataBySectionReportsProcWithoutSpecifications(TemplateOutput)
	...
	TemplateArea = TemplateOutput.GetArea("StringCommentsTop");
	TemplateArea.Parameters.Comment = NStr("en='Over the review period, posted documents of the Subcontractor report kind were found that do not have specifications in them. Default specification is set for products and services items specified in these documents as the goods.';vi='Trong thời gian xem xét, các tài liệu được đăng của loại báo cáo Nhà thầu phụ đã được tìm thấy không có thông số kỹ thuật trong đó. Đặc điểm kỹ thuật mặc định được đặt cho các sản phẩm và dịch vụ được chỉ định trong các tài liệu này là hàng hóa.'");
	ResultDocument.Put(TemplateArea);
	...
	For Each TableRow IN DataTable Do
		...
		If Not TableRow.DataProcessorExecuted Then
			...
			TemplateArea.Parameters.ExecuteAction = NStr("en='Set a specification';vi='Đặt bảng kê chi tiết'");
...
Procedure OutputDataBySectionReportsReprocWriteOffsMismatch(TemplateOutput)
	...
	TemplateArea = TemplateOutput.GetArea("StringCommentsTop");
	TemplateArea.Parameters.Comment = NStr("en='Over the review period posted documents of the Subcontractor report kind were found in which the written off inventory does not correspond to the specification.';vi='Trong thời gian xem xét, các tài liệu của loại báo cáo Nhà thầu phụ đã được tìm thấy trong đó hàng tồn kho bị xóa không tương ứng với đặc điểm kỹ thuật.'");
	ResultDocument.Put(TemplateArea);
	...
	For Each TableRow IN DataTable Do
		...
		If Not TableRow.DataProcessorExecuted Then
			...
			TemplateArea.Parameters.ResultAnalysis = NStr("en='Mismatch is found';vi='Tìm thấy không tương thích'");
			...
			TemplateArea.Parameters.ExecuteAction = NStr("en='Refill and repost';vi='Điền lại và kết chuyển'");
		...
		Else
			...
			TemplateArea.Parameters.ResultAnalysis = NStr("en='Corrected';vi='Đã sửa'");
...
Procedure OutputDataBySectionDocProductionWithoutSpecifications(TemplateOutput)
	...
	TemplateArea = TemplateOutput.GetArea("StringCommentsTop");
	TemplateArea.Parameters.Comment = NStr("en='Over the review period, posted documents of the Production kind were found that do not have specification (in the Products tabular sections).';vi='Trong thời gian xem xét, các tài liệu được đăng thuộc loại Sản xuất đã được tìm thấy không có thông số kỹ thuật (trong phần bảng sản phẩm).'");
	ResultDocument.Put(TemplateArea);
	...
	For Each CollectionItem IN DocProductionSpecifications Do
		...
		If Not CollectionItem.DataProcessorExecuted Then
			...
			TemplateArea.Parameters.ResultAnalysis = NStr("en='Suggested specifications';vi='Khuyến cáo bảng kê chi tiết'");
			...
			TemplateArea.Parameters.ExecuteAction = NStr("en='Set specifications';vi='Đặt bảng kê chi tiết'");
		...
		Else
			...
			TemplateArea = TemplateOutput.GetArea("StringProcessedReportReprocessing");
			TemplateArea.Parameters.ResultAnalysis = NStr("en='Specifications are filled in';vi='Điền bảng kê'");
...
Procedure OutputDataBySectionDocProductionWriteoffsMismatch(TemplateOutput)
	...
	TemplateArea = TemplateOutput.GetArea("StringCommentsTop");
	TemplateArea.Parameters.Comment = NStr("en='Over the review period, posted documents of the Production kind were found in which the written off inventory does not correspond to the specification.';vi='Trong thời gian xem xét, các tài liệu được đăng của loại Sản xuất đã được tìm thấy trong đó hàng tồn kho bị xóa không tương ứng với đặc điểm kỹ thuật.'");
	ResultDocument.Put(TemplateArea);
...
"часть Материалы по спецификации, обнаружено рекурсивное вхождение элемента';vi='Khi thử điền phần bảng"
"Nguyên vật liệu theo bảng kê chi tiết, đã tìm thấy sự xuất hiện đệ quy của phần tử'")+" "+Selection.ProductsAndServices+" "+NStr("en='in BOM';ru='в спецификации';vi='trong bảng kê chi tiết'")+" "+Selection.ProductionSpecification+"
...
Procedure OutputDataBySectionPurchasePricesAnalysis(TemplateOutput)
	...
	TemplateArea = TemplateOutput.GetArea("StringCommentsTop");
	TemplateArea.Parameters.Comment = NStr("en='Over the review period sharp changes of purchase prices were found for some products and services positions (5 or more times).';vi='Trong thời gian xem xét, những thay đổi mạnh về giá mua đã được tìm thấy đối với một số vị trí sản phẩm và dịch vụ (5 lần trở lên).'");
	ResultDocument.Put(TemplateArea);
	...
	For Each StringProductsAndServices IN RowsProductsAndServices Do
		...
		TemplateArea.Parameters.ResultAnalysis = NStr("en='Analysis result';vi='Kết quả phân tích'");
...
Procedure FillDataByCompaniesContractsControl()
	...
	While Selection.Next() Do
		...
		If RuleStructure.Property(DocumentName, DocumentRulesTable) Then
			...
			Except
				Message = New UserMessage;
				Message.Text = NStr("en='Unable to check document ';vi='Không thể kiểm tra tài liệu'") + String(Selection.DocumentRef) + Chars.LF + ErrorDescription();
				Message.SetData(Object);
				Message.Message(); 
			EndTry;	
...
Function GenerateQueryTextByDocuments(RuleStructure)
	...
	If QueryText <> "" Then
		QueryText = QueryText + Chars.LF + Chars.LF + NStr("en='ORDER BY';vi='ĐẶT BỞI'") + Chars.LF + Chars.Tab + NStr("en='Date';vi='Ngày'");	
	EndIf;
	...
	If ContractLocation.AttributeType = "Attribute" Then
		...
		If ControlMatchCompaniesInContracts Then
			...
			If CurCompany <> CurrContract.Company Then
				...
				NewRow.MessageText = NStr("en='Company specified in the document does not match company specified in the contract ';vi='Công ty quy định trong tài liệu không khớp với công ty quy định trong hợp đồng'") + TrimAll(CurrContract.Description);
...
Procedure CompareDocumentsData_ContractInAttribute(CheckObject, ContractLocationTable, CurCompany, CurrContract, TableOfResults, DocumentRef, RulesTablesRow, SubordinateLocation)
	...
	ElsIf ContractLocationSubordinated.AttributeType = "TabularSection" Then
		...
		If CompaniesMismatch OR ContractsMismatch Then
			...
			NewRow.SubordinateDocument = CheckObject;
			NewRow.MessageText = GenerateMessageTextByMismatch(CompaniesMismatch, ContractsMismatch) 
			+ NStr("en='. Location: ';vi='. Kho bãi:'") + SubordinateLocation + " " + RulesTablesRow.TabSecAttributeSynonym;
...
Function GenerateMessageTextByMismatch(CompaniesMismatch, ContractsMismatch)
	...
	If CompaniesMismatch AND ContractsMismatch Then
		...
		MessageText = NStr("en='Mismatches of companies to contracts are found';vi='Tìm thấy không tương thích giữa doanh nghiệp và hợp đồng'");
		
	ElsIf CompaniesMismatch Then
		
		MessageText = NStr("en='Companies mismatch is found';vi='Không tìm thấy doanh nghiệp phù hợp'");
		
	ElsIf ContractsMismatch Then
		
		MessageText = NStr("en='Contracts mismatch is found';vi='Hợp đồng không phù hợp được tìm thấy'");
...
Procedure OutputFirstPage()
	...
	For Each String_Level0 IN DataTree.Rows Do
		...
		For Each String_Level1 IN String_Level0.Rows Do
			...
			Except
				Message = New UserMessage;
				Message.Text = NStr("en='Description to section is not found ';vi='Không tìm thấy mô tả cho phần'") + String_Level1.AccountingSection;
				Message.SetData(Object);
				Message.Message(); 
			EndTry;
...
Procedure OutputFinalReport_AtServer()
	...
	TemplateArea.Parameters.Exit = NStr("en='EXIT';vi='THOÁT'");
...
Procedure RefillVendorDocuments(CounterpartyRef)
	...
	While Selection.Next() Do		
		...
		If IsPaymentDocument(Selection.Recorder) Then//decryption refilling
			...
			Except
				ErrorString = NStr("en='Unable to process document ';vi='Không thể xử lý chứng từ'") + String(Selection.Recorder);
				Message = New UserMessage;
...
Procedure ExecuteRepostByCurrencyRatesDifferences(PredefineVariant, DataBeenChanged)
	...
	For Each TableRow IN TableForReposting Do
		...
		Except
			...
			ErrorText = NStr("en='Unable to repost document ';vi='Không thể kết chuyển'") + String(TableRow.DocumentRef) + Chars.LF + ErrorDescription();
...
Procedure SetDefaultSpecification(CurArea, PredefineVariant, DataBeenChanged)
	...
	If RowArray.Count() > 0 Then
		...
		Except
			Message = New UserMessage;
			Message.Text = NStr("en='Unable to set specification.';vi='Không thể đặt bảng kê'") + Chars.LF + ErrorDescription();
			Message.SetData(Object);
			Message.Message();
			Return;
		EndTry;
...
Procedure SetSpecificationToProcReport(CurArea, PredefineVariant, DataBeenChanged)
	...
	If RowArray.Count() > 0 Then
		...
		Except
			Message = New UserMessage;
			Message.Text = NStr("en='Unable to write';vi='Không thể ghi'") + String(CurrentRef) + """" + Chars.LF + ErrorDescription();
			Message.SetData(Object);
			Message.Message();
...
Procedure CorrectNoncorrReportProc(CurArea, PredefineVariant, DataBeenChanged)
	...
	If RowArray.Count() > 0 Then
		...
		Except
			Message = New UserMessage;
			Message.Text = NStr("en='Unable to repost document ';vi='Không thể kết chuyển'") + String(DocumentRef) + Chars.LF + ErrorDescription();
			Message.SetData(Object);
			Message.Message(); 
			Return;
		EndTry;
...
Procedure SetSpecificationsInDocProcuction(CurArea, PredefineVariant, DataBeenChanged)
	...
	If RowArray.Count() > 0 Then
		...
		Except
			Message = New UserMessage;
			Message.Text = NStr("en='Unable to write';vi='Không thể ghi'") + String(DocumentRef) + """" + Chars.LF + ErrorDescription();
			Message.SetData(Object);
			Message.Message();
...
Function GenerateDocumentPresentation(DocumentRef)
	...
	DocumentPresentation = DocumentPresentation + NStr("en=' # ';vi=' số'")
												+ DocumentNumber
...
Procedure ExecuteProcedureOnWebClient(ProcedureCallText, CurArea, PredefineVariant, DataBeenChanged)
	...
	If ProcedureCallText = "UpdateData(PredefineVariant, DataBeenChanged)" Then
		...
		UpdateData(PredefineVariant, DataBeenChanged);
	...
	Else
		...
		Raise ProcedureCallText + NStr("en=' - handler is not defined for a web client';vi='- handler không được xác định cho máy khách web'");
