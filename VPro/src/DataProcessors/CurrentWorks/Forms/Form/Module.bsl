...
Procedure LaunchBackgroundJobOnServer(ExecutionBroken = False)
	...
	Else
		...
		JobDescription = NStr("en='Update current to-dos';ru='Обновление списка текущих дел';vi='Cập nhật danh sách công việc hiện tại'");
...
Function Group(GroupName, Parent = Undefined, GroupType = "")
	...
	If GroupType = "SectionTitle" Then
		NewGroup.Group = ChildFormItemsGroup.HorizontalIfPossible;
	Else
...
Procedure CreateTitle(Work, Group, SectionCollapsed)
	...
	Item.ToolTip = NStr("en='Expand/collapse section';ru='Развернуть/свернуть раздел';vi='Mở ra/cuộn vào phần hành'");
