...
Procedure SetupAtServer(RemoveActuality = False)
	...
	While Selection.Next() Do
		...
		Message.Text = NStr("en='The string %ProductsAndServices%%Charachteristic% is duplicated!';ru='Дублируется строка: %Номенклатура%%Характеристика%!';vi='Dòng bị nhân đôi: %ProductsAndServices%%Charachteristic%!'");
	...
	If Cancel Then
		Message = New UserMessage();
		Message.Text = NStr("en='Price setup canceled.';ru='Установка цен отменена!';vi='Đã hủy bỏ thiết lập giá!'");
		Message.Message();
		Return;
	EndIf;
	...
	While Selection.Next() Do
		...
		If Selection.double Then
			...
			Message = New UserMessage();
			Message.Text = NStr("en='Price for products and services %ProductsAndServices%%Characteristic% on %ToDate% is already set. The new price is not written.';ru='Для номенклатуры %Номенклатура%%Характеристика% на %НаДату% уже установлена цена! Новая цена не записана!';vi='Đối với mặt hàng %ProductsAndServices%%Characteristic% tại ngày %ToDate% đã thiết lập giá! Không thể ghi lại giá mới!'");
		...
		ElsIf Not ValueIsFilled(Selection.Price) AND (NOT RemoveActuality) Then
			...
			Message = New UserMessage();
			Message.Text = NStr("en='Price for products and services %ProductsAndServices%%Characteristic% is not specified.';ru='Для номенклатуры %Номенклатура%%Характеристика% не указана цена!';vi='Đối với mặt hàng %ProductsAndServices%%Characteristic% chưa chỉ ra giá!'");
			Message.Text = StrReplace(Message.Text, "%ProductsAndServices%", Selection.ProductsAndServices);
			Message.Text = StrReplace(Message.Text, "%Characteristic%", ?(ValueIsFilled(Selection.Characteristic), 
										 (" (" + Selection.Characteristic + ")"), ""));
			Message.Message();
...
"Продолжить?';vi='Sẽ xóa phần bảng!"
"Tiếp tục?'");
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.Property("PriceKind") AND ValueIsFilled(Parameters.PriceKind) Then
		...
		If ParameterPriceKind.CalculatesDynamically Then
			...
			MessageText = NStr("en='Cannot generate dynamic price kinds.';ru='Нельзя формировать динамические виды цен!';vi='Không thể tạo dạng giá động!'");
...
Procedure Set(Command)
	...
	If Not ValueIsFilled(PriceKindInstallation) Then
		...
		Message = New UserMessage();
		Message.Text = NStr("en='Step 1: The price kind is not selected.';ru='Шаг 1: Не выбран Вид цен!';vi='Bước 1: Chưa chọn Dạng giá!'");
	...
	If Not ValueIsFilled(InstallationPeriod) Then
		...
		Message = New UserMessage();
		Message.Text = NStr("en='Step 4: Price set date is not selected.';ru='Шаг 4: Не выбрана дата установки цен!';vi='Bước 4: Chưa chọn ngày đặt đơn giá!'");
