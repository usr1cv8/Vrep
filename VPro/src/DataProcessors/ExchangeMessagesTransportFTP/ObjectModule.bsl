...
Function GetExchangeMessage()
	...
	If ExchangeMessageFileTable.Count() = 0 Then
		...
		MessageString = NStr("en='Data exchange directory on server: ""%1""';ru='Каталог обмена информацией на сервере: ""%1""';vi='Thư mục trao đổi thông tin trên Server: ""%1""'");
...
Procedure ErrorMessagesInitialization()
	...
	// Common error codes
	ErrorMessages.Insert(001, NStr("en='Information exchange directory does not contain a message file with data.';ru='В каталоге обмена информацией не был обнаружен файл сообщения с данными.';vi='Trong thư mục trao đổi thông tin không tìm thấy tệp thông điệp trao đổi có dữ liệu.'"));
	...
	ErrorMessages.Insert(004, NStr("en='An error occurred when creating a temporary directory.';ru='Ошибка при создании временного каталога.';vi='Lỗi khi tạo thư mục tạm thời.'"));
	...
	ErrorMessages.Insert(101, NStr("en='Path on the server is not specified.';ru='Не задан путь на сервере.';vi='Chưa đặt đường dẫn trên Server.'"));
	ErrorMessages.Insert(102, NStr("en='An error occurred when initializing connection to the FTP server.';ru='Ошибка инициализации подключения к FTP-серверу.';vi='Lỗi khởi xướng kết nối đến server FTP.'"));
	ErrorMessages.Insert(103, NStr("en='An error occurred when connecting to the FTP server, check the path  correctness and access rights to the resource.';ru='Ошибка подключения к FTP-серверу, проверьте правильность задания пути и права доступа к ресурсу.';vi='Lỗi kết nối với server FTP, hãy kiểm tra đường dẫn và quyền truy cập đến nguồn.'"));
	ErrorMessages.Insert(104, NStr("en='An error occurred when searching for files on FTP server.';ru='Ошибка при поиске файлов на FTP-сервере.';vi='Lỗi khi tìm kiếm tệp trên Server FTP.'"));
	ErrorMessages.Insert(105, NStr("en='An error occurred when receiving the file from FTP server.';ru='Ошибка при получении файла с FTP-сервера.';vi='Lỗi khi nhận tệp từ Server FTP.'"));
	ErrorMessages.Insert(106, NStr("en='An error occurred when removing file on FTP server, check access rights to the resource.';ru='Ошибка удаления файла на FTP-сервере, проверьте права доступа к ресурсу.';vi='Lỗi xóa tệp trên Server-FTP, hẫy kiểm tra quyền truy cập đến nguồn.'"));
...
"Рекомендуется также обратиться к документации по FTP-серверу для настройки поддержки имен файлов с кириллицей.';vi='Không thể kiểm tra kết nối bằng tệp văn bản ""%1""."
"Có thể thư mục đã chọn không tồn tại hoặc không sử dụng được."
"Nên sử dụng bộ tài liệu về FTP-server để tùy chỉnh hỗ trợ tên tệp có chữ cái cyrillic'");
