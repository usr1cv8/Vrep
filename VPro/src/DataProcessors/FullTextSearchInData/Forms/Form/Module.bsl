	...
	If Not IsBlankString(Parameters.TransferredSearchString) Then
		...
		If SearchResults.Count() <> 0 Then
			...
			ShowedResultsFromTo = StringFunctionsClientServer.SubstituteParametersInString(
			                            NStr("en='Shown %1 - %2 out of %3';ru='Показаны %1 - %2 из %3';vi='Đã chỉ ra %1 - %2 trong số %3'"),
		...
		Else
			ShowedResultsFromTo = NStr("en='Not found';ru='Не найден';vi='Chưa tìm thấy'");
			Items.GoToNext.Enabled = False;
			Items.Back.Enabled = False;
		EndIf;
	Else
...
Procedure Search(Direction, Cancel = Undefined)
	...
	If IsBlankString(SearchString) Then
		CommonUseClientServer.MessageToUser(NStr("en='Enter a search object.';ru='Введите, что нужно найти.';vi='Hãy nhập những gì cần tìm.'"), , "SearchString");
		Cancel = True;
		Return;
	EndIf;
	...
	Status(StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Searching ""%1""...';ru='Выполняется поиск ""%1""...';vi='Thực hiện tìm kiếm ""%1""...'"), SearchString));
	...
	If SearchResults.Count() > 0 Then
		...
		If Direction = 0 AND Result.CurrentPosition = 0 AND Result.TooManyResults Then
			ShowMessageBox(, NStr("en='Too many results. Refine your search criteria.';ru='Слишком много результатов, уточните запрос.';vi='Có rất nhiều kết quả, hãy làm rõ truy vấn.'"));
		EndIf;
...
"<li>Try to reduce the number of the searched words.';ru='Комбинация слов ""%1"" нигде не встречается.<br><br> <b>Рекомендации:</b> <li>Убедитесь, что все слова написаны без ошибок. <li>Попробуйте использовать другие ключевые слова. <li>Попробуйте уменьшить количество искомых слов.';vi='Sự kết hợp các từ ""%1"" không xảy ra ở đâu.<br><br> <b>Đề xuất:</b> <li>Hãy chắc chắn rằng, đã viết đúng tất cả các từ. <li>Hãy thử sử dụng các từ khóa khác. <li>Hãy thử giảm số lượng từ tìm kiếm.'"),
