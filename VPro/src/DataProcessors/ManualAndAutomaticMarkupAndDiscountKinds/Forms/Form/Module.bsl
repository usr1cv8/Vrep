	...
	For Each AutoDiscountRow IN SelectedRowArray Do
		If AutoDiscountRow.Acts <> NewValue Then
			...
			Except
				...
				Message.Text = NStr("en='Failed to change the ""Active"" flag for automatic discounts, markups ""';vi='Không thể thay đổi cờ ""Hoạt động"" để tự động giảm giá, đánh dấu ""'")+AutoDiscountRow.Ref+"""!";
				Message.Field = "Acts";
...
"списка рекомендуется установить по полю ""Порядок"". Настроить необходимую сортировку?';vi='Sắp xếp...'");
		...
		Buttons.Add(DialogReturnCode.Yes, NStr("en='Customize';ru='Настроить';vi='Tùy chỉnh'"));
		Buttons.Add(DialogReturnCode.No, NStr("en='Do not configure';ru='Не настраивать';vi='Không tùy chỉnh'"));
...
Procedure SetListSortingByFieldOrderAtClient()
	...
	CommonUseClientServer.Validate(UserOrderSettings <> Undefined, NStr("en='Custom order setup setting was not found.';ru='Пользовательская настройка порядка не найдена.';vi='Không tìm thấy tùy chỉnh trình tự tự tạo.'"));
...
Procedure VisibleManagementServer()
	...
	If UseAutomaticDiscountsMarkups AND Not UseDiscountsMarkups Then
		...
		Title = NStr("en='Kinds of automatic discounts, markups';ru='Виды автоматических скидок, наценок';vi='Dạng chiết khấu, phụ thu tự động'");
	ElsIf Not UseAutomaticDiscountsMarkups AND UseDiscountsMarkups Then
		Items.GroupAutomaticDiscounts.Visible = False;
		Items.GroupManualAndAutomaticDiscounts.PagesRepresentation = FormPagesRepresentation.None;
		Title = NStr("en='Markup and discount kinds';ru='Виды скидок, наценок';vi='Dạng chiết khấu, phụ thu'");
	Else
		...
		Title = NStr("en='Discounts and markups';ru='Скидки и наценки';vi='Chiết khấu và phụ thu'");
	EndIf;
