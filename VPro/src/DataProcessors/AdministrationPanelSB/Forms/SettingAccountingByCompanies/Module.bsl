...
//
&AtServer
Function CancellationUncheckMultipleCompaniesAccounting()
	...
	While SelectionCompanies.Next() Do
		...
		If SelectionCompanies.Ref <> MainCompany Then
			...
			If RefsTable.Count() > 0 Then
				...
				MessageText = NStr("en='Companies that differ from the main one are used in the infobase. Cannot clear the check box.';ru='В базе используются организации, отличные от основной! Снятие опции запрещено!';vi='Trong cơ sở dữ liệu đã sử dụng doanh nghiệp khác với chính! Không được phép xóa bỏ tùy chọn!'");
...
EndFunction // CancellationUncheckMultipleCompaniesAccounting()
...
Function CancellationUncheckAccountingBySubsidiaryCompany()
	...
	If AreDocuments Then
		MessageText = NStr("en='There are posted documents of a company which differs from the company in the infobase. You cannot clear the ""Company accounting"" check box.';ru='В базе есть проведенные документы от организации, отличной от компании! Снятие флага ""Учет по компании"" запрещено!';vi='Trong cơ sở dữ liệu đã có chứng từ kết chuyển khác doanh nghiệp! Không được phép bỏ dấu hộp kiểm ""Kế toán theo doanh nghiệp""'");	
...
Procedure BeforeWriteAtServer(Cancel, CurrentObject, WriteParameters)
	...
	If Constants.FunctionalOptionAccountingByMultipleCompanies.Get() <> ConstantsSet.FunctionalOptionAccountingByMultipleCompanies
		...
		AND CancellationUncheckMultipleCompaniesAccounting() Then
	...
	If ConstantsSet.AccountingBySubsidiaryCompany 
		...
		MessageText = NStr("en='The ""Keep accounting by company"" check box is selected, but the company is not filled in.';ru='Установлен флаг ""Вести учет по компании"", но не заполнена ""Организация-компания""!';vi='Đã đặt dấu hộp kiểm ""Tiến hành kế toán theo Doanh nghiệp"" nhưng chưa điền ""Doanh nghiệp""!'");
...
// Procedure - event handler OnChange of the MultipleCompaniesAccounting field.
//
&AtClient
Procedure MultipleCompaniesAccountingOnChange(Item)
...
EndProcedure // MultipleCompaniesAccountingOnChange()
// 
