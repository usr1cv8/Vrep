...
Function CancellationUncheckFunctionalCurrencyTransactionsAccounting()
	...
	If Not QueryResult.IsEmpty() Then
		...
		MessageText = NStr("en='Currencies that are different from the national one are registered in the infobase. It is required to delete them. Cannot clear the check box.';ru='В базе заведены валюты, отличные от национальной! Необходимо их удалить. Снятие флага запрещено!';vi='Trong cơ sở đã nhập tiền tệ khác với nội tệ! Cần xóa bỏ chúng. Không được phép xóa dấu hộp kiểm!'");
...
Function CancellationToChangeAccountingCurrency()
	...
	If AreRecords Then
		...
		MessageText = NStr("en='There are records in ""value"" accumulation registers in the infobase. You cannot change the accounting currency.';ru='В базе есть движения по ""суммовым"" регистрам накопления! Изменение валюты учета запрещено!';vi='Trong cơ sở dữ liệu đã có bản ghi kết chuyển theo biểu ghi ""giá trị"" tích lũy! Không được phép thay đổi tiền tệ kế toán!'");	
...
// Procedure - event handler OnChange of the FunctionalOptionUseLoanContracts field.
//
&AtClient
Procedure FunctionalOptionUseLoanContractsOnChange(Item)
