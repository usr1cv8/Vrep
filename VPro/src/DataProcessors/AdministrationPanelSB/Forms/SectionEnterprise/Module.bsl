	...
	While SelectionOfBusinessActivity.Next() Do
		...
		If SelectionOfBusinessActivity.Ref <> Catalogs.BusinessActivities.MainActivity
			...
			If RefsTable.Count() > 0 Then
				...
				ErrorText = NStr("en='Business areas which are different from the main one are used in the infobase. Cannot disable the option.';ru='В базе используются направления деятельности, отличные от основного! Снятие опции запрещено!';vi='Trong cơ sở dữ liệu đã sử dụng lĩnh vực hoạt động kinh doanh khác với lĩnh vực hoạt động kinh doanh chính! Không được phép xóa bỏ tùy chọn!'");
				Break;
...
Function CancellationUncheckAccountingBySeveralDepartments() 
	...
	If Not QueryResult.IsEmpty() Then
		...
		ErrorText = NStr("en='Departments which are different from the main one are used in the infobase. Cannot disable the option.';ru='В базе используются подразделения, отличные от основного! Снятие опции запрещено!';vi='Trong cơ sở dữ liệu đã có sử dụng bộ phận khác với bộ phận chính! Không được phép xóa bỏ tùy chọn!'");
...
Function CancellationUncheckFunctionalOptionAccountingFixedAssets()
	...
	If Cancel Then
		...
		ErrorText = NStr("en='There are capital asset movements in the infobase. Cannot clear the check box.';ru='В базе присутствуют движения по внеоборотным активам! Снятие флага запрещено!';vi='Trong cơ sở thông tin có bản ghi kết chuyển theo tài sản dài hạn! Không được phép xóa dấu hộp kiểm!'");
