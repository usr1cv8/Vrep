...
Function CheckRecordsByPayrollSubsystemRegisters()
	...
	If Not ResultsArray[0].IsEmpty() Then
		...
		ErrorText = NStr("en='There are records in register ""Accruals and deductions"" in the infobase. You cannot clear the ""Salary"" check box.';ru='В информационной базе присутствуют движения по регистру ""Начисления и удержания""! Снятие флага ""Зарплата"" запрещено!';vi='Trong cơ sở thông tin có biến động theo biểu ghi ""Tính lương và giữ lại""! Cấm bỏ dấu hộp kiểm ""Tiền lương""!'");
	...
	If Not ResultsArray[1].IsEmpty() Then
		...
		ErrorText = ErrorText + ?(IsBlankString(ErrorText), "", Chars.LF) + NStr("en='There are records in register ""Settlements with employees"" in the infobase. You cannot clear the ""Salary"" check box.';ru='В информационной базе присутствуют движения по регистру ""Расчеты с персоналом""! Снятие флага ""Зарплата"" запрещено!';vi='Trong cơ sở thôn tin có biến động theo biểu ghi ""Hạch toán với người lao động""! Cấm bỏ dấu hộp kiểm ""Tiền lương""!'");
...
Function CancellationUncheckFunctionalOptionUsePayrollSubsystem()
	...
	If Not ResultsArray[0].IsEmpty() Then
		...
		ErrorText = NStr("en='Infobase contains documents ""Payroll""! It is prohibited to clear the ""Payroll"" check box!';ru='В информационной базе присутствуют документы ""Начисление зарплаты""! Снятие флага ""Зарплата"" запрещено!';vi='Trong cơ sở thông tin có chứng từ ""Tính lương""! Cấm bỏ dấu hộp kiểm ""Tiền lương""!'");
	...
	If Not ResultsArray[1].IsEmpty() Then
		...
		ErrorText = ErrorText + ?(IsBlankString(ErrorText), "", Chars.LF) + NStr("en='There are documents of the ""Job sheet"" kind in the infobase that are used to calculate employees'' salary. You cannot clear the ""Salary"" check box.';ru='В информационной базе присутствуют документы ""Сдельный наряд"", которые начисляют зарплату сотрудникам! Снятие флага ""Зарплата"" запрещено!';vi='Trong cơ sở thông tin có chứng từ ""Công khoán"" mà tính lương cho người lao động! Cấm bỏ dấu hộp kiểm ""Tiền lương""!'");
	...
	If Not ResultsArray[2].IsEmpty() Then
		...
		ErrorText = ErrorText + ?(IsBlankString(ErrorText), "", Chars.LF) + NStr("en='There are documents of the ""Work order"" kind in the infobase that are used to calculate employees'' salary. You cannot clear the ""Salary"" check box.';ru='В информационной базе присутствуют документы ""Заказ - наряд"", которые начисляют зарплату сотрудникам! Снятие флага ""Зарплата"" запрещено!';vi='Trong cơ sở thông tin có chứng từ ""Đơn hàng trọn gói"" mà tính lương cho người lao động! Cấm bỏ dấu hộp kiểm ""Tiền lương""!'");
	...
	If Not ResultsArray[3].IsEmpty() Then
		...
		ErrorText = ErrorText + ?(IsBlankString(ErrorText), "", Chars.LF) +  NStr("en='There are ""Entry of opening balance"" documents by accounting section ""Settlements with employees"" in the infobase. You cannot clear the ""Salary"" check box.';ru='В информационной базе присутствуют документы ""Ввод начальных остатвков"", по разделу учета ""Расчеты с персоналом""! Снятие флага ""Зарплата"" запрещено!';vi='Trong cơ sở thông tin có chứng từ ""Nhập số dư ban đầu"", theo phần hành kế toán ""Hạch toán với người lao động""! Cấm bỏ dấu hộp kiểm ""Tiền lương""!'");
	...
	If Not ResultsArray[6].IsEmpty() Then
		...
		ErrorText = ErrorText + ?(IsBlankString(ErrorText), "", Chars.LF) + NStr("en='There are employees with the ""Secondary employment"" employment type in the infobase. Clearing the ""Salary"" check box is prohibited.';ru='В информационной базе есть сотрудники с типом занятости ""Совместительство""! Снятие флага ""Зарплата"" запрещено!';vi='Trong cơ sở thông tin có người lao động với dạng công việc ""Kiêm nhiệm""! Cấm bỏ dấu hộp kiểm ""Tiền lương""!'");	
	...
	If Not ResultsArray[7].IsEmpty() Then
		...
		ErrorText = ErrorText + ?(IsBlankString(ErrorText), "", Chars.LF) + NStr("en='There are items of the ""Accrual and deduction kinds"" catalog with type ""Tax"" in the infobase. You cannot clear the ""Salary"" check box.';ru='В информационной базе присутствуют элементы справочника ""Виды начислений и удержаний"" с типом ""Налог""! Снятие флага ""Зарплата"" запрещено!';vi='Trong cơ sở thông tin có phần tử danh mục ""Dạng tính lương và giữ lại"" có kiểu ""Thuế""! Cấm bỏ dấu hộp kiểm ""Tiền lương""!'");
...
Function CancellationUncheckFunctionalOptionUseJobsharing()
	...
	If Not QueryResult.IsEmpty() Then
		...
		ErrorText = NStr("en='There are employees with the ""Secondary employment"" employment type in the base. Clearing the check box is prohibited.';ru='В базе есть сотрудники с типом занятости ""Совместительство""! Снятие флага запрещено!';vi='Trong cơ sở thông tin có người lao động với kiểu công việc ""Kiêm nhiệm""! Không được phép xóa dấu hộp kiểm!'");
...
Function CancellationUncheckFunctionalOptionAccountingDoIncomeTax()
	...
	If Not QueryResult.IsEmpty() Then
		...
		ErrorText = NStr("en='There are items of the ""Accrual and deduction kinds"" catalog with type ""Tax"" in the infobase. You cannot clear the ""Salary"" check box.';ru='В базе присутствуют элементы справочника ""Виды начислений и удержаний"" с типом ""Налог""! Снятие флага запрещено!';vi='Trong cơ sở thông tin có phần tử danh mục ""Dạng tính lương và giữ lại"" với kiểu ""Thuế""! Không được phép xóa dấu hộp kiểm!'");	
