...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	If CurrentPage = Items.DataImportPages.ChildItems.InformationAndBackupPerformingPage Then
		...
		WarningText = NStr("en='Stop preparing to restore data?';ru='Прервать подготовку к восстановлению данных?';vi='Ngừng khôi phục dữ liệu?'");
...
Procedure Done(Command)
	...
	InfobaseConnectionsServerCall.SetConnectionLock(NStr("en='Infobase recovery is in progress.';ru='Выполняется восстановление информационной базы.';vi='Đang phục hồi cơ sở thông tin.'"), "Backup");
...
Procedure SelectBackupFile()
	...
	FileOpeningDialog.Filter = NStr("en='Archive with backup (*.zip)|*.zip';ru='Архив с резервной копией (*.zip)|*.zip';vi='Lưu trữ với bản sao dự phòng (*.zip)|*.zip'");
	FileOpeningDialog.Title= NStr("en='Select a backup file';ru='Выберите файл резервной копии';vi='Hãy chọn tệp sao lưu dự phòng'");
...
Function CheckAttributesFilling()
	...
	If IsBlankString(FileName) Then
		MessageText = NStr("en='Backup file is not selected.';ru='Не выбран файл с резервной копией.';vi='Chưa chọn tệp có bản sao dự phòng.'");
	...
	If FileOfArchive.Extension <> ".zip" Then
		...
		MessageText = NStr("en='The selected file is not an archive with backup.';ru='Выбранный файл не является архивом с резервной копией.';vi='Tệp đã chọn không phải là tệp lưu trữ có bản sao dự phòng.'");
		CommonUseClientServer.MessageToUser(MessageText,, "Object.BackupImportingFile");
		Return False;
	...
	If ZipFile.Items.Count() <> 1 Then
		...
		MessageText = NStr("en='The selected file is not an archive with backup (contains more than one file).';ru='Выбранный файл не является архивом с резервной копией (содержит более одного файла).';vi='Tệp đã chọn không phải là tệp lưu trữ có bản sao dự phòng (có nhiều hơn một tệp).'");
		CommonUseClientServer.MessageToUser(MessageText,, "Object.BackupImportingFile");
		Return False;
	...
	If UPPER(FileInArchive.Extension) <> "1CD" Then
		...
		MessageText = NStr("en='The selected file is not an archive with backup (does not contain an infobase file).';ru='Выбранный файл не является архивом с резервной копией (не содержит файл информационной базы).';vi='Tệp đã chọn không phải là tệp lưu trữ có bản sao dự phòng (không có tệp cơ sở thông tin).'");
		CommonUseClientServer.MessageToUser(MessageText,, "Object.BackupImportingFile");
		Return False;
	...
	If UPPER(FileInArchive.BaseName) <> "1CV8" Then
		...
		MessageText = NStr("en='The selected file is not an archive with backup (invalid name of infobase file).';ru='Выбранный файл не является архивом с резервной копией (неправильное имя файла информационной базы).';vi='Tệp đã chọn không phải là tệp lưu trữ có bản sao dự phòng (sai tên tệp cơ sở thông tin).'");
		CommonUseClientServer.MessageToUser(MessageText,, "Object.BackupImportingFile");
		Return False;
...
"Подготовка к восстановлению данных из резервной копии отменена. Информационная база разблокирована.';vi='%1."
"Đã hủy chuẩn bị phục hồi dữ liệu từ bản sao dự phòng. Đã bỏ phong tỏa cơ sở thông tin.'"),
...
Procedure StartBackup() 
	...
	EventLogMonitorClient.AddMessageForEventLogMonitor(InfobaseBackupClient.EventLogMonitorEvent(), 
		"Information",
		NStr("en='Infobase backup is in progress:';ru='Выполняется резервное копирование информационной базы:';vi='Đang sao lưu dự phòng cơ sở thông tin:'") + " " + ScriptMainFileName);
