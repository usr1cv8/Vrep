...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not CommonUse.FileInfobase() Then
		Raise NStr("en='Back up data using external tools (DBMS tools) in the client/server mode.';ru='В клиент-серверном варианте работы резервное копирование следует выполнять сторонними средствами (средствами СУБД).';vi='Ở phương án làm việc Client-server, cần tiến hành sao lưu dự phòng bằng các công cụ bên ngoài (công cụ của Hệ quản trị cơ sở dữ liệu).'");
	EndIf;
	...
	If BackupSettings.DateOfLastBackup = Date(1, 1, 1) Then
		HeaderText = NStr("en='Backup has never been made';ru='Резервное копирование еще ни разу не проводилось';vi='Sao lưu dự phòng còn chưa được thực hiện lần nào'");
	Else
		HeaderText = NStr("en='Last backup: %1';ru='В последний раз резервное копирование проводилось: %1';vi='Ở lần tiếp theo đã thực hiện việc sao lưu dự phòng: %1'");
	...
	If ManualStart Then
		...
		Items.Next.Title = NStr("en='Save backup';ru='Сохранить резервную копию';vi='Lưu bản sao dự phòng'");
...
Procedure OnOpen(Cancel)
	...
	If CommonUseClientServer.IsLinuxClient() Then
		Cancel = True;
		MessageText = NStr("en='Backup is not available on the client running Linux OS.';ru='Резервное копирование недоступно в клиенте под управлением ОС Linux.';vi='Không cho phép sao lưu dự phòng tại Client chạy hệ điều hành Linux.'");
...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	If CurrentPage = Items.AssistantPages.ChildItems.InformationAndBackupPerformingPage Then
		...
		WarningText = NStr("en='Stop preparing for backup?';ru='Прервать подготовку к резервному копированию данных?';vi='Ngừng chuẩn bị sao lưu dự phòng dữ liệu?'");
...
Procedure GoToBackupInformationAndExecutionPage()
	...
	If InfobaseSessionCount() = 1 Then
		...
		InfobaseConnectionsServerCall.SetConnectionLock(NStr("en='For backup execution.';ru='Для выполнения резервного копирования.';vi='Để thực hiện bản sao dự phòng.'"), "Backup");
...
Procedure GoToBackupResultsPage()
	...
	Items.Cancel.Title = NStr("en='Close';ru='Закрыть';vi='Đóng'");
...
Function CheckAttributesFilling(ShowError = True)
	...
	If IsBlankString(Object.BackupDirectory) Then
		...
		MessageText = NStr("en='Backup directory is not selected.';ru='Не выбран каталог для резервной копии.';vi='Chưa chọn thư mục lưu bản sao dự phòng.'");
	...
	ElsIf FindFiles(Object.BackupDirectory).Count() = 0 Then
		...
		MessageText = NStr("en='Non-existing directory is specified.';ru='Указан несуществующий каталог.';vi='Chỉ ra thư mục không tồn tại.'");
		WriteAttributesCheckError(MessageText, "Object.BackupDirectory", ShowError);
		AttributesFilled = False;
	...
	Else
		...
		Try
			...
			TestFile.Close();
		Except
			MessageText = NStr("en='Cannot access directory with backups.';ru='Нет доступа к каталогу с резервными копиями.';vi='Không truy cập tới thư mục có bản sao dự phòng.'");
			WriteAttributesCheckError(MessageText, "Object.BackupDirectory", ShowError);
			AttributesFilled = False;
		EndTry;
	...
	If PasswordEnterIsRequired AND IsBlankString(IBAdministratorPassword) Then
		...
		MessageText = NStr("en='Administrator password is not specified.';ru='Не задан пароль администратора.';vi='Chưa xác định mật khẩu quản trị.'");
...
Procedure TimeOutLapse()
	...
	DetachIdleHandler("CheckThatConnectionIsSingle");
	QuestionText = NStr("en='Cannot disconnect all users from the base. Back up the data? (errors can occur during backup)';ru='Не удалось отключить всех пользователей от базы. Провести резервное копирование? (возможны ошибки при архивации)';vi='Không thể ngắt bỏ tất cả người sử dụng khỏi cơ sở. Tiến hành sao lưu dự phòng? (có thể lỗi khi lưu trữ)'");
	ExplanationText = NStr("en='Cannot disable the user.';ru='Не удалось отключить пользователя.';vi='Không thể ngắt bỏ người sử dụng.'");
...
"Подготовка к резервному копированию отменена. Информационная база разблокирована.';vi='%1."
"Đã hủy chuẩn bị sao lưu dự phòng. Đã bỏ phong tỏa cơ sở thông tin.'"),
...
Procedure SetTitleForButtonNext(IsNextButton)
	...
	Items.Next.Title = ?(IsNextButton, NStr("en='Next >';ru='Далее  >';vi='Tiếp theo >'"), NStr("en='Finish';ru='Готово';vi='Kết thúc'"));
...
Function GetPath(DialogMode)
	...
	Else
		FileOpeningDialog.Title= NStr("en='Select file';ru='Выберите файл';vi='Hãy chọn tệp'");
	EndIf;	
...
Procedure StartBackup()
	...
	EventLogMonitorClient.AddMessageForEventLogMonitor(InfobaseBackupClient.EventLogMonitorEvent(),
		"Information",  NStr("en='Infobase backup is in progress:';ru='Выполняется резервное копирование информационной базы:';vi='Đang sao lưu dự phòng cơ sở thông tin:'") + " " + ScriptMainFileName);
