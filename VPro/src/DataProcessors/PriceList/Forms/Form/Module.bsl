...
Procedure UpdateFormTitleAtServer()
	...
	ThisForm.Title	= NStr("en='Company Price-list';ru='Прайс-лист Компании';vi='Bảng giá của doanh nghiệp'") + 
		?(ValueIsFilled(ToDate), NStr("en=' on ';ru=' на ';vi=' ngày '") + Format(ToDate, "DLF=DD"), ".");
...
Procedure OnOpen(Cancel)
	...
	StatePresentation.Text = NStr("en='Click the Update command to generate a price list.';ru='Нажмите команду Обновить для формирования прайс-листа.';vi='Hãy nhấn nút Cập nhật để tạo bảng giá.'");
...
Function GetDecorationTitleContent(TabularSectionName) 
	...
	If Object[TabularSectionName].Count() < 1 Then
		DecorationTitle = NStr("en='Multiple filter is not filled';ru='Множественный фильтр не заполнен';vi='Chưa điền nhiều bộ lọc'");
...
Procedure Add(Command)
	...
	ElsIf DetailFromArea.Property("Dynamic")
		...
		CommonUseClientServer.MessageToUser(
			NStr("en='Impossible to add the price. Perhaps, dynamic price type is selected.';ru='Невозможно добавить цену. Возможно выбран динамический тип цен .';vi='Không thể thêm đơn giá. Có thể chọn kiểu giá động.'"));
		Return;
...
Procedure Copy(Command)
	...
	If Not TypeOf(DetailFromArea) = Type("Structure") 
		...
		CommonUseClientServer.MessageToUser(
			NStr("en='Impossible to copy the price. Perhaps, dynamic price type or the blank cell has been selected.';ru='Невозможно скопировать цену. Возможно выбран динамический тип цен или пустая ячейка.';vi='Không thể sao chép đơn giá. Có thể chọn kiểu giá động hoặc ô trống.'"));
		Return;
	...
	ElsIf DetailFromArea.Property("DetailsMatch") Then
		...
		If AvailablePriceKindsList.Count() < 1 Then
			...
			CommonUseClientServer.MessageToUser(
				NStr("en='No prices available for copying exist for the current products and services item in the current price list.';ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для копирования.';vi='Trong bảng giá hiện tại đối với mặt hàng này không có đơn giá khả dụng để sao chép.'"));
	...
	If Details = Undefined OR Not TypeOf(Details) = Type("Structure") //no details
		...
		CommonUseClientServer.MessageToUser(
			NStr("en='Dynamic price or empty cell is specified. Copying is not possible.';ru='Указана динамическая цена либо пустая ячейка. Копирование не возможно.';vi='Đã chỉ ra đơn giá động hoặc ô trống. Không thể sao chép.'"));
...
Procedure Change(Command)
	...
	If Not TypeOf(DetailFromArea) = Type("Structure")
		...
		CommonUseClientServer.MessageToUser(
			NStr("en='Impossible to change the price. Perhaps, dynamic price type or the blank cell has been selected.';ru='Невозможно изменить цену. Возможно выбран динамический тип цен или пустая ячейка.';vi='Không thể thay đổi giá. Có thể chọn kiểu giá động hoặc ô trống.'"));
		Return;
	...
	ElsIf DetailFromArea.Property("DetailsMatch") Then
		...
		If AvailablePriceKindsList.Count() < 1 Then
			...
			CommonUseClientServer.MessageToUser(
				NStr("en='No prices available for editing exist for the current products and services item in the current price list.';ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для изменения.';vi='Trong bảng giá hiện tại đối với mặt hàng này không có đơn giá khả dụng để thay đổi.'"));
...
Procedure History(Command)
	...
	If Not TypeOf(DetailFromArea) = Type("Structure")
		...
		CommonUseClientServer.MessageToUser(
			NStr("en='Cannot open price generation history.';ru='Невозможно открыть историю формирования цен.';vi='Không thể mở lịch sử thiết lập giá.'"));
		Return;
	...
	ElsIf DetailFromArea.Property("DetailsMatch") Then
		...
		If AvailablePriceKindsList.Count() < 1 Then
			CommonUseClientServer.MessageToUser(
				NStr("en='Cannot show price history for the current inventory.';ru='Для текущего запаса невозможно отобразить историю цены.';vi='Đối với vật tư hiện tại không thể hiển thị lịch sử giá.'"));
			Return;
		ElsIf AvailablePriceKindsList.Count() > 0 Then
			SelectedPriceKind = AvailablePriceKindsList[0].Value;
			Details 	= DetailFromArea.DetailsMatch.Get(SelectedPriceKind);
...
Procedure PrepareSpreadsheetDocumentInLongActions()
	...
	AssignmentResult = LongActions.ExecuteInBackground(
		...
		NStr("en='Prepare price list data';ru='Подготовка данных прайс-листа';vi='Soạn dữ liệu bảng giá'")
	);
	...
	Else
		...
		SmallBusinessServer.StateDocumentsTableLongOperation(
			...
			NStr("en='Generating the report...';ru='Отчет формируется...';vi='Đang lập báo cáo...'")
			);
...
Procedure CheckExecution()
	...
	ElsIf InterruptIfNotCompleted Then
		...
		StatePresentation.AdditionalShowMode = AdditionalShowMode.DontUse;
		StatePresentation.Picture = New Picture;
		StatePresentation.Text = NStr("en='Data is not relevant';ru='Данные не актуальны';vi='Dữ liệu không thiết thực'");
