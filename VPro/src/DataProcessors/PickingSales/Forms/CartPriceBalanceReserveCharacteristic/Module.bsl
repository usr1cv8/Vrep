...
Procedure SetSearchStringOnServerInputHint()
	...
	InputHint = ?(FulltextSearchSetPartially, NStr("en='Update the full-text search index...';ru='Необходимо обновить индекс полнотекстового поиска...';vi='Cần cập nhật chỉ mục tìm kiếm toàn văn...'"), NStr("en='(ALT+F3) Enter search text ...';ru='(ALT+F3) Введите текст поиска...';vi='(ALT+F3) Hãy nhập nội dung tìm kiếm...'"));
...
Procedure FilterByProductsAndServicesAnalogs(ProductsAndServices, MessageText)
	...
	If ListProductsAndServicesAnalogs.Count() = 0 Then
		...
		MessageText = NStr("en='There are no analogs for these products and services in the infobase.';ru='Для номенклатуры не заведены аналоги.';vi='Đối với mặt hàng không có mặt hàng tương tự'");
...
Procedure GetProductsAndServicesAnalogs(ProductsAndServices, ListProductsAndServicesAnalogs)
	...
	Query = New Query("SELECT * FROM InformationRegister.ProductsAndServicesAnalogs AS Analogs WHERE ProductsAndServices = &ProductsAndServices ORDER BY Priority");
...
Procedure OutputBoardUsePreviousPick()
	...
	If SelectionSettingsCache.OutputBoardUsePreviousPick Then
		...
		FormTitle = NStr("en='Advice';ru='Совет';vi='Lời khuyên'");
		MessageText = NStr("en='You can disable/enable use of a new selection form in the user settings.';ru='Выключить/включить использование новой формы подбора можно в настройках пользователя.';vi='Có thể tắt/bật việc sử dụng biểu mẫu chọn mới trong tùy chỉnh người sử dụng.'");
...
Procedure OutputAdviceGoBackToProductsAndServices()
	...
	If SelectionSettingsCache.OutputAdviceGoBackToProductsAndServices Then
		...
		FormTitle = NStr("en='Advice';ru='Совет';vi='Lời khuyên'");
		MessageText = NStr("en='You can return to the products and services list using context menu or the BackSpace button.';ru='Вернуться к списку номенклатуры можно при помощи контекстного меню или клавиши BackSpace.';vi='Có thể quay lại dnah sách mặt hàng thông qua menu ngữ cảnh hoặc nhấn nút BackSpace.'");
...
&AtClient
Procedure CartPriceBalanceReserveCharacteristicBatchStartChoice(Item, ChoiceData, StandardProcessing)
	
	NewParameter = New ChoiceParameter("filter.ExportDocument",True);
	NewArray = New Array();
	NewArray.Add(NewParameter);
	NewParameters = New FixedArray(NewArray);
	Items.CartPriceBalanceReserveCharacteristicBatch.ChoiceParameters = NewParameters;
