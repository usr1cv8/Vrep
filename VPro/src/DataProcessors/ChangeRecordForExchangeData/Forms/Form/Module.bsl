...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		...
		Else
			Raise StrReplace(
				NStr("en='Incorrect destination object parameters for running command ""%1""';ru='Некорректные параметры объектов назначения открытия команды ""%1""';vi='Sai tham số đối tượng xác định mở lệnh ""%1""'"),
	...
	If Not MonitorSettings() AND OpenNodeParameter Then
		...
		MessageText = StrReplace(
			NStr("en='Editing object registration is not available for ""%1"".';ru='Для ""%1"" редактирование регистрации объектов недоступно.';vi='Đối với ""%1"" không thể soạn việc ghi nhận đối tượng.'"),
...
Procedure ChoiceProcessing(ValueSelected, ChoiceSource)
	...
	Then
		Error = NStr("en='Unexpected result when querying from console';ru='Неожиданный результат выбора из консоли запросов';vi='Kết quả bất ngờ chọn từ bảng điều khiển truy vấn'");
	Else
...
"результат запроса на узле ""%1""?';vi='Đã ghi nhận...'"); 
	Else
		Text = NStr("en='Cancel query result"
"registration on the node ""%1""?';ru='Отменить"
"регистрацию результата запроса на узле ""%1""?';vi='Hủy bỏ"
"ghi nhận kết quả truy vấn tại nút ""%1""?'");
	EndIf;
	Text = StrReplace(Text, "%1", String(ExchangeNodeRef));
					 
	QuestionTitle = NStr("en='Confirmation';ru='Подтверждение';vi='Xác nhận'");
...
Procedure ShowExportResult(Command)
	...
	If Serialization.Count() > 0 Then
		...
		TitleText = NStr("en='Export result (in DIB mode)';ru='Результат стандартной выгрузки (РИБ)';vi='Kết quả kết xuất chuẩn (CSTTPT)'");
		Text.Show(TitleText);
	EndIf;
...
"subordinate node on the node ""%1""?';ru='Зарегистрировать данные для восстановления подчиненного узла РИБ на узле ""%1""?';vi='Ghi nhận dữ liệu để khôi phục nút trực thuộc CSTTPT tại nút ""%1""?'"),
		"%1", ExchangeNodeRef
	);
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("en='Not exported';ru='Не выгружалось';vi='Chưa kết xuất'"));
	...
	Item.Appearance.SetParameterValue("Text", NStr("en='No changes';ru='Нет изменений';vi='Không có thay đổi'"));
...
Procedure EditMessageNoOfConstant()
	...
	ToolTip = NStr("en='Number of the last sent message';ru='Номер отправленного';vi='Số gửi đi'"); 
...
Procedure SetTitleNumbersMessages()
	...
	Text = NStr("en='No. of sent %1, No. of received %2';ru='№ отправленного %1, № принятого %2';vi='Số gửi %1, số nhận %2'");
...
Procedure MessageAboutResultsOfRegistration(Command, Results)
	...
	If TypeOf(Command) = Type("Boolean") Then
		If Command Then
			TitleWarnings = NStr("en='Change registration:';ru='Регистрация изменений:';vi='Ghi nhận thay đổi:'");
			WarningText = NStr("en='%1 changes are registered"
"from %2 on the node ""%0""';ru='Зарегистрировано"
"%1 изменений из %2 на узле ""%0""';vi='Đã ghi nhận"
"%1 thay đổi từ %2 tại nút ""%0""'");
		Else
			TitleWarnings = NStr("en='Cancel registration:';ru='Отмена регистрации:';vi='Hủy bỏ ghi nhận:'");
			WarningText = NStr("en='Registration of %1 changes is canceled on the ""%0"" node.';ru='Отменена регистрация %1 изменений на узле ""%0"".';vi='Đã hủy bỏ đăng ký %1 thay đổi tại nút ""%0""'");
		EndIf;
	Else
		TitleWarnings = NStr("en='Change message number:';ru='Изменение номера сообщения:';vi='Thay đổi số thông báo:'");
		WarningText = NStr("en='Message number is changed"
"to %3 in the %1 object(s)';ru='Номер сообщения"
"изменен на %3 у %1 объекта(ов)';vi='Số thông báo"
"thay đổi sang %3 trong %1 đối tượng'");
	EndIf;
	
	WarningText = StrReplace(WarningText, "%0", ExchangeNodeRef);
	WarningText = StrReplace(WarningText, "%1", Format(Results.Successfully, "NZ="));
	WarningText = StrReplace(WarningText, "%2", Format(Results.Totals, "NZ="));
	WarningText = StrReplace(WarningText, "%3", Command);
	
	Warning = Results.Totals <> Results.Successfully;
	If Warning Then
...
"регистрацию ""%2"" на узле ""%1""?';vi='Hủy bỏ"
"đăng ký ""%2"" tại nút ""%1""?'"); 
	Else
		Text = NStr("en='Cancel registration of"
"the selected constants on the node ""%1""?';ru='Отменить регистрацию"
"выбранных констант на узле ""%1""?';vi='Hủy bỏ đăng ký"
"hằng đã chọn tại nút ""%1""?'"); 
	EndIf;
...
Procedure AddRegistrationInRefsList(ThisSelection = False)
	CurFormName = RefsListTableName + ".ChoiceForm";
...
"выбранных объектов на узле ""%1""?';vi='Hủy bỏ đăng ký"
"đối tượng đã chọn tại nút ""%1""?'"); 
	EndIf;
	Text = StrReplace(Text, "%1", ExchangeNodeRef);
...
Procedure AddSelectedObjectsRegistration(WithoutAccountingAutoRecord = True)
	...
	Text = NStr("en='Register %1 for export on the ""%2"" node? Changing registration of a large number of objects can take a long time.';ru='Зарегистрировать %1 для выгрузки на узле ""%2""? Изменение регистрации большого количества объектов может занять продолжительное время!';vi='Đăng ký %1 để kết xuất tại nút ""%2""? Việc thay đổi đăng ký số lượng lớn đối tượng có thể mất thời gian!'");
...
Procedure DeleteSelectedObjectsRegistration(WithoutAccountingAutoRecord = True)
	...
	Text = NStr("en='Cancel %1 registration for export on the ""%2"" node? Changing registration of a large number of objects can take a long time.';ru='Отменить регистрацию %1 для выгрузки на узле ""%2""? Изменение регистрации большого количества объектов может занять продолжительное время!';vi='Hủy bỏ đăng ký %1 để kết xuất tại nút ""%2""? Việc thay đổi ghi nhận số lượng lớn đối tượng có thể mất nhiều thời gian!'");
...
Procedure OperationWithResultsOfQuery(CommandOperations)
	...
	If CurFormName <> Undefined Then
		...
		If CommandOperations Then
			Text = NStr("en='Registration of query result changes';ru='Регистрация изменений результата запроса';vi='Ghi nhận thay đổi kết quả truy vấn'");
		Else
			Text = NStr("en='Cancel registration of request result changes';ru='Отмена регистрации изменений результата запроса';vi='Hủy bỏ ghi nhận thay đổi kết quả truy vấn'");
		EndIf;
...
"Настроить сейчас?';vi='Trong các tùy chỉnh, chưa chỉ ra bộ xử lý để thực hiện truy vấn."
"Tùy chỉnh ngay?'");
	
	QuestionTitle = NStr("en='Settings';ru='Настройки';vi='Tùy chỉnh'");
...
Procedure ReadMetadataTree()
	...
	StructureAutoRecordMetadata = Data.AutoRecordStructure;
	MetadataPresentationsStructure   = Data.PresentationsStructure;
	StructureNameMetadata            = Data.NamesStructure;
EndProcedure
...
Procedure FillCountRegistrationsInTree()
	...
	Zeros   = New Structure("CountChanges, CountExported, CountNotExported", 0,0,0);
	
	For Each Root IN MetadataTree.GetItems() Do
		AmountRoot = New Structure("CountChanges, CountExported, CountNotExported", 0,0,0);
		
		For Each CurGroup IN Root.GetItems() Do
			AmountGroup = New Structure("CountChanges, CountExported, CountNotExported", 0,0,0);
			
			ListOfNodes = CurGroup.GetItems();
			If ListOfNodes.Count() = 0 AND StructureNameMetadata.Property(CurGroup.MetaFullName) Then
				// Collection of nodes without nodes, sum manually, take auto registration from the structure.
				For Each MetaName IN StructureNameMetadata[CurGroup.MetaFullName] Do
			...
			Else
				...
				For Each Node IN ListOfNodes Do
					...
					If Found.Count() > 0 Then
						String = Found[0];
						FillPropertyValues(Node, String, "CountChanges, CountExported, CountNotExported");
						AmountGroup.CountChanges     = AmountGroup.CountChanges     + String.CountChanges;
						AmountGroup.CountExported   = AmountGroup.CountExported   + String.CountExported;
						AmountGroup.CountNotExported = AmountGroup.CountNotExported + String.CountNotExported;
					Else
			...
			EndIf;
			FillPropertyValues(CurGroup, AmountGroup);
			
			AmountRoot.CountChanges     = AmountRoot.CountChanges     + CurGroup.CountChanges;
			AmountRoot.CountExported   = AmountRoot.CountExported   + CurGroup.CountExported;
			AmountRoot.CountNotExported = AmountRoot.CountNotExported + CurGroup.CountNotExported;
			
			FormatCountChanges(CurGroup);
		EndDo;
...
Function QuerySelectControlLinks(Address)
	...
	If TypeOf(Result) = Type("Array") Then 
		...
		If Result.Columns.Find("Ref") = Undefined Then
			Return NStr("en='Column ""Ref"" is missing in the last query result.';ru='В последнем результате запроса отсутствует колонка ""Ссылка""';vi='Trong truy vấn cuối cùng, thiếu cột ""Tham chiếu""'");
		EndIf;
	Else		
		Return NStr("en='An error occurred when receiving the query result data';ru='Ошибка получения данных результата запроса';vi='Lỗi nhận dữ liệu kết quả truy vấn'");
	EndIf;
...
Procedure CustomizeEditChangesServer(CurrentRow)
	...
	RefsListTableName   = Data.MetaFullName;
	Description = Data.Description;
	CurrentObject   = ThisObject();
	
	If IsBlankString(RefsListTableName) Then
		Meta = Undefined;
	Else		
		Meta = CurrentObject.MetadataByFullname(RefsListTableName);
	EndIf;
	
	If Meta = Undefined Then
		CustomizeBlankPage(Description, RefsListTableName);
	...
	ElsIf Metadata.Constants.Contains(Meta) Then
		// A single constant
		AdjustConstantList(RefsListTableName, Description);
		NewPage = Items.PageOfConstant;
	...
	Then
		// Reference type
		AdjustRefsList(RefsListTableName, Description);
		NewPage = Items.PageRefsList;
		
	Else
		// Check for the records set
		Dimensions = CurrentObject.RegisterSetDimensions(RefsListTableName);
		If Dimensions <> Undefined Then
			CustomizeRecordSet(RefsListTableName, Dimensions, Description);
...
Procedure AdjustRefsList(TableName, Description)
	...
	RefsList.QueryText = "
		...
		| WHERE ChangesTable.Node = &SelectedNode
		|";
...
"';vi='Đã đăng ký"
"đối tượng"
"%1 Đã kết xuất đối tượng %2 Chưa kết xuất đối tượng:%3'");
...
"дереве метаданных и воспользуйтесь командами ""Зарегистрировать"" или ""Отменить регистрацию""';vi='%1."
""
"%2"
"Để ghi nhận hoặc hủy b ỏ ghi nhận trao đổi"
"dữ liệu"
"tại nút ""%3"" hãy chọn kiểu đối tượng bên trái trên"
"cây metadata và hãy sử dụng lệnh ""Đăng ký"" hoặc ""Hủy bỏ ghi nhận""'");
...
Function SerializationText(Serialization)
	...
	For Each Item IN Serialization Do
		...
		ElsIf Item.TypeCheckBox = 3 Then
			// Ref
...
Function GetDescriptionSelectedMetadata(WithoutAccountingAutoRecord, MetaNameGroup = Undefined, MetaNameNode = Undefined)
	...
	If MetaNameGroup = Undefined AND MetaNameNode = Undefined Then
		...
		Text = NStr("en='all objects %1 by the selected kind hierarchy';ru='все объекты %1 по выбранной иерархии вида';vi='Tất cả đối tượng %1 theo phân nhánh kiểu đã chọn'");
	...
	Else
		...
		Text = NStr("en='all objects of type ""%3"" %1';ru='все объекты типа ""%3"" %1';vi='Tất cả đối tượng kiểu ""%3"" %1'");
	...
	If WithoutAccountingAutoRecord Then
		FlagText = "";
	Else
		FlagText = NStr("en='with automatic registration flag';ru='с признаком авторегистрации';vi='với dấu hiệu tự động ghi nhận'");
	EndIf;
...
Function GetSelectedMetadataNames(WithoutAccountingAutoRecord)
	...
	For Each Root IN MetadataTree.GetItems() Do
		...
		GroupCount     = 0;
		NodesNumber     = 0;
		For Each RootGroup IN Root.GetItems() Do
			
			If RootGroup.Check = 0 Then
				Continue;
			ElsIf RootGroup.Check = 1 Then
				//	The whole group, look from where to select values.
				GroupCount = GroupCount + 1;
				GroupDetails = GetDescriptionSelectedMetadata(WithoutAccountingAutoRecord, RootGroup.Description);
				
				If RootGroup.GetItems().Count() = 0 Then
					// Try from the metadata names structure, consider everything marked.
					PresentationArray = MetadataPresentationsStructure[RootGroup.MetaFullName];
					ArrayAuto          = StructureAutoRecordMetadata[RootGroup.MetaFullName];
					NameArray          = StructureNameMetadata[RootGroup.MetaFullName];
					For IndexOf = 0 To NameArray.UBound() Do
						If WithoutAccountingAutoRecord Or ArrayAuto[IndexOf] = 2 Then
							...
							NodeDescription = GetDescriptionSelectedMetadata(WithoutAccountingAutoRecord, RootGroup.MetaFullName, NameArray[IndexOf]);
						EndIf;
					EndDo;
			...
			For Each Node IN RootGroup.GetItems() Do
				If Node.Check = 1 Then
					...
					If WithoutAccountingAutoRecord Or Node.AutoRecord = 2 Then
						...
						NodeDescription = GetDescriptionSelectedMetadata(WithoutAccountingAutoRecord, RootGroup.MetaFullName, Node.MetaFullName);
...
Procedure HandleProhibitionChangesNode()
	...
	If OperationsAllowed Then
		...
		Title = NStr("en='Registration of modifications for the data exchange';ru='Регистрация изменений для обмена данными';vi='Ghi nhận thay đổi để trao đổi dữ liệu'");
	Else
		Items.ExchangeNodeRef.Visible = False;
		Title = StrReplace(NStr("en='Register changes for exchange with ""%1""';ru='Регистрация изменений для обмена с ""%1""';vi='Ghi nhận các thay đổi để trao đổi với ""%1""'"), "%1", String(ExchangeNodeRef));
	EndIf;
...
"запуска на версии платформы 1С:Предприятие 8.3.5 с отключенным режимом совместимости или выше';vi='Bộ xử lý được dùng để"
"khởi động phiên bản nền tảng 1C:DOANH NGHIỆP 8.3.5 có ngắt chế độ tương thích hoặc cao hơn"
"'");
...
&AtServer
Procedure AdjustFilterOnChangeOnServer ()
	
	SetFiterInDynamicList();
	UpdatePageContent();
	
EndProcedure

&AtServer
Procedure SetFiterInDynamicList()
