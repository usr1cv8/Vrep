...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CurrentObject.ThisIsFileBase() Then
		...
		If CurItem <> Undefined Then
			CurItem.Presentation = NStr("en='In the directory:';ru='В каталоге:';vi='Trong thư mục:'");
		EndIf;
	EndIf;
...
Procedure AfterWorksWithFilesExpansionCheck(Result, AdditionalParameters) Export
	...
	If Result Then
		...
		Dialog.Filter = NStr("en='External data processors (*.epf) | *.epf';ru='Внешние обработки (*.epf)|*.epf';vi='Bộ xử lý ngoài (*.epf)|*.epf'");
