...
// Parameters:
	...
	Result = New Structure("Tree, NamesStructure, PresentationsStructure, AutoRecordStructure", 
		Tree, CurParameters.NamesStructure, CurParameters.PresentationsStructure, CurParameters.AutoRecordStructure);
	
	CurStringNumber = 1;
	MetadataLevelForm(CurStringNumber, CurParameters, 1,  2,  False,   "Constants",               NStr("en='Constants';ru='Константы';vi='Hằng'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 3,  4,  True, "Catalogs",             NStr("en='Catalogs';ru='Справочники';vi='Danh mục'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 5,  6,  True, "Sequences",      NStr("en='Sequences';ru='Последовательности';vi='Chuỗi nối tiếp'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 7,  8,  True, "Documents",               NStr("en='Documents';ru='Документы';vi='Chứng từ'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 9,  10, True, "ChartsOfCharacteristicTypes", NStr("en='Charts of characteristic types';ru='Планы видов характеристик';vi='Hệ thống dạng đặc tính'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 11, 12, True, "ChartsOfAccounts",             NStr("en='Charts of accounts';ru='Планы счетов';vi='Hệ thống tài khoản'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 13, 14, True, "ChartsOfCalculationTypes",       NStr("en='Charts of calculation types';ru='Планы видов расчета';vi='Hệ thống dạng tính toán'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 15, 16, True, "InformationRegisters",        NStr("en='Information registers';ru='Регистры сведений';vi='Biểu ghi thông tin'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 17, 18, True, "AccumulationRegisters",      NStr("en='Accumulation registers';ru='Регистры накопления';vi='Biểu ghi tích lũy'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 19, 20, True, "AccountingRegisters",     NStr("en='Accounting registers';ru='Регистры бухгалтерии';vi='Biểu ghi kế toán'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 21, 22, True, "CalculationRegisters",         NStr("en='Calculation registers';ru='Регистры расчета';vi='Biểu ghi tính toán'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 23, 24, True, "BusinessProcesses",          NStr("en='Business processes';ru='Деловые процессы';vi='Quy trình nghiệp vụ'"));
	MetadataLevelForm(CurStringNumber, CurParameters, 25, 26, True, "Tasks",                  NStr("en='Tasks';ru='Задания';vi='Nhiệm vụ'"));
...
Function GetNumberOfChanges(ListOfTables, ListOfNodes) Export
	...
	Query = New Query;
	Query.SetParameter("NodesList", ListOfNodes);
...
Procedure SetMarksUp(RowData) Export
	...
	If RowParent <> Undefined Then
		...
		For Each Descendant IN RowParent.GetItems() Do
			...
			NotAllFalse = NotAllFalse Or Boolean(Descendant.Check);
		EndDo;
		If AllTrue Then
			RowParent.Check = 1;
...
Function RegisterSetDimensions(TableName, AllDimensions = False) Export
	...
	Then
		...
		String.Title   = NStr("en='Recorder';ru='Регистратор';vi='Nguồn ghi'");
	EndIf;
	...
	If ThisIsInformationRegister AND Meta.MainFilterOnPeriod Then
		...
		String.Title   = NStr("en='Accounting period';ru='отчетный период';vi='Kỳ báo cáo'");
	EndIf;
	...
	If Metadata.CalculationRegisters.Contains(Meta.Parent()) Then
		...
		String.ValueType = Documents.AllRefsType();
		String.Title   = NStr("en='Recalculation object';ru='Объект перерасчета';vi='Đối tượng hạch toán lại'");
	EndIf;
...
Function REFPRESENTATION(ObjectViews) Export
	...
	If IsBlankString(Result) Then
		Result = NStr("en='Not defined';ru='Не определена';vi='Không xác định'");
	EndIf;
...
Function CheckSettingsCorrectness(SettingKey = "") Export
		...
		Try
			...
			Else
				If ThisIsFileBase() Then
					Text = NStr("en='The ""%1"" file is not available';ru='Файл ""%1"" не доступен';vi='Chưa sử dụng tệp ""%1""'");
				Else
					Text = NStr("en='File ""%1"" is not available on the server';ru='Файл ""%1"" не доступен на сервере';vi='Chưa sử dụng tệp ""%1"" trên Server'");
				EndIf;
	...
	Else
		...
		If Metadata.DataProcessors.Find(SettingAddressExternalDataQueryProcessors) = Undefined Then
			Text = NStr("en='Data processor ""%1"" is not found in the configuration';ru='Обработка ""%1"" не найдена в составе конфигурации';vi='Chưa tìm thấy bộ xử lý ""%1"" trong thành phần cấu hình'");
			Result.SettingAddressExternalDataQueryProcessors = StrReplace(Text, "%1", SettingAddressExternalDataQueryProcessors);
...
Function ExternalDataProcessorInfo() Export
	...
	Command.Presentation = NStr("en='Editing of object changes registration';ru='Редактирование регистрации изменений объекта';vi='Soạn ghi nhận thay đổi đối tượng'");
...
Procedure AddResults(Receiver, Source)
	...
	Receiver.Totals   = Receiver.Totals   + Source.Totals;
EndProcedure	
...
Function SSL_RefreshAndRegisterHostIOM(Val Node) Export
	...
	Result.Totals = Result.Successfully;
	Return Result;
EndFunction
