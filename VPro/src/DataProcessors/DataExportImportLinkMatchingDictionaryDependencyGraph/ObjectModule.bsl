...
Procedure AddVertex(Val MetadataObjectName, Val IfNotExist = True) Export
	...
	If AlreadyExists Then
		...
		If IfNotExist Then
			Return;
		Else
			Raise NStr("en='Duplication attempt';ru='Попытка дублирования!';vi='Thử sao chép!'");
		EndIf;
...
Function MetadataObject(Val DescriptionFull)
	...
	If MetadataObject = Undefined Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Metadata object %1 existing in the data file is missing in the current configuration.';ru='В текущей конфигурации отсутствует объект метаданных %1, присутствующих в файле данных!';vi='Trong cấu hình hiện tại không có đối tượng metadata %1 mà đang có trong tệp dữ liệu!'"),
			DescriptionFull
		);
...
Function Vertex(Val MetadataObject, Val ExceptionIfNotExist = True)
	...
	ElsIf Rows.Count() = 0 Then
		...
		If ExceptionIfNotExist Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='There is no vertex for metadata object %1 in the column.';ru='В графе отсутствует вершина для объекта метаданных %1!';vi='Trong cột thiếu đỉnh cho đối t ượng metadata %1!'"),
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Unique column violation for the metadata object %1.';ru='Нарушение уникальности граф для объекта метаданных %1!';vi='Vi phạm tính đơn trị của cột đối với đối tượng metadata %1!'"),
			MetadataObject.FullName());
...
Procedure SearchInDepth(Vertex, SortResult)
	...
	If Vertex.Color = Gray Then
		...
		Raise NStr("en='Recursive dependence';ru='Рекурсивная зависимость!';vi='Phụ thuộc đệ quy'");
