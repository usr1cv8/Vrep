...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(InfobaseNode) Then
		...
		Else
			DataExchangeServer.ShowMessageAboutError(NStr("en='Form parameters are not specified. Cannot open the form.';ru='Не заданы параметры формы. Форма не может быть открыта.';vi='Chưa chỉ ra tham số biểu mẫu. Biểu mẫu không thể mở được.'"), Cancel);
			Return;
		EndIf;
	...
	// We set form title.
	Title = NStr("en='Data synchronization with %1';ru='Синхронизация данных с ""%1""';vi='Đồng bộ hóa dữ liệu từ ""%1""'");
