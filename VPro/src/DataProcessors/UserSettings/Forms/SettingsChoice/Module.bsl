...
Procedure UpdatePageTitle(SettingsCount)
	...
	If SelectedSettingsPage = "ReportsSettingsPage" Then
		...
		HeaderText = ?(ReportsSettingsAmount = 0, NStr("en='Report settings';ru='Настройки отчета';vi='Tùy chỉnh báo cáo'"), NStr("en='Report settings (%1)';ru='Настройки отчетов (%1)';vi='Tùy chỉnh báo cáo (%1)'"));
	...
	ElsIf SelectedSettingsPage = "AppearancePage" Then
		...
		HeaderText = ?(AppearanceSettingsAmount = 0, NStr("en='Appearance';ru='Оформление';vi='Trang trí'"), NStr("en='Appearance (%1)';ru='Внешний вид (%1)';vi='Hình dạng ngoài (%1)'"));
	...
	ElsIf SelectedSettingsPage = "OtherSettingsPage" Then
		...
		HeaderText = ?(OtherSettingsAmount = 0, NStr("en='Other settings';ru='Прочие настройки';vi='Tùy chỉnh khác'"), NStr("en='Other settings (%1)';ru='Прочие настройки (%1)';vi='Tùy chỉnh khác (%1)'"));
...
Procedure ImportMarksValues(ValueTree, MarkedSettings, KindSettings)
	...
	If MarkedCount > 0 Then
		...
		If KindSettings = "ReportsSettings" Then
			...
			Items.ReportsSettingsPage.Title = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Report settings (%1)';ru='Настройки отчетов (%1)';vi='Tùy chỉnh báo cáo (%1)'"), MarkedCount);
		ElsIf KindSettings = "ExternalView" Then
			AppearanceSettingsAmount = MarkedCount;
			Items.AppearancePage.Title = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Appearance (%1)';ru='Внешний вид (%1)';vi='Hình dạng ngoài (%1)'"), MarkedCount);
		ElsIf KindSettings = "OtherSettings" Then
			OtherSettingsAmount = MarkedCount;
			Items.OtherSettingsPage.Title = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Other settings (%1)';ru='Прочие настройки (%1)';vi='Tùy chỉnh khác (%1)'"), MarkedCount);
		EndIf;
