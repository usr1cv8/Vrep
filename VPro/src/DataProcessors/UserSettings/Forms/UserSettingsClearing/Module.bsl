...
Procedure SwitchClearedSettingsOnChange(Item)
	...
	If SwitchWhomSettingsCleared = "SelectedUsers"
		...
		ShowMessageBox(,NStr("en='Cleanup of separate settings is available only when one user is selected.';ru='Очистка отдельных настроек доступна только при выборе одного пользователя.';vi='Xóa tùy chỉnh riêng chỉ sử dụng được khi chọn một người sử dụng.'"));
...
"пользователя или пользователей, которым необходимо очистить настройки.';vi='Hãy chọn"
"một người sử dụng hoặc nhiều người sử dụng mà cần xóa tùy chỉnh cho họ.'"), , "Source");
		Return False;
	EndIf;
	...
	If SwitchWhomSettingsCleared = "SelectedUsers" Then
		...
		If UserCount = 1 Then
			ExplanationWhoHasClearedSettings = NStr("en='user ""%1""';ru='пользователя ""%1""';vi='người sử dụng ""%1""'");
		...
		Else
			ExplanationWhoHasClearedSettings = NStr("en='%1 users';ru='%1 пользователям';vi='%1 cho người sử dụng'");
			ExplanationWhoHasClearedSettings = StringFunctionsClientServer.SubstituteParametersInString(
				ExplanationWhoHasClearedSettings, UserCount);
		EndIf;
		
	Else
		ExplanationWhoHasClearedSettings = NStr("en='All users';ru='Все пользователи';vi='Tất cả người sử dụng'");
	EndIf;
	
	If SwitchClearedSettings = "SomeSettings"
		AND SettingsCount = 0 Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Select settings to be cleared.';ru='Выберите настройки, которые необходимо очистить.';vi='Hãy chọn tùy chỉnh cần xóa'"), , "SwitchClearedSettings");
		Return False;
	EndIf;
	...
	If SwitchClearedSettings = "SomeSettings" Then
		...
		If SettingsCount = 1 Then
			...
			ExplanationText = NStr("en='""%1"" is cleared for %2';ru='""%1"" очищена у %2';vi='""%1"" đã xóa bởi %2'");
		...
		Else
			...
			ExplanationText = NStr("en='%1 is cleaned up for %2';ru='Очищено %1 у %2';vi='Xóa %1 của %2'");
			ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
	...
	ElsIf SwitchClearedSettings = "ClearAll" Then
		...
		ExplanationText = NStr("en='All settings %1 are cleaned up';ru='Очищены все настройки %1';vi='Xóa tất cả tùy chỉnh %1'");
		ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
