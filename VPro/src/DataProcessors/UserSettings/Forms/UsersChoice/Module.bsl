...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If ClearingSettings Then
		Title = NStr("en='Select users to clear settings';ru='Выбор пользователей для очистки настроек';vi='Chọn người sử dụng để xóa bỏ tùy chỉnh'");
		Items.Label.Title = NStr("en='Select users for whom it is required to clear settings';ru='Выберите пользователей, которым необходимо очистить настройки';vi='Hãy chọn người sử dụng cần xóa tùy chỉnh'");
	EndIf;
...
Procedure Select(Command)
	...
	If UsersTarget.Count() = 0 Then
		ShowMessageBox(,NStr("en='Mark one or multiple users.';ru='Необходимо отметить одного или несколько пользователей.';vi='Cần đánh dấu một hoặc nhiều người sử dụng.'"));
		Return;
	EndIf;
...
Procedure UpdateGroupTitle(Form, UsersGroup, UsersListRow, ValueMark)
	...
	If MarkedUser <> Undefined AND ValueMark <> UsersListRow.Check Then
		...
		UsersGroup.GroupDescriptionAndIsMarkedUsers = 
			StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1 (%2)';ru='%1 (%2)';vi='%1 (%2)'"),
