...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.User <> Undefined Then
		...
		Title = NStr("en='User settings';ru='Пользовательские настройки';vi='Tùy chỉnh người sử dụng'");
...
Procedure UserRefStartChoice(Item, ChoiceData, StandardProcessing)
	...
	If UseExternalUsers Then
		...
		TypeChoiceUsers.Add("ExternalUsers", NStr("en='External users';ru='Внешние пользователи';vi='Người sử dụng ngoài'"));
		TypeChoiceUsers.Add("Users", NStr("en='Users';ru='Пользователи';vi='Người sử dụng'"));
...
Procedure SettingsBeforeDelete(Item, Cancel)
	...
	Cancel = True;
	QuestionText = NStr("en='Clear the selected settings?';ru='Очистить выделенные настройки?';vi='Xóa các tùy chỉnh đã chọn?'");
...
Procedure CopyAllSettings(Command)
	...
	CopiedSettings.Add("ReportsSettings", NStr("en='Report settings';ru='Настройки отчета';vi='Tùy chỉnh báo cáo'"));
	CopiedSettings.Add("ExternalViewSettings", NStr("en='Appearance settings';ru='Настройки внешнего вида';vi='Tùy chỉnh hình dạng ngoài'"));
	CopiedSettings.Add("FormsData", NStr("en='Form data';ru='Данные форм';vi='Dữ liệu của biểu mẫu'"));
	CopiedSettings.Add("PersonalSettings", NStr("en='Personal settings';ru='Персональные настройки';vi='Tùy chỉnh cá nhân'"));
	CopiedSettings.Add("Favorites", NStr("en='Favorites';ru='Избранное';vi='Mục ưa thích'"));
	CopiedSettings.Add("PrintSettings", NStr("en='Print settings';ru='Настройки печати';vi='Tùy chỉnh in'"));
	CopiedSettings.Add(
		"OtherUserSettings", NStr("en='Settings of additional reports and data processors';ru='Настройки дополнительных отчетов и обработок';vi='Tùy chỉnh báo cáo và bộ xử lý bổ sung'"));
...
Procedure Clear(Command)
	...
	If SelectedRows.Count() = 0 Then
		...
		ShowMessageBox(,NStr("en='Select settings to delete.';ru='Необходимо выбрать настройки, которые требуется удалить.';vi='Cần chọn tùy chỉnh cần xóa bỏ.'"));
		Return;
...
Procedure ClearSelectedUserSettings(Command)
	...
	QuestionText = NStr("en='Clear the selected settings? It will open the user selection window whose settings should be cleared.';ru='Очистить выделенные настройки? Откроется окно выбора пользователей, которым необходимо очистить настройки.';vi='Xóa tùy chỉnh đã chọn? Mở cửa sổ chọn người sử dụng, mà cần xóa tùy chỉnh.'");
...
Procedure ClearAllSettings(Command)
	...
	QuestionText = NStr("en='Clear all settings of user ""%1""?';ru='Очистить все настройки у пользователя ""%1""?';vi='Xóa tất cả tùy chỉnh của người sử dụng ""%1""?'");
	QuestionText = StringFunctionsClientServer.SubstituteParametersInString(QuestionText, UserRef);
	QuestionButtons = New ValueList;
	QuestionButtons.Add("Clear", NStr("en='Clear';ru='Очистить';vi='Xóa'"));
	QuestionButtons.Add("Cancel", NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'"));
...
Procedure ClearReportsAndAppearanceSettings(Command)
	...
	QuestionText = NStr("en='Clear all settings of reports and appearance of user ""%1""?';ru='Очистить все настройки отчетов и внешнего вида у пользователя ""%1""?';vi='Xóa tất cả tùy chỉnh báo cáo và hình dạng ngoài của người sử dụng ""%1""?'");
	QuestionText = StringFunctionsClientServer.SubstituteParametersInString(QuestionText, UserRef);
	QuestionButtons = New ValueList;
	QuestionButtons.Add("Clear", NStr("en='Clear';ru='Очистить';vi='Xóa'"));
	QuestionButtons.Add("Cancel", NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'"));
...
"Продолжить?';vi='Bây giờ sẽ xóa tất cả tùy chỉnh của tất cả người sử dụng.'");
	QuestionText = StringFunctionsClientServer.SubstituteParametersInString(QuestionText, UserRef);
	QuestionButtons = New ValueList;
	QuestionButtons.Add("ClearAll", NStr("en='Clear all';ru='Очистить все';vi='Xóa tất cả'"));
	QuestionButtons.Add("Cancel", NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'"));
...
Procedure CalculateSettingsCount()
	...
	If SettingsCount <> 0 Then
		Items.ReportsSettingsPage.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Report settings (%1)';ru='Настройки отчетов (%1)';vi='Tùy chỉnh báo cáo (%1)'"), SettingsCount);
	Else
		Items.ReportsSettingsPage.Title = NStr("en='Report settings';ru='Настройки отчета';vi='Tùy chỉnh báo cáo'");
	EndIf;
	...
	If SettingsCount <> 0 Then
		Items.AppearancePage.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Appearance (%1)';ru='Внешний вид (%1)';vi='Hình dạng ngoài (%1)'"), SettingsCount);
	Else
		Items.AppearancePage.Title = NStr("en='Appearance';ru='Оформление';vi='Trang trí'");
	EndIf;
	...
	If SettingsCount <> 0 Then
		Items.OtherSettingsPage.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Other settings (%1)';ru='Прочие настройки (%1)';vi='Tùy chỉnh khác (%1)'"), SettingsCount);
	Else
		Items.OtherSettingsPage.Title = NStr("en='Other settings';ru='Прочие настройки';vi='Tùy chỉnh khác'");
	EndIf;
		...
		If Parameter.CopyAll Then
			...
			If SettingsArray.Count() = 7 Then
				ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='All settings copied %1';ru='Скопированы все настройки %1';vi='Đã sao chép tất cả tùy chỉnh %1'"), ExplanationToWhomSettingsAreCopied);
			Else
				ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='%1 are copied %2';ru='%1 скопированы %2';vi='%1 đã sao chép %2'"), SettingsNames, ExplanationToWhomSettingsAreCopied);
			EndIf;
				
			Status(NStr("en='Copying settings';ru='Выполняется копирование настроек';vi='Đang thực hiện sao chép tùy chỉnh'"));
			
			Report = Undefined;
			CopyAllSettingsAtServer(InfobaseUser, UsersTarget, SettingsArray, Report);
			
			If Report <> Undefined Then
				QuestionText = NStr("en='Not all report variants and settings were copied.';ru='Не все варианты отчетов и настройки были скопированы.';vi='Không phải tất cả phương án báo cáo và tùy chỉnh đều được sao chép.'");
				QuestionButtons = New ValueList;
			...
			ShowUserNotification(
				NStr("en='Copy settings';ru='Копирование настроек';vi='Sao chép tùy chỉnh'"), , ExplanationText, PictureLib.Information32);
...
Procedure ClearAllUserSettingsEnd(Response, AdditionalParameters) Export
	...
	ShowUserNotification(NStr("en='Clear settings';ru='Очистить настройки';vi='Xóa tùy chỉnh'"), ,
		NStr("en='All settings of all users are cleaned up';ru='Очищены все настройки всех пользователя';vi='Đã xóa tất cả tùy chỉnh của tất cả người sử dụng'"), PictureLib.Information32);
...
Procedure ClearAllSettingsEnd(Response, AdditionalParameters) Export
	...
	ExplanationText = NStr("en='All settings of user ""%1"" are cleaned up';ru='Очищены все настройки пользователя ""%1""';vi='Đã xóa tất cả tùy chỉnh của người sử dụng ""%1""'");
	ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(ExplanationText, UserRef);
	ShowUserNotification(
		NStr("en='Clear settings';ru='Очистить настройки';vi='Xóa tùy chỉnh'"), , ExplanationText, PictureLib.Information32);
...
Procedure ClearReportsAndAppearanceSettingsEnd(Response, AdditionalParameters) Export
	...
	ExplanationText = NStr("en='All settings of reports and appearance of user ""%1"" are cleaned up';ru='Очищены все настройки отчетов и внешнего вида у пользователя ""%1""';vi='Đã xóa tất cả tùy chỉnh báo cáo và hình dạng ngoài của người sử dụng ""%1""'");
	ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(ExplanationText, UserRef);
	ShowUserNotification(
		NStr("en='Clear settings';ru='Очистить настройки';vi='Xóa tùy chỉnh'"), , ExplanationText, PictureLib.Information32);
...
Procedure NotifyAboutDeleting(SettingsCount, SettingName = Undefined, UserCount = Undefined)
	...
	If SettingsCount = 1
		...
		ExplanationText = NStr("en='""%1"" is cleared for user ""%2""';ru='""%1"" очищена пользователю ""%2""';vi='""%1"" đã xóa bởi người sử dụng ""%2""'");
		ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
	...
	If UserCount <> Undefined Then
		...
		If SettingsCount = 1 Then
			ExplanationText = NStr("en='""%1"" cleared %2';ru='""%1"" очищена %2';vi='""%1"" đã xóa %2'");
			ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
				ExplanationText, SettingName, ExplanationWhomSettingsCleared);
		Else
			ExplanationText = NStr("en='Cleared %1 %2';ru='Очищено %1 %2';vi='Đã xóa %1 %2'");
			ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
...
Procedure CopySettings()
	...
	If SettingsTree.SelectedRows.Count() = 0 Then
		ShowMessageBox(,NStr("en='Select settings to copy.';ru='Необходимо выбрать настройки, которые требуется скопировать.';vi='Cần chọn tùy chỉnh cần sao chép.'"));
		Return;
...
"другим пользователям, необходимо его пересохранить со снятой пометкой ""Только для автора"".';vi='Không thể sao chép phương án báo cáo cá nhân."
"Để người sử dụng khác có thể sử dụng phương án báo cáo cá nhân"
"thì cần lưu lại phương án đó và bỏ dấu ""Chỉ đối với tác giả"".'"));
			Return;
...
"Копирование настроек личных вариантов отчетов не предусмотрено.';vi='Không thể sao chép tùy chỉnh phương án báo cáo riêng."
"Không xem xét việc sao chép tùy chỉnh các phương án báo cáo riêng.'"));
			Return;
		EndIf;
