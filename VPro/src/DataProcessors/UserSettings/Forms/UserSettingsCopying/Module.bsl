...
Procedure NotificationProcessing(EventName, Parameter, Source)
	...
	If Upper(EventName) = Upper("CaseUser") Then
		...
		ElsIf UserCount > 1 Then
			...
			SubjectAndNumberInWords = NumberInWords(
				UserCount,
				"L=en_US",
				NStr("en='user,of user,users,,,,,,0';ru='user,of user,users,,,,,,0';vi='người dùng,người dùng,người dùng,,,,,,0'"));
	...
	ElsIf Upper(EventName) = Upper("SettingsChoice") Then
		...
		If SettingsCount = 0 Then
			HeaderText = NStr("en='Select';ru='Выбрать';vi='Chọn'");
...
"пользователя самому себе, выберите другого пользователя.';vi='Không nên sao chép tùy chỉnh"
"của người sử dụng cho chính họ, hãy chọn người sử dụng khác.'"));
				UserRef = UserRefOld;
				Return;
			EndIf;
	...
	SettingsCount = 0;
	Items.ChooseSettings.Title = NStr("en='Select';ru='Выбрать';vi='Chọn'");
...
Procedure Copy(Command)
	...
	If UserRef = Undefined Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Select a user whose settings should be copied.';ru='Выберите пользователя, чьи настройки необходимо скопировать.';vi='Hãy chọn người sử dụng mà cần sao chép tùy chỉnh của họ'"), , "UserRef");
		Return;
	EndIf;
	
	If UserCount = 0 AND SwitchToWhomCopySettings <> "AllUsers" Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Select one or multiple users whose settings should be copied.';ru='Выберите одного или несколько пользователей, которым необходимо скопировать настройки.';vi='Hãy chọn một hoặc nhiều người sử dụng cần sao chép tùy chỉnh.'"), , "Receiver");
		Return;
	EndIf;
	
	If SwitchCopiedSettings = "CopySelected" AND SettingsCount = 0 Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Select settings to be copied.';ru='Выберите настройки, которые необходимо скопировать.';vi='Hãy chọn tùy chỉnh cần sao chép.'"), , "SwitchCopiedSettings");
		Return;
	EndIf;
...
Procedure CopySettings(CommandName)
	...
	Else
		ExplanationToWhomSettingsAreCopied = NStr("en='All users';ru='Все пользователи';vi='Tất cả người sử dụng'");
	EndIf;
	...
	If SwitchCopiedSettings = "CopySelected" Then
		...
		If Report = Undefined Then
			...
			ShowUserNotification(NStr("en='Copy settings';ru='Копирование настроек';vi='Sao chép tùy chỉnh'"), , ExplanationText, PictureLib.Information32);
	...
	Else
		...
		If Not SettingsCopied Then
			...
			WarningText = NStr("en='Settings were not copied as user ""%1"" has not saved any settings.';ru='Настройки не были скопированы, так как у пользователя ""%1"" не было сохранено ни одной настройки.';vi='Tùy chỉnh không được sao chép vì người sử dụng ""%1"" đã không lưu bất kỳ tùy chỉnh nào.'");
		...
		ExplanationText = NStr("en='All settings copied %1';ru='Скопированы все настройки %1';vi='Đã sao chép tất cả tùy chỉnh %1'");
