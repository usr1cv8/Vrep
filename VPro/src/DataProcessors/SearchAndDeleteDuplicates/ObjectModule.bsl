...
Function DuplicateSearchAreaManager(Val DataSearchAreaName) Export
	...
	Raise StrReplace(
		NStr("en='Unknown metadata object type ""%1""';ru='Неизвестный тип объекта метаданных ""%1""';vi='Sai kiểu đối tượng metadata ""%1""'"), 
	...
	While Selection.Next() Do
		...
		If ReturnPortionSize > 0 AND (DuplicatesTable.Count() > ReturnPortionSize) Then
			...
			If DuplicatesTable.Count() = 0 Then
				Result.ErrorDescription = NStr("en='Too many items are found, not all duplicate groups are defined.';ru='Найдено слишком много элементов, определены не все группы дублей.';vi='Đã tìm thấy quá nhiều phần tử, đã xác định không phải tất cả các nhóm của bản sao.'");
			Else
				Result.ErrorDescription = NStr("en='Too many items are found. Specify the duplicate search criteria.';ru='Найдено слишком много элементов. Уточните критерии поиска дублей.';vi='Đã tìm thấy quá nhiều phần  tử. Hãy làm rõ tiêu chí tìm kiếm bản sao.'");
			EndIf;
			Break;
		EndIf;
...
Procedure RunCommand(ExecuteParameters, ResultAddress) Export
	...
	Else
		...
		Raise NStr("en='Search and delete duplicates: Command ""%1"" is not supported.';ru='Поиск и удаление дублей: Команда ""%1"" не поддерживается.';vi='Tìm kiếm và xóa bỏ phần tử trùng lặp: Lệnh ""%1"" chưa được hỗ trợ.'");
