...
"Задайте условия отбора и сравнения и нажмите ""Найти дубли"".';vi='Chưa thực hiện tìm kiếm phần tử trùng lặp"
"Hãy xác định điều kiện lọc và so sánh, sau đó nhấn ""Tìm kiếm phần tử trùng lặp"".'");
	StatePresentation.Picture = Items.Warning32.Picture;
	
	StatePresentation = Items.SearchExecution.StatePresentation;
	StatePresentation.Visible = True;
	StatePresentation.Text = NStr("en='Duplicate search...';ru='Поиск дублей...';vi='Tìm kiếm phần tử trùng lặp...'");
	StatePresentation.Picture = Items.LongOperation48.Picture;
	
	StatePresentation = Items.DeleteExecution.StatePresentation;
	StatePresentation.Visible = True;
	StatePresentation.Text = NStr("en='Deleting duplicates ...';ru='Удаление дублей...';vi='Xóa phần tử trùng lặp...'");
	StatePresentation.Picture = Items.LongOperation48.Picture;
...
"Измените условия отбора и сравнения, нажмите ""Найти дубли""';vi='Chưa tìm thấy bản sao theo tham số đã chỉ ra."
"Hãy thay đổi điều kiện lọc và so sánh, sau đó nhấn ""Tìm kiếm bản sao""'");
	StatePresentation.Picture = Items.Warning32.Picture;
...
Procedure UpdateCandidateUsagePlaces(Val DataRow)
	...
	If RowData.GetParent() = Undefined Then
		...
		Items.CurrentDuplicateGroupDescription.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Duplicates (%2) are found for the ""%1"" item';ru='Для элемента ""%1"" найдены дубли (%2)';vi='Đối với phần tử ""%1"" đã tìm thấy bản sao (%2)'"),
	...
	If RowData.Quantity = 0 Then
		Items.CurrentDuplicateGroupDescription.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Item ""%1"" is not used';ru='Элемент ""%1"" не используется';vi='Không sử dụng phần tử ""%1""'"), 
			RowData.Description);
		
		Items.UsagePlacesPages.CurrentPage = Items.GroupDetails;
	Else
		Items.CandidateUsagePlaces.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Usage locations ""%1"" (%2)';ru='Места использования ""%1"" (%2)';vi='Nơi sử dụng ""%1"" (%2)'"), 
...
Procedure UpdateUnprocessedDuplicatesUsagePlaces(Val DataRow)
	...
	If RowData.Quantity = 0 Then
		Items.CurrentDuplicateGroupDescription1.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='The ""%1"" item is processed';ru='Элемент ""%1"" успешно обработан';vi='Đã xử lý thành công phần tử ""%1""'"), 
			RowData.Description);
		
		Items.UsagePlacesPagesRaw.CurrentPage = Items.GroupDetailsRaw;
	Else
		Items.CandidateUsagePlaces.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Cannot replace duplicates in some locations (%1)';ru='Не удалось заменить дубли в некоторых местах (%1)';vi='Không thể thay thế bản sao trong một số nơi (%1)'"), 
			RowData.Quantity);
...
Procedure SetColorAndConditionalDesign()
	...
	DesignElement.Appearance.SetParameterValue("Text", NStr("en='Not used';ru='Не используется';vi='Không sử dụng'"));
...
Procedure ChangeMainItemHierarchically(Val RowData, Val Parent)
	...
	Parent.Description = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1 (%2)';ru='%1 (%2)';vi='%1 (%2)'"), 
...
Function FillDuplicatesSearchResults(Val Data)
	...
	CandidateUsagePlaces.Clear();
	Items.CurrentDuplicateGroupDescription.Title = NStr("en='Duplicates are not found';ru='Дублей не найдено';vi='Chưa tìm thấy bản sao'");
	...
	If TotalDuplicatesFound = 0 Then
		FoundDuplicatesStatusDescription = New FormattedString(Items.Information16.Picture, " ",
			NStr("en='Duplicates are not found according to the specified conditions';ru='Не обнаружено дублей по указанным условиям';vi='Chưa tìm thấy bản sao theo điều kiện đã chỉ ra'"));
	Else
...
"и заменены во всех местах использования на оригиналы (отмечены стрелкой).';vi='Đã tìm thấy bản sao phần tử: %2 (trong số các phần tử: %1). Tất cả phần tử đã đánh dấu sẽ bị đặt dấu xóa"
"và được thay thế trong tất cả nơi sử dụng sang bản gốc (được đánh dấu bằng mũi tên).'"),
	...
	If CompleteWithoutErrors Then
		...
		If TotallyMain = 1 Then
			...
			If LastCandidate = Undefined Then
				FoundDuplicatesStatusDescription = New FormattedString(StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='All found duplicates (%1) are successfully grouped';ru='Все найденные дубли (%1) успешно объединены';vi='Tất cả bản sao đã tìm thấy (%1) được hợp nhất thành công'"), TotallyProcessed));
			Else
...
"успешно объединены в ""%2""';vi='Tất cả bản sao đã tìm thấy (%1)"
"được hợp nhất thành công vào ""%2""'"),
...
"Оставлено элементов (%2).';vi='Tất cả bản sao đã tìm thấy (%1) được hợp nhất thành công."
"Để lại các phần tử (%2).'"),
...
Procedure UpdateSearchRulesDescription()
	RulesText = "";
	Conjunction        = " " + NStr("en='AND';ru='А ТАКЖЕ';vi='VÀ NGOÀI RA'") + " ";
	
	For Each Rule In SearchRules Do
		
		If Rule.Rule = "Equal" Then
			Comparison = NStr("en='%1 matches';ru='%1 совпадает';vi='%1 trùng'");
		ElsIf Rule.Rule = "Like" Then
			Comparison = NStr("en='%1 has similar words';ru='%1 совпадает по похожим словам';vi='%1 trùng theo từ tương tự'");
		Else
			Comparison = "";
		EndIf;
	...
	If IsBlankString(RulesText) Then
		Items.SearchRules.Title = NStr("en='The rules are not set';ru='Правила не заданы';vi='Chưa xác định quy tắc'");
	Else
...
Procedure UpdateFilterDescription()
	...
	If IsBlankString(FilterDescription) Then
		FilterDescription = NStr("en='All items';ru='Все элементы';vi='Tất cả phần tử'");
...
Procedure InitializeSelectionAndRulesLinker()
	...
	AllComparisonVariants.Add("Equal",   NStr("en='Matches';ru='Совпадает';vi='Trùng'"));
	AllComparisonVariants.Add("Like", NStr("en='Match by related words';ru='Совпадает по похожим словам';vi='Trùng theo các từ giống nhau'"));
...
Procedure InitializeAssistantScript()
	...
	ButtonsAssistant.Next.Title = NStr("en='Find duplicates >';ru='Найти дубли >';vi='Tìm kiếm phần tử trùng lặp >'");
	ButtonsAssistant.Next.ToolTip = NStr("en='Find duplicates according to the specified criteria';ru='Найти дубли по указанным критериям';vi='Tìm kiếm phần tử trùng lặp theo tiêu chí đã chỉ ra'");
	ButtonsAssistant.Cancel.Title = NStr("en='Close';ru='Закрыть';vi='Đóng'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Refuse to search and replace duplicates';ru='Отказаться от поиска и замены дублей';vi='Hủy bỏ tìm kiếm và thay thế bản sao'");
	...
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Stop duplicate search';ru='Прервать поиск дублей';vi='Ngừng tìm kiếm phần tử trùng lặp'");
	...
	ButtonsAssistant = ButtonsAssistant();
	ButtonsAssistant.Back.Visible = False;
	ButtonsAssistant.Next.Title = NStr("en='Delete duplicates >';ru='Удалить дубли >';vi='Xóa bản sao >'");
	ButtonsAssistant.Next.ToolTip = NStr("en='Delete duplicates';ru='Удаление дублей';vi='Xóa phần tử trùng lặp'");
	ButtonsAssistant.Cancel.Title = NStr("en='Close';ru='Закрыть';vi='Đóng'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Refuse to search and replace duplicates';ru='Отказаться от поиска и замены дублей';vi='Hủy bỏ tìm kiếm và thay thế bản sao'");
	...
	ButtonsAssistant.Next.Visible = False;
	ButtonsAssistant.Cancel.Title = NStr("en='Abort';ru='Прервать';vi='Ngừng'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Stop duplicate removal';ru='Прервать удаление дублей';vi='Ngừng xóa bỏ phần tử trùng lặp'");
	...
	ButtonsAssistant = ButtonsAssistant();
	ButtonsAssistant.Back.Title = NStr("en='< New search';ru='< Новый поиск';vi='< Tìm kiếm mới'");
	ButtonsAssistant.Back.ToolTip = NStr("en='Start new search with different parameters';ru='Начать новый поиск с другими параметрами';vi='Bắt đầu tìm kiếm mới với các tham số khác'");
	ButtonsAssistant.Next.Visible = False;
	...
	ButtonsAssistant = ButtonsAssistant();
	ButtonsAssistant.Back.Visible = False;
	ButtonsAssistant.Next.Title = NStr("en='Repeat deletion >';ru='Повторить удаление >';vi='Xóa lại >'");
	ButtonsAssistant.Next.ToolTip = NStr("en='Delete duplicates';ru='Удаление дублей';vi='Xóa phần tử trùng lặp'");
	ButtonsAssistant.Cancel.Title = NStr("en='Close';ru='Закрыть';vi='Đóng'");
...
Procedure StepSearchNotExecutedBeforeNextAction(Val StepParameters, Val AdditionalParameters) Export
	...
	WarningText = NStr("en='Select area to search for duplicates';ru='Необходимо выбрать область поиска дублей';vi='Cần chọn vùng tìm kiếm bản sao'");
	...
	If Not InformationAboutJob.Completed Then
		If ShowDialogBeforeClosing Then
			...
			If StepByStepAssistantSettings.ProcedureName = "BackgroundDuplicatesSearch" Then
				QuestionText = NStr("en='Stop searching for duplicates and close the form?';ru='Прервать поиск дублей и закрыть форму?';vi='Ngừng tìm kiếm phần tử trùng lặp và đóng biểu mẫu?'");
			ElsIf StepByStepAssistantSettings.ProcedureName = "DuplicatesBackgroundDeletetion" Then
				QuestionText = NStr("en='Stop deleting duplicates and close the form?';ru='Прервать удаление дублей и закрыть форму?';vi='Ngừng xóa bản sao và đóng biểu mẫu?'");
			EndIf;
...
Function BackGroundJobStart(Val ProcedureName)
	...
	If ProcedureName = "BackgroundDuplicatesSearch" Then
		...
		MethodName = NStr("en='Search and delete duplicates: Search for duplicates';ru='Поиск и удаление дублей: Поиск дублей';vi='Tìm kiếm và xóa bỏ phần tử trùng lặp: Tìm kiếm bản sao'");
	...
	ElsIf ProcedureName = "DuplicatesBackgroundDeletetion" Then
		...
		MethodName = NStr("en='Search and deletion of duplicates: Delete duplicates';ru='Поиск и удаление дублей: Удаление дублей';vi='Tìm kiếm và xóa bỏ phần tử trùng lặp: Xóa bỏ bản sao'");
