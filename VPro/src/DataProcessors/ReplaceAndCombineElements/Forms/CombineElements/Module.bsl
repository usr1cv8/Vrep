...
Procedure UsagePlacesBeforeDeletion(Item, Cancel)
	...
	QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Delete item ""%1"" from the list for merging?';ru='Удалить из списка для объединения элемент ""%1""?';vi='Xóa từ danh sách để hợp nhất phần tử ""%1""?'"),
...
Procedure SetConditionalAppearance()
	...
	SearchAndDeleteDuplicates.AddConditionalAppearanceElement(ThisObject, Instruction);
...
Function CheckMergedRefs(Val RefsSet)
	...
	If QuantityRefs < 2 Then
		Result.Error = NStr("en='Specify several items for merging.';ru='Для объединения необходимо указать несколько элементов.';vi='Để hợp nhất, cần chỉ ra nhiều phần tử.'");
		Return Result;
	EndIf;
...
"Группы не могут быть объединены.';vi='Một trong các phần tử hợp nhất là nhóm."
"Nhóm không thể hợp nhất được.'");
...
"Такие элементы не могут быть объединены.';vi='Ở các phần tử hợp nhất có các đối tượng chủ khác nhau."
"Những phần tử này không thể hợp nhất được.'");
	ElsIf Control.QuantityRefs <> QuantityRefs Then
		Result.Error = NStr("en='All mergable items must be of the same type.';ru='Все объединяемые элементы должны быть одного типа.';vi='Tất cả phần tử hợp nhất cần có một kiểu.'");
	Else 
		// Everything is ok
...
"использования на ""%2"" (отмечен стрелкой).';vi='Phần tử (%1) sẽ <a href = ""DeletionModeSwitch"">bị đánh dấu xóa</a> và bị thay thế trong tất cả các nơi"
"sử dụng sang ""%2"" (bị đánh dấu bởi mũi tên).'");
		Else
...
"использования на ""%2"" (отмечен стрелкой).';vi='Phần tử (%1) sẽ <a href = ""DeletionModeSwitch"">bị xóa vĩnh viễn</a> và bị thay thế trong tất cả các nơi"
"sử dụng sang ""%2"" (đã hủy bỏ bởi mũi tên).'");
		EndIf;
	Else
...
"и заменены во всех местах использования на ""%2"" (отмечен стрелкой).';vi='Phần tử (%1) sẽ bị đánh dấu xóa"
"và thay thế trong tất cả các nơi sử dụng sang ""%2"" (bị đánh dấu bằng mũi tên)."
"'");
	EndIf;
...
Function EndingMessage()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Items (%1) are merged into ""%2""';ru='Элементы (%1) объединены в ""%2""';vi='Đã hợp nhất phần tử (%1) vào ""%2""'"),
		UsagePlaces.Count(),
		MainItem);
...
"автоматическая замена на ""%1"".';vi='Chưa thực hiện hợp nhất phần tử. Tại nhiều nơi sử dụng không thể"
"thay thế tự động sang ""%1"".'"),
		MainItem);
...
Procedure FillUsagePlaces(Val UsageTable)
	...
	For Each String IN UsageTable Do
		...
		UsingRow.NotUsed = ?(Places = 0, NStr("en='Not used';ru='Не используется';vi='Không sử dụng'"), "");
	EndDo;
	...
	HeaderText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Merge items of %1 into a single one';ru='Объединение элементов %1 в один';vi='Hợp nhất các phần tử %1 vào một'"),
		Presentation
	);
EndProcedure
...
Procedure AddUsagePlacesRows(Val RefArray)
	...
	For Each Refs IN RefArray Do
		...
		If ExistingRows.Count() = 0 Then
			...
			String.NotUsed    = NStr("en='Not calculated';ru='Не рассчитано';vi='Chưa tính'");
		Else
...
Procedure InitializeAssistantScript()
	...
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Refuse to merge items';ru='Отказаться от объединения элементов';vi='Hủy bỏ hợp nhất phần tử'");
	...
	ButtonsAssistant.Next.Title = NStr("en='Merge >';ru='Объединить >';vi='Hợp nhất >'");
	ButtonsAssistant.Next.ToolTip = NStr("en='Start merging items';ru='Начать объединение элементов';vi='Bắt đầu hợp nhất các phần tử'");
	...
	ButtonsAssistant.Next.Visible = False;
	ButtonsAssistant.Cancel.Title = NStr("en='Abort';ru='Прервать';vi='Ngừng'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Stop joining items';ru='Прервать объединение элементов';vi='Ngừng hợp nhất phần tử'");
	...
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Close grouping results';ru='Закрыть результаты объединения';vi='Đóng kết quả hợp nhất'");
	...
	ButtonsAssistant = ButtonsAssistant();
	ButtonsAssistant.Back.Title = NStr("en='< Home';ru='< Home';vi='< Trang chủ'");
	ButtonsAssistant.Back.ToolTip = NStr("en='Go back to the main item selection';ru='Вернутся к выбору основного элемента';vi='Quay lại chọn phần tử chính'");
	ButtonsAssistant.Next.DefaultButton = True;
	ButtonsAssistant.Next.Title = NStr("en='Repeat';ru='Повторить';vi='Lặp lại'");
	ButtonsAssistant.Next.ToolTip = NStr("en='Repeat grouping';ru='Повторить объединение';vi='Lặp lại hợp nhất'");
	ButtonsAssistant.Cancel.Title = NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Close grouping results';ru='Закрыть результаты объединения';vi='Đóng kết quả hợp nhất'");
...
Procedure StepSelectMainItemBeforeNextAction(Val StepParameters, Val AdditionalParameters) Export
	...
	WarningParameters.Insert("Title", NStr("en='Unable to merge items';ru='Невозможно объединить элементы';vi='Không thể hợp nhất phần tử'"));
...
Procedure AddAssistantStep(Val Page, Val Actions, Val Buttons)
	...
	StepDescription.ButtonNext.Insert("Title", NStr("en='Next >';ru='Далее  >';vi='Tiếp theo >'"));
	
	StepDescription.Insert("ButtonBack", New Structure(ButtonFields, True, True, False));
	StepDescription.ButtonBack.Insert("Title", NStr("en='< Back';ru='< Back';vi='< Quay lại'"));
	
	StepDescription.Insert("ButtonCancel",New Structure(ButtonFields, True, True, False));
	StepDescription.ButtonCancel.Insert("Title", NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'"));
	...
	If Not InformationAboutJob.Completed Then
		If ShowDialogBeforeClosing Then
			...
			If StepByStepAssistantSettings.ProcedureName = "DefineUsagePlacess" Then
				QuestionText = NStr("en='Stop searching for usage locations and close the form?';ru='Прервать поиск мест использования и закрыть форму?';vi='Ngắt tìm kiếm nơi sử dụng và đóng biểu mẫu?'");
			ElsIf StepByStepAssistantSettings.ProcedureName = "ReplaceRefs" Then
				QuestionText = NStr("en='Stop merging items and close the form?';ru='Прервать объединение элементов и закрыть форму?';vi='Ngắt hợp nhất các phần tử và đóng biểu mẫu?'");
			EndIf;
...
Function BackGroundJobStart(Val ProcedureName)
	...
	If ProcedureName = "DefineUsagePlacess" Then
		...
		MethodName = NStr("en='Search and deletion of duplicates: Determine usage locations';ru='Поиск и удаление дублей: Определение мест использования';vi='Tìm kiếm và xóa bỏ phần tử trùng lặp. Xác định nơi sử dụng'");
		MethodParameters = RefsArrayFromSet(UsagePlaces);
		
	ElsIf ProcedureName = "ReplaceRefs" Then
		
		MethodName = NStr("en='Search and deletion of duplicates: Merge items';ru='Поиск и удаление дублей: Объединение элементов';vi='Tìm kiếm và xóa bỏ phần tử trùng lặp. Hợp nhất các phần tử'");
		SubstitutionsPairs = New Map;
		For Each String IN UsagePlaces Do
			SubstitutionsPairs.Insert(String.Ref, MainItem);
		EndDo;
