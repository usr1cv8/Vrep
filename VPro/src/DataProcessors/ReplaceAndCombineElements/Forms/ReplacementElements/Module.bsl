...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Items["ListRefNew"].Title = NStr("en='Description';ru='Description';vi='Tên gọi'");
	...
	CommonUseClientServer.SetFilterDynamicListItem(
		...
		NStr("en='Do not show replaced';ru='Не показывать заменяемые';vi='Chưa hiển thị phần mục dược thay thế'"),
		True,
	...
	If ReplaceReferences.Count() > 1 Then
		Items.LabelSelectedType.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Select one of the ""%1"" items the selected values (%2) should be replaced with:';ru='Выберите один из элементов ""%1"", на который следует заменить выбранные значения (%2):';vi='Hãy chọn một trong các phần tử ""%1"" mà cần thay thế các giá trị đã chọn (%2):'"),
			MainMetadata.Presentation(), ReplaceReferences.Count());
	Else
		Title = NStr("en='Replace item';ru='Замена элемента';vi='Thay thế phần tử'");
		Items.LabelSelectedType.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Select one of the ""%1"" items ""%2"" should be replaced with:';ru='Выберите один из элементов ""%1"", на который следует заменить ""%2"":';vi='Hãy chọn một trong các phần tử ""%1"" mà cần thay thế ""%2"":'"),
			MainMetadata.Presentation(), ReplaceReferences[0].Ref);
	EndIf;
	Items.ToolTipSelectTargetItem.Title = NStr("en='Replacement item is not selected.';ru='Элемент для замены не выбран.';vi='Chưa chọn phần tử để thay thế'");
...
"и <a href = ""ПереключениеРежимаУдаления"">помечен на удаление</a>.';vi='Phần tử đã chọn sẽ được thay thế bằng ""%1"""
"và <a href = ""ПереключениеРежимаУдаления"">bị đánh dấu xóa</a>.'");
			Else
				ToolTipText = NStr("en='The selected item will be"
"replaced with %1 and <a href = ""DeletionModeSwitch>permanently deleted</a>.';ru='Выбранный элемент будет заменен на ""%1"""
"и <a href = ""ПереключениеРежимаУдаления"">удален безвозвратно</a>.';vi='Phần tử đã chọn sẽ bị thay thế bởi ""%1"""
"và <a href = ""ПереключениеРежимаУдаления"">bị xóa vĩnh viễn</a>.'");
			EndIf;
		Else
			ToolTipText = NStr("en='The selected item will be"
"replaced with %1 and marked for deletion.';ru='Выбранный элемент будет"
"заменен на ""%1"" и помечен на удаление.';vi='Chứng từ đã chọn sẽ"
"được thay thế sang ""%1"" và bị đánh dấu xóa.'");
		EndIf;
...
"и <a href = ""ПереключениеРежимаУдаления"">помечены на удаление</a>.';vi='Phần tử đã chọn (%1) sẽ được thay thế bằng ""%2"""
"và <a href = ""ПереключениеРежимаУдаления"">bị đánh dấu xóa</a>.'");
			Else
				ToolTipText = NStr("en='The selected items (%1) will be"
"replaced with %2 and <a href = ""DeletionModeSwitch>permanently deleted</a>.';ru='Выбранные элементы (%1) будут заменены на ""%2"""
"и <a href = ""ПереключениеРежимаУдаления"">удалены безвозвратно</a>.';vi='Các phần tử đã chọn (%1) sẽ được thay thế bằng ""%2"""
"và <a href = ""ПереключениеРежимаУдаления"">удалены безвозвратно</a>.'");
			EndIf;
		Else
			ToolTipText = NStr("en='The selected items (%1) will be"
"replaced with %2 and marked for deletion.';ru='Выбранные элементы (%1) будут"
"заменены на ""%2"" и помечен на удаление.';vi='Các phần tử đã chọn (%1) sẽ"
"được thay thế sang ""%2"" và được đánh dấu xóa'");
		EndIf;
...
Function EndingMessage()
	...
	If Count = 1 Then
		ResultText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Item ""%1"" is replaced with ""%2""';ru='Элемент ""%1"" заменен на ""%2""';vi='Đã thay thế phần tử ""%1"" sang ""%2""'"),
			ReplaceReferences[0].Ref,
			TargetItemResult);
	Else
		ResultText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Items (%1) are replaced with ""%2""';ru='Элементы (%1) заменены на ""%2""';vi='Đã thay thế phần tử (%1) sang ""%2""'"),
			Count,
			TargetItemResult);
	EndIf;
...
"автоматическая замена на ""%3""';vi='Không thể thay thế phần tử (%1 từ %2). Trong một số nơi sử dụng không thể "
"thay thế tự động sang ""%3""'"),
...
Function FillFailedReplacements(Val ReplacementResults)
	...
	For Each ResultRow IN ReplacementResults Do
		...
		If ErrorType = "UnknownData" Then
			ErrorString.Cause = NStr("en='Data which processing was not planned is detected.';ru='Обнаружена данные, обработка которых не планировалась.';vi='Tìm thấy dữ liệu mà chưa dự tính xử lý.'");
			
		ElsIf ErrorType = "LockError" Then
			ErrorString.Cause = NStr("en='The data is locked by another user.';ru='Данные заблокированы другим пользователем.';vi='Người sử dụng khác đã phong tỏa dữ liệu.'");
			
		ElsIf ErrorType = "DataChanged" Then
			ErrorString.Cause = NStr("en='Data is changed by another user.';ru='Данные изменены другим пользователем.';vi='Người sử dụng khác đã thay đổi dữ liệu.'");
			
		ElsIf ErrorType = "RecordingError" Then
			ErrorString.Cause = ResultRow.ErrorText;
			
		ElsIf ErrorType = "ErrorDelete" Then
			ErrorString.Cause = NStr("en='You cannot delete data.';ru='Невозможно удалить данные.';vi='Không thể xóa bỏ dữ liệu.'");
			
		Else
			ErrorString.Cause = NStr("en='Unknown error.';ru='Неизвестная ошибка.';vi='Lỗi không xác định.'");
...
Procedure InitializeReplacedReferences(Val RefArray)
	...
	If QuantityRefs = 0 Then
		ParametersErrorText = NStr("en='No item for replacement is specified.';ru='Не указано ни одного элемента для замены.';vi='Chưa chỉ ra bất kỳ phần tử nào để thay thế.'");
		Return;
	EndIf;
...
"Группы не могут быть заменены.';vi='Một trong các phần tử thay thế là nhóm."
"Nhóm không thể thay thế được.'");
		Return;
...
"Такие элементы не могут быть заменены.';vi='Các phần tử thay thế có đối tượng chủ khác nhau."
"Những phần tử như thế không thể thay thế được.'");
		Return;
	ElsIf Conditions.QuantityRefs <> QuantityRefs Then
		ParametersErrorText = NStr("en='All replaceable items must be of the same type.';ru='Все заменяемые элементы должны быть одного типа.';vi='Tất cả phần tử được thay thế cần có một kiểu.'");
		Return;
	EndIf;
	...
	If Result[2].Unload().Count() = 0 Then
		If QuantityRefs > 1 Then
			ParametersErrorText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='There is nothing to replace the selected items (%1) with.';ru='Выбранные элементы (%1) не на что заменить.';vi='Không có gì thay thế được các phần tử đã chọn (%1).'"), QuantityRefs);
		Else
			ParametersErrorText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='There is nothing to replace selected item ""%1"" with.';ru='Выбранный элемент ""%1"" не на что заменить.';vi='Không có gì thay thế được các phần tử đã chọn (%1).'"), CommonUse.SubjectString(TargetItem));
		EndIf;
		Return;
	EndIf;
...
Procedure InitializeAssistantScript()
	...
	ButtonsAssistant.Next.Title = NStr("en='Replace >';ru='Заменить >';vi='Thay thế >'");
	ButtonsAssistant.Next.ToolTip = NStr("en='Start replacing items';ru='Начать замену элементов';vi='Bắt đầu thay thế phần tử'");
	ButtonsAssistant.Cancel.Title = NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Refuse to replace items';ru='Отказаться от замены элементов';vi='Hủy bỏ thay thế phần tử'");
	...
	ButtonsAssistant.Cancel.Title = NStr("en='Abort';ru='Прервать';vi='Ngừng'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Stop item replacement';ru='Прервать замену элементов';vi='Ngừng thay thế phần tử'");
	...
	ButtonsAssistant.Cancel.Title = NStr("en='Close';ru='Закрыть';vi='Đóng'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Close results of item replacement';ru='Закрыть результаты замены элементов';vi='Đóng kết quả thay thế phần tử'");
	...
	ButtonsAssistant = ButtonsAssistant();
	ButtonsAssistant.Back.Title = NStr("en='< Back';ru='< Back';vi='< Quay lại'");
	ButtonsAssistant.Back.ToolTip = NStr("en='Return to target item selection';ru='Вернутся к выбору целевого элемента';vi='Quay lại chọn phần tử đích'");
	ButtonsAssistant.Next.Title = NStr("en='Repeat replacement >';ru='Повторить замену >';vi='Lặp lại thay thế >'");
	ButtonsAssistant.Next.ToolTip = NStr("en='Repeat item replacement';ru='Повторить замену элементов';vi='Lặp lại thay thế phần tử'");
	ButtonsAssistant.Cancel.Title = NStr("en='Close';ru='Закрыть';vi='Đóng'");
	ButtonsAssistant.Cancel.ToolTip = NStr("en='Close results of item replacement';ru='Закрыть результаты замены элементов';vi='Đóng kết quả thay thế phần tử'");
...
Procedure StepSelectTargetItemBeforeNextAction(Val StepParameters, Val AdditionalParameters) Export
	...
	ElsIf ReplaceReferences.Count() = 1 AND CurrentData.Ref = ReplaceReferences.Get(0).Ref Then
		ShowMessageBox( , NStr("en='An item cannot be replaced with itself.';ru='Нельзя заменять элемент сам на себя.';vi='Không nên thay thế chính phần tử vào chính nó.'"));
		Return;
		
	ElsIf AttributeValue(CurrentData, "IsFolder", False) Then
		ShowMessageBox( , NStr("en='Cannot replace item with group.';ru='Нельзя заменять элемент на группу.';vi='Không nên thay thế phần tử vào nhóm.'"));
		Return;
	EndIf;
...
"У выбранного элемента владелец ""%1"", а у заменяемого - ""%2"".';vi='Không thể thay thế sang phần tử trực thuộc đối tượng chủ khác."
"Trong phần tử đã chọn có đối tượng chủ ""%1"", còn trong phần tử thay thế - ""%2""."
"'"),
	...
	Text = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Item %1 is marked for deletion. Continue?';ru='Элемент %1 помечен на удаление. Продолжить?';vi='Phần tử %1 bị đặt dấu xóa. Tiếp tục?'"),
		CurrentData.Ref
	);
...
Procedure CheckAppliedAreaReplacementAdmissibility(Val StepParameters)
	...
	WarningParameters.Insert("Title", NStr("en='You cannot replace items';ru='Невозможно заменить элементы';vi='Không thể thay thế phần tử'"));
...
Function ButtonsAssistant()
	...
	Result.Next.Title = NStr("en='Next >';ru='Далее  >';vi='Tiếp theo >'");
	Result.Back.Title = NStr("en='< Back';ru='< Back';vi='< Quay lại'");
	Result.Cancel.Title = NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'");
	Return Result;
...
Procedure RunAssistant()
	If StepByStepAssistantSettings.StartPage = Undefined Then
		Raise NStr("en='Before you run this wizard, set the home page.';ru='Перед запуском мастера должна быть установлена начальная страница.';vi='Trước khi khởi động bản hướng dẫn, cần thiết lập bàn làm việc.'");
		...
		For Each StepDescription IN StepByStepAssistantSettings.Steps Do
			For Each KeyValue IN PossibleActions Do
				...
				If Not IsBlankString(HandlerName) Then
					...
					Except
						Text = StringFunctionsClientServer.SubstituteParametersInString(
							NStr("en='An error occurred when creating event handler %1 for page %2, procedure %3 is not defined';ru='Ошибка создания обработчика события %1 для страницы %2, не определена процедура %3';vi='Lỗi tạo hàm sự kiện %1 đối với trang %2, chưa xác định thủ tục %3'"),
							NameActions, 
...
Procedure AssistantStep(Val CommandCode)
	...
	ElsIf CommandCode = "Cancel" Then
		Direction = 0;
	Else
		Raise NStr("en='Incorrect wizard step command';ru='Некорректная команда шага помощника';vi='Sai lệnh của bước trợ giúp'");
	EndIf;
...
Procedure GoToAssistantStep(Val IdentifierStep, Val TriggerEvents = False)
	...
	If NextStep = Undefined Then
		Error = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Step of wizard %1 is not found';ru='Не найден шаг помощника %1';vi='Chưa tìm thấy bước trợ giúp %1'"),
			IdentifierStep
		);
		Raise Error;
	EndIf;
...
Procedure AssistantStepEnd(Val StepParameters)
	...
	ElsIf StepParameters = 1 AND NextStep > LastStep Then
		...
		Raise NStr("en='Attempting to go over the last wizard step';ru='Попытка выхода за последний шаг мастера';vi='Thử vượt qua bước cuối cùng của bản hướng dẫn'");
		
	ElsIf StepParameters = -1 AND NextStep < 0 Then
		// You are trying to take a step outside back.
		Raise NStr("en='Attempting to go back from the first wizard step';ru='Попытка выхода назад из первого шага мастера';vi='Thử quay lại từ bước đầu tiên của bản hướng dẫn'");
...
Function AssistantStepNumberByIdentifier(Val IdentifierStep)
	...
	Raise StrReplace(NStr("en='Step ""%1"" is not found.';ru='Не найдено шаг ""%1"".';vi='Chưa tìm thấy bước ""%1"".'"), "%1", SearchName);
EndFunction
...
Function BackgroundJobImportOnClient(InterruptIfNotCompleted, ShowDialogBeforeClosing)
	...
	If Not InformationAboutJob.Completed Then
		If ShowDialogBeforeClosing Then
			...
			QuestionText = NStr("en='Stop items replacement and close the form?';ru='Прервать замену элементов и закрыть форму?';vi='Ngừng thay thế phần tử và đóng biểu mẫu?'");
			
			Buttons = New ValueList;
			Buttons.Add(DialogReturnCode.Abort, NStr("en='Abort';ru='Прервать';vi='Ngừng'"));
			Buttons.Add(DialogReturnCode.No, NStr("en='Do not interrupt';ru='Не прерывать';vi='Không ngừng lại'"));
...
Function BackGroundJobStart()
	...
	Try
		Task = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Search and delete duplicates: Reference replacement';ru='Поиск и удаление дублей: Замена ссылок';vi='Tìm kiếm và xóa bỏ phần tử trùng lặp: Thay thế tham chiếu'"));
	Except
		ErrorInfo = ErrorInfo();
	EndTry;
