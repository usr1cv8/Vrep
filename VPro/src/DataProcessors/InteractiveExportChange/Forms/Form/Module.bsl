...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(Object.InfobaseNode) Then
		Text = NStr("en='Data exchange setting is not found.';ru='Настройка обмена данными не найдена.';vi='Chưa tìm thấy tùy chỉnh trao đổi dữ liệu.'");
...
Procedure AdditionalRegistrationBeforeDeleting(Item, Cancel)
	...
	If Quantity>1 Then
		PresentationText = NStr("en='Selected lines';ru='выбранные строки';vi='dòng đã chọn'");
	...
	QuestionText = NStr("en='Delete from additional data %1 ?';ru='Удалить из дополнительных данных %1 ?';vi='Xóa từ dữ liệu bổ sung %1?'");    
	...
	ElsIf TypeOfSelected= Type("Structure") Then
		If ValueSelected.ActionSelect=3 Then
			...
			If Not IsBlankString(ViewCurrentSettings) AND SettingRepresentation<>ViewCurrentSettings Then
				QuestionText  = NStr("en='Restore settings ""%1""?';ru='Восстановить настройки ""%1""?';vi='Phục hồi tùy chỉnh ""%1""?'");
...
Function EditingRowFilterAdditionalListServer(ChoiceStructure)
	...
	CurrentData.Quantity   = NStr("en='Not calculated';ru='Не рассчитано';vi='Chưa tính'");
...
Function AddInExportAdditionalContent(Item)
	...
	Else
		...
		String.Quantity = NStr("en='Not calculated';ru='Не рассчитано';vi='Chưa tính'");
		RefreshTotalQuantityLabel();
	EndIf;
...
Function QuantityUpdated()
	...
	BackgroundJobResult = LongActions.ExecuteInBackground(UUID,
		...
		JobParameters, NStr("en='Calculating the number of objects to be sent during synchronization';ru='Расчет количества объектов для отправки при синхронизации';vi='Tính số lượng đối tượng để gửi khi đồng bộ hóa'"));
...
Procedure RefreshTotalQuantityLabel(Quantity = Undefined) 
	...
	If Quantity = Undefined Then
		CountText = NStr("en='<not calculated>';ru='<не рассчитано>';vi='<chưa tính>'");
	Else
		CountText = NStr("en='Objects: %1';ru='Объектов: %1';vi='Đối tượng: %1'");
...
Procedure ResetTableCountsLabel()
	CountsText = NStr("en='Not calculated';ru='Не рассчитано';vi='Chưa tính'");
	For Each String IN Object.AdditionalRegistration Do
