...
Function InitializeThisObject(Val Source = "") Export
	...
	FillPropertyValues(ThisObject, Source, , "ComposerAllDocumentsFilter, AdditionalRegistration, AdditionalRegistrationScriptSite");
...
Function AllDocumentsFilterTitleGroup() Export
	Return NStr("en='All documents';ru='Все документы';vi='Tất cả chứng từ'");
EndFunction
...
Function AllCatalogsFilterGroupsTitle() Export
	Return NStr("en='All catalogs';ru='Все справочники';vi='Tất cả danh mục'");
EndFunction
...
Function RestoreCurrentFromSettings(Presentation, Variants = Undefined, AddressOfFormStore = Undefined) Export
	...
	If Result Then
		...
		ComposerAllDocumentsFilter.LoadSettings(ItemOfList.Value._LinkerSettingsForAllSelectedDocuments);
...
Function InitializeComposer(ListOfMetadataNames = Undefined, LimitUseOfSelection = False, AddressConservationScheme = Undefined)
	...
	For Each String IN NodeContentMetadataTable Do
		...
		If ChangeSetItems.Find(SetName) = Undefined Then
			...
			Set.Query = "
				...
				|	TYPE(" + FullMetadataName + ") AS ObjectRegistrationType,
	...
	AutoParameter.Value = NStr("en='By common rules';ru='По общим правилам';vi='Theo quy tắc chung'");
	
	AdditionalParameter = Parameters.Find("ReasonForRegistrationAdvanced");
	AdditionalParameter.Value = NStr("en='Additional Information';ru='Дополнительно';vi='Bổ sung'");
	
	ReferenceParameter = Parameters.Find("ReasonForRegistrationLink");
	ReferenceParameter.Value = NStr("en='By reference';ru='По ссылке';vi='Theo tham chiếu'");
...
Procedure AddAdditionalSetsOfTabularLayoutParts(SourceItems, AddingParameters)
	...
	If ObjectMetadata = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='The %1 incorrect name of metadata for registration at the %2 node';ru='Некорректное имя метаданных %1 для регистрации на узле %2';vi='Tên metadata %1 không xác định để ghi nhận tại nút %2'"),
...
Function TextAddOptions()
	...
	If ExportVariant = 0 Then
		// All automatic data
		Return NStr("en='Without additional data.';ru='Без дополнительных данных.';vi='Không có dữ liệu bổ sung.'");
