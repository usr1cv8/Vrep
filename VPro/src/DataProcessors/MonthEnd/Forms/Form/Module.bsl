...
Procedure ExecuteClosingMonthInLongOperation(ParametersStructureBackgroundJob)
	...
	AssignmentResult = LongActions.ExecuteInBackground(
		...
		NStr("en='Month-end closing is in progress';ru='Выполняется закрытие месяца';vi='Đang đóng tháng'")
	);
...
Procedure WarnAboutActiveBackgroundJob(Cancel = True)
	...
	Cancel = True;
	WarningText = NStr("en='Please wait while the process is finished (recommended) or cancel it manually.';ru='Дождитесь окончания рабочего процесса (рекомендуется) либо прервите его самостоятельно.';vi='Hãy chờ kết thúc quá trình làm việc (khuyến cáo) hoặc tự ngắt quá trình.'");
	ShowMessageBox(Undefined, WarningText, 10, NStr("en='it is impossible to close form.';ru='невозможно закрыть форму.';vi='không thể đóng biểu mẫu.'"));
	...
	For Ct = 1 To 12 Do
		...
		If Not ValueIsFilled(ThisForm["TextErrorCostAllocation" + Ct]) Then
			If Items.Find("CostAllocationPicture" + Ct).Picture = Items.Green.Picture Then
				ThisForm["TextErrorCostAllocation" + Ct] = NStr("en='Retail cost (value accounting) is successfully calculated.';ru='Расчет себестоимости в рознице (суммовой учет) выполнен успешно!';vi='Đã tính xong giá vốn trong bán lẻ (kế toán giá trị)!'");
			...
			ElsIf Items.Find("CostAllocationPicture" + Ct).Picture = Items.GreenIsNotRequired.Picture Then
				ThisForm["TextErrorCostAllocation" + Ct] = NStr("en='Retail cost (value accounting) is not required.';ru='Расчет себестоимости в рознице (суммовой учет) не требуется.';vi='Không cần tính giá vốn trong bán lẻ (kế toán giá trị).'");
			ElsIf Items.Find("CostAllocationPicture" + Ct).Picture = Items.Gray.Picture Then
				ThisForm["TextErrorCostAllocation" + Ct] = NStr("en='Costs are not allocated.';ru='Распределение затрат не производилось.';vi='Chưa tiến hành phân bổ chi phí.'");
			ElsIf Items.Find("CostAllocationPicture" + Ct).Picture = Items.Red.Picture Then
				ThisForm["TextErrorCostAllocation" + Ct] = NStr("en='Cost allocation is required.';ru='Требуется выполнить распределение затрат.';vi='Cần thực hiện phân bổ chi phí.'");
			EndIf;
		Else
		...
		If Not ValueIsFilled(ThisForm["TextErrorDirectCostCalculation" + Ct]) Then
			If Items.Find("DirectCostCalculationPicture" + Ct).Picture = Items.Green.Picture Then
				ThisForm["TextErrorDirectCostCalculation" + Ct] = NStr("en='Direct costs are calculated.';ru='Расчет прямых затрат выполнен успешно!';vi='Đã tính xong chi phí trực tiếp!'");
				MonthClosed = True; // month is closed if distribution was completed successfully
			ElsIf Items.Find("DirectCostCalculationPicture" + Ct).Picture = Items.GreenIsNotRequired.Picture Then
				ThisForm["TextErrorDirectCostCalculation" + Ct] = NStr("en='Direct cost calculation is not required.';ru='Расчет прямых затрат не требуется.';vi='Không cần tính chi phí trực tiếp.'");
			ElsIf Items.Find("DirectCostCalculationPicture" + Ct).Picture = Items.Gray.Picture Then
				ThisForm["TextErrorDirectCostCalculation" + Ct] = NStr("en='Direct costs were not calculated.';ru='Расчет прямых затрат не производился.';vi='Chưa tính chi phí trực tiếp.'");
			ElsIf Items.Find("DirectCostCalculationPicture" + Ct).Picture = Items.Red.Picture Then
				ThisForm["TextErrorDirectCostCalculation" + Ct] = NStr("en='Direct cost calculation is required.';ru='Требуется выполнить расчет прямых затрат.';vi='Cần thực hiện tính chi phí trực tiếp.'");
			EndIf;
		Else
			MonthClosed = True; // month is closed if there are any errors
		EndIf;
		...
		If Not ValueIsFilled(ThisForm["TextErrorActualCostCalculation" + Ct]) Then
			If Items.Find("ActualCostCalculationPicture" + Ct).Picture = Items.Green.Picture Then
				ThisForm["TextErrorActualCostCalculation" + Ct] = NStr("en='Actual cost is calculated successfully.';ru='Расчет фактической себестоимости выполнен успешно!';vi='Đã tính xong giá thành thực tế!'");
				MonthClosed = True; // month is closed if distribution was completed successfully
			ElsIf Items.Find("ActualCostCalculationPicture" + Ct).Picture = Items.GreenIsNotRequired.Picture Then
				ThisForm["TextErrorActualCostCalculation" + Ct] = NStr("en='Actual cost calculation is not required.';ru='Расчет фактической себестоимости не требуется.';vi='Không cần tính giá thành thực tế.'");
			ElsIf Items.Find("ActualCostCalculationPicture" + Ct).Picture = Items.Gray.Picture Then
				ThisForm["TextErrorActualCostCalculation" + Ct] = NStr("en='Actual cost calculation was not performed.';ru='Расчет фактической себестоимости не производился.';vi='Chưa tính giá thành thực tế.'");
			ElsIf Items.Find("ActualCostCalculationPicture" + Ct).Picture = Items.Red.Picture Then
				ThisForm["TextErrorActualCostCalculation" + Ct] = NStr("en='Actual cost calculation is required.';ru='Требуется выполнить расчет фактической себестоимости.';vi='Cần thực hiện tính giá thành thực tế.'");
			EndIf;
		Else
			MonthClosed = True; // month is closed if there are any errors
		EndIf;
		...
		If Not ValueIsFilled(ThisForm["TextErrorFinancialResultCalculation" + Ct]) Then
			If Items.Find("FinancialResultCalculationPicture" + Ct).Picture = Items.Green.Picture Then
				ThisForm["TextErrorFinancialResultCalculation" + Ct] = NStr("en='Financial result is calculated.';ru='Расчет финансового результата выполнен успешно!';vi='Đã xác định xong kết quả hoạt động kinh doanh!'");
				MonthClosed = True; // month is closed if distribution was completed successfully
			ElsIf Items.Find("FinancialResultCalculationPicture" + Ct).Picture = Items.GreenIsNotRequired.Picture Then
				ThisForm["TextErrorFinancialResultCalculation" + Ct] = NStr("en='Financial result calculation is not required.';ru='Расчет финансового результата не требуется.';vi='Không cần xác định kết quả hoạt động kinh doanh.'");
			ElsIf Items.Find("FinancialResultCalculationPicture" + Ct).Picture = Items.Gray.Picture Then
				ThisForm["TextErrorFinancialResultCalculation" + Ct] = NStr("en='Financial result was not calculated.';ru='Расчет финансового результата не производился.';vi='Chưa xác định kết quả hoạt động kinh doanh.'");
			ElsIf Items.Find("FinancialResultCalculationPicture" + Ct).Picture = Items.Red.Picture Then
				ThisForm["TextErrorFinancialResultCalculation" + Ct] = NStr("en='Financial result calculation is required.';ru='Требуется выполнить расчет финансового результата.';vi='Cần thực hiện tính kết quả hoạt động kinh doanh.'");
			EndIf;
		Else
			MonthClosed = True; // month is closed if there are any errors
		EndIf;
		...
		If Not ValueIsFilled(ThisForm["TextErrorExchangeDifferencesCalculation" + Ct]) Then
			If Items.Find("ExchangeDifferencesCalculationPicture" + Ct).Picture = Items.Green.Picture Then
				ThisForm["TextErrorExchangeDifferencesCalculation" + Ct] = NStr("en='Retail cost (value accounting) is successfully calculated.';ru='Расчет себестоимости в рознице (суммовой учет) выполнен успешно!';vi='Đã tính xong giá vốn trong bán lẻ (kế toán giá trị)!'");
				MonthClosed = True; // month is closed if distribution was completed successfully
			ElsIf Items.Find("ExchangeDifferencesCalculationPicture" + Ct).Picture = Items.GreenIsNotRequired.Picture Then
				ThisForm["TextErrorExchangeDifferencesCalculation" + Ct] = NStr("en='Retail cost (value accounting) is not required.';ru='Расчет себестоимости в рознице (суммовой учет) не требуется.';vi='Không cần tính giá vốn trong bán lẻ (kế toán giá trị).'");
			ElsIf Items.Find("ExchangeDifferencesCalculationPicture" + Ct).Picture = Items.Gray.Picture Then
				ThisForm["TextErrorExchangeDifferencesCalculation" + Ct] = NStr("en='Exchange rate differences are not calculated.';ru='Расчет курсовых разниц не производился.';vi='Chưa tính chênh lệch tỷ giá.'");
			ElsIf Items.Find("ExchangeDifferencesCalculationPicture" + Ct).Picture = Items.Red.Picture Then
				ThisForm["TextErrorExchangeDifferencesCalculation" + Ct] = NStr("en='Retail cost (value accounting) is required.';ru='Требуется выполнить расчет себестоимости в рознице (суммовой учет).';vi='Cần thực hiện tính giá vốn trong bán lẻ (kế toán giá trị).'");
			EndIf;
		Else
			MonthClosed = True; // month is closed if there are any errors
		EndIf;
		...
		If Not ValueIsFilled(ThisForm["TextErrorCalculationPrimecostInRetail" + Ct]) Then
			If Items.Find("RetailCostCalculationPicture" + Ct).Picture = Items.Green.Picture Then
				ThisForm["TextErrorCalculationPrimecostInRetail" + Ct] = NStr("en='Retail cost (value accounting) is successfully calculated.';ru='Расчет себестоимости в рознице (суммовой учет) выполнен успешно!';vi='Đã tính xong giá vốn trong bán lẻ (kế toán giá trị)!'");
				MonthClosed = True; // month is closed if distribution was completed successfully
			ElsIf Items.Find("RetailCostCalculationPicture" + Ct).Picture = Items.GreenIsNotRequired.Picture Then
				ThisForm["TextErrorCalculationPrimecostInRetail" + Ct] = NStr("en='Retail cost (value accounting) is not required.';ru='Расчет себестоимости в рознице (суммовой учет) не требуется.';vi='Không cần tính giá vốn trong bán lẻ (kế toán giá trị).'");
			ElsIf Items.Find("RetailCostCalculationPicture" + Ct).Picture = Items.Gray.Picture Then
				ThisForm["TextErrorCalculationPrimecostInRetail" + Ct] = NStr("en='Retail cost (value accounting) is not calculated.';ru='Расчет себестоимости в рознице (суммовой учет) не производился.';vi='Chưa tính giá vốn trong bán lẻ (kế toán giá trị).'");
			ElsIf Items.Find("RetailCostCalculationPicture" + Ct).Picture = Items.Red.Picture Then
				ThisForm["TextErrorCalculationPrimecostInRetail" + Ct] = NStr("en='Retail cost (value accounting) is required.';ru='Требуется выполнить расчет себестоимости в рознице (суммовой учет).';vi='Cần thực hiện tính giá vốn trong bán lẻ (kế toán giá trị).'");
			EndIf;
		Else
			MonthClosed = True; // month is closed if there are any errors
		EndIf;
		...
		If Not ValueIsFilled(ThisForm["TextErrorDepreciationAccrual" + Ct]) Then
			If Items.Find("DepreciationAccrualPicture" + Ct).Picture = Items.Green.Picture Then
				ThisForm["TextErrorDepreciationAccrual" + Ct] = NStr("en='Depreciation is accrued.';ru='Начисление амортизации выполнено успешно!';vi='Đã thực hiện trích khấu hao thành công!'");
				MonthClosed = True; // month is closed if distribution was completed successfully
			ElsIf Items.Find("DepreciationAccrualPicture" + Ct).Picture = Items.GreenIsNotRequired.Picture Then
				ThisForm["TextErrorDepreciationAccrual" + Ct] = NStr("en='Depreciation accrual is not required.';ru='Начисление амортизации не требуется.';vi='Không cần trích khấu hao.'");
			ElsIf Items.Find("DepreciationAccrualPicture" + Ct).Picture = Items.Gray.Picture Then
				ThisForm["TextErrorDepreciationAccrual" + Ct] = NStr("en='Depreciation is not accrued.';ru='Начисление амортизации не производилось.';vi='Chưa thực hiện trích khấu hao.'");
			ElsIf Items.Find("DepreciationAccrualPicture" + Ct).Picture = Items.Red.Picture Then
				ThisForm["TextErrorDepreciationAccrual" + Ct] = NStr("en='Depreciation accrual is required.';ru='Требуется выполнить начисление амортизации.';vi='Cần thực hiện trích khấu hao.'");
			EndIf;
		Else
			MonthClosed = True; // month is closed if there are any errors
		EndIf;
		...
		If Items.Find("CostAllocationPicture" + Ct).Picture = Items.GreenIsNotRequired.Picture
			...
			Items.Find("DecorationPerformClosingNotNeeded" + Ct).Title = NStr("en='Month-end closing is not required as there is no data for calculation.';ru='Закрытие месяца не требуется, т.к. нет данных для расчета.';vi='Không cần đóng tháng vì không có dữ liệu để tính toán.'");
		Else
...
Procedure ExecuteMonthEnd(Command)
	...
	If EndOfMonth(Date(CurYear, CurMonth, 1)) <= EndOfDay(EditProhibitionDate) Then
		ShowMessageBox(Undefined, NStr("en='Cannot close the month as it belongs to the period prohibited for editing.';ru='Нельзя закрыть месяц, т.к. он относится к запрещенному для редактирования периоду!';vi='Không thể đóng tháng vì tháng này có liên quan đến kỳ bị cấm soạn!'"));
		Return;
	EndIf;
	InitializeMonthEnd();
