...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	VariantsOfFilterStatusMap.Insert(ChoiceList.Add(
		"AllObjects", NStr("en='All data';ru='Все данные';vi='Tất cả dữ liệu'")
	).Value, New FixedStructure);
	
	VariantsOfFilterStatusMap.Insert(ChoiceList.Add(
		"MappedObjectsUnapproved", NStr("en='Changes';ru='Изменения';vi='Thay đổi'"),
	).Value, New FixedStructure("MappingState",  3));
	
	VariantsOfFilterStatusMap.Insert(ChoiceList.Add(
		"MappedObjects", NStr("en='Mapped data';ru='Сопоставленные данные';vi='So sánh dữ liệu'"),,
	).Value, New FixedStructure("MappingStateAdditional", 0));
	
	VariantsOfFilterStatusMap.Insert(ChoiceList.Add(
		"UnmappedObjects", NStr("en='Unmapped data';ru='Несопоставленные данные';vi='Dữ liệu không tương ứng'"),
	).Value, New FixedStructure("MappingStateAdditional", 1));
	
	VariantsOfFilterStatusMap.Insert(ChoiceList.Add(
		"UnmappedObjectsSink", NStr("en='Unmapped data of this base';ru='Несопоставленные данные этой базы';vi='Dữ liệu không tương ứng của cơ sở này'"),
	).Value, New FixedStructure("MappingState",  1));
	
	VariantsOfFilterStatusMap.Insert(ChoiceList.Add(
		"UnmappedSourceObjects", NStr("en='Unmapped second base data';ru='Несопоставленные данные второй базы';vi='Dữ liệu không tương ứng của cơ sở thứ hai'"),
	).Value, New FixedStructure("MappingState", -1));
	...
	EndIf;
	Title = NStr("en='Mapping of data ""[DataPresentation]""';ru='Сопоставление данных ""[ПредставлениеДанных]""';vi='So sánh dữ liệu ""[DataPresentation]""'");
	...
	ThisApplicationName = ?(IsBlankString(ThisApplicationName), NStr("en='This application';ru='Эта программа';vi='Chương trình này'"), ThisApplicationName);
	
	SecondApplicationName = CommonUse.ObjectAttributeValue(Object.InfobaseNode, "Description");
	SecondApplicationName = ?(IsBlankString(SecondApplicationName), NStr("en='The second application';ru='Вторая программа';vi='Chương trình thứ hai'"), SecondApplicationName);
...
"Оставшиеся несопоставленные данные можно связать друг с другом вручную.';vi='Để so sánh dữ liệu ""%1""'"),
...
Procedure ExecuteDataImport(Command)
	NString = NStr("en='Receive data to the infobase?';ru='Получить данные в информационную базу?';vi='Nhận dữ liệu vào cơ sở thông tin?'");
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("en='No mapping for object. The object will be copied';ru='Нет соответствия, объект будет скопирован';vi='Không có tương ứng, sẽ sao chép đối tượng'"));
...
Procedure ImportDataAfterQuestionOnDataReceiptConfirmation(Val QuestionResult, Val AdditionalParameters) Export
	...
	If Object.DataSuccessfullyImported Then
		NString = NStr("en='Data is already received. Receive data again?';ru='Данные уже были получены. Выполнить получение данных повторно?';vi='Đã nhận dữ liệu. Thực hiện nhận lại dữ liệu?'");
...
Procedure ImportDataAfterDataReceivedConfirmation()
	...
	Status(NStr("en='Receiving data. Please wait...';ru='Выполняется получение данных. Пожалуйста, подождите..';vi='Đang thực hiện nhận dữ liệu. Xin vui lòng đợi...'"));
...
"Перейти в журнал регистрации?';vi='Khi nhận dữ liệu xuất hiện lỗi."
...
Procedure RunUserFieldsTaskCheck(Cancel, UserFields)
	...
	If UserFields.Count() = 0 Then
		...
		NString = NStr("en='Specify at least one field to display';ru='Следует указать хотя бы одно поле для отображения';vi='Cần chỉ ra ít nhất một trường để hiển thị'");
	...
	ElsIf UserFields.Count() > MaximumQuantityOfCustomFields() Then
		...
		MessageString = NStr("en='Reduce the number of fields (select no more than %1 fields)';ru='Уменьшите количество полей (можно выбирать не более %1 полей)';vi='Hãy giảm số lượng trường (có thể chọn không quá %1 trường)'");
...
Procedure SetConnectionInteractively()
	...
	If Not (
		...
		ShowMessageBox(, NStr("en='Objects are already mapped';ru='Объекты уже сопоставлены';vi='Đối tượng đã được so sánh'"), 2);
	...
	Buttons.Add(DialogReturnCode.Yes,     NStr("en='Set';ru='Установить';vi='Thiết lập'"));
	...
	QuestionText = NStr("en='Map the selected objects?';ru='Установить соответствие между выбранными объектами?';vi='Đặt so sánh giữa các đối tượng đã chọn?'");
...
Function GetObjectToMap(Data)
	...
	If Result.Count() = 0 Then
		...
		Result.Add(NStr("en='<not specified>';ru='<не задан>';vi='<chưa chỉ ra>'"));
...
Procedure PerformMappingOfObjects(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Object mapping';ru='Сопоставление объектов';vi='Sự tương ứng đối tượng'")
		);
	...
	Except
		Cancel = True;
		WriteLogEvent(NStr("en='Object mapping wizard.Data analysis';ru='Помощник сопоставления объектов.Анализ данных';vi='Bộ trợ giúp so sánh đối tượng. Phân tích dữ liệu'", CommonUseClientServer.MainLanguageCode()),
