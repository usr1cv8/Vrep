...
Procedure Apply(Command)
	...
	If MarkedListItemArray.Count() = 0 Then
		...
		NString = NStr("en='Specify at least one field';ru='Следует указать хотя бы одно поле';vi='Cần chỉ ra ít nhất một trường'");
	...
	ElsIf MarkedListItemArray.Count() > MaximumQuantityOfCustomFields() Then
		...
		MessageString = NStr("en='Reduce the number of fields (select no more than [FieldQuantity] fields)';ru='Уменьшите количество полей (можно выбирать не более [КоличествоПолей] полей)';vi='Hãy giảm số lượng trường (có thể chọn không quá [FieldQuantity] trường)'");
