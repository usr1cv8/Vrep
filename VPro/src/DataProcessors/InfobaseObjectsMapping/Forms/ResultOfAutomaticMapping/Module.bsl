...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Parameters.Property("ExchangeMessageFileName") Then
		...
		NString = NStr("en='The form cannot be opened interactively.';ru='Форма не может быть открыта интерактивно.';vi='Biểu mẫu không thể mở trực tác.'");
...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	If  Object.TableOfAutomaticallyMappedObjects.Count() > 0
		...
		ShowMessageBox(, NStr("en='The form contains automatic mapping data. The action is canceled.';ru='Форма содержит данные автоматического сопоставления. Действие отменено.';vi='Biểu mẫu có dữ liệu so sánh tự động. Thao tác bị hủy bỏ.'"));
...
Procedure PerformMappingOfObjects(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Automatic object mapping';ru='Автоматическое сопоставление объектов';vi='Tự động so sánh đối tượng'")
		);
	...
	Except
		Cancel = True;
		WriteLogEvent(NStr("en='Object mapping wizard.Automatic mapping';ru='Помощник сопоставления объектов.Автоматическое сопоставление';vi='Bộ trợ giúp so sánh đối tượng. Tự động so sánh'", CommonUseClientServer.MainLanguageCode()),
