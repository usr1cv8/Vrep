...
Function ResultMappingObjects(Parameters) Export
	...
	If Parameters.FormAttributes.OnlyApplyTableUnapprovedRecords Then
		...
		If Cancel Then
			Raise NStr("en='Errors occurred when mapping objects.';ru='Возникли ошибки в процессе сопоставления объектов.';vi='Đã xuất hiện lỗi trong quá trình so sánh các đối tượng.'");
		EndIf;
...
Function ResultOfAutomaticMappingObjects(Parameters) Export
	...
	If Cancel Then
		Raise NStr("en='Errors occurred when mapping objects automatically.';ru='Возникли ошибки в процессе автоматического сопоставления объектов.';vi='Đã xuất hiện lỗi trong quá trình tự động so sánh các đối tượng.'");
	EndIf;
