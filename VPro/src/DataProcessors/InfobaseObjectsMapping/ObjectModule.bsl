...
Procedure ExecuteDataImportToInformationBase(Cancel, TableToImport) Export
	...
	If DataExchangeDataProcessor.ErrorFlag() Then
		NString = NStr("en='Errors occurred when importing exchange message: %1';ru='При загрузке сообщения обмена возникли ошибки: %1';vi='Khi kết nhập thông tin trao đổi có phát sinh lỗi: %1'");
	...
	For Each Item IN TableFieldList Do
		...
		If Attribute = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Attribute with the ""%2"" name is not defined for the ""%1"" metadata object';ru='Для объекта метаданных ""%1"" не определен реквизит с именем ""%2""';vi='Đối với đối tượng metadata ""%1"" chưa xác định mục tin có tên ""%2""'"),
