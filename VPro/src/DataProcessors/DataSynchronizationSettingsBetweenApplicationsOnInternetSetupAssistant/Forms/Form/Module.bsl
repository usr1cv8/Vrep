...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not DataExchangeSaaSReUse.DataSynchronizationSupported() Then
		...
		Raise NStr("en='Data synchronization for this configuration is not supported.';ru='Синхронизация данных для конфигурации не поддерживается!';vi='Không hỗ trợ đồng bộ hóa dữ liệu đối với cấu hình!'");
	...
	FilterSettingsAtNode = DataExchangeServer.FilterSettingsAtNode(ExchangePlanName, CorrespondentVersion);
	...
	Title = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Setting of data synchronization with ""%1""';ru='Настройка синхронизации данных с ""%1""';vi='Tùy chỉnh đồng bộ hóa dữ liệu với ""%1""'"), CorrespondentDescription);
	Items.DataSynchronizationDescription.Title = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Description of data synchronization with ""%1""';ru='Описание синхронизации данных с ""%1""';vi='Mô tả đồng bộ hóa dữ liệu với ""%1""'"), CorrespondentDescription);
...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	WarningText = NStr("en='Cancel data synchronization setting?';ru='Отменить настройку синхронизации данных?';vi='Hủy tùy chỉnh đồng bộ hóa dữ liệu?'");
...
Procedure ShowMessageAboutError(Cancel)
	...
	CommonUseClientServer.MessageToUser(NStr("en='Cannot execute the operation.';ru='Не удалось выполнить операцию.';vi='Không thể thực hiện giao dịch.'"),,,, Cancel);
...
Function Attachable_Begin_OnGoingNext(Cancel)
	...
	If NodeFiltersSettingsAvailable Then
		...
		If Not SettingsForm.CheckFilling() Then
			...
			CommonUseClientServer.MessageToUser(NStr("en='Configure required settings.';ru='Необходимо задать обязательные настройки.';vi='Không thể chỉ ra tùy chỉnh bắt buộc.'"),,, "DataExportSettingsDescription", Cancel);
...
Procedure DataAnalysisExpectation_LongOperationProcessing(Cancel)
	...
	Try
		...
		MethodParameters.Insert("FilterSettingsAtNode", NodesSettingFormContext);
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Setting of data synchronization between online applications (step 1)';ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 1)';vi='Tùy chỉnh đồng bộ hóa dữ liệu giữa các ứng dụng trên mạng Internet (bước 1)'"));
...
Procedure RunAutomaticDataMapping(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Setting of data synchronization between online applications (automatic data mapping)';ru='Настройка синхронизации данных между приложениями в сети Интернет (автоматическое сопоставление данных)';vi='Tùy chỉnh đồng bộ hóa dữ liệu giữa các ứng dụng trên mạng Internet (tự động so sánh dữ liệu)'"));
...
Procedure GetStatsComparison(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Setting of data synchronization between online applications (mapping statistics receipt)';ru='Настройка синхронизации данных между приложениями в сети Интернет (получение статистики сопоставления)';vi='Tùy chỉnh đồng bộ hóa dữ liệu giữa các ứng dụng trên mạng Internet (nhận thống kê so sánh)'"));
...
Procedure DataSynchronizationExpectation_LongOperationProcessing(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Setting of data synchronization between online applications (catalog synchronization)';ru='Настройка синхронизации данных между приложениями в сети Интернет (синхронизация справочников)';vi='Tùy chỉnh đồng bộ hóa dữ liệu giữa các ứng dụng trên mạng Internet (đồng bộ hóa danh mục)'"));
...
Procedure ImportParametersAccountingCorrespondent(Cancel, ErrorInfo = "", CorrespondentErrorMessage = "")
	...
	Try
		...
		If IsBlankString(CorrespondentErrorMessage) Then
			CorrespondentErrorMessage = NStr("en='Accounting parameters are not specified in the ""%1"" application.';ru='Не заданы параметры учета в приложении ""%1"".';vi='Chưa đặt tham số kế toán trong ứng dụng ""%1"".'");
		...
		If IsBlankString(ErrorInfo) Then
			ErrorInfo = NStr("en='Accounting parameters are not specified in this application.';ru='Не заданы параметры учета в этом приложении.';vi='Chưa đặt tham số kế toán trong ứng dụng này.'");
		EndIf;
...
Function Attachable_DataReceivingRules_OnGoingNext(Cancel)
	...
	If NodeDefaultValuesAvailable Then
		...
		If Not SettingsForm.CheckFilling() Then
			...
			CommonUseClientServer.MessageToUser(NStr("en='Configure required settings.';ru='Необходимо задать обязательные настройки.';vi='Không thể chỉ ra tùy chỉnh bắt buộc.'"),,, "ValuesDescriptionFullByDefault", Cancel);
	...
	If CorrespondentInfobaseNodeDefaultValuesAvailable Then
		...
		If Not SettingsForm.CheckFilling() Then
			...
			CommonUseClientServer.MessageToUser(NStr("en='Specify required settings for the second application.';ru='Необходимо задать обязательные настройки для второго приложения.';vi='Cần đặt tùy chỉnh bắt buộc đối với ứng dụng thứ hai.'")
...
Procedure SettingsSaveExpected_LongOperationProcessing(Cancel)
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			MethodParameters,
			NStr("en='Setting of data synchronization between online applications (step 2)';ru='Настройка синхронизации данных между приложениями в сети Интернет (шаг 2)';vi='Tùy chỉnh đồng bộ hóa dữ liệu giữa các ứng dụng trên mạng Internet (bước 2)'"));
