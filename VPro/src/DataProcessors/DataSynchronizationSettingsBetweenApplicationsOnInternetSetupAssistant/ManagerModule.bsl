...
Procedure SetExchangeStep1(Parameters, TemporaryStorageAddress) Export
	...
	FilterSettingsAtNode = Parameters.FilterSettingsAtNode;
	...
	Try
		...
		DataExchangeSaaS.CreateExchangeSetting(
			...
			FilterSettingsAtNode.FilterSettingsAtNode,
			Correspondent,
			,
			,
			Prefix);
		...
		ExchangePlans[ExchangePlanName].OnDataSendingSender(FilterSettingsAtNode.FilterSettingsAtNode, False);
		...
		Message.Body.FilterSettings = XDTOSerializer.WriteXDTO(FilterSettingsAtNode.CorrespondentInfobaseNodeFilterSetup);
...
Procedure RunAutomaticDataMapping(Parameters, TemporaryStorageAddress) Export
	...
	If Cancel Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Errors occurred while receiving an exchange message from the external resource for the ""%1"" correspondent.';ru='Возникли ошибки при получении сообщения обмена из внешнего ресурса для корреспондента ""%1"".';vi='Xuất hiện lỗi khi nhận thông báo trao đổi từ nguồn ngoài đối với cơ sở thông tin thứ hai ""%1"".'"),
	...
	If Cancel Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Errors occurred while analyzing exchange message for the ""%1"" correspondent.';ru='Возникли ошибки при анализе сообщения обмена для корреспондента ""%1"".';vi='Đã xuất hiện lỗi khi phân tích thông điệp trao đổi đối với đối ứng ""%1"".'"),
			String(Correspondent));
	EndIf;
	...
	If Cancel Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Errors occurred while automatically mapping data received from the ""%1"" correspondent.';ru='Возникли ошибки при выполнении автоматического сопоставления данных, полученных от корреспондента ""%1"".';vi='Đã xuất hiện lỗi khi thực hiện so sánh tự động dữ liệu đã nhận từ đối ứng ""%1"".'"),
			String(Correspondent));
	EndIf;
...
Procedure SynchronizeCatalogs(Parameters, TemporaryStorageAddress) Export
	...
	If Cancel Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Errors occurred while importing catalogs from the correspondent %1.';ru='Возникли ошибки в процессе загрузки справочников от корреспондента %1.';vi='Đã xảy ra lỗi trong quá trình kết nhập danh mục từ dữ liệu trao đổi tin %1.'"),
			String(Correspondent));
	EndIf;
	...
	If Cancel Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Errors occurred while exporting catalogs for the correspondent %1.';ru='Возникли ошибки в процессе выгрузки справочников для корреспондента %1.';vi='Đã xảy ra lỗi trong quá trình kết xuất danh mục đối với dữ liệu trao đổi tin %1.'"),
			String(Correspondent));
	EndIf;
...
Procedure GetStatsComparison(Parameters, TemporaryStorageAddress) Export
	...
	If Cancel Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Errors occurred while receiving statistics information for the ""%1"" correspondent.';ru='Возникли ошибки при получении информации статистики для корреспондента ""%1"".';vi='Xuất hiện lỗi khi nhận thông tin thống kê đối với cơ sở thông tin thứ hai ""%1"".'"),
