...
Procedure DeletionMarkedObjects(ExecuteParameters)
	...
	If Not Users.InfobaseUserWithFullAccess() Then
		Raise NStr("en='Insufficient rights to perform the operation.';ru='Недостаточно прав для выполнения операции.';vi='Không đủ quyền để thực hiện thao tác.'");
	EndIf;
...
Procedure TryToDeleteReference(ExecuteParameters, Ref, Information, Result)
	...
	If Object.DeletionMark <> True Then
		Result.Success = False;
		Result.ErrorInfo = NStr("en='Object is not marked for deletion.';ru='Объект не помечен на удаление.';vi='Đối tượng chưa bị đánh dấu xóa.'");
		Return;
	EndIf;
	...
	Else
		Result.Success = False;
		Result.ErrorInfo = NStr("en='Object is in use by other application objects.';ru='Объект используется в других объектах программы.';vi='Đối tượng được sử dụng trong các đối tượng khác của chương trình.'");
	EndIf;
EndProcedure
...
Procedure WriteWarning(Ref, ErrorInfo)
	...
	WriteLogEvent(
		NStr("en='Deletion of marked objects';ru='Удаление помеченных';vi='Xóa bỏ các đối tượng có đánh dấu'", CommonUseClientServer.MainLanguageCode()),
	...
	If TableRow.FoundReference = Undefined Then
		...
		Else
			Cause.FoundReference = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Incorrect references are detected (%1)';ru='Обнаружены неразрешимые ссылки (%1)';vi='Tìm thấy tham chiếu chưa được phép (%1)'"),
...
Procedure ClientMarkCollectionBypassProgress(ExecuteParameters, CollectionName)
	...
	If CollectionName = "BeforeSearchingMarkedToDelete" Then
		...
		Text = NStr("en='Preparation to search for the objects marked for deletion.';ru='Подготовка к поиску объектов, помеченных на удаление.';vi='Chuẩn bị tìm kiếm dữ liệu có đánh dấu xóa.'");
		
	ElsIf CollectionName = "FindMarkedForDeletion" Then
		
		Text = NStr("en='Search for objects marked for deletion.';ru='Поиск объектов, помеченных на удаление.';vi='Tìm kiếm các đối tượng có đánh dấu xóa.'");
		
	ElsIf CollectionName = "AllMarkedForDeletion" Then
		
		Text = NStr("en='Analysis of marked for deletion.';ru='Анализ помеченных на удаление.';vi='Phân tích các mục bị đặt dấu xóa.'");
		
	ElsIf CollectionName = "TechnologicalObjects" Then
		
		Text = NStr("en='Prepare for deletion.';ru='Подготовка к удалению.';vi='Chuẩn bị xóa.'");
		
	ElsIf CollectionName = "ExclusiveDeletion" Then
		
		Text = NStr("en='Removing objects.';ru='Выполняется удаление объектов.';vi='Đang thực hiện xóa đối tượng.'");
		
	ElsIf CollectionName = "CustomObjects" Then
		
		NotRemoved = ExecuteParameters.NotRemoved.Count();
		If NotRemoved = 0 Then
			Pattern = NStr("en='Deleted: %1 of %2 objects.';ru='Удалено: %1 из %2 объектов.';vi='Đã xóa bỏ %1 / %2 đối tượng.'");
		Else
			Pattern = NStr("en='Processed: %1 out of %2 objects; not deleted: %3.';ru='Обработано: %1 из %2 объектов, из них не удалено: %3.';vi='Đã xử lý: %1 / %2 đối tượng, trong số đó chưa xóa bỏ: %3.'");
		EndIf;
	...
	ElsIf CollectionName = "RepeatedlyDeleted" Then
		...
		Text = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Recheck not deleted objects: %1 of %2.';ru='Повторная проверка не удаленных объектов: %1 из %2.';vi='Kiểm tra lại các đối tượng chưa được xóa bỏ: %1 từ %2.'"),
			Format(ExecuteParameters.Number, "NZ=0; NG="),
			Format(ExecuteParameters.Total, "NZ=0; NG="));
	...
	ElsIf CollectionName = "ImpedingRemoval" Then
		...
		Text = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Analysis of objects that prevent deletion: %1 from %2.';ru='Анализ объектов, препятствующих удалению: %1 из %2.';vi='Phân tích các đối tượng mà cản trở việc xóa: %1 từ %2.'"),
			Format(ExecuteParameters.Number, "NZ=0; NG="),
			Format(ExecuteParameters.Total, "NZ=0; NG="));
