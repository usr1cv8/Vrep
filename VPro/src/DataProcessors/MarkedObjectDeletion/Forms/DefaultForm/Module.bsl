...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Users.InfobaseUserWithFullAccess() Then
		ErrorText = NStr("en='Insufficient rights to perform the operation.';ru='Недостаточно прав для выполнения операции.';vi='Không đủ quyền để thực hiện thao tác.'");
		Return; // Denial is set in OnOpen.
	EndIf;
	
	If CommonUseReUse.DataSeparationEnabled()
		AND Not CommonUse.UseSessionSeparator() Then
		ErrorText = NStr("en='To delete marked items, enter data area.';ru='Для удаления помеченных необходимо войти в область данных.';vi='Để xóa bỏ các đối tượng có đánh dấu, cần đăng nhập vào vùng dữ liệu.'");
		Return; // Denial is set in OnOpen.
	EndIf;
...
"Прервать?';vi='Vẫn còn đang thực hiện việc xóa bỏ các đối tượng có đánh dấu."
"Ngừng lại?'");
		Buttons = New ValueList;
		Buttons.Add(DialogReturnCode.Abort);
		Buttons.Add(DialogReturnCode.Ignore, NStr("en='Do not interrupt';ru='Не прерывать';vi='Không ngừng lại'"));
...
Procedure NotRemovedChangeTo(Command)
	...
	If RefArray.Count() = 0 Then
		ShowMessageBox(, NStr("en='Select objects';ru='Выберите объекты';vi='Hãy chọn các đối tượng'"));
		Return;
	EndIf;
...
Procedure BackgroundJobStartOnClient(Action)
	...
	Text = NStr("en='Deleting the objects marked for deletion...';ru='Удаляются объекты, помеченные на удаление...';vi='Đang xóa bỏ các đối tượng có đánh dấu...'");
	If JobParameters.SearchMarked AND JobParameters.DeleteMarked Then
		Status(NStr("en='Search and deletion of marked objects...';ru='Поиск и удаление помеченных объектов...';vi='Tìm kiếm và xóa bỏ các đối tượng có đánh dấu ...'"));
	ElsIf JobParameters.SearchMarked Then
		Text = NStr("en='Search for objects marked for deletion...';ru='Поиск помеченных на удаление объектов...';vi='Tìm kiếm các đối tượng có đặt dấu xóa...'");
		Status(Text);
	Else
		Status(NStr("en='Deleting the selected objects...';ru='Удаление выбранных объектов...';vi='Xóa đối tượng được chọn...'"));
	EndIf;
	...
	ElsIf Result.ErrorWhenInstallingExclusiveMode Then
		...
		Else
			StandardSubsystemsClientServer.DisplayWarning(
				Result,
				NStr("en='Cannot start deletion of the marked objects';ru='Не удалось запустить удаление помеченных объектов';vi='Không thể khởi động xóa đối tượng bị đánh dấu'"),
...
"чтобы узнать причину, по которой его не удалось удалить.';vi='Hãy chọn đối tượng"
"để nhận biết nguyên nhân mà ttheo đó đã không thể xóa đối tượng'");
	Else
		...
		Else
			...
			NotRemovedToolTip = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Usage locations of object ""%1"" (%2):';ru='Места использования объекта ""%1"" (%2):';vi='Nơi sử dụng đối tượng ""%1"" (%2):'"),
...
Procedure MarkSelectedTableObjectsForDeletion(ItemTable)
	...
	If QuantityCanBeDeleted = 0 Then
		ErrorText = NStr("en='Select object.';ru='Выберите объект.';vi='Hãy chọn đối tượng.'");
		If QuantitySelected = 1 Then
			If HasRegisterRecords Then
				ErrorText = NStr("en='Constant value is cleaned up from its card.';ru='Очистка значения константы выполняется из ее карточки.';vi='Đang xóa bỏ giá trị hằng từ thẻ của nó,'");
			ElsIf HasConstants Then
				ErrorText = NStr("en='Constant value is cleaned up from its card.';ru='Очистка значения константы выполняется из ее карточки.';vi='Đang xóa bỏ giá trị hằng từ thẻ của nó,'");
			EndIf;
		Else
			If HasRegisterRecords Or HasConstants Then
				ErrorText = NStr("en='Register record deletion or constant value cleanup is carried out from their cards.';ru='Удаление записей регистров или очистка значений констант выполняется из их карточек.';vi='Đang xóa bỏ bản ghi của biểu ghi hoặc đang xóa bỏ giá trị hằng từ thẻ.'");
			EndIf;
		EndIf;
		ShowMessageBox(, ErrorText);
		Return;
	...
	Else
		If HasMarkedForDeletion Then
			QuestionText = NStr("en='Mark the selected objects (%1) for deletion?';ru='Пометить выделенные объекты (%1) на удаление?';vi='Đặt dấu xóa các đối tượng được chọn (%1)?'");
		Else
			QuestionText = NStr("en='Mark the selected objects (%1) for deletion?';ru='Пометить выделенные объекты (%1) на удаление?';vi='Đặt dấu xóa các đối tượng được chọn (%1)?'");
		EndIf;
...
Procedure VisibleEnabled(Form)
	...
	Items.ButtonNext.Title = NStr("en='< Go home';ru='< В начало';vi='< Về đầu'");
	Items.CloseButton.Title = NStr("en='Close';ru='Закрыть';vi='Đóng'");
	Items.ButtonBack.Title = NStr("en='< Go home';ru='< В начало';vi='< Về đầu'");
	
	If CurrentPage = Items.PageDeleteModeSelection Then
		Items.ButtonBack.Visible = False;
		Items.ButtonNext.Visible = True;
		If Form.DeletionMode <> "Full" Then
			Items.ButtonNext.Title = NStr("en='Next >';ru='Далее  >';vi='Tiếp theo >'");
		EndIf;
		Items.ButtonNext.DefaultButton = True;
		Items.CloseButton.Title = NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'");
	ElsIf CurrentPage = Items.PageMarkedToRemove Then
		Items.ButtonBack.Visible = True;
		Items.ButtonBack.Title = NStr("en='Delete';ru='Удалить';vi='Xóa bỏ'");
		Items.ButtonNext.Visible = True;
		Items.ButtonNext.Title = NStr("en='Delete';ru='Удалить';vi='Xóa bỏ'");
		Items.ButtonNext.DefaultButton = True;
	ElsIf CurrentPage = Items.PageLongOperation Then
		Items.ButtonBack.Visible = False;
		Items.ButtonNext.Visible = False;
		Items.CloseButton.Title = NStr("en='Stop and close';ru='Прервать и закрыть';vi='Ngừng và đóng'");
	ElsIf CurrentPage = Items.PageReasonsRemovingUnavailable Then
		Items.ButtonBack.Visible = True;
		Items.ButtonNext.Visible = True;
		Items.ButtonNext.Title = NStr("en='Repeat deletion';ru='Повторяем удаление';vi='Lặp lại việc xóa'");
		Items.ButtonNext.DefaultButton = True;
...
Function BackGroundJobStart(Val JobParameters)
	...
	BackgroundJobResult = LongActions.ExecuteInBackground(
		...
		JobParameters,
		NStr("en='Deletion of marked objects (interactive)';ru='Удаление помеченных объектов (интерактивное)';vi='Xóa bỏ các đối tượng có đánh dấu (trực tác)'"));
...
Function ChangeObjectsDeletionMark(RefArray, DeletionMark)
	...
	If ObjectCount = 0 Then
		NotificationTitle = NStr("en='Object is not found';ru='Объект не найден';vi='Không tìm thấy đối tượng'");
	Else
		If DeletionMark Then
			NotificationTitle = NStr("en='Deletion mark is cleared';ru='Пометка удаления снята';vi='Hủy dấu xóa'");
		Else
			NotificationTitle = NStr("en='Deletion mark is cleared';ru='Пометка удаления снята';vi='Hủy dấu xóa'");
		EndIf;
...
Procedure BackgroundJobShowLongOperationPage(JobParameters)
	...
	If ShowDonut Then
		Items.LabelLongAction.Title = NStr("en='Please wait...';ru='Пожалуйста, подождите...';vi='Xin vui lòng đợi...'");
		Items.BackgroundOrderStatus.HorizontalAlign = ItemHorizontalLocation.Left;
	ElsIf JobParameters.DeleteMarked Then
		Items.LabelLongAction.Title = NStr("en='Deleting the objects marked for deletion...';ru='Удаляются объекты, помеченные на удаление...';vi='Đang xóa bỏ các đối tượng có đánh dấu...'");
		Items.BackgroundOrderStatus.HorizontalAlign = ItemHorizontalLocation.Center;
	Else
		Items.LabelLongAction.Title = NStr("en='Search for objects marked for deletion...';ru='Поиск объектов, помеченных на удаление...';vi='Tìm kiếm các đối tượng có đánh dấu xóa...'");
		Items.BackgroundOrderStatus.HorizontalAlign = ItemHorizontalLocation.Center;
	EndIf;
EndProcedure
...
"Удалено объектов: %1.';vi='Xóa bỏ các đối tượng có đánh dấu kết thúc thành công."
"Đã xóa bỏ đối tượng: %1.'"),
...
"Удалено"
"объектов: %1, Не удалено: %2.';vi='Đã hoàn tất việc xóa các đối tượng đã đánh dấu."
"Đã xóa"
"đối tượng: %1, Chưa xóa: %2."
"'"),
				Format(DeletedQuantity, "NZ=0; NG="),
			...
			If DeletedQuantity = 0 Then
				Items.LabelResultPartialRemoval.Title = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Cannot delete objects marked for deletion (%1):';ru='Не получилось удалить объекты, помеченные на удаление (%1):';vi='Chưa xóa bỏ đối tượng có đánh dấu (%1):'"),
					Format(NOTRemovedQuantity, "NZ=0; NG=")
				);
			Else
				Items.LabelResultPartialRemoval.Title = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Successfully deleted: %1 from %2, other objects are not deleted (%3):';ru='Успешно удалено: %1 из %2, остальные объекты не удалены (%3):';vi='Đã xóa bỏ thành công: %1 / %2, các đối tượng còn lại không bị xóa bỏ (%3):'"),
					Format(DeletedQuantity, "NZ=0; NG="),
		...
		If NotificationText <> Undefined Then
			StandardSubsystemsClientServer.DisplayNotification(
				Result,
				NStr("en='Deletion of marked objects';ru='Удаление помеченных';vi='Xóa bỏ các đối tượng có đánh dấu'"),
				NotificationText,
				NotificationPicture,
				URL);
		EndIf;
	...
	For Each Cause IN ImpedingRemoval Do
		...
		If Not RemovalObstacleString.IsError Then
			...
			If Cause.FoundReference = Undefined Then // Constant
				...
				RemovalObstacleString.Presentation = DetectedInformation.ItemPresentation + " (" + NStr("en='Constant';ru='Константа';vi='Hằng'") + ")";
			Else
