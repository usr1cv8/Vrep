...
Procedure Initialize(Container, RefReplacementStream, Handlers) Export
	...
	If CurrentSourceRefsColumnName = Undefined Or IsBlankString(CurrentSourceRefsColumnName) Then 
		Raise NStr("en='Column name with the original reference is not found';ru='Не найдено имя колонки с исходной ссылкой';vi='Không tìm thấy tên cột có tham chiếu gốc'");
	EndIf;
