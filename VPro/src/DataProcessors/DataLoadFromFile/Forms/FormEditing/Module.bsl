...
Procedure ResetSettings(Command)
	...
	ShowQueryBox(Notification, NStr("en='Set the column settings back to their original state?';ru='Установить настройки колонок в первоначальное состояние?';vi='Đặt tùy chỉnh cột trong trạng thái đầu?'"), QuestionDialogMode.YesNo);
EndProcedure
...
Procedure SetDataDesign()
	...
	FilterItem.Use = True;
	ConditionalAppearanceItem.Appearance.SetParameterValue("Text", NStr("en='Standard name';ru='Стандартное наименование';vi='Tên gọi chuẩn'"));
