...
Procedure InitializeSearchRefsMode(TemplateWithData, InformationByColumns, TypeDescription) Export
	...
	For Each Type IN TypeDescription.Types() Do 
		...
		ColumnsTitle = NStr("en='Entered data';ru='Введенные данные';vi='Dữ liệu đã nhập'");
...
Procedure InformationOnColumnsFromCatalogAttributes(ImportParameters, InformationByColumns) Export
	...
	For Each Attribute IN CatalogMetadata.Attributes Do
		...
		If Attribute.Type.ContainsType(Type("Boolean")) Then 
			ColumnTypeDescription = NStr("en='Check box, Yes or 1 / No or 0';ru='Флаг, Да или 1 / Нет или 0';vi='Hộp kiểm, Đồng ý hoặc 1 / Không hoặc 0'");
		ElsIf Attribute.Type.ContainsType(Type("Number")) Then 
			ColumnTypeDescription =  StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Digit, Length: %1, Accuracy: %2';ru='Число, Длина: %1, Точность: %2';vi='Số, Chiều dài: %1, Độ chính xác: %2'"),
		...
		ElsIf Attribute.Type.ContainsType(Type("String")) Then
			If Attribute.Type.StringQualifiers.Length > 0 Then
				...
				ColumnTypeDescription =  StringFunctionsClientServer.SubstituteParametersInString(NStr("en='String, maximum characters: %1';ru='Строка, макс. количество символов: %1';vi='Dòng, số lượng ký tự tối đa: %1'"), StringLength);
			Else
				ColumnTypeDescription = NStr("en='String of unlimited length';ru='Строка неограниченной длины';vi='Dòng có chiều dài không hạn chế'");
			EndIf;
		ElsIf Attribute.Type.ContainsType(Type("Date")) Then
			ColumnTypeDescription =  StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='%1';ru='%1';vi='%1'"),String(Attribute.Type.DateQualifiers.DateFractions));
		ElsIf Attribute.Type.ContainsType(Type("UUID")) Then 
			ColumnTypeDescription = NStr("en='UUID';ru='UUID';vi='UUID'");
		EndIf;
...
Procedure CreateColumnStandardAttributes(InformationByColumns, CatalogMetadata, ColumnName, Position)
	...
	ElsIf DataType = Type("Number") Then	
		TypePresentation = NStr("en='Number';ru='Номер';vi='STT'");
	Else
...
Procedure InitializeImportToTabularSection(TabularSectionFullName, TemplateNameWithTemplate, InformationByColumns, TemplateWithData, AdditionalParameters, Cancel) Export
	...
	If StructureObjectName.ObjectType = "Document" Then
		...
		Else
			Raise NStr("en='Template for data import from file is not found';ru='Не найден макет для загрузки данных из файла';vi='Không tìm thấy khuôn in để kết nhập dữ liệu từ tệp'");
			Cancel = True;
			Return;
		EndIf;
...
Procedure ImportCSVFileToTable(FileName, TemplateWithData, InformationByColumns) Export
	...
	If String = Undefined Then 
		MessageText = NStr("en='Cannot import data from this file. Make sure that data in the file is correct.';ru='Не получилось загрузить данные из этого файла. Убедитесь в корректности данных в файле.';vi='Không thể kết nhập dữ liệu từ tệp này. Hãy chắc chắn dữ liệu trong tệp là đúng.'");
		Return;
	EndIf;
...
Procedure WriteMatchedData(ExportParameters, StorageAddress) Export
	...
	For Each TableRow IN MatchedData Do 
		...
		Try
			...
			Else
				...
				If CatalogItem = Undefined Then
					...
					NStr("en='No products and services with SKU %1.';ru='Номенклатура с артикулом %1 не существует.';vi='Không có mặt hàng với nhãn hiệu %1.'"), TableRow.SKU);
					Raise MessageText;
				EndIf;
			EndIf;
		...
		Except
			RollbackTransaction();
			TableRow.RowMatchResult = "Skipped";
			TableRow.ErrorDescription = NStr("en='Cannot write due to incorrect data';ru='Невозможна запись из-за некорректности данных';vi='Không thể ghi lại do sai dữ liệu'");
		EndTry;
...
Procedure GenerateReportTemplate(TableReport, TemplateWithData)
	...
	FillCellTemplateTitle(Cell, NStr("en='Result';ru='Результат';vi='Kết quả'"), 12, NStr("en='Data import result';ru='Результат загрузки данных';vi='Kết quả kết nhập dữ liệu'"), True);
