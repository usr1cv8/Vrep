...
Procedure RequirmentSelect(Item, SelectedRow, Field, StandardProcessing)
	...
	If FieldName <> "DemandVendor" 
		AND FieldName <> "DemandProductsAndServices"
...
Procedure ShowZeroSales(Command)
	...
	If Demand.GetItems().Count() > 0 Then
		QuestionText = NStr("en='Tabular section will be filled in again. Continue?';ru='Табличная часть будет перезаполнена. Продолжить?';vi='Phần bảng sẽ được điền lại. Tiếp tục?'");
		Response = Undefined;
...
Procedure GenerateOrders(Command)
	...
	If OrderSetServer() Then
		...
		ShowUserNotification(
			,,
			NStr("en='Purchase orders are successfully created.';ru='Заказы поставщикам успешно созданы.';vi='Đã tạo thành công đơn hàng đặt nhà cung cấp.'"),
	...
	Else
		...
		Message.Text = NStr("en='No data to generate orders.';ru='Нет данных для формирования заказов.';vi='Không có dữ liệu để tạo đơn hàng.'");
		Message.Message();
...
Procedure DeleteOrder(Command)
	...
	ElsIf OrdersForDeleting.Count() = 1 Then
		...
		QuestionText = NStr("en='Selected document will be deleted. Continue?';ru='Выбранный документ будет удален. Продолжить?';vi='Chứng từ đã chọn sẽ bị xóa. Tiếp tục?'");
		
	Else
		
		QuestionText = NStr("en='Selected documents will be deleted. Continue?';ru='Выбранные документы будут удалены. Продолжить?';vi='Chứng từ đã chọn sẽ bị xóa. Tiếp tục?'");
...
Function OrderSetServer()
	...
	Try
		...
		For Each SuplierString IN RequirementTree.Rows Do
			...
			DocumentObject.Comment = NStr("en='Automatically created using the ""Goods demand calculation"" data processor';ru='Создан автоматически обработкой ""Расчет потребности товаров""';vi='Đã tạo tự động bộ xử lý ""Tính toán chi tiết hàng hóa""'");
...
"Дополнительное"
"описание: %ДополнительноеОписание%';vi='Khi tạo các đơn hàng đã xảy ra lỗi."
"Đã hủy bỏ việc tạo các đơn hàng."
"Mô tả bổ sung:"
"%AdditionalDetails%'"
		);
	...
	For Each TableRow IN Orders Do
		...
		Else
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Cannot post the %1 document.';ru='Не удалось провести документ: %1.';vi='Không thể kết chuyển chứng từ: %1.'"), String(DocumentObject));
...
Procedure OrdersDeleteServer()
	...
	For Each TableRow IN Orders Do
		...
		Except
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Cannot mark the document for deletion: %1.';ru='Не удалось пометить на удаление документ: %1.';vi='Không thể đánh dấu xóa chứng từ: %1.'"), String(DocumentObject));
