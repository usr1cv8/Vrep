...
Procedure UpdateChoiceListReplenishmentMethod()
	...
	If PurchasesAvailable Then
		If GetFunctionalOption("TransferRawMaterialsForProcessing") AND IsInRole("AddChangeProcessingSubsystem") Then
			Items.FilterReplenishmentMethod.ChoiceList.Add("Purchase and processing", NStr("ru = 'Закупка и переработка';
																							|vi = 'Mua hàng và gia công';
																							|en = 'Purchase and processing'"));
		Else
			Items.FilterReplenishmentMethod.ChoiceList.Add("Purchase", NStr("ru = 'Закупка';
																			|vi = 'Mua hàng';
																			|en = 'Purchase'"));
		EndIf;
	EndIf;
	...
	If AvailableProduction Then
		Items.FilterReplenishmentMethod.ChoiceList.Add("Production", NStr("ru = 'Производство';
																			|vi = 'Sản xuất';
																			|en = 'Production'"));
		If PurchasesAvailable Then
			Items.FilterReplenishmentMethod.ChoiceList.Add("All", NStr("ru = 'Все';
																		|vi = 'Tất cả';
																		|en = 'All'"));
		EndIf;
	EndIf;
...
Procedure RefreshData(QueryResult, StartDate, EndDate)
	...
	FillTableDemandBalance();
	
EndProcedure // UpdateData()

&AtServer
Procedure FillTableDemandBalance()
	
	If NOT FOInventoryReservation Then
		Return;
	EndIf; 
	
	Query = Новый Запрос;
	Query.SetParameter("DemandBalance", DemandBalance.Unload());
	Query.Text =
	"SELECT
	|	DemandBalance.ProductsAndServices AS ProductsAndServices,
	|	DemandBalance.Characteristic AS Characteristic,
	|	DemandBalance.CustomerOrder AS CustomerOrder
	|INTO DemandBalance
	|FROM
	|	&DemandBalance AS DemandBalance
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|SELECT
	...
	|	SUM(NestedQuery.Balance) AS Balance
	|FROM
	|	(SELECT
	|		ЗаказыПокупателейОстатки.CustomerOrder AS CustomerOrder,
	|		ЗаказыПокупателейОстатки.ProductsAndServices AS ProductsAndServices,
	|		ЗаказыПокупателейОстатки.Characteristic AS Characteristic,
	|		ЗаказыПокупателейОстатки.QuantityBalance AS Balance
	|	FROM
	|		AccumulationRegister.CustomerOrders.Balance(
	|				,
	|				(CustomerOrder, ProductsAndServices, Characteristic) IN
	|					(SELECT
	|						DemandBalance.CustomerOrder,
	|						DemandBalance.ProductsAndServices,
	|						DemandBalance.Characteristic
	|					FROM
	|						DemandBalance)) AS ЗаказыПокупателейОстатки
	|	
	|	UNION ALL
	|	
	|	SELECT
	|		ПотребностьВЗапасахОстатки.CustomerOrder,
	|		ПотребностьВЗапасахОстатки.ProductsAndServices,
	|		ПотребностьВЗапасахОстатки.Characteristic,
	|		ПотребностьВЗапасахОстатки.QuantityBalance
	|	FROM
	|		AccumulationRegister.InventoryDemand.Balance(
	|				,
	|				MovementType = VALUE(Enum.InventoryMovementTypes.Shipment)
	|					AND (CustomerOrder, ProductsAndServices, Characteristic) IN
	|						(SELECT
	|							DemandBalance.CustomerOrder,
	|							DemandBalance.ProductsAndServices,
	|							DemandBalance.Characteristic
	|						FROM
	|							DemandBalance)) AS ПотребностьВЗапасахОстатки
	|	
	|	UNION ALL
	|	
	|	SELECT
	|		РазмещениеЗаказовОстатки.CustomerOrder,
	|		РазмещениеЗаказовОстатки.ProductsAndServices,
	|		РазмещениеЗаказовОстатки.Characteristic,
	|		-РазмещениеЗаказовОстатки.QuantityBalance
	|	FROM
	...
	|						DemandBalance.Characteristic
	|					FROM
	|						DemandBalance)) AS РазмещениеЗаказовОстатки) AS NestedQuery
	|
	|GROUP BY
	...
	|
	|HAVING
	|	SUM(NestedQuery.Balance) > 0";
	
	DemandBalance.Load(Query.Execute().Unload());
...
Procedure AddProductsAndServicesCharacteristic(ProductsAndServicesCurrent, StructureDetailing, StructureDetails, StartDate, EndDate)
	...
	If OnlyDeficit AND StructureDetailing.Deficit.IndicatorValue > 0 
		...
		OpeningBalance.ProductsAndServices = NStr("ru = 'Начальные остатки по имуществу';
													|vi = 'Dư đầu';
													|en = 'Opening property balance'");
		
		Receipt = ProductsAndServicesItems.Add();
		Receipt.ProductsAndServices = NStr("ru = 'Приход';
											|vi = 'Tiếp nhận';
											|en = 'Receipt'");
		
		Demand = ProductsAndServicesItems.Add();
		Demand.ProductsAndServices = NStr("ru = 'Потребность';
											|vi = 'Nhu cầu';
											|en = 'Demand'");
		...
		Else
			...
			MinInventory.ProductsAndServices = NStr("ru = 'Минимальный запас';
													|vi = 'Vật tư tối thiểu';
													|en = 'Minimum inventory'");
			
			MaxInventory = ProductsAndServicesItems.Add();
			MaxInventory.ProductsAndServices = NStr("ru = 'Максимальный запас';
													|vi = 'Vật tư tối đa';
													|en = 'Maximum inventory'");
			
		EndIf;	
		
		ClosingBalance = ProductsAndServicesItems.Add();
		ClosingBalance.ProductsAndServices = NStr("ru = 'Сальдо на конец периода составило';
													|vi = 'Dư cuối';
													|en = 'Closing balance'");
...
Procedure UpdateRecommendationsAtServer()
		...
		If ProductsAndServices.Deficit > 0 Then
			...
			While BegOfDay(CurrentPeriod) <= BegOfDay(EndOfPeriod) Do
				...
				If ProductsAndServices[ColumnName] > 0 OR ProductsAndServices.Overdue > 0 AND CurrentPeriod = BegOfDay(CurrentDate()) Then
					...
					If FOInventoryReservation И ProductsAndServices.Rows.Количество() > 2 Then
						DemandRows = ProductsAndServices.Rows[2];
						Для каждого DetailRow Из DemandRows.Rows Цикл
							If ТипЗнч(DetailRow.ProductsAndServices) <> Тип("DocumentRef.CustomerOrder") 
								ИЛИ NOT ЗначениеЗаполнено(DetailRow.ProductsAndServices) Then
								Продолжить;
							EndIf;
							Deficit = ?(CurrentPeriod = НачалоДня(CurrentSessionDate()), DetailRow.Overdue, DetailRow[ColumnName]);
							If Deficit = 0 Then
								Продолжить;
							EndIf; 
							Demand.Вставить(DetailRow.ProductsAndServices, Deficit);
						КонецЦикла; 
					EndIf; 
					...
					DemandSource.Вставить(RowIndex, Demand);
					RowIndex = RowIndex + 1;
				EndIf;
	...
	While Selection.Next() Do
		...
		If RowCurrentIndex <> Selection.RowIndex Then
			...
			CurrentDemand = DemandSource.Получить(RowCurrentIndex);
			If ТипЗнч(CurrentDemand)=Тип("Соответствие") И FOInventoryReservation Then
				NewProductsAndServices.Orders = Новый ФиксированноеСоответствие(CurrentDemand);
			Else
				NewProductsAndServices.Orders = Новый ФиксированноеСоответствие(Новый Map);
			EndIf;
...
Procedure GenerateOrdersAtServer()
	...
	TableOrders.Columns.Add("CustomerOrders");
	...
	For Each RecommendationRow in RecommendationsProductsAndServices Do
		...
		For Each StringProductsAndServices in RecommendationsItems Do
			...
			If StringProductsAndServices.Selected Then
				...
				NewRow.CustomerOrders = RecommendationRow.Orders;
	...
	DemandTable = DemandBalance.Unload();
	DemandTable.Indexes.Add("ProductsAndServices, Characteristic, CustomerOrder");
	...
	For Each OrderParameters in TableOrders Do
		...
		Else // We will create orders for production.
			...
			For Each ResultRow in SearchResult Do
				...
				Для каждого CustomerOrderData Из ResultRow.CustomerOrders Цикл
					OrderQuantity = Мин(CustomerOrderData.Value, NewRow.Quantity);
					If OrderQuantity<=0 Then
						Продолжить;
					EndIf; 
					СтруктураПоискаПотребность = Новый Структура;
					СтруктураПоискаПотребность.Вставить("ProductsAndServices", NewRow.ProductsAndServices);
					СтруктураПоискаПотребность.Вставить("Characteristic", NewRow.Characteristic);
					СтруктураПоискаПотребность.Вставить("CustomerOrder", CustomerOrderData.Key);
					СтрокиПотребность = DemandTable.НайтиСтроки(СтруктураПоискаПотребность);
					If СтрокиПотребность.Count()=0 Then
						Продолжить;
					EndIf;
					СтрокаПотребность = СтрокиПотребность[0];
					CustomerDemand = СтрокаПотребность.Balance;
					OrderQuantity = Мин(CustomerDemand, OrderQuantity);
					If OrderQuantity<=0 Then
						Продолжить;
					EndIf; 
					
					If NewRow.Quantity =OrderQuantity Then
						NewRow.CustomerOrder = CustomerOrderData.Ключ;
					Else
						СтрокаЗаказ = CurrentOrder.Products.Вставить(CurrentOrder.Products.Индекс(NewRow));
						ЗаполнитьЗначенияСвойств(СтрокаЗаказ, NewRow);
						СтрокаЗаказ.Quantity = OrderQuantity;
						СтрокаЗаказ.CustomerOrder = CustomerOrderData.Key;
						NewRow.Quantity = NewRow.Quantity -OrderQuantity;
					EndIf; 
					СтрокаПотребность.Balance = СтрокаПотребность.Balance - OrderQuantity;
					
					If CurrentOrder.CustomerOrderPosition <> Enums.AttributePositionOnForm.InTabularSection Then
						CurrentOrder.CustomerOrderPosition  = Enums.AttributePositionOnForm.InTabularSection;
					EndIf;
		...
		CurrentOrder.Comment = NStr("ru = 'Сформирован автоматически сервисом ""Расчет потребностей в запасах"".';
									|vi = 'Được lập tự động bằng công cụ ""Tính toán nhu cầu vật tư"".';
									|en = 'Automatically generated by the ""Inventory demand calculation"" service.'");
...
Procedure SetConditionalAppearance(BeginOfPeriod, EndOfPeriod)
	...
	FilterItem.ComparisonType = DataCompositionComparisonType.Equal;
	FilterItem.RightValue = NStr("ru = 'Приход';
								|vi = 'Tiếp nhận';
								|en = 'Receipt'");
	...
	FilterItem.LeftValue = New DataCompositionField("Inventory.ProductsAndServices");
	FilterItem.ComparisonType = DataCompositionComparisonType.Equal;
	FilterItem.RightValue = NStr("ru = 'Потребность';
								|vi = 'Nhu cầu';
								|en = 'Demand'");
...
Procedure Refresh(Command)
	...
	If Not ValueIsFilled(Company) Then
		...
		Message.Text = NStr("ru = 'Не выбрана организация!';
							|vi = 'Chưa chọn doanh nghiệp!';
...
Procedure Reread(Command)
	...
	If ThisForm.Modified Then
		...
		QuestionStr = NStr("ru = 'Данные изменены. Перечитать данные?';
							|vi = 'Đã thay đổi dữ liệu. Đọc lại dữ liệu?';
