...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Items.DataProcessorCurrencyRatesImportProcess.Visible = false; // = RunMode.Local;
...
Procedure AdditionalReportsOnAdministration(Command)
	...
	ParametersForm.Insert("Title", NStr("en='Additional administration reports';ru='Дополнительные отчеты по администрированию';vi='Báo cáo bổ sung về quản trị hệ thống'"));
...
Procedure AdditionalAdministrativeDataProcessors(Command)
	...
	ParametersForm.Insert("WindowOpeningMode", FormWindowOpeningMode.LockOwnerWindow);
	ParametersForm.Insert("Title", NStr("en='Additional data processors for administration';ru='Дополнительные обработки по администрированию';vi='Bộ xử lý bổ sung về quản trị hệ thống'"));
	OpenForm("CommonForm.AdditionalReportsAndDataProcessors", ParametersForm, ThisObject);
EndProcedure
// End StandardSubsystems.AdditionalReportsAndDataProcessors
	...
	If RunMode.ThisIsSystemAdministrator Then
		...
		If AttributePathToData = "DeleteMarkedSchedule" OR AttributePathToData = "DeleteMarkedUse" OR AttributePathToData = "" Then
			...
			Else
				...
				Presentation = NStr("en='<Disabled>';ru='<Отключен>';vi='<Đã ngắt bỏ>'");
...
Procedure UpdateConfigurationUpdateSettings()
	...
	ApplicationTitleUpdates = NStr("en='Auto check for updates is disabled.';ru='Автоматическая проверка обновлений отключена.';vi='Đã ngắt bỏ tự động kiểm tra bản cập nhật.'");
	If ConfigurationUpdateOptions.CheckUpdateExistsOnStart = 2 Then
		ApplicationTitleUpdates = NStr("en='Updates are checked automatically every time the application starts.';ru='Автоматическая проверка обновлений выполняется при каждом запуске программы.';vi='Tự động kiểm tra bản cập nhật mỗi khi khởi động chương trình.'");
	ElsIf ConfigurationUpdateOptions.CheckUpdateExistsOnStart = 1 Then
		ApplicationTitleUpdates = NStr("en='Updates are checked automatically on schedule: %1.';ru='Автоматическая проверка обновлений выполняется по расписанию: %1.';vi='Tự động kiểm tra bản cập nhật theo lịch biểu: %1.'");
