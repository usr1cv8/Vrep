...
Procedure AddIdea(Command)
	...
	If IsBlankString(Description) Then 
		Raise NStr("en='The Name field must not be empty';ru='Поле Наименование не должно быть пустым';vi='Trường Tên gọi không được để trống'");
	EndIf;
	
	If IsBlankString(Subject) Then 
		Raise NStr("en='The Subject field must not be empty';ru='Поле Предмет не должно быть пустым';vi='Trường Chủ đề'");
	EndIf;
