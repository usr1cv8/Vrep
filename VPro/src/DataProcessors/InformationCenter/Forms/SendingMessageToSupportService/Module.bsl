...
Procedure Send(Command)
	...
	If Items.ReplyTo.Visible Then 
		If IsBlankString(AddressForAnswer) Then 
			Raise NStr("en='Enter an email address for response';ru='Необходимо ввести адрес электронной почты для ответа';vi='Cần nhập địa chỉ E-mail để phản hồi'");
		EndIf;
		If Not ParseStringWithPostalAddresses(AddressForAnswer) Then 
			...
			ShowQueryBox(Notification, NStr("en='Email address might be incorrect. Send the email?';ru='Адрес электронной почты возможно введен неверно. Отправить сообщение?';vi='Có thể đã nhập sai địa chỉ E-mail. Gửi thông báo?'"), QuestionDialogMode.YesNo);
			Return;
		EndIf;
	EndIf;
	
	SendMessageServer();
	ShowUserNotification(NStr("en='Message to technical support was sent.';ru='Сообщение в службу поддержки отправлено.';vi='Đã gửi thông báo cho bộ phận hỗ trợ.'"));
...
Procedure FillContentWithTemplate()
	...
	StringCursorPosition = NStr("en='CursorPosition';ru='ПозицияКурсора';vi='CursorPosition'");
...
Procedure PutFileWithAlertExtensionSizeAlert(Size, AdditionalParameters) Export
	...
	If Not TotalFilesSizeIsOptimal(Size) Then 
		WarningText = NStr("en='Cannot add the file. Size of the selected files exceeds the limit of %1 MB';ru='Не удалось добавить файл. Размер выбранных файлов превышает предел в %1 Мб';vi='Không thể thêm tệp. Kích cỡ các tệp đã chọn vượt quá giới hạn trong %1Mb'");
	...
	Status(NStr("en='File is added to the message.';ru='Файл добавляется к сообщению.';vi='Tệp được thêm vào thông báo.'"));
...
Procedure PlaceFilesWithoutExtensionAtServer(StorageAddress, FileNameAndExtension)
	...
	If Not TotalFilesSizeIsOptimal(FileSize) Then 
		WarningText = NStr("en='Size of the selected files exceeds the limit of %1 MB';ru='Размер выбранных файлов превышает предел в %1 Мб';vi='Dung lượng tệp đã chọn vượt quá giới hạn trong %1 Mb'");
		WarningText = StringFunctionsClientServer.SubstituteParametersInString(WarningText, MaximalFileSize);
		ShowMessageToUser(WarningText);
...
Procedure SendMessageServer()
	...
	If IsBlankString(MessageText) Then 
		Raise NStr("en='Text message cannot be empty.';ru='Текст сообщения не может быть пустым.';vi='Nội dung thông báo không được trống.'");
	EndIf;
...
Function CreateFormElementsForAttachedFile()
	...
	For Each SelectedFile IN SelectedFiles Do
		...
		FilePresentation = SelectedFile.FileName + SelectedFile.Extension + " (" + Round(SelectedFile.Size / 1024, 2) + NStr("en=' Kb';ru=' Кб';vi='Kb'") +")";
		...
		FileGroup.Group = ChildFormItemsGroup.HorizontalIfPossible;
		...
		DeleteFileButton.ToolTip = NStr("en='Delete file';ru='Удалить файл';vi='Xóa tệp'");
