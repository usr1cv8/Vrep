...
Procedure FillCorrespondence(DataByCorrespondence)
	...
	For Each InteractionItem IN DataByCorrespondence.Interactions Do 
		...
		NewInteraction.Definition = ?(IsBlankString(InteractionItem.Description), NStr("en='<No text>';ru='<Без текста>';vi='<Không có nội dung>'"), InteractionItem.Description);
		...
		NewInteraction.ExplanationToPicture = ?(InteractionItem.Incoming, NStr("en='incl.';ru='вкл.';vi='đã bật'"), NStr("en='Source.';ru='Источник.';vi='Nguồn.'"));
