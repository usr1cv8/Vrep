...
Procedure Send(Command)
	...
	If SendingResult Then 
		ShowUserNotification(NStr("en='The message was sent.';ru='Сообщение отправлено.';vi='Thông báo đã gửi.'"));
		Close();
	Else
		ClearMessages();
...
"Повторите попытку позже.';vi='Rất tiếc thông báo chưa được gửi."
"Hãy lặp lại thử sau.'"));
	EndIf;
...
Function CheckAttributesFilling()
	...
	Else 
		CommonUseClientServer.MessageToUser(
			NStr("en='The ""Reply to"" field is not filled in';ru='Не заполнено поле Адрес ответа';vi='Chưa điền trường Địa chỉ trả lời'"), ,
			"FromWhom");
	EndIf;
