...
Procedure OnOpen(Cancel)
	...
	If CommonUseClientServer.ThisIsWebClient() Then
		Cancel = True;
		ShowMessageBox(, NStr("en='Text extraction is not supported in Web client.';ru='Извлечение текстов не поддерживается в Веб-клиенте.';vi='Không hỗ trợ lấy ra nội dung trên web-client.'"));
		Return;
	EndIf;
	
	If CommonUseClientServer.IsLinuxClient() Then
		Cancel = True;
		MessageText = NStr("en='Text extraction is not supported on computers running Linux operating system.';ru='Извлечение текстов не поддерживается в клиенте под управлением ОС Linux.';vi='Không hỗ trợ lấy ra nội dung tại Client do hệ điều hành Linux quản lý.'");
...
""
"Обработано файлов: %1.';vi='Đã kết thúc việc trích ra nội dung từ'"),
...
Procedure UpdateOfCountdown()
	...
	MessageText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='%1 sec before text extraction start';ru='До начала извлечения текстов осталось %1 сек';vi='Trước khi bắt đầu lấy ra nội dung còn %1 giây'"),
		Left);
...
Procedure ExtractionOfTextsClient(PortionSize = Undefined)
	...
	Status(NStr("en='Text extraction started';ru='Начато извлечение текста';vi='Bắt đầu lấy ra nội dung'"));
	...
	Try
		...
		If FilesArray.Count() = 0 Then
			Status(NStr("en='There are no files to extract the text';ru='Нет файлов для извлечения текста';vi='Không có tệp để lấy ra nội dung'"));
			Return;
		EndIf; 
		...
		For IndexOf = 0 To FilesArray.Count() - 1 Do
			...
			Try
				...
				Status(NStr("en='Extracting file text';ru='Идет извлечение текста файла';vi='Tiến hành lấy ra nội dung tệp'"), Progress, NameWithExtension);
...
"extracting text from  file ""%1""';ru='Во время извлечения текста из файла ""%1"" произошла неизвестная ошибка';vi='Trong thời gian lấy ra nội dung từ tệp ""%1"" đã xảy ra lỗi không xác định'"),
...
"Обработано файлов: %1';vi='Đã hoàn thành trích dẫn nội dung."
"Đã xử lý tệp: %1'"),
			FilesArray.Count());
