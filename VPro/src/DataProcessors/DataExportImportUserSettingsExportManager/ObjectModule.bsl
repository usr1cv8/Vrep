	...
	For Each IBUser IN IBUsers Do
		...
		While ToContinueTo Do
			...
			Except
				...
				WriteLogEvent(
					NStr("en='DataExportImport.SettingExportSkipped';ru='ВыгрузкаЗагрузкаДанных.ВыгрузкаНастройкиПропущена';vi='DataExportImport.SettingExportSkipped'", Metadata.DefaultLanguage.LanguageCode),
...
"Пользователь=%3"
"Представление=%4"
"';vi='Đã bỏ qua việc kết xuất tùy chỉnh vì không thể đọc tùy chỉnh:"
"КлючНастроек=%1"
"КлючОбъекта=%2"
"Пользователь=%3"
"Представление=%4"
...
Procedure ExportSettingsItem(WriteStream, Val SettingsKey, Val ObjectKey, Val User, Val Presentation, Val Settings)
	...
	If FindDisallowedXMLCharacters(SettingsKey) > 0
		...
		WriteLogEvent(
			NStr("en='DataExportImport.SettingExportSkipped';ru='ВыгрузкаЗагрузкаДанных.ВыгрузкаНастройкиПропущена';vi='DataExportImport.SettingExportSkipped'", Metadata.DefaultLanguage.LanguageCode),
			EventLogLevel.Warning,,,
			NStr("en='Setting export is skipped as key parameters contain invalid characters.';ru='Выгрузка настройки пропущена, т.к. в ключевых параметрах содержатся недопустимые символы.';vi='Bỏ qua việc kết xuất tùy chỉnh, bởi vì trong các tham số đã bật có các ký tự không thể chấp nhận.'", Metadata.DefaultLanguage.LanguageCode));
