...
Procedure BeforeObjectExport(Container, ObjectExportManager, Serializer, Object, Artifacts, Cancel) Export
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Metadata object %1 cannot be processed by handler ServiceUsersInBTCSaaS.BeforeObjectExport().';ru='Объект метаданных %1 не может быть обработан обработчиком ServiceUsersInBTCSaaS.BeforeObjectExport().';vi='Đối tượng metadata %1 không thể được xử lý bởi hàm sự kiện ServiceUsersInBTCSaaS.BeforeObjectExport().'"),
...
Procedure BeforeMatchRefs(Container, MetadataObject, SourceRefsTable, StandardProcessing, Cancel) Export
	...
	Else
		...
		Raise NStr("en='Data type is incorrect';ru='Тип данных указан неверно';vi='Chỉ ra sai kiểu dữ liệu'");
...
Procedure BeforeObjectImport(Container, Object, Artifacts, Cancel) Export
	...
	Else
		...
		Raise ServiceTechnologyIntegrationWithSSL.SubstituteParametersInString(
			NStr("en='Metadata object %1 cannot be processed by handler ServiceUsersInBTCSaaS.BeforeObjectImport().';ru='Объект метаданных %1 не может быть обработан обработчиком ServiceUsersInBTCSaaS.BeforeObjectImport().';vi='Đối tượng metadata %1 không thể được xử lý bởi hàm sự kiện ServiceUsersInBTCSaaS.BeforeObjectImport().'"),
			Object.Metadata().FullName());
