...
"выполняется только администраторами.';vi='Không đủ quyền truy cập."
""
"Thay đổi thuộc tính nhiệm vụ thường kỳ"
"chỉ thực hiện được bởi người quản trị.'");
	EndIf;
	...
	If Find(", Add, Copy, Change,", ", " + Action + ",") = 0 Then
		...
		Raise NStr("en='Incorrect parameters of opening form ""Scheduled job"".';ru='Неверные параметры открытия формы ""Регламентное задание"".';vi='Sai tham số mở biểu mẫu ""Nhiệm vụ thường kỳ"".'");
	EndIf;
	...
	Else
		...
		If Task.Metadata = Undefined Then
			MetadataName        = NStr("en='<no metadata>';ru='<нет метаданных>';vi='<không có Metadata>'");
			MetadataSynonym     = NStr("en='<no metadata>';ru='<нет метаданных>';vi='<không có Metadata>'");
			MetadataMethodName  = NStr("en='<no metadata>';ru='<нет метаданных>';vi='<không có Metadata>'");
		Else
	...
	If Action <> "Change" Then
		ID = NStr("en='<will be created when writing>';ru='<будет создан при записи>';vi='<sẽ tạo lại khi ghi>'");
		Use = False;
...
Procedure TemplateSelectionForNewScheduledJob()
	...
	ScheduledJobMetadataDetails.ShowChooseItem(
		...
		NStr("en='Select a scheduled job template';ru='Выберите шаблон регламентного задания';vi='Hãy chọn khuôn mẫu nhiệm vụ thường kỳ'"));
...
Procedure RefreshFormTitle()
	...
	If Action = "Change" Then
		Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='%1 (Scheduled job)';ru='%1 (Регламентное задание)';vi='%1 (Nhiệm vụ thường kỳ)'"), Presentation);
	Else
		Title = NStr("en='Scheduled job (creation)';ru='Регламентное задание (создание)';vi='Nhiệm vụ thường kỳ (tạo mới)'");
	EndIf;
