...
""
"Работа с"
"регламентными и фоновыми заданиями выполняется только администраторами.';vi='Thiếu quyền truy cập."
""
"Làm việc với"
"nhiệm vụ thường kỳ và nhiệm vụ nền do người quản trị thực hiện.'");
	EndIf;
...
Procedure TableScheduledJobsBeforeDelete(Item, Cancel)
	...
	If Items.ScheduledJobTable.SelectedRows.Count() > 1 Then
		ShowMessageBox(, NStr("en='Cannot delete the predefined scheduled job.';ru='Невозможно удалить предопределенное регламентное задание.';vi='Không thể xóa bỏ nhiệm vụ thường kỳ định trước.'"));
	...
	Else
		ShowQueryBox(
			...
			NStr("en='Remove the scheduled job?';ru='Удалить регламентное задание?';vi='Xóa nhiệm vụ thường kỳ?'"), QuestionDialogMode.YesNo);
	EndIf;
...
Procedure ExecuteScheduledJobManually(Command)
	...
	If Items.ScheduledJobTable.CurrentData = Undefined Then
		ShowMessageBox(, NStr("en='Select scheduled job.';ru='Выберите регламентное задание.';vi='Hãy chọn nhiệm vụ thường kỳ.'"));
		Return;
	EndIf;
	...
	For Each SelectedRow IN SelectedRows Do
		...
		If ExecuteParameters.Started Then
			...
			ShowUserNotification(
				NStr("en='Scheduled job is launched';ru='Запущена процедура регламентного задания';vi='Đã khởi động thủ tục của nhiệm vụ thường kỳ'"), ,
				StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1."
"Procedure is launched in the background job %2';ru='%1."
"Процедура запущена в фоновом задании %2';vi='%1."
"Đã khởi động thủ tục ở nhiệm vụ nền %2'"),
...
"  регламентного задания ""%1"" уже выполняется в сеансе %2, открытом %3.';vi='Thủ tục"
"  của nhiệm vụ thường kỳ ""%1"" đã được thực hiện trong phiên làm việc %2 bằng cách mở %3.'"),
					CurrentData.Description,
	...
	If ErrorsCount > 0 Then
		TextAboutErrorsTitle = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Jobs are completed with errors (%1 out of %2)';ru='Задания выполнены с ошибками (%1 из %2)';vi='Nhiệm vụ đã thực hiện bị lỗi (%1 trong số %2)'"),
		...
		If ErrorsCount > 5 Then
			...
			Buttons.Add(1, NStr("en='Show errors';ru='Показать ошибки';vi='Hiển thị lỗi'"));
...
Procedure ConfigureSchedule(Command)
	...
	ElsIf Items.ScheduledJobTable.SelectedRows.Count() > 1 Then
		ShowMessageBox(, NStr("en='Select a scheduled job.';ru='Выберите одно регламентное задание.';vi='Hãy chọn một nhiệm vụ thường kỳ.'"));
	Else
...
Procedure CancelBackgroundJob(Command)
	...
	If Items.BackgroundJobTable.CurrentData = Undefined Then
		ShowMessageBox(, NStr("en='Select background job.';ru='Выберите фоновое задание.';vi='Hãy chọn nhiệm vụ nền.'"));
	Else
...
"сервером только через секунды, возможно потребуется обновить данные вручную.';vi='Nhiệm vụ đã bị hủy bỏ, nhưng"
"Server chưa thiết lập trạng thái hủy bỏ"
"chỉ sau vài giây, có thể thử cập nhật dữ liệu một cách thủ công.'"));
	EndIf;
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("en='<>';ru='<>';vi='<>'"));
	...
	FilterElement.RightValue = NStr("en='<not determined>';ru='<не определено>';vi='<chưa xác định>'");
...
Procedure ShowMessageAboutScheduledJobManualProcessingCompletion()
	...
	For Each Notification IN CompletionNotifications Do
		...
		ShowUserNotification(
			NStr("en='Scheduled job procedure is executed';ru='Выполнена процедура регламентного задания';vi='Thực hiện thủ tục của nhiệm vụ thường kỳ'"),
			,
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='%1."
"The procedure has been completed at the background job %2';ru='%1."
"Процедура завершена в фоновом задании %2';vi='%1."
"Đã hoàn tất thủ tục ở nhiệm vụ nền %2'"),
	...
	For Each Task IN CurrentTable Do
		...
		If ValueIsFilled(Update.ScheduledJobID) Then
			...
			Update.ScheduledJobDescription
				= ?(Rows.Count() = 0, NStr("en='<not specified>';ru='<не найден>';vi='<không tìm thấy>'"), Rows[0].Description);
		Else
