...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	WriteLogEvent(ConfigurationUpdate.EventLogMonitorEvent(), EventLogLevel.Information,,,
		NStr("en='Opening configuration update wizard...';ru='Открытие помощника обновления конфигурации...';vi='Mở trợ lý cập nhật cấu hình...'"));
	...
	Else
		...
		If Not ConfigurationIsReadyForUpgrade Then
			...
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Information",
				NStr("en='Configuration can not be updated. Update verification completion.';ru='Конфигурация не может быть обновлена. Завершение проверки обновления.';vi='Không thể cập nhật cấu hình. Hoàn thành kiểm tra việc bản cập nhật.'"));
			Cancel = True;
			Return;
		EndIf; 
...
Procedure FieldUpdateFileStartChoice(Item, ChoiceData, StandardProcessing)
	...
	Dialog.Filter		= NStr("en='All delivery files (*.cf*;*.cfu)|*.cf*;*.cfu|Configuration delivery files (*.cf)|*.cf|Configuration update delivery files(*.cfu)|*.cfu';ru='Все файлы поставки (*.cf*;*.cfu)|*.cf*;*.cfu|Файлы поставки конфигурации (*.cf)|*.cf|Файлы поставки обновления конфигурации(*.cfu)|*.cfu';vi='Tâts cả các tệp giao hàng (*.cf*;*.cfu)|*.cf*;*.cfu|Tệp cung cấp cấu hình (*.cf)|*.cf|Tệp cung cấp cập nhật cấu hình(*.cfu)|*.cfu'");
	Dialog.Title	= NStr("en='Select a configuration update delivery';ru='Выбор поставки обновления конфигурации';vi='Chọn bộ cài cập nhật cấu hình'");
...
Procedure NewInVersion(Command)
	...
	Else
		ShowMessageBox(, NStr("en='No update information is available.';ru='Информация об обновлении отсутствует.';vi='Chưa có thông tin về bản cập nhật.'"));
	EndIf;
	...
	While CurrentVersionFrom <> VersionBefore Do
		...
		If CurrentVersionFrom = NewCurrentVersionFrom AND NewCurrentVersionFrom <> VersionBefore Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Update to version %1 from the current version %2 is not available';ru='Недоступно обновление на версию %1 с текущей версии %2';vi='Không thể cập nhật lên phiên bản %1 từ phiên bản hiện tại %2'"), VersionBefore, VersionFrom);
		EndIf;
...
Procedure RunImportOfListOfUpdates(Val ImportFileAddress, TableUpdates = Undefined)
	...
	ErrorInfo = NStr("en='An error occurred when reading the update list file';ru='Ошибка при чтении файла списка обновлений';vi='Lỗi khi đọc tệp danh sách cập nhật'") + " " + FullPathFileExport;
	...
	If TableUpdates = Undefined Then // errors at file reading
		Raise NStr("en='An error occurred when reading the file';ru='Ошибка при чтении файла';vi='Lỗi khi đọc tệp'") + " " + FullPathFileExport;
   	EndIf;
...
Procedure BeforePageOpen(NewCurrentPage = Undefined)
	...
	ElsIf NewCurrentPage = Pages.ConnectionToSite Then
		...
		If Not IsBlankString(Object.TechnicalErrorInfo) Then
			Object.TechnicalErrorInfo = NStr("en='Technical information on the error:';ru='Техническая информация об ошибке:';vi='Thông tin kỹ thuật về lỗi:'") + Chars.LF + Object.TechnicalErrorInfo;
		EndIf;
...
"после чего можно будет установить это обновление.';vi='Để cập nhật lên phiên bản này, cần có nền tảng 1C:DOANH NGHIỆP"
"không thấp hơn phiên bản <b>%1</b>. Cần <a href = ""КакОбновитьПлатформу"">chuyển sang phiên bản nền tảng mới</a>,"
"sau đó có thể cài đặt bản cập nhật này.'");
	...
	ElsIf NewCurrentPage = Pages.UpdatesNotDetected Then
		...
		Items.InscriptionVersionForUpdate.Title			= ?(TypeOf(AvailableUpdateStructure) = Type("Structure"), NStr("en='Version available for update -';ru='Доступная версия для обновления -';vi='Phiên bản khả dụng để cập nhật -'") + " " + AvailableUpdateStructure.Version, "");
		...
		If CommonUseClientServer.CompareVersions(StandardSubsystemsClientReUse.ClientWorkParameters().ConfigurationVersion,
			...
			Items.GroupUpdateIsNotDetected.Title = NStr("en='No update required';ru='Обновление не требуется';vi='Không cần cập nhật'");
		...
		Else 
			...
			Items.GroupUpdateIsNotDetected.Title = NStr("en='Configuration update is not detected';ru='Обновления конфигурации не обнаружено';vi='Không tìm thấy bản cập nhật cấu hình'");
		EndIf;
	...
	ButtonNext.Title = ?(NextButtonFunction, NStr("en='Next >';ru='Далее  >';vi='Tiếp theo >'"), NStr("en='Finish';ru='Готово';vi='Kết thúc'"));
...
Function RestoreResultsOfPreviousStart(Cancel = False)
	...
	If PageName = Pages.AvailableUpdate.Name Then
		...
		If CheckUpdateFilesReceived() Then
			...
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Information",
				NStr("en='Configuration update files have already been received and saved locally.';ru='Обнаружено, что файлы обновления конфигурации уже были получены и сохранены локально.';vi='Phát hiện rằng đã nhận tệp cập nhật cấu hình và lưu cục bộ.'"));
...
Function ProcessPageConnectionToSite(OutputMessages = True)
	...
	If Not ValueIsFilled(Object.UpdateServerUserCode) Then
		If OutputMessages Then
			ShowMessageBox(, NStr("en='Specify a user code for update.';ru='Укажите код пользователя для обновления.';vi='Hãy chỉ ra mã người sử dụng để cập nhật.'"));
		EndIf;
	...
	If Not ValueIsFilled(Object.UpdatesServerPassword) Then
		If OutputMessages Then
			ShowMessageBox(, NStr("en='Specify a user password for update.';ru='Укажите пароль пользователя для обновления.';vi='Hãy chỉ ra mật khẩu người sử dụng để cập nhật.'"));
		EndIf;
...
Function ProcessPageChoiceUpdateMode(OutputMessages = True, FlagCompleteJobs = False)
	...
	ElsIf Object.UpdateMode = 2 Then  // Plan update
		...
		If Not PlanConfigurationChange() Then
			ShowMessageBox(, NStr("en='It is impossible to schedule the configuration update. Error information is saved in the log.';ru='Невозможно запланировать обновление конфигурации. Сведения об ошибке сохранены в журнал регистрации.';vi='Không thể lập kế hoạch cập nhật cấu hình. Thông tin về lỗi được lưu vào nhật ký sự kiện.'"));
			Return CurrentPage.Name;
		EndIf;
	...
	Message = NStr("en='Update mode is selected:';ru='Выбран режим обновления:';vi='Chọn biểu mẫu báo cáo'") + " ";
	If Object.UpdateMode = 0 Then   // Update now
		Message = Message + NStr("en='On exiting the application';ru='При завершении работы';vi='Khi kết thúc công việc'");
	ElsIf Object.UpdateMode = 1 Then  // On closing application
		Message = Message + NStr("en='schedule';ru='график';vi='lịch biểu'");
	ElsIf Object.UpdateMode = 2 Then  // Plan update
		Message = Message + NStr("en='schedule';ru='график';vi='lịch biểu'");
	EndIf;
	Message = Message + ".";
...
Procedure ProcessPressOfButtonNext(FlagCompleteJobs = False)
	...
	If Not StandardSubsystemsClientReUse.ClientWorkParameters().UpdateSettings.IsAccessForUpdate
		...
		ShowMessageBox(, NStr("en='Insufficient rights to update the configuration.';ru='Недостаточно прав для выполнения обновления конфигурации.';vi='Không đủ quyền để thực hiện cập nhật cấu hình'"));
	EndIf;
...
Function ProcessPageUpdateFile(OutputMessages = True)
	...
	If Object.NeedUpdateFile = 1 Then
		...
		If Not ValueIsFilled(Object.UpdateFileName) Then
			If OutputMessages Then
				CommonUseClientServer.MessageToUser(NStr("en='Specify a configuration update delivery file.';ru='Укажите файл поставки обновления конфигурации.';vi='Hãy chỉ ra tệp cung cấp bản cập nhật cấu hình.'"),,"Object.UpdateFileName");
			EndIf;
		...
		If Not File.Exist() OR Not File.IsFile() Then
			If OutputMessages Then
				CommonUseClientServer.MessageToUser(NStr("en='Delivery file of configuration update is not found.';ru='Файл поставки обновления конфигурации не найден.';vi='Không tìm thấy tệp cung cấp bản cập nhật cấu hình.'"),,"Object.UpdateFileName");
			EndIf;
			CurrentItem = Items.FieldUpdateFile;
			Return Pages.UpdateFile.Name;
		EndIf;
...
Function GoToChoiceOfUpdateMode(IsGoNext = False)
	...
	If AdministrationParameters = Undefined Then
		...
		FormTitle = NStr("en='Install update';ru='Установка обновления';vi='Cài đặt bản cập nhật'");
...
"обновления необходимо ввести параметры администрирования информационной базы';vi='Để cài đặt"
"bản cập nhật, cần nhập tham số quản trị cơ sở thông tin'");
...
"необходимо ввести параметры администрирования кластера серверов и информационной базы';vi='Để cài đặt bản cập nhật"
"cần nhập tham số quản trị của cụm Server và của cơ sở thông tin'");
...
Procedure AfterAdministrationParametersReceiving(Result, IsGoNext) Export
	...
	Else
		...
		WarningText = NStr("en='Failed to install the application update, i.e. correct"
"infobase administration parameters were not entered.';ru='Не удалось установить обновление программы, т.к. не были введены"
"корректные параметры администрирования информационной базы.';vi='Không thể cài đặt bản cập nhật chương trình, bởi vì chưa nhập"
"tham số quản trị hợp lệ của cơ sở thông tin.'");
...
Function CheckAccessToIB()
	...
	Except
		Result = False;
		MessageText = NStr("en='Cannot connect to the infobase:';ru='Не удалось выполнить подключение к информационной базе:';vi='Không thể thực hiện kết nối đến cơ sở thông tin:'") + Chars.LF;
		EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, 
	...
	If Result AND Not ClientWorkParameters.FileInfobase Then
		...
		Except
			Result = False;
			MessageText = NStr("en='Cannot connect to the server cluster:';ru='Не удалось выполнить подключение к кластеру серверов:';vi='Không thể thực hiện kết nối đến Cluster server:'") + Chars.LF;
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents,
				"Error", MessageText + DetailErrorDescription(ErrorInfo()));
			DetectedConnectionError = MessageText + BriefErrorDescription(ErrorInfo());
		EndTry;
...
Function CheckEMailSettings(CurrentPageName, OutputMessages = True)
	If Not CommonUseClientServer.EmailAddressMeetsRequirements(Object.EmailAddress) Then
		If OutputMessages Then
			ShowMessageBox(, NStr("en='Specify an allowable email address.';ru='Укажите допустимый адрес электронной почты.';vi='Hãy chỉ ra địa chỉ E-mail cho phép.'"));
		EndIf;
...
Function CreateSchedulerTask(Val ApplicationFileName, Val DateTime)
	...
	Try
		...
		If ErrorCode <> 0 Then	// Error codes: http://msdn2.microsoft.com/en-us/library/aa389389(VS.85).aspx.
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, 
				"Error", NStr("en='An error occurred while creating a scheduler task:';ru='Ошибка при создании задачи планировщика:';vi='Lỗi khi tạo nhiệm vụ của người lập kế hoạch:'")
					+ " " + ErrorCode);
			Return 0;
		EndIf;
		MessageText = NStr("en='Scheduler task is successfully planned (command: %1; date: %2; task code: %3).';ru='Задача планировщика успешно запланирована (команда: %1; дата: %2; код задачи: %3).';vi='Đã lập xong kế hoạch nhiệm vụ trong bảng kế hoạch (lệnh: %1; ngày: %2; mã nhiệm vụ: %3).'");
	...
	Except
		...
		EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Error",
			NStr("en='An error occurred while creating a scheduler task:';ru='Ошибка при создании задачи планировщика:';vi='Lỗi khi tạo nhiệm vụ của người lập kế hoạch:'")
				+ " " + ErrorDescription());
		Return 0;
	EndTry;
...
Function ValidateAcceptableDateUpdateAtServer(DateTime)
	...
	If DateTime < Now Then
		Return NStr("en='Configuration update can be scheduled only for a future date and time.';ru='Обновление конфигурации может быть запланировано только на будущую дату и время.';vi='Có thể dự tính cập nhật cấu hình chỉ vào một ngày và thời gian tương lai.'");
	EndIf;
	If DateTime > AddMonth(Now, 1) Then
		Return NStr("en='Configuration update can be scheduled not later than in a month from the current date.';ru='Обновление конфигурации может быть запланировано не позднее, чем через месяц относительно текущей даты.';vi='Có thể dự tính cập nhật cấu hình không quá một tháng so với ngày hiện tại.'");
	EndIf;
...
Function DeleteSchedulerTask(CodeTasks)
	...
	Try
		...
		If Not Result Then	// Error codes: http://msdn2.microsoft.com/en-us/library/aa389957(VS.85).aspx.
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Error",
				NStr("en='An error occurred while deleting the scheduler task:';ru='Ошибка при удалении задачи планировщика:';vi='Lỗi khi xóa nhiệm vụ của người lập kế hoạch:'")
					+ " " + ErrorCode);
			Return Result;
		EndIf;
		MessageText = NStr("en='Scheduler task is successfully removed (task code: %1).';ru='Задача планировщика успешно удалена (код задачи: %1).';vi='Đã xóa thành công nhiệm vụ trong bảng kế hoạch (mã nhiệm vụ: %1).'");
		EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Information",
...
"temporary file %1 %2';ru='Ошибка при удалении временного файла %1 %2';vi='Lỗi khi xóa tệp tạm thời %1 %2'");
		NameLogEvents =	ConfigurationUpdateClient.EventLogMonitorEvent();
		EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents,
...
Function UnpackUpdateInstallationPackage()
	...
	NameLogEvents = ConfigurationUpdateClient.EventLogMonitorEvent();
	EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, 
		"Information", NStr("en='Unpacking update distribution...';ru='Выполняется распаковка дистрибутива обновления...';vi='Thực hiện mở bản đóng gói cập nhật...'"));
	...
	For Each Update IN Object.AvailableUpdates Do
		...
		Try 
			...
			If Not FileExistsAtClient(UpdateFileName) Then
				EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Error",
					NStr("en='Update distribution does not contain 1cv8.cfu:';ru='Дистрибутив обновления не содержит 1cv8.cfu:';vi='Bản phân phối cập nhật không có tệp 1cv8.cfu:'")
			...
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Information",
				NStr("en='Update distribution files are successfully uncompressed:';ru='Файлы дистрибутива обновления успешно распакованы:';vi='Đã giải nén thành công các tệp có bản cập nhật:'")
					+ " " + UpdateFileName);
		Except
			EventLogMonitorClient.AddMessageForEventLogMonitor(NameLogEvents, "Error",
				NStr("en='Error while unzipping the updates distribution';ru='Ошибка при распаковке дистрибутива обновления:';vi='Lỗi khi giải nén tệp cập nhật:'")
					+ " " + ErrorDescription());
			Return False;
		EndTry;
		...
		Try
			...
			If Not FileExistsAtClient(UpdateFileName) Then
				EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
					"Error", NStr("en='Update distribution does not contain 1cv8.cfu:';ru='Дистрибутив обновления не содержит 1cv8.cfu:';vi='Bản phân phối cập nhật không có tệp 1cv8.cfu:'")
						+ " " + Update.PathToLocalFile);
				Return False;
			EndIf;
			Update.PathToLocalUpdateFile = UpdateFileName;
			EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
				"Information", NStr("en='Update distribution files are successfully uncompressed:';ru='Файлы дистрибутива обновления успешно распакованы:';vi='Đã giải nén thành công các tệp có bản cập nhật:'")
					+ " " + UpdateFileName);
			ZipReader.Close();
		Except
			EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
				"Error",  NStr("en='Error while unzipping the update distribution:';ru='Ошибка при распаковке дистрибутива обновления:';vi='Lỗi khi giải nén tệp cập nhật:'")
					+ " " + ErrorDescription());
			Return False;
		EndTry;
	EndDo;
...
Function GetFileOfTemplatesList(Val OutputMessages = True, AvailableUpdateForNextEdition = False)
	...
	If Result.Status <> True Then
		...
		Except
			EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(),
				"Error", 
				NStr("en='An error occurred while deleting a temporary file';ru='Ошибка при удалении временного файла';vi='Lỗi khi xóa tệp tạm thời'") + " " + PathToTemplatesListFile + Chars.LF +
				DetailErrorDescription(ErrorInfo()));
		EndTry;
		ErrorText = NStr("en='An error occurred when receiving a template list file:';ru='Ошибка при получении файла списка шаблонов:';vi='Lỗi khi nhận tệp danh sách khuôn mẫu:'") + " " + Result.ErrorInfo;
		If OutputMessages Then
	...
	If Not FileExistsAtClient(PathToTemplatesListFile) Then
		Return NStr("en='File does not exist:';ru='Файл не существует:';vi='Không có tệp:'") + " " + PathToTemplatesListFile;
	EndIf;
	...
	Except
		ErrorText	= NStr("en='An error occurred when unpacking a file with available update list:';ru='Ошибка при распаковке файла со списком доступных обновлений:';vi='Lỗi khi giải nén tệp có danh sách tệp cập nhật khả dụng:'") + " ";
...
Function GetUpdate(OutputMessages = True)
	...
	Message = "";
	If Object.UpdateSource = 0 Then
		Message = Message + NStr("en='Receiving files from the Internet...';ru='Получение файлов из Интернета...';vi='Nhận tệp từ Internet...'");
	Else
		Message = Message + NStr("en='Receiving update file from the specified source...';ru='Получение файла обновления из указанного источника...';vi='Nhận tệp cập nhật từ nguồn đã chỉ ra...'");
	EndIf;
	EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
		"Information", Message);
...
"Интернет по причине: %1';vi='Không thể xác nhận tính hợp pháp nhận bản cập nhật qua"
"Internet theo nguyên nhân: %1'");
				...
				Message = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en=' Get file %1 to %2';ru=' Получаем файл %1 в %2';vi=' Nhận tệp %1 trong %2'"),
				...
				If Result.Status <> True Then
					...
					Except
						EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
							"Error", NStr("en='An error occurred while deleting a temporary file';ru='Ошибка при удалении временного файла';vi='Lỗi khi xóa tệp tạm thời'") + " " +
			...
			Else
				...
				Message = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en=' Get file %1 to %2';ru=' Получаем файл %1 в %2';vi=' Nhận tệp %1 trong %2'"), File.Value.Address, File.Value.LocalPath);
				Items.LabelProgres.Title = Message;
				EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), , Message);
	...
	If CheckUpdateFilesReceived() Then
		EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
			"Information", NStr("en='Update files are successfully received.';ru='Файлы обновления успешно получены.';vi='Đã nhận tệp cập nhật.'"));
		GoToChoiceOfUpdateMode(True);
		Return Undefined;
	Else
		Message = NStr("en='An error occurred when receiving update files.';ru='Ошибка при получении файлов обновления.';vi='Lỗi khi nhận tệp cập nhật.'");
		EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
			"Error", Message);
...
"при копировании: %1 (источник: %2; приемник: %3)';vi='Lỗi"
"khi sao chép: %1 (nguồn : %2. đích: %3)'"), 
				DetailErrorDescription(ErrorInfo()),
...
Function GetUpdateFilesViaInternet(OutputMessages, AvailableUpdateForNextEdition = False)
	...
	If AvailableUpdate.PageName = Pages.AvailableUpdate.Name Then
		...
		If Object.AvailableUpdates.Count() = 0 Then
			EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
				"Information", NStr("en='The update cannot proceed: no updates available.';ru='Невозможно продолжить обновление: нет доступных обновлений.';vi='Không thể tiếp tục cập nhật: không có các bản cập nhật được phép.'"));
			Return Pages.UpdatesNotDetected.Name;
		EndIf;
...
Function CheckUpdateInternet(OutputMessages = True) 
	Pages		= Items.AssistantPages.ChildItems;
	EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), 
		"Information", NStr("en='Checking for updates online...';ru='Проверка обновления в Интернете...';vi='Kiểm tra bản cập nhật trên Internet...'"));
	...
	If Object.NeedUpdateFile = 1 Then
		...
		If Not File.Exist() OR Not File.IsFile() Then
			...
			EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), "Information",
				NStr("en='The update cannot proceed: the delivery file for configuration update was not found.';ru='Невозможно продолжить обновление: файл поставки обновления конфигурации не найден.';vi='Không thể tiếp tục cập nhật: không tìm thấy tệp cung cấp bản cập nhật cấu hình.'"));
...
Function LabelTextInfobaseBackup()
	...
	Result = NStr("en='Do not back up the infobase';ru='Не создавать резервную копию ИБ';vi='Không tạo bản sao lưu dự phòng CSDL'");
	
	If Object.CreateBackup = 1 Then
		Result = NStr("en='Create temporary backup of infobase';ru='Создавать временную резервную копию ИБ';vi='Tạo bản sao dự phòng tạm thời CSDL'");
	ElsIf Object.CreateBackup = 2 Then
		Result = NStr("en='Create infobase backup';ru='Создавать резервную копию ИБ';vi='Tạo bản sao dự phòng CSDL'");
	EndIf; 
	
	If Object.RestoreInfobase Then
		Result = Result + " " + NStr("en='and roll back in case of error';ru='и выполнять откат при нештатной ситуации';vi='và thực hiện quay lại trong tình huống khẩn cấp'");
	Else
		Result = Result + " " + NStr("en='and do not roll back in case of error';ru='и не выполнять откат при нештатной ситуации';vi='và không thực hiện quay lại trong tình huống khẩn cấp'");
	EndIf;
...
Procedure RunConfigurationUpdate()
	...
	EventLogMonitorClient.AddMessageForEventLogMonitor(ConfigurationUpdateClient.EventLogMonitorEvent(), "Information",
		NStr("en='Updating configuration:';ru='Выполняется процедура обновления конфигурации:';vi='Đang thực hiện thủ tục cập nhật cấu hình:'")
			+ " " + ScriptMainFileName);
...
Function ItIsPossibleToStartUpdate()
	...
	#If WebClient Or MobileClient Then
		...
		MessageText = NStr("en='Updating the application in web client is unavailable.';ru='Обновление программы недоступно в веб-клиенте.';vi='Không cho phép cập nhật chương trình trong Web-client.'");
	#EndIf
	
	If CommonUseClientServer.IsLinuxClient() Then
		ItIsPossibleToStartUpdate = False;
		MessageText = NStr("en='Updating the application is unavailable under Linux OS.';ru='Обновление программы недоступно в клиенте под управлением ОС Linux.';vi='Không cho phép cập nhật chương trình trong Client chạy hệ điều hành Linux.'");
	EndIf;
	
	If CommonUseClient.ClientConnectedViaWebServer() Then
		ItIsPossibleToStartUpdate = False;
		MessageText = NStr("en='Updating the application using web server connection is unavailable.';ru='Обновление программы недоступно при подключении через веб-сервер.';vi='Không thể cập nhật chương trình khi kết nối qua Web-server.'");
	EndIf;
...
Procedure DisplayUpdateOrder()
	...
	If Not IsBlankString(FileNameOrderUpdate) Then
		ConfigurationUpdateClient.OpenWebPage(FileNameOrderUpdate);
	Else
		ShowMessageBox(, NStr("en='Update order description is missing.';ru='Описание порядка обновления отсутствует.';vi='Thiếu bản mô tả trình tự cập nhật.'"));
	EndIf;
