...
Procedure CommandOK(Command)
	...
	If SettingsChanged Then
		...
		If RepeatPeriodInDay > 0 AND RepeatPeriodInDay < 60 * 5 Then
			CommonUseClientServer.MessageToUser(NStr("en='Check interval cannot be set to more often than once every 5 minutes.';ru='Интервал проверки не может быть задан чаще, чем один раз 5 минут.';vi='Không thể đưa ra khoảng thời gian kiểm tra nhiều hơn 5 phút mỗi lần.'"));
			Return;
		EndIf;
...
Function LabelTextOpenSchedule(Form)
	...
	Return ?(NOT IsBlankString(StringPresentationSchedule),
		StringPresentationSchedule, NStr("en='Not defined';ru='Не определена';vi='Không xác định'"));
