...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not AccessRight("Administration", Metadata) Then
		Raise NStr("en='Data processor use in interactive mode is available to administrator only.';ru='Использование обработки в интерактивном режиме доступно только администратору.';vi='Chỉ người sử dụng mới được phép sử dụng bộ xử lý trong chế độ trực tác.'");
	EndIf;
	...
	FormTitle = NStr("en='Universal data exchange in XML format (%VersionHandling%)';ru='Универсальный обмен данными в формате XML (%VersionHandling%)';vi='Trao đổi dữ liệu đa năng với định dạng XML (%VersionHandling%)'");
...
Procedure OnOpen(Cancel)
	...
	If StoredImportingMode
		...
		If Object.SettingAutomaticDataImport = 1 Then
			...
			ShowQueryBox(NOTifyDescription, NStr("en='Import data from the exchange file?';ru='Выполнить загрузку данных из файла обмена?';vi='Thực hiện kết nhập dữ liệu từ tệp trao đổi?'"), QuestionDialogMode.YesNo, , DialogReturnCode.Yes);
...
Procedure InfobaseForConnectionDirectoryStartChoice(Item, ChoiceData, StandardProcessing)
	...
	FileDialog.Title = NStr("en='Select an infobase directory';ru='Выберите каталог информационной базы';vi='Hãy chọn thư mục cơ sở thông tin'");
...
Procedure RulesFilenameOnChange(Item)
	...
	If IsBlankString(RulesFilename) Or Not File.Exist() Then
		MessageToUser(NStr("en='File of exchange rules is not found';ru='Не найден файл правил обмена';vi='Không tìm thấy tệp quy tắc trao đổi'"), "RulesFilename");
	...
	ShowQueryBox(NOTifyDescription, NStr("en='Import data exchange rules?';ru='Загрузить правила обмена данными?';vi='Kết nhập quy tắc trao đổi dữ liệu?'"), QuestionDialogMode.YesNo, , DialogReturnCode.Yes);
...
Procedure ReceiveInformationAboutExchangeFile(Command)
	...
	If IsClient Then
		...
		BeginPutFile(NOTifyDescription, FileURL,NStr("en='Exchange file';ru='Файл обмена';vi='Tệp trao đổi'"),, UUID);
...
Procedure ReceiveInformationAboutExchangeFileEnd(Result, Address, SelectedFileName, AdditionalParameters) Export
	...
	If Result Then
		...
		Except
			...
			MessageToUser(NStr("en='Cannot read the exchange file.';ru='Не удалось прочитать файл обмена.';vi='Không thể đọc tệp trao đổi.'"));
			ClearDataAboutFileForDataImport();
...
Procedure DeletionDelete(Command)
	...
	ShowQueryBox(NOTifyDescription, NStr("en='Delete the selected data in the infobase?';ru='Удалить выбранные данные в информационной базе?';vi='Xóa bỏ dữ liệu đã chọn trong cơ sở thông tin?'"), QuestionDialogMode.YesNo, , DialogReturnCode.No);
...
Procedure DeletionDeleteEnd(Result, AdditionalParameters) Export
	...
	If Result = DialogReturnCode.Yes Then
		...
		Status(NStr("en='Deleting data. Please wait...';ru='Выполняется удаление данных. Пожалуйста, подождите..';vi='Đang thực hiện xóa dữ liệu. Xin vui lòng đợi...'"));
		DeleteAtServer();
...
Procedure ImportDebuggingSetting(Command)
	...
	If IsClient Then
		...
		BeginPutFile(NOTifyDescription, ExchangeFileAddressInStorage,NStr("en='Exchange file';ru='Файл обмена';vi='Tệp trao đổi'"),, UUID);
...
Procedure ReadExchangeRules(Command)
	...
	If IsLinuxClient() AND DirectExporting = 1 Then
		ShowMessageBox(,NStr("en='Direct infobase connection is not supported if the client application is running on Linux.';ru='Прямое подключение к информационной базе не поддерживается в клиенте под управлением ОС Linux.';vi='Kết nối trực tiếp đến cơ sở thông tin không được hỗ trợ trên Client chạy hệ điều hành Linux.'"));
		Return;
	EndIf;
	...
	If IsClient Then
		...
		BeginPutFile(NOTifyDescription, RulesFileAddressInStorage,NStr("en='Exchange rule file';ru='Файл правил обмена';vi='Tệp quy tắc trao đổi'"),, UUID);
...
Procedure ReadExchangeRulesEnd(Result, Address, SelectedFileName, AdditionalParameters) Export
	...
	If Result Then
		...
		Status(NStr("en='Exchange rules are being read. Please wait...';ru='Выполняется чтение правил обмена. Пожалуйста, подождите..';vi='Đang thực hiện đọc quy tắc trao đổi. Xin vui lòng đợi...'"));
...
"Выберите другой файл для выгрузки данных.';vi='Tệp quy tắc trao đổi không thể trùng với tệp dữ liệu.'"));
		Return True;
...
Procedure FillListOfTypesAvailableForDeletion()
	...
	TreeRow.Presentation = NStr("en='Catalogs';ru='Справочники';vi='Danh mục'");
	...
	TreeRow = DataTree.Rows.Add();
	TreeRow.Presentation = NStr("en='Charts of characteristic types';ru='Планы видов характеристик';vi='Hệ thống dạng đặc tính'");
	...
	TreeRow = DataTree.Rows.Add();
	TreeRow.Presentation = NStr("en='Documents';ru='Документы';vi='Chứng từ'");
...
Procedure FileChoice(Item, PropertyName, CheckFileExistence, Val DefaultExtension = "xml",
	...
	FileDialog.Title = NStr("en='Select file';ru='Выберите файл';vi='Hãy chọn tệp'");
...
Function RunConnectionToIBReceiverAtServer()
	...
	If ConnectionResult <> Undefined Then
		...
		MessageToUser(NStr("en='Connection has been successfully established.';ru='Подключение успешно установлено.';vi='Kết nối đã thiết lập thành công.'"));
...
Procedure OpenImportingFileAtServer(FileURL)
	...
	Else
		...
		If Not FileAtServer.Exist() Then
			...
			MessageToUser(NStr("en='Exchange file is not found on server.';ru='Не найден файл обмена на сервере.';vi='Không tìm thấy tệp trao đổi trên Server.'"), "ExchangeFileName");
			Return;
...
Procedure RunImportFromFormEnd(Result, Address, SelectedFileName, AdditionalParameters) Export
	...
	If Result Then
		...
		Status(NStr("en='Importing data. Please wait...';ru='Выполняется загрузка данных. Пожалуйста, подождите..';vi='Đang kết nhập dữ liệu. Xin vui lòng đợi...'"));
...
Function FileNameAtServerOrClient(AttributeName ,Val FileURL, Val FilenameForExtension = ".xml",
	...
	Else
		...
		If Not FileAtServer.Exist() AND CheckFileExistence Then
			...
			MessageToUser(NStr("en='The specified file does not exist.';ru='Указанный файл не существует.';vi='Không tồn tại tệp đã chỉ ra.'"));
...
Procedure RunDumpFromForm()
	...
	Status(NStr("en='Exporting data. Please wait...';ru='Выполняется выгрузка данных. Пожалуйста, подождите..';vi='Đang thực hiện kết xuất dữ liệu. Xin vui lòng đợi...'"));
	...
	If IsClient AND Not DirectExporting AND Not Object.ErrorFlag Then
		...
		StoredFileName = ?(Object.ArchiveFile, NStr("en='Export file.zip';ru='Файл выгрузки.zip';vi='Tệp kết xuất.zip'"),NStr("en='Export file.xml';ru='Файл выгрузки.xml';vi='Tệp kết xuất.xml'"));
...
Function PerformExportAtServer()
	...
	If Not DirectExporting Then
		...
		If TempFileNameData = Undefined Then
			...
			Return Undefined;
			MessageToUser(NStr("en='Data file is not specified';ru='Не определен файл данных';vi='Chưa xác định tệp dữ liệu'"));
	...
	If ObjectForServer.HandlersDebugModeFlag Then
		...
		If Not File.Exist() Then
			...
			MessageToUser(NStr("en='External data processor file of event debuggers does not exist on the server';ru='Файл внешней обработки отладчиков событий не существует на сервере';vi='Không tồn tại tệp bộ xử lý ngoài gỡ rối sự kiện trên Server'"));
			Return Undefined;
		...
		If Cancel Then
			...
			MessageToUser(NStr("en='Cannot export event handlers';ru='Не удалось выгрузить обработчики событий';vi='Không thể kết xuất hàm sự kiện'"));
			Return "";
...
Procedure ChangeModeDataProcessors(RunMode)
	...
	?(RunMode, NStr("en='Operation mode (on client)';ru='Режим работы (на клиенте)';vi='Chế độ làm việc (trên Client)'"), NStr("en='Operation mode (on server)';ru='Режим работы (на сервере)';vi='Chế độ làm việc (trên Server)'"));
...
Function AttributeEmptyValue(Attribute, DataPath, Title)
	...
	If IsBlankString(Attribute) Then
		...
		MessageText = NStr("en='The ""%1"" field is not filled in';ru='Поле ""%1"" не заполнено';vi='Chưa điền trường ""%1""'");
...
"для запуска на версии платформы 1С:Предприятие 8.3 с отключенным режимом совместимости или выше';vi='Bộ xử lý được dùng"
"để khởi động sang phiên bản của nền tảng 1C:DOANH NGHIỆP 8.3 có ngắt chế độ tương thích hoặc cao hơn'");
