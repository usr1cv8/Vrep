...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	FormTitle = NStr("en='Configure handler debugging on data %Event%';ru='Настройка отладки обработчиков при %Событие% данных';vi='Tùy chỉnh gỡ rồi hàm sự kiện khi %Event% dữ liệu'");	
	Event = ?(Parameters.EventHandlersReadFromFileOfExchangeRules, NStr("en='export';ru='выгрузке';vi='cho kết xuất'"), NStr("en='import';ru='загрузке';vi='đối với kết nhập'"));
	FormTitle = StrReplace(FormTitle, "%Event%", Event);
	Title = FormTitle;
	
	ButtonTitle = NStr("en='Generate %Event% debug engine';ru='Сформировать модуль отладки %Событие%';vi='Lập mô-đun gỡ rối %Event%'");
	Event = ?(Parameters.EventHandlersReadFromFileOfExchangeRules, NStr("en='export';ru='выгрузки';vi='kết xuất'"), NStr("en='import';ru='загрузки';vi='kết nhập'"));
...
Procedure EventHandlersExternalDataProcessorFileNameStartChoice(Item, ChoiceData, StandardProcessing)
	...
	FileDialog.Filter     = NStr("en='External data processor file of event handlers (*.epf)|*.epf';ru='Файл внешней обработки обработчиков событий (*.epf)|*.epf';vi='Tệp bộ xử lý ngoài hàm sự kiện (*.epf)|*.epf'");
...
Procedure Done(Command)
	...
	If IsBlankString(Object.EventHandlersExternalDataProcessorFileName) Then
		...
		MessageToUser(NStr("en='Specify a name of the external data processor file.';ru='Укажите имя файла внешней обработки.';vi='Hãy chỉ ra tên tệp bộ xử lý ngoài.'"), "EventHandlersExternalDataProcessorFileName");
		Return;
	...
	If Not EventHandlersExternalDataProcessorFile.Exist() Then
		...
		MessageToUser(NStr("en='The specified file of external data processor does not exist.';ru='Указанный файл внешней обработки не существует.';vi='Không tồn tại tệp bộ xử lý ngoài đã chỉ ra.'"), "EventHandlersExternalDataProcessorFileName");
		Return;
...
Procedure ExportCodeHandlers(Command)
	...
	If Not IsBlankString(Object.TemporaryFileNameOfEventHandlers) Then
		...
		ButtonList.Add(DialogReturnCode.Yes, NStr("en='Export again';ru='Выгрузить повторно';vi='Kết xuất lại'"));
		ButtonList.Add(DialogReturnCode.No, NStr("en='Open module';ru='Открыть модуль';vi='Mở mô-đun'"));
		ButtonList.Add(DialogReturnCode.Cancel);
		
		NotifyDescription = New NotifyDescription("ExportHandlerCodeEnd", ThisObject);
		ShowQueryBox(NOTifyDescription, NStr("en='Debug engine with handler code is already exported.';ru='Модуль отладки с кодом обработчиков уже выгружен.';vi='Đã kết xuất mô-đun gỡ rối có mã hàm sự kiện.'"), ButtonList,,DialogReturnCode.No);
...
Procedure ShowEventHandlersInWindow()
	...
	If HandlerFile.Exist() AND HandlerFile.Size() <> 0 Then
		...
		TextDocument.Show(NStr("en='Handler debug engine';ru='Модуль отладки обработчиков';vi='Mô-đun gỡ rối của hàm sự kiện'"));
	EndIf;
	...
	If ErrorFile.Exist() AND ErrorFile.Size() <> 0 Then
		TextDocument = New TextDocument;
		TextDocument.Read(Object.TemporaryFileNameOfEventHandlers);
		TextDocument.Show(NStr("en='An error occurred when exporting the handler module';ru='Ошибки выгрузки модуля обработчиков';vi='Lỗi kết xuất mô-đun hàm sự kiện'"));
	EndIf;
