...
//  DecomposeStringIntoSubstringsArray("one two ", " ") - it will return an array of two items;
//  DecomposeStringIntoSubstringsArray("") - It returns an empty array;
...
Function deValueTypeAsString(ValueOrType) Export
	...
	Else
		...
		If Manager = Undefined Then
			...
			Text= NStr("en='Unknown type:';ru='Незвестный тип:';vi='Kiểu không xác định:'") + String(TypeOf(ValueType));
			MessageToUser(Text);
...
Procedure ExportEventHandlers(Cancel) Export
	...
	If ThisIsInteractiveMode Then
		...
		If ErrorFlag Then
			...
			MessageToUser(NStr("en='Errors occurred when exporting handlers.';ru='При выгрузке обработчиков были обнаружены ошибки.';vi='Khi kết xuất hàm sự kiện đã xuất hiện lỗi.'"));
			
		Else
			
			MessageToUser(NStr("en='Handlers are successfully exported.';ru='Обработчики успешно выгружены.';vi='Đã kết xuất thành công hàm sự kiện.'"));
...
Function ReplaceAlgorithmCallsByCodeOfTheseAlgorithmsInHandler(Handler, AlgorithmOwner, RequestedItemArray, Val PrefixString = "")
	...
	WriteTitleBlockAlgorithm(NewHandler, AlgorithmOwner, PrefixString, NStr("en='{ALGORITHM START}';ru='{НАЧАЛО АЛГОРИТМА}';vi='{ALGORITHM START}'"));
		...
		If HandlerAlgorithms.Count() <> 0 Then // IN this string there are algorithms calls.
			...
			For Each Algorithm IN HandlerAlgorithms Do
				...
				If RequestedItemArray.Find(Upper(Algorithm)) <> Undefined Then // Recursive algorithm call.
					...
					WriteTitleBlockAlgorithm(NewHandler, Algorithm, PrefixStringForInlineCode, NStr("en='{RECURSIVE ALGORITHM CALL}';ru='{RECURSIVE ALGORITHM CALL}';vi='{RECURSIVE ALGORITHM CALL}'"));
					
					OperatorString = NStr("en='CallException ""ALGORITHM RECURSIVE CALL: %1"";';ru='ВызватьИсключение ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: %1"";';vi='CallException ""LẦN GỌI THUẬT TOÁN ĐỆ QUY: %1"";'");
	...
	WriteTitleBlockAlgorithm(NewHandler, AlgorithmOwner, PrefixString, NStr("en='{ALGORITHM END}';ru='{КОНЕЦ АЛГОРИТМА}';vi='{ALGORITHM END}'"));
...
Procedure CommentsToObjectImport(NPP, Rulename, Source, ObjectType, GNPP = 0)
	...
	If CommentObjectProcessingFlag Then
		...
		If NPP <> 0 Then
			MessageString = SubstituteParametersInString(NStr("en='Import object No. %1';ru='Загрузка объекта № %1';vi='Kết nhập đối tượng số %1'"), NPP);
		Else
			MessageString = SubstituteParametersInString(NStr("en='Import object No. %1';ru='Загрузка объекта № %1';vi='Kết nhập đối tượng số %1'"), GNPP);
		EndIf;
...
Procedure DumpGroupOfProperties(Source, Receiver, IncomingData, OutgoingData, OCR, PGCR, PropertyCollectionNode, 
	...
	If ExportGroupToFile Then
		...
		Except
			...
			WriteInformationAboutErrorConversionHandlers(1000, ErrorDescription(), NStr("en='An error occurred when creating a temporary file for data export';ru='Ошибка при создании временного файла для выгрузки данных';vi='Lỗi khi tạo tệp tạm thời để kết xuất dữ liệu'"));
...
Function DumpByRule(Source					= Undefined,
	...
	If CommentObjectProcessingFlag Then
		...
		StringForUser = ?(GetRefNodeOnly, NStr("en='Conversion of reference to object: %1';ru='Конвертация ссылки на объект: %1';vi='Chuyển đổi tham chiếu đến đối tượng: %1'"), NStr("en='Object conversion: %1';ru='Конвертация объекта: %1';vi='Chuyển đổi đối tượng: %1'"));
...
Procedure ExportSelectionObject(Object, Rule, Properties=Undefined, IncomingData=Undefined, SelectionForDataExport = Undefined)
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = SubstituteParametersInString(NStr("en='Export object: %1';ru='Выгрузка объекта: %1';vi='Kết xuất đối tượng: %1'"), ObjectPresentation);
...
Procedure DumpDataByRule(Rule)
	...
	If CommentObjectProcessingFlag Then
		...
		MessageString = SubstituteParametersInString(NStr("en='Data export rule: %1 (%2)';ru='Правило выгрузки данных: %1 (%2)';vi='Quy tắc kết xuất dữ liệu: %1 (%2)'"), TrimAll(Rule.Name), TrimAll(Rule.Description));
			...
			If LastMetadataObject <> CurrentMetadataObject Then
				...
				If CurrentExportRuleRow <> Undefined Then
					...
					If CommentObjectProcessingFlag Then
						...
						MessageString = SubstituteParametersInString(NStr("en='Data export rule: %1 (%2)';ru='Правило выгрузки данных: %1 (%2)';vi='Quy tắc kết xuất dữ liệu: %1 (%2)'"),
...
Procedure ImportDataEchangeParameterValues()
	...
	If EventsAfterParameterImport.Property(Name, AfterParameterImportAlgorithm)
		...
		If HandlersDebugModeFlag Then
			...
			Raise NStr("en='Debugging of handler ""After parameter import"" is not supported.';ru='Отладка обработчика ""После загрузки параметра"" не поддерживается.';vi='Không hỗ trợ gỡ rối hàm sự kiện ""Sau khi kết nhập tham số"".'");
	...
	If Conversion.Property("AfterExchangeRuleImport", AfterExchangeRuleImportEventText)
		...
		Try
			...
			If HandlersDebugModeFlag Then
				...
				Raise NStr("en='Debugging of handler ""After exchange rules import"" is not supported.';ru='Отладка обработчика ""После загрузки правил обмена"" не поддерживается.';vi='Không hỗ trợ gỡ rối hàm sự kiện ""Sau khi kết nhập quy tắc trao đổi"".'");
		...
		Except
			...
			Text = NStr("en='Handler: ""AfterExchangeRulesImport"": %1';ru='Обработчик: ""ПослеЗагрузкиПравилОбмена"": %1';vi='Hàm sự kiện: ""AfterExchangeRulesImport"": %1'");
...
Procedure ReadData(ErrorInfoResultString = "") Export
				...
				If Not IsBlankString(AlgorithmText) Then
					...
					Try
						...
						If HandlersDebugModeFlag Then
							...
							Raise NStr("en='Debugging of handler ""After parameters import"" is not supported.';ru='Отладка обработчика ""После загрузки параметров"" не поддерживается.';vi='Không hỗ trợ gỡ rối hàm sự kiện ""Sau khi kết nhập tham số"".'");
						...
						If Cancel = True Then
							...
							If Not IsBlankString(CancelReason) Then
								ExceptionString = SubstituteParametersInString(NStr("en='Data import was canceled as: %1';ru='Загрузка данных отменена по причине: %1';vi='Đã hủy bỏ kết nhập dữ liệu theo nguyên nhân: %1'"), CancelReason);
				...
				If Not IsBlankString(AlgorithmText) Then
					...
					Try
						...
						If HandlersDebugModeFlag Then
							...
							Raise NStr("en='Global algorithm debugging is not supported.';ru='Отладка глобального алгоритма не поддерживается.';vi='Không hỗ trợ gỡ rối thuật toán toàn cục.'");
	...
	Except
		...
		ErrorString = SubstituteParametersInString(NStr("en='An error occurred when importing data: %1';ru='Ошибка при загрузке данных: %1';vi='Lỗi khi kết nhập dữ liệu: %1'"), ErrorDescription());
...
Function ExecuteActionsBeforeReadingData(DataRow = "") Export
	...
	MessageString = SubstituteParametersInString(NStr("en='Import start: %1';ru='Начало загрузки: %1';vi='Bắt đầu kết nhập: %1'"), CurrentSessionDate());
	...
	If Not IsBlankString(Conversion.BeforeDataImport) Then
		...
		Except
			ErrorMessageString = WriteInformationAboutErrorConversionHandlers(22, ErrorDescription(), NStr("en='BeforeDataImport (Conversion)';ru='ПередЗагрузкойДанных (конвертация)';vi='BeforeDataImport (chuyển đổi)'"));
			Cancel = True;
		EndTry;
...
Procedure ExecuteActionsAfterDataReadIsCompleted() Export
	...
	If Not IsBlankString(Conversion.AfterDataImport) Then
		...
		Except
			ErrorMessageString = WriteInformationAboutErrorConversionHandlers(23, ErrorDescription(), NStr("en='AfterDataImport (conversion)';ru='ПослеЗагрузкиДанных (конвертация)';vi='AfterDataImport (chuyển đổi)'"));
		EndTry;
	...
	WriteInExecutionProtocol(SubstituteParametersInString(
		NStr("en='Import end: %1';ru='Окончание загрузки: %1';vi='Kết thúc kết nhập: %1'"), CurrentSessionDate()), , False, , , True);
	WriteInExecutionProtocol(SubstituteParametersInString(
		NStr("en='Imported objects: %1';ru='Загружено объектов: %1';vi='Đã kết nhập đối tượng: %1'"), mImportedObjectCounter), , False, , , True);
	
	FinishExchangeProtocolLogging();
	
	If ThisIsInteractiveMode Then
		MessageToUser(NStr("en='Data import is complete.';ru='Загрузка данных завершена.';vi='Đã hoàn thành kết nhập dữ liệu.'"));
	EndIf;
...
Procedure CompressResultantExchangeFile()
	...
	Try
		...
		Archiver = New ZipFileWriter(ExchangeFileName, ExchangeFileCompressionPassword, NStr("en='Data exchange file';ru='Файл обмена данными';vi='Tệp trao đổi dữ liệu'"));
...
Procedure PassInformationAboutRecordsToReceiver(InformationToWriteToFile, ErrorStringInTargetInfobase = "") Export
	...
	If Not IsBlankString(ErrorStringInTargetInfobase) Then
		...
		MessageString = SubstituteParametersInString(NStr("en='Import in the target: %1';ru='Загрузка в приемнике: %1';vi='Kết nhập tại đích: %1'"), ErrorStringInTargetInfobase);
...
Procedure RunExport() Export
	...
	WriteInExecutionProtocol(SubstituteParametersInString(
		NStr("en='Export start: %1';ru='Начало выгрузки: %1';vi='Bắt đầu kết xuất: %1'"), CurrentSessionDate()), , False, , , True);
	...
	Except
		...
		WriteInExecutionProtocol(SubstituteParametersInString(
			NStr("en='An error occurred when exporting data: %1';ru='Ошибка при выгрузке данных: %1';vi='Lỗi kết xuất dữ liệu: %1'"), ErrorString), Undefined, True, , , True);
	...
	WriteInExecutionProtocol(SubstituteParametersInString(
		NStr("en='Export end: %1';ru='Окончание выгрузки: %1';vi='Kết thúc kết xuất: %1'"), CurrentSessionDate()), , False, , ,True);
	WriteInExecutionProtocol(SubstituteParametersInString(
		NStr("en='Exported objects: %1';ru='Выгружено объектов: %1';vi='Đã kết xuất đối tượng: %1'"), mExportedObjectCounter), , False, , , True);
	...
	If ThisIsInteractiveMode Then
		MessageToUser(NStr("en='Data is exported.';ru='Выгрузка данных завершена.';vi='Đã hoàn thành kết xuất dữ liệu.'"));
	EndIf;
...
Procedure MessagesInitialization()
	...
	deMessages.Insert(2,  NStr("en='An error occurred when unpacking an exchange file. The file is locked';ru='Ошибка распаковки файла обмена. Файл заблокирован';vi='Lỗi giải nén tệp trao đổi. Tệp đã bị phong tỏa'"));
	deMessages.Insert(3,  NStr("en='The specified exchange rules file does not exist';ru='Указанный файл правил обмена не существует';vi='Không tồn tại tệp quy tắc trao đổi đã chỉ ra'"));
	deMessages.Insert(4,  NStr("en='An error occurred when creating COM object Msxml2.DOMDocument';ru='Ошибка при создании COM-объекта Msxml2.DOMDocument';vi='Lỗi khi tạo COM-объекта Msxml2.DOMDocument'"));
	deMessages.Insert(5,  NStr("en='An error occurred when opening the exchange file';ru='Ошибка открытия файла обмена';vi='Lỗi mở tệp trao đổi'"));
	deMessages.Insert(6,  NStr("en='An error occurred when importing exchange rules';ru='Ошибка при загрузке правил обмена';vi='Lỗi khi kết nhập quy tắc trao đổi'"));
	deMessages.Insert(7,  NStr("en='Error in exchange rule format';ru='Ошибка формата правил обмена';vi='Lỗi định dạng quy tắc trao đổi'"));
	deMessages.Insert(8,  NStr("en='Incorrect file name for data export';ru='Не корректно указано имя файла для выгрузки данных';vi='Đã chỉ ra không đúng tên tệp để kết xuất dữ liệu'"));
	deMessages.Insert(9,  NStr("en='Error in exchange file format';ru='Ошибка формата файла обмена';vi='Lỗi định dạng tệp trao đổi'"));
	deMessages.Insert(10, NStr("en='File name for data export is not specified (Data file name)';ru='Не указано имя файла для выгрузки данных (Имя файла данных)';vi='Chưa chỉ ra tên tệp để kết xuất dữ liệu (Tên tệp dữ liệu)'"));
	deMessages.Insert(11, NStr("en='Link to a non-existing metadata object in the exchange rules';ru='Ссылка на несуществующий объект метаданных в правилах обмена';vi='Tham chiếu đến đối tượng Metadata không tồn tại trong các quy tắc trao đổi'"));
	deMessages.Insert(12, NStr("en='File name with exchange rules is not specified (Rule file name)';ru='Не указано имя файла с правилами обмена (Имя файла правил)';vi='Chưa chỉ ra tên tệp với các quy tắc trao đổi (Tên tệp quy tắc)'"));
	
	deMessages.Insert(13, NStr("en='An error occurred when receiving a value of the object property (by the name of the source property)';ru='Ошибка получения значения свойства объекта (по имени свойства источника)';vi='Lỗi nhận giá trị thuộc tính đối tượng (theo tên thuộc tính nguồn)'"));
	deMessages.Insert(14, NStr("en='An error occurred when receiving the object property value (by the target property name)';ru='Ошибка получения значения свойства объекта (по имени свойства приемника)';vi='Lỗi nhận giá trị thuộc tính đối tượng (theo tên thuộc tính đích)'"));
	
	deMessages.Insert(15, NStr("en='File name for data import is not specified (File name for import)';ru='Не указано имя файла для загрузки данных (Имя файла для загрузки)';vi='Chưa chỉ ra tên tệp để kết nhập dữ liệu (Tên tệp để kết nhập)'"));
	
	deMessages.Insert(16, NStr("en='An error occurred while receiving value of subobject property (by source property name)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства источника)';vi='Lỗi nhận giá trị thuộc tính đối tượng trực thuộc (theo tên thuộc tính nguồn)'"));
	deMessages.Insert(17, NStr("en='An error occurred while receiving value of subobject property (by target property name)';ru='Ошибка получения значения свойства подчиненного объекта (по имени свойства приемника)';vi='Lỗi nhận giá trị thuộc tính đối tượng trực thuộc (theo tên thuộc tính đích)'"));
	
	deMessages.Insert(19, NStr("en='An error occurred in event handler BeforeObjectImport';ru='Ошибка в обработчике события ПередЗагрузкойОбъекта';vi='Lỗi trong hàm sự kiện BeforeObjectImport'"));
	deMessages.Insert(20, NStr("en='An error occurred in event handler OnObjectImport';ru='Ошибка в обработчике события ПриЗагрузкеОбъекта';vi='Lỗi trong hàm sự kiện OnObjectImport'"));
	deMessages.Insert(21, NStr("en='An error occurred in event handler AfterObjectImport';ru='Ошибка в обработчике события ПослеЗагрузкиОбъекта';vi='Lỗi trong hàm sự kiện AfterObjectImport'"));
	deMessages.Insert(22, NStr("en='An error occurred in event handler BeforeDataImport (conversion)';ru='Ошибка в обработчике события ПередЗагрузкойДанных (конвертация)';vi='Lỗi trong hàm sự kiện BeforeDataImport (chuyển đổi)'"));
	deMessages.Insert(23, NStr("en='An error occurred in event handler AfterDataImport (conversion)';ru='Ошибка в обработчике события ПослеЗагрузкиДанных (конвертация)';vi='Lỗi trong hàm sự kiện AfterDataImport (chuyển đổi)'"));
	deMessages.Insert(24, NStr("en='An error occurred when removing an object';ru='Ошибка при удалении объекта';vi='Lỗi khi xóa đối tượng'"));
	deMessages.Insert(25, NStr("en='An error occurred when writing the document';ru='Ошибка при записи документа';vi='Lỗi khi ghi chứng từ'"));
	deMessages.Insert(26, NStr("en='An error occurred when writing the object';ru='Ошибка записи объекта';vi='Lỗi ghi đối tượng'"));
	deMessages.Insert(27, NStr("en='An error occurred in event handler BeforeProcessClearingRule';ru='Ошибка в обработчике события ПередОбработкойПравилаОчистки';vi='Lỗi trong hàm sự kiện BeforeProcessClearingRule'"));
	deMessages.Insert(28, NStr("en='An error occurred in event handler AfterClearingRuleProcessing';ru='Ошибка в обработчике события ПослеОбработкиПравилаОчистки';vi='Lỗi trong hàm sự kiện AfterClearingRuleProcessing'"));
	deMessages.Insert(29, NStr("en='An error occurred in event handler BeforeDeleteObject';ru='Ошибка в обработчике события ПередУдалениемОбъекта';vi='Lỗi trong hàm sự kiện BeforeDeleteObject'"));
	
	deMessages.Insert(31, NStr("en='An error occurred in event handler BeforeProcessExportRule';ru='Ошибка в обработчике события ПередОбработкойПравилаВыгрузки';vi='Lỗi trong hàm sự kiện BeforeProcessExportRule'"));
	deMessages.Insert(32, NStr("en='An error occurred in event handler AfterDumpRuleProcessing';ru='Ошибка в обработчике события ПослеОбработкиПравилаВыгрузки';vi='Lỗi trong hàm sự kiện AfterDumpRuleProcessing'"));
	deMessages.Insert(33, NStr("en='An error occurred in event handler BeforeObjectExport';ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';vi='Lỗi trong hàm sự kiện BeforeObjectExport'"));
	deMessages.Insert(34, NStr("en='An error occurred in event handler AfterObjectExport';ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';vi='Lỗi trong hàm sự kiện AfterObjectExport'"));
	
	deMessages.Insert(39, NStr("en='An error occurred when executing the algorithm from the exchange file';ru='Ошибка при выполнении алгоритма, содержащегося в файле обмена';vi='Lỗi khi thực hiện thuật toán có trong tệp trao đổi'"));
	
	deMessages.Insert(41, NStr("en='An error occurred in event handler BeforeObjectExport';ru='Ошибка в обработчике события ПередВыгрузкойОбъекта';vi='Lỗi trong hàm sự kiện BeforeObjectExport'"));
	deMessages.Insert(42, NStr("en='An error occurred in event handler OnObjectExport';ru='Ошибка в обработчике события ПриВыгрузкеОбъекта';vi='Lỗi trong hàm sự kiện OnObjectExport'"));
	deMessages.Insert(43, NStr("en='An error occurred in event handler AfterObjectExport';ru='Ошибка в обработчике события ПослеВыгрузкиОбъекта';vi='Lỗi trong hàm sự kiện AfterObjectExport'"));
	
	deMessages.Insert(45, NStr("en='Object conversion rule is not found';ru='Не найдено правило конвертации объектов';vi='Không tìm thấy quy tắc chuyển đổi đối tượng'"));
	
	deMessages.Insert(48, NStr("en='An error occurred in event handler BeforeExportProcessor of the property group';ru='Ошибка в обработчике события ПередОбработкойВыгрузки группы свойств';vi='Lỗi trong hàm sự kiện BeforeExportProcessor nhóm thuộc tính'"));
	deMessages.Insert(49, NStr("en='An error occurred in event handler AfterExportProcessor of the property group';ru='Ошибка в обработчике события ПослеОбработкиВыгрузки группы свойств';vi='Lỗi trong hàm sự kiện AfterExportProcessor nhóm thuộc tính'"));
	deMessages.Insert(50, NStr("en='Error in event handler BeforeExport (of collection object)';ru='Ошибка в обработчике события ПередВыгрузкой (объекта коллекции)';vi='Lỗi trong hàm sự kiện BeforeExport (đối tượng tập hợp)'"));
	deMessages.Insert(51, NStr("en='Error in event handler OnExport (of collection object)';ru='Ошибка в обработчике события ПриВыгрузке (объекта коллекции)';vi='Lỗi trong hàm sự kiện OnExport (đối tượng tập hợp)'"));
	deMessages.Insert(52, NStr("en='Error in event handler AfterExport (of collection object)';ru='Ошибка в обработчике события ПослеВыгрузки (объекта коллекции)';vi='Lỗi trong hàm sự kiện AfterExport (đối tượng tập hợp)'"));
	deMessages.Insert(53, NStr("en='An error occurred in global event handler BeforeObjectImporting (conversion)';ru='Ошибка в глобальном обработчике события ПередЗагрузкойОбъекта (конвертация)';vi='Lỗi trong hàm sự kiện toàn cục BeforeObjectImporting (chuyển đổi)'"));
	deMessages.Insert(54, NStr("en='An error occurred in global event handler AfterObjectImport (conversion)';ru='Ошибка в глобальном обработчике события ПослеЗагрузкиОбъекта (конвертация)';vi='Lỗi trong hàm sự kiện toàn cục AfterObjectImport (chuyển đổi)'"));
	deMessages.Insert(55, NStr("en='An error occurred in event handler BeforeExport (properties)';ru='Ошибка в обработчике события ПередВыгрузкой (свойства)';vi='Lỗi trong hàm sự kiện BeforeExport (thuộc tính)'"));
	deMessages.Insert(56, NStr("en='An error occurred in event handler OnExport (properties)';ru='Ошибка в обработчике события OnExport (свойства)';vi='Lỗi trong hàm sự kiện OnExport (thuộc tính)'"));
	deMessages.Insert(57, NStr("en='An error occurred in event handler AfterExport (properties)';ru='Ошибка в обработчике события ПослеВыгрузки (свойства)';vi='Lỗi trong hàm sự kiện AfterExport (thuộc tính)'"));
	
	deMessages.Insert(62, NStr("en='An error occurred in event handler BeforeDataExport (conversion)';ru='Ошибка в обработчике события ПередВыгрузкойДанных (конвертация)';vi='Lỗi trong hàm sự kiện BeforeDataExport (chuyển đổi)'"));
	deMessages.Insert(63, NStr("en='An error occurred in event handler AfterDataExport (conversion)';ru='Ошибка в обработчике события ПослеВыгрузкиДанных (конвертация)';vi='Lỗi trong hàm sự kiện AfterDataExport (chuyển đổi)'"));
	deMessages.Insert(64, NStr("en='An error occurred in global event handler BeforeObjectConversion (conversion)';ru='Ошибка в глобальном обработчике события ПередКонвертациейОбъекта (конвертация)';vi='Lỗi trong hàm sự kiện toàn cục BeforeObjectConversion (chuyển đổi)'"));
	deMessages.Insert(65, NStr("en='An error occurred in global event handler BeforeObjectExport (conversion)';ru='Ошибка в глобальном обработчике события ПередВыгрузкойОбъекта (конвертация)';vi='Lỗi trong hàm sự kiện toàn cục BeforeObjectExport (chuyển đổi)'"));
	deMessages.Insert(66, NStr("en='An error occurred when receiving a subordinate object collection from the incoming data';ru='Ошибка получения коллекции подчиненных объектов из входящих данных';vi='Lỗi nhận tập hợp đối tượng trực thuộc từ dữ liệu đến'"));
	deMessages.Insert(67, NStr("en='An error occurred when receiving the subordinate object properties from the incoming data';ru='Ошибка получения свойства подчиненного объекта из входящих данных';vi='Lỗi nhận thuộc tính đối tượng trực thuộc từ dữ liệu đến'"));
	deMessages.Insert(68, NStr("en='An error occurred when receiving the object properties from the incoming data';ru='Ошибка получения свойства объекта из входящих данных';vi='Lỗi nhận thuộc tính đối tượng từ dữ liệu đến'"));
	
	deMessages.Insert(69, NStr("en='An error occurred in global event handler AfterObjectExport (conversion)';ru='Ошибка в глобальном обработчике события ПослеВыгрузкиОбъекта (конвертация)';vi='Lỗi trong hàm sự kiện toàn cục AfterObjectExport (chuyển đổi)'"));
	
	deMessages.Insert(71, NStr("en='Match for the Source value is not found';ru='Не найдено соответствие для значения Источника';vi='Không tìm thấy sự tương ứng đối với giá trị Nguồn'"));
	
	deMessages.Insert(72, NStr("en='An error occurred when exporting data for the exchange plan node';ru='Ошибка при выгрузке данных для узла плана обмена';vi='Lỗi khi kết xuất dữ liệu đối với nút của sơ đồ trao đổi'"));
	
	deMessages.Insert(73, NStr("en='An error occurred in event handler SearchFieldsSequence';ru='Ошибка в обработчике события ПоследовательностьПолейПоиска';vi='Lỗi trong hàm sự kiện SearchFieldsSequence'"));
	
	deMessages.Insert(74, NStr("en='Import exchange rules for data export again';ru='Необходимо перезагрузить правила обмена для выгрузки данных';vi='Cần khởi động lại quy tắc trao đổi để kết xuất dữ liệu'"));
	
	deMessages.Insert(75, NStr("en='An error occurred when executing the algorithm after import of the parameter values';ru='Ошибка при выполнении алгоритма после загрузки значений параметров';vi='Lỗi khi thực hiện thuật toán sau khi kết nhập giá trị tham số'"));
	
	deMessages.Insert(76, NStr("en='An error occurred in event handler AfterObjectExportToFile';ru='Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл';vi='Lỗi trong hàm sự kiện AfterObjectExportToFile'"));
	
	deMessages.Insert(77, NStr("en='File of external data processor with connected procedures of event handlers is not specified';ru='Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий';vi='Chưa chỉ ra tệp bộ xử lý ngoài có các thủ tục của hàm sự kiện được kết nối'"));
	
	deMessages.Insert(78, NStr("en='An error occurred when creating an external data processor from a file with event handler procedures';ru='Ошибка создания внешней обработки из файла с процедурами обработчиков событий';vi='Lỗi tạo bộ xử lý ngoài từ tệp có các thủ tục của hàm sư kiện'"));
...
"вызовом, то укажите режим ""алгоритмы отлаживать как процедуры"" и повторите выгрузку.';vi='Mã thuật toán không thể được tích hợp vào hàm sự kiện do đã thấy các lần gọi thuật toán đệ quy."
"Nếu trong quá trình gỡ rối không thể gỡ rối mã"
"thuật toán thì hãy chỉ ra chế độ ""không gỡ rối thuật toán"" Nếu cần thực hiện gỡ rối thuật toán với lần gọi đệ quy"
"thì hãy chỉ ra chế độ ""thuật toán gỡ rối như thủ tục"" và hãy thử kết xuất.'"));
	
	deMessages.Insert(80, NStr("en='Data exchange can be executed only if you have full rights';ru='Обмен данными можно проводить только под полными правами';vi='Có thể trao đổi dữ liệu chỉ với quyền đầy đủ'"));
	
	deMessages.Insert(1000, NStr("en='An error occurred when creating a temporary file of data export';ru='Ошибка при создании временного файла выгрузки данных';vi='Lỗi khi tạo tệp tạm thời kết xuất dữ liệu'"));
...
Function DefineSufficiencyOfParametersForConnectionToInformationBase(ConnectionStructure, ConnectionString = "", ErrorMessageString = "")
	...
	If ConnectionStructure.FileModeVersion  Then
		...
		If IsBlankString(ConnectionStructure.InfobaseDirectory) Then
			...
			ErrorMessageString = NStr("en='Directory of the target infobase is not specified';ru='Не задан каталог информационной базы-приемника';vi='Chưa xác định thư mục cơ sở thông tin đích'");
	...
	Else
		...
		If IsBlankString(ConnectionStructure.ServerName) Then
			...
			ErrorMessageString = NStr("en='Name of 1C: Enterprise server of the target infobase is not specified';ru='Не задано имя сервера 1С:Предприятия информационной базы-приемника';vi='Chưa xác định tên Server 1C:DOANH NGHIỆP của cơ sở thông tin đích'");
		...
		If IsBlankString(ConnectionStructure.InfobaseNameAtServer) Then
			...
			ErrorMessageString = NStr("en='Name of the target infobase is not specified on 1C: Enterprise server';ru='Не задано имя информационной базы-приемника на сервере 1С:Предприятия';vi='Chưa xác định tên cơ sở thông tin đích trên Server 1C:DOANH NGHIỆP'");
...
"произошла следующая ошибка: %1';vi='Khi thử kết nối với COM-server"
"đã xảy ra lỗi sau: %1'");
...
Function RunConnectionToReceiverIB() Export
	...
	Except
		...
		Text = NStr("en='When trying to create handler UniversalXMLDataExchange, an error occurred: %1';ru='При попытке создания обработки УниверсальныйОбменДаннымиXML произошла ошибка: %1';vi='Khi thử tạo bộ xử lý UniversalXMLDataExchange đã xảy ra lỗi: %1'");
		Text = SubstituteParametersInString(Text, BriefErrorDescription(ErrorInfo()));
		MessageToUser(Text);
		ConnectionResult = Undefined;
	EndTry;
...
//
// Example:
//  SubstituteParametersInString(NStr("en='%1 went to %2';ru='%1 пошел в %2';vi='%1 chuyển sang %2'"), "John", "Zoo") = "John went to the Zoo".
//
Function SubstituteParametersInString(Val LookupString,
