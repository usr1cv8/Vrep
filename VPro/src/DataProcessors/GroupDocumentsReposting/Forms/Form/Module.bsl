...
Procedure ButtonExecute(Command)
	...
	If OnlySelectedCompanies = 1 AND Not ValueIsFilled(CompaniesTable) Then
		MessageText = NStr("en='Company list is filled in incorrectly.';ru='Не корректно заполнен список организаций.';vi='Chưa điền đúng danh sách doanh nghiệp.'");
...
"%2';vi='Đã kết chuyển lại chứng từ:"
"- đã kết chuyển %1 chứng từ;"
"%2'");
	If FailedToPost = 0 Then
		ErrorsMessageBox = NStr("en=' - failed to post documents: %1';ru=' - не удалось провести документов: %1';vi=' - không thể kết chuyển chứng từ: %1'");
	Else
		ErrorsMessageBox = NStr("en=' - failed to post documents: %1';ru=' - не удалось провести документов: %1';vi=' - không thể kết chuyển chứng từ: %1'");
...
Procedure InitializingCommandsDocumentAtClient()
	...
	If Result = 1 Then
		ShowMessageBox(Undefined,NStr("en='Reposting is already being performed. Await completion.';ru='Перепроведение уже выполняется. Ожидайте завершения.';vi='Đã thực hiện kết chuyển lại. Hãy chờ đến khi kết thúc.'"));
		Return;
	ElsIf Result = 2 Then
		ShowMessageBox(Undefined,NStr("en='Another user is already performing reposting.';ru='Перепроведение уже выполняется другим пользователем.';vi='Việc kết chuyển lại đã được thực hiện bởi người sử dụng khác.'"));
		Return;
	EndIf;
...
Function RunOnServer()
	...
	Else
		JobDescription = NStr("en='Group document reposting';ru='Групповое перепроведение документов';vi='Kết chuyển lại chứng từ theo nhóm'");
