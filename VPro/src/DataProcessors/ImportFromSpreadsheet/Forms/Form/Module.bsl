...
Procedure ReadFileSourceEnd(Exist, AdditionalParameters) Export
	...
	If Exist Then
		...
		Try
			...
			If LineCount < 1 Then
				...
				MessageText = NStr("en='No data in the file.';ru='В файле нет данных!';vi='Trong tệp không có dữ liệu!'");
		...
		Except
			...
			MessageText = NStr("en='File is not read.';ru='Файл не прочитан.';vi='Chưa đọc tệp.'");
			SmallBusinessServer.ShowMessageAboutError(, MessageText);
	...
	Else
		...
		MessageText = NStr("en='The %File% file does not exist.';ru='Файла %File% не существует!';vi='Không tồn tại tệp %File%!'");
			...
			Else
				Try
					...
					If Number(StringNumber) = 0 Then
						MessageText = NStr("en='Cannot import row No%LineNumber% because the value in the %ColumnNo% column is 0.';ru='Строка №%НомерСтроки% не может быть загружена, т.к. значение в колонке ""%НомерКолонки%"" равно 0.';vi='Dòng số%LineNumber% không thể kết nhập bởi vì giá trị trong cột ""%ColumnNo%"" bằng 0.'");
					...
					EndIf;
				Except
					MessageText = NStr("en='Cannot import row No%LineNumber% because value in the %ColumnNo% column is not a number.';ru='Строка №%НомерСтроки% не может быть загружена, т.к. значение в колонке ""%НомерКолонки%"" не является числом.';vi='Dòng số%LineNumber% không thể kết nhập bởi vì giá trị trong cột ""%ColumnNo%"" không phải là số.'");
					MessageText = StrReplace(MessageText, "%LineNumber%", String(RowCounter+1));
					MessageText = StrReplace(MessageText, "%ColumnNumber%", ColumnNumber+1);
					Message = New UserMessage;
					Message.Text = MessageText;
...
Procedure Import(Cancel)
	...
	For Each ImportRow IN Object.ImportingList Do
		...
		ElsIf OperationKind = "ProductsAndServices" Then
			...
			ObjectToWrite.ProductsAndServicesCategory = Catalogs.ProductsAndServicesCategories.WithoutCategory;
...
Procedure GreetingNext(Command)
	...
	If AttachFileSystemExtension() AND
		...
		MessageText = NStr("en='First specify path to the import file.';ru='Вначале укажите путь к файлу загрузки!';vi='Trước tiên hãy chỉ ra đường dẫn đến tệp kết nhập!'");
...
Procedure MappingNext(Command)
	...
	For AttributeCounter = 0 to Attributes.Count() - 1 Do
		If Attributes[AttributeCounter].IsRequired and Attributes[AttributeCounter].ColumnNumber = 0 Then
			Message = New UserMessage;
			Message.Text = NStr("en='It is required  to fill the necessary attribute columns';vi='Bắt buộc phải điền các cột thuộc tính cần thiết'");
