...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not DataExchangeSaaSReUse.DataSynchronizationSupported() Then
		ErrorText = NStr("en='Data synchronization for this configuration is not supported.';ru='Синхронизация данных для конфигурации не поддерживается!';vi='Không hỗ trợ đồng bộ hóa dữ liệu đối với cấu hình!'");
		
	ElsIf Not Parameters.Property("InfobaseNode", Object.InfobaseNode) Then
		ErrorText = NStr("en='Data processor is not intended for direct usage.';ru='Обработка не предназначена для непосредственного использования.';vi='Bộ xử lý không được dùng để sử dụng trực tiếp.'");
		
	ElsIf Object.InfobaseNode.IsEmpty() Then
		ErrorText = NStr("en='Data exchange setting is not found.';ru='Настройка обмена данными не найдена.';vi='Chưa tìm thấy tùy chỉnh trao đổi dữ liệu.'");
...
Procedure CorrespondentIdleHandler()
	...
	Else
		LongOperationState.ErrorInfo = NStr("en='Correspondent notification error';ru='Ошибка сообщения корреспонденту';vi='Lỗi thông điệp cho cơ sở thông tin đối ứng'");
...
Function BackgroundJobDumpsAtServer()
	...
	Task = BackgroundJobs.Execute("DataExchangeSaaS.ExchangeOnDemand", 
		BackgroundExecutionParameters, , NStr("en='Interactive exchange on request.';ru='Интерактивный обмен по требованию.';vi='Trao đổi trực tác theo yêu cầu.'"));
