...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If ValueIsFilled(Parameters.AddressInStorage) Then
		...
		If ValueIsFilled(DataStructure.ActionsStructure) Then
			...
			Else
				Raise NStr("en='Print mode is not set';ru='Режим печати не установлен';vi='Chưa đặt chế độ in'");;
			EndIf;
...
Function ShowUserAlertAboutPossibleError(Text, Quantity, CountTotal)
	...
	If Quantity < CountTotal Then
		...
		Text = Text
		        + ?(Text <> "", Chars.LF, "")
		        + NStr("en='Selection mark is set for %QuantitySelected% strings from %QuantityTotal%.';ru='Отметка выбора установлена для %КоличествоВыбранных% строк из %КоличествоВсего%.';vi='Đánh dấu chọn được thiết lập đối với %QuantitySelected% dòng trong số %QuantityTotal%.'");
	
		Text = Text
		        + Chars.LF
		        + NStr("en='Check: either the quantity is not filled in or the price tag (label) template is not specified. In this case, selection mark for lines cannot be specified.';ru='Проверьте: не заполнены либо количество, либо не задан шаблон ценника (этикетки). В этом случае отметка выбора для строк не может быть установлена.';vi='Hãy kiểm tra: chưa điền số lượng, hoặc chưa điền khuôn mẫu nhãn hiệu (mác giá). Trong trường hợp này không thể đặt dấu chọn đối với dòng.'");
...
Procedure FillInventoryTable(Command)
	...
	QuestionText = NStr("en='During repopulation all information entered manually will be lost, continue?';ru='При перезаполнении все введенные вручную данные будут потеряны, продолжить?';vi='Khi điền lại, tất cả dữ liệu đã nhập thủ công sẽ mất, tiếp tục?'");
...
Procedure Print(Command)
	...
	If SelectedRows.Count() = 0 Then
		...
		ShowMessageBox(Undefined,NStr("en='No goods selected';ru='Не выбрано ни одного товара';vi='Chưa chọn hàng hóa nào'"));
		Return;
	...
	If ThisForm.CheckFilling() Then
		...
		PrintParameters.Insert("FormTitle", NStr("en='Print price tags and labels';ru='Печать этикеток и ценников';vi='In nhãn tem và nhãn giá'"));
	...
	If CurrentData.Selected Then
		...
		If Mode = "TagsAndLabePrinting" Then
			...
			If (ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message.Text = NStr("en='Number of price tags is required';ru='Не заполнено количество ценников';vi='Chưa điền số lượng nhãn giá'");
			...
			ElsIf (NOT ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en='The price tags template is not selected';ru='Не выбран шаблон ценника';vi='Chưa chọn khuôn mẫu nhãn giá'");
			...
			ElsIf (NOT ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en='Number of labels not specified';ru='Не заполнено количество этикеток';vi='Chưa điền số lượng nhãn tem'");
			...
			ElsIf (NOT ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en='Label template is not selected';ru='Не выбран шаблон этикетки';vi='Chưa chọn khuôn mẫu nhãn tem'");
			...
			ElsIf (   ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en='Price tag and(or) label quantity is not filled in';ru='Не заполнено количество ценников и(или) этикеток';vi='Chưa điền số lượng nhãn giá và (hoặc) nhãn tem'");
				Message.Field = "Object.Products["+CurrentRow+"].PriceTagsQuantity";
				Message.Message();
			...
			ElsIf (NOT ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en='Price tag and(or) label templates are not selected';ru='Не выбраны шаблоны ценников и(или) этикеток';vi='Chưa chọn khuôn mẫu mác giá và (hoặc) nhãn hiệu'");
				Message.Field = "Object.Products["+CurrentRow+"].PriceTagsTemplate";
				Message.Message();
			...
			ElsIf (   ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en='The number of price tags and(or) the label template is not filled out';ru='Не заполнено количество ценников и(или) шаблон этикетки';vi='Chưa điền số lượng nhãn giá và (hoặc) khuôn mẫu nhãn tem'");
				Message.Field = "Object.Products["+CurrentRow+"].PriceTagsQuantity";
				Message.Message();
			...
			ElsIf (NOT ValueIsFilled(CurrentData.PriceTagsTemplate)
				...
				Message = New UserMessage;
				Message.Text = NStr("en='The number of labels and(or) the price tag template is required';ru='Не заполнено количество этикеток и(или) шаблон ценника';vi='Chưa điền số lượng nhãn tem và (hoặc) khuôn mẫu nhãn giá'");
				Message.Field = "Object.Products["+CurrentRow+"].LabelsQuantity";
				Message.Message();
...
Procedure SetPriceTagsQuantity(Command)
	...
	ShowInputNumber(New NotifyDescription("SetPriceTagsQuantityEnd", ThisObject, New Structure("ValueSelected", ValueSelected)), ValueSelected, NStr("en='Enter price tag quantity';ru='Введите количество ценников';vi='Nhập số lượng nhãn giá'"), 10, 2);
...
Procedure SetPriceTagsQuantityEnd(Result, AdditionalParameters) Export
    ...
    If (Result <> Undefined) Then
        ...
        Text = NStr("en='%Quantity% price tags are set.';ru='Установлено количество ценников %Количество%.';vi='Đã đặt số lượng mác giá %Quantity%.'");
...
Procedure SetLabelsQuantityEnd(Result, AdditionalParameters) Export
    ...
    If (Result <> Undefined) Then
        ...
        Text = NStr("en='%Quantity% labels are set.';ru='Установлено количество этикеток %Количество%.';vi='Đã đặt số lượng nhãn hiệu %Quantity%.'");
        Text = StrReplace(Text, "%Quantity%", ValueSelected);
...
Procedure SetPriceTagsTemplateEnd(Result, AdditionalParameters) Export
    ...
    If (Result <> Undefined) Then
        ...
        Text = NStr("en='The %Template% template set.';ru='Установлен шаблон ""%Шаблон%"".';vi='Đã đặt khuôn mẫu ""%Template%"".'");
...
Procedure SetModeTagsPrintingAtServer()
	...
	If PrintFromDocument Then
		Title = Nstr("en='Price tags printing from document';ru='Печать ценников из документа';vi='In nhãn giá từ chứng từ'");
	Else
		Title = Nstr("en='Tags printing';ru='Печать ценников';vi='In nhãn giá'");
	EndIf;
...
Procedure SetModeTagsPrintingAndLabelsAtServer()
	...
	If PrintFromDocument Then
		Title = Nstr("en='Printing labels and price tags from document';ru='Печать этикеток и ценников из документа';vi='In nhãn tem và nhãn giá từ chứng từ'");
	Else
		Title = Nstr("en='Print labels and tags';ru='Печать этикеток и ценников';vi='In nhãn tem và nhãn giá'");
	EndIf;
...
Procedure SetModeLabelsPrintingAtServer()
	...
	If PrintFromDocument Then
		Title = Nstr("en='Labels printing from document';ru='Печать этикеток из документа';vi='In nhãn tem từ chứng từ'");
	Else
		Title = Nstr("en='Labels printing';ru='Печать этикеток';vi='In nhãn tem'");
	EndIf;
...
Procedure SetMode(RegimeParameter)
	...
	ElsIf RegimeParameter = "LabelsPrinting" Then
		SetModeLabelsPrintingAtServer();
	Else
		Raise NStr("en='Set print mode is not supported';ru='Установленный режим печати не поддерживается';vi='Chưa hỗ trợ chế độ in đã thiết lập'");
	EndIf;
