...
Procedure Initialization() Export
	...
	mBodyMessageIsSimple	= NStr("en='Data exchange message';ru='Сообщение обмена данными';vi='thông điệp trao đổi dữ liệu'");
	mBodyMessageIsCompressed	= NStr("en='Compressed data exchange message';ru='Сжатое сообщение обмена данными';vi='thông điệp trao đổi dữ liệu được nén lại'");
	mBodyMessageIsBatched	= NStr("en='Package message of the data exchange';ru='Пакетное сообщение обмена данными';vi='thông điệp trao đổi dữ liệu theo gói'");
...
Function GetExchangeMessage()
	...
	If ExchangeMessageTable.Count() = 0 Then
		...
		MessageString = NStr("en='Emails with title ""%1"" are not found';ru='Не обнаружены письма с заголовком: ""%1""';vi='Không tìm thấy thư có tiêu đề: ""%1""'");
...
Procedure ErrorMessagesInitialization()
	...
	// Common error codes
	ErrorMessages.Insert(001, NStr("en='Exchange emails are not found.';ru='Не обнаружены сообщения обмена.';vi='Không tìm thấy thông điệp trao đổi.'"));
	ErrorMessages.Insert(002, NStr("en='An error occurred when unpacking a compressed message file.';ru='Ошибка при распаковке сжатого файла сообщения.';vi='Lỗi khi giải nén tệp thông báo.'"));
	ErrorMessages.Insert(003, NStr("en='An error occurred when compressing the exchange message file.';ru='Ошибка при сжатии файла сообщения обмена.';vi='Lỗi khi nén tệp thông điệp trao đổi.'"));
	ErrorMessages.Insert(004, NStr("ru = ""Error when creating temporary directory'."));
	ErrorMessages.Insert(005, NStr("en='Archive does not include exchange message file.';ru='Архив не содержит файл сообщения обмена.';vi='Nơi lưu trữ không chứa tệp thông điệp trao đổi.'"));
	ErrorMessages.Insert(006, NStr("en='Exchange message was not sent: allowed message size exceeded.';ru='Сообщение обмена не отправлено: превышен допустимый размер сообщения.';vi='Chưa gửi thông điệp trao đổi: vượt quá dung lượng cho phép của thông báo.'"));
	
	// Errors codes, that are dependent on the transport kind.
	ErrorMessages.Insert(101, NStr("en='Initialization error: email account of the exchange message transport is not specified.';ru='Ошибка инициализации: не указана учетная запись электронной почты транспорта сообщений обмена.';vi='Lỗi khởi xướng: chưa chỉ ra trương mục thư điện tử chuyển tải thông điệp trao đổi.'"));
	ErrorMessages.Insert(102, NStr("en='An error occurred when sending the email.';ru='Ошибка при отправке сообщения электронной почты.';vi='Lỗi khi gửi thông báo E-mail.'"));
	ErrorMessages.Insert(103, NStr("en='An error occurred when getting message titles from the email server.';ru='Ошибка при получении заголовков сообщений с сервера электронной почты.';vi='Lỗi khi nhận tiêu đề thông báo từ Server thư điện tử'"));
	ErrorMessages.Insert(104, NStr("en='Exchange emails are not found on email server.';ru='Не обнаружены сообщения обмена на почтовом сервере.';vi='Không tìm thấy thông điệp trao đổi trên Server hộp thư.'"));
	ErrorMessages.Insert(105, NStr("en='An error occurred when receiving a message from the email server.';ru='Ошибка при получении сообщения с сервера электронной почты.';vi='Lỗi khi nhận thông báo từ Server thư điện tử.'"));
	ErrorMessages.Insert(106, NStr("en='An error occurred when writing the exchange message file to the disk.';ru='Ошибка при записи файла сообщения обмена на диск.';vi='Lỗi khi ghi lại tệp thông điệp trao đổi trên đĩa.'"));
	ErrorMessages.Insert(107, NStr("en='Account parameter check is completed with errors.';ru='Проверка параметров учетной записи завершилась с ошибками.';vi='Kiểm tra tham số trương mục đã kết thúc có lỗi.'"));
	ErrorMessages.Insert(108, NStr("en='Exchange message size exceeds the allowable limit.';ru='Превышен допустимый размер сообщения обмена.';vi='Đã vượt quá dung lượng cho phép của thông điệp trao đổi.'"));
	ErrorMessages.Insert(109, NStr("en='Error: a file with message is not found in the mail message.';ru='Ошибка: в почтовом сообщении не найден файл с сообщением.';vi='Lỗi: không tìm thấy tệp có thông báo trong thông báo qua thư.'"));
