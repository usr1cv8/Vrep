...
Procedure WriteAuthorizationSettingsForStandardODataInterface(Val AuthorizationSettings) Export
	...
	If AuthorizationSettings.Used Then
		...
		StandardInterfaceUserOData.Description = NStr("en='Automatic REST service';ru='Автоматический REST-сервис';vi='REST-service tự động'");
		...
		Try
			...
			Comment = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='User is written for standard interface OData. IB user description: ------------------------------------------- %1 ------------------------------------------- Result: ------------------------------------------- %2 -------------------------------------------';ru='Выполнена запись пользователя для стандартного интерфейса OData. Описание пользователя ИБ: ------------------------------------------- %1 ------------------------------------------- Результат: ------------------------------------------- %2 -------------------------------------------';vi='Đang thực hiện ghi người sử dụng cho giao diện chuẩn OData. Mô tả người sử dụng CSTT: ------------------------------------------- %1 ------------------------------------------- Kết quả: ------------------------------------------- %2 -------------------------------------------'"),
			...
			WriteLogEvent(
				EventLogMonitorEventName(NStr("en='UserRecord';ru='ЗаписьПользователя';vi='UserRecord'")),
		...
		Except
			...
			Comment = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='An error occurred while writing a user for standard interface OData. IB user description: ------------------------------------------- %1 ------------------------------------------- Error text: ------------------------------------------- %2 -------------------------------------------';ru='При записи пользователя для стандартного интерфейса OData произошла ошибка. Описание пользователя ИБ: ------------------------------------------- %1 ------------------------------------------- Текст ошибки: ------------------------------------------- %2 -------------------------------------------';vi='Khi ghi lại người sử dụng đối với giao diện chuẩn OData đã xảy ra lỗi. Mô tả người sử dụng CSTT: ------------------------------------------- %1"
"------------------------------------------- Mô tả lỗi: ------------------------------------------- %2 -------------------------------------------"
"'"),
				CommonUse.ValueToXMLString(IBUserDescription),
...
Procedure CheckODataRoleContent() Export
	...
	If ExcessiveRights.Count() > 0 Then
		ErrorText = Chars.NBSp + NStr("en='The following rights are redundant in the role:';ru='Следующие права избыточно включены в состав роли:';vi='Các quyền sau đây bị thừa trong thành phần vai trò:'") + Chars.LF + Chars.CR + 
			ExcessiveOrMissingRightsDisplay(ExcessiveRights, 2);
		Errors.Add(ErrorText);
	EndIf;
	
	If MissingRights.Count() > 0 Then
		ErrorText = Chars.NBSp + NStr("en='The following rights must be included in the role:';ru='Следующие права должны быть включены в состав роли:';vi='Các quyền sau đây nên là thành phần vai trò:'") + Chars.LF + Chars.CR + 
	...
	If Errors.Count() > 0 Then
		...
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Errors in the role right content were found %1:';ru='Обнаружены ошибки в составе прав роли %1:';vi='Phát hiện lỗi trong thành phần quyền vai trò %1:'"),
			RoleForStandardODataInterface().Name);
...
Function EventLogMonitorEventName(Val Suffix)
	...
	Return NStr("en='StandardODataInterfaceSetup.';ru='НастройкаСтандартногоИнтерфейсаOData.';vi='StandardODataInterfaceSetup.'") + TrimAll(Suffix);
...
Procedure CheckODataRoleContentWhenUpdating() Export
	...
	Else
		Raise NStr("en='Handler shall not be used when data area division is enabled';ru='Обработчик не должен использоваться при выключенном разделении по областям данных';vi='Không cần sử dụng hàm sự kiện khi ngắt phân tách theo vùng dữ liệu'");
	EndIf;
...
Procedure CheckPossibilityToCreateUserForStandardODataInterfaceCalls()
	...
	If UserCount = 0 Then
		...
		Raise NStr("en='Cannot create separate login and password for automatic REST service because there are no other users in the application.';ru='Нельзя создать отдельные логин и пароль для использования автоматического REST-сервиса, т.к. в программе отсутствуют другие пользователи.';vi='Không thể tạo tên và mật khẩu riêng để sử dụng REST-service tự động vì trong chương trình không có người sử dụng khác.'");
