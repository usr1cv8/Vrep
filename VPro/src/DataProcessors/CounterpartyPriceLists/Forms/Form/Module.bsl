...
Procedure UpdateFormTitleAtServer()
	...
	ThisForm.Title	= NStr("en='Counterparty price lists';ru='прайс-листы контрагентов';vi='Bảng giá của đối tác'") + 
		?(ValueIsFilled(ToDate), NStr("en=' on ';ru=' на ';vi=' ngày '") + Format(ToDate, NStr("en = 'L=en; DLF=DD'; ru = 'L=ru; DLF=DD'; vi = 'L=vi; DLF=DD'")), NStr("en='.';ru='.';vi='.'"));
...
Procedure UpdateAtServer()
	...
	If Items.ShowTitle.Check Then
		...
		HeaderArea.Parameters.Title	 = Nstr("en='PRICE-SHEET';ru='ПРАЙС-ЛИСТ';vi='BẢNG GIÁ'");
...
Function GetDecorationTitleContent(TabularSectionName) 
	...
	If Object[TabularSectionName].Count() < 1 Then
		...
		DecorationTitle = Nstr("en='Multiple filter is not filled';ru='Множественный отбор не заполнен';vi='Chưa điền nhiều bộ lọc'");
...
"Возможно выбрана пустая ячейка.';vi='Không thể sao chép đơn giá."
"Có thể chọn ô trống.'")
					);
		Return;
...
"Копирование не возможно.';vi='Có thể chỉ ra ô hàng trống."
"Không thể sao chép.'")
				);
...
"Возможно выбрана пустая ячейка.';vi='Không thể thay đổi đơn giá."
"Có thể chọn ô trống.'")
					);
		Return;
...
"Возможно выбрана пустая ячейка.';vi='Không thể xóa bỏ đơn giá."
"Có thể chọn ô trống.'")
					);
		Return;
	...
	ElsIf DetailFromArea.Property("DetailsMatch") Then
		...
		If AvailablePriceKindsList.Count() < 1 Then
			...
			CommonUseClientServer.MessageToUser(
				NStr("en='No prices available for deletion exist for the current products and services item in the current price list.';ru='В текущем прайс-листе для данной номенклатурной позиции нет цен, доступных для удаления.';vi='Trong bảng giá hiện tại đối với mặt hàng này không có đơn giá khả dụng để xóa.'")
						);
...
Procedure History(Command)
	...
	If Not TypeOf(DetailFromArea) = Type("Structure")
		...
		CommonUseClientServer.MessageToUser(
			NStr("en='Cannot show price generation history.';ru='Невозможно отобразить историю формирования цен.';vi='Không thể lọc lịch sử thiết lập giá.'")
					);
		Return;
	...
	ElsIf DetailFromArea.Property("DetailsMatch") Then
		...
		If AvailablePriceKindsList.Count() < 1 Then
			...
			CommonUseClientServer.MessageToUser(
				NStr("en='Cannot show history of price generation for the current products and services item.';ru='Невозможно отобразить историю формирорвания цен для данной номенклатурной позиции.';vi='Không thể hiển thị lịch sử thiết lập giá đối với mặt hàng này.'")
						);
