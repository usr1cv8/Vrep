...
"Попробуйте выполнить операцию позже.';vi='Lỗi truy cập cạnh tranh đến tùy chỉnh cho phép sử dụng nguồn lực ngoài."
"Hãy thử thực hiện lại giao dịch sau.'");
		EndTry;
...
Function NewSecurityProfileDescription(Val SoftwareModuleType, Val SoftwareModuleID) Export
	...
	Pattern = NStr("en='[Infobase %1] %2 ""%3""';ru='[Infobase %1] %2 ""%3""';vi='[Infobase %1] %2 ""%3""'");
	...
	If IsBlankString(InfobaseName) Then
		Raise NStr("en='Infobase connection string does not contain the name of the infobase.';ru='Строка соединения информационной базы не содержит имени информационной базы!';vi='Dòng kết nối cơ sở thông tin không có tên cơ sở thông tin!'");
	EndIf;
	
	If SoftwareModuleType = Catalogs.MetadataObjectIDs.EmptyRef() Then
		Return StringFunctionsClientServer.SubstituteParametersInString(Pattern, InfobaseName,
			NStr("en='Security profile for infobase';ru='Профиль безопасности для информационной базы';vi='Hồ sơ an toàn cho cơ sở thông tin'"), InfobaseConnectionString());
	Else
