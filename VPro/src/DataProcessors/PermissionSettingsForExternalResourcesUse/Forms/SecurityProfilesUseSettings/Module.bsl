...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not WorkInSafeModeService.SecurityProfilesCanBeUsed() Then
		Raise NStr("en='Use of security profiles is not available for this configuration.';ru='Использование профилей безопасности недоступно для данной конфигурации!';vi='Không thể sử dụng hồ sơ an toàn đối với cấu hình này!'");
	EndIf;
	
	If Not WorkInSafeModeService.SecurityProfilesSetupAvailable() Then
		Raise NStr("en='Setting of security profiles is not available.';ru='Настройка профилей безопасности недоступна!';vi='Tùy chỉnh hồ sơ an toàn là không thể!'");
	EndIf;
...
Procedure StartUseSecurityProfilesUseSettings(Val UUID)
	...
	If Not WorkInSafeModeService.SecurityProfilesCanBeUsed() Then
		Raise NStr("en='Enabling automatic permission request is not available.';ru='Включение автоматического запроса разрешений недоступно!';vi='Không thể bật truy vấn cho phép tự động!'");
	EndIf;
