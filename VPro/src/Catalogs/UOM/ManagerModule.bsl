	...
	If Not Parameters.Property("Recursion")
		...
		If Not (Parameters.Property("DontUseClassifier") AND Parameters.DontUseClassifier = True) Then
			...
			// Add standard list by basic products and services UOM according to the classifier.
				PresentationUOM = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1 (storage unit)';ru='%1 (ед. хранения)';vi='%1 (đv. lưu)'"),
			...
			Else
				CommonUseClientServer.MessageToUser(NStr("en='Products and services are not filled in.';ru='Не заполнена номенклатура!';vi='Chưa điền mặt hàng!'"));
			EndIf;
		EndIf;
