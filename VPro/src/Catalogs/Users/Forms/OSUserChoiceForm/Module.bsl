...
Procedure Select(Command)
	...
	If Items.DomainTable.CurrentData = Undefined Then
		ShowMessageBox(, NStr("en='Select domain.';ru='Выберите домен.';vi='Hãy chọn tên miền.'"));
		Return;
	EndIf;
	DomainName = Items.DomainTable.CurrentData.DomainName;
	
	If Items.DomainUserTable.CurrentData = Undefined Then
		ShowMessageBox(, NStr("en='Select domain user.';ru='Выберите пользователя домена.';vi='Hãy chọn người sử dụng tên miền.'"));
		Return;
	EndIf;
