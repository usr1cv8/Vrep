	...
	If CommonUseReUse.DataSeparationEnabled() Then
		...
		If Not CanChangeUsers Then
			If Object.Ref.IsEmpty() Then
				Raise
					NStr("en='New users cannot be created in the demo mode.';ru='В демонстрационном режиме не поддерживается создание новых пользователей.';vi='Ở chế độ demo, không hỗ trợ tạo mới người sử dụng.'");
			EndIf;
			ReadOnly = True;
		EndIf;
...
Procedure BeforeWrite(Cancel, WriteParameters)
	...
	If CanLogOnToApplication Then
		QuestionsTitle = NStr("en='Infobase user record';ru='Запись пользователя информационной базы';vi='Ghi lại người sử dụng cơ sở thông tin'");
...
"в программу, требуется снять пометку на удаление с этого пользователя.';vi='Để được phép đăng nhập"
"vào chương trình, cần bỏ dấu xóa khỏi người sử dụng này.'"));
		Return;
	EndIf;
	...
	If Not AccessLevel.FullRights
	   ...
	   AND Not CanLogOnToApplication Then
		...
		ShowMessageBox(,
			NStr("en='After saving, only administrator can allow log on.';ru='После записи вход в программу сможет разрешить только администратор.';vi='Sau khi ghi lại thì chỉ người quản trị được phép đăng nhập vào chương trình.'"));
	EndIf;
...
Procedure SetConditionalAppearance()
	...
	FilterElement.RightValue = NStr("en='FullRights';ru='ПолныеПрава';vi='FullRights'");
