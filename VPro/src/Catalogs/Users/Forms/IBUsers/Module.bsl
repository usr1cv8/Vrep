...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Users.InfobaseUserWithFullAccess(, CommonUseReUse.ApplicationRunningMode().Local) Then
		Raise NStr("en='Insufficient rights to open infobase user list.';ru='Недостаточно прав для открытия списка пользователей информационной базы.';vi='Không đủ quyền để mở danh sách người sử dụng cơ sở thông tin.'");
	EndIf;
...
Procedure CancelMapping(Command)
	...
	Buttons.Add("CancelMapping", NStr("en='Clear mapping';ru='Отменить сопоставление';vi='Hủy đặt tương ứng'"));
	Buttons.Add("LeaveCompliance", NStr("en='Leave mapping';ru='Оставить сопоставление';vi='Để lại sự tương ứng'"));
...
"при обновлении информационной базы, поэтому не рекомендуется отменять сопоставление по любой другой причине.';vi='Hủy việc đặt tương ứng giữa người sử dụng cơ sở thông tin với người sử dụng trong danh mục."
""
"Việc hủy đặt tương ứng rất hiếm - chỉ khi đã đặt tương ứng sai, ví dụ, khi cập nhật cơ sở thông tin,"
"do đó không nên hủy đặt tương ưng do bất kỳ nguyên nhân nào khác.'"),
		Buttons,
		,
		"LeaveCompliance");
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("en='<No data>';ru='<Нет данных>';vi='<Không có dữ liệu>'"));
...
Procedure DeleteCurrentIBUser(DeleteLine = False)
	...
	ShowQueryBox(
		...
		NStr("en='Delete infobase user?';ru='Удалить пользователя информационной базы?';vi='Xóa người sử dụng cơ sở thông tin?'"),
