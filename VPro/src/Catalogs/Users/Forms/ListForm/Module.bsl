...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		...
		Items.UsersGroups.ChoiceMode = False;
...
Procedure ChangeParametersExtensionPickForm()
	...
	If GetFunctionalOption("UseUsersGroups") Then
		...
		Items.UsersList.Title                    = NStr("en='Users in group';ru='Пользователи в группе';vi='Người sử dụng trong nhóm'");
	...
	If Parameters.ChoiceMode Then
		...
		If Parameters.CloseOnChoice = False Then
			...
			If UserGroupChoice Then
				...
				Else
					Title = NStr("en='Select users and groups';ru='Подбор пользователей и групп';vi='Chọn người sử dụng và nhóm'");
				EndIf;
				
				Items.ChooseUser.Title =
					NStr("en='Select users';ru='Выбор пользователей';vi='Chọn người sử dụng'");
			...
			Else
				...
				If SettingsStored.AdvancedSelection Then
					Title = SettingsStored.FormHeaderSelection;
				Else
					Title = NStr("en='Pick users';ru='Подбор пользователей';vi='Chọn người sử dụng'");
				EndIf;
		...
		Else
			...
			If UserGroupChoice Then
				...
				Title = NStr("en='Select user or group';ru='Выбор пользователя или группы';vi='Chọn người sử dụng hoặc nhóm'");
				
				Items.ChooseUser.Title = NStr("en='Select user';ru='Выбрать пользователя';vi='Chọn người sử dụng '");
			Else
				Title = NStr("en='Select user';ru='Выбрать пользователя';vi='Chọn người sử dụng '");
			EndIf;
		EndIf;
	EndIf;
