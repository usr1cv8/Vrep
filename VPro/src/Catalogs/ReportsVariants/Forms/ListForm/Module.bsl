...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	ChoiceList.Add(1, NStr("en='Internal and Additional';ru='Внутренние и Дополнительные';vi='Nội bộ và Bổ sung'"));
	ChoiceList.Add(Enums.ReportsTypes.Internal, NStr("en='Internal';ru='Внутренние';vi='Nội bộ'"));
	ChoiceList.Add(Enums.ReportsTypes.Additional, NStr("en='Additional';ru='Дополнительные';vi='Bổ sung'"));
	ChoiceList.Add(Enums.ReportsTypes.External, NStr("en='External';ru='Внешние';vi='Ngoài'"));
...
Procedure SubsystemsTreeDrag(Item, DragParameters, StandardProcessing, String, Field)
	...
	If ArrangementParameters.Source.Ref = ArrangementParameters.Receiver.Ref Then
		ShowMessageBox(, NStr("en='The selected report variants are already in this section.';ru='Выбранные варианты отчетов уже в данном разделе.';vi='Phương án báo cáo được chọn đã có trong phần hành này.'"));
		Return;
	EndIf;
	
	If ArrangementParameters.Variants.Total = 1 Then
		If ArrangementParameters.Action = "Copy" Then
			QuestionTemplate = NStr("en='Place ""%1"" to ""%4""?';ru='Разместить ""%1"" в ""%4""?';vi='Sắp xếp ""%1"" vào ""%4""?'");
		Else
			QuestionTemplate = NStr("en='Move ""%1"" from ""%3"" to ""%4""?';ru='Переместить ""%1"" из ""%3"" в ""%4""?';vi='Chuyển ""%1"" từ ""%3"" vào ""%4""?'");
		EndIf;
	...
	Else
		...
		EndDo;
		If ArrangementParameters.Action = "Copy" Then
			QuestionTemplate = NStr("en='Place report variants ""%1"" (%2 pcs.) in ""%4""?';ru='Разместить варианты отчетов ""%1"" (%2 шт.) в ""%4""?';vi='Sắp xếp phương án báo cáo ""%1"" (%2) vào ""%4""?'");
		Else
			QuestionTemplate = NStr("en='Move report variants ""%1"" (%2 pcs.) from ""%3"" to ""%4""?';ru='Переместить варианты отчетов ""%1"" (%2 шт.) из ""%3"" в ""%4""?';vi='Chuyển phương án báo cáo ""%1"" (%2) từ ""%3"" sang ""%4""?'");
		EndIf;
	EndIf;
...
Function PlaceOptionsInSubsystem(ArrangementParameters)
	...
	For Each VariantRef IN ArrangementParameters.Variants.Array Do
		If VariantRef.ReportType = Enums.ReportsTypes.External Then
			CannotBeArranged = ?(CannotBeArranged = "", "", CannotBeArranged + Chars.LF)
				...
				+ String(VariantRef)
				+ " ("
				+ NStr("en='external';ru='Внешний';vi='Ngoài'")
				+ ")";
			Continue;
		ElsIf VariantRef.DeletionMark Then
			CannotBeArranged = ?(CannotBeArranged = "", "", CannotBeArranged + Chars.LF)
				...
				+ String(VariantRef)
				+ " ("
				+ NStr("en='Marked for deletion';ru='Помеченные на удаление';vi='Đánh dấu xóa'")
				+ ")";
			Continue;
		EndIf;
	...
	If ArrangementParameters.Variants.Total = Placed Then
		...
		If ArrangementParameters.Variants.Total = 1 Then
			If ArrangementParameters.Action = "Move" Then
				Pattern = NStr("en='Successfully transferred to %1"".';ru='Успешно перемещены в ""%1"".';vi='Đã hoàn tất điều chuyển vào ""%1"".'");
			Else
				Pattern = NStr("en='Successfully placed in %1"".';ru='Успешно размещены в ""%1"".';vi='Đã hoàn tất điều chuyển vào ""%1"".'");
			EndIf;
		...
		Else
			...
			OutputNotification.Text = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Report variants (%1).';ru='Варианты отчетов (%1).';vi='Các phương án báo cáo (%1).'"),
	...
	Else
		...
		If Not IsBlankString(CannotBeArranged) Then
			ErrorsText = ?(ErrorsText = "", "", ErrorsText + Chars.LF + Chars.LF)
				+ NStr("en='Cannot be placed in command interface:';ru='Не могут размещаться в командном интерфейсе:';vi='Không thể đặt vào giao diện lệnh:'")
				+ Chars.LF
				+ CannotBeArranged;
		EndIf;
		If Not IsBlankString(ArrangedAlready) Then
			ErrorsText = ?(ErrorsText = "", "", ErrorsText + Chars.LF + Chars.LF)
				+ NStr("en='Already located in this section:';ru='Уже размещены в этом разделе:';vi='Đã đặt vào phần hành này:'")
				+ Chars.LF
				+ ArrangedAlready;
		EndIf;
		
		If ArrangementParameters.Action = "Move" Then
			Pattern = NStr("en='Transferred report variants: %1 out of %2.';ru='Перемещено вариантов отчетов: %1 из %2.';vi='Điều chuyển các phương án báo cáo: %1 từ %2.'");
		Else
			Pattern = NStr("en='Placed report variants: %1 of %2.';ru='Размещено вариантов отчетов: %1 из %2.';vi='Sắp đặt phương án báo cáo: %1 từ %2.'");
		EndIf;
