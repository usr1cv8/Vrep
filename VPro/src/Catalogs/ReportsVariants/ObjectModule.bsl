...
Procedure BeforeWrite(Cancel)
	...
	If Not User AND DeletionMarkIsChangedByUser Then
		If DeletionMark Then
			ErrorText = NStr("en='Cannot mark predefined report variant for deletion.';ru='Пометка на удаление предопределенного варианта отчета запрещена.';vi='Cấm đánh dấu xóa phương án báo cáo định trước.'");
		Else
			ErrorText = NStr("en='Cannot unmark the predefined report variant for deletion.';ru='Снятие пометки удаления предопределенного варианта отчета запрещена.';vi='Đã cấm bỏ dấu xóa phương án báo cáo định trước.'");
		EndIf;
	...
	If Not DeletionMark AND DeletionMarkIsChangedByUser Then
		...
		If DescriptionIsBooked OR VariantKeyIsBooked Then
			ErrorText = NStr("en='An error occurred when clearing the deletion mark of report variant:';ru='Ошибка снятия пометки удаления варианта отчета:';vi='Lỗi bỏ dấu xóa phương án báo cáo:'");
			If DescriptionIsBooked Then
				ErrorText = ErrorText + StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Name ""%1"" is already used by another variant of this report.';ru='Наименование ""%1"" уже занято другим вариантом этого отчета.';vi='Tên gọi ""%1"" đã bị chiếm bởi phương án khác của báo cáo này.'"),
					Description);
			Else
				ErrorText = ErrorText + StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Key of variant ""%1"" is already used by another variant of this report.';ru='Ключ варианта ""%1"" уже занят другим вариантом этого отчета.';vi='Khóa phương án ""%1"" đã bị chiếm bởi phương án khác của báo cáo này.'"),
					VariantKey);
			EndIf;
...
"необходимо установить пометку удаления конфликтующего варианта отчета.';vi='Trước khi bỏ dấu xóa phương án báo cáo'");
			ReportsVariants.ErrorByVariant(Ref, ErrorText);
			Raise ErrorText;
		EndIf;
	EndIf;
	...
	If User Or ReportType = Enums.ReportsTypes.Additional Then
		...
		Except
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Cannot index a scheme of variant ""%1"" of report ""%2"":';ru='Не удалось проиндексировать схему варианта ""%1"" отчета ""%2"":';vi='Không thể đánh chỉ mục sơ đồ phương án ""%1"" của báo cáo ""%2"".'"),
				VariantKey,
				String(Report));
...
Function ControversialFieldValues(FieldName1, FieldName2)
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Inconsistent values of fields ""%1"" and ""%2""';ru='Противоречивые значения полей ""%1"" и ""%2""';vi='Các giá trị trái ngược của trường ""%1"" và ""%2""'"),
		FieldName1,
		FieldName2
	);
EndFunction
