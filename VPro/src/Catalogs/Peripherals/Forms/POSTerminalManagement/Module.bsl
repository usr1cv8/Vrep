...
Procedure ExecuteOperationByPaymentCardEnd(Result, Parameters) Export
	...
	If TypeOf(Result) = Type("Structure") Then
		...
		If Not Parameters.Property("OperationKind") Then
			MessageText = NStr("en='Transaction type is not specified.';ru='Не указан тип транзакции.';vi='Chưa chỉ ra kiểu giao tác.'");
...
"""%ErrorDescription%""."
"Không hủy bỏ giao dịch theo thẻ'");
...
"Đã hủy bỏ giao dịch theo thẻ.'");
				...
				CommonUseClientServer.MessageToUser(MessageText);
			Else
				MessageText = NStr("en='Operation is performed successfully.';ru='Операция выполнена успешно.';vi='Giao dịch đã thực hiện thành công.'");
				CommonUseClientServer.MessageToUser(MessageText);
			EndIf;
...
"""%ОписаниеОшибки%"".';vi='Khi in chứng từ đã xảy ra lỗi:"
"""%ErrorDescription%"".'");
				MessageText = StrReplace(MessageText, "%ErrorDescription%", Output_Parameters[1]);
				CommonUseClientServer.MessageToUser(MessageText);
			EndIf;
		Else
			MessageText = NStr("en='There is no last sales slip.';ru='Отсутствует последний слип чек.';vi='Không có phiếu cuối'");
			CommonUseClientServer.MessageToUser(MessageText);
		EndIf;
...
Procedure PrintLastSlipReceipt(Command)
	...
	EquipmentManagerClient.OfferSelectDevice(NOTifyDescription, "FiscalRegister",
			NStr("en='Select a fiscal data recorder to print POS receipts.';ru='Выберите фискальный регистратор для печати эквайринговых чеков';vi='Hãy chọn máy ghi nhận tiền mặt để in phiếu thanh toán qua thẻ'"), 
			NStr("en='Fiscal data recorder for printing acquiring receipts is not connected.';ru='Фискальный регистратор для печати эквайринговых чеков не подключен.';vi='Chưa kết nối máy ghi nhận tiền mặt để in phiếu thanh toán thẻ.'"));
