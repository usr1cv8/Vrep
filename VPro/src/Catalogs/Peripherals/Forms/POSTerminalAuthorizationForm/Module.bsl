...
Procedure RunOperation(Command)
	...
	If Amount = 0 Then
		Errors = NStr("en='Payment cannot be made for the amount equals to zero.';ru='Платеж не может быть осуществлен на нулевую сумму.';vi='Thanh toán không thể thực hiện với số tiền bằng 0.'");
	EndIf;
	
	If ShowCardNumber AND Not ValueIsFilled(CardNumber) Then
		Errors = Errors + ?(IsBlankString(Errors),"",Chars.LF) + NStr("en='Payment cannot be made without a card number.';ru='Платеж не может быть осуществлен без номера карты.';vi='Thanh toán không thể thực hiện được mà không có số thẻ.'");
	EndIf;
...
"Повторите считывание или считайте другую карту';vi='Chỉ ra sai thẻ hoặc xảy ra lỗi khi đọc thẻ."
"Lặp lại việc đọc thẻ hoặc đọc thẻ khác'"));
		EndIf;
	Else
		CommonUseClientServer.MessageToUser(NStr("en='Invalid card is specified or an error occurred while reading the card."
"Repeat reading or read another card';ru='Указана неверная карта или произошла ошибка при считывании карты."
...
"Значение будет изменено на максимально возможное.';vi='Số tiền thanh toán theo thẻ vượt quá tiền gửi cần thiết."
"Giá trị sẽ thay đổi lên mức cao nhất.'"));
