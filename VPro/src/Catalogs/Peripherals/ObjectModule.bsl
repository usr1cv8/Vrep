...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Not IsBlankString(Description) Then
		...
		If Not Query.Execute().IsEmpty() Then
			CommonUseClientServer.MessageToUser(NStr("en='Non-unique item name specified. Specify a unique name.';ru='Указано неуникальное наименование элемента. Укажите уникальное наименование.';vi='Đã chỉ ra tên gọi phần tử không đơn trị. Hãy chỉ ra tên gọi đơn trị.'"), ThisObject, , , Cancel);
		EndIf;
	EndIf;
...
Procedure OnCopy(CopiedObject)
	...
	Description = NStr("en='%Description% (copy)';ru='%Description% (копия)';vi='%Description% (bản sao)'");
