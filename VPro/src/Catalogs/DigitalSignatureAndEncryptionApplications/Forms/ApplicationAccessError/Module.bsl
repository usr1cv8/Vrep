...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If ValueIsFilled(ErrorOnClient)
	   AND ValueIsFilled(ErrorOnServer) Then
		...
		ErrorDescription =
			  NStr("en='ON THE SERVER:';ru='НА СЕРВЕРЕ:';vi='TRÊN SERVER:'")
			+ Chars.LF + Chars.LF + ErrorOnServer.ErrorDescription
			+ Chars.LF + Chars.LF
			+ NStr("en='ON THE COMPUTER:';ru='НА КОМПЬЮТЕРЕ:';vi='TRÊN MÁY TÍNH:'")
...
Procedure CopyIntoExchangeBuffer(Command)
	...
	ShowInputString(New NotifyDescription("CopyIntoExchangeBufferEnd", ThisObject),
		String, NStr("en='Error text for copying';ru='Текст ошибки для копирования';vi='Nội dung lỗi sao chép'"),, True);
