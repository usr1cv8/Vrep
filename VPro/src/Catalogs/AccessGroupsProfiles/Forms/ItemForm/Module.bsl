		...
		If String.Check AND Left(String.Synonym, 1) = "?" Then
			CommonUseClientServer.AddUserError(Errors,
				...
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Role ""%1"" was not found in metadata.';ru='Роль ""%1"" не найдена в метаданных.';vi='Không tìm thấy vai trò ""%1"" trong metadata.'"),
					String.Synonym),
				"Roles",
				TreeItems.IndexOf(String),
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='The ""%2"" role in line %1 is not found in the metadata.';ru='Роль ""%2"" в строке %1 не найдена в метаданных.';vi='Không tìm thấy vai trò ""%2"" trong dòng %1 trong metadata.'"),
					"%1", String.Synonym));
		EndIf;
	EndDo;
...
Procedure RestoreByInitialFilling(Command)
	...
	ShowQueryBox(
		...
		NStr("en='Restore the profile to its initial state?';ru='Восстановить профиль по содержимому начального заполнения?';vi='Phục hồi hồ sơ theo nội dung điền ban đầu?'"),
...
Procedure RestoreByInitialFillingEnd(Response, NotSpecified) Export
	...
	If UpdateAccessGroups Then
		Text = NStr("en='Profile ""%1"" is restored to its initial state, access groups of the profile are updated.';ru='Профиль ""%1"" восстановлен по содержимому начального заполнения, группы доступа профиля обновлены.';vi='Hồ sơ ""%1"" phục hồi theo nội dung điền ban đầu, nhóm truy cập hồ sơ được cập nhật.'");
	Else
		Text = NStr("en='Profile ""%1"" is restored to its initial state, access groups of the profile are not updated.';ru='Профиль ""%1"" восстановлен по содержимому начального заполнения, группы доступа профиля не обновлены.';vi='Hồ sơ ""%1"" phục hồi theo nội dung điền ban đầu, nhóm truy cập hồ sơ không được cập nhật.'");
	EndIf;
...
"will be deleted, and missing access kinds will be added.';ru='Обновить группы доступа, использующие этот профиль?"
""
"Будут удалены лишние виды доступа с заданными для них"
"значениями доступа и добавлены недостающие виды доступа';vi='Cập nhật nhóm truy cập sử dụng hồ sơ này?"
""
"Sẽ xóa dạng truy cập thừa với giá trị truy cập"
"đã đặt và thêm dạng truy cập bị thiếu'");
