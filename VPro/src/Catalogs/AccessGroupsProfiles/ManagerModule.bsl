...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	For Each Section IN Sections Do
		...
		Work.Presentation = NStr("en='Not compatible with the current version';ru='Не совместимы с текущей версией';vi='Không tương thích với phiên bản hiện tại'");
		...
		Work.ThereIsWork      = IncompatibleAccessGroupProfilesQuantity > 0;
		Work.Important        = True;
		Work.Presentation = NStr("en='Access group profiles';ru='Профили групп доступа';vi='Hồ sơ nhóm truy cập'");
		Work.Quantity    = IncompatibleAccessGroupProfilesQuantity;
...
"поставляемого профиля ""%1"" роль ""%2"" не найдена в метаданных.';vi='Khi kiểm tra"
"hồ sơ đã đặt ""%1"" không tìm thấy vai trò ""%2"" trong metadata.'"),
...
""
"';vi='Đã đặt sai"
"giá trị trong thủ tục ПриЗаполненииПоставляемыхПрофилейГруппДоступа mô-đun chung УправлениеДоступомПереопределяемый."
""
"'");
...
"Ложь, тогда свойство ЗапретитьИзменениеПрофилей тоже должно быть установлено Ложь.';vi='Khi"
"trong tham số ПараметрыОбновления"
"thuộc tính ОбновлятьИзмененныеПрофили đã thiết lập"
"False thì thuộc tính ЗапретитьИзменениеПрофилей cũng cần đặt là False.'");
	EndIf;
	...
	AdministratorProfileDescription.Description  = NStr("en='Administrator';ru='Администратор';vi='Người quản trị'");
...
""
"Под профилем осуществляется работа со всеми разделами: - Продажи, Закупки, Сервис, Производство, Деньги, Зарплата, Предприятие, Анализ и Настройки.';vi='Hồ sơ được dùng cho công việc của người quản lý và thực hiện các chức năng Service."
"Có thể cấp quyền truy cập không hạn chế với tất cả dữ liệu của hệ thống thông tin."
""
"Có hồ sơ có thể làm việc với tất cả các phần hành: - Bán hàng, Mua hàng, Dịch vụ, Sản xuất, Vốn bằng tiền, Tiền lương, Doanh nghiệp, Phân tích và Tùy chỉnh.'");
	//End SB.
...
"""%1 (%2)"" роль ""%3"" не найдена в метаданных.';vi='Trong mô tả hồ sơ"
"""%1 (%2)"" không tìm thấy vai trò ""%3"" trong metadata.'"),
...
"""%1 (%2)"" указана недопустимая роль ""%3"".';vi='Trong mô tả hồ sơ"
"""%1 (%2)"" đã chỉ ra vai trò không hợp lệ ""%3"".'"),
					ProfileDescription.Name,
					ProfileDescription.ID,
					Role);
			EndIf;
		...
		If ProfilesProperties.Get(ProfileDescription.ID) <> Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Profile with ID ""%1"" already exists.';ru='Профиль с идентификатором ""%1"" уже существует.';vi='Đã có hồ sơ với Tên định danh ""%1"".'"),
				ProfileDescription.ID);
		EndIf;
...
"указан несуществующий вид доступа ""%2"".';vi='Trong mô tả hồ sơ ""%1"""
"đã chỉ ra dạng truy cập ""%2"" không tồn tại.'"),
					ProfileDescription.Name);
			EndIf;
...
"- ""ВначалеВсеРазрешены"","
"- ""Предустановленный"".';vi='Trong mô tả hồ sơ ""%1"""
"đối với dạng truy cập ""%2"" đã chỉ ra diễn giải ""%3"" không xác định."
""
"Chỉ có các diễn giải sau khả dụng:"
"- ""ВначалеВсеЗапрещены"" hoặc """","
"- ""ВначалеВсеРазрешены"","
"- ""Предустановленный"".'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
...
"""%3"".';vi='Trong mô tả hồ sơ ""%1"""
"đã chỉ ra dạng truy cập không tồn tại ""%2"""
"đối với giá trị truy cập"
"""%3"".'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
					  ProfileDescription.ID),
...
"""%3"".';vi='Trong mô tả hồ sơ ""%1"""
"đối với dạng truy cập ""%2"""
"không tồn tại kiểu giá trị truy cập"
"""%3"" này.'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
					  ProfileDescription.ID),
...
"""%3"".';vi='Trong mô tả hồ sơ ""%1"""
"đối với dạng truy cập ""%2"""
"đã chỉ ra kiểu giá trị truy cập không phải là tham chiếu"
"""%3"".'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
					  ProfileDescription.ID),
...
"типа, который не указан в свойствах вида доступа ""%2"".';vi='Trong mô tả hồ sơ ""%1"""
"đã chỉ ra giá trị truy cập ""%3"""
"của kiểu mà chưa chỉ ra trong thuộc tính dạng truy cập ""%2"".'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
					  ProfileDescription.ID),
					AccessKind,
...
"""%3"".';vi='Trong mô tả hồ sơ ""%1"""
"đối với dạng truy cập ""%2"""
"đã chỉ ra trùng giá trị truy cập"
"""%3"".'"),
					?(ValueIsFilled(ProfileDescription.Name),
					  ProfileDescription.Name,
					  ProfileDescription.ID),
...
"профиля ""%1"" роль ""%2"" не найдена в метаданных.';vi='Khi cập nhật hồ sơ đã cấp"
"""%1"" không tìm thấy vai trò ""%2"" trong metadata.'"),
					ProfileProperties.Description,
					Role);
			EndIf;
