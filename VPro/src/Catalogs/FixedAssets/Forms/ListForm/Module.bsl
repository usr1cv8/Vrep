...
Procedure SetEnabled()
	...
	If CurrentData <> Undefined Then
		...
		ElsIf Not CurrentData.Property("RowGroup")
			  AND ValueIsFilled(CurrentData.State)
			  AND CurrentData.State = NStr("en='Not accepted for accounting';vi='Không tiếp nhận vào kế toán'") Then
...
Function GetPeriodOfLastDepreciation(Val Company)
	...
	Company = ?(GetFunctionalOption("MultipleCompaniesAccounting"), Company, Catalogs.Companies.MainCompany);
	
	Query.SetParameter("Company", SmallBusinessServer.GetCompany(Company));
	Selection = Query.Execute().Select();
	
	If Selection.Next() Then
		Return NStr("en='Last accrual: ';ru='Последнее начисление: ';vi='Lần tính lương gần nhất:'") + Format(Selection.Date, "DLF=DD");
	Else
		Return "";
	EndIf;
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	List.Parameters.SetParameterValue("NotAcceptedForAccounting", NStr("en = 'Not accepted for accounting'; ru = 'Не принято к учёту'; vi = 'Không tiếp nhận vào kế toán'"));
