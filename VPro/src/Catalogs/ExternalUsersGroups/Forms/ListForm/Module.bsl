...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.ChoiceMode Then
		...
		If Parameters.CloseOnChoice = False Then
			// Choice mode.
			Title = NStr("en='Select external user groups';ru='Подбор групп внешних пользователей';vi='Chọn nhóm người sử dụng ngoài'");
			Items.List.Multiselect = True;
			Items.List.SelectionMode = TableSelectionMode.MultiRow;
		Else
			Title = NStr("en='Select an external user group';ru='Выбор группы внешних пользователей';vi='Chọn nhóm người sử dụng ngoài'");
		EndIf;
	EndIf;
EndProcedure
