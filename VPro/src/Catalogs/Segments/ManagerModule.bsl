...
Function GetAvailableFilterRules() Export
	...
	CounterpartyPropertiesGroup.Presentation = NStr("en='Attributes (main, additional)';ru='Реквизиты (основные, дополнительные)';vi='Mục tin (chính, bổ sung)'");
	CounterpartyPropertiesGroup.IsFolder = True;
	
	NewRule = CounterpartyPropertiesGroup.Rows.Add();
	NewRule.Name = "Tag";
	NewRule.Presentation = NStr("en='Tag';ru='Тег';vi='Nhãn đánh dấu'");
	...
	GroupContactInformation.Presentation = NStr("en='Addresses (geography)';ru='Адреса (география)';vi='Địa chỉ (địa lý)'");
	...
	While Selection.Next() Do
		...
		If Selection.Type = Enums.ContactInformationTypes.Address Then
			...
			NewRule.IsFolder = False;
			NewRule.MultipleUse = False;
			NewRule.Presentation = NStr("en='Country';ru='Страна';vi='Nước'") + " (" + Lower(RuleAddress.Presentation) + ")";
			NewRule.ValueProperties.Insert("TypeRestriction", TypeDescriptionRow);
			NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
			AddComparisonTypes(NewRule, "BeginsWith,NotBeginsWith,Contains,NotContains", 3);
			...
			NewRule.IsFolder = False;
			NewRule.MultipleUse = False;
			NewRule.Presentation = NStr("en='Region';ru='Регион';vi='Vùng'") + " (" + Lower(RuleAddress.Presentation) + ")";
			NewRule.ValueProperties.Insert("TypeRestriction", TypeDescriptionRow);
			NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
			AddComparisonTypes(NewRule, "BeginsWith,NotBeginsWith,Contains,NotContains", 3);
			...
			NewRule.IsFolder = False;
			NewRule.MultipleUse = False;
			NewRule.Presentation = NStr("en='City';ru='Город';vi='Thành phố'") + " (" + Lower(RuleAddress.Presentation) + ")";
			NewRule.ValueProperties.Insert("TypeRestriction", TypeDescriptionRow);
			NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
			AddComparisonTypes(NewRule, "BeginsWith,NotBeginsWith,Contains,NotContains", 3);
	...
	EventGroup.Presentation = NStr("en='Events (remoteness, quantity)';ru='События (давность, количество)';vi='Sự kiện (thời gian kéo dài, số lượng)'");
	EventGroup.IsFolder = True;
	
	NewRule = EventGroup.Rows.Add();
	NewRule.Name = "EventsDateLast";
	NewRule.Presentation = NStr("en='Last event date';ru='Дата последнего события';vi='Ngày của sự kiện gần đây nhất'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = True;
	NewRule.ValueProperties.Insert("TypeRestriction", TypeDescriptionStandardDate);
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
	...
	RuleEventsQuantity.Presentation = NStr("en='Number of events';ru='Количество событий';vi='Số lượng sự kiện'");
	...
	NewRule.Presentation = NStr("en='For period';ru='По периоду';vi='Theo kỳ'") + " (" + Lower(RuleEventsQuantity.Presentation) + ")";
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	...
	GroupAdditional.Presentation = NStr("en='Event clarifications';ru='Уточнения событий';vi='Lời giải thích sự kiện'");
	GroupAdditional.IsFolder = True;
	
	NewRule = GroupAdditional.Rows.Add();
	NewRule.Name = "EventsState";
	NewRule.Presentation = NStr("en='Event state';ru='Состояние событий';vi='Trạng thái sự kiện'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	...
	NewRule.Presentation = NStr("en='Event type';ru='Тип события';vi='Kiểu sự kiện'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	...
	GroupOrders.Presentation = NStr("en='Customer orders (remoteness, quantity)';ru='Заказы покупателей (давность, количество)';vi='Đơn hàng của khách (thời hạn, số lượng)'");
	GroupOrders.IsFolder = True;
	
	NewRule = GroupOrders.Rows.Add();
	NewRule.Name = "CustomerOrdersDateLast";
	NewRule.Presentation = NStr("en='Last order date';ru='Дата последнего заказа';vi='Ngày của đơn hàng gần đây nhất'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = True;
	NewRule.ValueProperties.Insert("TypeRestriction", TypeDescriptionStandardDate);
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
	...
	RuleOrdersQuantity.Presentation = NStr("en='Number of orders';ru='Количество заказов';vi='Số lượng đơn hàng'");
	...
	NewRule.Presentation = NStr("en='For period';ru='По периоду';vi='Theo kỳ'") + " (" + Lower(RuleOrdersQuantity.Presentation) + ")";
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	NewRule.ValueProperties.Insert("TypeRestriction", TypeDescriptionStandardPeriod);
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
	...
	GroupAdditional.Presentation = NStr("en='Clarifications of customer orders';ru='Уточнения заказов покупателей';vi='Giải thích đơn hàng của khách'");
	GroupAdditional.IsFolder = True;
	
	NewRule = GroupAdditional.Rows.Add();
	NewRule.Name = "CustomerOrdersOrderState";
	NewRule.Presentation = NStr("en='State of customer orders';ru='Состояние заказов покупателя';vi='Trạng thái đơn hàng của khách'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	...
	Account_sGroup.Presentation = NStr("en='Proforma invoices (issue date, quantity)';ru='Счета на оплату (давность, количество)';vi='Hóa đơn thanh toán (thời gian kéo dài, số lượng)'");
	Account_sGroup.IsFolder = True;
	
	NewRule = Account_sGroup.Rows.Add();
	NewRule.Name = "InvoicesForPaymentLastDate";
	NewRule.Presentation = NStr("en='Last account date';ru='Дата последнего счета';vi='Ngày của tài khoản gần đây nhất'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = True;
	NewRule.ValueProperties.Insert("TypeRestriction", TypeDescriptionStandardDate);
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
	...
	RuleAccountsQuantity.Presentation = NStr("en='Number of accounts';ru='Количество счетов';vi='Số lượng tài khoản'");
	...
	NewRule.Presentation = NStr("en='For period';ru='По периоду';vi='Theo kỳ'") + " (" + Lower(RuleAccountsQuantity.Presentation) + ")";
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	NewRule.ValueProperties.Insert("TypeRestriction", TypeDescriptionStandardPeriod);
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
	...
	GroupSalesProductsAndServices.Presentation = NStr("en='Sales (products and services)';ru='Продажи (номенклатура)';vi='Bán hàng (mặt hàng)'");
	...
	NewRule.Presentation = NStr("en='ProductAndServices';ru='ProductAndServices';vi='ProductAndServices'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	...
	NewRule.Presentation = NStr("en='Products and services group';ru='Группа номенклатуры';vi='Nhóm mặt hàng'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	NewRule.ValueProperties.Insert("TypeRestriction", New TypeDescription("CatalogRef.ProductsAndServices"));
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Folders);
	...
	NewRule.Presentation = NStr("en='Products and services group';ru='Номенклатурная группа';vi='Nhóm sản phẩm'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	...
	GroupAdditional.Presentation = NStr("en='Sales clarification by products and services';ru='Уточнения продаж по номенклатуре';vi='Lời giải thích bán hàng theo mặt hàng'");
	GroupAdditional.IsFolder = True;
	
	NewRule = GroupAdditional.Rows.Add();
	NewRule.Name = "SalesProductsAndServicesPeriod";
	NewRule.Presentation = NStr("en='For the period (sales by products and services)';ru='За период (продажи по номенклатуре)';vi='Toàn kỳ (bán hàng theo mặt hàng)'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	NewRule.ValueProperties.Insert("TypeRestriction", TypeDescriptionStandardPeriod);
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
	...
	GroupSalesIncome.Presentation = NStr("en='Sales (revenue, profit)';ru='Продажи (выручка, прибыль)';vi='Bán hàng (doanh thu, lợi nhuận)'");
	GroupSalesIncome.IsFolder = True;
	
	NewRule = GroupSalesIncome.Rows.Add();
	NewRule.Name = "SalesIncomeIncome";
	NewRule.Presentation = NStr("en='Revenue (man. currency)';ru='Выручка (упр. валюте)';vi='Doanh thu (tiền tệ quản trị)'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = True;
	...
	NewRule.Presentation = NStr("en='Gross profit (man. currency)';ru='Валовая прибыль (упр. валюте)';vi='Lãi gộp (tiền tệ quản trị)'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = True;
	NewRule.ValueProperties.Insert("TypeRestriction", CurrencyTypeDescription);
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
	...
	GroupAdditional.Presentation = NStr("en='Clarifications of revenue from sales';ru='Уточнения выручки от продаж';vi='Giải thích doanh thu bán hàng'");
	GroupAdditional.IsFolder = True;
	
	NewRule = GroupAdditional.Rows.Add();
	NewRule.Name = "SalesIncomePeriod";
	NewRule.Presentation = NStr("en='For a period (revenue from sales)';ru='За период (выручка от продаж)';vi='Toàn kỳ (doanh thu bán hàng)'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = False;
	NewRule.ValueProperties.Insert("TypeRestriction", TypeDescriptionStandardPeriod);
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
	...
	GroupDebt.Name = "Debt";
	GroupDebt.Presentation = NStr("en='Debt (debt, overdue)';ru='Задолженность (долг, просрочка)';vi='Công nợ (nợ, nợ quá hạn)'");
	GroupDebt.IsFolder = True;
	
	NewRule = GroupDebt.Rows.Add();
	NewRule.Name = "CustomerDebtAmount";
	NewRule.Presentation = NStr("en='Amount of customer debt (man. currency)';ru='Сумма долга покупателя (упр. валюте)';vi='Số tiền công nợ của khách hàng (tiền tệ quản trị)'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = True;
	NewRule.ValueProperties.Insert("TypeRestriction", CurrencyTypeDescription);
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
	...
	NewRule.Presentation = NStr("en='Customer overdue period (days)';ru='Срок просрочки покупателя (дней)';vi='Thời hạn quá hạn của khách hàng (ngày)'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = True;
	...
	NewRule.Presentation = NStr("en='Amount of debt to supplier (man. currency)';ru='Сумма долга поставщику (упр. валюте)';vi='Số tiền công nợ phải trả nhà cung cấp (tiền tệ quản trị)'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = True;
	NewRule.ValueProperties.Insert("TypeRestriction", CurrencyTypeDescription);
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
	...
	NewRule.Presentation = NStr("en='Supplier overdue period (days)';ru='Срок просрочки поставщику (дней)';vi='Thời hạn quá hạn nhà cung cấp (ngày)'");
	NewRule.IsFolder = False;
	NewRule.MultipleUse = True;
	NewRule.ValueProperties.Insert("TypeRestriction", New TypeDescription("Number"));
	NewRule.ValueProperties.Insert("ChoiceFoldersAndItems", FoldersAndItems.Auto);
...
Procedure AddComparisonTypes(Rule, AddedComparisonsKinds, DefaultTypeNumber = 1)
	...
	For Each KindInString IN TypesArray Do
		If KindInString = "Equal" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.Equal,			 NStr("en='Equal';ru='равных';vi='bằng'"));
		ElsIf KindInString = "NotEqual" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.NotEqual,		 NStr("en='Not equal';ru='Не равно';vi='Không bằng'"));
		ElsIf KindInString = "Greater" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.Greater,		 NStr("en='Greater';ru='большая';vi='lớn'"));
		ElsIf KindInString = "GreaterOrEqual" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.GreaterOrEqual, NStr("en='More or equal';ru='Больше или равно';vi='Lớn hơn hoặc bằng'"));
		ElsIf KindInString = "Less" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.Less,		 NStr("en='Less';ru='Меньше';vi='Nhỏ hơn'"));
		ElsIf KindInString = "LessOrEqual" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.LessOrEqual, NStr("en='Less or equal';ru='Меньше или равно';vi='Nhỏ hơn hoặc bằng'"));
		ElsIf KindInString = "InList" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.InList,		 NStr("en='In the list';ru='В списке';vi='Trong danh sách'"));
		ElsIf KindInString = "NotInList" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.NotInList,		 NStr("en='Not in the list';ru='Не в списке';vi='Không trong danh sách'"));
		ElsIf KindInString = "InHierarchy" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.InHierarchy,		 NStr("en='In group';ru='В группе';vi='Trong nhóm'"));
		ElsIf KindInString = "NotInHierarchy" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.NotInHierarchy,	 NStr("en='Not in group';ru='Не в группе';vi='Không trong nhóm'"));
		ElsIf KindInString = "Filled" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.Filled,		 NStr("en='filled in';ru='заполненный';vi='đã điền'"));
		ElsIf KindInString = "NotFilled" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.NotFilled,	 NStr("en='Not filled in';ru='Не заполнено';vi='Chưa điền'"));
		ElsIf KindInString = "BeginsWith" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.BeginsWith,	 NStr("en='Begins with';ru='Начинается с';vi='Bắt đầu từ'"));
		ElsIf KindInString = "NotBeginsWith" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.NotBeginsWith,	 NStr("en='Does not begin with';ru='Не начинается с';vi='Không bắt đầu từ'"));
		ElsIf KindInString = "Contains" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.Contains,		 NStr("en='Contains';ru='Содержит';vi='Có chứa'"));
		ElsIf KindInString = "NotContains" Then
			Rule.AvailableComparisonTypes.Add(DataCompositionComparisonType.NotContains,	 NStr("en='Does not contain';ru='Не содержит';vi='Không chứa'"));
		EndIf;
	EndDo;
