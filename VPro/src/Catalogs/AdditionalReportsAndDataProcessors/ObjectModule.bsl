	...
	If Not AdditionalReportsAndDataProcessors.CheckGlobalProcessing(Type) Then
		If Not UseForObjectForm AND Not UseForListForm 
			...
			CommonUseClientServer.MessageToUser(
				NStr("en='Disable publishing or select at least one of the forms to use';ru='Необходимо отключить публикацию или выбрать для использования как минимум одну из форм';vi='Cần ngắt bỏ đăng tải hoặc chọn để sử dụng như là tối thiểu một trong những biểu mẫu'")
				,
				,
				,
...
""
"Для продолжения необходимо изменить вид Публикации с ""%2"" на ""%3"" или ""%4"".';vi='Tên ""%1"", sử dụng dữ liệu báo cáo (bộ xử lý) đã bị chiếm bởi báo cáo (bộ xử lý) bổ sung khác. "
""
"Để tiếp tục cần thay đổi dạng Đăng tải từ ""%2"" sang ""%3"" hoặc ""%4"".'"),
					ObjectName,
			...
			Else
				ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Name ""%1"" used by the report (data processor) ""%2"" is already in use by another published additional report (data processor).';ru='Имя ""%1"", используемое отчетом (обработкой) ""%2"", уже занято другим опубликованным дополнительным отчетом (обработкой).';vi='Báo cáo (bộ xử lý) bổ sung được đăng tải khác đã chiếm tên ""%1"" của báo cáo (bộ xử lý) ""%2"" .'"),
					ObjectName,
...
Procedure BeforeWrite(Cancel)
	...
	If IsNew() AND Not AdditionalReportsAndDataProcessors.AddRight(ThisObject) Then
		Raise NStr("en='Insufficient rights to add additional reports or data processors.';ru='Недостаточно прав для добавления дополнительных отчетов или обработок.';vi='Không đủ quyền để thêm báo cáo bổ sung hoặc bộ xử lý.'");
	EndIf;
	
	// Preliminary checks
	If Not IsNew() AND Type <> CommonUse.ObjectAttributeValue(Ref, "Type") Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Cannot change the kind of existing additional report or data processor.';ru='Невозможно сменить вид существующего дополнительного отчета или обработки.';vi='Không thể thay đổi dạng báo cáo hoặc bộ xử lý bổ sung hiện tại.'"),,,,
			Cancel);
		Return;
	EndIf;
...
Function TaskPresentation(Command)
	...
	Return (
		...
		+ TrimAll(Description)
		+ " / "
		+ NStr("en='Command';ru='команда';vi='Lệnh'")
		+ ": "
...
Procedure OnWriteGlobalReport(Cancel)
	...
	If CommonUse.SubsystemExists("StandardSubsystems.ReportsVariants") Then
		...
		Try
			...
			EndIf;
		Except
			ErrorText = NStr("en='Connection error:';ru='Ошибка подключения:';vi='Lỗi kết nối:'") + Chars.LF + DetailErrorDescription(ErrorInfo());
