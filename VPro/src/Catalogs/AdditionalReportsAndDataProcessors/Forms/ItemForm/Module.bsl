...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not AddRight Then
		If IsNew Then
			Raise NStr("en='Insufficient access rights to add additional reports and data processors.';ru='Недостаточно прав доступа для добавления дополнительных отчетов или обработок.';vi='Không đủ quyền truy cập để thêm báo cáo bổ sung hoặc bộ xử lý.'");
		Else
	...
	If ShowDialogLoadFromFileOnOpen AND Not Items.LoadFromFile.Visible Then
		Raise NStr("en='Insufficient rights to import additional reports and data processors';ru='Недостаточно прав для загрузки дополнительных отчетов и обработок';vi='Không đủ quyền để kết nhập báo cáo và bộ xử lý bổ sung'");
	EndIf;
...
Procedure AdditionalReportVariantsBeforeDelete(Item, Cancel)
	...
	If Not Variant.User Then
		ShowMessageBox(, NStr("en='Cannot mark predefined report variant for deletion.';ru='Пометка на удаление предопределенного варианта отчета запрещена.';vi='Cấm đánh dấu xóa phương án báo cáo định trước.'"));
		Return;
	EndIf;
...
Procedure AdditionalReportVariantsOpen(Command)
	...
	If Variant = Undefined Then
		ShowMessageBox(, NStr("en='Select report variant.';ru='Выберите вариант отчета.';vi='Hãy chọn phương án báo cáo.'"));
		Return;
	EndIf;
...
"в открытых окнах необходимо их закрыть и открыть заново.';vi='Để áp dụng những thay đổi"
"trong cửa sổ mở ra, cần đóng cửa sổ và mở lại một lần nữa.'");
...
Procedure UpdateFromFileStart()
	...
	If Object.Ref.IsEmpty() Then
		...
		DialogueParameters.Title = NStr("en='Select a file of external report or data processor';ru='Выберите файл внешнего отчета или обработки';vi='Hãy chọn tệp báo cáo hoặc bộ xử lý ngoài'");
	ElsIf Object.Type = TypeAdditionalReport Or Object.Type = ReportKind Then
		DialogueParameters.FilterIndex = 1;
		DialogueParameters.Title = NStr("en='Select an external report file';ru='Выберите файл внешнего отчета';vi='Hãy chọn tệp báo cáo ngoài'");
	Else
		DialogueParameters.FilterIndex = 2;
		DialogueParameters.Title = NStr("en='Select an external data processor file';ru='Выберите файл внешней обработки';vi='Hãy chọn tệp bộ xử lý ngoài'");
	EndIf;
...
Procedure UpdateFromFileAfterFileSelection(PlacedFiles, RegistrationParameters) Export
	...
	Else
		...
		StandardSubsystemsClient.ReturnResultAfterShowWarning(
			NStr("en='File extension does not match those of the external report (ERF) or data processor (EPF).';ru='Расширение файла не соответствует расширению внешнего отчета (ERF) или обработки (EPF).';vi='Đuôi tệp không tương ứng với đuôi tệp báo cáo ngoài (ERF) hoặc bộ xử lý (EPF).'"),
...
Procedure UpdateFromFileMechanicsAtClient(RegistrationParameters)
	...
	If RegistrationParameters.Success Then
		NotificationTitle = ?(RegistrationParameters.IsReport, NStr("en='External report file is imported';ru='Файл внешнего отчета загружен';vi='Đã kết nhập tệp báo cáo ngoài'"), NStr("en='External data processor file is imported';ru='Файл внешней обработки загружен';vi='Đã kết nhập tệp bộ xử lý ngoài'"));
...
Procedure UpdateFromFileShowConflicts(RegistrationParameters)
	If RegistrationParameters.ConflictsCount > 1 Then
		If RegistrationParameters.IsReport Then
			QuestionTitle = NStr("en='Conflicts during external report import';ru='Конфликты при загрузке внешнего отчета';vi='Xung đột khi kết nhập báo cáo ngoài'");
...
"3. ""[Open]"" - отменить загрузку и открыть список конфликтующих отчетов.';vi='Tên nội bộ của báo cáo ""[Name]"" đã bị chiếm bởi báo cáo bổ sung khác: ([Count]): [List]."
" "
"Hãy chọn:"
"1. ""[Continue]"" - thực hiện kết nhập, sau khi ngắt đăng tải báo cáonày."
"2. ""[Disable]"" - thực hiện kết nhập, sau khi ngắt đăng tải báo cáo khác."
"3. ""[Open]"" - hủy kết nhập và mở thẻ báo cáo khác.'");
		Else
			QuestionTitle = NStr("en='Conflicts occurred during import of external data processor';ru='Конфликты при загрузке внешней обработки';vi='Xung đột khi kết nhập bộ xử lý ngoài'");
...
"3. ""[Open]"" - отменить загрузку и открыть список конфликтующих обработок.';vi='Tên nội bộ của bộ xử lý ""[Name]"" đã bị chiếm bởi bộ xử lý bổ sung khác: ([Count]): [List]."
" "
"Hãy chọn:"
"1. ""[Continue]"" - thực hiện kết nhập, sau khi ngắt đăng tải bộ xử lý này."
"2. ""[Disable]"" - thực hiện kết nhập, sau khi ngắt đăng tải tất cả bộ xử lý xung đột khác."
"3. ""[Open]"" - hủy kết nhập và mở danh sách các bộ xử lý bị xung đột.'");
		EndIf;
		DisableButtonPresentation = NStr("en='Disable conflicting';ru='Отключить конфликтующие';vi='Tắt xung đột'");
		OpenButtonPresentation = NStr("en='Cancel and show list';ru='Отменить и показать список';vi='Hủy bỏ và hiển thị danh sách'");
	Else
		If RegistrationParameters.IsReport Then
			QuestionTitle = NStr("en='Conflict during external report import';ru='Конфликт при загрузке внешнего отчета';vi='Xung đột khi kết nhập báo cáo ngoài'");
...
"3. ""[Open]"" - отменить загрузку и открыть карточку другого отчета.';vi='Tên nội bộ của báo cáo ""[Name]"" đã bị chiếm bởi báo cáo bổ sung khác: [List]."
" "
"Hãy chọn:"
"1. ""[Continue]"" - thực hiện kết nhập, sau khi ngắt đăng tải báo cáo này."
"2. ""[Disable]"" - thực hiện kết nhập, sau khi ngắt đăng tải báo cáo khác."
"3. ""[Open]"" - hủy kết nhập và mở thẻ báo cáo khác.'");
			DisableButtonPresentation = NStr("en='Disable another report';ru='Отключить другой отчет';vi='Ngắt báo cáo khác'");
		Else
			QuestionTitle = NStr("en='Conflict occurred during import of external data processor';ru='Конфликт при загрузке внешней обработки';vi='Xung đột khi kết nhập bộ xử lý ngoài'");
...
"3. ""[Open]"" - отменить загрузку и открыть карточку другой обработку.';vi='Tên nội bộ của bộ xử lý ""[Name]"" đã bị chiếm bởi bộ xử lý bổ sung khác: [List]."
" "
"Hãy chọn:"
"1. ""[Continue]"" - thực hiện kết nhập, sau khi ngắt đăng tải bộ xử lý này."
"2. ""[Disable]"" - thực hiện kết nhập, sau khi ngắt đăng tải bộ xử lý khác."
"3. ""[Open]"" - hủy kết nhập và mở thẻ bộ xử lý khác.'");
			DisableButtonPresentation = NStr("en='Disable another data processor';ru='Отключить другую обработку';vi='Ngắt bộ xử lý khác'");
		EndIf;
		OpenButtonPresentation = NStr("en='Cancel and show';ru='Отменить и открыть';vi='Hủy bỏ và mở ra'");
	EndIf;
	ContinueButtonPresentation = NStr("en='In debug mode';ru='В режиме отладки';vi='Ở chế độ gỡ rối'");
...
Procedure UpdateFromFileConflictsResolution(Response, RegistrationParameters) Export
	...
	ElsIf Response = "CancelAndOpen" Then
		...
		If ShowList Then // List form with the conflicting selection.
			...
			FormTitle = NStr("en='Additional reports and data processors with internal name ""%1""';ru='Дополнительные отчеты и обработки с внутреннем именем ""%1""';vi='Báo cáo và bộ xử lý bổ sung với tên nội bộ ""%1""'");
...
Procedure OpenOption()
	...
	If Not ValueIsFilled(Variant.Ref) Then
		ErrorText = NStr("en='Report variant ""%1"" is not registered.';ru='Вариант отчета ""%1"" не зарегистрирован.';vi='Phương án báo cáo ""%1"" chưa được ghi nhận.'");
...
"вариантом запуска ""%1"" не может использоваться в регламентных заданиях.';vi='Không thể dùng lệnh có"
"phương án khởi động ""%1"" trong nhiệm vụ thường kỳ.'");
...
Function DisabledSchedulePresentation()
	Return NStr("en='Schedule is not set';ru='Расписание не задано';vi='Lịch biểu chưa chỉ ra'");
EndFunction

&AtClientAtServerNoContext
Function UsersQuickAccessPresentation(UserCount)
	If UserCount = 0 Then
		Return NStr("en='No';ru='Нет';vi='Không'");
	EndIf;
	
	LastDigit = UserCount - 10 * Int(UserCount/10);
	
	If LastDigit = 1 Then
		QuickAccessView = NStr("en='User %1';ru='%1 пользователя';vi='%1 người sử dụng'");
	ElsIf LastDigit = 2 Or LastDigit = 3 Or LastDigit = 4 Then
		QuickAccessView = NStr("en='User %1';ru='%1 пользователя';vi='%1 người sử dụng'");
	Else
		QuickAccessView = NStr("en='%1 users';ru='%1 пользователей';vi='%1 người sử dụng'");
	EndIf;
...
Procedure UpdateFromFileMechanicsAtServer(RegistrationParameters)
	...
	Else
		If RegistrationParameters.IsReport Then
			ErrorTitle = NStr("en='Report is not connected';ru='Отчет не подключен';vi='Báo cáo chưa được kết nối'");
		Else
			ErrorTitle = NStr("en='Data processor is not connected';ru='Обработка не подключена';vi='Bộ xử lý không được kết nối'");
		EndIf;
...
Function PrepareFormParametersMetadataObjectSelect()
	...
	FormParameters.Insert("Title", NStr("en='Additional data processor purpose';ru='Назначение дополнительной обработки';vi='Tên bộ xử lý bổ sung'"));
...
Procedure SetVisibleEnabled(Registration = False)
	...
	If Object.Type = TypeAdditionalReport AND Object.UsesVariantsStorage Then
		...
		If Registration OR VariantCount = 0 Then
			...
			Items.PageVariants.Title = NStr("en='Report variants';ru='Варианты отчета';vi='Phương án báo cáo'");
		Else
			...
			Items.PageVariants.Title = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Report variants (%1)';ru='Варианты отчета (%1)';vi='Phương án báo cáo (%1)'"),
	...
	If CommandsCount = 0 Then
		...
		Items.PageCommands.Title = NStr("en='Commands';ru='Команды';vi='Lệnh'");
	...
	Else
		...
		Items.PageCommands.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Commands (%1)';ru='Команды (%1)';vi='Lệnh (%1)'"),
	...
	Else
		...
		Items.PermissionPage.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Permissions (%1)';ru='Разрешения (%1)';vi='Cho phép (%1)'"),
...
"application using ""personal"" security profile %1 which  allows only following operations:';ru='Дополнительный отчет или обработка будет подключаться к программе с использованием ""персонального"" профиля безопасности %1, в котором будут разрешены только следующие операции:';vi='Báo cáo hoặc bộ xử lý bổ sung sẽ được kết nối với chương trình bằng cách sử dụng hồ sơ bảo mật ""cá nhân"" %1, trong đó sẽ chỉ cho phép các giao dịch sau:'"), SafeMode);
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='%1 not a correct connection mode for additional reports and data processors that require permissions for security profile usage.';ru='%1 не является корректным режимом подключения для дополнительных отчетов и обработок, требующих разрешений на использование профилей безопасности!';vi='%1 không phải là chế độ kết nối đúng đối với báo cáo và bộ xử lý bổ sung mà được phép sử dụng hồ sơ an toàn!'"),
				SafeMode);
		EndIf;
	...
	If ThisIsGlobalDataProcessor Then
		If Object.Sections.Count() = 0 Then
			PrescriptionPresentation = NStr("en='Undefined';ru='Неопределено';vi='Không xác định'");
		Else
	...
	If IsNew Then
		Title = ?(IsReport, NStr("en='Additional report (creation)';ru='Дополнительный отчет (создание)';vi='Báo cáo bổ sung (tạo mới)'"), NStr("en='Additional data processor (creation)';ru='Дополнительная обработка (создание)';vi='Bộ xử lý bổ sung (tạo mới)'"));
	Else
		Title = Object.Description + " " + ?(IsReport, NStr("en='(Additional report)';ru='(Дополнительный отчет)';vi='(Báo cáo bổ sung)'"), NStr("en='(Additional data processor)';ru='(Дополнительная обработка)';vi='(Bộ xử lý bổ sung)'"));
	EndIf;
	...
	For Each ItemCommand In Object.Commands Do
		...
		Else
			ItemCommand.ScheduledJobPresentation = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Not applied for commands with launch option ""%1""';ru='Не применимо для команд с вариантом запуска ""%1""';vi='Không áp dụng đối với lệnh có phương án khởi động ""%1""'"),
...
Procedure AdditionalReportVariantsFill()
	...
	Except
		ErrorText = NStr("en='Cannot receive a list of report variants due to the error that occurred when connecting this report:';ru='Не удалось получить список вариантов отчета из-за ошибки, возникшей при подключении этого отчета:';vi='Không thể nhận danh sách phương án báo cáo do lỗi phát sinh khi kết nối báo cáo này:'");
...
Procedure FileNameForDebuggingStartChoice(Item, ChoiceData, StandardProcessing)
	...
	OpeningFileDialogue.Title = NSTR("en='Select file for debugging';ru='Выберите файл для отладки';vi='Hãy chọn tệp để gỡ rối'");
	If OpeningFileDialogue.Choose() Then
	    ...
	    EndDo;
	Else
	    DoMessageBox( NSTR("en='File is not selected!';vi='Chưa chọn tệp!'"));
	EndIf;
EndProcedure
