...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If AllPublicationsBesidesUnused.Count() > 1 Then
		...
		ArrayPresentation = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='%1 or %2';ru='%1 или %2';vi='%1 hoặc %2'"),
	...
	ChoiceList.Add(1, NStr("en='Only reports';ru='Только отчеты';vi='Chỉ báo cáo'"));
	ChoiceList.Add(2, NStr("en='Only data processors';ru='Только обработки';vi='Chỉ bộ xử lý'"));
...
"Выберите дополнительный отчет или обработку.';vi='Không thể thực hiện lệnh đối với đối tượng đã chỉ ra."
"Hãy chỉ ra báo cáo hoặc bộ xử lý bổ sung.'"));
		Return False;
	EndIf;
...
"Выберите дополнительный отчет или обработку.';vi='Không thể thực hiện lệnh đối với nhóm."
"Hãy chọn báo cáo hoặc bộ xử lý bổ sung.'"));
		Return False;
	EndIf;
	Return True;
EndFunction
...
Procedure SetConditionalAppearance()
	...
	AdditionalReportsAndDataProcessors.AddConditionalAppearanceElement(ThisObject, Instruction);
...
Function PublicationChange(PublicationVariant)
	...
	If SelectedRows.Count() = 0 Then
		MessageText = NStr("en='No additional report (data processor) is selected';ru='Не выбран ни один дополнительный отчет (обработка)';vi='Chưa chọn báo cáo (bộ xử lý) bổ sung nào'");
	...
	If SelectedRows.Count() = 1 Then
		...
		MessageText = NStr("en='Publication of additional report (data processor) ""%1"" is changed';ru='Изменена публикация дополнительного отчета (обработки) ""%1""';vi='Đã thay đổi việc đăng tải báo cáo (bộ xử lý) bổ sung ""%1""'");
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessageText, ObjectName);
	Else
		MessageText = NStr("en='Publication of %1 additional reports (data processors) is changed';ru='Изменена публикация у %1 дополнительных отчетов (обработок)';vi='Đã thay đổi việc đăng tải %1 báo cáo (bộ xử lý) bổ sung'");
	...
	OutputNotification.Title = NStr("en='Publication is changed';ru='Изменена публикация';vi='Đã thay đổi việc đăng tải'");
