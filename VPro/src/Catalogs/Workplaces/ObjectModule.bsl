...
Procedure OnCopy(CopiedObject)
	...
	Description = ?(IsBlankString(String(Code)), NStr("en='Work place';ru='рабочее место';vi='chỗ làm việc'"), String(Code))
	             + ?(IsBlankString(String(Code)), ": ", "/")
	             + ?(IsBlankString(String(CurrentUser)), NStr("en='User';ru='Пользователь';vi='Người sử dụng'"), String(CurrentUser));
