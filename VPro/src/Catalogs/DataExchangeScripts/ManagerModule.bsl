...
Procedure CreateScenario(InfobaseNode, Schedule = Undefined) Export
	...
	Description = NStr("en='Automatic data synchronization with %1';ru='Автоматическая синхронизация данных с %1';vi='Tự động đồng bộ hóa dữ liệu với %1'");
...
Procedure SetScheduledJobParameters(ScheduledJobObject, JobSchedule, CurrentObject)
	...
	ScheduledJobDescription = NStr("en='Exchanging using scenario: %1';ru='Выполнение обмена по сценарию: %1';vi='Thực hiện trao đổi theo kịch bản: %1'");
...
Procedure WriteScheduledJob(Cancel, ScheduledJobObject)
	...
	Except
		...
		MessageString = NStr("en='An error occurred when saving the exchange schedule. Error description: %1';ru='Произошла ошибка при сохранении расписания выполнения обменов. Подробное описание ошибки: %1';vi='Xảy ra lỗi khi lưu lịch biểu thực hiện trao đổi. Mô tả chi tiết lỗi: %1'");
...
Procedure AddDumpToDataExchangeScripts(DataExchangeScenario, InfobaseNode) Export
	...
	Filter = New Structure("InfobaseNode, RunningAction", InfobaseNode, Enums.ActionsAtExchange.DataExport);
...
Procedure AddImportingToDataExchangeScripts(DataExchangeScenario, InfobaseNode) Export
	...
	Filter = New Structure("InfobaseNode, RunningAction", InfobaseNode, Enums.ActionsAtExchange.DataImport);
	If ExchangeSettings.FindRows(Filter).Count() = 0 Then
