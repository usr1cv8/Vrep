...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If IsNew
		...
		Description = NStr("en='Synchronization scenario for %1';ru='Сценарий синхронизации для %1';vi='Kịch bản đồng bộ hóa đối với %1'");
...
Procedure RefreshSchedulePresentation()
	...
	If SchedulePresentation = String(New JobSchedule) Then
		...
		SchedulePresentation = NStr("en='Schedule is not set';ru='Расписание не задано';vi='Lịch biểu chưa chỉ ra'");
...
Procedure ExecuteDataExchangeAtClient()
	...
	If CurrentlyProcessedLineNumber > LineCount Then // exit from recursion
		...
		Status(NStr("en='Data is synchronized.';ru='Данные синхронизированы.';vi='Dữ liệu đã được đồng bộ hóa.'"), ?(OutputState, 100, Undefined));
		Return; // exit
	EndIf;
	...
	MessageString = NStr("en='%1 is executed for %2';ru='Выполняется %1 для %2';vi='Đang thực hiện %1 đối với %2'");
