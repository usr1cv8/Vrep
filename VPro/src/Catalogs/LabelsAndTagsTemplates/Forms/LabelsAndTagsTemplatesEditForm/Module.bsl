...
Function CheckIsSomethingFitsSomewhere()
	...
	If Not (SpreadsheetDocumentField.PrintArea.Left = 0 AND SpreadsheetDocumentField.PrintArea.Right = 0) Then
		...
		If CountByHorizontal <> ArrayOfTables.Count() Then
			MessageText = NStr("en='Maximal amount by horizontal: ';ru='Максимальное количество по горизонтали: ';vi='Số lượng tối đa theo chiều ngang:'") + ArrayOfTables.Count();
	...
	If Not (SpreadsheetDocumentField.PrintArea.Top = 0 AND SpreadsheetDocumentField.PrintArea.Bottom = 0) Then
		...
		If VerticalQuantity <> ArrayOfTables.Count() Then
			MessageText = NStr("en='Maximal amount by vertical: ';ru='Максимальное количество по вертикали: ';vi='Số lượng tối đa theo chiều dọc:'") + ArrayOfTables.Count();
...
Procedure SetPrintArea(Command)
	...
	Else
		...
		Message.Text = NStr("en='Incorrect print area';ru='Некорректная область печати';vi='Sai vùng in'");
...
Procedure DefaultLabel(Command)
	...
	ShowQueryBox(Notification,NStr("en='The template being edited will be replaced with the default template, continue?';ru='Редактируемый шаблон будет заменен на шаблон по умолчанию, продолжить?';vi='Khuôn mẫu đang soạn sẽ thay thế cho khuôn mẫu theo mặc định, tiếp tục?'"), QuestionDialogMode.YesNo);
...
Procedure Union(Command)
	...
	Else
		...
		ClearMessages();
		Message = New UserMessage;
		Message.Text = NStr("en='Incorrect area.';ru='Некорректная область!';vi='Sai vùng!'");
		Message.Field = "SpreadsheetDocumentField";
		Message.Message();
