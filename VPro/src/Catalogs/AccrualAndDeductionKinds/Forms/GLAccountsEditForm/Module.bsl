...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CancelGLExpenseAccountChange(AccrualDeductionKind) Then
		...
		Items.GLAccountsGroup.ToolTip	= NStr("en='Records are registered for this kind of accrual (deduction) in the infobase. Cannot change the GL account.';ru='В базе есть движения по этому виду начисления (удержания)! Изменение счета учета запрещено!';vi='Trong cơ sở có bản ghi kết chuyển theo dạng tính lương (giữ lại tiền lương) này! Cấm thay đổi tài khoản kế toán!'");
...
Procedure OnOpen(Cancel)
	...
	If IsTax Then
		...
		ShowMessageBox(, NStr("en='GL accounts are not edited for the Tax accrual kind.';ru='Для типа вида начисления Налог счета учета не редактируются!';vi='Đối với kiểu dạng tính Thuế tài khoản kế toán không thể chỉnh sửa được!'"));
		Cancel = True;
