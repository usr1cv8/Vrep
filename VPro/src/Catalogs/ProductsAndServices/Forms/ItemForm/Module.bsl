...
Procedure SeeAttachedFile()
	...
	Else
		...
		MessageText = NStr("en='No preview image';ru='Отсутстует изображение для просмотра';vi='Không có hình ảnh xem trước'");
...
Procedure AddImageAtClient()
	...
	If Not ValueIsFilled(Object.Ref) Then
		...
		QuestionText = NStr("en='To select an image, write the object. Write?';ru='Для выбора изображения необходимо записать объект. Записать?';vi='Để chọn hình ảnh, cần ghi lại đối tượng. Ghi lại?'");
		Response = Undefined;
...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(Object.Ref) And Not ValueIsFilled(Parameters.CopyingValue)Then
		ProductsAndServicesCategoriesServer.FillProductsAndServicesPropertyByCategory(Object);
	EndIf;
	...
	If Not ValueIsFilled(Object.Ref) Then
		...
		If Not ValueIsFilled(Parameters.CopyingValue) Then
				...
				Object.VATRate = SmallBusinessReUse.GetVATRateWithoutVAT();
		EndIf;
	...
	// Work with prices
	PriceKind = ?(Object.ProductsAndServicesType = Enums.ProductsAndServicesTypes.Operation, 
					Catalogs.PriceKinds.Accounting,
					Catalogs.PriceKinds.GetMainKindOfSalePrices());
	FillBasicSalesPriceOnServer();
...
Procedure ProductsAndServicesCategoryOnChange(Item)
	...
	ProductsAndServicesCategoryOnChangeAtServer();

	If Object.ProductsAndServicesType <> ProductsAndServicesType Then
		SetVisibleAndEnabled(True);
	EndIf;
...
&AtServer
Procedure ProductsAndServicesCategoryOnChangeAtServer()
	
	If ValueIsFilled(Object.ProductsAndServicesCategory) Then
		ProductsAndServicesType = Object.ProductsAndServicesType;
		ProductsAndServicesCategoriesServer.FillProductsAndServicesPropertyByCategory(Object);
		If Items.ProductsAndServicesType.ChoiceList.FindByValue(Object.ProductsAndServicesType) = Undefined Then
			Object.ProductsAndServicesType = Items.ProductsAndServicesType.ChoiceList.Get(0).Value;
		EndIf;
	EndIf;
...
Procedure PriceChangeHistoryClick(Item)
	...
	If Not ValueIsFilled(Object.Ref) Then
		...
		WarningText = NStr("en='Item is not written. Cannot open the price history of an unwritten item.';ru='Элемент не записан. Открыть историю цен незаписанного элемента не возможно.';vi='Phần tử chưa được ghi lại. Không thể mở lịch sử giá của phần tử chưa được ghi lại.'");
		HeaderText 		= NStr("en='Cannot open the price history';ru='Невозможно открыть историю цен';vi='Không thể mở lịch sử giá'");
...
Procedure ChangeImage(Command)
	...
	Else
		...
		MessageText = NStr("en='No image for editing';ru='Отсутстует изображение для редактирования';vi='Không có hình vẽ để soạn'");
		CommonUseClientServer.MessageToUser(MessageText,, "PictureURL");
...
Function AdditionalAttributeCreationSettings()
	...
	If ValueIsFilled(Object.ProductsAndServicesCategory) Then
		CurrentSetOfProperties = Object.ProductsAndServicesCategory.PropertySet;
	Else
		CurrentSetOfProperties = Catalogs.AdditionalAttributesAndInformationSets.Catalog_ProductsAndServices_Common;
	EndIf;
