...
Procedure Fill(Command)
	...
	If Object.LowerBarButtons.Count()>0 Then
		...
		QuestionText = NStr("en='Button table is filled in. Clear?';ru='Таблица кнопок заполнена. Очистить?';vi='Bảng nút lệnh đã điền. Xóa?'");
...
Procedure FillByProductsAndServicesAtServer(ProductsAndServicesGroup)
	...
	If Object.Workplace.IsEmpty() Then
		...
		Message.Text = NStr("en='Select working';vi='Chọn làm việc'");
		Message.Message();
		Return;
	EndIf;
	...
	Message = New UserMessage;
	Message.Text = NStr("en='Filling in is completed!';vi='Hoàn thành điền vào!'");
	Message.Message();
...
Procedure CopySettingsToSourceFromReceiver(Settings, TheReceiver = False)
	...
	If SourceSettings.Ref = ReceiverSettings.Ref Then
		Message = New UserMessage;
		Message.Text = NStr("en='Current settings are selected! Copying is not completed!';vi='Cài đặt hiện tại đã được chọn! Sao chép chưa hoàn tất!'");
		Message.Message();
		Return;
	EndIf;
