...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CancelGLAccountChange(Ref) Then
		Items.GLAccountsGroup.ToolTip = NStr("en='Records are registered for this retail outlet in the infobase. Cannot change the GL account.';ru='В базе есть движения по этой розничной точке! Изменение счета учета запрещено!';vi='Trong cơ sở có bản ghi kết chuyển theo điểm bán lẻ này! Cấm thay đổi tài khoản kế toán!'");
...
Procedure OnOpen(Cancel)
	...
	If Not ThisIsRetailAccrualAccounting Then
		Cancel = True;
		ShowMessageBox(, NStr("en='GL accounts are edited only for retail with value accounting.';ru='Счета учетов редактируются только для розницы с суммовым учетом!';vi='Tài khoản kế toán chỉ được soạn cho bán lẻ với kế toán giá trị!'"));
	EndIf;
