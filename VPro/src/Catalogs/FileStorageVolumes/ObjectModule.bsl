...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Not AdditionalProperties.Property("SkipMainFillingCheck") Then
		...
		If Not OrderNumberIsUnique(FillOrder, Ref) Then
			ErrorText = NStr("en='Fill order is not unique. Volume with such order already exists in the system';ru='Порядок заполнения не уникален - в системе уже есть том с таким порядком';vi='Trình tự điền không đơn trị - trong hệ thống đã có quyển có trình tự như vậy'");
		...
		If MaximumSize <> 0 Then
			...
			If MaximumSize < CurrentSize Then
				ErrorText = NStr("en='Max size of the volume is smaller than the current size';ru='Максимальный размер тома меньше, чем текущий размер';vi='Dung lượng tối đa của quyển nhỏ hơn dung lượng hiện tại'");
		...
		If IsBlankString(FullPathWindows) AND IsBlankString(FullPathLinux) Then
			ErrorText = NStr("en='Full path is not entered';ru='Не заполнен полный путь';vi='Chưa điền đường dẫn đầy đủ'");
		...
		If Not GetFunctionalOption("SecurityProfilesAreUsed")
		   ...
		   AND (    Left(FullPathWindows, 2) <> "\\"
			...
			ErrorText = NStr("en='Path to the volume must have the UNC format (\\servername\resource).';ru='Путь к тому должен быть в формате UNC (\\servername\resource).';vi='Đường dẫn đến quyển cần có định dạng UNC (\\servername\resource).'");
			CommonUseClientServer.MessageToUser(ErrorText, , "FullPathWindows", "Object", Cancel);
			Return;
		EndIf;
	EndIf;
