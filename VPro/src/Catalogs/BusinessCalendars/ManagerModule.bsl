...
Procedure Print(ObjectsArray, PrintParameters, PrintFormsCollection, PrintObjects, OutputParameters) Export
	...
	If CommonUse.SubsystemExists("StandardSubsystems.Print") Then
		...
		PrintManagementModule.OutputSpreadsheetDocumentToCollection(
				...
				"BusinessCalendar", NStr("en='Business calendar';ru='Производственный календарь';vi='Lịch sản xuất'"),
...
Function PrintingFormBusinessCalendar(ParametersForPrintingForms) Export
	...
	While SelectionByYear.Next() Do
		...
		While SelectionByQuarter.Next() Do
			...
			NumberOfQuarter.Parameters.NumberOfQuarter = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='quarter %1';ru='%1 квартал';vi='%1 quý'"), SelectionByQuarter.QuarterWeek);
			...
			If SelectionByQuarter.QuarterWeek = 2 
				...
				ColumnOfMonth.Parameters.NameOfMonth 		= StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='%1 half-year';ru='%1 полугодие';vi='%1 nửa năm'"), SelectionByQuarter.QuarterWeek / 2);
				SpreadsheetDocument.Join(ColumnOfMonth);
			EndIf;
		...
		ColumnOfMonth.Parameters.NameOfMonth 		= StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='%1 year';ru='%1 год';vi='%1 năm'"), Format(SelectionByYear.YearCalendar, "NG="));
		SpreadsheetDocument.Join(ColumnOfMonth);
	...
	ColumnOfMonth.Parameters.WorkingTime24 	= WorkingTime24Year;
	ColumnOfMonth.Parameters.NameOfMonth 		= StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='%1 year';ru='%1 год';vi='%1 năm'"), Format(YearNumber, "NG="));
	...
	ColumnOfMonth.Parameters.NameOfMonth 		= NStr("en='Average monthly quantity';ru='Среднемесячное количество';vi='Số lượng hàng tháng'");
	SpreadsheetDocument.Join(ColumnOfMonth);
