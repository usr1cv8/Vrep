...
Function Attachable_PageEmployee1_OnGoingNext(Cancel)
	...
	If IsBlankString(Object.Description) Then
		...
		MessageText = NStr("en=""Fill in employee's full name."";ru='Необходимо заполнить ФИО сотрудника.';vi='Cần điền họ tên người lao động.'");
...
Function Attachable_PageEmploymentContract1_OnGoingNext(Cancel)
	...
	If Not ValueIsFilled(EmploymentContractEmploymentContractDate) Then
		...
		MessageText = NStr("en='Fill in hiring date.';ru='Необходимо заполнить дату приема на работу.';vi='Cần điền ngày tiếp nhận làm việc.'");
	...
	If Not ValueIsFilled(EmploymentContractCompany) Then
		...
		MessageText = NStr("en='Fill in company.';ru='Необходимо заполнить организацию.';vi='Cần điền doanh nghiệp.'");
	...
	If DepartmentsAreUsed
		...
		MessageText = NStr("en='Fill in department.';ru='Необходимо заполнить подразделение.';vi='Cần điền bộ phận.'");
	...
	If Not ValueIsFilled(EmploymentContractPosition) Then
		...
		MessageText = NStr("en=""Fill in employee's position."";ru='Необходимо заполнить должность сотрудника.';vi='Cần điền chức vụ người lao động.'");
	...
	If UsedStaffSchedule Then
		...
		If Not ValueIsFilled(EmploymentContractOccupiedRates) Then
			...
			MessageText = NStr("en='Fill in quantity of held rates.';ru='Необходимо заполнить количество занимаемых ставок.';vi='Cần điền số lượng thuế suất áp dụng.'");
...
Function Attachable_PageEmploymentContract2_OnGoingNext(Cancel)
	...
	If AccrualsAndDeductions.Count() < 1 Then
		...
		MessageText = NStr("en='Fill in the table of accruals and deductions.';ru='Необходимо заполнить таблицу начислений и удержаний.';vi='Cần điền bảng tính lương và giữ lại tiền lương.'");
...
Procedure ExecuteLongOperationAtServer()
	...
	Try
		...
		//::: Individual record
		EventLogMonitorEvent = NStr("en='New employee ind. entry';ru='Запись физ. лица нового сотрудника';vi='Bản ghi cá nhân của người lao động mới'");
		If AssociateWithIndividual AND FillVariant = 1 Then
		...
		//::: Employee record
		EventLogMonitorEvent = NStr("en='New employee record';ru='Запись нового сотрудника';vi='Ghi lại người lao động mới'");
		...
		If AcceptForEmploymentContract Then
			...
			EventLogMonitorEvent = NStr("en='Add rates to staff list';ru='Добавление ставок в штатное расписание';vi='Thêm bậc lương vào danh sách biên chế'");
			...
			EventLogMonitorEvent = NStr("en='New employee hiring entry';ru='Запись приема на работу нового сотрудника';vi='Bản ghi tiếp nhận công việc của người lao động mới'");
...
Procedure RunControlStaffSchedule(DataStructure, Errors, Cancel)
	...
	ElsIf Not SelectionNumberOfRates.Next() Then
		...
		MessageText = NStr("en='Rates for position %3 are not available in the staff list of company %2 by business unit %2.';ru='В штатном расписании организации %1 по структурной единице %2 не предусмотрены ставки для должности %3!';vi='Trong quản lý biên chế doanh nghiệp %1 theo đơn vị cấu trúc %2 không xem xét thuế suất đối với chức vụ %3!'");
...
"Свободно ставок %4, а требуется ставок %5.';vi='Trong quản lý biên chế doanh nghiệp %1 theo đơn vị cấu trúc %2 không có đủ số lượng vị trí làm việc còn trống đối với chức vụ %3!"
"Vị trí %4 còn trống, còn cần vị trí %5.'");
...
Procedure CheckInd(Errors)
	...
	If Object.OccupationType = Enums.OccupationTypes.MainWorkplace
		...
		If Selection.Next() Then
			...
			MessageText = NStr("en='Individual to whom the current employee was referred already has an employee with primary employment.';ru='Физическое лицо, к которому отнесли текущего сотрудника, уже имеет сотрудника с основным местом работы.';vi='Cá nhân mà có liên quan đến người lao động hiện tại, đã có người lao động với chỗ làm việc chính.'");
			CommonUseClientServer.AddUserError(Errors, , MessageText, Undefined);
