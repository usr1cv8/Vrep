...
Procedure BeforeWrite(Cancel)
		...
		If Not PrivilegedMode() AND DigitallySignedObjectRecord <> True AND Not DeletionMarkIsMarked Then
			...
			If ValueIsFilled(Ref) Then
				...
				If DigitallySigned AND RefDigitallySigned Then
					Raise NStr("en='Cannot edit the digitally signed version.';ru='Подписанную версию нельзя редактировать.';vi='Không thể soạn phiên bản đã đặt mua.'");
				EndIf;
	...
	If Owner.CurrentVersion = Ref Then
		If DeletionMark = True AND Owner.DeletionMark <> True Then
			Raise NStr("en='Active version cannot be deleted.';ru='Активную версию нельзя удалить.';vi='Không thể xóa bỏ phiên bản hiện thời.'");
		EndIf;
	ElsIf ParentalVersion.IsEmpty() Then
		If DeletionMark = True AND Owner.DeletionMark <> True Then
			Raise NStr("en='The first version cannot be deleted.';ru='Первую версию нельзя удалить.';vi='Không thể xóa bỏ phiên bản đầu tiên.'");
		EndIf;
