...
Procedure ActivateExecute()
	...
	Else
		ShowMessageBox(, NStr("en='It is allowed to change the active version only for the files that are not locked.';ru='Смена активной версии разрешена только для не занятых файлов.';vi='Thay thế phiên bản hiện thời chỉ cho phép đối với tệp chưa bị chiếm.'"));
	EndIf;
...
Procedure Compare(Command)
	...
	#If WebClient Or MobileClient Then
		ShowMessageBox(, NStr("en='Version comparison is not supported in web client.';ru='Сравнение версий не поддерживается в веб-клиенте.';vi='Việc so sánh các phiên bản không được hỗ trợ trên Web-client.'"));
		Return;
	#EndIf
...
"   HTML (.html .htm) Текстовый документ OpenDocument (.odt)';vi='Chỉ hỗ trợ so sánh các phiên bản"
"   đối với tệp"
"   có kiểu sau: Văn bản dạng bảng (.txt)"
"   Văn bản có định dạng RTF (.rtf) Văn bản Microsoft"
"   Word (.doc, .docx) Văn bản"
"   HTML (.html .htm) Văn bản OpenDocument (.odt)'");
		...
		If StandardSubsystemsClientReUse.ClientWorkParameters().ThisIsBasicConfigurationVersion Then
			CommonUseClientServer.MessageToUser(
				NStr("en='This operation is not supported in base version.';ru='Данная операция не поддерживается в базовой версии.';vi='Giao dịch này không được hỗ trợ trong phiên bản căn bản.'"));
			Return;
		EndIf;
...
Procedure MarkToDeleteUnmark()
	...
	If CurrentData.DeletionMark Then 
		QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Clear mark for deletion for ""%1""?';ru='Снять с ""%1"" пометку на удаление?';vi='Bỏ dấu xóa ""%1""?'"),
			String(CurrentData.Ref));
	Else
		QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Mark ""%1"" for deletion?';ru='Пометить ""%1"" на удаление?';vi='Đặt dấu xóa ""%1""?'"),
			String(CurrentData.Ref));
	EndIf;
...
Procedure VersionsComparisonAutomatic(Result, ExecuteParameters) Export
	...
	If ExecuteParameters.CurrentStep = 2 Then
		...
		StatusText = NStr("en='Comparing versions of file ""%1""...';ru='Выполняется сравнение версий файла ""%1""...';vi='Đang thực hiện so sánh phiên bản của tệp ""%1""...'");
