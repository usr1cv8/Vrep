	   ...
	   AND IBUserRoles.Count() = 0 Then
		...
		If Not WriteParameters.Property("WithEmptyListOfRoles") Then
			...
			ShowQueryBox(
				...
				NStr("en='Infobase user was not assign any role. Continue?';ru='Пользователю информационной базы не установлено ни одной роли. Продолжить?';vi='Người sử dụng cơ sở thông tin chưa thiết lập bất kỳ vai trò nào. Tiếp tục?'"),
				QuestionDialogMode.YesNo,
				,
				,
				NStr("en='Infobase user record';ru='Запись пользователя информационной базы';vi='Ghi lại người sử dụng cơ sở thông tin'"));
			Return;
		EndIf;
	EndIf;
...
"программу, требуется снять пометку на удаление с этого внешнего пользователя.';vi='Để cho phép đăng nhập"
"vào chương trình, cần bỏ dấu xóa khỏi người sử dụng ngoài này.'"));
		Return;
	EndIf;
	...
	If Not AccessLevel.FullRights
	   ...
	   AND Not CanLogOnToApplication Then
		...
		ShowMessageBox(,
			NStr("en='After saving, only administrator can allow to log on.';ru='После записи вход в программу сможет разрешить только администратор.';vi='Sau khi ghi lại thì chỉ người quản trị được phép đăng nhập vào chương trình.'"));
	EndIf;
...
Procedure DefineUserInconsistenciesWithUserIB(WriteParameters = Undefined)
	...
	ElsIf AccessLevel.ListManagement Then
		...
		If IBUserOSAuthentication <> False Then
			PropertiesAdjustment.Add(NStr("en='OS authentication (enabled)';ru='Аутентификация ОС (включена)';vi='Đăng nhập hệ điều hành (đã bật)'"));
		EndIf;
		...
		If IBUserFullName <> Object.Description Then
			...
			RefineFullName = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Full name ""%1""';ru='Полное имя ""%1""';vi='Tên đầy đủ ""%1""'"), IBUserFullName);
		...
		If IBUserOSUser <> "" Then
			PropertiesAdjustment.Add(NStr("en='OS user (specified)';ru='Пользователь ОС (указан)';vi='Người sử dụng hệ điều hành (đã chỉ ra)'"));
		EndIf;
		
		If IBUserShowInList Then
			FoundDifferencesCanBeResolvedWithoutAdministrator = True;
			PropertiesAdjustment.Add(NStr("en='Show in the selection list (enabled)';ru='Показывать в списке выбора (включено)';vi='Hiển thị trong danh sách chọn (bật)'"));
		EndIf;
		
		If IBUserRunMode <> "Auto" Then
			FoundDifferencesCanBeResolvedWithoutAdministrator = True;
			PropertiesAdjustment.Add(NStr("en='Start mode (not Auto)';ru='Режим запуска (не Авто)';vi='Chế độ khởi động (không Tự động)'"));
		EndIf;
...
"от указанных в этой форме: %1.';vi='Các thuộc tính sau của người sử dụng cơ sở thông tin khác"
"với những thuộc tính đã chỉ ra trong biểu mẫu này: %1.'"), StringPropertyClarification)
				+ Chars.LF
				+ ?(ShowCommandsDifferences Or FoundDifferencesCanBeResolvedWithoutAdministrator,
					NStr("en='Click ""Write"" to resolve the differences and not to show this message again.';ru='Нажмите ""Записать"", чтобы устранить различия и не выводить это предупреждение.';vi='Hãy nhấn ""Ghi lại"" để loại bỏ sự khác biệt và không hiển thị cảnh báo này.'"),
					NStr("en='Contact your administrator to resolve the differences.';ru='Обратитесь к администратору, чтобы устранить различия.';vi='Hãy liên hệ với người quản trị để khắc phục sự khác biệt.'"));
		Else
			ShowMismatch = False;
		EndIf;
	Else
