...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.ChoiceMode Then
		...
		If Parameters.CloseOnChoice = False Then
			// Choice mode.
			Title = NStr("en='Select user groups';ru='Подбор групп пользователей';vi='Chọn nhóm người sử dụng '");
			Items.List.Multiselect = True;
			Items.List.SelectionMode = TableSelectionMode.MultiRow;
		Else
			Title = NStr("en='Select user group';ru='Выбор группы пользователей';vi='Chọn nhóm người sử dụng '");
		EndIf;
		AutoTitle = False;
