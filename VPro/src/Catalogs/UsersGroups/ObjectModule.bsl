...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Parent = Catalogs.UsersGroups.AllUsers Then
		CommonUseClientServer.AddUserError(Errors,
			"Object.Parent",
			NStr("en='Predefined group ""All users"" cannot be a parent group.';ru='Предопределенная группа ""Все пользователи"" не может быть родителем.';vi='Nhóm định trước ""Tất cả người sử dụng"" không thể là lớp trên.'"),
			"");
	EndIf;
	...
	For Each CurrentRow IN Content Do;
		...
		If Not ValueIsFilled(CurrentRow.User) Then
			CommonUseClientServer.AddUserError(Errors,
				...
				NStr("en='User is not selected.';ru='Пользователь не выбран.';vi='Người sử dụng chưa chọn.'"),
				"Object.Content",
				LineNumber,
				NStr("en='User in line %1 is not selected.';ru='Пользователь в строке %1 не выбран.';vi='Người sử dụng trong dòng %1 chưa chọn.'"));
			Continue;
		EndIf;
		...
		If FoundValues.Count() > 1 Then
			CommonUseClientServer.AddUserError(Errors,
				"Object.Content[%1].User",
				NStr("en='User is repeated.';ru='Пользователь повторяется.';vi='Người sử dụng lặp lại.'"),
				"Object.Content",
				LineNumber,
				NStr("en='User in line %1 is repeated.';ru='Пользователь в строке %1 повторяется.';vi='Người sử dụng trong dòng %1 lặp lại.'"));
		EndIf;
	EndDo;
...
"группа ""Все пользователи"" может быть только в корне.';vi='Nhóm"
"định trước ""Tất cả người sử dụng"" chỉ có thể nằm tại gốc.'");
		EndIf;
		If Content.Count() > 0 Then
			Raise
				NStr("en='Adding users to"
"the folder ""All users"" is not supported.';ru='Добавление пользователей в группу"
"""Все пользователи"" не поддерживается.';vi='Thêm người sử dụng vào nhóm"
"""Tất cả người sử dụng"" không được hỗ trợ.'");
		EndIf;
	Else
		If Parent = Catalogs.UsersGroups.AllUsers Then
			Raise
