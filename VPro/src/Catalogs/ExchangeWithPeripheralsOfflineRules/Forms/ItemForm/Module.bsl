...
Procedure BeforeWriteAtServer(Cancel, CurrentObject, WriteParameters)
	...
	If ProductQuantity > 0 Then
		If Products[ProductQuantity-1].Code > CurrentObject.MaximumCode AND CurrentObject.MaximumCode <> 0 Then
			...
			NStr("en='In the Goods tabular section, rows with a code that exceeds the maximum allowed value are detected: %1. Edit a maximum code or reduce the number of goods for export using filter.';ru='В табличной части ""Товары"" найдены строки с кодом, превышающим максимальное допустимое значение: %1. Измените максимальный код или уменьшите количество товаров к выгрузке при помощи отбора.';vi='Trong phần bảng ""Hàng hóa"" tìm thấy dòng với mã vượt quá giá trị tối đa cho phép: %1. Hãy thay đổi mã tối đa hoặc giảm số lượng hàng hóa kết xuất bằng cách lọc.'"), CurrentObject.MaximumCode),,"Products",,Cancel);
			Return;
		EndIf;
	EndIf;
...
Procedure MaximumCodeOnChange(Item)
	...
	Status(NStr("en='Updating the goods list...';ru='Выполняется обновление списка товаров...';vi='Đang cập nhật danh sách hàng hóa...'"));
	UpdateListOfProductsOnServer();
...
Procedure ProductsBeforeEndOfEditing(Item, NewRow, CancelEdit, Cancel)
	...
	If Not CancelEdit Then
		...
		If FoundString <> Undefined Then
			...
			ErrorDescription = NStr("en='Such code is already assigned for products and services %ProductsAndServices%';ru='Такой код уже назначен для номенклатуры %Номенклатура%';vi='Mã này đã được dùng cho mặt hàng %ProductsAndServices%'");
			ErrorDescription = StrReplace(ErrorDescription, "%ProductsAndServices%", """" + FoundString.ProductsAndServices + """"
			+ ?(ValueIsFilled(FoundString.Characteristic), " " + NStr("en='with characteristic';ru='с характеристикой';vi='với đặc tính'") + " """ + FoundString.Characteristic + """", "")
			+ ?(ValueIsFilled(FoundString.Batch), " " + NStr("en='with the batch';ru='с партией';vi='với lô'") + " """ + FoundString.Batch + """", "")
			+ ?(ValueIsFilled(FoundString.MeasurementUnit), " " + NStr("en='with unit';ru='с единицей';vi='với đơn vị'") + " """ + FoundString.MeasurementUnit + """", ""))+NStr("en='. Places are exchanged.';ru='. Выполнен обмен местами.';vi='. Đã thực hiện trao đổi vị trí.'");
...
Procedure ProductsBeforeDeleting(Item, Cancel)
	...
	For Each SelectedRow IN Items.Products.SelectedRows Do
		...
		If ValueIsFilled(CurrentData.ProductsAndServices) AND CurrentData.Used Then
			ShowMessageBox(, (NStr("en='After deletion, new goods codes will be assigned to products and services that meet the specified filter.';ru='Номенклатуре, соответствующей заданному отбору, после удаления будут заново назначены коды товаров.';vi='Mặt hàng, tương ứng phễu lọc đưa ra, sau khi xóa sẽ được cấp lại mã hàng.'")));
			Break;
		EndIf;
	EndDo;
...
Procedure DeleteFreeGoodsCodesInListEnd(Command)
	...
	If Products.Count() > 0 Then
		...
		Else
			ShowMessageBox(Undefined,NStr("en='No data to delete';ru='Нет данных для удаления';vi='Không có dữ liệu để xóa'"));
		EndIf;
	Else
		ShowMessageBox(Undefined,NStr("en='No data to delete';ru='Нет данных для удаления';vi='Không có dữ liệu để xóa'"));
	EndIf;
...
Procedure ImportFilterSettingsByDefault()
	...
	Else
		Raise NStr("en='Incorrect peripherals type';ru='Некорректный тип подключаемого оборудования';vi='Sai kiểu thiết bị bán hàng'");
	EndIf;
