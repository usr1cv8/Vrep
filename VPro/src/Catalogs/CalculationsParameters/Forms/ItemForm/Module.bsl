...
Function QueryCorrect()
	...
	Try
        ...
        If QueryBuilder.Dimensions.Count() > 0 Then
			...
			Message.Text = NStr("en='Query should not contain totals.';ru='Запрос не должен содержать итоги!';vi='Truy vấn không được có phần tổng kết!'");
			Message.Message();			
			Return False;
		EndIf; 
		If QueryBuilder.SelectedFields.Count() > 1 Then
		    Message = New UserMessage();
			Message.Text = NStr("en='The request cannot contain more than one indicator.';ru='Запрос должен содержать не более одного показателя!';vi='Truy vấn cần có không nhiều hơn một chỉ tiêu!'");
			Message.Message();			
			Return False;
		EndIf;
		Return True;
...
Function CheckForIDDuplication(Cancel)
	...
	If Cancel Then
	  	...
	  	Message = New UserMessage();
		Message.Text = NStr("en='The calculation parameter with the same ID already exists.';ru='Параметр расчета с таким идентификатором уже существует!';vi='Tham số tính toán với tên tham số như vậy đã tồn tại!'");
		Message.Message();
...
Function CheckForIndicatorChoice()
	...
	Message = New UserMessage();
	Message.Text = NStr("en='Indicator is not selected.';ru='Не выбран показатель!';vi='Chưa chọn chỉ tiêu!'");
	Message.Message();
...
Procedure SetFilter()
	...
	TypeOfComparison = New Map;
	TypeOfComparison.Insert("Greater than", 			DataCompositionComparisonType.Greater);
	TypeOfComparison.Insert("Greater than or equal to",	DataCompositionComparisonType.GreaterOrEqual);
	TypeOfComparison.Insert("In group",			DataCompositionComparisonType.InHierarchy);
	TypeOfComparison.Insert("In list",			DataCompositionComparisonType.InList);
	TypeOfComparison.Insert("In list group",DataCompositionComparisonType.InListByHierarchy);
	TypeOfComparison.Insert("Filled",			DataCompositionComparisonType.Filled);
	TypeOfComparison.Insert("Less than",			DataCompositionComparisonType.Less);
	TypeOfComparison.Insert("Less than or equal to",	DataCompositionComparisonType.LessOrEqual);
	TypeOfComparison.Insert("Begins with",		DataCompositionComparisonType.BeginsWith);
	TypeOfComparison.Insert("Not in group",		DataCompositionComparisonType.NotInHierarchy);
	TypeOfComparison.Insert("Not in list",		DataCompositionComparisonType.NotInList);
	TypeOfComparison.Insert("Not in list group",DataCompositionComparisonType.NotInListByHierarchy);
	TypeOfComparison.Insert("Empty",		DataCompositionComparisonType.NotFilled);
	TypeOfComparison.Insert("Does not begin with",	DataCompositionComparisonType.NotBeginsWith);
	TypeOfComparison.Insert("Not like",		DataCompositionComparisonType.NotLike);
	TypeOfComparison.Insert("Not equal",			DataCompositionComparisonType.NotEqual);
	TypeOfComparison.Insert("Does not contain",		DataCompositionComparisonType.NotContains);
	TypeOfComparison.Insert("Like",			DataCompositionComparisonType.Like);
	TypeOfComparison.Insert("Equal to",				DataCompositionComparisonType.Equal);
	TypeOfComparison.Insert("Contains",			DataCompositionComparisonType.Contains);
	...
	If Not Object.CustomQuery Then
		...
		For Each ParameterString IN Object.QueryParameters Do
			...
			If ParameterString.Name <> "PointInTime"
				...
				Else
					SavedComparisonType = TypeOfComparison[ParameterString.ComparisonType];
					FilterItem.ComparisonType =  ?(SavedComparisonType = Undefined, DataCompositionComparisonType.Equal, SavedComparisonType);
				EndIf; 
	...
	If Object.CustomQuery Then
		...
		If EditQuery Then
			If Not QueryCorrect() Then
				...
				ShowQueryBox(New NotifyDescription("EditQueryEnd", ThisObject), NStr("en='Query contains error. Clear query and return to original one?';ru='Запрос содержит ошибку! Очистить запрос и вернуться к исходному?';vi='Truy vấn có lỗi! Xóa truy vấn và quay lại truy vấn nguồn?'"), QuestionDialogMode.YesNo, 0);
                Return;
			Else
...
Procedure SpecifyValueAtPayrollCalculationOnChange(Item)
	...
	If Object.SpecifyValueAtPayrollCalculation Then
		...
		MessageText = NStr("en='After the check box is selected, all attributes will be cleared. Continue?';ru='После установки флага все реквизиты будут очищены! Продолжить?';vi='Sau khi đặt dấu hộp kiểm tất cả mục tin sẽ bị xóa bỏ! Tiếp tục?'");
...
Procedure CreateFixedAmount(Command)
	...
	ShowQueryBox(Notification,NStr("en='Calculation parameter will be completely repopulated. Continue?';ru='Параметр расчета будет полностью перезаполнен! Продолжить?';vi='Sẽ điền lại toàn bộ tham số tính toán! Tiếp tục?'"), QuestionDialogMode.YesNo, 0);
