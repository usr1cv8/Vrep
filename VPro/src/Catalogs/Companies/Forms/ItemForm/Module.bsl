...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(Object.Ref) And Not GetFunctionalOption("MultipleCompaniesAccounting") Then
		ErrorText = NStr("en='It is forbidden to create a new company"
"with the off parameter setting accounting ""Use several companies""';ru='Запрещено создавать новую организацию"
"при выключенной настройке параметра учета ""Учет по нескольким организациям"".';vi='Đã cấm tạo mới doanh nghiệp"
"khi tắt tùy chỉnh tham số kế toán ""Kế toán theo nhiều doanh nghiệp"".'");
		Raise ErrorText;
	EndIf;
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Object.LegalEntityIndividual = Enums.CounterpartyKinds.Individual Then
		If Not ValueIsFilled(IndividualFullName.Surname) 
			...
			MessageText = NStr("en='Is not filled full name';ru='Не заполнено ФИО';vi='Chưa điền Họ tên'");
...
Procedure PrefixOnChange(Item)
	...
	If StrFind(Object.Prefix, "-") > 0 Then
		...
		ShowMessageBox(Undefined, NStr("en='It is impossible to use the symbol ""-"" in the prefix of company';ru='Нельзя в префиксе организации использовать символ ""-"".';vi='Trong tiền tố doanh nghiệp không nên sử dụng ký tự ""-"".'"));
...
Procedure AddAdditionalAttribute(Command)
	...
	FormParameters.Insert("CurrentSetOfProperties", PredefinedValue("Catalog.AdditionalAttributesAndInformationSets.Catalog_Companies"));
...
Function LockIndividualOnEdit()
	...
	If Not Parameters.Key.IsEmpty() And Not IndividualLocked Then
		If Not LockIndividualOnEditAtServer() Then
			ShowMessageBox(, NStr("en='You can not make changes to the personal data of an individual. Perhaps the data is edited by another user.';ru='Не удается внести изменения в личные данные физического лица. Возможно данные редактируются другим пользователем.';vi='Không thể đưa vào các thay đổi trong dữ liệu cá nhân của cá nhân. Có thể là dữ liệu đang được soạn bởi người sử dụng khác.'"));
			ReadIndividual(Object.Individual);
			Return False;
		Else
