...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If ItIsAuthentication Then
		Items.FormDrillDown.Title = NStr("en='OK';ru='Ок';vi='Ок'");
		Items.ExplanationEnhancedPassword.Title = NStr("en='Click OK to enter the password.';ru='Нажмите ОК, чтобы перейти к вводу пароля.';vi='Hãy nhấn Đồng ý để chuyển đến nhập mật khẩu.'");
	EndIf;
...
"из полного списка или откройте сертификат и укажите программу вручную.';vi='Trong chứng thư đã chọn chưa chỉ ra chương trình đối với khóa đóng."
"Hãy chọn lại chứng thư"
"từ danh sách đầy đủ hoặc mở ra chứng thư và chỉ ra chương trình một cách thủ công.'"));
...
Procedure WriteEncryptionCertificatesAtServer(ObjectsDescription, FormID, Error)
	...
	Except
		ErrorInfo = ErrorInfo();
		RollbackTransaction();
		Error.Insert("ErrorDescription", NStr("en='An error occurred during the encryption certificate cleanup:';ru='При очистке сертификатов шифрования возникла ошибка:';vi='Khi xóa chứng thư mã hóa đã xảy ra lỗi:'")
...
Procedure ShowError(ErrorOnClient, ErrorOnServer)
	...
	DigitalSignatureServiceClient.ShowRequestToApplicationError(
		NStr("en='Cannot decrypt data';ru='Не удалось расшифровать данные';vi='Không thể giải mã dữ liệu'"), "",
