...
Procedure Next(Command)
	...
	If Items.Certificates.CurrentData = Undefined Then
		ShowMessageBox(, NStr("en='Select certificates that you want to add.';ru='Выделите сертификаты, которые требуется добавить.';vi='Hãy chọn chứng thư cần thêm vào.'"));
		Return;
	EndIf;
...
Procedure NextAfterCertificateSearch(Result, NotSpecified) Export
	...
	If Result.Property("CertificateNotFound") Then
		Context.Insert("ErrorDescription", NStr("en='Certificate is not found on the computer (it might have been deleted).';ru='Сертификат не найден на компьютере (возможно удален).';vi='Không tìm thấy chứng thư trong máy tính (có thể là chứng thư đã bị xóa).'"));
	Else
...
Procedure ShowPageCertificatePropertiesAdjustment(Form, CryptoCertificate, CertificateData)
	...
	String = ?(ValueIsFilled(Form.Certificate), NStr("en='Update calendar';ru='Обновить календарь';vi='Cập nhật lịch'"), NStr("en='Add';ru='Добавить';vi='Thêm'"));
