...
Procedure LegalAddressStartChoice(Item, ChoiceData, StandardProcessing)
	...
	AddressPresentationSelectionStart(ThisObject, Item, ChoiceData, StandardProcessing,
		"LegalAddress", NStr("en='Counterparty legal address';ru='Юридический адрес контрагента';vi='Địa chỉ đăng ký của đối tác'"));
...
Procedure ActualAddressStartChoice(Item, ChoiceData, StandardProcessing)
	...
	AddressPresentationSelectionStart(ThisObject, Item, ChoiceData, StandardProcessing,
		"ActualAddress", NStr("en='Physical address of the counterparty';ru='Фактический адрес контрагента';vi='Địa chỉ thực tế của đối tác'"));
...
Procedure PhoneStartChoice(Item, ChoiceData, StandardProcessing)
	...
	PhonePresentationSelectionStart(ThisObject, Item, ChoiceData, StandardProcessing,
		"Phone", NStr("en='Company phone';ru='Телефон организации';vi='Điện thoại của doanh nghiệp'"));
...
"Обратитесь к администратору.';vi='Chương trình %1 vẫn chưa được bổ sung vào danh sách chương trình sử dụng."
"Hãy liên hệ với người quản trị.'"),
					Presentation));
...
"Выполните добавление в главном узле информационной базы.';vi='Chương trình %1 vẫn chưa được bổ sung vào danh sách các chương trình sử dụng."
"Hãy thực hiện thêm vào nút chính của cơ sở thông tin.'"),
					Presentation));
		Else
			Buttons = New ValueList;
			Buttons.Add("Add",    NStr("en='Add';ru='Добавить';vi='Thêm'"));
			Buttons.Add("DoNotAdd", NStr("en='Do not add';ru='Не добавлять';vi='Không thêm'"));
			ShowQueryBox(
...
"Добавить?';vi='Chương trình %1 vẫn chưa được bổ sung vào danh sách các chương trình sử dụng."
"Thêm?'"),
					Presentation),
				Buttons,, "DoNotAdd");
		EndIf;
...
"дискету или другой носитель информации и используется в дальнейшем для создания электронных подписей.';vi='Khóa chữ ký điện tử -"
"đây là thông tin bảo mật được lưu trên máy tính, ổ đĩa USB,"
"đĩa mềm hoặc thiết bị lưu trữ thông tin khác và sử dụng sau này để tạo chữ ký điện tử.'"));
		
	ElsIf URL = "CertificateQuery" Then
		ShowMessageBox(,
			NStr("en='Certificate request is not the secret information. It is created based on the digital signature key, sent together with the certificate issue application and required to issue certificate.';ru='Запрос на сертификат - это не секретная информация, которая создается на основе ключа электронной подписи, отправляется вместе с заявлением на выпуск сертификата и требуется для выпуска сертификата.';vi='Yêu cầu chứng thư - đây không phải là thông tin bảo mật, mà tạo trên cơ sở khóa chữ ký điện tử, được gửi cùng với thông báo phát hành chứng thư và cần thiết để phát hành chứng thư.'"));
	EndIf;
...
"5. Копия страхового свидетельства обязательного пенсионного страхования (СНИЛС) владельца сертификата.';vi='Bộ"
"chứng từ: 1. Thông báo phát hành chứng thư (đã soạn ở bước trước đó)."
"2. Bản sao giấy chứng nhận đăng ký tại cơ quan thuế (MST)."
"3. Bản sao giấy chứng nhận đăng ký cá nhân kinh doanh (SĐK kinh doanh)."
"4. Bản sao chứng nhận sở hữu chứng thư."
"5. Bản sao chứng nhận bảo hiểm hưu trí bắt buộc của chủ sở hữu chứng thư.'");
	Else
		WarningText =
			NStr("en=""Set"
"of documents: 1. Application for certificate issue (prepared in the previous step)."
...
""
"* Копии документов заверяются подписью руководителя и печатью организации.';vi='Bộ"
"chứng từ: 1. Thông báo phát hành chứng thư (đã soạn ở bước trước đó)."
"2. Bản sao* chứng nhận đăng ký tại cơ quan thuế (MST)."
"3. Bản sao* chứng nhận đăng ký pháp nhân (MĐK)."
"4. Bản sao* văn bản ủy quyền người đại diện doanh nghiệp đã chỉ ra để phát hành chứng thư."
"5. Bản sao* chứng nhận bảo hiểm hưu trí bắt buộc của đại diện doanh nghiệp đã chỉ ra để phát hành chứng thư."
"6. Bản sao* văn bản xác nhận ủy quyền"
"   của người lãnh đạo đã ký thông báo (biên bản họp cổ đông, quyết định của chủ sở hữu, điều lệ)."
""
"* Bản sao các loại giấy tờ cần có chữ ký của người lãnh đạo và dấu của doanh nghiệp.'");
	EndIf;
...
Procedure ExportRootCertificate(Command)
	...
	DigitalSignatureServiceClient.SaveCertificate(, RootCertificateAddress,
		NStr("en='Root certificate of 1C RDC LLC';ru='Корневой сертификат ООО НПЦ 1С';vi='Chứng thư gốc 1C'"));
...
Procedure PrintDocuments(Command)
	...
	PrintedFormName = NStr("en='Application for certificate issue';ru='Заявление на выпуск сертификата';vi='Thông báo phát hành chứng thư'");
...
Procedure GoToPageAgreement()
	...
	Items.FormClose.Title = NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'");
...
Procedure GoToPageCertificateQueryPreparationOnServerOnImport()
	...
	Items.FormBack.Visible = True;
	Items.FormClose.Title = NStr("en='Close';ru='Закрыть';vi='Đóng'");
...
Procedure GoToPageStatementSending()
	...
	If Not ValueIsFilled(Object.Application) Then
		CommonUseClientServer.MessageToUser(StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='The ""Digital signature application"" field is not populated';ru='Поле ""Программа электронной подписи"" не заполнено';vi='Chưa điền trường ""Chương trình chữ ký điện tử""'")), , "Application");
...
Procedure GoToPageStatementSendingAfterCreatingCryptographyManager(Result, Context) Export
	...
	If TypeOf(Result) <> Type("CryptoManager") Then
		...
		DigitalSignatureServiceClient.ShowRequestToApplicationError(
			NStr("en='Create DS key';ru='Создание ключа электронной подписи';vi='Tạo khóa chữ ký điện tử'"),, Result, New Structure);
		Return;
	EndIf;
...
Procedure GoToPageStatementProcessingPending()
	...
	Buttons = New ValueList;
	Buttons.Add("Send",   NStr("en='Send';ru='Отправлять';vi='Gửi đi'"));
	Buttons.Add("DoNotSend", NStr("en='Do not send';ru='Не отправлять';vi='Không gửi'"));
...
"Заявление не будет принято пока не будут получены печатные документы.';vi='Chứng từ vẫn chưa được in."
"Giấy đề nghị sẽ không được tiếp nhận cho đến khi nhận được chứng từ in.'");
	EndIf;
...
"Заявление может обрабатываться дольше обычного.';vi='Chưa chỉ ra MST của doanh nghiệp dịch vụ."
"Giấy đề nghị có thể được xử lý lâu hơn bình thường.'");
		EndIf;
...
Function GoToPageStatementProcessingPendingOnServer(ErrorDescription)
	...
	RequestProcessingState = NStr("en='Application is accepted for processing.';ru='Заявление принято для обработки.';vi='Giấy đề nghị đã được tiếp nhận để xử lý.'");
...
Procedure UpdateUpdateDateStatesInTitle(Form)
	...
	Form.Items.RequestProcessingState.Title = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Request processing state (on %1)';ru='Состояние обработки заявления (на %1)';vi='Trạng thái xử lý giấy đề nghị (trên %1)'"),
...
Procedure GoToPageCertificateSetup()
	...
	If Result <> True Then
		AfterWrite(NStr("en='Application is rejected.';ru='Заявление отклонено.';vi='Giấy đề nghị đã bị từ chối.'"));
		Close();
		Return;
	EndIf;
...
Procedure GoToPageCertificateSetupAfterInstallation(Installed, NoSpecified) Export
	...
	If Installed Then
		...
		AfterWrite(NStr("en='Application is executed.';ru='Заявление исполнено.';vi='Giấy đề nghị đã được thực hiện.'"));
		Close();
	Else
		AfterWrite();
		WriteStatement();
...
Procedure UpdateCertificateInstallationDateInHeader(Form)
	...
	Form.Items.ErrorCertificateSetup.Title = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='An error occurred when installing certificate (to %1)';ru='Ошибка установки сертификата (на %1)';vi='Lỗi thiết lập chứng thư (trên %1)'"),
...
Procedure CloseAssistantAfterCertificateInstallationEnd(Installed, NoSpecified) Export
	...
	AfterWrite(NStr("en='Certificate is installed.';ru='Сертификат установлен.';vi='Đã thiết lập chứng thư.'"));
	Close();
...
Procedure OnChangingCompanyOnServer(Import = False)
	...
	If ThisIsIndividualEntrepreneur Then
		Items.OGRN.Title = NStr("en='OGRNIE';ru='ОГРНИП';vi='SĐK kinh doanh của CNKD'");
	Else
		Items.OGRN.Title = NStr("en='OGRN';ru='ОГРН';vi='Số đăng ký kinh doanh'");
	EndIf;
	...
	Try
		...
		If DetailedResult.Result <> "Correct" Then
			...
			If Not ValueIsFilled(Message) Then
				Message = NStr("en='Address is not filled in';ru='Адрес не заполнен';vi='Chưa điền địa chỉ'");
			EndIf;
		EndIf;
	Except
		ErrorInfo = ErrorInfo();
...
Function BankAccountMeetsRequirements(BankAccount, MessageText)
	...
	If Not StringFunctionsClientServer.OnlyNumbersInString(Value) Then
		MessageText = NStr("en='Account number can contain only digits.';ru='Расчетный счет должен состоять только из цифр.';vi='Tài khoản ngân hàng chỉ được là chữ số.'");
		Return False;
	EndIf;
	
	If StrLen(Value) <> 20 Then
		MessageText = NStr("en='Account number consists of 20 digits.';ru='Расчетный счет должен состоять из 20 цифр.';vi='Tài khoản hạch toán cần có 20 chữ số.'");
		Return False;
	EndIf;
	...
	If CommonUse.SubsystemExists("StandardSubsystems.WorkWithCounterparties") Then
		...
		If Not ModuleRegulatedDataClientServer.AccountKeyDigitMeetsRequirements(Value, BIN) Then
			MessageText = NStr("en='Check digit of account does not match the calculated value with branch ID taken into account';ru='Контрольное число счета не совпадает с рассчитанным с учетом БИК';vi='Số lượng tài khoản kiểm tra không trùng với số lượng BIK đã tính'");
			Return False;
		EndIf;
	EndIf;
...
Function BICMeetsRequirements(BIN, MessageText)
	...
	If Not StringFunctionsClientServer.OnlyNumbersInString(Value) Then
		MessageText = NStr("en='Branch ID must contain only digits.';ru='БИК должен состоять только из цифр.';vi='MNH chỉ gồm các chữ số.'");
		Return False;
	EndIf;
	
	If StrLen(Value) <> 9 Then
		MessageText = NStr("en='Branch ID must consist of 9 digits.';ru='БИК должен состоять из 9 цифр.';vi='MNH cần có 9 chữ số.'");
		Return False;
	EndIf;
...
Function CorrespondentBankAccountMeetsRequirements(CorrespondentAccount, MessageText)
	...
	If Not StringFunctionsClientServer.OnlyNumbersInString(Value) Then
		MessageText = NStr("en='Correspondent account should contain only digits.';ru='Корреспондентский счет должен состоять только из цифр.';vi='Tài khoản trung gian chỉ nên gồm chữ số.'");
		Return False;
	EndIf;

	If StrLen(Value) <> 20 Then
		MessageText = NStr("en='Correspondent account should contain  20 digits.';ru='Корреспондентский счет должен состоять из 20 цифр.';vi='Tài khoản trung gian cần có 20 chữ số.'");
		Return False;
	EndIf;
...
Function AddressMeetsRequirements(Val AddressMXL, MessageText)
	...
	If Not AddressStructure.Property("StateCode") Then
		MessageText = MessageText + NStr("en='This is not Russian address';ru='Это не российский адрес';vi='Đây không phải là địa chỉ Việt Nam'");
		Return False;
	EndIf;
	
	// Check if a state was specified.
	If Not AddressStructure.Property("Region") Or Not ValueIsFilled(AddressStructure.Region) Then
		MessageText = MessageText + NStr("en='Region is not specified';ru='Не указан регион';vi='Chưa chỉ ra khu vực'");
		Return False;
	EndIf;
	
	// Check if the state was specified correctly - state code is defined.
	If Not ValueIsFilled(AddressStructure.StateCode) Then
		MessageText = MessageText + NStr("en='Incorrect region (region code is not defined)';ru='Некорректный регион (код региона не определен)';vi='Sai khu vực (chưa xác định mã khu vực)'");
		Return False; 
	EndIf;
	...
	If Not ValueIsFilled(SettlementEntirely) Then
		MessageText = MessageText + NStr("en='Specify the settlement';ru='Не указан населенный пункт';vi='Chưa chỉ ra điểm dân cư'");
		Return False;
	EndIf;
...
Function PhoneMeetsRequirements(Val PhoneXML, MessageText)
	...
	If StrReplace(PhoneStructure.CountryCode, "+", "") <> "7" Then
		MessageText = MessageText + NStr("en='Country code is not Russian (should be ""7"")';ru='Код страны не российский (должен быть ""7"")';vi='Mã nước không phải là Việt Nam (cần là ""84"")'");
		Return False;
	EndIf;
	...
	If Not ValueIsFilled(PhoneNumberWithoutCountryCode) Then
		MessageText = MessageText + NStr("en='Phone number is not entered';ru='Не заполнен номер телефона';vi='Chưa điền số điện thoại'");
		Return False;
	EndIf;
	
	If StrLen(OnlyDigits(PhoneNumberWithoutCountryCode)) <> 10 Then
		MessageText = MessageText + NStr("en='Phone number with the city code should contain 10 digits';ru='Номер телефона с кодом города должен состоять из 10-и цифр';vi='Số điện thoại bao gồm mã thành phố cần có 10 chữ số'");
		Return False;
	EndIf;
	...
	Else
		...
		Else
			If Attributes.Chiefexecutive <> Undefined Then
				List.Add("Chiefexecutive", NStr("en='Chiefexecutive';ru='Директор';vi='Giám đốc'"));
			...
			If Attributes.ChiefAccountant <> Undefined Then
				List.Add("ChiefAccountant", NStr("en='Chief accountant';ru='Главный бухгалтер';vi='Kế toán trưởng'"));
		...
		If List.Count() = 0 Then
			...
			If ThisIsIndividualEntrepreneur Then
				Items.Employee.Title = NStr("en='Individual entrepreneur';ru='Индивидуальный предприниматель';vi='Cá nhân kinh doanh'");
			Else
				Items.Employee.Title = NStr("en='Employee';ru='Сотрудник';vi='Người lao động'");
			EndIf;
		Else
			List.Add("Employee", NStr("en='Employee';ru='Сотрудник';vi='Người lao động'"));
...
Procedure OnChangingDocumentTypeOnServer()
	...
	If DocumentKind1 = "21" Then
		Items.DocumentNumber.Title = NStr("en='Series and number';ru='Серия и номер';vi='Ký hiệu và số'");
		Items.DocumentNumber.Mask = "99 99 999999";
	Else
		Items.DocumentNumber.Title = NStr("en='Number';ru='Number';vi='Số'");
...
Function RFPassportNumberMeetsRequirements(Val RFPassportNumber, MessageText)
	...
	If IsBlankString(RowOfDigits) Then
		MessageText = MessageText + NStr("en='Passport number is not specified';ru='Номер паспорта не заполнен';vi='Chưa điền số giấy phép'");
		Return False;
	EndIf;
	
	If StrLen(RowOfDigits) < 10 Then
		MessageText  =  MessageText + NStr("en='Incorrect passport number';ru='Номер паспорта задан неполностью';vi='Đã đưa ra số giấy phép không đầy đủ'");
		Return False;
	EndIf;
	
	If Not StringFunctionsClientServer.OnlyNumbersInString(RowOfDigits) Then
		Result = False;
		MessageText = MessageText + NStr("en='Passport number must contain only digits.';ru='Номер паспорта должен состоять только из цифр.';vi='Số giấy phép chỉ gồm các chữ số.'");
	EndIf;
...
Procedure ValueSelectionStart(Item, ChoiceData, StandardProcessing)
	...
	Else
		...
		TypeChoiceList.ShowChooseItem(
			...
			NStr("en='Select data type';ru='Выбор типа данных';vi='Chọn kiểu dữ liệu'"),
...
Function ValidateAttribute(Cancel, AttributeName)
	...
	If Not ValueIsFilled(ThisObject[AttributeName]) Then
		CommonUseClientServer.MessageToUser(StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='The ""%1"" field is not filled in';ru='Поле ""%1"" не заполнено';vi='Chưa điền trường ""%1""'"), TitleFields), , AttributeName, , Cancel);
...
Function PreparedDocument()
	...
	Template.Parameters.IdentityCard = DocumentViewPresentation + " " + DocumentNumberPresentation + " "
		+ NStr("en='from';ru='from';vi='từ'") + " " + DocumentIssueDatePresentation + " "
		+ NStr("en='issued';ru='выданный';vi='được cấp'") + " " + DocumentWhoIssued;
...
"заявление по причине: %1';vi='Không thể gửi"
"thông báo do nguyên nhân: %1'");
	...
	If Not ValueIsFilled(Response) Then
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(ErrorDescriptionTemplate,
			NStr("en='Server returned an empty response.';ru='Сервер вернул пустой ответ.';vi='Server trả về câu trả lời rỗng.'"));
		Return False;
	EndIf;
...
""
"After concluding the contract please send the statement once again.';ru='Заявление не принято по причине: Пользователь %1 не имеет действующего договора на ИТС (its.1c.ru). Для заключения договора обратитесь в обслуживающую организацию.  После заключения договора отправьте заявление еще раз.';vi='Chưa tiếp nhận thông báo do nguyên nhân: Người sử dụng %1 không có hơp đồng ITS còn hiệu lực (its.1c.ru). Để ký hợp đồng, hãy liên hệ với doanh nghiệp cung cấp dịch vụ. Sau khi ký hợp đồng, hãy gửi lại thông báo một lần nữa.'"),
			AuthenticationParametersOnSite.Login);
	Else
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(ErrorDescriptionTemplate,
			ErrorDescription);
...
Function TicketToSupportSite(ErrorDescription)
	...
	Except
		...
		If Find(BriefErrorDescription, "IncorrectLoginOrPasswordExceptionApi") > 0 Then
			BriefErrorDescription = NStr("en='Incorrect user name or password.';ru='Некорректное имя пользователя или пароль.';vi='Sai tên người sử dụng hoặc mật khẩu.'");
			AuthenticationParametersOnSite = Undefined;
		EndIf;
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Failed to connect to support website"
"due to:% 1';ru='Не удалось подключиться к"
"сайту поддержки по причине: %1';vi='Không thể kết nối"
"đến Website hỗ trợ do nguyên nhân: %1'"),
...
"Удалите его и создайте новое.';vi='Không thể đọc truy vấn đã tạo trước đó tới chứng thư."
"Giấy đề nghị không thể gửi đi được."
"Hãy xóa đi và tạo giấy đề nghị mới.'");
...
Function GetCertificate()
	...
	ErrorDescriptionTemplate =
		NStr("en='Cannot update the state due to: %1';ru='Не удалось обновить состояние по причине: %1';vi='Không thể cập nhật trạng thái theo nguyên nhân: %1'");
	...
	If Not ValueIsFilled(Response) Then
		RequestProcessingState = StringFunctionsClientServer.SubstituteParametersInString(
			ErrorDescriptionTemplate, NStr("en='Server returned an empty response.';ru='Сервер вернул пустой ответ.';vi='Server trả về câu trả lời rỗng.'"));
		Return Undefined;
	EndIf;
	...
	If ResultCode = "0" Then
		...
		Else
			ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(ErrorDescriptionTemplate,
				NStr("en='Server returned empty data.';ru='Сервер вернул пустые данные.';vi='Server trả về dữ liệu rỗng.'"));
		EndIf;
		
	ElsIf ResultCode = "1" Then
		ErrorDescription = NStr("en='The application has not been processed yet, try again later.';ru='Заявление еще не обработано, попробуйте позже.';vi='Giấy đề nghị vẫn chưa được xử lý, hãy thử lại sau.'");
	Else
		DOMNode = DOMBuilder.GetElementByTagName("errorMessage");
		If DOMNode.Count() > 0 Then
			ErrorDescription = DOMNode[0].TextContent;
		Else
			ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Server returned error code: %1';ru='Сервер вернул код ошибки: %1';vi='Server trả về mã lỗi: %1'"), String(ResultCode));
		EndIf;
	EndIf;
	...
	Try
		...
		If Not FileFound Then
			Raise NStr("en='Invalid data format.';ru='Неверный формат данных.';vi='Sai định dạng dữ liệu.'");
		EndIf;
	Except
		ErrorInfo = ErrorInfo();
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
...
"ответ сервера по причине: %1';vi='Không thể tách ra"
"phản hồi của Server do nguyên nhân: %1'"),
			BriefErrorDescription(ErrorInfo));
	EndTry;
...
Procedure SetCertificateAfterCreatingCryptographyManager(Manager, Context) Export
	...
	If TypeOf(Manager) <> Type("CryptoManager") Then
		DigitalSignatureServiceClient.ShowRequestToApplicationError(
			NStr("en='Install the certificate on this computer';ru='Установка сертификата на компьютер';vi='Thiết lập chứng thư trên máy tính'"),, Manager, New Structure);
...
"на компьютер требуется установить расширение для веб-клиента 1С:Предприятия.';vi='Để cài đặt chứng thư"
"trên máy tính, cần cài đặt phần mở rộng đối với Web-client 1C:DOANH NGHIỆP.'"),
			NStr("en='To install a certificate on"
"your computer, it is required to install additional external component.';ru='Для установки сертификата"
"на компьютер требуется установить дополнительную внешнюю компоненту.';vi='Để cài đặt chứng thư"
"trên máy tính, cần cài đặt cấu phần ngoài bổ sung.'"));
	EndIf;
...
Procedure SetCertificateWhenCryptographyObjectCreationErrorAppearsAfterConnectingFilesExtension(Attached, Context) Export
	...
	If Not Attached Then
		ErrorCertificateSetup = NStr("en='File operation extension is not installed.';ru='Не установлено расширение для работы с файлами.';vi='Không thể cài đặt phần mở rộng để làm việc với tệp.'");
	Else
...
Procedure SetCertificateWhenCryptographyObjectCreationErrorAppearsAfterConnectingCryptographyExtension(Attached, Context) Export
	...
	If Not Attached Then
		ErrorCertificateSetup = NStr("en='Cryptography operation extension is not installed.';ru='Не установлено расширение для работы с криптографией.';vi='Chưa thiết lập phần mở rộng để làm việc với mật mã.'");
	Else
		ErrorCertificateSetup = NStr("en='Additional external component is not installed.';ru='Не установлена дополнительная внешняя компонента.';vi='Chưa thiết lập cấu phần bổ sung ngoài.'");
	EndIf;
...
Procedure SetCertificateAfterGettingPermission(PermissionsReceived, Context) Export
	...
	If Not PermissionsReceived Then
		ErrorCertificateSetup =
			NStr("en='Saving certificates to the temporary folder is canceled by user.';ru='Сохранение сертификатов во временную папку отменено пользователем.';vi='Người sử dụng đã hủy lưu chứng thư vào thư mục tạm thời.'");
		ExecuteNotifyProcessing(Context.Notification, False);
		Return;
	EndIf;
...
Procedure SetCertificateAfterReceivingFiles(ReceivedFiles, Context) Export
	...
	If ReceivedFiles = Undefined
	 Or ReceivedFiles.Count() <> 2 Then
		...
		ErrorCertificateSetup = NStr("en='Certificates were not saved into a temporary folder.';ru='Сертификаты не были сохранены во временную папку.';vi='Chứng thư đã không được lưu vào thư mục tạm thời.'");
...
"Имя контейнера: ""%1""."
"Путь к контейнеру: ""%2"".';vi='Không thể tìm thấy vật chứa khóa trên máy tính."
"Tên vật chứa: ""%1""."
"Đường dẫn tới vật chứa: ""%2"".'"),
			KeyContainerName,
			KeyContainerPath);
		ExecuteNotifyProcessing(Context.Notification, False);
		Return;
...
Procedure SetCertificateAfterCallSetCertificateToContainerAndStorage(Result, CallParameters, Context) Export
	...
	If Result <> True AND Not ValueIsFilled(ErrorCertificateSetup) Then
		ErrorCertificateSetup = NStr("en='Cannot install a personal certificate.';ru='Не удалось установить личный сертификат.';vi='Không thể thiết lập chứng thư riêng.'");
	EndIf;
...
Procedure SetCertificateAfterCertificateSearch(Result, Context) Export
	...
	If TypeOf(Result) <> Type("CryptoCertificate") Then
		If Result.Property("CertificateNotFound") Then
			SearchError =
				NStr("en='Cannot find a personal certificate installed on the computer.';ru='Не удалось найти личный сертификат, установленный на компьютер.';vi='Không thể tìm thấy chứng thư riêng, thiết lập trên máy tính.'");
		Else
...
"на компьютер по причине: %1';vi='Không thể tìm thấy chứng thư riêng, đã thiết lập"
"trên máy tính do nguyên nhân: %1'"),
...
Procedure SetCertificateAfterCallingImportCertificate(Result, CallParameters, Context) Export
	...
	If Result <> True AND Not ValueIsFilled(ErrorCertificateSetup) Then
		ErrorCertificateSetup = NStr("en='Cannot install the root certificate.';ru='Не удалось установить корневой сертификат.';vi='Không thể thiết lập chứng thư gốc.'");
	EndIf;
...
Procedure SetCertificateAfterRootCertificateSearch(Result, Context) Export
	...
	If TypeOf(Result) <> Type("CryptoCertificate") Then
		If Result.Property("CertificateNotFound") Then
			SearchError =
				NStr("en='Cannot find a root certificate installed on the computer.';ru='Не удалось найти корневой сертификат, установленный на компьютер.';vi='Không thể tìm thấy chứng thư gốc, thiết lập trên máy tính.'");
		Else
			SearchError = StringFunctionsClientServer.SubstituteParametersInString(
...
"на компьютер по причине: %1';vi='Không thể tìm thấy chứng thư gốc cài đặt"
"trong máy tính do nguyên nhân: %1'"),
				Result.ErrorDescription);
		EndIf;
		ErrorCertificateSetup = TrimL(ErrorCertificateSetup + Chars.LF + Chars.LF) + SearchError;
		SetCertificateDeleteTemporaryDirectoryAndEnd(False, Context);
...
Function RFStateNameByRecommendationsForDSVKC(StateCode)
	...
	If Left(String, 3) <> StateCode + " " Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Region of the Russian Federation with the ""%1"" code does not exist.';ru='Регион РФ с кодом ""%1"" не существует.';vi='Khu vực Việt Nam với mã ""%1"" không tồn tại.'"), StateCode);
	EndIf;
...
Procedure ImportStatement()
	...
	ElsIf CurrentObject.RequestStatus = Enums.CertificateIssueRequestState.Rejected Then
		AutoTitle = False;
		Title = NStr("en='The application using which the certificate was not received';ru='Заявление, по которому не удалось получить сертификат';vi='Giấy đề nghị mà không nhận được chứng thư'");
		Items.FormNext.Visible = False;
		Items.FormClose.Title = NStr("en='Close';ru='Закрыть';vi='Đóng'");
	...
	ElsIf CurrentObject.RequestStatus = Enums.CertificateIssueRequestState.Executed Then
		AutoTitle = False;
		Title = NStr("en='Application which was used to get the certificate';ru='Заявление, по которому был получен сертификат';vi='Giấy đề nghị mà đã nhận được chứng thư'");
...
Procedure FillAttributesStatements(All = False)
	...
	If All Then
		...
		String.Attribute = NStr("en='Digital signature application';ru='Программа электронной подписи';vi='Chương trình chữ ký điện tử'");
		String.Value = ApplicationPresentation;
	EndIf;
	
	// Completion of information about company.
	FillAttribute(NStr("en='Company information';ru='Сведения об организации';vi='Thông tin về doanh nghiệp'"),, True);
	...
	FillAttribute(NStr("en='Certificate owner information';ru='Сведения о владельце сертификата';vi='Thông tin về chủ sở hữu chứng thư'"),, True);
	...
	String = RequestAttributes.Add();
	String.Attribute = NStr("en='Identity document';ru='Документ, удостоверяющий личность';vi='Giấy tờ cá nhân'");
	String.Value = Items.DocumentKind1.ChoiceList.FindByValue(DocumentKind1).Presentation;
	
	String = RequestAttributes.Add();
	If DocumentKind1 = "21" Then
		String.Attribute = NStr("en='Series and number';ru='Серия и номер';vi='Ký hiệu và số'");
		String.Value = AttributePresentationRFPassportNumber(DocumentNumber);
	Else
		String.Attribute = NStr("en='Number';ru='Number';vi='Số'");
	...
	If Not ThisIsIndividualEntrepreneur Then
		...
		FillAttribute(NStr("en='Information about manager';ru='Сведения о руководителе';vi='Thông tin về lãnh đạo'"),, True);
		
		String = RequestAttributes.Add();
		String.Attribute = NStr("en='Full name of the manager';ru='ФИО руководителя';vi='Họ tên người lãnh đạo'");
		String.Value = DocumentsHead;
		
		String = RequestAttributes.Add();
		String.Attribute = NStr("en='Manager position';ru='Должность руководителя';vi='Chức vụ của người lãnh đạo'");
		String.Value = DocumentsHeadPosition;
		
		String = RequestAttributes.Add();
		String.Attribute = NStr("en='Manager action basis';ru='Основание действий руководителя';vi='Cơ sở hoạt động của lãnh đạo'");
		String.Value = DocumentsHeadBasis;
	EndIf;
	
	// Completion of information about printed documents.
	FillAttribute(NStr("en='Information on service company';ru='Сведения об обслуживающей организации';vi='Thông tin về doanh nghiệp dịch vụ'"),, True);
	
	String = RequestAttributes.Add();
	String.Attribute = NStr("en='Partner name';ru='Наименование партнера';vi='Tên gọi đối tác'");
	String.Value = DocumentsPartner;
	
	String = RequestAttributes.Add();
	String.Attribute = NStr("en='Partner TIN';ru='ИНН партнера';vi='MST đối tác'");
...
Procedure SetAgreement()
	...
	HTMLText = StringFunctionsClientServer.SubstituteParametersInString(HTMLText,
		...
		NStr("en='Scientific and production center 1C, LLC';ru='Scientific and production center ""1C"", LLC';vi='Trung tâm sản xuất và khoa học ""1C"". LLC'"),
		// Address of certification center regulations.
		NStr("en='http://ca.1c.en/reglament.pdf';ru='http://ca.1c.en/reglament.pdf';vi='http://ca.1c.en/reglament.pdf'"),
		// Network of authorized certification centers.
		NStr("en='authorized federal authority on the use of digital signature';ru='уполномоченного федерального органа в области использования электронной подписи';vi='cơ quan Nhà nước có thẩm quyền trong việc sử dụng chữ ký điện tử'"));
...
Procedure AfterSelectingApplicationAfterCreatingCryptographyManager(Manager, Context) Export
	...
	If TypeOf(Manager) <> Type("CryptoManager") Then
		...
		If Context.ShowError Then
			DigitalSignatureServiceClient.ShowRequestToApplicationError(
				NStr("en='Select a digital signature application';ru='Выбор программы электронной подписи';vi='Chọn chương trình chữ ký điện tử'"),, Manager, New Structure);
		EndIf;
...
Procedure WriteStatement()
	...
	If CurrentObject.RequestStatus = Enums.CertificateIssueRequestState.NotPrepared Then
		CurrentObject.Description = Surname + " " + Name + " " + Patronymic + " (" + NStr("en='Application for new certificate';ru='Заявление на новый сертификат';vi='Giấy đề nghị chứng thư mới'") + ")";
		// Company.
	...
	ElsIf CurrentObject.RequestStatus = Enums.CertificateIssueRequestState.Rejected Then
		If ThisIsNewState Then
			UpdateValue(Content, "UpdateDateConditions");
			UpdateValue(Content, "RequestProcessingState");
			NewDescription = Surname + " " + Name + " " + Patronymic + " (" + NStr("en='Delete';ru='Удалить';vi='Xóa bỏ'") + ")";
...
Procedure AfterWrite(NotificationText = "")
	...
	If Not ValueIsFilled(NotificationText) Then
		NotificationText = NStr("en='Application is saved.';ru='Заявление сохранено.';vi='Giấy đề nghị đã được lưu lại.'");
	EndIf;
...
"и запроса на сертификат требуется установить расширение для веб-клиента 1С:Предприятия.';vi='Để tạo khóa chữ ký điện tử"
"và yêu cầu chứng thư, cần cài đặt phần mở rộng đối với Web-client 1C:DOANH NGHIỆP.'"),
			NStr("en='To create a digital signature key and a certificate request, install an additional external component.';ru='Для создания ключа электронной подписи и запроса на сертификат требуется установить дополнительную внешнюю компоненту.';vi='Để tạo khóa chữ ký điện tử và yêu cầu chứng thư, cần cài đặt cấu phần ngoài bổ sung.'"));
	EndIf;
...
Procedure CreateKeyAndCertificateQueryAfterKeyContainerExistenceCheck(Exists, Context) Export
	...
	If Exists Then
		ShowUserNotification(NStr("en='Previously created key container is used:';ru='Используется ранее созданный контейнер ключа:';vi='Đang sử dụng kho chứa khóa đã tạo trước đây:'"),,
			KeyContainerPath);
...
"to:% 1';ru='Не удалось"
"выполнить операцию по причине: %1';vi='Không thể"
"thực hiện giao dịch do nguyên nhân: %1'"),
		BriefErrorDescription(ErrorInfo)));
	
	DigitalSignatureServiceClient.ShowRequestToApplicationError(
		NStr("en='Create DS key';ru='Создание ключа электронной подписи';vi='Tạo khóa chữ ký điện tử'"),, Error, New Structure);
...
"ключа с помощью программы %1, требуются права администратора операционной системы.';vi='Chưa tạo khóa chữ ký điện tử."
""
"Cần lưu ý rằng, để tạo"
"khóa nhờ chương trình %1, cần có quyền quản trị hệ thống.'"),
				Rows[0].Presentation));
		Else
			ShowMessageBox(,
				NStr("en='Digital signature key is not created.';ru='Ключ электронной подписи не создан.';vi='Khóa chữ ký điện tử chưa được tạo ra.'"));
		EndIf;
		ExecuteNotifyProcessing(Context.Notification, False);
		Return;
	EndIf;
	...
	ShowUserNotification(NStr("en='New key container is created:';ru='Создан новый контейнер ключа:';vi='Đã tạo kho chứa khóa mới:'"),, KeyContainerPath);
...
Procedure CreateKeyAndCertificateQueryAfterCallErrorCreateCertificateQuery(ErrorInfo, StandardProcessing, Context) Export
	...
	CreateKeyAndCertificateQueryEnd(Context, ErrorInfo,
		NStr("en='Cannot execute the operation due to:';ru='Не удалось выполнить операцию по причине:';vi='Không thể thực hiện giao dịch do:'"));
...
Procedure CreateKeyAndCertificateQueryAfterCallErrorGetOpenKey(ErrorInfo, StandardProcessing, Context) Export
	...
	CreateKeyAndCertificateQueryEnd(Context, ErrorInfo,
		NStr("en='Cannot receive a public key part due to:';ru='Не удалось получить открытую часть ключа по причине:';vi='Không thể nhận phần khóa mở bởi vì:'"));
...
Procedure CreateKeyAndCertificateQueryAfterCallErrorCalculateEntityKeyIdentifier(ErrorInfo, StandardProcessing, Context) Export
	...
	CreateKeyAndCertificateQueryEnd(Context, ErrorInfo,
		NStr("en='Cannot calculate the subject key ID due to:';ru='Не удалось вычислить идентификатор ключа субъекта по причине:';vi='Không thể tính Tên định danh của khóa chủ thể bởi vì:'"));
...
Procedure CreateKeyAndCertificateQueryEnd(Context, ErrorInfo = Undefined, ErrorTitle = "")
	...
	If ErrorInfo <> Undefined Then
		...
		DigitalSignatureServiceClient.ShowRequestToApplicationError(
			NStr("en='Create certificate request';ru='Создание запроса на сертификат';vi='Tạo truy vấn tới chứng thư'"),, Error, New Structure);
		
	ElsIf Not Context.QueryCreated Then
		ShowMessageBox(, NStr("en='Certificate request is not created.';ru='Запрос на сертификат не создан.';vi='Truy cập tới chứng thư chưa được tạo ra.'"));
	EndIf;
...
Procedure GetNewKeyContainerName(Notification)
	...
	If ThisIsIndividualEntrepreneur Then
		...
		Subject = PrepareRowForContainerName(Surname) + " "
			...
			+ NStr("en='Individual entrepreneur';ru='Индивидуальный предприниматель';vi='Cá nhân kinh doanh'");
	Else
...
Procedure CreateCryptographyObjectAfterConnectingExternalComponent(Connected, Context) Export
	...
	Else
		Buttons = New ValueList;
		Buttons.Add("Set",      NStr("en='Set';ru='Установить';vi='Thiết lập'"));
		Buttons.Add("DoNotInstall", NStr("en='Do not set';ru='Не устанавливать';vi='Chưa thiết lập'"));
...
"компоненту по причине: %1';vi='Không thể cài đặt cấu phần ngoài"
"bổ sung do nguyên nhân: %1'"),
			BriefErrorDescription(ErrorInfo)));
	...
	If Not Connected Then
		ShowMessageBox(
			New NotifyDescription("CreateCryptographyObjectAfterErrorWarning",
				ThisObject, Context),
			NStr("en='Cannot connect an additional external component.';ru='Не удалось подключить дополнительную внешнюю компоненту.';vi='Không thể kết nối cấu phần bổ sung ngoài.'"));
	Else
		CreateCryptographyObjectContinuation(Context);
	EndIf;
...
"работы с криптографией по причине: %1';vi='Không thể tạo đối tượng cấu phần ngoài để"
"làm việc với mã hóa do nguyên nhân: %1'"),
			BriefErrorDescription(ErrorInfo)));
		ExecuteNotifyProcessing(Context.ContinuationProcessor, False);
		Return;
	EndTry;
...
"двоичными данными по причине: %1';vi='Không thể tạo đối tượng cấu phần ngoài để làm việc với"
"dữ liệu nhị phân do nguyên nhân: %1'"),
				BriefErrorDescription(ErrorInfo)));
			ExecuteNotifyProcessing(Context.ContinuationProcessor, False);
			Return;
		EndTry;
...
"через внешнюю компоненту по причине: %1';vi='Chương trình chữ ký điện tử không khả dụng"
"thông qua cấu phần ngoài do nguyên nhân: %1'"),
			BriefErrorDescription(ErrorInfo)));
