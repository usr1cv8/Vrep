...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CheckOnSelection Then
		Items.FormCheck.Title = NStr("en='Check and continue';ru='Проверить и продолжить';vi='Kiểm tra và tiếp tục'");
	...
	For Each Checking IN Checks Do
		...
		Group.Group = ChildFormItemsGroup.HorizontalIfPossible;
...
Procedure ShowAlertOnFailureToContinue()
	...
	ShowMessageBox(,
		NStr("en='Cannot continue as not all required checks are passed.';ru='Не удалось продолжить, т.к. пройдены не все требуемые проверки.';vi='Không thể tiếp tục bởi vì chưa thực hiện hết các kiểm tra cần thiết.'"));
...
Procedure CheckOnClientSideAfterCertificateInitialization(CryptoCertificate, Context) Export
	...
	If Context.CryptoCertificate.Subject.Property("SN") Then
		ErrorDescription = "";
	Else
		ErrorDescription = NStr("en='The ""SN"" field is not found in the certificate subject description.';ru='В описании субъекта сертификата не найдено поле ""SN"".';vi='Trong mô tả chủ thể chứng thư không tìm thấy trường ""SN"".'");
	EndIf;
...
Procedure CheckOnClientSideAfterCertificateSearch(Result, Context) Export
	...
	If TypeOf(Result) <> Type("CryptoCertificate") Then
		ErrorDescription = Result.ErrorDescription + Chars.LF + Chars.LF
			+ NStr("en='Cannot check signing, created signature and decryption.';ru='Проверка подписания, созданной подписи и расшифровки не могут быть выполнены.';vi='Không thể kiểm tra chữ ký, chữ ký đã tạo và giải mã.'");
	Else
		ErrorDescription = "";
	EndIf;
...
Procedure CheckOnClientSideAfterCertificateCheck(Result, Context) Export
	...
	Else
		ErrorDescription = NStr("en='Application for private key was not specified in the certificate.';ru='Программа для использования закрытого ключа не указана в сертификате.';vi='Chương trình dùng để sử dụng khóa đóng chưa được chỉ ra trong chứng thư.'");
...
"подписи, шифрования и расшифровки не могут быть выполнены.';vi='Không thể kiểm tra thao tác ký tên, chữ ký"
"đã tạo, mã hóa và giải mã.'");
	EndIf;
...
Procedure SetItem(Form, StartElement, AtServer, ErrorDescription = Undefined, IsError = False)
	...
	If ErrorDescription = Undefined Then
		...
		ItemPicture.ToolTip   = NStr("en='No check was performed.';ru='Проверка не выполнялась.';vi='Chưa thực hiện kiểm tra.'");
	...
	Else
		...
		ItemPicture.ToolTip   = NStr("en='Check succeeded.';ru='Проверка выполнена успешно.';vi='Đã kiểm tra thành công.'");;
