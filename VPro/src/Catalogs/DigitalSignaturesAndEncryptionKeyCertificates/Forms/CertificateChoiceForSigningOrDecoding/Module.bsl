...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.InsertIntoList Then
		InsertIntoList = True;
		Items.Select.Title = NStr("en='Add';ru='Добавить';vi='Thêm'");
		
		Items.ExplanationEnhancedPassword.Title =
			NStr("en='Click Add to enter the password.';ru='Нажмите Добавить, чтобы перейти к вводу пароля.';vi='Hãy nhấn Thêm để chuyển đến nhập mật khẩu.'");
		
		PersonalListOnAdd = Parameters.PersonalListOnAdd;
		Items.ShowAll.ToolTip =
			NStr("en='Show all certificates without filter (for example, including added and expired)';ru='Показать все сертификаты без отбора (например, включая добавленные и просроченные)';vi='Hiển thị tất cả chứng thư mà không lọc (ví dụ, bao gồm cả các chứng thư bổ sung và quá hạn)'");
	EndIf;
	...
	If ForEncryptionAndDecryption = True Then
		If Parameters.InsertIntoList Then
			Title = NStr("en='Add a certificate for data encryption and decryption';ru='Добавление сертификата для шифрования и расшифровки данных';vi='Thêm chứng thư để mã hóa và giải mã dữ liệu'");
		Else
			Title = NStr("en='Select a certificate to encrypt and decrypt data';ru='Выбор сертификата для шифрования и расшифровки данных';vi='Chọn chứng thư để mã hóa và giải mã dữ liệu'");
		EndIf;
	ElsIf ForEncryptionAndDecryption = False Then
		If Parameters.InsertIntoList Then
			Title = NStr("en='Add a certificate for signing data';ru='Добавление сертификата для подписания данных';vi='Thêm chứng thư để ký dữ liệu'");
		EndIf;
	ElsIf DigitalSignature.UseEncryption() Then
		Title = NStr("en='Add a certificate for signing and encrypting data';ru='Добавление сертификата для подписания и шифрования данных';vi='Thêm chứng thư để ký và mã hóa dữ liệu'");
	Else
		Title = NStr("en='Add a certificate for signing data';ru='Добавление сертификата для подписания данных';vi='Thêm chứng thư để ký dữ liệu'");
	EndIf;
	...
	If CreateDigitalSignaturesAtServer Then
		Items.GroupCertificates.Title =
			NStr("en='Personal certificates on computer and on server';ru='Личные сертификаты на компьютере и сервере';vi='Chứng thư riêng trên máy tính và Server'");
	EndIf;
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Items.CertificateCompany.Visible
	   ...
	   AND Not ValueIsFilled(CertificateCompany) Then
		...
		MessageText = NStr("en='Company is not populated.';ru='Поле Организация не заполнено.';vi='Chưa điền trường Doanh nghiệp.'");
...
Procedure CertificatesNotAvailableOnClientLabelClick(Item)
	...
	DigitalSignatureServiceClient.ShowRequestToApplicationError(
		NStr("en='Certificates on computer';ru='Сертификаты на компьютере';vi='Chứng thư trên máy tính'"), "", ErrorReceivingCertificatesOnClient, New Structure);
...
Procedure CertificatesNotAvailableAtServerLabelClick(Item)
	...
	DigitalSignatureServiceClient.ShowRequestToApplicationError(
		NStr("en='Certificates on server';ru='Сертификаты на сервере';vi='Chứng thư trên Server'"), "", ErrorReceivingCertificatesAtServer, New Structure);
...
Procedure SelectAfterCertificateCheck(Result, Context) Export
	...
	If Result.Property("ErrorDescription") Then
		...
		If ForEncryptionAndDecryption = True Then
			FormTitle = NStr("en='Encryption and decryption check';ru='Проверка шифрования и расшифровки';vi='Kiểm tra mã hóa và giải mã'");
		Else
			FormTitle = NStr("en='Digital signature verification';ru='Проверка установки электронной подписи';vi='Kiểm tra thiết lập chữ ký điện tử'");
		EndIf;
		DigitalSignatureServiceClient.ShowRequestToApplicationError(
...
Procedure GoToCurrentCertificateChoice(Notification)
	...
	If Items.Certificates.CurrentData = Undefined Then
		Result.ErrorDescription = NStr("en='Select a certificate to be used.';ru='Выделите сертификат, который будет использоваться.';vi='Hãy chọn chứng thư sẽ sử dụng.'");
...
"Откройте заявление на выпуск сертификата и выполните требуемые шаги.';vi='Đối với chứng thư này, giấy đề nghị xuất xưởng vẫn chưa được thực hiện."
"Hãy mở giấy đề nghị xuất xưởng và thực hiện các bước cần thiết.'");
		ExecuteNotifyProcessing(Notification, Result);
		Return;
	EndIf;
	...
	If CertificateAtServer Then
		...
		Else
			Result.ErrorDescription = NStr("en='Certificate is not found on the server (it might have been deleted).';ru='Сертификат не найден на сервере (возможно удален).';vi='Không tìm thấy chứng thư trên Server (có thể là chứng thư đã bị xóa).'");
			Result.UpdateCertificatesList = True;
			ExecuteNotifyProcessing(Notification, Result);
		EndIf;
		Return;
...
Procedure GoToCurrentCertificateChoiceAfterCertificateSearch(SearchResult, Context) Export
	...
	If TypeOf(SearchResult) <> Type("CryptoCertificate") Then
		If SearchResult.Property("CertificateNotFound") Then
			Context.Result.ErrorDescription = NStr("en='Certificate is not found on the computer (it might have been deleted).';ru='Сертификат не найден на компьютере (возможно удален).';vi='Không tìm thấy chứng thư trong máy tính (có thể là chứng thư đã bị xóa).'");
		Else
...
Procedure GoToCurrentCertificateChoiceAfterFillingCertificateProperties(Context)
	...
	If InsertIntoList Then
		String = ?(ValueIsFilled(Certificate), NStr("en='Update calendar';ru='Обновить календарь';vi='Cập nhật lịch'"), NStr("en='Add';ru='Добавить';vi='Thêm'"));
...
Procedure CheckCertificateAfterCreatingCryptographyManager(Result, Context) Export
	...
	If TypeOf(Result) <> Type("CryptoManager")
	   AND Result.Common Then
		...
		If ForEncryptionAndDecryption = True Then
			Result.Insert("ErrorTitle", NStr("en='Cannot pass the encryption check due to:';ru='Не удалось пройти проверку шифрования по причине:';vi='Không thể tiến hành kiểm tra mã hóa bởi nguyên nhân:'"));
		Else
			Result.Insert("ErrorTitle", NStr("en='Cannot pass the signing check due to:';ru='Не удалось пройти проверку подписания по причине:';vi='Không thể tiến hành kiểm tra việc ký tên bởi nguyên nhân:'"));
		EndIf;
...
Function CheckCertificateAndWriteInCatalog(Val PasswordValue, ErrorOnServer)
	...
	If DigitalSignatureService.CryptoManager("", False, ErrorOnServer) = Undefined
	   AND ErrorOnServer.Common Then
		...
		If ForEncryptionAndDecryption = True Then
			ErrorOnServer.Insert("ErrorTitle", NStr("en='Cannot pass the encryption check due to:';ru='Не удалось пройти проверку шифрования по причине:';vi='Không thể tiến hành kiểm tra mã hóa bởi nguyên nhân:'"));
		Else
			ErrorOnServer.Insert("ErrorTitle", NStr("en='Cannot pass the signing check due to:';ru='Не удалось пройти проверку подписания по причине:';vi='Không thể tiến hành kiểm tra việc ký tên bởi nguyên nhân:'"));
		EndIf;
		Return False;
	EndIf;
...
"%2';vi='Không thể tiến hành kiểm tra mã hóa bằng chương trình %1 bởi nguyên nhân:"
"%2'"),
...
"помощью программы %1 по причине: %2';vi='Không thể chuyển đến kiểm tra diễn giải"
"thông qua chương trình %1 do nguyên nhân: %2'"),
			ApplicationDescription.Description,
			ErrorPresentation);
...
"программы %1 по причине: %2';vi='Không thể kiểm tra chữ ký bởi"
"chương trình %1 do nguyên nhân: %2'"),
			ApplicationDescription.Description,
			ErrorPresentation);
