		...
		For Each String IN Rows Do
			...
			If TypeOf(CertificateData) <> Type("BinaryData") Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='The ""%1"" certificate data is not found in catalog';ru='Данные сертификата ""%1"" не найдены в справочнике';vi='Không tìm thấy dữ liệu chứng thư ""%1"" trong danh mục'"), Selection.Presentation);
			EndIf;
			Try
...
"не корректны по причине: %2';vi='Dữ liệu của chứng thư ""%1"" trong danh mục"
"không sai do nguyên nhân: %2'"),
...
"Выберите другой сертификат.';vi='Chứng thư riêng đã chọn đã hết hạn."
"Hãy chọn chứng thư khác.'"));
...
"Выберите другой сертификат.';vi='Ở chứng thư riêng đã chọn chưa chỉ ra chương trình dành cho khóa đóng."
"Hãy chọn chứng thư khác.'"));
			ShowError(Context.ErrorOnClient, Context.ErrorOnServer);
			ExecuteNotifyProcessing(Context.Notification, False);
			Return;
...
Function CertificatesProperties(Val Refs, Val FormID)
	...
	While Selection.Next() Do
		...
		If TypeOf(CertificateData) <> Type("BinaryData") Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='The ""%1"" certificate data is not found in catalog';ru='Данные сертификата ""%1"" не найдены в справочнике';vi='Không tìm thấy dữ liệu chứng thư ""%1"" trong danh mục'"), Selection.Description);
		EndIf;
...
Procedure WriteEncryptionCertificatesAtServer(ObjectsDescription, CertificatesAddress, FormID, Error)
	...
	Except
		ErrorInfo = ErrorInfo();
		RollbackTransaction();
		Error.Insert("ErrorDescription", NStr("en='An error occurred when writing encryption certificates:';ru='При записи сертификатов шифрования возникла ошибка:';vi='Khi ghi lại chứng thư mã hóa xảy ra lỗi:'")
...
Procedure ShowError(ErrorOnClient, ErrorOnServer)
	...
	DigitalSignatureServiceClient.ShowRequestToApplicationError(
		NStr("en='Cannot encrypt data';ru='Не удалось зашифровать данные';vi='Không thể mã hóa dữ liệu'"), "",
