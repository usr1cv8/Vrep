...
"Подобрать?';vi='Có thể chọn tiền tệ từ bảng mã hiệu."
"Chọn?'");
	Notification = New NotifyDescription("CurrenciesBeforeAddStartEnd", ThisObject);
	SelectionButtons = New ValueList();
	SelectionButtons.Add(DialogReturnCode.Yes, NStr("en='Pick';ru='Выбрать';vi='Chọn'"));
	SelectionButtons.Add(DialogReturnCode.No, NStr("en='Create';ru='Создать';vi='Tạo mới'"));
	SelectionButtons.Add(DialogReturnCode.Cancel, NStr("en='Cancel';ru='Отмена';vi='Hủy bỏ'"));
