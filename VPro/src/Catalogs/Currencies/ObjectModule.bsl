...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If Cancel Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Exchange rates can be linked to the rate of independent currency only.';ru='Курс валюты можно связать только с курсом независимой валюты.';vi='Tỷ giá hối đoái chỉ có thể liên hệ tới tỷ giá tiền tệ độc lập.'"));
	EndIf;
	...
	If Not IsNew()
		...
		CommonUseClientServer.MessageToUser(
			NStr("en='The currency cannot be subordinate as it is the main one for other currencies.';ru='Валюта не может быть подчиненной, так как она является основной для других валют.';vi='Tiền tệ không thể trực thuộc, bởi vì nó là tiền tệ chính đối với các loại tiền tệ khác.'"));
		Cancel = True;
	EndIf;
...
Procedure UpdateExchangeRate(SubordinateCurrency) 
	...
	ElsIf SetRateMethod = Enums.CurrencyRateSetMethods.CalculationByFormula Then
		...
		If MainCurrencies.Count() = 0 Then
			ErrorText = NStr("en='At least one main currency is to be used in the formula.';ru='В формуле должна быть использована хотя бы одна основная валюта.';vi='Trong biểu mẫu cần sử dụng ít nhất một tiền tệ chính.'");
