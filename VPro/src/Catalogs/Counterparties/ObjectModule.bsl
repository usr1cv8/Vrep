...
Procedure OnWrite(Cancel)
	...
	If Not IsFolder And AdditionalProperties.Property("NewMainContract") Then
		...
		ContractObject.AdditionalProperties.Insert("NewPriceKind");
...
Procedure GenerateBasicInformation() Export
	...
	If Not IsBlankString(TIN) Then
		RowsArray.Add(NStr("en='TIN';ru='ИНН';vi='Mã số thuế'") + " " + TIN);
	EndIf;
...
Procedure CheckChangePossibility(Cancel)
	...
	If DoOperationsByContracts <> PreviousValues.DoOperationsByContracts
		...
		If Not QueryResult.IsEmpty() Then
			MessageText = NStr("en='Records are registered for mutual settlements with the counterparty in the infobase. Cannot change the settlements accounting dimension.';ru='В базе присутствуют движения по взаиморасчетам с контрагентом. Изменение аналитики учета взаиморасчетов запрещено.';vi='Trong cơ sở thông tin đã phát sinh công nợ với đối tác. Không thể thay đổi đối tượng phân tích hạch toán công nợ.'");
