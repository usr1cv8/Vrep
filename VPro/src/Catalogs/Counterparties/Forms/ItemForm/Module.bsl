...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Object.LegalEntityIndividual = Enums.CounterpartyKinds.Individual Then
		Items.DescriptionFull.Title	= NStr("en='Name, surname';ru='Фамилия, имя, отчество';vi='Họ tên'");
	Else
		Items.DescriptionFull.Title	= NStr("en='Legal name';ru='Публичное наименование';vi='Tên gọi pháp lý'");
	EndIf;
	...
	AdditionalParameters = PropertiesManagementOverridable.FillAdditionalParameters(Object, "AdditionalAttributes", , "FormCommandsStandart");
...
Procedure AddAdditionalAttribute(Command)
	...
	FormParameters = New Structure;
	FormParameters.Insert("CurrentSetOfProperties", PredefinedValue("Catalog.AdditionalAttributesAndInformationSets.Catalog_Counterparties"));
...
Procedure SetFormTitle(Form)
	...
	If Object.Customer Then
		RelationshipKinds.Add(NStr("en='Customer';ru='Покупатель';vi='Khách hàng'"));
	EndIf;
	
	If Object.Supplier Then
		RelationshipKinds.Add(NStr("en='Supplier';ru='Поставщик';vi='Nhà cung cấp'"));
	EndIf;
	...
	Else	
		Title = Object.Description + " (" + NStr("en='Counterparty';ru='Контрагент';vi='Đối tác'");
	EndIf;
...
&AtClient
Procedure ProcessingDuplicatesChoiceFormClosing(Result, AdditionalParameters) Export
	...
	If Not IsBlankString(Object.TIN) Then
		...
		If DuplicatesNumber > 0 Then
			...
			ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("ru = 'Уже есть контрагенты с таким ИНН (%1)';
					|en = 'There are already counterparties with such a TIN (%1)';
					|vi = 'Đã có đối tác có Mã số thuế này (%1)'"), DuplicatesNumber);
...
EndProcedure // ОбновитьЗависимостиДополнительныхРеквизитов()
...
EndProcedure // Подключаемый_ПриИзмененииДополнительногоРеквизита()
// End StandardSubsystems.Properties
