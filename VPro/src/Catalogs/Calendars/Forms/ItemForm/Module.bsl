...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Object.FillTemplate.FindRows(New Structure("DayIncludedInSchedule", True)).Count() = 0 Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Days included in the work schedule are not marked';ru='Не отмечены дни, включаемые в график работы';vi='Chưa đánh dấu ngày được đưa vào lịch làm việc'"), , "Object.FillTemplate", , Cancel);
	EndIf;
...
Procedure CurrentYearNumberOnChange(Item)
	...
	If ModifiedResults Then
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(
							NStr("en='Write the changed data for the year of %1?';ru='Записать измененные данные за %1 год?';vi='Ghi lại dữ liệu đã thay đổi trong %1 năm?'"), 
...
Procedure SetConditionalAppearance()
	...
	Item.Appearance.SetParameterValue("Text", NStr("en='Fill in timetable';ru='Заполнить расписание';vi='Điền lịch biểu'"));
...
Procedure GenerateFillTemplate(FillMethod, FillTemplate, Val CycleLength, Val BeginnigDate = Undefined)
	...
	If FillMethod = PredefinedValue("Enum.WorkScheduleFillingMethods.ByWeeks") Then
		FillTemplate[0].DayPresentation = NStr("en='Monday';ru='Понедельник';vi='Thứ hai'");
		FillTemplate[1].DayPresentation = NStr("en='Tuesday';ru='Вторник';vi='Thứ ba'");
		FillTemplate[2].DayPresentation = NStr("en='Wednesday';ru='Среда';vi='Thứ tư'");
		FillTemplate[3].DayPresentation = NStr("en='Thursday';ru='Четверг';vi='Thứ năm'");
		FillTemplate[4].DayPresentation = NStr("en='Friday';ru='Пятница';vi='Thứ sáu'");
		FillTemplate[5].DayPresentation = NStr("en='Saturday';ru='Суббота';vi='Thứ bảy'");
		FillTemplate[6].DayPresentation = NStr("en='Sunday';ru='Воскресенье';vi='Chủ nhật'");
	Else
...
Function ScheduleForDayPresentation(Form, DayNumber)
	...
	If Seconds = 0 Then
		Return NStr("en='Fill in timetable';ru='Заполнить расписание';vi='Điền lịch biểu'");
	EndIf;
	
	Hours = Round(Seconds / 3600, 1);
	
	Return StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1 h. (%2)';ru='%1 ч. (%2)';vi='%1 g. (%2)'"), Hours, IntervalsPresentation);
...
Procedure ClarifyOccupancyGraphics(Form)
	...
	If Not ValueIsFilled(Form.DateOccupancyRate) Then
		Form.OccupancyInformationText = NStr("en='Work schedule is not filled in';ru='График работы не заполнен';vi='Lịch biểu làm việc chưa điền'");
	...
	Else	
		If Not ValueIsFilled(Form.Object.PlanningHorizon) Then
			Form.OccupancyInformationText = StringFunctionsClientServer.SubstituteParametersInString(
													NStr("en='Work schedule is filled in up to %1';ru='График работы заполнен до %1';vi='Lịch biểu làm việc đã điền đến %1'"), 
		...
		Else											
			...
			Form.OccupancyInformationText = StringFunctionsClientServer.SubstituteParametersInString(
													NStr("en='Work schedule is filled in up to %1, considering planning horizon schedule should be filled in up to %2';ru='График работы заполнен до %1, с учетом горизонта планирования график должен быть заполнен до %2';vi='Lịch biểu làm việc đã điền đến %1, có tính đến đường chân trời lập kế hoạch lịch biểu phải được điền đến %2'"), 
													Format(Form.DateOccupancyRate, "DLF=D"),
...
Procedure FillTextFillResultInformation(Form)
	...
	If Form.ManualEdit Then
		InformationalText = NStr("en='The current year work schedule is changed manually. Click ""Fill in from template"" to return to automatic population.';ru='График работы на текущий год изменен вручную. Нажмите ""Заполнить по шаблону"" чтобы вернуться к автоматическому заполнению.';vi='Đã thay đổi thủ công lịch biểu làm việc trong năm nay. Hãy nhấn ""Điền theo khuôn mẫu"" để quay lại điền tự động.'");
	...
	Else
		If Form.ResultFillByPattern Then
			If ValueIsFilled(Form.Object.BusinessCalendar) Then
				InformationalText = NStr("en='Work schedule is updated on changing a bus. calendar for the current year.';ru='График работы автоматически обновляется при изменении производственного календаря за текущий год.';vi='Lịch biểu làm việc tự động được cập nhật khi thay đổi lịch sản xuất trong kỳ hiện tại.'");
				InformationalPicture = PictureLib.Information;
			EndIf;
		Else
			InformationalText = NStr("en='The displayed result does not match the template setting. Click ""Fill in from template"" to see how the work schedule looks like with the template changes.';ru='Нажмите ""Заполнить по шаблону"", чтобы увидеть как выглядит график работы с учетом изменений шаблона.';vi='Hãy nhấn ""Điền theo khuôn mẫu"" để xem lịch làm việc có những thay đổi khuôn mẫu sẽ như thế nào.'");
			InformationalPicture = PictureLib.Warning;
			AvailableFillingPattern = True;
		EndIf;
	EndIf;
...
Procedure FillInformationTextEditing(Form)
	...
	If Form.ManualEdit Then
		InformationalPicture = PictureLib.Warning;
		InformationalText = NStr("en='Current year work schedule is changed manually. Changes are highlighted in population results.';ru='График работы на текущий год изменен вручную. Изменения выделены в результатах заполнения.';vi='Lịch biểu làm việc cho năm hiện tại đã thay đổi thủ công. Thay đổi được chọn trong các kết quả điền.'");
	EndIf;
