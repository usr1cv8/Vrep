...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.Property("Filter") AND Parameters.Filter.Property("Owner") Then
		...
		If NOT ValueIsFilled(ProductsAndServicesOwner)
			...
			AutoTitle = False;
			Title = NStr("en='Serial numbers are stored only for inventories';ru='Серийные номера хранятся только для запасов';vi='Số sê-ri chỉ được lưu đối với vật tư'");
...
Procedure BarcodesReceived(BarcodesData) Export
	...
	If NOT UseSerialNumbers Then
		...
		Message.Text = NStr("en='No account by serial numbers for this products!"
"Select the ""Use serial numbers"" check box in products and services card';ru='Для номенклатуры не ведется учет по серийным номерам!"
"Установите флаг ""Использовать серийные номера"" в карточке номенклатуры';vi='Đối với mặt hàng không tiến hành ghi nhận theo số sê-ri!"
"Hãy đặt dấu hộp kiểm ""Sử dụng số sê-ri"" trên thẻ mặt hàng'");
		Message.Message();
		Return;
	EndIf;
...
Function CreateSerialNumber(SerialNumberString, ProductsAndServicesOwner)
	...
	Try
		...
		MessageString = NStr("en='Created serial number: %1%';ru='Создан серийный номер: %1%';vi='Đã tạo số sê-ri: %1%'");
...
Function GetSerialNumberByBarcode(BarcodeData, ProductsAndServicesOwner)
	...
	Else	
		MessageString = NStr("en='Entered barcode %1% is bound to other product (serial number): %2%';ru='Введенный штрихкод %1% привязан к другой номенклатуре (серийному номеру): %2%';vi='Mã vạch đã nhập %1% đã gắn với mặt hàng (số sê-ri) khác: %2%'");
