...
"помощи сканера штрихкода (считывателя магнитных карт) или введите код вручную';vi='Hãy đọc thẻ ưu đãi"
"bằng cách quét mã vạch (đầu đọc thẻ từ) hoặc nhập mã thủ công'");
...
"карту при помощи считывателя магнитных карт или введите магнитный код вручную';vi='Hãy đọc"
"thẻ ưu đãi bằng máy đọc thẻ từ hoặc nhập thủ công mã từ '");
...
"при помощи сканера штрихкода или введите штрихкод вручную';vi='Hãy đọc thẻ ưu đãi"
"bằng máy quét mã vạch hoặc nhập mã vạch thủ công'");
		EndIf;
	Else
		If Not ValueIsFilled(DefaultCodeType) Then
			Text = NStr("en='Read the discount card with"
"barcode scanner (magnetic card reader)';ru='Считайте дисконтную карту"
"при помощи сканера штрихкода (считывателя магнитных карт)';vi='Hãy đọc thẻ ưu đãi"
"bằng máy quét mã vạch (đầu đọc thẻ từ)'");
		ElsIf DefaultCodeType = PredefinedValue("Enum.CardCodesTypes.MagneticCode") Then
			Text = NStr("en='Read the discount card"
"with barcode scanner';ru='Считайте дисконтную"
"карту при помощи сканера штрихкода';vi='Hãy đọc thẻ ưu đãi"
	...
	If Items.Pages.CurrentPage = Items.Pages.ChildItems.GroupReadingDiscountCard Then
		...
		If Not ValueIsFilled(CardCode) Then
			...
			If CodeType = PredefinedValue("Enum.CardCodesTypes.Barcode") Then
				MessageText = NStr("en='Barcode is not filled in.';ru='Штрихкод не заполнен.';vi='Chưa điền mã vạch.'");
			Else
				MessageText = NStr("en='Magnetic code is not filled in.';ru='Магнитный код не заполнен.';vi='Chưa điền mã từ.'");
			EndIf;
...
"Выберите подходящую карту.';vi='Tìm thấy nhiều thẻ ưu đãi có mã từ ""%1""."
"Hãy nhập thẻ tương ứng.'");
		Else
...
"Выберите подходящую карту.';vi='Đã tìm thấy một số thẻ ưu đãi có mã vạch ""%1""."
"Hãy chọn thẻ thích hợp.'");
		EndIf;
...
Function HandleReceivedCodeOnServer(Data, CardCodeType, Preprocessing, ThereAreFoundCards = False)
	...
	For Each TSRow IN DiscountCards.RegisteredDiscountCards Do
		...
		NewRow.Description = String(TSRow.Ref) + ?(ValueIsFilled(TSRow.Counterparty) AND ValueIsFilled(TSRow.Ref), StringFunctionsClientServer.SubstituteParametersInString(NStr("en=' Client: %1';ru=' Клиент: %1';vi=' Client: %1'"), String(TSRow.Counterparty)), "");
	...
	If DiscountCards.RegisteredDiscountCards.Count() = 0 Then
		For Each TSRow IN DiscountCards.NotRegisteredDiscountCards Do
			...
			NewRow.Description = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Register a new card: %1';ru='Зарегистрировать новую карту: %1';vi='Đăng ký thẻ mới: %1'"), String(TSRow.CardKind))+?(TSRow.ThisIsMembershipCard, " (Named, ", " (")+TSRow.CardType+")";
...
Procedure HandleReceivedCodeOnClient(Data, ReceivedCodeType, Preprocessing)
	...
	If Not Result Then
		...
		If CodeType = PredefinedValue("Enum.CardCodesTypes.Barcode") Then
			MessageText = NStr("en='Card with the ""%1"" barcode is not registered and there is no suitable discount card kind.';ru='Карта со штрихкодом ""%1"" не зарегистрирована и нет ни одного подходящего вида дисконтных карт.';vi='Thẻ với mã vạch ""%1"" chưa được đăng ký và không có dạng thẻ ưu đãi tương ứng.'");
		Else
			MessageText = NStr("en='Card with the ""%1"" magnetic code is not registered and there is no suitable discount card kind.';ru='Карта с магнитным кодом ""%1"" не зарегистрирована и нет ни одного подходящего вида дисконтных карт.';vi='Thẻ với mã từ ""%1"" chưa được đăng ký và không có dạng thẻ ưu đãi tương ứng.'");
		EndIf;
...
"Выберите подходящую карту.';vi='Tìm thấy nhiều thẻ ưu đãi có mã ""%1""."
"Hãy nhập thẻ tương ứng.'");
			LabelChoiceDiscountCard = StringFunctionsClientServer.SubstituteParametersInString(Text, CardCode);
...
"Выберите подходящий вид карты для регистрации новой дисконтной карты.';vi='Thẻ với mã vạch ""%1"" chưa được đăng ký."
"Hãy chọn dạng thẻ tương ứng để đăng ký thẻ ưu đãi mới.'");
			Else
...
"Выберите подходящий вид карты для регистрации новой дисконтной карты.';vi='Thẻ với mã từ ""%1"" chưa được đăng ký."
"Hãy chọn dạng thẻ tương ứng để đăng ký thẻ ưu đãi mới.'");			   
			EndIf;				   
			LabelChoiceDiscountCard = StringFunctionsClientServer.SubstituteParametersInString(Text, CardCode);
		EndIf;
