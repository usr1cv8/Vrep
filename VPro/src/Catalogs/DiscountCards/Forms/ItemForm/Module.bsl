...
Procedure CheckDiscountCardsDuplicates(Form)
	...
	If DuplicateItemsNumber > 0 Then
		...
		If DuplicateItemsNumber = 1 Then
			...
			DuplicatesMessageParametersStructure.Insert("DuplicateItemsNumber", NStr("en='One';ru='Одно';vi='Một'"));
			DuplicatesMessageParametersStructure.Insert("Declension", NStr("en='card';ru='карта';vi='thẻ'"));
			
		ElsIf DuplicateItemsNumber < 5 Then
			
			DuplicatesMessageParametersStructure.Insert("DuplicateItemsNumber", DuplicateItemsNumber);
			DuplicatesMessageParametersStructure.Insert("Declension", NStr("en='cards';ru='карты';vi='thẻ'"));
			
		Else
			
			DuplicatesMessageParametersStructure.Insert("DuplicateItemsNumber", DuplicateItemsNumber);
			DuplicatesMessageParametersStructure.Insert("Declension", NStr("en='cards';ru='карт';vi='thẻ'"));
			
		EndIf;	
		
		LabelTextOnDuplicates = NStr("en='With such code, there are [DuplicateItemsNumber] [Declension]';ru='С таким кодом есть [КоличествоЭлементовДублей] [Склонение]';vi='Với mã này đã có [DuplicateItemsNumber] [Declension]'");
...
Procedure FillInInformationOnDiscountCard()
	...
	Else
		Items.SalesAmountOnDiscountCard.Title = NStr("en='Sales amount';ru='Сумма продаж';vi='Số tiền bán hàng'");
	EndIf;
	...
	If ReceivedCodeType = PredefinedValue("Enum.CardCodesTypes.MagneticCode") Then
		...
		Else
			...
			ElsIf Result.Count() = 0 Then
				...
				EndIf;
			Else
				ValueSelected = Result.ChooseItem(NStr("en='Select magnetic stripe card code';ru='Выбор кода магнитной карты';vi='Chọn mã thẻ từ'"));
