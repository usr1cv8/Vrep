	...
	If IsNew() Then
		...
		If Not FileOperationsService.IsRight("FilesAdd", FileOwner) Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Insufficient rights to add files to folder ""%1"".';ru='Недостаточно прав для добавления файлов в папку ""%1"".';vi='Không đủ quyền để thêm tệp vào thư mục ""%1"".'"),
				String(FileOwner));
		EndIf;
	Else
		...
		If Not PrivilegedMode() AND DigitallySignedObjectRecord <> True AND Not DeletionMarkIsMarked Then
			...
			If ValueIsFilled(Ref) Then
				...
				If DigitallySigned AND RefDigitallySigned Then
					Raise NStr("en='Signed file cannot be edited.';ru='Подписанный файл нельзя редактировать.';vi='Không thể soạn tệp đã đặt mua.'");
				EndIf;	
				
				If Encrypted AND RefEncrypted AND DigitallySigned AND Not RefDigitallySigned Then
					Raise NStr("en='Encrypted file cannot be signed.';ru='Зашифрованный файл нельзя подписывать.';vi='Tệp đã mã hóa không thể ký.'");
				EndIf;	
		...
		If DeletionMarkChanged Then
			...
			If Not FileOperationsService.IsRight("FileDeletionMark", FileOwner) Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Insufficient rights to mark files for deletion in folder ""%1"".';ru='Недостаточно прав для пометки файлов на удаление в папке ""%1"".';vi='Không đủ quyền để đặt dấu xóa tệp trong thư mục ""%1"".'"),
					String(FileOwner));
			EndIf;
...
"т.к. он занят для редактирования пользователем ""%2""';vi='Không thể xóa tệp ""%1"","
"bởi vì tệp này đã bị chiếm để soạn bởi người sử dụng ""%2""'"),
					FullDescr,
					String(IsEditing) );
			EndIf;
...
"т.к. он занят для редактирования пользователем ""%2"".';vi='Không được đổi tên tệp ""%1"","
"bởi vì tệp đang bị chiếm để soạn bởi người sử dụng ""%2"".'"),
					VIBDescription,
					String(IsEditing));
			EndIf;
		EndIf;
