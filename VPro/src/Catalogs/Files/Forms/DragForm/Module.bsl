...
"Hãy sử dụng lệnh ""Tạo mới"" trong danh sách tệp.'");
	...
	If DirectoriesOnly Then
		Title = NStr("en='Folder import';ru='Загрузка папок';vi='Kết nhập thư mục'");
	EndIf;
...
Procedure ImportFiles()
	...
	If SelectedFiles.Count() = 0 Then
		CommonUseClientServer.MessageToUser(
			NStr("en='No files to add.';ru='Нет файлов для добавления.';vi='Không có tệp để thêm.'"), , "SelectedFiles");
		FieldsNotFilled = True;
	EndIf;
	
	If FolderForAdding.IsEmpty() Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Specify the folder.';ru='Укажите папку.';vi='Hãy chỉ ra thư mục.'"), , "FolderForAdding");
		FieldsNotFilled = True;
	EndIf;
