...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If FileExtensionInList Then
		...
		If Not ValueIsFilled(Encoding) Then
			Encoding = NStr("en='Default';ru='По умолчанию';vi='Theo mặc định'");
		EndIf;
...
""
"Скопировать сведения об ЭП в новый файл?';vi='Tệp ""%1"" đã được ký tên."
"Việc sao chép thông tin về chữ ký điện tử sang tệp mới sẽ làm cho tệp không thể thay đổi được."
""
"Sao chép thông tin về chữ ký điện tử sang tệp mới?'"),
...
""
"Закрыть карточку?';vi='Tệp đã bị chiếm để soạn."
""
"Đóng thẻ?'");
	...
	If CreationMode = "FromTemplate" AND Not Object.DigitallySigned Then
		If NewFile AND NewFileWritten AND (NOT FileEdited) Then
			If Not AnswerToQuestionReceivedOpenForEditing Then
				Cancel = True;
				QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Open file ""%1"" for editing?';ru='Открыть файл ""%1"" для редактирования?';vi='Mở tệp ""%1"" để soạn?'"),
...
""
"Данные будут записаны.';vi='Vẫn chưa ghi lại dữ liệu. Chỉ có thể điền"
"lệnh ""Mã hóa"" sau khi ghi lại dữ liệu."
""
...
Procedure Delete(Command)
	...
	ShowQueryBox(Handler, NStr("en='Delete the selected signatures?';ru='Удалить выделенные подписи?';vi='Xóa chữ ký được chọn?'"), QuestionDialogMode.YesNo);
...
Procedure AddAdditionalAttribute(Command)
	...
	FormParameters.Insert("CurrentSetOfProperties", PredefinedValue("Catalog.AdditionalAttributesAndInformationSets.Catalog_Files"));
...
Procedure FillListOfSignatures()
	...
	Else
		HeaderText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Digital signatures (%1)';ru='Электронные подписи (%1)';vi='Chữ ký điện tử (%1)'"),
