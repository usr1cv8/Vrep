...
"Используйте команду ""Создать"" в списке файлов.';vi='Trong Web-Client không được phép kết nhập thư mục."
"Hãy sử dụng lệnh ""Tạo mới"" trong danh sách tệp.'");
...
Procedure FindExecute()
	...
	If SearchString = "" Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Search text is not entered.';ru='Не указан текст для поиска.';vi='Chưa chỉ ra nội dung để tìm kiếm.'"), , "SearchString");
		Return;
	EndIf;
...
"файл или папку, наименование или код которых содержит ""%1"".';vi='Không thể tìm thấy"
"tệp hoặc thư mục, mà tên gọi hoặc mã của nó có chứa ""%1"".'"),
			SearchString);
		ShowMessageBox(, WarningText);
	Else
...
Procedure MoveToFolder(Command)
	...
	FormParameters = New Structure;
	FormParameters.Insert("Title",    NStr("en='Install directory';ru='Установить каталог';vi='Đặt thư mục'"));
		...
		ElsIf ValueType = Type("CatalogRef.Files") Then
			...
			If Action = DragAction.Copy Then
				...
				If DraggingValue.Count() = 1 Then
					NotificationTitle = NStr("en='File is copied.';ru='Файл скопирован.';vi='Tệp đã sao chép.'");
					NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='%1"
"file has been copied to the %2 folder';ru='Файл"
"""%1"" скопирован в папку ""%2""';vi='Đã sao chép"
"tệp ""%1"" vào thư mục ""%2""'"),
						DraggingValue[0],
						String(FolderForAdding));
				Else
					NotificationTitle = NStr("en='Files are copied.';ru='Файлы скопированы.';vi='Tệp đã sao chép.'");
					NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='Files (%1 pcs) were copied into folder ""%2""';ru='Файлы (%1 шт.) скопированы в папку ""%2""';vi='Tệp (%1 c.) được sao chép sang thư mục ""%2""'"),
						DraggingValue.Count(),
						String(FolderForAdding));
				EndIf;
			...
			Else
				...
				If DraggingValue.Count() = 1 Then
					NotificationTitle = NStr("en='File is moved.';ru='Файл перенесен.';vi='Tệp đã chuyển.'");
					NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='The"
"file ""%1"" was moved to the folder ""%2""';ru='Файл"
"""%1"" перенесен в папку ""%2""';vi='Đã chuyển tệp"
"""%1"" sang thư mục ""%2""'"),
						String(DraggingValue[0]),
						String(FolderForAdding));
				Else
					NotificationTitle = NStr("en='Files are moved.';ru='Файлы перенесены.';vi='Tệp đã chuyển.'");
					NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='Files (%1 pcs) were moved into folder ""%2""';ru='Файлы (%1 шт.) перенесены в папку ""%2""';vi='Tệp (%1 c.) được chuyển vào thư mục ""%2""'"),
		...
		ElsIf ValueType = Type("CatalogRef.FileFolders") Then
			...
			If ParentChanged <> True Then
				If InfiniteLoopFound = True Then
					ShowMessageBox(, NStr("en='Levels looping.';ru='Зацикливание уровней.';vi='Các lớp bị lặp lại'"));
				EndIf;
				Return;
			EndIf;
			...
			If DraggingValue.Count() = 1 Then
				...
				NotificationTitle = NStr("en='Folder is moved.';ru='Папка перенесена.';vi='Thư mục đã chuyển.'");
				NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Folder"
"""%1"" moved to ""%2""';ru='Папка"
"""%1"" перенесена в папку ""%2""';vi='Thư mục"
"""%1"" đã chuyển vào thư mục ""%2""'"),
					String(DraggingValue[0]),
					String(FolderForAdding));
			Else
				NotificationTitle = NStr("en='Folders are moved.';ru='Папки перенесены.';vi='Thư mục đã chuyển.'");
				NotificationText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Folders (%1 pcs.) are moved to the ""%2"" folder';ru='Папки (%1 шт.) перенесены в папку ""%2""';vi='Thư mục (%1 c) đã chuyển sang thư mục ""52""'"),
					String(DraggingValue.Count()),
					String(FolderForAdding));
			EndIf;
			ShowUserNotification(NotificationTitle, , NotificationText, PictureLib.Information32);
