...
"Используйте команду ""Создать"" в списке файлов.';vi='Trong Web-client kết nhập tệp không được hỗ trợ."
...
//   DragParameters - Drag-and-drop parameters.
	...
	ElsIf TypeOf(DragParameters.Value) = Type("File") AND DragParameters.Value.IsFile() = False Then
		...
		ShowMessageBox(, NStr("en='Select files only (without directory).';ru='Выберите только файлы без каталогов.';vi='Hãy chọn chỉ tệp mà không chọn thư mục.'"));
		Return;
	...
	ElsIf TypeOf(DragParameters.Value) = Type("Array") Then
		...
		If TypeDragged <> Type("File") Then
			ShowMessageBox(, NStr("en='Select files.';ru='Выберите файлы.';vi='Hãy chọn tệp.'"));
			Return;
		EndIf;
