...
Procedure OnOpen(Cancel)
	...
	#If WebClient Or MobileClient Then
		ShowMessageBox(, NStr("en='Directory export is not supported in Web client.';ru='В Веб-клиенте экспорт каталогов не поддерживается.';vi='Trong Web-client kết xuất thư mục không được hỗ trợ.'"));
		Cancel = True;
		Return;
	#EndIf
...
"Возможно, папка еще не создана.';vi='Không thể mở thư mục kết xuất."
"Có thể, thư mục còn chưa được tạo.'");
...
Procedure SaveFolder()
	...
	If Not ExportDirectory.Exist() Then
		...
		Except
			ErrorText = NStr("en='Cannot create an export folder:';ru='Не удалось создать папку выгрузки:';vi='Không thể tạo thư mục kết xuất:'");
...
"Пожалуйста, подождите.';vi='Đang thực hiện kết xuất tệp ""%1""..."
"Xin vui lòng đợi.'"),
		String(WhatSaving) ));
...
"экспорт папки ""%1"" в каталог на диске ""%2"".';vi='Đã kết xuất thành công"
"thư mục ""%1"" vào thư mục trên ổ đĩa ""%2"".'"),
...
Procedure BypassFileTree4(ExecuteParameters)
	...
	Except
		ErrorText = NStr("en='An error occurred when creating folder ""%1"":';ru='Ошибка создания папки ""%1"":';vi='Lỗi tạo thư mục ""%1"":'");
...
""
"Повторить экспорт этого файла?';vi='Thay thế"
"tệp"
"""%1"" có thư mục với tên gọi như vậy."
""
"Thử kết xuất lại tệp này?'"),
...
""
"Заменить существующий файл файлом из хранилища файлов?';vi='Trong"
"thư mục ""%1"" đã tồn tại"
"tệp ""%2"" dung lượng của tệp hiện tại = %3 byte, ngày thay đổi %4."
"dung lượng của tệp đã lưu = %5 byte, ngày thay đổi %6."
""
"Thay thế tệp hiện tại bằng tệp từ kho chứa tệp?'"),
					ExecuteParameters.BaseSaveDirectoryFile,
