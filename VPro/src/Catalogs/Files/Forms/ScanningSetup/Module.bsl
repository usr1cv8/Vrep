...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not UseImageMagickForConvertionToPDF Then
		Items.FormatOfScannedImage.Title = NStr("en='Format';ru='Формат';vi='Định dạng'");
	Else
		Items.FormatOfScannedImage.Title = NStr("en='Type';ru='Тип';vi='Kiểu'");
	EndIf;
...
Procedure PathToConversionApplicationStartChoice(Item, ChoiceData, StandardProcessing)
	...
	Filter = NStr("en='Executable files(*.exe)|*.exe';ru='Исполняемые файлы(*.exe)|*.exe';vi='Tệp đang thực hiện(*.exe)|*.exe'");
	FileOpeningDialog.Filter = Filter;
	FileOpeningDialog.Multiselect = False;
	FileOpeningDialog.Title = NStr("en='Select a file to convert to PDF';ru='Выберите файл для преобразования в PDF';vi='Hãy chọn tệp để chuyển sang PDF'");
...
Procedure RefreshStatus1()
	...
	If Not FileOperationsServiceClient.InitializeComponent() Then
		ScanComponentVersion = NStr("en='Scan component is not installed';ru='Компонента сканирования не установлена';vi='Cấu phần quét ảnh chưa cài đặt'");
		Items.DeviceName.Enabled = False;
		Return;
	EndIf;
...
Procedure ReadScannerSettings()
	...
	MessageText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Gathering information about scanner ""%1""...';ru='Идет сбор сведений о сканере ""%1""...';vi='Đang thu thập thông tin về máy quét ""%1""...'"), DeviceName);
