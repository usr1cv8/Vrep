...
Procedure AcceptAllAsSeparateFiles(Command)
	...
	If ErrorsCount > 0 Then
		...
		Else
			ShortAllTextErrors = StrReplace(NStr("en='Performing failed (%1).';ru='При выполнении возникли ошибки (%1).';vi='Khi thực hiện đã xuất hiện lỗi (%1).'"), "%1", String(ErrorsCount));
...
Procedure ConvertEnumsToParametersAndGetPresentation()
	...
	If UseImageMagickForConvertionToPDF Then
		If OnePageStorageFormat = Enums.OnePageFilesStorageFormats.PDF Then
			...
			Presentation = Presentation + NStr("en='Storage format:';ru='Формат хранения:';vi='Định dạng lưu:'") + " ";
	...
	If Upper(PictureFormat) = "JPG" Then
		Presentation = Presentation +  NStr("en='Quality:';ru='Качество:';vi='Chất lượng:'") + " " + String(JPGQuality) + ". ";
	EndIf;	
	
	If Upper(PictureFormat) = "TIF" Then
		Presentation = Presentation +  NStr("en='Deflate:';ru='дефлятируем:';vi='Nén:'") + " " + String(TIFFCompression) + ". ";
	EndIf;
	
	Presentation = Presentation + NStr("en='Multi-page storage format:';ru='Формат хранения многостраничный:';vi='Định dạng lưu nhiều trang:'") + " ";
	...
	Presentation = Presentation + StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Resolution: %1 dpi. %2.';ru='Разрешение: %1 dpi. %2.';vi='Cho phép: %1 dpi. %2.'") + " ",
		String(Resolution), String(ChromaticityEnum));
	
	If Not RotationEnum.IsEmpty() Then
		Presentation = Presentation +  NStr("en='Rotation:';ru='Поворот:';vi='Xoay:'")+ " " + String(RotationEnum) + ". ";
	EndIf;	
	
	If Not PaperSizeEnum.IsEmpty() Then
		Presentation = Presentation +  NStr("en='Paper size:';ru='Размер бумаги:';vi='Khổ giấy:'") + " " + String(PaperSizeEnum) + ". ";
	EndIf;	
	
	If DoubleSidedScan = True Then
		Presentation = Presentation +  NStr("en='Scan both sides';ru='Двустороннее сканирование';vi='Quét ảnh hai chiều'") + ". ";
	EndIf;	
...
"путь к программе преобразования в PDF в форме настроек сканирования.';vi='Không tìm thấy tệp ""%1""."
"Hãy kiểm tra"
"đã cài đặt chương trình ImageMagick và chỉ ra đúng"
"đường dẫn đến chương trình chuyển đổi sang PDF trong biểu mẫu tùy chỉnh quét ảnh hay chưa.'"),
		ResultFile);
