...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not EditMode Then
		TitleString = TitleString + NStr("en=' (read only)';ru=' (только просмотр)';vi=' (chỉ xem)'");
	EndIf;
	Title = TitleString;
	...
	If ValueIsFilled(FileData.Version) Then
		...
		If ValueIsFilled(FileTextEncoding) Then
			...
			EndIf;
		Else
			EncodingPresentation = NStr("en='Default';ru='По умолчанию';vi='Theo mặc định'");
		EndIf;
...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	If Modified Then
		...
		NameAndExtension = CommonUseClientServer.GetNameWithExtention(
			...
			FileData.Extension);
		QuestionText = StrReplace(NStr("en='File ""%1"" was changed.';ru='Файл ""%1"" был изменен.';vi='Tệp ""%1"" đã thay đổi.'"), "%1", NameAndExtension);
...
Procedure SaveAs(Command)
	...
	Filter = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='All files (*.%1)|*.%1';ru='Все файлы (*.%1)|*.%1';vi='Tất cả tệp (*.%1)|*.%1'"), FileData.Extension, FileData.Extension);
	...
	If FileChoice.Choose() Then
		...
		Status(NStr("en='File successfully saved';ru='Файл успешно сохранен';vi='Tệp đã lưu thành công'"), , SelectedFullFileName);
...
Procedure ShowDifference(Command)
	#If WebClient Or MobileClient Then
		ShowMessageBox(, NStr("en='Version comparison is not supported in web client.';ru='Сравнение версий в веб-клиенте не поддерживается.';vi='So sánh phiên bản trên Web-client không được hỗ trợ.'"));
	#Else
