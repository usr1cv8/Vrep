	...
	For Each ClassifierString In ClassifierTable() Do
		...
		Except
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='An error occurred when writing country %1 (code %2) when updating the classifier, %3';ru='Ошибка записи страны мира %1 (код %2) при обновлении классификатора, %3';vi='Lỗi ghi lại quốc gia trên thế giới %1 (mã %2) khi cập nhật bảng mã hiệu, %3'"),
...
Function RefByClassifier(Val Filter, Val AdditionalData = Undefined) Export
	...
	If SearchData=Undefined Then
		Raise NStr("en='Invalid world country classifier code';ru='Некорректный код страны мира для поиска в классификаторе';vi='Mã của quốc gia trên thế giới không đúng để tìm kiếm trong bảng mã hiệu'");
	EndIf;
