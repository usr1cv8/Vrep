...
Procedure SetAsContractByDefault(Command)
	...
	If CurrentListRow = Undefined Then
		...
		MessageText = NStr("en='Contract that should be set as the Default contract is not selected';ru='Не выбран договор, который необходимо установить как Договор по умолчанию';vi='Chưa chọn hợp đồng mà cần thiết lập như là Hợp đồng theo mặc định'");
...
Procedure ChangeCardOfCounterpartyAndChangeListAppearance(Counterparty, NewContractByDefault, ContractsListByDefault)
	...
	Except
		...
		MessageText = NStr("en='Cannot change the default contract in the counterparty card.';ru='Не удалось поменять договор по умолчанию в карточке контрагента.';vi='Không thể thay đổi hợp đồng theo mặc định trong thẻ đối tác.'");
		CommonUseClientServer.MessageToUser(MessageText);
