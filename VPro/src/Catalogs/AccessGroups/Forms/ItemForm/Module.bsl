...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CommonUseReUse.DataSeparationEnabled()
		...
		If Not ActionsWithServiceUser.ChangeAdmininstrativeAccess Then
			Raise
				NStr("en='Insufficient access rights to edit administrators.';ru='Не достаточно прав доступа для изменения состава администраторов.';vi='Không đủ quyền truy cập để thay đổi thành phần người quản trị.'");
		EndIf;
	EndIf;
...
""
"Установить профиль в группе доступа (нет - открыть только для просмотра)?';vi='Ở nhóm truy cập Người quản trị phải là hồ sơ Người quản trị."
""
"Đặt hồ sơ trong nhóm truy cập (không - chỉ mở để xem)?'"),
...
""
"Обновить виды доступа в группе доступа (нет - открыть только для просмотра)?';vi='Đã thay đổi thành phần dạng truy cập hồ sơ của nhóm truy cập này."
""
"Cập nhật dạng truy cập trong nhóm truy cập (không - chỉ mở để xem)?'"),
				QuestionDialogMode.YesNo,
				,
				DialogReturnCode.No);
...
"не влияет на права участников так как ее профиль помечен на удаление.';vi='Nhóm truy cập"
"không ảnh hưởng đến quyền của những người tham gia vì hồ sơ của nhóm đã bị đặt dấu xóa.'"));
	EndIf;
	...
	For Each CurrentRow IN TreeOfUsersRows Do
		...
		If Not ValueIsFilled(Participant) Then
			CommonUseClientServer.AddUserError(Errors,
				"GroupUsers[%1].User",
				ClarifyMessage(NStr("en='User is not selected.';ru='Пользователь не выбран.';vi='Người sử dụng chưa chọn.'"), Participant),
				"GroupUsers",
				LineNumber,
				ClarifyMessage(NStr("en='User in line %1 is not selected.';ru='Пользователь в строке %1 не выбран.';vi='Người sử dụng trong dòng %1 chưa chọn.'"), Participant));
			Continue;
		EndIf;
		...
		If FoundValues.Count() > 1 Then
			...
			If TypeOf(CurrentRow.User) = Type("CatalogRef.Users") Then
				SingleErrorText      = NStr("en='User ""%2"" is repeated.';ru='Пользователь ""%2"" повторяется.';vi='Người sử dụng ""%2"" bị lặp lại.'");
				SeveralErrorText = NStr("en='The ""%2"" user in line %1 is repeated.';ru='Пользователь ""%2"" в строке %1 повторяется.';vi='Người sử dụng ""%2"" trong dòng %1 bị lặp lại.'");
				
			ElsIf TypeOf(CurrentRow.User) = Type("CatalogRef.ExternalUsers") Then
				SingleErrorText      = NStr("en='External user ""%2"" is repeated.';ru='Внешний пользователь ""%2"" повторяется.';vi='Người sử dụng ngoài ""%2"" bị lặp lại.'");
				SeveralErrorText = NStr("en='External user ""%2"" in line %1 is repeated.';ru='Внешний пользователь ""%2"" в строке %1 повторяется.';vi='Người sử dụng ngoài ""%2"" tại dòng %1 bị lặp lại.'");
				
			ElsIf TypeOf(CurrentRow.User) = Type("CatalogRef.UsersGroups") Then
				SingleErrorText      = NStr("en='The ""%2"" user group is repeated.';ru='Группа пользователей ""%2"" повторяется.';vi='Nhóm người sử dụng ""%2"" bị lặp lại.'");
				SeveralErrorText = NStr("en='The ""%2"" user group in line %1 is repeated.';ru='Группа пользователей ""%2"" в строке %1 повторяется.';vi='Nhóm người sử dụng ""%2"" trong dòng %1 bị lặp lại.'");
			Else
				SingleErrorText      = NStr("en='External user group ""%2"" is repeated.';ru='Группа внешних пользователей ""%2"" повторяется.';vi='Nhóm người sử dụng ngoài ""%2"" bị lặp lại.'");
				SeveralErrorText = NStr("en='External user group ""%2"" in line %1 is repeated.';ru='Группа внешних пользователей ""%2"" в строке %1 повторяется.';vi='Nhóm người sử dụng ngoài ""%2"" tại dòng %1 bị lặp lại.'");
			EndIf;
		...
		If Object.Ref = Catalogs.AccessGroups.Administrators
		   AND TypeOf(CurrentRow.User) <> Type("CatalogRef.Users") Then
			...
			If TypeOf(CurrentRow.User) = Type("CatalogRef.ExternalUsers") Then
				SingleErrorText      = NStr("en='External user ""%2"" is invalid in predefined access group Administrators.';ru='Внешний пользователь ""%2"" недопустим в предопределенной группе доступа Администраторы.';vi='Người sử dụng ngoài ""%2"" không hợp lệ trong nhóm truy cập định trước Người quản trị.'");
				SeveralErrorText = NStr("en='External user ""%2"" in line %1 is invalid in predefined group Administrators.';ru='Внешний пользователь ""%2"" в строке %1 недопустим в предопределенной группе доступа Администраторы.';vi='Người sử dụng ngoài ""%2"" tại dòng %1 không khả dụng trong nhóm truy cập định trước Người quản trị.'");
				
			ElsIf TypeOf(CurrentRow.User) = Type("CatalogRef.UsersGroups") Then
				SingleErrorText      = NStr("en='The ""%2"" user group is invalid in predefined access group Administrators.';ru='Группа пользователей ""%2"" недопустима в предопределенной группе доступа Администраторы.';vi='Nhóm người sử dụng ""%2"" không khả dụng trong nhóm truy cập Người quản trị định trước.'");
				SeveralErrorText = NStr("en='The ""%2"" user group in line %1 is invalid in predefined access group Administrators.';ru='Группа пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';vi='Nhóm người sử dụng ""%2"" trong dòng %1 không khả dụng trong nhóm định trước truy cập Người quản trị.'");
			Else
				SingleErrorText      = NStr("en='External user group ""%2"" is invalid in predefined access group Administrators.';ru='Группа внешних пользователей ""%2"" недопустима в предопределенной группе доступа Администраторы.';vi='Nhóm người sử dụng ngoài ""%2"" không khả dụng trong nhóm truy cập định trước Người quản trị.'");
				SeveralErrorText = NStr("en='External user group ""%2"" in line %1 is invalid in predefined access group Administrators.';ru='Группа внешних пользователей ""%2"" в строке %1 недопустима в предопределенной группе доступа Администраторы.';vi='Nhóm người sử dụng ngoài ""%2"" tại dòng %1 không khả dụng trong nhóm truy cập Người quản trị định trước.'");
			EndIf;
		...
		If Object.UsersType <> Undefined Then
			...
			If TypeOf(CurrentRow.User) = Type("CatalogRef.Users") Then
				...
				If TypeOf(Object.UsersType) <> Type("CatalogRef.Users") Then
					SingleErrorText      = NStr("en='User ""%2"" is invalid for the selected participant type.';ru='Пользователь ""%2"" недопустим для указанного типа участников.';vi='Người sử dụng ""%2"" không hợp lệ đối với kiểu người tham gia đã chỉ ra.'");
					SeveralErrorText = NStr("en='User ""%2"" in row %1 is invalid for the selected participant type.';ru='Пользователь ""%2"" в строке %1 недопустим для указанного типа участников.';vi='Người sử dụng ""%2"" trong dòng %1 không hợp lệ đối với kiểu người tham gia đã chỉ ra.'");
				EndIf;
				
			ElsIf TypeOf(CurrentRow.User) = Type("CatalogRef.UsersGroups") Then
				
				If TypeOf(Object.UsersType) <> Type("CatalogRef.Users") Then
					SingleErrorText      = NStr("en='The ""%2"" user group is invalid for the specified participant type.';ru='Группа пользователей ""%2"" недопустима для указанного типа участников.';vi='Nhóm người sử dụng ""%2"" không khả dụng đối với kiểu người tham gia này.'");
					SeveralErrorText = NStr("en='The ""%2"" user group in line %1 is invalid for the specified participant type.';ru='Группа пользователей ""%2"" в строке %1 недопустима для указанного типа участников.';vi='Nhóm người sử dụng ""%2"" trong dòng %1 không khả dụng đối với kiểu người tham gia này.'");
				EndIf;
			Else
				...
				If TypeOf(CurrentRow.User) = Type("CatalogRef.ExternalUsers") Then
					...
					If DescriptionOfTheAuthorizationObjectType = Undefined
					 OR TypeOf(Object.UsersType) <> TypeOf(DescriptionOfTheAuthorizationObjectType.TypeOfAuthorizationObjects) Then
						...
						SingleErrorText      = NStr("en='External user ""%2"" is invalid for the specified type of participants.';ru='Внешний пользователь ""%2"" недопустим для указанного типа участников.';vi='Người sử dụng ngoài ""%2"" không hợp lệ đối với kiểu người tham gia đã chỉ ra.'");
						SeveralErrorText = NStr("en='External user ""%2"" in line %1 is invalid for the specified type of participants.';ru='Внешний пользователь ""%2"" в строке %1 недопустим для указанного типа участников.';vi='Người sử dụng ngoài ""%2"" tại dòng %1 không hợp lệ đối với kiểu người tham gia đã chỉ ra.'");
					EndIf;
					...
					If DescriptionOfTheAuthorizationObjectType = Undefined
					 ...
					 OR DescriptionOfTheAuthorizationObjectType.TypeOfAuthorizationObjects <> Undefined
					   AND TypeOf(Object.UsersType) <> TypeOf(DescriptionOfTheAuthorizationObjectType.TypeOfAuthorizationObjects) Then
						...
						SingleErrorText      = NStr("en='External user group ""%2"" is invalid for the specified participant type.';ru='Группа внешних пользователей ""%2"" недопустима для указанного типа участников.';vi='Nhóm người sử dụng ngoài ""%2"" không khả dụng đối với kiểu người tham gia đã chỉ ra.'");
						SeveralErrorText = NStr("en='External user group ""%2"" in line %1 is invalid for the specified participant type.';ru='Группа внешних пользователей ""%2"" в строке %1 недопустима для указанного типа участников.';vi='Nhóm người sử dụng ngoài ""%2"" tại dòng %1 không khả dụng đối với kiểu người tham gia đã chỉ ra.'");
					EndIf;
				EndIf;
			EndIf;
...
"Их нельзя удалить в этом списке.';vi='Người sử dụng"
"trong nhóm hiển thị để thông báo rằng, họ đang nhận quyền truy cập nhóm người sử dụng."
"Không thể xóa người sử dụng đó trong danh sách này.'"));
		Else
			ShowMessageBox(,
...
"Их нельзя удалить в этом списке.';vi='Người sử dụng ngoài"
"trong nhóm hiển thị để thông tin rằng, họ nhận quyền truy cập của nhóm người sử dụng ngoài."
"Không thể xóa những người sử dụng đó trong danh sách này.'"));
		EndIf;
	EndIf;
	...
	If CurrentObject.Ref <> Catalogs.AccessGroups.Administrators Then
		...
		If ValueIsFilled(CurrentObject.User) Then
			...
			Title
				...
				+ CurrentObject.User
				+ " "
				+ NStr("en='(Access group)';ru='(Группа доступа)';vi='(Nhóm truy cập)'");
...
Procedure FillListOfUserTypes()
	...
	UserTypes.Add(
		Undefined,
		NStr("en='Arbitrary participants';ru='Произвольные участники';vi='Người tham gia tùy ý'"));
	
	UserTypes.Add(
		Catalogs.Users.EmptyRef(),
		NStr("en='Common users';ru='Обычные пользователи';vi='Người sử dụng thường'"));
		...
		For Each AuthorizationObjectRefType IN TypesOfLinksAuthorizationObject Do
			...
			UserTypes.Add(
				...
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='External users (%1)';ru='Внешние пользователи (%1)';vi='Người sử dụng ngoài (%1)'"),
	...
	UsersTypePresentation =
		?(FoundItem = Undefined,
		  StringFunctionsClientServer.SubstituteParametersInString(
		      NStr("en='Unknown type ""%1""';ru='Неизвестный тип ""%1""';vi='Kiểu không xác định ""%1""'"),
...
Procedure PickSelectUsersEnd(SelectionAndPickOutOfExternalUsers, Pick) Export
	...
	If SelectionAndPickOutOfExternalUsers Then
		...
		Else
			ShowMessageBox(, NStr("en='Insufficient rights to select external users.';ru='Недостаточно прав для выбора внешних пользователей.';vi='Không đủ quyền để chọn người sử dụng ngoài.'"));
		EndIf;
	Else
...
Function SelectedMembersGroupAccess()
	...
	FormHeaderSelection = NStr("en='Select participants of access group';ru='Подбор участников группы доступа';vi='Chọn người tham gia vào nhóm truy cập'");
