...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Items.AccountSetupTitle.Title = ?(ContextMode,
		NStr("en='To send emails, configure the email account';ru='Для отправки писем необходимо настроить учетную запись электронной почты';vi='Để gửi E-mail, cần tùy chỉnh trương mục E-mail'"),
		NStr("en='Enter account parameters';ru='Введите параметры учетной записи';vi='Hãy nhập tham số trương mục E-mail'"));
		
	If Not ContextMode Then
		Title = NStr("en='Create email account';ru='Создание учетной записи электронной почты';vi='Tạo trương mục E-mail'");
	Else
		Title = NStr("en='Email account setting';ru='Настройка учетной записи электронной почты';vi='Tùy chỉnh trương mục E-mail'");
	EndIf;
...
Procedure ShowQuestionBoxBeforeClosingForm()
	QuestionText = NStr("en='Entered data is not written. Close the form?';ru='Введенные данные не записаны. Закрыть форму?';vi='Không ghi lại dữ liệu đã nhập. Đóng biểu mẫu?'");
	NotifyDescription = New NotifyDescription("FormClosingConfirmed", ThisObject);
	Buttons = New ValueList;
	Buttons.Add("Close", NStr("en='Close';ru='Закрыть';vi='Đóng'"));
	Buttons.Add(DialogReturnCode.Cancel, NStr("en='Keep open';ru='Не закрывать';vi='Không đóng'"));
	ShowQueryBox(NOTifyDescription, NStr("en='Entered data is not written. Close the form?';ru='Введенные данные не записаны. Закрыть форму?';vi='Không ghi lại dữ liệu đã nhập. Đóng biểu mẫu?'"), Buttons,,
		DialogReturnCode.Cancel, NStr("en='Account setting';ru='Настройка учетной записи';vi='Tùy chỉnh trương mục E-mail'"));
EndProcedure
	...
	If UseForSending Then
		Result.Add(
			WorkInSafeMode.PermissionForWebsiteUse(
				...
				NStr("en='Email.';ru='Эл. адрес.';vi='E-mail.'")));
	EndIf;
...
Procedure CheckFillingOnPageAccountSetting(Cancel)
	...
	If IsBlankString(EmailAddress) Then
		CommonUseClientServer.MessageToUser(NStr("en='Enter email address';ru='Введите адрес электронной почты';vi='Hãy nhập địa chỉ E-mail'"), , "EmailAddress", , Cancel);
	ElsIf Not CommonUseClientServer.EmailAddressMeetsRequirements(EmailAddress, True) Then
		CommonUseClientServer.MessageToUser(NStr("en='Email address is invalid';ru='Адрес электронной почты введен неверно';vi='Đã nhập sai địa chỉ E-mail'"), , "EmailAddress", , Cancel);
	EndIf;
	
	If IsBlankString(PasswordForReceivingEmails) Then
		CommonUseClientServer.MessageToUser(NStr("en='Enter a password';ru='Введите пароль';vi='Hãy nhập mật khẩu'"), , "PasswordForReceivingEmails", , Cancel);
	EndIf;
...
Procedure SetCurrentPageItems()
	...
	If CurrentPage = Items.AccountConfiguredSuccessfully Then
		If ContextMode Then
			NextButtonTitle = NStr("en='Continue';ru='Продолжить';vi='Tiếp tục'");
		Else
			NextButtonTitle = NStr("en='Close';ru='Закрыть';vi='Đóng'");
		EndIf;
	Else
		If CurrentPage = Items.AccountSetup
			AND CheckingCompletedWithErrors Then
				NextButtonTitle = NStr("en='Repeat';ru='Повторить';vi='Lặp lại'");
		ElsIf CurrentPage = Items.AccountSetup
			AND SettingsMethod = "automatically" Then
			If ContextMode Then
				NextButtonTitle = NStr("en='Customize';ru='Настроить';vi='Tùy chỉnh'");
			Else
				NextButtonTitle = NStr("en='Generate';ru='Сформировать отчет';vi='Tạo mới'");
			EndIf;
		Else
			NextButtonTitle = NStr("en='Next >';ru='Далее  >';vi='Tiếp theo >'");
		EndIf;
	EndIf;
	...
	If Not ContextMode AND CurrentPage = Items.AccountConfiguredSuccessfully Then
		Items.GoToSettingsButton.Title = NStr("en='Go to account';ru='Перейти к учетной записи';vi='Chuyển đến trương mục E-mail'");
	Else
		Items.GoToSettingsButton.Title = NStr("en='Configure connection parameters manually';ru='Настроить параметры подключения вручную';vi='Tùy chỉnh tham số kết nối thủ công'");
	EndIf;
...
"параметров учетной записи %1 завершена.';vi='Đã kết thúc"
"tùy chỉnh tham số trương mục E-mail %1.'"), EmailAddress);
	EndIf;
...
Function CheckConnectionToOutgoingMailServer()
	...
	Subject = NStr("en='1C:Enterprise test message';ru='Тестовое сообщение 1С:Предприятие';vi='Thông báo 1C:DOANH NGHIỆP'");
	Body = NStr("en='This message is sent by subsystem of 1C:Enterprise email';ru='Это сообщение отправлено подсистемой электронной почты 1С:Предприятие';vi='Thông báo này đã gửi bởi phân hệ thư điện tử 1C:DOANH NGHIỆP'");
...
Procedure CheckAccountSettings()
	...
	If Not IsBlankString(OutgoingMailServerMessage) Then
		ErrorText = NStr("en='Cannot connect to the outgoing email server:';ru='Не удалось подключиться к серверу исходящей почты:';vi='Không thể kết nối đến Server thư đi:'" + Chars.LF)
			+ OutgoingMailServerMessage + Chars.LF;
	EndIf;
	
	If Not IsBlankString(IncomingMailServerMessage) Then
		ErrorText = ErrorText
			+ NStr("en='Cannot connect to the incoming email server:';ru='Не удалось подключиться к серверу входящей почты:';vi='Không thể kết nối đến Server thư đến:'" + Chars.LF)
			+ IncomingMailServerMessage;
	EndIf;
...
"Настройте параметры подключения вручную.';vi='Không thể xác định tùy chỉnh kết nối."
"Hãy tùy chỉnh tham số kết nối thủ công.'");
