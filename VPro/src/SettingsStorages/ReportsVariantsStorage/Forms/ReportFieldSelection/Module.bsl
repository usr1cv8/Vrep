...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Parameters.Property("Mode", Mode) Then
		Raise NStr("en='Service parameter ""Mode"" is not transferred.';ru='Не передан служебный параметр ""Режим"".';vi='Chưa truyền tham số hệ thống ""Mode"".'");
	EndIf;
	...
	If Mode <> "Filters" AND Mode <> "SelectedFields" AND Mode <> "Sort" AND Mode <> "GroupFields" Then
		Raise StrReplace(NStr("en='Incorrect value of the ""Mode"" parameter: ""%1"".';ru='Некорретное значение параметра ""Режим"": ""%1"".';vi='Sai giá trị tham số ""Chế độ"": ""%1"".'"), "%1", String(Mode));
	EndIf;
	...
	If Mode = "GroupFields" Then
		...
		StringDetails.Presentation = NStr("en='<Detailed records>';ru='<Детальные записи>';vi='<Bản ghi chi tiết>'");
