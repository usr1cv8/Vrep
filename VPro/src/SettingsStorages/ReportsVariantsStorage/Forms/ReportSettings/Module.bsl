...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Parameters.Property("SettingsComposer", SettingsComposer) Then
		Raise NStr("en='The SettingsLinker service parameter is not passed.';ru='Не передан служебный параметр ""КомпоновщикНастроек"".';vi='Chưa truyền tham số hệ thống ""SettingsLinker"".'");
	EndIf;
	If Not Parameters.Property("ReportSettings", ReportSettings) Then
		Raise NStr("en='Service parameter ReportSettings is not passed.';ru='Не передан служебный параметр ""НастройкиОтчета"".';vi='Chưa truyền tham số hệ thống ""ReportSettings"".'");
	EndIf;
	If Not Parameters.Property("OptionName", OptionName) Then
		Raise NStr("en='Service parameter VariantName is not passed.';ru='Не передан служебный параметр ""ВариантНаименование"".';vi='Chưa truyền tham số hệ thống ""VariantName"".'");
	EndIf;
	...
	If CurrentCDHostIdentifier <> Undefined Then
		...
		If Not Parameters.Property("Title", Title) Then
			Raise NStr("en='Service parameter ""Title"" is not transferred.';ru='Не передан служебный параметр ""Заголовок"".';vi='Chưa truyền tham số hệ thống ""Title"".'");
		EndIf;
		If Not Parameters.Property("CurrentCDHostType", CurrentCDHostType) Then
			Raise NStr("en='Service parameter CurrentDCNode is not passed.';ru='Не передан служебный параметр ""ТипТекущегоУзлаКД"".';vi='Chưa truyền tham số hệ thống ""CurrentDCNode"".'");
		EndIf;
	Else
		If Not ValueIsFilled(OptionName) Then
			OptionName = ReportSettings.Description;
		EndIf;
		Title = NStr("en='Report settings';ru='Настройки отчета';vi='Tùy chỉnh báo cáo'") + " """ + OptionName + """";
	EndIf;
	...
	Else
		For Each ListItemDoubleInForm IN ValuesListInForm Do
			If ListItemDoubleInForm.Value = Value AND ListItemDoubleInForm <> ListItemInForm Then
				Status(NStr("en='Duplicate records are detected. Editing canceled.';ru='Обнаружены дублирующиеся записи. Редактирование отменено.';vi='Đã tìm thấy các bản ghi trùng lặp. Đã hủy soạn.'"));
...
Procedure VariantStructureAddGroupingAfterFieldSelection(AvailableDCField, ExecuteParameters) Export
	...
	If AvailableDCField = "<>" Then
		...
		Presentation = NStr("en='<Detailed records>';ru='<Детальные записи>';vi='<Bản ghi chi tiết>'");
	Else
...
Procedure SelectPeriodFromDropdownList(Result, ChoiceParameters) Export
	...
	If ChoiceParameters.Variant = Undefined Or ChoiceParameters.Variant = StandardPeriodVariant.Custom Then
		...
		PeriodsList.Add(NavigationItemDescription, NStr("en='Relative...';ru='Относительный...';vi='Tương đối...'"));
	...
	Else
		...
		NavigationItemDescription.Insert("Variant",                  Undefined);
		NavigationItemDescription.Insert("InitialValueIndex", Undefined);
		PeriodsList.Add(NavigationItemDescription, NStr("en='Fixed...';ru='Фиксированный...';vi='Xác định...'"));
...
Function ExecuteStep(CaseUser, Script)
	...
	Else
		Script.ErrorText = StrReplace(NStr("en='Error in script: Unknown step ""%1"".';ru='Ошибка в сценарии: Неизвестный шаг ""%1"".';vi='Lỗi trong kịch bản: Bước không xác định ""%1"".'"), "%1", Script.CurrentStep);
		Return False;
...
Function FieldsTablesDefineSelectedRows(CaseUser, Script)
	...
	For Each RowIdentifier IN SelectedRows Do
		...
		If Script.TableName = "Filters" Then
			...
			ElsIf TreeRow.IsParameter Then
				If Script.Action = "Move" Then
					Script.ErrorText = StrReplace(NStr("en='Parameters cannot be transferred.';ru='Параметры не могут быть перемещены.';vi='Tham số không thể bị dịch chuyển.'"), "%1", TreeRow.Presentation);
				ElsIf Script.Action = "Group" Then
					Script.ErrorText = NStr("en='Parameters cannot be group participants.';ru='Параметры не могут быть участниками групп.';vi='Tham số không thể là thành phần của nhóm.'");
				ElsIf Script.Action = "Delete" Then
					Script.ErrorText = NStr("en='Parameters cannot be deleted.';ru='Параметры не могут быть удалены.';vi='Tham số không thể bị xóa'");
				EndIf;
				Return False; // Pause script.
			EndIf;
		EndIf;
		If Script.Action = "Move" Or Script.Action = "Group" Then
			...
			ElsIf Script.CurrentParent <> Parent Then
				If Script.Action = "Move" Then
					Script.ErrorText = NStr("en='The selected items cannot be transferred as they have different parents.';ru='Выбранные элементы не могут быть перемещены, поскольку они принадлежат разным родителям.';vi='Không thể điều chuyển các phần tử đã chọn bởi vì chúng thuộc các lớp trên khác nhau.'");
				ElsIf Script.Action = "Group" Then
					Script.ErrorText = NStr("en='The selected items cannot be grouped as they have different parents.';ru='Выбранные элементы не могут быть сгруппированы, поскольку они принадлежат разным родителям.';vi='Không thể gom nhóm các phần tử đã chọn vì chúng thuộc các phần tử khác nhau.'");
				EndIf;
				Return False; // Pause script.
			EndIf;
		EndIf;
	...
	If Script.TreeRows.Count() = 0 Then
		Script.ErrorText = NStr("en='Select items.';ru='Выберите элементы.';vi='Hãy chọn phần tử.'");
		Return False; // Pause script.
	EndIf;
...
Function InputTitle(CaseUser, Script)
	...
	If Script.CurrentStepRefiner = "" Then // First call
		...
		ShowInputString(Script.Handler, , NStr("en='Group name';ru='Название группы';vi='Tên gọi của nhóm'"), 100, False);
...
Procedure FiltersSelectAccessLevel(TableName, RowIdentifier, TableRow)
	...
	List = New ValueList;
	List.Add(2, NStr("en='In report header';ru='В шапке отчета';vi='Trong phần đầu báo cáo'"), , PictureLib.QuickAccess);
	If Not TableRow.IsParameter Then
		List.Add(1, NStr("en='Only check box in report title';ru='Только флажок в шапке отчета';vi='Chỉ các hộp kiểm trong phần đầu báo cáo'"), , PictureLib.RapidAccessWithCheckBox);
	EndIf;
	List.Add(4, NStr("en='In report settings';ru='В настройках отчета';vi='Trong các tùy chỉnh báo cáo'"), , PictureLib.Attribute);
	If Not TableRow.IsParameter Then
		List.Add(3, NStr("en='Only check box in report settings';ru='Только флажок в настройках отчета';vi='Chỉ các hộp kiểm trong tùy chỉnh báo cáo'"), , PictureLib.UsualAccessWithCheckBox);
	EndIf;
	List.Add(5, NStr("en='Do not show';ru='Не показывать';vi='Không hiển thị'"), , PictureLib.ReportHiddenSetting);
...
Procedure FiltersSelectComparisonType(TableRow)
	...
	If TypeInformation.LimitedLength Then
		...
		If TypeInformation.ContainsObjectTypes Then
			...
			List.Add(DataCompositionComparisonType.InListByHierarchy); // NStr("en='In list including subordinate';ru='В списке включая подчиненные';vi='Trong danh sách bao gồm trực thuộc'")
			List.Add(DataCompositionComparisonType.NotInListByHierarchy); // NStr("en='Not in the list, including subordinate';ru='Не в списке включая подчиненные';vi='Không bao gồm trực thuộc trong danh sách'").
			
			List.Add(DataCompositionComparisonType.InHierarchy); // NStr("en='In group';ru='В группе';vi='Trong nhóm'")
			List.Add(DataCompositionComparisonType.NotInHierarchy); // NStr("en='Not in group';ru='Не в группе';vi='Không trong nhóm'")
...
Procedure FieldsTablesUngroup(TableName)
	...
	If SelectedRows.Count() <> 1 Then
		ShowMessageBox(, NStr("en='Select a group.';ru='Выберите одну группу.';vi='Hãy chọn một nhóm.'"));
		Return;
	EndIf;
	
	TreeGroup = TableAttribute.FindByID(SelectedRows[0]);
	If TreeGroup = Undefined Or Not TreeGroup.IsFolder Then
		ShowMessageBox(, NStr("en='Select group.';ru='Выберите группу.';vi='Hãy chọn nhóm'"));
		Return;
	EndIf;
...
Procedure FieldsTablesChangeNodeVariantStructure(TableName, RowIdentifier, TableRow)
	...
	If TableRow.Type = "Chart" Then
		FormParameters.Insert("Title", NStr("en='Set up chart of the %2 report';ru='Настройка диаграммы отчета ""%2""';vi='Tùy chỉnh biểu đồ báo cáo ""%2""'"));
	Else
		FormParameters.Insert("Title", NStr("en='Set up %1 grouping of the %2 report';ru='Настройка группировки ""%1"" отчета ""%2""';vi='Tùy chỉnh gom nhóm ""%1"" báo cáo ""%2""'"));
	EndIf;
...
Function FieldsTablesFindNode(ThisObject, TableName, TableRow = Undefined, DCNodeIdentifier = Undefined)
	If ThisObject.ExtendedMode = 1 Then
		...
		ElsIf TableName = "VariantStructure" Then
			Return RootNode;
		Else
			Raise StrReplace(NStr("en='Changing of nodes of the ""%1"" table is not supported.';ru='Изменение узлов таблицы ""%1"" не поддерживается.';vi='Không hỗ trợ thay đổi các nút của bảng ""%1"".'"), "%1", TableName);
		EndIf;
	Else
...
Procedure VisibleEnabledCorrectness(EventName = "")
	...
	If EventName = "OnCreateAtServer" Then
		If VariantNodeChangingMode Then
			...
			Items.CloseAndGenerate.Title = NStr("en='Complete';ru='Закончить редактирование';vi='Kết thúc soạn'");
...
Procedure QuickSettingsCreateControlItemsAndLoadValues(FillingParameters, Information)
	...
	For Each BookmarkDescription IN OutputGroups.AdditionalBookmarks Do
		...
		If OutputGroup.Size > 0 Then
			...
			Columns.Group         = ChildFormItemsGroup.HorizontalIfPossible;
	...
	For Each SettingProperty IN AddedTablesWithFlags Do
		...
		ElsIf SettingProperty.Type = "SettingsStructure" Then
			...
			For Each StructureItem IN UserSetting.Structure Do
				...
				Else
					Presentation = NStr("en='Table / chart';ru='Таблица / диаграмма';vi='Bảng / biểu đồ'");
				EndIf;
...
Procedure AdvancedSettingsLoadValues(FillingParameters, Information)
	...
	If Not VariantNodeChangingMode AND RootRow.Type <> "NestedObjectSettings" Then
		...
		ParentRows.Clear();
		RootRow.Title = "";
		RootRow.Presentation = NStr("en='Report';ru='Отчет';vi='Báo cáo'");
...
Procedure RegisterParameters(Information)
	...
	RowSection.Presentation = NStr("en='Parameters';ru='Параметры';vi='Tham số'");
...
Procedure RegisterFilterItems(Information, RowsSetVariant = Undefined, FormRowsSet = Undefined)
	...
	If FormRowsSet = Undefined Then
		...
		If Not VariantNodeChangingMode Then
			...
			RowSection.ThisIsSection = True;
			RowSection.Presentation = NStr("en='Filters';ru='Отборы';vi='Lọc'");
