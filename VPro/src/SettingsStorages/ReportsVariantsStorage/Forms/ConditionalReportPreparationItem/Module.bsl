...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Parameters.Property("CurrentCDHostIdentifier", CurrentCDHostIdentifier) Then
		Raise NStr("en='Service parameter CurrentKDNodeIdentifier is not passed.';ru='Не передан служебный параметр ""ИдентификаторТекущегоУзлаКД"".';vi='Chưa truyền tham số hệ thống ""CurrentKDNodeIdentifier"".'");
	EndIf;
	If Not Parameters.Property("DCIdentifier", DCIdentifier) Then
		Raise NStr("en='Service parameter DCIdentifier is not passed.';ru='Не передан служебный параметр ""ИдентификаторКД"".';vi='Chưa truyền tham số hệ thống ""DCIdentifier"".'");
	EndIf;
	...
	If KDNode = Undefined Then
		Raise NStr("en='Report node not found.';ru='Не найден узел отчета.';vi='Không tìm thấy nút báo cáo.'");
	EndIf;
	KDItem = KDNode.GetObjectByID(DCIdentifier);
	If KDItem = Undefined Then
		Raise NStr("en='Item of conditional appearance is not found.';ru='Не найден элемент условного оформления.';vi='Không tìm thấy phần tử trang trí quy ước'");
	EndIf;
