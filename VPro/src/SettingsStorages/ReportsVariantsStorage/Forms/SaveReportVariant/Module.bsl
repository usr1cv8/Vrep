...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	If Not ValueIsFilled(OptionName) Then
		CommonUseClientServer.MessageToUser(
			NStr("en='Name is not populated';ru='Поле ""Наименование"" не заполнено';vi='Chưa điền trường  ""Tên gọi"".'"),
			,
			"Description");
		Cancel = True;
	ElsIf ReportsVariants.DescriptionIsBooked(Context.ReportRef, VariantRef, OptionName) Then
		CommonUseClientServer.MessageToUser(
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='""%1"" is already used, enter another Name.';ru='""%1"" занято, необходимо указать другое Наименование.';vi='""%1"" đã bị chiếm, cần chỉ ra Tên gọi khác."
"'"),
				OptionName
			),
			,
...
Procedure DescriptionStartChoice(Item, ChoiceData, StandardProcessing)
	...
	CommonUseClient.ShowMultilineTextEditingForm(Notification, Items.Definition.EditText,
		NStr("en='Definitions';ru='Определение';vi='Xác định'"));
EndProcedure
...
Procedure VariantsOfReportBeforeDelition(Item, Cancel)
	...
	If Not Context.FullRightsForVariants AND Not Variant.AuthorCurrentUser Then
		WarningText = NStr("en='Insufficient access rights to delete the report variant ""%1"".';ru='Недостаточно прав доступа для удаления варианта отчета ""%1"".';vi='Không đủ quyền truy cập để xóa phương án báo cáo ""%1"".'");
	...
	If Not Variant.User Then
		ShowMessageBox(, NStr("en='Cannot delete the predefined report variant.';ru='Невозможно удалить предопределенный вариант отчета.';vi='Không thể xóa phương án báo cáo định trước.'"));
		Return;
	EndIf;
...
Procedure ExecuteBatch(Result, Package) Export
	...
	If Package.CheckPage1 = True Then
		...
		If Not ValueIsFilled(OptionName) Then
			ErrorText = NStr("en='Name is not populated';ru='Поле ""Наименование"" не заполнено';vi='Chưa điền trường  ""Tên gọi"".'");
		...
		If Not Package.VariantIsNew Then
			...
			If Not RightVariantModification(Variant, RightVariantSettings(Variant, Context.FullRightsForVariants)) Then
				ErrorText = NStr("en='Insufficient rights to change variant ""%1"". Select another variant or change the name.';ru='Недостаточно прав для изменения варианта ""%1"", необходимо выбрать другой вариант или изменить Наименование.';vi='Không đủ quyền để thay đổi phương án ""%1"", cần chọn phương án khác hoặc thay đổi Tên gọi.'");
...
"Заменить помеченный на удаление вариант отчета?';vi='Đánh dấu xóa phương án báo cáo ""%1"". "
"Thay thế phương án báo cáo bị đánh dấu xóa?'");
					DefaultButton = DialogReturnCode.No;
				Else
					QuestionText = NStr("en='Replace a previously saved variant of report ""%1""?';ru='Заменить ранее сохраненный вариант отчета ""%1""?';vi='Thay thế phương án báo cáo ""%1"" đã lưu trước đó?'");
...
Procedure OpenVariantForModification()
	...
	If Not RightVariantSettings(Variant, Context.FullRightsForVariants) Then
		WarningText = NStr("en='Insufficient access rights to change variant ""%1"".';ru='Недостаточно прав доступа для изменения варианта ""%1"".';vi='Không đủ quyền truy cập để thay đổi phương án ""%1"".'");
...
Function GenerateFreeName(Variant, ReportVariants)
	VariantNameTemplate = TrimAll(Variant.Description) +" - "+ NStr("en='copy';ru='копия';vi='sao chép'");
...
Procedure CheckAndWriteOnServer(Package)
	...
	If VariantIsNew AND ReportsVariants.DescriptionIsBooked(Context.ReportRef, VariantRef, OptionName) Then
		ErrorText = NStr("en='""%1"" is already used, enter another Name.';ru='""%1"" занято, необходимо указать другое Наименование.';vi='""%1"" đã bị chiếm, cần chỉ ra Tên gọi khác."
"'");
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(ErrorText, OptionName);
		CommonUseClientServer.MessageToUser(ErrorText, , "OptionName");
		Package.Cancel = True;
...
"список предопределенных вариантов внешнего отчета ""%1"":';vi='Không thể nhận"
"danh sách các phương án báo cáo ngoài định trước ""%1"":'"),
					Context.ReportRef
