...
"Выбрать этот варианта отчета?';vi='Phương án đã chọn của báo cáo đã bị đặt dấu xóa.'");
	...
	For Each TableRow IN VariantTable Do
		...
		If TreeRowSet = Undefined Then
			...
			If TableRow.GroupNumber = 2 Then
				TreeGroup.Description = NStr("en='Hidden in report panels';ru='Скрытые в панелях отчетов';vi='Ẩn trên thanh lệnh báo cáo'");
				TreeGroup.PictureIndex = 0;
				TreeGroup.AuthorPicture = -1;
			ElsIf TableRow.GroupNumber = 3 Then
				TreeGroup.Description = NStr("en='Marked for deletion';ru='Помеченные на удаление';vi='Đánh dấu xóa'");
