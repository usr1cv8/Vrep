...
Function CheckConnection(ExchangePlanName, NodeCode, Result)
	...
	If ExchangePlans[ExchangePlanName].FindByCode(NodeCode).IsEmpty() Then
		Result = NStr("en='Configuring data synchronization is disabled online by the application administrator.';ru='Возможность синхронизации данных отключена администратором приложения в Интернете.';vi='Người quản trị ứng dụng trên Internet ngắt bỏ tính năng đồng bộ hóa dữ liệu.'");
		Return False;
	EndIf;
...
Procedure ValidateLockInformationBaseForUpdate()
	...
	If ValueIsFilled(InfobaseUpdateService.InfobaseLockedForUpdate()) Then
		...
		Raise NStr("en='Data synchronization is temporary unavailable due to online application update.';ru='Синхронизация данных временно недоступна в связи с обновлением приложения в Интернете.';vi='Tạm thời không cho phép đồng bộ hóa dữ liệu do cập nhật ứng dụng trên Internet.'");
