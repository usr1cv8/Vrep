	...
	Try
		...
		If CommonUseReUse.DataSeparationEnabled() Then
			...
			If CommonUseReUse.CanUseSeparatedData() Then
				...
				Raise NStr("en='Operation cannot be called in session which is started with specified separators. Verify that the web service is published correctly.';ru='Операция не может быть вызвана в сеансе, который запущен с указанием разделителей. Проверьте правильность публикации веб-сервиса!';vi='Không thể gọi ra giao dịch trong phiên làm việc được khởi động có chỉ ra đường chia tách. Hãy kiểm tra tính đúng đắng khi đăng tải Web-service!'");
			...
			Else
				...
				If DataAreaKey <> Constants.DataAreaKey.Get() Then
					Raise NStr("en='Incorrect data area key.';ru='Некорректный ключ области данных!';vi='Sai khóa vùng dữ liệu!'");
				EndIf;
		...
		Else
			Raise NStr("en='The operation cannot be run for the infobase in which separation by data areas is disabled.';ru='Операция не может быть выполнена для информационной базы, в которой отключено разделение по областям данных!';vi='Không thể thực hiện giao dịch đối với cơ sở thông tin mà trong đó đã ngắt phân chia theo các vùng dữ liệu!'");
		EndIf;
