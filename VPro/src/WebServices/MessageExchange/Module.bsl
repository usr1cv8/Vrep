...
Function DeliverMessages(SenderCode, StreamStorage)
	...
	If Sender.IsEmpty() Then
		...
		Raise NStr("en='Invalid endpoint connection settings.';ru='Заданы неправильные настройки подключения к конечной точке.';vi='Chỉ ra sai tùy chỉnh kết nối tới điểm cuối.'");
...
""
"Эти сообщения будут обработаны в рамках очереди обработки сообщений системы';vi='Đã xảy ra lỗi khi gửi và nhận thông báo nhanh - nhiều thông báo chưa được gửi do đã đặt phong tỏa vùng dữ liệu!"
""
"Những thông báo này sẽ được xử lý trong khuôn khổ trình tự xử lý thông báo hệ thống'");
...
Function CheckConnectionAtSender(SenderCode)
	...
	If MessageExchangeInternal.ThisNodeCode() <> SenderCode Then
		...
		Raise NStr("en='Receiver base connection settings indicate the another sender.';ru='Настройки подключения базы получателя указывают на другого отправителя.';vi='Tùy chỉnh kết nối cơ sở nơi nhận chỉ ra cho nơi gửi khác.'");
