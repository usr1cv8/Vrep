...
"click the ""Access rights"" link, and then click ""Access rights report"".';ru='Чтобы открыть отчет откройте карточку пользователя,"
"перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".';vi='Để mở báo cáo, hãy mở thẻ người sử dụng,"
"chuyển đến tham chiếu ""Quyền truy cập"" và nhấn nút ""Báo cáo theo quyền truy cập"".'");
		Else
			Raise
				NStr("en='To open report, open the user card or user group card,"
"click the ""Access rights"" link, and then click ""Access rights report"".';ru='Чтобы открыть отчет откройте карточку пользователя или группы пользователей,"
"перейдите по ссылке ""Права доступа"", нажмите на кнопку ""Отчет по правам доступа"".';vi='Để mở báo cáo, hãy mở thẻ người sử dụng hoặc nhóm người sử dụng,"
"chuyển đến tham chiếu ""Quyền truy cập"" và nhấn nút ""Báo cáo theo quyền truy cập"".'");
		EndIf;
	EndIf;
	...
	If Parameters.User <> Users.AuthorizedUser()
	   AND Not Users.InfobaseUserWithFullAccess() Then
		...
		Raise NStr("en='Insufficient rights to view the report.';ru='Недостаточно прав для просмотра отчета.';vi='Không đủ quyền để xem báo cáo.'");
	EndIf;
...
Procedure OutputReport(Ref)
	...
	If TypeOf(Ref) = Type("CatalogRef.Users") Then
		Properties.Insert("ReportHeader",				NStr("en='User rights report';ru='Отчет по правам пользователя';vi='Báo cáo theo quy tắc của người sử dụng'"));
		Properties.Insert("RolesByProfilesGrouping",	NStr("en='User roles by profiles';ru='Роли пользователя по профилям';vi='Vai trò của người sử dụng theo hồ sơ'"));
		Properties.Insert("ObjectPresentation",			NStr("en='User: %1';ru='Пользователь: %1';vi='Người sử dụng: %1'"));
		
	ElsIf TypeOf(Ref) = Type("CatalogRef.ExternalUsers") Then
		Properties.Insert("ReportHeader",				NStr("en='External user rights report';ru='Отчет по правам внешнего пользователя';vi='Báo cáo theo quy tắc của người sử dụng ngoài'"));
		Properties.Insert("RolesByProfilesGrouping",	NStr("en='Roles of external user by profiles';ru='Роли внешнего пользователя по профилям';vi='Vai trò của người sử dụng ngoài theo hồ sơ'"));
		Properties.Insert("ObjectPresentation",			NStr("en='External user: %1';ru='Внешний пользователь: %1';vi='Người sử dụng ngoài: %1'"));
		
	ElsIf TypeOf(Ref) = Type("CatalogRef.UsersGroups") Then
		Properties.Insert("ReportHeader",				NStr("en='User group rights report';ru='Отчет по правам группы пользователей';vi='Báo cáo theo quy tắc của nhóm người sử dụng'"));
		Properties.Insert("RolesByProfilesGrouping",	NStr("en='User group roles by profiles';ru='Роли группы пользователей по профилям';vi='Vai trò của nhóm người sử dụng theo hồ sơ'"));
		Properties.Insert("ObjectPresentation",			NStr("en='User group: %1';ru='Группа пользователей: %1';vi='Nhóm người sử dụng: %1'"));
	Else
		Properties.Insert("ReportHeader",				NStr("en='Report on group rights of external users';ru='Отчет по правам группы внешних пользователей';vi='Báo cáo theo quy tắc của nhóm người sử dụng ngoài'"));
		Properties.Insert("RolesByProfilesGrouping",	NStr("en='Roles of external user group by profiles';ru='Роли группы внешних пользователей по профилям';vi='Vai trò của nhóm người sử dụng ngoài theo hồ sơ'"));
		Properties.Insert("ObjectPresentation",			NStr("en='External user group: %1';ru='Группа внешних пользователей: %1';vi='Nhóm người sử dụng ngoài: %1'"));
	EndIf;
	...
	If Not OutputGroupRights Then
		...
		If IBUserHasBeenRead Then
			...
			If Not ValueIsFilled(InfobaseUserProperties.OSUser) Then
				Area.Parameters.OSUser = NStr("en='Not specified';ru='Не указан';vi='Chưa chỉ ra'");
			EndIf;
			Document.Put(Area, 2);
		Else
	...
	If DetailedInformationAboutAccessRights Then
		...
		If OnePersonalGroup Then
			If TypeOf(Ref) = Type("CatalogRef.Users") Then
				AccessPresentation = NStr("en='User access limitations';ru='Ограничения доступа пользователя';vi='Hạn chế truy cập của người sử dụng'");
				
			ElsIf TypeOf(Ref) = Type("CatalogRef.ExternalUsers") Then
				AccessPresentation = NStr("en='Access restrictions of external user';ru='Ограничения доступа внешнего пользователя';vi='Hạn chế truy cập của người sử dụng ngoài'");
				
			ElsIf TypeOf(Ref) = Type("CatalogRef.UsersGroups") Then
				AccessPresentation = NStr("en='Access restrictions of user group';ru='Ограничения доступа группы пользователей';vi='Hạn chế truy cập của nhóm người sử dụng'");
			Else
				AccessPresentation = NStr("en='Access restrictions of external user group';ru='Ограничения доступа группы внешних пользователей';vi='Hạn chế truy cập của nhóm người sử dụng ngoài'");
			EndIf;
		Else
			If TypeOf(Ref) = Type("CatalogRef.Users") Then
				AccessPresentation = NStr("en='User access groups';ru='Группы доступа пользователя';vi='Nhóm truy cập người sử dụng'");
				
			ElsIf TypeOf(Ref) = Type("CatalogRef.ExternalUsers") Then
				AccessPresentation = NStr("en='Groups of external user access';ru='Группы доступа внешнего пользователя';vi='Nhóm truy cập người sử dụng ngoài'");
				
			ElsIf TypeOf(Ref) = Type("CatalogRef.UsersGroups") Then
				AccessPresentation = NStr("en='Access groups user groups';ru='Группы доступа группы пользователей';vi='Nhóm truy cập nhóm người sử dụng'");
			Else
				AccessPresentation = NStr("en='Access groups external user groups';ru='Группы доступа группы внешних пользователей';vi='Nhóm truy cập nhóm người sử dụng ngoài'");
			EndIf;
		EndIf;
	...
	Area.Parameters.ObjectRightsGroupingPresentation = NStr("en='View objects';ru='Просмотр объектов';vi='Xem xét đối tượng'");
	Document.Put(Area, 1);
	...
	For Each ObjectsKindDescriptionFull IN ObjectRights.Rows Do
		...
		If SimplifiedInterface Then
			Area.Parameters.ProfilePresentationsOrAccessGroups = NStr("en='Profiles';ru='Профили';vi='Hồ sơ'");
		Else
			Area.Parameters.ProfilePresentationsOrAccessGroups = NStr("en='Access groups';ru='Группы доступа';vi='Nhóm truy cập'");
		EndIf;
		...
		If DetailedInformationAboutAccessRights Then
			Area.Parameters.ProfilePresentationsOrAccessGroups = NStr("en='(profile, roles)';ru='(профиль, роли)';vi='(hồ sơ, vai trò)'");
		Else
		...
		For Each ObjectDescription IN ObjectsKindDescriptionFull.Rows Do
			...
			If ObjectDescription.ReadingNotLimited Then
				If ObjectDescription.view Then
					ObjectPresentationClarification = NStr("en='(view, not limited)';ru='(просмотр, не ограничен)';vi='(xem, không hạn chế)'");
				Else
					ObjectPresentationClarification = NStr("en='(view*, not limited)';ru='(просмотр*, не ограничен)';vi='(xem*, không hạn chế)'");
				EndIf;
			Else
				If ObjectDescription.view Then
					ObjectPresentationClarification = NStr("en='(view, limited)';ru='(просмотр, ограничен)';vi='(xem, hạn chế)'");
				Else
					ObjectPresentationClarification = NStr("en='(view*, limited)';ru='(просмотр*, ограничен)';vi='(xem*, hạn chế)'");
				EndIf;
			EndIf;
...
"%2)'")
	...
	Area = Template.GetArea("ObjectRightsGrouping");
	Area.Parameters.ObjectRightsGroupingPresentation = NStr("en='Object editing';ru='Редактирование объектов';vi='Soạn đối tượng'");
	Document.Put(Area, 1);
...
"modification, not limited)';ru='(добавление, не ограничено"
"изменение, не ограничено)';vi='(thêm, không hạn chế"
"thay đổi, không hạn chế)'");
						ElsIf Not ObjectDescription.InteractiveInsert AND ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en='(adding*, not limited"
"modification, not limited)';ru='(добавление*, не ограничено"
"изменение, не ограничено)';vi='(thay đổi*, không hạn chế"
"thay đổi, không hạn chế)'");
						ElsIf ObjectDescription.InteractiveInsert AND Not ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en='(adding,  not limited"
"modification*, not limited)';ru='(добавление, не ограничено"
"изменение*, не ограничено)';vi='(thêmg, không hạn chế"
"thay đổi*, không hạn chế)'");
...
"modification, not limited)';ru='(добавление, ограничено"
"изменение, не ограничено)';vi='(thêm, hạn chế"
"thay đổi, không hạn chế)'");
						ElsIf Not ObjectDescription.InteractiveInsert AND ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en='(adding*, limited"
"modification, not limited)';ru='(добавление*, ограничено"
...
"thay đổi*, không hạn chế)'");
					...
					ElsIf ObjectDescription.AddingNotLimited AND Not ObjectDescription.ChangingNotLimited Then
						If ObjectDescription.InteractiveInsert AND ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en='(adding, not limited"
"modification, limited)';ru='(добавление, не ограничено"
"изменение, ограничено)';vi='(thêm, không hạn chế"
"thay đổi, hạn chế)'");
						ElsIf Not ObjectDescription.InteractiveInsert AND ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en='(adding*, not limited"
"modification, limited)';ru='(добавление*, не ограничено"
...
"thay đổi*, hạn chế)'");
						Else // NO ObjectDescription.InteractiveInsert AND NO ObjectDescription.Editing
							ObjectPresentationClarification = NStr("en='(adding*, not limited"
"modification*, limited)';ru='(добавление*, не ограничено"
"изменение*, ограничено)';vi='(bổ sung*, không hạn chế"
"thay đổi*, hạn chế)'");
						EndIf;
					Else // NO ObjectDescription.AddingWithoutRestriction AND NO ObjectDescription.ChangingWithoutRestriction
						If ObjectDescription.InteractiveInsert AND ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en='(adding, limited"
"modification, limited)';ru='(добавление, ограничено"
"изменение, ограничено)';vi='(thêm, hạn chế"
"thay đổi, hạn chế)'");
						ElsIf Not ObjectDescription.InteractiveInsert AND ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en='(adding*, limited"
"modification, limited)';ru='(добавление*, ограничено"
...
"modification, not limited)';ru='(добавление не доступно"
"изменение, не ограничено)';vi='(không thể bổ sung"
"thay đổi, không hạn chế)'");
...
"thay đổi*, không hạn chế)'");
					...
					Else // NO ObjectDescription.ChangeWithoutRestriction
						If ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en='(adding is not available"
"modification, limited)';ru='(добавление не доступно"
"изменение, ограничено)';vi='(không thể bổ sung"
"thay đổi, hạn chế)'");
						Else // NO ObjectDescription.Editing
							ObjectPresentationClarification = NStr("en='(adding is not available"
"modification*, limited)';ru='(добавление не доступно"
"изменение*, ограничено)';vi='(thêm không khả dụng"
"thay đổi*, hạn chế)'");
						EndIf;
					EndIf;
				...
				Else // NO ObjectDescription.Adding AND NO ObjectDescription.Changing
					ObjectPresentationClarification = NStr("en='(adding is not available"
"modification is not available)';ru='(добавление не доступно"
"изменение не доступно)';vi='(không thể bổ sung"
"không thể thay đổi)'");
				EndIf;
			Else
				If ObjectDescription.Update1 Then
					If ObjectDescription.ChangingNotLimited Then
						If ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en='(change, not limited)';ru='(изменение, не ограничено)';vi='(thay đổi, không hạn chế)'");
						Else // NO ObjectDescription.Editing
							ObjectPresentationClarification = NStr("en='(change *, not limited)';ru='(изменение*, не ограничено)';vi='(thay đổi*, không hạn chế)'");
						EndIf;
					Else
						If ObjectDescription.Edit Then
							ObjectPresentationClarification = NStr("en='(change, limited)';ru='(изменение, ограничено)';vi='(thay đổi, hạn chế)'");
						Else // NO ObjectDescription.Editing
							ObjectPresentationClarification = NStr("en='(change *, limited)';ru='(изменение*, ограничено)';vi='(thay đổi*, hạn chế)'");
						EndIf;
					EndIf;
				Else // NO ObjectDescription.Change
					ObjectPresentationClarification = NStr("en='(change is not available)';ru='(изменение не доступно)';vi='(không được phép thay đổi)'");
				EndIf;
			EndIf;
...
"%2)'")
	...
	For Each ObjectTypeDescription IN RightSettings.Rows Do
		...
		TitleForSubfolders =
			NStr("en='For subfolders';ru='Для подпапок';vi='Đối với thư mục con'");
		PromtForSubfolders = NStr("en='Rights are not only to the current folder but also for its subfolders';ru='Права не только для текущей папки, но и для ее нижестоящих папок';vi='Quyền không chỉ đối với thư mục hiện tại mà còn đối với thư mục lớp dưới'");
		...
		TitleSettingIsReceivedFromGroup = NStr("en='Rights setting received from group';ru='Настройка прав получена от группы';vi='Đã nhận tùy chỉnh quyền từ nhóm'");
		
		Area = Template.GetArea("RightsSettingsLegendRowInheritance");
		Area.Parameters.ToolTip = NStr("en='Rights inheritance from upstream folders';ru='Наследование прав от вышестоящих папок';vi='Kế thừa quyền từ thư mục lớp trên'");
		Document.Put(Area, 2);
		...
		TextYes  = NStr("en='Yes';ru='Да';vi='Đồng ý'");
		TextNo = NStr("en='No';ru='Нет';vi='Không'");
	...
	If AccessTypeDescription.Rows.Count() = 0 Then
		...
		ElsIf AccessTypeDescription.AllAllowed Then
			If AccessTypeDescription.AccessKind = Catalogs.Users.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en='%1 (no prohibited, the current  user is always permitted)';ru='%1 (без запрещенных, текущий пользователь всегда разрешен)';vi='%1 (không cấm, người sử dụng hiện tại luôn được phép)'");
				
			ElsIf AccessTypeDescription.AccessKind = Catalogs.ExternalUsers.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en='%1 (no prohibited, the current external user is always permitted)';ru='%1 (без запрещенных, текущий внешний пользователь всегда разрешен)';vi='%1 (không cấm, người sử dụng ngoài hiện tại luôn được phép)'");
			Else
				AccessKindPresentationTemplate = NStr("en='%1 (without prohibited)';ru='%1 (без запрещенных)';vi='%1 (không cấm)'");
			EndIf;
		Else
			If AccessTypeDescription.AccessKind = Catalogs.Users.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en='%1 (without permitted, current user is always permitted)';ru='%1 (без разрешенных, текущий пользователь всегда разрешен)';vi='%1 (không cho phép, người sử dụng hiện tại luôn được phép)'");
				
			ElsIf AccessTypeDescription.AccessKind = Catalogs.ExternalUsers.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en='%1 (no permitted, the current external user is always permitted)';ru='%1 (без разрешенных, текущий внешний пользователь всегда разрешен)';vi='%1 (không cho phép, người sử dụng ngoài hiện tại luôn được phép)'");
			Else
				AccessKindPresentationTemplate = NStr("en='%1 (without permitted)';ru='%1 (без разрешенных)';vi='%1 (không cho phép)'");
			EndIf;
		EndIf;
	Else
		If AccessTypeDescription.AllAllowed Then
			If AccessTypeDescription.AccessKind = Catalogs.Users.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en='%1 (prohibited, the current user is always permitted):';ru='%1 (запрещенные, текущий пользователь всегда разрешен):';vi='%1 (cấm, người sử dụng hiện tại luôn được phép):'");
				
			ElsIf AccessTypeDescription.AccessKind = Catalogs.ExternalUsers.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en='%1  (prohibited, the current external user is always permitted):';ru='%1 (запрещенные, текущий внешний пользователь всегда разрешен):';vi='%1 (cấm, người sử dụng ngoài hiện tại luôn được phép):'");
			Else
				AccessKindPresentationTemplate = NStr("en='%1 (prohibited):';ru='%1 (запрещенные):';vi='%1 (cấm):'");
			EndIf;
		Else
			If AccessTypeDescription.AccessKind = Catalogs.Users.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en='%1 (permitted, the current user is always permitted):';ru='%1 (разрешенные, текущий пользователь всегда разрешен):';vi='%1 (được phép, người sử dụng hiện tại luôn được phép):'");
				
			ElsIf AccessTypeDescription.AccessKind = Catalogs.ExternalUsers.EmptyRef() Then
				AccessKindPresentationTemplate =
					NStr("en='%1 (permitted, the current external user is always permitted):';ru='%1 (разрешенные, текущий внешний пользователь всегда разрешен):';vi='%1 (được phép, người sử dụng ngoài hiện tại luôn được phép):'");
			Else
				AccessKindPresentationTemplate = NStr("en='%1 (permitted):';ru='%1 (разрешенные):';vi='%1 (được phép):'");
			EndIf;
		EndIf;
	EndIf;
...
Function StartModePresentation(RunMode)
	...
	If RunMode = "Auto" Then
		StartModePresentation = NStr("en='Auto';ru='Авто';vi='Tự động'");
		
	ElsIf RunMode = "OrdinaryApplication" Then
		StartModePresentation = NStr("en='Standard application';ru='Обычное приложение';vi='Ứng dụng thường'");
		
	ElsIf RunMode = "ManagedApplication" Then
		StartModePresentation = NStr("en='Managed application';ru='Управляемое приложение';vi='Ứng dụng được điều khiển'");
	Else
	...
	While IndexOf >= 0 Do
		...
		If AccessTypeProperties = Undefined Then
			...
			If RightSettingsOwnerMetadata = Undefined Then
				String.Presentation = NStr("en='Unknown access kind';ru='Неизвестный вид доступа';vi='Dạng truy cập không xác định'");
			Else
				String.Presentation = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Set rights to %1';ru='Настройки прав на %1';vi='Tùy chỉnh quyền đối với %1'"), RightSettingsOwnerMetadata.Presentation());
			EndIf;
