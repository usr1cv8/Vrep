...
"мониторинг активности пользователей в программе (насколько интенсивно и с какими объектами работают пользователи).';vi='Cho phép thực hiện"
"theo dõi hoạt động của người sử dụng trong chương trình (cường độ lớn nhất mà người sử dụng hay làm việc với những đối tượng nào).'");
...
"информация о том, с какими объектами работал пользователь в программе.';vi='Chi tiết"
"thông tin về người sử dụng đã làm việc với đối tượng nào trong chương trình.'");
	
	VariantSettings = ModuleReportsVariants.VariantDesc(Settings, ReportSettings, "EventLogMonitorControl");
	VariantSettings.Definition = NStr("en='List of critical entries in the event log.';ru='Список критичных записей журнала регистрации.';vi='Danh sách bản ghi quan trọng của nhật ký chứng từ.'");
	...
	VariantSettings.Definition = NStr("en='Displays scheduled job schedule in the application.';ru='Выводит график выполнения регламентных заданий в программе.';vi='Hiển thị lịch biểu thực hiện nhiệm vụ thường kỳ trong chương trình.'");
...
Function RowWeekOfYear(DateOfYear)
	Return StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Week %1';ru='Неделя %1';vi='Tuần %1'"), WeekOfYear(DateOfYear));
EndFunction
		...
		For Each UserName IN TemporaryArray Do
			If UserName = Item Then
				...
				UserAndNumber = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='%1 (%2)';ru='%1 (%2)';vi='%1 (%2)'"), Item, CounterNumberOfSessionsUser);
			EndIf;
		EndDo;
...
Function GenerateReportForDurationJobsOfScheduledJobs(FillingParameters) Export
	...
	If FilterOutput.Value = DataCompositionTextOutputType.Output
		...
		If ScheduledJobsSessionsMinimumLength > 0 Then
			DisplayModeIntervals = NStr("en='Enabled';ru='Включено';vi='Bật'");
		Else
			DisplayModeIntervals = NStr("en='Enabled';ru='Включено';vi='Bật'");
		EndIf;
			...
			For Each Item IN ArrayRoutineMaintenanceJobs Do
				If Not TypeOf(Item) = Type("Number")
					...
					Area.Parameters.ScheduledJobList = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='%1 (session %2)';ru='%1 (сеанс %2)';vi='%1 (phiên làm việc %2)'"), Item, ArrayRoutineMaintenanceJobs.Get(IndexOfScheduledJobs+1));
	...
	For Each RowScheduledJobs IN TableSessionsRoutineMaintenanceJobs Do
		...
		If DurationIntervalJobSchedule >= ScheduledJobsSessionsMinimumLength Then
			If CurrentEvent <> RowScheduledJobs.NameEvents Then
				If CurrentEvent <> Undefined
					...
					PointName = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='%1 (%2 from %3)';ru='%1 (%2 из %3)';vi='%1 (%2 / %3)'"), Point.Value, 
				...
				ElsIf Not ValueIsFilled(RowScheduledJobs.EventMetadata) Then
					PointName = NStr("en='Background jobs';ru='Фоновые задания';vi='Nhiệm vụ nền'");
					Point = GanttChart.SetPoint(PointName);
					CommonDurationOfST = 0;
				EndIf;
 			EndIf;
			...
			Interval.Text	= StringFunctionsClientServer.SubstituteParametersInString(
								NStr("en='%1 - %2';ru='%1 - %2';vi='%1 - %2'"), Format(Interval.Begin, "DLF=T"), Format(Interval.End, "DLF=T"));
			SignChangeDots = False;
			// Do not fill in decryption for background jobs.
			If RowScheduledJobs.EventMetadata <> "" Then
		...
		If CurrentDate <> DateAmountAtSameTimeScheduledJobs Then              
			If TableRow <> Undefined Then
				...
				TableRow.DateAmountAtSameTimeScheduledJobs = StringFunctionsClientServer.SubstituteParametersInString(
										NStr("en='%1 - %2';ru='%1 - %2';vi='%1 - %2'"), Format(CurrentDate, "DLF=T"), 
...
Function DecryptionScheduledJobs(Details) Export
	...
	If Details.Get(8) = 0 Then
		DisplayModeIntervals = NStr("en='Disabled';ru='Отключено';vi='Đã ngắt'");
	Else
		DisplayModeIntervals = NStr("en='Disabled';ru='Отключено';vi='Đã ngắt'");
	EndIf;
	...
	Area = Template.GetArea("Table");
	Area.Parameters.TypeTasks = NStr("en='Scheduled';ru='Регламентное';vi='Thường kỳ'");
...
Function DurationOfScheduledJobs(DurationOfST)
	...
	ElsIf DurationOfST <= 60 Then
		CommonDurationOfST = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='%1 sec';ru='%1 сек';vi='%1 giây'"), DurationOfST);
	ElsIf 60 < DurationOfST <= 3600 Then
		...
		CommonDurationOfST = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='%1 min %2 sec';ru='%1 мин %2 сек';vi='%1 phút %2 giây'"), DurationMinutes, DurationSeconds);
	ElsIf DurationOfST > 3600 Then
		...
		CommonDurationOfST = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='%1 h %2 min';ru='%1 ч %2 мин';vi='%1 giờ %2 phút'"), DurationHours, DurationMinutes);
	EndIf;
