...
Procedure ResultDetailProcessing(Item, Details, StandardProcessing)
	...
	If Not CurrentVariantDescription = NStr("en='Duration of scheduled jobs';ru='Продолжительность работы регламентных заданий';vi='Thời lượng công việc của nhiệm vụ thường kỳ'") Then
	...
	If TypeDetails = "DecryptionScheduledJobs" Then
		...
		VariantDetails.Add("InfoAboutScheduledJob", NStr("en='Scheduled job information';ru='Сведения о регламентном задании';vi='Thông tin về nhiệm vụ thường kỳ'"));
		VariantDetails.Add("OpenEventLogMonitor", NStr("en='Go to event log';ru='Перейти к журналу регистрации';vi='Chuyển đến nhật ký sự kiện'"));
...
Procedure ResultDetailDataProcessorEnd(SelectedVariant, Details) Export
	...
	If Action = "InfoAboutScheduledJob" Then
		...
		For Each GanttChartPoint IN ListPoints Do
			...
			If GanttChartPoint.Value = NStr("en='Background jobs';ru='Фоновые задания';vi='Nhiệm vụ nền'") Then
				Continue;
			EndIf;
...
Function GenerateReportServer(ReportParameters)
	...
	ExecutionResult = LongActions.ExecuteInBackground(
		...
		ReportParameters,
		NStr("en='Running report: Event log analysis';ru='Выполнение отчета: Анализ журнала регистрации';vi='Thực hiện báo cáo: Phân tích nhật ký sự kiện'"));
