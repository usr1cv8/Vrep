...
Procedure FillCheckProcessing(Cancel, CheckedAttributes)
	...
	If ReportVariant = "GanttChart" Then
		...
		If Not ValueIsFilled(DayPeriod.Date) Then
			CommonUseClientServer.MessageToUser(
				NStr("en='Value of field Day is not entered.';ru='Не заполнено значение поля День!';vi='Chưa điền giá trị trường Ngày!'"), , );
			Cancel = True;
			Return;
		EndIf;
		...
		AND EndSelection.Use Then
			CommonUseClientServer.MessageToUser(
				NStr("en='The period start value cannot be later than the period end value.';ru='Значение начала периода не может быть больше значения конца!';vi='Giá trị đầu kỳ không thể lớn hơn giá trị cuối kỳ!'"), , );
			Cancel = True;
			Return;
		EndIf;
	...
	ElsIf ReportVariant = "ActiveUser" Then
		...
		If Not ValueIsFilled(User) Then
			CommonUseClientServer.MessageToUser(
				NStr("en='Value of field User is not entered.';ru='Не заполнено значение поля Пользователь!';vi='Chưa điền giá trị trường Người sử dụng!'"), , );
			Cancel = True;
			Return;
		EndIf;
		...
		If Reports.EventsLogAnalysis.IBUserName(User) = Undefined Then
			CommonUseClientServer.MessageToUser(
				NStr("en='The report can be generated only for the user for whom a name for logon to the application is specified.';ru='Формирование отчета возможно только для пользователя, которому указано имя для входа в программу.';vi='Chỉ có thể lập báo cáo đối với người sử dụng mà đã chỉ ra tên để đăng nhập vào chương trình'"), , );
			Cancel = True;
			Return;
		EndIf;
	...
	ElsIf ReportVariant = "UsersActivityAnalysis" Then
		...
		If Not ValueIsFilled(UsersAndGroups) Then
			CommonUseClientServer.MessageToUser(
				NStr("en='Value of field Users is not entered.';ru='Не заполнено значение поля Пользователи!';vi='Chưa điền giá trị trường Người sử dụng!'"), , );
			Cancel = True;
			Return;
		EndIf;
