...
Procedure ConfigureReportsVariants(Settings, ReportSettings) Export
	ReportSettings.Description = NStr("en='External Resources used by the application and additional modules';ru='Внешние ресурсы, используемые программой и дополнительными модулями';vi='Tài nguyên ngoài mà được sử dụng bởi chương trình và các mô-đun bổ sung'");
...
"компоненты"
"Контрольная сумма"
"Шаблон командной строки Протокол Адрес Интернет-ресурса Порт';vi='Tên"
"và tên"
"lóp COM"
"Tên"
"máy tính"
"Địa chỉ Đọc dữ liệu"
"Bản ghi dữ liệu"
"Tên khuôn in hoặc tệp"
"cấu phần"
"Kiểm soát số tiền"
"Khuôn mẫu dòng lệnh Giao thức Địa chỉ dịch vụ Internet Cổng'");
...
Function PresentationRequestsPermissionsToUseExternalResources(Val AdministrationOperations, Val AddedPermissionsDescription, Val DeletedPermissionsDescription, Val AsRequired = False) Export
	...
	For Each ModulesTableString IN ModulesTable Do
		...
		If Adding.Count() > 0 Then
			...
			If AsRequired Then
				...
				HeaderText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='It is required to use the following external resources for %1 ""%2"":';ru='Для %1 ""%2"" требуется использование следующих внешних ресурсов:';vi='Đối với %1 ""%2"" cần sử dụng những nguồn lực ngoài như sau:'"),
			...
			Else
				...
				HeaderText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='The following permissions for external resource usage will be granted to %1 ""%2"":';ru='Для %1 ""%2"" будут предоставлены следующие разрешения на использование внешних ресурсов:';vi='Đối với %1 ""%2"" sẽ cấp phép sử dụng nguồn lực ngoài như sau:'"),
					Lower(Dictionary.Genitive),
					ModuleName);
		...
		If ToDelete.Count() > 0 Then
			...
			If AsRequired Then
				Raise NStr("en='Incorrect permission request';ru='Некорректный запрос разрешений';vi='Yêu cầu cấp phép không hợp lệ'");
			EndIf;
			
			HeaderText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='The following permissions for external resource usage previously granted to %1 ""%2"" will be deleted:';ru='Будут удалены следующие ранее предоставленные для %1 ""%2"" разрешения на использование внешних ресурсов:';vi='Sẽ xóa những cho phép sử dụng nguồn lực ngoài mà trước đó cấp cho %1 ""%2"" sau đây:'"),
					Lower(Dictionary.Genitive),
					ModuleName);
...
Procedure GenerateOperationsPresentation(SpreadsheetDocument, Val Template, Val AdministrationOperations)
	...
	For Each Description IN AdministrationOperations Do
		...
		If Description.Operation = Enums.OperationsAdministrationSecurityProfiles.Delete Then
			...
			HeaderText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Security profile for %1 ""%2"" will be deleted.';ru='Будет удален профиль безопасности для %1 ""%2"".';vi='Hồ sơ an toàn cho %1 ""%2"" sẽ bị xóa.'"),
					Lower(Dictionary.Genitive),
					ModuleName);
					...
					Else
						...
						If DefinitionType = "FileSystemAccess" Then
							...
							If Resolution.Path = "/temp" Then
								AuthorizationAdding.Insert("Path", NStr("en='Temporary file directory';ru='Каталог временных файлов';vi='Thư mục tệp tạm thời'"));
							EndIf;
							
							If Resolution.Path = "/bin" Then
								AuthorizationAdding.Insert("Path", NStr("en='Directory to which the 1C:Enterprise server is installed';ru='Каталог, в который установлен сервер 1С:Предприятия';vi='Thư mục mà trong đó đã cài đặt Server 1C:DOANH NGHIỆP'"));
							EndIf;
...
Function ConfigurationModuleDictionary() Export
	...
	Result.Insert("Nominative", NStr("en='Application';ru='Программа';vi='Chương trình'"));
	Result.Insert("Genitive", NStr("en='Application';ru='Программы';vi='Chương trình'"));
