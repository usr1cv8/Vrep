...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.DocumentRef = Undefined Then
		Raise NStr("en='The report can be opened only from documents.';ru='Открытие отчета предусмотрено только из документов.';vi='Chỉ được mở báo cáo từ chứng từ.'");
	EndIf;
...
Procedure OnOpen(Cancel)
	...
	If Not DiscountsAreCalculated Then
		...
		Else
			Items.WarningDecoration.Visible = True;
			Items.WarningDecoration.Title = NStr("en='IN the document discounts and markups are not calculated!';vi='Trong chứng từ chưa tính chiết khấu và phụ thu!'");
		EndIf;
	EndIf;
...
Procedure Generate()
	...
	If TypeOf(DocumentObject) = Type("DocumentObject.CustomerInvoice") Then
		...
		If AutomaticDiscountsCalculationParametersStructure.SalesByOrders AND Not AutomaticDiscountsCalculationParametersStructure.SalesExceedingOrder Then
			AdditionalMessageText = NStr("en='Discounts are calculated based on order data!';ru='Скидки рассчитываются на основе данных заказа!';vi='Tính chiết khấu dựa trên dữ liệu của đơn hàng!'");
		ElsIf AutomaticDiscountsCalculationParametersStructure.SalesByOrders AND AutomaticDiscountsCalculationParametersStructure.SalesExceedingOrder Then
			AdditionalMessageText = NStr("en='Discounts are calculated based on order data! Strings over the order are calculated separately!';ru='Скидки рассчитываются на основе данных заказа! Строки вне порядка вычисляются отдельно!';vi='Tính chiết khấu dựa trên dữ liệu của đơn hàng! Tính toán riêng biệt các dòng không theo trình tự!'");			
		Else
	...
	If TypeOf(DocumentObject) = Type("DocumentObject.InvoiceForPayment") Then
		...
		If DocumentObject.BasisDocument <> Undefined AND Not DocumentObject.BasisDocument.IsEmpty() Then
			If TypeOf(DocumentObject.BasisDocument) = Type("DocumentRef.CustomerOrder")
				...
				Then
				AdditionalMessageText = NStr("en='Discounts are calculated based on data of the basis document!';ru='Скидки рассчитываются на основе данных базового документа!';vi='Tính chiết khấu dựa trên dữ liệu của chứng từ cơ sở!'");
			Else
				DisplayAdditionalMessage = False;
			EndIf;
		Else
