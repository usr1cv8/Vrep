...
Function QueryByPlannedPaymentsBalance()
	...
	Query.SetParameter("OverduePayments", NStr("en='Total Overdue payments';ru='Итого просроченные платежи';vi='Tổng số thanh toán quá hạn'"));
	Query.SetParameter("ScheduledPayments", NStr("en='Total scheduled payments';ru='Итого запланированные платежи';vi='Tổng số thanh toán dự tính'"));
	Query.SetParameter("TextSummary", NStr("en='Total payments';ru='Всего платежи';vi='Tổng số thanh toán'"));
...
Function QueryByCashAssetsBalance()
	...
	Query.SetParameter("Company", SmallBusinessServer.GetCompany(Company));
	Query.SetParameter("WorkingDate", Report.WorkingDate);
	Query.SetParameter("PaymentStatus", NStr("en='Available remaining goods';ru='Доступный остаток';vi='Số dư được phép'"));
	Query.SetParameter("TextSummary", NStr("en='Total payments';ru='Всего платежи';vi='Tổng số thanh toán'"));
	Query.SetParameter("Payment", NStr("en='Current balance';ru='Текущий остаток';vi='Số dư hiện tại'"));
