...
Function RefreshReport() Export 
	...
	If BeginOfPeriod <> Undefined AND EndOfPeriod <> Undefined AND BeginOfPeriod > EndOfPeriod Then		
		...
		Message.Text = NStr("en='Incorrect period is specified: period start cannot be less or equal to the period end.';ru='Неправильно задан период: начало периода должно быть меньше или равно окончанию!';vi='Chỉ ra sai kỳ: đầu kỳ phải nhỏ hơn hoặc bằng cuối kỳ!'");
		Message.Message();		
		Return Undefined;	
	EndIf; 
