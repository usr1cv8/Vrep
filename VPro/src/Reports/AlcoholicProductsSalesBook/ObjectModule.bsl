...
Procedure OnComposeResult(ResultDocument, DetailsData, StandardProcessing)
	...
	If Not ValueIsFilled(Company) Then
		Raise NStr("en='Fill in the company';ru='Не заполнено значение обязательного параметра ""Организация""';vi='Chưa điền giá trị cho tham số bắt buộc ""Doanh nghiệp""'");
	EndIf;
	If Not ValueIsFilled(Warehouse) Then
		Raise NStr("en='Fill in the warehouse';ru='Не заполнено значение обязательного параметра ""Склад""';vi='Chưa điền giá trị cho tham số bắt buộc ""Kho bãi""'");
	EndIf;
	...
	While Selection.Next() Do
		...
		Else
			...
			If TypeOf(Selection.Recorder) = Type("DocumentRef.RetailReport")
				...
				ContentOperations = NStr("en='Sold products';ru='Проданная продукция';vi='Sản phẩm bán'");
				
			ElsIf TypeOf(Selection.Recorder) = Type("DocumentRef.CustomerInvoice")
				AND Selection.OperationKind = Enums.OperationKindsCustomerInvoice.ReturnToVendor Then
				
				ContentOperations = NStr("en='Products returned to the supplier';ru='Продукция, возвращенная поставщику';vi='Sản phẩm trả lại nhà cung cấp'");
			...
			ElsIf TypeOf(Selection.Recorder) = Type("DocumentRef.InventoryWriteOff") Then
				...
				ContentOperations = NStr("en='Product shortage';ru='Недостачи продукции';vi='Thiếu hàng'");
			...
			ElsIf TypeOf(Selection.Recorder) = Type("DocumentRef.InventoryTransfer")
				...
				ContentOperations = NStr("en='Products transferred to another department';ru='Продукция, переданная в другое подразделение';vi='Sản phẩm chuyển giao cho bộ phận khác'");
				
			Else
				
				ContentOperations = NStr("en='Written off products';ru='Списанная продукция';vi='Sản phẩm ghi giảm'");
