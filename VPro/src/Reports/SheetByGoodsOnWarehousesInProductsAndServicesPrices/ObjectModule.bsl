...
Procedure OnComposeResult(ResultDocument, DetailsData, StandardProcessing)
	...
	If Not BeginOfPeriod = Date(1,1,1)
		...
		MessageText	 	= NStr("en='Period start cannot be greater than period end';ru='Дата начала периода не должна превышать дату окончания.';vi='Ngày đầu kỳ không được lớn hơn ngày cuối kỳ.'");
	...
	If Not ValueIsFilled(PriceKind)
		...
		MessageText	 	= NStr("en='The price kind for report generation is not selected.';ru='Не выбран вид цены для формирования отчета.';vi='Chưa chọn dạng giá để lập báo cáo.'");
		CommonUseClientServer.MessageToUser(MessageText);
...
Function PrepareReportParameters(ReportSettings)
	...
	EndOfPeriod  = Date(1,1,1);
	TitleOutput = False;
	Title = NStr("en='Goods in products and services prices';vi='Hàng hóa trong sản phẩm và dịch vụ';");
	...
	If ParameterKindOfPrice <> Undefined
		...
		ParameterKindOfPrice.UserSettingPresentation = NStr("en='Price kind';ru='Вид цены';vi='Dạng giá'");
