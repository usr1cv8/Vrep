...
Procedure MakeExecute()
	...
	If Constants.FunctionalOptionAccountingByMultipleCompanies.Get() AND Not ValueIsFilled(Company) Then
		MessageText = NStr("en='Company is not selected.';ru='Не выбрана организация!';vi='Chưa chọn doanh nghiệp!'");
	...
	If Not ValueIsFilled(RegistrationPeriod) Then
		MessageText = NStr("en='The registration period is required.';ru='Не выбран период регистрации!';vi='Chưa chọn kỳ ghi nhận!'");
	...
	If Not Constants.FunctionalCurrencyTransactionsAccounting.Get() AND Not ValueIsFilled(Currency) Then
		MessageText = NStr("en='Currency is not selected.';ru='Не выбрана валюта!';vi='Chưa chọn tiền tệ!'");
	...
	If QueryResult[0].IsEmpty() Then
		...
		Message.Text = NStr("en='No data to generate the report.';ru='Нет данных для формирования отчета!';vi='Không có dữ liệu để lập báo cáo!'");
		Message.Message();
		Return;
	EndIf;
    ...
    AreaSpace.Parameters.TextPadding = NStr("en='Company: ';vi='Doanh nghiệp: ';") + Company;
	SpreadsheetDocument.Put(AreaSpace);

	SelectionSubdepartment = QueryResult[0].Select(QueryResultIteration.ByGroups, "Department");
	While SelectionSubdepartment.Next() Do

        AreaSpace.Parameters.TextPadding = NStr("en='Department: ';vi='Bộ phận: ';") + SelectionSubdepartment.Department;
		SpreadsheetDocument.Put(AreaSpace);
		...
		While IndividualSelection.Next() Do
			...
			AreaHeader.Parameters.Title = NStr("en='Payroll sheet for ';vi='Bảng lương cho ';")  + Format(RegistrationPeriod , "DF=""MMMM yyyy '.' """);
			AreaHeader.Parameters.Company = Company;
			AreaHeader.Parameters.Fill(IndividualSelection);
			PresentationIndividual = SmallBusinessServer.GetSurnameNamePatronymic(IndividualSelection.Surname, IndividualSelection.Name, IndividualSelection.Patronymic, False);
			...
			While EmployeePaymentsSelection.FIndNext(StructureSearchBySelection) Do
				...
				AreaPaymentDetails.Parameters.PaymentText = "" + EmployeePaymentsSelection.DocumentPresentation + ?(EmployeePaymentsSelection.AdvanceFlag, "(advance) #", " #") + 
																TrimAll(EmployeePaymentsSelection.Number) + NStr("en=' dated ';vi=' ngày ';") + Format(EmployeePaymentsSelection.Date, "DF=dd.MM.yyyy");
			...
			If IndividualSelection.BalanceAtBegin < 0 Then
				FooterArea.Parameters.TextByBeginOfDebtPeriod = NStr("en='Employee's debt on month start:';vi='Nợ của nhân viên vào đầu tháng:';");
			Else	
				FooterArea.Parameters.TextByBeginOfDebtPeriod = NStr("en='Company's debt on month start:';vi='Nợ công ty vào đầu tháng:';");
			EndIf; 
			If IndividualSelection.ClosingBalance < 0 Then
				FooterArea.Parameters.TextAtEndOfPeriodOfDebt = NStr("en='Employee's debt on month end:';vi='Nợ của nhân viên vào cuối tháng:';");
			Else	
				FooterArea.Parameters.TextAtEndOfPeriodOfDebt = NStr("en='Company's debt on month end:';vi='Nợ công ty vào cuối tháng:';");
			EndIf; 
