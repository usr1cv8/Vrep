...
Procedure OnComposeResult(ResultDocument, DetailsData, StandardProcessing)
	...
	SettingsParameterValue = DataParameters.FindParameterValue(New DataCompositionParameter("Overdue"));
	If SettingsParameterValue <> Undefined Then
		SettingsParameterValue.Value = NStr("en='Overdue';ru='Просрочено';vi='Quá hạn'");
		SettingsParameterValue.Use = True;
	EndIf;
	
	SettingsParameterValue = DataParameters.FindParameterValue(New DataCompositionParameter("ForToday"));
	If SettingsParameterValue <> Undefined Then
		SettingsParameterValue.Value = NStr("en='For today';ru='Сегодня';vi='Hôm nay'");
		SettingsParameterValue.Use = True;
	EndIf;

	SettingsParameterValue = DataParameters.FindParameterValue(New DataCompositionParameter("Planned"));
	If SettingsParameterValue <> Undefined Then
		SettingsParameterValue.Value = NStr("en='Planned';ru='Запланировано';vi='Đã lập kế hoạch'");
		SettingsParameterValue.Use = True;
	EndIf;
