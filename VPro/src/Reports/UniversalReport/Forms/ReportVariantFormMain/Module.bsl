...
Procedure OnCreateAtServer(cancel, StandardProcessing)
	...
	If Parameters.Property("VariantPresentation") And ValueIsFilled(Parameters.VariantPresentation) Then
		AutoTitle = False;
		Title = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Изменение варианта отчета ""%1""';ru='Изменение варианта отчета ""%1""';vi='Thay đổi phương án báo cáo ""%1""'"), Parameters.VariantPresentation);
	EndIf;
	...
	If Not ValueIsFilled(Parameters.VariantPresentation) Then
		Parameters.Property("OptionName", Parameters.VariantPresentation);
	EndIf;
...
Procedure OnLoadUserSettingsAtServer(Settings)
	NewSettings = Report.SettingsComposer.GetSettings();
	Report.SettingsComposer.LoadFixedSettings(New DataCompositionSettings);
	ReportsClientServer.LoadSettings(Report.SettingsComposer, NewSettings);
EndProcedure
...
Procedure FinishEdit(Command)
	...
	Else
		...
		ChoiceResult.Insert("VariantModified", VariantModified);
		ChoiceResult.Insert("UserSettingsModified", VariantModified Or UserSettingsModified);
		
		#If WebClient Then
			ChoiceResult.VariantModified = True; // Для платформы.
			ChoiceResult.UserSettingsModified = True;
		#EndIf
		
		If ChoiceResult.UserSettingsModified Then
...
Procedure SetCurrentRow()
	Items.SettingsComposerSettings.CurrentRow = SettingsStructureItemID;
EndProcedure
