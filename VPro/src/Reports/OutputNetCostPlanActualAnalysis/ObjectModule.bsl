...
Procedure GenerateTableOutputAndPlanCosts(TableOutput, TablePlanCostsOnOutput, ParameterValues)
	...
	|	ISNULL(DefaultSpecifications.Specification, VALUE(Catalog.Specifications.EmptyRef)) AS DefaultSpecification,
	...
	|		ON ProductReleaseTurnovers.ProductsAndServices = DefaultSpecifications.ProductsAndServices
	|			AND ProductReleaseTurnovers.Characteristic = DefaultSpecifications.Characteristic
	|
	|INDEX BY
	|	Products,
	...
	|	ProductRelease.CustomerOrder AS CustomerOrder,
	...
	|			THEN ProductRelease.DefaultSpecification
	...
	|	ISNULL(ProductRelease.Products, VALUE(ChartOfAccounts.Managerial.EmptyRef)) AS ProductsAndServices,
	|	ProductRelease.ProductCharacteristic AS Characteristic,
	|	ProductRelease.ProductsQuantity AS Quantity,
	|	CASE
	|		WHEN ProductRelease.ProductionSpecification = VALUE(Catalog.Specifications.EmptyRef)
	|			THEN ProductRelease.DefaultSpecification
	|		ELSE ProductRelease.ProductionSpecification
	...
	|					THEN ProductRelease.DefaultSpecification
	|				ELSE ProductRelease.ProductionSpecification
	|			END = VALUE(Catalog.Specifications.EmptyRef)
	|			THEN VALUE(Enum.SpecificationContentRowTypes.Material)
	|		ELSE VALUE(Enum.SpecificationContentRowTypes.Node)
	|	END AS ContentRowType
	...
	|	CompositionOutput.CustomerOrder AS CustomerOrder,
	...
	|	CompositionOutput.ProductsAndServices AS ProductsAndServices,
	...
	|	CASE
	|		WHEN CompositionOutput.ContentRowType = VALUE(Enum.SpecificationContentRowTypes.Node)
	|			THEN VALUE(Enum.SpecificationContentRowTypes.EmptyRef)
	|		ELSE CompositionOutput.ContentRowType
	|	END AS ContentRowTypeReport,
	|	ISNULL(CASE
	|			WHEN CompositionOutput.ContentRowType <> VALUE(Enum.SpecificationContentRowTypes.Node)
	|				THEN CompositionOutput.ProductsAndServices
	|			ELSE VALUE(Catalog.ProductsAndServices.EmptyRef)
	|		END, VALUE(ChartOfAccounts.Managerial.EmptyRef)) AS ProductsAndServicesReport,
	|	CASE
	|		WHEN CompositionOutput.ContentRowType <> VALUE(Enum.SpecificationContentRowTypes.Node)
	|			THEN CompositionOutput.Characteristic
	|		ELSE VALUE(Catalog.ProductsAndServicesCharacteristics.EmptyRef)
	|	END AS CharacteristicReport,
	|	CASE
	|		WHEN CompositionOutput.ContentRowType <> VALUE(Enum.SpecificationContentRowTypes.Node)
	|			THEN CompositionOutput.Quantity
	|		ELSE 0
	|	END AS ReportQuantity,
	|	CASE
	|		WHEN CompositionOutput.ContentRowType <> VALUE(Enum.SpecificationContentRowTypes.Node)
	|			THEN CompositionOutput.Specification
	|		ELSE VALUE(Catalog.Specifications.EmptyRef)
	|	END AS SpecificationReport,
	|	CASE
	|		WHEN CompositionOutput.ContentRowType <> VALUE(Enum.SpecificationContentRowTypes.Node)
	|			THEN CompositionOutput.ProductsAndServices.ExpensesGLAccount
	|		ELSE VALUE(ChartOfAccounts.Managerial.EmptyRef)
	|	END AS CostAccountingAccountReport
	|FROM
	...
	|			OR CompositionOutput.Specification <> VALUE(Catalog.Specifications.EmptyRef))";
