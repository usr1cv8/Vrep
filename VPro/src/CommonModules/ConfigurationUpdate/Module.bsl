...
Procedure AbortExecuteIfExternalUserAuthorized(Val MessageText = "") Export
	...
	If UsersClientServer.IsExternalUserSession() Then
		...
		If IsBlankString(ErrorMessage) Then
			ErrorMessage = NStr("en='This operation is not available for external system user.';ru='Данная операция не доступна внешнему пользователю системы.';vi='Giao dịch này không sử dụng được đối với người sử dụng ngoài hệ thống.'");
		EndIf;
...
Procedure FinishUpdate(Val UpdateResult, Val Email, Val UpdateAdministratorName) Export
	...
	MessageText = NStr("en='Completing update from the external script.';ru='Завершение обновления из внешнего скрипта.';vi='Kết thúc cập nhật từ script ngoài.'");
	WriteLogEvent(EventLogMonitorEvent(), EventLogLevel.Information,,,MessageText);
	
	If Not CheckAccessForUpdate() Then
		MessageText = NStr("en='Insufficient rights to complete the configuration update.';ru='Недостаточно прав для завершения обновления конфигурации.';vi='Không đủ quyền để kết thúc cập nhật cấu hình.'");
	...
	If CommonUse.SubsystemExists("StandardSubsystems.EmailOperations")
		...
		Try
			...
			MessageText = NStr("en='Update notification is successfully sent to the email address:';ru='Уведомление об обновлении успешно отправлено на адрес электронной почты:';vi='Đã gửi thành công thông báo về việc cập nhật đến địa chỉ email:'")
				+ " " + Email;
			WriteLogEvent(EventLogMonitorEvent(), EventLogLevel.Information,,,MessageText);
		Except
			MessageText = NStr("en='An error occurred when sending the email:';ru='Ошибка при отправке письма электронной почты:';vi='Lỗi khi gửi email:'")
...
Procedure SendNotificationAboutUpdate(Val UserName, Val AddressOfDestination, Val SuccessfulRefresh)
	...
	Subject = ? (SuccessfulRefresh, NStr("en='Configuration ""%1"" is successfully updated, version %2';ru='Успешное обновление конфигурации ""%1"", версия %2';vi='Cập nhật thành công cấu hình ""%1"", phiên bản %2'"), 
		NStr("en='Configuration ""%1"" update error, version %2';ru='Ошибка обновления конфигурации ""%1"", версия %2';vi='Lỗi cập nhật cấu hình ""%1"", phiên bản %2'"));
	Subject = StringFunctionsClientServer.SubstituteParametersInString(Subject, Metadata.BriefInformation, Metadata.Version);
	
	Details = ?(SuccessfulRefresh, NStr("en='Configuration is successfully updated.';ru='Обновление конфигурации завершено успешно.';vi='Cập nhật cấu hình đã kết thúc thành công.'"), 
		NStr("en='Configuration update failed. Details have been written to the event log.';ru='При обновлении конфигурации произошли ошибки. Подробности записаны в журнал регистрации.';vi='Khi cập nhật cấu hình xảy ra lỗi. Chi tiết được ghi vào nhật ký sự kiện.'"));
...
"Конфигурация:"
"%2"
"Версия: %3 Строка соединения: %4';vi='%1"
""
"Cấu hình:"
"%2"
"Phiên bản: %3 Dòng liên kết: %4'"),
