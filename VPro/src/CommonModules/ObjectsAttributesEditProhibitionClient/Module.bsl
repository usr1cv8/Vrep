...
Procedure CheckReferencesToObject(Val ContinuationProcessor, Val RefArray, Val SynonymsOfAttributes) Export
	...
	DialogTitle = NStr("en='Allow editing attributes';ru='Разрешение редактирования реквизитов';vi='Cho phép soạn mục tin'");
...
"Поиск мест использования может занять длительное время.';vi='Để tránh làm sai lệch dữ liệu trong chương trinh,"
"không cho phép soạn các mục tin sau:"
"%1."
""
"Trước khi cho phép soạn, nên đánh giá hậu quả,"
"bằng cách kiểm tra tất cả những nơi sử dụng phần tử này trong chương trình."
"Việc tìm kiếm những nơi sử dụng có thể mất nhiều thời gian.'");
...
"Поиск мест использования может занять длительное время.';vi='Để tránh sai lệch dữ liệu trong chương trình,"
"không cho phép soạn mục tin %1."
""
"Trước khi cho phép soạn mục tin này, nên đánh giá hậu quả,"
"bằng cách kiểm tra tất cả những nơi sử dụng ""%2"" trong chương trình."
"Việc tìm kiếm những nơi sử dụng có thể mất nhiều thời gian.'");
	EndIf;
	...
	Else
		ObjectsPresentation = StringFunctionsClientServer.SubstituteParametersInString( 
			NStr("en='selected items (%1)';ru='выбранных элементов (%1)';vi='phần tử đã chọn (%1)'"), RefArray.Count());
	EndIf;
	...
	Buttons.Add(DialogReturnCode.Yes, NStr("en='Check and allow';ru='Проверить и разрешить';vi='Kiểm tra và cho phép'"));
...
Procedure ShowMessageBoxAllVisibleAttributesUnlocked(ContinuationProcessor = Undefined) Export
	...
	ShowMessageBox(ContinuationProcessor,
		NStr("en='Editing all visible object attributes allowed.';ru='Редактирование всех видимых реквизитов объекта уже разрешено.';vi='Cho phép soạn tất cả mục tin nhìn thấy của đối tượng.'"));
