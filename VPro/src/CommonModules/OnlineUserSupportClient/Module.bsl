...
Procedure StartMechanismFromMenu(LaunchParameters = Undefined) Export
	...
	If Not CommonUseClient.SubsystemExists("OnlineUserSupport.OnlineSupportMonitor") Then
		...
		Raise NStr("en='Embedding error. The ""Online user support dashboard"" subsystem is missing.';ru='Ошибка встраивания. Отсутствует подсистема ""Монитор Интернет-поддержки пользователей"".';vi='Lỗi chèn. Không có phân hệ ""Bảng điều khiển hỗ trợ người dùng qua Internet"".'");
...
Procedure StartWorkWithEDFOperatorMechanism(
	...
	If Not CommonUseClient.SubsystemExists("OnlineUserSupport.1CTaxcomConnection") Then
		...
		Raise NStr("en='Embedding error. The ""Connection to 1C Taxcom"" subsystem is unavailable.';ru='Ошибка встраивания. Отсутствует подсистема ""Подключение 1С-Такском"".';vi='Lỗi nhúng. Thiếu phân hệ ""Kết nối 1C-Takscom"".'");
	...
	Except
		...
		OpenForm("CommonForm.OnlineUserSupportInternetAccessError",
			New Structure("StartLocation, StartParameters, ErrorDescription, AtSystemOperationStart",
				LaunchLocation,
				LaunchParameters,
				NStr("en='Unknown error. For more details see event log.';ru='Неизвестная ошибка. Подробнее см. в журнале регистрации.';vi='Lỗi không xác định. Chi tiết xem trong nhật ký sự kiện.'"),
				OnStart));
...
Procedure HandleIUSStartStructure(LaunchLocation, Val StartStructure) Export
	...
	ElsIf StartStructure.Action = "ShowMessage" Then
		...
		If StartStructure.OnStart Then
			ShowUserNotification(NStr("en='Online user support';ru='Интернет-поддержка пользователей';vi='Hỗ trợ người sử dụng qua Internet'"),
				,
...
Procedure ServiceCommandsDataProcessor(
	...
	Except
		...
		HandlerContext.FullErrorDescription = NStr("en='Non-handled exception is thrown:';ru='Произошло необрабатываемое исключение:';vi='Xảy ra trường hợp ngoại lệ không được xử lý:'")
			+ " " + DetailErrorDescription(ErrorInfo());
...
Procedure AddServiceCommands(
	...
	Except
		...
		If TypeOf(HandlerContext) = Type("Structure") Then
			...
			HandlerContext.UserErrorDescription =
				NStr("en='An error occurred while connecting to the online support service.';ru='Ошибка при подключении к сервису Интернет-поддержки.';vi='Lỗi khi kết nối tới dịch vụ hỗ trợ qua Internet.'");
		EndIf;
	...
	Except
		...
		If TypeOf(HandlerContext) = Type("Structure") Then
			...
			HandlerContext.ActionsOnErrorForServer.Add("CreateLogRegistrationRecord");
			HandlerContext.ActionOnErrorForClient = "DisplayFormConnectionNotAvailable";
			HandlerContext.UserErrorDescription = NStr("en='An error occurred while retrieving data from the online support server.';ru='Ошибка при получении данных с сервера Интернет-поддержки.';vi='Lỗi khi nhận dữ liệu từ Server hỗ trợ qua Internet.'");
		EndIf;
...
Procedure DisplayStatusServiceCall()
	...
	Status(
		,
		,
		NStr("en='Waiting for response from the online user support server';ru='Ожидание ответа от сервера Интернет-поддержки пользователей';vi='Chờ phản hồi từ Server hỗ trợ người sử dụng qua Internet'"),
		PictureLib.OnlineUserSupport);
...
"Перейти к авторизации нового пользователя?';vi='Sẽ ngắt kết nối người sử dụng"
"với tên đăng nhập %1 đến Internet. Để kết nối lại, cần nhập lại tên đăng nhập và mật khẩu."
"Chuyển sang đăng nhập người sử dụng mới?'");
