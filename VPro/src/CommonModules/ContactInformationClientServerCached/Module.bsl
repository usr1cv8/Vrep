...
Function PostalCodeSerializationCode() Export
	Return ContactInformationClientServerCached.AddressingObjectSerializationCode(NStr("en='Zip code';ru='Почтовый индекс';vi='Mã bưu chính'"));
EndFunction
...
Function AddressingObjectTypesNationalAddresses() Export
	...
	Result.Add(AddressingObjectString("1010", NStr("en='Building';ru='Дом';vi='Nhà'"),  1, 1, 2));
	Result.Add(AddressingObjectString("1050", NStr("en='Unit';ru='Корпус';vi='Dãy nhà'"),      2, 1));;
	Result.Add(AddressingObjectString("2010", NStr("en='Apartment';ru='Квартира';vi='Căn hộ'"), 3, 1));
	Result.Add(AddressingObjectString("2030", NStr("en='Office';ru='Офис';vi='Văn phòng'"),    3, 2));
	Result.Add(AddressingObjectString("2050", NStr("en='Room';ru='Комната';vi='Phòng'"),      3, 5));

	//  Abbreviations required for backward compatibility during parsing
	Result.Add(AddressingObjectString("2010", NStr("en='app';ru='кв.';vi='căn hộ'"),       3, 6));
	Result.Add(AddressingObjectString("2030", NStr("en='off';ru='оф.';vi='VP'"),       3, 7));
	
	// Additional information objects
	Result.Add(AddressingObjectString("10100000", NStr("en='Zip code';ru='Почтовый индекс';vi='Mã bưu chính'")));
