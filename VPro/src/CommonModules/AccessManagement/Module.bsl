...
"can depend only on the Reading right.';ru='Ошибка в функции ЕстьРоль модуля УправлениеДоступом."
...
"может быть зависимость только от права Чтения.';vi='Lỗi trong hàm HasRole mô-đun AccessManagement."
"Trong tập hợp giá trị truy cập đã chỉ ra dạng truy cập EditingRight"
"của bảng với tên (ID) ""%1""."
"Khi hạn chế kiểm tra vai trò (như là quyền bổ sung)"
"có thể chỉ phụ thuộc vào quyền Đọc.'"),
					String.AccessValue);
			EndIf;
...
"""ПравоЧтения"", ""ПравоИзменения"", если они используются.';vi='Lỗi trong hàm HasRole của mô-đun AccessManagement."
"Tập hợp giá trị truy cập có chứa dạng truy cập không xác định ""%2"""
"mà không cần chỉ ra."
""
"Chỉ cần chỉ ra các dạng truy cập đặc biệt"
"""ReadingRight"", ""ChangingRight"", nếu chúng được sử dụng.'"),
...
"Набор значений доступа содержит неизвестный вид доступа ""%2"".';vi='Lỗi trong hàm HasRole của mô-đun AccessManagement."
"Tập hợp giá trị truy cập có chứa dạng truy cập không xác định ""%2"".'"),
				TypeOf(ObjectReference),
				String.AccessKind);
		EndIf;
...
Function IsRight(Right, ObjectReference, User = Undefined) Export
	...
	If RightsDescriptionFull = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Description of possible rights for the ""%1"" table is not found';ru='Не найдено описание возможных прав для таблицы ""%1""';vi='Chưa tìm thấy bản mô tả quyền có thể cho bảng ""%1""'"),
	...
	If RightDetails = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Right description ""%1"" for table ""%2"" is not found';ru='Не найдено описание права ""%1"" для таблицы ""%2""';vi='Chưa tìm thấy bản mô tả quyền ""%1"" cho  bảng ""%2""'"),
			Right,
			ObjectReference.Metadata().FullName());
	EndIf;
...
Procedure OnFormCreationAccessValues(Form,
	...
	ErrorTitle =
		NStr("en='An error occurred in the OnCreateAccessValueForm procedure "
"of the AccessManagement general module.';ru='Ошибка в процедуре ПриСозданииФормыЗначенияДоступа"
"общего модуля УправлениеДоступом.';vi='Lỗi trong thủ tục OnCreateAccessValueForm"
"của mô-đun chung AccessManagement.'");
...
"в форме значения доступа у реквизита ГруппаДоступа.';vi='Đối với giá trị truy cập kiểu ""%1"", có dử dụng dạng truy cập"
"""%2"" với kiểu giá trị ""%3"" đã đặt trong mô-đun định trước."
"Nhưng kiểu này không trùng với kiểu ""%4"""
"trong biểu mẫu giá trị truy cập của mục tin ГруппаДоступа.'"),
	...
	If ValueGroupsForChanges.Count() = 0
	   AND NewCreation Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Permitted ""%1"" are required for adding.';ru='Для добавления требуются разрешенные ""%1"".';vi='Để bổ sung cần cho phép ""%1"".'"),
...
Function AccessValuesGroupsAllowingAccessValuesChange(AccessValuesType, ReturnAll = False) Export
	...
	ErrorTitle =
		NStr("en='An error occurred in the AccessValueGroupsAllowingAccessValuesChange procedure "
"of the AccessManagement common module.';ru='Ошибка в процедуре ГруппыЗначенийДоступаРазрешающиеИзменениеЗначенийДоступа"
"общего модуля УправлениеДоступом.';vi='Lỗi trong thủ tục AccessValueGroupsAllowingAccessValuesChange"
"mô-đun chung AccessManagement.'");
...
"""Write dependent access value sets"".';ru='Неверные параметры."
"Тип объекта ""%1"""
"не найден ни в одной из подписок на события"
"""Записать наборы значений доступа"","
"""Записать зависимые наборы значений доступа"".';vi='Sai tham số."
"Kiểu đối tượng ""%1"""
"không tìm thấy trong bất kỳ giấy đặt mua sự kiện"
"""Ghi nhận tập hợp giá trị truy cập"","
"""Ghi nhận tập hợp giá trị truy cập phụ thuộc"".'"),
			ValueTypeObject);
	EndIf;
	...
	If Table.Count() = 0 Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Object ""%1"" generated an empty access value set.';ru='Объект ""%1"" сформировал пустой набор значений доступа.';vi='Đối tượng ""%1"" tạo tập hợp rỗng giá trị truy cập.'"),
			ValueTypeObject);
	EndIf;
...
// ProfileDescription.Description  = NStr("en='User';ru='Пользователь';vi='Người sử dụng'");
...
""
"После обновления будут повторяться настройки следующих новых имен прав: %1.';vi='Lỗi trong tham số thủ tục RenameRightInObjectRightsSettings mô-đun chung AccessManagement."
""
"Sau khi cập nhật, sẽ tùy chỉnh lại tên gọi của các quyền sau: %1.'"),
				RepeatedRightsNewNames);
		EndIf;
...
""
"Specify only special access"
"kinds ""ReadingRight"", ""ChangingRight"" if they are used.';ru='Объект ""%1"" сформировал набор значений доступа,"
"содержащий известный вид доступа ""%2"", который не требуется указывать."
""
"Указывать требуется только специальные виды доступа"
"""ПравоЧтения"", ""ПравоИзменения"", если они используются.';vi='Đối tượng ""%1"" đã tạo tập hợp các giá trị truy cập"
"có chứa dạng truy cập đã biết ""%2"" mà không cần chỉ ra."
""
"Chỉ cần chỉ ra các dạng truy cập đặc biệt"
"""ПравоЧтения"", ""ПравоИзменения"", nếu chúng được sử dụng.'"),
				TypeOf(ObjectReference),
				String.AccessKind);
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Object ""%1"" generated access"
"values set containing unknown access kind ""%2"".';ru='Объект ""%1"" сформировал набор значений доступа,"
"содержащий неизвестный вид доступа ""%2"".';vi='Đối tượng ""%1"" đã tạo tập hợp các giá trị truy cập"
"bao gồm dạng truy cập không xác định ""%2"".'"),
				TypeOf(ObjectReference),
				String.AccessKind);
		EndIf;
...
Function AccessValuesGroupsProperties(AccessValueType, ErrorTitle)
	...
	If AccessTypeProperties = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			ErrorTitle + Chars.LF + Chars.LF +
			NStr("en='For the access values of the ""%1"" type"
"access value groups are not used.';ru='Для значений доступа типа ""%1"""
"не используются группы значений доступа.';vi='Đối với giá trị truy cập kiểu ""%1"""
"không sử dụng nhóm giá trị truy cập.'"),
			String(AccessValueType));
	EndIf;
