...
Procedure ConnectEquipmentEnd(ExecutionResult, Parameters) Export
	...
	If Not ExecutionResult.Result Then
		MessageText = NStr("en='An error occurred when connecting the equipment:""%ErrorDetails%"".';ru='При подключении оборудования произошла ошибка:""%ОписаниеОшибки%"".';vi='Khi kết nối thiết bị, đã xảy ra lỗi: ""%ErrorDescription%"".'");
...
Procedure DisableEquipmentEnd(ExecutionResult, Parameters) Export
	...
	If Not ExecutionResult.Result Then
		MessageText = NStr("en='An error occurred when disconnecting the equipment: ""%ErrorDescription%"".';ru='При отключении оборудования произошла ошибка: ""%ОписаниеОшибки%"".';vi='Khi ngắt thiết bị, đã xảy ra lỗi: ""%ErrorDescription%"".'");
