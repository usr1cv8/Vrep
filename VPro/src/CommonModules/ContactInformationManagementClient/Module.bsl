...
Procedure GoToWebLink(Val FieldValues, Val Presentation = "", ExpectedKind = Undefined) Export
	...
	If TypeOf(ReferenceAddress) <> Type("String") Then
		Raise NStr("en='An error occurred when receiving the reference, incorrect contact information type';ru='Ошибка получения ссылки, неверный тип контактной информации';vi='Lỗi nhận tham chiếu, kiểu thông tin liên hệ không đúng'");
	EndIf;
...
Function AndBitwise(BinaryArray1, BinaryArray2)
	Result = New Array;
...
Function ORBitwise(BinaryArray1, BinaryArray2)
	Result = New Array;
	For Index = 0 To BinaryArray1.Count()-1 Do
...
Procedure AfterSelectionFromMenuSkype(SelectedItem, Parameters) Export
	...
	#If Not WebClient And Not MobileClient Then
		If IsBlankString(TelephonySoftwareIsInstalled("skype")) Then
			ShowMessageBox(Undefined, NStr("en='To make a call on Skype is required to install the program.';ru='Для совершения звонка по Skype требуется установить программу.';vi='Để thực hiện cuộc gọi qua Skype, cần cài đặt chương trình.'"));
			Return;
		EndIf;
	#EndIf
	...
	MessageText = NStr("en='To start Skype, you must install the extension work with files.';ru='Для запуска Skype необходимо установить расширение работы с файлами.';vi='Để khởi động Skype, cần cài đặt phần mở rộng làm việc với tệp.'");
