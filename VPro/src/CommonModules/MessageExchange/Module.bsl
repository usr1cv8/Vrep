...
Procedure DeliverMessages() Export
	...
	If TransactionActive() Then
		Raise NStr("en='You cannot receive quick system messages in an active transaction.';ru='Доставка быстрых сообщений системы не может выполняться в активной транзакции.';vi='Việc truyền nhanh thông báo của hệ thống không thể thực hiện trong giao tác đang làm việc.'");
	EndIf;
	...
	ElsIf TypeOf(Recipient) = Type("Array") Then
		...
		For Each Item IN Recipient Do
			...
			If TypeOf(Item) <> Type("ExchangePlanRef.MessageExchange") Then
				...
				Raise NStr("en='Incorrect recipient is specified for the MessagesExchange.SendMessage() method.';ru='Указан неправильный получатель для метода ОбменСообщениями.ОтправитьСообщение().';vi='Chỉ ra sai người nhận đối với phương thức MessagesExchange.SendMessage().'");
...
Procedure SendMessageToRecipient(MessageChannel, MessageBody, Recipient, IsInstantMessage)
	...
	If Not TransactionActive() Then
		...
		Raise NStr("en='Messages can be sent only in a transaction.';ru='Отправка сообщений может выполняться только в транзакции.';vi='Gửi thông báo chỉ có thể thực hiện trong giao tác.'");
		
	EndIf;
	
	If Not ValueIsFilled(MessageChannel) Then
		
		Raise NStr("en='Value of the MessagesChannel parameter is not specified for the MessagesExchange.SendMessage method.';ru='Не задано значение параметра ""КаналСообщений"" для метода ОбменСообщениями.ОтправитьСообщение.';vi='Chưa chỉ ra giá trị tham số ""MessagesChannel"" đối với phương thức MessagesExchange.SendMessage method.'");
		
	ElsIf StrLen(MessageChannel) > 150 Then
		
		Raise NStr("en='The length of message channel name cannot be longer than 150 characters.';ru='Длина имени канала сообщений не должна превышать 150 символов.';vi='Chiều dài của tên kênh thông báo không được vượt quá 150 ký tự.'");
		
	ElsIf Not ValueIsFilled(Recipient) Then
		
		Raise NStr("en='Value of the Recipient parameter is not specified for the MessagesExchange.SendMessage method.';ru='Не задано значение параметра ""Получатель"" для метода ОбменСообщениями.ОтправитьСообщение.';vi='Chưa chỉ ra giá trị tham số ""Người nhận"" đối với phương thức ОбменСообщениями.ОтправитьСообщение.'");
		
	ElsIf CommonUse.ObjectAttributeValue(Recipient, "Locked") Then
		
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Attempting to send message to the locked endpoint ""%1"".';ru='Попытка отправки сообщения заблокированной конечной точке ""%1"".';vi='Thử gửi thông báo cho điểm cuối đã khóa'"),
			String(Recipient));
	EndIf;
