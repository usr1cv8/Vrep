...
Function ProgramEventsParameters() Export
	...
	If Not SavedParameters.Property("EventsHandlers") Then
		ParameterPresentation = NStr("en='EVENT HANDLERS';ru='Обработчики событий';vi='Hàm sự kiện'");
	EndIf;
...
"""%1""';ru='Ошибка обновления информационной базы."
"Не заполнен параметр служебных событий:"
"""%1"".';vi='Lỗi cập nhật cơ sở thông tin."
"Chưa điền tham số sự kiện hệ thống:"
"""%1"".'")
...
"does not match configuration version in metadata ""%4"".';ru='Ошибка при подготовке описаний подсистем:"
"версия ""%2"" конфигурации ""%1"" (см. процедуру %3.ПриДобавленииПодсистемы)"
"не совпадает с версией конфигурации в метаданных ""%4"".';vi='Lỗi khi soạn mô tả phân hệ:"
"phiên bản ""%2"" của cấu hình ""%1"" (xem thủ tục %3.ПриДобавленииПодсистемы)"
"không trùng với phiên bản cấu hình trong metadata ""%4"".'"),
				Description.Name,
				Description.Version,
	...
	For Each KeyAndValue IN AllRequiredSubsystems Do
		If SubsystemDescriptions.ByNames.Get(KeyAndValue.Key) = Undefined Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='An error occurred while"
"preparing subsystems descriptions: subsystem ""%1"" is not found required for subsystems: %2.';ru='Ошибка при подготовке описаний подсистем:"
"не найдена подсистема ""%1"" требуемая для подсистем: %2.';vi='Lỗi khi chuẩn bị mô tả phân hệ:"
"chưa tìm thấy phân hệ ""%1"" cần cho phân hệ: %2.'"),
				KeyAndValue.Key,
				DependentSubsystems);
		EndIf;
	EndDo;
...
""
"Объект метаданных не найден по полному имени: ""%1"".';vi='Lỗi khi thực hiện hàm CommonUse.MetadataObjectID()."
""
"Không tìm thấy đối tượng metadata theo tên đầy đủ: ""%1"".'"),
...
""
"Для объекта метаданных ""%1"" не найден идентификатор в справочнике ""Идентификаторы объектов метаданных"".';vi='Lỗi khi thực hiện hàm CommonUse.MetadataObjectID()."
""
"Đối với đối tượng metadata ""%1"" không tìm thấy tên (ID) trong danh mục ""Tên (ID) đối tượng metadata"".'")
			+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
			FullMetadataObjectName);
...
""
"Для объекта метаданных ""%1"" найдено несколько идентификаторов в справочнике ""Идентификаторы объектов метаданных"".';vi='Lỗi khi thực hiện hàm CommonUse function.MetadataObjectID()."
""
"Đối với đối tượng metadata ""%1"" đã tìm thấy nhiều tên (ID) trong danh mục ""Tên (ID) đối tượng metadata"".'")
			+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
			FullMetadataObjectName);
	EndIf;
...
""
"Для объекта метаданных ""%1"" найден идентификатор в справочнике ""Идентификаторы объектов метаданных"", которому соответствует удаленный объект метаданных.';vi='Lỗi khi thực hiện hàm CommonUse function.MetadataObjectID()."
""
"Đối với đối tượng metadata ""%1"" đã tìm thấy tên (ID) trong danh mục ""Tên (ID) đối tượng metadata"" tương ứng với đối tượng metadata đã xóa.'")
			+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
				FullMetadataObjectName);
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
...
""
"Для объекта метаданных ""%1"" найден идентификатор в справочнике ""Идентификаторы объектов метаданных"", который соответствует другому объекту метаданных ""%2"".';vi='Lỗi khi thực hiện hàm CommonUse function.MetadataObjectID()."
""
"Đối với đối tượng metadata ""%1"" đã tìm thấy tên (ID) trong danh mục ""Tên (ID) đối tượng metadata"" tương ứng với đối tượng metadata khác ""%2"".'")
			+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
				FullMetadataObjectName,
...
"- ReportsMail, "
"- AccessManagement.';ru='Невозможно отключить справочник Идентификаторы объектов метаданных,"
"если используется любая из следующих подсистем:"
"- ВариантыОтчетов,"
"- ДополнительныеОтчетыИОбработки,"
"- РассылкаОтчетов,"
"- УправлениеДоступом.';vi='Không thể ngắt danh mục Tên định danh đối tượng metadata,"
"nếu sử dụng một trong các phân hệ sau đây:"
"- ВариантыОтчетов,"
"- ДополнительныеОтчетыИОбработки,"
"- РассылкаОтчетов,"
"- УправлениеДоступом.'");
...
Function PreparedByHandlersOfServerEvents(Event, Service = False, FirstTry = True)
	...
	If Handlers = Undefined Then
		If Service Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Server service event is not found %1.';ru='Не найдено серверное служебное событие ""%1"".';vi='Chưa tìm thấy sự kiện hệ thống Server ""%1"".'"), Event);
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Server event is not found %1.';ru='Не найдено серверное событие ""%1"".';vi='Chưa tìm thấy sự kiện Server ""%1"".'"), Event);
		EndIf;
	EndIf;
