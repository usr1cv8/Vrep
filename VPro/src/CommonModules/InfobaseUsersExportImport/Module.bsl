...
Function RunModeString(Val RunMode)
	...
	Else
		MessagePattern = NStr("en='Unknown start mode of client application %1';ru='Неизвестный режим запуска клиентского приложения %1';vi='Sai chế độ khởi động ứng dụng Client %1'");
		MessageText = ServiceTechnologyIntegrationWithSSL.SubstituteParametersInString(MessagePattern, RunMode);
		Raise(MessageText);
	EndIf;
