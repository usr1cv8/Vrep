...
Procedure NodeConfigurationFormCommandCloseForm(Form) Export
	...
	OnCloseExchangePlanNodeSettingsForm(Form, "FilterSettingsAtNode");
...
Procedure SettingFormBeforeClose(Cancel, Form) Export
	...
	If Form.Modified Then
		...
		QuestionText = NStr("en='Data was changed. Close the form without saving the changes?';ru='Данные изменены. Закрыть форму без сохранения изменений?';vi='Dữ liệu đã thay đổi. Đóng biểu mẫu mà không lưu lại các thay đổi?'");
...
Procedure OpenDataExchangeSettingAssistant(Val ExchangePlanName, ExchangeWithServiceSetup = False) Export
	...
	If Find(ExchangePlanName, "SettingID") > 0 Then
		...
		AdditionalSetting = Right(ExchangePlanName, StrLen(ExchangePlanName) - CharactersQuantityExchangePlanName - 9);
...
Function HyperlinkHeadersOfDataImport() Export
	...
	Structure = New Structure;
	Structure.Insert("Undefined",               NStr("en='Data receive was not performed';ru='Получение данных не выполнялось';vi='Chưa thực hiện nhận dữ liệu'"));
	Structure.Insert("Error",                     NStr("en='Cannot receive data';ru='Не удалось получить данные';vi='Không thể nhận dữ liệu'"));
	Structure.Insert("CompletedWithWarnings", NStr("en='Data is received with warnings';ru='Данные получены с предупреждениями';vi='Đã nhận dữ liệu kèm cảnh báo'"));
	Structure.Insert("Success",                      NStr("en='Data is successfully received';ru='Данные успешно получены';vi='Đã nhận thành công dữ liệu'"));
	Structure.Insert("Execution",                 NStr("en='Receiving data ...';ru='Выполняется получение данных...';vi='Đang nhận dữ liệu...'"));
	
	Structure.Insert("Warning_ExchangeMessageHasBeenPreviouslyReceived", NStr("en='No new data to receive';ru='Нет новых данных для получения';vi='Không có dữ liệu mới để nhận'"));
	Structure.Insert("Error_MessageTransport",                      NStr("en='Cannot receive data';ru='Не удалось получить данные';vi='Không thể nhận dữ liệu'"));
...
Function HyperlinkHeadersOfDataDump() Export
	...
	Structure = New Structure;
	Structure.Insert("Undefined", NStr("en='Data was not sent';ru='Отправка данных не выполнялась';vi='Chưa thực hiện gửi dữ liệu'"));
	Structure.Insert("Error",       NStr("en='Cannot send data';ru='Не удалось отправить данные';vi='Không thể gửi dữ liệu'"));
	Structure.Insert("Success",        NStr("en='Data is successfully sent';ru='Данные успешно отправлены';vi='Đã gửi thành công dữ liệu'"));
	Structure.Insert("Execution",   NStr("en='Sending data...';ru='Выполняется отправка данных...';vi='Đang gửi dữ liệu...'"));
	
	Structure.Insert("Warning_ExchangeMessageHasBeenPreviouslyReceived", NStr("en='Data is sent with warnings';ru='Данные отправлены с предупреждениями';vi='Đã gửi dữ liệu kèm cảnh báo'"));
	Structure.Insert("CompletedWithWarnings",                     NStr("en='Data is sent with warnings';ru='Данные отправлены с предупреждениями';vi='Đã gửi dữ liệu kèm cảnh báo'"));
	Structure.Insert("Error_MessageTransport",                      NStr("en='Cannot send data';ru='Не удалось отправить данные';vi='Không thể gửi dữ liệu'"));
...
Procedure FileDirectoryChoiceHandler(Object, Val PropertyName, StandardProcessing = False, Val DialogueParameters = Undefined, CompletionAlert = Undefined) Export
	...
	DialogDefaults.Insert("Title", NStr("en='Specify directory';ru='Укажите каталог';vi='Hãy chỉ ra thư mục'") );
	
	SetStructureDefaultValues(DialogueParameters, DialogDefaults);
	
	WarningText = NStr("en='Install extension for 1C:Enterprise web client for this operation.';ru='Для данной операции необходимо установить расширение для веб-клиента 1С:Предприятие.';vi='Đối với giao dịch này, cần thiết lập phần mở rộng đối với Web-client 1C:DOANH NGHIỆP.'");
...
Procedure FileChoiceHandler(Object, Val PropertyName, StandardProcessing = False, Val DialogueParameters = Undefined, CompletionAlert = Undefined) Export
	...
	DialogDefaults.Insert("Title",                   NStr("en='Select file';ru='Выберите файл';vi='Hãy chọn tệp'"));
...
Procedure PassFilesToServer(CompletionAlert, Val FileNames, Val FormID = Undefined, Val WarningText = Undefined) Export
	...
	For Each FileName IN FileNames Do
		...
		If IsBlankString(FileName) Then
			HasEmpty = True;
			FileDescription.ErrorDescription = NStr("en='File is not selected.';ru='Файл не выбран.';vi='Chưa chọn tệp.'");
		EndIf;
	...
	If WarningText = Undefined Then
		If FileNames.Count() = 1 Then
			WarningText = NStr("en='To transfer the file to the server, install the 1C:Enterprise web client.';ru='Для передачи файла на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.';vi='Để chuyển tệp lên Server, cần thiết lập phần mở rộng đối với Web-server 1C:DOANH NGHIỆP.'");
		Else
			WarningText = NStr("en='To transfer files to the server, install extension for web client 1C:Enterprise.';ru='Для передачи файлов на сервер необходимо установить расширение для веб-клиента 1С:Предприятие.';vi='Để chuyển tệp lên Server, cần thiết lập phần mở rộng đối với Web-client 1C:DOANH NGHIỆP.'");
		EndIf;
	EndIf;
...
Procedure PassFileToServerEndExtensionConnectionAfterFileExistenceCheck(Exists, AdditionalParameters) Export
	...
	If Not Exists Then
		...
		Item.ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='File ""%1"" does not exist or cannot be accessed';ru='Файл ""%1"" не существует или к нему нет доступа';vi='Tệp ""%1"" không tồn tại hoặc không được truy cập đến tệp'"), Item.Name);
...
Procedure SelectAndSaveFileOnClientAfterWorksWithFilesExtensionConnection(Attached, AdditionalParameters) Export
	...
	DialogDefaults = New Structure;
	DialogDefaults.Insert("Title",               NStr("en='Select a file to save to';ru='Выберите файл для сохранения';vi='Hãy chọn tệp để lưu'"));
	DialogDefaults.Insert("Multiselect",      False);
	DialogDefaults.Insert("Preview", False);
...
Procedure DeleteSynchronizationSetting(Val InfobaseNode) Export
	...
	QuestionText = NStr("en='Delete data synchronization setting?';ru='Удалить настройку синхронизации данных?';vi='Xóa tùy chỉnh đồng bộ hóa dữ liệu?'");
...
Procedure FormOpeningHandlerAfterClosingCurrentOne(Val ClosingResult, Val AdditionalParameters) Export
	...
	OpenForm(AdditionalParameters.FormName, AdditionalParameters.Parameters,
		OpenParameters.Owner, OpenParameters.Uniqueness, OpenParameters.Window, OpenParameters.NavigationRef, OpenParameters.ClosingAlertDescription, OpenParameters.WindowOpeningMode
	);
...
Procedure OnInstructionOpenHowToChangeDataSynchronizationPassword(Val AddressForAccountPasswordRecovery) Export
	...
	If IsBlankString(AddressForAccountPasswordRecovery) Then
		...
		ShowMessageBox(, NStr("en='Address for account password recovery is not set.';ru='Адрес для восстановления пароля учетной записи не задан.';vi='Chưa đặt địa chỉ để khôi phục mật khẩu trương mục E-mail.'"));
...
Function OpenFormAdditionsExportingsAllDocuments(Val ExportAddition, Val Owner=Undefined, Val Uniqueness=Undefined, Val Window=Undefined) Export
	...
	FormParameters.Insert("Title", NStr("en='Add documents to send';ru='Добавление документов для отправки';vi='Thêm chứng từ để gửi'") );
