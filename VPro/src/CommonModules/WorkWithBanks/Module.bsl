...
Function permissions()
	...
	Port = Undefined;
	Definition = NStr("en='Import banks classifier from the Internet.';ru='Загрузка классификатора банков из интернета.';vi='Kết nhập bảng mã hiệu ngân hàng từ Internet.'");
...
Procedure SupplementMessageText(ClassifierImportParameters) Export
	...
	If IsBlankString(ClassifierImportParameters["MessageText"]) Then
		MessageText = NStr("en='The RF bank classifier was imported successfully.';ru='Загрузка классификатора банков РФ выполнена успешно.';vi='Việc kết nhập bảng mã hiệu ngân hàng đã thực hiện thành công.'");
	Else
	...
	If ClassifierImportParameters["Exported"] > 0 Then
		...
		NStr("en='"
"New updated: %1.';ru='"
"Загружено новых: %1.';vi='"
"Đã kết nhập mới: %1.'"), ClassifierImportParameters["Exported"]);
...
"Обновлено записей: %1.';vi='"
"Đã cập nhật bản ghi: %1.'"), ClassifierImportParameters["Updated"]);
...
Procedure GetRBCData(ClassifierImportParameters, StorageAddress = "") Export
	...
	Except
		MessageText = NStr("en='Problems with the bank classifier file received from the RBK website occurred.';ru='Возникли проблемы с файлом классификатора банков, полученным с сайта РБК.';vi='Phát sinh vấn đề với tệp bảng mã hiệu ngân hàng nhận được từ Website 1C.'");
...
"Архив не содержит информацию - классификатор банков.';vi='Đã có vấn đề với tệp bảng mã hiệu ngân hàng nhận từ Website 1C."
"Kho lưu trữ không chứa thông tin - bảng mã hiệu ngân hàng.'");
		ClassifierImportParameters.Insert("MessageText", MessageText);
		If Not IsBlankString(StorageAddress) Then
			PutToTempStorage(ClassifierImportParameters, StorageAddress);
		EndIf;		
...
"Архив не содержит информацию о регионах.';vi='Đã có vấn đề với tệp bảng mã hiệu ngân hàng nhận từ Website 1C."
"Kho lưu trữ không có thông tin về các địa phương.'");
		ClassifierImportParameters.Insert("MessageText", MessageText);
		If Not IsBlankString(StorageAddress) Then
			PutToTempStorage(ClassifierImportParameters, StorageAddress);
		EndIf;
...
"Архив не содержит информацию о недействующих банках.';vi='Phát sinh vấn đề với tệp chứa bảng mã hiệu ngân hàng đã nhận từ trang Web Ngân hàng nhà nước Việt Nam."
"Tệp không chứa thông tin về ngân hàng hết hiệu lực.'");
		ClassifierImportParameters.Insert("MessageText", MessageText);
		If Not IsBlankString(StorageAddress) Then
			PutToTempStorage(ClassifierImportParameters, StorageAddress);
		EndIf;
...
Procedure ImportBanksClassifier() Export
	...
	If CommonUse.IsSubordinateDIBNode() Then
		WriteLogEvent(EventNameInEventLogMonitor(), EventLevel, , , NStr("en='Import is not supported in subordinate DIB node';ru='Загрузка в подчиненном узле РИБ не предусмотрена';vi='Không thể kết nhập tại nút phụ CSTTPT'"));
		Return;
	EndIf;
...
"Возможно неточные или неправильные настройки подключения к Интернету на сервере 1С:Предприятие.';vi='"
"Có thể tùy chỉnh sai hoặc chưa chính xác việc kết nối đến Internet trên Server 1C:DOANH NGHIỆP."
"'");
		Else
			AdditionalMessage =
				NStr("en='"
...
Function GetBankFieldsStructure(Val String, States)
	...
	If Region = Undefined Then
		Region = NStr("en='Other territories';ru='Другие территории';vi='Khu vực khác'");
		StateCode = "";
	EndIf;
		...
		If Not File.Exist() Then
			WriteErrorInEventLogMonitor(
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='File %1 is not found';ru='Не найден файл %1';vi='Không tìm thấy tệp %1'"),
					FileName));
			Result = False;
		EndIf;
	EndDo;
...
Function EventNameInEventLogMonitor()
	...
	Return NStr("en='Banks classifier import. RBK website';ru='Загрузка классификатора банков. Сайт РБК';vi='Kết nhập bảng mã hiệu ngân hàng. Website 1C'", CommonUseClientServer.MainLanguageCode());
