...
Function ReportByObjectVersioning(ObjectReference, Val ObjectVersioning = Undefined) Export
	...
	EndIf;
	Definition = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='№ %1 / (%2) / %3';ru='№ %1 / (%2) / %3';vi='TT %1 / (%2) / %3'"), 
...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	Handler.Comment = NStr("en='Update information about written object versions.';ru='Обновление сведений о записанных версиях объектов.';vi='Cập nhật thông tin về các phiên bản đã ghi lại của đối tượng.'");
	
	Handler = Handlers.Add();
	Handler.Version = "2.2.4.13";
	Handler.Procedure = "ObjectVersioning.TransferVersioningSettings";
	Handler.Comment = NStr("en='Versioning setting update.';ru='Обновление настроек версионирования.';vi='Cập nhật tùy chỉnh phiên bản hóa.'");
...
Procedure RefreshInformationAboutObjectsVersions(Parameters) Export
	...
	While Selection.Next() Do
		...
		Except
			WriteLogEvent(
				NStr("en='Versioning';ru='Версионирование';vi='Đánh số phiên bản'", CommonUseClientServer.MainLanguageCode()),
...
"%3';vi='Không thể cập nhật thông tin về phiên bản số %1 của đối tượng ""%2"" theo nguyên nhân:"
	...
	If Selection.Count() > 0 Then
		If RecordsProcessed = 0 Then
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='The RefreshInformationAboutObjectsVersions procedure cannot process some records of information register ObjectVersionings (skipped): %1';ru='Процедуре RefreshInformationAboutObjectsVersions не удалось обработать некоторые записи регистра сведений ObjectVersionings (пропущены): %1';vi='Thủ tục RefreshInformationAboutObjectsVersions không thể xử lý nhiều bản ghi của biểu ghi thông tin ObjectVersionings (bỏ qua): %1'"), 
					Selection.Count());
			Raise MessageText;
		EndIf;
...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	ToolTip = NStr("en='Obsolete versions: %1 (%2)';ru='Устаревших версий: %1 (%2)';vi='Phiên bản cũ: %1 (%2)'");
	...
	For Each Section IN Sections Do
		...
		Work.Presentation = NStr("en='Obsolete object versions';ru='Устаревшие версии объектов';vi='Phiên bản cũ của đối tượng'");
		...
		If Not Object.IsNew() AND (PostingChanged Or VersionDiffersFromPreviouslyRecorded(Object)) Then
			...
			If InfoAboutObjectVersion.VersionNumber = 1 Then
				If ObjectIsVersioning(Object.Ref, False) Then
					...
					VersionParameters.Insert("Comment", NStr("en='Version was created from the existing object';ru='Версия создана по уже имеющемуся объекту';vi='Phiên bản đã tạo theo đối tượng đã có'"));
...
Function CollisionsOrUnacceptedQuantity(ExchangeNodes, ThisIsCollisionsQuantity,
	...
	ElsIf ExchangePlans.AllRefsType().ContainsType(TypeOf(ExchangeNodes)) Then
		FilterRow = "AND ObjectVersionings.VersionAuthor = & ExchangeNodes";
		Query.SetParameter("ExchangeNodes", ExchangeNodes);
	Else
		FilterRow = "AND ObjectVersionings.VersionAuthor In (&ExchangeNodes)";
		Query.SetParameter("ExchangeNodes", ExchangeNodes);
	EndIf;
	...
	If ValueIsFilled(Period) Then
		...
		FilterRow = "And (ObjectVersionings.VersionDate >= &StartDate And ObjectVersionings.VersionDate <= &EndDate)";
	...
	If ValueIsFilled(ThisIsCollisionsQuantity) Then
		...
		Else
			...
			If ValueIsFilled(SearchString) Then
				...
				FilterRow = "AND ObjectVersionings.Comment LIKE &Comment";
...
Function ObjectFromBinaryData(BinaryData)
	...
	If XMLReader.Read() Then
		...
		Else
			XMLReader.Close();
			Raise NStr("en='An error occurred while restoring the object';ru='Ошибка при восстановлении объекта';vi='Lỗi khi phục hồi đối tượng'");
		EndIf;
	Else
		XMLReader.Close();
		Raise NStr("en='Data reading error';ru='Ошибка чтения данных';vi='Lỗi đọc dữ liệu'");
	EndIf;
...
Procedure WriteOverPreviousVersion(Ref, Object, VersionNumber, InfoAboutObjectVersion)
	...
	If RecordSet.Count() = 0 Then
		...
		VersionsData.Insert("Comment", NStr("en='Version was created during data synchronization.';ru='Версия создана при синхронизации данных.';vi='Phiên bản đã tạo khi đồng bộ hóa dữ liệu.'"));
...
Procedure ValidateRightsForObjectModifying(MetadataObject)
	...
	If Not PrivilegedMode() AND Not AccessRight("Update", MetadataObject)Then
		MessageText = NStr("en='Insufficient rights to change ""%1"".';ru='Недостаточно прав на изменение ""%1"".';vi='Không đủ quyền thay đổi ""%1"".'");
...
Procedure OnTransitionToObjectVersioning(ObjectReference, Val VersionNumber) Export
	...
	Object.AdditionalProperties.Insert("ObjectVersioningCommentToVersion",
		StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Transfer to version No. %1 from %2 is performed';ru='Выполнен переход к версии №%1 от %2';vi='Đã thực hiện chuyển sang phiên bản số %1 ngày %2'"),
...
Function RestoreObjectByXML(Val AddressInTemporaryStorage = "", ErrorMessageText = "") Export
	...
	Except
		WriteLogEvent(NStr("en='Versioning';ru='Версионирование';vi='Đánh số phiên bản'", CommonUseClientServer.MainLanguageCode()),
...
"Техническая информация об ошибке: %1';vi='Không thể chuyển sang phiên bản đã chọn."
"Nguyên nhân có thể: phiên bản đối tượng đã được ghi lại vào phiên bản khác của chương trình."
"Thông tin kỹ thuật về lỗi: %1'");
...
Function InfoAboutObjectVersion(Val Ref, Val VersionNumber) Export
	MessageFailedToGetVersion = NStr("en='Cannot receive a previous version of the object.';ru='Не удалось получить предыдущую версию объекта.';vi='Không thể nhận phiên bản đối tượng trước đó.'");
	...
	Result = New Structure("ObjectVersioning, VersionAuthor, VersionDate, Comment");
	...
	If Result.ObjectVersioning = Undefined Then
		Raise NStr("en='The selected object version is missing in the application.';ru='Выбранная версия объекта отсутствует в программе.';vi='Phiên bản đã chọn của đối tượng không có trong chương trình.'");
	EndIf;
...
Procedure AddingInformationAboutVersionOfObjectInThe(Object, VersionAuthor)
	...
	If TypeOf(Object) <> Type("ObjectDeletion") AND CommonUse.ThisIsObjectOfReferentialType(Object.Metadata()) Then
		...
		InfoAboutObjectVersion.Insert("Comment", NStr("en='Version is received while synchronizing data.';ru='Версия получена при синхронизации данных.';vi='Phiên bản đã nhận khi đồng bộ hóa dữ liệu.'"));
...
Procedure WriteObjectVersion(Source, WriteModePosting = False) Export
	...
	//	Return;
	//EndIf;
	// Unconditional check for DataExchange.Load is not required, because at the time of writing 
	// a versioned object during exchange, it is necessary to save its current version in the version history.
	
	If Not GetFunctionalOption("UseObjectVersioning") Then
		Return;
	EndIf;
	
	If CommonUse.ReferenceTypeValue(Source) Then
...
Function DataSizeString(Val DataSize) Export
	...
	MeasurementUnit = NStr("en='byte';ru='байт';vi='byte'");
	If 1024 <= DataSize AND DataSize < 1024 * 1024 Then
		DataSize = DataSize / 1024;
		MeasurementUnit = NStr("en='MB';ru='MB';vi='MB'");
	ElsIf 1024 * 1024 <= DataSize AND  DataSize < 1024 * 1024 * 1024 Then
		DataSize = DataSize / 1024 / 1024;
		MeasurementUnit = NStr("en='Gb';ru='Гб';vi='Gb'");
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='%1 %2';ru='%1 %2';vi='%1 %2'"), DataSize, MeasurementUnit);
...
Function GetAttributePresentationInLanguage(Val AttributeName) Export
	...
	If AttributeName = "Number" Then
		Return NStr("en='Number';ru='Номер';vi='STT'");
	ElsIf AttributeName = "Name" Then
		Return NStr("en='Name';ru='Наименование';vi='Tên gọi'");
	ElsIf AttributeName = "Code" Then
		Return NStr("en='Code';ru='Код';vi='Mã'");
	ElsIf AttributeName = "IsFolder" Then
		Return NStr("en='Is folder';ru='Это группа';vi='Đây là nhóm'");
	ElsIf AttributeName = "Description" Then
		Return NStr("en='Description';ru='Наименование';vi='Tên gọi'");
	ElsIf AttributeName = "Date" Then
		Return NStr("en='Date';ru='Дата';vi='Ngày'");
	ElsIf AttributeName = "Posted" Then
		Return NStr("en='Posted';ru='Проведен';vi='Đã kết chuyển'");
	ElsIf AttributeName = "DeletionMark" Then
		Return NStr("en='Deletion mark';ru='Пометка удаления';vi='Đặt dấu xóa'");
	ElsIf AttributeName = "Ref" Then
		Return NStr("en='Ref';ru='Ссылка';vi='Tham chiếu'");
	ElsIf AttributeName = "Parent" Then
		Return NStr("en='Parent';ru='Родитель';vi='Lớp trên'");
	ElsIf AttributeName = "Owner" Then
		Return NStr("en='Owner';ru='Владелец';vi='Đối tượng chủ'");
	Else
