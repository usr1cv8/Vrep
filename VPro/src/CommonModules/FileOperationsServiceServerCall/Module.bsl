...
Function SaveChangesAndReleaseFile(FileData, FileInformation,
	...
	If Not FileDataCurrent.CurrentUserIsEditing Then
		Raise NStr("en='File is not used by the current user';ru='Файл не занят текущим пользователем';vi='Tệp không bị chiếm bởi người sử dụng hiện tại'");
	EndIf;
	...
	If FileStorageType = Enums.FileStorageTypes.InVolumesOnDrive Then
		If Not VersionRef.Volume.IsEmpty() Then
			...
			Except
				...
				WriteLogEvent(
					NStr("en='Files.Open file';ru='Файлы.Открытие файла';vi='Tệp.Mở tệp'",
...
Function FilesImportGenerateReport(FilenamesWithErrorsArray) Export
	...
	HeaderArea.Parameters.Description = NStr("en='Cannot import the following files:';ru='Не удалось загрузить следующие файлы:';vi='Không thể kết nhập những tệp sau:'");
