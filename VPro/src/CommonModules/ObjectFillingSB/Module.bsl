...
// 
// Parameters:
//  Object - DocumentObject - Обрабатываемый документ.
//  TSName - String - Имя обрабатываемой табличной части.
//  FieldName - String - Имя заполняемого поля.
//  NameOfPositionField - String - Имя поля текущего положения реквизита.
// 
Procedure FillTableByHeader(Object, TSName, FieldName, NameOfPositionField) Export
	
	If Object[NameOfPositionField]=Enums.AttributePositionOnForm.InTabularSection Then
		Return;
	EndIf;
	
	For Each TabularSectionRow In Object[TSName] Do
		TabularSectionRow[FieldName] = Object[FieldName];
	EndDo;
...
Procedure CallHandlerBeforeFilling(FillingStrategy, DocumentObject, FillingData)
	...
	If TypeOf(FillingStrategy) <> Type("Map") Then
		Raise NStr("en='Invalid parameter type ""FillingHandler"": expected String or Map.';ru='Некорректный тип параметра ""ОбработчикЗаполнения"": ожидается Строка или Соответствие.';vi='Sai kiểu tham số ""FillingHandler"": đang chờ kiểu String hoặc Map.'");
	EndIf;
...
Procedure SupplementValuesFromSettings(FillingData, DocumentObject)
	...
	SupplementCustomerOrderPositionInProductionDocuments(ValuesFromSettings, DocumentObject);
...
Procedure SupplementStampBase(ValuesFromSettings, DocumentObject)
	...
	If ValueIsFilled(DocumentObject.Contract) Then
		...
		"StampBase",
		StrTemplate(
		NStr("en='Contract: %1';ru='Договор: %1';vi='Hợp đồng: %1'"),
...
Procedure SupplementCustomerOrderPositionInProductionDocuments(ValuesFromSettings, DocumentObject)
	
	If TypeOf(DocumentObject.Ref) <> Type("DocumentRef.ProductionOrder") Then
		Return;
	EndIf;
	...
	CustomerOrderPositionInProductionDocuments = SmallBusinessReUse.GetValueByDefaultUser(
	Users.CurrentUser(),
	"CustomerOrderPositionInProductionDocuments");
	
	If ValueIsFilled(CustomerOrderPositionInProductionDocuments) Then
		ValuesFromSettings.Insert("CustomerOrderPosition", CustomerOrderPositionInProductionDocuments);
	Else
		ValuesFromSettings.Insert("CustomerOrderPosition", Enums.AttributePositionOnForm.InHeader);
	EndIf;
...
Procedure SupplementPredefinedPriceKind(CatalogPredefinedItems, DocumentObject)
	...
	If TypeOf(DocumentObject) = Type("DocumentObject.ReceiptCRReturn") Then
		Return;
	EndIf;
...
Procedure RenameEventFieldsProject(RenamedFields)
	...
	If Not GetFunctionalOption("ProjectManagement") Then
		Return;
	EndIf;
...
Procedure RenameFieldsCounterparty(RenamedFields)
	...
	If RenamedFields.Property("Contract")
		...
		And RenamedFields.Counterparty = CommonUse.ObjectAttributeValue(
		RenamedFields.Contract,
		"Owner") Then
		
	Else
		
		RenamedFields.Insert("Contract", Catalogs.CounterpartyContracts.ContractByDefault(RenamedFields.Counterparty));
...
Procedure RenameFieldsStructuralUnit(RenamedFields, DocumentObject)
	...
	StructuralUnitFieldsDescription.Insert("DisposalsStructuralUnit", "DisposalsRecipient");
