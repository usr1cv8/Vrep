...
Function OCRByName(ExchangeComponents, Name) Export
	...
	If ConversionRule = Undefined Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Credit slip with name %1 is not found';ru='Не найдено ПКО с именем %1';vi='Không tìm thấy phiếu trả tiền với tên gọi %1'"), Name);
...
"Имя"
"ПОД: %1 Описание ошибки: %2';vi='Lỗi thực hiện thuật toán nhận lựa chọn dữ liệu."
"Tên"
"DER: %1 Mô tả lỗi: %2'"),
...
Function XDTOObjectFromXDTOData(
	...
	For Each Property IN XDTOType.Properties Do
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Unknown property type <%1>. Object type: %2';ru='Неизвестный тип свойства <%1>. Тип объекта: %2';vi='Sai kiểu thuộc tính <%1>. Kiểu đối tượng: %2'"),
				Property.Name,
				String(XDTOType)
			);
		EndIf;
...
"Тип"
"источника: %1 Тип приемника: %2';vi='Lỗi sơ đồ XML. Đối với đích, cần đặt một thuộc tính."
"Kiểu"
"nguồn: %1 Kiểu đích: %2'"),
...
Procedure OpenExportFile(ExchangeComponents, ExchangeFileName = "") Export
	...
	If ExchangeComponents.IsExchangeThroughExchangePlan Then
		...
		If TransactionActive() Then
			Raise NStr("en='Exchange data lock cannot be set in an active transaction.';ru='Блокировка на обмен данными не может быть установлена в активной транзакции.';vi='Không thể phong tỏa trao đổi dữ liệu trong khi giao tác đang hoạt động.'");
		EndIf;
...
""
"Подробности:"
"%1';vi='Lỗi thiết lập khóa trao đổi dữ liệu."
"Có thể, thực hiện trao đổi dữ liệu bằng phiên làm việc khác."
""
"Chi tiết:"
"%1'"),
...
Procedure OpenImportFile(ExchangeComponents, ExchangeFileName) Export
	...
	While IterationNumber = 0 Do
		...
		Except
			...
			ErrorMessageString = NStr("en='An error occurred when importing data: %1';ru='Ошибка при загрузке данных: %1';vi='Lỗi khi kết nhập dữ liệu: %1'");
		...
		If IsExchangeThroughExchangePlan Then
			...
			If ExchangePlanManager.ExchangeFormat() <> ExchangeFormat.BasicFormat Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Exchange message format <%1> does not correspond to exchange plan format <%2>.';ru='Формат сообщения обмена <%1> не соответствует формату плана обмена <%2>.';vi='Định dạng thông báo trao đổi <%1> không tương ứng với định dạng của sơ đồ trao đổi <%2>.'"),
			...
			If ExchangeFormatVersionsArray(ExchangeComponents.CorrespondentNode).Find(ExchangeFormat.Version) = Undefined Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Version of exchange message format <%1> is not supported.';ru='Версия формата сообщения обмена <%1> не поддерживается.';vi='Không hỗ trợ phiên bản định dạng thông báo trao đổi <%1>.'"),
		...
		If MaxCommonVersion <> CorrespondentVersionNumber Then
			...
			WriteInExecutionProtocol(ExchangeComponents, 
										NStr("en='Exchange format version number is changed.';ru='Изменен номер версии формата обмена.';vi='Đã thay đổi số phiên bản định dạng trao đổi.'"),,False,,, True);
		EndIf;
...
Procedure WriteObjectToIB(ExchangeComponents, Object, Type, WriteObject = False, Val SendBack = False, UUIDString = "") Export
	...
	If CommonUseReUse.DataSeparationEnabled()
		...
		ErrorMessageString = NStr("en='Attempting to change shared data (%1) in split mode.';ru='Попытка изменение неразделенных данных (%1) в разделенном режиме.';vi='Thử thay đổi dữ liệu không chia tách (%1) ở chế độ chia tách.'");
...
Function FormatVersionExchangeManager(Val InfobaseNode, Val FormatVersion) Export
	...
	If Result = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Conversion Manager for exchange format version <%1> is not defined.';ru='Не определен Менеджер конвертации для версии формата обмена <%1>.';vi='Chưa xác định Bộ quản lý chuyển đổi đối với phiên bản định dạng trao đổi <%1>.'"),
			FormatVersion
		);
	EndIf;
...
"Задано более одного правила конвертации для типа источника <%1>.';vi='Lỗi quy tắc chuyển đổi dữ liệu định trước."
"Đã đặt hơn một quy tắc chuyển đổi đối với kiểu nguồn <%1>.'"),
				String(Type)
			);
...
"Правило конвертации не определено для типа источника <%1>.';vi='Lỗi quy tắc chuyển đổi dữ liệu định trước."
"Chưa xác định quy tắc chuyển đổi đối với kiểu nguồn <%1>.'"),
			String(Type)
		);
...
"Задано более одного правила конвертации для типа источника <%1> и типа приемника <%2>.';vi='Lỗi quy tắc chuyển đổi dữ liệu định trước."
"Đã đặt hơn một quy tắc chuyển đổi đối với kiểu nguồn <%1> và kiểu đích <%2>.'"),
				String(Type),
				String(XDTOType)
			);
...
"Правило конвертации не определено для типа источника <%1> и типа приемника <%2>.';vi='Lỗi quy tắc chuyển đổi giá trị định trước."
"Không xác định quy tắc chuyển đổi đối với kiểu nguồn <%1> và kiểu đích <%2>.'"),
			String(Type),
			String(XDTOType)
		);
...
"Тип значения"
"источника: <%1> Значение источника: <%2>';vi='Không tìm thấy quy tắc chuyển đổi đối với giá trị dữ liệu định trước."
"Kiểu giá trị"
"nguồn:  <%1> Giá trị nguồn: <%2>'"),
			TypeOf(Value),
			String(Value)
		);
	EndIf;
...
"Тип"
"источника: %1 Описание ошибки: %2';vi='Lỗi thực hiện hàm xử lý AfterAllDataConversion."
"Kiểu"
"nguồn: %1 Mô tả lỗi: %2'"),
				String(Object),
				DetailErrorDescription(ErrorInfo())
...
Function ArrangeExchangeFormat(Val ExchangeFormat)
	...
	If FormatItems.Count() = 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Non-canonical name of the exchange format <%1>';ru='Неканоническое имя формата обмена <%1>';vi='Tên phi tiêu chuẩn định dạng trao đổi <%1>'"),
			ExchangeFormat
		);
	EndIf;
...
Function FindRefbyPublicIdidentifier(XDTOObjectUDID, ExchangeComponents, IBObjectValueType)
	...
	If Selection.Count() > 0 Then
		...
		If  SuitableTypeRefsQuantity > 1 Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Several links are assigned for unique identifier <%1> and node <%2>.';ru='Для уникального идентификатора <%1> и узла <%2> назначено несколько ссылок.';vi='Đối với tên (ID) đơn trị <%1> và nút <%2> đã ấn định nhiều tham chiếu.'"),
...
"Тип ссылки"
"XDTO: %1 Описание ошибки: %2';vi='Không tìm thấy DER đối với kiểu tham chiếu XDTO."
"Kiểu tham chiếu"
"XDTO: %1 Mô tả lỗi: %2'"),
				String(XDTORefType),
				DetailErrorDescription(ErrorInfo())
...
"Тип объекта"
"XDTO: %1 Описание ошибки: %2';vi='Không tìm thấy DER đối với kiểu đối tượng XDTO."
"Kiểu đối tượng"
"XDTO: %1 Mô tả lỗi: %2'"),
...
"Объект метаданных: %1';vi='Không tìm thấy DER đối với đối tượng metadata."
"Đối tượng metadata: %1'"),
...
Function DERByName(ExchangeComponents, Name)
	...
	If DataProcessorRule = Undefined Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='DER with name %1 is not found';ru='Не найдено ПОД с именем %1';vi='Không tìm thấy DER với tên %1'"), Name);
...
"Объект:"
"%1 Описание ошибки: %2';vi='Lỗi thực hiện hàm xử lý DER OnProcessing."
"Đối tượng:"
"%1 Mô tả lỗi: %2'"),
...
"Имя"
"ПОД: %1 Описание ошибки: %2';vi='Lỗi thực hiện hàm xử lý DER DataSelection."
"Tên"
"DER: %1 Mô tả lỗi: %2'"),
					DataProcessorRule.Name,
					DetailErrorDescription(ErrorInfo()));
...
"Объект:"
"%1 Описание ошибки: %2';vi='Lỗi thực hiện hàm xử lý OnSendData."
"Đối tượng:"
"%1 Mô tả lỗi: %2'"),
			String(IBData),
			DetailErrorDescription(ErrorInfo())
...
"Тип"
"источника: %1 Описание ошибки: %2';vi='Lỗi thực hiện hàm xử lý OnConvertXDTOData."
"Kiểu"
"nguồn: %1 Mô tả lỗi: %2'"),
			String(XDTOData),
			DetailErrorDescription(ErrorInfo())
...
"Объект:"
"%1 Описание ошибки: %2';vi='Lỗi thực hiện hàm xử lý BeforeReceivedDataWrite."
"Đối tượng:"
"%1 Mô tả lỗi: %2'"),
			String(IBData),
			DetailErrorDescription(ErrorInfo())
...
"Имя"
"плана обмена: %1 GetAllExchangeFormatVersions(<ExchangeFormatVersions>)';vi='Chưa đặt phiên bản định dạng trao đổi."
"Tên"
"sơ đồ trao đổi: %1 Thủ tục: GetAllExchangeFormatVersions(<ExchangeFormatVersions>)'"),
...
Procedure CheckVersion(Val Version)
	...
	If Versions.Count() = 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Non-canonical presentation of the exchange format version: <%1>.';ru='Неканоническое представление версии формата обмена: <%1>.';vi='Trình bày phi tiêu chuẩn phiên bản định dạng trao đổi: <%1>.'"),
			Version
		);
	EndIf;
...
Procedure DeleteDocumentRegisterRecords(DocumentObject, Cancel)
	...
	For Each RegisterRecordRow IN RegisterRecordTable Do
		...
		If Not AccessRight("Update", Set.Metadata()) Then
			...
			Raise NStr("en='Access right violation:';ru='Нарушение прав доступа:';vi='Vi phạm quyền truy cập:'") + " " + RegisterRecordRow.Name;
			Return;
		EndIf;
	...
	For Each RegisterRecordRow IN RecordTableRowToProcessArray Do		
		...
		Except
			...
			Raise NStr("en='Operation failed:';ru='Операция не выполнена:';vi='Chưa thực hiện giao dịch:'") + " " + RegisterRecordRow.Name
