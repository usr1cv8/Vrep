...
Procedure OpenPlacingVariantsInSectionsDialog(OptionsArray, AdditionalParameters = Undefined, Owner = Undefined) Export
	...
	If TypeOf(OptionsArray) <> Type("Array") OR OptionsArray.Count() < 1 Then
		ShowMessageBox(, NStr("en='Select report variants to be placed in sections.';ru='Выберите варианты отчетов, которые необходимо разместить в разделах.';vi='Hãy chọn phương án báo cáo mà cần sắp xếp chúng trong các phần hành.'"));
		Return;
	EndIf;
...
Procedure OpenUsersSettingsResetDialog(OptionsArray, Owner = Undefined) Export
	...
	If TypeOf(OptionsArray) <> Type("Array") OR OptionsArray.Count() < 1 Then
		ShowMessageBox(, NStr("en='Select report variants for which it is required to reset the custom settings.';ru='Выберите варианты отчетов, для которых необходимо сбросить пользовательские настройки.';vi='Hãy chọn phương án báo cáo mà cần xác lập lại tùy chỉnh tự tạo.'"));
		Return;
	EndIf;
...
Procedure OpenDirectoryPropertiesResetDialog(OptionsArray, Owner = Undefined) Export
	...
	If TypeOf(OptionsArray) <> Type("Array") OR OptionsArray.Count() < 1 Then
		ShowMessageBox(, NStr("en='Select the application report variants which location settings are to be reset.';ru='Выберите варианты отчетов программы, для которых необходимо сбросить настройки размещения.';vi='Hãy chọn phương án báo cáo của chương trình mà cần xác lập lại tùy chỉnh sắp xếp.'"));
		Return;
	EndIf;
...
Procedure OpenReportOption(Form) Export
	...
	ElsIf Variant.ReportType = PredefinedValue("Enum.ReportsTypes.External") Then
		...
		ShowMessageBox(, NStr("en='External report variant can be opened only from the report form.';ru='Вариант внешнего отчета можно открыть только из формы отчета.';vi='Có thể phương án báo cáo ngoài chỉ từ biểu mẫu báo cáo.'"));
