...
Function Parameters() Export
	...
	If Not SavedParameters.Property("PossibleRightsForObjectRightsSettings") Then
		ParameterPresentation = NStr("en='Possible rights for setting object rights';ru='Возможные права для настройки прав объектов';vi='Quyền có thể để tùy chỉnh quyền đối tượng'");
		
	ElsIf Not SavedParameters.Property("ProvidedAccessGroupsProfiles") Then
		ParameterPresentation = NStr("en='Supplied access group profiles';ru='Поставляемые профили групп доступа';vi='Hồ sơ nhóm truy cập kèm theo bộ cài đặt'");
		
	ElsIf Not SavedParameters.Property("AccessGroupsPredefinedProfiles") Then
		ParameterPresentation = NStr("en='Access group predefined profiles';ru='Предопределенные профили групп доступа';vi='Hồ sơ nhóm truy cập định trước'");
		
	ElsIf Not SavedParameters.Property("AccessKindsProperties") Then
		ParameterPresentation = NStr("en='Access kind properties';ru='Свойства видов доступа';vi='Thuộc tính dạng truy cập'");
	EndIf;
...
"ограничения доступа: ""%1"".';vi='Lỗi cập nhật cơ sở thông tin."
"Chưa điền tham số"
"hạn chế truy cập: ""%1"".'")
	...
	For LineNumber = 1 To StrLineCount(LimitationsOfRights) Do
		...
		If ValueIsFilled(CurrentRow) Then
			...
			If StrOccurrenceCount(CurrentRow, ".") <> 3 AND StrOccurrenceCount(CurrentRow, ".") <> 5 Then
				ErrorExplanation = NStr("en='The string should be formatted as ""<Full table name>.<Right name>.<Access kind name>[.Object table]"".';ru='Строка должна быть в формате ""<Полное имя таблицы>.<Имя права>.<Имя вида доступа>[.Таблица объекта]"".';vi='Dòng phải có định dạng ""<Tên gọi đầy đủ của bảng>.<Tên quyền>.<Tên dạng truy cập>[.Object table]"".'");
			Else
				...
				If Metadata.FindByFullName(Table) = Undefined Then
					ErrorExplanation = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='Table ""%1"" is not found.';ru='Не найдена таблица ""%1"".';vi='Không tìm thấy bảng ""%1"".'"),
						Table);
				
				ElsIf Right <> "Read" AND Right <> "Update" Then
					ErrorExplanation = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='Right ""%1"" is not found.';ru='Не найдено право ""%1"".';vi='Không tìm thấy quyền ""%1"".'"),
						Right);
				
				ElsIf Upper(AccessKind) = Upper("Object") Then
					If Metadata.FindByFullName(ObjectTable) = Undefined Then
						ErrorExplanation = StringFunctionsClientServer.SubstituteParametersInString(
							NStr("en='Table of object ""%1"" is not found.';ru='Не найдена таблица объекта ""%1"".';vi='Không tìm thấy bảng đối tượng ""%1"".'"),
							ObjectTable);
					Else
				...
				ElsIf Upper(AccessKind) = Upper("RightSettings") Then
					If Metadata.FindByFullName(ObjectTable) = Undefined Then
						ErrorExplanation = StringFunctionsClientServer.SubstituteParametersInString(
							NStr("en='Table of right settings owner ""%1"" is not found.';ru='Не найдена таблица владельца настроек прав ""%1"".';vi='Không tìm thấy bảng đối tượng chủ tùy chỉnh quyền ""%1"".'"),
							ObjectTable);
					Else
				...
				ElsIf AccessKindsByNames.Get(AccessKind) = Undefined Then
					ErrorExplanation = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='Access kind ""%1"" is not found.';ru='Не найден вид доступа ""%1"".';vi='Không tìm thấy dạng truy cập ""%1"".'"),
						AccessKind);
				Else
...
""
"';vi='Lỗi tại dòng mô tả dạng hạn chế"
"quyền của đối tượng metadata: ""%1"".'"), CurrentRow) + ErrorExplanation);
			Else
