	...
	For Each TableRow In Form.ContactInformation Do
		...
		If KindProperties.Mandatory And IsBlankString(TableRow.Presentation)
			...
			CommonUseClientServer.MessageToUser(
				StrTemplate(NStr("en='Contact information kind ""%1"" is not filled.';ru='Вид контактной информации ""%1"" не заполнен.';vi='Chưa điền dạng thông tin liên hệ ""%1"".'"), KindProperties.KindPresentation),,, AttributeName);
...
Procedure RefreshContactInformationItems(Form, WidthKindField = 8) Export
	...
	For Each DataCI In Form.ContactInformation Do
		...
		GroupValueCI.Group			= ChildFormItemsGroup.HorizontalIfPossible;
		...
		If DataCI.Type = Enums.ContactInformationTypes.Address Then
			...
			AddContextMenuCommand(Form,
				...
				NStr("en='Address on Google Maps';ru='Адрес на Google Maps';vi='Địa chỉ trên Google Maps'"),
				NStr("en='Show address on Google Maps';ru='Показывает адрес на карте Google Maps';vi='Hiển thị địa chỉ trên bản đồ Google Maps'"),
				FieldPresentation
			);
		...
		If ForContactInformationTypeIsAvailableCommentInput(DataCI.Type) Then
			...
			FieldComment.InputHint		= NStr("en='Note';ru='Прим.';vi='Ghi chú'");
	...
	If EmailAddress <> Undefined And EmailAddress.Type() = XDTOFactory.Type(Namespace, "Email") Then
		Try
			...
			If Result.Count() > 1 Then
				...
				ErrorString = NStr("en='You can enter only one email address';ru='Допускается ввод только одного адреса электронной почты';vi='Cho phép nhập chỉ một địa chỉ email'");
...
Procedure InitializeForm(Form)
	...
	GroupValuesCI.Title				= NStr("en='Contact information values';ru='Значения контактной информации';vi='Giá trị thông tin liên hệ'");
	...
	Command.Title	= NStr("en='+ phone, address';ru='+ телефон, адрес';vi='+ điện thoại, địa chỉ'");
