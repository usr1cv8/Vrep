	...
	If MessagesSaaS.BodyContainTypedMessage(MessageBody, Message) Then
		...
		If CommonUseReUse.CanUseSeparatedData()
			...
			If CommonUse.SessionSeparatorValue() <> Message.Body.Zone Then
				MessagePattern = NStr("en='Attempting to send message on behalf of area %1 from area %2';ru='Попытка отправить сообщение от имени области %1 из области %2';vi='Thử gửi thông báo theo tên vùng %1 từ vùng %2'");
		...
		Else
			...
			If AreaKeyCheckAtMessageAvailable() Then
				...
				If AreaCurrentKey <> Message.Body.ZoneKey Then
					...
					Raise NStr("en='Incorrect key of the data area in the message.';ru='Неверный ключ области данных в сообщении!';vi='Sai khóa vùng dữ liệu trong thông báo!'");
