...
Function GenerateAmountInWords(AmountAsNumber, Currency, DisplayAmountWithoutCents = False) Export
	...
	// 1C - CHIENTN - 27-12-2018 - Replace to this-
	UserLanguage = CurrentLanguage().LanguageCode;
	If UserLanguage = "vi" Then
		SubjectParameters	= CommonUse.ObjectAttributesValues(Currency, "InWordParametersInHomeLanguage");   	
		Result = NumberInWords(Amount, "L=vi_VN;FS=False", SubjectParameters.InWordParametersInHomeLanguage);
		Result = Left(Result, Find(Result, "0") - 1);
	Else
		SubjectParameters	= CommonUse.ObjectAttributesValues(Currency, "InWordParametersInEnglish");   	
		Result = NumberInWords(Amount, "L=en_US;FS=False", SubjectParameters.InWordParametersInEnglish);
...
Procedure ImportActualRate(ExportParameters = Undefined, ResultAddress = Undefined) Export
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		Raise NStr("en='Invalid call of the ""ImportRelevantCurrencyRate"" procedure.';ru='Недопустимый вызов процедуры ""ЗагрузитьАктуальныйКурс"".';vi='Gọi ra thủ tục ""ImportRelevantCurrencyRate"" không hợp lệ.'");
	EndIf;
	
	CommonUse.OnStartExecutingScheduledJob();
	
	EventName = NStr("en='Currency.Exchange rates import';ru='Валюты.Загрузка курсов валют';vi='Tiền tệ.Kết nhập tỷ giá hối đoái'",
		CommonUseClientServer.MainLanguageCode());
	
	WriteLogEvent(EventName, EventLogLevel.Information, , ,
		NStr("en='Scheduled import of exchange rates is started';ru='Начата регламентная загрузка курсов валют';vi='Bắt đầu kết nhập thường kỳ tỷ giá hối đoái'"));
	...
	If ErrorsOccuredOnImport Then
		WriteLogEvent(
			...
			, 
			,
			NStr("en='Errors occurred during the scheduled job of the exchange rate import';ru='Во время регламентного задания загрузки курсов валют возникли ошибки';vi='Trong thời gian nhiệm vụ thường kỳ kết nhập tỷ giá hối đoái phát sinh lỗi'"));
	Else
		WriteLogEvent(
			EventName,
			...
			,
			,
			NStr("en='Scheduled download of exchange rates is completed.';ru='Завершена регламентная загрузка курсов валют.';vi='Kết thúc kết nhập tỷ giá hối đoái.'"));
	EndIf;
...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	For Each Section IN Sections Do
		...
		Work.Presentation  = NStr("en='Exchange rates are outdated';ru='Курсы валют устарели';vi='Tỷ giá ngoại tệ cũ'");
		Work.Important         = True;
...
Function permissions()
	...
	Port = Undefined;
	Definition = NStr("en='Import exchange rates from the Internet.';ru='Загрузка курсов валют из Интернета.';vi='Kết nhập tỷ giá hối đoái từ Internet'");
...
Function ImportCurrencyRateFromFile(Val Currency, Val PathToFile, Val ImportBeginOfPeriod, Val ImportEndOfPeriod) Export
	...
	ElsIf NumberOfImportedDays = 0 Then
		ExplanationAboutExporting = NStr("en='Exchange rates %1 - %2 are not imported. No data available.';ru='Курсы валюты %1 - %2 не загружены. Нет данных.';vi='Tỷ giá hối đoái %1 - %2 chưa kết nhập. Không có dữ liệu.'");
	Else
		ExplanationAboutExporting = NStr("en='Not all exchange rates for currency %1 are imported - %2.';ru='Загружены не все курсы по валюте %1 - %2.';vi='Kết nhập không phải tất cả tỷ giá hối đoái %1 - %2.'");
	EndIf;
...
"курсами"
"валюты (%1 - %2): %3 Возможно, нет доступа к веб сайту с курсами валют, либо указана несуществующая валюта.';vi='Không thể nhận tệp dữ liệu với"
"tỷ giá"
"hối đoái (%1 - %2): %3 Có thể là không thể truy cập đến Website có tỷ giá hối đoái, hoặc là đã chỉ ra tiền tệ không tồn tại.'"),
...
Procedure RefreshDataOnAdditionalCurrency937() Export
	...
	If Not Currency.IsEmpty() Then
		...
		Currency.DescriptionFull = NStr("en='Bolivar';ru='Боливар';vi='Bolivar'");
