...
Function CurrentUserSubsystems() Export
	...
	RootRow.Presentation = NStr("en='All sections';ru='Все разделы';vi='Tất cả các phần hành'");
...
Function SubsystemsPresentations() Export
	...
	////EndIf;
	//
	//HomePageID = ReportsVariantsClientServer.HomePageID();
	//For Each Section In ReportsVariants.SectionList() Do
	////	
	//	SectionMetadata = Section.Value;
	//	If Not (TypeOf(SectionMetadata) = Type("MetadataObject") And StrStartsWith(SectionMetadata.FullName(), "Subsystem"))
	//		And Not (TypeOf(SectionMetadata) = Type("String") And SectionMetadata = HomePageID) Then
	//		Raise NStr("ru = 'Некорректно определены значения разделов в процедуре ВариантыОтчетовПереопределяемый.ОпределитьРазделыСВариантамиОтчетов';
	//								|en = 'Некорректно определены значения разделов в процедуре ВариантыОтчетовПереопределяемый.ОпределитьРазделыСВариантамиОтчетов';");
	//	EndIf;
	//	
	//	IsStartPage = (SectionMetadata = HomePageID);
	//	If Not IsStartPage
	//		And (Not AccessRight("View", SectionMetadata)
	//			Or Not CommonUse.MetadataObjectAvailableByFunctionalOptions(SectionMetadata)) Then
	//		Continue; 
	//	EndIf;
	//	
	//	TableRow = Result.Add();
	...
	//	Else
	//		TableRow.FullName     = SectionMetadata.FullName();
	//		TableRow.Presentation = SectionMetadata.Presentation();
	//	EndIf;
	//	TableRow.Ref = Catalogs.MetadataObjectIDs.EmptyRef();
	//	If Not IsStartPage Then
	//		AddSubsystems(Result, SectionMetadata);
	//	EndIf;
	//EndDo;
	//
	//Result.Indexes.Add("FullName");
	//
	//SubsystemsReferences = CommonUse.MetadataObjectIDs(Result.UnloadColumn("FullName"), False);
	//For Each SubsystemRef In SubsystemsReferences Do
	//	TableRow = Result.Find(SubsystemRef.Key, "FullName");
	//	If TableRow <> Undefined Then 
	//		TableRow.Ref = SubsystemRef.Value;
	//	EndIf;
	//EndDo;
	//
	Return Result;
