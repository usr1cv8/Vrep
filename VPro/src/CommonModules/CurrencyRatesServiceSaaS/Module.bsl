...
Procedure ImportRates() Export
	...
	If Descriptors.Descriptor.Count() < 1 Then
		Raise(NStr("en='There is no data of the ExchangeRates kind in the service manager';ru='В менеджере сервиса отсутствуют данные вида ""ExchangeRates""';vi='Trong quản lý dịch vụ thiếu dữ liệu dạng ""ExchangeRates""'"));
	EndIf;
...
Procedure ProcessProvidedRatesForDay(Val Handle, Val PathToFile)
	...
	If CurrencyRatesDate = "" Then
		Raise NStr("en='Data of the ""ExchangeRatesForDay"" kind does not contain the ""Date"" characteristics. Cannot update the rates.';ru='Данные вида ""КурсыВалютЗаДень"" не содержат характеристики ""Дата"". Обновление курсов невозможно.';vi='Dữ liệu dạng ""ExchangeRatesForDay"" không có đặc điểm ""Ngày"". Không thể cập nhật được tỷ giá.'"); 
	EndIf;
