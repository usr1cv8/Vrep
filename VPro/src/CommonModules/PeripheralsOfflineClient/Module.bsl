...
Procedure AsynchronousExportProductsInEquipmentOffline(EquipmentType, DeviceArray, MessageText = "", ShowMessageBox = True, NotificationOnImplementation, ModifiedOnly = True) Export
	...
	Status(NStr("en='Exporting goods to offline peripherals...';ru='Выполняется выгрузка товаров в оборудование Offline...';vi='Đang kết xuất hàng hóa vào thiết bị Offline...'"));
	...
	For Each DeviceIdentifier IN DeviceArray Do
		...
		If StructureData.Data.Count() = 0 Then
			...
			If StructureData.ExportedRowsWithErrorsCount = 0 Then
				ErrorsDescriptionFull = GenerateErrorDescriptionForDevice(DeviceIdentifier, Undefined, ErrorsDescriptionFull, NStr("en='There is no data to export.';ru='Нет данных для выгрузки!';vi='Không có dữ liệu để kết xuất!'"));
			Else
				ErrorsDescriptionFull = GenerateErrorDescriptionForDevice(DeviceIdentifier, Undefined, ErrorsDescriptionFull, StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Data is not exported. Errors detected: %1';ru='Данные не выгружены. Обнаружено ошибок: %1';vi='Dữ liệu không được kết xuất. Tìm thấy lỗi"" %1'"), StructureData.ExportedRowsWithErrorsCount));
			EndIf;
			Continue;
		EndIf;
...
Procedure AsynchronousClearProductsInEquipmentOffline(EquipmentType, DeviceArray, MessageText = "", ShowMessageBox = True, NotificationOnImplementation) Export
	...
	Status(NStr("en='Clearing goods in offline peripherals...';ru='Выполняется очистка товаров в оборудовании Offline...';vi='Đang xóa hàng hóa trong thiết bị Offline...'"));
...
Procedure AsynchronousImportReportAboutRetailSales(DeviceArray, MessageText = "", ShowMessageBox = True, NotificationOnImplementation) Export
	...
	Status(NStr("en='Retail sales reports are being imported from the offline CR...';ru='Выполняется загрузка отчетов о розничных продажах из ККМ Offline...';vi='Đang thực hiện kết xuất báo cáo bán lẻ từ máy tính tiền Offline...'"));
...
Function GenerateErrorDescriptionForDevice(DeviceIdentifier, Output_Parameters, ErrorsDescriptionFull, ErrorDescription)
	...
	Return ErrorsDescriptionFull
	      + Chars.LF
	      + StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Error description for the %1 device: %2';ru='Описание ошибок для устройства %1: %2';vi='Mô tả lỗi cho thiết bị %1: %2'"), DeviceIdentifier, ErrorDescription)
...
Procedure AsynchronousExportProductsInEquipmentOfflineFragment(Parameters)
	...
	If Parameters.NeedToPerform > 0 Then
		...
		If Parameters.Completed = Parameters.NeedToPerform Then
			MessageText = NStr("en='The goods have been successfully exported.';ru='Товары успешно выгружены!';vi='Hàng hóa đã kết xuất thành công!'");
		ElsIf Parameters.Completed > 0 Then
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='The goods have been successfully exported for %1 devices from %2.';ru='Товары успешно выгружены для %1 устройств из %2.';vi='Hàng hóa đã kết xuất thành công cho %1 thiết bị từ %2.'"), Parameters.Completed, Parameters.NeedToPerform) + Parameters.ErrorsDescriptionFull;
		Else
			MessageText = NStr("en='Cannot export the goods:';ru='Выгрузить товары не удалось:';vi='Không kết xuất được hàng hóa:'") + Parameters.ErrorsDescriptionFull;
		EndIf;
	...
	Else
		If Parameters.ShowMessageBox Then
			ShowMessageBox(Undefined, NStr("en='Select a device for which the goods should be exported.';ru='Необходимо выбрать устройство, для которого требуется выгрузить товары.';vi='Cần chọn thiết bị mà cần kết xuất hàng hóa.'"), 10);
		EndIf;
	EndIf;
...
Procedure AsynchronousClearProductsInEquipmentOfflineFragment(Parameters)
	...
	If Parameters.NeedToPerform > 0 Then
		...
		If Parameters.Completed = Parameters.NeedToPerform Then
			MessageText = NStr("en='Goods are successfully cleared.';ru='Товары успешно очищены!';vi='Đã xóa bỏ thành công hàng hóa!'");
		ElsIf Parameters.Completed > 0 Then
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Goods are successfully cleared for %1 devices out of %2. %3';ru='Товары успешно очищены для %1 устройств из %2. %3';vi='Hàng hóa đã được xóa bỏ thành công cho %1 thiết bị từ %2. %3'"), Parameters.Completed, Parameters.NeedToPerform, Parameters.ErrorsDescriptionFull);
		Else
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Cannot clear the goods: %1';ru='Очистить товары не удалось: %1';vi='Không thể xóa bỏ hàng hóa: %1'"), Parameters.ErrorsDescriptionFull);
		EndIf;
	...
	Else
		If Parameters.ShowMessageBox Then
			ShowMessageBox(Undefined, NStr("en='Select a device for which the goods should be cleared.';ru='Необходимо выбрать устройство, для которого требуется очистить товары.';vi='Cần chọn thiết bị mà cần xóa bỏ hàng hóa.'"), 10);
		EndIf;
	EndIf;
...
Procedure AsynchronousImportReportAboutRetailSalesFragment(Parameters)
	...
	If Parameters.NeedToPerform > 0 Then
		...
		If Parameters.Completed = Parameters.NeedToPerform Then
			MessageText = NStr("en='The retail sales reports have been successfully imported.';ru='Отчеты о розничных продажах успешно загружены!';vi='Báo cáo bán lẻ đã kết nhập thành công!'");
		ElsIf Parameters.Completed > 0 Then
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Retail sales reports have been successfully imported for %1 devices from %2.';ru='Успешно загружены отчеты о розничных продажах для %1 устройств из %2.';vi='Đã kết nhập báo cáo bán lẻ thành công cho %1 thiết bị từ %2.'"), Parameters.Completed, Parameters.NeedToPerform) + Parameters.ErrorsDescriptionFull;
		Else
			MessageText = NStr("en='Cannot import retail sales reports:';ru='Отчеты о розничных продажах загрузить не удалось:';vi='Không thể kết nhập báo cáo bán lẻ:'") + Parameters.ErrorsDescriptionFull;
		EndIf;
