...
Procedure BeforeObjectExport(Container, ObjectExportManager, Serializer, Object, Artifacts, Cancel) Export
	...
	If AttributesWithValuesStorage = Undefined Then
		...
		Raise ServiceTechnologyIntegrationWithSSL.SubstituteParametersInString(
			NStr("en='Metadata object %1 cannot be processed by handler ValueStorageDataExportImport.BeforeObjectExport().';ru='Объект метаданных %1 не может быть обработан обработчиком ВыгрузкаЗагрузкаДанныхХранилищЗначений.ПередВыгрузкойОбъекта()!';vi='Hàm sự kiện ValueStorageDataExportImport.BeforeObjectExport()"
"không thể xử lý đối tượng metadata %1!'"),
	...
	Else
		...
		Raise ServiceTechnologyIntegrationWithSSL.SubstituteParametersInString(
			NStr("en='Unexpected metadata object: %1.';ru='Неожиданный объект метаданных: %1!';vi='Đối tượng metadata bất thường: %1!'"),
...
Procedure BeforeUpdateConstant(Container, Object, Artifact)
	...
	Else
		...
		Raise ServiceTechnologyIntegrationWithSSL.SubstituteParametersInString(
			NStr("en='Type of owner {%1}%2 cannot be used for metadata object %3.';ru='Тип владельца {%1}%2 не должен использоваться для объекта метаданных %3!';vi='Kiểu sở hữu {%1}%2 không được sử dụng đối với đối tượng metadata %3!'"),
...
Procedure ImportValueStorage(Container, ValueStorage, Artifact)
	...
	Else
		...
		Raise ServiceTechnologyIntegrationWithSSL.SubstituteParametersInString(
			NStr("en='Unexpected placement type of value storage data in export container: (%1)%2.';ru='Неожиданный тип размещения данных хранилища значений в контейнере выгрузки: {%1}%2!';vi='Kiểu sắp xếp dữ liệu bất thường của kho chứa giá trị trong container kết xuất: {%1}%2!'"),
