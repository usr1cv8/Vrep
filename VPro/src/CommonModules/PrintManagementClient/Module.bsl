...
#EndIf
	...
	If PrintForms.Cancel Then
		CommonUseClientServer.MessageToUser(NStr("en='You have no rights to send print form on printer. Contact your administrator.';ru='Нет прав для вывода печатной формы на принтер, обратитесь к администратору.';vi='Không có quyền đưa mẫu in vào máy in, hãy liên hệ với người quản trị.'"));
		Return;
	EndIf;
...
"Данные будут записаны.';vi='Dữ liệu vẫn chưa được ghi lại."
"Chỉ có thể thực hiện thao tác ""%1"" sau khi ghi lại dữ liệu."
"Dữ liệu sẽ được ghi lại.'"),
...
Function TemplateArea(Val RefsOnTemplate, Val DetailsOfArea) Export
		...
		Else
			Raise
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Area type is not specified or specified incorrectly: %1.';ru='Тип области не указан или указан не корректно: %1.';vi='Kiểu vùng chưa chỉ ra hoặc chỉ ra sai: %1.'"), DetailsOfArea.AreaType);
		EndIf;
	...
	ElsIf RefsOnTemplate.Type = "ODT" Then
		...
		Else
			Raise
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Area type is not specified or specified incorrectly: %1.';ru='Тип области не указан или указан не корректно: %1.';vi='Kiểu vùng chưa chỉ ra hoặc chỉ ra sai: %1.'"), DetailsOfArea.AreaName);
		EndIf;
...
Procedure JoinArea(Val PrintForm,
	...
	Try
		...
		If PrintForm.Type = "DOC" Then
			...
			ElsIf	DetailsOfArea.AreaType = "TableRow" Then
				...
				EndIf;
			Else
				Raise(NStr("en='Area type is not specified or specified incorrectly.';ru='Тип области не указан или указан не корректно.';vi='Kiểu vùng chưa chỉ ra hoặc chỉ ra sai.'"));
			EndIf;
	...
	Except
		...
		ErrorInfo = ErrorInfo + " " +
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='An error occurred when attempting to get area ""%1"" from the template.';ru='Ошибка при попытке вывести область ""%1"" из макета.';vi='Lỗi khi thử đưa ra vùng ""%1"" từ khuôn in.'"),
...
Function CheckDocumentsPosted(DocumentsArray, FormSource = Undefined) Export
	...
	If UnpostedDocumentsCount > 0 Then
		...
		If UnpostedDocumentsCount = 1 Then
			QuestionText = NStr("en='To print documents, it is required to post them first. Post the documents and continue?';ru='Для того чтобы распечатать документы, их необходимо предварительно провести. Выполнить проведение документов и продолжить?';vi='Để in chứng từ, cần kết chuyển trước. Thực hiện kết chuyển chứng từ và tiếp tục?'");
		Else
			QuestionText = NStr("en='To print documents, it is required to post them first. Post the documents and continue?';ru='Для того чтобы распечатать документы, их необходимо предварительно провести. Выполнить проведение документов и продолжить?';vi='Để in chứng từ, cần kết chuyển trước. Thực hiện kết chuyển chứng từ và tiếp tục?'");
		EndIf;
		...
		MessagePattern = NStr("en='Document %1 is not posted: %2 Cannot print.';ru='Документ %1 не проведен: %2 Печать невозможна.';vi='Chứng từ %1 chưa kết chuyển: %2 Không thể in.'");
	...
	If UnpostedDocumentsCount > 0 Then
		...
		DialogText = NStr("en='Cannot post one or several documents.';ru='Не удалось провести один или несколько документов.';vi='Không thể kết chuyển một hoặc nhiều chứng từ.'");
		DialogButtons = New ValueList;
		
		If AreDocumentsWhichCanPrint Then
			DialogText = DialogText + " " + NStr("en='Continue?';ru='Продолжить?';vi='Tiếp tục?'");
			DialogButtons.Add(DialogReturnCode.Ignore, NStr("en='Continue';ru='Продолжить';vi='Tiếp tục'"));
...
#If WebClient Or MobileClient Then
	MessageText = NStr("en='To continue printing, install the file operation extension.';ru='Для продолжения печати необходимо установить расширение работы с файлами.';vi='Để tiếp tục in cần cài đặt phần mở rộng làm việc với tệp."
"'");
