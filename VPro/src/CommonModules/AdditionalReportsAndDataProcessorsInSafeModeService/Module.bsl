...
Function ExternalModuleContainerDictionary() Export
	...
	Result.Insert("Nominative", NStr("en='Additional report of data processor';ru='Дополнительного отчета или обработки';vi='Báo cáo hoặc bộ xử lý bổ sung'"));
	Result.Insert("Genitive", NStr("en='Additional report of data processor';ru='Дополнительного отчета или обработки';vi='Báo cáo hoặc bộ xử lý bổ sung'"));
	...
	For Each ScriptStep IN Script Do
		...
		Else
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Unknown action kind for script stage: %1';ru='Неизвестный вид действия для этапа сценария: %1';vi='Dạng thao tác không xác định đối với giai đoạn kịch bản: %1'"),
		...
		For Each MethodParameter IN MethodParameters Do
			...
			Else
				...
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Unknown parameter for script stage: %1';ru='Неизвестный параметр для этапа сценария: %1';vi='Tham số không xác định đối với giai đoạn kịch bản: %1'"),
...
Function GeneratePermissionPresentation(Val permissions) Export
	...
	For Each Resolution IN permissions Do
		...
		Result = Result + StringFunctionsClientServer.SubstituteParametersInString(
			...
			NStr("en='More...';ru='Подробнее...';vi='Chi tiết...'"));
...
Function GenerateDetailedPermissionsDescription(Val TypePermissions, Val PermissionParameters) Export
	...
	Result = Result + StringFunctionsClientServer.SubstituteParametersInString(
		"<P><FONT size=2>%1%2</FONT></P>", DetailsPermissions, ?(
			HasParameters,
			NStr("en=' with following restrictions:';ru=' со следующими ограничениями:';vi='có hạn chế như sau:'"),
			"."));
...
Function PermissionIsNotGrantedTextOfException(Val SessionKey, Val TypeRequiredPermissions)
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Permission (%2)%3 is not provided for additional report or data processor %1.';ru='Дополнительному отчету или обработке %1 не предоставлено разрешение {%2}%3!';vi='Không cấp phép {%2}%3 cho báo cáo hoặc bộ xử lý bổ sung %1!'"),
...
"разрешение {%2}%3 при значении ограничителя %4 равном %5!';vi='Đối với báo cáo hoặc bộ xử lý"
"bổ sung %1 không cho phép"
"{%2}%3 khi giá trị giới hạn %4 bằng %5!'"),
		String(SessionKey), TypeRequiredPermissions.NamespaceURI, TypeRequiredPermissions.Name,
...
"или обработки %1 при предоставлении разрешения {%2}%3 не был указан обязательный ограничитель %4!';vi='Đối với báo cáo hoặc bộ xử lý"
"ngoài %1, khi cho phép {%2}%3 đã không chỉ ra đường hạn chế bắt buộc %4!'"),
		String(SessionKey), TypeRequiredPermissions.NamespaceURI, TypeRequiredPermissions.Name,
