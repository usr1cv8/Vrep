...
Procedure ProcessMessage(Val MessageChannel, Val MessageBody, Val Sender) Export
	...
	Try
		...
		EndIf;
	Except
		WriteLogEvent(NStr("en='Supplied data.Message processing error';ru='Поставляемые данные.Ошибка обработки сообщения';vi='Dữ liệu cài đặt.Lỗi xử lý thông báo'", 
...
Procedure ProcessNewDescriptor(Val Handle) Export
	...
	WriteLogEvent(NStr("en='Supplied data.New data is available';ru='Поставляемые данные.Доступны новые данные';vi='Dữ liệu cài đặt.Dùng được dữ liệu mới'", 
		CommonUseClientServer.MainLanguageCode()), 
		EventLogLevel.Information, ,
		, ?(Import, NStr("en='Import job was added to the queue.';ru='В очередь добавлено задание на загрузку.';vi='Theo trình tự bổ sung nhiệm vụ cần kết nhập.'"), NStr("en='Data import is not required.';ru='Загрузка данных не требуется.';vi='Không cần kết nhập dữ liệu.'"))
...
Procedure ImportData(Val HandleXML) Export
	...
	Except
		WriteLogEvent(NStr("en='Supplied data.An error occurred when working with XML';ru='Поставляемые данные.Ошибка работы с XML';vi='Dữ liệu cài đặt.Lỗi làm việc với XML'", 
			CommonUseClientServer.MainLanguageCode()), 
			EventLogLevel.Error, ,
	...
	WriteLogEvent(NStr("en='Supplied data. Data import';ru='Поставляемые данные.Загрузка данных';vi='Dữ liệu cài đặt.Kết nhập dữ liệu'", 
		CommonUseClientServer.MainLanguageCode()), 
		EventLogLevel.Information, ,
		, NStr("en='Import is started';ru='Загрузка начата';vi='Bắt đầu kết nhập'") + Chars.LF + SuppliedData.GetDataDescription(Handle));
	...
	If ValueIsFilled(Handle.FileGUID) Then
		...
		If ExportFileName = Undefined Then
			WriteLogEvent(NStr("en='Supplied data. Data import';ru='Поставляемые данные.Загрузка данных';vi='Dữ liệu cài đặt.Kết nhập dữ liệu'", 
				CommonUseClientServer.MainLanguageCode()), 
				EventLogLevel.Information, ,
				, NStr("en='File cannot be imported';ru='Файл не может быть загружен';vi='Không thể kết nhập tệp'") + Chars.LF 
	...
	WriteLogEvent(NStr("en='Supplied data. Data import';ru='Поставляемые данные.Загрузка данных';vi='Dữ liệu cài đặt.Kết nhập dữ liệu'", 
		CommonUseClientServer.MainLanguageCode()), 
		EventLogLevel.Note, ,
		, NStr("en='Import is completed successfully';ru='Загрузка успешно выполнена';vi='Đã kết nhập thành công'") + Chars.LF + SuppliedData.GetDataDescription(Handle));
	...
	For Each Handler IN GetHandlers(Handle.DataType) Do
		...
		Try
			...
			SetRawData.Write();			
		Except
			WriteLogEvent(NStr("en='Supplied data. Data processor error';ru='Поставляемые данные.Ошибка обработки';vi='Dữ liệu cài đặt.Lỗi xử lý'", 
				CommonUseClientServer.MainLanguageCode()), 
				EventLogLevel.Error, ,
				, DetailErrorDescription(ErrorInfo())
				+ Chars.LF + SuppliedData.GetDataDescription(Handle)
				+ Chars.LF + StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Handler code: %1';ru='Код обработчика: %1';vi='Mã hàm sự kiện: %1'"), Handler.ProcessorCode));
	...
	If TransactionActive() Then
		...
		WriteLogEvent(NStr("en='Supplied data. Data processor error';ru='Поставляемые данные.Ошибка обработки';vi='Dữ liệu cài đặt.Lỗi xử lý'", 
			...
			,
			, 
			NStr("en='Upon completion of handler execution Transaction was not closed when the handler finished';ru='По завершении выполнения обработчика не была закрыта транзакция';vi='Không thể đóng giao tác cho đến khi hàm xử lý kết thúc thực hiện'")
				 + Chars.LF + SuppliedData.GetDataDescription(Handle));
	...
	If HasErrors Then
		...
		WriteLogEvent(NStr("en='Supplied data. Data processor error';ru='Поставляемые данные.Ошибка обработки';vi='Dữ liệu cài đặt.Lỗi xử lý'", 
			CommonUseClientServer.MainLanguageCode()), 
			EventLogLevel.Information, , ,
			NStr("en='Data processing will be restarted due to handler error.';ru='Обработка данных будет запущена повторно из-за ошибки обработчика.';vi='Sẽ khởi động lại việc xử lý dữ liệu do lỗi của hàm sự kiện.'")
			 + Chars.LF + SuppliedData.GetDataDescription(Handle));
	Else
		SetRawData.Clear();
		SetRawData.Write();
		...
		WriteLogEvent(NStr("en='Supplied data. Data import';ru='Поставляемые данные.Загрузка данных';vi='Dữ liệu cài đặt.Kết nhập dữ liệu'", 
			CommonUseClientServer.MainLanguageCode()), 
			EventLogLevel.Information, ,
			, NStr("en='New data is processed';ru='Новые данные обработаны';vi='Đã xử lý dữ liệu mới'") + Chars.LF + SuppliedData.GetDataDescription(Handle));
...
Procedure NotifyAboutCancelProcessing(Val Handler, Val Handle)
	...
	Try
		...
		WriteLogEvent(NStr("en='Supplied data. Cancel processor';ru='Поставляемые данные.Отмена обработки';vi='Dữ liệu cài đặt.Hủy bỏ xử lý'", 
			CommonUseClientServer.MainLanguageCode()), 
			EventLogLevel.Information, ,
			, SuppliedData.GetDataDescription(Handle)
			+ Chars.LF + StringFunctionsClientServer.SubstituteParametersInString(
...
Function GetFileFromStorage(Val Handle)
	...
	Except
		WriteLogEvent(NStr("en='Supplied data. Storage error';ru='Поставляемые данные.Ошибка хранилища';vi='Dữ liệu cài đặt.Lỗi kho chứa'", 
			CommonUseClientServer.MainLanguageCode()), 
			EventLogLevel.Error, ,
			, DetailErrorDescription(ErrorInfo())
			+ Chars.LF + SuppliedData.GetDataDescription(Handle));
