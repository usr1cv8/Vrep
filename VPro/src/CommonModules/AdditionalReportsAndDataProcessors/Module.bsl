...
Function ConnectExternalDataProcessor(Ref) Export
	...
	WriteNote(Ref, NStr("en='Connection, SafeMode = ""%1"".';ru='Подключение, БезопасныйРежим = ""%1"".';vi='Kết nối, SafeMode = ""%1"".'"), SafeMode);
	...
	If Ref.Publication = Enums.AdditionalReportsAndDataProcessorsPublicationOptions.DebugMode and ValueIsFilled(ref.FileNameForDebugging) Then
		...
		If FileOnDisk.Exist() Then 
			...
			DataProcessorName = Manager.Create(ref.FileNameForDebugging, False);
...
Procedure PrintByExternalSource(Ref, SourceParameters, PrintFormsCollection,
	...
	If ExternalDataProcessorObject = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='External data processor ""%1"" (type ""%2"") is not supported by the ""Additional reports and data processors"" subsystem';ru='Внешняя обработка ""%1"" (тип ""%2"") не обслуживается подсистемой ""Дополнительные отчеты и обработки""';vi='Bộ xử lý ngoài ""%1"" (kiểu ""%2"") không được phục vụ bằng phân hệ con ""Báo cáo và bộ xử lý bổ xung""'"),
			String(Ref),
			String(TypeOf(Ref)));
	EndIf;
	...
	For Each Str IN PrintFormsCollection Do
		If Str.SpreadsheetDocument = Undefined Then
			ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Spreadsheet document for %1 was not generated in the print processor';ru='В обработчике печати не был сформирован табличный документ для: %1';vi='Trong hàm sự kiện in chưa tạo ra văn bản dạng bảng đối với: %1'"),
				Str.TemplateName);
...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	Work.Presentation = NStr("en='Additional reports and data processors';ru='Дополнительные отчеты и обработки';vi='Báo cáo và bộ xử lý bổ sung'");
...
"Для включения подсистемы обратитесь к администратору программы.';vi='Chưa kết nối phần tử ""%1"" vì phân hệ ""%2"" đã bị ngắt trong tùy chỉnh chương trình."
"Để bật sử dụng phân hệ, hãy liên hệ với người quản trị chương trình.'"),
			"'"+ String(Ref) +"'",
	...
	If Kind = Enums.AdditionalReportsAndDataProcessorsKinds.Report
		...
		Except
			ReportParameters.Errors = 
				StrReplace(NStr("en='An error occurred while connecting additional report ""%1"":';ru='При подключении дополнительного отчета ""%1"" возникла ошибка:';vi='Khi kết nối báo cáo bổ sung ""%1"" đã xảy ra lỗi:'"), "%1", String(Ref))
				+ Chars.LF
	...
	Else
		...
		ReportParameters.Errors = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Item %1 is not an additional report';ru='Элемент %1 не является дополнительным отчетом';vi='Phần tử %1 không phải là báo cáo bổ sung'"), 
			"'"+ String(Ref) +"'");
...
Procedure ReceiveAdditionalReportsAndDataProcessorsSettings(UserRef, Settings) Export
	...
	SettingName = NStr("en='Settings of quick access to additional reports and data processors';ru='Настройки быстрого доступа к дополнительным отчетам и обработкам';vi='Tùy chỉnh truy cập nhanh đến báo cáo và bộ xử lý bổ sung'");
			...
			If Not Selection.IsFolder AND Not ValueIsFilled(Selection.PermissionsCompatibilityMode) Then
				...
				Except
					...
					ErrorText = """" + Object.Description + """:"
						+ Chars.LF + NStr("en='Cannot define a compatibility mode of extensions due to:';ru='Не удалось определить режим совместимости разрешений по причине:';vi='Không thể xác định chế độ tương thích cho phép theo nguyên nhân:'")
						+ Chars.LF + DetailErrorDescription(ErrorInfo())
						+ Chars.LF
						+ Chars.LF + NStr("en='Object was locked in the compatibility mode with version 2.1.3.';ru='Объект заблокирован в режиме совместимости с версией 2.1.3.';vi='Đối tượng bị phong tỏa trong chế độ tương thích với phiên bản 2.1.3.'");
...
Procedure RunDataProcessorInScheduledJob(ExternalDataProcessor, CommandID) Export
	...
	WriteInformation(ExternalDataProcessor, NStr("en='Command %1: Start.';ru='Команда %1: Запуск.';vi='Lệnh %1: Khởi động.'"), CommandID);
	...
	Except
		WriteError(
			...
			NStr("en='Command %1: Execution error:%2';ru='Команда %1: Ошибка выполнения:%2';vi='Lệnh %1: Lỗi thực hiện: %2'"),
			CommandID,
			Chars.LF + DetailErrorDescription(ErrorInfo()));
	EndTry;
	
	// Events log monitor record
	WriteInformation(ExternalDataProcessor, NStr("en='Command %1: End.';ru='Команда %1: Завершение.';vi='Lệnh %1: Kết thúc.'"), CommandID);
...
Function AssignedMetadataObjectsByExternalObjectKind(Kind) Export
	...
	For Each TypeOrMetadata IN TypesOrMetadataArray Do
		...
		If MetadataObject = AdditionalReportsAndDataProcessorsClientServer.DesktopID() Then
			...
			NewPurpose.Presentation = NStr("en='Desktop';ru='Рабочий стол';vi='Bàn làm việc'");
		Else
...
Function PresentationOfSection(Section) Export
	If Section = AdditionalReportsAndDataProcessorsClientServer.DesktopID()
		...
		Return NStr("en='Desktop';ru='Рабочий стол';vi='Bàn làm việc'");
	EndIf;
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Section ""%1""';ru='Раздел %1';vi='Phần hành %1'"), 
...
Procedure GeneratePopupCommandsFill(Form, Parameters)
	...
	If Popup = Undefined Then
		...
		Popup.Title = NStr("en='Fill in';ru='Заполнить';vi='Điền'");
...
Function ExecuteExternalObjectCommand(ExternalObject, CommandID, CommandParameters, ResultAddress)
	...
	If CommandDetails = Undefined Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Command %1 is not found.';ru='Команда %1 не обнаружена!';vi='Chưa tìm thấy lệnh %1!'"), CommandID);
...
Function RegisterDataProcessor(Val Object, Val RegistrationParameters) Export
	...
	If RegistrationData.SafeMode
		...
		// do nothing
	Else
		Result.ErrorText = NStr("en='To connect data processor run in the unsafe mode, administrative rights are required.';ru='Для подключения обработки, запускаемой в небезопасном режиме, требуются административные права.';vi='Để kết nối với bộ xử lý khởi động ở chế độ không an toàn, cần có quyền quản trị.'");
		Return Result;
	EndIf;
...
"Для загрузки нового объекта нажмите ""Создать"".';vi='Dạng đối tượng kết nhập (%1) không tương ứng với đối tượng hiện tại (%2)."
"Để kết nhập đối tượng mới, hãy nhấn nút ""Tạo mới"".'"),
	...
	ElsIf RegistrationParameters.IsReport <> (RegistrationData.Type = TypeAdditionalReport OR RegistrationData.Type = ReportKind) Then
		Result.ErrorText = NStr("en='Data processor kind from external data processor data does not correspond to its extension.';ru='Вид обработки, указанный в сведениях о внешней обработке, не соответствует ее расширению.';vi='Dạng bộ xử lý đã chỉ ra trong thông tin về bộ xử lý ngoài không tương ứng với phần mở rộng.'");
		Return Result;
	EndIf;
	...
	For Each CommandDetails IN RegistrationData.Commands Do
		...
		If Not ValueIsFilled(CommandDetails.StartVariant) Then
			CommonUseClientServer.MessageToUser(
				StrReplace(NStr("en='The launch method is not defined for the ""%1"" command.';ru='Для команды ""%1"" не определен способ запуска.';vi='Đối với lệnh ""%1"" chưa '"), "%1", CommandDetails.Presentation));
		EndIf;
...
"Возможно, он не подходит для этой версии программы.';vi='Không thể kết nối với báo cáo bổ sung từ tệp."
"Có thể là báo cáo không phù hợp với phiên bản này của chương trình.'");
		Else
...
"Возможно, она не подходит для этой версии программы.';vi='Không thể kết nối với bộ xử lý ngoài từ tệp."
"Có thể là bộ xử lý không phù hợp với phiên bản này của chương trình.'");
		EndIf;
		RegistrationResult.ErrorText = ErrorText;
		RegistrationResult.BriefErrorDescription = BriefErrorDescription(ErrorInfo);
		ErrorText = ErrorText + Chars.LF + Chars.LF + NStr("en='Technical information:';ru='Техническая информация:';vi='Thông tin kỹ thuật:'") + Chars.LF;
