...
Function QuestionAboutLongSynchronizationSettingCheckBox(ValueOfFlag = Undefined, SettingDetails = Undefined) Export
	...
	SettingDetails.Insert("Presentation", NStr("en='Show warning about long synchronization';ru='Показывать предупреждение о длительной синхронизации';vi='Hiển thị cảnh báo về đồng bộ hóa trường kỳ'"));
...
Procedure CreateOfflineWorkplaceInitialImage(Parameters,
	...
	OfflineWorkplaceCreationAssistant.CreateOfflineWorkplaceInitialImage(
				Parameters.FilterSettingsAtNode,
...
Function SetupPackageFileName() Export
	...
	Return NStr("en='Offline.zip';ru='Автономная работа.zip';vi='Làm việc độc lập.zip'");
...
Function DataTransferRestrictionsDescriptionFull(OfflineWorkplace) Export
	...
	FilterSettingsAtNode = DataExchangeServer.FilterSettingsAtNode(OfflineWorkExchangePlan, "");
	
	If FilterSettingsAtNode.Count() = 0 Then
		Return "";
	EndIf;
	
	Attributes = New Array;
	
	For Each Item IN FilterSettingsAtNode Do
	...
	For Each Item IN FilterSettingsAtNode Do
		...
		If TypeOf(Item.Value) = Type("Structure") Then
			...
			For Each NestedItem IN Item.Value Do
				...
				FilterSettingsAtNode[Item.Key][NestedItem.Key] = Table.UnloadColumn(NestedItem.Key);
				
			EndDo;
			
		Else
			
			FilterSettingsAtNode[Item.Key] = AttributeValues[Item.Key];
			
		EndIf;
		
	EndDo;
	
	Return DataExchangeServer.DataTransferRestrictionsDescriptionFull(OfflineWorkExchangePlan, FilterSettingsAtNode, "");
EndFunction
...
Function OfflineWorkplacesMonitor() Export
	...
	For Each SynchronizationSetting IN SynchronizationSettings Do
		...
		Else
			SynchronizationSetting.SynchronizationDatePresentation = NStr("en='not executed';ru='не выполнялась';vi='chưa thực hiện'");
		EndIf;
...
Function EventLogMonitorEventCreatingOfflineWorkplace() Export
	...
	Return NStr("en='Offline work. Generation of offline work place';ru='Автономная работа.Создание автономного рабочего места';vi='Làm việc độc lập. Tạo chỗ làm việc độc lập'", CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorEventDeletionOfflineWorkplace() Export
	...
	Return NStr("en='Offline work. Deletion of offline work place';ru='Автономная работа.Удаление автономного рабочего места';vi='Làm việc độc lập. Xóa chỗ làm việc độc lập'", CommonUseClientServer.MainLanguageCode());
...
Function OfflineWorkplaceDescriptionByDefault()
	...
	Result = NStr("en='Offline work - %1';ru='Автономная работа - %1';vi='Làm việc độc lập - %1'");
...
Function OfflineWorkplacePrefixAllowedSymbols()
	...
	Return NStr("en='ABCDEFGCHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';ru='АБВГДЕЖЗИКЛМНОПРСТУФХЦЧШЭЮЯабвгдежзиклмнопрстуфхцчшэюя';vi='АБВГДЕЖЗИКЛМНОПРСТУФХЦЧШЭЮЯабвгдежзиклмнопрстуфхцчшэюя'"); // 54 characters
...
Procedure SynchronizeDataWithApplicationInInternet() Export
	...
	If Not ThisIsOfflineWorkplace() Then
		...
		DetailedErrorReprasentationForEventLogMonitor =
			NStr("en='This infobase is not an offline workplace. Data synchronization is cancelled.';ru='Эта информационная база не является автономным рабочим местом. Синхронизация данных отменена.';vi='Cơ sở thông tin này không phải là chỗ làm việc độc lập. Hủy bỏ đồng bộ hóa dữ liệu.'",
	...
	If Cancel Then
		Raise NStr("en='Errors occurred when synchronizing the data with an online application (see the event log).';ru='В процессе синхронизации данных с приложением в Интернете возникли ошибки (см. журнал регистрации).';vi='Trong quá tình đồng bộ hóa dữ liệu với ứng dụng trên Internet đã nảy sinh lỗi (xem nhật ký sự kiện)'");
	EndIf;
...
"места должен выполняться в файловом режиме работы информационной базы.';vi='Cần thực hiện khởi động lần đầu chỗ làm việc"
"độc lập ở chế độ File-server làm việc với cơ sở thông tin.'");
	EndIf;
...
"Завершите работу конфигуратора и повторите запуск программы.';vi='Có thể, đã mở cơ sở thông tin ở chế độ Bộ thiết kế."
"Kết thúc công việc của Bộ thiết kế và khởi động lại chương trình.'");
	EndTry;
...
"Возможно, справочник пользователей не включен в состав плана обмена автономной работы.';vi='Chưa thực hiện đăng nhập cho người sử dụng."
"Có thể, danh mục người sử dụng không có trong thành phần của sơ đồ trao đổi khi làm việc độc lập.'");
		EndIf;
...
"Работа с автономным рабочим место невозможна.';vi='Chưa chuyển các tùy chỉnh sang chỗ làm việc độc lập."
"Không thể làm việc với chỗ làm việc độc lập.'");
	EndIf;
...
Function EventLogMonitorEventDataSynchronization()
	...
	Return NStr("en='Offline.Data synchronization';ru='Автономная работа.Синхронизация данных';vi='Làm việc độc lập. Đồng bộ hóa dữ liệu'", CommonUseClientServer.MainLanguageCode());
