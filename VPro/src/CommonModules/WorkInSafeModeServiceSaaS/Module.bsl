...
Function ExternalModuleExecutionMode(Val ExternalModule) Export
	...
	Else
		Raise NStr("en='The function cannot be called in the Infobase with disabled separation by data areas.';ru='Функция не предназначена для вызова в информационной базе, в которой выключено разделение по областям данных!';vi='Hàm không được dùng để gọi ra trong cơ sở thông tin, trong đó đã ngắt phân chia theo các vùng dữ liệu!'");
	EndIf;
...
Function AuthorizationKey(Val Resolution) Export
	...
	If StrLen(AuthorizationKey) > 32 Then
		Raise NStr("en='Key length excess';ru='Превышение длины ключа';vi='Tăng chiều dài khóa'");
	EndIf;
...
Function PermissionsTableRow(Val PermissionTable, Val Filter, Val AddWithout = True) Export
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Violation of row uniqueness in the permission table by filter %1';ru='Нарушение уникальности строк в таблице разрешений по отбору %1';vi='Vi phạm tính đơn trị của các dòng trong bảng cho phép theo bộ lọc %1'"),
...
Procedure LockGrantedPermissionsRegisters(Val ApplicationModule = Undefined, Val LockExternalModulesConnectionModes = True) Export
	...
	If Not TransactionActive() Then
		Raise NStr("en='Transaction is not active';ru='Транзакция не активна!';vi='Giao tác chưa được kích hoạt!'");
	EndIf;
...
"%2 - ТипВладельца: %3 - ИдентификаторВладельца: %4 - КлючРазрешения: %5.';vi='Trùng lặp"
"phần mở rộng"
"theo các trường khóa:"
"- ТипПрограммногоМодуля:"
"%1 - ИдентификаторПрограммногоМодуля:"
...
"- ТипВладельца: %3 - ИдентификаторВладельца: %4 - КлючРазрешения: %5.';vi='Xung đột"
"phần mở rộng theo khóa: -"
"ТипПрограммногоМодуля: %1 -"
"ИдентификаторПрограммногоМодуля: %2"
...
"ТипПрограммногоМодуля: %1"
"- ИдентификаторПрограммногоМодуля: %2"
"- ТипВладельца: %3 - ИдентификаторВладельца: %4 - КлючРазрешения: %5.';vi='Thử"
"xóa sự cho phép không tồn tại:"
"- SowtwareModuleType: %1"
"- SowtwareModuleIdentifier: %2"
"- OwnerType: %3 - OwnerIdentifier: %4 - PermissionKey: %5.'"),
			String(SoftwareModuleType),
			String(SoftwareModuleID),
			String(OwnerType),
			String(IDOwner),
...
Function PackageProcessingResult(Val PackageIdentifier) Export
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Query pack %1 is not found';ru='Не найден пакет запросов %1';vi='Không tìm thấy gói truy vấn %1'"), PackageIdentifier);
	...
	While Selection.Next() Do
		...
		If StandardProcessing Then
			...
			Else
				...
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Unserialized key program module: - Type: 1% - ID: %2';ru='Не сериализован программный модуль по ключу: - Тип: %1 - Идентификатор: %2';vi='Không đặt trình tự mô-đun chương trình theo khóa: - Kiểu: %1 - Tên (ID): %2'"),
					Selection.SoftwareModuleType,
					Selection.SoftwareModuleID);
