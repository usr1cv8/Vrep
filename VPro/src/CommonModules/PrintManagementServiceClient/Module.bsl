...
Procedure ExecuteConnectedPrintCommandPrintingObjectsPreparation(AdditionalParameters)
	...
	If PrintObjects.Count() = 0 Then
		Raise NStr("en='Command cannot be executed for the specified object.';ru='Команда не может быть выполнена для указанного объекта!';vi='Lệnh không thể điền đối với đối tượng đã chỉ ra!'")
	EndIf;
...
Procedure CheckDocumentsPostingPostingDialog(Parameters) Export
	...
	If PrintManagementServerCall.PostingRightAvailable(Parameters.UnpostedDocuments) Then
		If Parameters.UnpostedDocuments.Count() = 1 Then
			QuestionText = NStr("en='To print the document, post it first. Post the document and continue?';ru='Для того чтобы распечатать документ, его необходимо предварительно провести. Выполнить проведение документа и продолжить?';vi='Để in chứng từ, cần kết chuyển trước. Thực hiện kết chuyển chứng từ và tiếp tục?'");
		Else
	...
	Else
		If Parameters.UnpostedDocuments.Count() = 1 Then
			WarningText = NStr("en='To print the document, post it first. Insufficient rights to post the document, cannot print.';ru='Для того чтобы распечатать документ, его необходимо предварительно провести. Недостаточно прав для проведения документа, печать невозможна.';vi='Để in chứng từ, trước hết cần kết chuyển văn bản đó. Không đủ quyền kết chuyển văn bản, không thể in.'");
		Else
			WarningText = NStr("en='To print the documents, post them first. Insufficient rights to post the documents, cannot print.';ru='Для того чтобы распечатать документы, их необходимо предварительно провести. Недостаточно прав для проведения документов, печать невозможна.';vi='Để in chứng từ, trước hết cần kết chuyển văn bản. Không đủ quyền để kết chuyển văn bản, không thể in.'");
		EndIf;
...
Procedure CheckDocumentsPostingDocumentsPosting(QuestionResult, AdditionalParameters) Export
	...
	MessagePattern = NStr("en='Document %1 is not posted: %2';ru='Документ %1 не проведен: %2';vi='Chứng từ %1 chưa kết chuyển: %2'");
...
Procedure ShowQuestionAboutFileOperationsExtensionSetting(NOTifyDescription) Export
	#If WebClient Or MobileClient Then
		MessageText = NStr("en='To continue printing, install extension for 1C:Enterprise web client.';ru='Для продолжения печати необходимо установить расширение для веб-клиента 1С:Предприятие.';vi='Để tiếp tục in, cần thiết lập phần mở rộng đối với Web-client 1C:DOANH NGHIỆP.'");
