...
"параметра сеанса ТекущийПользователь в сеансе без указания всех разделителей.';vi='Không cho phép nhận"
"tham số phiên làm việc ТекущийПользователь trong phiên làm việc mà chưa chỉ ra tất cả đường chia tách.'");
	EndIf;
		...
		Else
			...
			If Not ResultExternalUsers.IsEmpty() Then
				...
				If Not ExternalUsers.UseExternalUsers() Then
					...
					ErrorMessageText = NStr("en='External users are disabled.';ru='Внешние пользователи отключены.';vi='Người sử dụng ngoài bị ngắt bỏ.'");
...
"возникла ошибка: ""%2""."
""
"Обратитесь к администратору.';vi='Chưa thực hiện đăng nhập. Công việc với hệ thống sẽ kết thúc."
"Không tìm thấy người sử dụng %1 trong danh mục ""Người sử dụng""."
""
"Khi thử thêm người sử dụng vào danh mục"
"đã xảy ra lỗi: ""%2""."
""
...
"Возникла внутренняя ошибка при поиске пользователя.';vi='"
"Đã xuất hiện lỗi nội bộ khi tìm kiếm người sử dụng.'");
		Raise ErrorMessageText;
	EndIf;
...
"режим конфигурирования 1С:Предприятия для этого использовать не следует.';vi='Đang thực hiện khởi động theo tên người sử dụng có vai trò ""Toàn quyền"","
"mà chưa ghi nhận họ trong danh sách người sử dụng."
"Đã thực hiện tự động ghi nhận trong danh sách người sử dụng."
""
"Để đưa ra danh sách và tùy chỉnh quyền người sử dụng sử dụng danh sách Người sử dụng, để thực hiện điều này không cần sử dụng chế độ thiết kế cấu hình 1C:DOANH NGHIỆP.'");
			
			AfterWriteAdministratorOnAuthorization(Comment);
			
			WriteLogEvent(
				NStr("en='Users.Administrator is registered in the Users catalog';ru='Пользователи.Администратор зарегистрирован в справочнике Пользователи';vi='Người sử dụng.Người quản trị được ghi nhận trong danh mục Người sử dụng'",
...
""
"Для ведения списка и настройки прав пользователей предназначен список Пользователи,"
"режим конфигурирования 1С:Предприятия для этого использовать не следует.';vi='Không thể khởi động theo tên người sử dụng có quyền Quản trị,"
"bởi vì chưa ghi nhận họ trong danh sách người sử dụng."
""
"Để quản lý danh sách và tùy chỉnh quyền người sử dụng, sử dụng danh sách Người sử dụng,"
"để thực hiện điều này không cần sử dụng chế độ thiết kế cấu hình 1C:DOANH NGHIỆP.'");
		EndIf;
	EndIf;
...
Procedure ProcessRolesInterface(Action, Parameters) Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='An error occurred in the UsersService.RolesInterfaceProcessor() procedure Incorrect value of the Action parameter: %1.';ru='Ошибка в процедуре ПользователиСлужебный.ОбработатьИнтерфейсРолей() Неверное значение параметра Действие: ""%1"".';vi='Lỗi trong thủ tục UsersService.RolesInterfaceProcessor()  Sai giá trị tham số Thao tác: ""%1"".'"),
			Action);
	EndIf;
...
// See PropertiesManagementOverridable.OnGetPredefinedPropertySets.
...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	For Each Section IN Sections Do
		...
		Work.Presentation  = NStr("en='Incorrect information on users';ru='Некорректные сведения о пользователях';vi='Sai thông tin người sử dụng'");
...
Function PasswordStringStoredValue(Val Password,
	...
	Try
		...
		TempInfobaseUser.Name = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Temporary user (%1)';ru='Временный пользователь (%1)';vi='Người sử dụng hiện thời (%1)'"),
			New UUID);
...
Procedure BeginOfDBUserProcessing(UserObject,
	...
	ProcessingParameters.Insert("MessageTextNotEnoughRights",
		NStr("en='Insufficient rights to change infobase user.';ru='Недостаточно прав для изменения пользователя информационной базы.';vi='Không đủ quyền để thay đổi người sử dụng cơ sở thông tin.'"));
...
"В параметре ОписаниеПользователяИБ не указано свойство Действие.';vi='Lỗi khi ghi người sử dụng ""%1""."
"Trong tham số IBUserDescription chưa chỉ ra thuộc tính Thao tác.'"),
			UserObject.Ref);
	EndIf;
...
"неверное значение ""%2"" свойства Действие.';vi='Lỗi khi ghi người sử dụng ""%1""."
"Trong tham số IBUserDescription chỉ ra"
"sai giá trị ""%2"" của thuộc tính Thao tác.'"),
			UserObject.Ref,
...
"ИБ с пользователем в справочнике, с которым уже сопоставлен другой пользователем ИБ.';vi='Lỗi khi ghi lại người sử dụng ""%1""."
"Không thể so sánh người sử dụng"
"CSTT với người sử dụng trong danh mục, mà đã được so sánh với người sử dụng khác trong CSTT.'"),
...
"сопоставлен с другим пользователем в справочнике ""%2"".';vi='Lỗi khi ghi lại người sử dụng ""%1""."
"Không thể so sánh người sử dụng của CSTT"
"với người sử dụng này trong danh mục, bởi vì người sử dụng này"
"đã được so sánh với người sử dụng khác trong danh mục ""%2"".'"),
				FoundUser,
				UserObject.Description);
		EndIf;
...
"Продолжить?';vi='Trong danh sách người sử dụng"
"chương trình đã thêm người sử dụng đầu tiên, do đó người này sẽ tự động được ấn định vai trò ""Toàn quyền''''."
"Tiếp tục?'");
...
"Сначала создайте администратора в справочнике Пользователи.';vi='Người sử dụng đầu tiên cơ sở thông tin cần có toàn quyền."
"Người sử dụng ngoài không thể có toàn quyền."
"Ban đầu hãy tạo người quản trị trong danh mục Người sử dụng.'");
			Return True;
		EndIf;
	EndIf;
...
Function UnspecifiedUserProperties() Export
	...
	Properties.Insert("FullNameForSearch", NStr("en='<Not specified>';ru='<Не указан>';vi='<Chưa chỉ ra>'"));
...
Function CheckIBUserFullName(Val IBUserDescription, Cancel) Export
	...
	If IBUserDescription.Property("Name") Then
		...
		If IsBlankString(Name) Then
			...
			CommonUseClientServer.MessageToUser(
				NStr("en='Login is not entered.';ru='Не заполнено Имя (для входа).';vi='Chưa điền Tên gọi (để đăng nhập).'"),
				,
				"Name",
				,
				Cancel);
		...
		ElsIf StrLen(Name) > 64 Then
			...
			CommonUseClientServer.MessageToUser(
				NStr("en='Login cannot exceed 64 characters.';ru='Имя (для входа) превышает 64 символа.';vi='Tên (để đăng nhập) đã vượt quá 64 ký tự.'"),
				,
				"Name",
				,
				Cancel);
		...
		ElsIf Find(Name, ":") > 0 Then
			CommonUseClientServer.MessageToUser(
				NStr("en='Login contains invalid character "":"".';ru='Имя (для входа) содержит запрещенный символ "":"".';vi='Tên (để đăng nhập) có ký tự bị cấm ""."".'"),
				,
				"Name",
				,
				Cancel);
			   ...
			   AND IBUser.UUID
				...
				If FoundUser = Undefined
				 OR Not Users.InfobaseUserWithFullAccess() Then
					...
					ErrorText = NStr("en='Login is already in use.';ru='Имя (для входа) уже занято.';vi='Tên (để đăng nhập) đã bị chiếm.'");
				Else
					ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='Login is already used by user ""%1"".';ru='Имя (для входа) уже занято для пользователя ""%1"".';vi='Tên (để đăng nhập) đã bị chiếm cho người sử dụng ""%1"".'"),
						String(FoundUser));
				EndIf;
	...
	If IBUserDescription.Property("Password") Then
		...
		If IBUserDescription.Password <> Undefined
			...
			CommonUseClientServer.MessageToUser(
				NStr("en='Password and password confirmation do not match.';ru='Пароль и подтверждение пароля не совпадают.';vi='Mật khẩu và xác nhận lại mật khẩu không trùng khớp.'"),
				,
				"Password",
				,
				Cancel);
...
"format ""\\DomainName\UserName"".';vi='Người dùng hệ điều hành cần"
"có dạng ""\\DomainName\UserName"".'"),
					,
					"OSUser",
					,
					Cancel);
...
"""%1"" роль"
"""%2"" группы внешних пользователей ""%3"" не найдена в метаданных.';vi='Khi cập nhật"
"vai trò"
"người sử dụng ngoài"
"""%1"" không tìm thấy vai trò"
"""%2"" nhóm người sử dụng ngoài ""%3"" trong metadata.'"),
				TrimAll(Selection.ExternalUser.Description),
				Selection.Role,
				String(Selection.ExternalUserGroup));
			
			WriteLogEvent(
				NStr("en='Users.Role was not found in metadata';ru='Пользователи.Роль не найдена в метаданных';vi='Người sử dụng.Chưa tìm thấy vai trò trong metadata'",
				     CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
				,
				,
...
Function AuthorizationObjectInUse(Val AuthorizationObjectRef,
	...
	If Result Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='External user related to object ""%1"" already exists.';ru='Уже существует внешний пользователь, связанный с объектом ""%1"".';vi='Đã tồn tại người sử dụng ngoài mà có liên hệ với đối tượng ""%1"".'"),
...
Function UserTransferToNewGroup(UserArray, GroupSource,
	...
	If DisplacedUsersArray.Count() = 0 AND ArrayIsNotDisplacedUsers.Count() = 0 Then
		If UserArray.Count() = 1 Then
			MessageText = NStr("en='User ""%1"" is already a member of group ""%2"".';ru='Пользователь ""%1"" уже включен в группу ""%2"".';vi='Người sử dụng ""%1"" đã có trong nhóm ""%2"".'");
			NameRoamingUser = CommonUse.ObjectAttributeValue(UserArray[0], "Description");
		Else
			MessageText = NStr("en='All the selected users are already included in group ""%2"".';ru='Все выбранные пользователи уже включены в группу ""%2"".';vi='Đã đưa tất cả người sử dụng được chọn vào nhóm ""%2"".'");
			NameRoamingUser = "";
		EndIf;
...
Function WordEndingGenerating(ConvertedNumber, MeasurementUnitInWordParameters = Undefined) Export
	...
	If MeasurementUnitInWordParameters = Undefined Then
		MeasurementUnitInWordParameters = NStr("en='user,of user,users,,,,,,0';ru='user,of user,users,,,,,,0';vi='người dùng,người dùng,người dùng,,,,,,0'");
	EndIf;
...
Function GeneratingMessageToUser(UserArray, GroupReceiver,
	...
	If QuantityNotDisplacedUsers > 0 Then
		...
		If QuantityNotDisplacedUsers = 1 Then
			...
			UserMessage = NStr("en='User ""%1"" cannot be included in group ""%2"",';ru='Пользователь ""%1"" не может быть включен в группу ""%2"",';vi='Không thể đưa người sử dụng ""%1"" vào nhóm ""%2"",'");
			UserMessage = UserMessage + Chars.LF + 
									?(NOT UserTypeIsSameAsWithGroup, 
									NStr("en='because only %3 are its members.';ru='Т.к. в состав ее участников входят только %3.';vi='Bởi vì trong thành phần tham gia chỉ bao gồm %3.'"),
									NStr("en='because the ""All users of the specified type"" check box is selected for the group.';ru='Т.к. у группы стоит признак ""Все пользователи заданного типа"".';vi='Bởi vì trong nhóm có dấu hiệu ""Tất cả người sử dụng với kiểu đã đặt"".'"));
		Else
			Subject = "";
...
"или у группы стоит признак ""Все пользователи заданного типа"".';vi='Không phải tất cả người sử dụng có thể có trong nhóm ""%2"","
"bởi vì thành phần người tham gia nhóm này chỉ bao gồm %3"
"hoặc nhóm có dấu hiệu ""Tất cả người sử dụng của kiểu đã xác định"".'");
		EndIf;
	...
	ElsIf UserCount = 1 Then
		...
		If GroupReceiver = Catalogs.UsersGroups.AllUsers
			Or GroupReceiver = Catalogs.ExternalUsersGroups.AllExternalUsers Then
			StringAction = NStr("en='excluded from the group';ru='исключен из группы';vi='loại bỏ khỏi nhóm'");
			GroupDescription = CommonUse.ObjectAttributeValue(GroupSource, "Description");
		ElsIf Move Then
			StringAction = NStr("en='transferred to group';ru='перемещен в группу';vi='đã chuyển vào nhóm'");
		Else
			StringAction = NStr("en='included in group';ru='включен в группу';vi='đã đưa vào nhóm'");
		EndIf;
		
		UserMessage = NStr("en='""%1"" %2 ""%3""';ru='""%1"" %2 ""%3""';vi='""%1"" %2 ""%3""'");
	ElsIf UserCount > 1 Then
		
		RowObject = WordEndingGenerating(UserCount);
		If GroupReceiver = Catalogs.UsersGroups.AllUsers Then
			StringAction = NStr("en='excluded from the group';ru='исключены из группы';vi='loại bỏ khỏi nhóm'");
			GroupDescription = CommonUse.ObjectAttributeValue(GroupSource, "Description");
		ElsIf Move Then
			StringAction = NStr("en='transferred to group';ru='перемещены в группу';vi='đã di chuyển vào nhóm'");
		Else
			StringAction = NStr("en='included in group';ru='включены в группу';vi='đã đưa vào nhóm'");
		EndIf;
		UserMessage = NStr("en='%1 %2 ""%3""';ru='%1 %2 ""%3""';vi='%1 %2 ""%3""'");
	EndIf;
...
""
"Không tìm thấy"
"người sử dụng ""%1"" trong danh mục ""Người sử dụng"" và ""Người sử dụng ngoài""."
""
"Hãy liên hệ với người quản trị.'");
	Else
		ErrorMessageText = NStr("en='Authorization not executed. System work will be complete."
...
"Пользователь ""%1"" не найден в справочнике ""Пользователи""."
""
"Обратитесь к администратору.';vi='Đăng nhập không thực hiện được. Làm việc với hệ thống sẽ kết thúc."
""
"Người sử dụng ""%1"" không tìm thấy trong danh mục ""Người sử dụng""."
""
"Hãy liên hệ với người quản trị.'");
	EndIf;
...
Procedure UpdateHierarhicalUsersGroupsContents(UsersGroup,
	...
	Query.Text = StrReplace(Query.Text, "&UserInRegisterFilter",    UserInRegisterFilter);
	Query.Text = StrReplace(Query.Text, "&UserInCatalogFilter", UserInCatalogFilter);
...
"права на изменение общих данных: ""%3"".';vi='Thử thay đổi thành phần vai trò"
"của người sử dụng"
"%1 với vai trò  ""%2"", quyền được cấp"
"để thay đổi dữ liệu chung: ""%3"".'"),
					IBUser.FullName,
					Role.Presentation(),
					TableString);
			Else
...
"права на изменение общих данных: ""%3"".';vi='Thử ấn định"
"cho người sử dụng"
"%1 vai trò không hợp lệ ""%2"", quyền được"
"cấp để thay đổi dữ liệu chung: ""%3"".'"),
					IBUser.FullName,
					Role.Presentation(),
					TableString);
			EndIf;
			
			EventName = NStr("en='Users.An error occurred when assigning roles to the infobase user';ru='Пользователи.Ошибка при установке ролей пользователю ИБ';vi='Người sử dụng. Lỗi khi thiết lập vai trò cho người sử dụng CSTT'",
			     CommonUseClientServer.MainLanguageCode());
...
"пользователя"
"""%1"" с ролью ""%2"", предоставляющей право ""%3"".';vi='Thử thay đổi thành phần vai trò của"
"người sử dụng"
"""%1"" với vai trò ""%2"", được cấp cho quyền ""%3"".'"),
						String(IBUser),
						Role.Presentation(),
						Right);
				Else
...
"""%1"""
"недопустимую роль ""%2"", предоставляющую право ""%3"".';vi='Thử ấn định cho người sử dụng"
"""%1"""
"vai trò không hợp lệ ""%2"", quyền được cấp ""%3"".'"),
						String(IBUser),
						Role.Presentation(),
						Right);
				EndIf;
	...
	If DeletedRoles.Count() = 1 Then
		RemovalMessageText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Cannot delete unavailable role of user ""%1"".';ru='Невозможно удалить недоступную роль у пользователя ""%1"".';vi='Không thể xóa vai trò không khả dụng đối với người sử dụng ""%1"".'"),
			IBUser.FullName);
	ElsIf DeletedRoles.Count() > 1 Then
		RemovalMessageText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Cannot delete unavailable roles of user ""%1"".';ru='Невозможно удалить недоступные роли у пользователя ""%1"".';vi='Không thể xóa vai trò không khả dụng của người sử dụng ""%1"".'"),
			IBUser.FullName);
	EndIf;
...
"не может быть добавлена роль ""%2"".';vi='Đối với người sử dụng ""%1"""
"không thể thêm vai trò ""%2"".'"),
			IBUser.FullName,
	...
	ElsIf AddedRoles.Count() > 1 Then
		...
		EndDo;
		AddingMessageText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Roles cannot be added to the ""%1"" user: %2.';ru='Пользователю ""%1"" не могут быть добавлены роли:%2.';vi='Đối với người sử dụng ""%1"" không thể thêm vai trò: %2.'"), 
			IBUser.FullName,
			Roles);
	EndIf;
...
"Реквизит Служебный не допускается изменять в подписках на события.';vi='Lỗi khi ghi người sử dụng ""%1""."
"Không cho phép thay đổi mục tin Hệ thống khi đăng ký đến sự kiện."
"'"),
			UserObject.Ref);
	EndIf;
...
"Нельзя пометить подготовленным пользователя, которому разрешен вход в программу.';vi='Lỗi khi ghi lại người sử dụng ""%1""."
"Không thể đánh dấu người sử dụng được phép đăng nhập vào chương trình là đã được soạn.'"),
			UserObject.Ref);
	EndIf;
...
"Không cho phép thay đổi mục tin InfobaseUserID."
...
"Обновление реквизита выполняется автоматически.';vi='Lỗi khi ghi lại người sử dụng ""%1""."
"Không thể thay đổi mục tin InfobaseUserProperties."
"Đang thực hiện cập nhật tự động mục tin.'"),
			UserObject.Ref);
	EndIf;
...
"Нельзя помечать на удаление пользователя, которому разрешен вход в программу.';vi='Lỗi khi ghi lại người sử dụng ""%1""."
"Không thể đặt dấu xóa người sử dụng được phép đăng nhập vào chương trình.'"),
			UserObject.Ref);
	EndIf;
...
"Нельзя пометить недействительным пользователя, которому разрешен вход в программу.';vi='Lỗi khi ghi lại người sử dụng ""%1""."
"Không thể đặt dấu người sử dụng không có hiệu lực mà được phép đăng nhập vào chương trình.'"),
			UserObject.Ref);
	EndIf;
