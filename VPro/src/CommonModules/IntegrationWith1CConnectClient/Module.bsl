...
Procedure Select1CConnectFile(ClosingAlert, PathToFile = "") Export
	...
	SuggestionText = NStr("en='To select 1C-Connect application you shall install the file work extension.';ru='Для выбора приложения 1С-Коннект необходимо установить расширение работы с файлами.';vi='Để chọn ứng dụng 1C-Connect, cần cài đặt phần mở rộng làm việc với tệp.'");
...
Procedure Select1CConnectFileAfterExtensionInstallation(Attached, AdditionalParameters) Export
	...
	Dialog.Title = NStr("en='Select the executable 1C-Connect file';ru='Выберите исполняемый файл 1С-Коннект';vi='Hãy chọn tệp thực thi 1C-Connect'");
	...
	Filter = NStr("en='connect.exe (*.exe)|*.exe';ru='сonnect.exe (*.exe)|*.exe';vi='сonnect.exe (*.exe)|*.exe'");
...
Procedure Run1CConnect() Export
	...
	If Not IsWindowsClient() Then
		ShowMessageBox(,NStr("en='To work with the 1C-Connect application, you need to have Microsoft Windows operating system.';ru='Для работы с приложением 1С-Коннект необходима операционная система Microsoft Windows.';vi='Để làm việc với ứng dụng 1C-Connect, cần có hệ điều hành Microsoft Windows.'"));
		Return
	EndIf;
	
	Notification = New NotifyDescription("Run1CConnectAfterExtensionInstallation", ThisObject);
	MessageText = NStr("en='To start 1C-Connect, you shall install the file work extension.';ru='Для запуска 1С-Коннект необходимо установить расширение работы с файлами.';vi='Để khởi động 1C-Connect, cần cài đặt phần mở rộng làm việc với tệp.'");
