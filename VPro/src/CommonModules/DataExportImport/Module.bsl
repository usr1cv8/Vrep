...
Function ExportDataIntoArchive(Val ExportParameters) Export
	...
	If Not ValidateRights() Then
		Raise NStr("en='Insufficient access rights to export data.';ru='Недостаточно прав доступа для выгрузки данных!';vi='Không đủ quyền truy cập để kết xuất dữ liệu!'");
	EndIf;
...
Procedure ImportDataFromArchive(Val ArchiveName, Val ExportParameters) Export
	...
	If Not ValidateRights() Then
		Raise NStr("en='Insufficient access rights to import data.';ru='Недостаточно прав доступа для загрузки данных!';vi='Không đủ quyền truy cập để kết nhập dữ liệu!'");
	EndIf;
...
Function ExportArchiveIsCompatibleWithCurrentConfiguration(Val ArchiveName) Export
	...
	Try
		...
		If ImportDescriptionItem = Undefined Then
			Raise NStr("en='File DumpInfo.xml is missing in the export file.';ru='В файле выгрузки отсутствует файл DumpInfo.xml!';vi='Trong tệp kết xuất không có tệp DumpInfo.xml!'");
		EndIf;
