...
Function ClientWorkParametersOnStart(Parameters) Export
	...
	ProccesClientParametersAtServer(Parameters);	
...
Function ServerPlatformTypeAsString() Export
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Unknown platform type ""%1""';ru='Неизвестный тип платформы ""%1""';vi='Kiểu nền tảng không xác định ""%1""'"),
...
Function WriteErrorToEventLogMonitorAtStartOrExit(StopWork, Val Event, Val ErrorText) Export
	...
	If Event = "Start" Then
		EventName = NStr("en='Start application';ru='Запуск программы';vi='Khởi động chương trình'", CommonUseClientServer.MainLanguageCode());
		If StopWork Then
			ErrorDescriptionBegin = NStr("en='An exception is thrown when starting the application. Application launch is aborted.';ru='Возникла исключительная ситуация при запуске программы. Запуск программы аварийно завершен.';vi='Đã xuất hiện tình huống đặc biệt khi khởi động chương trình. Đã bị lỗi khi khởi động chương trình.'");
		Else
			ErrorDescriptionBegin = NStr("en='An exception is thrown when starting the application.';ru='Возникла исключительная ситуация при запуске программы.';vi='Đã xuất hiện tình huống đặc biệt khi khởi động chương trình.'");
		EndIf;
	Else
		EventName = NStr("en='Exit application';ru='Завершение программы';vi='Kết thúc chương trình'", CommonUseClientServer.MainLanguageCode());
		ErrorDescriptionBegin = NStr("en='An exception is thrown when closing the application.';ru='Возникла исключительная ситуация при завершении программы.';vi='Đã xuất hiện tình huống đặc biệt khi kết thúc chương trình.'");
	EndIf;
...
Procedure ProccesClientParametersAtServer(Parameters)
