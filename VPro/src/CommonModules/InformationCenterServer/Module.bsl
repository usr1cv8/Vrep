...
Procedure OutputContextReferences(Form, FormGroup, GroupCount = 3, RefsCountInGroup = 1, OutputReferenceAll = True, PathToForm = "") Export
	...
	Try
		...
		FormGroup.Group = ChildFormItemsGroup.HorizontalIfPossible;
...
Function GetEventNameForEventLogMonitor() Export
	...
	Return NStr("en='Info center';ru='Информационный центр';vi='Trung tâm thông tin'", ServiceTechnologyIntegrationWithSSL.MainLanguageCode());
...
Function GetInformationCenterProxy() Export
	...
	If Not ServiceTechnologyIntegrationWithSSL.SubsystemExists("StandardSubsystems.SaaS") Then
		Raise(NStr("en='Cannot connect to the service manager.';ru='Не возможно подключиться к Менеджеру сервиса.';vi='Không thể kết nối đến Bộ quản lý dịch vụ.'"));
	EndIf;
...
"Пожалуйста, повторите попытку позже.';vi='Trung tâm ý tưởng tạm thời không hoạt động."
"Xin hãy vui lòng thử lại sau.'")
...
Function GenerateCommentsTitle(Val IdeaPresentation) Export 
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Comments: %1';ru='Комментарии: %1';vi='Ghi chú: %1'"), IdeaPresentation.CommentsCount);
...
Function GenerateImplementationDateTitle(Val IdeaPresentation) Export 
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Planned sale date: %1';ru='Плановая дата реализации: %1';vi='Ngày bán hàng dự kiến: %1'"), IdeaPresentation.PlanMadeDatePresentation);
...
Function GenerateRejectionDate(Val IdeaPresentation) Export 
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Rejected: %1';ru='Отклонено: %1';vi='Đã từ chối: %1'"), Format(IdeaPresentation.ClosingDate, "DLF=DD"));
...
Function GenerateImplementationDate(Val IdeaPresentation) Export 
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Sold: %1';ru='Реализовано: %1';vi='Đã bán: %1'"), Format(IdeaPresentation.ClosingDate, "DLF=DD"));
...
"Пожалуйста, повторите попытку позже.';vi='Bộ phận hỗ trợ tạm thời không có."
...
Function GenerateXMLWithTechnicalParameters(AdditionalParameters = Undefined) Export
	...
	Try
		DeleteFiles(XMLFile);
	Except
		WriteLogEvent(NStr("en='Information center. Sending a message to the technical support. Cannot remove a temporary file of technical parameters.';ru='Информационный центр. Отправка сообщения в техподдержку. Не удалось удалить временный файл технических параметров.';vi='Trung tâm thông tin. Gửi thông tin đến bộ phận hỗ trợ kỹ thuật. Không thể xóa bỏ tệp tham số kỹ thuật tạm thời.'"), 
...
Procedure GenerateOutputGroups(Form, RefsTable, FormGroup, GroupCount, RefsCountInGroup, OutputReferenceAll)
	...
	If OutputReferenceAll Then 
		...
		Item.Title               = NStr("en='All';ru='Все';vi='Tất cả'");
		Item.Hyperlink             = True;
