...
Function SecurityProfile(Val ClusterAdministrationParameters, Val ProfileName) Export
	...
	If SecurityProfiles.Count() <> 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Security profile %2 is not registered in server cluster %1.';ru='В кластере серверов %1 не зарегистрирован профиль безопасности %2!';vi='Trong cụm Server %1 chưa đăng ký hồ sơ an toàn %2!'"), ClusterIdentifier, ProfileName);
	EndIf;
...
Procedure CreateSecurityProfile(Val ClusterAdministrationParameters, Val SecurityProfileProperties) Export
	...
	If SecurityProfiles.Count() = 1 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Security profile %2 is already registered in server cluster %1.';ru='В кластере серверов %1 уже зарегистрирован профиль безопасности %2!';vi='Trong cụm Server %1 đã đăng ký hồ sơ an toàn %2!'"), ClusterIdentifier, ProfileName);
	EndIf;
...
Function GetCluster(Val ClusterAdministrationParameters)
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Server cluster with port %1 is not found';ru='Не обнаружен кластер серверов с портом %1';vi='Không tìm thấy cụm Server với cổng %1'"), ClusterAdministrationParameters.ClusterPort);
	EndIf;
...
Function GetInfobase(Val ClusterIdentifier, Val ClusterAdministrationParameters, Val InformationBaseAdministrationParameters)
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Infobase %2 is not registered in the %1 server cluster.';ru='В кластере серверов %1 не зарегистрирована информационная база %2!';vi='Trong cụm Server %2 chưa đăng ký cơ sở thông tin %2!'"), ClusterIdentifier, InformationBaseAdministrationParameters.NameInCluster);
	EndIf;
...
Function RunCommand(Val Pattern, Val ClusterAdministrationParameters, Val ParameterValues = Undefined)
	...
	#If Server Then
		...
		If SafeMode() Then
			Raise NStr("en='Cluster administration is unavailable in the safe mode.';ru='Администрирование кластера невозможно в безопасном режиме!';vi='Không thể quản trị cụm Server ở chế độ an toàn!'");
		EndIf;
		
		If CommonUseReUse.DataSeparationEnabled() Then
			Raise NStr("en='Applied infobase cannot administer cluster in SaaS.';ru='В модели сервиса недопустимо выполнение прикладной информационной базой функций администрирования кластера!';vi='Trong mô hình dịch vụ đám mây, cơ sở thông tin ứng dụng không thể thực hiện các chức năng quản trị cụm Server!'");
		EndIf;
...
"- Для компьютеров с ОС Linux требуется установить пакет 1c-enterprise83-server*.';vi='Không thể thực hiện giao dịch quản trị cụm Server do nguyên nhân: không tìm thấy tệp %1!"
""
"Để quản trị cụm Server thông qua Server quản trị (ras), cần cài đặt Client-server quản trị (ras) trên máy tính này."
"Để cài đặt:"
"- Đối với máy tính chạy hệ điều hành Windows, cần cài lại nền tảng, sau khi cài đặt cấu phần ""Quản trị Server 1C:DOANH NGHIỆP"";"
"- Đối với máy tính chạy hệ điều hành Linux, cần cài đặt gói 1c-enterprise83-server*.'"),
