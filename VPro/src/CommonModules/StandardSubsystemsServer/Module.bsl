...
Function SessionParametersSetting(SessionParameterNames) Export
	...
	If SessionParameterNames.Find("CanceledLongOperations") <> Undefined Then
		SessionParameters.CanceledLongOperations = New FixedArray(New Array);
	EndIf;
...
Procedure SaveAuthenticationParametersOnSite(SavedParameters) Export
	...
	CommonUseClientServer.Validate(NOT CommonUse.SubsystemExists("OnlineUserSupport"), 
		NStr("en='Invalid procedure call.';ru='Недопустимый вызов процедуры.';vi='Gọi ra thủ tục không hợp lệ.'"), "AddressClassifierService.SaveAuthenticationParametersOnSite");
...
Function ClientParametersOnServer() Export
	...
	If ClientParameters.Count() = 0
	   AND CurrentRunMode() <> Undefined Then
		...
		Raise NStr("en='Client parameters on server are not entered.';ru='Не заполнены параметры клиента на сервере.';vi='Chưa điền tham số Client trên Server.'");
	EndIf;
...
Function RunBackgroundJobWithClientContext(MethodName, Parameters = Undefined, Key = "", Description = "") Export
	...
	If CurrentRunMode() = Undefined Then
		Raise NStr("en='You can start background job with the customer context only when there is the customer.';ru='Запуск фонового задания с контекстом клиента возможен только при наличии клиента.';vi='Chỉ có thể khởi động nhiệm vụ nền với Client ngữ cảnh khi có Client.'");
	EndIf;
...
"базы: - Невозможно установить монопольный режим - Версия конфигурации не предусматривает обновление без установки монопольного режима.';vi='Không thể thực hiện"
"cập nhật cơ sở thông tin: - Không thể thiết lập chế độ độc quyền - Phiên bản cấu hình không hỗ trợ cập nhật mà chưa thiết lập chế độ độc quyền.'");
...
Procedure CheckForUpdatesApplicationWorkParameters(ConstantName, ParameterNames = "", Cancel = Undefined) Export
	...
	If ParameterNames <> "" Then
		...
		If UpdateNeeded Then
			...
			If CurrentRunMode() = Undefined Then
				Raise
					NStr("en='Logon to the application is temporarily unavailable due to update to the newer version';ru='Вход в программу временно невозможен в связи с обновлением на новую версию';vi='Tạm thời không thể đăng nhập vào chương trình do đang cập nhật phiên bản mới'");
			Else
				Raise
	...
	If EnableContent.Count() <> 0
		...
		If EnableContent.Count() <> 0 Then
			...
			DetailsExceptions1 = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='The following metadata objects should be included in exchange plan %1: %2';ru='В состав плана обмена %1 должны входить следующие объекты метаданных: %2';vi='Thành phần hệ thống trao đổi %1 cần có các đối tượng metadata sau: %2'"),
				ExchangePlanName,
		...
		If ExcludeFromComposition.Count() <> 0 Then
			...
			DetailsExceptions2 = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='The following metadata objects should NOT be included in exchange plan %1: %2';ru='В состав плана обмена %1 НЕ должны входить следующие объекты метаданных: %2';vi='Thành phần hệ thống trao đổi %1 KHÔNG cần có các đối tượng metadata sau: %2'"),
				ExchangePlanName,
...
"Требуется запретить авторегистрацию для следующих объектов метаданных: %2';vi='Thành phần hệ thống trao đổi %1 không cần có đối tượng với dấu hiệu thiết lập làtự động ghi danh. Cần cấm tự động ghi danh đối với các đối tượng metadata như sau: %2.'"),
				ExchangePlanName,
...
Procedure RegisterObjectInAllNodes(Val Object, Val ExchangePlanName) Export
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		...
		If CommonUseReUse.CanUseSeparatedData() Then
			Raise NStr("en='Changes registration of shared data in the split mode.';ru='Регистрация изменений неразделенных данных в разделенном режиме.';vi='Việc ghi nhận dữ liệu chưa phân tách ở chế độ phân tách.'");
		EndIf;
		
		If Not CommonUseReUse.IsSeparatedMetadataObject("ExchangePlan." + ExchangePlanName,
				CommonUseReUse.MainDataSeparator())
			Then
			Raise NStr("en='Changes registration for undivided exchange plans is not supported.';ru='Регистрация изменений для неразделенных планов обмена не поддерживается.';vi='Không hỗ trợ ghi nhận thay đổi cho các sơ đồ trao đổi chưa phân tách.'");
		EndIf;
		
		If CommonUseReUse.IsSeparatedMetadataObject(Object.Metadata().FullName(),
				CommonUseReUse.MainDataSeparator())
			Then
			Raise NStr("en='Changes registration for divided objects is not supported.';ru='Регистрация изменений для разделенных объектов не поддерживается.';vi='Không hỗ trợ ghi nhận thay đổi đối với đối tượng đã phân tách.'");
		EndIf;
...
Procedure WhenFillingOutPermitsForAccessToExternalResources(PermissionsQueries) Export
	...
	permissions.Add(WorkInSafeMode.PermissionToUseTemporaryFilesDirectory(True, True,
		NStr("en='For the application operation.';ru='Для возможности работы программы.';vi='Để có thể làm việc với chương trình.'")));
	permissions.Add(WorkInSafeMode.PermissionToUseExternalComponent("CommonTemplate.DeclinationComponentFullName", 
		NStr("en='To use functions of full name conjugation.';ru='Для использования функций по склонению ФИО.';vi='Để sử dụng chức năng lệch họ tên.'")));
	...
	Except
		WriteLogEvent(
			...
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='An error occurred when cleaning up temporary file directory ""%1"":%2';ru='Ошибка очистки каталога временных файлов ""%1"":%2';vi='Lỗi xóa thư mục tệp tạm thời ""%1"":%2'"),
				PathToDirectory,
...
"  to update infobase data during the next start.';vi='Đối với người phát triển: bạn cần"
"cập nhật dữ liệu trợ giúp có tác động đến công việc của ứng dụng."
"Để cập nhật, bạn cần:"
"- sử dụng bộ xử lý dữ liệu ngoài ""Công cụ của người phát triển: Cập nhật dữ liệu trực thuộc""."
"- khởi động ứng dụng với tham số dòng lệnh 1C:DOANH NGHIỆP 8"
" ""/C RunInfobaseUpdate"","
"- tăng số phiên bản cấu hình"
" để cập nhật dữ liệu cơ sở thông tin trong lần khởi động tiếp theo.'");
...
//
// Parameters:
//  Row - String - Строка для преобразования.
// 
// Returns:
//  String - Строка, содержащая только допустимые символы для наименования колонок таблицы значений.
//
Function TransformStringToValidColumnDescription(Row) Export
	
	InvalidCharacters = ":;!@#$%^&-~`'.,?{}[]+=*/|\ ()_""";
	Result = "";
	For IndexOf = 1 To StrLen(Row) Do
		Char =  Mid(Row, IndexOf, 1);
		If StrFind(InvalidCharacters, Char) > 0 Then
...
Function ClientParametersAtServer() Export
	...
	If ClientParameters.Count() = 0
	   And CurrentRunMode() <> Undefined Then
		...
		Raise NStr("en='Не заполнены параметры клиента на сервере.';ru='Не заполнены параметры клиента на сервере.';vi='Chưa điền tham số Client trên Server.'");
	EndIf;
...
Function AddClientWorkParametersOnStart(Parameters) Export
	...
	Parameters.Insert("CanUseSeparatedData",	CommonUseReUse.CanUseSeparatedData());
	...
	Parameters.Insert("AskConfirmationOnClose", AskConfirmationOnExit());
...
Function EventsHandlers() Export
	...
	If AreRequiredEventsWithoutHandlers.Count() > 0 Then
		EventName  = NStr("en='EVENT HANDLERS';ru='Обработчики событий';vi='Hàm sự kiện'", CommonUseClientServer.MainLanguageCode());
		
		Comment = NStr("en='Handlers are not defined for the following required events:';ru='Для следующих обязательных событий не определены обработчики:';vi='Đối với sự kiện bắt buộc tiếp theo chưa xác định hàm sự kiện:'")
...
"Подробности см. в журнале регистрации.';vi='Đối với các sự kiện bắt buộc chưa xác định hàm sự kiện."
...
"быть выполнено в разделенном режиме модели сервиса.';vi='Không thể thực hiện cập nhật tham số làm việc của chương trình"
"trong chế độ phân tách của mô hình dịch vụ.'");
	EndIf;
...
"cập nhật cơ sở thông tin: - Không thể thiết lập chế độ độc quyền - Phiên bản cấu hình không hỗ trợ cập nhật mà chưa thiết lập chế độ độc quyền.'");
			...
			If ValueIsFilled(CriticalChangesList) Then
				...
				If RunImport Then
					EventName = NStr("en='Metadata object IDs.Import critical changes';ru='Идентификаторы объектов метаданных.Требуется загрузить критичные изменения';vi='Tên định danh đối tượng metadata. Cần kết nhập các thay đổi quan trọng'",
						CommonUseClientServer.MainLanguageCode());
				Else
					// Setting in the IB subordinate node during the first start.
					EventName = NStr("en='Metadata object IDs.It is required to execute critical changes';ru='Идентификаторы объектов метаданных.Требуется выполнить критичные изменения';vi='Tên định danh của đối tượng metadata. Cần thực hiện các thay đổi quan trọng'",
						CommonUseClientServer.MainLanguageCode());
				EndIf;
...
"регистрации в событии ""Идентификаторы объектов метаданных.Требуется загрузить критичные изменения"").';vi='Từ nút chính không kết nhập các thay đổi trong danh mục ""Tên định danh đối tượng metadata"":"
"khi kiểm tra đã phát hiện ra rằng, cần kết nhập các thay đổi quan trọng (chi tiết xem trong nhật ký sự kiện "
"""Tên định danh đối tượng metadata. Cần kết nhập các thay đổi quan trọng"").'");
				Else
					// Setting in the IB subordinate node during the first start.
					ErrorText =
...
"регистрации в событии ""Идентификаторы объектов метаданных.Требуется выполнить критичные изменения"").';vi='Tại nút chính đã không cập nhật danh mục ""Tên đối tượng metadata"":"
"khi kiểm tra đã phát hiện ra cần thực hiện những thay đổi quan trọng (chi tiết xem trong nhật ký sự kiện"
"""Tên đối tượng metadata. Cần thực hiện những thay đổi quan trọng"").'");
				EndIf;
...
Procedure DisablePredefinedItemsDeletionMarkupBeforeWriting(Source, Cancel) Export
	...
	If Source.IsNew() Then
		Raise
			NStr("en='Cannot create a predefined item marked for deletion.';ru='Недопустимо создавать предопределенный элемент помеченный на удаление.';vi='Không được phép tạo phần tử định trước đã bị đánh dấu xóa.'");
	Else
...
"на удаление предопределенный элемент: ""%1"".';vi='Không thể đặt"
"dấu xóa phần tử định trước: ""%1"".'"),
				String(Source.Ref));
...
"элемент, помеченный на удаление: ""%1"".';vi='Không thể gắn phần tử đã đặt dấu xóa"
"với tên của phần tử định trước: ""%1"".'"),
				String(Source.Ref));
		EndIf;
	EndIf;
...
"удалять предопределенный элемент ""%1"".';vi='Không thể"
"xóa phần tử định trước ""%1"".'"),
		String(Source.Ref));
...
"Необходимо использовать вариант языка ""%2"".';vi='Không hỗ trợ phương án ngôn ngữ hệ thống trong cấu hình ""%1""."
"Cần sử dụng phương án ngôn ngữ ""%2"".'"),
	...
	If CommonUseClientServer.CompareVersions(SystemInfo.AppVersion, MinimalPlatformVersion) < 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='For launch 1C:Enterprise platform version %1 or higher is required.';ru='Для запуска необходима версия платформы 1С:Предприятие %1 или выше.';vi='Để khởi động, cần có phiên bản nền tảng 1C:DOANH NGHIỆP %1 hoặc cao hơn.'"), MinimalPlatformVersion);
	EndIf;
...
"with 1C:Enterprise version not less then 8 in configuration.3.5 or Do not use.';ru='Режим совместимости конфигурации с 1С:Предприятием версии %1 не поддерживается. Для запуска установите в конфигурации режим совместимости с 1С:Предприятием версии не ниже 8.3.5 или ""Не использовать"".';vi='Không hỗ trợ chế độ tương thích của cấu hình với 1C:DOANH NGHIỆP phiên bản %1. Để khởi động, trong cấu hình hãy cài đặt chế độ tương thích với 1C:DOANH NGHIỆP phiên bản không dưới 8.3.5 hoặc ""Không sử dụng"".'"),
			UnavailableMode);
	EndIf;
	
	// Check if the configuration version is filled in.
	If IsBlankString(Metadata.Version) Then
		Raise NStr("en='Property Version of the configuration is not entered.';ru='Не заполнено свойство конфигурации Версия.';vi='Chưa điền thuộc tính của cấu hình Phiên bản.'");
	Else
		Try
...
"Правильный формат, например: ""1.2.3.45"".';vi='Chưa điền đúng thuộc tính cấu hình Phiên bản: ""%1""."
"Định dạng đúng, ví dụ: ""1.2.3.45"".'"),
				Metadata.Version);
		EndTry;
		If ZeroVersion Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
...
"Версия не может быть нулевой.';vi='Chưa điền đúng thuộc tính cấu hình Phiên bản: ""%1""."
"Phiên bản không thể là không.'"),
				Metadata.Version);
		EndIf;
	EndIf;
...
"указаны стандартные роли АдминистраторСистемы и ПолныеПрава или указаны лишние роли.';vi='Trong cấu hình, ở thuộc tính DefaultRoles chưa"
"chỉ ra các vai trò chuẩn SystemAdministrator và FullRights hoặc đã chỉ ra các vai trò thừa.'");
	EndIf;
...
""
"Событие"
"""%1"" уже добавлено.';vi='Lỗi khi đưa ra danh sách sự kiện."
""
"Đã thêm"
"sự kiện ""%1"".'"),
				Event);
...
""
"Ошибка в имени модуля ""%2"".';vi='Lỗi khi"
"soạn hàm xử lý sự kiện ""%1""."
""
"Lỗi trong tên mô-đun ""%2"".'"),
						Event,
						Handler.Module);
				EndIf;
...
"подготовке обработчиков события ""%1""."
""
"Модуль ""%2"" уже добавлен.';vi='Lỗi khi"
"soạn hàm sự kiện ""%1""."
""
"Đã thêm mô-đun ""%2"".'"),
						Event,
...
""
"Обработчик ""%2"""
"уже добавлен для события ""%3"".';vi='Lỗi khi"
"soạn hàm xử lý sự kiện ""%1""."
""
"Hàm xử lý ""%2"""
"đã được bổ sung đối với sự kiện ""%3"".'"),
						Event,
						HandlerName,
