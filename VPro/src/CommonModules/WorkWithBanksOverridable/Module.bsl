	...
	For ind = 0 To Refs.UBound() Do
		...
		If Not IsBlankString(ParametersObject.ParentCode) AND Not ValueIsFilled(ParametersObject.Parent) Then
			...
			If Not ValueIsFilled(BankObject.Parent) Then
				EventName = ?(EventName = "",
					NStr("en='Pick from classifier';ru='Подбор из классификатора';vi='Chọn từ bảng mã hiệu'"), EventName);
				ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Cannot receive a parent from the item with branch ID %1';ru='Не смогли получить родителя у элемента с БИК %1';vi='Không thể nhận lớp trên của phần tử có mã ngân hàng %1'"), TrimAll(ParametersObject.Code));
