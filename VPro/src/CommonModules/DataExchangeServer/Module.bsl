...
Procedure NodesSettingFormOnCreateAtServer(Form, Cancel) Export
	...
	If Parameters.Property("GetDefaultValues") Then
		...
		FilterSsettingsAtNode                   = FilterSettingsAtNode(ExchangePlanName, Parameters.CorrespondentVersion, , SettingID);
	...
	Else
		...
		FilterSettingsAtNode = Form.Parameters.Settings.FilterSettingsAtNode;
		CorrespondentInfobaseNodeFilterSetup = Form.Parameters.Settings.CorrespondentInfobaseNodeFilterSetup;
		
	EndIf;
	
	Context.Insert("FilterSettingsAtNode", FilterSettingsAtNode);
...
Procedure NodeConfigurationFormOnCreateAtServer(Form, ExchangePlanName) Export
	...
	ValidateRequiredFormAttributes(Form, "FilterSettingsAtNode, CorrespondentVersion");
	
	Form.CorrespondentVersion   = Form.Parameters.CorrespondentVersion;
	Form.FilterSettingsAtNode = FilterSettingsAtNode(ExchangePlanName, Form.CorrespondentVersion, , SettingID);
	
	NodeSettingsFormOnCreateAtServerHandler(Form, "FilterSettingsAtNode");
...
Procedure CorrespondentInfobaseNodeSettingsFormOnCreateAtServer(Form, ExchangePlanName, Data = Undefined) Export
	...
	Form.FilterSettingsAtNode      = CorrespondentInfobaseNodeFilterSetup(ExchangePlanName, Form.CorrespondentVersion, , SettingID);
...
Function HeaderStructureHyperlinkMonitorProblems(Nodes = Undefined) Export
	...
	If Quantity > 0 Then
		...
		Title = NStr("en='Warnings (%1)';ru='Предупреждения (%1)';vi='Cảnh báo (%1)'");
		Title = StringFunctionsClientServer.SubstituteParametersInString(Title, Quantity);
		Picture = PictureLib.Warning;
		
	Else
		
		Title = NStr("en='No warnings';ru='Предупреждений нет';vi='Không có cảnh báo'");
		Picture = New Picture;
...
Procedure BeforeCheckingIdentifiersOfMetadataObjectsInSubordinateNodeDIB(Cancel = False) Export
	...
	Except
		...
		WriteLogEvent(
			NStr("en='Data exchange. Import the ""Metadata object IDs"" catalog';ru='Обмен данными.Загрузка справочника ""Идентификаторы объектов метаданных""';vi='Trao đổi dữ liệu Kết nhập danh mục ""Tên đối tượng metadata""'", CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,,,
			DetailErrorDescription(ErrorInfo()));
		
		Raise
			NStr("en='Changes of the ""Metadata object IDs"" catalog are not imported from the main node: data import error. For more information, see event log.';ru='Из главного узла не загружены изменения справочника ""Идентификаторы объектов метаданных"": ошибка загрузки данных.См. подробности в журнале регистрации.';vi='Từ nút chính chưa kết nhập các thay đổi của danh mục ""Tên (ID) đối tượng metadata"": lỗi kết nhập dữ liệu. Chi tiết xem trong Nhật ký sự kiện.'");
	EndTry;
	SetPrivilegedMode(True);
	SetDataExchangeMessageImportModeBeforeStart("ImportAllowed", False);
	SetPrivilegedMode(False);
...
""
"После этого запустите программу.';vi='Đã kết nhập những thay đổi của chương trình nhận được từ nút chính."
"Hãy kết thúc công việc với chương trình. Hãy mở chương trình ở chế độ"
"Bộ thiết kế và thực hiện lệnh ""Cập nhật cấu hình cơ sở dữ liệu (F7)""."
""
"Sau đó khởi động chương trình.'");
		EndIf;
...
"%1"
"';vi='Chưa kết nhập từ nút chính các thay đổi của danh mục"
"""Tên (ID) đối tượng metadata"": khi kiểm tra đã phát hiện rằng,"
"cần kết nhập những thay đổi quan trọng (chi tiết xem trong Nhật ký sự kiện, sự kiện cụ thể ""Tên (ID)"" đối tượng metadata. Cần kết nhập những thay đổi quan trọng'''')."
""
"Nếu thông báo trao đổi dữ liệu không khả dụng,"
"hãy khởi động lại chương trình, tùy chỉnh tham số kết nối và đồng bộ lại."
"Nếu chưa nhận tất cả những thay đổi cần thiết"
"từ nút chính thì tại nút chính hãy cập nhật"
"cơ sở thông tin, ghi nhận dữ liệu"
"để khôi phục nút CSTTPT và đồng bộ lại dữ liệu trong cả nút chính và nút phụ."
""
"Để cập nhật cơ sở thông tin tại"
"nút chính, hãy khởi động chương trình với tham số khởi động ЗапуститьОбновлениеИнформационнойБазы."
"%1'"),
...
"изменений"", в которой выполните команду ""Зарегистрировать данные для восстановления подчиненного узла РИБ"" (меню Еще).';vi='Để ghi nhận dữ liệu nhằm khôi phục nút trực thuộc"
"CSTTPT, tại nút chính, trong phần hành ""Quản trị"""
"hãy mở ""Tùy chỉnh đồng bộ hóa dữ liệu"", chuyển đến tham chiếu ""Làm việc độc lập"". Bằng cách nhấn lệnh ""Thành phần dữ liệu gửi đi"""
"hãy mở ra biểu mẫu ""Ghi nhận"
"thay đổi"", trong đó thực hiện lệnh ""Ghi nhận dữ liệu để khôi phục nút trực thuộc CSTTPT"" (menu Hơn nữa).'"),
				NStr("en='To register data for restoration of DIB subordinate node, open"
"Data synchronization settings in the main node"
...
"данных"" откройте форму ""Регистрация"
"изменений"", в которой выполните команду ""Зарегистрировать данные для восстановления подчиненного узла РИБ"" (меню Еще).';vi='Để ghi nhận dữ liệu nhằm khôi phục nút phụ"
"CSTTPT, tại nút chính, trong phân hệ ""Quản trị"""
"hãy mở ""Tùy chỉnh đồng bộ hóa dữ liệu"", chuyển đến tham chiếu ""Đồng bộ hóa dữ liệu"". Bằng cách nhấn lệnh ""Thành phần dữ liệu"
"gửi đi"", mở ra biểu mẫu ""Ghi nhận"
"thay đổi"", trong đó hãy thực hiện lệnh ""Ghi nhận dữ liệu để phục hồi nút trực thuộc CSTTPT"" (menu Hơn nữa).'")));
...
Procedure ExecuteUpdateOfDataExchangeRules() Export
	...
	Cancel = False;
	ExecuteUpdateOfTypicalRulesVersionForDataExchange(Cancel, ImportedFromFileExchangeRules, ImportedFromFileRecordRules);
	
	If Cancel Then
		Raise NStr("en='Errors occurred during update of data exchange rules (see the event log).';ru='При обновлении правил обмена данными возникли ошибки (см. Журнал регистрации).';vi='Khi cập nhật quy tắc trao đổi dữ liệu phát sinh lỗi (xem Nhật ký sự kiện).'");
	EndIf;
...
Procedure UpdateScheduledJobsOfDataExchangeScripts() Export
	...
	While Selection.Next() Do
		...
		If Cancel Then
			Raise NStr("en='An error occurred when updating a scheduled job for the data exchange scenario.';ru='Ошибка при обновлении регламентного задания для сценария обмена данными.';vi='Lỗi khi cập nhật nhiệm vụ thường kỳ đối với kịch bản trao đổi dữ liệu.'");
		EndIf;
...
Function SelectChanges(Val Node, Val MessageNo, Val FilterSample = Undefined) Export
	...
	If TransactionActive() Then
		Raise NStr("en='Data change selection is prohibited in an active transaction.';ru='Выборка изменений данных запрещена в активной транзакции.';vi='Cấm chọn thay đổi dữ liệu trong giao tác đã kích hoạt.'");
	EndIf;
...
Function DefaultExchangePlanSettings(ExchangePlanName) Export
	...
	AssistantFormTitle = NStr("en='Data synchronization with %Application% (configuration)';ru='Синхронизация данных с %Программа% (настройка)';vi='Đồng bộ hóa dữ liệu với %Application% (tùy chỉnh)'");
...
Function FTPServerNameAndPath(Val ConnectionString) Export
	...
	If (Upper(Left(ConnectionString, 6)) <> "FTP://"
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='FTP connection string does not match the format: ""%1""';ru='Строка подключения к FTP-ресурсу не соответствует формату: ""%1""';vi='Dòng kết nối đến nguồn FTP không tương ứng với định dạng ""%1""'"), ConnectionString
		);
	EndIf;
	...
	If ConnectionParameters.Count() < 3 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Server name is not specified in FTP resource connection string: ""%1""';ru='В строке подключения к FTP-ресурсу не указано имя сервера: ""%1""';vi='Tại dòng kết nối đến nguồn FTP chưa chỉ ra tên Server: ""%1""'"), ConnectionString
		);
	EndIf;
...
Procedure WhenFillingOutProfileGroupsAccessProvided(ProfileDescriptions, UpdateParameters) Export
	...
	ProfileDescription.Description = NStr("en='Data synchronization with other applications';ru='Синхронизация данных с другими программами';vi='Đồng bộ hóa dữ liệu với các chương trình khác'");
...
"которым должны быть доступны средства для мониторинга и синхронизации данных с другими программами.';vi='Được ấn định thêm cho những người sử dụng"
"cần truy cập đến công cụ để theo dõi và đồng bộ hóa dữ liệu với chương trình khác.'");
...
Procedure ExecuteDataExchangeByScenarioOfExchangeData(Cancel, ExchangeExecutionSettings, LineNumber = Undefined) Export
	...
	While Selection.Next() Do
		...
		Else
			...
			MessageString = NStr("en='Data exchange process start for setting %1';ru='Начало процесса обмена данными по настройке %1';vi='Bắt đầu quá trình trao đổi dữ liệu theo tùy chỉnh %1'", CommonUseClientServer.MainLanguageCode());
...
Procedure ExecuteDataExchangeByScheduledJob(ExchangeScenarioCode) Export
	...
	If Not ValueIsFilled(ExchangeScenarioCode) Then
		Raise NStr("en='Data exchange scenario is not specified.';ru='Не задан сценарий обмена данными.';vi='Chưa chỉ ra kịch bản trao đổi dữ liệu.'");
	EndIf;
	...
	Else
		MessageString = NStr("en='Data exchange script with code %1 is not found.';ru='Сценарий обмена данными с кодом %1 не найден.';vi='Kịch bản trao đổi dữ liệu có mã %1 không tìm thấy.'");
...
Function GetExchangeMessageToTemporaryDirectory(Cancel, InfobaseNode, ExchangeMessageTransportKind, OutputMessages = True) Export
	...
	If ExchangeSettingsStructure.Cancel Then
		...
		If OutputMessages Then
			NString = NStr("en='Errors occurred while initializing processing of exchange message transport.';ru='При инициализации обработки транспорта сообщений обмена возникли ошибки.';vi='Khi khởi xướng bộ xử lý chuyển tải thông điệp trao đổi phát sinh lỗi.'");
	...
	If ExchangeSettingsStructure.ExchangeProcessResult <> Undefined Then
		...
		If OutputMessages Then
			NString = NStr("en='Errors occurred while receiving exchange messages.';ru='При получении сообщений обмена возникли ошибки.';vi='Khi nhận thông điệp trao đổi phát sinh lỗi.'");
			CommonUseClientServer.MessageToUser(NString,,,, Cancel);
		EndIf;
...
Function GetExchangeMessageFromCorrespondentInfobaseToTempDirectory(Cancel, InfobaseNode, OutputMessages = True) Export
	...
	Except
		If OutputMessages Then
			Message = NStr("en='Cannot exchange: %1';ru='Не удалось произвести обмен: %1';vi='Không thể thực hiện trao đổi: %1'");
	...
	If Cancel Then
		...
		If OutputMessages Then
			...
			Message = NStr("en='Cannot export data: %1';ru='Не удалось выгрузить данные: %1';vi='Không thể kết xuất dữ liệu: %1'");
...
Function GetExchangeMessageToTempDirectoryFromCorrespondentInfobaseOverWebService(
	...
	If Proxy = Undefined Then
		...
		Cancel = True;
		Message = NStr("en='An error occurred when establishing connection to the second infobase: %1';ru='Ошибка при установке подключения ко второй информационной базе: %1';vi='Lỗi khi đặt kết nối tới cơ sở thông tin thứ hai: %1'");
	...
	Except
		...
		Cancel = True;
		Message = NStr("en='When exporting data, errors occurred in the second infobase: %1';ru='При выгрузке данных возникли ошибки во второй информационной базе: %1';vi='Khi kết nhập dữ liệu phát sinh lỗi trong cơ sở thông tin thứ hai: %1'", CommonUseClientServer.MainLanguageCode());
		Message = StringFunctionsClientServer.SubstituteParametersInString(Message, DetailErrorDescription(ErrorInfo()));
		ExchangeSettingsStructure = New Structure("EventLogMonitorMessageKey");
		ExchangeSettingsStructure.EventLogMonitorMessageKey = GetEventLogMonitorMessageKey(InfobaseNode, Enums.ActionsAtExchange.DataImport);
		WriteLogEventOfDataExchange(Message, ExchangeSettingsStructure, True);
	...
	If LongOperation Then
		ExchangeSettingsStructure = New Structure("EventLogMonitorMessageKey");
		ExchangeSettingsStructure.EventLogMonitorMessageKey = GetEventLogMonitorMessageKey(InfobaseNode, Enums.ActionsAtExchange.DataImport);
		WriteLogEventOfDataExchange(NStr("en='Pending data from the correspondent base...';ru='Ожидание получения данных от базы-корреспондента...';vi='Chờ nhận dữ liệu từ cơ sở đối ứng...'",
	...
	Except
		...
		Cancel = True;
		Message = NStr("en='Errors occurred when receiving an exchange message from file transfer service: %1';ru='Возникли ошибки при получении сообщения обмена из сервиса передачи файлов: %1';vi='Phát sinh lỗi khi nhận thông điệp trao đổi từ công cụ truyền tệp: %1'", CommonUseClientServer.MainLanguageCode());
		Message = StringFunctionsClientServer.SubstituteParametersInString(Message, DetailErrorDescription(ErrorInfo()));
		ExchangeSettingsStructure = New Structure("EventLogMonitorMessageKey");
		ExchangeSettingsStructure.EventLogMonitorMessageKey = GetEventLogMonitorMessageKey(InfobaseNode, Enums.ActionsAtExchange.DataImport);
		WriteLogEventOfDataExchange(Message, ExchangeSettingsStructure, True);
	...
	Except
		Cancel = True;
		Message = NStr("en='Errors occurred while receiving exchange messages: %1';ru='При получении сообщения обмена возникли ошибки: %1';vi='Khi nhận thông điệp trao đổi phát sinh lỗi: %1'", CommonUseClientServer.MainLanguageCode());
		Message = StringFunctionsClientServer.SubstituteParametersInString(Message, DetailErrorDescription(ErrorInfo()));
		ExchangeSettingsStructure = New Structure("EventLogMonitorMessageKey");
		ExchangeSettingsStructure.EventLogMonitorMessageKey = GetEventLogMonitorMessageKey(InfobaseNode, Enums.ActionsAtExchange.DataImport);
		WriteLogEventOfDataExchange(Message, ExchangeSettingsStructure, True);
...
Procedure ExecuteDataExchangeForInfobaseNodeOverFileOrString(InfobaseNode = Undefined,
	...
	If InfobaseNode = Undefined Then
		...
		If InfobaseNode.IsEmpty() Then
			ErrorMessageString = NStr("en='Exchange plan node %1 with code %2 is not found.';ru='Узел плана обмена %1 с кодом %2 не найден.';vi='Nút sơ đồ trao đổi %1 có mã %2 không tìm thấy.'");
	...
	If ExchangeSettingsStructure.Cancel Then
		ErrorMessageString = NStr("en='An error occurred when initializing the data exchange process.';ru='Ошибка при инициализации процесса обмена данными.';vi='Lỗi khi khởi xướng tiến trình trao đổi dữ liệu.'");
		FixEndExchange(ExchangeSettingsStructure);
		Raise ErrorMessageString;
	EndIf;
		...
		If ExchangeSettingsStructure.UseLargeDataTransfer Then
			...
			If ExchangeProcessResultCompleted(ExchangeSettingsStructure.ExchangeProcessResult) Then
				...
				Try
					...
					If LongOperation Then
						WriteLogEventOfDataExchange(NStr("en='Pending data import in the correspondent base...';ru='Ожидание загрузки данных в базе-корреспонденте...';vi='Chờ kết nhập dữ liệu trong cơ sở đối ứng...'",
							CommonUseClientServer.MainLanguageCode()), ExchangeSettingsStructure);
						Return;
					EndIf;
...
Procedure ExecuteExchangeActionForInfobaseNodeByExternalConnection(Cancel, InfobaseNode,
	...
	If ExchangeSettingsStructure.DoDataImport Then
		...
		If ObjectConversionRules = Undefined Then
			...
			NString = NStr("en='Conversion rules for exchange plan %1 are not specified in the second infobase. Exchange is canceled.';ru='Не заданы правила конвертации во второй информационной базе для плана обмена %1. Обмен отменен.';vi='Chưa chỉ ra quy tắc chuyển đổi trong cơ sở thông tin thứ hai đối với sơ đồ trao đổi %1. Trao đổi bị hủy bỏ.'",
		...
		Except
			WriteLogEventOfDataExchange(
				StringFunctionsClientServer.SubstituteParametersInString(NStr("en='An error occurred in the second infobase: %1';ru='Возникла ошибка во второй информационной базе: %1';vi='Xuất hiện lỗi trong cơ sở thông tin thứ hai: %1'"),
...
Procedure BeforeReadingMessageExchange(Val Recipient, ExchangeMessage, StandardProcessing)
	...
	If IsSubordinateDIBNode()
		...
		If TypeOf(SavedExchangeMessage) = Type("BinaryData") Then
			...
			WriteEventGetData(Recipient, NStr("en='The exchange message was received from cache.';ru='Сообщение обмена получено из кэша.';vi='Thông điệp trao đổi đã được nhận từ cache.'"));
...
Procedure ExecuteStandardNodeChangeImport(
	...
	If MessageReader.Sender <> InfobaseNode Then // Message is intended not for this node.
		...
		WriteLogEvent(EventLogMonitorMessageKey, EventLogLevel.Error,
			InfobaseNode.Metadata(), InfobaseNode, NStr("en='The exchange message contains data for another infobase node.';ru='Сообщение обмена содержит данные для другого узла информационной базы.';vi='thông điệp trao đổi có dữ liệu đối với nút khác của cơ sở thông tin.'",
			CommonUseClientServer.MainLanguageCode()));
		//
		Return;
	EndIf;
...
Function GetFileFromStorageInService(Val FileID, Val InfobaseNode, Val PartSize = 1024, Val AuthenticationParameters = Undefined) Export
	...
	Else
		...
		Comment = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Start of the Internet exchange message receiving (number of file parts is %1).';ru='Начало получения сообщения обмена из Интернета (количество частей файла %1).';vi='Bắt đầu nhận thông điệp trao đổi từ Internet (số lượng các phần trong tệp %1).'"),
		...
		Comment = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='End of exchange message receiving from the Internet (size of a compressed exchange message is %1 MB).';ru='Окончание получения сообщения обмена из Интернета (размер сжатого сообщения обмена %1 Мб).';vi='Kết thúc nhận thông điệp trao đổi từ Internet (dung lượng thông điệp trao đổi nén %1 Mb).'"),
...
Procedure OnReceiveFileFromStore(Val FileID, FileName)
	...
	If QueryResult.IsEmpty() Then
		Definition = NStr("en='A file with ID %1 not found.';ru='Файл с идентификатором %1 не обнаружен.';vi='Không tìm thấy tệp có Tên định danh %1.'");
...
Function DataExchangeMonitorTable(Val ExchangePlans, Val ExchangePlanAdditionalProperties = "", Val OnlyErrors = False) Export
	...
	If OnlyErrors Then
		Filter = "
			|	WHERE ISNULL(DataExchangeStatusExport.ExchangeProcessResult, 0) <> 0 OR ISNULL(DataExchangeStatusImport.ExchangeProcessResult, 0) <> 0"
		;
	Else
		Filter = "";
	EndIf;
...
Procedure CheckUseDataExchange() Export
	...
	If GetFunctionalOption("UseDataSynchronization") <> True Then
		...
		MessageText = NStr("en='Data synchronization is prohibited by administrator.';ru='Синхронизация данных запрещена администратором.';vi='Người quản trị cấm đồng bộ hóa dữ liệu.'");
...
Procedure CheckIfExchangesPossible() Export
	...
	If Not Users.RolesAvailable("DataSynchronization, DataSynchronizationSetting") Then
		...
		Raise NStr("en='Insufficient rights to data synchronization.';ru='Недостаточно прав для синхронизации данных.';vi='Không đủ quyền để đồng bộ hóa dữ liệu.'");
	...
	ElsIf InfobaseUpdate.InfobaseUpdateRequired()
		...
		Raise NStr("en='Infobase is being updated.';ru='Информационная база находится в состоянии обновления.';vi='Cơ sở thông tin đang có trạng thái cập nhật.'");
...
Procedure CheckExchangesAdministrationPossibility() Export
	...
	If Not Users.RolesAvailable("DataSynchronizationSetting") Then
		...
		Raise NStr("en='Insufficient rights to administer data synchronization.';ru='Недостаточно прав для администрирования синхронизации данных.';vi='Không đủ quyền để quản trị đồng bộ hóa dữ liệu.'");
...
"Для синхронизации данных через прямое подключение требуется использовать ОС Windows.';vi='Không thể đồng bộ hóa dữ liệu thông qua kết nối trực tiếp đến Server dưới hệ điều hành Linux."
"Để đồng bộ hóa dữ liệu thông qua kết nối trực tiếp, cần sử dụng hệ điều hành Windows.'");
...
Procedure FixEndExchange(ExchangeSettingsStructure) Export
	...
	If ExchangeSettingsStructure.IsDIBExchange Then
		MessageString = NStr("en='%1, %2';ru='%1, %2';vi='%1, %2'", CommonUseClientServer.MainLanguageCode());
	...
	Else
		MessageString = NStr("en='%1, %2; Processed objects: %3';ru='%1, %2; Объектов обработано: %3';vi='%1, %2; Đối tượng đã xử lý: %3'", CommonUseClientServer.MainLanguageCode());
		MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString,
							ExchangeSettingsStructure.ExchangeProcessResult,
							ExchangeSettingsStructure.ActionOnExchange,
...
Function UnpackZIPFile(Val ArchiveFileFullName, Val FileUnpackPath, Val ArchivePassword = "") Export
	...
	Except
		...
		MessageString = NStr("en='An error occurred when unpacking archive files %1 to directory: %2';ru='Ошибка при распаковке файлов архива: %1 в каталог: %2';vi='Lỗi khi giải nén tệp lưu trữ: %1 sang thư mục: %2'");
...
Function PackIntoZipFile(Val ArchiveFileFullName, Val FilePackingMask, Val ArchivePassword = "") Export
	...
	Except
		...
		MessageString = NStr("en='An error occurred when packing archive files: %1 from directory: %2';ru='Ошибка при запаковке файлов архива: %1 из каталог: %2';vi='Lỗi khi nén tệp lưu trữ: %1 trong thư mục: %2'");
...
Function GetEventLogMonitorMessageKey(InfobaseNode, ActionOnExchange) Export
	...
	MessageKey = NStr("en='Data exchange.[ExchangePlanName].Node[NodeCode].[ActionOnExchange]';ru='Обмен данными.[ИмяПланаОбмена].Узел [КодУзла].[ДействиеПриОбмене]';vi='Trao đổi dữ liệu.[ExchangePlanName].Node[NodeCode].[ActionOnExchange]'",
		CommonUseClientServer.MainLanguageCode());
...
Function DataAreaNumberByExchangePlanNodeCode(Val NodeCode) Export
	...
	If TypeOf(NodeCode) <> Type("String") Then
		Raise NStr("en='Invalid type of parameter number [1].';ru='Неправильный тип параметра номер [1].';vi='Sai kiểu tham số số [1].'");
	EndIf;
...
Procedure FillFormData(Form)
	...
	FilterSettingsAtNode = Form.Context.FilterSettingsAtNode;
	
	For Each SettingItem IN FilterSettingsAtNode Do
...
Procedure ValidateRequiredFormAttributes(Form, Val Attributes)
	...
	If MissingAttributes.Count() > 0 Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en='No required attributes of node configuration form: %1';ru='Отсутствуют обязательные реквизиты формы настройки узла: %1';vi='Không có mục tin bắt buộc trong biểu mẫu tùy chỉnh nút: %1'"),
...
"Реквизит ""%1"".';vi='Không hỗ trợ kiểu dữ liệu phức hợp cho giá trị theo mặc định."
"Mục tin ""%1"".'");
...
"Реквизит ""%1"".';vi='Hỗ trợ chọn giá trị theo mặc định chỉ đối với danh mục.'");
				MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString, Attribute.FullName());
				Raise MessageString;
			EndIf;
...
"Для предупреждения возможного возникновения ошибок при работе с программой рекомендуется актуализировать правила обмена из файла.';vi='Đối với sơ đồ trao đổi %1 sử dụng quy tắc trao đổi kết nhập từ tệp."
"Các quy tắc này có thể không tương thích với phiên bản mới của chương trình."
"Để cảnh báo lỗi có thể xuất hiện khi làm việc với chương trình nên thiết thực hóa quy tắc trao đổi từ tệp.'",
				CommonUseClientServer.MainLanguageCode());
...
Procedure CheckConnectionOfExchangeMessagesTransportDataProcessor(Cancel, SettingsStructure, TransportKind, ErrorInfo = "") Export
	...
	If Not DataProcessorObject.ConnectionIsDone() Then
		...
		AdditionalMessage = NStr("en='See technical information on the error in the event log.';ru='Техническую информацию об ошибке см. в журнале регистрации.';vi='Xem thông tin kỹ thuật về lỗi trong nhật ký sự kiện.'");
		...
		WriteLogEvent(NStr("en='Exchange message transport';ru='Транспорт сообщений обмена';vi='Truyền thông báo trao đổi'", CommonUseClientServer.MainLanguageCode()),
...
Function InstallOuterDatabaseJoin(SettingsStructure) Export
	...
	If NoFullRights Then
		Result.DetailedErrorDescription = NStr("en='User specified for connection to another application should have been assigned roles ""System administrator"" and ""Full rights""';ru='Пользователю, указанному для подключения к другой программе, должны быть назначены роли ""Администратор системы"" и ""Полные права""';vi='Người sử dụng mà đã chỉ ra để kết nối với chương trình khác cần được gán vai trò ""Quản trị hệ thống"" và ""Toàn quyền""'");
	...
	Else
		...
		If StateUnacceptable Then
			Result.DetailedErrorDescription = NStr("en='Another application is being updated.';ru='Другая программа находится в состоянии обновления.';vi='Chương trình khác đang có trạng thái cập nhật.'");
			Result.ErrorShortInfo   = Result.DetailedErrorDescription;
			Result.Join = Undefined;
		EndIf;
...
Function IsConnectionToCorrespondent(Val Correspondent, Val SettingsStructure, UserMessage = "") Export
	...
	EventLogMonitorEvent = NStr("en='Data exchange.Connection check';ru='Обмен данными.Проверка подключения';vi='Trao đổi dữ liệu. Kiểm tra kết nối'", CommonUseClientServer.MainLanguageCode());
	...
	For Each ExchangePlanName IN DataExchangeReUse.SSLExchangePlans() Do
		...
		If ImportedFromFileExchangeRules.Find(ExchangePlanName) = Undefined
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Updating data conversion rules for exchange plan %1';ru='Выполняется обновление правил конвертации данных для плана обмена %1';vi='Đang thực hiện cập nhật quy tắc chuyển đổi dữ liệu đối với sơ đồ trao đổi %1'"), ExchangePlanName);
			WriteLogEvent(EventLogMonitorMessageTextDataExchange(),
		...
		If ImportedFromFileRecordRules.Find(ExchangePlanName) = Undefined
			...
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Updating data registration rules for exchange plan %1';ru='Выполняется обновление правил регистрации данных для плана обмена %1';vi='Đang thực hiện cập nhật quy tắc ghi nhận dữ liệu đối với sơ đồ trao đổi %1'"), ExchangePlanName);
			WriteLogEvent(EventLogMonitorMessageTextDataExchange(),
				EventLogLevel.Information,,, MessageText);
	...
	If File.Exist() AND File.IsFile() Then
		...
		If MaximumValidMessageSize <> 0 Then
			...
			If PackageSize > MaximumValidMessageSize Then
				...
				MessageString = NStr("en='Outgoing package size is %1 KB and exceeds allowable limit of %2 KB.';ru='Размер исходящего пакета составил %1 Кбайт, что превышает допустимое ограничение %2 Кбайт.';vi='Dung lượng gói đi là %1 Kbyte, vượt quá giới hạn cho phép %2 Kbyte.'");
	...
	If GetFunctionalOption("UseDataSynchronization") = True Then
		...
		If InfobaseNode <> Undefined Then
			...
			Try
				...
				If Cancel Then
					Raise NStr("en='Receiving data from the main node is completed with errors.';ru='Получение данных из главного узла завершилось с ошибками.';vi='Đã kết thúc nhận dữ liệu từ nút chính bị lỗi.'");
				EndIf;
			Except
				SetPrivilegedMode(True);
				SetDataExchangeMessageImportModeBeforeStart("ImportAllowed", False);
...
"Синхронизация отменена.';vi='Đã tìm thấy việc quay lại cấu hình cơ sở dữ liệu."
"Đã hủy đồng bộ hóa.'"));
				Return;
			EndIf;
		EndIf;
...
Function FilterSettingsAtNode(Val ExchangePlanName, Val CorrespondentVersion, FormName = "", SettingID = "") Export
	...
	Result = ExchangePlans[ExchangePlanName].FilterSettingsAtNode(CorrespondentVersion, FormName, SettingID);
...
Function CorrespondentInfobaseDataTransferRestrictionDetails(Val ExchangePlanName, Val FilterSettingsAtNode, Val CorrespondentVersion, SettingID = "") Export
	
	If IsBlankString(CorrespondentVersion) Then
		CorrespondentVersion = "0.0.0.0";
	EndIf;
	
	Return ExchangePlans[ExchangePlanName].CorrespondentInfobaseDataTransferRestrictionDetails(FilterSettingsAtNode, CorrespondentVersion, SettingID);
...
Procedure RunControlRecordsUndividedData(Val Data) Export
	...
	If CommonUseReUse.DataSeparationEnabled()
		...
		ExceptionsRepresentation = NStr("en='Access right violation';ru='Нарушение прав доступа!';vi='Vi phạm quyền truy cập!'");
		ErrorMessage = NStr("en='Access right violation';ru='Нарушение прав доступа!';vi='Vi phạm quyền truy cập!'", CommonUseClientServer.MainLanguageCode());
...
Function ValuesSettingsSelectionsOnSite(Val InfobaseNode, Val CorrespondentVersion)
	...
	FilterSettingsAtNode = FilterSettingsAtNode(ExchangePlanName, CorrespondentVersion, , SavedExchangePlanNodeSettingsVariant(InfobaseNode));
	
	For Each Setting IN FilterSettingsAtNode Do
...
Procedure RunInitOfExchangeSettingsStructure(ExchangeSettingsStructure, ExchangeExecutionSettings, LineNumber)
	...
	ExchangeSettingsStructure.EventLogMonitorMessageKey = NStr("en='Data exchange description';ru='Обмен данными описание';vi='Trao đổi dữ liệu mô tả'");
...
Procedure RunCheckOfMainFieldsOfExchangeSettingsStructure(ExchangeSettingsStructure)
	...
	If Not ValueIsFilled(ExchangeSettingsStructure.InfobaseNode) Then
		...
		ErrorMessageString = NStr("en='Infobase node with which information shall be exchanged is not specified. Exchange is canceled.';ru='Не задан узел информационной базы с которым нужно производить обмен информацией. Обмен отменен.';vi='Chưa chỉ ra nút cơ sở thông tin mà cần thực hiện trao đổi thông tin. Trao đổi bị hủy bỏ.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
		SetExchangeInitEnd(ExchangeSettingsStructure);
	...
	ElsIf Not ValueIsFilled(ExchangeSettingsStructure.ExchangeTransportKind) Then
		...
		ErrorMessageString = NStr("en='Exchange transport kind is not specified. Exchange is canceled.';ru='Не задан вид транспорта обмена. Обмен отменен.';vi='Chưa chỉ ra dạng truyền trao đổi. Trao đổi bị hủy bỏ.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf Not ValueIsFilled(ExchangeSettingsStructure.ActionOnExchange) Then
		...
		ErrorMessageString = NStr("en='Executed action (export/ import) is not specified. Exchange is canceled.';ru='Не указано выполняемое действие (выгрузка / загрузка). Обмен отменен.';vi='Chưa chỉ ra thao tác đã thực hiện (kết xuất / kết nhập). Đã hủy bỏ trao đổi.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
...
Procedure RunExchangeStructureValidityCheck(ExchangeSettingsStructure, UseTransportSettings = True)
	...
	ElsIf ExchangeSettingsStructure.InfobaseNode.DeletionMark Then
		...
		ErrorMessageString = NStr("en='Infobase node is marked for deletion. Exchange is canceled.';ru='Узел информационной базы помечен на удаление. Обмен отменен.';vi='Nút cơ sở thông tin đặt dấu xóa. Trao đổi bị hủy bỏ.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf ExchangeSettingsStructure.InfobaseNode = ExchangeSettingsStructure.CurrentExchangePlanNode Then
		...
		ErrorMessageString = NStr("en='Unable to properly communicate with the current infobase node. The exchange has been canceled.';ru='Нельзя организовать обмен данными с текущим узлом информационной базы. Обмен отменен.';vi='Không thể tổ chức trao đổi dữ liệu với nút cơ sở thông tin này. Trao đổi bị hủy bỏ.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf IsBlankString(ExchangeSettingsStructure.InfobaseNodeCode)
		...
		ErrorMessageString = NStr("en='One of exchange nodes has an empty code. Exchange is canceled.';ru='Один из узлов обмена имеет пустой код. Обмен отменен.';vi='Một trong số các nút trao đổi có mã rỗng. Trao đổi bị hủy bỏ.'",
			CommonUseClientServer.MainLanguageCode());
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf ExchangeSettingsStructure.DebuggingExportHandlers Then
		...
		If Not ExportDataProcessorFile.Exist() Then
			...
			ErrorMessageString = NStr("en='External data processor file for export debugging does not exist. Exchange is canceled.';ru='Файл внешней обработки для отладки выгрузки не существует. Обмен отменен.';vi='Không tồn tại tệp của bộ xử lý ngoài để gỡ rối kết xuất. Hủy bỏ trao đổi.'",
				CommonUseClientServer.MainLanguageCode());
			WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
	...
	ElsIf ExchangeSettingsStructure.DebuggingImportHandlers Then
		...
		If Not ImportDataProcessorFile.Exist() Then
			...
			ErrorMessageString = NStr("en='External data processor file for import debugging does not exist. Exchange is canceled.';ru='Файл внешней обработки для отладки загрузки не существует. Обмен отменен.';vi='Không tồn tại tệp của bộ xử lý ngoài để gỡ rối kết nhập. Hủy bỏ trao đổi.'",
				CommonUseClientServer.MainLanguageCode());
			WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
...
Procedure SetExchangeRulesOfDataDump(DataExchangeXMLDataProcessor, ExchangeSettingsStructure)
	...
	If ObjectConversionRules = Undefined Then
		...
		// Exchange rules should be specified.
		NString = NStr("en='Conversion rules are not specified for exchange plan %1. Data export is canceled.';ru='Не заданы правила конвертации для плана обмена %1. Выгрузка данных отменена.';vi='Chưa chỉ ra quy tắc chuyển đổi đối với sơ đồ trao đổi %1. Kết xuất dữ liệu hủy bỏ.'",
			CommonUseClientServer.MainLanguageCode());
...
Procedure SetDataImportExchangeRules(DataExchangeXMLDataProcessor, ExchangeSettingsStructure)
	...
	If ObjectConversionRules = Undefined Then
		...
		// Exchange rules should be specified.
		NString = NStr("en='Conversion rules are not specified for exchange plan %1. Data import is canceled.';ru='Не заданы правила конвертации для плана обмена %1. Загрузка данных отменена.';vi='Chưa đặt quy tắc chuyển đổi đối với hệ thống trao đổi %1. Đã hủy kết nhập dữ liệu.'",
			CommonUseClientServer.MainLanguageCode());
		ErrorMessageString = StringFunctionsClientServer.SubstituteParametersInString(NString, ExchangeSettingsStructure.ExchangePlanName);
		WriteLogEventOfDataExchange(ErrorMessageString, ExchangeSettingsStructure, True);
		SetExchangeInitEnd(ExchangeSettingsStructure);
...
Procedure RegisterErrorDocument(Object, ExchangeNode, ErrorMessage) Export
	...
	If Not IsBlankString(TrimAll(MessageText)) Then
		...
		CauseErrors = NStr("en=' Due to %1.';ru=' По причине %1.';vi=' Nguyên nhân %1.'");
		CauseErrors = StringFunctionsClientServer.SubstituteParametersInString(CauseErrors, MessageText);
		
	EndIf;
	
	MessageString = NStr("en='Cannot post document %1 received from another infobase.%2 Maybe not all required attributes are filled in.';ru='Не удалось провести документ %1, полученный из другой информационной базы.%2 Возможно не заполнены все реквизиты, обязательные к заполнению.';vi='Không thể kết chuyển chứng từ %1 đã nhận từ cơ sở thông tin khác.%2 Có thể là chưa điền tất cả các mục tin bắt buộc điền.'",
		CommonUseClientServer.MainLanguageCode());
...
Procedure RegisterErrorRecordsObject(Object, ExchangeNode, ErrorMessage) Export
	...
	MessageString = NStr("en='Cannot write object %1 received from another infobase. %2 Maybe not all required attributes are filled in.';ru='Не удалось записать объект %1, полученный из другой информационной базы.%2 Возможно не заполнены все реквизиты, обязательные к заполнению.';vi='Không thể ghi lại đối tượng %1 đã nhận từ cơ sở thông tin khác. %2 Có thể là chưa điền tất cả các mục tin bắt buộc điền.'",
		CommonUseClientServer.MainLanguageCode());
	MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString, String(Object), CauseErrors);
...
Function SynchronizationDatePresentation(Val SynchronizationDate) Export
	...
	If Not ValueIsFilled(SynchronizationDate) Then
		Return NStr("en='Synchronization has never been performed.';ru='Синхронизация не выполнялась.';vi='Chưa thực hiện đồng bộ hóa.'");
	EndIf;
	
	Return StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Last synchronization: %1';ru='Последняя синхронизация: %1';vi='Đồng bộ hóa gần đây nhất: %1'"),
...
Function RelativeSynchronizationDate(Val SynchronizationDate) Export
	...
	If Not ValueIsFilled(SynchronizationDate) Then
		...
		Return NStr("en='Never';ru='Никогда';vi='Chưa bao giờ'");
	...
	ElsIf Interval < 60 * 5 Then // 5 min
		...
		Result = NStr("en='Now';ru='Сейчас';vi='Hiện tại'");
		
	ElsIf Interval < 60 * 15 Then // 15 min
		
		Result = NStr("en='5 minutes ago';ru='5 минут назад';vi='5 phút trước'");
		
	ElsIf Interval < 60 * 30 Then // 30 min
		
		Result = NStr("en='15 minutes ago';ru='15 минут назад';vi='15 phút trước'");
		
	ElsIf Interval < 60 * 60 * 1 Then // 1 hour
		
		Result = NStr("en='30 minutes ago';ru='30 минут назад';vi='30 phút trước'");
		
	ElsIf Interval < 60 * 60 * 2 Then // 2 hours
		
		Result = NStr("en='1 hour ago';ru='1 час назад';vi='1 giờ trước'");
		
	ElsIf Interval < 60 * 60 * 3 Then // 3 hours
		
		Result = NStr("en='2 hours ago';ru='2 часа назад';vi='2 giờ trước'");
	...
	Else
		...
		If DaysNumberDifference = 0 Then // today
			...
			Result = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Today, %1';ru='Сегодня, %1';vi='Hôm nay, %1'"), Format(SynchronizationDate, "DLF=T"));
			
		ElsIf DaysNumberDifference = 1 Then // yesterday
			
			Result = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Yesterday, %1';ru='Вчера, %1';vi='Hôm qua, %1'"), Format(SynchronizationDate, "DLF=T"));
			
		ElsIf DaysNumberDifference = 2 Then // day before yesterday
			
			Result = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Day before yesterday, %1';ru='Позавчера, %1';vi='Ngày hôm kia, %1'"), Format(SynchronizationDate, "DLF=T"));
...
Function EventLogMonitorMessageTextEstablishingConnectionToWebService() Export
	...
	Return NStr("en='Data exchange.Connecting to web service';ru='Обмен данными.Установка подключения к web-сервису';vi='Trao đổi dữ liệu.Thiết lập kết nối với Web-service'", CommonUseClientServer.MainLanguageCode());
...
Function DataExchangeRuleImportingEventLogMonitorMessageText() Export
	...
	Return NStr("en='Data exchange.Rule import';ru='Обмен данными.Загрузка правил';vi='Trao đổi dữ liệu.Kết nhập quy tắc'", CommonUseClientServer.MainLanguageCode());
...
Function DataExchangeCreationEventLogMonitorMessageText() Export
	...
	Return NStr("en='Data exchang.Creating data exchange';ru='Обмен данными.Создание обмена данными';vi='Trao đổi dữ liệu.Tạo trao đổi dữ liệu'", CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorMessageTextRemovingTemporaryFile() Export
	...
	Return NStr("en='Data exchange.Removing temporary file';ru='Обмен данными.Удаление временного файла';vi='Trao đổi dữ liệu. Xóa bỏ tệp tạm thời'", CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorMessageTextDataExchange() Export
	...
	Return NStr("en='Data exchange description';ru='Обмен данными описание';vi='Trao đổi dữ liệu mô tả'", CommonUseClientServer.MainLanguageCode());
...
Procedure SetDataExchangeMessageFromMainNode(ExchangeMessage, MasterNode) Export
	...
	WriteEventGetData(MasterNode, NStr("en='The exchange message was written to cache.';ru='Сообщение обмена записано в кэш.';vi='Thông điệp trao đổi đã được ghi lại trong cache.'"));
...
Procedure ClearDataExchangeMessageFromMainNode() Export
	...
	WriteEventGetData(MasterNode(), NStr("en='The exchange message was deleted from cache.';ru='Сообщение обмена удалено из кэша.';vi='Đã xóa thông báo trao đổi từ cache.'"));
...
Function ExternalModuleContainerDictionary() Export
	...
	Result.Insert("Nominative", NStr("en='Configure data synchronization';ru='Настройка синхронизация данных';vi='Tùy chỉnh đồng bộ hóa dữ liệu'"));
	Result.Insert("Genitive", NStr("en='Data synchronization settings';ru='Настройки синхронизации данных';vi='Tùy chỉnh đồng bộ hóa dữ liệu'"));
...
Function InteractiveExportChange(Val InfobaseNode, Val AddressOfFormStore, Val IsNodeScript=Undefined) Export
	...
	ScriptParametersAdditions.VariantNoneAdds.Insert("Explanation", 
		NStr("en='Only data according to general settings will be sent.';ru='Будут отправлены только данные согласно общим настройкам.';vi='Sẽ gửi chỉ dữ liệu theo tùy chỉnh chung.'")
	); 
	...
	ScriptParametersAdditions.VariantAllDocuments.Insert("Explanation",
		NStr("en='All the period documents which satisfy the filter conditions will be sent additionally.';ru='Дополнительно будут отправлены все документы за период, удовлетворяющие условиям отбора.';vi='Sẽ gửi thêm tất cả chứng từ trong kỳ thỏa mãn điều kiện lọc.'")
	); 
	...
	ScriptParametersAdditions.VariantArbitraryFilter.Insert("Explanation",
		NStr("en='Data will be sent additionally according to the filter.';ru='Дополнительно будут отправлены данные согласно отбору.';vi='Sẽ gửi thêm dữ liệu theo lọc.'")
	); 
	...
	ScriptParametersAdditions.VariantAdditionally.Insert("Explanation",
		NStr("en='Additional data on settings will be sent.';ru='Будут отправлены дополнительные данные по настройкам.';vi='Sẽ gửi thông tin bổ sung theo tùy chỉnh.'")
	); 
...
Function InteractiveUpdateExportingsResetSettings(ExportAddition, Val SettingRepresentation) Export
	...
	If Result Then
		FillPropertyValues(ExportAddition, ProcessingOfAddition, "ExportVariant, AllDocumentsFilterPeriod, AddressLinkerAllDocuments");
...
Procedure InteractiveUpdateExportingsAttributeBySettings(Form, Val SettingsAdditionsExportings, Val AdditionAttributeName="ExportAddition") Export
	...
	TableAttributePath = AdditionsAttribute.Name + ".AdditionalRegistrationScriptSite";
	If Form.GetAttributes(TableAttributePath).Count()=0 Then
		Adding.Clear();
...
Function FilterPresentationExportAddition(Val Period, Val Filter, Val DetailsOfEmptySelection=Undefined) Export
	...
	If IsBlankString(SelectionString) Then
		If DetailsOfEmptySelection=Undefined Then
			SelectionString = NStr("en='All objects';ru='Все объекты';vi='Tất cả đối tượng'");
		Else
...
Function PresentationDetailedAdditionOfExport(Val AdditionalRegistration, Val DetailsOfEmptySelection=Undefined) Export
	...
	ElsIf DetailsOfEmptySelection=Undefined Then
		Return NStr("en='Additional data is not selected';ru='Дополнительные данные не выбраны';vi='Chưa chọn dữ liệu bổ sung'");
...
Function ExportAdditionNameAutoSaveSettings() Export
	Return NStr("en='Last sent (saved automatically)';ru='Последняя отправка (сохраняется автоматически)';vi='Gửi lần cuối (tự động lưu lại)'");
EndFunction
...
Procedure InteractiveRegisterAdditionalExportingsDataUpdate(Val ExportAddition) Export
	...
	ObjectOfReport = DataProcessors.InteractiveExportChange.Create();
	FillPropertyValues(ObjectOfReport, ExportAddition,,"AdditionalRegistration, AdditionalRegistrationScriptSite");
...
Procedure OnFillingToDoListForSynchronizationWarning(CurrentWorks)
	...
	For Each Section IN Sections Do
		...
		Work.Presentation  = NStr("en='Synchronization warnings';ru='Предупреждения при синхронизации';vi='Cảnh báo khi đồng bộ hóa'");
...
Procedure OnFillingToDoListUpdateRequired(CurrentWorks)
	...
	For Each Section IN Sections Do
		...
		Work.Important         = True;
		Work.Presentation  = NStr("en='Update application version';ru='Обновить версию программы';vi='Cập nhật phiên bản chương trình'");
		...
		Else
			...
			Work.FormParameters = New Structure("Title,TemplateName",
				NStr("en='Install update';ru='Установка обновления';vi='Cài đặt bản cập nhật'"), "InstructionHowToInstallUpdateManually");
		EndIf;
		Work.Owner       = Section;
...
Procedure OnFillCurrensTodosListCheckCompatibilityWithCurrentVersion(CurrentWorks)
	...
	Work.Presentation = NStr("en='Exchange rules';ru='Правила обмена';vi='Quy tắc trao đổi'");
	...
	If ToDosGroup = Undefined Then
		...
		ToDosGroup.Presentation = NStr("en='Check compatibility';ru='Проверить совместимость';vi='Kiểm tra việc tương thích'");
