...
Procedure SetDataAreaSessionLock(Parameters, Val LocalTime = True, Val DataArea = -1) Export
	...
	If CommonUseReUse.CanUseSeparatedData() Then
		...
		ElsIf DataArea <> CommonUse.SessionSeparatorValue() Then
			Raise NStr("en='Running a session with used separator values, you cannot set a session lock for a data area that is different from the one used in the session.';ru='Из сеанса с используемыми значениями разделителей нельзя установить блокировку сеансов области данных, отличной от используемой в сеансе!';vi='Từ phiên làm việc với giá trị chia tách đang sử dụng không nên đặt phong tỏa phiên làm việc của vùng dữ liệu khác với vùng dữ liệu đang sử dụng trong phiên làm việc!'");
		EndIf;
		
	Else
		
		If DataArea = -1 Then
			Raise NStr("en='Cannot set lock for data area sessions as data area was not specified.';ru='Невозможно установить блокировку сеансов области данных - не указана область данных!';vi='Không thể đặt phong tỏa phiên làm việc của vùng dữ liệu - chưa chỉ ra vùng dữ liệu!'");
		EndIf;
...
Procedure OnAddParametersJobsClientLogicStandardSubsystemsRunning(Parameters) Export
	...
	If ValueIsFilled(CurrentMode.End) Then
		LockPeriod = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='for the period from %1 to %2';ru='на период с %1 по %2';vi='cho kỳ từ %1 đến %2'"),
			CurrentMode.Begin, CurrentMode.End);
	Else
		LockPeriod = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='from %1';ru='с %1';vi='từ %1'"), CurrentMode.Begin);
	EndIf;
	If ValueIsFilled(CurrentMode.Message) Then
		LockReason = NStr("en='due to:';ru='по причине:';vi='theo nguyên nhân:'") + Chars.LF + CurrentMode.Message;
	Else
		LockReason = NStr("en='to conduct scheduled operations';ru='для проведения регламентных работ';vi='để tiến hành công việc thường kỳ'");
	EndIf;
...
""
"Приложение временно недоступно.';vi='Người quản trị ứng dụng đặt phong tỏa công việc của người sử dụng %1 %2."
""
"Ứng dụng tạm thời không sử dụng được.'"),
...
""
"Войти в заблокированное приложение?';vi='Người quản trị ứng dụng đặt phong tỏa công việc của người sử dụng %1 %2."
""
"Đăng nhập ứng dụng đã bị phong tỏa?'"),
			LockPeriod, LockReason);
	EndIf;
...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	If LockParameters.Use Then
		If CurrentSessionDate < LockParameters.Begin Then
			If LockParameters.End <> Date(1, 1, 1) Then
				Message = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Scheduled from %1 to %2';ru='Запланирована с %1 по %2';vi='Dự tính từ %1 đến %2'"), 
					Format(LockParameters.Begin, "DLF=DT"), Format(LockParameters.End, "DLF=DT"));
			Else
				Message = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Scheduled from %1';ru='Запланирована с %1';vi='Dự tính từ %1'"), 
		...
		ElsIf LockParameters.End <> Date(1, 1, 1) AND CurrentSessionDate > LockParameters.End AND LockParameters.Begin <> Date(1, 1, 1) Then
			Importance = False;
			Message = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Not valid (expired %1)';ru='Не действует (истек срок %1)';vi='Không có hiệu lực (đã quá hạn %1)'"), 
				Format(LockParameters.End, "DLF=DT"));
		Else
			If LockParameters.End <> Date(1, 1, 1) Then
				Message = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='from %1 to %2';ru='с %1 по %2';vi='từ %1 đến %2'"), 
					Format(LockParameters.Begin, "DLF=DT"), Format(LockParameters.End, "DLF=DT"));
			Else
				Message = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='from %1';ru='с %1';vi='từ %1'"), 
					Format(LockParameters.Begin, "DLF=DT"));
			EndIf;
			Importance = True;
		EndIf;
	Else
		Message = NStr("en='Invalid';ru='Ошибочный';vi='Lỗi'");
		Importance = False;
	EndIf;
	...
	For Each Section IN Sections Do
		...
		Work.Presentation  = NStr("en='User operation locking';ru='Блокировка работы пользователей';vi='Phong tỏa công việc người sử dụng'");
...
"http://<веб-адрес сервера>/?C=РазрешитьРаботуПользователей';vi='%1"
"Để cho phép công việc của người sử dụng có thể mở ứng dụng có tham số РазрешитьРаботуПользователей. Ví dụ:"
...
"""1С:Предприятие"" с параметрами: ENTERPRISE %2 /CРазрешитьРаботуПользователей /UC%3';vi='%1"
"Để cho phép người sử dụng làm việc, hãy sử dụng bảng điều khiển cụm Server hoặc khởi động ""1C:DOANH NGHIỆP"" với tham số: ENTERPRISE %2 /CРазрешитьРаботуПользователей /UC%3'");
	EndIf;
	MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessageText,
		...
		NStr("en='<permission code>';ru='<код разрешения>';vi='<mã cho phép>'"));
...
Function EnabledSessionsMessage() Export
	...
	Message = NStr("en='Cannot disable sessions:';ru='Не удалось отключить сеансы:';vi='Không thể ngắt phiên làm việc:'");
...
"могут"
"быть завершены принудительно: %1 %2';vi='Đang có các phiên làm việc với chương trình, mà"
"không thể"
"buộc kết thúc: %1 %2'"),
