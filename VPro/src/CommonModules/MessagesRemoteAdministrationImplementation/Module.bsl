...
Procedure PrepareDataArea(Val DataAreaCode, Val FromExporting, Val Variant, Val ExportID) Export
	...
	Try
		If Not ValueIsFilled(Constants.InfobaseUsageMode.Get()) Then
			MessageText = NStr("en='Infobase operation mode is not set';ru='Не установлен режим работы информационной базы';vi='Chưa thiết lập chế độ làm việc của cơ sở thông tin'");
			Raise(MessageText);
		EndIf;
		...
		If RecordManager.Selected() Then
			If RecordManager.Status = Enums.DataAreaStatuses.Deleted Then
				MessagePattern = NStr("en='Data area %1 is removed';ru='Область данных %1 удалена';vi='Vùng dữ liệu %1 đã bị xóa'");
			...
			ElsIf RecordManager.Status = Enums.DataAreaStatuses.ToDelete Then
				MessagePattern = NStr("en='Data area %1 is being removed';ru='Область данных %1 в процессе удаления';vi='Vùng dữ liệu %1 trong quá trình xóa'");
				MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, DataAreaCode);
				Raise(MessageText);
			ElsIf RecordManager.Status = Enums.DataAreaStatuses.New Then
				MessagePattern = NStr("en='Data area %1 is being prepared for use';ru='Область данных %1 в процессе подготовки к использованию';vi='Vùng dữ liệu %1 trong quá trình chuẩn bị sử dụng'");
				MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, DataAreaCode);
				Raise(MessageText);
			ElsIf RecordManager.Status = Enums.DataAreaStatuses.Used Then
				MessagePattern = NStr("en='Data area %1 is being used.';ru='Область данных %1 используется.';vi='Vùng dữ liệu %1 đang sử dụng.'");
				MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, DataAreaCode);
				Raise(MessageText);
			EndIf;
		EndIf;
...
Procedure DeleteDataArea(Val DataAreaCode) Export
	...
	Try
		...
		If Not RecordManager.Selected() Then
			MessagePattern = NStr("en='Data area %1 does not exist.';ru='Область данных %1 не существует.';vi='Vùng dữ liệu %1 không tồn tại.'");
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, DataAreaCode);
			Raise(MessageText);
		EndIf;
...
Procedure SetInfobaseParameters(Parameters) Export
	...
	Try
		...
		For Each KeyAndValue IN Parameters Do
			...
			If CurParameterString = Undefined Then
				MessagePattern = NStr("en='Unknown name of parameter %1';ru='Не известное имя параметра %1';vi='Không xác định tên tham số %1'");
				MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, KeyAndValue.Key);
				WriteLogEvent(NStr("en='RemoteAdministration.SetInfobaseParameters';ru='УдаленноеАдминистрирование.УстановитьПараметрыИБ';vi='RemoteAdministration.SetInfobaseParameters'", 
					CommonUseClientServer.MainLanguageCode()),
					EventLogLevel.Warning, , , MessageText);
				Continue;
			ElsIf CurParameterString.WriteProhibition Then
				MessagePattern = NStr("en='Parameter %1 is read only';ru='Параметр %1 может использоваться только для чтения';vi='Tham số %1 có thể chỉ được sử dụng để đọc'");
				MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, KeyAndValue.Key);
				Raise(MessageText);
			EndIf;
	...
	Except
		RollbackTransaction();
		WriteLogEvent(NStr("en='IB parameter setting';ru='Установка параметров ИБ';vi='Đặt tham số CSTT'", 
			CommonUseClientServer.MainLanguageCode()), 
...
Function GetLanguageByCode(Val LanguageCode)
	...
	If ValueIsFilled(LanguageCode) Then
		...
		MessagePattern = NStr("en='Unsupported language code: %1';ru='Неподдерживаемый код языка: %1';vi='Mã ngôn ngữ không được hỗ trợ: %1'");
...
"электронной почты: %1 Ошибка: %2';vi='Đã chỉ ra sai địa chỉ"
"E-mail: %1 Lỗi: %2'");
...
Function GetAreaUserByServiceUserID(Val ServiceUserID)
	...
	If Result.IsEmpty() Then
		MessagePattern = NStr("en='User with user identifier of service %1 is not found';ru='Не найден пользователь с идентификатором пользователя сервиса %1';vi='Không tìm thấy người sử dụng có tên người sử dụng dịch vụ %1'");
...
Function GetInfobaseUserByDataAreaUser(Val DataAreaUser)
	...
	If IBUser = Undefined Then
		MessagePattern = NStr("en='The infobase user does not exist for the data area user with identifier %1';ru='Для пользователя области данных с идентификатором %1 не существует пользователя информационной базы';vi='Đối với người sử dụng vùng dữ liệu có tên %1 không tồn tại người sử dụng cơ sở thông tin'");
