...
Function ValidateAbilityToMove(Refs, List, RepresentedAsList)
	...
	If Not AccessParameters.Accessibility Then
		Return NStr("en='Insufficient rights to change item order.';ru='Недостаточно прав для изменения порядка элементов.';vi='Không đủ quyền để thay đổi trình tự phần tử.'");
	EndIf;
	
	For Each GroupItem IN List.SettingsComposer.GetSettings().Structure Do
		If GroupItem.Use Then
			Return NStr("en='To change the item sequence, disable all groups.';ru='Для изменения порядка элементов необходимо отключить все группировки.';vi='Để thay đổi trình tự phần tử cần ngắt bỏ tất cả gom nhóm.'");
		EndIf;
	EndDo;
	...
	If Information.HasParent AND RepresentedAsList AND Not ListContainsFilterOnParent(List) Then
		Return NStr("en='To change the item sequence, set view mode ""Tree"" or ""Hierarchical list"".';ru='Для изменения порядка элементов необходимо установить режим просмотра ""Дерево"" или ""Иерархический список"".';vi='Để thay đổi trình tự phần tử cần đặt chế độ xem ""Cây"" hoặc ""Danh sách phân nhánh"".'");
	EndIf;
	
	// For subordinated catalogs owner filter shall be set.
	If Information.HasOwner AND Not ListContainsFilterByOwner(List) Then
		Return NStr("en='To change the item sequence, set filter by field ""Owner"".';ru='Для изменения порядка элементов необходимо установить отбор по полю ""Владелец"".';vi='Để thay đổi trình tự phần tử cần đặt phễu lọc theo trường ""Đối tượng chủ"".'");
	EndIf;
	...
	If Information.HasFolders Then
		...
		If IsFolder AND Not Information.ForFolders Or Not IsFolder AND Not Information.ForItems Then
			Return NStr("en='The selected item cannot be transferred.';ru='Выбранный элемент нельзя перемещать.';vi='Không nên di chuyển phần tử đã chọn.'");
		EndIf;
	EndIf;
