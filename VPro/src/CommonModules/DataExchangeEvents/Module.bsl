...
Procedure AddRecipients(Object, Nodes) Export
	...
	For Each Item IN Nodes Do
		...
		Except
			...
			MessageString = NStr("en='Registration of object [FullName] is not specified for exchange plan content [ExchangePlanName]';ru='Для состава плана обмена [ИмяПланаОбмена] не указана регистрация объекта [ПолноеИмя]';vi='Đối với thành phần sơ đồ trao đổi [ExchangePlanName] chưa chỉ ra việc ghi nhận đối tượng [FullName]'");
...
Procedure RecordChangesData(Val Recipient, Val Data, Val CheckPermissionOfUplouding=True) Export
	...
	Else
		...
		If ObjectExportMode = Enums.ExchangeObjectsExportModes.ExportIfNecessary Then
			...
			If IsNewObject Then
				Raise NStr("en='Registration of unwritten objects that can be exported by reference is not supported.';ru='Регистрация незаписанных объектов, выгружаемых по ссылке, не поддерживается.';vi='Không hỗ trợ ghi nhận các đối tượng chưa được ghi lại mà được kết xuất theo tham chiếu.'");
			EndIf;
...
Procedure RegisterObjectChange(ExchangePlanName, Object, Cancel, AdditionalParameters = Undefined)
		...
		If CommonUseReUse.DataSeparationEnabled() Then
			...
			Else
				...
				If SeparatedData(MetadataObject) Then
					Raise NStr("en='Change registration of separated data in the split mode.';ru='Регистрация изменений разделенных данных в неразделенном режиме.';vi='Việc ghi nhận dữ liệu phân tách ở chế độ chưa phân tách.'");
				EndIf;
	...
	Except
		WriteLogEvent(NStr("en='Data exchange.Objects registration rules';ru='Обмен данными.Правила регистрации объектов';vi='Trao đổi dữ liệu.Quy tắc ghi nhận đối tượng'", CommonUseClientServer.MainLanguageCode()),
...
Function ObjectModifiedForExchangePlan(Source, MetadataObject, ExchangePlanName, WriteMode, RecordObjectChangeToExport)
	...
	Except
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='An error occurred when determining modification of object: %1';ru='Ошибка определения модифицированности объекта: %1';vi='Lỗi xác định thay đổi đối tượng: %1'"),
...
"Описание"
"ошибки: %2';vi='Lỗi thực hiện quy tắc ghi nhận đối tượng đối với sơ đồ trao đổi %1."
"Mô tả"
"lỗi: %2'"),
			ExchangePlanName,
			DetailErrorDescription(ErrorInfo()));
...
Function GetPropertiesValuesForRef(Ref, PropertiesOfObject, Val ObjectPropertiesString, Val MetadataObjectName) Export
	...
	Except
		MessageString = NStr("en='An error occurred when receiving the reference properties. Query execution failed: [ErrorDescription]';ru='Ошибка при получении свойств ссылки. Ошибка выполнения запроса: [ОписаниеОшибки]';vi='Lỗi khi nhận thuộc tính tham chiếu. Lỗi thực hiện truy vấn: [ErrorDescription]'");
...
Function NodesArrayByPropertiesValues(PropertyValues, Val QueryText, Val ExchangePlanName, Val FlagAttributeName, Val Exporting = False) Export
	...
	Else
		...
		If Exporting Then
			QueryText = StrReplace(QueryText, "[FilterByAttributeCheckBoxCondition]",
				"And (ExchangePlanMainTable.[FlagAttributeName] = VALUE(Enum.ExchangeObjectsExportModes.ExportByCondition)
				|OR ExchangePlanMainTable.[FlagAttributeName] = VALUE(Enum.ExchangeObjectsExportModes.ExportManually)
				|OR ExchangePlanMainTable.[FlagAttributeName] = VALUE(Enum.ExchangeObjectsExportModes.EmptyRef))"
			);
		Else
			QueryText = StrReplace(QueryText, "[FilterByAttributeCheckBoxCondition]",
				"AND (ExchangePlanMainTable.[FlagAttributeName] = VALUE(Enum.ExchangeObjectsExportModes.ExportByCondition)
	...
	Except
		MessageString = NStr("en='An error occurred when receiving a list of target nodes. Failed to execute the query: [ErrorDescription]';ru='Ошибка при получении списка узлов получателей. Ошибка выполнения запроса: [ОписаниеОшибки]';vi='Lỗi khi nhận danh sách nút người nhận. Lỗi thực hiện truy vấn: [ErrorDescription]'");
		MessageString = StrReplace(MessageString, "[ErrorDescription]", DetailErrorDescription(ErrorInfo()));
		Raise MessageString;
	EndTry;
...
Function RecordSetByType(MetadataObject)
	...
	Else
		...
		MessageString = NStr("en='The record set is not provided for metadata object %1.';ru='Для объекта метаданных %1 не предусмотрено набора записей.';vi='Đối với đối tượng metadata %1 chưa xem xét tập hợp bản ghi.'");
...
"// {Начало алгоритма} "
"[ЗначениеКонстанты] "
"// {Окончание алгоритма}';vi='Lỗi thuật toán"
"giá trị hằng:"
"Sơ đồ trao đổi: [ExchangePlanName]"
"Đối tượng metadata: [MetadataObjectName]"
"Mô tả lỗi: "
"[Description]"
"Thuật toán: "
"// {Algorithm start} [ConstantValue] // {Algorithm end}'");
...
"План обмена: [ИмяПланаОбмена];"
"Объект метаданных: [ОбъектМетаданныхИмя]"
"Описание ошибки: [Описание]';vi='Lỗi khi thực hiện hàm sự kiện: ""[HandlerName]"";"
"Sơ đồ trao đổi: [ExchangePlanName];"
"Đối tượng metadata: [MetadataObjectName]"
"Mô tả lỗi: [Description]'");
			MessageString = StrReplace(MessageString, "[HandlerName]",      NStr("en='Before processing';ru='Перед обработкой';vi='Trước khi xử lý'"));
			MessageString = StrReplace(MessageString, "[ExchangePlanName]",      ORR.ExchangePlanName);
			MessageString = StrReplace(MessageString, "[MetadataObjectName]", ORR.MetadataObjectName);
...
"Описание ошибки: [Описание]';vi='Lỗi khi thực hiện hàm sự kiện: ""[HandlerName]""; Sơ đồ trao đổi: [ExchangePlanName]; Đối tượng Metadata: [MetadataObjectName]"
"Mô tả lỗi: [Description]'");
			MessageString = StrReplace(MessageString, "[HandlerName]",      NStr("en='When processing';ru='При обработке';vi='Khi xử lý'"));
			MessageString = StrReplace(MessageString, "[ExchangePlanName]",      ORR.ExchangePlanName);
			MessageString = StrReplace(MessageString, "[MetadataObjectName]", ORR.MetadataObjectName);
			MessageString = StrReplace(MessageString, "[Definition]",            DetailErrorDescription(ErrorInfo()));
			Raise MessageString;
...
"Описание ошибки: [Описание]';vi='Lỗi khi thực hiện hàm sự kiện: ""[HandlerName]""; Sơ đồ trao đổi: [ExchangePlanName]; Đối tượng Metadata: [MetadataObjectName]"
"Mô tả lỗi: [Description]'");
			MessageString = StrReplace(MessageString, "[HandlerName]",      NStr("en='When processing (optional)';ru='При обработке (дополнительный)';vi='Khi xử lý (bổ xung)'"));
			MessageString = StrReplace(MessageString, "[ExchangePlanName]",      ORR.ExchangePlanName);
			MessageString = StrReplace(MessageString, "[MetadataObjectName]", ORR.MetadataObjectName);
			MessageString = StrReplace(MessageString, "[Definition]",            DetailErrorDescription(ErrorInfo()));
			Raise MessageString;
...
"Описание ошибки: [Описание]';vi='Lỗi khi thực hiện hàm sự kiện: ""[HandlerName]""; Sơ đồ trao đổi: [ExchangePlanName]; Đối tượng Metadata: [MetadataObjectName]"
"Mô tả lỗi: [Description]'");
			MessageString = StrReplace(MessageString, "[HandlerName]",      NStr("en='After processing';ru='После обработки';vi='Sau khi xử lý'"));
			MessageString = StrReplace(MessageString, "[ExchangePlanName]",      ORR.ExchangePlanName);
			MessageString = StrReplace(MessageString, "[MetadataObjectName]", ORR.MetadataObjectName);
...
Function DataDifferent(Data1, Data2, PropertyList = Undefined, ExcludingProperties = Undefined) Export
	...
	Else
		...
		Raise NStr("en='Invalid value of parameter [1] of method CommonUse.DataDifferent is set.';ru='Задано недопустимое значение параметра [1] метода CommonUse.DataDifferent.';vi='Đã chỉ ra giá trị tham số không hợp lệ [1] phương thức CommonUse.DataDifferent.'");
	...
	If HasCollision Then
		...
		If DataExchangeReUse.UseVersioning(Sender) Then
			...
			If RefExists Then
				...
				If WriteObject Then
					Comment = NStr("en='Previous version (automatic conflict resolution).';ru='Предыдущая версия (автоматическое разрешение конфликта).';vi='Phiên bản trước (tự động giải quyết xung đột).'");
				Else
					Comment = NStr("en='Current version (automatic conflict resolution).';ru='Текущая версия (автоматическое разрешение конфликта).';vi='Phiên bản hiện tại (tự động giải quyết xung đột).'");
				EndIf;
			...
			If WriteObject Then
				...
				InfoAboutObjectVersion.Comment = NStr("en='Accepted version (automatic conflict resolution)';ru='Принятая версия (автоматическое разрешение конфликта).';vi='Phiên bản được tiếp nhận (tự động cho phép xung đột).'");
				
				DataItem.AdditionalProperties.InfoAboutObjectVersion = New FixedStructure(InfoAboutObjectVersion);
				
			Else
				
				Comment = NStr("en='Rejected version (automatic conflict resolution).';ru='Отклоненная версия (автоматическое разрешение конфликта).';vi='Phiên bản không tồn tại (tự động cho phép xung đột).'");
...
Procedure RegisterDataProhibitionImportByDate(DataItem, Sender, ErrorInfo)
	...
	If DataExchangeReUse.UseVersioning(Sender) AND CommonUse.ThisIsObjectOfReferentialType(DataItem.Metadata()) Then
		...
		If CommonUse.RefExists(ObjectReference) Then
			...
			Comment = NStr("en='Version was created during data synchronization.';ru='Версия создана при синхронизации данных.';vi='Phiên bản đã tạo khi đồng bộ hóa dữ liệu.'");
			InfoAboutObjectVersion = New Structure("Comment", Comment);
		...
		Else
			...
			ErrorMessageString = NStr("en='%1 cannot be imported to the prohibited period.%2%2%3';ru='%1 запрещено загружать в запрещенный период.%2%2%3';vi='%1 đã cấm kết nhập vào kỳ cấm.%2%2%3'");
...
"Объект этой информационной базы был заменен версией объекта из второй информационной базы.';vi='Xuất hiện xung đột thay đổi đối tượng."
"Đối tượng của cơ sở thông tin này đã bị thay đổi bởi phiên bản đối tượng từ cơ sở thông tin thứ hai.'");
...
"Объект из второй информационной базы не принят. Объект этой информационной базы не изменен.';vi='Xuất hiện xung đột thay đổi đối tượng."
"Chưa tiếp nhận đối tượng từ cơ sở thông tin thứ hai. Không thay đổi đối tượng của cơ sở thông tin này.'");
...
Function GetDataObjectAsStringBeforeChange(Object, ObjectMetadata, IsReferenceType, RefExists, ObjectInDataBase)
	...
	If IsReferenceType Then
		...
		Else
			...
			ObjectString = NStr("en='Object is removed';ru='Объект удален';vi='Đối tượng đã bị xóa'");
