...
Function ExecuteScheduledJobManually(Val Task) Export
	...
	If LastBackgroundJobProperties <> Undefined
	   AND LastBackgroundJobProperties.State = BackgroundJobState.Active Then
		...
		EndIf;
	Else
		BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Start manually: %1';ru='Запуск вручную: %1';vi='Khởi động thủ công: %1'"), ScheduledJobPresentation(Task));
...
Function TextUndefined() Export
	...
	Return NStr("en='<not determined>';ru='<не определено>';vi='<chưa xác định>'");
...
Procedure CancelBackgroundJob(ID) Export
	...
	Else
		Raise NStr("en='Background job is not found on the server.';ru='Фоновое задание не найдено на сервере.';vi='Nhiệm vụ nền không tìm thấy trên Server.'");
	EndIf;
	
	If BackgroundJob.State <> BackgroundJobState.Active Then
		Raise NStr("en='Cannot complete the job, it cannot be canceled.';ru='Задание не выполняется, его нельзя отменить.';vi='Nhiệm vụ chưa thực hiện, không thể hủy bỏ.'");
	EndIf;
