	...
	If ContactInformationClientServer.IsXMLContactInformation(Text) Then
		...
		If ErrorText = Undefined Then
			...
			ElsIf ExpectedType = EnumSkype And TypeFound <> XDTOFactory.Type(Namespace, "Skype") Then
				ErrorText = NStr("en='Contact information deserialization error. Skype is expected.';ru='Ошибка десериализации контактной информации, ожидается ""skype""';vi='Lỗi sắp xếp lại thông tin liên hệ, đang chờ ""skype"".'");
			EndIf;
		EndIf;
...
Function AddressFillErrors(Val XDTOAddress, Val InformationKind, Val ResultByGroups = False) Export
	...
	If TypeOf(AddressUS) <> Type("XDTODataObject") Or AddressUS.Type() <> XDTOFactory.Type(Namespace, "AddressUS") Then
		...
		If CheckFlags.DomesticAddressOnly Then
			ErrorText = NStr("en='Only domestic addresses are allowed.';ru='Адрес должен быть только локальным.';vi='Địa chỉ cần ở trong nước.'");
	...
	If Not XDTOContactInformationFilled(AddressUS) Then
		...
		If CheckFlags.Mandatory Then
			...
			ErrorText = NStr("en='Address is not filled in.';ru='Адрес не заполнен.';vi='Chưa điền địa chỉ.'");
	...
	If ResultByGroups Then
		...
		ElsIf ErrorsCount > 0 Then
			...
			AllErrors.Add(AddressErrorGroup(ErrorGroupDescription,
				NStr("en='Parts of the address do not correspond to the address classifier:';ru='Части адреса не соответствуют адресному классификатору:';vi='Một phần địa chỉ không tương ứng với bảng phân loại địa chỉ:'")));
...
Function AddressFillErrorsCommonGroups(Val AddressData, Val InformationKind) Export
	...
	Else
		...
		If XTDOContactInformation.Presentation <> Presentation Then
			Result.Add(AddressErrorGroup("PresentationNotMatchingFieldSet",
				NStr("en='The address does not match the field set values.';ru='Адрес не соответствует значениям в наборе полей.';vi='Địa chỉ không tương ứng với giá trị trong tập hợp các trường.'")));
			AddAddressFillError(Result[0], "",
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Address presentation for contact information kind ""%1"" is different from address data.';ru='Представление адреса для вида контактной информации ""%1"" отличается от данных в адресе.';vi='Việc trình bày địa chỉ cho dạng thông tin liên hệ ""%1"" khác với dữ liệu trong địa chỉ. '"),
					String(InformationKind.Description)));
		EndIf;
	EndIf;
	
	MandatoryFieldsNotFilled = AddressErrorGroup("MandatoryFieldsNotFilled",
		NStr("en='Required fields are not entered:';ru='Не заполнены обязательные поля:';vi='Chưa điền các trường bắt buộc:'"));
	Result.Add(MandatoryFieldsNotFilled);
	
	FieldAbbreviationsNotSpecified = AddressErrorGroup("FieldAbbreviationsNotSpecified",
		NStr("en='Abbreviations are not specified for fields:';ru='Не указано сокращение для полей:';vi='Chưa chỉ ra tên viết tắt cho các trường:'"));
	Result.Add(FieldAbbreviationsNotSpecified);
	
	InvalidFieldCharacters = AddressErrorGroup("InvalidFieldCharacters",
		NStr("en='Invalid characters are found in fields:';ru='Найдены недопустимые символы в полях:';vi='Đã tìm thấy các ký tự không được phép trong các trường:'"));
	Result.Add(InvalidFieldCharacters);
	
	FieldLengthsNotMatching = AddressErrorGroup("FieldLengthsNotMatching",
		NStr("en='Field length does not match the predefined value for fields:';ru='Не соответствует установленной длина полей:';vi='Chiều dài các trường không tương ứng với chiều dài quy định:'"));
	...
	If IsBlankString(Index) Then
		AddAddressFillError(MandatoryFieldsNotFilled, ContactInformationClientServerCached.PostalCodeXPath(),
			NStr("en='Zip code is not specified.';ru='Не указан почтовый индекс.';vi='Chưa chỉ ra mã bưu chính.'"), "Index");
	EndIf;
	
	State = AddressUS.Region;
	If IsBlankString(State) Then
		AddAddressFillError(MandatoryFieldsNotFilled, "Region",
			NStr("en='Region is not specified.';ru='Не указан регион.';vi='Chưa chỉ ra tỉnh thành.'"), "State");
	EndIf;
	...
	If SkipBuildingAddressCheck(AddressUS) Then
		...
		If UnitNotSpecified Then
			AddAddressFillError(MandatoryFieldsNotFilled, 
				...
				NStr("en='House or block is not specified.';ru='Не указан дом или корпус';vi='Chưa chỉ ra nhà hoặc dãy nhà'"), 
				NStr("en='Building';ru='Дом';vi='Nhà'")
			);
		EndIf;
	...
	Else
		...
		If BuildingData = Undefined Then
			AddAddressFillError(MandatoryFieldsNotFilled, 
				ContactInformationClientServerCached.AdditionalAddressingObjectNumberXPath("Building"),
				NStr("en='House or estate is not specified.';ru='Не указан дом или владение (домовладение).';vi='Chưa chỉ ra nhà hoặc khu nhà.'"),
				NStr("en='Building';ru='Дом';vi='Nhà'")
			);
		ElsIf IsBlankString(BuildingData.Value) Then
			AddAddressFillError(MandatoryFieldsNotFilled, BuildingData.XPath,
				NStr("en='Value of the house or estate is not entered.';ru='Не заполнено значение дома или владения (домовладения).';vi='Chưa điền giá trị nhà hoặc khu nhà'"),
				NStr("en='Building';ru='Дом';vi='Nhà'")
			);
		EndIf;
	...
	If Not IsBlankString(State) Then
		...
		If IsBlankString(ContactInformationClientServer.Abbr(State)) Then
			AddAddressFillError(FieldAbbreviationsNotSpecified, "Region",
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Abbreviation is not specified in the name of region ""%1"".';ru='Не указано сокращение в названии региона ""%1"".';vi='Chưa chỉ ra đại từ trước tỉnh thành ""%1"".'"), State), NStr("en='Region';ru='Регион';vi='Vùng'"));
		EndIf;
		If StrLen(State) > 50 Then
			AddAddressFillError(FieldLengthsNotMatching, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Name of region ""%1"" should be less than 50 characters.';ru='Название региона ""%1"" должно быть короче 50 символов.';vi='Tên gọi tỉnh thành ""%1"" cần ngắn hơn 50 ký tự.'"), State), NStr("en='Region';ru='Регион';vi='Vùng'"));
		EndIf;
		If Not StringFunctionsClientServer.OnlyLatinInString(State, False, AllowedBesidesLatin) Then
			AddAddressFillError(InvalidFieldCharacters, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='The name of state ""%1"" contains non-Latin characters.';ru='В названии региона ""%1"" есть не латинские символы.';vi='Tên khu vực ""%1"" có chứa các ký tự không phải Latin.'"), State), NStr("en='Region';ru='Регион';vi='Vùng'"));
		EndIf
	EndIf;
	...
	If Not IsBlankString(County) Then
		...
		If IsBlankString(ContactInformationClientServer.Abbr(County)) Then
			AddAddressFillError(FieldAbbreviationsNotSpecified, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("ru = 'Не указано сокращение в названии района ""%1"".'; en = 'Abbreviation is not specified for county ""%1""'."), County), NStr("en='District';ru='Район';vi='Quận'"));
		EndIf;
		If StrLen(County) > 50 Then
			AddAddressFillError(FieldLengthsNotMatching, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Name of district ""%1"" should be less than 50 characters.';ru='Название района ""%1"" должно быть короче 50 символов.';vi='Tên gọi quận huyện ""%1"" cần ngắn hơn 50 ký tự.'"), County), NStr("en='District';ru='Район';vi='Quận'"));
		EndIf;
		If Not StringFunctionsClientServer.OnlyLatinInString(County, False, AllowedBesidesLatin) Then
			AddAddressFillError(InvalidFieldCharacters, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='The name of county ""%1"" contains non-Latin characters.';ru='В названии района ""%1"" есть не латинские символы.';vi='Tên khu vực ""%1"" có chứa các ký tự không phải Latin.'"), County), NStr("en='District';ru='Район';vi='Quận'"));
		EndIf;
	EndIf;
	...
	If Not IsBlankString(City) Then
		...
		If IsBlankString(ContactInformationClientServer.Abbr(City)) Then
			AddAddressFillError(FieldAbbreviationsNotSpecified, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Abbreviation is not specified in the name of city ""%1"".';ru='Не указано сокращение в названии города ""%1"".';vi='Chưa chỉ ra đại từ trước phường xã ""%1"".'"), City), NStr("en='City';ru='Город';vi='Thành phố'"));
		EndIf;
		If StrLen(City) > 50 Then
			AddAddressFillError(FieldLengthsNotMatching, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='City name ""%1"" should be less than 50 characters.';ru='Название города ""%1"" должно быть короче 50 символов.';vi='Tên gọi phường xã ""%1"" cần ngắn hơn 50 ký tự.'"), City), NStr("en='City';ru='Город';vi='Thành phố'"));
		EndIf;
		If Not StringFunctionsClientServer.OnlyLatinInString(City, False, AllowedBesidesLatin) Then
			AddAddressFillError(InvalidFieldCharacters, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='City name ""%1"" contains non-Latin characters.';ru='В названии города ""%1"" есть не кириллические символы.';vi='Trong tên gọi phường xã ""%1"" không phải là chữ cái tiếng Việt.'"), City), NStr("en='City';ru='Город';vi='Thành phố'"));
		EndIf;
	EndIf;
	...
	If Not IsBlankString(Settlement) Then
		...
		If IsBlankString(ContactInformationClientServer.Abbr(Settlement)) Then
			AddAddressFillError(FieldAbbreviationsNotSpecified, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Abbreviation is not specified in the settlement name ""%1"".';ru='Не указано сокращение в названии населенного пункта ""%1"".';vi='Chưa chỉ ra đại từ trước tên gọi khu dân cư ""%1"".'"), Settlement
				), NStr("en='Settlement';ru='Населенный пункт';vi='Khu dân cư'"));
		EndIf;
		If StrLen(Settlement) > 50 Then
			AddAddressFillError(FieldLengthsNotMatching, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Name of settlement ""%1"" should be less than 50 characters.';ru='Название населенного пункта ""%1"" должно быть короче 50 символов.';vi='Tên gọi khu dân cư ""%1"" cần ngắn hơn 50 ký tự.'"), Settlement
				), NStr("en='Settlement';ru='Населенный пункт';vi='Khu dân cư'"));
		EndIf;
		If Not StringFunctionsClientServer.OnlyLatinInString(Settlement, False, AllowedBesidesLatin) Then
			AddAddressFillError(InvalidFieldCharacters, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Settlement name ""%1"" contains non-Latin characters.';ru='В названии населенного пункта ""%1"" есть не кириллические символы.';vi='Trong tên gọi khu dân cư ""%1"" không phải là chữ cái tiếng Việt.'"), Settlement
				), NStr("en='Settlement';ru='Населенный пункт';vi='Khu dân cư'"));
		EndIf;
	EndIf;
	...
	If Not IsBlankString(Street) Then
		...
		If IsBlankString(ContactInformationClientServer.Abbr(Street)) Then
			AddAddressFillError(FieldAbbreviationsNotSpecified, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Abbreviation is not specified in the name of street ""%1"".';ru='Не указано сокращение в названии улицы ""%1"".';vi='Chưa chỉ ra đại từ trước tên gọi đường phố ""%1"".'"), Street
				), NStr("en='Street';ru='Улица';vi='Đường phố'"));
		EndIf;
		If StrLen(County) > 50 Then
			AddAddressFillError(FieldLengthsNotMatching, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Name of street ""%1"" should be less than 50 characters.';ru='Название улицы ""%1"" должно быть короче 50 символов.';vi='Tên gọi đường phố ""%1"" cần ngắn hơn 50 ký tự.'"), Street
				), NStr("en='Street';ru='Улица';vi='Đường phố'"));
		EndIf;
		If Not StringFunctionsClientServer.OnlyLatinInString(Street, False, AllowedBesidesLatin) Then
			AddAddressFillError(InvalidFieldCharacters, Field,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='The name of street ""%1"" contains non-Latin characters.';ru='В названии улицы ""%1"" есть не латинские символы.';vi='Tên đường phố ""%1"" có chứa các ký tự không phải Latin.'"), Street
				), NStr("en='Street';ru='Улица';vi='Đường phố'"));
		EndIf;
	EndIf;
	...
	If Not IsBlankString(Index) Then
		...
		If StrLen(Index) <> 6 Or Not StringFunctionsClientServer.OnlyDigitsInString(Index) Then
			AddAddressFillError(FieldLengthsNotMatching, Field,
				NStr("en='Zip code should contain 6 digits.';ru='Почтовый индекс должен состоять из 6 цифр.';vi='Mã bưu chính cần có 6 chữ số.'"),
				NStr("en='Index';ru='Индекс';vi='Index'")
			);
		EndIf;
	EndIf;
	...
	For Each UnitData In BuildingsUnits.Buildings Do
		If StrLen(UnitData.Value) > 10 Then
			AddAddressFillError(FieldLengthsNotMatching, UnitData.XPath,
				StringFunctionsClientServer.SubstituteParametersInString( 
					NStr("en='Value of field ""%1"" must be shorter than 10 characters.';ru='Значение поля ""%1"" должно быть короче 10 символов.';vi='Giá trị trường ""%1"" cần phải ít hơn 10 ký tự.'"), UnitData.Type
				), UnitData.Type);
		EndIf;
	EndDo;
	...
	If IsBlankString(City) And IsBlankString(Settlement) Then
			AddAddressFillError(MandatoryFieldsNotFilled, NStr("en='City and Settlement fields cannot both be empty';ru='Город или населенный пункт должны быть заполнены';vi='Cần điền Thành phố hoặc Khu dân cư'"),
				NStr("en='Settlement';ru='Населенный пункт';vi='Khu dân cư'")
			);
	EndIf;
	...
	If Not SkipStreetAddressCheck(AddressUS) Then
		...
		If IsBlankString(Settlement) And IsBlankString(Street) Then
			AddAddressFillError(MandatoryFieldsNotFilled, "Street",
				NStr("en='If the settlement is undefined, the street is mandatory.';ru='Если населенный пункт не заполнен, то улица должна быть указана обязательно.';vi='Nếu chưa điền khu dân cư thì bắt buộc phải chỉ ra đường phố.'"), 
				NStr("en='Street';ru='Улица';vi='Đường phố'")
			);
		EndIf;
