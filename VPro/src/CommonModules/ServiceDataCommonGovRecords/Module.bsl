	...
	If Proxy <> Undefined Then
		...
		Except
			...
			ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='TIN %1:';ru='ИНН %1:';vi='Mã số thuế %1:'"), TIN)
...
Function LegalEntitiesDetailsByName(Val Description, Val StateCode = "", Val Address = "") Export
	...
	If Proxy <> Undefined Then
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Name - %1:';ru='Наименование - %1:';vi='Tên gọi - %1:'"), Description)
				+ Chars.LF + DetailErrorDescription(ErrorInfo);
		EndTry;
	EndIf;
...
Procedure FillInPensionFundDetails(XDTODataObject, Attributes)
	...
	If XDTODataObject.PensionFundRegistrationInfo <> Undefined Then
		...
		PFRRegistrationNumber = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1-%2-%3';ru='%1-%2-%3';vi='%1-%2-%3'"), 
		...
		If XDTODataObject.PensionFundRegistrationInfo.PensionFundAgencyInfo <> Undefined Then
			...
			PFRBodyCode = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1-%2';ru='%1-%2';vi='%1-%2'"), 
...
Procedure HandleServiceError(ErrorDescription, AttributesStructure)
	...
	If ErrorDescription = "AuthenticationParametersAreNotSpecified" Then
		...
		ErrorDescription = NStr("en='Login and password for online support are not specified';ru='Не указаны логин и пароль для доступа к интернет-поддержке';vi='Chưa chỉ ra tên đăng nhập và mật khẩu truy cập vào hỗ trợ qua Internet'");
		EventText   = NStr("en='Access error';ru='Ошибка доступа';vi='Lỗi truy cập'", MainLanguageCode);
		
	ElsIf Find(ErrorDescription, """status"":401") > 0 Then
		ErrorText  = NStr("en='Incorrect login and password for online support.';ru='Неверно указаны логин и пароль для доступа к интернет-поддержке';vi='Chỉ sai tên đăng nhập và mật khẩu để truy cập tới hỗ trợ qua Internet'");
		EventText = NStr("en='Access error';ru='Ошибка доступа';vi='Lỗi truy cập'", MainLanguageCode);
		
	ElsIf Find(ErrorDescription, "SERVER-1:") > 0 Then
		ErrorText  = NStr("en='Legal entity TIN is not specified';ru='Не указан ИНН юридического лица';vi='Chưa chỉ ra MST pháp nhân'");
		EventText = NStr("en='Data receiving error';ru='Ошибка получения данных';vi='Lỗi nhận dữ liệu'", MainLanguageCode);
		
	ElsIf Find(ErrorDescription, "SERVER-2:") > 0 Then
		ErrorText  = NStr("en='TIN of a legal entity must contain 10 digits';ru='ИНН юридического лица должен состоять из 10 цифр';vi='Mã số thuế doanh nghiệp cần có 10 chữ số'");
		EventText = NStr("en='Data receiving error';ru='Ошибка получения данных';vi='Lỗi nhận dữ liệu'", MainLanguageCode);
		
	ElsIf Find(ErrorDescription, "SERVER-3:") > 0 Then
		ErrorText  = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Cannot find data to fill in attributes by TIN %1';ru='Не удалось найти данные для заполнения реквизитов по ИНН %1';vi='Không tìm thấy dữ liệu để điền mục tin theo MST %1'"),
			AttributesStructure.TIN);
		EventText = NStr("en='Data receiving error';ru='Ошибка получения данных';vi='Lỗi nhận dữ liệu'", MainLanguageCode);
		
	ElsIf Find(ErrorDescription, "SERVER-4:") > 0 Then
		ErrorText  = NStr("en=""Entrepreneur's TIN is not specified"";ru='Не указан ИНН предпринимателя';vi='Chưa chỉ ra MST cá nhân kinh doanh'");
		EventText = NStr("en='Data receiving error';ru='Ошибка получения данных';vi='Lỗi nhận dữ liệu'", MainLanguageCode);
		
	ElsIf Find(ErrorDescription, "SERVER-5:") > 0 Then
		ErrorText  = NStr("en='TIN of an individual entrepreneur must contain 12 digits';ru='ИНН предпринимателя должен состоять из 12 цифр';vi='MST doanh nghiệp phải có 12 chữ số'");
		EventText = NStr("en='Data receiving error';ru='Ошибка получения данных';vi='Lỗi nhận dữ liệu'", MainLanguageCode);
	...
	ElsIf Find(ErrorDescription, "SERVER-7:") > 0 Then
		ErrorText  = NStr("en='The maximum number of service calls per day has been exceeded';ru='Превышен лимит количества вызовов сервиса за один день';vi='Vượt quá giới hạn số lần gọi Server trong 1 ngày'");
		EventText = NStr("en='Access error';ru='Ошибка доступа';vi='Lỗi truy cập'", MainLanguageCode);
		
	ElsIf Find(ErrorDescription, "SERVER-8:") > 0 Then
		ErrorText  = NStr("en='No valid ITS contract';ru='Отсутствует действующий договор ИТС';vi='Thiếu hợp đồng ITS còn hiệu lực'");
		EventText = NStr("en='Access error';ru='Ошибка доступа';vi='Lỗi truy cập'", MainLanguageCode);
		
	Else
		ErrorText  = NStr("en='An error occurred when operating with the service (for more information, see the event log)';ru='Ошибка при работе с сервисом (подробнее см. Журнал регистрации)';vi='Lỗi khi làm việc với dịch vụ (chi tiết xem trong Nhật ký sự kiện)'");
		EventText = NStr("en='An error occurred when operating with the service';ru='Ошибка при работе с сервисом';vi='Lỗi khi làm việc với dịch vụ'", MainLanguageCode);
	EndIf;
	
	AttributesStructure.ErrorDescription = ErrorText;
	
	EventName = NStr("en='United state_register data service.';ru='Сервис данных единых гос_реестров.';vi='Dịch vụ dữ liệu sổ đăng ký kinh doanh.'", MainLanguageCode) + " " + EventText;
