...
Function GetParameterValue(Object, Document = Undefined, Parameter, PresentationParameter = Undefined) Export
	...
	ElsIf Parameter = Parameters.PositionOfContactPersonOfCounterparty Then
		...
		Else
			If Object.Owner.ContactPerson.Roles.Count() <> 0 Then 
				ParameterValue = ?(Object.Owner.ContactPerson.Roles[0].Role.Description <> "",
							Object.Owner.ContactPerson.Roles[0].Role.Description, "");
			Else
				ParameterValue = "";
			EndIf;
		EndIf;
	...
	ElsIf Parameter = Parameters.ContactPersonOfCounterpartyInitials Then
		...
		Initials	= SmallBusinessServer.GetSurnameNamePatronymic(Surname, NameParts.Name, NameParts.Patronymic, False);
	...
	ElsIf Parameter = Parameters.CounterpartyName Then
	...
	ElsIf Parameter = "PassportData" Then
		...
		Query.Text = 
			...
			|WHERE
			|	LegalDocuments.Owner = &Counterparty
			|	AND LegalDocuments.DocumentKind = &DocumentKind";
		
		Query.SetParameter("DocumentKind", Catalogs.DocumentKinds.Passport);
		...
		PassportData.Insert("ExpiryDate", "");
		PassportData.Insert("Authority", "");
		...
		While SelectionDetailRecords.Next() Do 
			...
			If ValueIsFilled(SelectionDetailRecords.ExpiryDate) Then 
				PassportData.ExpiryDate = Format(SelectionDetailRecords.ExpiryDate, "DF='dd.MM.yyyy'");
			EndIf;
			If ValueIsFilled(SelectionDetailRecords.Authority) Then 
				PassportData.Authority = SelectionDetailRecords.Authority;
			EndIf;
		EndDo;
