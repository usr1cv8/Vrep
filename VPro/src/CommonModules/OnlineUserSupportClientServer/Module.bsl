...
Function OnlineSupportResourceDescription() Export
	...
	Return New Structure("Protocol, Address, Port, Description",
		...
		"webits.1c.en",
		443,
		NStr("en='Online user support';ru='Интернет-поддержка пользователей';vi='Hỗ trợ người sử dụng qua Internet'"));
...
Function DefineStartPossibilityByLocationAndParameters(LaunchLocation, InternetSupportParameters) Export
	...
	ElsIf Not InternetSupportParameters.UseOnlineSupport Then
		...
		Result.Insert("Message",
			NStr("en='Online user support cannot be used in the current work mode.';ru='Использование Интернет-поддержки пользователей запрещено в текущем режиме работы.';vi='Cấm sử dụng hỗ trợ người sử dụng qua Internet trong chế độ làm việc hiện tại.'"));
	...
	ElsIf Not InternetSupportParameters.LaunchAllowed Then
		...
		Result.Insert("Action" , "ShowMessage");
		Result.Insert("Message",
			NStr("en='You have no rights to launch online user support. Contact your administrator.';ru='Недостаточно прав для запуска Интернет-поддержки пользователей. Обратитесь к администратору.';vi='Không đủ quyền để khởi động hỗ trợ người sử dụng qua Internet. Hãy liên hệ với người quản trị.'"));
...
"Не удалось получить WSDL-описание с сервера Интернет-поддержки пользователей (%1).';vi='Lỗi khi tạo mô tả Web-service."
"Không thể nhận mô tả WSDL từ Server hỗ trợ người sử dụng qua Internet (%1):'"),
					"%1",
					WSDLAddress)
				+ " " + DetailErrorDescription(ErrorInfo());
			Raise ErrorMessage;
...
"Ошибка чтения WSDL-описания веб-сервиса Интернет-поддержки пользователей:';vi='Lỗi khi tạo mô tả Web-Service (%1)."
"Lỗi khi đọc mô tả WSDL Web-Service hỗ trợ người sử dụng qua Internet:'"),
				"%1",
				WSDLAddress)
			+ " " + DetailErrorDescription(ErrorInfo());
		Raise ErrorMessage;
...
"Lỗi khi đọc mô tả WSDL Web-Service hỗ trợ người sử dụng qua Internet:'"),
				...
				WSDLAddress)
			+ Chars.LF
			+ NStr("en='Data type description item is missing (<wsdl:types ...>).';ru='Отсутствует элемент описания типов данных (<wsdl:types ...>).';vi='Không có phần tử mô tả kiểu dữ liệu (<wsdl:types ...>).'");
...
"Lỗi khi đọc mô tả WSDL Web-Service hỗ trợ người sử dụng qua Internet:'"),
				"%1",
				WSDLAddress)
			+ " " + NStr("en='Data type description item is missing (<xs:schema ...>)';ru='Отсутствует элемент описания типов данных (<xs:schema ...>)';vi='Không có phần tử mô tả kiểu dữ liệu (<xs:schema ...>)'");
...
"Ошибка при создании схемы данных из WSDL-описания веб-сервиса Интернет-поддержки пользователей:';vi='Lỗi khi tạo mô tả Web-Service (%1)."
"Lỗi khi tạo sơ đồ dữ liệu từ mô tả WSDL Web-Service hỗ trợ người sử dụng qua Internet.'"),
				"%1",
				WSDLAddress)
			+ " " + DetailErrorDescription(ErrorInfo());
		Raise ErrorMessage;
...
"Ошибка при создании фабрики XDTO из WSDL-описания веб-сервиса Интернет-поддержки пользователей:';vi='Lỗi khi tạo mô tả Web-service (%1)."
"Lỗi khi tạo yếu tố XDTO từ mô tả WSDL Web-Service hỗ trợ người sử dụng qua Internet:'"),
				"%1",
				WSDLAddress)
			+ " " + DetailErrorDescription(ErrorInfo());
		Raise ErrorMessage;
...
"Lỗi khi đọc mô tả WSDL Web-Service hỗ trợ người sử dụng qua Internet:'"),
				"%1",
				WSDLAddress)
			+ " " + NStr("en='Names space URI in WSDL description is missing.';ru='Отсутствует URI пространства имен в WSDL-описании.';vi='Không có không gian tên URI trong mô tả WSDL.'");
...
"Lỗi khi đọc mô tả WSDL Web-Service hỗ trợ người sử dụng qua Internet:'"),
				"%1",
				WSDLAddress)
			+ " " + NStr("en='Description of the web services is missing in WSDL description (<wsdl:service ...>).';ru='Отсутствует описание веб-сервисов в WSDL-описании (<wsdl:service ...>).';vi='Không có mô tả Web-service trong mô tả WSDL (<wsdl:service ...>).'");
...
"Lỗi khi đọc mô tả WSDL Web-Service hỗ trợ người sử dụng qua Internet:'"),
				"%1",
				WSDLAddress)
			+ " " + NStr("en='No description of ports in WSDL description (<wsdl:port ...>).';ru='Отсутствует описание портов в WSDL-описании (<wsdl:port ...>).';vi='Không có mô tả cổng trong mô tả WSDL (<wsdl:port ...>).'");
...
"Lỗi khi đọc mô tả WSDL Web-Service hỗ trợ người sử dụng qua Internet:'"),
				"%1",
				WSDLAddress)
			+ " " + StrReplace(NStr("en='Cannot define name of the service port (%1).';ru='Не удалось определить имя порта сервиса (%1).';vi='Không thể xác định tên cổng Service (%1).'"),
				"%1",
				ServiceName);
...
"Lỗi khi đọc mô tả WSDL Web-Service hỗ trợ người sử dụng qua Internet:'"),
				"%1",
				WSDLAddress)
			+ " " + StrReplace(NStr("en='Cannot detect URL of the defined service port (%1).';ru='Не удалось определить URL заданного порта сервиса (%1).';vi='Không thể xác định URL của cổng Service đã đặt (%1).'"),
				"%1",
				PortName);
...
Function OUSService_isReady(OUSServiceDescription) Export
	...
	Except
		...
		ErrorMessage = StrReplace(NStr("en='An error occurred while calling the isReady operation of service (%1):';ru='Ошибка при вызове операции isReady сервиса (%1):';vi='Lỗi khi gọi ra thao tác isReady của Service (%1):'"),
				"%1",
...
"Ошибка SOAP:';vi='Lỗi khi gọi ra thao tác isReady của Service (%1)."
"Lỗi SOAP:'"),
			"%1",
			OUSServiceDescription.WSDLAddress)
...
"Не удалось определить тип корневого свойства isConfigurationSupported.';vi='Lỗi khi gọi ra thao tác isConfigurationSupported của Service (%1)."
"Không thể xác định kiều thuộc tính gốc isConfigurationSupported.'"),
			"%1",
			OUSServiceDescription.WSDLAddress);
		Raise ErrorMessage;
	EndIf;
	...
	Except
		...
		ErrorMessage = StrReplace(NStr("en='An error occurred when calling the isConfigurationSupported operation of the (%1) service:';ru='Ошибка при вызове операции isConfigurationSupported сервиса (%1):';vi='Lỗi khi gọi ra thao tác isConfigurationSupported của Service (%1):'"),
			"%1",
			OUSServiceDescription.WSDLAddress)
			+ " " + DetailErrorDescription(ErrorInfo());
...
"Не удалось определить тип корневого свойства isConfigurationSupportedResponse.';vi='Lỗi khi gọi ra thao tác isConfigurationSupported của Service (%1)."
"Không thể xác định kiểu thuộc tính gốc isConfigurationSupportedResponse.'"),
			"%1",
			OUSServiceDescription.WSDLAddress);
		Raise ErrorMessage;
	EndIf;
...
Function OUSService_process(QueryParameters, OUSServiceDescription) Export
	...
	Except
		...
		ErrorMessage = StrReplace(NStr("en='An error occurred while calling the process operation of service (%1):';ru='Ошибка при вызове операции process сервиса (%1):';vi='Lỗi khi gọi ra thao tác process của Service (%1):'"),
			"%1",
			OUSServiceDescription.WSDLAddress)
			+ " " + DetailErrorDescription(ErrorInfo());
...
Function OUSService_sendmailtonet(QueryParameters, OUSServiceDescription) Export
	...
	Except
		...
		ErrorMessage = StrReplace(NStr("en='An error occurred while calling the sendmailtonet operation of service (%1).';ru='Ошибка при вызове операции sendmailtonet сервиса (%1).';vi='Lỗi khi gọi ra thao tác sendmailtonet của Service (%1).'"),
			"%1",
			OUSServiceDescription.WSDLAddress)
			+ " " + DetailErrorDescription(ErrorInfo());
...
"Не удалось определить тип корневого свойства processResponse.';vi='Lỗi khi gọi ra thao tác sendmailtonet của Service (%1)."
"Không thể xác định kiểu thuộc tính gốc processResponse.'"),
			"%1",
			OUSServiceDescription.WSDLAddress);
		Raise ErrorMessage;
...
Function SendSOAPQuery(EnvelopeText, OUSServiceDescription)
	...
	Except
		ErrorMessage = NStr("en='An error of the network connection occurred while sending the request.';ru='Ошибка сетевого соединения при отправке запроса.';vi='Lỗi kết nối theo mạng khi gửi truy vấn.'")
			+ Chars.LF
			+ DetailErrorDescription(ErrorInfo());
		Raise ErrorMessage;
	EndTry;
...
Function ReadResponseInSOAPEnvelope(ResponseBody, OUSServiceDescription, ValueType)
	...
	Except
		...
		ErrorMessage = NStr("en='An error occurred while reading SOAP response:';ru='Ошибка чтения ответа SOAP:';vi='Lỗi đọc phản hồi SOAP:'")
			+ " " + DetailErrorDescription(ErrorInfo())
			+ Chars.LF
			+ NStr("en='Response body:';ru='Тело ответа:';vi='Nội dung phản hồi:'")
			+ Chars.LF
			+ ResponseBody;
	...
	If ResponseReading.NodeType = XMLNodeType.StartElement
		...
		ErrorMessage = NStr("en='An error of SOAP Server occurred while processing the query:';ru='Ошибка SOAP-Сервера при обработке запроса:';vi='Lỗi Server-SOAP khi xử lý truy vấn:'")
	...
	Except
		...
		ErrorMessage = StrReplace(NStr("en='An error occurred while reading object (%1) in SOAP envelope:';ru='Ошибка чтения объекта (%1) в конверте SOAP:';vi='Lỗi đọc đối tượng (%1) khi chuyển đổi SOAP:'"),
				"%1",
				String(ValueType))
			+ " " + DetailErrorDescription(ErrorInfo())
			+ Chars.LF
...
Function DescriptionSOAPExceptionToRow(ExceptionSOAP)
	...
	If Not IsBlankString(ExceptionSOAP.FaultActor) Then
		Result = Result + ?(IsBlankString(Result), "", Chars.LF + NStr("en='Error source:';ru='Источник ошибки:';vi='Nguồn lỗi:'") + " ")
...
Function StructureServerResponse(
	...
	Except
		...
		HandlerContext.UserErrorDescription =
			NStr("en='Unknown error.For more information, see the event log.';ru='Неизвестная ошибка.См. подробности в журнале регистрации.';vi='Lỗi không xác định. Chi tiết xem trong Nhật ký sự kiện.'");
...
Function StructureMessageOrQuestionToUser(ServerCommand) Export
	...
	If Not CommandStructure.Property("Title") OR IsBlankString(CommandStructure.Title) Then
		CommandStructure.Insert("Title", NStr("en='Online user support';ru='Интернет-поддержка пользователей';vi='Hỗ trợ người sử dụng qua Internet'"));
	EndIf;
...
Function InternalFormParameters(FormIndex, LaunchLocation) Export
	...
	ElsIf FormIndex = "bh1" Then
		FormParameters.Insert("Title", NStr("en='Online user support';ru='Интернет-поддержка пользователей';vi='Hỗ trợ người sử dụng qua Internet'"));
...
Function StructureAddressClassifierRecord(ServerCommand) Export
		...
		For Each Parameter IN CountriesListParameters Do
			...
			If CurrParameterName = "country" Then
				...
				If CountryIdentifier <> Undefined Then
					...
					CountryStatesList.Insert(0, "-1", NStr("en='<Not selected>';ru='<не выбран>';vi='<chưa chọn>'"));
	...
	CountriesList.Insert(0, "-1", NStr("en='<not selected>';ru='<не выбрана>';vi='<chưa chọn>'"));
...
Function StructuredUserCompaniesRecord(ServerCommand) Export
	...
	CompaniesList.Insert(0, "-1", NStr("en='<add a new company>';ru='<добавить новую организацию>';vi='<thêm doanh nghiệp mới>'"));
...
Procedure AddServiceCommands(
	...
	If TransferedQueryParameters <> Undefined Then
		...
		For Each TransferredParameter IN TransferedQueryParameters Do
			...
			Else
				...
				Except
					InfError = ErrorInfo();
					ErrorMessage = StrReplace(NStr("en='An error occurred while converting transferred data. %1';ru='Ошибка при преобразовании передаваемых данных. %1';vi='Lỗi khi chuyển đổi dữ liệu đang chuyển: %1'"),
						"%1",
	...
	If CommandsStructureArray = Undefined OR CommandsStructureArray.Count() = 0 Then
		Raise NStr("en='Empty server response.';ru='Пустой ответ сервера.';vi='Phản hồi rỗng của Server.'");
	EndIf;
...
Function SendEmailThroughService(MessageStructure, OUSNetworkParameters) Export
	...
	Except
		OnlineUserSupportServerCall.WriteErrorInEventLogMonitor(
			NStr("en='An error occurred while sending the email.';ru='Ошибка при отправке электронного письма.';vi='Lỗi khi gửi E-mail.'")
				+ " " + DetailErrorDescription(ErrorInfo()));
		Return False;
	EndTry;
