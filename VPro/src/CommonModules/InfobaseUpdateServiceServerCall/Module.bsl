...
Procedure WriteExecutionTimeUpdate(UpdateBeginTime, UpdateEndTime) Export
	...
	DurationHours = ?(Hours = 0, "",
		StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1 hour';ru='%1 час';vi='%1 giờ'"), Hours));
	DurationMinutes = ?(Minutes = 0, "",
		StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1 min';ru='%1 мин';vi='%1 phút'"), Minutes));
	DurationSeconds = ?(Seconds = 0, "",
		StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1 sec';ru='%1 сек';vi='%1 giây'"), Seconds));
