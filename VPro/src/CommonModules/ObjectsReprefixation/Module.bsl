...
Procedure ServiceSetIBPrefixAndReprefixAllObjectsCreatedInThisIB(
	...
	If TransactionActive() Then
		...
		Raise NStr("en='Cannot change infobase prefix during transaction.';ru='Изменение префикса информационной базы не может выполняться в транзакции.';vi='Không thể thay đổi tiền tố cơ sở thông tin trong giao tác.'");
		
	ElsIf Not Users.InfobaseUserWithFullAccess() Then
		
		Raise NStr("en='Insufficient rights to change infobase prefix.';ru='Недостаточно прав для изменения префикса информационной базы.';vi='Không đủ quyền để thay đổi tiền tố cơ sở thông tin.'");
...
"Недостаточная длина %1. Минимальная длина %1 объекта должна составлять %3 символов.';vi='Không thể chuyển đổi %1 của đối tượng %2."
"Chiều dài bị thiếu %1. Chiều dài tối thiểu %1 của đối tượng cần có %3 ký tự.'");
		MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString,
					?(ThisIsDocument, NStr("en='Numbers';ru='номера';vi='số'"), NStr("en='code';ru='кода';vi='mã'")),
					String(Object),
...
Procedure CheckPrefixSetPossibility()
	...
	If Not FunctionalOptionInUse Then
		...
		Raise NStr("en='Object reprefixation is not available.';ru='Перепрефиксация объектов недоступна.';vi='Không cho phép thêm lại tiền tố đối tượng.'");
...
Function EventLogMonitorMessageTextPereprefixionObjects()
	...
	Return NStr("en='Object prefixation.Infobase prefix change';ru='Префиксация объектов.Изменение префикса информационной базы';vi='Thêm tiền tố đối tượng.Thay đổi tiền tố cơ sở thông tin'", CommonUseClientServer.MainLanguageCode());
