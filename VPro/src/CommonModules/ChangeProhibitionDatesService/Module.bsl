...
Procedure RecalculateCurrentValuesOfRelativeProhibitionDates(WriteResultDescriptionToEventLogMonitor = True, ResultDescription = "") Export
	...
	If ThereAreNoRelativeDates Then
		ResultDescription = NStr("en='Relative closing dates are not specified.';ru='Относительные даты запрета не заданы.';vi='Ngày tương đối cấm chưa chỉ ra.'");
...
"При пересчете возникли ошибки:';vi='Tính lại một số giá trị hiện tại trong ngày tương đối cấm."
"Khi tính lại phát sinh lỗi:'");
		Else
			ResultDescription =
...
"При пересчете возникли ошибки:';vi='Chưa tính lại giá trị hiện tại ngày cấm."
"Khi tính lại phát sinh lỗi:'");
		EndIf;
	...
	Else
		If AreUpdatedDates Then
			ResultDescription =
				NStr("en='Current values of relative closing dates are recalculated.';ru='Успешно пересчитаны текущие значения относительных дат запрета.';vi='Đã tính lại giá trị hiện tại trong các ngày tương đối cấm.'");
		Else
			ResultDescription =
				NStr("en='Current values of relative closing dates were already recalculated today.';ru='Сегодня уже пересчитаны текущие значения относительных дат запрета.';vi='Hôm nay đã tính lại giá trị hiện tại của ngày tương đối cấm.'");
		EndIf;
	EndIf;
	
	If WriteResultDescriptionToEventLogMonitor Then
		WriteLogEvent(
			NStr("en='Change closing dates.Relative date recalculation';ru='Даты запрета изменения.Пересчет относительных дат';vi='Ngày cấm thay đổi.Tính lại ngày tương đối'",
...
"Подробности в журнале регистрации.';vi='Không thể tính lại tất cả các ngày cấm liên quan."
...
"запрета изменения в"
"источнике данных для таблицы ""%1"" не задано поле даты.';vi='Để kiểm tra"
"việc cấm thay đổi"
"trong nguồn dữ liệu đối với bảng ""%1"" chưa đặt trường ngày.'"),
				Table));
...
"запрета изменения в"
"источнике данных для таблицы ""%1"" неверно задано поле даты: %2.';vi='Để kiểm tra"
"việc cấm thay đổi trong"
"nguồn dữ liệu đối với bảng ""%1"", đã đặt sai trường ngày: %2.'"),
				Table,
				DataSource.DataField));
		EndIf;
...
"данных для таблицы ""%1"" неверно задано поле объекта: %2.';vi='Để kiểm tra"
"việc cấm thay đổi trong nguồn"
"dữ liệu đối với bảng ""%1"", đã đặt sai trường đối tượng: %2."
"'"),
					Table,
					DataSource.ObjectField));
			EndIf;
...
Function GetObjectFieldsStructure(MetadataObject, DataSources, Table) Export
	...
	For Each DataSource IN DataSources Do
		...
		AddField(MetadataObject,
		             ...
		             DataSource.DataField,
		             Table,
		             NStr("en='date field';ru='поле даты';vi='trường ngày'"));
		...
		If ValueIsFilled(DataSource.ObjectField) Then
			...
			AddField(MetadataObject,
			             ...
			             DataSource.ObjectField,
			             Table,
			             NStr("en='object field';ru='поле объекта';vi='trường đối tượng'"));
		EndIf;
	EndDo;
...
"источники данных для таблицы ""%1"".';vi='Để kiểm tra cấm thay đổi không tìm thấy"
"nguồn dữ liệu đối với bảng ""%1"".'"),
			Filter.Table));
	EndIf;
...
Procedure AddField(MetadataObject,
	...
	If Fields.Count() = 0 Then
		Raise(StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='The %2 is not specified to check prohibition change in the data source for table ""%1"".';ru='Для проверки запрета изменения в источнике данных для таблицы ""%1"" не задано %2.';vi='Để kiểm tra việc cấm thay đổi trong nguồn dữ liệu đối với bảng ""%1"", chưa đặt %2.'"),
			Table,
			FieldTypeForMessages));
...
"запрета изменения в"
"источнике данных для таблицы ""%1"" неверно задано %2: ""%3""';vi='Để kiểm tra"
"việc cấm thay đổi"
"trong nguồn dữ liệu đối với bảng ""%1"", đã đặt sai %2: ""%3""'"),
			Table,
			FieldTypeForMessages,
			Field));
	EndIf;
...
"запрета изменения в"
"источнике данных"
"для таблицы ""%1"" неверно задано %2: не задано поле заданной табличной части ""%3"".';vi='Để kiểm tra"
"việc cấm thay đổi"
"trong nguồn dữ liệu"
"đối với bảng ""%1"" đã đặt sai %2: chưa đặt trường của phần bảng đã thiết lập ""%3"".'"),
				Table,
				FieldTypeForMessages,
				Field));
...
"запрета изменения"
"в источнике данных"
"для таблицы ""%1"" неверно задано %2: неверно задано поле заданной табличной части ""%3"".';vi='Để kiểm tra"
"việc cấm thay đổi"
"trong nguồn dữ liệu"
"đối với bảng ""%1"" đã đặt sai %2: đặt sai trường của phần bảng đã thiết lập ""%3"".'"),
				Table,
				FieldTypeForMessages,
				Field));
		EndIf;
