...
Procedure SignFile(AttachedFile, FormID, AdditionalParameters = Undefined) Export
	...
	If Not ValueIsFilled(AttachedFile) Then
		ShowMessageBox(, NStr("en='File to be signed is not selected.';ru='Не выбран файл, который нужно подписать.';vi='Chưa chọn tệp cần ký tên.'"));
		Return;
	EndIf;
	
	If Not CommonUseClient.SubsystemExists("StandardSubsystems.DigitalSignature") Then
		ShowMessageBox(, NStr("en='Adding digital signatures is not supported.';ru='Добавление электронных подписей не поддерживается.';vi='Không hỗ trợ thêm chữ ký điện tử.'"));
		Return;
	EndIf;
...
"электронную подпись, включите в настройках программы использование электронных подписей.';vi='Để thêm chữ ký điện tử, trong tùy chỉnh chương trình hãy bật sử dụng chữ ký điện tử.'"));
		Return;
	EndIf;
