...
Function GetDatesArrayByCalendar(Val WorkSchedule, Val DateFrom, Val DaysArray, Val CalculateNextDateFromPrevious = False, CallingException = True) Export
	...
	While Selection.Next() Do
		If Selection.DateInCalendar = Undefined Then
			ErrorInfo = NStr("en='The ""%1"" business calendar is not filled in from date %2 for the specified number of working days.';ru='Производственный календарь ""%1"" не заполнен с даты %2 на указанное количество рабочих дней.';vi='Chưa điền lịch sản xuất ""%1"" từ ngày %2 với số ngày làm việc đã chỉ ra.'");
...
Function GetDatesDiffByCalendar(Val WorkSchedule, Val StartDate, Val EndDate, CallingException = True) Export
	...
	If Result.IsEmpty() Then
		If CallingException Then
			ErrorInfo = NStr("en='The ""%1"" work schedule is not filled in for period %2.';ru='График работы ""%1"" не заполнен на период %2.';vi='Chưa điền lịch biểu làm việc ""%1"" vào kỳ %2.'");
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				ErrorInfo,
		...
		Else 
			...
			If CallingException Then
				ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(
									NStr("en='Cannot determine the nearest workday for date %1, work schedule might not be populated.';ru='Невозможно определить ближайшую рабочую дату для даты %1, возможно, график работы не заполнен.';vi='Không thể xác định ngày làm việc gần nhất đối với ngày %1, có thể, chưa điền lịch biểu làm việc.'"), 
...
Function SchedulesOfWorkOnPeriod(Graphics, StartDate, EndDate) Export
	...
	Raise NStr("en='Subsystem ""Work schedules"" not found.';ru='Подсистема ""Графики работы"" не обнаружена.';vi='Không tìm thấy phân hệ ""Lịch biểu làm việc"".'");
