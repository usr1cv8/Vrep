...
Procedure LockCurrentDataArea(Val CheckNoOtherSessions = False, Val SeparatedLock = False) Export
	...
	If Not CommonUseReUse.CanUseSeparatedData() Then
		Raise(NStr("en='Area can be locked only when separator usage is enabled';ru='Блокировка области может быть установлена только при включенном использовании разделителей';vi='Phong tỏa vùng chỉ có thể được thiết lập khi bật tính năng sử dụng đường chia tách'"));
	EndIf;
...
"области данных по причине: %1';vi='Không thể thiết lập phong tỏa"
"vùng dữ liệu do: %1'");
				TextOfComment = StringFunctionsClientServer.SubstituteParametersInString(CommentTemplate, 
					DetailErrorDescription(ErrorInfo()));
				WriteLogEvent(
					NStr("en='Data area locking';ru='Блокировка области данных';vi='Phong tỏa vùng dữ liệu'", CommonUseClientServer.MainLanguageCode()),
					EventLogLevel.Error,
					,
					,
					TextOfComment);
		...
		If ConflictSessions.Count() > 0 Then
			...
			For Each ConflictSession IN ConflictSessions Do
				...
				TextSessions = TextSessions + StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='%1 (session - %2)';ru='%1 (сеанс - %2)';vi='%1 (phiên làm việc - %2)'", CommonUseClientServer.MainLanguageCode()),
			...
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Operation cannot be performed as other users are using the application: %1';ru='Операция не может быть выполнена, т.к. в приложении работают другие пользователи: %1';vi='Không thể thực hiện giao dịch vì trong ứng dụng có người sử dụng khác đang làm việc: %1'",
...
Procedure CopyAreaData(Val SourceArea, Val DestinationArea) Export
	...
	Except
		WriteLogEvent(NStr("en='Copy data area';ru='Копирование области данных';vi='Sao chép vùng dữ liệu'", CommonUseClientServer.MainLanguageCode()), 
	...
	While Selection.Next() Do
		...
		Else
			...
			ChangeAreasStateAndNotifyManager(Manager, ?(Manager.Status = Enums.DataAreaStatuses.New,
				"FatalError", "ErrorDelete"), NStr("en='Number of attempts to process the area is exceeded';ru='Исчерпано количество попыток обработки области';vi='Tận dụng hết số lượng thử nghiệm xử lý vùng'"));
...
Function GetServiceManagerProxy(Val UserPassword = Undefined) Export
	...
	If Not ValueIsFilled(ServiceManagerAddress) Then
		Raise(NStr("en='Parameters of connection with the service manager are not set.';ru='Не установлены параметры связи с менеджером сервиса.';vi='Chưa đặt tham số liên kết với bộ quản lý công cụ.'"));
	EndIf;
...
Procedure SetSessionSeparation(Val Use = Undefined, Val DataArea = Undefined) Export
	...
	If Not CommonUseReUse.SessionWithoutSeparator() Then
		Raise(NStr("en='You can change session separation only from the session running without separators specified';ru='Изменить разделение сеанса возможно только из сеанса запущенного без указания разделителей';vi='Thay đổi chia tách phiên làm việc chỉ có thể trong phiên làm việc đã được khởi động mà không chỉ ra đường chia tách'"));
	EndIf;
...
Function SessionSeparatorValue() Export
	...
	Else
		If Not CommonUse.UseSessionSeparator() Then
			Raise(NStr("en='Separator value is not set';ru='Не установлено значение разделителя';vi='Chưa đặt giá trị đường chia tách'"));
		EndIf;
...
Procedure ProcessInformationAboutWebServiceError(Val ErrorInfo, Val SubsystemName = "", Val WebServiceName = "", Val OperationName = "") Export
	...
	EventName = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='%1.Web service operation call error';ru='%1.Ошибка вызова операции web-сервиса';vi='%1.Lỗi gọi ra giao dịch Web-service'", CommonUseClientServer.MainLanguageCode()),
		SubsystemName);
	
	ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='An error occurred when calling operation %1 of web service %2: %3';ru='Ошибка при вызове операции %1 веб-сервиса %2: %3';vi='Lỗi khi gọi ra thao tác %1 của Web-service %2: %3'", CommonUseClientServer.MainLanguageCode()),
		OperationName,
		WebServiceName,
...
"сервиса требуется внедрить библиотеку ""1С:Библиотека технологии сервиса"" версии не младше %1!';vi='Trong cấu hình chưa nhập thư viện ""1C:Thư viện các phân hệ chuẩn""."
"Nếu chưa nhập thư viện này thì không thể sử dụng cấu hình ở dịch vụ đám mây."
""
"Để sử dụng cấu hình ở dịch vụ đám mây,"
"cần kết nhập thư viện ""1C:Thư viện các phân hệ chuẩn"" với phiên bản không thấp hơn %1!'", Metadata.DefaultLanguage.LanguageCode),
			RequiredSTLVersion());
...
""
"Используемая версия: %1, требуется версия не младше %2!';vi='Để sử dụng cấu hình trong dịch vụ đám mây với"
"phiên bản Thư viện phân hệ chuẩn hiện tại, cần cập nhật phiên bản cần thiết của thư viện ""1C:Thư viện công nghệ dịch vụ""!"
""
"Phiên bản đang sử dụng: %1, cần có phiên bản không dưới %2!"
...
"Проверьте наличие и корректность внедрения подсистемы ""%1"".';vi='Cần thực hiện giao dịch theo nguyên nhân - không triển khai phân hệ ""%1"" trong cấu hình."
"Phân hệ này được đặt trong thành phần thư viện công nghệ của dịch vụ mà cần được triển khai riêng trong thành phần cấu hình."
"Hãy kiểm tra sự tồn tại và tính chính xác khi triển khai phân phân hệ ""%1"".'"),
		SubsystemName
	);
...
Procedure OnCheckingSafeModeDataSharing() Export
	...
	If SafeMode() = False
		...
		If SeparationSwitched Then
			...
			WriteLogEvent(NStr("en='Publication error';ru='Ошибка публикации';vi='Lỗi đăng tải'", CommonUseClientServer.MainLanguageCode()), 
				EventLogLevel.Error,
				,
				,
				NStr("en='Safe mode for data separation is not enabled on publishing';ru='При публикации не включен режим безопасного разделения данных';vi='Khi đăng tải chưa bật chế độ phân chia dữ liệu an toàn'"));
			Raise(NStr("en='Infobase is published incorrectly. The session will be closed.';ru='Информационная база опубликована некорректно. Работа сеанса будет прекращена.';vi='Công bố sai cơ sở thông tin. Sẽ ngắt bỏ phiên làm việc.'"));
		EndIf;
...
""
"Попробуйте запустить приложение через несколько минут.';vi='Tạm thời không thể khởi động ứng dụng."
"Đang thực hiện giao dịch bảo trì ứng dụng thường kỳ."
""
"Hãy thử khởi động ứng dụng sau vài phút.'");
...
Function ParametersSelections(Val FullMetadataObjectName) Export
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='The ParametersSelections() function should not be used for object %1.';ru='Функция ParametersSelections() не должна использоваться для объекта %1.';vi='Hàm ParametersSelections() không cần sử dụng đối với đối tượng %1.'"),
...
Function GetRecordManagerOfDataAreas(Val DataArea, Val Status)
	...
	Try
		...
		If Not RecordManager.Selected() Then
			MessagePattern = NStr("en='Data area %1 is not found';ru='Область данных %1 не найдена';vi='Vùng dữ liệu %1 không tìm thấy'");
		...
		ElsIf RecordManager.Status <> Status Then
			MessagePattern = NStr("en='Status of data area %1 is not ""%2""';ru='Статус области данных %1 не равен ""%2""';vi='Trạng thái vùng dữ liệu %1 không bằng ""%2""'");
	...
	Except
		RollbackTransaction();
		WriteLogEvent(NStr("en='Prepare data area';ru='Подготовка области данных';vi='Chuẩn bị vùng dữ liệu'", CommonUseClientServer.MainLanguageCode()), 
			EventLogLevel.Error, , , DetailErrorDescription(ErrorInfo()));
		Raise;
	EndTry;
...
Procedure ChangeAreasStateAndNotifyManager(Val RecordManager, Val ResultOf, Val ErrorInfo)
	...
	ElsIf ResultOf = "ErrorDelete" Then
		...
		IncludeMessageAboutError = True;
	Else
		Raise NStr("en='Unexpected return code';ru='Неожиданный код возврата';vi='Mã trả lại bất thường'");
	EndIf;
...
Function PrepareDataFromExportingArea(Val DataArea, Val ExportFileID, ErrorInfo)
	...
	If ExportFileName = Undefined Then
		...
		ErrorInfo = NStr("en='No initial info file for the area';ru='Нет файла начальных данных для области';vi='Không có tệp dữ liệu ban đầu cho vùng'");
...
Procedure WhenCompletingTablesOfParametersOfIB(Val ParameterTable) Export
	...
	CurParameterString.Description = NStr("en='Configuration version';ru='Версия конфигурации';vi='Phiên bản cấu hình'");
...
Function GetFileFromStorage(Val FileID, Val ConnectionParameters, 
		...
		Try
			...
			If FileProperties.Exist() Then
				...
				WriteInJournalFileStoreEvent(
					NStr("en='Extract';ru='Извлечение';vi='Lấy ra'", CommonUseClientServer.MainLanguageCode()),
					FileID,
					SizeReceivedFile,
			...
			EndIf;
		Except
			WriteLogEvent(NStr("en='Receive file from the storage';ru='Получение файла из хранилища';vi='Nhận tệp từ kho chứa'", CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,,, DetailErrorDescription(ErrorInfo()));
			ExchangeOverFS = False;
		EndTry;
	...
	If Dearchiver.Items.Count() > 1 Then
		Raise(NStr("en='The received archive contains more than one file';ru='В полученном архиве содержится более одного файла';vi='Trong lưu trữ đã nhận có nhiều hơn một tệp'"));
	EndIf;
...
"измерения';vi='Đơn vị'", CommonUseClientServer.MainLanguageCode()),
					Result,
					SizeOfSourceFile,
					CurrentUniversalDate() - ExecutionStarted,
					ExchangeOverFS);
...
"измерения';vi='Đơn vị'", CommonUseClientServer.MainLanguageCode()),
				...
				ExchangeOverFS);
		Except
			WriteLogEvent(NStr("en='Add file.Exchange via FS';ru='Добавление файла.Обмен через ФС';vi='Thêm tệp.Trao đổi thông qua quỹ BHXH'", CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,,, DetailErrorDescription(ErrorInfo()));
			ExchangeOverFS = False;
		EndTry;
	...
	If Not ExchangeOverFS Then
		...
		If Description.Data = Undefined Then
			...
			Else
				Raise(StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Add file to the storage. File %1 not found.';ru='Добавление файла в хранилище. Не найден файл %1.';vi='Thêm tệp trong kho chứa. Chưa tìm thấy tệp %1.'"), FileProperties.DescriptionFull));
			EndIf;
		Else
...
Function GetFileNameWithData(Val AddressDataFile, Val FileName = "")
	...
	If TypeOf(AddressDataFile) = Type("String") Then // File data address in a temporary storage.
		If IsBlankString(AddressDataFile) Then
			Raise(NStr("en='Invalid storage address.';ru='Неверный адрес хранилища.';vi='Sai địa chỉ kho chứa.'"));
		EndIf;
	...
	Else
		Raise(NStr("en='Invalid data type';ru='Неверный тип данных';vi='Sai kiểu dữ liệu'"));
	EndIf;
...
Procedure WriteInJournalFileStoreEvent(Val Event,
	...
	WriteLogEvent(
		NStr("en='File storage';ru='Хранилище файлов';vi='Kho chứa tệp'", CommonUseClientServer.MainLanguageCode()) + "." + Event,
...
Procedure DeleteTemporaryFiles(Val FileName)
	...
	Try
		...
		EndIf;
	Except
		WriteLogEvent(NStr("en='Deleting temporary file';ru='Выполнение операции удаления временного файла';vi='Thực hiện thao tác xóa tệp tạm thời'", 
			CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,,, DetailErrorDescription(ErrorInfo()));
		Return;
	EndTry;
...
Function StructuralObjectToXDTOObject(Val StructuralTypeValue)
	...
	If Not StructuralTypeToSerialize(StructuralType) Then
		ErrorInfo = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Type ""%1"" is not structural or currently its serialization is not supported.';ru='Тип %1 не является структурным или его сериализация в настоящее время не поддерживается.';vi='Kiểu %1 không phải là cấu trúc hoặc sê-ri hóa vào thởi điểm hiện tại không được hỗ trợ.'"),
			StructuralType);
		Raise(ErrorInfo);
	EndIf;
...
"Дескриптор файла: %1';vi='Tệp dữ liệu ban đầu đã chỉ ra không phù hợp đối với cấu hình này."
"Mô tả tệp: %1'"),
	...
	If Not DataFileFound Then
		...
		If Descriptors.Descriptor.Count() = 0 Then
			...
			NStr("en='Service manager does not include the initial data file for the current configuration version.';ru='В менеджере сервиса нет файла начальных данных для текущей версии конфигурации.';vi='Trong Quản lý dịch vụ không có tệp dữ liệu ban đầu cho phiên bản cấu hình hiện tại.'");
			Return "FatalError";
		EndIf;
		
		If Descriptors.Descriptor[0].FileGUID <> ExportFileID Then
			MessageAboutFatalError = 
			NStr("en='The initial data file available in the service manager differs from the one specified in the message for the area preparation. Cannot prepare the area.';ru='Файл начальных данных, имеющийся в менеджере сервиса, отличается от указанного в сообщении на подготовку области. Область не может быть подготовлена.';vi='Tệp dữ liệu ban đầu có trong bộ quản lý dịch vụ khác với tệp đã chỉ ra trong thông báo để chuẩn bị vùng. Không thể chuẩn bị vùng.'");
			Return "FatalError";
		EndIf;
		...
		If ExportFileName = Undefined Then
			MessageAboutFatalError = 
			NStr("en='Service manager no longer contains the required file with initial data, it might have been replaced. Area cannot be prepared.';ru='В менеджере сервиса больше нет требуемого файла начальных данных, вероятно он был заменен. Область не может быть подготовлена.';vi='Trong quản lý dịch vụ không cần tệp dữ liệu ban đầu, có lẽ đã thay thế tệp đó. Đã không thể chuẩn bị vùng.'");
			Return False;
		EndIf;
...
Procedure ControlDataOnWriteBrain(Val Source)
	...
	If CommonUseReUse.DataSeparationEnabled() AND CommonUseReUse.CanUseSeparatedData() Then
		...
		ExceptionsRepresentation = NStr("en='Access right violation';ru='Нарушение прав доступа!';vi='Vi phạm quyền truy cập!'", CommonUseClientServer.MainLanguageCode());
...
"Следующие объекты метаданных не удовлетворяют этому критерию: %3.';vi='Tất cả các đối tượng metadata không thuộc thành phần"
"đường chia tách Thư viện các phân hệ chuẩn (%1), cần được ghi nhận"
"vào sự kiện (%2) để kiểm soát việc từ chối ghi lại dữ liệu chưa chia tách trong các phiên làm việc chia tách."
"Những đối tượng metadata sau không thỏa mãn tiêu chí này: %3.'"),
...
"Следующие объекты не удовлетворяют этому критерию: %2';vi='Tất cả đối tượng metadata của cấu hình cần phải được tách bởi không quá một đường chia tách Thư viện phân hệ chuẩn (%1)."
"Các đối tượng tiếp sau không đáp ứng thể loại này: %2'"),
	...
	If Not IsBlankString(ResultingExcept) Then
		...
		ResultingExcept = NStr("en='Errors in configuration metadata structure are detected:';ru='Обнаружены ошибки в структуре метаданных конфигурации:';vi='Tìm thấy lỗi trong cấu trúc metadata của cấu hình:'") + Chars.LF + Chars.CR + ResultingExcept;
...
"в дереве метаданных конфигурации до общего реквизита %2 по порядку!';vi='Đã phát hiện sự vi phạm cấu trúc metadata"
"của cấu hình: theo thứ tự, mục tin chung %1 cần được đặt"
"trước mục tin chung %2 trên cây metadata của cấu hình!'"),
