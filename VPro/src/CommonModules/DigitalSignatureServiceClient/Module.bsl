...
"of the DigitalSignatureClient general module: Incorrect value of the  %1 CertificateData parameter.';ru='Ошибка при вызове процедуры ОткрытьСертификат общего модуля ЭлектроннаяПодписьКлиент: Некорректное значение параметра ДанныеСертификата ""%1"".';vi='Lỗi khi gọi thủ tục OpenCertificate mô-đun chung DigitalSignatureClient. Sai giá trị tham số CertificateData ""%1"".'"), String(Context.CertificateData));
...
Procedure SaveCertificateContinuation(Context)
	...
	Dialog.Title = NStr("en='Select a file to save the certificate to';ru='Выберите файл для сохранения сертификата';vi='Hãy chọn tệp để lưu chứng chỉ'");
	Dialog.Filter    = NStr("en='Certificate files (*.cer)|*.cer|All files (*.*)|*.*';ru='Файлы сертификатов (*.cer)|*.cer|Все файлы (*.*)|*.*';vi='Tệp chứng thư (*.cer)|*.cer|Tất cả các tệp (*.*)|*.*'");
...
Procedure SaveCertificateAfterReceivingFiles(ReceivedFiles, Context) Export
	...
	Else
		...
		ShowUserNotification(NStr("en='Certificate is saved to file:';ru='Сертификат сохранен в файл:';vi='Chứng thư đã lưu trong tệp:'"),,
...
Procedure SaveCertificateQueryAfterSettingExpansion(ExtensionAttached, Context) Export
	...
	Dialog = New FileDialog(FileDialogMode.Save);
	Dialog.Title = NStr("en='Select a file to save the certificate request to';ru='Выберите файл для сохранения запроса на сертификат';vi='Hãy chọn tệp để lưu câu hỏi về chứng thư'");
	Dialog.Filter    = NStr("en='Certificate files (*.p10)|*.p10|All files (*.*)|*.*';ru='Файлы сертификатов (*.p10)|*.p10|Все файлы (*.*)|*.*';vi='Tệp chứng thư (*.p10)|*.p10|Tất cả các tệp (*.*)|*.*'");
	Dialog.Multiselect = False;
...
Procedure SaveCertificateQueryAfterReceivingFiles(ReceivedFiles, Context) Export
	...
	Else
		HasReceivedFiles = True;
		ShowUserNotification(NStr("en='Certificate request is saved to file:';ru='Запрос на сертификат сохранен в файл:';vi='Truy vấn đến chứng thư đã lưu trong tệp:'"),,
			ReceivedFiles[0].Name);
	EndIf;
...
Procedure SaveSignatureAfterConnectingFileOperationsExtension(Attached, Context) Export
	...
	Filter = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Digital signature files (*.%1)|*.%1|All files (*.*)|*.*';ru='Файлы электронных подписей (*.%1)|*.%1|Все файлы (*.*)|*.*';vi='Tệp chữ ký điện tử (*.%1)|*.%1|Tất cả các tệp (*.*)|*.*'"),
		ExtensionForSignatureFiles);
	
	FileOpeningDialog.Filter = Filter;
	FileOpeningDialog.Multiselect = False;
	FileOpeningDialog.Title = NStr("en='Select a file to save the signature to';ru='Выберите файл для сохранения подписи';vi='Chọn tệp để lưu chữ ký'");
...
Procedure SaveSignatureAfterGettingFile(ReceivedFiles, Context) Export
	...
	ShowUserNotification(NStr("en='Digital signature is saved to file:';ru='Электронная подпись сохранена в файл:';vi='Đã lưu chữ ký điện tử vào tệp:'"),,
		ReceivedFiles[0].Name);
...
"не найден на компьютере по причине: %1';vi='Không tìm thấy"
"chứng thư trên máy tính do nguyên nhân: %1'"),
			ErrorPresentation);
	Else
		ErrorText = NStr("en='Certificate is not found on the computer.';ru='Сертификат не найден на компьютере.';vi='Không tìm thấy chứng thư trên máy tính.'");
	EndIf;
...
Procedure CheckSignatureOnClientAfterSignatureCheck(Certificate, Context) Export
	...
	If Certificate = Undefined Then
		ExecuteNotifyProcessing(Context.Notification,
			NStr("en='Certificate is not found in signature data.';ru='Сертификат не найден в данных подписи.';vi='Không tìm thấy chứng thư trong dữ liệu chữ ký.'"));
		Return;
	EndIf;
...
Procedure CheckCertificateOnClientAfterErrorChecks(ErrorInfo, StandardProcessing, Context) Export
	...
	If Context.ErrorDescriptionOnServer <> Undefined Then
		ErrorDescription = Context.ErrorDescriptionOnServer + " " + NStr("en='(on server)';ru='(на сервере)';vi='(trên Server)'") + Chars.LF
			+ ErrorDescription + " " + NStr("en='(on client)';ru='(на клиенте)';vi='(trên Client)'");
	EndIf;
...
Procedure CreateCryptographyManagerAfterWorkWithCryptographyExpansionConnection(Attached, Context) Export
	...
	FormTitle = NStr("en='Digital signature and encryption application is required';ru='Требуется программа электронной подписи и шифрования';vi='Cần chương trình chữ ký điện tử và mã hóa'");
	Operation       = Context.Operation;
	
	If Operation = "Signing" Then
		ErrorTitle = NStr("en='Cannot sign data due to:';ru='Не удалось подписать данные по причине:';vi='Không thể ký dữ liệu do nguyên nhân:'");
		
	ElsIf Operation = "SignatureCheck" Then
		ErrorTitle = NStr("en='Cannot check the signature due to:';ru='Не удалось проверить подпись по причине:';vi='Không thể kiểm tra chữ ký do nguyên nhân:'");
		
	ElsIf Operation = "Encryption" Then
		ErrorTitle = NStr("en='Cannot encrypt data due to:';ru='Не удалось зашифровать данные по причине:';vi='Không thể mã hóa dữ liệu do nguyên nhân:'");
		
	ElsIf Operation = "Details" Then
		ErrorTitle = NStr("en='Cannot decrypt data due to:';ru='Не удалось расшифровать данные по причине:';vi='Không thể giải mã dữ liệu bởi vì:'");
		
	ElsIf Operation = "CertificateCheck" Then
		ErrorTitle = NStr("en='Cannot check the certificate due to:';ru='Не удалось проверить сертификат по причине:';vi='Không thể kiểm tra chứng thư do nguyên nhân:'");
		
	ElsIf Operation = "GetCertificates" Then
		ErrorTitle = NStr("en='Cannot receive certificates due to:';ru='Не удалось получить сертификаты по причине:';vi='Không thể nhận chứng thư do nguyên nhân:'");
...
"Неверное значение параметра Операция ""%1"".';vi='Lỗi trong chức năng CryptographyManager."
"Sai giá trị tham số Giao dịch ""%1"".'"), Operation);
	Else
		ErrorTitle = NStr("en='Cannot execute the operation due to:';ru='Не удалось выполнить операцию по причине:';vi='Không thể thực hiện giao dịch do:'");
	EndIf;
	...
	If Not Attached Then
		ErrorText =
			NStr("en='Install an Internet browser extension to work with digital signature and encryption.';ru='В обозреватель интернет требуется установить расширение для работы с электронной подписью и шифрованием.';vi='Trong trình duyệt Internet cần cài đặt phần mở rộng để làm việc với chữ ký điện tử và mã hóa.'");
...
Procedure CreateCryptographyManagerAfterCycle(Context) Export
	...
	If ErrorProperties.Errors.Count() = 0 Then
		ErrorText = NStr("en='Usage of no application is possible.';ru='Не предусмотрено использование ни одной программы.';vi='Chưa có sẵn để sử dụng bất kỳ chương trình nào.'");
		ErrorProperties.Insert("Description", ErrorText);
		...
		If Not StandardSubsystemsClientReUse.ClientWorkParameters().InfobaseUserWithFullAccess Then
			ErrorText = ErrorText + Chars.LF + Chars.LF + NStr("en='Contact administrator.';ru='Обратитесь к администратору.';vi='Hãy liên hệ với người quản trị.'");
		EndIf;
	...
	Else
		If ValueIsFilled(Context.Application) Then
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Application %1 is not available or installed on the computer.';ru='Программа %1 не доступна или не установлена на компьютере.';vi='Chương trình %1 không khả dụng hoặc không được cài đặt trên máy tính.'"), Context.Application);
		Else
			ErrorText = NStr("en='None of the applications are available or installed on computer.';ru='Ни одна из программ не доступна или не установлена на компьютере.';vi='Không có chương trình nào khả dụng hoặc được cài đặt trên máy tính.'");
		EndIf;
		ErrorProperties.Insert("Description", ErrorText);
	EndIf;
...
Procedure AddCertificateForEncryptionOnlyFromFileAfterConnectingExtension(Attached, Context) Export
	...
	Dialog.Title = NStr("en='Select a certificate file (only for encryption)';ru='Выберите файл сертификата (только для шифрования)';vi='Hãy chọn tệp chứng thư (chỉ để mã hóa)'");
	Dialog.Filter = NStr("en='Certificate X.509 (*.cer;*.crt)|*.cer;*.crt|All files(*.*)|*.*';ru='Сертификат X.509 (*.cer;*.crt)|*.cer;*.crt|Все файлы(*.*)|*.*';vi='Chứng thư X.509 (*.cer;*.crt)|*.cer;*.crt|Tất cả tệp(*.*)|*.*'");
...
Procedure AddCertificateForEncryptionOnlyFromFileAfterPostingFile(Address, Context)
	...
	If Form = Undefined Then
		ShowMessageBox(,
			NStr("en='Certificate file must have DER X.509 format, operation aborted.';ru='Файл сертификата должен быть в формате DER X.509, операция прервана.';vi='Tệp giấy chứng thư cần phải có định dạng DER X.509, thao tác bị ngắt.'"));
		Return;
	EndIf;
	
	If Not Form.IsOpen() Then
		Buttons = New ValueList;
		Buttons.Add("Open", NStr("en='Open';ru='Открыть';vi='Mở'"));
		...
		ShowQueryBox(
			...
			NStr("en='Certificate is already added.';ru='Сертификат уже добавлен.';vi='Chứng thư đã được bổ sung.'"), Buttons);
	EndIf;
...
"процедуры ПоказатьОшибкуОбращенияКПрограмме указан некорректный тип параметра ОшибкаНаКлиенте.';vi='Đối với"
"thủ tục ПоказатьОшибкуОбращенияКПрограмме đã chỉ ra sai kiểu tham số ОшибкаНаКлиенте.'");
	EndIf;
...
"процедуры ПоказатьОшибкуОбращенияКПрограмме указан некорректный тип параметра ОшибкаНаСервере.';vi='Đối với"
"thủ tục ПоказатьОшибкуОбращенияКПрограмме đã chỉ ra sai kiểu tham số ОшибкаНаСервере.'");
	EndIf;
...
Procedure StartSelectingCertificateWhenFilterIsSet(Form) Export
	...
	Text = NStr("en='Certificates that can be used for this operation are limited.';ru='Сертификаты, которые могут быть использованы для этой операции ограничены.';vi='Những chứng thư mà có thể sử dụng cho giao dịch này đã bị hạn chế.'");
	...
	If ValueIsFilled(AvailableCertificates) Then
		Title = NStr("en='The following trusted certificates are available for selection:';ru='Следующие разрешенные сертификаты доступны для выбора:';vi='Được phép chọn các chứng thư sau đây:'");
	...
	If ValueIsFilled(UnavailableCertificates) Then
		If DigitalSignatureClientServer.CommonSettings().CreateDigitalSignaturesAtServer Then
			If ValueIsFilled(AvailableCertificates) Then
				Title = NStr("en='The following trusted certificates were not found either on computer, or on server:';ru='Следующие разрешенные сертификаты не найдены ни на компьютере, ни на сервере:';vi='Không tìm thấy các chứng thư sau đây trên máy tính và Server:'");
			Else
				Title = NStr("en='The following trusted certificates were not found on computer:';ru='Следующие разрешенные сертификаты не найдены на компьютере:';vi='Không tìm thấy các chứng thư sau đây trên máy tính:'");
			EndIf;
		Else
			If ValueIsFilled(AvailableCertificates) Then
				Title = NStr("en='The following trusted certificates were not found on computer:';ru='Следующие разрешенные сертификаты не найдены на компьютере:';vi='Không tìm thấy các chứng thư sau đây trên máy tính:'");
			Else
				Title = NStr("en='None of the following trusted certificates was found on the computer:';ru='Ни один из следующих разрешенных сертификатов не найден на компьютере:';vi='Không có chứng thư nào trong số những chứng thư dưới đây được tìm thấy trên máy tính:'");
			EndIf;
		EndIf;
...
Procedure AddSignatureFromFileAfterCreatingCryptographyManager(Result, Context) Export
	...
	If Context.CheckCryptographyManagerOnClient
	   AND TypeOf(Result) <> Type("CryptoManager") Then
		...
		ShowRequestToApplicationError(
			NStr("en='Digital signature and encryption application is required';ru='Требуется программа электронной подписи и шифрования';vi='Cần chương trình chữ ký điện tử và mã hóa'"),
...
Procedure SaveDataWithSignature(DataDescription, ResultProcessing = Undefined) Export
	...
	ServerParameters = New Structure;
	ServerParameters.Insert("DataTitle",     NStr("en='Data';ru='Данные';vi='Dữ liệu'"));
...
Procedure SaveDataWithSignatureAfterSavingDataFile(Result, Context) Export
	...
	If Result.Property("ErrorDescription") Then
		Error = New Structure("ErrorDescription",
			NStr("en='An error occurred when writing file:';ru='При записи файла возникла ошибка:';vi='Khi ghi lại tệp đã xảy ra lỗi:'") + Chars.LF + Result.ErrorDescription);
		
		ShowRequestToApplicationError(
			NStr("en='Cannot save signatures with the file';ru='Не удалось сохранить подписи вместе с файлом';vi='Không thể lưu lại chữ ký cùng với tệp'"), "", Error, New Structure);
		Return;
...
Procedure SaveDataWithSignatureAfterGettingFiles(ReceivedFiles, Context) Export
	...
	Text = NStr("en='Folder with files:';ru='Папка с файлами:';vi='Thư mục có các tệp:'") + Chars.LF;
	Text = Text + Context.PathToFiles;
	Text = Text + Chars.LF + Chars.LF;
	
	Text = Text + NStr("en='Files:';ru='Файлы:';vi='Tệp:'") + Chars.LF;
...
"с электронной подписью и шифрованием необходимо установить расширение для работы с файлами.';vi='Trước khi cài đặt phần mở rộng để làm việc"
"với chữ ký điện tử và mã hóa, cần cài đặt phần mở rộng để làm việc với tệp.'"),
		False);
		...
		If Context.OnClientSide Then
			...
			Except
				...
				ErrorOnClient = New Structure("ErrorDescription",
					NStr("en='An error occurred when writing the signature:';ru='При записи подписи возникла ошибка:';vi='Khi ghi lại chữ ký đã xảy ra lỗi:'")
...
Procedure ExecuteOnSideCycleAfterWriteSignature(Result, Context) Export
	...
	If Result.Property("ErrorDescription") Then
		...
		Error = New Structure("ErrorDescription",
			NStr("en='An error occurred when writing the signature:';ru='При записи подписи возникла ошибка:';vi='Khi ghi lại chữ ký đã xảy ra lỗi:'") + Chars.LF + Result.ErrorDescription);
		ExecuteOnSideAfterCycle(Error, Context);
		Return;
	EndIf;
...
Procedure ExecuteOnSideCycleAfterWriteEncryptedData(Result, Context) Export
	...
	If Result.Property("ErrorDescription") Then
		...
		Error = New Structure("ErrorDescription",
			NStr("en='An error occurred when writing encrypted data:';ru='При записи зашифрованных данных возникла ошибка:';vi='Khi ghi lại dữ liệu mã hóa đã xảy ra lỗi:'")
			+ Chars.LF + Result.ErrorDescription);
		ExecuteOnSideAfterCycle(Error, Context);
		Return;
	EndIf;
...
Procedure ExecuteOnSideCycleAfterWriteDecryptedData(Result, Context) Export
	...
	If Result.Property("ErrorDescription") Then
		...
		Error = New Structure("ErrorDescription",
			NStr("en='An error occurred when writing decrypted data:';ru='При записи расшифрованных данных возникла ошибка:';vi='Khi ghi lại dữ liệu đã giải mã xảy ra lỗi:'")
			+ Chars.LF + Result.ErrorDescription);
		ExecuteOnSideAfterCycle(Error, Context);
		Return;
	EndIf;
	 ...
	 Or TypeOf(Result.Data) <> Type("BinaryData")
	   AND TypeOf(Result.Data) <> Type("String") Then
		...
		If TypeOf(Result) <> Type("Structure") Or Not Result.Property("ErrorDescription") Then
			Error = New Structure("ErrorDescription", NStr("en='Incorrect data type.';ru='Некорректный тип данных.';vi='Sai kiểu dữ liệu.'"));
		Else
...
Procedure GetDataFromDataDescriptionEnd(PlacedFiles, Context) Export
	...
	If PlacedFiles = Undefined Or PlacedFiles.Count() = 0 Then
		Result = New Structure("ErrorDescription",
			NStr("en='User canceled data transfer.';ru='Передача данных отменена пользователем.';vi='Người sử dụng đã hủy truyền dữ liệu.'"));
	Else
