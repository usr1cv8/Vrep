...
Function ChoiceFormNameByDocument(DocumentFullName, TabularSectionName) Export
	...
	If FoundStrings.Count() = 0 Then
		...
		MessageText = NStr("en='Cannot define which selection form to use.';ru='Не удалось определить какую форму подбора использовать.';vi='Không thể xác định biểu mẫu nào được chọn sử dụng.'");
