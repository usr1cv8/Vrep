	...
	While IsNextPortion Do
		...
		For CountElements = 0 To EndPosition Do
			...
			Else
				...
				Raise NStr("en='Unknown error';ru='Неизвестная ошибка';vi='Lỗi không xác định'");
...
Function SearchGoods(SearchString, ErrorDescription) Export
	...
	ElsIf Result = "TooManyResults" Then
		...
		ErrorDescription = NStr("en='Too many results. Refine your search.';ru='Слишком много результатов. Уточните запрос.';vi='Có quá nhiều kết quả. Hãy làm rõ truy vấn.'");
		Return SearchResult;
		
	ElsIf Result = "FoundNothing" Then
		
		ErrorDescription = NStr("en='No results found';ru='Ничего не найдено';vi='Không tìm thấy gì '");
		Return SearchResult;
