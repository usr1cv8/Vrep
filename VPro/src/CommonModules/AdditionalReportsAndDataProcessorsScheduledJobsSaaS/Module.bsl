...
Procedure SetJobParameters(Task, Use, Parameters, Schedule) Export
	...
	If Not Constants.AllowAdditionalReportsAndDataProcessorsPerformByProceduralTasksSaaS.Get() Then
		Raise NStr("en='It is prohibited by the service administration to periodically perform additional data processor commands as jobs.';ru='Периодическое выполнение команд дополнительных обработок в качестве заданий запрещено администрацией сервиса!';vi='Người quản trị dịch vụ cấm thực hiện định kỳ các lệnh xử lý bổ sung dưới dạng nhiệm vụ!"
"'");
	EndIf;
	...
	If Schedule.ExecutionRequired(ChackedDate, OriginalDate) Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Schedule set for execution of commands of additional report or data processor as jobs should be maximum once in %1 seconds.';ru='Расписание, задаваемое для выполнения команд дополнительного отчета или обработки в качестве заданий, должно быть не чаще, чем 1 раз в %1 секунд!';vi='Lịch biểu được xác định để thực hiện lệnh trên báo cáo hoặc bộ xử lý bổ sung như là nhiệm vụ, cần phải không quá 1 lần trong %1 giấy!'"), MinInterval);
