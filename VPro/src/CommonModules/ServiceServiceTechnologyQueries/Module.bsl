	...
	If Filter.Count() > 0 Then
		For Each FilterDescription IN Filter Do
			...
			ElsIf FilterDescription.ComparisonType = ComparisonType.LessOrEqual Then
				LogicalOperator = "<=";
			Else
				MessagePattern = NStr("en='Comparison kind %1 is not supported.';ru='Вид сравнения %1 не поддерживается.';vi='Không hỗ trợ dạng so sánh %1.'");
				MessageText = ServiceTechnologyIntegrationWithSSL.SubstituteParametersInString(MessagePattern, FilterDescription.ComparisonType);
			EndIf;
