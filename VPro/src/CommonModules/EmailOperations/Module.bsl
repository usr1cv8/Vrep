...
Function SendE_Mail(Val UserAccount,
	...
	If TypeOf(UserAccount) <> Type("CatalogRef.EmailAccounts")
		...
		Raise NStr("en='Account is not filled in or filled in incorrectly.';ru='Учетная запись не заполнена или заполнена неправильно.';vi='Trương mục E-mail chưa điền hoặc điền sai.'");
	EndIf;
	
	If EmailParameters = Undefined Then
		Raise NStr("en='Sending parameters are not specified.';ru='Не заданы параметры отправки.';vi='Chưa chỉ ra tham số gửi.'");
	EndIf;
	...
	If TypeOfRecipient = Undefined AND TypeOfCc = Undefined AND TypeOfBcc = Undefined Then
		Raise NStr("en='No recipient is specified.';ru='Не указано ни одного получателя.';vi='Chưa chỉ ra bất kỳ người nhận.'");
	EndIf;
...
Function ImportEMails(Val UserAccount,
	...
	If Not UseForReceiving Then
		Raise NStr("en='Account is not intended for receiving messages.';ru='Учетная запись не предназначена для получения сообщений.';vi='Trương mục E-mail được dùng để nhận thông báo.'");
	EndIf;
