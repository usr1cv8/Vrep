...
Procedure UnknownChannelNameError(Val MessageChannel) Export
	...
	MessagePattern = NStr("en='Unknown message channel name %1';ru='Неизвестное имя канала сообщений %1';vi='Sai tên kênh thông báo %1'");
...
Procedure WriteMessageStartProcessing(Val Message) Export
	...
	WriteLogEvent(NStr("en='SaaS messages.Start processing';ru='Сообщения в модели сервиса.Начало обработки';vi='Thông báo trong mô hình dịch vụ đám mây.Bắt đầu xử lý'",
...
Procedure WriteEventEndDataProcessors(Val Message) Export
	...
	WriteLogEvent(NStr("en='SaaS messages.End processing';ru='Сообщения в модели сервиса.Окончание обработки';vi='Thông báo trong mô hình dịch vụ đám mây.Kết thúc xử lý'",
		CommonUseClientServer.MainLanguageCode()),
		EventLogLevel.Information,
		,
		,
...
Procedure DeliverQuickMessages() Export
	...
	If TransactionActive() Then
		Raise(NStr("en='Quick message delivery is unavailable in the transaction';ru='Доставка быстрых сообщений невозможна в транзакции';vi='Không thể cung cấp thông báo nhanh chóng trong giao tác'"));
	EndIf;
	...
	Try
		BackgroundJobs.Execute(JobMethodName, , JobKey, NStr("en='Instant message delivery';ru='Доставка быстрых сообщений';vi='Cung cấp thông báo nhanh chóng'"))
	Except
		// Additional exception data processor
		// is not required, the expected exception - duplication of a job with the same key.
		WriteLogEvent(NStr("en='Instant message delivery';ru='Доставка быстрых сообщений';vi='Cung cấp thông báo nhanh chóng'",
	...
	If BodyContainTypedMessage(MessageBody, Message) Then
		If MessagesSaaSReUse.TypeAreaBody().IsDescendant(Message.Body.Type()) Then
			If CommonUse.SessionSeparatorValue() <> Message.Body.Zone Then
				WriteLogEvent(NStr("en='SaaS messages.Send message';ru='Сообщения в модели сервиса.Отправка сообщения';vi='Thông báo trong mô hình dịch vụ đám mây.Gửi thông báo'",
					CommonUseClientServer.MainLanguageCode()),
					EventLogLevel.Error,
					,
					,
				...
				ErrorTemplate = NStr("en='An error occurred when sending the message. Data area %1 does not match the current one (%2).';ru='Ошибка при отправке сообщения. Область данных %1 не совпадает с текущей (%2).';vi='Lỗi khi gửi thông điệp. Vùng dữ liệu %1 không trùng với vùng hiện tại (%2).'");
...
Procedure OnMessageSending(MessageChannel, MessageBody, MessageObject) Export
	...
	If BodyContainTypedMessage(MessageBody, Message) Then
		...
		WriteLogEvent(NStr("en='SaaS messages.Send';ru='Сообщения в модели сервиса.Отправка';vi='Thông báo trong mô hình dịch vụ đám mây.Gửi'",
			CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Information,
			,
			,
...
Procedure OnMessageReceiving(MessageChannel, MessageBody, MessageObject) Export
	...
	If BodyContainTypedMessage(MessageBody, Message) Then
		...
		WriteLogEvent(NStr("en='SaaS messages.Receive';ru='Сообщения в модели сервиса.Получение';vi='Thông báo trong mô hình dịch vụ đám mây.Nhận'",
			CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Information,
			,
			,
...
Function MessagePresentationForLog(Val Message)
	...
	Pattern = NStr("en='Channel: %1';ru='Канал: %1';vi='Kênh: %1'", CommonUseClientServer.MainLanguageCode());
	...
	Pattern = NStr("en='Title: %1';ru='Заголовок: %1';vi='Tiêu đề: %1'", CommonUseClientServer.MainLanguageCode());
	Presentation = Presentation + Chars.LF + StringFunctionsClientServer.SubstituteParametersInString(Pattern, 
		Record.Close());
		
	If MessagesSaaSReUse.TypeAreaBody().IsDescendant(Message.Body.Type()) Then
		Pattern = NStr("en='Data area: %1';ru='Область данных: %1';vi='Vùng dữ liệu: %1'", CommonUseClientServer.MainLanguageCode());
		Presentation = Presentation + Chars.LF + StringFunctionsClientServer.SubstituteParametersInString(Pattern, 
...
Procedure BroadcastMessageToCorrespondentIfNecessary(Message, Val InformationConnection, Val RecipientPresentation) Export
	...
	If InterfaceMessages = Undefined Then
		Raise NStr("en='Cannot define an interface of the message being sent: interface handler is not registered for any type used in the message.';ru='Не удалось определить интерфейс отправляемого сообщения: ни для одного из типов, используемых в сообщении, не зарегистрирован обработчик интерфейса!';vi='Không thể xác định giao diện thông báo đã gửi: chưa ghi nhận bất cứ hàm sự kiện giao diện nào cho một trong số các kiểu được sử dụng trong thông báo!'");
	EndIf;
	
	If Not InformationConnection.Property("URL") 
			Or Not ValueIsFilled(InformationConnection.URL) Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='URL of the service of message exchange with infobase %1 is not specified';ru='Не задан URL сервиса обмена сообщениями с информационной базой %1';vi='Chưa chỉ ra URL công cụ trao đổi thông báo với cơ sở thông tin %1'"), RecipientPresentation);
	EndIf;
	...
	If CorrespondentVersion = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Correspondent %1 does not support receiving versions of interface %2 messages supported by the current infobase.';ru='Корреспондент %1 не поддерживает получение версий сообщений интерфейса %2, поддерживаемых текущей информационной базой!';vi='Nơi trao đổi tin %1 chưa hỗ trợ nhận phiên bản thông báo giao diện %2 mà được hỗ trợ bởi cơ sở thông tin hiện tại!'"),
