...
Function CryptoManager(Operation, ShowError = True, ErrorDescription = "", Application = Undefined) Export
	...
	If Operation = "Signing" Then
		ErrorTitle = NStr("en='Cannot sign data on server %1 due to:';ru='Не удалось подписать данные на сервере %1 по причине:';vi='Không thể ký dữ liệu trên Server %1 do nguyên nhân:'");
		
	ElsIf Operation = "SignatureCheck" Then
		ErrorTitle = NStr("en='Cannot check the signature on server %1 due to:';ru='Не удалось проверить подпись на сервере %1 по причине:';vi='Không thể kiểm tra chữ ký trên Server %1 do nguyên nhân:'");
		
	ElsIf Operation = "Encryption" Then
		ErrorTitle = NStr("en='Cannot encrypt data on server %1 due to:';ru='Не удалось зашифровать данные на сервере %1 по причине:';vi='Không thể mã hóa dữ liệu trên Server %1 do nguyên nhân:'");
		
	ElsIf Operation = "Details" Then
		ErrorTitle = NStr("en='Cannot decrypt data on server %1 due to:';ru='Не удалось расшифровать данные на сервере %1 по причине:';vi='Không thể giải mã dữ liệu trên Server %1 bởi vì:'");
		
	ElsIf Operation = "CertificateCheck" Then
		ErrorTitle = NStr("en='Cannot check the certificate on server %1 due to:';ru='Не удалось проверить сертификат на сервере %1 по причине:';vi='Không thể kiểm tra chứng thư trên Server %1 do nguyên nhân:'");
		
	ElsIf Operation = "GetCertificates" Then
		ErrorTitle = NStr("en='Cannot receive certificates on server %1 due to:';ru='Не удалось получить сертификаты на сервере %1 по причине:';vi='Không thể nhận chứng thư trên Server %1 do nguyên nhân:'");
...
"Sai giá trị tham số Giao dịch ""%1"".'"), Operation);
	...
	Else
		ErrorTitle = NStr("en='Cannot execute the operation on server %1 due to:';ru='Не удалось выполнить операцию на сервере %1 по причине:';vi='Không thể thực hiện thao tác trên Server %1 vì lý do:'");
	EndIf;
	...
	ElsIf ValueIsFilled(Application) Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Application %1 is not available or  installed.';ru='Программа %1 не доступна или не установлена.';vi='Chương trình %1 không khả dụng hoặc không được cài đặt.'"), Application);
	Else
		ErrorText = NStr("en='None of the applications are available or installed.';ru='Ни одна из программ не доступна или не установлена.';vi='Không có chương trình nào khả dụng hoặc được cài đặt.'");
	EndIf;
	...
	If Not Users.InfobaseUserWithFullAccess(,, False) Then
		ErrorText = ErrorText + Chars.LF + Chars.LF
			+ NStr("en='Contact administrator.';ru='Обратитесь к администратору.';vi='Hãy liên hệ với người quản trị.'");
	EndIf;
...
"';vi='Không tìm thấy"
"chứng thư trên Server do nguyên nhân: %1'"),
			ErrorPresentation);
	Else
		ErrorText = NStr("en='Certificate is not found on the server.';ru='Сертификат не найден на сервере.';vi='Không tìm thấy chứng thư trên Server.'");
	EndIf;
		
	If Not Users.InfobaseUserWithFullAccess(,, False) Then
		ErrorText = ErrorText + Chars.LF + NStr("en='Contact administrator.';ru='Обратитесь к администратору.';vi='Hãy liên hệ với người quản trị.'")
	EndIf;
...
Procedure CheckPresentationUniqueness(Presentation, CertificatRef, Field, Cancel) Export
	...
	If Not Query.Execute().IsEmpty() Then
		MessageText = NStr("en='Certificate with such presentation already exists';ru='Сертификат с таким представлением уже существует.';vi='Đã tồn tại chứng thư với cách trình bày như thế.'");
...
"базы возникла ошибка: %2';vi='Khi nhận"
"dữ liệu chứng thư ""%1"" từ cơ sở"
"thông tin, đã xảy ra lỗi: %2'"),
			Certificate,
