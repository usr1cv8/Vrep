...
Function ConnectEquipment(ClientID, EETypes = Undefined,
	...
	If Not Result Then
		ErrorDescription = NStr("en='First, you need to select the work place of the current session peripherals.';ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.';vi='Trước tiên, cần chọn chỗ làm việc của thiết bị ngoài trong phiên làm việc hiện tại.'");
		Return False;
	EndIf;
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.';vi='%Description%: Không thể kết nhập Driver thiết bị."
"Hãy kiểm tra Driver có được cài đặt và ghi nhận chính xác vào hệ thống hay chưa.'");
				...
				If DriverHandler = Undefined Then
					// Error message prompting that the driver can not be imported.
					ErrorDescription = ErrorDescription +  NStr("en='Cannot connect the driver handler.';ru='Не удалось подключить обработчик драйвера.';vi='Không thể kết nối hàm sự kiện của Driver.'");
					FinalResult = False;
					Continue;
				Else
				...
				Else
					...
					ErrorDescription = ErrorDescription + ?(IsBlankString(ErrorDescription), "", Chars.LF)
								   + NStr("en='Cannot connect the ""%Description%"" device: %ErrorDescription% (%ErrorCode%)';ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';vi='Không thể kết nối thiết bị ""%Description%"": %ErrorDescription% (%ErrorCode%)'");
					ErrorDescription = StrReplace(ErrorDescription, "%Description%"  , Device.Description);
...
"Укажите другое устройство.';vi='Thiết bị đã chọn không thể sử dụng được để kết nối."
"Hãy chỉ ra thiết bị khác.'");
		FinalResult = False;
	EndIf;
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.';vi='%Description%: Không thể kết nhập khiển thiết bị."
"Hãy kiểm tra xem khiển đã cài đặt và ghi nhận trong hệ thống có chính xác không.'");
		...
		If Not Result Then
			ErrorDescription = NStr("en='An error occurred when disconnecting the ""%Description%"" device: %ErrorDescription% (%ErrorCode%)';ru='При отключении устройства ""%Наименование%"" произошла ошибка: %ОписаниеОшибки% (%КодОшибки%)';vi='Khi ngắt bỏ thiết bị ""%Description%"" xảy ra lỗi: %ErrorDescription% (%ErrorCode%)'");
			ErrorDescription = StrReplace(ErrorDescription, "%Description%", ConnectedDevice.Description);
			ErrorDescription = StrReplace(ErrorDescription, "%ErrorDescription%", Output_Parameters[1]);
			ErrorDescription = StrReplace(ErrorDescription, "%ErrorCode%", Output_Parameters[0]);
		Else
...
"Hãy kiểm tra xem khiển đã cài đặt và ghi nhận trong hệ thống có chính xác không.'");
					...
					If Not Result Then
						ErrorDescription = ErrorDescription + ?(IsBlankString(ErrorDescription), "", Chars.LF)
									   + NStr("en='An error occurred when disconnecting the ""%Description%"" device: %ErrorDescription% (%ErrorCode%)';ru='При отключении устройства ""%Наименование%"" произошла ошибка: %ОписаниеОшибки% (%КодОшибки%)';vi='Khi ngắt bỏ thiết bị ""%Description%"" xảy ra lỗi: %ErrorDescription% (%ErrorCode%)'");
						ErrorDescription = StrReplace(ErrorDescription, "%Description%", ConnectedDevice.Description);
						ErrorDescription = StrReplace(ErrorDescription, "%ErrorDescription%", Output_Parameters[1]);
						ErrorDescription = StrReplace(ErrorDescription, "%ErrorCode%", Output_Parameters[0]);
					Else
...
Procedure StartEnableEquipmentEnd(ExecutionResult, Parameters) Export
	...
	If ExecutionResult.Result Then
		...
		If Parameters.AlertOnConnect <> Undefined Then
			ErrorDescription = NStr("en='No errors.';ru='Ошибок нет.';vi='Không bị lỗi.'");
	...
	Else
		// Inform user that a peripheral failed to be connected.
		If Parameters.AlertOnConnect <> Undefined Then
			ErrorDescription = NStr("en='Cannot connect the ""%Description%"" device: %ErrorDescription% (%ErrorCode%)';ru='Не удалось подключить устройство ""%Наименование%"": %ОписаниеОшибки% (%КодОшибки%)';vi='Không thể kết nối thiết bị ""%Description%"": %ErrorDescription% (%ErrorCode%)'");
		...
		For Each Device IN EquipmentList Do
			...
			If ConnectedDevice = Undefined Then // If device is not enabled earlier.
				...
				If DriverHandler = Undefined Then
					...
					If AlertOnConnect <> Undefined Then
						ErrorDescription = NStr("en='Cannot connect the driver handler.';ru='Не удалось подключить обработчик драйвера.';vi='Không thể kết nối hàm sự kiện của Driver.'");
						ExecutionResult = New Structure("Result, ErrorDetails", False, ErrorDescription);
						ExecuteNotifyProcessing(AlertOnConnect, ExecutionResult);
					EndIf;
					Continue;
...
"Hãy kiểm tra Driver có được cài đặt và ghi nhận chính xác vào hệ thống hay chưa.'");
	...
	ElsIf  DeviceIdentifier <> Undefined AND AlertOnConnect <> Undefined Then
		ErrorDescription =  NStr("en='The selected peripheral can not be used for connection. Specify other device.';ru='Выбранное устройство не может использоваться для подключения. Укажите другое устройство.';vi='Thiết bị đã chọn không thể sử dụng để kết nối. Hãy chỉ ra thiết bị khác.'");
		ExecutionResult = New Structure("Result, ErrorDetails", Result, ErrorDescription);
		ExecuteNotifyProcessing(AlertOnConnect, ExecutionResult);
	EndIf;
...
"Check if the driver is correctly installed and registered in the system.';ru='%Description%: Не удалось загрузить драйвер устройства."
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.';vi='%Description%: Không thể kết nhập Driver thiết bị."
"Hãy kiểm tra Driver đã được cài đặt và ghi nhận vào chương trình chính xác hay chưa.'");
			ErrorDescription = StrReplace(ErrorDescription, "%Description%", ConnectedDevice.Description);
		...
		Else
			...
			If DriverHandler = Undefined Then
				// Error message prompting that the driver can not be imported.
				Output_Parameters = New Array();
				MessageText = NStr("en='Cannot connect the driver handler.';ru='Не удалось подключить обработчик драйвера.';vi='Không thể kết nối hàm sự kiện của Driver.'");
				Output_Parameters.Add(999);
				Output_Parameters.Add(MessageText);
				Output_Parameters.Add(NStr("en='Not set';ru='Не установлен';vi='Chưa thiết lập'"));
			Else
				// Forcefully replace asynchronous handler to the synchronous one as you work in the synchronous mode.
				If DriverHandler = PeripheralsUniversalDriverClientAsynchronously Then
					DriverHandler = PeripheralsUniversalDriverClient;
	...
	Else
		...
		Output_Parameters = New Array();
		MessageText = NStr("en='Device is not connected. Before performing the operation the device should be connected.';ru='Устройство не подключено. Перед выполнением операции устройство должно быть подключено.';vi='Chưa kết nối thiết bị. Trước khi thực hiện giao dịch, cần kết nối thiết bị.'");
		Output_Parameters.Add(999);
		Output_Parameters.Add(MessageText);
	EndIf;
...
"Проверьте, что драйвер корректно установлен и зарегистрирован в системе.';vi='Không thể kết nhập khiển của thiết bị."
"Hãy kiểm tra xem khiển đã cài đặt và ghi nhận trong hệ thống chính xác chưa.'");
			Output_Parameters.Add(999);
			Output_Parameters.Add(MessageText);
			Output_Parameters.Add(NStr("en='Not set';ru='Не установлен';vi='Chưa thiết lập'"));
	...
	Else
		...
		Output_Parameters = New Array();
		MessageText = NStr("en='The device is connected. Before you start, disconnect the device.';ru='Устройство подключено. Перед выполнением операции устройство должно быть отключено.';vi='Thiết bị đã được kết nối. Trước khi thực hiện giao dịch, cần ngắt thiết bị.'");
		Output_Parameters.Add(999);
		Output_Parameters.Add(MessageText);
		Output_Parameters.Add(NStr("en='Set';ru='Установлен';vi='Đã cài đặt'"));
	EndIf;
...
"Hãy kiểm tra xem khiển đã cài đặt và ghi nhận trong hệ thống chính xác chưa.'");
	...
	Else
		...
		If DriverHandler = Undefined Then
			...
			ErrorText = NStr("en='Cannot connect the driver handler.';ru='Не удалось подключить обработчик драйвера.';vi='Không thể kết nối hàm sự kiện của Driver.'");
			Output_Parameters = New Array();
			Output_Parameters.Add(999);
			Output_Parameters.Add(ErrorText);
			Output_Parameters.Add(NStr("en='Not set';ru='Не установлен';vi='Chưa thiết lập'"));
...
Procedure StartExecuteAdditionalCommand(AlertOnEnd, Command, InputParameters, ID, Parameters) Export
	...
	Else
		// Report an error saying that the device is enabled.
		ErrorText = NStr("en='The device is connected. Before you start, disconnect the device.';ru='Устройство подключено. Перед выполнением операции устройство должно быть отключено.';vi='Thiết bị đã được kết nối. Trước khi thực hiện giao dịch, cần ngắt thiết bị.'");
		Output_Parameters = New Array();
		Output_Parameters.Add(999);
		Output_Parameters.Add(ErrorText);
		Output_Parameters.Add(NStr("en='Set';ru='Установлен';vi='Đã cài đặt'"));
...
Procedure ExecuteEquipmentSetup(ID) Export
	...
	Else
		CommonUseClientServer.MessageToUser(NStr("en='An error occurred when initializing the driver setting form.';ru='Произошла ошибка инициализации формы настройки драйвера.';vi='Đã xảy ra lỗi khởi tạo biểu mẫu tùy chỉnh Driver.'")); 
	EndIf;
...
Procedure ExecuteEquipmentSettingEnd(Result, Parameters) Export
	...
	If TypeOf(Result) = Type("Structure") Then
		...
		Else
			ErrorInfo = NStr("en='Cannot save the device parameters.';ru='Не удалось сохранить параметры устройства.';vi='Không thể lưu tham số thiết bị.'");
...
Procedure BeginEnableExtensionFileOperationsEnd(Attached, AdditionalParameters) Export
	...
	If Not Attached AND AdditionalParameters.OfferInstallation Then
		...
		MessageText = NStr("en='To continue, you need to install an extension for 1C:Enterprise web client. Install?';ru='Для продолжении работы необходимо установить расширение для веб-клиента ""1С:Предприятие"". Установить?';vi='Để tiếp tục làm việc, cần thiết lập phần mở rộng cho Web-client ""1C:DOANH NGHIỆP"". Cài đặt?'");
...
Procedure StartDriverFileSelection(AlertOnSelection) Export 
	...
	FileOpeningDialog.Title = NStr("en='Select a driver file';ru='Выберите файл драйвера';vi='Hãy chọn tệp Driver'");
	FileOpeningDialog.Filter = NStr("en='Driver file';ru='Файл драйвера';vi='Tệp Driver'") + ?(EquipmentManagerClientReUse.IsLinuxClient(), "(*.zip)|*.zip", "(*.zip, *.exe)| *.zip; *.exe");  
...
Procedure OfferSelectDevice(SelectionNotification, EquipmentType, HeaderTextSelect, 
	...
	If Not RefreshClientWorkplace() Then
		MessageText = NStr("en='First, you need to select the work place of the current session peripherals.';ru='Предварительно необходимо выбрать рабочее место подключаемого оборудования текущего сеанса.';vi='Trước tiên, cần chọn chỗ làm việc của thiết bị ngoài trong phiên làm việc hiện tại.'");
...
Procedure StartWeightReceivingFromElectronicScales(AlertOnGetWeight, UUID) Export
	...
	OfferSelectDevice(NOTifyDescription, "ElectronicScales",
		NStr("en='Select electronic scales';ru='Выберите электронные весы';vi='Hãy chọn cân điện tử'"), NStr("en='Electronic scales are not connected.';ru='Электронные весы не подключены.';vi='Cân điện tử chưa kết nối.'"), NStr("en='Electronic scales are not selected.';ru='Электронные весы не выбраны.';vi='Cân điện tử chưa được chọn.'"));
...
"Additional description: |%Additionaldescription%';ru='При использовании электронных весов произошла ошибка."
"Дополнительное описание: |%ДополнительноеОписание%';vi='Khi sử dụng cân điện tử, đã xảy ra lỗi."
"Mô tả bổ sung: |%Additionaldescription%'");
...
"Дополнительное описание: %ДополнительноеОписание%';vi='Khi kết nối cân điện tử đã xảy ra lỗi."
"Mô tả bổ sung: %ДополнительноеОписание%'");
...
Procedure StartDataExportVTSD(AlertOnDataExport, UUID, ProductsExportTable) Export
	...
	OfferSelectDevice(NOTifyDescription, "DataCollectionTerminal",
		NStr("en='Select data collection terminal';ru='Выберите терминал сбора данных';vi='Hãy chọn thiết bị thu thập dữ liệu'"), NStr("en='Data collection terminal is not connected.';ru='Терминал сбора данных не подключен.';vi='Chưa kết nối thiết bị thu thập dữ liệu.'"));
...
"%ОписаниеОшибки%"
"Данные в терминал сбора данных не выгружены.';vi='Khi kết xuất dữ liệu ra thiết bị thu thập dữ liệu đã xảy ra lỗi."
"%ErrorDescription%"
"Dữ liệu chưa được kết xuất ra thiết bị thu thập dữ liệu.'");
...
"%ОписаниеОшибки%"
"Данные в терминал сбора данных не выгружены.';vi='Khi kết nối thiết bị đã xảy ra lỗi."
"%ОписаниеОшибки%"
"Dữ liệu chưa được kết xuất ra máy thu thập dữ liệu.'");
		MessageText = StrReplace(MessageText, "%ErrorDescription%", ErrorDescription);
		CommonUseClientServer.MessageToUser(MessageText);
	EndIf;
...
"%ОписаниеОшибки%"
"Данные из терминала сбора данных не загружены.';vi='Khi kết nhập dữ liệu từ thiết bị thu thập dữ liệu xảy ra lỗi."
"%ErrorDetails%"
"Dữ liệu từ thiết bị thu thập dữ liệu chưa kết nhập được.'");
			MessageText = StrReplace(MessageText, "%ErrorDescription%", Output_Parameters[1]);
			CommonUseClientServer.MessageToUser(MessageText);
		EndIf;
...
"%ОписаниеОшибки%"
"Данные из терминала сбора данных не загружены.';vi='Khi kết nối thiết bị đã xảy ra lỗi."
"%ОписаниеОшибки%"
"Dữ liệu từ máy thu thập dữ liệu chưa được kết nhập.'");
		MessageText = StrReplace(MessageText, "%ErrorDescription%", ErrorDescription);
		CommonUseClientServer.MessageToUser(MessageText);
	EndIf;
...
"%ОписаниеОшибки%';vi='Khi xóa dữ liệu trong thiết bị thu thập dữ liệu đã xảy ra lỗi."
"%ОписаниеОшибки%'");
			MessageText = StrReplace(MessageText, "%ErrorDescription%", Output_Parameters[1]);
			CommonUseClientServer.MessageToUser(MessageText);
		Else
...
Procedure StartEnablePOSTerminal(AlertOnEnd, UUID, POSTerminal = Undefined) Export
	...
	Else
		...
		OfferSelectDevice(NOTifyDescription, "POSTerminal",
			NStr("en='Select a POS terminal';ru='Выберите эквайринговый терминал';vi='Hãy chọn thiết bị thanh toán thẻ'"), NStr("en='POS terminal is not connected.';ru='Эквайринговый терминал не подключен.';vi='Thiết bị thanh toán thẻ chưa kết nối.'"));
	EndIf;
...
"Операция не была проведена.';vi='Khi kết nối thiết bị"
"thanh toán thẻ, đã xảy ra lỗi: ""%ErrorDescription%""."
	...
	Else
		...
		Else
			...
			OfferSelectDevice(NOTifyDescription, "FiscalRegister",
					NStr("en='Select a fiscal data recorder to print POS receipts.';ru='Выберите фискальный регистратор для печати эквайринговых чеков';vi='Hãy chọn máy ghi nhận tiền mặt để in phiếu thanh toán qua thẻ'"), NStr("en='Fiscal data recorder for printing acquiring receipts is not connected.';ru='Фискальный регистратор для печати эквайринговых чеков не подключен.';vi='Chưa kết nối máy ghi nhận tiền mặt để in phiếu thanh toán thẻ.'"));
		EndIf;
...
"""%ОписаниеОшибки%""."
"Операция по карте не была выполнена.';vi='Khi kết nối máy ghi nhận tiền mặt, đã xảy ra lỗi:"
"""%ErrorDescription%""."
"Chưa thực hiện giao dịch với thẻ.'");
		MessageText = StrReplace(MessageText, "%ErrorDescription%", ErrorDescription);
		CommonUseClientServer.MessageToUser(MessageText);
...
Procedure ExecuteTotalsRevisionPOSTerminalEnd(Result, Parameters) Export
	...
	If Not TypeOf(Result) = Type("Structure") Then
		MessageText = NStr("en='An error occurred while executing the operation.';ru='При выполнении операции произошла ошибка.';vi='Khi thực hiện giao dịch, đã xảy ra lỗi.'");
		CommonUseClientServer.MessageToUser(MessageText);
		Return;
	EndIf;
...
"""%ОписаниеОшибки%""."
"Отмена по карте не была произведена.';vi='Khi thực hiện giao dịch, đã xảy ra lỗi:"
"""%ErrorDescription%""."
"Chưa hủy giao dịch theo thẻ.'");
		MessageText = StrReplace(MessageText, "%ErrorDescription%", Output_Parameters[1]);
		CommonUseClientServer.MessageToUser(MessageText);
	Else
...
"возникла ошибка: ""%ОписаниеОшибки%"".';vi='Khi in phiếu tính tiền,"
"đã xảy ra lỗi: ""%ErrorDescription%"".'");
					MessageText = StrReplace(MessageText, "%ErrorDescription%", Output_Parameters[1]);
					CommonUseClientServer.MessageToUser(MessageText);
				EndIf;
			Else
				MessageText = NStr("en='Totals reconciliation successfully completed.';ru='Операция сверки итогов успешно выполнена.';vi='Đã thực hiện thành công giao dịch đối chiếu tổng kết.'");
				CommonUseClientServer.MessageToUser(MessageText);
			EndIf;
		EndIf;
...
Procedure StartDataExportToScalesWithLabelsPrinting(AlertOnDataExport, ClientID, DeviceIdentifier = Undefined, ProductsExportTable, PartialExport = False) Export
	...
	If ProductsExportTable.Count() = 0 Then
		MessageText = NStr("en='There is no data to export.';ru='Нет данных для выгрузки!';vi='Không có dữ liệu để kết xuất!'");
		CommonUseClientServer.MessageToUser(MessageText);
		Return;
	EndIf;
	...
	If DeviceIdentifier = Undefined Then
		...
		OfferSelectDevice(NOTifyDescription, "LabelsPrintingScales",
			NStr("en='Select label printing scales';ru='Выберите весы с печатью этикеток';vi='Hãy chọn cân có in nhãn mác'"), NStr("en='Label printing scales are not connected.';ru='Весы с печатью этикеток не подключены.';vi='Không kết nối với cân có in nhãn tem.'"));
	Else
...
Procedure StartDataExportToScalesWithLabelsPrintingFileExtensionEnd(Attached, Parameters) Export
	...
	If Not Attached Then
		MessageText = NStr("en='The operation is not available without extension for 1C:Enterprise web client installed.';ru='Данная операция не доступна без установленного расширения для веб-клиента ""1С:Предприятие"".';vi=""Giao dịch này không khả dụng mà chưa cài đặt phần mở rộng đối với Web-client ''1C:DOANH NGHIỆP''.""");
		CommonUseClientServer.MessageToUser(MessageText);
		If Parameters.NextAlert <> Undefined Then
...
"%ОписаниеОшибки%"
"Данные не выгружены.';vi='Khi kết xuất dữ liệu vào thiết bị đã xảy ra lỗi."
"%ErrorDescription%"
"Chưa kết xuất dữ liệu.'");
			MessageText = StrReplace(MessageText, "%ErrorDescription%", Output_Parameters[1]);
			CommonUseClientServer.MessageToUser(MessageText);
		Else
			If Parameters.NextAlert <> Undefined Then
				ExecuteNotifyProcessing(Parameters.NextAlert, True);
			Else
				MessageText = NStr("en='Data downloaded successfully.';ru='Данные выгружены успешно.';vi='Đã kết xuất dữ liệu thành công.'");
				CommonUseClientServer.MessageToUser(MessageText);
			EndIf;
		EndIf;
...
"%ОписаниеОшибки%"
"Данные не выгружены.';vi='Khi kết nối thiết bị đã xảy ra lỗi."
"%ОписаниеОшибки%'");
		MessageText = StrReplace(MessageText, "%ErrorDescription%", ErrorDescription);
		CommonUseClientServer.MessageToUser(MessageText);
	EndIf;
...
"%ОписаниеОшибки%';vi='Khi xóa dữ liệu, trong thiết bị đã xảy ra lỗi."
"%ОписаниеОшибки%'");
			MessageText = StrReplace(MessageText, "%ErrorDescription%", Output_Parameters[1]);
		...
		Else
			If Parameters.NextAlert <> Undefined Then
				ExecuteNotifyProcessing(Parameters.NextAlert, True);
			Else
				MessageText = NStr("en='Data is successfully cleared.';ru='Очистка данных успешно завершена.';vi='Xóa dữ liệu thành công.'");
				CommonUseClientServer.MessageToUser(MessageText);
			EndIf;
		EndIf;
...
"%ОписаниеОшибки%';vi='Khi kết nối thiết bị đã xảy ra lỗi."
"%ОписаниеОшибки%'");
		MessageText = StrReplace(MessageText, "%ErrorDescription%", ErrorDescription);
		CommonUseClientServer.MessageToUser(MessageText);
	EndIf;
...
Procedure StartProductsCleaningInCROffline(AlertWhenClearingData, UUID, DeviceIdentifier = Undefined) Export
	...
	If DeviceIdentifier = Undefined Then
		...
		OfferSelectDevice(NOTifyDescription, "CashRegistersOffline",
			NStr("en='Select offline CR';ru='Выберите ККМ Offline';vi='Hãy chọn máy tính tiền Offline'"), NStr("en='Offline cash registers are not connected.';ru='ККМ Offline не подключены.';vi='Không kết nối MTT Offline.'"));
	Else
...
Procedure StartCleaningProductsToCROfflineFileExtensionEnd(Attached, Parameters) Export
	...
	If Result Then
		...
		Status(NStr("en='Clearing goods in offline cash register...';ru='Выполняется очистка товаров в ККМ Offline...';vi='Đang xóa hàng hóa trong máy tính tiền Offline...'"));
...
Procedure StartDataExportToCROfflineFileExtensionEnd(Attached, Parameters) Export
	...
	If Result Then
		...
		Status(NStr("en='Goods are being exported to offline CR...';ru='Выполняется выгрузка товаров в ККМ Offline...';vi='Đang thực hiện kết xuất hàng hóa vào máy tính tiền Offline...'")); 
...
Procedure StartImportRetailSalesReportFromCROfflineFileExtensionEnd(Attached, Parameters) Export
	...
	If Result Then
		Status(NStr("en='Importing goods from offline cash register...';ru='Выполняется загрузка товаров из ККМ Offline...';vi='Đang thực hiện kết nhập hàng hóa từ máy tính tiền Offline...'"));
...
"%ОписаниеОшибки%';vi='Khi kết nhập dữ liệu từ MTT Offline, đã xảy ra lỗi."
"%ErrorDescription%'");
			MessageText = StrReplace(MessageText, "%ErrorDescription%", Output_Parameters[1]);
			CommonUseClientServer.MessageToUser(MessageText);
		Else
...
Function CodeCorrespondsToMCTemplate(TracksData, PatternData) Export
	...
	For Iterator = 1 To 3 Do
		If PatternData["TrackAvailability"+String(Iterator)] Then
			...
			If Right(curRow, StrLen(PatternData["Suffix" + String(Iterator)])) <> PatternData["Suffix" + String(Iterator)] Then
				CommonUseClientServer.MessageToUser(NStr("en='Track';ru='Дорожка';vi='Đường dân'") + Chars.NBSp + String(Iterator) 
					+ ". "+NStr("en='Card suffix does not correspond to the template suffix.';ru='Суффикс карты не соответствует суффиксу шаблона.';vi='Hậu tố thẻ không tương ứng với hậu tố khuôn mẫu.'"));
				CheckPassed = False;
			EndIf;
			
			If Left(curRow, StrLen(PatternData["Prefix" + String(Iterator)])) <> PatternData["Prefix" + String(Iterator)] Then
				CommonUseClientServer.MessageToUser(NStr("en='Track';ru='Дорожка';vi='Đường dân'") + Chars.NBSp + String(Iterator) 
					+ ". " + NStr("en='Card prefix does not correspond to the template prefix.';ru='Префикс карты не соответствует префиксу шаблона.';vi='Tiền tố của thẻ không tương ứng với tiền tố khuôn mẫu.'"));
				CheckPassed = False;
			EndIf;
			
			If Find(curRow, PatternData["BlocksDelimiter"+String(Iterator)]) = 0 Then
				CommonUseClientServer.MessageToUser(NStr("en='Track';ru='Дорожка';vi='Đường dân'") + Chars.NBSp + String(Iterator) 
					+ ". "+NStr("en='Card block separator does not correspond to template block separator.';ru='Разделитель блоков карты не соответствует разделителю блоков шаблона.';vi='Đường chia tách khối thẻ không tương ứng với đường chia tách khối khuôn mẫu.'"));
				CheckPassed = False;
			EndIf;
				
			If StrLen(curRow) <> PatternData["CodeLength"+String(Iterator)] Then
				CommonUseClientServer.MessageToUser(NStr("en='Track';ru='Дорожка';vi='Đường dân'") + Chars.NBSp + String(Iterator) 
					+ ". " + NStr("en='Card code length does not correspond to the template code length.';ru='Длина кода карты не соответствует длине кода шаблона.';vi='Chiều dài của mã thẻ không tương ứng với chiều dài của mã khuôn mẫu.'"));
				CheckPassed = False;
			EndIf;
	...
	If OneTrackExist Then 
		Return True;
	Else
		CommonUseClientServer.MessageToUser(NStr("en='No available track is specified in the template.';ru='В шаблоне не указано ни одной доступной дорожки.';vi='Trong khuôn mẫu chưa chỉ ra bất kỳ một đường dẫn hữu dụng nào.'"));
		Return False;
	EndIf;
...
"Файл ""%Файл%"" в макете не найден.';vi='Lỗi cài đặt Driver từ phân phối trong khuôn in."
"Không tìm thấy tệp ""%File%"" trong khuôn in.'");
...
"Необходимый файл в архиве не найден.';vi='Lỗi cài đặt Driver từ bản đóng gói được lưu trữ."
"Không tìm thấy tệp cần thiết trong lưu trữ.'");
			CommonUseClientServer.MessageToUser(ErrorText); 
...
Procedure SetupDriver(ID, AlertFromDistributionOnEnd = Undefined, AlertFromArchiveOnEnd = Undefined) Export
	...
	Try
		If DriverData.AsConfigurationPart Then
			...
			If DriverData.SuppliedAsDistribution Then
				#If WebClient Or MobileClient Then
					CommonUseClientServer.MessageToUser(NStr("en='This driver is not supported in the web client.';ru='Данный драйвер не поддерживает работу в веб-клиенте.';vi='Driver này không hỗ trợ làm việc trong Web-client.'")); 
				#Else
					If EquipmentManagerClientReUse.IsLinuxClient() Then
						CommonUseClientServer.MessageToUser(NStr("en='The driver cannot be installed and used in the Linux environment.';ru='Данный драйвер не может быть установлен и использован в среде Linux.';vi='Không thể cài đặt và sử dụng Driver này trong môi trường Linux.'")); 
						Return;
					EndIf;
	...
	Except
		CommonUseClientServer.MessageToUser(NStr("en='An error occurred while installing the driver.';ru='Произошла ошибка при установке драйвера.';vi='Đã xảy ra lỗi khi cài đặt Driver.'")); 
	EndTry;  
