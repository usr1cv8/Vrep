				...
				If ModuleSaaSOperations.DataAreaBlocked(DataArea) Then
					...
					Else
						Raise StringFunctionsClientServer.SubstituteParametersInString(
							NStr("en='Cannot perform an email exchange due to: data area %1 is locked.';ru='Не удалось выполнить обмен сообщениями по причине: область данных %1 заблокирована!';vi='Không thể thực hiện trao đổi thông điệp theo nguyên nhân: vùng dữ liệu %1 đã bị phong tỏa!'"),
							DataArea);
					EndIf;
				EndIf;
...
"в элементах со значениями разделителей, совпадающих со значениями разделителей сеанса.';vi='Đã khởi động bộ xử lý sắp xếp thông báo hệ thống"
"từ phiên làm việc với giá trị đường chia tách đã đặt. Bộ xử lý sẽ chỉ làm việc"
"với các thông báo đã lưu trong danh mục được chia tách,"
"trong các phần tử với giá trị đường chia tách trùng với giá trị đường chia tách của phiên làm việc.'")
		);
		...
		Try
			...
			Try
				...
				If Selection.EndPointLocked Then
					...
					Raise NStr("en='Attempting to process the message received from the locked endpoint.';ru='Попытка обработки сообщения, полученного от заблокированной конечной точки.';vi='Thử xử lý thông báo đã nhận từ điểm cuối đã khóa.'");
				EndIf;
				
				If FoundStrings.Count() = 0 Then
					MessageObject.Locked = True;
					Raise NStr("en='Message handler is not set.';ru='Не назначен обработчик для сообщения.';vi='Chưa ấn định hàm sự kiện đối với thông báo.'");
				EndIf;
				...
				For Each TableRow IN FoundStrings Do
					...
					If TransactionActive() Then
						...
						EndDo;
						MessageObject.Locked = True;
						Raise NStr("en='The transaction was not recorded in the message handler.';ru='В обработчике сообщения не была зафиксирована транзакция.';vi='Trong bộ xử lý thông báo chưa ghi nhận giao tác.'");
					EndIf;
			...
			Except
				...
				WriteLogEvent(ThisSubsystemEventLogMonitorMessageText(),
						...
						StringFunctionsClientServer.SubstituteParametersInString(
							NStr("en='An error occurred when processing message %1: %2';ru='Ошибка обработки сообщения %1: %2';vi='Lỗi xử lý thông báo %1: %2'"),
			...
			If ProcessMessageInUndividedData AND CommonUseReUse.DataSeparationEnabled() AND CommonUseReUse.CanUseSeparatedData() Then
				...
				ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='After data processor of %1 channel message session separation was not disabled.';ru='После обработки сообщения канала %1 не было выключено разделение сеанса!';vi='Sau khi xử lý thông điệp của kênh %1, không thể ngắt phân chia phiên làm việc!'"),
...
Procedure SetLeadingEndPointAtRecipient(ThisEndPointCode, LeadingEndPointCode) Export
	...
	If ExchangePlans.MessageExchange.FindByCode(ThisEndPointCode) <> ThisNode() Then
		ErrorMessageString = NStr("en='Incorrect parameters of connection to the end point were set. Connection parameters indicate another end point.';ru='Заданы неверные параметры подключения к конечной точке. Параметры подключения указывают на другую конечную точку.';vi='Chỉ ra sai tham số kết nối tới điểm cuối. Tham số kết nối chỉ ra cho điểm cuối khác.'");
...
"Параметры подключения указывают на другую конечную точку.';vi='Xác định sai tham số kết nối đến điểm cuối."
"Chỉ ra tham số kết nối cho điểm cuối khác.'", CommonUseClientServer.MainLanguageCode());
	...
	Try
		...
		If EndPointNode.IsEmpty() Then
			...
			Raise NStr("en='Endpoint is not found in the correspondent base.';ru='Конечная точка в базе-корреспонденте не обнаружена.';vi='Điểm cuối trong cơ sở đối ứng không tìm thấy.'");
	...
	Try
		...
		While CanReadXML(XMLReader) Do
			...
			If TypeOf(Data) = Type("ObjectDeletion") Then
				...
				Raise NStr("en='Passing of object ObjectDeletion via instant message tool is not supported.';ru='Передача объекта УдалениеОбъекта через механизм быстрых сообщений не поддерживается!';vi='Không hỗ trợ truyền đối tượng ObjectDeletion thông qua cơ chế thông báo nhanh!'");
...
Function EndPointConnectionEventLogMonitorMessageText() Export
	...
	Return NStr("en='Message exchange.Endpoint connection';ru='Обмен сообщениями.Подключение конечной точки';vi='Trao đổi bằng thông báo. Kết nối điểm cuối'", CommonUseClientServer.MainLanguageCode());
...
Function LeadingEndPointSettingEventLogMonitorMessageText() Export
	...
	Return NStr("en='Message exchange.Setting leading endpoint';ru='Обмен сообщениями.Установка ведущей конечной точки';vi='Trao đổi bằng thông báo.Đặt điểm cuối chính'", CommonUseClientServer.MainLanguageCode());
...
Function ThisSubsystemEventLogMonitorMessageText() Export
	...
	Return NStr("en='Message exchange';ru='Обмен сообщениями';vi='Trao đổi thông báo'", CommonUseClientServer.MainLanguageCode());
...
Procedure ConnectEndPointAtSender(Cancel,
	...
	If Not EndPointNode.IsEmpty() Then
		Cancel = True;
		ErrorMessageString = NStr("en='Endpoint is already connected to the infobase; point name: %1';ru='Конечная точка уже подключена к информационной базе; наименование точки: %1';vi='Điểm cuối đã kết nối với cơ sở thông tin; tên gọi của điểm: %1'", CommonUseClientServer.MainLanguageCode());
