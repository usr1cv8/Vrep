...
Procedure OnStart(Parameters) Export
	...
	If FixedParametersOfInfobaseBackup.RecoverHasBeenPerformed Then
		NotificationText = NStr("en='Data is restored successfully.';ru='Восстановление данных проведено успешно.';vi='Phục hồi dữ liệu đã thực hiện thành công.'");
		ShowUserNotification(NStr("en='Data is restored.';ru='Данные восстановлены.';vi='Dữ liệu đã được phục hồi.'"), , NotificationText);
	EndIf;
...
Procedure OnGetListOfWarningsToCompleteJobs(Warnings) Export
	...
	WarningParameters.FlagText = NStr("en='Back up';ru='Выполнить резервное копирование';vi='Thực hiện sao lưu dự phòng'");
	...
	If DeletionParameters.RestrictionType <> "StoreAll" AND StoringDirectory <> Undefined Then
		...
		Except
			...
			EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(), "Error",
				NStr("en='Cannot clear a directory with backups.';ru='Не удалось провести очистку каталога с резервными копиями.';vi='Không thể xóa thư mục có bản sao dự phòng.'") + Chars.LF 
...
Procedure CheckInfobaseBackup(Parameters)
	...
	Else
		...
		ShowUserNotification(NStr("en='Backup';ru='Резервное копирование';vi='Sao lưu dự phòng'"),
			"e1cib/command/CommonCommand.ShowBackupResult",
			NStr("en='Backup was successfully performed';ru='Резервное копирование проведено успешно';vi='Đã thực hiện sao lưu dự phòng thành công'"), PictureLib.Information32);
...
Procedure NotifyUserAboutBackingUp(VariantNotifications) Export
	...
	If VariantNotifications = "Overdue" Then
		...
		ExplanationText = NStr("en='Automatic backup was not executed.';ru='Автоматическое резервное копирование не было выполнено.';vi='Tự động sao lưu dự phòng không thực hiện được.'"); 
		ShowUserNotification(NStr("en='Backup';ru='Резервное копирование';vi='Sao lưu dự phòng'"),
	...
	ElsIf VariantNotifications = "YetNotConfigured" Then
		...
		ExplanationText = NStr("en='We recommend that you configure backup for the infobase.';ru='Рекомендуется настроить резервное копирование информационной базы.';vi='Nên tùy chỉnh sao lưu dự phòng cơ sở thông tin.'"); 
		ShowUserNotification(NStr("en='Backup';ru='Резервное копирование';vi='Sao lưu dự phòng'"),
...
Function EventLogMonitorEvent() Export
	...
	Return NStr("en='Infobase backup';ru='Резервное копирование информационной базы';vi='Sao lưu dự phòng cơ sở thông tin'", CommonUseClientServer.MainLanguageCode());
...
Function ClientParametersOfBackup() Export
	#If Not WebClient And Not MobileClient Then
		...
		ParametersStructure.Insert("EventLogMonitorEvent", NStr("en='Infobase backup';ru='Резервное копирование ИБ';vi='Sao lưu dự phòng CSTT'"));
