...
"т.к. не требуется обеспечивать сопоставление ссылок на его предопределенные элементы!';vi='Đối tượng metadata %1 không thể xử lý bởi hàm sự kiện"
"ВыгрузкаЗагрузкаПредопределенныхДанных.ПередВыгрузкойОбъекта(),"
"bởi vì không cần đảm bảo tính tương ứng của tham chiếu tới các phần tử định trước của đối tượng!'", Metadata.DefaultLanguage.LanguageCode),
...
"ВыгрузкаЗагрузкаПредопределенныхДанных.ПередВыгрузкойОбъекта(),"
"т.к. не может содержать предопределенных элементов!';vi='Hàm sự kiện PredefinedDataExportImport.BeforeObjectExport() "
"không thể xử lý đối tượng metadata %1 vì không thể chứa"
"các phần tử định trước!'", Metadata.DefaultLanguage.LanguageCode),
			Object.Metadata().FullName()
		);
			...
			If Not QueryResult.IsEmpty() Then
				...
				Else
					...
					Raise StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='Duplication of predefined items %1 is detected in table %2.';ru='Обнаружено дублирование предопределенных элементов %1 в таблице %2!';vi='Đã tìm thấy sự trùng lặp các phần tử định trước %1 trong bảng %2!'", Metadata.DefaultLanguage.LanguageCode),
