...
Function ObjectAttributesValues(Ref, Val Attributes) Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Invalid type of Attributes second parameter: %1';ru='Неверный тип второго параметра Реквизиты: %1';vi='Sai kiểu tham số thứ hai Mục tin: %1'"),
...
//
// Parameters:
//  References - Array, FixedArray - ссылки на объекты.
//                    Если массив пуст, то результатом будет пустое соответствие.
//  Attributes - String - имена реквизитов перечисленные через запятую, в формате требований к свойствам
//                       структуры. Например, "Код, Наименование, Родитель".
//            - Array, FixedArray - имена реквизитов в формате требований
//                       к свойствам структуры.
//  SelectAllowedItems - Boolean - если True, то запрос к объектам выполняется с учетом прав пользователя;
...
//
// Returns:
//  Map - список объектов и значений их реквизитов:
//   * Key - AnyRef - ссылка на объект;
//   * Value - Structure - значения реквизитов:
//    ** Key - String - имя реквизита;
//    ** Value - Arbitrary - значение реквизита.
// 
Function ObjectAttributeValues(References, Val Attributes, SelectAllowedItems = False) Export
	...
	If IsBlankString(Attributes) Then 
		Raise 
			NStr("en='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- Поле объекта должно быть указано';ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- Поле объекта должно быть указано';vi='Tham số thứ hai không hợp lệ Các thuộc tính trong hàm Mục đích chung. ОбщегоНазначения.ЗначенияРеквизитовОбъектов:"
"- Trường được chỉ định'");
	...
	If StrFind(Attributes, ".") <> 0 Then 
		Raise 
			NStr("en='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- Обращение через точку не поддерживается';ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- Обращение через точку не поддерживается';vi='Tham số thứ hai không hợp lệ Thuộc tính trong chức năng ОбщегоНазначения.ЗначенияРеквизитовОбъектов:"
"- Tham chiếu qua dấu chấm không được hỗ trợ'");
	EndIf;
	...
	ReferenceByTypes = New Map;
	For Each Ref In References Do
		Type = TypeOf(Ref);
		If ReferenceByTypes[Type] = Undefined Then
			ReferenceByTypes[Type] = New Array;
		EndIf;
		ReferenceByTypes[Type].Add(Ref);
	EndDo;
	
	TextsOfRequests = New Array;
	QueryOptions = New Structure;
	
	MetadataObjectNames = New Array;
	
	For Each ReferenceByType In ReferenceByTypes Do
		Type = ReferenceByType.Key;
	
		Try
			FullMetadataObjectName = Metadata.FindByType(Type).FullName();
		Except
			Raise 
				NStr("en='Неверный первый параметр Ссылки в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- Значения массива должны быть ссылками';ru='Неверный первый параметр Ссылки в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: "
"- Значения массива должны быть ссылками';vi='Tham số đầu tiên không hợp lệ của ОбщегоНазначения.ЗначенияРеквизитовОбъектов:"
"- Giá trị mảng phải là tham chiếu'");
		EndTry;
		...
		QueryOptions.Insert(ParameterName, ReferenceByType.Value);
		
		TextsOfRequests.Add(QueryText);
	EndDo;
	
	QueryText = StrConcat(TextsOfRequests, Chars.LF + "UNION ALL" + Chars.LF);
	...
	Except
		...
		For Each FullMetadataObjectName In MetadataObjectNames Do
			Result = FindObjectAttributesAvailabilityError(FullMetadataObjectName, Attributes);
		...
		If ValueIsFilled(ErrorList) Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: %1';ru='Неверный второй параметр Реквизиты в функции ОбщегоНазначения.ЗначенияРеквизитовОбъектов: %1';vi='Tham số thứ hai không hợp lệ Thuộc tính trong chức năng Mục đích chung. ОбщегоНазначения.ЗначенияРеквизитовОбъектов:%1'"), 
...
Function PostDocuments(Documents) Export
	...
	For Each DocumentRef In documents Do
		...
		If DocumentObject.CheckFilling() Then
			...
			EndTry;
		Else
			ErrorPresentation = NStr("en='Document fields are not populated.';ru='Поля документа не заполнены.';vi='Trường chứng từ chưa điền.'");
		EndIf;
		...
		If Errors.Count() > 0 Then
			...
			For Each KeyValue In SubstitutionsPairs Do
				If Errors[KeyValue.Key] = Undefined Then
					...
					ErrorText = NStr("en='Not replaced due to the previous issues.';ru='Замена не была выполнена из-за предыдущих проблем.';vi='Không thực hiện thay thế vì nhữn vấn đề trước đó.'");
					Cause = ReplacementErrorDescription("RecordingError", ForWhat, SubjectString(ForWhat), ErrorText);
					AddReplacementResult(ReplacementResult, What, Cause);
				EndIf;
			EndDo;
...
Function UsagePlaces(Val RefsSet, Val ResultAddress = "") Export
	...
	For Each String In UsagePlaces Do
		...
		ElsIf MetaConstants.Contains(Meta) Then
			Presentation = Meta.Presentation() + " (" + NStr("en='constant';ru='постоянная';vi='liên tục'") + ")";
			
		ElsIf MetaSequences.Contains(Meta) Then
			Presentation = Meta.Presentation() + " (" + NStr("en='sequence';ru='последовательность';vi='chuỗi nối tiếp'") + ")";
...
Function ReadXMLToTable(Val XML) Export
	...
	If Not Read.Read() Then
		Raise NStr("en='Empty XML';ru='Пустой XML';vi='XML rỗng'");
	ElsIf Read.Name <> "Items" Then
		Raise NStr("en='An error occurred in XML structure';ru='Ошибка в структуре XML';vi='Lỗi trong cấu trúc XML'");
	EndIf;
...
Function IdenticalCollections(RowsCollection1, RowsCollection2, ColumnNames = "", ExcludingColumns = "", IncludingRowOrder = False) Export
	...
	If IsBlankString(ColumnNames) Then
		If SpecialCollectionTypes.Find(TypeOf(RowsCollection1)) <> Undefined 
			...
			EndIf;
		Else
			ErrorMessage = NStr("en='Specify names of fields for comparison for the collection of type %1';ru='Для коллекции типа %1 необходимо указать имена полей, по которым производится сравнение';vi='Đối với tập hợp kiểu %1 cần chỉ ra tên trường, theo đó tiến hành so sánh'");
...
Function COMConnectorIdentifier(Val COMConnectorName) Export
	...
	ErrorMessage = NStr("en='CLSID is not specified for class %1';ru='На задан CLSID для класса %1';vi='Đã đặt CLSID đối với lớp %1'");
...
Function SubjectString(SubjectRef) Export
	...
	If IsBlankString(Result) Then
		If SubjectRef = Undefined Or SubjectRef.IsEmpty() Then
			Result = NStr("en='not specified';ru='не задан';vi='chưa chỉ ra'");
...
Function ObjectXDTOInXMLString(Val XDTODataObject, Val Factory = Undefined) Export
	...
	Else
		...
		If ErrorInfo <> Undefined Then
			WriteLogEvent(
				NStr("en='Standard subsystems';ru='Стандартные подсистемы';vi='Phân hệ chuẩn'", CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
				,
				,
...
Function WSProxy(Val WSDLAddress,
	...
	If MakeTestCall AND Timeout <> Undefined AND Timeout > 20 Then
		...
		Try
			WSProxyPing.Ping();
		Except
			WriteLogEvent(NStr("en='WSProxy';ru='WSПрокси';vi='ProxyWS'", CommonUseClientServer.MainLanguageCode()),
...
Function ExecuteQueryBeyondTransaction(Val Query) Export
	...
	If TransactionActive() Then
		Raise(NStr("en='Transaction is active. Cannot execute a query outside the transaction.';ru='Транзакция активна. Выполнение запроса вне транзакции невозможно.';vi='Giao tác đang hoạt động. Không thể thực hiện truy vấn ngoài giao tác.'"));
	EndIf;
...
"Рекомендуется запрещать выполнение регламентных заданий на время обновления.';vi='Tạm thời không thể đăng nhập vào chương trình do đang cập nhật phiên bản mới"
"Nên cấm thực hiện nhiệm vụ thường kỳ tại thời điểm cập nhật.'");
	EndIf;
...
"Рекомендуется запрещать выполнение регламентных заданий на время восстановления.';vi='Tạm thời không thể đăng nhập vào chương trình đến khi khôi phục kết nối với nút chính."
"Nên cấm thực hiện nhiệm vụ thường kỳ trong thời gian khôi phục.'");
	EndIf;
...
Function TimeIntervalAsString(BeginTime, EndTime = Undefined) Export
	...
	ElsIf BeginTime > EndTime Then
		Raise NStr("en='The interval end date cannot be earlier than the start date.';ru='Дата окончания интервала не может быть меньше даты начала.';vi='Ngày kết thúc khoảng không thể nhỏ hơn ngày bắt đầu.'");
	EndIf;
	...
	If IntervalSizeInDays > 365 Then
		DetailsOfInterval = NStr("en='more than a year';ru='более года';vi='hơn 1 năm'");
	ElsIf IntervalSizeInDays > 31 Then
		DetailsOfInterval = NStr("en='more than a month';ru='более месяца';vi='hơn 1 tháng'");
	ElsIf IntervalSizeInDays >= 1 Then
		...
		SubjectAndNumberInWords = NumberInWords(
			IntervalSizeInDays,
			"L=en_US",
			NStr("en='day, day, days,,,,,,0';ru='день, дня, дней,,,,,,0';vi='ngày.ngày.ngày,,,,,,0'"));
	...
	Else
		DetailsOfInterval = NStr("en='less than a day';ru='менее одного дня';vi='ít hơn một ngày'");
	EndIf;
...
Function GetConfigurationMetadataTree(Filter = Undefined) Export
	...
	NewMetadataObjectCollectionRow("Constants",               NStr("en='Constants';ru='Константы';vi='Hằng'"),                 PictureLib.Constant,              PictureLib.Constant,                    CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("Catalogs",             NStr("en='Catalogs';ru='Справочники';vi='Danh mục'"),               PictureLib.Catalog,             PictureLib.Catalog,                   CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("Documents",               NStr("en='Documents';ru='Документы';vi='Chứng từ'"),                 PictureLib.Document,               PictureLib.DocumentObject,               CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("ChartsOfCharacteristicTypes", NStr("en='Charts of characteristic types';ru='Планы видов характеристик';vi='Hệ thống dạng đặc tính'"), PictureLib.ChartOfCharacteristicTypes, PictureLib.ChartOfCharacteristicTypesObject, CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("ChartsOfAccounts",             NStr("en='Charts of accounts';ru='Планы счетов';vi='Hệ thống tài khoản'"),              PictureLib.ChartOfAccounts,             PictureLib.ChartOfAccountsObject,             CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("ChartsOfCalculationTypes",       NStr("en='Charts of characteristic types';ru='Планы видов характеристик';vi='Hệ thống dạng đặc tính'"), PictureLib.ChartOfCharacteristicTypes, PictureLib.ChartOfCharacteristicTypesObject, CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("InformationRegisters",        NStr("en='Information registers';ru='Регистры сведений';vi='Biểu ghi thông tin'"),         PictureLib.InformationRegister,        PictureLib.InformationRegister,              CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("AccumulationRegisters",      NStr("en='Accumulation registers';ru='Регистры накопления';vi='Biểu ghi tích lũy'"),       PictureLib.AccumulationRegister,      PictureLib.AccumulationRegister,            CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("AccountingRegisters",     NStr("en='Accounting registers';ru='Регистры бухгалтерии';vi='Biểu ghi kế toán'"),      PictureLib.AccountingRegister,     PictureLib.AccountingRegister,           CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("CalculationRegisters",         NStr("en='Calculation registers';ru='Регистры расчета';vi='Biểu ghi tính toán'"),          PictureLib.CalculationRegister,         PictureLib.CalculationRegister,               CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("BusinessProcesses",          NStr("en='Business processes';ru='Деловые процессы';vi='Quy trình nghiệp vụ'"),           PictureLib.BusinessProcess,          PictureLib.BusinessProcessObject,          CollectionsOfMetadataObjects);
	NewMetadataObjectCollectionRow("Tasks",                  NStr("en='Tasks';ru='Задания';vi='Nhiệm vụ'"),                    PictureLib.Task,                 PictureLib.TaskObject,                 CollectionsOfMetadataObjects);
...
Function ObjectManagerByFullName(FullName) Export
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Unknown metadata object type %1';ru='Неизвестный тип объекта метаданных %1';vi='Sai kiểu đối tượng metadata ""%1""'"), FullName);
...
Function ObjectByDescriptionFull(FullName) Export
	...
	If RowArray.Count() >= 2 Then
		...
		Name = RowArray[1];
	Else
		Raise StrReplace(NStr("en='Incorrect full name of the report or data processor %1.';ru='Некорректное полное имя отчета или обработки %1.';vi='Sai tên đầy đủ của báo cáo hoặc bộ xử lý %1.'"), "%1", FullName);
	EndIf;
	...
	Else
		Raise StrReplace(NStr("en='%1 is not a report or a data processor.';ru='%1 не является отчетом или обработкой.';vi='%1 không là báo cáo hay bộ xử lý.'"), "%1", FullName);
	EndIf;
EndFunction
...
Function ObjectKindByKind(Type) Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Incorrect type of parameter value (%1)';ru='Неверный тип значения параметра (%1)';vi='Sai kiểu giá trị tham số (%1)'"), String(Type));
...
""
"Metadata object is not found by the type: %1';ru='Ошибка при выполнении функции ОбщегоНазначения.ИдентификаторОбъектаМетаданных()."
""
"Объект метаданных не найден по типу: %1.';vi='Lỗi khi thực hiện hàm CommonUse function.MetadataObjectID()."
""
"Không tìm thấy đối tượng metadata kiểu: %1.'"),
				MetadataObjectDesc);
		Else
...
""
"Identifier %1 is not found in the ""Metadata objects identifiers catalog"".';ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору()"
""
"Идентификатор %1 не найден в справочнике ""Идентификаторы объектов метаданных"".';vi='Lỗi khi thực hiện hàm CommonUse function.MetadataObjectByID()."
""
"Không tìm thấy tên (ID) %1 trong danh mục ""Tên (ID) đối tượng metadata"".'")
		...
		If CheckResult.MetadataObject = Undefined Then
			If CheckResult.MetadataObjectKey = Undefined Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='An error occurred during the execution of CommonUse function.MetadataObjectByID()."
""
"Nonexistent metadata object %2 corresponds to identifier %1 in the ""Metadata objects identifiers catalog"".';ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору()"
""
"Идентификатору %1 найденому в справочнике ""Идентификаторы объектов метаданных"", соответствует несуществующий объект метаданных %2.';vi='Lỗi khi thực hiện hàm CommonUse function.MetadataObjectByID()."
""
"Không tìm thấy tên (ID) %1 trong danh mục ""Tên (ID) đối tượng metadata"" tương ứng với đối tượng metadata không tồn tại %2.'")
					+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
					String(ID),
			...
			Else
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='An error occurred during the execution of CommonUse function.MetadataObjectByID()."
""
"%1 identifier found in the Metadata objects identifiers catalog corresponds to the removed metadata object.';ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору()"
""
"Идентификатору %1 найденому в справочнике ""Идентификаторы объектов метаданных"", соответствует удаленный объект метаданных.';vi='Lỗi khi thực hiện hàm CommonUse function.MetadataObjectByID()"
""
"Tên (ID) %1 được tìm thấy trong danh mục """"Tên (ID) đối tượng metadata"""", tương ứng với đối tượng metadata đã xóa.'")
					+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
					String(ID));
			EndIf;
		Else
...
"соответствует объект метаданных %2, полное имя которого отличается от заданного в идентификаторе.';vi='Lỗi khi thực hiện hàm CommonUse function.MetadataObjectID()."
""
"Tên định danh %1 đã tìm thấy trong danh mục ""Tên định danh đối tượng metadata"","
"tương ứng với đối tượng metadata %2 mà tên gọi đầy đủ của đối tượng này khác với tên đã đặt trong Tên định danh.'")
				+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
				String(ID),
	...
	If Exporting[0].DeletionMark Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='An error occurred during the execution of CommonUse function.MetadataObjectByID()."
""
"%1 identifier is found in the ""Metadata objects identifier catalog"", but the ""Deletion markup"" attribute value is set to TRUE.';ru='Ошибка при выполнении функции ОбщегоНазначения.ОбъектМетаданныхПоИдентификатору()"
""
"Идентификатор %1 найден в справочнике ""Идентификаторы объектов метаданных"", но значение реквизита ""Пометка удаления"" установлено ИСТИНА.';vi='Lỗi khi thực hiện hàm CommonUse function.MetadataObjectByID()."
""
"Không tìm thấy tên (ID) %1 trong danh mục ""Tên (ID) đối tượng metadata"", nhưng giá trị mục tin ""Đặt dấu xóa"" là True.'")
			+ StandardSubsystemsServer.SpecificationOfErrorParametersWorkApplicationForDeveloper(),
			String(ID));
	EndIf;
...
Procedure AddRenaming(Total, IBVersion, FormerFullName, NewFullName, LibraryID = "") Export
	...
	ErrorTitle =
		NStr("en='An error occurred in the OnAddMetadataObjectsRenaming procedure of the CommonUseOverridable common module.';ru='Ошибка в процедуре ПриДобавленииПереименованийОбъектовМетаданных общего модуля ОбщегоНазначенияПереопределяемый.';vi='Lỗi trong thủ tục OnAddMetadataObjectsRenaming của mô-đun chung CommonUseOverridable.'");
	
	If FormerCollectionName <> NewCollectionName Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			ErrorTitle + Chars.LF + Chars.LF
			+ NStr("en='Types names of the renamed metadata object do not match. Previous type: %1, new type: %2.';ru='Не совпадают имена типов переименованного объекта метаданных. Прежний тип: %1, новый тип: %2.';vi='Không trùng tên gọi các kiểu đối tượng metadata đổi tên. Kiểu cũ: %1, kiểu mới: %2.'"),
			FormerFullName,
			NewFullName);
	EndIf;
...
""
"It is required to describe renamings only for the following types: %2.';ru='Для типа объекта метаданных %1 не"
"требуется описывать переименование, так как сведения об объектах метаданных этого типа обновляются автоматически."
""
"Описывать переименования требуется только для следующих типов: %2.';vi='Đối với kiểu đối tượng metadata ""%1"" không"
"cần mô tả đổi tên, vì thông tin về đối tượng metadata kiểu này được cập nhật tự động."
""
"Chỉ cần mô tả đổi tên đối với các kiểu sau: %2.'"),
			FormerFullName,
			NewFullName,
			ValidTypesList);
	EndIf;
...
// Defines if metadata object belongs to the Documents common type.
// 
// Parameters:
//  MetadataObject - MetadataObject - object for which it is required to define whether it belongs to the specified type.
// 
...
Function IsDocument(MetadataObject) Export
	...
	Return Metadata.Documents.Contains(Metadata);
...
Procedure LockInfobase(Val CheckNoOtherSessions = True) Export
	...
	Else
		...
		Else
			Raise(NStr("en='Subsystem ""SaaS operations"" is not available';ru='Подсистема ""Работа в модели сервиса"" не доступна';vi='Phân hệ ""Làm việc trong mô hình dịch vụ"" không sử dụng được'"));
		EndIf;
	EndIf;
...
Function GetInterfaceVersions(Val Address, Val User, Val Password = Undefined, Val Interface = Undefined) Export
	...
	If Not ConnectionParameters.Property("URL") 
		...
		Raise(NStr("en='URL of the service is not specified.';ru='Не задан URL сервиса.';vi='Chưa chỉ ra URL service.'"));
	EndIf;
...
Function GetInterfaceVersionsViaExternalConnection(ExternalConnection, Val InterfaceName) Export
	...
	Except
		MessageString = NStr("en='Correspondent does not support the subsystems interfaces versioning. Error description: %1';ru='Корреспондент не поддерживает версионирование интерфейсов подсистем. Описание ошибки: %1';vi='Người trao đổi tin không hỗ trợ đánh số phiên bản giao diện phân hệ con. Mô tả lỗi: %1'");
		MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString, DetailErrorDescription(ErrorInfo()));
		WriteLogEvent(NStr("en='Receive interface versions';ru='Получение версий интерфейса';vi='Nhận phiên bản giao diện'", CommonUseClientServer.MainLanguageCode()),
...
Function CommonModule(Name) Export
	...
	If TypeOf(Module) <> Type("CommonModule") Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Common module ""%1"" was not found.';ru='Общий модуль ""%1"" не найден.';vi='Chưa tìm thấy mô-đun chung ""%1"".'"), Name);
	EndIf;
...
// Возвращает True, если клиентское приложение является Веб-клиентом.
//
// Returns:
//  Boolean - если нет клиентского приложения, возвращается Ложь.
//
...
// Создать структуру свойств динамического списка для вызова SetDynamicListProperties().
//
// Returns:
//  Structure - любое поле может иметь значение Неопределено, если оно не устанавливается:
//     * QueryText - String - новый текст запроса.
//     * MainTable - String - имя основной таблицы.
//     * DynamicDataRead - Boolean - признак использования динамического считывания.
//
Function DynamicListPropertyStructure() Export
...
//
// Parameters:
//  List - FormTable - элемент формы динамического списка, для которого устанавливаются свойства.
//  ParametersStructure - See DynamicListPropertyStructure().
//
...
//
// Parameters:
//  Data   - Arbitrary - любое сериализуемое значение.
//  Algorithm - HashFunction   - алгоритм расчета контрольной суммы. По умолчанию, MD5.
// 
// Returns:
//  String - контрольная сумма строкой без пробелов (например 32 символа).
//
Function StringChecksum(Val Data, Val Algorithm = Undefined) Export
...
//  FixData - Boolean, Undefined - если Истина - фиксировать,
...
Function PrepareDataCacheVersions(Val DataType, Val ReceivingParameters) Export
	...
	Else
		TextPattern = NStr("en='Unknown data type of versions cache: %1';ru='Неизвестный тип данных кэша версий: %1';vi='Kiểu dữ liệu cache phiên bản không xác định: %1'");
...
Function WSDefinitions(Val WSDLAddress, Val UserName, Val Password, Val Timeout = 10) Export
	...
	Except
		WriteLogEvent(NStr("en='Receive WSDL';ru='Получение WSDL';vi='Nhận WSDL'", CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error, , , DetailErrorDescription(ErrorInfo()));
	EndTry;
		...
		If Not CommonUseReUse.SessionWithoutSeparator() Then
			...
			Else 
				If DataArea <> SessionSeparatorValue() Then
					Raise(NStr("en='Cannot access data from another data area in this session.';ru='В данном сеансе недопустимо обращение к данным из другой области данных!';vi='Trong phiên làm việc này không truy cập được tới dữ liệu từ vùng dữ liệu khác!'"));
				EndIf;
			EndIf;
		EndIf;
...
Function GetWSDL(Val Address, Val UserName, Val Password, Val Timeout)
	...
	If Not FileDescription.Status Then
		Raise(NStr("en='An error occurred when receiving a description file of the web service:';ru='Ошибка получения файла описания web-сервиса:';vi='Lỗi nhận tệp mô tả Web-service:'") + Chars.LF + FileDescription.ErrorInfo)
	EndIf;
...
""
"Description file address may be incorrect: %1';ru='Ошибка получения файла описания web-сервиса: "
"В полученном файле не содержится ни одного описания сервиса."
""
"Возможно, адрес файла описания указан неверно: %1';vi='Lỗi nhận tệp mô tả Web-service: Trong tệp đã nhận không chứa mô tả Service nào."
""
"Có thể là đã chỉ ra sai địa chỉ tệp mô tả: %1'");
...
Procedure CheckingDataFixed(Data, DataInValueOfFixedTypes = False)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='An error occurred in the FixedData function of the CommonUse common module. Data of the %1 type can not be recorded.';ru='Ошибка в функции ФиксированныеДанные общего модуля ОбщегоНазначения. Данные типа %1 не могут быть зафиксированы.';vi='Lỗi trong hàm FixedData mô-đun chung CommonUse. Không thể cố định dữ liệu kiểu %1.'"),
		String(DataType) );
...
Function ManagerServerModule(Name)
	...
	If Not ObjectFound Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='%1 metadata object is not found or manager module receipt is not supported for it.';ru='Объект метаданных %1 не найден, либо для него не поддерживается получение модуля менеджера.';vi='Không tìm thấy đối tượng metadata %1, hoặc không hỗ trợ nhận mô-đun quản lý đối với đối tượng này.'"), Name);
	EndIf;
	...
	For Each UsagePlace In MarkupResult.UsagePlaces Do
		...
		Else
			...
			Else
				...
				Text = NStr("en='Unknown data type %1 for replacement %2';ru='Неизвестный тип данных %1 для проведения замены %2';vi='Không xác định kiểu dữ liệu%1 để thay thế %2'");
...
Procedure ReplaceInConstant(Results, Val UsagePlace, Val WriteParameters, Val InternalTransaction = True)
	...
	If InternalTransaction Then
		...
		Except
			...
			Error = NStr("en='Cannot lock constant %1';ru='Не удалось заблокировать константу %1';vi='Không thể phong tỏa hằng %1'");
	...
	If ActionState = "" Then
		...
		If ReplacementMade Then
			...
			Except
				...
				Error = NStr("en='Cannot write %1 due to: %2';ru='Не удалось записать %1 по причине: %2';vi='Không thể ghi lại %1 do nguyên nhân: %2'");
				Error = StrReplace(Error, "%1", DataPresentation);
...
Procedure ReplaceInObject(Results, Val UsagePlace, Val WriteParameters, Val InternalTransaction = True)
	...
	If InternalTransaction Then
		...
		Except
			// Add record about an unsuccessful attempt to lock the result.
			Error = NStr("en='Unable to lock one or several objects from the list %1';ru='Не удалось заблокировать один или несколько объектов из списка %1';vi='Không thể phong tỏa một hoặc nhiều đối tượng từ danh sách %1'");
...
Procedure ReplaceInSet(Results, Val UsagePlace, Val WriteParameters, Val InternalTransaction = True)
	...
	If InternalTransaction Then
		...
		Except
			// Add record about an unsuccessful attempt to lock the result.
			Error = NStr("en='Cannot lock set %1';ru='Не удалось заблокировать набор %1';vi='Không thể phong tỏa tập hợp %1'");
			Error = StrReplace(Error, "%1", DataPresentation);
	...
	For Each Ref In RefsList Do
		...
		Except
			AddReplacementResult(DeletionResult, Ref, 
				ReplacementErrorDescription("LockError", Ref, REFPRESENTATION, NStr("en='An error occurred when locking reference for deletion.';ru='Ошибка блокирования ссылки для удаления';vi='Lỗi phong tỏa tham chiếu để xóa'")));
		EndTry
	EndDo;
	...
	For Each Ref In ToDelete Do
		...
		Except
			...
			AddReplacementResult(DeletionResult, Ref, 
				...
				NStr("en='Removal failed';ru='Ошибка удаления';vi='Lỗi xóa'") + Chars.LF + TrimAll( BriefErrorDescription(ErrorInfo))));
		EndTry;
	...
	For Each String In TableSearchAgain Do
		...
		If FinalTable.FindRows(Filter).Count() = 0 Then
			AddReplacementResult(FinalTable, Ref, 
				...
				NStr("en='Data was added or changed by another user';ru='Данные были добавлены или изменены другим пользователем';vi='Người sử dụng khác đã thêm hoặc thay đổi dữ liệu'")));
		EndIf;
	EndDo;
...
Function SetDimensionsDescription(Val Meta, Cache)
	...
	If Metadata.InformationRegisters.Contains(Meta) Then
		...
		If MetaPeriod = Periodicity.RecorderPosition Then
			...
			DimensionData.Presentation = NStr("en='Recorder';ru='Регистратор';vi='Nguồn ghi'");
		...
		ElsIf MetaPeriod = Periodicity.Year Then
			...
			DimensionData.Presentation = NStr("en='Accounting period';ru='Отчетный период';vi='Kỳ báo cáo'");
...
Procedure WriteObject(Val Object, Val WriteParameters)
	...
	If AttributesTest.Hierarchical Or AttributesTest.ExtDimensionTypes <> Undefined Then 
		...
		If Object.Parent = Object.Ref Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Circular reference occurs in hierarchy when writing %1.';ru='При записи %1 возникает циклическая ссылка в иерархии.';vi='Khi ghi lại ""%1"" đã xuất hiện tham chiếu phân nhánh theo chu kỳ.'"),
				String(Object));
			EndIf;
	...
	For Each MetaOwner In AttributesTest.Owners Do
		...
		If Object.Owner = Object.Ref Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Circular reference occurs in subordination when writing %1.';ru='При записи %1 возникает циклическая ссылка в подчинении.';vi='Khi ghi lại ""%1"" có xuất hiện tham chiếu trực thuộc theo chu kỳ.'"),
				String(Object));
		EndIf;
		Break;
...
Function EventLogMonitorEventRefsReplacements()
	Return NStr("en='Search and delete duplicates';ru='Поиск и удаление дубликатов';vi='Tìm kiếm và xóa phần tử trùng lặp'", 
...
Procedure ReplaceRefWithLongTransaction(ReplacementResult, Val Ref, Val ReplacementParameters, Val SearchTable)
	...
	Except
		...
		Error = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Cannot lock all usage locations %1';ru='Не удалось заблокировать все места использования %1';vi='Không thể phong tỏa tất cả chỗ sử dụng %1'"),
			Ref
		);
...
Function EvaluateDataValueByPath(Val Data, Val DataPath)
	...
	For IndexOf = 1 To PathPartsQuantity Do
		...
		If Test[AttributeName] = Undefined Then
			...
			If Test[AttributeName] = Undefined Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='An error occurred while calculating %1 for object %2. Attribute %3 is not found';ru='Ошибка вычисления %1 для объекта %2. Реквизит %3 не найден';vi='Lỗi tính %1 đối với đối tượng %2. Không tìm thấy mục tin %3'"),
...
// 
// Parameters:
//  FullMetadataObjectName - String - полное имя проверяемого объекта.
//  CheckedExpressions       - Array - имена полей или проверяемые выражения объекта метаданных.
// 
// Returns:
//  Structure - Результат проверки.
...
//
// Если Результат.Ошибка Then
//     ВызватьИсключение Результат.ОписаниеОшибки;
// КонецЕсли;
//
Function FindObjectAttributesAvailabilityError(FullMetadataObjectName, CheckedExpressions)
	...
	If ObjectMetadata = Undefined Then 
		Return New Structure("Error, ErrorDescription", True, 
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Ошибка получения метаданных ""%1""';ru='Ошибка получения метаданных ""%1""';vi='Lỗi khi nhận siêu dữ liệu ""%1""'"), FullMetadataObjectName));
	EndIf;
	...
	Scheme = New QuerySchema;
	Package = Scheme.QueryBatch.Add(Type("QuerySchemaSelectQuery"));
	Operator = Package.Operators.Get(0);
	
	Source = Operator.Sources.Add(FullMetadataObjectName, "Table");
	ErrorText = "";
	
	For Each CurrentExpression In CheckedExpressions Do
		
		If Not FieldOfQuerySchemeSourceAvailable(Source, CurrentExpression) Then 
			ErrorText = ErrorText + Chars.LF + StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='- Поле объекта ""%1"" не найдено';ru='- Поле объекта ""%1"" не найдено';vi='- Trường đối tượng ""%1"" không tìm thấy'"), CurrentExpression);
		EndIf;
...
//
Function FieldOfQuerySchemeSourceAvailable(OperatorSource, CheckedExpression)
	
	FieldNameParts = StrSplit(CheckedExpression, ".");
	AvailableFields = OperatorSource.Source.AvailableFields;
	
	FieldNameCurrentPart = 0;
	While FieldNameCurrentPart < FieldNameParts.Count() Do 
		
		CurrentField = AvailableFields.Find(FieldNameParts.Get(FieldNameCurrentPart)); 
		...
		FieldNameCurrentPart = FieldNameCurrentPart + 1;
...
Function CopyStructure(SourceStructure, FixData)
	...
	For Each KeyAndValue In SourceStructure Do
		ResultStructure.Insert(KeyAndValue.Key, CopyRecursive(KeyAndValue.Value, FixData));
	EndDo;
...
Function CopyMap(SourceMap, FixData)
	...
	For Each KeyAndValue In SourceMap Do
		ResultMap.Insert(KeyAndValue.Key, CopyRecursive(KeyAndValue.Value, FixData));
	EndDo;
...
Function CopyArray(ArraySource, FixData)
	
	ResultArray = New Array;
	
	For Each Item In ArraySource Do
		ResultArray.Add(CopyRecursive(Item, FixData));
	EndDo;
...
Function CopyValueList(SourceList, FixData)
	...
	For Each ListElement In SourceList Do
		ResultList.Add(
			CopyRecursive(ListElement.Value, FixData), 
...
//
// Parameters:
//  MethodName  - String - имя экспортной процедуры в формате
//                       <имя объекта>.<имя процедуры>, где <имя объекта> - это
//                       общий модуль или модуль менеджера объекта.
//  Parameters  - Array - параметры передаются в процедуру <ИмяЭкспортнойПроцедуры>
...
//
Procedure ExecuteConfigurationMethod(Val MethodName, Val Parameters = Неопределено) Export
	
	//ПроверитьИмяПроцедурыКонфигурации(ИмяМетода);
	
	If SubsystemExists("StandardSubsystems.SecurityProfiles") Then
		ModuleSafeModeManager = CommonModule("WorkInSafeMode");
		If ModuleSafeModeManager.ИспользуютсяПрофилиБезопасности()
			И Не ModuleSafeModeManager.УстановленБезопасныйРежим() Then
			
			InfobaseProfile = ModuleSafeModeManager.ПрофильБезопасностиИнформационнойБазы();
	...
	ParametersString = "";
	If Parameters <> Undefined И Parameters.Count() > 0 Then
...
//
// Parameters:
//  Object    - Arbitrary - объект встроенного языка 1С:Предприятия, содержащий методы (например, ОбработкаОбъект).
//  MethodName - String       - имя экспортной процедуры модуля объекта обработки.
//  Parameters - Array       - параметры передаются в процедуру <ИмяПроцедуры>
//                             в порядке расположения элементов массива.
//
Procedure ExecuteObjectMethod(Val Object, Val MethodName, Val Parameters = Undefined) Export
	...
	Try
		Test = Новый Структура(MethodName, MethodName);
		If Test = Undefined Then 
			Raise NStr("ru='Синтетический тест';vi='Kiểm thử tổng hợp'");
		EndIf;
	Except
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("ru='Некорректное значение параметра ИмяМетода (%1) в ОбщегоНазначения.ВыполнитьМетодОбъекта';vi='Sai giá trị tham số ИмяМетода (%1) trong ОбщегоНазначения.ВыполнитьМетодОбъекта'"), MethodName);
	EndTry;
...
Function TrimStringUsingChecksum(Row, MaxLength) Export
	CommonUseClientServer.Validate(MaxLength >= 32, NStr("en='Параметр МаксимальнаяДлина не может быть меньше 32';ru='Параметр МаксимальнаяДлина не может быть меньше 32';vi='Tham số Chiều dài tối đa không được nhỏ hơn 32'"),
		"CommonUse.TrimStringUsingChecksum");
	
	Result = Row;
	If StrLen(Row) > MaxLength Then
		Result = Left(Row, MaxLength - 32);
		DataHashing = New DataHashing(HashFunction.MD5);
		DataHashing.Append(Mid(Row, MaxLength - 32 + 1));
		Result = Result + StrReplace(DataHashing.HashSum, " ", "");
	EndIf;
	Return Result;
EndFunction
