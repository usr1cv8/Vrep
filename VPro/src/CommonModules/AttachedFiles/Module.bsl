...
Function GetFileBinaryData(Val AttachedFile) Export
	...
	CommonUseClientServer.Validate(FileObject <> Undefined, 
		StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Attached file ""%1"" is not found (%2)';ru='Не найден присоединенный файл ""%1"" (%2)';vi='Không tìm thấy tệp đính kèm ""%1"" (%2)'"),
...
""
"the AttachedFiles register Ref to file: ""%1"".';ru='Двоичные данные файла отсутствуют в регистре AttachedFiles"
""
"Ссылка на файл: ""%1"".';vi='Không có dữ liệu nhị phân của tệp trong biểu ghi AttachedFiles"
""
"Tham chiếu đến tệp: ""%1"".'"),
				GetURL(AttachedFile));
			WriteLogEvent(NStr("en='Files.Open file';ru='Файлы.Открытие файла';vi='Tệp.Mở tệp'", CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
	...
	Else
		...
		Except
			...
			WriteLogEvent(NStr("en='Files.Receive a file from the volume';ru='Файлы.Получение файла из тома';vi='Tệp.Nhận tệp từ quyển'", CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
				Metadata.Catalogs[AttachedFile.Metadata().Name],
				AttachedFile,
				ErrorInfo);
...
Function AddFile(Val FilesOwner,
	...
	ErrorTitle = NStr("en='An error occurred when adding the attached file.';ru='Ошибка при добавлении присоединенного файла.';vi='Lỗi khi thêm tệp đính kèm.'");
	
	If NewRefToFile = Undefined Then
		CatalogName = AttachedFilesService.CatalogNameStorageFiles(FilesOwner, "", ErrorTitle,
			NStr("en='In this case, parameter NewRefToFile must be specified.';ru='В этом случае параметр ""НоваяСсылкаНаФайл"" должен быть указан.';vi='Trong trường hợp này cần chỉ ra tham số ""NewRefToFile"".'"));
...
"Ссылка на новый файл не заполнена.';vi='Lỗi khi thêm tệp đính kèm."
"Chưa điền tham chiếu đến tệp mới.'");
		EndIf;
...
"присоединенного файла ""%1"": %2';vi='Lỗi khi thêm"
"tệp đính kèm ""%1"": %2'");
		...
		WriteLogEvent(
			NStr("en='File.Adding an attached file';ru='Файлы.Добавление присоединенного файла';vi='Tệp.Thêm tệp đính kèm'",
...
Function NewRefToFile(FilesOwner, CatalogName = Undefined) Export
	...
	ErrorTitle = NStr("en='An error occurred when receiving a new reference to the attached file.';ru='Ошибка при получении новой ссылки на присоединенный файл.';vi='Lỗi khi nhận tham chiếu mới cho tệp đính kèm.'");
...
Function GetAttachedFilesObjectFormNameByOwner(Val FilesOwner) Export
	...
	ErrorTitle = NStr("en='An error occurred when receiving a form name of the attached file.';ru='Ошибка при получении имени формы присоединенного файла.';vi='Lỗi khi nhận tên biểu mẫu tệp đính kèm.'");
	EndErrors = NStr("en='In this case, the form cannot be received.';ru='В этом случае получение формы невозможно.';vi='Trong trường hợp này, không thể nhận biểu mẫu.'");
...
"Work with files to attached files of the Attached files subsystem.';ru='Ошибка при конвертации присоединенных файлов подсистемы Работа с файлами в присоединенные файлы подсистемы Присоединенные файлы.';vi='Lỗi khi chuyển đổi tệp đính kèm của phân hệ Làm việc với tệp sang tệp đính kèm của phân hệ Tệp đính kèm.'");
...
"владелец в файле ""%1"".';vi='Chưa điền"
"đối tượng chủ trong tệp ""%1"".'"),
			Source.Description);
		
		If InfobaseUpdate.InfobaseUpdateInProgress() Then
			
			WriteLogEvent(
				NStr("en='Files. An error occurred when writing the file while updating IB';ru='Файлы.Ошибка записи файла при обновлении ИБ';vi='Tệp.Lỗi ghi lại tệp khi cập nhật CSDL'",
				     CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
				,
				Source.Ref,
...
Procedure OnCreateAtServerAttachedFile(Val Form) Export
	...
	Form.Title = NStr("en='Attached file';ru='Присоединенный файл';vi='Tệp đính kèm'");
	...
	Decoration.Title = NStr("en='In order to proceed to the file card, click the hyperlink';ru='Для того, чтобы перейти к карточке файла, нажмите на гиперссылку';vi='Để chuyển đến thẻ tệp, hãy nhấn vào hyperlink'");
	
	Button = Form.Items.Add("GoToFileForm1", Type("FormButton"));
	Button.Type        = FormButtonType.Hyperlink;
	Button.Title  = NStr("en='Go to file form';ru='Перейти к форме файла';vi='Chuyển tới biểu mẫu tệp'");
...
Procedure CallFormOpeningException(Form) Export
	...
	Raise NStr("en='Cannot open the form directly.';ru='Самостоятельное использование формы не предусмотрено.';vi='Tự sử dụng biểu mẫu không được xem xét'");
...
"Ref to file: ""%2"".';ru='%1"
""
"Ссылка на файл: ""%2"".';vi='%1"
""
"Tham chiếu đến tệp: ""%2"".'"),
			ErrorInfo,
			GetURL(File) );
	EndIf;
...
Procedure MarkToDeleteAttachedFiles(Val Source, CatalogName = Undefined)
	...
	Except
		...
		Raise NStr("en='An error occurred when marking the attached files for deletion.';ru='Ошибка при пометке на удаление присоединенных файлов.';vi='Lỗi khi đánh dấu xóa tệp đính kèm.'")
			+ Chars.LF
			+ ErrorPresentation;
	EndTry;
...
"занятый для редактирования.';vi='Không thể xóa ""%1"""
"vì có tệp đính kèm ""%2"""
"đã bị chiếm để soạn.'"),
					String(Source.Ref),
