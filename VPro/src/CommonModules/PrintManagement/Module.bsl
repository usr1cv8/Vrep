...
Function PrintedFormsTemplate(FullPathToTemplate) Export
	...
	Else
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Template ""%1"" is not found. Operation aborted.';ru='Макет ""%1"" не найден. Операция прервана.';vi='Khuôn in ""%1"" không tìm thấy. Giao dịch bị ngừng.'"), FullPathToTemplate);
	EndIf;
...
Function QRCodeData(QRString, CorrectionLevel, Size) Export
	...
	Except
		WriteLogEvent(NStr("en='QR code generation';ru='Формирование QR-кода';vi='Tạo mã QR'", CommonUseClientServer.MainLanguageCode()),
...
// 
// Returns:
//  Structure - настройки записи табличного документа в файл:
//   * SavingFormats - Array - коллекция значений типа ТипФайлаТабличногоДокумента,
//                                  либо значений типа ТипФайлаТабличногоДокумента, преобразованных в строку;
//   * PackIntoArchive   - Boolean - если установить значение Истина, будет создан один файл архива с файлами указанных форматов;
//   * TransliterateFilesNames - Boolean - если установить Истина, то имена полученных файлов будут на латинице.
//   * SignatureAndSeal    - Boolean - если установить Истина и сохраняемый табличный документ поддерживает размещение
//                                  подписей и печатей, то в записанных файлах будут размещены подписи и печати.
//
Function SavingSettings() Export
	
	Return PrintManagementClientServer.SavingSettings();
...
//
// Parameters:
//  PrintCommands  - Structure, Array - команда или несколько команд печати формы,
//                                       See PrintManagement.PrintCommandsForms.
//  ObjectList - Array    - ссылки на печатаемые объекты.
//  SavingSettings - See PrintManagement.SavingSettings.
//
// Returns:
//  ValueTable - файлы печатных форм:
//   * FileName - String - имя файла;
//   * BinaryData - BinaryData - файл печатной формы.
//
Function PrintToFile(PrintCommands, ObjectList, SavingSettings) Export
	...
	If TypeOf(PrintCommands) <> Type("Array") Then
		CommandsList = CommonUseClientServer.ValueInArray(PrintCommands);
	EndIf;
	
	For Each PrintCommand In CommandsList Do
		ExecutePrintToFileCommand(PrintCommand, SavingSettings, ObjectList, Result);
	EndDo;
	
	If SavingSettings.PackIntoArchive Then
		BinaryData = PackIntoArchive(Result);
		Result.Clear();
		File = Result.Add();
...
//
// Parameters:
//   AdditionalInformationProcessorRef - CatalogRef.AdditionalReportsAndDataProcessors - внешняя обработка.
//   SourceParameters            - Structure - структура со свойствами:
//       * CommandID - String - список макетов, перечисленных через запятую.
//       * DestinationObjects    - Array
//   PrintFormsCollection - See описание процедуры Печать() в документации.
//   PrintObjects         - ValueList  - See описание процедуры Печать() в документации.
//   OutputParameters       - Structure       - See описание процедуры Печать() в документации.
//   
Procedure PrintByExternalSource(AdditionalInformationProcessorRef, SourceParameters, PrintFormsCollection,
	PrintObjects, OutputParameters) Export
	
	ModuleAdditionalReportsAndDataProcessors = CommonUse.CommonModule("AdditionalReportsAndDataProcessors");
	ExternalDataProcessorObject = ModuleAdditionalReportsAndDataProcessors.ExternalDataProcessorObject(AdditionalInformationProcessorRef);
	If ExternalDataProcessorObject = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Внешняя обработка ""%1"" (тип ""%2"") не обслуживается.';ru='Внешняя обработка ""%1"" (тип ""%2"") не обслуживается.';vi='Không hỗ trợ bộ xử lý ngoài ""%1"" (kiểu ""%2"").'"),
			String(AdditionalInformationProcessorRef),
			String(TypeOf(AdditionalInformationProcessorRef)));
	EndIf;
	
	PrintFormsCollection = PrepareCollectionOfPrintForms(SourceParameters.CommandID);
	OutputParameters = PrepareOutputParametersStructure();
	OutputParameters.Insert("AdditionalInformationProcessorRef", AdditionalInformationProcessorRef);
	...
	For Each PrintForm In PrintFormsCollection Do
		If PrintForm.SpreadsheetDocument = Undefined Then
			ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='В обработчике печати не был сформирован табличный документ для: %1';ru='В обработчике печати не был сформирован табличный документ для: %1';vi='Trong bộ xử lý in chưa tạo văn bản dạng bảng đối với: %1'"),
...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	Work.Presentation = NStr("en='Print form templates';ru='Макеты печатных форм';vi='Khuôn in'");
...
Function permissions()
	...
	permissions.Add( 
		WorkInSafeMode.PermissionToUseExternalComponent("CommonTemplate.QRCodePrintComponent", NStr("en='Print QR codes.';ru='Печать QR кодов.';vi='In mã QR.'"))
	);
...
Function GeneratePrintForms(Val PrintManagerName, Val TemplateNames, Val ObjectsArray, Val PrintParameters, 
	...
	For Each TemplateName IN TemplateNames Do
		...
		For Each PrintFormDescription IN TemporaryCollectionForOnePrintForm Do
			CommonUseClientServer.Validate(
				...
				StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Number of copies is not specified for print form ""%1"".';ru='Не задано количество экземпляров для печатной формы ""%1"".';vi='Chưa đặt số lượng bản in đối với mẫu in ""%1"".'"),
		...
		If Cancel Then
			ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='An error occurred when generating print form ""%1"". Contact your administrator.';ru='При формировании печатной формы ""%1"" возникла ошибка. Обратитесь к администратору.';vi='Khi tạo mẫu in ""%1"" xảy ra lỗi. Hãy liên hệ với người quản trị.'"), TemplateName);
			Raise ErrorMessageText;
		EndIf;
...
Procedure AddPrintCommands(Form, PrintCommands, Val CommandsPlacementPlace = Undefined)
	...
	Else
		If Not OnePrintCommand Then
			...
			PopupPrint.Title = NStr("en='Print';ru='Печать';vi='In'");
...
Function RequiredAttributesString(DocumentData, MessageText)
	...
	For Each Item IN MandatoryAttributes Do
		If Not ValueIsFilled(DocumentData[Item.Key]) Then
			MessageText = NStr("en='Required attribute is not entered: %1';ru='Не заполнен обязательный реквизит: %1';vi='Chưa điền mục tin bắt buộc: %1'");
...
"должна быть меньше 300 символов: ""%2""';vi='Không thể tạo"
"mã QR đối với chứng từ %1 Dòng của các mục tin bắt buộc"
"cần ít hơn 300 ký tự: ""%2""'");
...
Function ComponentQRCodeGeneration(Cancel)
	...
	ErrorText = NStr("en='Cannot connect an external component to generate QR code';ru='Не удалось подключить внешнюю компоненту для генерации QR-кода';vi='Không thể kết nối đến cấu phần ngoài để khởi tạo mã QR'");
...
Procedure MessagePrintingFormIsUnavailable(Object)
	MessageText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='%1 was not printed. The selected print form is not available.';ru='Печать %1 не выполнена: выбранная печатная форма недоступна.';vi='Chưa in %1: mẫu in đã chọn không khả dụng.'"),
		Object);
...
Procedure ExecutePrintToFileCommand(PrintCommand, SavingSettings, ObjectList, Result)
	...
	If PrintCommand.PrintManager = "StandardSubsystems.AdditionalReportsAndDataProcessors" Then
		...
		PrintInfo = GenerateExternalPrintForm(Source, PrintCommand.ID, ObjectList);
	Else
		PrintInfo = GeneratePrintForms(PrintCommand.PrintManager, PrintCommand.ID,
		ObjectList, PrintCommand.AdditionalParameters);
	EndIf;
	
	PrintFormsCollection = PrintInfo.PrintFormsCollection;
	PrintObjects = PrintInfo.PrintObjects;
	
	AreasSignaturesAndSeals = Undefined;
	If SavingSettings.SignatureAndSeal Then
		AreasSignaturesAndSeals = AreasSignaturesAndSeals(PrintObjects);
	EndIf;
	
	FormatsTable = SpreadsheetDocumentSavingFormatsSettings();
	
	For Each PrintForm In PrintFormsCollection Do
		If ValueIsFilled(PrintForm.OfficeDocuments) Then
			For Each OfficeDocument In PrintForm.OfficeDocuments Do
				File = Result.Add();
				File.FileName = OfficeDocumentFileName(OfficeDocument.Value, SavingSettings.TransliterateFilesNames);
		...
		For Each ObjectMatchToPrintedForm In PrintFormsByObjects Do
			...
			PrintObject = ObjectMatchToPrintedForm.Key;
			SpreadsheetDocument = ObjectMatchToPrintedForm.Value;
			
			For Each Format In SavingSettings.SavingFormats Do
				FileType = Format;
				...
				SpecifiedPrintFormsNames = PrintForm.FileNamePrintedForm;
				PrintedFormName = PrintForm.TemplateSynonym;
				
				FileName = ObjectPrintedFormFileName(PrintObject, SpecifiedPrintFormsNames, PrintedFormName) + "." + FileExtention;
				If SavingSettings.TransliterateFilesNames Then
					FileName = StringFunctionsClientServer.StringInLatin(FileName)
				EndIf;
				FileName = CommonUseClientServer.ReplaceProhibitedCharsInFileName(FileName);
				
				File = Result.Add();
				File.FileName = FileName;
				File.BinaryData = SpreadsheetDocumentInBinaryData(SpreadsheetDocument, FileType);
			EndDo;
		EndDo;
	EndDo;
...
Function PackIntoArchive(FileList)
	...
	TemporaryFolder = FileSystem.CreateTemporaryDirectory();
	
	CreateDirectory(TemporaryFolder);
	
	For Each File In FileList Do
		FileName = TemporaryFolder + File.FileName;
	...
	FileSystem.DeleteTemporaryDirectory(TemporaryFolder);
...
// Returns:
// 	BinaryData - 
Function SpreadsheetDocumentInBinaryData(SpreadsheetDocument, Format)
	
	TempFileName = GetTempFileName();
	SpreadsheetDocument.Write(TempFileName, Format);
	
	If Format = SpreadsheetDocumentFileType.HTML Then
		InsertImagesToHTML(TempFileName);
	EndIf;
...
Function GenerateExternalPrintForm(AdditionalInformationProcessorRef, ID, ObjectList)
	
	SourceParameters = New Structure;
	SourceParameters.Insert("CommandID", ID);
	SourceParameters.Insert("DestinationObjects", ObjectList);
	
	PrintFormsCollection = Undefined;
	PrintObjects = New ValueList;
	OutputParameters = PrepareOutputParametersStructure();
	
	PrintByExternalSource(AdditionalInformationProcessorRef, SourceParameters, PrintFormsCollection,
	PrintObjects, OutputParameters);
...
Procedure InsertImagesToHTML(HTMLFileName) Export
...
Function ObjectsSignaturesAndSeals(Val PrintObjects) Export
	...
	ObjectList = PrintObjects.UnloadValues();
	SignaturesAndSeals = New Map;
//	SSLSubsystemsIntegration.OnGetSignaturesAndSeals(ObjectList, SignaturesAndSeals);
//	PrintManagementOverridable.OnGetSignaturesAndSeals(ObjectList, SignaturesAndSeals);
...
Procedure AddSignatureAndSeal(SpreadsheetDocument, AreasSignaturesAndSeals) Export
	...
	For Each Draw In SpreadsheetDocument.Drawings Do
		Position = StrFind(Draw.Name, "_Document_");
		If Position > 0 Then
			ObjectAreaName = Mid(Draw.Name, Position + 1);
			...
			Picture = SignaturesAndSealsSet[Left(Draw.Name, Position - 1)];
			If Picture <> Undefined Then
				Draw.Picture = Picture;
			EndIf;
			Draw.Line = New Line(SpreadsheetDocumentDrawingLineType.None);
		EndIf;
	EndDo;
...
Procedure RemoveSignatureAndSeal(SpreadsheetDocument, HideSignaturesAndSeals = False) Export
	...
	For Each Draw In SpreadsheetDocument.Drawings Do
		If IsSignatureOrSeal(Draw) Then
			Draw.Picture = New Picture;
			Draw.Line = New Line(SpreadsheetDocumentDrawingLineType.None);
			If HideSignaturesAndSeals Then
				DrawingsToDelete.Add(Draw);
			EndIf;
		EndIf;
	EndDo;
	
	For Each Draw In DrawingsToDelete Do
		SpreadsheetDocument.Drawings.Delete(Draw);
	EndDo;
	
EndProcedure

Function IsSignatureOrSeal(Draw) Export
	
	Return Draw.DrawingType = SpreadsheetDocumentDrawingType.Picture And StrFind(Draw.Name, "_Document_") > 0;
	
EndFunction

Function ObjectPrintedFormFileName(PrintObject, FileNamePrintedForm, PrintedFormName) Export
	
	If PrintObject = Undefined Or PrintObject = "PrintObjectsNotSpecified" Then
		If ValueIsFilled(PrintedFormName) Then
			Return PrintedFormName;
		EndIf;
	...
	Return DefaultPrintedFormFileName(PrintObject, PrintedFormName);
...
Function DefaultPrintedFormFileName(PrintObject, PrintedFormName)
	...
	If CommonUse.ThisIsDocument(Metadata.FindByType(TypeOf(PrintObject))) Then
		...
		If DocumentContainsNumber Then
			...
			Pattern = NStr("en='[PrintedFormName] № [Number] от [Date]';ru='[PrintedFormName] № [Number] от [Date]';vi='[PrintedFormName] số [Number] ngày [Date]'");
		Else
			AttributesList = "Date";
			Pattern = NStr("en='[PrintedFormName] от [Date]';ru='[PrintedFormName] от [Date]';vi='[PrintedFormName] ngày [Date]'");
		EndIf;
		
		ParametersForInsert = CommonUse.ObjectAttributesValues(PrintObject, AttributesList);
		If CommonUse.SubsystemExists("StandardSubsystems.ObjectPrefixation") And DocumentContainsNumber Then
			ModuleObjectPrefixationClientServer = CommonUse.CommonModule("ObjectPrefixationClientServer");
			ParametersForInsert.Number = ModuleObjectPrefixationClientServer.NumberForPrinting(ParametersForInsert.Number);
		EndIf;
	...
	Else
		...
		Pattern = NStr("en='[PrintedFormName] - [ObjectPresentation] - [CurrentDate]';ru='[PrintedFormName] - [ObjectPresentation] - [CurrentDate]';vi='[PrintedFormName] - [ObjectPresentation] - [CurrentDate]'");
