...
Function InitializePrintFormOOWriter(Val Pattern = Undefined) Export
	...
	Except
		EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(), "Error",
			NStr("en='An error occurred when contacting service manager (com.sun.star.ServiceManager).';ru='Ошибка при связи с сервис менеджером (com.sun.star.ServiceManager).';vi='Lỗi khi liên kết với công cụ bộ quản lý (com.sun.star.ServiceManager).'") + 
	...
	Except
		EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(), "Error",
			NStr("en='An error occurred when launching Desktop service (com.sun.star.frame.Desktop).';ru='Ошибка при запуске сервиса Desktop (com.sun.star.frame.Desktop).';vi='Lỗi khi khởi động công cụ Desktop (com.sun.star.frame.Desktop).'") + 
			+ Chars.LF + DetailErrorDescription(ErrorInfo()),,True);
		FailedToGeneratePrintForm(ErrorInfo());
	EndTry;
...
Function GetAreaBeginPosition(Val xDocument, Val AreaName)
	...
	If xFound = Undefined Then
		Raise NStr("en='Template area start is not found:';ru='Не найдено начало области макета:';vi='Không tìm thấy đầu vùng khuôn in:'") + " " + AreaName;	
	EndIf;
	Return xFound.End;
...
Function GetAreaEndPosition(Val xDocument, Val AreaName)
	...
	xFound = xDocument.findFirst(xSearchDescr);
	If xFound = Undefined Then
		Raise NStr("en='End of template area is not found:';ru='Не найден конец области макета:';vi='Không tìm thấy cuối vùng khuôn in:'") + " " + AreaName;	
	EndIf;
	Return xFound.Start;
...
Function EventLogMonitorEvent()
	Return NStr("en='Print';ru='Печать';vi='In'");
EndFunction

Procedure FailedToGeneratePrintForm(ErrorInfo)
#If WebClient Or MobileClient Then
	CorrectionText = NStr("en='Working via web client requires Windows Internet Explorer.';ru='При работе через веб, требуется браузер Internet Explorer под управлением операционной системы Windows.';vi='Khi làm việc qua Web, cần trình duyệt Internet Explorer với hệ điều hành Windows.'");
#Else		
	CorrectionText = "";	
#EndIf
...
"Для вывода печатных форм в формате OpenOffice.org Writer требуется, чтобы на компьютере был установлен пакет OpenOffice.org. %2';vi='Không thể tạo mẫu in: %1."
"Để đưa ra mẫu in với định dạng OpenOffice.org Writer cần thiết trên máy tính đã cài đặt gói OpenOffice.org. %2'"),
