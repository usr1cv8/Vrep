...
// 	CommandParameters.SupportText = NStr("en='Executing the command...';ru='Выполняется команда...';vi='Đang thực hiện lệnh...'");
...
Procedure RunCommandInBackground(CommandID, CommandParameters, Form) Export
	...
	If WrongType OR AdditionalInformationProcessorRef = PredefinedValue("Catalog.AdditionalReportsAndDataProcessors.EmptyRef") Then
		...
		ErrorText = NStr("en='Incorrect value of parameter AdditionalDataProcessorRef:';ru='Некорректное значение параметра ""ДополнительнаяОбработкаСсылка"":';vi='Sai giá trị tham số ""AdditionalDataProcessorRef"":'") + Chars.LF;
		If WrongType Then
			ErrorText = ErrorText + StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Type ""%1"" is transferred, ""%2"" was expected.';ru='Передан тип ""%1"", ожидался ""%2"".';vi='Truyền kiểu ""%1"", dự tính ""%2"".'"),
				String(TypeOf(AdditionalInformationProcessorRef)),
				String(Type("CatalogRef.AdditionalReportsAndDataProcessors")));
		Else
			ErrorText = ErrorText + NStr("en='Null reference passed. Data processor might have been opened directly.';ru='Передана пустая ссылка. Вероятно, обработка была открыта напрямую.';vi='Truyền tham chiếu rỗng. Có thể, đã mở trực tiếp bộ xử lý.'");
		EndIf;
...
Function ExecuteAllocatedCommandAtClient(Form, ItemName) Export
	...
	If Object.Ref.IsEmpty() OR Form.Modified Then
		QuestionText = StrReplace(
			NStr("en='To run the ""%1"" command, write the data.';ru='Для выполнения команды ""%1"" необходимо записать данные.';vi='Để thực hiện lệnh ""%1"" cần ghi lại dữ liệu.'"),
			"%1",
			ExecuteCommand.Presentation);
		
		Buttons = New ValueList;
		Buttons.Add(DialogReturnCode.Yes, NStr("en='Write and continue';ru='Записать и продолжить';vi='Ghi lại và tiếp tục'"));
...
Procedure ShowNotificationOnCommandExecution(ExecuteCommand) Export
	If ExecuteCommand.ShowAlert Then
		ShowUserNotification(NStr("en='Executing the command...';ru='Команда выполняется...';vi='Đã thực hiện lệnh...'"), , ExecuteCommand.Presentation);
	EndIf;
EndProcedure
...
"Команда ""%2"" не может быть выполнена.';vi='Đối với báo cáo hoặc bộ xử lý ""%1"" chưa"
"ấn định biểu mẫu chính hoặc biểu mẫu chính không được đùng để khởi động trong ứng dụng thường."
"Không thể thực hiện lệnh ""%2"".'"),
...
Procedure EditMultilineText(FormOrHandler, EditText, PropsOwner, AttributeName, Val Title = "") Export
	...
	If IsBlankString(Title) Then
		Title = NStr("en='Note';ru='Примечание';vi='Ghi chú'");
	EndIf;
...
Procedure ExportToFile(ExportParameters) Export
	MessageText = NStr("en='To export an external data processor (repot) to the file, install extension for 1C:Enterprise web client.';ru='Для выгрузки внешней обработки (отчета) в файл рекомендуется установить расширение для веб-клиента 1С:Предприятие.';vi='Để kết xuất bộ xử lý (báo cáo) ngoài vào tệp, cần thiết lập phần mở rộng đối với Web-client 1C:DOANH NGHIỆP.'");
...
Procedure ExportToFileEnd(Attached, ExportParameters) Export
	...
	SaveFileDialog.Title = NStr("en='Specify file';ru='Укажите файл';vi='Hãy chỉ ra tệp'");
