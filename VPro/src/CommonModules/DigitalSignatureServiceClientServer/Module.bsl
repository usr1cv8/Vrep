	...
	If Application <> Undefined Then
		...
		If ApplicationNotFound Then
			CryptographyManagerAddError(Errors, Application,
				NStr("en='Not for usage.';ru='Не предусмотрена для использования.';vi='Chưa có sẵn để sử dụng'"), True);
			Return Undefined;
		EndIf;
...
Function CryptographyManagerApplicationProperties(ApplicationDescription, IsLinux, Errors, IsServer,
	...
	If Not ValueIsFilled(ApplicationDescription.ApplicationName) Then
		CryptographyManagerAddError(Errors, ApplicationDescription.Ref,
			NStr("en='Application name is not specified.';ru='Не указано имя программы.';vi='Chưa chỉ ra tên chương trình.'"), True);
		Return Undefined;
	EndIf;
	
	If Not ValueIsFilled(ApplicationDescription.ApplicationType) Then
		CryptographyManagerAddError(Errors, ApplicationDescription.Ref,
			NStr("en='Application type is not specified.';ru='Не указан тип программы.';vi='Chưa chỉ ra kiểu chương trình.'"), True);
		Return Undefined;
	EndIf;
	...
	If IsLinux Then
		...
		If Not ValueIsFilled(PathToApplication) Then
			CryptographyManagerAddError(Errors, ApplicationDescription.Ref,
				NStr("en='Not for usage.';ru='Не предусмотрена для использования.';vi='Chưa có sẵn để sử dụng'"), IsServer, , , True);
			Return Undefined;
		EndIf;
	Else
		PathToApplication = "";
...
Function CryptographyManagerAlgorithmsSet(ApplicationDescription, Manager, Errors) Export
	...
	Except
		...
		CryptographyManagerAddError(Errors, ApplicationDescription.Ref, StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Unknown signature algorithm ""%1"" is selected.';ru='Выбран неизвестный алгоритм подписи ""%1"".';vi='Đã chọn thuật toán chữ ký không xác định ""%1"".'"), SignAlgorithm), True);
		Return False;
	EndTry;
	...
	Except
		...
		// The platform uses the general Unknown cryptography algorithm message. Required more specific.
		CryptographyManagerAddError(Errors, ApplicationDescription.Ref, StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Unknown hash algorithm ""%1"" is selected.';ru='Выбран неизвестный алгоритм хеширования ""%1"".';vi='Đã chọn thuật toán Hash không xác định ""%1"".'"), HashAlgorithm), True);
		Return False;
	EndTry;
	...
	Except
		...
		// The platform uses the general Unknown cryptography algorithm message. Required more specific.
		CryptographyManagerAddError(Errors, ApplicationDescription.Ref, StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Unknown encryption algorithm ""%1"" is selected.';ru='Выбран неизвестный алгоритм шифрования ""%1"".';vi='Đã chọn thuật toán mã hóa không xác định ""%1"".'"), EncryptionAlgorithm), True);
		Return False;
	EndTry;
...
Procedure CryptographyManagerApplicationNotFound(ApplicationDescription, Errors, IsServer) Export
	...
	CryptographyManagerAddError(Errors, ApplicationDescription.Ref,
		NStr("en='Application was not found on computer.';ru='Программа не найдена на компьютере.';vi='Không tìm thấy chương trình trên máy tính.'"), IsServer, True);
...
Function CryptographyManagerApplicationNameMatch(ApplicationDescription, ApplicationNameReceived, Errors, IsServer) Export
	...
	If ApplicationNameReceived <> ApplicationDescription.ApplicationName Then
		CryptographyManagerAddError(Errors, ApplicationDescription.Ref, StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Another application with name ""%1"" received.';ru='Получена другая программа с именем ""%1"".';vi='Nhận chương trình khác với tên gọi ""%1"".'"), ApplicationNameReceived), IsServer, True);
		Return False;
	EndIf;
...
Procedure AddCertificatesProperties(Table, CertificatesArray, WithoutFilter, PrintsOnly = False) Export
	...
	#If WebClient Or MobileClient Or ThinClient Then
...
Function DataReceiveErrorTitle(Operation) Export
	...
	If Operation = "Signing" Then
		Return NStr("en='Error occurred when receiving data for signing:';ru='При получении данных для подписания возникла ошибка:';vi='Khi nhận dữ liệu để ký tên đã xảy ra lỗi:'");
		
	ElsIf Operation = "Encryption" Then
		Return NStr("en='An error occurred when retrieving data to be encrypted:';ru='При получении данных для шифрования возникла ошибка:';vi='Khi nhận dữ liệu để mã hóa đã xảy ra lỗi:'");
	Else
		Return NStr("en='An error occurred while receiving decryption data:';ru='При получении данных для расшифровки возникла ошибка:';vi='Khi nhận dữ liệu để giải mã đã xảy ra lỗi:'");
	EndIf;
...
Function EmptySignatureData(SignatureData, ErrorDescription) Export
	...
	If Not ValueIsFilled(SignatureData) Then
		ErrorDescription = NStr("en='Empty signature is generated.';ru='Сформирована пустая подпись.';vi='Lập chữ ký rỗng.'");
		Return True;
	EndIf;
...
Function EmptyEncryptedData(EncryptedData, ErrorDescription) Export
	...
	If Not ValueIsFilled(EncryptedData) Then
		ErrorDescription = NStr("en='Empty data is generated.';ru='Сформированы пустые данные.';vi='Lập dữ liệu rỗng.'");
		Return True;
	EndIf;
