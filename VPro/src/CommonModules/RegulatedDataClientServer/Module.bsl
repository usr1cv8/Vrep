...
Function TINMeetsTheRequirements(Val TIN, ThisLegalEntity, MessageText) Export
	...
	If ThisLegalEntity = Undefined Then
		MessageText = MessageText + NStr("en='TIN owner type is not specified.';ru='Не определен тип владельца ИНН.';vi='Chưa xác định kiểu đối tượng chủ MST.'");
		Return False;
	EndIf;
	
	If Not StringFunctionsClientServer.OnlyNumbersInString(TIN) Then
		MeetsRequirements = False;
		MessageText = MessageText + NStr("en='TIN contains only digits.';ru='ИНН должен состоять только из цифр.';vi='MST phải là các chữ số'");
	EndIf;

	If  ThisLegalEntity AND TINLength <> 10 Then
		MeetsRequirements = False;
		MessageText = MessageText + ?(ValueIsFilled(MessageText), Chars.LF, "")
			+ NStr("en='TIN of a legal entity must contain 10 digits.';ru='ИНН юридического лица должен состоять из 10 цифр.';vi='MST pháp nhân phải có 10 chữ số.'");
	ElsIf Not ThisLegalEntity AND TINLength <> 12 Then
		MeetsRequirements = False;
		MessageText = MessageText + ?(ValueIsFilled(MessageText), Chars.LF, "")
			+ NStr("en='TIN of an individual must contain 12 digits.';ru='ИНН физического лица должен состоять из 12 цифр.';vi='MST cá nhân phải có 12 chữ số.'");
	EndIf;
	...
	If MeetsRequirements Then
		...
		If ThisLegalEntity Then
			...
			If CheckDigit <> Number(Mid(TIN, 10, 1)) Then
				MeetsRequirements = False;
				MessageText = MessageText + ?(ValueIsFilled(MessageText), Chars.LF, "")
				               + NStr("en='Check digit of TIN does not match the calculated value.';ru='Контрольное число для ИНН не совпадает с рассчитанным.';vi='Số kiểm soát đối với MST không trùng khớp với số đã tính.'");
			EndIf;
...
Function MSRNMeetsTheRequirements(Val OGRN, ThisLegalEntity, MessageText) Export
	...
	If ThisLegalEntity = Undefined Then
		MessageText = MessageText + NStr("en='OGRN owner type is not defined.';ru='Не определен тип владельца ОГРН.';vi='Chưa xác định kiểu đối tượng chủ số đăng ký kinh doanh.'");
		Return False;
	EndIf;

	If Not StringFunctionsClientServer.OnlyNumbersInString(OGRN) Then
		MeetsRequirements = False;
		MessageText = MessageText + NStr("en='OGRN should contain only digits.';ru='ОГРН должен состоять только из цифр.';vi='Số đăng ký kinh doanh phải là các chữ số'")
	EndIf;

	If ThisLegalEntity AND OGRNLength <> 13 Then
		MeetsRequirements = False;
		MessageText = MessageText + ?(ValueIsFilled(MessageText), Chars.LF, "")
		               + NStr("en='OGRN of legal entity should contain 13 digits.';ru='ОГРН юридического лица должен состоять из 13 цифр.';vi='Số đăng ký kinh doanh phải có 13 chữ số'");
	ElsIf Not ThisLegalEntity AND OGRNLength <> 15 Then
		MeetsRequirements = False;
		MessageText = MessageText + ?(ValueIsFilled(MessageText), Chars.LF, "")
		               + NStr("en=""Individual's OGRN should contain 15 digits."";ru='ОГРН физического лица должен состоять из 15 цифр.';vi='Số đăng ký kinh doanh phải có 15 chữ số'");
	EndIf;
	...
	If MeetsRequirements Then
		...
		If ThisLegalEntity Then
			...
			If CheckDigit <> Right(OGRN, 1) Then
				MeetsRequirements = False;
				MessageText = MessageText + ?(ValueIsFilled(MessageText), Chars.LF, "")
				               + NStr("en='Check digit of OGRN does not match the calculated value.';ru='Контрольное число для ОГРН не совпадает с рассчитанным.';vi='Số kiểm soát đối với số đăng ký kinh doanh không trùng khớp với số đã tính.'");
			EndIf;
...
Function PFRInsuaranceNumberMeetsTheRequirements(Val InsuranceNumber, MessageText) Export
	...
	If IsBlankString(RowOfDigits) Then
		MessageText = MessageText + NStr("en='Insurance number was not specified';ru='Страховой номер не заполнен';vi='Số bảo hiểm chưa điền'");
		Return False;
	EndIf;
	
	If StrLen(RowOfDigits) < 11 Then
		MessageText = MessageText + NStr("en='Insurance number incomplete';ru='Страховой номер задан неполностью';vi='Số bảo hiểm chỉ ra không đầy đủ'");
		Return False;
	EndIf;
	
	If Not StringFunctionsClientServer.OnlyNumbersInString(RowOfDigits) Then
		MessageText = MessageText + NStr("en='Insurance number must contain only digits.';ru='Страховой номер должен состоять только из цифр.';vi='Số bảo hiểm phải là các chữ số.'");
		Return False;
	EndIf;
	...
	If Number(Left(RowOfDigits, 9)) > 1001998 Then
		...
		If Balance <> ChecksumNumber Then
			MessageText = MessageText + NStr("en='Check digit of insurance number does not match the calculated value.';ru='Контрольное число для страхового номера не совпадает с рассчитанным.';vi='Số kiểm soát đối với số bảo hiểm không trùng khớp với số đã tính.'");
			Return False;
		EndIf;
	EndIf;
