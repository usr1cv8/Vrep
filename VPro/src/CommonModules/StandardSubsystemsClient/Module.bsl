...
Procedure SetAdvancedApplicationCaption(OnLaunch = False) Export
	...
	Else
		CaptionPattern = "%1 / %2";
		ApplicationCaption = StringFunctionsClientServer.SubstituteParametersInString(CaptionPattern, 
			NStr("en='Separators are not set';ru='Не установлены разделители';vi='Chưa đặt đường chia tách'"), ClientParameters.DetailedInformation);
	EndIf;
...
Procedure BeforeExit(Cancel = False, WarningText) Export
	...
	// No warnings in thick client (ordinary application).
	...
	If NOT ClientParameter("CanUseSeparatedData") Then
		Return;
	EndIf;
	...
	For Each Handler In EventHandlers Do
		Handler.Module.BeforeExit(Cancel, Warnings);
	EndDo;
	
	If Warnings.Count() = 0 Then
		If NOT ClientParameter("AskConfirmationOnClose") Then
			Return;
		EndIf;
		
		WarningText = NStr("en='Do you want to exit the application?';ru='Завершить работу с программой?';vi='Kết thúc công việc với chương trình?'");
		Cancel = True;
	Else
		Cancel = True;
		...
		For Each Warning In Warnings Do
			WarningArray.Add(Warning.ExplanationText);
		EndDo;
		...
		AttachIdleHandler("ShowNotificationOnExit", 0.1, True);
	EndIf;
	
	SetClientParameter("WarningsOnExit", Warnings);
...
Function ClientParameter(ParameterName = Undefined) Export
	...
	If ClientParameters = Undefined Then
		StandardSubsystemsClientReUse.ClientWorkParametersOnStart();
	EndIf;
...
Procedure ShowWarningAndContinue(Parameters, WarningText) Export
	...
	If Parameters.Cancel Then
		...
		Buttons.Add("Restart", NStr("en='Restart';ru='Перезапустить';vi='Khởi động lại'"));
		Buttons.Add("Complete",     NStr("en='Exit';ru='Завершить';vi='Kết thúc'"));
...
Procedure ShowExecutionResult(Form, Result, EndProcessor = Undefined) Export
	...
	If Result.Property("OutputWarning") AND Result.OutputWarning.Use Then
		...
		If ValueIsFilled(OutputWarning.ErrorsText) Then
			Buttons = New ValueList;
			Buttons.Add(1, NStr("en='More...';ru='Подробнее...';vi='Chi tiết...'"));
			If TypeOf(Form) = Type("ManagedForm") AND ValueIsFilled(OutputWarning.PathToFormAttribute) Then
				Buttons.Add(2, NStr("en='Go to attribute';ru='Перейти к реквизиту';vi='Chuyển đến mục tin'"));
			EndIf;
			Buttons.Add(0, NStr("en='Continue';ru='Продолжить';vi='Tiếp tục'"));
	...
	If TypeOf(Response) = Type("Number") Then
		If Response = 1 Then
			...
			If IsBlankString(Title) Then
				Title = NStr("en='Explanation';ru='Расшифровка';vi='Diễn giải'");
			EndIf;
...
"Необходимо предварительно обновить версию платформы 1С:Предприятие.';vi='Không thể đăng nhập vào chương trình."
"Không nên cập nhật trước phiên bản nền tảng 1C:DOANH NGHIỆP.'");
		Else
			MessageText = NStr("en='It is impossible to log in the application."
"It is necessary to contact the administrator to update the 1C:Enterprise platform version.';ru='Вход в программу невозможен."
"Необходимо обратиться к администратору для обновления версии платформы 1С:Предприятие.';vi='Không thể đăng nhập vào chương trình."
"Cần liên hệ với người quản trị để cập nhật phiên bản nền tảng 1C:DOANH NGHIỆP.'");
		EndIf;
	Else
		If ClientParameters.HasAccessForUpdateVersionsOfPlatform Then
			MessageText = 
...
"В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.';vi='Cần kết thúc công việc với chương trình và cập nhật phiên bản nền tảng 1C:DOANH NGHIỆP."
"Trong trường hợp ngược lại, sẽ không sử dụng được một số tính năng của chương trình hoặc các tính năng này hoạt động không chính xác.'");
		Else
			MessageText = 
...
"В противном случае некоторые возможности программы будут недоступны или будут работать некорректно.';vi='Cần kết thúc làm việc với chương trình và liên hệ với người quản trị để cập nhật phiên bản nền tảng 1C:DOANH NGHIỆP."
"Trong trường hợp ngược lại, sẽ không sử dụng một số tính năng của chương trình hoặc các tính năng đó hoạt động không chính xác.'");
		EndIf;
	EndIf;
...
"Обратитесь к администратору за подробностями.';vi='Tạm thời không thể đăng nhập vào chương trình đến khi phục hồi kết nối với nút chính."
...
"перейдите в меню Все функции - Стандартные - Активные пользователи.';vi='Để mở danh sách người sử dụng đang làm việc,"
"hãy chuyển đến menu Tất cả tính năng - Chuẩn - Người sử dụng đang làm việc.'"));
...
"запуска ВойтиВОбластьДанных, дополнительным параметром необходимо указать значение разделителя.';vi='Khi chỉ ra tham số"
"khởi động ВойтиВОбластьДанных, bằng tham số bổ sung cần chỉ ra giá trị chia tách.'");
	EndIf;
	...
	Except
		Raise
			NStr("en='The separator value in the LogOnDataArea parameter must be a number.';ru='Значением разделителя в параметре ВойтиВОбластьДанных должно быть число.';vi='Giá trị đường chia tách trong tham số LogOnDataArea phải là một số.'");
	EndTry;
...
#If WebClient Or MobileClient Or ThickClientOrdinaryApplication Then
	Return;
#EndIf
...
Procedure AskExitConfirmation(Parameters, ResponseProcessor) Export
	...
	Buttons = New ValueList;
	Buttons.Add("DialogReturnCode.Yes",  NStr("en='Exit';ru='Завершить';vi='Kết thúc'"));
	...
	QuestionParameters.Title = NStr("en='Exit';ru='Завершить';vi='Kết thúc'");
	QuestionParameters.DontAskAgain = False;
	
	ShowQuestionToUser(ResponseProcessor, NStr("en='Exit the application?';ru='Завершить работу с программой?';vi='Kết thúc làm việc với chương trình?'"), Buttons, QuestionParameters);
...
Procedure OpenApplicationWarningForm(Parameters, ResponseProcessor, UserWarning, FormName, FormParameters)
	...
	If ActionHyperlink.Property("ApplicationWarningForm", Form) Then
		...
		If ActionHyperlink.Property("ApplicationWarningFormParameters", FormParameters) Then
			...
			FormParameters.Insert("YesButtonTitle",  NStr("en='Complete';ru='Закончить редактирование';vi='Kết thúc soạn'"));
			FormParameters.Insert("TitleNoButton", NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'"));
...
Procedure ProcessErrorOnStartOrEnd(Parameters, ErrorInfo, Event, StopWork = False)
	...
	WarningText = ErrorDescriptionBegin + Chars.LF
		+ NStr("en='Technical information on the error is recorded in the event log.';ru='Техническая информация об ошибке записана в журнал регистрации.';vi='Thông tin kỹ thuật về lỗi đã được ghi lại trong nhật ký sự kiện.'")
		+ Chars.LF + Chars.LF
...
Procedure ShowFilePlaceWhenConnectingFileExtensions(ExtensionAttached, AdditionalParameters) Export
	...
	If DialogueParameters.Property("Mode") Then
		...
		If Mode = FileDialogMode.ChooseDirectory Then
			Raise NStr("en='Directory selection is not supported.';ru='Выбор каталога не поддерживается';vi='Không hỗ trợ chọn thư mục'");
		EndIf;
	Else
