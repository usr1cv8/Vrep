...
Procedure DeleteFilesAtServer(FormerPathOnVolume) Export
	...
	If FileTemporary.Exist() Then
		...
		Except
			WriteLogEvent(
				NStr("en='Files.Delete files from the volume on exchange';ru='Файлы.Удаление файлов в томе при обмене';vi='Tệp.Xóa tệp trong quyển khi trao đổi'",
...
Function AddFileToVolume(BinaryDataOrPath, ModificationTimeUniversal, BaseName, Extension,
	...
	If Selection.Count() = 0 Then
		Raise NStr("en='There are no volumes to place files in';ru='Нет ни одного тома для размещения файлов';vi='Không có bất kỳ quyển nào để sắp xếp tệp'");
	EndIf;
			...
			If VolumeRef.MaximumSize <> 0 Then
				...
				If NewSize > VolumeRef.MaximumSize Then
					...
					Raise StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='Maximum volume size exceeded (%1 MB).';ru='Превышен максимальный размер тома (%1 Мб).';vi='Vượt quá dung lượng tối đa của quyển (%1 Mb).'"),
...
"Список"
""
"ошибок: %1';vi='Không thể thêm tệp vào một quyển nào."
"Danh sách"
""
"lỗi: %1'");
	
	WriteLogEvent(
		NStr("en='Files. Adding a file';ru='Файлы.Добавление файла';vi='Tệp.Thêm tệp'", CommonUseClientServer.MainLanguageCode()),
		EventLogLevel.Error,,,
...
"добавить файл: ""%1.%2""."
""
"Обратитесь к администратору.';vi='Không thể"
"thêm tệp: ""%1.%2""."
""
...
Procedure CryptographyOnCreateFormAtServer(Form, ThisIsListForm = True, RowsPictureOnly = False) Export
	...
	If ESigning AND Encryption Then
		Title = NStr("en='Encryption';ru='Шифрование';vi='Mã hóa'");
		ToolTip = NStr("en='Encryption existence';ru='Наличие шифрования';vi='Có mã hóa'");
		Picture  = PictureLib["DigitallySignedEncryptedTitle"];
	ElsIf ESigning Then
		Title = NStr("en='Digital signature';ru='Электронная подпись';vi='Chữ ký điện tử'");
		ToolTip = NStr("en='Digital signature existence';ru='Наличие электронной подписи';vi='Có chữ ký điện tử'");
...
"должен быть в формате UNC (\\servername\resource)';vi='Đường dẫn đến dữ liệu File-server"
"cần có định dạng UNC (\\servername\resource)'"),
						,
...
"файловой базе должен быть в формате UNC (\\servername\resource)';vi='Đường dẫn"
"đến dữ liệu File-server với định dạng UNC (\\servername\resource)'"),
					,
	...
	If IsBlankString(FileBaseFullName) Then
		...
		CommonUseClientServer.MessageToUser(
			NStr("en='Specify a full name of the file base (file 1cv8.1cd)';ru='Укажите полное имя файловой базы (файл 1cv8.1cd)';vi='Hãy chỉ ra tên gọi đầy đủ cơ sở File-server (tệp 1cv8.1cd)'"),,
			"WindowsFileBaseFullName");
		Return False;
...
"Введите другое имя файла.';vi='Đã tồn tại tệp ""%1""."
"Hãy nhập tên khác của tệp.'"),
				FileBaseFullName),, "WindowsFileBaseFullName");
		Return False;
	EndIf;
	...
	If AreFilesInVolumes Then
		...
		If IsBlankString(PathToArchiveWithVolumeFiles) Then
			CommonUseClientServer.MessageToUser(
				NStr("en='Specify a full name of an archive with volume files (file *.zip)';ru='Укажите полное имя архива с файлами томов (файл *.zip)';vi='Hãy chỉ ra tên gọi đầy đủ lưu trữ có các tệp trong quyển (tệp *.zip)'"),, 
				"PathToArchiveWithWindowsVolumesFiles");
			Return False;
		EndIf;
...
"файлами томов должен быть в формате UNC (\\servername\resource).';vi='Đường dẫn đến lưu trữ với"
"các tệp trong quyển cần có định dạng UNC (\\servername\resource).'"),
					,
					"PathToArchiveWithWindowsVolumesFiles");
				Return False;
			EndIf;
...
Procedure ExtractTextFromFilesOnServer() Export
	...
	WriteLogEvent(
		NStr("en='Files.Text extraction';ru='Файлы.Извлечение текста';vi='Tệp.Trích dẫn nội dung'",
		     CommonUseClientServer.MainLanguageCode()),
		EventLogLevel.Information,
		,
		,
		NStr("en='Scheduled text extraction is started';ru='Начато регламентное извлечения текста';vi='Bắt đầu lấy ra nội dung'"));
...
"текста"
"из файла"
"""%1"" произошла ошибка: ""%2"".';vi='Khi lấy ra thường kỳ"
"nội dung"
"từ tệp"
"""%1"", đã xảy ra lỗi: ""%2"".'"),
					NameWithExtensionFile,
	...
	WriteLogEvent(
		...
		,
		,
		NStr("en='Scheduled text extraction is completed';ru='Закончено регламентное извлечение текста';vi='Kết thúc lấy nội dung ra thường kỳ'"));
