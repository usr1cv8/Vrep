...
Function ProcessStartParameters(Val LaunchParameterValue, Val LaunchParameters) Export
	...
	If LaunchParameterValue = Upper("AllowUsersWork") Then
		...
		If Not InfobaseConnectionsServerCall.AllowUsersWork() Then
			MessageText = NStr("en='Launch parameter AllowUsersWork is not processed. You are not authorized to administer the infobase.';ru='Параметр запуска РазрешитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.';vi='Tham số khởi động AllowUsersWork không được xử lý. Không có quyền quản trị cơ sở thông tin.'");
			ShowMessageBox(,MessageText);
			Return False;
		EndIf;
		
		EventLogMonitorClient.AddMessageForEventLogMonitor(InfobaseConnectionsClientServer.EventLogMonitorEvent(),,
			NStr("en='Started with the ""AllowUsersWork"" parameter. The application will be closed.';ru='Выполнен запуск с параметром ""AllowUsersWork"". Работа программы будет завершена.';vi='Đã thực hiện khởi động với tham số ""AllowUsersWork"". Công việc với chương trình sẽ kết thúc.'"), ,True);
		Exit(False);
		Return True;
	...
	ElsIf LaunchParameterValue = Upper("TerminateUserSessions") Then
		...
		If Not InfobaseConnectionsServerCall.SetConnectionLock() Then
			MessageText = NStr("en='The EndUsersWork launch parameter is not processed. You are not authorized to administer the infobase.';ru='Параметр запуска ЗавершитьРаботуПользователей не отработан. Нет прав на администрирование информационной базы.';vi='Tham số khởi động EndUsersWork không được xử lý. Không có quyền quản trị cơ sở thông tin.'");
			ShowMessageBox(,MessageText);
			Return False;
		EndIf;
...
Procedure TerminateThisSession(ShowQuestion = True) Export
	...
	MessageText = NStr("en='User operation in the application is prohibited. Close this session?';ru='Работа пользователей с программой запрещена. Завершить работу этого сеанса?';vi='Kết thúc công việc của người sử dụng với chương trình. Kết thúc công việc của phiên làm việc này?'");
	Title = NStr("en='End the current session';ru='Завершение работы текущего сеанса';vi='Kết thúc công việc của phiên làm việc hiện tại'");
...
Procedure OnlineDataProcessorBeforeStart(Parameters, NotSpecified) Export
	...
	If Not IsBlankString(QuestionText) Then
		...
		Buttons.Add(DialogReturnCode.Yes, NStr("en='Login';ru='Логин';vi='Tên đăng nhập'"));
		If ClientParameters.CanRemoveLock Then
			Buttons.Add(DialogReturnCode.No, NStr("en='Unlock and log on';ru='Снять блокировку и войти';vi='Bỏ phong tỏa và đăng nhập'"));
		EndIf;
...
Procedure AskQuestionOnWorkEnd(MessageText) Export
	...
	QuestionText = NStr("en='%1 Close?';ru='%1 Завершить работу?';vi='%1 Kết thúc công việc?'");
