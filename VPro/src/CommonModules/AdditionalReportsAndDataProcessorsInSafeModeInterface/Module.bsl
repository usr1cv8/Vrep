...
"Для использования COM-класса требуется переработать дополнительную обработку для работы без режима обратной совместимости';vi='Không thể cho phép sử dụng lớp COM %1 đối với bộ xử lý bổ sung làm việc ở chế độ tương thích ngược với cơ chế cho phép đã thực thi trong phiên bản Thư viện các phân hệ chuẩn 2.1.3."
"Để sử dụng lớp COM, cần điều chỉnh bộ xử lý bổ sung để làm việc không ở chế độ tương thích ngược'"),
				  ProgId
		);
