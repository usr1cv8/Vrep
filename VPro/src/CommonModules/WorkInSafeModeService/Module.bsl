...
"на использование внешней компоненты: некорректное имя макета %1!';vi='Không thể tạo phần mở rộng"
"để sử dụng cấu phần ngoài: sai tên khuôn in %1!'"), TemplateName);
	EndIf;
...
"на использование внешней компоненты, поставляемой в макете %1: макет %1 не обнаружден в составе конфигурации!';vi='Không thể tạo phần mở rộng"
"để sử dụng cấu phần ngoài đặt trong khuôn in %1: khuôn in %1 không được tìm thấy trong thành phần cấu hình!'"), TemplateName);
	EndIf;
...
"на использование внешней компоненты: макет %1 не содержит двоичных данных!';vi='Không thể tạo phần mở rộng"
"để sử dụng cấu phần ngoài: khuôn in %1 không chứa dữ liệu nhị phân!'"), TemplateName);
	EndIf;
...
"на использование внешней компоненты, поставляемой в макете %1: в архиве не обнаружен файл MANIFEST.XML!';vi='Không thể tạo phần mở rộng"
"để sử dụng cấu phần ngoài đặt trong khuôn in %1: trong lưu trữ không tìm thấy tệp MANIFEST.XML!'"), TemplateName);
	EndIf;
...
Procedure OnWriteServiceData(Object) Export
	...
	If WorkInSafeMode.SafeModeIsSet() Then
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Writing of object %1 is unavailable: safe mode is set: %2.';ru='Запись объекта %1 недоступна: установлен безопасный режим: %2!';vi='Không cho phép ghi lại đối tượng %1: đã đặt chế độ an toàn: %2!'"),
