...
Procedure OnAddUpdateHandlers(Handlers) Export
	...
	//Handler.Comment      = NStr("en='Updates data of the first show.';ru='Актуализирует данные первого показа.';vi='Đang cập nhật dữ liệu hiển thị lần đầu.'");
...
Function CacheForFirstShow() Export
	...
	//	
	//	If Lower(PagesPackage.Section) = Lower(NStr("en='Advertising';ru='Реклама';vi='Quảng cáo'")) Then
...
Function ExtractPackageFiles(TemplateName) Export
	...
	Except
		WriteLogEvent(
			NStr("en='Information on start';ru='Информация при запуске';vi='Thông tin khi khởi động'", CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,
			,
			,
