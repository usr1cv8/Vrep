...
Function GetXSLTransformFromCommonTemplate(Val CommonTemplateName) Export
	...
	Except
		WriteLogEvent(NStr("en='Receive XSL';ru='Получение XSL';vi='Nhận XSL'", CommonUseClientServer.MainLanguageCode()),
...
Function SeparatedMetadataObjects(Val Delimiter) Export
	...
	If CommonAttributeMetadata = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Common attribute %1 was not found in configuration.';ru='Общий реквизит %1 не обнаружен в конфигурации!';vi='Chưa tìm thấy mục tin chung %1 trong cấu hình!'"), Delimiter);
	EndIf;
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Data separation is not used for common attribute %1.';ru='Для общего реквизита %1 не используется разделение данных!';vi='Đối với mục tin chung %1 không sử dụng phân chia dữ liệu!'"), Delimiter);
	...
	For Each SequenceMetadata IN Metadata.Sequences Do
		...
		If SequenceMetadata.Documents.Count() = 0 Then
			...
			MessagePattern = NStr("en='Sequence %1 does not include any documents.';ru='В последовательность %1 не включено ни одного документа.';vi='Trong chuỗi nối tiếp %1 không có bất kỳ chứng từ nào.'");
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, SequenceMetadata.Name);
			WriteLogEvent(NStr("en='Receive separate metadata objects';ru='Получение разделенных объектов метаданных';vi='Nhận đối tượng metadata riêng'", 
	...
	For Each DocumentJournalMetadata IN Metadata.DocumentJournals Do
		...
		If DocumentJournalMetadata.RegisteredDocuments.Count() = 0 Then
			...
			MessagePattern = NStr("en='Journal %1 does not contain any documents.';ru='В журнал %1 не включено ни одного документа.';vi='Trong nhật ký %1 không có bất kỳ chứng từ nào.'");
	...
	If WantedUpdate Then
		...
		Else
			...
			DescriptionSchTask = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Updating version cache. Identifier of record %1. Data type %2.';ru='Обновление кэша версий. Идентификатор записи %1. Тип данных %2.';vi='Cập nhật cache phiên bản. Tên bản ghi %1. Kiểu dữ liệu %2.'"),
				ID,
				DataType);
				...
				Except
					For Each OriginalTask IN Tasks Do
						...
						If Job.ErrorInfo <> Undefined Then
							WriteLogEvent(NStr("en='Updating version cache';ru='Обновление кэша версий';vi='Cập nhật cache phiên bản'", CommonUseClientServer.MainLanguageCode()),
								EventLogLevel.Error,
								,
								,
						...
						Else
							...
							Raise(NStr("en='An unknown error occurred while updating the version cache';ru='Неизвестная ошибка при выполнения задания обновления кэша версий';vi='Lỗi không xác định khi thực hiện nhiệm vụ cập nhật cache các phiên bản'"));
						EndIf;
					EndDo;
					
					Raise(NStr("en='An unknown error occurred while updating the version cache';ru='Неизвестная ошибка при обновлении кэша версий';vi='Lỗi không xác định khi cập nhật cache các phiên bản'"));
				EndTry;
			EndIf;
...
"Идентификатор"
"записи: %1 Тип данных: %2';vi='Lỗi khi cập nhật cache phiên bản. Chưa nhận dữ liệu."
"Tên (ID)"
"bản ghi: %1 Kiểu dữ liệu: %2'");
...
Function InterfaceOptions() Export
	...
	Except
		ErrorInfo = ErrorInfo();
		EventName = NStr("en='Interface configuration';ru='Настройка интерфейса';vi='Tùy chỉnh giao diện'", CommonUseClientServer.MainLanguageCode());
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='An error occurred while receiving"
"interface options: %1';ru='При получении опций интерфейса"
"произошла ошибка: %1';vi='Khi nhận tùy chỉnh giao diện"
"đã xảy ra lỗi: %1'"),
			DetailErrorDescription(ErrorInfo));
