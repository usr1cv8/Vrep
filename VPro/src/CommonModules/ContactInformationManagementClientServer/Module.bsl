...
Function LocalityAddressPartsStructure() Export
	...
	Result.Insert("State",	ItemAddressStructure(NStr("en='Region';ru='Регион';vi='Vùng'"),	NStr("en='Address region';ru='Регион адреса';vi='Tỉnh, thành của địa chỉ'"),	"RFTerritorialEntity",	1));
	Result.Insert("Region",	ItemAddressStructure(NStr("en='District';ru='Район';vi='Quận'"),	NStr("en='Address county';ru='Район адреса';vi='Quận, huyện trong địa chỉ'"),		"PrRayMO/Region",		3));
	Result.Insert("City",	ItemAddressStructure(NStr("en='City';ru='Город';vi='Thành phố'"),		NStr("en='Address city';ru='Город адреса';vi='Phường, xã của địa chỉ'"),		"City",					4));
	Result.Insert("Settlement", ItemAddressStructure(NStr("en='Settlement';ru='Населенный пункт';vi='Khu dân cư'"),
		NStr("en='Settlement addresses';ru='Населенный пункт адреса';vi='Khu dân cư trong địa chỉ'"), "Settlement",  6, True));
	Result.Insert("Street", ItemAddressStructure(NStr("en='Street';ru='Улица';vi='Đường phố'"),
		NStr("en='Address street';ru='Улица адреса';vi='Đường phố địa chỉ'"), "Street", 7));
	Result.Insert("AdditionalItem", ItemAddressStructure(NStr("en='Additional item';ru='Дополнительный элемент';vi='Phần tử bổ sung'"),
		NStr("en='Additional address item';ru='Дополнительный элемент адреса';vi='Phần tử bổ sung địa chỉ'"), "AddEMailAddress[TypeAdrEl='10200000']", 90));
	Result.Insert("SubordinateItem", ItemAddressStructure(NStr("en='Subordinate item';ru='Подчиненный элемент';vi='Phần tử trực thuộc'"),
		NStr("en='Subordinate address item';ru='Подчиненный элемент адреса';vi='Phần tử địa chỉ trực thuộc'"), "AddEMailAddress[TypeAdrEl='10400000']", 91));
