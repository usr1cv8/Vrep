...
// 	NStr("en='Error message.';ru='Сообщение об ошибке.';vi='Thông báo lỗi.'"), ,
...
// CommonUseClientServer.MessageToUser(
// 	NStr("en='Error message.';ru='Сообщение об ошибке.';vi='Thông báo lỗi.'"), InfobaseObject, "Responsible",,Cancel);
//
// 4. For output messages to link on object infobases.
// CommonUseClientServer.MessageToUser(
// 	NStr("en='Error message.';ru='Сообщение об ошибке.';vi='Thông báo lỗi.'"), Refs, , , Cancel);
//
...
//                    collection, for example, NStr("en='User is not selected.';vi='Người dùng không được chọn.'").
...
Function TextFillingErrors(FieldKind = "Field", MessageKind = "Filling",
	...
	If Upper(FieldKind) = "Field" Then
		If Upper(MessageKind) = "FillType" Then
			Pattern = NStr("en='The ""%1"" field is not filled in';ru='Поле ""%1"" не заполнено';vi='Chưa điền trường ""%1""'");
...
""
"%4';vi='Điền sai cột ""%1"" trong dòng %2 của danh sách ""%3""."
""
"%4'");
		ElsIf Upper(MessageKind) = "CORRECTNESS" Then
			Pattern = NStr("en='Column %1 is filled in incorrectly in %2 row of %3 list."
...
"%4'");
	...
	ElsIf Upper(FieldKind) = "LIST" Then
		If Upper(MessageKind) = "FillType" Then
			Pattern = NStr("en='No lines entered to list ""%3""';ru='Не введено ни одной строки в список ""%3""';vi='Chưa nhập bất kỳ dòng nào trong danh sách ""%3""'");
		ElsIf Upper(MessageKind) = "CORRECTNESS" Then
			Pattern = NStr("en='List %3 is filled in incorrectly."
""
"%4';ru='Некорректно заполнен список ""%3""."
""
"%4';vi='Điền sai danh sách ""%3""."
""
"%4'");
		EndIf;
	EndIf;
		...
		If SearchKey AND StructureReceiver.Property(KeyAndValue.Key) Then
			...
			Else
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Structure intersection of source and receiver by key ""%1"".';ru='Пересечение структур источника и приемника по ключу ""%1"".';vi='Giao nhau giữa các cấu trúc nguồn và đích theo khóa ""%1"".'"),
					KeyAndValue.Key);
			EndIf
		EndIf;
...
//
// Parameters:
//  Object       - Arbitrary - объект, у которого нужно проверить наличие реквизита или свойства;
//  AttributeName - String       - имя реквизита или свойства.
//
// Returns:
//  Boolean - True, если есть.
//
...
Procedure SetSpreadsheetDocumentFieldState(SpreadsheetDocumentField, Status = "DontUse") Export
	...
	If TypeOf(SpreadsheetDocumentField) = Type("FormField") 
		...
		ElsIf Upper(Status) = "IRRELEVANCE" Then
			...
			StatePresentation.Picture                       = New Picture;
			StatePresentation.Text                          = NStr("en='The report is not generated. Click ""Run report"" to generate the report.';ru='Отчет не сформирован. Нажмите ""Сформировать"" для получения отчета.';vi='Chưa lập báo cáo. Hãy nhấn nút ""Lập báo cáo"" để nhận báo cáo.'");;
		ElsIf Upper(Status) = "REPORTCREATION" Then  
			StatePresentation.Visible                      = True;
			StatePresentation.AdditionalShowMode = AdditionalShowMode.Irrelevance;
			StatePresentation.Picture                       = PictureLib.LongOperation48;
			StatePresentation.Text                          = NStr("en='Generating the report...';ru='Отчет формируется...';vi='Đang lập báo cáo...'");
		Else
			Raise(NStr("en='Invalid parameter value (parameter number 2)';ru=""Недопустимое значение параметра (параметр номер '2')"";vi=""Giá trị tham số không hợp lệ (tham số số '2')"""));
		EndIf;
	Else
		Raise(NStr("en='Invalid parameter value (parameter number 1)';ru=""Недопустимое значение параметра (параметр номер '1')"";vi=""Giá trị tham số không hợp lệ (tham số số '1')"""));
	EndIf;
...
Function CompareVersions(Val VersionString1, Val VersionString2) Export
	...
	If Version1.Count() <> 4 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Invalid format for parameter VersionRow1: %1';ru='Неправильный формат параметра СтрокаВерсии1: %1';vi='Sai định dạng tham số VersionRow1: %1'"), VersionString1);
	EndIf;
	Version2 = StringFunctionsClientServer.DecomposeStringIntoSubstringsArray(Row2, ".");
	If Version2.Count() <> 4 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
	    	NStr("en='Invalid format for parameter VersionRow2: %1';ru='Неправильный формат параметра СтрокаВерсии2: %1';vi='Sai định dạng tham số VersionRow2: %1'"), VersionString2);
	EndIf;
...
Function InstallOuterDatabaseJoin(Parameters) Export
	...
	#If Server Or ThickClientOrdinaryApplication Or ExternalConnection Then
		...
		ErrorShortInfo = NStr("en='Direct connection to the infobase on a server under OS Linux is not available.';ru='Прямое подключение к информационной базе недоступно на сервере под управлением ОС Linux.';vi='Không thể kết nối trực tiếp đến cơ sở thông tin trên Server dưới hệ điều hành Linux.'");
	#Else
		IsLinux = IsLinuxClient();
		ErrorShortInfo = NStr("en='Direct connection to the infobase on a client under OS Linux is not available.';ru='Прямое подключение к информационной базе недоступно на клиенте под управлением ОС Linux.';vi='Không thể kết nối trực tiếp tới cơ sở thông tin trên Client dưới hệ điều hành Linux.'");
	#EndIf
	...
	Except
		...
		ErrorMessageString = NStr("en='Cannot connect to another application: %1';ru='Не удалось подключиться к другой программе: %1';vi='Không thể kết nối đến chương trình khác: %1'");
	...
	If FileModeWork Then
		...
		If IsBlankString(Parameters.InfobaseDirectory) Then
			ErrorMessageString = NStr("en='Location of the infobase directory is not specified.';ru='Не задано месторасположение каталога информационной базы.';vi='Chưa chỉ ra vị trí thư mục cơ sở thông tin.'");
	...
	Else
		...
		If IsBlankString(Parameters.PlatformServerName) Or IsBlankString(Parameters.InfobaseNameAtPlatformServer) Then
			ErrorMessageString = NStr("en='Required connection parameters are not specified: ""Server name""; ""Name of infobase on the server"".';ru='Не заданы обязательные параметры подключения: ""Имя сервера""; ""Имя информационной базы на сервере"".';vi='Chưa chỉ ra tham số kết nối bắt buộc: ""Tên Server""; ""Tên cơ sở thông tin trên Server"".'");
			FillCheckingError = True;
		EndIf;
	...
	For Each AddressString IN ArrayOFAddresses Do
		...
		If AddressDefined Then 
			If HasProhibitedCharsInAlias Then
				ErrorDescription = NStr("en='The string does not contain an email address';ru='Строка не содержит адреса электронной почты';vi='Dòng không có địa chỉ email'");
			EndIf;
		Else
			If StringContainsEmail Then 
				ErrorDescription = NStr("en='Email address contains errors';ru='Адрес электронной почты содержит ошибки';vi='Địa chỉ email bị lỗi'");
			Else
				ErrorDescription = NStr("en='The string does not contain an email address';ru='Строка не содержит адреса электронной почты';vi='Dòng không có địa chỉ email'");
			EndIf;
		EndIf;	
...
Function SplitStringWithEmailAddresses(Val EmailAddressString, RaiseException = True) Export
	...
	ProhibitedCharsMessage = NStr("en='There is a prohibited character %1 in the email address %2';vi='Có ký tự không hợp lệ %1 trong địa chỉ E-mail %2'");
	MessageInvalidEmailFormat = NStr("en='Incorrect email address %1';vi='Sai địa chỉ E-mail %1'");
...
Function ParseStringWithPostalAddresses(Val EmailAddressString, CallingException = True) Export
	...
	ProhibitedCharsMessage = NStr("en='Invalid character ""%1"" in email address ""%2""';ru='Недопустимый символ ""%1"" в адресе электронной почты ""%2""';vi='Ký tự không hợp lệ ""%1"" trong địa chỉ hòm thư ""%2""'");
	MessageInvalidEmailFormat = NStr("en='Incorrect email address ""%1""';ru='Некорректный адрес электронной почты ""%1""';vi='Sai địa chỉ E-mail ""%1""'");
...
#If Not ThinClient AND Not WebClient And Not MobileClient AND Not ThickClientManagedApplication Then
...
Procedure Validate(Val Condition, Val Message = "", Val CheckContext = "") Export
	...
	If Condition <> True Then
		If IsBlankString(Message) Then
			ErrorMessage = NStr("en='Invalid operation';ru='Недопустимая операция';vi='Giao dịch không được phép'"); // Assertion failed
		Else
			ErrorMessage = Message;
		EndIf;
		If Not IsBlankString(CheckContext) Then
			ErrorMessage = ErrorMessage + " " +
				StringFunctionsClientServer.SubstituteParametersInString(NStr("en='in %1';ru='in %1';vi='trong %1'"), CheckContext);
		EndIf;
		Raise ErrorMessage;
	EndIf;
...
Procedure CheckParameter(Val ProcedureOrFunctionName, Val ParameterName, Val ParameterValue, 
	...
	Validate(TypeOf(ProcedureOrFunctionName) = Type("String"), 
		NStr("en='Invalid value of parameter ExpectedTypes';ru='Недопустимо значение параметра ОжидаемыеТипы';vi='Giá trị tham số ExpectedTypes không được phép'"), Context);
	Validate(TypeOf(ParameterName) = Type("String"), 
		NStr("en='Invalid value of the ParameterName parameter';ru='Недопустимо значение параметра ИмяПараметра';vi='Giá trị tham số ParameterName không được phép'"), Context);
...
"Dự kiến: %3; đã chuyển giá trị: %4 (kiểu %5).'");
	Validate((ThisTypeDescription AND ExpectedTypes.ContainsType(TypeOf(ParameterValue)))
		...
		StringFunctionsClientServer.SubstituteParametersInString(InvalidParameter, 
			...
			?(ParameterValue <> Undefined, ParameterValue, NStr("en='Undefined';ru='Неопределено';vi='Không xác định'")), TypeOf(ParameterValue)));
	...
	If TypeOf(ParameterValue) = Type("Structure") AND ExpectedPropertyTypes <> Undefined Then
		...
		Validate(TypeOf(ExpectedPropertyTypes) = Type("Structure"), 
			NStr("en='Invalid value of the ProcedureOrFunctionName parameter';ru='Недопустимо значение параметра ИмяПроцедурыИлиФункции';vi='Giá trị tham số ProcedureOrFunctionName không được phép'"), Context);
...
"В структуре ожидалось свойство %3 (тип %4).';vi='Giá trị tham số không hợp lệ %1 (Cấu trúc) trong %2."
"Trong cấu trúc dự kiến thuộc tính %3 (kiểu %4).'");
...
"Ожидалось: %4; передано значение: %5 (тип %6).';vi='Giá trị không hợp lệ của thuộc tính %1 trong tham số %2 (Cấu trúc) trong %3."
"Dự kiến: %4; đã chuyển giá trị: %5 (kiểu %6).'");
		For Each Property IN ExpectedPropertyTypes Do
			...
			Validate((ThisTypeDescription AND ExpectedPropertyType.ContainsType(TypeOf(PropertyValue)))
				...
				StringFunctionsClientServer.SubstituteParametersInString(InvalidProperty, 
					...
					?(PropertyValue <> Undefined, PropertyValue, NStr("en='Undefined';ru='Неопределено';vi='Không xác định'")), TypeOf(PropertyValue)));
...
//
Function DistributeAmountProportionallyToFactors(Val DistributingAmount, Ratios, Val Precision = 2) Export
	
	If Ratios.Count() = 0 Or Not ValueIsFilled(DistributingAmount) Then
		Return Undefined;
	EndIf;
	...
	DistributedAmount = 0;
	For IndexOf = 0 To Ratios.Count() - 1 Do
		Result[IndexOf] = Round(DistributingAmount * Ratios[IndexOf] / RatiosAmount, Precision, 1);
		DistributedAmount = DistributedAmount + Result[IndexOf];
	EndDo;
	
	// Assign rounding errors to ratio with the maximum weight.
	If Not DistributedAmount = DistributingAmount Then
		Result[MaxRatioIndex] = Result[MaxRatioIndex] + DistributingAmount - DistributedAmount;
	EndIf;
...
Function CheckAndPrepareEmailAddress(Val AddresseeFullName, Val EmailAddress)
	...
	ProhibitedCharInRecipientName = NStr("en='There is a prohibited character in the addressee name.';vi='Trong tên người nhận có ký tự không hợp lệ.'");
	EmailContainsProhibitedChar = NStr("en='There is a prohibited character in the email address.';vi='Trong địa chỉ E-mail có ký tự không hợp lệ.'");
	BorderChars = "<>[]";
...
Function CheckAndPrepareMailAddress(Val FullNameAddressee, Val MailAddress)
	...
	ProhibitedCharInRecipientName = NStr("en='Recipient name contains invalid characters.';ru='Недопустимый символ в имени адресата.';vi='Ký tự không hợp lệ trong tên người nhận.'");
	EmailContainsProhibitedChar = NStr("en='Invalid character in mailing address.';ru='Недопустимый символ в почтовом адресе.';vi='Ký tự không hợp lệ trong địa chỉ thư.'");
	BorderChars = "<>[]";
...
//
// Parameters:
//  List - ValueList - список значений из которого будет формироваться массив значений;
// 
// Returns:
//  Array - items array from the marked items of the values list.
//
