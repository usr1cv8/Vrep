...
Function RecalculateFromCurrencyToCurrency(Amount, InitRate, FinRate,	RepetitionBeg = 1, RepetitionEnd = 1) Export
	...
	If InitRate = 0 OR FinRate = 0 OR RepetitionBeg = 0 OR RepetitionEnd = 0 Then
		Message = New UserMessage();
		Message.Text = NStr("en='Null exchange rate has been found. Recalculation isn't executed.';vi='';");
		Message.Message();
		Return Amount;
	EndIf;
...
Function GetPresentationOfWeekDay(CalendarWeekDay) Export
	...
	If WeekDayNumber = 1 Then
		...
		Return NStr("en='Mon';ru='Пн';vi='Thứ hai'");
		
	ElsIf WeekDayNumber = 2 Then
		
		Return NStr("en='Tue';ru='Вт';vi='Thứ ba'");
		
	ElsIf WeekDayNumber = 3 Then
		
		Return NStr("en='Wed';ru='Ср';vi='Thứ tư'");
		
	ElsIf WeekDayNumber = 4 Then
		
		Return NStr("en='Thu';ru='Чт';vi='Thứ năm'");
		
	ElsIf WeekDayNumber = 5 Then
		
		Return NStr("en='Fri';ru='Пт';vi='Thứ sáu'");
		
	ElsIf WeekDayNumber = 6 Then
		
		Return NStr("en='Sa';ru='Sa';vi='Thứ bảy'");
		
	Else
		
		Return NStr("en='Sun';ru='Вс';vi='CN'");
	...
	If CommandParameter.Form.FormName = "Catalog.CounterpartyContracts.Form.ItemForm" Then
		...
		If Not AllEditedParametersFilled Then
			ShowQueryBox(New NotifyDescription("PrintCounterpartyContractQuestion", ThisObject,
			               ...
			               NStr("en='Not all manually edited fields are filled in, continue printing?';ru='Не все редактируемые вручную поля заполнены, продолжить печать?';vi='Vẫn chưa điền tất cả các trường soạn thủ công. Tiếp tục in?'"), QuestionDialogMode.YesNo);
		Else
...
Procedure CreateEmail(Val FieldsValues, Val Presentation = "", ExpectedKind = Undefined, FormObject) Export
	...
	If InformationType <> PredefinedValue("Enum.ContactInformationTypes.EmailAddress") Then
		Raise StrReplace(NStr("en='Cannot create email by contact information with type ""%1""';ru='Нельзя создать письмо по контактной информацию с типом ""%1""';vi='Không thể tạo E-mail theo thông tin liên hệ có kiểu ""%1""'"),
			"%1", InformationType);
	EndIf;
	...
	If TypeOf(MailAddress) <> Type("String") Then
		Raise NStr("en='An error occurred when receiving the email address, incorrect contact information type';ru='Ошибка получения адреса электронной почты, неверный тип контактной информации';vi='Lỗi nhận địa chỉ E-mail, kiểu thông tin liên hệ không đúng'");
	EndIf;
	...
	SuggestionText = NStr("en='To send the email, install the file operation extension.';ru='Для отправки письма необходимо установить расширение для работы с файлами.';vi='Để gửi E-mail, cần thiết lập phần mở rộng để làm việc với tệp.'");
...
Procedure EnableFileOperationsExtensionEnd(Attached, AdditionalParameters) Export
	...
	If Attached Then
		...
		Else // If there are no settings, then file opening dialog.
			...
			Dialog.Title = NStr("en='Select a file for import...';ru='Выберите файл для загрузки...';vi='Hãy chọn tệp để kết nhập...'");
			Dialog.Filter = NStr("en='Files of exchange with 1C (*.xml)|*.xml|Files of exchange with 1C (*.txt)|*.txt|All files (*.*)|*.*';ru='Файлы обмена с 1С (*.xml)|*.xml|Файлы обмена с 1С (*.txt)|*.txt|Все файлы (*.*)|*.*';vi='Tệp trao đổi với 1C (*.xml)|*.xml|Tệp trao đổi với 1C (*.txt)|*.txt|Tất cả các tệp (*.*)|*.*'");
...
Procedure ReadTextDocument(AdditionalParameters)
	...
	Except
		MessageText = NStr("en='An error occurred while reading the %File% file.';ru='Ошибка чтения файла %Файл%.';vi='Lỗi đọc tệp %File%.'");
...
Procedure ImportDataFromFileStatementsFragment(AdditionalParameters)
	...
	Status(
		NStr("en='Loading statement file';ru='Загружается файл выписки';vi='Kêt nhập tệp bản sao kê'"),
		,
		NStr("en='Importing statement file';ru='Производится загрузка файла выписки';vi='Tiến hành kết nhập tệp của bản sao kê'"),
...
#Region TabularSectionProcessing

Procedure CalculateAmountsInTSRow(TabularSectionRow, CalculationParameters) Export

	If Not CalculationParameters.Property("CalculateDiscountAmount") And Not CalculationParameters.Property("CalculateDiscountPercent") Then
		CalculationParameters.Insert("CalculateDiscountAmount", True);
	EndIf;
	If Not CalculationParameters.Property("CalculateAmount") And Not CalculationParameters.Property("CalculatePrice") Then
		CalculationParameters.Insert("CalculateAmount", True);
	EndIf;

	If TabularSectionRow.Property("Multiplicity") And TabularSectionRow.Property("Factor") Then
		RowCount = TabularSectionRow.Count * TabularSectionRow.Multiplicity * TabularSectionRow.Factor;
	Else
		RowCount = TabularSectionRow.Count;
	EndIf; 
	
	If CalculationParameters.Property("ResetFlagDiscountsAreCalculated") And CalculationParameters.ResetFlagDiscountsAreCalculated
		And TabularSectionRow.Property("AutomaticDiscountsPercent")
		Then
		TabularSectionRow.AutomaticDiscountsPercent = 0;
		TabularSectionRow.AutomaticDiscountAmount = 0;
	EndIf;
	...
	If TabularSectionRow.Property("DiscountMarkupPercent") Then
		...
		HasDiscountBonus = TabularSectionRow.Property("PaymentByBonusDiscountAmount");
		
		If CalculationParameters.Property("CalculatePrice") And CalculationParameters.CalculatePrice=True Then //Введена сумма строки, нужно пересчитать цену и сумму скидки
			If RowCount <> 0 Then
				TabularSectionRow.Price = (TabularSectionRow.Amount - ?(HasDiscountBonus, TabularSectionRow.PaymentByBonusDiscountAmount, 0))/((1 - TabularSectionRow.DiscountMarkupPercent/100)*RowCount);
				TabularSectionRow.AmountDiscountsMarkups = Round((RowCount * TabularSectionRow.Price - ?(HasDiscountBonus, TabularSectionRow.PaymentByBonusDiscountAmount, 0)) * TabularSectionRow.DiscountMarkupPercent / 100, 2);
			EndIf;
		ElsIf CalculationParameters.Property("CalculateDiscountPercent") And CalculationParameters.CalculateDiscountPercent And TabularSectionRow.Amount > 0 Then //Введена сумма скидки, нужно пересчитать процент скидки
			If TabularSectionRow.AmountDiscountsMarkups >= TabularSectionRow.Amount Then
				TabularSectionRow.AmountDiscountsMarkups = TabularSectionRow.Amount;
				TabularSectionRow.Amount = 0;
				TabularSectionRow.DiscountMarkupPercent = 100;
			Else
				TabularSectionRow.DiscountMarkupPercent = Round(100*TabularSectionRow.AmountDiscountsMarkups / (RowCount * TabularSectionRow.Price - ?(HasDiscountBonus, TabularSectionRow.PaymentByBonusDiscountAmount, 0)), 2);
			EndIf;
			TabularSectionRow.Amount = RowCount * TabularSectionRow.Price - TabularSectionRow.AmountDiscountsMarkups - ?(HasDiscountBonus, TabularSectionRow.PaymentByBonusDiscountAmount, 0);
		Else  //Если введен процент скидки, или если указаны и процент, и сумма, пересчитываем сумму скидки из процента, указанного в строке
			TabularSectionRow.AmountDiscountsMarkups = Round((RowCount * TabularSectionRow.Price - ?(HasDiscountBonus, TabularSectionRow.PaymentByBonusDiscountAmount, 0)) * TabularSectionRow.DiscountMarkupPercent / 100, 2);
			TabularSectionRow.Amount = RowCount * TabularSectionRow.Price - TabularSectionRow.AmountDiscountsMarkups - ?(HasDiscountBonus, TabularSectionRow.PaymentByBonusDiscountAmount, 0);
		EndIf;
	Else 	
		If CalculationParameters.Property("CalculatePrice") And CalculationParameters.CalculatePrice=True Then //Введена сумма строки, нужно пересчитать цену с сумму скидки
			TabularSectionRow.Price = ?(RowCount=0, 0, TabularSectionRow.Amount / RowCount);
		Else
			TabularSectionRow.Amount = TabularSectionRow.Price * RowCount;
		EndIf;
	EndIf;
	
	If TabularSectionRow.Property("DiscountAmount") Then
		TabularSectionRow.DiscountAmount = TabularSectionRow.AmountDiscountsMarkups;
	EndIf; 
	
	CalculateVATAmountAndTotal(TabularSectionRow, CalculationParameters);
	
EndProcedure

Procedure CalculateVATAmountAndTotal(TabularSectionRow, CalculationParameters) Export
	...
	If CommonUseClientServer.HasAttributeOrObjectProperty(TabularSectionRow, "Total") Then
		TabularSectionRow.Total = TabularSectionRow.Amount + ?(CalculationParameters.AmountIncludesVAT, 0, TabularSectionRow.VATAmount);
	ElsIf CommonUseClientServer.HasAttributeOrObjectProperty(TabularSectionRow, "AmountWithVAT") Then // в документе ВыводИзОборотаИСМП
		TabularSectionRow.AmountWithVAT = TabularSectionRow.Amount + ?(CalculationParameters.AmountIncludesVAT, 0, TabularSectionRow.VATAmount);
	EndIf;
	
EndProcedure // РассчитатьСуммуНДС()

Function NewCostIncludingChangesByVAT2018(CostCurrent, VATRateCurrent, VATRateNew) Export
	
	VATRateCalculated = VATRateCurrent + 100;
	
	CostNew = (CostCurrent - (CostCurrent*VATRateCurrent/VATRateCalculated)) + (CostCurrent - (CostCurrent*VATRateCurrent/VATRateCalculated)) * VATRateNew / 100;
