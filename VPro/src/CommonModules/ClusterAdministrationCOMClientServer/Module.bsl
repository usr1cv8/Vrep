...
Function GetCluster(ConnectionToServerAgent, Val ClusterPort, Val ClusterAdministratorName, Val ClusterAdministratorPassword)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Cluster %2 is not found on server %1';ru='На рабочем сервере %1 не найден кластер %2';vi='Trong Server đang làm việc %1 không tìm thấy cụm Server %2'"),
...
Function WorkingProcessConnection(COMConnector, ConnectionToServerAgent, Cluster)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Active processes are not found in server cluster %1:%2.';ru='В кластере серверов %1:%2 не найдено активных рабочих процессов.';vi='Trong cụm Server %1:%2 không tìm thấy quá trình đang làm việc.'"),
		Cluster.HostName,
...
Function GetIBDescription(ConnectionToServerAgent, Cluster, Val NameInCluster)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Infobase ""%3"" was not found in server cluster %1:%2.';ru='В кластере серверов %1:%2 не найдена информационная база ""%3""!';vi='Trong cụm Server %1:%2 không tìm thấy cơ sở thông tin ""%3""!'"),
		Cluster.HostName,
		Format(Cluster.MainPort, "NG=0"),
		NameInCluster
	);
	...
	For Each Infobase IN WorkingProcessConnection.GetInfobases() Do
		...
		If Infobase.Name = NameInCluster Then
			...
			If Not ValueIsFilled(Infobase.DBMS) Then
				...
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='The administrator user name or password is incorrect in the cluster of servers %2:%3 in infobase %1 (name: ""%4"").';ru='Неправильные имя и пароль администратора информационной базы %1 в кластере серверов %2:%3 (имя: ""%4"").';vi='Sai tên và mật khẩu của người quản trị cơ sở thông tin %1 trong cụm Server %2:%3 (tên: ""%4"").'"),
					NameInCluster,
...
Function GetSecurityProfile(ConnectionToServerAgent, Cluster, ProfileName)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Security profile ""%3"" is not found in server cluster %1:%2.';ru='В кластере серверов %1:%2 не найден профиль безопасности ""%3""!';vi='Trong cụm Server %1:%2 không tìm thấy hồ sơ an toàn ""%3""!'"),
		Cluster.HostName,
		Format(Cluster.MainPort, "NG=0"),
		ProfileName
	);
