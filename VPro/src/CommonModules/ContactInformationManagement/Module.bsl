...
Procedure OnCreateAtServer(Form, Object, ItemForPlacementName = "", TitleLocationContactInformation = "") Export
		...
		Else
			...
			Except
				...
				CommonUseClientServer.MessageToUser(
					NStr("en='Contact information analysis error, possibly due to invalid field value format.';ru='Ошибка анализа контактной информации, возможно из-за неверного формата поля.';vi='Lỗi phân tích thông tin liên hệ, có thể là do sai định dạng trường.'"), ,
	...
	Form, Parent, ChildFormItemsGroup.HorizontalIfPossible, 5);
	...
	If Form.AddedContactInformationItemList.Count() > 0 Then
		...
		Command.ToolTip = NStr("en='Add an additional contact information field';ru='Добавить дополнительное поле контактной информации';vi='Thêm trường bổ sung của thông tin liên hệ'");
		...
		Button.Title = NStr("en='Add';ru='Добавить';vi='Thêm'");
...
Function AddressByContactInformationDocument(XMLString, Val NewValue = Undefined) Export
	...
	If AddressUS = Undefined Then
		Raise NStr("en='Cannot determine the address by document, expecting a domestic address';ru='Невозможно определить адрес, ожидается локальный адрес';vi='Không thể xác định địa chỉ, cần có địa chỉ cục bộ'");
	EndIf;
...
Function ContactInformationAddressCountry(Val XMLString) Export
	...
	If Address = Undefined Or Address.Type() <> XDTOFactory.Type(Namespace, "Address") Then
		Raise NStr("en='Cannot determine country; address pending.';ru='Невозможно определить страну, ожидается адрес.';vi='Không thể xác định quốc gia, đang chờ địa chỉ.'");
	EndIf;
...
Function ContactInformationAddressState(Val XMLString) Export
	...
	Address = XDTOAddress.Content;
	If Address = Undefined Or Address.Type() <> XDTOFactory.Type(Namespace, "Address") Then
		Raise NStr("en='Cannot determine state, expecting an address.';ru='Невозможно определить регион, ожидается адрес.';vi='Không thể xác định khu vực, cần có địa chỉ.'");
	EndIf;
...
Function ContactInformationAddressCity(Val XMLString) Export
	...
	Address = XDTOAddress.Content;
	If Address = Undefined Or Address.Type() <> XDTOFactory.Type(Namespace, "Address") Then
		Raise NStr("en='Cannot determine city; address pending.';ru='Невозможно определить город, ожидается адрес.';vi='Không thể xác định thành phố, đang chờ địa chỉ.'");
	EndIf;
...
Function ContactInformationAddressDomain(Val XMLString) Export
	...
	Raise NStr("en='Cannot determine domain; email or web link pending.';ru='Невозможно определить домен, ожидается электронная почта или веб-ссылка.';vi='Không thể xác định domain, đang chờ E-mail hoặc tham số Web.'");	
EndFunction
...
Function ContactInformationPhoneNumber(Val XMLString) Export
	...
	Raise NStr("en='Cannot determine number; phone call or fax pending.';ru='Невозможно определить номер, ожидается телефона или факс.';vi='Không thể xác định số thứ tự, đang chờ số điện thoại hoặc Fax.'");
EndFunction
...
Procedure CreateContactInformationTemporaryTable(TempTablesManager, ObjectArray, ContactInfoTypes = Undefined, CIKinds = Undefined) Export
	...
	Else
		Raise NStr("en='Incorrect value for the array of contact information owners.';ru='Неверное значение для массива владельцев контактной информации.';vi='Sai giá trị đối với mảng lớp trên của thông tin liên hệ.'");
	EndIf;
...
Procedure AddContactInformationString(Form, Result)
	...
	If TypeOf(AddedKind)= Type("CatalogRef.ContactInformationKinds") Then
		CIKindInformation = CommonUse.ObjectAttributesValues(AddedKind, "Type, Description, EditInDialogOnly, Tooltip");
	Else
...
Function TextBox(Form, EditInDialogOnly, Type, AttributeName, ToolTip, IsNewCIKind = False, Mandatory = False)
	...
	Else
		...
		Command.ToolTip = NStr("en='Enter a comment';ru='Ввести комментарий';vi='Nhập ghi chú'");
		Command.Action = "Attachable_ContactInformationExecuteCommand";
		Button.Title = NStr("en='Enter a comment';ru='Ввести комментарий';vi='Nhập ghi chú'");
		Button.CommandName = CommandName;
...
Function Action(Form, Type, AttributeName, ActionGroup, AddressCount, HasComment = False)
	...
	If (Type = Enums.ContactInformationTypes.WebPage
		...
		If Type = Enums.ContactInformationTypes.Address Then
			...
			Item.Title = NStr("en='Fill in';ru='Заполнить';vi='Điền'");
			Command.ToolTip = NStr("en='Fill in the address';ru='Заполнить адрес из другого поля';vi='Điền tự động'");
			Command.Picture = PictureLib.MoveLeft;
			Command.ModifiesStoredData = True;
			
		ElsIf Type = Enums.ContactInformationTypes.WebPage Then
			
			Item.Title = NStr("en='Navigate';ru='Перейти';vi='Chuyển đến'");
			Command.ToolTip = NStr("en='Click URL';ru='Перейти по ссылке';vi='Chuyển theo tham chiếu'");
			Command.Picture = PictureLib.ContactInformationGotoURL;
			
		ElsIf Type = Enums.ContactInformationTypes.EmailAddress Then
			
			Item.Title = NStr("en='Write an email';ru='Написать письмо';vi='Viết thư'");
			Command.ToolTip = NStr("en='Write an email';ru='Написать письмо';vi='Viết thư'");
	...
	If EmailAddress <> Undefined And EmailAddress.Type() = XDTOFactory.Type(Namespace, "Email") Then
		Try
			...
			If Result.Count() > 1 Then
				...
				ErrorString = NStr("en='Only one email address is allowed';ru='Разрешен только один email';vi='Chỉ cho phép một E-mail'");
...
Procedure RefreshContactInformationKind(Kind, Type, ToolTip, CanChangeEditMode, EditInDialogOnly, Mandatory, Order = Undefined, AllowMultipleValueInput = False, ValidationSettings = Undefined) Export
	...
	ParameterError   = NStr("en='Invalid address validation settings';ru='Параметры проверки адреса';vi='Tham số kiểm tra địa chỉ'");
