...
Function InterfaceVersionsCurrentIB(Val InterfaceName) Export
	...
	If Result = Undefined OR Result.Count() = 0 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Current infobase does not support the %1 interface.';ru='Текущая информационная база не поддерживает интерфейс %1!';vi='Cơ sở thông tin hiện tại không hỗ trợ giao diện %1!'"), InterfaceName);
	Else
		Return Result;
	EndIf;
