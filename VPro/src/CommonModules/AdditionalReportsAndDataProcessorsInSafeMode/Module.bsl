...
"временный файл ""%1"": %2';vi='Không thể xóa"
"tệp tạm thời ""%1"": %2'"),
			TempFile,
...
Function GetFileFromInternet(Val SessionKey, Val URL, Val Port = 0, Val UserName = "", Val Password = "", Val Timeout = 20,Val SecureConnection = Undefined, Val PassiveConnection = Undefined) Export
	...
	If Not CommonUse.SubsystemExists("StandardSubsystems.GetFilesFromInternet") Then
		Raise NStr("en='Call of function AdditionalReportsAndDataProcessorsInSafeMode.GetFileFromInternet is not supported in this configuration.';ru='В данной конфигурации не поддерживается вызов функции ДополнительныеОтчетыИОбработкиВБезопасномРежиме.ПолучитьФайлИзИнтернета!';vi='Trong cấu hình này không hỗ trợ gọi hàm AdditionalReportsAndDataProcessorsInSafeMode.GetFileFromInternet.'");
	EndIf;
	...
	If Upper(Protocol) = "FTP" Then
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorInfo = NStr("en='An error occurred when establishing FTP connection with server %1:';ru='Ошибка при создании FTP-соединения с сервером %1:';vi='Lỗi khi tạo FTP-connection với Server %1:'") + Chars.LF + "%2";
	...
	Else
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorInfo = NStr("en='An error occurred when setting up HTTP connection with server %1:';ru='Ошибка при создании HTTP-соединения с сервером %1:';vi='Lỗi khi tạo HTTP-connection với Server %1:'") + Chars.LF + "%2";
	...
	Except
		ErrorInfo = ErrorInfo();
		ErrorInfo = NStr("en='An error occurred when receiving the file from server %1:';ru='Ошибка при получении файла с сервера %1:';vi='Lỗi khi nhận tệp từ Server %1:'") + Chars.LF + "%2";
...
Function ImportFileInInternet(Val SessionKey, Val BinaryDataAddress, Val URL, Val Port = 0, Val UserName = "", Val Password = "", Val Timeout = 20,Val SecureConnection = Undefined, Val PassiveConnection = Undefined) Export
	...
	If Not CommonUse.SubsystemExists("StandardSubsystems.GetFilesFromInternet") Then
		Raise NStr("en='Call of function AdditionalReportsAndDataProcessorsInSafeMode.ImportFileInInternet is not supported in this configuration.';ru='В данной конфигурации не поддерживается вызов функции ДополнительныеОтчетыИОбработкиВБезопасномРежиме.ПередатьФайлВИнтернет!';vi='Trong cấu hình này không hỗ trợ gọi hàm AdditionalReportsAndDataProcessorsInSafeMode.ImportFileInInternet'");
	EndIf;
	...
	Except
		ErrorInfo = ErrorInfo();
		ErrorInfo = NStr("en='An error occurred when transferring file to the server %1:';ru='Ошибка при передаче файла на сервер %1:';vi='Lỗi khi truyền tệp đến Server %1:'") + Chars.LF + "%2";
...
Procedure CheckCorrectnessOfMethodNameOfExternalObject(Val MethodName)
	...
	For Each ForbiddenSymbol IN ForbiddenSymbols Do
		...
		If Find(MethodName, ForbiddenSymbol) > 0 Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='%1 not a correct method name for COM object or external component object.';ru='%1 не является корректным именем метода для COM-объекта или объекта внешней компоненты!';vi='%1 không phải là tên phương thức đúng dành cho đối tượng COM hoặc đối tượng cấu phần ngoài!'"),
				MethodName);
...
"в безопасном режиме должны вызываться только из сценария!';vi='Đã gọi ra sai hàm mô-đun chung "
"AdditionalReportsAndDataProcessorsInSafeMode!"
"Hàm đã kết xuất của mô-đun này để sử dụng"
"ở chế độ an toàn cần được gọi ra chỉ từ kịch bản!'");
