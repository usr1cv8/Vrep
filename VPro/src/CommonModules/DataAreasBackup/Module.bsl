...
Procedure SetActivityFlagInZone(Val DataArea = Undefined, Val Status = True)
	...
	If DataArea = Undefined Then
		...
		Else
			Raise NStr("en='Parameter DataArea is required while calling procedure SetActivityFlagInZone from undivided session.';ru='При вызове процедуры УстановитьФлагАктивностиВОбласти из неразделенного сеанса параметр ОбластьДанных является обязательным!';vi='Khi gọi thủ tục УстановитьФлагАктивностиВОбласти từ phiên làm việc chưa phân chia thì tham số"
"ОбластьДанных là bắt buộc!'");
		EndIf;
	Else
		If Not CommonUseReUse.SessionWithoutSeparator()
				AND DataArea <> CommonUse.SessionSeparatorValue() Then
			
			Raise(NStr("en='Working with data of the area apart from the current one is prohibited';ru='Запрещено работать с данными области кроме текущей';vi='Cấm làm việc với dữ liệu vùng ngoại trừ vùng hiện tại'"));
...
Procedure CopiesCreation(Val CreationParameters, Val StateBuilding) Export
	...
	Parameters.Insert("Type", NStr("en='Everyday';vi='Hàng'"));
	Parameters.Insert("incorporated", "CreateDaily");
	Parameters.Insert("Periodicity", NStr("en='Day';vi='Ngày'"));
	...
	Parameters = New Structure;
	Parameters.Insert("Type", NStr("en='Monthly';vi='Hàng tháng'"));
	Parameters.Insert("incorporated", "CreateMonthly");
	Parameters.Insert("Periodicity", NStr("en='Month';vi='Tháng'"));
	...
	Parameters = New Structure;
	Parameters.Insert("Type", NStr("en='Annual';vi='Hàng năm'"));
	Parameters.Insert("incorporated", "CreateAnnual");
	Parameters.Insert("Periodicity", NStr("en='Year';vi='Năm'"));
	...
	If Not RequiredCreating Then
		WriteLogEvent(
			EventLogMonitorEvent() + "." 
				+ NStr("en='Skip creation';ru='Пропуск создания';vi='Bỏ qua việc tạo mới'", CommonUseClientServer.MainLanguageCode()),
	...
	WriteEventToLog(
		NStr("en='Create';ru='Создание';vi='Tạo mới'", CommonUseClientServer.MainLanguageCode()),
		ParametersEvents);
...
"Номер"
"попытки:"
"%2 По причине: %3';vi='Khi tạo bản sao dự phòng của vùng %1, đã xảy ra lỗi."
"Số lần"
"thử:"
"%2 Nguyên nhân: %3"
"'");
		Level = EventLogLevel.Warning;
		Event = NStr("en='Creation iteration error';ru='Ошибка итерации создания';vi='Lỗi khởi xướng tạo mới'", CommonUseClientServer.MainLanguageCode());
	Else
...
"Номер"
"попытки:"
"%2 По причине: %3';vi='Khi tạo bản sao dự phòng của vùng %1, đã xảy ra lỗi không thể phục hồi."
"Số lần"
"thử:"
"%2 Nguyên nhân: %3'");
		Level = EventLogLevel.Error;
		Event = NStr("en='Creation error';ru='Ошибка создания';vi='Lỗi tạo mới'", CommonUseClientServer.MainLanguageCode());
	EndIf;
...
Procedure ExportAreaToMSStorage(Val ExportParameters, StorageAddress = Undefined) Export
	...
	If Not Users.InfobaseUserWithFullAccess() Then
		Raise(NStr("en='Access violation';ru='Нарушение прав доступа';vi='Vi phạm quyền truy cập'"));
	EndIf;
	...
	Except
		...
		WriteLogEvent(NStr("en='Data area backup';ru='Создание резервной копии области данных';vi='Tạo bản sao dự phòng vùng dữ liệu'", CommonUseClientServer.MainLanguageCode()), 
...
Function EventLogMonitorEvent()
	...
	Return NStr("en='Application backup';ru='Резервное копирование приложений';vi='Sao lưu dự phòng ứng dụng'", CommonUseClientServer.MainLanguageCode());
...
"резервного копирования: %1';vi='Lỗi khi nhận tùy chỉnh"
"bản sao dự phòng: %1'");
...
"резервного копирования: %1';vi='Lỗi khi lưu tùy chỉnh"
"bản sao dự phòng: %1'");
		MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, ErrorInfo);
		Raise(MessageText);
	EndIf;
