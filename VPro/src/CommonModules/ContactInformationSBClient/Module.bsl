...
Procedure ActionCIClick(Form, Item) Export
	...
	If DataCI.Type = PredefinedValue("Enum.ContactInformationTypes.Skype") Then
		...
		List = New ValueList;
		List.Add("Call", NStr("en='Call';ru='Позвонить';vi='Gọi điện'"));
		List.Add("StartChat", NStr("en='Start chat';ru='Начать чат';vi='Bắt đầu chat'"));
...
Procedure AddContactInformationKindSelected(SelectedItem, AdditionalParameters) Export
	...
	If KindProperties.ShowInFormAlways = False Then
		...
		QuestionText = StrTemplate(NStr("en='Add the ability to input the type of contact information ""%1""?';ru='Добавить возможность ввода вида контактной информации ""%1""?';vi='Thêm khả năng nhập dạng thông tin liên hệ ""%1""?'"), SelectedItem.Value);
		QuestionTitle = NStr("en='Confirm adding';ru='Подтверждение добавления';vi='Xác nhận bổ sung'");
