...
"программы требуются административные права (роли ""Администратор системы"" и ""Полные права"").';vi='Tạm thời không thể đăng nhập vào chương trình do đang cập nhật phiên bản mới"
"Để kết thúc cập nhật phiên bản"
"chương trình, cần có quyền quản trị (vai trò ""Quản trị hệ thống"" và ""Toàn quyền"").'");
...
"Обратитесь к администратору сервиса за подробностями.';vi='Không thể đăng nhập vào ứng dụng đồng thời do cập nhật phiên bản mới."
"Hãy liên lạc với người quản trị Service để biết thêm chi tiết.'");
...
"Обратитесь к администратору за подробностями.';vi='Không thể đăng nhập vào chương trình do đang cập nhật lên phiên bản mới"
...
Function RunInfobaseUpdate(UpdateParameters) Export
	...
	If Not AreRightsForInfobaseUpdate() Then
		Message = NStr("en='Insufficient rights to update the application version.';ru='Недостаточно прав для обновления версии программы.';vi='Không đủ quyền để cập nhật phiên bản của chương trình.'");
		WriteError(Message);
		Raise Message;
	EndIf;
...
"Будет выполнен переход с другой программы.';vi='Đã thay đổi tên cấu hình thành ""%1""."
"Sẽ thực hiện chuyển giao từ chương trình khác.'"),
			Metadata.Name);
	ElsIf DataUpdateMode = "VersionUpdate" Then
		Message = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Initial data population up to version ""%1"" is in progress.';ru='Выполняется начальное заполнение данных до версии ""%1"".';vi='Đang thực hiện điền ban đầu dữ liệu đến phiên bản ""%1"".'"),
	...
	Message = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Infobase is successfully updated to version ""%1"".';ru='Обновление информационной базы на версию ""%1"" выполнено успешно.';vi='Việc cập nhật cơ sở thông tin lên phiên bản ""%1"" đã hoàn tất thành công.'"), MetadataVersion);
	WriteInformation(Message);
...
"Ожидалось: %3; передано значение: %4 (тип %5).';vi='Giá trị tham số %1 không hợp lệ trong %2."
"Dự kiến: %3; đã chuyển giá trị: %4 (kiểu %5).'");
		Message = StringFunctionsClientServer.SubstituteParametersInString(Message,
			"DataUpdateMode", "OnDefineDataUpdateMode", 
			NStr("en='InitialFilling, VersionUpdate or TransferFromAnotherApplication';ru='НачальноеЗаполнение, ОбновлениеВерсии или ПереходСДругойПрограммы';vi='InitialFilling, VersionUpdate or TransferFromAnotherApplication'"), 
...
Function RunUpdateIteration(Val IterationUpdate, Val Parameters) Export
	...
	If CurrentIBVersion <> "0.0.0.0"
		...
		If HandlersToExecute = Undefined Then
			If IterationUpdate.ThisMainConfiguration Then 
				MessagePattern = NStr("en='Update plan of configuration %1 from version %2 to version %3 is not found';ru='Не найден план обновления конфигурации %1 с версии %2 на версию %3';vi='Không tìm thấy hệ thống cập nhật cấu hình %1 từ phiên bản %2 sang phiên bản %3'");
			Else
				MessagePattern = NStr("en='Update plan of library %1 from version %2 to version %3 is not found';ru='Не найден план обновления библиотеки %1 с версии %2 на версию %3';vi='Chưa tìm thấy kế hoạch cập nhật thư viện %1 từ phiên bản %2 sang phiên bản %3'");
			EndIf;
	...
	For Each Version IN HandlersToExecute.Rows Do
		...
		If Version.Version = "*" Then
			Message = NStr("en='Required procedures of infobase update are in progress.';ru='Выполняются обязательные процедуры обновления информационной базы.';vi='Đang thực hiện các thủ tục bắt buộc để cập nhật cơ sở thông tin.'");
		Else
			NewInfobaseVersion = Version.Version;
			If CurrentIBVersion = "0.0.0.0" Then
				Message = NStr("en='Initial data population is in progress.';ru='Выполняется начальное заполнение данных.';vi='Đang thực hiện điền ban đầu dữ liệu.'");
			ElsIf IterationUpdate.ThisMainConfiguration Then 
				Message = NStr("en='Updating the infobase from version %1 to version %2.';ru='Выполняется обновление информационной базы с версии %1 на версию %2.';vi='Đang thực hiện việc cập nhật cơ sở thông tin từ phiên bản %1 lên phiên bản %2.'");
			Else
				Message = NStr("en='Updating data of library %3 from version %1 to version %2.';ru='Выполняется обновление данных библиотеки %3 с версии %1 на версию %2.';vi='Đang thực hiện cập nhật dữ liệu thư viện %3 từ phiên bản %1 sang phiên bản %2.'");
			EndIf;
			Message = StringFunctionsClientServer.SubstituteParametersInString(Message,
		...
		If Version.Version = "*" Then
			Message = NStr("en='Required procedures of infobase update are performed.';ru='Выполнены обязательные процедуры обновления информационной базы.';vi='Đã thực hiện thủ tục bắt buộc cập nhật cơ sở thông tin.'");
		Else
			If IterationUpdate.ThisMainConfiguration Then 
				Message = NStr("en='Infobase update from version %1 to version %2 is completed.';ru='Выполнено обновление информационной базы с версии %1 на версию %2.';vi='Đã hực hiện cập nhật cơ sở thông tin từ phiên bản %1 lên phiên bản %2.'");
			Else
				Message = NStr("en='Data of library %3 is updated from version %1 to version %2.';ru='Выполнено обновление данных библиотеки %3 с версии %1 на версию %2.';vi='Đã thực hiện cập nhật dữ liệu của thư viện %3 từ phiên bản %1 sang phiên bản %2.'");
			EndIf;
...
Function RefreshInfobaseInBackground(FormUUID, IBBlock) Export
	...
	Try
		...
		Result = LongActions.ExecuteInBackground(
			...
			IBUpdateParameters,
			NStr("en='Background update of the infobase';ru='Фоновое обновление информационной базы';vi='Cập nhật cơ sở thông tin nền'"));
...
""
"Подробности ошибки:"
"%1';vi='Không thể thực hiện cập nhật cơ sở thông tin:"
"- Không thể thiết lập chế độ độc quyền"
"- Phiên bản cấu hình không cho phép việc cập nhật mà chưa thiết lập chế độ độc quyền"
""
"Chi tiết lỗi:"
"%1'"),
...
Procedure GoFromAnotherApplication()
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		Raise NStr("en='When working in SaaS, transfer from another application is unexpected.';ru='При работе в модели сервиса переход с другой программы не предусмотрен.';vi='Khi làm việc trong mô hình dịch vụ đám mây, không thể chuyển từ chương trình khác.'");
	EndIf;
...
"с"
"другой программы"
"""%1"" произошла ошибка: ""%2"".';vi='Khi gọi hàm xử lý chuyển"
"từ"
"chương trình khác"
"""%1"" đã xảy ra lỗi: ""%2"".'"),
				HandlerName,
...
Function EventLogMonitorEvent() Export
	...
	Return NStr("en='Infobase update';ru='Обновление информационной базы';vi='Cập nhật cơ sở thông tin'", CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorMessageTextProtocol() Export
	...
	Return EventLogMonitorEvent() + ". " + NStr("en='Execution protocol';ru='Протокол выполнения';vi='Giao thức thực hiện'", CommonUseClientServer.MainLanguageCode());
...
"обработчика ""%1"" не должен быть установлен признак ""ExclusiveMode"".';vi='Trong hàm xử lý"
"hoãn lại ""%1"" không cần đặt dấu ""ExclusiveMode"".'");
...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	For Each Section IN Sections Do
		...
		Work.Presentation = NStr("en='Application update is not completed';ru='Обновление программы не завершено';vi='Chưa kết thúc cập nhật chương trình'");
...
Procedure DisableUpdateHandlers(LibraryID, HandlersToExecute, MetadataVersion, HandlersExecutionProcess)
	...
	For Each DisabledHandler IN SwitchableHandlers Do
		...
		ElsIf RunningHandler <> Undefined AND RunningHandler.Version <> "*"
			AND DisabledHandler.Version = MetadataVersion Then
			ErrorMessage = NStr("en='The handler of %1 update cannot be disabled as it is executed only when changing to version %2.';ru='Обработчик обновления %1 не может быть отключен, так как он выполняется только при переходе на версию %2';vi='Không thể ngắt hàm xử lý cập nhật %1, bởi vì hàm này chỉ thực hiện khi chuyển sang phiên bản %2'");
		...
		ElsIf RunningHandler = Undefined Then
			ErrorMessage = NStr("en='Disabled update handler %1 does not exist';ru='Отключаемый обработчик обновления %1 не существует';vi='Không tồn tại hàm cập nhật %1 bị ngắt bỏ'");
			ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(ErrorMessage,
...
"обработчика"
"обновления:"
"""%1"" произошла ошибка: ""%2"".';vi='Khi gọi ra"
"hàm xử lý"
"cập nhật:"
"""%1"", đã xảy ra lỗi: ""%2"".'"),
			HandlerName,
...
Procedure ValidateNestedTransaction(TransactionActiveOnExecutionStart, ProcessorsName)
	...
	EventName = EventLogMonitorEvent() + ". " + NStr("en='Executing handlers';ru='Выполнение обработчиков';vi='Thực hiện hàm sự kiện'", CommonUseClientServer.MainLanguageCode());
...
"При активных транзакциях, открытых выше по стеку, исключение также необходимо пробрасывать выше по стеку.';vi='Lỗi thực hiện hàm xử lý"
"cập nhật %1: Hàm xử lý cập nhật đã bị tác động bởi ngoại lệ khi giao tác ngoài có hiệu lực."
"Khi giao tác có hiệu lực, mở ra cao hơn so với ngăn xếp, ngoại lệ cũng cần được chuyển lên cao hơn so với ngăn xếp.'");
...
"обработчика обновления %1: Обработчиком обновления была закрыта лишняя транзакция, открытая ранее (выше по стеку).';vi='Lỗi thực hiện"
"hàm xử lý cập nhật %1: Hàm xử lý cập nhật đã đóng giao tác thừa, sau khi mở ra trước đó (cao hơn so với ngăn xếp).'");
			Comment = StringFunctionsClientServer.SubstituteParametersInString(CommentTemplate, ProcessorsName);
...
"обработчика обновления %1: Открытая внутри обработчика обновления транзакция осталась активной (не была закрыта или отменена).';vi='Lỗi thực hiện"
"hàm xử lý cập nhật %1: Giao tác mở ra bên trong hàm xử lý cập nhật vẫn còn hoạt động (không bị đóng lại hoặc hủy bỏ).'");
			Comment = StringFunctionsClientServer.SubstituteParametersInString(CommentTemplate, ProcessorsName);
	...
	For Each Handler IN IterationUpdate.Handlers Do
		...
		If IsBlankString(Handler.Version) Then
			...
			If Handler.InitialFilling <> True Then
				ErrorDescription = NStr("en='The Version or InitialFilling property is not filled out in the handler.';ru='У обработчика не заполнено свойство Версия или свойство НачальноеЗаполнение.';vi='Trong hàm sự kiện chưa điền thuộc tính Версия hoặc thuộc tính НачальноеЗаполнение.'");
			EndIf;
...
"Правильный формат, например: ""2.1.3.70"".';vi='Trong hàm sự kiện đã điền sai thuộc tính Version: ""%1""."
"Ví dụ định dạng đúng: ""2.1.3.70"".'"),
					Metadata.Version);
			EndTry;
...
"Версия не может быть нулевой.';vi='Trong hàm sự kiện đã điền sai thuộc tính Version: ""%1""."
"Phiên bản không thể bằng 0.'"),
					Metadata.Version);
			EndIf;
...
"свойство Приоритет или свойство ВыполнятьВГруппеОбязательных.';vi='Hàm xử lý chưa điền đúng"
"thuộc tính Mức độ ưu tiên hoặc thuộc tính ВыполнятьВГруппеОбязательных.'");
			EndIf;
		EndIf;
...
"Допустимое значение: ""Монопольно"", ""Отложенно"", ""Оперативно"".';vi='Trong hàm sự kiện ""%1"" chưa điền đúng thuộc tính ExecutionMode."
"Giá trị có thể có ""Độc quyền"", ""Hoãn lại"", ""Hiện thời"".'"),
				Handler.Procedure);
		EndIf;
...
"свойство Опциональный или свойство НачальноеЗаполнение.';vi='Trong hàm xử lý chưa điền"
"thuộc tính Optional hoặc thuộc tính InitialFilling.'");
		EndIf;
		...
		If IterationUpdate.ThisMainConfiguration Then
			ErrorTitle = NStr("en='An error occurred in the property of the configuration update handler';ru='Ошибка в свойстве обработчика обновления конфигурации';vi='Lỗi trong thuộc tính hàm cập nhật cấu hình'");
		Else
			ErrorTitle = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='An error occurred in the property of library update handler %1 of version %2';ru='Ошибка в свойстве обработчика обновления библиотеки %1 версии %2';vi='Lỗi trong thuộc tính hàm cập nhật thư viện %1 phiên bản %2'"),
...
Function CountHandlersToCurrentVersion(IterationsUpdate)
	...
	Message = NStr("en='To update the application to a new version, the following handlers will be executed: %1';ru='Для обновления программы на новую версию будут выполнены обработчики: %1';vi='Để cập nhật chương trình sang phiên bản mới, sẽ thực hiện hàm xử lý: %1'");
...
Procedure WriteInformationAboutUpdate(Handler, HandlersExecutionProcess, InBackground)
	...
	If Not CommonUseReUse.DataSeparationEnabled() Then
		Message = NStr("en='Update handler %1 is in progress (%2 of %3).';ru='Выполняется обработчик обновления %1 (%2 из %3).';vi='Hàm sự kiện cập nhật đang thực hiện %1 (%2 trong số %3).'");
		Message = StringFunctionsClientServer.SubstituteParametersInString(
	...
	For Each HandlerTreeLibrary IN DataAboutUpdate.HandlerTree.Rows Do
		...
		For Each HandlerTreeVersion IN HandlerTreeLibrary.Rows Do
			...
			For Each UpdateHandler IN HandlerTreeVersion.Rows Do
				...
				Try
					MessageAboutRunningHandler = NStr("en='Updating ""%1"".';ru='Выполняется процедура обновления ""%1"".';vi='Đang thực hiện thủ tục cập nhật ""%1"".'");
