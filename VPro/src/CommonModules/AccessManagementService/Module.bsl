...
""
"Пользователей информационной базы следует создавать в режиме 1С:Предприятия.';vi='Đã phát hiện rằng,"
"người sử dụng cơ sở thông tin với"
"vai trò"
"""Toàn quyền"" đã được tạo trong"
"Bộ thiết kế: - không tìm thấy người sử dụng trong"
"danh mục Người sử dụng, - người sử dụng đã được ghi nhận trong danh mục Người sử dụng, - người sử dụng đã được thêm vào nhóm truy cập Người quản trị."
""
"Cần tạo người sử dụng cơ sở thông tin ở chế độ 1C:DOANH NGHIỆP.'");
...
"прав для добавления пользователя ""%1"" в группу доступа Администраторы.';vi='Không đủ"
"quyền để thêm người sử dụng ""%1"" vào nhóm truy cập Quản trị hệ thống.'"),
				...
				If OldRoles[String.Role] = Undefined Then
					...
					Else
						...
						For Each Profile IN Profiles Do
							WriteLogEvent(
								NStr("en='Access management.Role is not found in metadata';ru='Управление доступом.Роль не найдена в метаданных';vi='Quản lý truy cập.Vai trò không tìm thấy trong Metadata'",
...
"ролей пользователя"
"""%1"" роль"
"""%2"" профиля групп доступа ""%3"" не найдена в метаданных.';vi='Khi cập nhật"
"vai trò người sử dụng"
"""%1"" không tìm thấy vai trò"
"""%2"" của hồ sơ nhóm truy cập ""%3"" trong metadata.'"),
									String(CurrentUser),
									String.Role,
									String(Profile)),
...
"бы один пользователь, которому разрешен вход в программу.';vi='Trong nhóm"
"truy cập Người quản trị cần có ít nhất"
"một người sử dụng được phép đăng nhập vào chương trình.'");
	EndIf;
...
"chương trình có thêm người sử dụng đầu tiên, do đó sẽ tự động đưa người này vào nhóm truy cập Quản trị hệ thống."
"Tiếp tục?'")
...
Procedure DataFillingForAccessLimit(DataQuantity = 0, OnlyCacheAttributes = False, HasChanges = Undefined) Export
	...
	If DataQuantity < 10000 Then
		WriteLogEvent(
			NStr("en='Access management.Data population for access restriction';ru='Управление доступом.Заполнение данных для ограничения доступа';vi='Quản lý truy cập.Điền dữ liệu để hạn chế truy cập'",
				 CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Information,
			,
			,
			NStr("en='Data population for access restriction is completed.';ru='Завершено заполнение данных для ограничения доступа.';vi='Kết thúc điền dữ liệu để hạn chế truy cập.'"),
			EventLogEntryTransactionMode.Transactional);
	...
	Else
		WriteLogEvent(
			...
			,
			,
			NStr("en='Part of data for access restriction is written.';ru='Выполнена запись части данных для ограничения доступа.';vi='Đã thực hiện ghi lại phần dữ liệu để hạn chế truy cập.'"),
			EventLogEntryTransactionMode.Transactional);
	EndIf;
...
"У объекта"
"типа ""%1"" не найдена табличная часть ""Наборы значений доступа"".';vi='Sai tham số."
"Trong đối tượng"
"kiểu ""%1"" không tìm thấy phần bảng ""Tập hợp giá trị truy cập"".'"),
			ValueTypeObject);
	EndIf;
...
"Тип"
"объекта ""%1"" не найден"
"в подписках на события ""Записать наборы значений доступа"".';vi='Sai tham số"
"Không tìm thấy"
"kiểu đối tượng ""%1"""
"khi ghi nhận trong sự kiện ""Ghi nhận tập hợp giá trị truy cập"".'"),
				ValueTypeObject);
	EndIf;
...
"сведений НаборыЗначенийДоступа в измерении Объект не задан тип ""%1"".';vi='Lỗi khi ghi lại"
"tập hợp giá trị truy cập: trong biểu ghi"
"thông tin AccessValueSets, chiều đo Đối tượng chưa đặt kiểu ""%1"".'"),
...
"Тип"
"объекта ""%1"" не найден"
"в подписках на события ""Заполнить наборы значений доступа табличных частей"".';vi='Sai tham số."
"Không tìm thấy"
"kiểu đối tượng ""%1"""
"khi ghi nhận sự kiện ""Điền tập hợp giá trị truy cập phần bảng"".'"),
						ValueTypeObject);
			EndIf;
...
"Тип"
"объекта ""%1"" не найден"
"в подписке на события ""Записать зависимые наборы значений доступа"".';vi='Sai tham số."
"Không tìm thấy"
"kiểu đối tượng ""%1"""
"trong sự kiện ""Ghi lại tập hợp các giá trị truy cập phụ thuộc''''.'"),
			ValueTypeObject);
	EndIf;
...
"Тип"
"объекта ""%1"" не найден"
"в подписке на события ""Записать наборы значений доступа"".';vi='Sai tham số."
"Không tìm thấy"
"kiểu đối tượng ""%1"""
"khi ghi nhận sự kiện ""Ghi lại tập hợp giá trị truy cập"".'"),
				ValueTypeObject));
	EndIf;
...
"сведений НаборыЗначенийДоступа в измерении Объект не задан тип %1';vi='Lỗi khi ghi lại"
"tập hợp giá trị truy cập: trong biểu ghi"
"thông tin AccessValueSets, số đo Đối tượng chưa đặt kiểu %1'"),
				ObjectReference.Metadata().FullName()));
	EndIf;
...
Procedure OnChangeLimitAccessOnRecordsLevel(RecordLevelSecurityEnabled) Export
	...
	If RecordLevelSecurityEnabled Then
		...
		WriteLogEvent(
			...
			,
			,
			NStr("en='Data population for access restriction is started.';ru='Начато заполнение данных для ограничения доступа.';vi='Bắt đầu điền dữ liệu để hạn chế truy cập.'"),
			EventLogEntryTransactionMode.Transactional);
...
"""<Condition parameter name>:<Field name>"" ""%1"" format.';ru='Ошибка при выполнении процедуры УправлениеДоступом.УстановитьУсловиеОтбораВЗапросе().  В параметре ИмяПараметраУсловияОтбораИмяПоля не найден разделитель (двоеточие) в следующей строке формата ""<Имя параметра условия>:<Имя поля>"" ""%1"".';vi='Lỗi khi thực hiện thủ tục AccessManagement.SetFilterCriteriaInQuery(). Trong tham số"
"ParameterNameFilterConditionsFieldName không tìm thấy đường chia tách (dấu hai chấm) tại xâu ký tự có định dạng ""<Tên tham số điều kiện>:<Tên trường>"" ""%1"".'"),
				CurrentRow);
		EndIf;
	...
	Try
		If Data.NewRecordsContainOnlyDifferences Then
			...
			If Data.NameOfFirstDimension = Undefined Then
				Raise
					NStr("en='Incorrect parameters in the UpdateRecordsets procedure.';ru='Некорректные параметры в процедуре ОбновитьНаборыЗаписей.';vi='Sai tham số trong thủ tục UpdateRecordsets.'");
			Else
...
""
"Наверный параметр ОписаниеОбъектаМетаданных.';vi='Lỗi"
"trong hàm ПустаяСсылкаОбъектаМетаданных của mô-đun chung УправлениеДоступомСлужебный."
""
"Sai tham số ОписаниеОбъектаМетаданных.'");
	EndIf;
...
"""%1"".';vi='Lỗi trong hàm MetadataObjectEmptyRef"
"của mô-đun chung AccessManagementService."
""
"Không thể nhận tham chiếu rỗng đối với đối tượng metadata"
"""%1"".'"),
...
""
"В тексте запроса не найдена строка ""&ПодстановкаПоляВидИзмененияСтроки"".';vi='Lỗi trong"
"giá trị tham số ТекстЗапросаВыбораСтарых thủ tục ТекстЗапросаВыбораИзменений mô-đun УправлениеДоступомСлужебный."
""
"Trong nội dung truy vấn không tìm thấy xâu ký tự ""&ПодстановкаПоляВидИзмененияСтроки"".'");
	EndIf;
...
""
"В тексте запроса не найдена строка ""&ПодстановкаПоляВидИзмененияСтроки"".';vi='Lỗi trong"
"giá trị tham số ТекстЗапросаВыбораНовых процедуры ТекстЗапросаВыбораИзменений mô-đun УправлениеДоступомСлужебный."
""
"Trong nội dung truy vấn không tìm thấy xâu ký tự ""&ПодстановкаПоляВидИзмененияСтроки"".'");
	EndIf;
...
"""%1"""
"возникла ошибка: %2';vi='Khi cập nhật tập hợp phụ thuộc"
"các giá trị truy cập đối tượng"
"""%1"""
"đã xảy ra lỗi: %2'"),
...
"Допустимые типы: Структура, Неопределено.';vi='Sai kiểu tập hợp thuộc tính ""%1""."
"Các kiểu được phép sử dụng: Cấu trúc, Chưa xác định.'"),
	...
	For Each KeyAndValue IN Parameters Do
		If Not AllParameters.Property(KeyAndValue.Key) Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Non-existing parameter %1 is specified';ru='Указан несуществующий параметр %1';vi='Chỉ ra tham số không tồn tại %1'"), KeyAndValue.Key);
		EndIf;
	...
	If ValueIsFilled(MandatoryParameters) Then
		...
		For Each KeyAndValue IN MandatoryParameters Do
			If Not Parameters.Property(KeyAndValue.Key) Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Required parameter %1 is not specified';ru='Не указан обязательный параметр %1';vi='Chưa chỉ ra tham số bắt buộc %1'"), KeyAndValue.Key);
			EndIf;
		EndDo;
	EndIf;
...
""
"Операция может быть выполнена только интерактивно.';vi='Để thay đổi"
"truy cập quản trị, cần có mật khẩu của người sử dụng Service."
""
"Chỉ có thể thực hiện giao dịch một cách trực tác.'");
		EndIf;
...
""
"Допустимо только 2 значения: ""1"" и ""-1"".';vi='Lỗi"
"trong thủ tục ОбновитьНаборыЗаписей của mô-đun chung УправлениеДоступомСлужебный."
""
"Sai giá trị"
"tham số НовыеЗаписи - cột ВидИзмененияСтроки có chứa giá trị không khả dụng ""%1""."
""
"Chỉ có 2 giá trị hợp lệ: ""1"" и ""-1"".'"),
...
""
"Не удалось найти"
"требуемую в строку в значении параметра НовыеЗаписи.';vi='Lỗi"
"trong thủ tục ОбновитьНаборыЗаписей mô-đun chung УправлениеДоступомСлужебный."
""
"Không tìm thấy"
"dòng cần thiết trong giá trị tham số НовыеЗаписи.'");
...
Procedure FillPresentationTableAllAllowedInForm(Form, IsProfile)
	...
	If IsProfile Then
		...
		String.Presentation = NStr("en='All prohibited, exceptions are assigned in access groups';ru='Все запрещены, исключения назначаются в группах доступа';vi='Cấm tất cả, các trường hợp ngoại lệ được quy định trong các nhóm truy cập'");
		
		String = Form.PresentationsAllAllowed.Add();
		String.Name = "InitiallyAllAllowed";
		String.Presentation = NStr("en='All permitted, exceptions are assigned in access groups';ru='Все разрешены, исключения назначаются в группах доступа';vi='Cho phép tất cả, các trường hợp ngoại lệ được quy định trong các nhóm truy cập'");
		
		String = Form.PresentationsAllAllowed.Add();
		String.Name = "AllProhibited";
		String.Presentation = NStr("en='All prohibited, exceptions are assigned in a profile';ru='Все запрещены, исключения назначаются в профиле';vi='Cấm tất cả, các trường hợp ngoại lệ được quy định trong hồ sơ'");
		
		String = Form.PresentationsAllAllowed.Add();
		String.Name = "AllAllowed";
		String.Presentation = NStr("en='All permitted, exceptions are assigned in a profile';ru='Все разрешены, исключения назначаются в профиле';vi='Cho phép tất cả, các trường hợp ngoại lệ được quy định trong hồ sơ'");
	Else
		String = Form.PresentationsAllAllowed.Add();
		String.Name = "AllProhibited";
		String.Presentation = NStr("en='All prohibited';ru='Все запрещены';vi='Cấm tất cả'");
		
		String = Form.PresentationsAllAllowed.Add();
		String.Name = "AllAllowed";
		String.Presentation = NStr("en='All allowed';ru='Все разрешены';vi='Cho phép tất cả'");
	EndIf;
