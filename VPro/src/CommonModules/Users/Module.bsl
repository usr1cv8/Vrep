...
Function UnspecifiedUserFullName() Export
	...
	Return NStr("en='<Not specified>';ru='<Не указан>';vi='<Chưa chỉ ra>'");
...
Function ReadIBUser(Val ID,
	...
	If IBUser = Undefined Then
		ErrorDescription = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Infobase user ""%1"" is not found.';ru='Пользователь информационной базы ""%1"" не найден.';vi='Không tìm thấy người sử dụng cơ sở thông tin ""%1"".'"), ID);
		Return False;
	EndIf;
...
"создать"
"пользователя информационной базы %1, так как он уже существует.';vi='Không thể"
"tạo"
"người sử dụng cơ sở thông tin %1, bởi vì đã tồn tại người sử dụng này.'"),
			OldProperties.Name,
			OldProperties.UUID);
		Return False;
	Else
		If UpdatedProperties.Property("OldPassword")
		   AND TypeOf(UpdatedProperties.OldPassword) = Type("String") Then
			...
			If Not OldPasswordIsSame Then
				ErrorDescription = ErrorDescriptionOnIBUserWrite(
					NStr("en='When writing user of infobase %1, old password was specified incorrectly.';ru='При записи пользователя информационной базы %1, старый пароль указан не верно.';vi='Khi ghi lại người sử dụng cơ sở thông tin %1, đã chỉ ra sai mật khẩu cũ.'"),
					OldProperties.Name,
					OldProperties.UUID);
				Return False;
			EndIf;
...
"произошла"
"ошибка: ""%2"".';vi='Khi ghi lại người sử dụng"
"cơ sở thông tin %1"
"đã xảy ra"
"lỗi: ""%2"".'"),
			IBUser.Name,
...
"информационной базы %1"
"произошла"
"ошибка: ""%2"".';vi='Khi xóa người sử dụng"
"cơ sở thông tin %1"
"đã xảy ra"
"lỗi: ""%2"".'"),
				IBUser.Name,
...
"информационной базы"
"Имя (для входа): ""%1"" превышает длину 64 символа.';vi='Lỗi ghi lại người sử dụng"
"cơ sở thông tin"
"Tên (để đăng nhập): ""%1"" vượt quá chiều dài 64 ký tự.'"),
								PropertyValue);
...
"информационной базы"
"Имя (для входа): ""%1"" содержит запрещенный символ "":"".';vi='Lỗi ghi lại người sử dụng"
"cơ sở thông tin"
"Tên (để đăng nhập):  ""%1"" chứa giá trị không được phép "":"".'"),
								PropertyValue);
						EndIf;
					EndIf;
...
Procedure FindAmbiguousInfobaseUsers(Val User,
	...
	ErrorDescription = NStr("en='Error in the data base:';ru='Ошибка в базе данных:';vi='Lỗi trong cơ sở dữ liệu:'") + Chars.LF;
		...
		If String.AmbiguousID <> CurrentAmbiguousID Then
			...
			Else
				...
				If CurrentInfobaseUser = Undefined Then
					NameForEntering = NStr("en='<not specified>';ru='<не найден>';vi='<không tìm thấy>'");
				Else
...
"сервиса с идентификатором ""%2"" соответствует более одного элемента в справочнике Пользователи:';vi='Người sử dụng"
"Service với tên (ID) ""%2"" tương ứng với hơn một phần tử trong danh mục Người sử dụng:'"),
						NStr("en='IB user %1 with"
"identifier %2 corresponds to more than one item in the Users catalog:';ru='Пользователю"
"ИБ ""%1"" с идентификатором ""%2"" соответствует более одного элемента в справочнике Пользователи:';vi='Người sử dụng"
"CSTT ""%1"" với tên (ID) ""%2"" tương ứng với hơn một phần tử trong danh mục Người sử dụng:'") ),
						NameForEntering,
						CurrentAmbiguousID);
			EndIf;
		EndIf;
...
"	""%1"" с идентификатором ссылки %2';vi='"
"	""%1"" có tên tham chiếu %2'"),
					String.User,
					String.User.UUID());
		EndIf;
	EndDo;
...
Function ErrorDescriptionOnIBUserWrite(ErrorTemplate,
	...
	If WriteInJournal Then
		WriteLogEvent(
			NStr("en='Users.An error occurred when writing infobase user';ru='Пользователи.Ошибка записи пользователя ИБ';vi='Người sử dụng.Lỗi ghi người sử dụng CSTT'",
