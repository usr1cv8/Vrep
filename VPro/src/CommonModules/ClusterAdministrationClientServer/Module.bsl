...
Function VerifyFilterConditions(Val VerifiedObject, Val Filter = Undefined) Export
	...
	For Each Condition IN Filter Do
		...
		Else
			...
			Raise NStr("en='Filter is set incorrectly.';ru='Некорректно задан фильтр!';vi='Đặt sai bộ lọc!'");
...
Function AdministrationManager(Val AdministrationParameters)
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Unknown connection type: %1.';ru='Неизвестный тип подключения: %1!';vi='Kiểu kết nối không xác định: %1!'"), AdministrationParameters.ConnectionType);
