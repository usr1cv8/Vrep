...
Function SerializationCodePostalIndex() Export
	...
	Return AddressingObjectSerializationCode(NStr("en='Zip code';ru='Почтовый индекс';vi='Mã bưu chính'"));
...
Function TypesOfAddressingAddressesRF() Export
	...
	Result.Add(ObjectStringAddress("1010", NStr("en='House';ru='дом';vi='Nhà'"),          1, 1, 2));
	Result.Add(ObjectStringAddress("1020", NStr("en='Ownership';ru='Владение';vi='Khu'"),     1, 2, 1));
	Result.Add(ObjectStringAddress("1030", NStr("en='Home-ownership';ru='Домовладение';vi='Sở hữu nhà'"), 1, 3, 3));
	
	Result.Add(ObjectStringAddress("1050", NStr("en='Block';ru='блок';vi='phong tỏa'"),     2, 1));
	Result.Add(ObjectStringAddress("1060", NStr("en='Construction';ru='строительство';vi='xây dựng'"),   2, 2, 1));
	Result.Add(ObjectStringAddress("1080", NStr("en='Email';ru='Письмо';vi='E-mail'"),     2, 3, 3));
	Result.Add(ObjectStringAddress("1070", NStr("en='Construction';ru='Сооружение';vi='Công trình'"), 2, 4, 2));
	Result.Add(ObjectStringAddress("1040", NStr("en='Land lot';ru='Участок';vi='Khu đất'"),    2, 5));
	
	Result.Add(ObjectStringAddress("2010", NStr("en='Apartment';ru='Квартира';vi='Căn hộ'"),  3, 1));
	Result.Add(ObjectStringAddress("2030", NStr("en='Office';ru='офис';vi='Văn phòng'"),      3, 2));
	Result.Add(ObjectStringAddress("2040", NStr("en='Box';ru='коробка';vi='hộp'"),      3, 3));
	Result.Add(ObjectStringAddress("2020", NStr("en='Unit';ru='Единица"
"измерения';vi='Đơn vị'"), 3, 4));
	Result.Add(ObjectStringAddress("2050", NStr("en='Room';ru='Комната';vi='Phòng'"),   3, 5));
	//  Our abbreviations for supporting the backward match during parsing.
	Result.Add(ObjectStringAddress("2010", NStr("en='application.';ru='приложение.';vi='ứng dụng.'"),       3, 6));
	Result.Add(ObjectStringAddress("2030", NStr("en='f';ru='ж';vi='f'"),       3, 7));
	
	// Adjusting objects
	Result.Add(ObjectStringAddress("10100000", NStr("en='Zip code';ru='Почтовый индекс';vi='Mã bưu chính'")));
	Result.Add(ObjectStringAddress("10200000", NStr("en='Address point';ru='Адресная точка';vi='Địa chỉ'")));
	Result.Add(ObjectStringAddress("10300000", NStr("en=""Gardeners' partnership"";ru='Садовое товарищество';vi='Hội nhà vườn'")));
	Result.Add(ObjectStringAddress("10400000", NStr("en='Item of street-road network, planning structure of additional address element';ru='Элемент улично-дорожной сети, планировочной структуры дополнительного адресного элемента';vi='Phần tử của mạng lưới đường phố, cấu trúc quy hoạch của phần tử địa chỉ bổ sung'")));
	Result.Add(ObjectStringAddress("10500000", NStr("en='Industrial area';ru='Промышленная зона';vi='Khu công nghiệp'")));
	Result.Add(ObjectStringAddress("10600000", NStr("en='Garage construction co-operative';ru='Гаражно-строительный кооператив';vi='Ga-ra'")));
	Result.Add(ObjectStringAddress("10700000", NStr("en='TERRITORY';ru='ТЕРРИТОРИЯ';vi='LÃNH THỔ'")));
