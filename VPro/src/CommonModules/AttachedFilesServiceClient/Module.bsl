...
Function GetFileIntoWorkingDirectory(Val FileBinaryDataAddress,
	...
	Except
		...
		ErrorInfo = NStr("en='An error occurred when creating a directory on the drive:';ru='Ошибка создания каталога на диске:';vi='Lỗi tạo thư mục trên đĩa:'") + " " + ErrorInfo;
...
"при"
"помещении файла ""%1"" во временное хранилище.';vi='Lỗi"
"khi"
"sắp xếp tệp ""%1"" vào kho chứa tạm thời.'"),
				PathToFile) );
		Return Result;
	EndIf;
...
Procedure AddFilesByDragging(Val FileOwner, Val FormID, Val FileNameArray) Export
	...
	If AttachedFilesArray.Count() = 1 Then
		...
		ShowUserNotification(
			NStr("en='Create';ru='Создание';vi='Tạo mới'"),
			GetURL(AttachedFile),
			AttachedFile,
...
Procedure PlaceEditedFileOnDriveIntoStorageExtensionRequested(FileOperationsExtensionConnected, AdditionalParameters) Export
	...
	If FileOperationsExtensionConnected Then
		...
		Else
			CommonUseClientServer.MessageToUser(
				NStr("en='File is not found in the working directory.';ru='Файл не найден в рабочем каталоге.';vi='Không tìm thấy tệp trong thư mục làm việc.'"));
		EndIf;
...
"""%2""."
""
"Продолжить?';vi='Tên tệp được chọn"
"""%1"""
"khác với tên tệp trong kho chứa"
"""%2""."
""
"Tiếp tục?'"),
...
Procedure SelectFileOnDriveAndPlaceIntoStorageExtensionRequested(FileOperationsExtensionConnected, AdditionalParameters) Export
	...
	If FileOperationsExtensionConnected Then
		...
		FileChoice.Filter = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='All files (*.%1)|*.%1';ru='Все файлы (*.%1)|*.%1';vi='Tất cả tệp (*.%1)|*.%1'"), FileData.Extension);
...
Procedure OpenDirectoryWithFileExtensionRequested(FileOperationsExtensionConnected, AdditionalParameters) Export
	...
	If FileOperationsExtensionConnected Then
		...
		If IsBlankString(UserWorkingDirectory) Then
			ShowMessageBox(, NStr("en='Working directory is not specified';ru='Не задан рабочий каталог';vi='Chưa xác định thư mục làm việc'"));
			Return;
		EndIf;
...
Procedure PlaceAttachedFile(Notification, AttachedFile, FormID, AdditionalParameters = Undefined) Export
	...
	Context.Insert("ErrorTitle",
		NStr("en='Cannot place a file from the computer to the file storage due to:';ru='Не удалось поместить файл с компьютера в хранилище файлов по причине:';vi='Không thể đặt tệp từ máy tính vào kho chứa tệp do nguyên nhân:'") + Chars.LF);
...
Procedure PlaceAttachedFileAfterConnectionExpansions(ExtensionAttached, Context) Export
	...
	If Not ExtensionAttached Then
		...
		Result.Insert("ErrorDescription", Context.ErrorTitle +
			NStr("en='File operation extension was not installed for the web browser.';ru='В обозреватель интернет не установлено расширение для работы с файлами.';vi='Trong trình duyệt Internet chưa cài đặt phần mở rộng để làm việc với tệp.'"));
...
Procedure PlaceAttachedFileAfterGettingWorkingDirectory(Result, Context) Export
	...
	If Context.FullFileName <> Context.FullNameOfThePlacedFile Then
		...
		Action.Insert("ErrorTitle", Context.ErrorTitle +
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Directory is not created due to:';ru='Создание каталога не выполнено по причине:';vi='Không thể tạo thư mục do nguyên nhân:'"), Context.FileDir));
		...
		Action.Insert("ErrorTitle", Context.ErrorTitle +
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Cannot change property ""View only"" of the file as:';ru='Изменение свойства файла ""Только просмотр"" не выполнено по причине:';vi='Chưa thực hiện thay đổi thuộc tính của tệp ""Chỉ đọc"" do:'"), Context.FullFileName));
		ActionsWithFile.Add(Action);
		...
		Action.Insert("ErrorTitle", Context.ErrorTitle +
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='File is not deleted due to:';ru='Удаление файла не выполнено по причине:';vi='Không thể xóa tệp do nguyên nhân:'"), Context.FullFileName));
		ActionsWithFile.Add(Action);
		...
		Action.Insert("ErrorTitle", Context.ErrorTitle +
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='File is not copied due to:';ru='Копирование файла не выполнено по причине:';vi='Không thể sao chép tệp do nguyên nhân:'"), Context.FullFileName));
		ActionsWithFile.Add(Action);
	EndIf;
	...
	Action.Insert("ErrorTitle", Context.ErrorTitle +
		StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='File properties were not received due to:';ru='Получение свойств файла не выполнено по причине:';vi='Không thể nhận thuộc tính tệp do nguyên nhân:'"), Context.FullFileName));
	ActionsWithFile.Add(Action);
...
Procedure GetAttachedFile(Notification, AttachedFile, FormID, AdditionalParameters = Undefined) Export
	...
	Context.Insert("ErrorTitle",
		NStr("en='Cannot receive the file to the computer from the file storage due to:';ru='Не удалось получить файл на компьютер из хранилища файлов по причине:';vi='Không thể nhận tệp vào máy tính từ kho chứa tệp do nguyên nhân:'") + Chars.LF);
	...
	If Context.ForEditing
	   AND Context.FileData.IsEditing <> UsersClientServer.AuthorizedUser() Then
		...
		Result.Insert("ErrorDescription", Context.ErrorTitle + StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='The file is already being edited by user %1.';ru='Файл уже редактирует пользователь %1.';vi='Tệp đã được soạn bởi người sử dụng %1.'"), String(Context.FileData.IsEditing)));
		ExecuteNotifyProcessing(Context.Notification, Result);
		Return;
	EndIf;
...
Procedure GetAttachedFileAfterGettingWorkingDirectory(Result, Context) Export
	...
	Action.Insert("ErrorTitle", Context.ErrorTitle +
		StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Cannot set file properties due to:';ru='Установка свойств файла не выполнено по причине:';vi='Chưa thiết lập thuộc tính của tệp do nguyên nhân:'"), Context.FullFileName));
	ActionsWithFile.Add(Action);
...
"""%1"" уже зашифрован.';vi='Tệp"
"""%1"" đã được mã hóa.'"), String(AttachedFile)));
		ExecuteNotifyProcessing(ResultHandler, Undefined);
		Return;
	EndIf;
	
	If ValueIsFilled(FileData.IsEditing) Then
		ShowMessageBox(, NStr("en='Cannot encrypt locked file.';ru='Нельзя зашифровать занятый файл.';vi='Không thể mã hóa tệp đang chiếm.'"));
		ExecuteNotifyProcessing(ResultHandler, Undefined);
		Return;
	EndIf;
...
Procedure NotifyAboutChangeAndDeleteFileInWorkDirectory(Val AttachedFile, Val FileData) Export
	...
	If CommonUseClient.SubsystemExists("StandardSubsystems.DigitalSignature") Then
		...
		ModuleDigitalSignatureClient.InformAboutObjectEncryption(
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='File: %1';ru='Файл: %1';vi='Tệp: %1'"), AttachedFile));
	EndIf;
...
Procedure UpdateStateAboutFileSaving(Val SelectedFiles, Val File, Val CurrentPosition = Undefined);
	...
	If SelectedFiles.Count() > 1 Then
		...
		If CurrentPosition = Undefined Then
			Status(NStr("en='File saving is complete.';ru='Сохранение файлов завершено.';vi='Đã hoàn tất lưu tệp.'"));
		Else
			...
			LabelMore = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Saving file ""%1"" (%2 MB) ...';ru='Сохраняется файл ""%1"" (%2 Мб) ...';vi='Lưu tệp ""%1"" (%2 Mb)'"), File.Name, SizeInMB);
				
			StatusText = NStr("en='Saving multiple files.';ru='Сохранение нескольких файлов.';vi='Lưu nhiều tệp.'");
...
"файла ""%1"" (%2 Мб) завершено.';vi='Đã lưu lại"
"tệp ""%1"" (%2 Mb).'"), File.Name, SizeInMB);
		Else
			ExplanationText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Is Saved file ""%1"" (%2 MB)."
"Please wait...';ru='Сохраняется файл ""%1"" (%2 Мб)."
"Пожалуйста, подождите...';vi='Lưu lại tệp ""%1"" (%2 Mb)."
"Xin vui lòng đợi trong giây lát...'"), File.Name, SizeInMB);
		EndIf;
...
Procedure AddFilesExtensionRequested(FileOperationsExtensionConnected, AdditionalParameters) Export
	...
	If FileOperationsExtensionConnected Then
		...
		FileChoice.Title = NStr("en='Select file';ru='Выбор файла';vi='Chọn tệp'");
		FileChoice.Filter = ?(ValueIsFilled(Filter), Filter, NStr("en='All files';ru='Все файлы';vi='Tất cả tệp'") + " (*.*)|*.*");
...
"Mb) is being saved. Please wait...';ru='Сохраняется файл ""%1"" (%2 Мб) Пожалуйста, подождите.';vi='Đang lưu tệp ""%1"" (%2 Mb) Vui lòng chờ trong giây lát.'"),
				FileData.FileName, 
		...
		If GetFiles(FilesToReceive, ReceivedFiles, , False) Then
			Status(NStr("en='The file was successfully saved.';ru='Файл успешно сохранен.';vi='Đã lưu thành công tệp.'"), , FileChoice.FullFileName);
		EndIf;
