...
Procedure ConnectCorrespondent(Message, Sender)
	...
	If ThisEndPoint.IsEmpty()
		...
		ErrorPresentation = NStr("en='Endpoint does not match the expected one. Expected endpoint code %1. Current endpoint code %2.';ru='Конечная точка не соответствует ожидаемой. Код ожидаемой конечной точки %1. Код текущей конечной точки %2.';vi='Điểm cuối không tương ứng với điểm dự tính. Mã điểm cuối dự tính %1. Mã điểm cuối hiện tại %2.'");
	...
	If Correspondent.IsEmpty() Then // We connect the correspondent's end point
		...
		If Cancel Then // We send a message to the error service manager
			...
			ErrorPresentation = NStr("en='An error occurred when connecting to the exchange correspondent endpoint. The endpoint code of exchange correspondent %1.';ru='Ошибка подключения конечной точки корреспондента обмена. Код конечной точки корреспондента обмена %1.';vi='Lỗi kết nối điểm cuối nơi trao đổi thông tin trao đổi. Mã điểm cuối nơi trao đổi thông tin trao đổi %1.'");
			ErrorPresentation = StringFunctionsClientServer.SubstituteParametersInString(ErrorPresentation,
				Body.RecipientId);
...
"Код подключенной конечной точки корреспондента обмена %2.';vi='Lỗi khi kết nối điểm cuối nơi trao đổi thông tin trao đổi."
"Việc tùy chỉnh kết nối web-service không tương ứng với tùy chỉnh dự tính."
"Mã điểm cuối nơi trao đổi thông tin trao đổi dự tính %1."
"Mã điểm cuối nơi trao đổi thông tin trao đổi đã kết nối %1.'");
			ErrorPresentation = StringFunctionsClientServer.SubstituteParametersInString(ErrorPresentation,
				Body.RecipientId,
...
"Код этой"
"конечной токи %1 Код конечной точки корреспондента обмена %2';vi='Lỗi cập nhật tham số kết nối điểm cuối này và điểm cuối của cơ sở trao đổi."
"Mã điểm cuối"
"này %1 Mã điểm cuối của cơ sở trao đổi %2'");
			ErrorPresentation = StringFunctionsClientServer.SubstituteParametersInString(ErrorPresentation,
				MessageExchangeInternal.ThisNodeCode(),
				Body.RecipientId);
...
Procedure SetTransportSettings(Message, Sender)
	...
	If Correspondent.IsEmpty() Then
		MessageString = NStr("en='Correspondent endpoint with the ""%1"" code is not found.';ru='Не найдена конечная точка корреспондента с кодом ""%1"".';vi='Chưa tìm thấy điểm cuối đối ứng có mã ""%1"".'");
...
Function EventLogMonitorMessageTextConnectionCorrespondent()
	...
	Return NStr("en='Data exchange.Exchange correspondent connection';ru='Обмен данными.Подключение корреспондента обмена';vi='Trao đổi dữ liệu. Kết nối đối ứng trao đổi'", CommonUseClientServer.MainLanguageCode());
