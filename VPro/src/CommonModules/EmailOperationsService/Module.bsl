	...
	If AccountSettings.UseForSending Then
		...
		Except
			ErrorInfo = StringFunctionsClientServer.SubstituteParametersInString(
									NStr("en='An error occurred while sending a message: %1';ru='Ошибка при отправке сообщения: %1';vi='Lỗi khi gửi thông báo: %1'"),
									BriefErrorDescription(ErrorInfo()) );
		EndTry;
		If Not AccountSettings.UseForReceiving Then
			AdditionalMessage = Chars.LF + NStr("en='(Email sending check is completed.)';ru='(Выполнена проверка отправки электронных сообщений.)';vi='(Đã kiểm tra gửi thông báo điện tử)'");
		EndIf;
	EndIf;
		...
		Except
			...
			ErrorInfo = ErrorInfo
								+ StringFunctionsClientServer.SubstituteParametersInString(
										NStr("en='An error occurred while accessing the incoming message server: %1';ru='Ошибка доступа к серверу входящих сообщений: %1';vi='Lỗi truy cập tới Server thông báo đến: %1'"),
										BriefErrorDescription(ErrorInfo()) );
		EndTry;
		If Not AccountSettings.UseForSending Then
			AdditionalMessage = Chars.LF + NStr("en='(Email receiving check is completed).';ru='(Выполнена проверка получения электронных сообщений.)';vi='(Đã kiểm tra nhận thông báo điện tử)'");
		EndIf;
	EndIf;
...
Procedure CheckPossibilityOfSendingTestMessages(Val UserAccount, Val Password = Undefined)
	...
	EmailParameters.Insert("Subject", NStr("en='1C:Enterprise test message';ru='Тестовое сообщение 1С:Предприятие';vi='Thông báo 1C:DOANH NGHIỆP'"));
	EmailParameters.Insert("Body", NStr("en='This message is sent by subsystem of 1C:Enterprise email';ru='Это сообщение отправлено подсистемой электронной почты 1С:Предприятие';vi='Thông báo này đã gửi bởi phân hệ thư điện tử 1C:DOANH NGHIỆP'"));
...
Function EventLogMonitorEvent()
	Return NStr("en='Email account check';ru='Проверка учетной записи электронной почты';vi='Kiểm tra trương mục email'", CommonUseClientServer.MainLanguageCode());
EndFunction
...
Procedure OnFillAccessKinds(AccessKinds) Export
	...
	AccessKind.Presentation = NStr("en='Email accounts';ru='Учетные записи электронной почты';vi='Trương mục E-mail'");
