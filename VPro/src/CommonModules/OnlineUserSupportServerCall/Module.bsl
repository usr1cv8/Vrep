...
Procedure ServiceCommandsDataProcessor(
	...
	Except
		...
		HandlerContext.UserErrorDescription =
			NStr("en='Unknown error. For more details see the event log.';ru='Неизвестная ошибка. Подробнее см. в журнале регистрации.';vi='Lỗi không xác định. Chi tiết xem trong nhật ký sự kiện.'");
...
Procedure AddServiceCommands(
	...
	Except
		...
		If TypeOf(HandlerContext) = Type("Structure") Then
			...
			HandlerContext.UserErrorDescription =
				NStr("en='Error on connecting online support service.';ru='Ошибка при подключении к сервису Интернет-поддержки';vi='Lỗi khi kết nối tới dịch vụ hỗ trợ qua Internet'");
		EndIf;
	...
	Except
		...
		If TypeOf(HandlerContext) = Type("Structure") Then
			...
			HandlerContext.ActionOnErrorForClient = "DisplayFormConnectionNotAvailable";
			HandlerContext.UserErrorDescription =
				NStr("en='An error occurred while retrieving data from the online support server.';ru='Ошибка при получении данных с сервера Интернет-поддержки.';vi='Lỗi khi nhận dữ liệu từ Server hỗ trợ qua Internet.'");
		EndIf;
...
Procedure ClearUserUOSSettings() Export
	...
	Except
		ErrorInfo = NStr("en='An error occurred while processing user logout from online support. %1';ru='Ошибка при обработке выхода пользователя из Интернет-поддержки. %1';vi='Lỗi khi xử lý lấy ra người sử dụng từ hỗ trợ qua Internet. %1'");
...
Function LogEventOnlineUserSupportError()
	...
	Return NStr("en='Online user support.Error';ru='Интернет-поддержка пользователей.Ошибка';vi='Hỗ trợ người sử dụng qua Internet. Lỗi'",
...
Function LogEventOnlineUserSupportInformation()
	...
	Return NStr("en='Online user support.Information';ru='Интернет-поддержка пользователей.Информация';vi='Hỗ trợ người sử dụng qua Internet. Thông tin'",
		CommonUseClientServer.MainLanguageCode());
