...
Function XMLReferenceType(Val Value) Export
	...
	Else
		...
		Raise ServiceTechnologyIntegrationWithSSL.SubstituteParametersInString(
			NStr("en='An error occurred while determining XMLType reference for object %1: the object is not a reference type.';ru='Ошибка при определении XMLТипа ссылки для объекта %1: объект не является ссылочным!';vi='Lỗi khi xác định kiểu XML của tham chiếu đối với đối tượng %1: đối tượng không phải là tham chiếu!'"),
