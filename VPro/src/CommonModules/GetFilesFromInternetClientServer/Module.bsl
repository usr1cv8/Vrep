...
Function GetFileFromInternet(Val URL, Val SaveSetting, Val ConnectionOptions = Undefined,
	...
	If FTPProtocolIsUsed Then
		...
		Except
			ErrorInfo = ErrorInfo();
			ErrorInfo = NStr("en='Cannot establish FTP connection with server %1:';ru='Не удалось установить FTP-соединение с сервером %1:';vi='Không thể thiết lập liên kết FTP với Server %1:'") + Chars.LF + "%2";
	...
	Else
		...
		Except
			ErrorInfo    = ErrorInfo();
			ErrorMessage = NStr("en='Cannot establish HTTP connection with the %1 server:';ru='Не удалось установить HTTP-соединение с сервером %1:';vi='Không thể thiết lập liên kết HTTP với Server %1:'") + Chars.LF + "%2";
	...
	Except
		ErrorInfo    = ErrorInfo();
		ErrorMessage = NStr("en='Cannot receive the file from server %1:';ru='Не удалось получить файл с сервера %1:';vi='Không thể nhận tệp từ Server %1:'") + Chars.LF + "%2";
		If RecordError Then
			WriteErrorInEventLogMonitor(
				StringFunctionsClientServer.SubstituteParametersInString(ErrorMessage, ServerName, 
				DetailErrorDescription(ErrorInfo)));
...
Function EventLogMonitorEvent() Export
	...
	Return NStr("en='Receive files from the Internet';ru='Получение файлов из Интернет';vi='Nhận tệp từ Internet'", CommonUseClientServer.MainLanguageCode());
