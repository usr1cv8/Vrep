		...
		If LibraryID = Metadata.Name Then
			...
			While Selection.Next() Do
				...
				If Not Selection.Updated Then
					...
					CommentTemplate = NStr("en='Configuration version was updated before updating a version of library %1';ru='Обновление версии конфигурации было выполнено до обновления версии библиотеки %1';vi='Đã thực hiện cập nhật phiên bản cấu hình trước khi cập nhật phiên bản thư viện %1'");
				...
				If DescriptionUpdateYourPlan = Undefined Then
					...
					CommentTemplate = NStr("en='Update plan of library %1 is not found';ru='Не найден план обновления библиотеки %1';vi='Không tìm thấy kế hoạch cập nhật thư viện %1'");
					TextOfComment = StringFunctionsClientServer.SubstituteParametersInString(CommentTemplate, Selection.Name);
					WriteLogEvent(
						InfobaseUpdate.EventLogMonitorEvent(),
						EventLogLevel.Error,
...
"план обновления библиотеки %1 Требуется план обновления на версию %2, найден план для обновления на версию %3';vi='Đã tìm thấy sai"
"kế hoạch cập nhật thư viện %1 Cần có kế hoạch cập nhật đến phiên bản %2, đã tìm thấy kế hoạch để cập nhật đến phiên bản %3'");
					TextOfComment = StringFunctionsClientServer.SubstituteParametersInString(CommentTemplate, Selection.Name);
					WriteLogEvent(
						InfobaseUpdate.EventLogMonitorEvent(),
						EventLogLevel.Error,
...
"Обратитесь к администратору.';vi='Chưa thực hiện phần cập nhật cơ sở thông tin chung."
...
Procedure ScheduleDataAreaUpdate(Val LockAreas = True, Val LockMessage = "") Export
	...
	If IsBlankString(LockMessage) Then
		...
		If IsBlankString(LockMessage) Then
			LockMessage = NStr("en='System is locked for update.';ru='Система заблокирована для выполнения обновления.';vi='Hệ thống bị phong tỏa để thực hiện cập nhật.'");
		EndIf;
	EndIf;
	...
	If RecordKey <> Undefined Then
		...
		Except
			WriteLogEvent(InfobaseUpdate.EventLogMonitorEvent() + ". " 
				+ NStr("en='Update data area';ru='Обновление области данных';vi='Cập nhật vùng dữ liệu'", Metadata.DefaultLanguage.LanguageCode),
				EventLogLevel.Error,,,
				DetailErrorDescription(ErrorInfo()));
			Raise(NStr("en='An error occurred when updating the data area. Writing of data area versions is locked.';ru='Ошибка обновления области данных. Запись версий области данных заблокирована.';vi='Lỗi cập nhật vùng dữ liệu. bản ghi phiên bản vùng dữ liệu bị phong tỏa.'"));
		EndTry;
	EndIf;
	Return RecordKey;
...
"функции ОбновлениеИнформационнойБазыСлужебныйПовтИсп.МинимальнаяВерсияИБ() недоступен из сеансов с установленным значением разделителей модели сервиса!';vi='Không thể"
"gọi ra hàm ОбновлениеИнформационнойБазыСлужебныйПовтИсп.МинимальнаяВерсияИБ() từ phiên làm việc với giá trị đường chia tách đã thiết lập trong mô hình dịch vụ đám mây!'");
	EndIf;
