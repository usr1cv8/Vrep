...
Function CurrencyDescription(Currency) Export
	
	If NOT ЗначениеЗаполнено(Currency) ИЛИ ТипЗнч(Currency)<>Тип("СправочникСсылка.Currencies") Then
		Return "";
	EndIf; 
	
	Return Currency.InWordParametersInEnglish;
...
//
Function GetCharCurrencyPresentation(Currency) Export
	
	If Not ValueIsFilled(Currency) Or TypeOf(Currency)<>Type("CatalogRef.Currencies") Then
		Return "";
	EndIf; 
	
//	Return Currency.SymbolicPresentation;
	Return Currency.Description;
	
EndFunction // ПолучитьВалютуУчета()
...
Function ExchangeWithBookkeepingConfigured() Export
	...
	|	1 AS Field1
	|FROM
	|	ExchangePlan.ExchangeCompanyManagementAccounting AS ExchangeCompanyManagementAccounting
	|WHERE
	|	ExchangeCompanyManagementAccounting.Ref <> &ThisNodeACC
	|	AND ExchangeCompanyManagementAccounting.Code <> """"";
	
	Query.SetParameter("ThisNodeACC", ExchangePlans.ExchangeCompanyManagementAccounting.ThisNode());
