...
Function PaymentBases(EnumKind, Period = Undefined) Export
	...
	Else
		PaymentBases.Add(UnfilledValue(), NStr("en='0 - value is not filled in';ru='0 - значение не заполняется';vi='0 - chưa điền giá trị'"));
	EndIf;
...
Function PaymentTypes(EnumKind, Period = Undefined) Export
	...
	Else
		PaymentTypes.Add(UnfilledValue(), NStr("en='0 - value is not filled in';ru='0 - значение не заполняется';vi='0 - chưa điền giá trị'"));
	EndIf;
...
//
Function CalculateVATAmount(Amount, AmountIncludesVAT, VATRate) Экспорт

	Если AmountIncludesVAT Тогда
		AmountWithoutVAT = 100 * Amount / (100 + VATRate);
		VATAmount = Amount - AmountWithoutVAT;
	Иначе
		AmountWithoutVAT = Amount;
	КонецЕсли;

	Если НЕ AmountIncludesVAT Тогда
		VATAmount = AmountWithoutVAT * VATRate / 100;
	КонецЕсли;
	
	Возврат VATAmount;

EndFunction // РассчитатьСуммуНДС()

// Выполняет расчет суммы НДС для строки раздела ГТД.
//
// Параметры:
//  ДанныеСтроки  - Структура - данные строки ГТД для расчета.
//  ДанныеОбъекта  - Структура - данные реквизитов шапки документа ГТД по импорту для расчета.
//
// Возвращаемое значение:
//  Число - полученная сумма НДС.
//
Function CalculateVATAmountCCD(Val RowData, Val ObjectData, Val NationalCurrency) Export 
	
	// Таможенная стоимость влияет только на расчет НДС первого раздела.
	FirstSection = RowData.LineNumber = 1;
	
	If Not ЗначениеЗаполнено(ObjectData.Currency) Then

		VatBase = 0;

	Else // Надо все пересчитать в валюту регл. учета.
		If FirstSection Then
			
			CustomsCostCur = WorkWithCurrencyRatesClientServer.RecalculateFromCurrencyToCurrency(RowData.CustomsCost,
									ObjectData.Currency, NationalCurrency,
									ObjectData.ExchangeRate, 1,
									ObjectData.Multiplicity, 1);
		...
		//DutyAmountCur = RowData.DutyAmount;
		

		VatBase = CustomsCostCur +  RowData.DutyAmount;;

	EndIf;

	Return SmallBusinessClientServer.CalculateVATAmount(VatBase, False, SmallBusinessReUse.GetVATRateValue(RowData.VATRate));
	
КонецФункции

Procedure DistributeVATAndFeeInSection(Object, SectionNumber) Export

	// Проверим, есть ли что распределять.
	SectionRow = Object.Sections.Получить(SectionNumber - 1);
	CustomCost = SectionRow.CustomsCost;
	FeeAmount        = SectionRow.DutyAmount;
	VATAmount            = SectionRow.VATAmount;

	RowArray  = Object.Inventory.НайтиСтроки(New Structure("SectionNumber", SectionNumber));
	DistributionBasis = New Array();

	TotalCost = 0;
	For Each ЭлементМассива In RowArray Do
		TotalCost = TotalCost + ЭлементМассива.InvoiceCost;
		DistributionBasis.Добавить(ЭлементМассива.InvoiceCost);
	EndDo;;
	...
	If TotalCost = 0 Then
		...
		If RowArray.Count() > 0 Then
			MessageText = НСтр("ru='Общая сумма фактурной стоимости раздела %1 нулевая! Распределение невозможно.';
				|en = 'Total amount of section invoice cost equal zero! Distribution impossible';
				|vi='Tổng giá trị hóa đơn của mục %1 bằng 0! Không thể phân bổ.'");
			MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessageText, SectionNumber);
			CommonUseClientServer.MessageToUser(MessageText,, "Inventory", "Object");
		EndIf;
	...
	If Not FeeAmount = 0 Then
		DistributedResult_Fee = CommonUseClientServer.DistributeAmountProportionallyToFactors(FeeAmount, DistributionBasis);
	EndIf;
	If Not VATAmount = 0 Then
		DistibutedResult_VAT = CommonUseClientServer.DistributeAmountProportionallyToFactors(VATAmount, DistributionBasis);
	EndIf;

	For RowIndex = 0 To RowArray.Количество()-1 Do
		If Not FeeAmount = 0 Then
			RowArray[RowIndex].DutyAmount = DistributedResult_Fee[RowIndex];
		Else
			RowArray[RowIndex].DutyAmount = 0;
		EndIf;
		If Not VATAmount = 0 Then
			RowArray[RowIndex].VATAmount = DistibutedResult_VAT[RowIndex];
		Else
			RowArray[RowIndex].VATAmount = 0;
		EndIf;
	EndDo;
