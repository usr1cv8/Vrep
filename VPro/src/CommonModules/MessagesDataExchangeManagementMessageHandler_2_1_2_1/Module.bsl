...
Procedure SetExchangeStep1(Message, Sender)
	...
	Try
		...
		If Not IsBlankString(ThisNodeCode)
			...
			MessageString = NStr("en='Predefined node code in the ""%1"" application does not match the expected one ""%2"". Exchange plan: %3';ru='Код предопределенного узла в этом приложении ""%1"" не соответствует ожидаемому ""%2"". План обмена: %3';vi='Mã nút định trước trong ứng dụng này ""%1"" không phù hợp với mã dự kiến ""%2"". Sơ đồ trao đổi: %3'");
		...
		FilterSettingsAtNode = XDTOSerializer.ReadXDTO(Body.FilterSettings);
		
		// {Handler: AtReceivingSenderData} Begin
		ExchangePlans[Body.ExchangePlan].OnSendersDataGet(FilterSettingsAtNode, False);
...
Function ExchangeCorrespondent(Val ExchangePlanName, Val Code)
	...
	If Not ValueIsFilled(Result) Then
		MessageString = NStr("en='Exchange plan node is not found; exchange plan name %1; node code %2';ru='Не найден узел плана обмена; имя плана обмена %1; код узла %2';vi='Không tìm thấy nút sơ đồ trao đổi; tên sơ đồ trao đổi %1; mã nút %2'");
