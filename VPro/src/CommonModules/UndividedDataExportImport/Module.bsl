...
Procedure BeforeExportType(Container, Serializer, MetadataObject, Cancel) Export
	...
	If Not CommonUseSTL.ThisIsReferenceData(MetadataObject) Then 
		Raise NStr("en='References can be replaced only in the reference data';ru='Замена ссылок доступна только в ссылочных данных';vi='Chỉ cho phép thay thế tham chiếu bằng dữ liệu tham chiếu'");
	EndIf;
...
Procedure AfterObjectExport(Container, ObjectExportManager, Serializer, Object, Artifacts) Export
	...
	If Container.AdditionalProperties.CommonDataRequireMatchingRefs.Find(MetadataObject) <> Undefined Then
		...
		If Not CommonUseSTL.ThisIsReferenceData(MetadataObject) Then 
			Raise NStr("en='You can substitute references only in reference data';ru='Подмена ссылок доступна только в ссылочных данных';vi='Thay thế tham chiếu truy cập chỉ trong dữ liệu có tham chiếu'");
		EndIf;
...
Procedure ReferencesUseControlUndividedDataInSeparated() Export
	...
	Except
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Errors found in the structure of metadata configuration: %1';ru='Обнаружены ошибки в структуре метаданных конфигурации: %1';vi='Đã tìm thấy lỗi trong cấu trúc metadata của cấu hình: %1'", Metadata.DefaultLanguage.LanguageCode),
			ErrorText
		);
...
Procedure CheckNaturalKeysTakesPresence(Val MetadataObject, Val NaturalKeyFields)
	...
	MessageText = ServiceTechnologyIntegrationWithSSL.SubstituteParametersInString(
...
"Проверьте обработчик ПриОпределенииТиповТребующихЗагрузкиВЛокальнуюВерсию.';vi='Đối với kiểu dữ liệu %1 chưa chỉ ra khóa tự nhiên để thay thế tham chiếu."
"Hãy kiểm tra hàm sự kiện WhenDetermineTypesRequireImportInLocalVersion'"),
...
Procedure UndividedDataReferencesControlOnExport(Container, Object, FieldsForUndividedDataReferencesControl)
	...
	For Each FieldForUndividedDataReferencesControl IN FieldsForUndividedDataReferencesControl Do
		...
		If ObjectNameStructure[0] <> FieldNameStructure[0] Or ObjectNameStructure[1] <> FieldNameStructure[1] Then
			...
			Raise NStr("en='Incorrect cache of shared data control on export.';ru='Некорректный кэш контроля неразделенных данных при выгрузке!';vi='Sai cache kiểm soát dữ liệu chưa phân tán khi kết xuất!'");
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Metadata object %1 is not supported.';ru='Объект метаданных %1 не поддерживается!';vi='Không hỗ trợ đối tượng metadata %1!'", Metadata.DefaultLanguage.LanguageCode),
				FullMetadataObjectName
			);
		EndIf;
...
"---------------------------------------------------------------------------------------------------------------------------"
"%5"
"---------------------------------------------------------------------------------------------------------------------------';vi='Đối tượng metadata %1 đã nằm trong danh sách đối tượng mà không cần so sánh tham chiếu khi kết xuất / kết nhập"
"dữ liệu (trong thủ tục định trước "
"ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхНеТребующихСопоставлениеСсылокПриЗагрузке(),"
"nhưng khi đó đối với đối tượng không đảm bảo yêu cầu không có các tham chiếu chưa được so sánh khi kết xuất."
""
"Đã tìm thấy tham chiếu chưa được so sánh khi kết xuất đối tượng %2, trong đó để làm giá trị mục tin %3"
"đã thiết lập tham chiếu đến đối tượng %1 mà không thể so sánh chính xác khi kết nhập dữ liệu."
"Cần xem lại lô-gic sử dụng đối tượng %1 và đảm bảo trong đó không có các tham chiếu chưa được so sánh"
"trong dữ liệu kết xuất."
""
"Thông tin chẩn đoán:"
"1. Trình tự hóa đối tượng kết xuất:"
"---------------------------------------------------------------------------------------------------------------------------"
"2. Trình tự hóa đối tượng tham chiếu chưa được so sánh"
"---------------------------------------------------------------------------------------------------------------------------"
"%5"
"---------------------------------------------------------------------------------------------------------------------------'");
