	...
	If PerformanceEstimationServerCallReUse.ExecutePerformanceMeasurements() Then
		...
		#If Client Then
			If Not ValueIsFilled(KeyOperation) Then
				Raise NStr("en='Key operation is not specified.';ru='Не указана ключевая операция.';vi='Chưa chỉ ra giao dịch chính.'");
			EndIf;
...
Procedure WriteToEventLogMonitor(EventName, Level, MessageText) Export
	...
	WriteLogEvent(EventName,
		Level,
		,
		NStr("en='Performance assessment';ru='Оценка производительности';vi='Đánh giá hiệu suất'"),
		MessageText);
