...
"функции ДополнительныеОтчетыИОбработкиВМоделиСервиса.ПоставляемаяОбработка() доступно только сеансов с установленным разделением данных!';vi='Chỉ có thể"
"sử dụng hàm ДополнительныеОтчетыИОбработкиВМоделиСервиса.ПоставляемаяОбработка() trong các phiên làm việc đã thiết lập chia tách dữ liệu!'");
...
"function usage is only available in sessions with the specified data separation.';ru='Использование"
"функции ДополнительныеОтчетыИОбработкиВМоделиСервиса.ИспользуемаяОбработка() доступно только сеансов с установленным разделением данных!';vi='Chỉ có thể"
"sử dụng hàm ДополнительныеОтчетыИОбработкиВМоделиСервиса.ИспользуемаяОбработка() trong phiên làm việc đã thiết lập đường chia tách dữ liệu!'");
...
Procedure SetIncludedProcessingInDataArea(Val InstallationDetails, Val QuickAccess, Val Tasks, Val Sections, Val CatalogsAndDocuments, Val SettingsLocationOfCommands, Val AdditionalReportVariants, Val Responsible) Export
	...
	WriteLogEvent(
		NStr("en='Supplied additional reports and data processors.Installation of supplied data processor in data area initiated';ru='Поставляемые дополнительные отчеты и обработки.Инициирована установка поставляемой обработки в область данных';vi='Báo cáo và bộ xử lý bổ sung cài đặt.Bắt đầu thiết lập bộ xử lý cài đặt vào vùng dữ liệu'",
	...
	Try
		...
		If CommonUse.RefExists(SuppliedDataProcessor) Then
			...
			WriteLogEvent(NStr("en='Additional supplied reports and data processors.Installation in the data area';ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных';vi='Báo cáo và bộ xử lý bổ sung cài đặt.Thiết lập vào vùng dữ liệu'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
		...
		Else
			...
			WriteLogEvent(NStr("en='Additional supplied reports and data processors. Installation to the data area is deferred';ru='Поставляемые дополнительные отчеты и обработки.Установка в область данных отложена';vi='Báo cáo và bộ xử lý bổ sung cài đặt."
"Hoãn thiết lập vào vùng dữ liệu'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
				String(InstallationDetails.ID),
...
Procedure DeleteComesFromDataProcessingAreas(Val SuppliedDataProcessor, Val IDOfUsedDataProcessor) Export
	...
	Try
		...
		Try
			...
			WriteLogEvent(NStr("en='Supplied additional reports and data processors.Delete from the data area';ru='Поставляемые дополнительные отчеты и обработки.Удаление из области данных';vi='Báo cáo và bộ xử lý bổ sung cài đặt.Xóa bỏ khỏi vùng dữ liệu'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
				SuppliedDataProcessor,
		...
		Except
			...
			WriteLogEvent(NStr("en='Supplied additional reports and data processors.Error deleting from the data area';ru='Поставляемые дополнительные отчеты и обработки.Ошибка удаления из область данных';vi='Báo cáo và bộ xử lý bổ sung cài đặt.Lỗi xóa khỏi vùng dữ liệu'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Error,
				,
				SuppliedDataProcessor,
...
Procedure ProcessAdditionalInformationProcessorSettingToDataAreaError(Val SuppliedDataProcessor, Val InstallationID, Val ErrorMessage) Export
	...
	WriteLogEvent(NStr("en='Additional supplied reports and data processors. An error occurred during installation to the data area';ru='Поставляемые дополнительные отчеты и обработки.Ошибка установки в область данных';vi='Báo cáo và bộ xử lý bổ sung cài đặt.Lỗi thiết lập vào vùng dữ liệu'",
		CommonUseClientServer.MainLanguageCode()),
		EventLogLevel.Error,
		,
		SuppliedDataProcessor,
...
Procedure OnEnableExternalProcessor(Val Ref, StandardProcessing, Result) Export
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		...
		If TypeOf(Ref) <> Type("CatalogRef.AdditionalReportsAndDataProcessors") Or Ref = Catalogs.AdditionalReportsAndDataProcessors.EmptyRef() Then
			Raise NStr("en='A non-existent additional data processor connection requested.';ru='Запрошено подключение несуществующей дополнительной обработки!';vi='Đã truy vấn kết nối bộ xử lý bổ sung không tồn tại!'");
		EndIf;
...
Procedure OnCreateExternalDataProcessor(Val Ref, StandardProcessing, Result) Export
	...
	If Not StandardProcessing Then
		...
		If DataProcessorName = Undefined Then
			Raise NStr("en='The creation is requested for object of a non-existent additional data processor.';ru='Запрошено создание объекта несуществующей дополнительной обработки!';vi='Đã truy vấn tạo đối tượng bộ xử lý bổ sung không tồn tại!'");
		EndIf;
...
Procedure UpdateSettingsUsedByDataProcessors(Val Ref) Export
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Additional data processor with %1 ID is not supplied.';ru='Дополнительная обработка с идентификатором %1 не является поставляемой!';vi='Bộ xử lý bổ sung có tên %1 không phải là bộ xử lý được cung cấp!'"),
			String(Ref.UUID()));
...
Procedure AvailableNewData(Val Handle, Import) Export
	...
	If Handle.DataType = ProvidedDataKindId() Then
		...
		Else
			...
			WriteLogEvent(
				NStr("en='Supplied additional reports and data processors.Supplied data processor import canceled';ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки отменена';vi='Báo cáo và bộ xử lý bổ sung cài đặt.Hủy bỏ bộ xử lý cài đặt'", 
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
				,
				NStr("en='The supplied processor is not compatible with this configuration';ru='Поставляемая обработка несовместима с данной конфигурацией';vi='Bộ xử lý cài đặt không tương ứng với cấu hình này'") + Chars.LF + Chars.CR + ProvidedDataProcessorDescription.Compatibility);
...
Procedure ProcessSuppliedAdditionalReportsAndDataProcessors(Handle, PathToFile)
	...
	WriteLogEvent(NStr("en='Supplied additional reports and data processors.Import supplied data processor';ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки';vi='Báo cáo và bộ xử lý bổ sung cài đặt.Kết nhập bộ xử lý cài đặt'", 
		CommonUseClientServer.MainLanguageCode()),
		EventLogLevel.Information,
		,
		,
		NStr("en='Supplied data processor import initiated';ru='Инициирована загрузка поставляемой обработки';vi='Đã khởi xướng kết nhập bộ xử lý được cung cấp'") + Chars.LF + Chars.CR + ProvidedDataProcessorDescription.Manifest);
	...
	If SuppliedDataProcessor.GUIDVersion <> ProvidedDataProcessorDescription.Version Then
		...
		WriteLogEvent(NStr("en='Supplied additional reports and data processors.Supplied data processor is imported';ru='Поставляемые дополнительные отчеты и обработки.Загрузка поставляемой обработки завершена';vi='Báo cáo và bộ xử lý bổ sung cài đặt.Đã hoàn tất kết nhập bộ xử lý cài đặt'",
			CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Information,
			,
			SuppliedDataProcessor.Ref,
			NStr("en='Supplied data processor import is completed.';ru='Завершена загрузка поставляемой обработки';vi='Kết thúc kết nhập bộ xử lý được cung cấp'") + Chars.LF + Chars.CR + ProvidedDataProcessorDescription.Manifest);
		...
		For Each InstallationOfDataProcessors IN UsedDataProcessors Do
			...
			WriteLogEvent(NStr("en='Supplied additional reports and data processors.Update of the supplied data processor settings is scheduled';ru='Поставляемые дополнительные отчеты и обработки.Запланирована актуализация настроек поставляемой обработки';vi='Báo cáo và bộ xử lý bổ sung cài đặt.Dự tính cập nhật tùy chỉnh bộ xử lý cài đặt.'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
				SuppliedDataProcessor.Ref,
				NStr("en='Data area';ru='Область данных:';vi='Vùng dữ liệu:'") + InstallationOfDataProcessors.DataArea);
		...
		For Each ItemInQueue IN InstallationsQueue Do
			...
			WriteLogEvent(
				NStr("en='Additional supplied reports and data processors. Deferred installation of the supplied data processor in the data area is scheduled';ru='Поставляемые дополнительные отчеты и обработки.Запланирована отложенная установка поставляемой обработки в область данных';vi='Báo cáo và bộ xử lý bổ sung cài đặt.Dự tính hoãn thiết lập bộ xử lý cài đặt vào vùng dữ liệu'",
				CommonUseClientServer.MainLanguageCode()),
				EventLogLevel.Information,
				,
				SuppliedDataProcessor.Ref,
				NStr("en='Data area';ru='Область данных:';vi='Vùng dữ liệu:'") + ItemInQueue.DataArea);
	...
	If CommonUseReUse.DataSeparationEnabled() Then
		...
		Else
			...
			If Not Constants.AdditionalReportsAndDataProcessorsIndependentUseSaaS.Get() Then
				...
				Raise NStr("en='This additional report cannot be used in the service.';ru='Этот дополнительный отчет или обработка не может быть использован в сервисе!';vi='Không thể sử dụng báo cáo hoặc bộ xử lý bổ sung này trong Service!'");
