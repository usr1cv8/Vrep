...
Procedure ExecuteConfigurationMethod(Val MethodName, Val Parameters = Undefined) Export
	...
	Except
		ErrorInfo = ErrorInfo();
		Raise NStr("en='An error occurred when calling the ExecuteConfigurationMethod procedure of the WorkInSafeMode common module.';ru='Ошибка при вызове процедуры ВыполнитьМетодКонфигурации общего модуля РаботаВБезопасномРежиме.';vi='Lỗi khi gọi ra thủ tục ExecuteConfigurationMethod của mô-đun chung WorkInSafeMode.'")
...
Procedure ExecuteObjectMethod(Val Object, Val MethodName, Val Parameters = Undefined) Export
	...
	Except
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Incorrect value of parameter MethodName (%1)';ru='Некорректное значение параметра ИмяМетода (%1)';vi='Sai giá trị tham số MethodName (%1)'"),
			MethodName);
	EndTry;
...
Procedure ValidateConfigurationMethodName(Val MethodName) Export
	...
	If NameParts.Count() <> 2 AND NameParts.Count() <> 3 Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Invalid format for parameter MethodName (%1)';ru='Неправильный формат параметра ИмяМетода (%1)';vi='Sai định dạng tham số MethodName (%1)'"),
			MethodName);
	EndIf;
...
"формат параметра MethodName (%1): Не найден общий модуль ""%2"".';vi='Sai"
"định dạng tham số MethodName (%1): Không tìm thấy mô-đun chung ""%2"".'"),
			MethodName,
			ObjectName);
	EndIf;
...
"формат параметра MethodName (%1): Не найден менеджер объекта ""%2"".';vi='Sai"
"định dạng tham số MethodName (%1): Không tìm thấy bộ quản lý đối tượng ""%2"".'"),
				MethodName,
				FullObjectName);
		EndIf;
	EndIf;
	...
	Except
		WriteLogEvent(NStr("en='Method is safely performed';ru='Безопасное выполнение метода';vi='Thực hiện phương thức bảo mật'", CommonUseClientServer.MainLanguageCode()),
...
"формат параметра ИмяМетода (%1): Имя метода ""%2"" не соответствует требованиям образования имен процедур и функций.';vi='Sai"
"định dạng tham số MethodName (%1): Tên phương thức ""%2"" không tương ứng với yêu cầu đặt tên gọi thủ tục và hàm.'"),
			MethodName,
			ObjectMethodName);
	EndTry;
...
""
"Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';vi='Không thể thực hiện các hàm xử lý thiết lập tham số phiên làm việc bởi nguyên nhân: thiếu hồ sơ bảo mật %1 trong cụm Server 1C:DOANH NGHIỆP, hoặc đã cấm sử dụng hồ sơ bảo mật này làm hồ sơ bảo mật trong chế độ an toàn."
""
"Để khôi phục khả năng làm việc của chương trình cần ngắt sử dụng hồ sơ bảo mật qua bảng điều khiển Cluster và thiết lập lại hồ sơ bảo mật bằng giao diện cấu hình (các hàm tương ứng nằm trong phần hành tùy chỉnh chương trình).'"),
					InfobaseProfile);
...
""
"Для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).';vi='Không thể thực hiện các hàm xử lý thiết lập tham số phiên làm việc bởi nguyên nhân: hồ sơ bảo mật %1 không chứa giấy phép thiết lập chế độ ưu tiên."
"Có thể, giấy phép đã được soạn qua bảng điều khiển Cluster."
""
"Để khôi phục khả năng làm việc của chương trình cần ngắt sử dụng hồ sơ bảo mật qua bảng điều khiển Cluster và tùy chỉnh lại"
"hồ sơ bảo mật của giao diện cấu hình (các lệnh tương ứng nằm trong phần hành tùy chỉnh chương trình).'"),
				InfobaseProfile);
...
""
"Возможно, для информационной базы через консоль кластера был установлен профиль безопасности, не допускающий выполнения внешних модулей без установки безопасного режима. В этом случае для восстановления работоспособности программы требуется отключить использование профиля безопасности через консоль кластера и заново настроить профили безопасности с помощью интерфейса конфигурации (соответствующие команды находятся в разделе настроек программы).При этом программа будет автоматически корректно настроена на использование совместно с включенными профилями безопасности.';vi='Không thể thực hiện các hàm xử lý thiết lập tham số phiên làm việc bởi nguyên nhân: %1."
""
"Có thể, đối với cơ sở thông tin qua bảng điểu khiển Cluster đã thiết lập hồ sơ bảo mật, không cho phép thực hiện mô-đun ngoài mà không thiết lập chế độ an toàn. Trong trường hợp này, để khôi phục khả năng làm việc của chương trình cần ngắt sử dụng hồ sơ bảo mật qua bảng điều khiển Cluster và tùy chỉnh lại hồ sơ bảo mật bằng giao diện cấu hình (các lệnh tương ứng nằm trong phần hành tùy chỉnh chương trình). Khi đó, chương trình sẽ tự động tùy chỉnh chính xác để sử dụng tương thích với hồ sơ bảo mật.'"),
...
Function ObjectManagerByName(Name)
	...
	Raise StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Cannot receive manager for object ""%1""';ru='Не удалось получить менеджер для объекта ""%1""';vi='Không thể nhận bộ quản lý đối với đối tượng ""%1""'"), Name);
...
Function PossibleToExecuteSessionParametersSetupHandlersWithoutSafeModeInstallation() Export
	...
	Except
		...
		WriteLogEventTemplate = NStr("en='An error occurred while setting session parameters: -------------------------------------------------------------------------------------------- %1 --------------------------------------------------------------------------------------------Application start will be unavailable.';ru='При установке параметров сеанса произошла ошибка: -------------------------------------------------------------------------------------------- %1 -------------------------------------------------------------------------------------------- Запуск программы будет невозможен.';vi='Khi thiết lập tham số phiên làm việc, đã xảy ra lỗi: -------------------------------------------------------------------------------------------- %1 -------------------------------------------------------------------------------------------- Không thể khởi động chương trình.'");
		...
		WriteLogEvent(
			NStr("en='Set session parameters';ru='Установка параметров сеанса';vi='Thiết lập tham số phiên làm việc'", CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,
			,
			,
			WriteLogEventText);
