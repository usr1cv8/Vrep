	...
	For Each ScriptItem IN ScriptApplicationPermissions Do
		...
		If ScriptItem.Operation = OperationKinds.Creating Then
			...
			If ClusterAdministrationClientServer.SecurityProfileExists(ClusterAdministrationParameters, ScriptItem.Profile) Then
				...
				CommonUseClientServer.MessageToUser(
					StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Security profile %1 already exists in the server cluster. Settings in the security profile will be replaced...';ru='Профиль безопасности %1 уже присутствует в кластере серверов. Настройки в профиле безопасности будут замещены...';vi='Có hồ sơ bảo mật %1 trong cụm Server. Các tùy chỉnh trong hồ sơ bảo mật sẽ được thay thế ...'"), ScriptItem.Profile));
		...
		ElsIf ScriptItem.Operation = OperationKinds.Delete Then
			...
			Else
				...
				CommonUseClientServer.MessageToUser(
					StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Security profile %1 does not exist in the server cluster. Security profile might have been deleted earlier...';ru='Профиль безопасности %1 отсутствует в кластере серверов. Возможно, профиль безопасности был удален ранее...';vi='Thiếu hồ sơ bảo mật %1 trong cụm Server. Có thể, hồ sơ bảo mật đã bị xóa trước đó ...'"), ScriptItem.Profile));
...
Procedure AfterCompletingPermissionsQueryForExternalResourcesUsing(Result, Status) Export
	...
	If Result = DialogReturnCode.OK Then
		...
		ShowUserNotification(NStr("en='Set permissions';ru='Настройка разрешений';vi='Tùy chỉnh cho phép'"),,
			NStr("en='Changes made to security profile settings in server cluster.';ru='Внесены изменения в настройки профилей безопасности в кластере серверов.';vi='Thay đổi trong tùy chỉnh hồ sơ an toàn trong cụm Server.'"));
