...
Function FullNameOfServiceUser(Val ID = Undefined) Export
	...
	Result = NStr("en='<Service user %1>';ru='<Служебный пользователь %1>';vi='<Người sử dụng dịch vụ>'");
		...
		If CommonUseReUse.CanUseSeparatedData() Then
			...
			If Not UserRegisteredAsUnseparated(UserID) Then
				...
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='User with ID %1 is not registered as an undivided one.';ru='Пользователь с идентификатором %1 не зарегистрирован в качестве неразделенного!';vi='Người sử dụng với Tên định danh %1 chưa đăng ký để phân tách!'"),
					String(UserID)
				);
...
Function ExceptionTextUnseparatedUsersWriteProhibited()
	...
	Return NStr("en='It is prohibited to write undivided users when the separator use is enabled.';ru='Запись неразделенных пользователей при включенном использовании разделителей запрещена.';vi='Cấm ghi lại người sử dụng chưa được chia tách khi bật sử dụng đường chia tách.'");
