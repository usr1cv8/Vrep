...
Function CheckCommandParameterType(Val Parameter, Val ExpectedType) Export
	...
	If Not Result Then
		ShowMessageBox(,NStr("en='Action cannot be executed for the selected item.';ru='Действие не может быть выполнено для выбранного элемента.';vi='Không thể thực hiện thao tác đối với phần tử đã chọn.'"));
	EndIf;
...
//    MessageText = NStr("en='Document printing requires the file system extension to be installed.';vi='Để in chứng từ, cần cài đặt hệ mở rộng của tệp.'");
...
//    MessageText = NStr("en='To print the document, install the file operation extension.';ru='Для печати документа необходимо установить расширение работы с файлами.';vi='Để in chứng từ, cần thiết lập phần mở rộng làm việc với tệp.'");
...
#If Not WebClient And Not MobileClient Then
	...
	If ReturnCode = Undefined Or ReturnCode > 0 Then
		...
		MessageText = NStr("en='An error occurred when registering component comcntr.';ru='Ошибка при регистрации компоненты comcntr.';vi='Lỗi khi ghi nhận cấu phần comcntr.'") + Chars.LF
			+ NStr("en='Regsvr32 error code:';ru='Код ошибки regsvr32:';vi='Mã lỗi regsvr32:'") + " " + ReturnCode;
			
		If ReturnCode = 5 Then
			MessageText = MessageText + " " + NStr("en='Insufficient access rights.';ru='Недостаточно прав доступа.';vi='Không đủ quyền truy cập.'");
		EndIf;
		
		EventLogMonitorClient.AddMessageForEventLogMonitor(
			NStr("en='Comcntr component registration';ru='Регистрация компоненты comcntr';vi='Ghi nhận cấu phần comcntr'", CommonUseClientServer.MainLanguageCode()), "Error", MessageText);
		...
		ShowMessageBox(,MessageText + Chars.LF + NStr("en='See details in log.';ru='Подробности см. в Журнале регистрации.';vi='Chi tiết xem trong Nhật ký sự kiện.'"));
...
"Перезапустить?';vi='Để kết thúc ghi nhận lại cấu phần comcntr, cần khởi động lại chương trình."
"Khởi động lại?'");
...
//
// Example: 
//  WarningText = NStr("en='Close wizard?';ru='Закрыть помощник?';vi='Đóng trợ giúp?'");
...
#If Not WebClient And Not MobileClient Then
	If TypeOf(Module) <> Type("CommonModule") Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Common module ""%1"" was not found.';ru='Общий модуль ""%1"" не найден.';vi='Chưa tìm thấy mô-đun chung ""%1"".'"), Name);
	EndIf;
#EndIf
...
//
Procedure ShowCommentEditingForm(Val MultilineText, Val OwnerForm, Val AttributeName = "Object.Comment", 
	...
	FormTitle = ?(Title <> Undefined, Title, NStr("en='Note';ru='Примечание';vi='Ghi chú'"));
...
Function FileOperationsExtensionConnected(SuggestionText = Undefined, WarningText = Undefined) Export
	...
	If Result = "NotAttached" Then
		...
		Else
			MessageText = NStr("en='Action is not available as file operation extension is not connected in Web client.';ru='Действие недоступно, так как не подключено расширение работы с файлами в Веб-клиенте.';vi='Thao tác không hợp lệ, bởi vì không kết nối phần mở rộng làm việc với tệp trong Web-client.'")
		EndIf;
	EndIf;
...
"Отменить и закрыть?';vi='Dữ liệu đã thay đổi, những thay đổi đã nhập sẽ bị hủy bỏ."
"Hủy bỏ và đóng?'"),
		WarningText);
	Result = DoQueryBox(QuestionText, QuestionDialogMode.YesNo, , DialogReturnCode.No, 
		NStr("en='Cancel changes';ru='Отменить изменения';vi='Hủy bỏ thay đổi'"));
...
Procedure OpenCommentEditForm(Val MultilineText, EditResult,
	...
	OpenMultilineTextEditForm(MultilineText, EditResult, Modified, 
		NStr("en='Note';ru='Примечание';vi='Ghi chú'"));
...
Procedure CheckFileOperationsExtensionConnectedEnd(ExtensionAttached, AdditionalParameters) Export
	...
	If IsBlankString(MessageText) Then
		MessageText = NStr("en='The action is not available as an extension for 1C:Enterprise web client is not installed.';ru='Действие недоступно, так как не установлено расширение для веб-клиента 1С:Предприятие.';vi='Thao tác không hợp lệ vì chưa thiết lập phần mở rộng đối với Web-client 1C:DOANH NGHIỆP.'")
	EndIf;
	ShowMessageBox(, MessageText);
...
Procedure ConfirmFormClosing() Export
	...
	If IsBlankString(Parameters.WarningText) Then
		QuestionText = NStr("en='Data was changed. Save the changes?';ru='Данные были изменены. Сохранить изменения?';vi='Đã thay đổi dữ liệu. Lưu các thay đổi?'");
	Else
