...
Procedure DeleteSignatureRow(DigitallySignedObject, RowIndex)
	...
	If DigitallySignedObject.DigitalSignatures.Count() < RowIndex + 1 Then
		Raise NStr("en='String with signature was not found.';ru='Строка с подписью не найдена.';vi='Dòng có chữ ký không tìm thấy.'");
	EndIf;
	...
	If Not Users.InfobaseUserWithFullAccess() Then 
		If TabularSectionRow.Signer <> Users.CurrentUser() Then
			Raise NStr("en='Insufficient rights to delete the signature.';ru='Недостаточно прав на удаление подписи.';vi='Không đủ quyền xóa bỏ chữ ký.'");
		EndIf;
	EndIf;
