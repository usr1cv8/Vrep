...
"создании каталога ""%1"": ""%2"".';vi='Lỗi khi"
"tạo thư mục ""%1"": ""%2"".'"),
					FullSubDirectory,
...
"""%1"" (%2 Мб) превышает максимально допустимый размер файла (%3 Мб).';vi='Kích cỡ tệp"
"""%1"" (%2 Mb) vượt quá kích cỡ tối đa được phép của tệp (%3 Mb).'"),
			File.Name,
...
"Обратитесь к администратору.';vi='Cấm kết nhập tệp có phần mở rộng ""%1""."
...
Function MessageStringAboutImpossibilityOfLockedFileSigning(FileRef = Undefined) Export
	...
	If FileRef = Undefined Then
		Return NStr("en='Cannot sign locked file.';ru='Нельзя подписать занятый файл.';vi='Không thể ký tệp đang bị chiếm.'");
	Else
		Return StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Cannot sign locked file: %1.';ru='Нельзя подписать занятый файл: %1.';vi='Không thể ký tệp đang bị chiếm: %1.'"),
			String(FileRef) );
	EndIf;
...
Function MessageStringAboutImpossibilityOfEncryptedFileSigning(FileRef = Undefined) Export
	...
	If FileRef = Undefined Then
		Return NStr("en='Cannot sign encrypted file.';ru='Нельзя подписать зашифрованный файл.';vi='Không thể ký tệp đang mã hóa.'");
	Else
		Return StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='Cannot sign encrypted file: %1.';ru='Нельзя подписать зашифрованный файл: %1.';vi='Không thể ký tệp đang mã hóa: %1'"),
						String(FileRef) );
	EndIf;
...
"Обратитесь к администратору.';vi='Lỗi"
"mở tệp: ""%1""."
""
"Không tìm thấy tệp trong kho chứa tệp."
"Có thể là tệp đã bị xóa bởi chương trình diệt virus."
"Hãy liên hệ với người quản trị.'"),
			FileName);
	Else
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
