		...
		While Selection.Next() Do 
			Try
				...
				If ValueIsFilled(Selection.Pattern)
					...
					MessagePattern = NStr("en='Job template of queue with ID %1 is not found';ru='На найден шаблон задания очереди с идентификатором %1';vi='Để tìm thấy khuôn in nhiệm vụ tiếp theo có tên %1'");
					MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, Selection.Pattern);
					WriteLogEvent(NStr("en='Job queue.Execution';ru='Очередь заданий.Выполнение';vi='Trình tự các nhiệm vụ.Thực hiện'", 
...
"обработчика ошибок:"
"%2 По причине: %3';vi='Lỗi khi thực hiện"
"hàm xử lý lỗi"
"Tên ảo phương thức: %1 Phương thức"
"của hàm xử lý lỗi:"
"%2 Nguyên nhân: %3'");
		...
		WriteLogEvent(
			NStr("en='Scheduled job queue. An error of the error handler occurred';ru='Очередь регламентных заданий.Ошибка обработчика ошибок';vi='Trình tự các nhiệm vụ thường kỳ.Lỗi hàm sự kiện lỗi'", 
				CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,
			,
			,
...
"Псевдоним"
"метода: %1 Уровень рекурсии: %2';vi='Đã thực hiện hàm xử lý bỏ nhiệm vụ."
"Tên ảo"
"phương thức: %1 Mức đệ quy: %2'");
			TextOfComment = StringFunctionsClientServer.SubstituteParametersInString(
				CommentTemplate,
				HandlerErrorsParameters.JobMethodName,
				RecursionCounter);
			...
			WriteLogEvent(
				NStr("en='Scheduled job queue. End jobs of error processing';ru='Очередь регламентных заданий.Снятие заданий обработки ошибок';vi='Trình tự các nhiệm vụ thường kỳ.Bỏ nhiệm vụ xử lý lỗi'",
					CommonUseClientServer.MainLanguageCode()),
...
Procedure ExecuteConfigurationMethod(MethodName, Parameters = Undefined)
	...
	If TransactionActive() Then
		...
		ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='There are active transactions before the handler %1 can start.';ru='Перед стартом выполнения обработчика %1 есть активные транзакции!';vi='Trước khi khởi động thực hiện hàm sự kiện %1 có các giao tác hiện hoạt!'"),
				MethodName);
			
		WriteLogEvent(NStr("en='Scheduled job queue.Fulfillment';ru='Очередь регламентных заданий.Выполнение';vi='Trình tự các nhiệm vụ thường kỳ.Thực hiện'", 
			CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,
			,
			,
	...
	Try
		...
		If TransactionActive() Then
			...
			MessagePattern = NStr("en='Transaction was not closed after the handler %1 finished';ru='По завершении выполнения обработчика %1 не была закрыта транзакция';vi='Sau khi hoàn tất thực hiện hàm sự kiện %1 chưa đóng giao tác'");
		...
		If Not(DelimiterSet) AND CommonUse.UseSessionSeparator() Then
			...
			ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Session separation was not disabled after the handler %1 finished.';ru='По завершении выполнения обработчика %1 не было выключено разделение сеанса!';vi='Khi kết thúc thực hiện hàm sự kiện %1 không ngắt chia tách phiên làm việc!'"),
				MethodName);
		...
		ElsIf DelimiterSet AND SeparatorValue <> SaaSOperations.SessionSeparatorValue() Then
			...
			ErrorMessageText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Session separator value was changed after the handler %1 finished.';ru='По завершении выполнения обработчика %1 было изменено значение разделителя сеанса!';vi='Khi kết thúc thực hiện hàm sự kiện %1 đã thay đổi giá trị đường chia tách phiên làm việc!'"),
				MethodName);
...
Function GetErrorHandlerParameters(Val Task,Val ErrorInfoExecutionJobs = Undefined)
	...
	If Result.ProcessorExist Then
		...
		If ErrorInfoExecutionJobs = Undefined Then
			...
			ErrorInfoExecutionJobs = ErrorInformationAssistant(NStr("en='Job was completed with an unknown error, it could have been caused by the process failure.';ru='Задание завершилось с неизвестной ошибкой, возможно вызванной падением рабочего процесса.';vi='Đã kết thúc nhiệm vụ có lỗi không xác định, có thể là do quá trình làm việc bị ngắt.'"));
...
Procedure ExecuteHandlerTasks(Pattern, Alias, Parameters)
	...
	If MethodName = Undefined Then
		MessagePattern = NStr("en='Method %1 cannot be called via the job queue.';ru='Метод %1 не разрешен к вызову через очередь заданий.';vi='Phương thức %1 không cho phép gọi ra qua trình tự nhiệm vụ.'");
	...
	Try
		...
		If CommonUseReUse.IsSeparatedConfiguration() AND CommonUseReUse.IsSeparatedMetadataObject(Task.Metadata().FullName(), CommonUseReUse.SupportDataSplitter()) Then
			...
			If ValueIsFilled(Task.Pattern)
				...
				MessagePattern = NStr("en='The %1 queue job template is not found';ru='На найден шаблон задания очереди %1';vi='Không tìm thấy khuôn mẫu đặt trình tự %1'");
				MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, Task.Pattern);
				
				WriteLogEvent(NStr("en='Job queue.Planning';ru='Очередь заданий.Планирование';vi='Trình tự các nhiệm vụ,Lập kế hoạch'", 
					CommonUseClientServer.MainLanguageCode()), 
					EventLogLevel.Error,
					,
					,
		...
		ElsIf Task.JobState = Enums.JobStates.Running Then
			...
			WriteExecutionControlEventLogMonitor(NStr("en='Scheduled job queue. Completed with errors';ru='Очередь регламентных заданий.Завершено с ошибками';vi='Trình tự các nhiệm vụ. Đã hoàn tất có lỗi'", 
				CommonUseClientServer.MainLanguageCode()), Selection.ID, 
				NStr("en='Active job was forcibly terminated';ru='Исполняющее задание было принудительно завершено';vi='Bắt buộc hoàn tất nhiệm vụ đang thực hiện'"));
			...
			Else // No schedule
				...
				If CommonUseReUse.IsSeparatedConfiguration() AND CommonUseReUse.IsSeparatedMetadataObject(
					...
					If ValueIsFilled(Task.Pattern) Then // Job by template without schedule.
						...
						MessagePattern = NStr("en='The schedule was not found for the %1 queue job template';ru='Для шаблон заданий очереди %1 не найдено расписание';vi='Đối với khuôn mẫu nhiệm vụ đặt trình tự %1, không tìm thấy lịch biểu'");
						MessageText = StringFunctionsClientServer.SubstituteParametersInString(MessagePattern, Task.Pattern);
						WriteLogEvent(NStr("en='Job queue.Planning';ru='Очередь заданий.Планирование';vi='Trình tự các nhiệm vụ,Lập kế hoạch'", 
							CommonUseClientServer.MainLanguageCode()), 
							EventLogLevel.Error,
...
Procedure ExecuteJobQueue(Val Ref, Val ActiveBackgroundJob, 
	...
	Try
		WriteExecutionControlEventLogMonitor(NStr("en='Scheduled job queue.Start';ru='Очередь регламентных заданий.Старт';vi='Trình tự các nhiệm vụ.Bắt đầu'", 
		...
		WriteExecutionControlEventLogMonitor(NStr("en='Scheduled job queue. Successfully completed';ru='Очередь регламентных заданий.Завершено успешно';vi='Trình tự các nhiệm vụ. Đã hoàn tất thành công'", 
			CommonUseClientServer.MainLanguageCode()), Ref);
