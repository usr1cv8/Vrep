...
Procedure OnTransferringToFolderExecution(TransportParameters, Attachments) Export
	...
	Comment = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Mailing reports %1 from %2';ru='Рассылка отчетов %1 от %2';vi='Gửi báo cáo %1 ngày %2'"),
...
Procedure OnFillingInPossibleRightsForObjectRightsSettings(PossibleRights) Export
	...
	Right.Name           = "Read";
	Right.Title     = NStr("en='Read';ru='Чтение';vi='Đọc'");
	Right.ToolTip     = NStr("en='Reading folders and files';ru='Чтение папок и файлов';vi='Đọc thư mục và tệp'");
	...
	Right.Title     = NStr("en='Folder change';ru='Изменение папок';vi='Thay đổi thư mục'");
	Right.ToolTip     = NStr("en='Addition, change"
"and deletion mark of files folders';ru='Добавление, изменение и пометка удаления папок файлов';vi='Thêm, thay đổi và đặt dấu xóa thư mục tệp'");
	...
	Right.Title     = NStr("en='File change';ru='Изменение файлов';vi='Tùy chỉnh tệp'");
	Right.ToolTip     = NStr("en='Change files in folder';ru='Изменение файлов в папке';vi='Thay đổi tệp trong thư mục'");
	// Rights required for this right.
	Right.RequiredRights.Add("Read");
	// Rights for standard templates of access restrictions.
	...
	Right.RightsOwner  = "Catalog.FileFolders";
	Right.Name           = "FilesAdd";
	Right.Title     = NStr("en='Add files';ru='Добавление файлов';vi='Thêm tệp'");
	Right.ToolTip     = NStr("en='Add files to folder';ru='Добавление файлов в папку';vi='Thêm tệp vào thư mục'");
	// Rights required for this right.
	...
	Right.Title     = NStr("en='Deletion mark';ru='ПометкаУдаления';vi='Đặt dấu xóa'");
	Right.ToolTip     = NStr("en='File deletion mark in the folder';ru='Пометка удаления файлов в папке';vi='Đặt dấu xóa tệp trong thư mục'");
	// Rights required for this right.
	Right.RequiredRights.Add("FilesUpdate");
	...
	Right.Title     = NStr("en='Right management';ru='Управление правами';vi='Quản lý quyền'");
	Right.ToolTip     = NStr("en='Folder right management';ru='Управление правами папки';vi='Quản lý quyền thư mục'");
	// Rights required for this right.
	Right.RequiredRights.Add("Read");
...
Procedure AtFillingToDoList(CurrentWorks) Export
	...
	For Each Section IN Sections Do
		...
		Work.Presentation  = NStr("en='Files being edited';ru='Редактируемые файлы';vi='Tệp đang soạn'");
...
Function GetEncodingsList() Export
	...
	EncodingsList.Add("ibm852",       NStr("en='IBM852 (Central European DOS)';ru='IBM852 (Центральноевропейская DOS)';vi='IBM852 (DOS Trung Âu)'"));
	EncodingsList.Add("ibm866",       NStr("en='IBM866 (Cyrillic DOS)';ru='IBM866 (Кириллица DOS)';vi='IBM866 (DOS Cyrillic)'"));
	...
	EncodingsList.Add("iso-8859-15",  NStr("en='ISO-8859-15 (Latin 9 ISO)';ru='ISO-8859-15 (Латиница 9 ISO)';vi='ISO-8859-15 (9 ISO Latin)'"));
...
Function EventLogMonitorForExchange() 
	...
	Return NStr("en='Files.Cannot send the file during data exchange';ru='Файлы.Не удалось отправить файл при обмене данными';vi='Tệp.Không thể gửi tệp khi trao đổi dữ liệu'", CommonUseClientServer.MainLanguageCode());
