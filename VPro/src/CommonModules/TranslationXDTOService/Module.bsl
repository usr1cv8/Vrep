...
Function ExecuteBroadcast(Val InitialObject, Val InitialVersionsDetails, Val DetailsOfResultingVersions) Export
	...
	If TransmissionChainOfInterface = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Handler of translation from version %1 to version %2 is not registered in the configuration.';ru='В конфигурации не зарегистрирован обработчик трансляции из версии %1 в версию %2!';vi='Trong cấu hình không ghi nhận hàm chuyển đổi từ phiên bản %1 sang phiên bản %2!'"),
...
Function TransmitObject(Val Object, Val RulesBroadcastInterfaces)
	...
	If TransmissionChainOfInterface = Undefined Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Cannot define a translation handler for package {%1}, cannot execute the standard translation to process this property';ru='Не удалось определить обработчик трансляции для пакета {%1}, невозможно выполнение стандартной трансляции для обработки данного свойства';vi='Không thể xác định hàm sự kiện chuyển đổi cho gói {%1}, không thể thực hiện chuyển đổi chuẩn để xử lý thuộc tính này'"), 
			SourceObjectPackage);
	EndIf;
...
Function StandardProcessing(Val Object, Val PackageResultObject, Val RulesBroadcastInterfaces)
	...
	Else
		...
		If ResultedObjectType = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Cannot execute the standard translation processing of type %1 to package %2: type %1 does not exist in package %2.';ru='Не удалось выполнить стандартную обработку трансляции типа %1 в пакет %2: тип %1 не существует в пакете %2!';vi='Không thể thực hiện bộ xử lý chuyển đổi chuẩn kiểu %1 sang gói %2: không tồn tại kiểu %1 trong gói %2!'"),
	...
	For Each Property IN ResultedObjectType.Properties Do
		...
		If PropertyOfOriginal = Undefined Then
			...
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Cannot execute standard processing of conversion type %1 to type %2: property %3 is not defined for type %1.';ru='Не удалось выполнить стандартную обработку конвертации типа %1 в тип %2: свойство %3 не определено для типа %1!';vi='Không thể thực hiện bộ xử lý chuyển đổi chuẩn kiểu %1 sang kiểu %2: chưa xác định thuộc tính %3 cho kiểu %1!'"),
				"{" + SourceObjectType.NamespaceURI + "}" + SourceObjectType.Name,
	...
	For Each Property IN SourceObjectType.Properties Do
		...
		If TranslatedProperty = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Cannot execute standard processing of conversion type %1 to type %2: property %3 is not defined for type %2.';ru='Не удалось выполнить стандартную обработку конвертации типа %1 в тип %2: свойство %3 не определено для типа %2!';vi='Không thể thực hiện bộ xử lý chuyển đổi chuẩn kiểu %1 sang kiểu %2: chưa xác định thuộc tính %3 cho kiểu %2!'"),
				"{" + SourceObjectType.NamespaceURI + "}" + SourceObjectType.Name,
				"{" + ResultedObjectType.NamespaceURI + "}" + ResultedObjectType.Name,
				Property.LocalName);
		EndIf;
