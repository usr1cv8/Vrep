...
"команды ""%1"" в браузере Google Chrome не поддерживается.';vi='Không hỗ trợ"
"thực hiện lệnh ""%1"" trên trình duyệt Google Chrome.'");
	Else
		WarningText = NStr("en='For execution of command"
" ""%1"" you should install the extension for 1C: Enterprise web client.';ru='Для выполнения команды ""%1"" необходимо установить расширение для веб-клиента 1С:Предприятие.';vi='Để thực hiện lệnh ""%1"", cần cài đặt phần mở rộng đối với Web-client 1C:DOANH NGHIỆP.'");
	EndIf;
...
Function GetImportedFilesList() Export
	...
	FileOpeningDialog.Filter             = NStr("en='All files(*.*)|*.*';ru='Все файлы(*.*)|*.*';vi='Tất cả tệp(*.*)|*.*'");
	FileOpeningDialog.Multiselect = True;
	FileOpeningDialog.Title          = NStr("en='Select files';ru='Выбрать файлы';vi='Chọn tệp'");
...
Procedure CorrectFileName(FileName, DeleteIncorrectSymbols = False) Export
	...
	ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='File name cannot contain the following characters: %1';ru='В имени файла не должно быть следующих символов: %1';vi='Trong tên tệp không có các ký tự sau: %1'"), StrException);
...
"пользователя по причине: %1';vi='Không thể nhận thư mục làm việc trong dữ liệu của"
"người sử dụng do nguyên nhân: %1'"), BriefErrorDescription(ErrorInfo)));
		...
		If Parameters.SizeInFileStorage <> 0
		   AND Parameters.SizeInFileStorage <> Parameters.SizeInWorkingDirectory Then
			...
			Parameters.Insert("Title",
				NStr("en='File size is different';ru='Размер файла отличается';vi='Dung lượng tệp là khác nhau'"));
...
"заменить им существующий или открыть существующий без обновления?';vi='Kích cỡ tệp trong thư mục làm việc và trong kho chứa tệp là khác nhau."
""
"Lấy ra tệp từ kho chứa tệp và"
"thay thế tệp hiện tại hoặc mở ra tệp hiện tại mà không cập nhật?'"));
		Else
	...
	ElsIf Parameters.UniversalModificationDateInWorkingDirectory
		...
		If FileData.InWorkingDirectoryForRead = False Then
			...
			Parameters.Insert("Title", NStr("en='A new file is in the file storage';ru='В хранилище файлов новый файл';vi='Trong kho chứa tệp tệp mới'"));
...
""
"Взять файл из хранилища файлов и"
"заменить им существующий или открыть существующий без обновления?';vi='Tệp trong kho chứa tệp được đánh dấu"
"như là đang bị chiếm để soạn, có ngày thay đổi muộn (mới) hơn so với trong thư mục làm việc."
""
"Lấy ra tệp từ kho chứa tệp và"
"thay thế tệp hiện tại hoặc mở ra tệp hiện tại mà không cập nhật?'"));
		Else
	...
	ElsIf Parameters.UniversalModificationDateInWorkingDirectory
		...
		Else
			...
			Parameters.Insert("Title", NStr("en='New file in the working directory';ru='В рабочем каталоге новый файл';vi='Trong thư mục làm việc tệp mới'"));
...
"заменить его на файл из хранилища файлов c потерей изменений и открыть?';vi='Tệp trong thư mục làm việc có"
"ngày thay đổi muộn hơn (mới hơn) so với trong kho chứa tệp. Có thể là tệp đã bị thay đổi."
""
"Mở tệp hiện tại hoặc"
"thay thế bằng tệp từ kho chứa tệp sẽ làm mất những thay đổi và mở ra?'"));
		EndIf;
	EndIf;
...
Procedure CheckSignaturesAfterRowCheck(Result, AdditionalParameters) Export
	...
	If Result = True Then
		SignatureRow.Status  = NStr("en='Correct';ru='Исправить';vi='Sửa chữa'");
		SignatureRow.Wrong = False;
	Else
		SignatureRow.Status  = NStr("en='Incorrect';ru='Неверна';vi='Sai'") + ". " + String(Result);
...
Procedure HandleFileAfterReceivingFiles(ReceivedFiles, Context) Export
	...
	If TypeOf(ReceivedFiles) <> Type("Array") Or ReceivedFiles.Count() = 0 Then
		HandleFileAfterError(NStr("en='File receipt was canceled.';ru='Получение файла было отменено.';vi='Đã hủy nhận tệp.'"), , Context);
		Return;
	EndIf;
...
Procedure HandleFileAfterPlacingFiles(PlacedFiles, Context) Export
	...
	If TypeOf(PlacedFiles) <> Type("Array") Or PlacedFiles.Count() = 0 Then
		HandleFileAfterError(NStr("en='File placement was canceled.';ru='Помещение файла было отменено.';vi='Đã hủy xếp lại tệp.'"), , Context);
		Return;
	EndIf;
