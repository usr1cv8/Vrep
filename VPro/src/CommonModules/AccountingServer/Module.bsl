...
Procedure FillChartOfAccountsFromTemplate()
	...
	TestValue2 = TrimAll(Template.Area(CurString, 11, CurString, 11).Text);
	...
	While GoOn Do
		...
		For Each KeyAndValue in Data Do
			...
			ElsIf FieldName = "MethodOfDistribution" Then
				...
				EndIf;
			ElsIf FieldName = "ClosingAccount" Then
				If ValueIsFilled(Value) Then
					Data[FieldName]	= ChartOfAccountManager[Value];
				Else
					Data[FieldName]	= ChartOfAccountManager.EmptyRef();
				EndIf;
