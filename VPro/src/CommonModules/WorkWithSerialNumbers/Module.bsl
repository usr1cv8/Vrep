		...
		For Each StringInventory In Inventory Do
			...
			If StringInventory.ProductsAndServices.UseSerialNumbers Then
				...
				If FilterSerialNumbers.Count() <> RowInventoryQuantity Then
					...
					MessageText = NStr("en='The quantity of serial numbers in tabular section %TSName% differs from the quantity of units in the line %Number%.';ru='Число серийных номеров табличной части %TSName% отличается от количества единиц в строке %Number%.';vi='Số lượng số sê-ri trong phần bảng %TSName% khác với số lượng đơn vị tính trong dòng %Number%.'");
...
Procedure ExecuteDuplicatesControl(Cancel, Val Inventory, Val SerialNumbers, Val incForm, FieldNameConnectionKey)
	...
	If NOT QueryResult.IsEmpty() Then
		...
		While QueryResultSelection.Next() Do
			MessageText = NStr("en='Serial number ""%SerialNumber%"", specified in line %LineNumber%, has already been used.';ru='Серийный номер ""%СерийныйНомер%"", указанный в строке %НомерСтроки%, указан повторно.';vi='Số sê-ri ""%SerialNumber%"", đã chỉ ra tại dòng %LineNumber%, bị lặp lại.'"
			);
	...
	If UseSerialNumbersBalance() = True Then
		...
		If Object.ProductsAndServices.UseSerialNumbers Then
			...
			If Object.SerialNumbers.Count() <> ObjectQuantity Then
				MessageText = NStr("en='The quantity of serial numbers differs from the quantity in the document.';ru='Число серийных номеров отличается от количества в документе.';vi='Số lượng số sê-ri khác với số lượng trong chứng từ.'");
				MessageText = MessageText + NStr("en='Serial numbers - %QuantityOfNumbers%, need %QuantityInRow%';ru=' Серийных номеров - %QuantityOfNumbers%, нужно %QuantityInRow%';vi=' Số sê-ri - %QuantityOfNumbers%, cần %QuantityInRow%'");
