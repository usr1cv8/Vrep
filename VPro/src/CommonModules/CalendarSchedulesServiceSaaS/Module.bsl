...
Function StartElement(Val XMLReader, Val Name)
	...
	If XMLReader.NodeType <> XMLNodeType.StartElement Or XMLReader.Name <> Name Then
		WriteLogEvent(NStr("en='Supplied data.Calendar schedules';ru='Поставляемые данные.Календарные графики';vi='Dữ liệu cài đặt.Lịch biểu'", 
			Metadata.DefaultLanguage.LanguageCode), EventLogLevel.Error,
			,, StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Incorrect format of the data file. Start of item %1 is expected';ru='Неверный формат файла данных. Ожидается начало элемента %1';vi='Sai định dạng tệp dữ liệu. Dự tính bắt đầu phần tử %1'"), Name));
		Return False;
	EndIf;
...
Function EndElement(Val XMLReader, Val Name)
	...
	If XMLReader.NodeType <> XMLNodeType.EndElement Or XMLReader.Name <> Name Then
		WriteLogEvent(NStr("en='Supplied data.Calendar schedules';ru='Поставляемые данные.Календарные графики';vi='Dữ liệu cài đặt.Lịch biểu'", 
			Metadata.DefaultLanguage.LanguageCode), EventLogLevel.Error,
			,, StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Incorrect format of the data file. End of item %1 is expected';ru='Неверный формат файла данных. Ожидается конец элемента %1';vi='Sai định dạng tệp dữ liệu. Dự tính kết thúc phần tử %1'"), Name));
		Return False;
	EndIf;
