...
Function SendSMS(RecipientNumbers, Val Text, SenderName = "", TranslateToTranslit = False) Export
	...
	If Not SMSSendSettingFinished() Then
		Result.ErrorDescription = NStr("en='Incorrect settings of the provider for SMS sending.';ru='Неверно заданы настройки провайдера для отправки SMS.';vi='Đưa sai tùy chỉnh người cung cấp để gửi SMS.'");
		Return Result;
	EndIf;
