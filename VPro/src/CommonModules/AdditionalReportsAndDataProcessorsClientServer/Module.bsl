...
Function ChooserAndSaveDialog() Export
	...
	Filter = NStr("en='External reports and data processors (*.%1, *.%2)|*.%1;*.%2|External reports (*.%1)|*.%1|External data processors (*.%2)|*.%2';ru='Внешние отчеты и обработки (*.%1, *.%2)|*.%1;*.%2|Внешние отчеты (*.%1)|*.%1|Внешние обработки (*.%2)|*.%2';vi='Báo cáo và bộ xử lý ngoài (*.%1, *.%2)|*.%1;*.%2|Báo cáo ngoài (*.%1)|*.%1|Bộ xử lý ngoài (*.%2)|*.%2'");
...
Function SubsystemDescription(LanguageCode) Export
	...
	Return NStr("en='Additional reports and data processors';ru='Дополнительные отчеты и обработки';vi='Báo cáo và bộ xử lý bổ sung'", ?(LanguageCode = Undefined, CommonUseClientServer.MainLanguageCode(), LanguageCode));
