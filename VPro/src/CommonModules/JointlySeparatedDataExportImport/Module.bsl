...
"ВыгрузкаЗагрузкаСовиестноРазделенныхДаннных.ЗаполнитьКэшСовместноРазделенныхОбъектов()';vi='Đối tượng metadata %1 không thể là hàm sự kiện JointlySeparatedDataExport.BeforeObjectExport() "
"vì không có các đối tượng đồng tách trong cache."
"Nếu sau khi soạn cấu trúc metadata của cấu hình cache chưa được cập nhật - cần thực hiện cập nhật cache của đối tượng đồng tách"
"bằng cách gọi ra phương thức JointlySeparatedDataExportImport.FillCacheJointlySeparatedObjects()'", Metadata.DefaultLanguage.LanguageCode),
...
"Если после редактирования структуры метаданных конфигурации кэш не обновлялся - необходимо выполнять"
"обновление кэша совместно-разделенных объектов с помощью вызова метода"
"ВыгрузкаЗагрузкаСовиестноРазделенныхДаннных.ЗаполнитьКэшСовместноРазделенныхОбъектов()';vi='Đối tượng metadata %1 không thể là hàm sự kiện ointlySeparatedDataExport.BeforeObjectImport(),"
"vì không có trong cache của đối tượng đồng tách."
"Nếu sau khi soạn cấu trúc metadata của cấu hình, chưa cập nhật cache - cần thực hiện cập nhật cache"
"của các đối tượng đồng tách bằng cách gọi ra phương thức"
"JointlySeparatedDataExportImport.FillCacheJointlySeparatedObjects()'", Metadata.DefaultLanguage.LanguageCode),
			Object.Metadata().FullName()
		);
