...
Procedure OpenDataForViewing(CurrentData) Export
	...
	If CurrentData = Undefined Or CurrentData.Data = Undefined Then
		ShowMessageBox(, NStr("en='This event log entry is not related to data (see the ""Data"" column)';ru='Эта запись журнала регистрации не связана с данными (см. колонку ""Данные"")';vi='Bản ghi của nhật ký sự kiện không liên quan đến dữ liệu (xem cột ""Dữ liệu"")'"));
		Return;
	EndIf;
...
"%1';vi='Bản ghi của nhật ký sự kiện không liên quan đến dữ liệu, nhưng không thể hiển thị chúng."
"%1'");
		If CurrentData.Event = "_$Data$_.Delete" Then 
			...
			WarningText =
					StringFunctionsClientServer.SubstituteParametersInString(
						WarningText,
						NStr("en='Data is deleted from infobase';ru='Данные удалены из информационной базы';vi='Dữ liệu bị xóa khỏi cơ sở thông tin'"));
		Else
			WarningText =
				StringFunctionsClientServer.SubstituteParametersInString(
						WarningText,
						NStr("en='Maybe the data is deleted from the infobase.';ru='Возможно, данные удалены из информационной базы';vi='Có thể. dữ liệu bị xóa từ cơ sở thông tin'"));
		EndIf;
