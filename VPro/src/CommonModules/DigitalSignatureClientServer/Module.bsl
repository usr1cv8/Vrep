...
Function CertificatePresentation(Certificate, Patronymic = False, ValidityPeriod = True) Export
	...
	Return SubjectPresentation(Certificate, Patronymic) + ", "
		+ StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='until %1';ru='до %1';vi='đến %1'"), Format(Certificate.EndDate, "DF=MM.yyyy"));
...
Function FillCertificateDataDescription(Table, Certificate) Export
	...
	If Certificate.UseToSign AND Certificate.UseForEncryption Then
		Purpose = NStr("en='Data signing, Data encryption';ru='Подписание данных, Шифрование данных';vi='Ký tên dữ liệu, Mã hóa dữ liệu'");
		
	ElsIf Certificate.UseToSign Then
		Purpose = NStr("en='Data signing';ru='Подписание данных';vi='Ký dữ liệu'");
	Else
		Purpose = NStr("en='Data encryption';ru='Шифрование данных';vi='Mã hóa dữ liệu'");
	EndIf;
	
	Table.Clear();
	String = Table.Add();
	String.Property = NStr("en='Issued to:';ru='Кому выдан:';vi='Cấp cho ai:'");
	String.Value = CertificateStructure.IssuedToWhom;
	
	String = Table.Add();
	String.Property = NStr("en='Issued by:';ru='Кем выдан:';vi='Cơ quan cấp:'");
	String.Value = CertificateStructure.WhoIssued;
	
	String = Table.Add();
	String.Property = NStr("en='Valid until:';ru='Действителен до:';vi='Có hiệu lực đến:'");
	String.Value = Format(CertificateStructure.ValidUntil, "DF=dd.MM.yyyy");
	
	String = Table.Add();
	String.Property = NStr("en='Assignment:';ru='Назначение:';vi='Mục đích:'");
