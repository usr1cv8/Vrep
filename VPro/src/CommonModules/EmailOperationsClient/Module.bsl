...
"Обратитесь к администратору.';vi='Để gửi E-mail cần tùy chỉnh trương mục E-mail."
...
Procedure CheckAccount(Val UserAccount) Export
	...
	Status(NStr("en='Check account';ru='Проверка учетной записи';vi='Kiểm tra trương mục email'"),,NStr("en='Checking the account. Please wait...';ru='Выполняется проверка учетной записи. Пожалуйста, подождите..';vi='Đang kiểm tra trương mục E-mail. Vui lòng chờ trong giây lát...'"));
...
"записи завершилась с ошибками: %1';vi='Việc kiểm tra tham số trương mục"
"E-mail đã kết thúc bị lỗi: %1'"), ErrorInfo ),,
			NStr("en='Check account';ru='Проверка учетной записи';vi='Kiểm tra trương mục email'"));
	Else
		ShowMessageBox(ResultHandler, StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Check account';ru='Проверка учетной записи';vi='Kiểm tra trương mục email'"),
			AdditionalMessage),,
