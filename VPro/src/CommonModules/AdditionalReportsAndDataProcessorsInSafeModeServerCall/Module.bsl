...
"в безопасном режиме должны вызываться только из сценария или из контекста клиентского приложения!';vi='Đã gọi ra sai hàm của mô-đun chung AdditionalReportsAndDataProcessorsInSafeModeServerCall!"
"Các hàm đã kết xuất của mô-đun này để sử dụng"
"ở chế độ an toàn cần được gọi ra chỉ từ kịch bản hoặc từ ngữ cảnh của ứng dụng Client!'");
