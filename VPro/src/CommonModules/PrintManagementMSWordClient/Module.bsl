...
#EndIf
	...
	Except
		...
		EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(), "Error",
			DetailErrorDescription(ErrorInfo()),,True);
		Raise(NStr("en='An error occurred when opening a template file.';ru='Ошибка при открытии файла шаблона.';vi='Lỗi khi mở tệp khuôn mẫu.'") + Chars.LF 
...
"Для вывода печатных форм в формате Microsoft Word требуется, чтобы на компьютере был установлен пакет Microsoft Office. %2';vi='Không thể tạo mẫu in: %1."
"Để đưa ra mẫu in với định dạng Microsoft Word cần thiết trên máy tính đã cài đặt gói Microsoft Office. %2'"),
