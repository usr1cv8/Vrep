						...
						If Selection2.Next() Then
							...
							If CorrespondentEndPoint = Undefined Then
								...
								Raise StringFunctionsClientServer.SubstituteParametersInString(
									NStr("en='Correspondent endpoint for the ""%1"" address is not determined.';ru='Не определена конечная точка корреспондента для адреса ""%1"".';vi='Chưa xác định điểm cuối đối ứng đối với địa chỉ ""%1"".'"),
									Selection2.Address);
							EndIf;
...
Function InformationExchangeDirectoryRelative(Val Folder)
	...
	If IsBlankString(Folder) Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Invalid format of the ""%1"" data exchange directory.';ru='Неправильный формат каталога обмена информацией ""%1"".';vi='Sai định dạng thư mục trao đổi thông tin ""%1"".'"),
			Folder);
	EndIf;
...
Procedure OnDeleteOutdatedMessagesExchange() Export
	...
	While Selection.Next() Do
		...
		If MessageFile.Exist() Then
			...
			Except
				WriteLogEvent(NStr("en='Data exchange description';ru='Обмен данными описание';vi='Trao đổi dữ liệu mô tả'", CommonUseClientServer.MainLanguageCode()),
...
Procedure ExecuteDataExchangeScenarioActionInFirstInfobase(ScenarioRowIndex, DataExchangeScenario) Export
	...
	If ScenarioString.InfobaseNumber = 1 Then
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='The ""%1"" unknown action on data exchange between data areas.';ru='Неизвестное действие ""%1"" при обмене данными между областями данных.';vi='Thao tác không xác định ""%1"" khi trao đổi dữ liệu giữa các vùng dữ liệu.'"),
...
Function GeneratePredefinedNodeDescription() Export
	...
	Return ?(IsBlankString(ApplicationName), NStr("en='online application';ru='приложение в Интернете';vi='ứng dụng trên Internet'"), ApplicationName);
EndFunction
...
Function CorrespondentEndPoint(Val Correspondent) Export
	...
	If QueryResult.IsEmpty() Then
		MessageString = NStr("en='The correspondent''s endpoint is not set for ""%1"".';ru='Не назначена конечная точка корреспондента для ""%1"".';vi='Chưa xác định điểm cuối đối ứng đối với ""%1"".'");
...
Procedure AvailableNewData(Val Handle, Import) Export
	...
	If Handle.DataType = ProvidedDataKindId() Then
		...
		Else
			...
			MessageText = NStr("en='Supplied exchange rules are not suitable for the current configuration and are intended for exchange plan %1 of configuration %2 of version %3';ru='Поставляемые правила обмена не подходят для текущей конфигурации и предназначены для плана обмена %1 конфигурации %2 версии %3';vi='Các quy tắc trao đổi được cài đặt kèm theo không phù hợp cho cấu hình hiện tại và được dùng cho sơ đồ trao đổi %1 cấu hình %2 phiên bản %3'");
			...
			WriteLogEvent(NStr("en='Supplied data exchange rules. Import of the supplied rules is canceled';ru='Поставляемые правила обмена данными.Загрузка поставляемых правил отменена';vi='Các quy tắc cài đặt trao đổi dữ liệu. Kết nhập các tệp cài đặt đã bị hủy bỏ'",
...
Procedure CreateExchangeSetting(
	...
	Try
		...
		If IsBlankString(ThisNodeCode) Then
			...
			MessageString = NStr("en='Code is not specified for the  ""%1"" predefined exchange node.';ru='Для предопределенного узла плана обмена ""%1"" не задан код.';vi='Đối với nút định trước của sơ đồ trao đổi ""%1"" chưa xác định mã.'");
		...
		If IsBlankString(GetFunctionalOption("InfobasePrefix")) Then
			...
			If IsBlankString(Prefix) Then
				Raise NStr("en='Prefix for this application is not set up in the service manager.';ru='В Менеджере сервиса не установлен префикс для этого приложения.';vi='Trong Quản lý dịch vụ chưa thiết lập tiền tố cho ứng dụng này.'");
			EndIf;
...
"Назначенное"
"значение ""%1"" Фактическое значение ""%2"".';vi='Lỗi ấn định mã nút thứ hai."
"Giá trị"
"đã ấn định ""%1"" Giá trị thực tế ""%2"".'");
		...
		If TransportSettings.ExchangeMessageTransportKindByDefault = Enums.ExchangeMessagesTransportKinds.FILE Then
			...
			If IsBlankString(FILEInformationExchangeCommonDirectory) Then
				...
				MessageString = NStr("en='Information exchange directory is not specified for endpoint ""%1"".';ru='Не задан каталог обмена информацией для конечной точки ""%1"".';vi='Chưa xác định thư mục trao đổi thông tin cho điểm cuối ""%1"".'");
			...
			If Not CommonDirectory.Exist() Then
				...
				MessageString = NStr("en='The ""%1"" data exchange directory does not exist.';ru='Каталог обмена информацией ""%1"" не существует.';vi='Không tồn tại thư mục trao đổi thông tin ""%1"".'");
		...
		Else
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Kind of exchange message transport ""%1"" for endpoint %2 is not supported.';ru='Вид транспорта сообщений обмена ""%1"" для конечной точки %2 не поддерживается.';vi='Không hỗ trợ dạng truyền thông điệp trao đổi ""%1"" đối với điểm cuối %2.'"),
...
Procedure SaveSessionData(Val Message, Val Presentation = "") Export
	...
	If Not IsBlankString(Presentation) Then
		Presentation = StringFunctionsClientServer.SubstituteParametersInString(NStr("en=' {%1}';ru=' {%1}';vi=' {%1}'"), Presentation);
	EndIf;
	
	MessageString = NStr("en='Message exchange session of the ""%1"" application successfully completed.%2';ru='Сессия обмена сообщениями системы ""%1"" успешно завершена.%2';vi='Đã hoàn tất thành công phiên trao đổi thông báo hệ thống ""%1"".%2'",
...
"exchange session performance. %2 Error description from the correspondent: %3';ru='Ошибка выполнения сессии обмена сообщениями системы ""%1"".%2 Описание ошибки из корреспондента: %3';vi='Lỗi thực hiện phiên trao đổi thông báo hệ thống ""%1"".%2 Mô tả lỗi từ cơ sở thông tin thứ hai: %3'", CommonUseClientServer.MainLanguageCode());
	MessageString = StringFunctionsClientServer.SubstituteParametersInString(MessageString,
...
Function EventLogMonitorEventDataSyncronizationSetting() Export
	...
	Return NStr("en='Data exchange in SaaS.Data synchronization settings';ru='Обмен данными в модели сервиса.Настройка синхронизации данных';vi='Trao đổi dữ liệu trong mô hình dịch vụ. Tùy chỉnh đồng bộ dữ liệu'",
		CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorEventDataSynchronizationMonitor() Export
	...
	Return NStr("en='Data exchange in SaaS.Data synchronization monitor';ru='Обмен данными в модели сервиса.Монитор синхронизации данных';vi='Trao đổi dữ liệu trong mô hình dịch vụ. Màn hình đồng bộ dữ liệu'",
		CommonUseClientServer.MainLanguageCode());
...
Function EventLogMonitorEventDataSynchronization() Export
	...
	Return NStr("en='Data exchange in SaaS.Data synchronization';ru='Обмен данными в модели сервиса.Синхронизация данных';vi='Trao đổi dữ liệu trong mô hình dịch vụ đám mây.Đồng bộ hóa dữ liệu'",
		CommonUseClientServer.MainLanguageCode());
...
Function SystemMessagesExchangeSessionEventLogMonitorEvent()
	...
	Return NStr("en='Data exchange in SaaS.Sessions of application message exchange';ru='Обмен данными в модели сервиса.Сессии обмена сообщениями системы';vi='Trao đổi dữ liệu trong mô hình dịch vụ. Phiên trao đổi thông báo của hệ thống'",
		CommonUseClientServer.MainLanguageCode());
...
Function FindInfobaseNode(Val ExchangePlanName, Val NodeCode)
	...
	If Result = Undefined Then
		Message = NStr("en='Exchange plan node is not found. Exchange plan name %1; node code %2 or %3';ru='Не найден узел плана обмена. Имя плана обмена %1; код узла %2 или %3';vi='Không tìm thấy nút sơ đồ trao đổi. Tên sơ đồ trao đổi %1; mã nút %2 hoặc %3'");
...
"Обратитесь к администратору.';vi='Cấm thay đổi dữ liệu chưa được chia tách (%1),  mà được kết nhập từ ứng dụng trong chỗ làm việc tự động."
...
"Текущий пользователь является неразделенным.';vi='Chỉ có thể tạo mới chỗ làm việc độc lập nhân danh người sử dụng phân tán."
"Người sử dụng hiện tại không được phân tán.'");
