...
Function Parameters() Export
	...
	If Not SavedParameters.Property("UnavailableRolesByUserTypes") Then
		ParameterPresentation = NStr("en='Unavailable roles';ru='Недоступные роли';vi='Vai trò không hợp lệ'");
		
	ElsIf Not SavedParameters.Property("AllRoles") Then
		ParameterPresentation = NStr("en='All roles';ru='Все роли';vi='Tất cả vai trò'");
...
"работы пользователей: ""%1"".';vi='Lỗi cập nhật cơ sở thông tin."
"Chưa điền tham số"
"làm việc của người sử dụng: ""%1"".'")
...
Function IsExternalUserSession() Export
	...
	If CommonUseReUse.DataSeparationEnabled()
		AND CommonUseReUse.SessionWithoutSeparator() Then
			Return False;
	EndIf;
	...
	InfobaseUserID	= InfoBaseUser.UUID;
	
	Users.FindAmbiguousInfobaseUsers(Undefined, InfobaseUserID);
