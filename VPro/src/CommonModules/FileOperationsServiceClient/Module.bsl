...
Procedure OnExit(Warnings) Export
	...
	UserWarning.HyperlinkText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Open list of edited files (%1)';ru='Открыть список редактируемых файлов (%1)';vi='Mở danh sách tệp đang soạn (%1)'"),
...
"т.к. он никем не занят.';vi='Không thể giải phóng tệp ""%1"","
"bởi vì không ai chiếm tệp.'"),
			String(ObjectRef));
		Return False;
	Else
...
"т.к. он занят пользователем ""%2"".';vi='Không thể giải phóng tệp ""%1"","
"bởi vì người sử dụng đã chiếm tệp ""%2"".'"),
			String(ObjectRef),
			String(IsEditing));
		Return False;
	EndIf;
...
Procedure ReleaseFileWithoutQuestion(FileData, UUID = Undefined) Export
	...
	ShowUserNotification(NStr("en='File is released';ru='Файл освобожден';vi='Tệp đã giải phóng'"),
...
Procedure MoveFilesToFolder(ObjectsRef, Folder) Export
	...
	For Each FileData IN DataFiles Do
		...
		ShowUserNotification(
			NStr("en='Transfer file';ru='Перенос файла';vi='Chuyển đổi tệp'"),
			FileData.URL,
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='The"
"""%1"" file is moved to the ""%2"" folder.';ru='Файл"
"""%1"" перенесен в папку ""%2"".';vi='Tệp"
"""%1"" đã được chuyển vào thư mục ""%2"".'"),
				String(FileData.Ref),
				String(Folder)),
...
""
"Освободить файл?';vi='Không thể xếp tệp"
"""%1"" (%2)."
"trong kho chứa tệp, bởi vì chưa tìm thấy tệp trong thư mục làm việc."
""
"Giải phóng tệp?"
""
"'"),
...
""
"Освободить файл?';vi='Không thể xếp tệp ""%1"" vào kho chứa tệp:"
...
"""%1"" в хранилище файлов, возможно он заблокирован другой программой.';vi='Không thể đặt tệp"
"""%1"" vào kho chứa tệp, có thể là tệp đã bị phong tỏa bởi chương trình khác.'");
...
"Пожалуйста, подождите.';vi='Đang thực hiện truyền tệp ""%1"" (%2 Mb)..."
"Xin vui lòng đợi.'"),
...
""
"Повторить операцию?';vi='Không thể đặt tệp vào"
"kho chứa tệp do nguyên nhân: ""%1""."
""
"Lặp lại thao tác?'"),
...
"""%1"" обновлен и освобожден.';vi='Tệp"
"""%1"" đã được cập nhật và giải phóng.'");
		Else
			ExplanationTemplate = NStr("en='File"
"""% 1"" has not been changed and unlocked.';ru='Файл"
"""%1"" не изменился и освобожден.';vi='Tệp"
"""%1"" không thay đổi và giải phóng.'");
		EndIf;
		
		ShowUserNotification(
			NStr("en='Editing is complete';ru='Редактирование закончено';vi='Soạn kết thúc'"),
...
""
"Повторить операцию?';vi='Không thể đặt"
"tệp ""%1"" vào"
"kho chứa tệp do nguyên nhân ""%2""."
""
"Lặp lại thao tác?'"),
		String(ExecuteParameters.FileData.Ref),
		BriefErrorDescription(ErrorInfo));
...
Procedure FinishEditingWithoutExtensionAfterFileImport(Placed, Address, SelectedFileName, ExecuteParameters) Export
	...
	DataDescription.Insert("Operation",            NStr("en='File encryption';ru='Шифрование файла';vi='Mã hóa tệp'"));
	DataDescription.Insert("DataTitle",     NStr("en='File';ru='Файловый';vi='File-server'"));
...
Procedure UpdateFromFileOnDrive(ResultHandler, FileData, FormID) Export
		...
		#If Not WebClient And Not MobileClient Then
			If StandardSubsystemsClientReUse.ClientWorkParameters().ThisIsBasicConfigurationVersion Then
				CommonUseClientServer.MessageToUser(
					NStr("en='This command is not supported in base version.';ru='Данная команда не поддерживается в базовой версии.';vi='Không hỗ trợ lệnh này trong phiên bản căn bản.'"));
				ReturnResult(ResultHandler, False);
				Return;
			EndIf;
		#EndIf
	...
	Dialog.Title                   = NStr("en='Select file';ru='Выбор файла';vi='Chọn tệp'");
	...
	If ValueIsFilled(ExtensionForEncryptedFiles) Then
		Filter = NStr("en='File *.%1)|*.%1|Encrypted file (*.%2)|*.%2|All files (*.*)|*.*';ru='Файл (*.%1)|*.%1|Зашифрованный файл (*.%2)|*.%2|Все файлы (*.*)|*.*';vi='Tệp (*.%1)|*.%1|Tệp mã hóa (*.%2)|*.%2|Tất cả các tệp (*.*)|*.*'");
	Else
		Filter = NStr("en='File (*.%1)|*.%1|All files (*.*)|*.*';ru='Файл (*.%1)|*.%1|Все файлы (*.*)|*.*';vi='Tệp (*.%1)|*.%1|Tất cả tệp (*.*)|*.*'");
	EndIf;
...
Procedure UpdateFromFileOnDiskBeforeDecryption(Result, FileURL, SelectedFileName, ExecuteParameters) Export
	...
	DataDescription = New Structure;
	DataDescription.Insert("Operation",              NStr("en='File decryption';ru='Расшифровка файла';vi='Giải mã tệp'"));
	DataDescription.Insert("DataTitle",       NStr("en='File';ru='Файловый';vi='File-server'"));
...
""
"Операция прервана.';vi='Tệp"
"""%1"" trong kho chứa tệp có"
"ngày thay đổi muộn hơn (%2) so với tệp đã chọn (%3)."
""
"Đã ngắt giao dịch.'"),
			String(ExecuteParameters.FileData.Ref),
...
""
"Операция прервана.';vi='Tệp"
"""%1"" trong thư mục làm việc có ngày"
"thay đổi muộn hơn (%2) tệp đã chọn (%3).'"),
					String(ExecuteParameters.FileData.Ref),
...
"перед выполнением обновления из файла на диске.';vi='Tệp"
"""%1"" trong thư mục làm việc đã được mở ra để soạn."
""
"Hãy kết thúc soạn"
"trước khi thực hiện cập nhật từ tệp trên đĩa.'"),
						FullFileName);
...
Procedure LockFileByRefAfterInstallingExtension(ExtensionIsSet, ExecuteParameters) Export
	...
	ShowUserNotification(
		NStr("en='Edit file';ru='Редактирование файла';vi='Soạn tệp'"),
		ExecuteParameters.FileData.URL,
		StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='The"
"file ""%1"" busy for editing.';ru='Файл"
"""%1"" занят для редактирования.';vi='Tệp"
"""%1"" đã bị chiếm để soạn.'"), String(ExecuteParameters.FileData.Ref)),
		PictureLib.Information32);
...
Procedure LockFilesByReferencesAfterInstallingExtension(ExtensionIsSet, ExecuteParameters) Export
	...
	ShowUserNotification(
		NStr("en='Lock files';ru='Занять файлы';vi='Chiếm tệp'"),
		,
		StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Files (%1 of %2) are locked for editing.';ru='Файлы (%1 из %2) заняты для редактирования.';vi='Tệp (%1 trong %2) đang bị chiếm để soạn.'"),
			LockedCount,
...
""
"Продолжить?';vi='Hủy soạn"
"tệp có thể làm mất những thay đổi của bạn."
""
"Tiếp tục?'"),
...
Procedure UnlockFilesByReferencesAfterAnswerToQuestionCancelEditing(Response, ExecuteParameters) Export
	...
	ShowUserNotification(
		NStr("en='Cancel file editing';ru='Отменить редактирование файлов';vi='Hủy bỏ soạn tệp'"),,
		StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='File editing is canceled (%1 of %2).';ru='Отменено редактирование файлов (%1 из %2).';vi='Hủy bỏ soạn tệp (%1 trong %2).'"),
...
"Продолжить?';vi='Hủy"
"soạn"
"tệp ""%1"" có thể làm mất những thay đổi của bạn.'"),
			String(ExecuteParameters.ObjectRef));
...
Procedure UnlockFileAfterAnswerToQuestionCancelEdit(Response, ExecuteParameters) Export
	...
	If ExecuteParameters.ContinueWork Then
		...
		If Not ExecuteParameters.DoNotAskQuestion Then
			ShowUserNotification(
				NStr("en='File is released';ru='Файл освобожден';vi='Tệp đã giải phóng'"),
				ExecuteParameters.FileData.URL,
...
""
"Освободить файл?';vi='Không thể đặt tệp"
"""%1"" vào kho chứa tệp: Không"
"tìm thấy tệp trong thư mục làm việc: %2."
""
"Giải phóng tệp?'"),
				String(ExecuteParameters.FileData.Ref),
				ExecuteParameters.FullPathToFile);
...
""
"Освободить файл?';vi='Không thể xếp tệp"
"""%1"" vào kho chứa tệp: Không tìm thấy tệp trong thư mục làm việc."
""
"Giải phóng tệp?'"),
				String(ExecuteParameters.FileData.Ref));
		EndIf;
...
"Xin vui lòng đợi.'"),
	...
	If ExecuteParameters.ShowAlert Then
		If VersionUpdated Then
			ShowUserNotification(
				NStr("en='New version is saved';ru='Новая версия сохранена';vi='Phiên bản mới đã lưu'"),
				ExecuteParameters.FileData.URL,
				ExecuteParameters.FileData.FullDescrOfVersion,
				PictureLib.Information32);
		Else
			ShowUserNotification(
				NStr("en='New version is not saved';ru='Новая версия не сохранена';vi='Chưa lưu phiên bản mới'"),,
				NStr("en='File is not changed';ru='Файл не изменился';vi='Vẫn giữ nguyên tệp'"),
				PictureLib.Information32);
...
"Расшифруйте файл и зашифруйте заново.';vi='Ở tệp mã hóa chưa chỉ ra chứng thư."
"Hãy giải mã tệp và mã hóa lại một lần nữa.'"));
...
Procedure LockFileAfterSettingExtension(ExtensionIsSet, ExecuteParameters) Export
	...
	ShowUserNotification(
		...
		ExecuteParameters.FileData.URL,
		StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Unlock the files left?';ru='Освободить остальные файлы?';vi='Giải phóng tệp còn lại?'"),
			String(ExecuteParameters.FileData.Ref)),
		PictureLib.Information32);
...
Procedure ReleaseFiles(ResultHandler, ObjectsRef) Export
	...
	If ListImpossibleRelease.Count() > 0 Then
		QuestionWithListDialog(
			...
			NStr("en='Unlock the files left?';ru='Освободить остальные файлы?';vi='Giải phóng tệp còn lại?'"),
			NStr("en='The following errors occurred while trying to unlock the files:';ru='При попытке освободить файлы возникли следующие ошибки:';vi='Khi cố giải phóng tệp phát sinh các lỗi sau:'"),
			NStr("en='Unlock files';ru='Освободить файлы';vi='Giải phóng tệp'"));
	Else
		ShowQueryBox(
			Handler,
...
"файлов может привести к потере ваших изменений."
""
"Освободить файлы?';vi='Việc giải phóng"
"tệp có thể làm mất những thay đổi của các bạn."
""
"Giải phóng tệp?'"),
			QuestionDialogMode.YesNo,
			,
...
"файл ""%1"" из рабочего каталога?';vi='Xóa"
"tệp ""%1"" khỏi thư mục làm việc?'");
...
"файла из хранилища файлов в рабочий каталог.';vi='Lỗi nhận"
"tệp từ kho chứa tệp trong thư mục làm việc.'"));
...
""
"Получить файл из хранилища файлов?';vi='Tệp"
"""%1"" không có trong thư mục làm việc."
""
"Nhận tệp từ kho chứa tệp?'"),
			FileName),
		QuestionDialogMode.YesNo);
...
Procedure FreePlaceInWorkingDirectory(ResultHandler, VersionAttributes)
	...
	#If WebClient Or MobileClient Then
		...
		ReturnResultAfterShowWarning(
			ResultHandler,
			NStr("en='Working directory cleanup is not supported in web client.';ru='Очистка рабочего каталога не поддерживается в веб-клиенте.';vi='Không hỗ trợ xóa thư mục làm việc trên Web-client.'"),
			Undefined);
		Return;
	#EndIf
...
Procedure ClearWorkingDirectory(ResultHandler, FilesSizeInWorkingDirectory, AddedFileSize, ToClearAll) Export
	...
	#If WebClient Or MobileClient Then
		ReturnResultAfterShowWarning(ResultHandler, NStr("en='Working directory cleanup is not supported in web client.';ru='Очистка рабочего каталога не поддерживается в веб-клиенте.';vi='Không hỗ trợ xóa thư mục làm việc trên Web-client.'"), Undefined);
		Return;
	#EndIf
...
"найден файл ""%1""."
""
"Удалить его из рабочего каталога?';vi='Đang xóa thư mục làm việc khi thêm tệp."
""
"Trong kho chứa tệp không"
"tìm thấy tệp ""%1"".'"),
...
"найден файл ""%1""."
""
"Удалить его из рабочего каталога?';vi='Trong kho chứa tệp không"
"tìm thấy tệp ""%1""."
""
"Xóa tệp khỏi thư mục làm việc?'"),
					ExecuteParameters.DirectoryName + ExecuteParameters.Item.Path);
			EndIf;
			
			Buttons = New ValueList;
			Buttons.Add("Yes",         NStr("en='Yes';ru='Да';vi='Đồng ý'"));
			Buttons.Add("YesForAll",  NStr("en='Yes to all';ru='Да для всех';vi='Đồng ý đối với tất cả'"));
			Buttons.Add("No",        NStr("en='No';ru='Нет';vi='Không'"));
			Buttons.Add("NoForAll", NStr("en='No for all';ru='Нет для всех';vi='Không phải cho tất cả'"));
	...
	If ExecuteParameters.StepNumber = 2 Then
		...
		FileOnDrive.SetReadOnly(False);
		If ExecuteParameters.ToClearAll = False Then
			QuestionHeader = NStr("en='Clearing the working directory while adding a file.';ru='Выполняется очистка рабочего каталога при добавлении файла.';vi='Đang xóa thư mục làm việc khi thêm tệp.'");
		Else
			QuestionHeader = NStr("en='Clearing the working directory.';ru='Выполняется очистка рабочего каталога.';vi='Đang xóa thư mục làm việc.'");
		EndIf;
...
"%2';vi='Chiều dài đường dẫn đến tệp (thư mục làm việc cộng với tên tệp) vượt quá %1 ký tự"
"%2'"),
						ExecuteParameters.FullPathMaxLength,
						ExecuteParameters.FullFileName);
					
					MessageText = MessageText + Chars.CR + Chars.CR
						+ NStr("en='Replace the file name with a shorter one.';ru='Измените имя файла на более короткое.';vi='Hãy thay đổi tên tệp cho ngắn gọn hơn.'");
...
Procedure GetFromServerAndRegisterInLocalFilesCacheOfferToSelectDirectory(Response, ExecuteParameters) Export
	...
	QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='File path length exceeds %1 characters: %2 Choose another working directory?';ru='Длина пути к файлу превышает %1 символов: %2 Выбрать другой основной рабочий каталог?';vi='Chiều dài đường dẫn đến tệp vượt quá %1 ký tự: %2. Chọn thư mục làm việc chính khác?'"),
		ExecuteParameters.FullPathMaxLength,
		ExecuteParameters.FullFileName);
...
Procedure GetFromServerAndRegisterInLocalFilesCacheStartDirectorySelection(Response, ExecuteParameters) Export
	...
	Title = NStr("en='Select another main working directory';ru='Выберите другой основной рабочий каталог';vi='Hãy chọn thư mục làm việc chính khác'");
...
""
"Переименуйте один из файлов в информационной базе.';vi='Trong thư mục làm việc đã có"
"tệp ""%1"" liên quan với tệp khác trong cơ sở thông tin."
""
"Mã tệp trong kho chứa tệp: %2."
"Mã tệp trong thư mục làm việc: %3."
""
"Hãy đổi tên một trong các tệp trong cơ sở thông tin.'"),
				ExecuteParameters.FullFileName,
...
"(У двух папок не должно быть одинакового рабочего каталога).';vi='Trong thư mục làm việc đã"
"có"
"tệp ""%1"" liên quan đến tệp khác trong cơ sở thông tin."
""
"Mã tệp trong kho chứa tệp: %2."
"Mã tệp trong thư mục làm việc: %3.'"),
				ExecuteParameters.FullFileName,
...
"превышает %1 символов ""%2"".';vi='Chiều dài đường dẫn đầy đủ đến tệp (thư mục làm việc và tên tệp)"
"vượt quá %1 ký tự ""%2"".'"),
		ExecuteParameters.FullPathMaxLength,
		ExecuteParameters.FullFileName);
	...
	If StrLen(ExecuteParameters.FileData.OwnerWorkingDirectory) > ExecuteParameters.FullPathMaxLength - 5 Then
		MessageText = MessageText + Chars.CR + Chars.CR
			+ NStr("en='Change folder names or move this folder to another one.';ru='Измените имена папок или перенесите текущую папку в другую папку.';vi='Hãy thay đổi tên thư mục hoặc chuyển thư mục này sang thư mục khác.'");
		ReturnResultAfterShowWarning(ExecuteParameters.ResultHandler, MessageText, False);
		Return;
	EndIf;
...
"(Содержимое рабочего каталога будет перенесено в выбранный каталог).';vi='Chiều dài đường dẫn đầy đủ đến tệp (thư mục làm việc cộng với tên tệp) vượt quá %1 ký tự"
"""%2""."
""
"Chọn thư mục làm việc chính khác?"
"(Nội dung của thư mục làm việc sẽ được chuyển sang thư mục đã chọn).'"),
...
Procedure CheckFullPathMaxLengthInWorkingDirectoryStartDirectorySelection(Response, ExecuteParameters) Export
	...
	// Choose another path to the working directory.
	Title = NStr("en='Select another working directory';ru='Выберите другой рабочий каталог';vi='Hãy chọn thư mục làm việc khác'");
...
""
"Повторить операцию?';vi='Không thể"
"sao chép tệp ""%1""."
"Có thể là tệp đã bị chiếm bởi người sử dụng khác."
""
"Lặp lại giao dịch?'"),
...
"рабочий"
"каталог ""%1"""
"входит в старый рабочий каталог ""%2"".';vi='Thư mục"
"làm việc"
"đã chọn ""%1"""
"nằm trong thư mục làm việc cũ ""%2"".'"),
			ReceiverDirectory,
			SourceDirectory);
...
"обратно"
"содержимое"
"каталога ""%1"" в каталог ""%2"".';vi='Không thể sao chép"
"lại"
"nội dung thư mục ""%1"" vào thư mục ""%2"".'"),
			ExecuteParameters.ReceiverDirectory,
			ExecuteParameters.SourceDirectory);
...
Procedure FilesImportAfterSizesCheck(Result, ExecuteParameters) Export
	...
	If ExecuteParameters.QuantitySummary = 0 Then
		If ExecuteParameters.ImportMode Then
			ReturnResult(ExecuteParameters.ResultHandler, Undefined);
		Else
			ReturnResultAfterShowWarning(ExecuteParameters.ResultHandler, NStr("en='There are no files to add';ru='Нет файлов для добавления';vi='Không có tệp để thêm'"), Undefined);
		EndIf;
		Return;
	EndIf;
...
""
"Продолжить импорт папки?';vi='Đã tồn tại thư mục ""%1""."
"Tiếp tục kết nhập thư mục?'"),
...
Procedure FilesImportAfterCycleContinued(ExecuteParameters)
	...
	If ExecuteParameters.AllFilesStructuresArray.Count() > 1 Then
		...
		StatusText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='File are imported. Imported files: %1';ru='Импорт файлов завершен. Загружено файлов: %1';vi='Hoàn thành kết nhập tệp. Đã kết nhập %1 tệp'"), String(ExecuteParameters.AllFilesStructuresArray.Count()) );
			
		If ExecuteParameters.ImportMode Then
			StatusText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Files have been imported. Files imported: %1';ru='Загрузка файлов завершена. Загружено файлов: %1';vi='Kết nhập tệp đã kết thúc. Kết nhập tệp: %1'"), String(ExecuteParameters.AllFilesStructuresArray.Count()) );
		EndIf;
	...
	If ExecuteParameters.AllFilesStructuresArray.Count() = 1 Then
		...
		ShowUserNotification(
			NStr("en='Change';ru='Изменение:';vi='Thay đổi:'"),
			Ref,
			item0.File,
			PictureLib.Information32);
	EndIf;
	...
	If ExecuteParameters.FilenamesWithErrorsArray.Count() <> 0 Then
		...
		If ExecuteParameters.ImportMode Then
			Parameters.Insert("Title", NStr("en='Report on file import';ru='Отчет о загрузке файлов';vi='Báo cáo kết nhập tệp'"));
		EndIf;
...
"Возможно, файл был изменен.';vi='Ngày"
"thay đổi tệp ""%1"" trong thư mục làm việc muộn (mới) hơn ngày trong kho chứa tệp."
"Có thể là tệp đã bị thay đổi.'"),
					String(ExecuteParameters.FileData.Ref));
...
Procedure SaveAsWithExtensionAfterDecryption(DataDescription, ExecuteParameters) Export
	...
	Filter = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='All files (*.%1)|*.%1';ru='Все файлы (*.%1)|*.%1';vi='Tất cả tệp (*.%1)|*.%1'"), Extension, Extension);
...
"Пожалуйста, подождите.';vi='Đang thực hiện lưu tệp ""%1"" (%2 Mb)..."
"Xin vui lòng đợi.'"),
		FileName, 
		FileFunctionsServiceClientServer.GetStringWithFileSize(SizeInMB));
	...
	Status(NStr("en='File successfully saved';ru='Файл успешно сохранен';vi='Tệp đã lưu thành công'"), , FullFileName);
...
"Пожалуйста, подождите.';vi='Đang"
"thu thập thông tin về thư mục ""%1""."
"Vui lòng chờ trong giây lát.'"),
	...
	If ArrayTooLargeFiles.Count() <> 0 Then 
		...
		For Each File IN ArrayTooLargeFiles Do
			...
			StringText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='%1 (%2 MB)';ru='%1 (%2 МБ)';vi='%1 (%2 MB)'"), String(File), String(FileSizeInMb));
			BigFiles.Add(StringText);
		EndDo;
		
		Parameters.Insert("BigFiles", BigFiles);
		Parameters.Insert("ImportMode", CheckParameters.ImportMode);
		Parameters.Insert("Title", NStr("en='File import warning';ru='Предупреждение при загрузке файлов';vi='Cảnh báo khi kết nhập tệp'"));
...
Procedure Encrypt(ResultHandler, FileData, UUID) Export
	...
	If ExecuteParameters.FileData.Encrypted Then
		WarningText = NStr("en='File ""%1"" is already encrypted.';ru='Файл ""%1"" уже зашифрован.';vi='Tệp ""%1"" đã mã hóa.'");
	...
	If Not ExecuteParameters.FileData.IsEditing.IsEmpty() Then
		WarningText = NStr("en='Cannot encrypt locked file.';ru='Нельзя зашифровать занятый файл.';vi='Không thể mã hóa tệp đang chiếm.'");
		ReturnResultAfterShowWarning(ExecuteParameters.ResultHandler, WarningText, ExecuteParameters);
		Return;
	EndIf;
	...
	If ExecuteParameters.FileData.CountVersions > 1 Then
		FilePresentation = FilePresentation + " (" + StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Versions: %1';ru='Версий: %1';vi='Phiên bản: %1'"), ExecuteParameters.FileData.CountVersions) + ")";
	EndIf;
	...
	DataDescription.Insert("SetPresentation", NStr("en='Files (%1)';ru='Файлы (%1)';vi='Tệp (%1)'"));
...
Function AddFromFileSystemWithExtensionSynchronously(ExecuteParameters) Export
	...
	If Not ExecuteParameters.Property("FullFileName") Then
		...
		FileDialog.Title = NStr("en='Select file';ru='Выбор файла';vi='Chọn tệp'");
		FileDialog.Filter = NStr("en='All files (*.*)|*.*';ru='Все файлы (*.*)|*.*';vi='Tất cả tệp (*.*)|*.*'");
...
"Пожалуйста, подождите..';vi='Đang lưu lại tệp ""%1"" (%2 Mb)."
"Vui lòng chờ trong giây lát...'"),
		FileName,
		FileFunctionsServiceClientServer.GetStringWithFileSize(SizeInMB));
	Status(StatusText);
	...
	ShowUserNotification(
		NStr("en='Created:';ru='Создание:';vi='Tạo mới:'"),
...
Procedure AddFromFileSystemWithoutExtensionAfterFileImport(Placed, Address, SelectedFileName, ExecuteParameters) Export
	...
	If PathStrings.Count() < 2 Then
		QuestionText = NStr("en='Specify file with extension.';ru='Необходимо указать файл с расширением.';vi='Cần chỉ ra tệp với phần mở rộng.'");
		Buttons = New ValueList;
		Buttons.Add(DialogReturnCode.Retry, NStr("en='Select another file';ru='Выбрать другой файл';vi='Chọn tệp khác'"));
...
Procedure InformAboutEncryption(FilesArrayInWorkingDirectoryForDelete,
	...
	ModuleDigitalSignatureClient.InformAboutObjectEncryption(
		StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='File: %1';ru='Файл: %1';vi='Tệp: %1'"), FileRef));
...
Procedure SignFile(FilesArray, FormID, EndProcessor) Export
	...
	DataDescription.Insert("Operation",            NStr("en='File signing';ru='Подписание файла';vi='Ký tên tệp'"));
	DataDescription.Insert("DataTitle",     NStr("en='File';ru='Файловый';vi='File-server'"));
	DataDescription.Insert("DataSet",         DataSet);
	DataDescription.Insert("SetPresentation", NStr("en='Files (%1)';ru='Файлы (%1)';vi='Tệp (%1)'"));
...
Procedure FilesImportRecursivelyWithoutDialogs(Val Owner, Val SelectedFiles, Val ExecuteParameters, Val AskQuestionFolderAlreadyExists)
	...
	For Each SelectedFile IN SelectedFiles Do
		...
		If Not SelectedFile.Exist() Then
			...
			Record.Insert("Error", NStr("en='File is not present on disk.';ru='Файл отсутствует на диске.';vi='Không có tệp trên đĩa.'"));
		...
		Try
			...
			LabelMore = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='IsProcessed file ""%1"" (%2 MB)...';ru='IsProcessed file ""%1"" (%2 MB)...';vi='IsProcessed file ""%1"" (%2 MB)...'"),
				SelectedFile.Name, 
				FileFunctionsServiceClientServer.GetStringWithFileSize(SizeInMB));
				
			StatusText = NStr("en='Importing files from disk...';ru='Импорт файлов с диска...';vi='Kết nhập tệp từ đĩa...'");
			If ExecuteParameters.ImportMode Then
				StatusText = NStr("en='Importing files from disk...';ru='Импорт файлов с диска...';vi='Kết nhập tệp từ đĩa...'");
			EndIf;
			...
			If ExecuteParameters.ImportMode Then
				If FileOperationsServiceServerCall.IsFileWithSuchName(BaseName, Owner) Then
					Record = New Structure;
					Record.Insert("FileName", SelectedFile.FullName);
					Record.Insert("Error", NStr("en='A file with this name already exists in the file storage.';ru='Файл с таким именем уже есть в хранилище файлов.';vi='Đã có tệp với tên như vậy trong kho chưa tệp.'"));
					ExecuteParameters.FilenamesWithErrorsArray.Add(Record);
					Continue;
				EndIf;
			EndIf;
...
"помещения"
"файла ""%1"" во временное хранилище.';vi='Lỗi"
"đặt"
"tệp ""%1"" vào kho chứa tạm thời.'"),
	...
	Else
		...
		If CountTotal > 0 Then 
			...
			FullDetails = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Files (%1 pcs) are transferred to %2';ru='Файлы (%1 шт) перенесены в %2';vi='Tệp (%1 c) đã chuyển sang %2'"),
				CountTotal,
				FileOwner);
			
			ShowUserNotification(
				NStr("en='Files are moved';ru='Файлы перенесены';vi='Tệp đã chuyển'"),
				,
				FullDetails,
				PictureLib.Information32);
...
Function CheckLockedFilesOnExit()
	...
	ApplicationWarningFormParameters.Insert("MessageQuestion",      NStr("en='Exit the application?';ru='Завершить работу с программой?';vi='Kết thúc làm việc với chương trình?'"));
	ApplicationWarningFormParameters.Insert("MessageTitle",   NStr("en='You have locked the following files for editing:';ru='Следующие файлы заняты вами для редактирования:';vi='Tệp tiếp theo đã bị bạn chiếm để soạn:'"));
	ApplicationWarningFormParameters.Insert("Title",            NStr("en='Exit';ru='Завершить';vi='Kết thúc'"));
...
Procedure SetComponent(ResultHandler) Export
	...
	If InitializeComponent() Then
		Status(NStr("en='Scan component already installed.';ru='Компонента сканирования уже установлена.';vi='Cấu phần quét ảnh đã cài đặt.'"));
...
Function ScanComponentVersion() Export
	...
	If Not InitializeComponent() Then
		Return NStr("en='Scan component is not installed';ru='Компонента сканирования не установлена';vi='Cấu phần quét ảnh chưa cài đặt'");
	EndIf;
...
"права на запись в каталог ""%1"".';vi='Sai đường dẫn hoặc không có"
"quyền đối với bản ghi trong thư mục ""%1"".'"),
				DirectoryName);
...
""
"Выберите другой каталог.';vi='Trong thư mục"
"làm việc"
"đã chọn ""%1"" đã có các tệp."
""
