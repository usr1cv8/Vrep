...
Procedure NotifyPendingHandlersNotImplemented() Export
	...
	ShowUserNotification(
		NStr("en='Application operations are temporarily restricted';ru='Работа в программе временно ограничена';vi='Đã hạn chế làm việc đồng thời trong chương trình'"),
		ProcessorsURL(),
		NStr("en='Update to a new version is incomplete';ru='Не завершен переход на новую версию';vi='Chưa kết thúc việc chuyển đổi sang phiên bản mới'"),
...
"запуска ЗарегистрироватьПолноеИзменениеИОМДляПодчиненныхУзловРИБ можно использовать только совместно с параметром ЗапуститьОбновлениеИнформационнойБазы.';vi='Chỉ có thể sử dụng"
"tham số khởi động ЗарегистрироватьПолноеИзменениеИОМДляПодчиненныхУзловРИБ cùng với tham số"
