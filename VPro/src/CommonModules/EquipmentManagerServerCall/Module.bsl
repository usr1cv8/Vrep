...
Function GetBarcodePicture(BarcodeParameters) Export
	...
	If ExternalComponent = Undefined Then
		Raise NStr("en='An error occurred while connecting external barcode printing component.';ru='Ошибка подключения внешней компоненты печати штрихкода!';vi='Lỗi kết nối cấu phần ngoài để in mã vạch!'");
	EndIf;
	...
	If BarcodeParameters.Property("AngleOfRotation") Then
		ExternalComponent.CanvasRotation = BarcodeParameters.AngleOfRotation;
	Else
