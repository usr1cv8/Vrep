		...
		Else
			...
			Else // Enums.FileStorageTypes.InInfobase
				...
				Except
					...
					WriteLogEvent(
						NStr("en='Files.Cannot send the file during data exchange';ru='Файлы.Не удалось отправить файл при обмене данными';vi='Tệp.Không thể gửi tệp khi trao đổi dữ liệu'",
...
Procedure UpdateBinaryDataOfFileAtServer(Val AttachedFile,
	...
	Try
		...
		Else
			...
			Except
				ErrorInfo = ErrorInfo();
				WriteLogEvent(
					NStr("en='Files.Writing file on disk';ru='Файлы.Запись файла на диск';vi='Tệp.Ghi tệp vào đĩa'", CommonUseClientServer.MainLanguageCode()),
					EventLogLevel.Error,
	...
	Except
		...
		EndIf;
		WriteLogEvent(
			NStr("en='Files.Updating the attached file data in the file storage';ru='Файлы.Обновление данных присоединенного файла в хранилище файлов';vi='Tệp.Cập nhật dữ liệu của tệp đính kèm vào kho chứa tệp'",
			     CommonUseClientServer.MainLanguageCode()),
			EventLogLevel.Error,
			,
			,
...
"У владельца файлов"
"типа ""%1"" указан несуществующий справочник ""%2"".';vi='Lỗi khi xác định tên gọi danh mục để lưu tệp."
"Trong đối tượng chủ của các tệp"
"với kiểu ""%1"" đã chỉ ra danh mục không tồn tại ""%2"".'"),
...
"У владельца файлов"
"типа ""%1"" указано"
"имя справочника ""%2"" без окончания ""ПрисоединенныеФайлы"".';vi='Lỗi khi xác định tên gọi danh mục để lưu tệp."
"Trong đối tượng chủ của các tệp"
"với kiểu  ""%1"" đã chỉ ra"
"tên danh mục ""%2"" mà không có kết thúc ""ПрисоединенныеФайлы"".'"),
				String(TypeOfOwnerFiles),
				String(KeyAndValue.Key));
...
"У владельца"
"файлов типа ""%1"" основной справочник указан более одного раза.';vi='Lỗi khi xác định tên gọi danh mục để lưu tệp."
"Trong đối tượng chủ"
"các tệp với kiểu ""%1"" đã chỉ ra hơn một lần danh mục chính.'"),
					String(TypeOfOwnerFiles),
					String(KeyAndValue.Key));
			EndIf;
...
"У владельца"
"файлов типа ""%1"" не имеется справочников для хранения файлов.';vi='Lỗi khi xác định tên danh mục để lưu tệp."
"Đối tượng chủ"
"của các tệp với kiểu ""%1"" không có danh mục để lưu tệp.'"),
			String(TypeOfOwnerFiles));
	EndIf;
...
"""%1"" типа ""%2"" нет справочников для хранения файлов.';vi='Trong đối tượng chủ của tệp"
"""%1"" kiểu ""%2"" không có danh mục để lưu tệp.'"),
			String(FilesOwner),
...
"""%1"" типа ""%2"" нет справочника ""%3"" для хранения файлов.';vi='Trong đối tượng chủ của tệp"
"""%1"" kiểu ""%2"" không có danh mục ""%3"" để lưu tệp.'"),
			String(FilesOwner),
			String(TypeOf(FilesOwner)),
			String(CatalogName));
	EndIf;
...
"""%1"" типа ""%2"" не указан основной справочник для хранения файлов.';vi='Trong đối tượng chủ của tệp"
"""%1"" kiểu ""%2"" chưa chỉ ra danh mục chính để lưu tệp.'") + Chars.LF;
...
"файла в томе: ""%1""."
""
"Ссылка на файл: ""%2"".';vi='Lỗi, khi lưu"
"tệp trong quyển: ""%1""."
""
