...
"поле объекта не обнаружено!';vi='Không thể sử dụng trường %1 để làm trường khóa tự nhiên của đối tượng %2:"
"không tìm thấy trường của đối tượng!'", Metadata.DefaultLanguage.LanguageCode),
					NaturalKeyField,
...
"значений типа ХранилищеЗначения в качестве полей естественного ключа не поддерживается!';vi='Không thể sử dụng trường %1 để làm trường khóa tự nhiên đối tượng %2: không hỗ trợ sử dụng"
"giá trị kiểu ХранилищеЗначения để làm trường khóa tự nhiên!'", Metadata.DefaultLanguage.LanguageCode),
							NaturalKeyField,
							CommonClassifierType.FullName()
						);
...
"                                  ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхПоддерживающихСопоставлениеСсылокПриЗагрузке()';vi='Không thể sử dụng trường %1 để làm trường khóa tự nhiên của đối tượng %2:"
"để làm kiểu trường, có thể sử dụng đối tượng %3 không thuộc tập hợp"
"dữ liệu chung thông qua thủ tục xác định lại"
...
"неразделенный объект %2';vi='Để làm giá trị hằng chia tách %1 có sử dụng tham chiếu đến "
"đối tượng chưa phân tách %2'", Metadata.DefaultLanguage.LanguageCode),
...
"неразделенный объект %3';vi='Để làm giá trị mục tin %1 của đối tượng chia tách %2 có sử dụng tham chiếu đến đối tượng không chia tách %3'", Metadata.DefaultLanguage.LanguageCode),
			FieldName,
			MetadataObject.FullName(),
			MetadataRefs.FullName()
		);
...
"неразделенный объект %3';vi='Để làm giá trị chiều đo, nguồn hoặc mục tin %1 của tập hợp các bản ghi phân tách %2 có sử dụng tham chiếu đến đối tượng %3 không phân tách'", Metadata.DefaultLanguage.LanguageCode),
			FieldName,
			MetadataObject.FullName(),
			MetadataRefs.FullName()
		);
	...
	Else
		...
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Unexpected metadata object: %1.';ru='Неожиданный объект метаданных: %1!';vi='Đối tượng metadata bất thường: %1!'", Metadata.DefaultLanguage.LanguageCode),
			MetadataObject.FullName()
		);
...
"но при выгрузке была диагностирована попытка выгрузки ссылки на неразделенный объект).';vi=' (để làm kiểu giá trị với đối tượng có đặt kiểu dữ liệu phức hợp "
"mà có thể chứa tham chiếu đến dữ liệu phân tán hoặc không phân tán,"
"nhưng khi kết xuất đã dự đoán thử kết xuất tham chiếu đến đối tượng không phân tán). '", Metadata.DefaultLanguage.LanguageCode);
...
"ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхПоддерживающихСопоставлениеСсылокПриЗагрузке().';vi='Đồng thời đối tượng chưa phân bổ %1 không nằm trong thành phần kiểu dữ liệu chung,"
"mà có thể thực hiện so sánh tham chiếu khi kết xuất và kết nhập."
"Tình huống này là không hợp lệ vì khi kết nhập dữ liệu đã kết xuất vào CSTT khác"
"sẽ kết nhập các tham chiếu ""bị hỏng"" đến đối tượng %1."
""
"Để sửa chữa tình huống này, đối với đối tượng %1 cần thực thi cơ chế xác định các trường"
"mà xác định riêng khóa tự nhiên của đối tượng và bật đối tượng %1 trong thành phần"
"kiểu dữ liệu chung mà có thể thực hiện so sánh tham chiếu khi kết xuất và kết nhập,"
"sau khi chỉ ra đối tượng metadata %1 trong thủ tục DataExportImportPredefined.WhenFillingCommonDataTypesSupportingMatchingRefsOnImport().'", Metadata.DefaultLanguage.LanguageCode),
		MetadataRefs.FullName()
	);
...
"ВыгрузкаЗагрузкаДанныхПереопределяемый.ПриЗаполненииТиповОбщихДанныхНеТребующихСопоставлениеСсылокПриЗагрузке().';vi='"
"Nếu việc so sánh chính xác tham chiếu đến dữ liệu không phân tán trong CSTTPT mà đã kết xuất dữ liệu và CSTT mà đã kết nhập dữ liệu được đảm bảo thông qua các cơ chế khác thì cần chỉ ra đối tượng metadata %1 trong thủ tục"
