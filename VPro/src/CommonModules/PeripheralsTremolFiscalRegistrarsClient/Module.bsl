...
"подключаемого оборудования в форме ""Подключение и настройка оборудования"".';vi='Chưa thiết lập tham số thiết bị."
"Để thiết bị làm việc chính xác cần chỉ ra tham số làm việc."
"Có thể thực hiện việc này bằng biểu mẫu ""Tùy chỉnh tham số"" mô-đen thiết bị đã kết nối trong biểu mẫu ""Kết nối và tùy chỉnh thiết bị"".'"));
		Result = False;
	EndIf;
...
Function RunCommand(Command, InputParameters = Undefined, Output_Parameters = Undefined,
	...
	Else
		Output_Parameters.Add(999);
		Output_Parameters.Add(NStr("en='Presentation ""%Presentation%"" not supported by this driver.';ru='Команда ""%Presentation%"" не поддерживается данным драйвером.';vi='Driver này không hỗ trợ lệnh ""%Presentation%"".'"));
...
Function OpenReceipt(DriverObject, Parameters, ConnectionParameters, ReturnReceipt, FiscalReceipt, Output_Parameters) Export
	...
	ErrorText = NStr("en='Error opening the receipt: ';ru='Ошибка при открытии чека: ';vi='Lỗi khi mở phiếu tính tiền:'");
...
Function PrintFiscalLine(DriverObject, Parameters, ConnectionParameters,
	...
	If CurrentString = Undefined Then 
		Result = False;
		ErrorDescription = NStr("en='The tax group could not be determined. Check the hardware settings.';ru='Не удалось определить налоговую группу. Проверьте настроки оборудования.';vi='Không thể xác định nhóm thuế. Hãy kiểm tra tùy chỉnh thiết bị.'")
	Else
	...
	If DriverObject.SellFree(Description, TAXGroup, Price, Count, -DiscountPercent) <> 0 Then
		ErrorText = NStr("en='Error while printing receipt string: ';ru='Ошибка при печати строки чека: ';vi='Lỗi khi in xâu ký tự trong phiếu tính tiền:'");
...
Function PrintNotFiscalLine(DriverObject, Parameters, ConnectionParameters, TextString, Output_Parameters)
	...
	If DriverObject.PrintText(TextString,0) <> 0 Then
		ErrorText = NStr("en='Error while printing text: ';ru='Ошибка при печати текста: ';vi='Lỗi khi in chứng từ:'");
...
Function CloseReceipt(DriverObject, Parameters, ConnectionParameters, PaymentsTable, Output_Parameters) Export
	...
	If CashPaymentAmount > 0 Then
		If DriverObject.Payment(CashPaymentAmount,0,0) <> 0 Then
			ErrorText = Output_Parameters.Add(NStr("en='Error registering cash payment: ';ru='Ошибка при регистрации наличного платежа: ';vi='Lỗi khi ghi nhận thanh toán bằng tiền mặt:'"));
	...
	If NonCashAmount > 0 Then
		If DriverObject.Payment(NonCashAmount,4,0) <> 0 Then
			ErrorText = Output_Parameters.Add(NStr("en='Error registering non-cash payment: ';ru='Ошибка при регистрации безналичного платежа : ';vi='Lỗi khi ghi nhận thanh toán bằng chuyển khoản:'"));
			ErrorDescription = ErrorDescription + ?(ErrorDescription="","",Chars.CR)
				+ ErrorText + DriverObject.GetErrorString(DriverObject.errorCode,0);
			Result = False;
		EndIf;
	...
	If GCPayment И (NonCashPaymentAmount2 > 0) Then
		If DriverObject.Payment(NonCashPaymentAmount2,GCPaymentСode,0) <> 0 Then
			ErrorText = Output_Parameters.Add(NStr("en='Error registering gift certificate payment: ';ru='Ошибка при регистрации платежа подарочными сертификатами : ';vi='Lỗi khi ghi nhận thanh toán bằng phiếu quà tặng:'"));
			ErrorDescription = ErrorDescription + ?(ErrorDescription="","",Chars.CR)
				+ ErrorText + DriverObject.GetErrorString(DriverObject.errorCode,0);
		EndIf;
	EndIf;
	...
	If DriverObject.CloseFiscalBon() <> 0 Then
		ErrorText = Output_Parameters.Add(NStr("en='Error closing the receipt: ';ru='Error closing the receipt: ';vi='Lỗi khi đóng biên nhận:'"));
		ErrorDescription = ErrorDescription + ?(ErrorDescription="","",Chars.CR)
			+ ErrorText+DriverObject.GetErrorString(DriverObject.errorCode,0);
		Result = False;
	EndIf;
			...
			If Result Then 
				If CashPaymentAmount > 0 Then
					If DriverObject.OpenTill() <> 0 Then
						ErrorText = Output_Parameters.Add(NStr("en='Error openint the money box: ';ru='Ошибка при открытии денежного ящика: ';vi='Lỗi khi mở hộp đựng tiền:'"));
						ErrorDescription = ErrorText + DriverObject.GetErrorString(DriverObject.errorCode,0);
						Result = False;
					EndIf;
				EndIf;
...
Function CancelReceipt(DriverObject, Parameters, ConnectionParameters, Output_Parameters)
	...
	If DriverObject.TerminateBon(0) <> 0 Then    // 1 - pay current receippts, 0 - clear
		ErrorText = Output_Parameters.Add(NStr("en='Error canceling the receipt: ';ru='Ошиба при аннулировании чека: ';vi='Lỗi khi hủy phiếu tính tiền:'"));
		ErrorDescription = ErrorText + DriverObject.GetErrorString(DriverObject.errorCode,0);
		Output_Parameters.Add(999);
		Output_Parameters.Add(ErrorDescription);
		Result = False;
...
Function Encashment(DriverObject, Parameters, ConnectionParameters, EncashmentType, Amount, Output_Parameters)
	...
	If DriverObject.OfficialSums(1,UserPassword,0,?(EncashmentType = 1, Amount, -Amount)) <> 0 Then
		ErrorText = Output_Parameters.Add(NStr("en='Error in depositing/withdrawing money: ';ru='Ошибка при внесении/изъятии денег: ';vi='Lỗi khi nhập/rút tiền:'"));
		ErrorDescription = ErrorText + DriverObject.GetErrorString(DriverObject.errorCode,0);
		Result = False;
	EndIf;
...
Function PrintReportWithoutBlanking(DriverObject, Parameters, ConnectionParameters, Output_Parameters)
	...
	If Result = DriverObject.ReportDaily(0,DetailedReport)<> 0 Then
		ErrorText = Output_Parameters.Add(NStr("en='Error printing Z-Report: ';ru='Ошибка при печати Z-отчета: ';vi='Lỗi khi in báo cáo Z:'"));
		ErrorDescription = ErrorText + DriverObject.GetErrorString(DriverObject.errorCode,0);
		Result = False;
	EndIf;
...
Function GetDriverVersion(DriverObject, Parameters, ConnectionParameters, Output_Parameters)
	...
	Output_Parameters.Add(NStr("en='Installed';ru='Установлен';vi='Đã cài đặt'"));
	Output_Parameters.Add(NStr("en='Undefined';ru='Не определена';vi='Không xác định'"));
