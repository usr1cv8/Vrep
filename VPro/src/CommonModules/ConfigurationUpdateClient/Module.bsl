...
#If Not WebClient And Not MobileClient Then
	...
	If UpdateSettings.ConfigurationChanged AND UpdateSettings.HasAccessForChecksUpdate Then
		...
		ShowUserNotification(NStr("en='Update configuration';ru='Обновление конфигурации';vi='Cập nhật cấu hình'"),
			"e1cib/app/DataProcessor.ConfigurationUpdate",
			NStr("en='The configuration is different from the basic configuration of the infobase.';ru='Конфигурация отличается от основной конфигурации информационной базы.';vi='Cấu hình khác với cấu hình chính của cơ sở thông tin.'"), 
	...
	If IsAffordableInternetEnabledRefresh AND UpdateSettings.HasAccessForChecksUpdate Then
		...
		Parameters.NeedUpdateFile = ConfigurationUpdateOptions.NeedUpdateFile;
		Parameters.FlagOfAutoTransitionToPageWithUpdate = True;
		ShowUserNotification(NStr("en='Configuration update is available';ru='Доступно обновление конфигурации';vi='Cho phép cập nhật cấu hình'"),
			"e1cib/app/DataProcessor.ConfigurationUpdate",
			NStr("en='Version:';ru='Версия:';vi='Phiên bản:'") + " " + Parameters.FileParametersUpdateChecks.Version, 
			PictureLib.Information32);
		Return;	
	EndIf;
...
"%1';vi='Không thể tạo thư mục tạm thời để kiểm tra sự tồn tại của bản cập nhật."
"%1'");
...
"%1';vi='Không thể kiểm tra sự tồn tại của bản cập nhật."
"%1'"), Result.ErrorInfo);
		If OutputMessages Then
			ShowMessageBox(, ErrorInfo);
		EndIf;
...
Function InstallationPackageParameters(Val FileName) Export
	...
	If Not File.Exist() Then
		Return NStr("en='Update description file is not received';ru='Файл описания обновлений не получен';vi='Tệp mô tả bản cập nhật chưa nhận được'");
	EndIf;	
	...
	For LineNumber = 1 To TextDocument.LineCount() Do
		...
		Else
			Return NStr("en='Incorrect update information format';ru='Неверный формат сведений о наличии обновлений';vi='Sai định dạng thông tin về sự tồn tại của bản cập nhật'");
		EndIf;
	EndDo;
...
Procedure CheckUpdateExistsViaInternet(OutputMessages = False, UpdateAvailableForNewEdition = False) Export
	...
	Status(NStr("en='Check for updates online';ru='Проверка наличия обновления в Интернете';vi='Kiểm tra có bản cập nhật trên Internet'"));
	Parameters = GetAvailableConfigurationUpdate(); 
	...
	If TypeOf(Parameters.FileParametersUpdateChecks) = Type("String") Then
		EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(), "Warning",
			NStr("en='Cannot connect to the Internet to check for updates.';ru='Невозможно подключиться к сети Интернет для проверки обновлений.';vi='Không thể kết nối với mạng Internet để kiểm tra bản cập nhật.'"));
	...
	If CommonUseClientServer.CompareVersions(ConfigurationVersion, Parameters.LastConfigurationVersion) >= 0 Then
		...
		If UpdatesNotDetected Then
			...
			EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(), "Information",
				NStr("en='Update is not required: the newest configuration version is already installed.';ru='Обновление не требуется: последняя версия конфигурации уже установлена.';vi='Không cần cập nhật: phiên bản mới nhất của cấu hình đã được cài đặt.'"));
	...
	MessageText = NStr("en='Newer configuration version is found on the Internet: %1.';ru='Обнаружена более новая версия конфигурации в Интернете: %1.';vi='Đã tìm thấy phiên bản cấu hình mới hơn trên Internet: %1.'");
...
Procedure CheckUpdateOnSchedule() Export
	...
	EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(),, 
		NStr("en='Check for updates on the Internet on schedule.';ru='Проверка наличия обновления в сети Интернет по расписанию.';vi='Kiểm tra việc có sẵn bản cập nhật trên mạng Internet theo lịch biểu.'"));
	...
	If Parameters.UpdateSource <> -1 AND Parameters.PageName = AvailableUpdatePageDescription Then
		...
		EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(),,
			NStr("en='New configuration version is detected:';ru='Обнаружена новая версия конфигурации:';vi='Tìm thấy phiên bản cấu hình mới:'") + " " +
	...
	Else
		EventLogMonitorClient.AddMessageForEventLogMonitor(EventLogMonitorEvent(),, 
			NStr("en='No available updates were found.';ru='Доступных обновлений не обнаружено.';vi='Bản cập nhật hợp lệ không tìm thấy.'"));
	EndIf;
	ConfigurationUpdateServerCall.WriteStructureOfAssistantSettings(ConfigurationUpdateOptions, ApplicationParameters["StandardSubsystems.MessagesForEventLogMonitor"]);
...
Function EventLogMonitorEvent() Export
	Return NStr("en='Update configuration';ru='Обновление конфигурации';vi='Cập nhật cấu hình'", CommonUseClientServer.MainLanguageCode());
EndFunction
...
Procedure OnExit(Warnings) Export
	...
	If ApplicationParameters["StandardSubsystems.OfferInfobaseUpdateOnSessionExit"] = True Then
		...
		WarningParameters.FlagText  = NStr("en='Install configuration update';ru='Установить обновление конфигурации';vi='Cài đặt cập nhật cấu hình'");
...
Function CheckUpdateImportLegality(QueryParameters) Export
	...
	Except
		ErrorText = NStr("en='An error occurred when creating description of web service for update legality check.';ru='Ошибка создания описания веб-сервиса проверки легальности получения обновления.';vi='Lỗi tạo mô tả Web-service kiểm tra tính hợp pháp khi nhận bản cập nhật.'");
...
"Повторите попытку позднее';vi='Tạm thời Service kiểm tra tính hợp pháp khi nhận bản cập nhật không khả dụng."
"Hãy thử lại sau'");
...
"Проверьте параметры подключения к Интернету';vi='Không thể kết nối đến Service kiểm tra tính hợp pháp khi nhận bản cập nhật."
"Hãy kiểm tra tham số kết nối với Internet'");
		Return WebServerResponceStructure(ErrorText, True,,, DetailErrorDescription(ErrorInfo()));
...
"Обратитесь к администратору';vi='Lỗi thực hiện kiểm tra tính hợp pháp khi nhận bản cập nhật.'");
		Return WebServerResponceStructure(ErrorText, True,,,DetailErrorDescription(ErrorInfo()));
	...
	Else
		...
		Result = WebServerResponceStructure(NStr("en='Unexpected update service response';ru='Неожиданный ответ сервиса проверки легальности получения обновлений';vi='Trả lời bất thường của Service kiểm tra tính hợp pháp khi nhận bản cập nhật'"), True);
...
Function WebServerResponceStructure(ErrorText, RecordToEventLogMonitor,
	...
	If RecordToEventLogMonitor Then
		...
		If IsBlankString(MessageText) Then
			MessageText = NStr("en='%ErrorText. Error code: %ErrorCode';ru='%ErrorText. Код ошибки: %ErrorCode.';vi='%ErrorText. Mã lỗi: %ErrorCode.'");
...
"Не удалось получить WSDL-описание с сервера проверки легальности скачивания обновления (%1): %2.';vi='Lỗi khi tạo mô tả Web-service."
"Không thể nhận mô tả WSDL từ Server kiểm tra tính hợp pháp khi tải bản cập nhật (%1): %2.'"),
...
"Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления: %2.';vi='Lỗi khi đọc mô tả Web-service (%1)."
"Lỗi đọc mô tả WSDL Web-service kiểm tra tính hợp pháp khi tải bản cập nhật: %2.'"),
			WSDLAddress, DetailErrorDescription(ErrorInfo()));
		Raise ErrorMessage;
	EndTry;
...
"Отсутствует элемент описания типов данных (<wsdl:types ...>).';vi='Lỗi khi tạo mô tả Web-service (%1)."
"Lỗi đọc mô tả WSDL Web-service kiểm tra tính hợp pháp khi tải bản cập nhật:"
"Không có phần tử mô tả kiểu dữ liệu (<wsdl:types ...>).'"), WSDLAddress);
		Raise ErrorMessage;
...
"проверки легальности скачивания обновления: Отсутствует элемент описания типов данных (<xs:schema ...>)';vi='Lỗi khi tạo mô tả Web-service (%1)."
"Lỗi đọc mô tả WSDL Web-service"
"kiểm tra tính hợp lệ khi tải về bản cập nhật: Không có phần tử mô tả kiểu dữ liệu (<xs:schema ...>)'"), WSDLAddress);
		Raise ErrorMessage;
...
"веб-сервиса проверки легальности скачивания обновления: %2';vi='Lỗi khi tạo mô tả Web-service (%1)."
"Lỗi khi tạo sơ đồ dữ liệu từ mô tả WSDL"
"Web-service kiểm tra tính hợp lệ khi tải về bản cập nhật: %2'"), WSDLAddress, DetailErrorDescription(ErrorInfo()));
		Raise ErrorMessage;
...
"Ошибка при создании фабрики XDTO из WSDL-описания веб-сервиса проверки легальности скачивания обновления: %2';vi='Lỗi khi tạo mô tả Web-service (%1)."
"Lỗi khi tạo yếu tố XDTO từ mô tả WSDL Web-service kiểm tra tính hợp pháp khi tải bản cập nhật: %2'"),
			WSDLAddress, DetailErrorDescription(ErrorInfo()));
		Raise ErrorMessage;
...
"Ошибка чтения WSDL-описания веб-сервиса"
"проверки легальности скачивания обновления: Отсутствует URI пространства имен в WSDL-описании.';vi='Lỗi khi tạo mô tả Web-service (%1)."
"Lỗi đọc mô tả WSDL Web-service"
"kiểm tra tính hợp lệ khi tải bản cập nhật: Không có không gian tên URI trong mô tả WSDL.'"), WSDLAddress);
		Raise ErrorMessage;
...
"Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:"
"Отсутствует описание веб-сервисов в WSDL-описании (<wsdl:service ...>).';vi='Lỗi khi tạo mô tả Web-service (%1)."
"Lỗi đọc mô tả WSDL Web-service kiểm tra tính hợp pháp khi tải bản cập nhật:"
"Không có mô tả Web-service trong mô tả WSDL (<wsdl:service ...>).'"), WSDLAddress);
		Raise ErrorMessage;
...
"Ошибка чтения WSDL-описания веб-сервиса проверки легальности скачивания обновления:"
"Отсутствует описание портов в WSDL-описании (<wsdl:port ...>).';vi='Lỗi khi tạo mô tả Web-service (%1)."
"Lỗi đọc mô tả WSDL Web-service kiểm tra tính hợp pháp khi tải bản cập nhật:"
"Không có mô tả cổng trong mô tả WSDL (<wsdl:port ...>).'"), WSDLAddress);
		Raise ErrorMessage;
...
"веб-сервиса интернет-поддержки пользователей: Не удалось определить URL заданного порта сервиса (%2).';vi='Lỗi khi tạo mô tả Web-service (%1)."
"Lỗi đọc mô tả WSDL"
"Web-service hỗ trợ người sử dụng qua Internet: Không thể xác định URL của cổng Service đã đặt (%2).'"), WSDLAddress, PortName);
...
Function ServiceAvailable(LegalityCheckServiceDescription)
	...
	Except
		...
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='An error occurred when calling operation isReady of service (%1): %2';ru='Ошибка при вызове операции isReady сервиса (%1): %2';vi='Lỗi khi gọi ra thao tác isReady của Service (%1): %2'"),
...
"Не удалось определить тип корневого свойства isReadyResponse.';vi='Lỗi khi gọi ra thao tác isReady của Service (%1)."
"Không thể xác định kiểu thuộc tính gốc isReadyResponse.'"),
			"%1",
	...
	Except
		ErrorMessage = StrReplace(NStr("en='An error occurred when calling operation isReady of service (%1).';ru='Ошибка при вызове операции isReady сервиса (%1).';vi='Lỗi khi gọi ra thao tác isReady của Service (%1).'"),
			"%1",
			LegalityCheckServiceDescription.WSDLAddress)
			+ Chars.LF
			+ DetailErrorDescription(ErrorInfo())
			+ Chars.LF + Chars.LF
			+ NStr("en='Query body:';ru='Тело запроса:';vi='Nội dung truy vấn:'")
			+ Chars.LF
			+ EnvelopeText;
...
"isReady сервиса (%1) Ошибка SOAP: %2';vi='Lỗi khi gọi giao dịch"
"isReady của Service (%1) Lỗi SOAP: %2'"), LegalityCheckServiceDescription.WSDLAddress, DescriptionSOAPExceptionToRow(Value));
		Raise ErrorMessage;
...
Function RefreshReceivedLegally(QueryParameters, LegalityCheckServiceDescription)
	...
	Except
		...
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='An error occurred when calling operation process of service (%1): %2';ru='Ошибка при вызове операции process сервиса (%1): %2';vi='Lỗi khi gọi ra thao tác process của Service (%1): %2'"),
			LegalityCheckServiceDescription.WSDLAddress, DetailErrorDescription(ErrorInfo()));
		Raise ErrorMessage;
...
"Не удалось определить тип корневого свойства processResponse.';vi='Lỗi khi gọi ra thao tác process của Service (%1)."
"Không thể xác định kiểu thuộc tính gốc processResponse.'"),
			"%1",
			LegalityCheckServiceDescription.WSDLAddress);
		Raise ErrorMessage;
	EndIf;
	...
	Except
		...
		ErrorMessage = StrReplace(NStr("en='An error occurred when calling operation process of service (%1).';ru='Ошибка при вызове операции process сервиса (%1).';vi='Lỗi khi gọi ra thao tác process của Service (%1).'"),
			"%1",
			LegalityCheckServiceDescription.WSDLAddress)
			+ Chars.LF
			+ DetailErrorDescription(ErrorInfo())
...
Function SendSOAPQuery(EnvelopeText, LegalityCheckServiceDescription)
	...
	Except
		ErrorMessage = NStr("en='A connection error occurred when sending a SOAP query.';ru='Ошибка сетевого соединения при отправке SOAP-запроса.';vi='Lỗi kết nối theo mạng khi gửi truy vấn SOAP.'")
			+ Chars.LF
			+ DetailErrorDescription(ErrorInfo());
		Raise ErrorMessage;
	EndTry;
...
"чтения"
"ответа SOAP:"
"%1 Тело ответа: %2';vi='Lỗi"
"đọc"
"phản hồi SOAP:"
"%1 Nội dung phản hồi: %2'"), DetailErrorDescription(ErrorInfo()), ResponseBody);
		Raise ErrorMessage;
...
"%1 Nội dung phản hồi: %2'"), DetailErrorDescription(ErrorInfo()), ResponseBody);
		...
		ErrorMessage = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='The SOAP-server error occurred when processing the query: %1';ru='Ошибка SOAP-Сервера при обработке запроса: %1';vi='Lỗi Server SOAP khi xử lý truy vấn: %1'"), DescriptionSOAPExceptionToRow(ExceptionDetails));
		Raise ErrorMessage;
...
"(%1)"
"в конверте"
"SOAP: %2 Тело ответа: %3';vi='Lỗi đọc đối tượng"
"(%1)"
"trong phong bì"
"SOAP: %2 Nội dung phản hồi: %3'"), String(ValueType), DetailErrorDescription(ErrorInfo()), ResponseBody);
		Raise ErrorMessage;
...
Function DescriptionSOAPExceptionToRow(ExceptionSOAP)
	...
	If Not IsBlankString(ExceptionSOAP.FaultCode) Then
		Result = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Error code: %1';ru='Код ошибки: %1';vi='Mã lỗi: %1'"), ExceptionSOAP.FaultCode);
	EndIf;
	
	If Not IsBlankString(ExceptionSOAP.FaultString) Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Error line: %1';ru='Строка ошибки: %1';vi='Lỗi dòng: %1'"), ExceptionSOAP.FaultString);
		Result = Result + Chars.LF + ErrorText;
	EndIf;
	
	If Not IsBlankString(ExceptionSOAP.FaultActor) Then
		ErrorText = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Error source: %1';ru='Источник ошибки: %1';vi='Nguồn lỗi: %1'"), ExceptionSOAP.FaultActor);
		Result = Result + Chars.LF + ErrorText;
	EndIf;
