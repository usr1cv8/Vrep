...
Procedure OpenFilterFormByCompanies(Command)
	...
	OpenForm("ExchangePlan.ExchangeCompanyManagementAccounting.Form.ChoiceFormCompanies",
		FormParameters,
		ThisForm);
...
Procedure OpenFormFilterByDocumentTypes(Command)
	...
	OpenForm("ExchangePlan.ExchangeCompanyManagementAccounting.Form.DocumentKindsChoiceForm",
		FormParameters,
		ThisForm);
...
Procedure CompanyFilterClean(Command)
	...
	QuestionText   = NStr("en='Clear filter by companies?';ru='Очистить отбор по организациям?';vi='Xóa lọc theo các doanh nghiệp?'");
	Response = Undefined;
...
Procedure ClearDocumentTypesFilter(Command)
	...
	HeaderText = NStr("en='Confirmation';ru='Подтверждение';vi='Xác nhận'");
	QuestionText   = NStr("en='Clear filter by document kinds?';ru='Очистить отбор по видам документов?';vi='Xóa lọc theo các dạng chứng từ?'");
	Response = Undefined;
...
Procedure RefreshDescriptionCommandsForms()
	...
	If Companies.Count() > 0 Then
		...
		NewTitleCompanies = ExchangePlans.ExchangeCompanyManagementAccounting.ShortPresentationOfCollectionsOfValues(SelectedCompanies);
		
	Else
		
		NewTitleCompanies = NStr("en='Select companies ';ru='Выбрать организации ';vi='Chọn doanh nghiệp'");
	...
	If DocumentKinds.Count() > 0 Then
		...
		NewTitleOfDocuments = ExchangePlans.ExchangeCompanyManagementAccounting.ShortPresentationOfCollectionsOfValues(SelectedDocumentKinds);
		
	Else
		
		NewTitleOfDocuments = NStr("en='Select documents types ';ru='Выбрать виды документов ';vi='Chọn dạng chứng từ'");
...
Procedure GetContextDetails()
	...
	Else
		...
		If UseDocumentTypesFilter Then
			TextDescription = TextDescription + Chars.LF + NStr("en='Documents are automatically registered for sending:';ru='Документы автоматически регистрируются к отправке:';vi='Chứng từ được tự động ghi nhận để gửi:'");
		Else
		...
		If UseCompaniesFilter Then
			...
			PresentationCollections = ExchangePlans.ExchangeCompanyManagementAccounting.ShortPresentationOfCollectionsOfValues(CollectionValues);
...
Function ExportContextForms()
	...
	SettingsStructure = ThisObject.Context.FilterSettingsAtNode;
	...
	ThisObject.Context.FilterSettingsAtNode = SettingsStructure;
