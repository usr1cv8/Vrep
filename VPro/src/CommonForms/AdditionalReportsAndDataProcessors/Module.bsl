...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If KindOfDataProcessors = Enums.AdditionalReportsAndDataProcessorsKinds.ObjectFilling Then
		...
		Title = NStr("en='Object population commands';ru='Команды заполнения объектов';vi='Lệnh điền đối tượng'");
	ElsIf KindOfDataProcessors = Enums.AdditionalReportsAndDataProcessorsKinds.Report Then
		ThisIsAppointedDataProcessors = True;
		AreReports = True;
		Title = NStr("en='Reports';ru='Отчеты';vi='Báo cáo'");
	ElsIf KindOfDataProcessors = Enums.AdditionalReportsAndDataProcessorsKinds.PrintForm Then
		ThisIsAppointedDataProcessors = True;
		Title = NStr("en='Additional print forms';ru='Дополнительные печатные формы';vi='Mẫu in bổ sung'");
	ElsIf KindOfDataProcessors = Enums.AdditionalReportsAndDataProcessorsKinds.CreatingLinkedObjects Then
		ThisIsAppointedDataProcessors = True;
		Title = NStr("en='Commands for linked object creation';ru='Команды создания связанных объектов';vi='Lệnh tạo đối tượng rằng buộc'");
	ElsIf KindOfDataProcessors = Enums.AdditionalReportsAndDataProcessorsKinds.AdditionalInformationProcessor Then
		...
		Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Additional data processors (%1)';ru='Дополнительные обработки (%1)';vi='Bộ xử lý bổ sung (%1)'"), 
	...
	ElsIf KindOfDataProcessors = Enums.AdditionalReportsAndDataProcessorsKinds.AdditionalReport Then
		...
		AreReports = True;
		Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Additional reports (%1)';ru='Дополнительные отчеты (%1)';vi='Báo cáo bổ sung (%1)'"), 
			AdditionalReportsAndDataProcessors.PresentationOfSection(SectionReference));
	EndIf;
...
Procedure RunDataProcessorByParameters()
	...
	ElsIf DataProcessorData.StartVariant = PredefinedValue("Enum.AdditionalDataProcessorsCallMethods.CallOfServerMethod")
		...
		Items.ExplanatoryDecoration.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Executing the ""%1"" command...';ru='Выполняется команда ""%1""...';vi='Đang thực hiện lệnh ""%1""...'"),
...
Function ExecuteProcessingServerMethodAtServer(ServerCallParameters)
	...
	Try
		BackgroundJobResult = LongActions.ExecuteInBackground(
			...
			NStr("en='Additional reports and data processors: Running server method of data processor';ru='Дополнительные отчеты и обработки: Выполнение серверного метода обработки';vi='Báo cáo và bộ xử lý bổ sung: Thực hiện phương thức xử lý Client-server'"));
...
Procedure ShowProcessingExecutionResult(Result, CloseThisForm)
	...
	If ExecuteCommand.ShowAlert Then
		...
		If ExecutionResult.OutputNotification.Use <> True Then
			...
			ExecutionResult.OutputNotification.Title = NStr("en='Command executed';ru='Команда выполнена';vi='Đã thực hiện lệnh'");
...
"Подробности см. в журнале регистрации.';vi='Không thể thực hiện lệnh ""%1""."
