...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If TypeOf(Parameters.User) = Type("CatalogRef.ExternalUsers") Then
		Items.Profiles.Title = NStr("en='External user profiles';ru='Профили внешнего пользователя';vi='Hồ sơ người sử dụng ngoài'");
	Else
		Items.Profiles.Title = NStr("en='User profiles';ru='Профили пользователя';vi='Hồ sơ người sử dụng'");
	EndIf;
...
"у одного пользователя, которому разрешен вход в программу.';vi='Cần thiết lập hồ sơ Người quản trị đối với một người sử dụng được phép đăng nhập vào chương trình.'");
							EndIf;
						EndIf;
					EndIf;
				EndIf;
