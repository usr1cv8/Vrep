...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.Node = Manager.ThisNode() Then
		Raise
			NStr("en='Cannot create initial image for this node.';ru='Создание начального образа для данного узла невозможно.';vi='Không thể tạo ảnh ban đầu để với nút này.'");
	Else
	...
	If Not StandardSubsystemsServerCall.ClientWorkParameters().FileInfobase Then
		Items.PathToArchiveWithVolumeFiles.InputHint = NStr("en='\\server name\resource\files.zip';ru='\\имя сервера\resource\files.zip';vi='\\tên Server\resource\files.zip'");
...
Procedure CreateInitialImage(Command)
	...
	Status(
		NStr("en='Data synchronization';ru='Синхронизация данных';vi='Đồng bộ hóa dữ liệu'"),
		,
		NStr("en='Initial image is being created...';ru='Осуществляется создание начального образа...';vi='Đang thực hiện tạo ảnh ban đầu...'"),
...
"на данной платформе не поддерживается.';vi='Không hỗ trợ tạo ảnh ban đầu tệp cơ sở thông tin"
"trên nền tảng này.'");
		EndIf;
	...
	ShowMessageBox(Handler, NStr("en='Initial image has been created.';ru='Создание начального образа успешно завершено.';vi='Việc tạo ảnh ban đầu kết thúc thành công.'"));
EndProcedure
...
Procedure FileSavingHandlerAfterConnectionExpansionFileOperations(Attached, AdditionalParameters) Export
	...
	Dialog.Title                = NStr("en='Select a file to save to';ru='Выберите файл для сохранения';vi='Hãy chọn tệp để lưu'");
