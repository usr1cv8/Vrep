...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Title = NStr("en='Equipment:';ru='Оборудование:';vi='Thiết bị:'") + Chars.NBSp  + String(ID);
...
"Continue?';vi='Để cài đặt Driver, cần có kết nối Internet."
"Tiếp tục?'");
...
Procedure DeviceTestEnd(ExecutionResult, Parameters) Export
	...
	MessageText = ?(ExecutionResult.Result,  NStr("en='Test succeeded. %AdditionalDetails%';ru='Тест успешно выполнен. %AdditionalDetails%';vi='Đã thử nghiệm thành công. %AdditionalDetails%'"),
	                               NStr("en='Test failed. %AdditionalDetails%';ru='Тест не пройден. %AdditionalDetails%';vi='Chưa tiến hành thử nghiệm: %AdditionalDetails%'"));
...
Procedure AdditionalAction(Command)
	...
	MessageText = ?(Result,  NStr("en='Operation is performed successfully.';ru='Операция выполнена успешно.';vi='Giao dịch đã thực hiện thành công.'"),
	                               NStr("en='Operation execution error.';ru='Ошибка выполнения операции.';vi='Lỗi thực hiện giao dịch.'") + Chars.NBSp + Output_Parameters[1]);
	CommonUseClientServer.MessageToUser(MessageText);
	...
	If XMLReader.Name = "Settings" AND XMLReader.NodeType = XMLNodeType.StartElement Then  
		While XMLReader.Read() Do  
			...
			If XMLReader.Name = "Parameter" AND XMLReader.NodeType = XMLNodeType.StartElement Then  
				...
				If Items.Find(Parameter_Name) = Undefined Then
					...
					If BaseGroup = Undefined Then
						...
						BaseGroup.Title = NStr("en='Parameters';ru='Параметры';vi='Tham số'");
					EndIf;
			...
			If XMLReader.Name = "Page" AND XMLReader.NodeType = XMLNodeType.StartElement Then
				...
				PageTitle = ?(IsBlankString(PageTitle), NStr("en='Parameters';ru='Параметры';vi='Tham số'"), PageTitle);
			...
			If XMLReader.Name = "Group" AND XMLReader.NodeType = XMLNodeType.StartElement Then  
				...
				TitleGroups = ?(IsBlankString(TitleGroups), NStr("en='Parameters';ru='Параметры';vi='Tham số'"), TitleGroups);
...
Procedure StartAdditionalCommandExecutionEnd(ExecutionResult, Parameters) Export
	...
	If ExecutionResult.Result Then
		...
		If IntegrationLibrary AND Not MainDriverIsSet Then
			DriverIsSet = NStr("en='Integration library is installed';ru='Установлена интеграционная библиотека';vi='Đã cài đặt thư viện tích hợp'");
			DriverVersion = NStr("en='Not defined';ru='Не определена';vi='Không xác định'");
			Items.SetupDriver.Title = NStr("en='Install the main driver supply';ru='Установить основную поставку драйвера';vi='Cài đặt bộ cài chính của Driver'");
		EndIf;
		Items.DriverAndVersion.Enabled = True;
	Else
		...
		DriverVersion  = NStr("en='Not defined';ru='Не определена';vi='Không xác định'");
		If Not IsBlankString(DriverMessage) AND DriverIsSet = NStr("en='Set';ru='Установлен';vi='Đã cài đặt'") Then
	...
	Items.Driver.TextColor = ?(DriverVersion = NStr("en='Not defined';ru='Не определена';vi='Không xác định'"), ErrorColor, TextColor);
	Items.Version.TextColor  = Items.Driver.TextColor ;
	Items.DriverDescription.TextColor = ?(DriverDescription = NStr("en='Undefined';ru='Неопределено';vi='Không xác định'"), ErrorColor, TextColor);
	Items.DetailsDriver.TextColor     = ?(DetailsDriver     = NStr("en='Undefined';ru='Неопределено';vi='Không xác định'"), ErrorColor, TextColor);
	Items.DetailsDriver.Visible = Not IsBlankString(DetailsDriver);
	
	Items.SetupDriver.Enabled = Not (DriverIsSet = NStr("en='Set';ru='Установлен';vi='Đã cài đặt'"));
	Items.DeviceTest.Enabled = (NOT DriverIsSet = NStr("en='Not set';ru='Не установлен';vi='Chưa thiết lập'")) 
