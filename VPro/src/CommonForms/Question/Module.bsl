	...
	Else
		If WaitingButtonName <> "" Then
			NewHeader = (
				...
				NStr("en='%1 sec left';ru='осталось %1 сек.';vi='còn lại %1 giây'"),
...
Function StandardSet(Buttons)
	...
	If Buttons = "DialogModeQuestion.YesNo" Then
		Result.Add("DialogReturnCode.Yes",  NStr("en='Yes';ru='Да';vi='Đồng ý'"));
		Result.Add("DialogReturnCode.No", NStr("en='No';ru='Нет';vi='Không'"));
	ElsIf Buttons = "QuestionDialogMode.YesNoCancel" Then
		Result.Add("DialogReturnCode.Yes",     NStr("en='Yes';ru='Да';vi='Đồng ý'"));
		Result.Add("DialogReturnCode.No",    NStr("en='No';ru='Нет';vi='Không'"));
		Result.Add("DialogReturnCode.Cancel", NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'"));
	ElsIf Buttons = "DialogModeQuestion.OK" Then
		Result.Add("DialogReturnCode.OK", NStr("en='OK';ru='Ок';vi='Ок'"));
	ElsIf Buttons = "QuestionDialogMode.OKCancel" Then
		Result.Add("DialogReturnCode.OK",     NStr("en='OK';ru='Ок';vi='Ок'"));
		Result.Add("DialogReturnCode.Cancel", NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'"));
	ElsIf Buttons = "QuestionDialogMode.RetryCancel" Then
		Result.Add("ReturnDialogCode.Retry", NStr("en='Repeat';ru='Повторить';vi='Lặp lại'"));
		Result.Add("DialogReturnCode.Cancel",    NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'"));
	ElsIf Buttons = "QuestionDialogMode.AbortRetryIgnore" Then
		Result.Add("DialogReturnCode.Abort",   NStr("en='Abort';ru='Прервать';vi='Ngừng'"));
		Result.Add("ReturnDialogCode.Retry",  NStr("en='Repeat';ru='Повторить';vi='Lặp lại'"));
		Result.Add("DialogReturnCode.Ignore", NStr("en='Ignore';ru='Пропустить';vi='Bỏ qua'"));
	EndIf;
