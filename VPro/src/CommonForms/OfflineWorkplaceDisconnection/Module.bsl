...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(OfflineWorkplace) Then
		Raise NStr("en='An offline work place is not specified.';ru='Не задано автономное рабочее место.';vi='Chưa xác định chỗ làm việc độc lập.'");
	EndIf;
...
Procedure LongOperationIdleHandler()
	...
	Except
		...
		LongOperation = False;
		SkipBack();
		ShowMessageBox(,NStr("en='Errors occurred during the operation.';ru='В процессе работы возникли ошибки.';vi='Trong quá trình làm việc đã nảy sinh lỗi.'"));
...
Procedure DeleteOfflineWorkplace(Cancel)
	...
	Result = LongActions.ExecuteInBackground(
					...
					NStr("en='Deletion of offline work place';ru='Удаление автономного рабочего места';vi='Xóa chỗ làm việc độc lập'"));
