...
Function GetDocumentPresentationToPrint(Selection)
	...
	If (Selection.DocumentAmount <> 0) AND (Selection.DocumentAmount <> NULL) Then
		DocumentPresentation = DocumentPresentation + " " + NStr("en='to the amount of';ru='на сумму';vi='với số tiền'") + " " + Selection.DocumentAmount + " " + Selection.Currency + ".";
	EndIf;
...
Procedure RefreshTreeStructureOfSubjection()
	...
	Else
		CommonUseClientServer.MessageToUser(
			NStr("en='The document for which the dependence structure report is generated is no longer available.';ru='Документ, для которого сформирован отчет о структуре подчиненности, стал недоступен.';vi='Chứng từ mà lập báo cáo về cấu trúc trực thuộc không sử dụng được'"));
	EndIf;
...
Function PostDocumentAtServer(DocumentRef)
	...
	If DocumentRef.DeletionMark Then
		...
		Return NStr("en='Document marked for deletion cannot be posted.';ru='Помеченный на удаление документ не может быть проведен!';vi='Chứng từ đã đặt dấu xóa không thể kết chuyển được!'");
	...
	Except
		...
		ErrorText = NStr("en='Action (posting) cannot be performed as the %1 document is locked by the user.';ru='Действие (проведение) не может быть выполнено, так как документ %1 заблокирован пользователем!';vi='Thao tác (kết chuyển) không thể thực hiện, bởi vì chứng từ %1 đã bị người sử dụng phong tỏa!'");
...
Function UndoPostingDocumentAtServer(DocumentRef)
	...
	Except
		...
		ErrorText = NStr("en='Action (posting cancellation) cannot be performed as the %1 document is locked by the user.';ru='Действие (отмена проведения) не может быть выполнено, так как документ %1 заблокирован пользователем!';vi='Thao tác (hủy bỏ kết chuyển) không thể thực hiện, bởi vì chứng từ %1 đã bị người sử dụng phong tỏa!'");
		Return StringFunctionsClientServer.SubstituteParametersInString(ErrorText, DocumentRef);
...
Function SetDeletionMarkOfDocumentAtServer(DocumentRef)
	...
	Except
		...
		ErrorText = NStr("en='Action (set / clear deletion mark) cannot be performed as the %1 document is locked by the user.';ru='Действие (установить / снять пометку удаления) не может быть выполнено, так как документ %1 заблокирован пользователем!';vi='Thao tác (đặt/bỏ dấu xóa) không thể thực hiện, bởi vì chứng từ %1 đã bị người sử dụng phong tỏa!'");
		Return StringFunctionsClientServer.SubstituteParametersInString(ErrorText, DocumentRef);
...
Procedure PostDocument(Command)
	...
	If ValueIsFilled(CurrentDocument) Then
		...
		Except
			...
			ErrorText = NStr("en='Cannot post the %1 document.';ru='Не удалось провести документ %1!';vi='Không thể kết chuyển chứng từ %1!'");
...
Procedure UndoPostingDocument(Command)
	...
	If ValueIsFilled(CurrentDocument) Then
		...
		Except
			...
			ErrorText = NStr("en='Cannot cancel the %1 document posting.';ru='Не удалось отменить проведение документа %1!';vi='Không thể hủy bỏ kết chuyển chứng từ %1!'");;
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(ErrorText, CurrentDocument);
...
Procedure SetDeletionMarkDocument(Command)
	...
	If ValueIsFilled(CurrentDocument) Then
		...
		Except
			...
			ErrorText = NStr("en='Cannot set the deletion mark for document %1.';ru='Не удалось установить пометку удаления на документ %1!';vi='Không thể đặt dấu xóa cho chứng từ %1!'");
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(ErrorText, CurrentDocument);
	...
	If ValueIsFilled(CurrentDocument) Then
		...
		Try
			...
			If Form.IsOpen() Then
				...
				ErrorText = NStr("en='Document is already opened.';ru='Документ уже открыт!';vi='Chứng từ đã mở!'");
				CommonUseClientServer.MessageToUser(
		...
		Except
			...
			ErrorText = NStr("en='Failed to open the %1 document.';ru='Не удалось открыть документ %1!';vi='Không thể mở chứng từ %1!'");
			CommonUseClientServer.MessageToUser(
				StringFunctionsClientServer.SubstituteParametersInString(ErrorText, CurrentDocument)
				);
...
Procedure FindInList(Command)
	...
	Except
		...
		ErrorText = NStr("en='Cannot open the %1 document in the list.';ru='Не удалось открыть в списке документ %1!';vi='Không thể mở trong danh sách chứng từ %1!'");
		CommonUseClientServer.MessageToUser(
			StringFunctionsClientServer.SubstituteParametersInString(ErrorText, CurrentDocument)
			);
