...
Procedure CheckFillOfFormAttributes(Cancel)
	...
	For Each RowPrepayment IN FilteredAdvances Do
		...
		If Not ValueIsFilled(RowPrepayment.Document) Then
			...
			Message.Text = NStr("en='The ""Document"" column is not filled in the row ';ru='Не заполнена колонка ""Документ"" в строке ';vi='Chưa điền cột ""Chứng từ"" trong dòng'")
				+ String(LineNumber)
				+ NStr("en=' of the ""Filtered advances"" list.';ru=' списка ""Отобранные авансы"".';vi=' danh sách ""Ứng trước được chọn"".'");
		...
		If Not ValueIsFilled(RowPrepayment.Amount) Then
			Message = New UserMessage();
			Message.Text = NStr("en='The ""Amount"" column is not filled in the row ';ru='Не заполнена колонка ""Сумма"" в строке ';vi='Chưa điền cột ""Thành tiền"" trong dòng'")
				+ String(LineNumber)
				+ NStr("en=' of the ""Filtered advances"" list.';ru=' списка ""Отобранные авансы"".';vi=' danh sách ""Ứng trước được chọn"".'");
			Message.Field = "Document";
			Message.Message();
