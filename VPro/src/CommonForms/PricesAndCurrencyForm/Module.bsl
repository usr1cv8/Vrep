...
Procedure CheckFillOfFormAttributes(Cancel, OnlyPriceKindIsNotFilled = False)
	...
	If (RefillPrices OR RegisterVendorPrices) AND PriceKindCounterpartyIsAttribute Then
		If Not ValueIsFilled(CounterpartyPriceKind) Then
			...
			Message.Text = NStr("en='Counterparty price kind for population is not selected.';ru='Не выбран вид цен контрагента для заполнения!';vi='Chưa chọn dạng giá của đối tác để điền!'");
	...
	If DocumentCurrencyIsAttribute Then
		If Not ValueIsFilled(DocumentCurrency) Then
            Message = New UserMessage();
			Message.Text = NStr("en='Document currency is not filled in.';ru='Не заполнена валюта документа!';vi='Chưa điền tiền tệ chứng từ!'");
	...
	If VATTaxationIsAttribute Then
		If Not ValueIsFilled(VATTaxation) Then
            Message = New UserMessage();
			Message.Text = NStr("en='Taxation is not filled in.';ru='Не заполнено налогообложение!';vi='Chưa điền thuế!'");
	...
	If ContractIsAttribute Then
		If Not ValueIsFilled(PaymentsRate) Then
			Message = New UserMessage();
			Message.Text = NStr("en='Settlements currency exchange rate is zero.';ru='Обнаружен нулевой курс валюты расчетов!';vi='Tìm thấy tỷ giá ngoại tệ hạch toán bằng 0!'");
		...
		If Not ValueIsFilled(SettlementsMultiplicity) Then
			Message = New UserMessage();
			Message.Text = NStr("en='Document currency rate conversion factor is zero.';ru='Обнаружена нулевая кратность курса валюты документа!';vi='Tìm thấy bội số tỷ giá hối đoái bằng 0 trong chứng từ!'");
	...
	If RefillPrices AND PriceKindIsAttribute Then
		If Not ValueIsFilled(PriceKind) Then
			...
			Else
				Message = New UserMessage();
				Message.Text = NStr("en='Price kind for population is not selected.';ru='Не выбран вид цены для заполнения!';vi='Chưa chọn dạng giá để điền!'");
...
Procedure CommandOK(Command)
	...
	If Not Cancel OR OnlyPriceKindIsNotFilledAndCardIsFilled Then
		...
		// DiscountCards
		StructureOfFormAttributes.Insert("RefillDiscounts",			RefillPrices Or (DiscountKindOnOpen <> DiscountKind));
...
Procedure ConfigureLabelOnDiscountCard()
	...
	If Not DiscountCard.IsEmpty() Then
		If Not Counterparty.IsEmpty() AND DiscountCard.Owner.ThisIsMembershipCard AND DiscountCard.CardOwner <> Counterparty Then
			...
			Message = New UserMessage;
			Message.Text = NStr("en='Discount card owner does not match with a counterparty in the document.';vi='Chủ sở hữu thẻ chiết khấu không khớp với đối tác trong chứng từ.'");
	...
	Else
		...
		Items.DiscountCard.ToolTip = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Discount by the card is %1%';ru='Скидка по карте составляет %1%';vi='Chiết khấu theo thẻ là %1%'"), DiscountPercentByDiscountCard);
