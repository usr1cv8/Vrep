...
"доступа не настраиваются для объектов типа ""%1"".';vi='Không thể"
"tùy chỉnh quyền truy cập đối với đối tượng kiểu"
"""%1"".'"),
...
Procedure Reread(Command)
	...
	Else
		ShowQueryBox(
			...
			NStr("en='Data is changed. Read without saving?';ru='Данные изменены. Прочитать без сохранения?';vi='Dữ liệu đã thay đổi. Đọc mà không lưu lại?'"),
...
Procedure WriteBegin(Close = False)
	...
	If ConfirmCancelRightsManagement = True Then
		...
		Buttons.Add("WriteAndClose", NStr("en='Save and close';ru='Сохранить и закрыть';vi='Lưu và đóng'"));
		Buttons.Add("Cancel", NStr("en='Cancel';ru='Отменить';vi='Hủy bỏ'"));
		ShowQueryBox(
			New NotifyDescription("WriteAfterConfirmation", ThisObject),
			NStr("en='After writing, you cannot assign access rights.';ru='После записи настройка прав станет недоступной.';vi='Sau khi ghi lại, tùy chỉnh quyền sẽ không khả dụng.'"),
			Buttons,, "Cancel");
	Else
		If Close Then
			Close();
...
Procedure AddAttributesOrFormItems(NewAttributes = Undefined)
	...
	If NewAttributes = Undefined AND Parameters.ObjectReference.Metadata().Hierarchical Then
		...
		Item.Title = NStr("en='For subfolders';ru='Для подпапок';vi='Đối với thư mục con'");
		Item.ToolTip = NStr("en='Rights not only to the current folder, but also to its subfolders';ru='Права не только для текущей папки, но и для ее нижестоящих папок';vi='Quyền không chỉ đối với thư mục hiện tại mà còn đối với thư mục lớp dưới'");
		
		Item = AddItem("RightGroupsSettingOwner", Type("FormField"), Items.RightGroups);
		Item.ReadOnly = True;
		Item.DataPath    = "RulesGroups.SettingOwner";
		Item.Title = NStr("en='Inherited from';ru='Наследуется от';vi='Được kế thừa từ'");
		Item.ToolTip = NStr("en='Folder from which access rights settings are inherited';ru='Папка, от которой наследуются настройка прав';vi='Thư mục mà từ đó có kế thừa tùy chỉnh quyền'");
	...
	While Not Cancel AND LineNumber >= 0 Do
		...
		If NoFilledRight Then
			CommonUseClientServer.MessageToUser(
				NStr("en='No access right is specified.';ru='Не заполнено ни одно право доступа.';vi='Chưa điền quyền truy cập nào.'"),
				,
		...
		If Not ValueIsFilled(CurrentRow["User"]) Then
			CommonUseClientServer.MessageToUser(
				NStr("en='User or group is not entered.';ru='Не заполнен пользователь или группа.';vi='Chưa điền người sử dụng hoặc nhóm.'"),
				,
		...
		If RightGroups.FindRows(Filter).Count() > 1 Then
			If TypeOf(Filter.User) = Type("CatalogRef.Users") Then
				MessageText = NStr("en='Setting for user group ""%1"" already exists.';ru='Настройка для группы пользователей ""%1"" уже есть.';vi='Đã có tùy chỉnh đối với nhóm người sử dụng ""%1"".'");
			Else
				MessageText = NStr("en='Setting for user group ""%1"" already exists.';ru='Настройка для группы пользователей ""%1"" уже есть.';vi='Đã có tùy chỉnh đối với nhóm người sử dụng ""%1"".'");
			EndIf;
			CommonUseClientServer.MessageToUser(
...
"в форме настройки прав вышестоящей папки ""%1"".';vi='Các quyền này đã được kế thừa, có thể thay đổi những quyền đó"
"trong biểu mẫu tùy chỉnh quyền thư mục lớp trên ""%1"".'"),
			CurrentSettingOwner);
...
"прав следует снять флажок ""%1"".';vi='Để xóa tất cả các quyền"
"kế thừa, cần bỏ dấu hộp kiểm ""%1"".'"),
...
Procedure CheckPermissionToManageRights()
	...
	Raise NStr("en='Right setting is not available.';ru='Настройка прав недоступна.';vi='Không thể tùy chỉnh quyền.'");
