...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	If IsBlankString(AddressInStorage) AND SelectedRecipients.GetItems().Count() > 0 Then
		...
		QuestionText = NStr("en='Transfer marked recipients?';ru='Перенести отмеченных адресатов?';vi='Chuyển vào người nhận đã đánh dấu?'");
		
		Notification = New NotifyDescription("BeforeCloseingTransferProposed", ThisForm);
		ShowQueryBox(Notification, QuestionText, QuestionDialogMode.YesNoCancel, , DialogReturnCode.Yes, NStr("en='Address book';ru='Адресная книга';vi='Sổ địa chỉ'"));
	...
	If DoNotShowGroupSelectionQuestion = False Then
		For Each SelectedRow IN Items.CounterpartiesList.SelectedRows Do
			...
			If SelectedRow.IsEmpty() Or Data.IsFolder Then
				...
				QuestionParameters.Insert("Title", NStr("en='Confirm group selection';ru='Подтверждение выбора группы';vi='Xác nhận chọn nhóm'"));
				QuestionParameters.Insert("MessageText", NStr("en='Select all counterparties of the group?';ru='Выбрать всех контрагентов группы?';vi='Chọn tất cả đối ứng của nhóm?'"));
