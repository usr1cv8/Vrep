...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If PickMode Then
		Title = NStr("en='Select exchange plan nodes';ru='Подбор узлов планов обмена';vi='Chọn nút sơ đồ trao đổi'");
	EndIf;
	If Multiselect Then
...
Procedure Select(Command)
	...
	Else
		...
		If CurrentData = Undefined Then
			ShowMessageBox(, NStr("en='Node is not selected.';ru='Узел не выбран.';vi='Nút chưa chọn.'"));
		Else
...
Procedure AddUsedExchangePlan(MetadataObject)
	...
	If Parameters.ChooseAllNodes Then
		...
		NewRow.NodePresentation       = NStr("en='<All infobases>';ru='<Все информационные базы>';vi='<Tất cả cơ sở thông tin>'");
	EndIf;
