...
Procedure EnterInDataAreaAfterExit()
	...
	If Not IsFilledDataArea(DataArea) Then
		...
		ShowQueryBox(NOTifyDescription, NStr("en='The selected data area is not used, continue logon?';ru='Выбранная область данных не используется, продолжить вход?';vi='Vùng dữ liệu đã chọn không được sử dụng, tiếp tục đăng nhập?'"),
