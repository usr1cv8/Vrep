...
Procedure DeviceTest(Command)
	...
	AdditionalDetails = ?(TypeOf(Output_Parameters) = Type("Array")
	                           ...
	                           NStr("en='Additional description:';ru='Дополнительное описание:';vi='Bản mô tả bổ sung:'") + " " + Output_Parameters[1],
	                           "");
	
	If Result Then
		MessageText = NStr("en='Test completed successfully. %AdditionalDetails%%Linefeed%';ru='Тест успешно выполнен.%ПереводСтроки%%ДополнительноеОписание%';vi='Thử nghiệm đã thực hiện thành công.%Linefeed%%AdditionalDetails%'");
	...
	Else
		MessageText = NStr("en='Test failed.%Linefeed% %AdditionalDetails%';ru='Тест не пройден.%ПереводСтроки%%ДополнительноеОписание%';vi='Thử nghiệm không thực hiện được.%Linefeed%%AdditionalDetails%'");
		MessageText = StrReplace(MessageText, "%Linefeed%", ?(IsBlankString(AdditionalDetails),
		                                                                  "",
		                                                                  Chars.LF));
		MessageText = StrReplace(MessageText, "%AdditionalDetails%", ?(IsBlankString(AdditionalDetails),
...
Procedure UpdateInformationAboutDriver()
	...
	Else
		...
		Version  = NStr("en='Not defined';ru='Не определена';vi='Không xác định'");
	EndIf;

	Items.Driver.TextColor  = ?(Driver = NStr("en='Not set';ru='Не установлен';vi='Chưa thiết lập'"), ErrorColor, TextColor);
	Items.Version.TextColor = ?(Version = NStr("en='Not defined';ru='Не определена';vi='Không xác định'"), ErrorColor, TextColor);

	Items.SetupDriver.Enabled = Not (Driver = NStr("en='Installed';ru='Установлен';vi='Đã cài đặt'"));
