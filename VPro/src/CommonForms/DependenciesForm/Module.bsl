...
Procedure OpenDocumentForm()
	...
	Try
		...
		If Items.DocumentsTree.CurrentData.Ref = DocumentRef AND Form.IsOpen() Then
			...
			Message.Text = NStr("en='Document is already opened.';ru='Документ уже открыт!';vi='Chứng từ đã mở!'");
			Message.Message();
		EndIf;
...
Function PostServer(DocumentRef)
	...
	Except
		Message = New UserMessage();
		Message.Text = NStr("en='Cannot lock the document.';ru='Невозможно заблокировать документ.';vi='Không thể phong tỏa chứng từ.'");
		Message.Message();
	EndTry; 
...
"отчет о структуре подчиненности был удален, или же стал недоступен.';vi='Chứng từ mà theo đó đã lập"
"báo cáo về cấu trúc trực thuộc đã bị xóa hoặc không có hiệu lực.'"));
	EndIf;		
...
Procedure Post(Button)
	...
	Except
		...
		Message = New UserMessage();
		Message.Text = NStr("en='Cannot post the %Document% document.';ru='Не удалось провести документ %Document%!';vi='Không thể kết chuyển chứng từ %Document%!'");;
...
Procedure UndoPosting(Button)
	...
	Except
		...
		Message = New UserMessage();
		Message.Text = NStr("en='Cannot unpost the %Document% document.';ru='Не удалось сделать непроведенным документ %Документ%!';vi='Không thể làm chứng từ trở thành chưa kết chuyển %Document%!'");;
		Message.Text = StrReplace(Message.Text, "%Document%", Items.DocumentsTree.CurrentData.DocumentPresentation);
		Message.Message();
...
Procedure SetDeletionMark(Button)
	...
	Except
		...
		Message = New UserMessage();
		Message.Text = NStr("en='Cannot mark the %Document% document for deletion.';ru='Не удалось установить пометку удаления на документ %Document%!';vi='Không thể đặt dấu xóa chứng từ %Document%!'");;
