...
Procedure InventoryTableCCDNumberStartChoice(Item, ChoiceData, StandardProcessing)
	...
	If Not Items.InventoryTable.CurrentData.IsInventoryItem Then
		...
		MessageText = NStr("en='You can specify the CCD number only for the products and services with the <Inventory> type.';ru='Указать номер ГТД возможно только для номенклатуры с типом <Запас>.';vi='Chỉ có thể chỉ ra số tờ khai hải quan đối với mặt hàng có kiểu <VậtTư>.'");
...
"Продолжить?';vi='Đã chọn tờ khai hải quan có ngày đăng ký trước ngày trên chứng từ."
"Tiếp tục?'");
...
Procedure InventoryTableCountryOfOriginStartChoice(Item, ChoiceData, StandardProcessing)
	...
	If Not Items.InventoryTable.CurrentData.IsInventoryItem Then
		...
		MessageText = NStr("en='You can specify the country of origin only for the products and services of the <Inventory> type.';ru='Указать страну происхождения возможно только для номенклатуры с типом <Запас>.';vi='Chỉ có thể chỉ ra nước xuất xứ đối với mặt hàng có kiểu <VậtTư>.'");
		CommonUseClientServer.MessageToUser(MessageText);
...
Procedure SetMarkForRowsWithSameCountry(Command)
	...
	If CurrentStringData = Undefined Then
		...
		MessageText = NStr("en='Highlight a row of the tabular section and click again...';ru='Выделите строку табличной части и повторите нажатие...';vi='Hãy chọn dòng của phần bảng và nhấn lại...'");
		CommonUseClientServer.MessageToUser(MessageText);
		Return;
