...
Procedure SetActivePage()
	...
	ThisForm.Title = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Company information filling wizard (Step %1/%2)';ru='Помощник заполнения информации об организации (Шаг %1/%2)';vi='Trợ giúp điền thông tin về doanh nghiệp (Bước %1/%2)'"),
...
Procedure ExecuteActionsOnTransitionToNextPage(Cancel)
	...
	If mCurrentPageNumber = 2 Then
		...
		If Not ValueIsFilled(Company.Description) Then
			MessageText = NStr("en='Specify short name.';ru='Укажите краткое наименование.';vi='Hãy chỉ ra tên gọi ngắn gọn.'");
		...
		AND Not ValueIsFilled(Individual.Description) Then
			MessageText = NStr("en='Specify the full name.';ru='Укажите ФИО.';vi='Hãy chỉ ra họ tên'");
			CommonUseClientServer.MessageToUser(
				MessageText,
				,
				"Description",
...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	If Not mFormRecordCompleted
		...
		ShowQueryBox(NOTifyDescription, NStr("en='Save changes?';ru='Сохранить изменения?';vi='Lưu lại thay đổi?'"), QuestionDialogMode.YesNoCancel);
