...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	QuestionText = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Save changes in %1?';ru='Сохранить изменения в %1?';vi='Lưu lại thay đổi trong %1?'"), DocumentName);
...
Procedure StartSaveFileDialog(Val EndProcessor)
	...
	SaveFileDialog.Filter = NStr("en='Spreadsheet document';ru='Табличный документ';vi='Văn bản dạng bảng'") + " (*.mxl)|*.mxl";
...
Function NewDocumentName()
	Return NStr("en='New';ru='Новый';vi='Mới'");
EndFunction
...
Procedure SetTitle()
	...
	Title = DocumentName;
	If IsNew() Then
		Title = Title + " (" + NStr("en='create';ru='создать';vi='tạo mới'") + ")";
	ElsIf EditProhibited Then
		Title = Title + " (" + NStr("en='view only';ru='только просмотр';vi='chỉ để xem'") + ")";
	EndIf;
