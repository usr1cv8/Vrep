...
Procedure SetExplanationText()
	...
	If mCurrentPageNumber = 0 Then
		Items.DecorationNextActionExplanation.Title = NStr("en='Click Next to fill cash funds balance';ru='Нажмите кнопку ""Далее"" для заполнения остатков денежных средств';vi='Nhấn nút Tiếp theo để điền số dư tiền mặt, tiền gửi'");
	ElsIf mCurrentPageNumber = 1 Then
		Items.DecorationNextActionExplanation.Title = NStr("en='Click Next to fill balances of products';ru='Нажмите кнопку ""Далее"" для заполнения остатков товаров';vi='Nhấn nút Tiếp theo để điền số dư về hàng hóa'");
	ElsIf mCurrentPageNumber = 2 Then
		Items.DecorationNextActionExplanation.Title = NStr("en='Click Next to fill balance by acounts payable';ru='Нажмите кнопку ""Далее"" для заполнения остатков по расчетам с поставщиками';vi='Nhấn nút Tiếp theo để điền công nợ phải trả'");
	ElsIf mCurrentPageNumber = 3 Then
		Items.DecorationNextActionExplanation.Title = NStr("en='Click Next to fill balance by acounts receivable';ru='Нажмите кнопку ""Далее"" для заполнения остатков по расчетам с покупателями';vi='Nhấn nút Tiếp theo để điền số dư công nợ phải thu'");
	ElsIf mCurrentPageNumber = 4 Then
		Items.DecorationNextActionExplanation.Title = NStr("en='Press ""Next"" to proceed to the final step';ru='Нажмите кнопку ""Далее"" для перехода к итоговому шагу';vi='Nhấn ""Tiếp theo"" để chuyển đến bước cuối'");
	ElsIf mCurrentPageNumber = 5 Then
		Items.DecorationNextActionExplanation.Title = NStr("en='To complete, it is required to click Finish';ru='Для завершения необходимо нажать кнопку ""Закончить""';vi='Để hoàn tất, cần nhấn nút Kết thúc'");
	EndIf;
...
Procedure SetActivePage()
	...
	ThisForm.Title = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Opening balance input wizard (Step %1/%2)';ru='Помощник ввода начальных остатков (Шаг %1/%2)';vi='Trợ giúp nhập số dư ban đầu (Bước %1/%2)'"),
...
Procedure SetButtonsEnabled()
	...
	If mCurrentPageNumber = mLastPage Then
		Items.GoToNext.Title = NStr("en='Finish';ru='Закончить';vi='Kết thúc'");
		Items.GoToNext.Representation = ButtonRepresentation.Text;
		Items.GoToNext.Font = New Font(Items.GoToNext.Font,,,True);
	Else
		Items.GoToNext.Title = NStr("en='Next';ru='Далее';vi='Tiếp theo'");
...
Function AddProductsAndServicesAtServer(ProductsAndServices, UseBatches, UseCharacteristics)
	...
	Else
		...
		NewProductsAndServices.ProductsAndServicesCategory = Catalogs.ProductsAndServicesCategories.WithoutCategory;
...
Procedure ExecuteActionsOnTransitionToNextPage(Cancel)
	...
	If mCurrentPageNumber = 1 Then
		...
		If Not ValueIsFilled(AccountingCurrency) Then
			MessageText = NStr("en='Specify accounting currency.';ru='Укажите валюту учета.';vi='Hãy chỉ ra tiền tệ kế toán.'");
		...
		If Not ValueIsFilled(NationalCurrency) Then
			MessageText = NStr("en='Specify national currency.';ru='Укажите национальную валюту.';vi='Hãy chỉ ra tiền tệ quốc gia.'");
			CommonUseClientServer.MessageToUser(
				MessageText,
				,
				"NationalCurrency",
		...
		For Each CurRow IN EnterOpeningBalanceBankAndPettyCash.CashAssets Do
			If Not ValueIsFilled(CurRow.BankAccountPettyCash) Then
				MessageText = NStr("en='Specify banking account or petty cash in the line ';ru='Укажите банковский счет или кассу в строке ';vi='Hãy chỉ ra tài khoản ngân hàng hoặc tiền mặt trong dòng'") + CurRow.LineNumber + ".";
				CommonUseClientServer.MessageToUser(
					MessageText,
					,
			...
			If Not ValueIsFilled(CurRow.CashCurrency) Then
				MessageText = NStr("en='Specify currency in the line ';ru='Укажите валюту в строке ';vi='Hãy chỉ ra ngoại tệ trong dòng'") + CurRow.LineNumber + ".";
				CommonUseClientServer.MessageToUser(
					MessageText,
					,
					"EnterOpeningBalanceBankAndPettyCash",
			...
			If Not ValueIsFilled(CurRow.AmountCur) Then
				MessageText = NStr("en='Specify amount in the line ';ru='Укажите сумму в строке ';vi='Hãy chỉ ra số tiền trong dòng'") + CurRow.LineNumber + ".";
				CommonUseClientServer.MessageToUser(
					MessageText,
					,
					"EnterOpeningBalanceBankAndPettyCash",
			...
			AND Not ValueIsFilled(CurRow.Amount) Then
				MessageText = NStr("en='Specify amount by accounting currency in the line ';ru='Укажите сумму в валюте учета в строке ';vi='Hãy chỉ ra số tiền theo tiền tệ kế toán trong dòng'") + CurRow.LineNumber + ".";
				CommonUseClientServer.MessageToUser(
					MessageText,
					,
					"EnterOpeningBalanceBankAndPettyCash",
	...
	ElsIf mCurrentPageNumber = 2 Then
		...
		For Each CurRow IN EnterOpeningBalanceProducts.Inventory Do
			If Not ValueIsFilled(CurRow.StructuralUnit) Then
				MessageText = NStr("en='Specify structural unit in the line ';ru='Укажите структурную единицу в строке ';vi='Hãy chỉ ra kho bãi trong dòng'") + CurRow.LineNumber + ".";
				CommonUseClientServer.MessageToUser(
					MessageText,
					,
			...
			If Not ValueIsFilled(CurRow.ProductsAndServices) Then
				MessageText = NStr("en='Specify item of list of goods in the line ';ru='Укажите номенклатуру в строке ';vi='Hãy chỉ ra mặt hàng trong dòng'") + CurRow.LineNumber + ".";
				CommonUseClientServer.MessageToUser(
					MessageText,
					,
					"EnterOpeningBalanceProducts",
			...
			If Not ValueIsFilled(CurRow.MeasurementUnit) Then
				MessageText = NStr("en='Specify unit of measurement in the line ';ru='Укажите единицу измерения в строке ';vi='Hãy chỉ ra đơn vị tính trong dòng'") + CurRow.LineNumber + ".";
				CommonUseClientServer.MessageToUser(
					MessageText,
					,
					"EnterOpeningBalanceProducts",
			...
			If Not ValueIsFilled(CurRow.Quantity) Then
				MessageText = NStr("en='Specify quantity in the line ';ru='Укажите количество в строке ';vi='Hãy chỉ ra số lượng trong dòng'") + CurRow.LineNumber + ".";
				CommonUseClientServer.MessageToUser(
					MessageText,
					,
					"EnterOpeningBalanceProducts",
	...
	ElsIf mCurrentPageNumber = 3 Then
		...
		For Each CurRow IN EnterOpeningBalanceCounterpartiesSettlements.AccountsPayable Do
			If Not ValueIsFilled(CurRow.Counterparty) Then
				MessageText = NStr("en='Specify counterparty in the line ';ru='Укажите контрагента в строке ';vi='Hãy chỉ ra đối tác trong dòng'") + CurRow.LineNumber + ".";
				CommonUseClientServer.MessageToUser(
					MessageText,
					,
			...
			If Not ValueIsFilled(CurRow.Contract) Then
				MessageText = NStr("en='Specify contract in the line ';ru='Укажите договор в строке ';vi='Hãy chỉ ra hợp đồng trong dòng'") + CurRow.LineNumber + ".";
				CommonUseClientServer.MessageToUser(
					MessageText,
					,
					"EnterOpeningBalanceCounterpartiesSettlements",
...
Procedure BeforeClose(Cancel, StandardProcessing)
	...
	If Not mFormRecordCompleted
		...
		Text = NStr("en='Save changes?';ru='Сохранить внесенные изменения?';vi='Lưu các thay đổi?'");
...
Procedure AddProductsAndServices(Command)
	...
	If Not ValueIsFilled(ProductsAndServices) Then
		MessageText = NStr("en='First, fill in products and services name.';ru='Вначале заполните наименование номенклатуры!';vi='Ban đầu, hãy điền tên gọi mặt hàng!'");
...
Procedure DocumentsListEnterOpeningBalance(Command)
	...
	If Modified Then
		Text = NStr("en='All entered data will be saved. Continue?';ru='Все введенные данные будут сохранены. Продолжить?';vi='Sẽ lưu lại tất cả dữ liệu đã nhập."
"Tiếp tục?'");
...
Procedure EnterOpeningBalanceBankAndPettyCashCashAssetsCurrencyOfCashAssetsStartChoice(Item, ChoiceData, StandardProcessing)
	...
	If TypeOf(TabularSectionRow.BankAccountPettyCash) = Type("CatalogRef.BankAccounts") Then
		ShowMessageBox(Undefined,NStr("en='Cannot change the cash currency of the bank account.';ru='Для банковского счета нельзя изменить валюту денежных средств!';vi='Đối với tài khoản ngân hàng, không được phép thay đổi tiền tệ!'"));
