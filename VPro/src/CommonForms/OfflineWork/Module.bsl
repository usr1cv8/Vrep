...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not OfflineWorkService.ThisIsOfflineWorkplace() Then
		Raise NStr("en='This infobase is not an offline work place.';ru='Эта информационная база не является автономным рабочим местом.';vi='Cơ sở thông tin này không phải là chỗ làm việc độc lập.'");
	EndIf;
...
"Необходимо установить полученное обновление, после чего синхронизация будет продолжена.';vi='Nhận cập nhật chương trình từ Internet."
"Cần thiết lập cập nhật nhận được, sau đó sẽ tiếp tục đồng bộ hóa.'");
	Else
		Items.InformationLabelReceivedRefreshEnabled.Title = NStr("en='Software update has been exported from the Internet."
"Contact the infobase administrator to install the received update.';ru='Получено обновление программы из Интернета."
"Обратитесь к администратору информационной базы для установки полученного обновления.';vi='Đã nhận bản cập nhật chương trình từ Internet."
"Hãy liên hệ với người quản trị cơ sở thông tin để cài đặt bản cập nhật đã nhận.'");
	EndIf;
...
Procedure OnChangeScheduleDataSynchronization()
	...
	Items.DataSynchronizationScheduleVariant.ChoiceList.Add(1, NStr("en='Every day at 10:00 a.m., except for Saturdays and Sundays';ru='Каждый день в 10:00, кроме сб. и вс.';vi='Hàng ngày vào lúc 10:00, ngoại trừ thứ 7 và chủ nhật'"));
	Items.DataSynchronizationScheduleVariant.ChoiceList.Add(2, NStr("en='Every hour';ru='Каждый час';vi='Hàng giờ'"));
	Items.DataSynchronizationScheduleVariant.ChoiceList.Add(3, NStr("en='Every day at 10:00 a.m., except for Saturdays and Sundays';ru='Каждый день в 10:00, кроме сб. и вс.';vi='Hàng ngày vào lúc 10:00, ngoại trừ thứ 7 và chủ nhật'"));
