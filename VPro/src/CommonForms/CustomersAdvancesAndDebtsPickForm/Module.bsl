...
Procedure CheckFillOfFormAttributes(Cancel)
	...
	For Each RowListFilteredAdvancesAndDebts IN ListFilteredAdvancesAndDebts Do
		...
		AND Not ValueIsFilled(RowListFilteredAdvancesAndDebts.ExchangeRate) Then
			...
			Message.Text = NStr("en='Column ""Exchange rate"" is not filled in the row. ';ru='Не заполнена колонка ""Курс"" в строке ';vi='Chưa điền cột ""Tỷ giá"" trong dòng'")
				+ String(LineNumber)
				+ NStr("en=' ""Filtered Advances and debts"" list.';ru=' списка ""Отобранные авансы и долги"".';vi=' danh sách ""Ứng trước và công nợ được chọn"".'");
		...
		AND Not ValueIsFilled(RowListFilteredAdvancesAndDebts.Multiplicity) Then
			Message = New UserMessage();
			Message.Text = NStr("en='Column ""Multiplicity"" is not filled in the row. ';ru='Не заполнена колонка ""Кратность"" в строке ';vi='Chưa điền cột ""Bội số"" trong dòng'")
				+ String(LineNumber)
				+ NStr("en=' ""Filtered Advances and debts"" list.';ru=' списка ""Отобранные авансы и долги"".';vi=' danh sách ""Ứng trước và công nợ được chọn"".'");
			Message.Field = "Document";
			Message.Message();
		...
		If Not ValueIsFilled(RowListFilteredAdvancesAndDebts.SettlementsAmount) Then
			Message = New UserMessage();
			Message.Text = NStr("en='Field ""Settlements amount"" is not filled in row ';ru='Не заполнена колонка ""Сумма расчетов"" в строке ';vi='Chưa điền cột ""Số tiền tính toán"" trong dòng'")
				+ String(LineNumber)
				+ NStr("en=' ""Filtered Advances and debts"" list.';ru=' списка ""Отобранные авансы и долги"".';vi=' danh sách ""Ứng trước và công nợ được chọn"".'");
			Message.Field = "Document";
			Message.Message();
		...
		AND Not ValueIsFilled(RowListFilteredAdvancesAndDebts.AccountingAmount) Then
			Message = New UserMessage();
			Message.Text = NStr("en='No filled column ""Accounting Amount"" in row. ';ru='Не заполнена колонка ""Сумма учета"" в строке ';vi='Chưa điền cột ""Số tiền kế toán"" trong dòng'")
				+ String(LineNumber)
				+ NStr("en=' ""Filtered advances and debts"" list.';ru=' списка ""Отобранные авансы и долги"".';vi=' danh sách ""Ứng trước và công nợ được chọn"".'");
			Message.Field = "Document";
			Message.Message();
...
Procedure AddRowIntoFiltered(CurrentRow)
	...
	If AskAmount Then
		...
		ShowInputNumber(NOTifyDescription, SettlementsAmount, NStr("en='Enter settlements amount';ru='Введите сумму расчетов';vi='Hãy nhập số tiền hạch toán'"));
