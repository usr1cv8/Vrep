	...
	For Each StrAddress IN ExceptionsAddresses Do
		If Not IsBlankString(StrAddress.ServerAddress) Then
			...
			If Not IsBlankString(ProhibitedChars) Then
				...
				MessageText = StrReplace(NStr("en='Address contains invalid characters: %1';ru='В адресе найдены недопустимые символы: %1';vi='Trong địa chỉ đã tìm thấy các ký tự không được phép: %1'"),
					"%1",
					ProhibitedChars);
