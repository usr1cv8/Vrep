		...
		If ReportVariantsCommandsVisible // Commands are not enabled by the form opening parameters.
			...
			If ValueIsFilled(PredefinedVariant) Then
				...
				If Disabled.Find(PredefinedVariant) <> Undefined Then // Option is disabled.
					Text = NStr("en='Context variant ""%1"" of the ""%2"" report is opened without context.';ru='Контекстный вариант ""%1"" отчета ""%2"" открыт без контекста.';vi='Phương án ngữ cảnh ""%1"" của báo cáo ""%2"" mở mà không cần ngữ cảnh'");
	...
	If TypeOf(ParametersForm.Filter) = Type("Structure") Then
		...
		For Each KeyAndValue In ParametersForm.Filter Do
			...
			If TypeOf(AvailableDCField) = Type("DataCompositionFilterAvailableField") Then
				...
				Continue;
			EndIf;
			ErrorText = NStr("en='Cannot set fixed filter ""%1"".';ru='Не удалось установить фиксированный отбор ""%1"".';vi='Không thể thiết lập phễu lọc cố định ""%1"".'");
	...
	For Each DCUsersSetting In DCUserSettings.Items Do
		...
		If Type = "SettingsParameterValue"
			...
			If BeginOfPeriod.AutoMarkIncomplete
				...
				ErrorText = NStr("en='Period is not specified';ru='Не указан период';vi='Chưa chỉ ra kỳ'");
				DataPath = BeginOfPeriod.DataPath;
			ElsIf Value.StartDate > Value.EndDate Then
				ErrorText = NStr("en='Period end should be later than its start';ru='Конец периода должен быть больше начала';vi='Cuối kỳ cần lớn hơn đầu kỳ'");
...
Procedure SendByEMail(Command)
	...
	If StatePresentation.Visible = True
		...
		QuestionText = NStr("en='Report not generated. Generate?';ru='Отчет не сформирован. Сформировать?';vi='Chưa lập báo cáo. Lập báo cáo?'");
...
Procedure Attachable_LoadReportVariant(Command)
	...
	If Found.Count() = 0 Then
		ShowMessageBox(, NStr("en='Report variant was not found.';ru='Вариант отчета не найден.';vi='Không tìm thấy phương án báo cáo.'"));
		Return;
	EndIf;
	FormVariant = Found[0];
...
Procedure BackgroundJobCheckAtClient()
	If BackGroundJobFinished() Then
		ShowUserNotification(NStr("en='Report is generated';ru='Отчет сформирован';vi='Báo cáo đã lập'"), , Title);
...
Procedure VisibleEnabledCorrectness(Changes = "")
	...
	EndIf;
	If EncryptingMode Then
		Title = Title + " (" + NStr("en='Explanation';ru='Расшифровка';vi='Diễn giải'") + ")";
	EndIf;
...
Function BackGroundJobStart()
	...
	Try
		BackgroundJobResult = LongActions.ExecuteInBackground(
			...
			NStr("en='Report variants: Report generation';ru='Варианты отчетов: Формирование отчета';vi='Phương án báo cáo: Lập báo cáo'"));
	Except
		ErrorInfo = ErrorInfo();
	EndTry;
	AdditProperties.Delete("VariantKey");
...
Procedure LoadVariant(VariantKey)
	...
	StatePresentation.Text     = NStr("en='Another report variant is selected. Click ""Generate"" to receive the report.';ru='Выбран другой вариант отчета. Нажмите ""Сформировать"" для получения отчета.';vi='Đã chọn phương án báo cáo khác. Hãy nhấn nút ""Lập báo cáo"" để nhận báo cáo.'");
EndProcedure
...
Procedure FormationErrorsShow(ErrorInfo)
	If TypeOf(ErrorInfo) = Type("ErrorInfo") Then
		...
		DetailErrorDescription = NStr("en='Generation error:';ru='Ошибка при формировании:';vi='Lỗi khi lập:'") + Chars.LF + DetailErrorDescription(ErrorInfo);
...
Procedure AfterChangingKeyStates(FillingParameters)
	If FillingParameters.Event.Name <> "AfterGenerating" Then
		...
		ElsIf FillingParameters.VariantModified
			...
			StatePresentation = Items.ReportSpreadsheetDocument.StatePresentation;
			StatePresentation.Visible = True;
			StatePresentation.Text     = NStr("en='Settings were changed. Click ""Generate"" to get the report.';ru='Изменились настройки. Нажмите ""Сформировать"" для получения отчета.';vi='Đã thay đổi tùy chỉnh. Nhấn nút ""Lập báo cáo"" để nhận được báo cáo.'");
