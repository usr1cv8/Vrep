...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not ValueIsFilled(Parameters.UserAccount) Then
		...
		If AvailableAccounts.Count() = 0 Then
			MessageText = NStr("en='Available email accounts are not found, contact the system administrator.';ru='Не обнаружены доступные учетные записи электронной почты, обратитесь к администратору системы.';vi='Không tìm thấy trương mục thư điện tử hợp lệ, hãy liên hệ với người quản trị hệ thống.'");
	...
	ElsIf TypeOf(Parameters.UserAccount) = Type("ValueList") Then
		...
		If AccountsSet.Count() = 0 Then
			MessageText = NStr("en='Email accounts are not specified, contact the system administrator.';ru='Не указаны учетные записи для отправки сообщения, обратитесь к администратору системы.';vi='Chưa chỉ ra trương mục thư điện tử để gửi thông báo, hãy liên hệ với người quản trị hệ thống.'");
			CommonUseClientServer.MessageToUser(MessageText,,,, Cancel);
			Return;
		EndIf;
...
Procedure ContinueSendingEmailsWithPassword()
	...
	If EmailParameters = Undefined Then
		CommonUseClientServer.MessageToUser(NStr("en='An error occurred while generating email message parameters';ru='Ошибка формирования параметров почтового сообщения';vi='Lỗi tạo tham số thông báo qua E-mail'"));
		Return;
	EndIf;
	
	SendE_Mail(UserAccount, EmailParameters);
	SaveReplyTo(ReplyTo);
	Status(NStr("en='Message is sent successfully';ru='Сообщение успешно отправлено';vi='Đã gửi thành công thông báo'"));
