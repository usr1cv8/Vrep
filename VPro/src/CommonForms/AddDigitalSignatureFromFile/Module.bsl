...
Procedure OK(Command)
	...
	If Signatures.Count() = 0 Then
		ShowMessageBox(, NStr("en='No signature file is selected';ru='Не выбрано ни одного файла подписи';vi='Chưa chọn bất kỳ tệp chữ ký nào'"));
		Return;
	EndIf;
...
Procedure OKEnd(Result, Context) Export
	...
	If Result.Property("ErrorDescription") Then
		...
		DigitalSignatureServiceClient.ShowRequestToApplicationError(
			NStr("en='Cannot add a digital signature from the file';ru='Не удалось добавить электронную подпись из файла';vi='Không thể thêm chữ ký điện tử từ tệp'"), "", Error, New Structure);
		Return;
	EndIf;
...
Procedure SelectFileAfterConnectionFileOperationsExtension(Attached, Context) Export
	...
	Dialog.Title = NStr("en='Select a digital signature file';ru='Выберите файл электронной подписи';vi='Hãy chọn tệp chữ ký điện tử'");
	Dialog.Filter = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Signature files (*.%1)|*.%1|All files(*.*)|*.*';ru='Файлы подписи (*.%1)|*.%1|Все файлы(*.*)|*.*';vi='Tệp chữ ký (*.%1)|*.%1|Tất cả các tệp(*.*)|*.*'"),
...
Procedure SelectFileAfterObtainingCertificatesFromSignaturesError(ErrorInfo, StandardProcessing, Context) Export
	...
	ErrorOnClient = New Structure("ErrorDescription", StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='An error occurred when receiving certificates from the signature file: %1';ru='При получении сертификатов из файла подписи произошла ошибка: %1';vi='Khi nhận chứng thư từ tệp chữ ký, đã xảy ra lỗi: %1'"),
...
Procedure SelectFileAfterReceivingCertificatesFromSignature(Certificates, Context) Export
	...
	If Certificates.Count() = 0 Then
		ErrorOnClient = New Structure("ErrorDescription",
			NStr("en='Signature file does not include any certificates.';ru='В файле подписи нет ни одного сертификата.';vi='Trong tệp chữ ký không có chứng thực nào.'"));
...
Function AddRowAtServer(Address, FileName, AddNewRow, ErrorOnServer, SignatureData)
	...
	If Certificates.Count() = 0 Then
		ErrorOnServer.Insert("ErrorDescription", NStr("en='Signature file does not include any certificates.';ru='В файле подписи нет ни одного сертификата.';vi='Trong tệp chữ ký không có chứng thực nào.'"));
		Return False;
	EndIf;
...
Procedure ShowError(ErrorOnClient, ErrorOnServer)
	...
	DigitalSignatureServiceClient.ShowRequestToApplicationError(
		NStr("en='Cannot receive a signature from the file';ru='Не удалось получить подпись из файла';vi='Không thể nhận chữ ký từ tệp'"), "", ErrorOnClient, ErrorOnServer);
