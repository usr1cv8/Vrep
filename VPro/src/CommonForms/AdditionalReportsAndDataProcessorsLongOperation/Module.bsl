...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Else
		SupportText = NStr("en='Executing the command...';ru='Команда выполняется...';vi='Đã thực hiện lệnh...'");
	EndIf;
	...
	Else
		Title = NStr("en='Please wait';ru='Пожалуйста, подождите';vi='Xin vui lòng đợi'");
	EndIf;
	...
	Try
		...
		AssignmentResult = LongActions.ExecuteInBackground(
			...
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Running additional report or data processor ""%1"", command name ""%2""';ru='Выполнение дополнительного отчета или обработки ""%1"", имя команды ""%2""';vi='Thực hiện báo cáo hoặc bộ xử lý bổ sung ""%1"", tên lệnh ""%2""'"),
...
Procedure Cancel(Command)
	QuestionText = NStr("en='Long action is still in process.';ru='Длительная операция еще выполняется.';vi='Còn đang thực hiện giao dịch trường kỳ.'");
	
	Buttons = New ValueList;
	Buttons.Add(1, NStr("en='Continue';ru='Продолжить';vi='Tiếp tục'"));
