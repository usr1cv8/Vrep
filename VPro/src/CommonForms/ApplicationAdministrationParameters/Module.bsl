...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CommonUse.FileInfobase() AND Parameters.QueryClusterAdministrationParameters Then
		Raise NStr("en='Setting of the server cluster parameters is available only in the client/server mode.';ru='Настройка параметров кластера серверов доступна только в клиент-серверном режиме работы.';vi='Có thể tùy chỉnh tham số cụm Server ở chế độ làm việc Client-server.'");
	EndIf;
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If CommonUseReUse.CanUseSeparatedData() Then
		...
		If IBUser = Undefined Then
			CommonUseClientServer.MessageToUser(NStr("en='The specified user does not have access to the infobase.';ru='Указанный пользователь не имеет доступа к информационной базе.';vi='Người sử dụng đã chọn không có quyền truy cập tới cơ sở thông tin.'"),,
				FieldName,,Cancel);
			Return;
		EndIf;
		
		If Not Users.InfobaseUserWithFullAccess(IBUser, True) Then
			CommonUseClientServer.MessageToUser(NStr("en='User does not have administrative rights.';ru='У пользователя нет административных прав.';vi='Ở người sử dụng không có quyền quản trị.'"),,
				FieldName,,Cancel);
			Return;
		EndIf;
