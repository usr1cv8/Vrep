...
"Передано: %1';vi='Sai giá trị tham số CommandParameter khi gọi ra phương thức PrintManagementClient.ExecutePrintCommand method."
"Dự kiến: Array, AnyRef."
"Đã chuyển: %1'"), TypeOf(Parameters.CommandParameter)));
	EndIf;
	...
	SetFormTitle();
	If IsSetPrint() Then
		Items.copies.Title = NStr("en='Set copies';ru='Копий комплекта';vi='Bản sao bộ cài đặt'");
	EndIf;
	...
	If Upper(ChoiceSource.FormName) = Upper("CommonForm.PrintFormSaving") Then
		...
		If ValueSelected <> Undefined AND ValueSelected <> DialogReturnCode.Cancel Then
			...
			Else
				...
				EndDo;
				Status(NStr("en='Saving completed successfully.';ru='Сохранение успешно завершено.';vi='Đã thực hiện lưu thành công.'"));
			EndIf;
		EndIf;
...
Procedure GoToDocument(Command)
	...
	ChoiceList.ShowChooseItem(NOTifyDescription, NStr("en='Go to print form';ru='Перейти к печатной форме';vi='Chuyển đến mẫu in '"));
	...
	For Each PrintFormSetting IN PrintFormsSettings Do
		If PrintFormSetting.Quantity <> 1 Then
			PrintFormSetting.Presentation = PrintFormSetting.Presentation 
				+ " (" + PrintFormSetting.Quantity + " " + NStr("en='copy';ru='экз.';vi='bản'") + ")";
		EndIf;
	EndDo;
EndProcedure
	...
	If IsAllowedOutput Then
		For Each PrintFormSetting IN PrintFormsSettings Do
			...
			If SpreadsheetDocumentField.Output = UseOutput.Disable Then
				PrintFormSetting.Presentation = PrintFormSetting.Presentation + " (" + NStr("en='print only';ru='только печать';vi='chỉ in'") + ")";
...
Procedure SetPrinterNameInPrintButtonToolTip()
	If PrintFormsSettings.Count() > 0 Then
		...
		If Not IsBlankString(PrinterName) Then
			ThisObject.Commands["Print"].ToolTip = NStr("en='Print using printer';ru='Напечатать на принтере';vi='In tại máy in'") + " (" + PrinterName + ")";
		EndIf;
	EndIf;
EndProcedure
...
Procedure SetFormTitle()
	...
	Else
		If IsSetPrint() Then
			Title = NStr("en='Document print';ru='Печать документов';vi='In chứng từ'");
		ElsIf TypeOf(Parameters.CommandParameter) <> Type("Array") Or Parameters.CommandParameter.Count() > 1 Then
			Title = NStr("en='Print document';ru='Печать документа';vi='In chứng từ'");
		Else
			Title = NStr("en='Print document';ru='Печать документа';vi='In chứng từ'");
		EndIf;
	EndIf;
...
Function DefaultPrintedFormFileName(PrintObject, PrintedFormName)
	...
	If CommonUse.ThisIsDocument(Metadata.FindByType(TypeOf(PrintObject))) Then
		...
		Pattern = NStr("en='[PrintedFormName] [Number] dated [Date]';ru='[PrintedFormName] [Number] от [Date]';vi='[PrintedFormName] [Number] từ [Date]'");
	Else
...
Function GetFileNameForArchive()
	...
	For Each PrintFormSetting IN PrintFormsSettings Do
		...
		Else
			Result = NStr("en='Documents';ru='Документы';vi='Chứng từ'");
			Break;
		EndIf;
	EndDo;
...
Procedure SavePrintFormsToFolder(FilesListInTempStorage, Val Folder = "")
	...
	Status(NStr("en='Saving completed successfully.';ru='Сохранение успешно завершено.';vi='Đã thực hiện lưu thành công.'"), , NStr("en='to folder:';ru='в папку:';vi='vào thư mục:'") + " " + Folder);
	...
	If CommonUse.SubsystemExists("StandardSubsystems.AttachedFiles") Then
		...
		For Each File IN FilesInTemporaryStorage Do
			Result.Add(ModuleAttachedFiles.AddFile(ObjectForAttaching, 
				File.Presentation, , , , File.AddressInTemporaryStorage, , NStr("en='Print form';ru='Печатная форма';vi='Mẫu in'")));
		EndDo;
	EndIf;
	Return Result;
EndFunction
...
Procedure OpenTemplateForEdit()
	...
	DisplayCurrentPrintFormState(NStr("en='The template is being edited';ru='Макет редактируется';vi='Khuôn in được soạn'"));
...
Procedure RegeneratePrintForm(TemplateName, AttributeName)
	...
	If Cancel Then
		Raise NStr("en='Print form was not regenerated.';ru='Печатная форма не была переформирована.';vi='Chưa tạo lại mẫu in.'");
	EndIf;
