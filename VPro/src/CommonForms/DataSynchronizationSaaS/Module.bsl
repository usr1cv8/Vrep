...
"В менеджере сервиса воспользуйтесь командой ""Синхронизация данных"".';vi='Để ngắt bỏ đồng bộ hóa dữ liệu, hãy chuyển đến quản lý dịch vụ. Trong quản lý dịch vụ, hãy sử dụng lệnh ""Đồng bộ hóa dữ liệu"".'"));
		Else
...
Procedure GoToNumberOnChange(Val IsGoNext)
	...
	If GoToRowsCurrent.Count() = 0 Then
		Raise NStr("en='Page for displaying is not defined.';ru='Не определена страница для отображения.';vi='Chưa xác định trang để hiển thị.'");
	EndIf;
...
"В менеджере сервиса воспользуйтесь командой ""Синхронизация данных"".';vi='Để tùy chỉnh đồng bộ hóa dữ liệu, hãy chuyển đến quản lý dịch vụ."
"Trong quản lý dịch vụ, hãy sử dụng lệnh ""Đồng bộ hóa dữ liệu"".'"));
		Else
...
Procedure RefreshDataSynchronizationSettings()
	...
	For Each Setting IN DataSynchronizationSettings Do
		...
		If Setting.SynchronizationSet Then
			...
			If SynchronizationStatus <> Undefined Then
				...
				If SynchronizationStatus.Status = 1 Then // Service administrator intervention is required
					...
					Setting.State = NStr("en='Errors occurred while synchronizing data';ru='Ошибки при синхронизации данных';vi='Lỗi khi đồng bộ hóa dữ liệu'");
					
				ElsIf SynchronizationStatus.Status = 2 Then // User can solve problems individually
					
					Setting.State = NStr("en='Data synchronization issues';ru='Проблемы при синхронизации данных';vi='Vấn đề khi đồng bộ hóa dữ liệu'");
					
					CountProblems = CountProblems + SynchronizationStatus.CountProblems;
					
				ElsIf SynchronizationStatus.Status = 3 Then
					
					Setting.State = NStr("en='Data synchronization is configured';ru='Синхронизация данных настроена';vi='Đã tùy chỉnh đồng bộ hóa dữ liệu'");
		...
		Else
			...
			Setting.Definition = NStr("en='Data synchronization is not configured';ru='Синхронизация данных не настроена';vi='Chưa tùy chỉnh đồng bộ hóa dữ liệu'");
		...
		For Each SettingFromServiceManager IN SynchronizationSettingsFromServiceManager Do
			...
			If Setting.CorrespondentEndPoint.IsEmpty() Then
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Correspondent endpoint with the ""%1"" code is not found.';ru='Не найдена конечная точка корреспондента с кодом ""%1"".';vi='Chưa tìm thấy điểm cuối đối ứng có mã ""%1"".'"),
