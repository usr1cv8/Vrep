...
Procedure UserWorkingDirectoryStartChoice(Item, ChoiceData, StandardProcessing)
	...
	Title = NStr("en='Select a main working directory';ru='Выберите основной рабочий каталог';vi='Hãy chọn thư mục làm việc chính'");
...
""
"Продолжить?';vi='Từ thư mục làm việc chính sẽ xóa bỏ tất cả các tệp, ngoại trừ các tệp đang bị chiếm để soạn."
""
"Tiếp tục?'");
...
"Пожалуйста, подождите.';vi='Đang xóa thư mục làm việc chính..."
"Xin vui lòng đợi.'"));
...
Procedure ClearLocalFilesCacheEnd(Result, ExecuteParameters) Export
	...
	Status(NStr("en='Main working directory is cleaned up successfully.';ru='Очистка основного рабочего каталога успешно завершена.';vi='Xóa thư mục làm việc chính đã kết thúc thành công.'"));
