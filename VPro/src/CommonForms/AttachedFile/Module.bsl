...
"пометку на удаление с файла ""%1""?';vi='Để thực hiện thao tác, cần ghi lại những thay đổi của tệp."
"Ghi lại thay đổi và bỏ dấu xóa tệp ""%1""?'");
		Else
			QuestionText = NStr("en='File changes have to be written for actuation."
...
"пометить на удаление файл ""%1""?';vi='Để thực hiện thao tác, cần ghi lại những thay đổi của tệp."
"Ghi lại những thay đổi và"
"đặt dấu xóa tệp ""%1""?'");
		EndIf;
	Else
		If ThisObject.Object.DeletionMark Then
...
Procedure StandardReread(Command)
	...
	QuestionText = NStr("en='Data is changed. Reread?';ru='Данные изменены. Перечитать данные?';vi='Đã thay đổi dữ liệu. Đọc lại dữ liệu?'");
...
Procedure DeleteDS(Command)
	...
	ShowQueryBox(NOTifyDescription, NStr("en='Delete the selected signatures?';ru='Удалить выделенные подписи?';vi='Xóa chữ ký được chọn?'"), QuestionDialogMode.YesNo);
...
Procedure UpdateTitle()
	...
	If ValueIsFilled(ThisObject.Object.Ref) Then
		Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='%1 (Attached file)';ru='%1 (Присоединенный файл)';vi='%1 (Tệp đính kèm)'"), String(ThisObject.Object.Ref));
	Else
		Title = NStr("en='Attached file (Creation)';ru='Присоединенный файл (Создание)';vi='Tệp đính kèm (Tạo mới)'")
	EndIf;
...
Procedure FillEncryptionList()
	...
	HeaderText = NStr("en='Decryption allowed';ru='Разрешено расшифровывать';vi='Cho phép giải mã'");
...
Procedure FillListOfSignatures()
	...
	HeaderText = NStr("en='Digital signatures';ru='Электронные подписи';vi='Chữ ký điện tử'");
...
Function ProcessFileWriteCommand()
	...
	If IsBlankString(ThisObject.Object.Description) Then
		CommonUseClientServer.MessageToUser(
			NStr("en='To continue, specify the file name.';ru='Для продолжения укажите имя файла.';vi='Để tiếp tục hãy chỉ ra tên tệp.'"), , "Description", "Object");
		Return False;
	EndIf;
...
Function WriteFile(Val ObjectParameter = Undefined)
	...
	Except
		If TransactionActive Then
			RollbackTransaction();
			WriteLogEvent(NStr("en='Files.An error occurred when writing the attached file';ru='Файлы.Ошибка записи присоединенного файла';vi='Tệp.Lỗi ghi tệp đính kèm'", CommonUseClientServer.MainLanguageCode()),
