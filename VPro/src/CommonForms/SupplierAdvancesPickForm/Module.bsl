	...
	For Each RowPrepayment IN Prepayment Do
		...
		AND Not ValueIsFilled(RowPrepayment.ExchangeRate) Then
			...
			Message.Text = NStr("en='Column ""Exchange rate"" is not filled in the row. ';ru='Не заполнена колонка ""Курс"" в строке ';vi='Chưa điền cột ""Tỷ giá"" trong dòng'")
				+ String(LineNumber)
				+ NStr("en=' of ""Accounts"").';ru=' списка ""Расшифровка расчетов"".';vi=' danh sách ""Diễn giải tính toán"".'");
		...
		AND Not ValueIsFilled(RowPrepayment.PaymentAmount) Then
			Message = New UserMessage();
			Message.Text = NStr("en='Field ""Payment amount"" is required (row # ';ru='Не заполнена колонка ""Сумма платежа"" в строке ';vi='Chưa điền cột ""Số tiền thanh toán"" trong dòng'")
				+ String(LineNumber)
				+ NStr("en=' of ""Accounts"").';ru=' списка ""Расшифровка расчетов"".';vi=' danh sách ""Diễn giải tính toán"".'");
			Message.Field = "Document";
			Message.Message();
