...
"только в форме объекта-владельца.';vi='Có thể xem danh sách tệp đính kèm'");
	EndIf;
	...
	If Parameters.ChoiceMode Then
		...
		Title = NStr("en='Select attached file';ru='Выбор присоединенного файла';vi='Chọn tệp đính kèm'");
...
"другим пользователем.';vi='Không thể tiến hành thao tác, bởi vì người sử dụng khác đã chiếm tệp để soạn.'");
		Else
			WarningText = NStr("en='Action is unavailable because file"
"is served for editing by another user.';ru='Действие недоступно, так как файл занят для редактирования"
"другим пользователем.';vi='Không thể tiến hành thao tác, bởi vì người sử dụng khác đã chiếm tệp để soạn.'");
...
Procedure ConfigureDynamicList(CatalogNameFilesStorage)
	...
	ErrorTitle = NStr("en='An error occurred when configuring the dynamic list of attached files.';ru='Ошибка при настройке динамического списка присоединенных файлов.';vi='Lỗi khi tùy chỉnh danh sách động tệp đính kèm.'");
	EndErrors = NStr("en='In this case, dynamic list configuration is not supported.';ru='В этом случае настройка динамического списка невозможна.';vi='Trong trường hợp này, không thể tùy chỉnh danh sách động.'");
