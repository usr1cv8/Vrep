...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Not Users.InfobaseUserWithFullAccess() Then
		...
		Raise NStr("en='Insufficient rights to set up offline work.';ru='Недостаточно прав для настройки автономной работы.';vi='Không đủ quyền để tùy chỉnh làm việc độc lập.'");
		
	ElsIf Not OfflineWorkService.OfflineWorkSupported() Then
		
		Raise NStr("en='Offline work is not available in the application.';ru='Возможность автономной работы в программе не предусмотрена.';vi='Không xem xét tính năng làm việc độc lập trong chương trình.'");
...
Procedure HowToSetOfflineWorkplace(Command)
	...
	FormParameters.Insert("Title", NStr("en='How to configure an offline work place';ru='Как настроить автономное рабочее место';vi='Làm thế nào để tùy chỉnh chỗ làm việc độc lập'"));
