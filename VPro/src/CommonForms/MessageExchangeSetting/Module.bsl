...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CommonUseReUse.DataSeparationEnabled()
		...
		CommonUseClientServer.MessageToUser(
			NStr("en='Subsystem configuration is not supported in the split mode.';ru='Настройка подсистемы в разделенном режиме не поддерживается.';vi='Tùy chỉnh phân hệ ở chế độ chia tách không được hỗ trợ.'"),,,, Cancel);
		Return;
	EndIf;
