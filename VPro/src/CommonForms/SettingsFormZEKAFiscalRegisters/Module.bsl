...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Title = NStr("en='FR';ru='ФР';vi='QT'") + " """ + String(ID) + """";
...
Procedure DeviceTest(Presentation)
	...
	For Each TestFile In StructureOfFiles Do
		...
		If Not File.Exist() Then
				...
				NStr("en='File %1 is not exist. Check path to file.';ru='Файл %1 не существует. Проверьте путь к файлу.';vi='Không tồn tại tệp %1. Hãy kiểm tra đường dẫn đến tệp.'"),
				TestFile.Key);
	...
	TextMessage = NStr("en='Success! All files are exist.';ru='Успешно! Все файлы существуют.';vi='Thành công! Đã có tất cả các tệp.'");
...
Function OpenFileDialog(Path, FileName)
	...
	FileDialog.Title = StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Path to file ""%1""';ru='Путь к файлу ""%1""';vi='Đường dẫn đến tệp ""%1""'"),
		FileName);
