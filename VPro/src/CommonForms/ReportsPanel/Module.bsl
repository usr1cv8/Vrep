...
Procedure OnOpen(Cancel)
	...
	If ShowNotificationOnToolTips Then
		ShowUserNotification(
			NStr("en='New feature';ru='Новая возможность';vi='Tính năng mới'"),
			"e1cib/data/SettingsStorage.ReportsVariantsStorage.Form.DescriptionNewOptionForDescriptionsOutput",
			NStr("en='Display descriptions in report panels';ru='Вывод описаний в панелях отчетов';vi='Đưa ra mô tả trên thanh báo cáo'"),
...
Procedure VariantPress(Item)
	...
	ElsIf Not ValueIsFilled(Variant.ReportName) Then
		...
		WarningText = StrReplace(NStr("en='Report name for the ""%1"" variant is not filled in.';ru='Не заполнено имя отчета для варианта ""%1"".';vi='Chưa điền tên báo cáo cho phương án ""%1"".'"), "%1", Variant.Description);
...
Function EnteredSearchStringIsTooShort(Text)
	...
	If StrLen(Text) < 2 Then
		ShowMessageBox(, NStr("en='Search string is too short.';ru='Введена слишком короткая строка поиска.';vi='Đã nhập dòng tìm kiếm quá ngắn.'"));
		Return True;
	EndIf;
	...
	If Not ThereIsNormalWord Then
		ShowMessageBox(, NStr("en='Words for search are too short.';ru='Введены слишком короткие слова для поиска.';vi='Đã nhập từ tìm kiếm quá ngắn.'"));
		Return True;
	EndIf;
...
Procedure ResetSettings(Command)
	QuestionText = NStr("en='Reset report placement settings?';ru='Сбросить настройки расположения отчетов?';vi='Hủy bỏ tùy chỉnh vị trí báo cáo?'");
...
Procedure RefreshReportsPanel(Val Event = "")
	...
	// Title.
	SetupModeSuffix = " (" + NStr("en='setting';ru='настройка';vi='tùy chỉnh'") + ")";
...
Procedure FillInformationAboutSubsystemsAndSetTitle()
	...
	If CurrentSectionRef = Catalogs.MetadataObjectIDs.EmptyRef() Then
		Raise StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Section ""%1"" is not connected to the ""%2"" subsystem. See module ReportsVariantsPredefined, procedure DetermineSectionsWithReportsVariants.';ru='Раздел ""%1"" не подключен к подсистеме ""%2"".См. модуль ReportsVariantsPredefined, процедуру DetermineSectionsWithReportsVariants.';vi='Phần hành ""%1"" không được kết nối đến phân hệ ""%2"". Xem mô-đun ReportsVariantsPredefined, thủ tục DetermineSectionsWithReportsVariants.'"),
...
Procedure FillReportsPanel()
	...
	Else
		...
		If FillingParameters.OtherSections.Count() = 0 Then
			...
			Label.Title = "    " + NStr("en='Reports are not found in other sections.';ru='Отчеты в других разделах не найдены.';vi='Không tìm thấy báo cáo trong các phần hành khác.'") + Chars.LF;
			Label.Height = 2;
		EndIf;
		...
		If FillingParameters.DontOutput > 0 Then // Output of information text.
			LabelTitle = NStr("en='The first %1 reports from other sections are displayed, specify a search query.';ru='Выведены первые %1 отчетов из других разделов, уточните поисковый запрос.';vi='Đã chọn %1 đầu tiên của báo cáo từ các phần hành khác, hãy làm rõ truy vấn tìm kiếm.'");
...
Procedure InitializeFillingParameters(FillingParameters)
	...
	TemplateOptionGroups.Group = ChildFormItemsGroup.HorizontalIfPossible;
...
Procedure FindReportsVariantsForWithdrawal(FillingParameters)
	...
	LangKey = CurrentLanguage().LanguageCode;
...
Procedure OuputSectionVariants(FillingParameters, SectionRef)
	...
	If FillingParameters.VariantsQuantity = 0 Then
		...
		If FillingParameters.CurrentSectionVariantsDisplayed Then
			...
			If ValueIsFilled(SearchString) Then
				If FillingParameters.OnlyCurrentSection Then
					Label.Title = NStr("en='Reports not found.';ru='Отчеты не найдены.';vi='Không tìm thấy báo cáo.'");
				Else
					Label.Title = NStr("en='Reports are not found in the current section.';ru='Отчеты в текущем разделе не найдены.';vi='Không tìm thấy báo cáo trong phần hành hiện tại.'");
					Label.Height = 2;
				EndIf;
			Else
				Label.Title = NStr("en='Report panel of this section does not contain any reports.';ru='В панели отчетов этого раздела не размещено ни одного отчета.';vi='Trên thanh báo cáo của phần hành này không có báo cáo nào.'");
			EndIf;
...
Procedure DefineGroupsAndDecorationsForVariantsOutput(FillingParameters)
	...
	SectionGroup.Group = ChildFormItemsGroup.HorizontalIfPossible;
...
Function AddSubsystemGroup(FillingParameters, OutputOrderString, IntoGroup)
	...
	If OutputOrderString.NestingLevel > 1 Then
		...
		IndentGroup.Group              = ChildFormItemsGroup.HorizontalIfPossible;
	...
	If IlluminationRequired Then
		If OutputOrderString.ThisIsContinuationOfThe Then
			Suffix = NStr("en='(continue)';ru='(продолжение)';vi='(tiếp tục)'");
	...
	Else
		If OutputOrderString.ThisIsContinuationOfThe Then
			SubsystemPresentation = SubsystemPresentation + " " + NStr("en='(continue)';ru='(продолжение)';vi='(tiếp tục)'");
		EndIf;
...
Function AddReportVariantItems(FillingParameters, Variant, IntoGroup, NestingLevel = 0)
	...
	If ValueIsFilled(Variant.Author) Then
		Label.ToolTip = TrimL(Label.ToolTip + Chars.LF) + NStr("en='Author:';ru='Автор:';vi='Người lập:'") + " " + TrimAll(String(Variant.Author));
	EndIf;
	...
	If FillingParameters.UseBacklight Then
		...
		If HighlightParameters <> Undefined Then
			...
			If HighlightParameters.AuthorPresentation.FoundWordsCount > 0 Then
				...
				EndIf;
				ToolTipContent.Add(NStr("en='Author:';ru='Автор:';vi='Người lập:'") + " ");
			...
			If HighlightParameters.UserSettingsNames.FoundWordsCount > 0 Then
				If ToolTipContent.Count() > 0 Then
					ToolTipContent.Add(Chars.LF);
				EndIf;
				ToolTipContent.Add(NStr("en='Saved settings:';ru='Сохраненные настройки:';vi='Tùy chỉnh đã lưu:'") + " ");
			...
			If HighlightParameters.FieldNames.FoundWordsCount > 0 Then
				If ToolTipContent.Count() > 0 Then
					ToolTipContent.Add(Chars.LF);
				EndIf;
				ToolTipContent.Add(NStr("en='Fields:';ru='Поля:';vi='Trường:'") + " ");
			...
			If HighlightParameters.ParametersAndFiltersNames.FoundWordsCount > 0 Then
				If ToolTipContent.Count() > 0 Then
					ToolTipContent.Add(Chars.LF);
				EndIf;
				ToolTipContent.Add(NStr("en='Setting:';ru='Настройка:';vi='Tùy chỉnh:'") + " ");
