...
// adaptation
Function GetBarcodeByProduct(Номенклатура, Характеристика = Undefined, Партия = Undefined, ЕдиницаИзмерения = Undefined) Export
	
	Query = Новый Запрос;
	Query.Text = 
	"ВЫБРАТЬ
	|	ШтрихкодыНоменклатуры.Barcode КАК Штрихкод,
	|	ШтрихкодыНоменклатуры.ProductsAndServices КАК Номенклатура,
	|	ШтрихкодыНоменклатуры.Characteristic КАК Характеристика,
	|	ШтрихкодыНоменклатуры.Batch КАК Партия,
	|	ШтрихкодыНоменклатуры.MeasurementUnit КАК ЕдиницаИзмерения
	|ИЗ
	|	InformationRegister.ProductsAndServicesBarcodes КАК ШтрихкодыНоменклатуры
	|ГДЕ
	|	ШтрихкодыНоменклатуры.ProductsAndServices = &Номенклатура
	|	И (&Характеристика = НЕОПРЕДЕЛЕНО
	|			ИЛИ ШтрихкодыНоменклатуры.Characteristic = &Характеристика)
	|	И (&Партия = НЕОПРЕДЕЛЕНО
	|			ИЛИ ШтрихкодыНоменклатуры.Batch = &Партия)
	|	И (&ЕдиницаИзмерения = НЕОПРЕДЕЛЕНО
	|			ИЛИ ШтрихкодыНоменклатуры.MeasurementUnit = &ЕдиницаИзмерения)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Номенклатура,
	|	Характеристика,
	...
	Query.SetParameter("Номенклатура", Номенклатура);
	Query.SetParameter("Характеристика", Характеристика);
	Query.SetParameter("Партия", Партия);
	Query.SetParameter("ЕдиницаИзмерения", ЕдиницаИзмерения);
	
	Выборка = Query.Execute().Select();
	If Выборка.Next() Then
		Return Выборка.Штрихкод;
	Else
		Return "";
	EndIf;
