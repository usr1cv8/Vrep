...
"Отложите их в сторону как неотсканированные.';vi='Không chỉ ra mặt hàng tương ứng cho tất cả các mã vạch mới. Những hàng hóa này sẽ được chuyển vào chứng từ. Hãy sắp xếp chúng sang một bên như là chưa quét.'"
			);
...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Selection.Next() Then // Barcode is already written in the database
		...
		ErrorDescription = NStr("en='Such barcode is already specified  for products and services %ProductsAndServices%';ru='Такой штрихкод уже назначен для номенклатуры %Номенклатура%';vi='Mã vạch như vậy đã được dùng cho mặt hàng %ProductsAndServices%'");
		ErrorDescription = StrReplace(ErrorDescription, "%ProductsAndServices%", """" + Selection.ProductsAndServicesPresentation + """"
						+ ?(ValueIsFilled(Selection.Characteristic), " " + NStr("en='with characteristic';ru='с характеристикой';vi='với đặc tính'") + " """ + Selection.CharacteristicPresentation + """", "")
						+ ?(ValueIsFilled(Selection.Batch), " """ + NStr("en='in batch';ru='в партии';vi='theo lô'") + " " + Selection.BatchPresentation + """", ""));
...
"Отложите их в сторону как неотсканированные.';vi='Tất cả hàng hóa sẽ không được đưa vào chứng từ. Hãy hoãn lại hàng hóa sang một bên như chưa quét.'"
		);
