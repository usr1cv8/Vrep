...
Procedure SetVersioningVariantOnPosting(Command)
	...
	If SelectNoPostingDocumentsType() Then
		ShowMessageBox(, NStr("en='Documents that cannot be posted are set to versioning mode ""Version on writing"".';ru='Документам, которые не могут быть проведены, установлен режим версионирования ""Версионировать при записи"".';vi='Thiết lập chế độ đánh số phiên bản ""Đánh số phiên bản khi ghi lại"" cho các chứng từ mà không thể kết chuyển được.'"));
	EndIf;
	...
	For Each Type IN TypeArray Do
		If AllCatalogs.ContainsType(Type) Then
			If CatalogsNod = UNDEFINED Then
				...
				CatalogsNod.SynonymNameObject = NStr("en='Catalogs';ru='Справочники';vi='Danh mục'");
		...
		ElsIf AllDocuments.ContainsType(Type) Then
			If DocumentsNode = UNDEFINED Then
				...
				DocumentsNode.SynonymNameObject = NStr("en='Documents';ru='Документы';vi='Chứng từ'");
		...
		ElsIf BusinessProcesses.AllRefsType().ContainsType(Type) Then
			If NodeBusinessProcesses = Undefined Then
				...
				NodeBusinessProcesses.SynonymNameObject = NStr("en='Business processes';ru='Деловые процессы';vi='Quy trình kinh doanh'");
...
Procedure RunScheduledJob()
	...
	Else
		BackgroundJobDescription = StringFunctionsClientServer.SubstituteParametersInString(NStr("en='Start manually: %1';ru='Запуск вручную: %1';vi='Khởi động thủ công: %1'"), ScheduledJobMetadata.Synonym);
...
Function StatusTextCalculation()
	Return NStr("en='Search for outdated versions...';ru='Поиск устаревших версий...';vi='Tìm kiếm phiên bản cũ...'");
EndFunction

&AtClientAtServerNoContext
Function StatusTextClearing()
	Return NStr("en='Clearing obsolete versions...';ru='Выполняется очистка устаревших версий...';vi='Đang xóa các phiên bản cũ...'");
EndFunction
...
Procedure DisplayInformationAboutOutdatedVersions()
	...
	If InformationAboutLegacyVersions.DataSize > 0 Then
		Items.InformationAboutLegacyVersions.Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Total obsolete versions: %1 (%2)';ru='Всего устаревших версий: %1 (%2)';vi='Toàn bộ phiên bản cũ: %1 (%2)'"),
			InformationAboutLegacyVersions.CountVersions,
			InformationAboutLegacyVersions.DataSizeString);
	Else
		Items.InformationAboutLegacyVersions.Title = NStr("en='All obsolete versions: no';ru='Всего устаревших версий: нет';vi='Toàn bộ phiên bản cũ: không có'");
	EndIf;
