...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.DataImportingProhibitionDates Then
		...
		If Not SectionsProperties.UseProhibitionDatesOfDataImport Then
			Raise(NStr("en='Closing dates of data import are not used.';ru='Даты запрета загрузки данных не используются.';vi='Không sử dụng ngày cấm kết nhập dữ liệu.'"));
		EndIf;
		
		Title = NStr("en='Closing dates of data import';ru='Даты запрета загрузки данных';vi='Ngày cấm kết nhập dữ liệu'");
		Items.ProhibitionDateSetting.ChoiceList.FindByValue("NoProhibition").Presentation =
			NStr("en='Data import allowed';ru='Нет запрета загрузки данных';vi='Không cấm kết nhập dữ liệu'");
		Items.ProhibitionDateSetting.ChoiceList.FindByValue("ForAllUsers").Presentation =
			NStr("en='For all infobases';ru='Для всех информационных баз';vi='Đối với tất cả cơ sở thông tin'");
		Items.ProhibitionDateSetting.ChoiceList.FindByValue("ByUsers").Presentation =
			NStr("en='By infobases';ru='По информационным базам';vi='Theo cơ sở thông tin'");
		
		Items.Reports.ToolTip
			= NStr("en='Reports by the data"
"import prohibition dates set ""By the infobases"".';ru='Отчеты по"
"датам запрета загрузки данных, установленным ""По информационным базам"".';vi='Báo cáo theo"
"ngày cấm kết nhập dữ liệu mà đã thiết lập ""Theo cơ sở thông tin""'");
		
		Commands.ProhibitionDatesByUsers.Title
			= NStr("en='Closing dates by infobases';ru='Даты запрета по информационным базам';vi='Ngày cấm theo cơ sở thông tin'");
...
"загрузки данных по информационным базам и программам.';vi='Ngày cấm"
"kết nhập dữ liệu theo các cơ sở thông tin và chương trình.'");
		
		Commands.ProhibitionDatesBySectionsObjectsForUsers.Title =
			NStr("en='Closing dates by sections and objects for infobases';ru='Даты запрета по разделам и объектам для информационных баз';vi='Ngày cấm theo phân hệ và đối tượng đối với cơ sở thông tin'");
...
"загрузки данных по разделам и объектам для информационных баз и программ.';vi='Ngày cấm"
"kết nhập dữ liệu theo các phần hành và đối tượng cho cơ sở thông tin và chương trình.'");
		
		Items.UsersFullPresentation.Title =
			NStr("en='Application: infobase';ru='Программа: информационная база';vi='Chương trình: cơ sở thông tin'");
	...
	Else
		...
		ListOfUserTypes.Add(
			Type("CatalogRef.Users"),        NStr("en='User';ru='Пользователь';vi='Người sử dụng'"));
		
		ListOfUserTypes.Add(
			Type("CatalogRef.ExternalUsers"), NStr("en='External user';ru='Внешний пользователь';vi='Người sử dụng ngoài'"));
	EndIf;
		...
		If CurrentSpecifiedMode <> SpecifiedModeValueInList.Value Then
			...
			ShowQueryBox(
				...
				MessageTextSpecifiedModeNotUsed(
					...
					CurrentUser,
					ThisObject) + Chars.LF + Chars.LF + NStr("en='Continue?';ru='Продолжить?';vi='Tiếp tục?'"),
				QuestionDialogMode.YesNo);
			Return;
		EndIf;
	EndIf;
...
Procedure SetProhibitionDatesCommandBar(Context)
	...
	Else
		If Context.ProhibitionDateSpecifiedMode = "BySections" Then
			...
			SetProperty(Items.ProhibitionDataSections.Title,   NStr("en='Sections';ru='секции';vi='quầy"
"hàng'"));
			SetProperty(Items.ProhibitionDatesPick.Visible, False);
			SetProperty(Items.ProhibitionDatesAdd.Visible,  False);
			SetProperty(Items.ProhibitionDatesContextMenuAdd.Visible, False);
		...
		ElsIf Context.ProhibitionDateSpecifiedMode = "ByObjects" Then
			...
			SetProperty(Items.ProhibitionDataSections.Visible,   True);
			SetProperty(Items.ProhibitionDataSections.Title,   NStr("en='Common date';ru='Общая дата';vi='Ngày chung'"));
			SetProperty(Items.ProhibitionDatesPick.Visible, True);
			SetProperty(Items.ProhibitionDatesAdd.Visible,  True);
			SetProperty(Items.ProhibitionDatesContextMenuAdd.Visible, True);
			SetProperty(Items.ProhibitionDatesChange.Representation, ButtonRepresentation.Auto);
	...
	Else
		...
		If Context.AllowDataChangingToProhibitionDate Then
			...
			If Context.CurrentDateAtServer > PermissionTerm Then
				LabelText = Chars.LF +
					NStr("en='Period of changing data from %3 to %4 expired on %2';ru='Срок возможности изменения данных с %3 по %4 истек %2';vi='Thời hạn có thể thay đổi dữ liệu từ %3 đến %4 trôi qua %2'");
			Else
				If CalculateProhibitionDate Then
					Context.ProhibitionDate = CalculatedProhibitionDates.Previous;
				EndIf;
				LabelText = Chars.LF +
					NStr("en='For %2 data might change from %3 to %4';ru='По %2 возможно изменение данных с %3 по %4';vi='Theo %2 có thể thay đổi dữ liệu từ %3 đến %4'") + Chars.LF +
					NStr("en='After %2 it will be prohibited to change %4 data';ru='После %2 будет запрещено изменение данных по %4';vi='Sau %2 sẽ cấm thay đổi dữ liệu theo %4'") + Chars.LF;
			EndIf;
		Else
		...
		Context.Items.AutomaticalDateExplanation.Title =
			StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Changing of data on %1 is prohibited';ru='Запрещено изменение данных по %1';vi='Đã cấm thay đổi dữ liệu theo %1'") + LabelText,
...
Function DescriptionsProhibitionDateList()
	...
	List = New ValueList;
	List.Add("",                "(" + NStr("en='Default';ru='По умолчанию';vi='Theo mặc định'") + ")");
	List.Add("Custom",      NStr("en='Custom date';ru='Произвольная дата';vi='Ngày tùy ý'"));
	List.Add("LastYearEnd",     NStr("en='Last year end';ru='Конец прошлого года';vi='Cuối năm ngoái'"));
	List.Add("LastQuarterEnd", NStr("en='Last quarter end';ru='Конец прошлого квартала';vi='Cuối quý trước'"));
	List.Add("LastMonthEnd",   NStr("en='Last month end';ru='Конец прошлого месяца';vi='Cuối tháng trước'"));
	List.Add("LastWeekEnd",    NStr("en='Last week end';ru='Конец прошлой недели';vi='Cuối tuần trước'"));
	List.Add("PreviousDay",        NStr("en='Previous day';ru='Предыдущий день';vi='Ngày trước'"));
	...
	If Context.Parameters.DataImportingProhibitionDates Then
		For Each ListValue IN Context.ListOfUserTypes Do
			If TypeOf(ListValue.Value) = TypeOf(User) Then
				...
				Else
					Return ListValue.Presentation + ": " +
						NStr("en='<All infobases>';ru='<Все информационные базы>';vi='<Tất cả cơ sở thông tin>'");
				EndIf;
			EndIf;
		EndDo;
	EndIf;
...
Function CommentTextForAllUsers()
	...
	Return "(" + NStr("en='Default';ru='По умолчанию';vi='Theo mặc định'") + ")";
...
Function PresentationCommonDateText()
	...
	Return "<" + NStr("en='Common date';ru='Общая дата';vi='Ngày chung'") + ">";
...
Function ReportByObjectCommandHeaderText()
	...
	Return NStr("en='Report by objects';ru='Отчет по объектам';vi='Báo cáo theo các đối tượng'");
...
Function ReportBySectionsCommandHeaderText()
	...
	Return NStr("en='Report by sections';ru='Отчет по разделам';vi='Báo cáo theo các phần hành'");
...
Function QuestionTextCloseForm()
	...
	Return NStr("en='Close the form?';ru='Закрыть форму?';vi='Đóng biểu mẫu?'");
...
Function QuestionTextRecalculateProhibitionDates()
	...
	Return NStr("en='Clear the unfilled lines and recalculate relative closing dates?';ru='Очистить незаполненные строки и пересчитать относительные даты запрета?';vi='Xóa dòng chưa điền và tính lại ngày cấm tương đối?'");
...
Function QuestionTextUpdateData()
	...
	Return NStr("en='Update data?';ru='Обновить данные?';vi='Cập nhật dữ liệu?'");
...
Function QuestionTextDeleteAllProhibitionDatesExceptDatesForAllUsers()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Delete all closing dates except for the set ones %1?';ru='Удалить все даты запрета, кроме установленных %1?';vi='Xóa tất cả ngày cấm, ngoại trừ ngày đã thiết lập %1?'"),
		ValueForAllUsers);
...
Function QuestionTextDeleteAllProhibitionDates()
	...
	Return NStr("en='Delete all closing dates?';ru='Удалить все даты запрета?';vi='Xóa tất cả ngày cấm?'");
...
Function MessageTextValueForAllUsersNotChange()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Value %1 is not being changed.';ru='Значение %1 не изменяется.';vi='Giá trị %1 không bị thay đổi.'"),
		PresentationTextForAllUsers(ThisObject));
...
Function MessageTextCommentForAllUsersNotChange()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Comment %1 cannot be changed.';ru='Комментарий %1 не изменяется.';vi='Không thay đổi ghi chú %1.'"),
		PresentationTextForAllUsers(ThisObject));
...
Function MessageTextFirstSelectUser()
	...
	Return NStr("en='Select user first.';ru='Сначала выберите пользователя.';vi='Trước tiên, hãy chọn người sử dụng.'");
...
"даты запрета ""%1"" не настроена, поэтому будет сохранена установка даты запрета ""%2"".';vi='Chưa thiết lập"
"ngày cấm ""%1"", bởi vì sẽ"
"lưu việc thiết lập ngày cấm ""%2"".'"),
...
"не настроен ""%2"", поэтому будет сохранен способ указания даты запрета ""%3"".';vi='Phương thức"
"chỉ ra ngày cấm ""%1"""
"chưa được thiết lập ""%2"", bởi vì sẽ lưu phương thức"
"chỉ ra ngày cấm ""%3"".'"),
			SpecifiedModeInForm,
...
"настроен для ""%2"", поэтому будет сохранен способ указания даты запрета ""%3"".';vi='Phương thức"
"chỉ ra ngày cấm ""%1"" không"
"được tùy chỉnh cho ""%2"", bởi vì sẽ lưu phương thức"
"chỉ ra ngày cấm ""%3"".'"),
			SpecifiedModeInForm,
			CurrentUser,
			SpecifiedModeInDataBase);
...
Function QuestionTextClearBlankRows()
	...
	Return NStr("en='Clear unfilled lines?';ru='Очистить незаполненные строки?';vi='Xóa các dòng chưa điền?'");
...
Function MessageTextForDeletingSelectOneRow()
	...
	Return NStr("en='Mark one line for deletion.';ru='Для удаления выделите одну строку.';vi='Để xóa hãy chọn một dòng.'");
...
Function QuestionTextDeleteProhibitionDatesForAllUsers()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='Delete the %1 closing dates?';ru='Удалить даты запрета %1?';vi='Xóa ngày cấm %1?'"), Lower(ValueForAllUsers));
...
Function QuestionTextDeleteProhibitionDatesForUser()
	...
	Return NStr("en='Delete closing dates for a user?';ru='Удалить даты запрета для пользователя?';vi='Xóa ngày cấm đối với người sử dụng?'");
...
Function QuestionTextDeleteProhibitionDatesForUsersGroups()
	...
	Return NStr("en='Delete closing dates for a user group?';ru='Удалить даты запрета для группы пользователей?';vi='Xóa ngày cấm đối với nhóm người sử dụng?'");
...
Function QuestionTextDeleteProhibitionDatesForExternalUser()
	...
	Return NStr("en='Delete closing dates for an external user?';ru='Удалить даты запрета для внешнего пользователя?';vi='Xóa ngày cấm đối với người sử dụng ngoài?'");
...
Function QuestionTextDeleteProhibitionDatesForExternalUsersGroups()
	...
	Return NStr("en='Delete closing dates for external users group?';ru='Удалить даты запрета для группы внешних пользователей?';vi='Xóa ngày cấm đối với nhóm người sử dụng ngoài?'");
...
Function QuestionTextDeleteProhibitionDates()
	...
	Return NStr("en='Remove closing dates?';ru='Удалить даты запрета?';vi='Xóa ngày cấm?'");
...
Function MessageTextForAllUsersProhibitionDatesIsNotSet()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='%1 closing dates are not set.';ru='%1 даты запрета не установлены.';vi='Không thiết lập %1 ngày cấm.'"), ValueForAllUsers);
...
Function MessageTextValueAlreadyAddedToList()
	...
	Return NStr("en='The value has already been added to the list.';ru='Значение уже добавлено в список.';vi='Đã bổ sung giá trị vào danh sách.'");
...
Function MessageTextUpdateFormF5Data()
	...
	Return NStr("en='Update form data (F5).';ru='Обновите данные формы (F5).';vi='Hãy cập nhật dữ liệu biểu mẫu (F5).'");
...
Function QuestionTextDeleteProhibitionDatesForSectionsAndObjects()
	...
	Return NStr("en='Delete closing dates for sections and objects?';ru='Удалить даты запрета для разделов и объектов?';vi='Xóa ngày cấm đối với phần hành và đối tượng?'");
...
Function QuestionTextDeleteProhibitionDatesForObjects()
	...
	Return NStr("en='Delete closing dates for objects?';ru='Удалить даты запрета для объектов?';vi='Xóa ngày cấm đối với đối tượng?'");
...
Function QuestionTextDeleteProhibitionDatesForSections()
	...
	Return NStr("en='Delete closing dates for sections?';ru='Удалить даты запрета для разделов?';vi='Xóa ngày cấm đối với phần hành?'");
...
Function MessageTextInSelectedSectionProhibitionDatesForObjectNotSet()
	...
	Return NStr("en='Closing dates for objects are not set in the selected section.';ru='В выбранном разделе даты запрета для объектов не устанавливаются.';vi='Trong phần hành đã chọn không thiết lập ngày cấm cho đối tượng.'");
...
Function MessageTextCommonDateCanBeSet()
	...
	Return NStr("en='<Common date> can be set.';ru='<Общая дата> может быть установлена.';vi='Có thể thiết lập <Ngày chung>.'");
...
"уже заполнены,"
"можно установить даты запрета для разделов.';vi='Phần hành"
"đã được điền,"
"mạ có thể thiết lập ngày cấm đối với các phần hành"
".'");
...
"уже заполнены,"
"можно установить даты запрета для разделов и объектов.';vi='Phần hành"
"đã được điền,"
"có thể thiết lập ngày cấm đối với các phần hành và đối tượng.'");
...
"уже выбран, можно установить дату запрета.';vi='Đối tượng"
"đã được chọn, có thể thiết lập ngày cấm.'");
...
Function MessageTextFirstSelectObject()
	...
	Return NStr("en='Select object first.';ru='Сначала выберите объект.';vi='Trước tiên, hãy chọn đối tượng.'");
...
Function QuestionTextDeleteProhibitionDatesForSection()
	...
	Return NStr("en='Delete a closing date for a section?';ru='Удалить дату запрета для раздела?';vi='Xóa ngày cấm đối với phần hành?'");
...
Function QuestionTextDeleteCommonProhibitionDate()
	...
	Return NStr("en='Delete common closing date?';ru='Удалить общую дату запрета?';vi='Xóa ngày cấm chung?'");
...
Function QuestionTextDeleteObjectProhibitionDate()
	...
	Return NStr("en='Delete a closing date for an object?';ru='Удалить дату запрета для объекта?';vi='Xóa ngày cấm cho đối tượng?'");
...
Function MessageTextForAllUsersSectionsAlwaysShow()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='%1 sections are always shown.';ru='%1 разделы всегда показываются.';vi='Luôn hiển thị %1 các phần hành.'"), ValueForAllUsers);
...
Function MessageTextWhenDeletingProhibitionDateCleared()
	...
	Return NStr("en='On deletion, closing date is cleared.';ru='При удалении дата запрета очищается.';vi='Khi xóa ngày cấm bị xóa.'");
...
Function MessageTextForAllUsersCommonDateAlwaysShow()
	...
	Return StringFunctionsClientServer.SubstituteParametersInString(
		NStr("en='%1 <Common date> is always shown.';ru='%1 <Общая дата> всегда показывается.';vi='Luôn hiển thị %1 <Ngày chung>.'"), ValueForAllUsers);
...
Function SectionTitleText()
	...
	Return NStr("en='Section';ru='Раздел';vi='Phần hành'");
...
Function SectionWithObjectsTitleText()
	...
	Return NStr("en='Section, object';ru='Раздел, объект';vi='Phần hành, đối tượng'");
...
Function MessageTextNotEnoughRightsForChoiceOfExternalUsers()
	...
	Return NStr("en='Insufficient rights to select external users.';ru='Недостаточно прав для выбора внешних пользователей.';vi='Không đủ quyền để chọn người sử dụng ngoài.'");
...
Function TitleTextChoiceDataType()
	...
	Return NStr("en='Select data type';ru='Выбор типа данных';vi='Chọn kiểu dữ liệu'");
...
Function MessageTextSettingsWithUnselectedUsersNotSaved(Context)
	...
	If Context.Parameters.DataImportingProhibitionDates Then
		Return NStr("en='Settings with unselected infobases are not saved.';ru='Настройки с невыбранными информационными базами не сохранены.';vi='Không lưu lại tùy chỉnh với cơ sở thông tin chưa chọn.'");
	EndIf;
	
	Return NStr("en='Settings with unselected users are not saved.';ru='Настройки с невыбранными пользователями не сохранены.';vi='Không lưu lại tùy chỉnh chưa chọn người sử dụng.'");
...
Function MessageTextSettingsWithUnselectedObjectsNotSaved()
	...
	Return NStr("en='Settings with unselected objects are not saved.';ru='Настройки с невыбранными объектами не сохранены.';vi='Không lưu lại tùy chỉnh chưa chọn đối tượng.'");
...
Function MessageTextSettingsWithUnfilledProhibitionDatesForObjectsNotSaved()
	...
	Return NStr("en='Settings with unfilled closing dates for objects are not saved.';ru='Настройки с незаполненными датами запрета для объектов не сохранены.';vi='Không lưu lại tùy chỉnh chưa điền ngày cấm cho đối tượng.'");
...
Function MessageTextSettingsWithUnfilledProhibitionDatesForSectionsNotSaved()
	...
	Return NStr("en='Settings with unfilled closing dates for sections are not saved.';ru='Настройки с незаполненными датами запрета для разделов не сохранены.';vi='Không lưu lại tùy chỉnh chưa điền ngày cấm đối với các phần hành.'");
...
Function MessageTextCommonDateAlreadyShown()
	...
	Return NStr("en='<Common date> is already shown.';ru='<Общая дата> уже показана.';vi='Đã hiển thị <Ngày chung>.'");
...
Function MessageTextAllSectionsAlreadyShown()
	...
	Return NStr("en='All sections are already shown.';ru='Все разделы уже показаны.';vi='Đã hiển thị tất cả các phần hành.'");
...
Function TitleChoiceOfRequiredSections()
	...
	Return NStr("en='Select required sections';ru='Выбор требуемых разделов';vi='Chọn các phần hành cần thiết'");
...
Function MessageTextUnfilledRowsAreUnmarked()
	...
	Return NStr("en='Unfilled rows are unchecked.';ru='Снято выделение с незаполненных строк.';vi='Bỏ chọn dòng trống.'");
...
Function MessageTextSelectedStringsNotFilledIn()
	...
	Return NStr("en='The selected lines are not filled in.';ru='Выделенные строки не заполнены.';vi='Chưa điền dòng đã chọn.'");
