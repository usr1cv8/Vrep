...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If ObjectVersioning.LastVersionNumber(Ref) = 0 Then
		...
		Items.NoneOfVersions.Title = StringFunctionsClientServer.SubstituteParametersInString(
	       NStr("en='Previous versions are missing: ""%1"".';ru='Предыдущие версии отсутствуют: ""%1"".';vi='Không có các phiên bản trước: ""%1"".'"),
	       String(Ref));
	EndIf;
	...
	Attributes = NStr("en='All';ru='Все';vi='Tất cả'")
EndProcedure
...
Procedure GenerateChangesReport(Command)
	...
	If ComparedVersions.Count() < 2 Then
		ShowMessageBox(, NStr("en='To generate a report on changes, select at least two versions.';ru='Для формирования отчета по изменениям необходимо выбрать хотя бы две версии.';vi='Để lập báo cáo theo các thay đổi, cần chọn ít nhất hai phiên bản.'"));
		Return;
	EndIf;
...
"%1"
"Перейти на выбранную версию с отменой проведения?';vi='Chưa thực hiện chuyển sang phiên bản do nguyên nhân:"
"%1"
"Chuyển sang phiên bản đã chọn có hoãn kết chuyển?'"),
			ErrorMessageText);
			
		NotifyDescription = New NotifyDescription("GoToSelectedVersionQueryIsAsked", ThisObject);
		Buttons = New ValueList;
		Buttons.Add("Goto", NStr("en='Navigate';ru='Перейти';vi='Chuyển đến'"));
	...
	Else //Result = "RecoveryIsCompleted"
		...
		If FormOwner <> Undefined Then
			...
			EndTry;
		EndIf;
		ShowMessageBox(, NStr("en='Migration to the previous version is completed successfully.';ru='Переход к старой версий выполнен успешно.';vi='Đã chuyển thành công sang phiên bản cũ.'"));
	EndIf;
...
Function GoToVersionServer(Ref, VersionNumber, UndoPosting = False)
	...
	AddressInTemporaryStorage = PutToTempStorage(Information.ObjectVersioning);
