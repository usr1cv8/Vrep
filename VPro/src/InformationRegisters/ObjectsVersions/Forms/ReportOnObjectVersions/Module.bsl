	...
	If TypeOf(Parameters.ComparedVersions) = Type("Array") Then
		...
		If ComparedVersions.Count() > 1 Then
			...
			Title = StringFunctionsClientServer.SubstituteParametersInString(
			                 NStr("en='Compare versions ""%1"" (No.No. %2)';ru='Сравнение версий ""%1"" (№№ %2)';vi='So sánh các phiên bản ""%1"" (SốSố %2)'"),
			                 CommonUse.SubjectString(ObjectReference),
			                 StringOfVersionsNumber);
		Else
			Title = StringFunctionsClientServer.SubstituteParametersInString(
			                 NStr("en='Version of object ""%1"" No.%2';ru='Версия объекта ""%1"" №%2';vi='Phiên bản của đối tượng ""%1"" №%2'"),
			                 ObjectReference,
	...
	For VersionIndex = 2 To VersionsNumbersArray.Count() Do
		...
		For Each MapItem IN ObjectVersioning.TabularSections Do
			...
			For Each TableRow IN TablesVersionRef Do
				TableRow.RowId_Versioning = TableRow.LineNumber;
			EndDo;
			...
			If TableWithChanges <> Undefined Then
				...
				For Each TSItem IN ModifiedRows Do
					...
					TablesVersionRef[TSItem.IndexOfTS1-1].RowId_Versioning = RowTCH.RowId_Versioning;
				...
				For Each TSItem IN AddedStrings Do
					TablesVersionRef[TSItem.IndexOfTS1-1].RowId_Versioning = IncrementCounter(CounterUniqueId, TableName);
				...
				For IndexOf = 1 To TablesVersionRef.Count() Do
					If TablesVersionRef[IndexOf-1].RowId_Versioning = Undefined Then
						...
						For IndByVT_Current = 1 To RowArrayCurrentPM.Count() Do
							If IndByVT_Current <= ArrayOfStringsPreviousTS.Count() Then
								RowArrayCurrentPM[IndByVT_Current-1].RowId_Versioning = ArrayOfStringsPreviousTS[IndByVT_Current-1].RowId_Versioning;
							EndIf;
				...
				For Each TSItem IN DeletedStrings Do
					...
					VirtualRow.RowId_Versioning = ChangesTableTableParts[TableName][PreviousVersionNumber][TSItem.IndexTTN0-1].RowId_Versioning;
			...
			While VersionsIndex >= 1 Do
				...
				If FoundString <> Undefined Then
					...
					If Not TablePartsSectionTitleShow Then
						...
						ReportTS.Put(CommonPatternOfHeaderSectionOfPM);
					...
					If Not CurrentTabularSectionChanged Then
						...
						ReportTS.Put(CurrentTSCapTemplate);
					...
					If StringUniqueIdChanged = False Then
						...
						ReportTS.Put(CapTSRowsTemplate);
...
Function OutputCompositionResultsToReport(DetailsChangesTable,
	...
	If ChangedDetailsNumber = 0 Then
		...
		ReportTS.Put(DetailsAreaNotChanged);
		ReportTS.EndRowGroup();
	Else
...
Function OutputHeader(ReportTS, VersionsNumbersArray, NumberOfVersions)
	...
	AreaHeader.Parameters.ReportDescription = NStr("en='Object version change report';ru='Отчет по изменениям версий объекта';vi='Báo cáo thay đổi phiên bản đối tượng'");
...
Function ReadInitialAttributeValuesAndTabularSections(AttributesTable,
	...
	ObjVersioning  = ObjectVersioning.VersionParsing(ObjectReference, MinObjectVersioning);
	AddLineNumbersInTabularSections(ObjVersioning.TabularSections);
	
	Attributes      = ObjVersioning.Attributes;
	TabularSections = ObjVersioning.TabularSections;
	...
	For Each ValueTableRow IN Attributes Do
		...
		NewRow.Versioning_TypeValues = ValueTableRow.AttributeType;
	...
	For Each TSItem IN TabularSections Do
		...
		For IndexOf = 1 To CurrentTV.Count() Do
			CurrentTV[IndexOf-1].RowId_Versioning = IndexOf;
			CurrentTV[IndexOf-1].Versioning_Modification = False;
		EndDo;
	
	EndDo;
	
	Return ObjVersioning;
...
Procedure AddLineNumbersInTabularSections(TabularSections)
	...
	For Each Map IN TabularSections Do
		...
		If Table.Columns.Find("LineNumber") <> Undefined Then
			Continue;
		EndIf;
		Table.Columns.Insert(0, "LineNumber",,NStr("en='line No.';ru='№ строки';vi='TT dòng'"));
