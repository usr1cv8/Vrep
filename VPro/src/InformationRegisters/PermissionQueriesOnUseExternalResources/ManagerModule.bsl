...
Function PossiblePermissionQueryToUseExternalResources()
	...
	Else
		...
		If Not Users.InfobaseUserWithFullAccess() Then
			...
			Raise NStr("en='Insufficient access rights to request permissions to use external resources.';ru='Недостаточно прав доступа для запроса разрешений на использование внешних ресурсов!';vi='Không đủ quyền truy cập để yêu cầu được phép sử dụng nguồn ngoài!'");
