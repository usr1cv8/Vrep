		...
		For Each Record IN ThisObject Do
			...
			If Record.SafeMode <> CurrentSafeMode Then
				...
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					NStr("en='Safe mode %1 differs from current %2';ru='Безопасный режим %1 отличается от текущего %2';vi='Chế độ an toàn %1 khác với chế độ hiện tại %2'"),
			...
			If ProgramModule <> Catalogs.MetadataObjectIDs.EmptyRef() Then
				...
				If Record.SafeMode <> SoftwareModuleSafeMode Then
					...
					Raise StringFunctionsClientServer.SubstituteParametersInString(
						NStr("en='The permission query cannot be executed from safe mode %2 for application module %1';ru='Для программного модуля %1 не может быть выполнен запрос разрешений из безопасного режима %2';vi='Đối với mô-đun chương trình %1 không thể thực hiện truy vấn cho phép từ chế độ an toàn %2'"),
