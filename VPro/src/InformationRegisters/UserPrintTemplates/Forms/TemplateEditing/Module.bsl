...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If Parameters.OnlyOpening Then
		Title = NStr("en='Open print form template';ru='Открытие макета печатной формы';vi='Mở khuôn mẫu in'");
	EndIf;
	...
	If Not CommonUseClientServer.ThisIsWebClient() AND TemplateType = "MXL" Then
		Items.DoesLabelChangesNotWebClient.Title = NStr("en='After you make changes in the template, click ""End editing""';ru='После внесения необходимых изменений в макет нажмите на кнопку ""Завершить изменение""';vi='Sau khi đưa các thay đổi cần thiết vào khuôn in, hãy nhấn nút ""Kết thúc thay đổi""'");
	EndIf;
...
Procedure SetApplicationNameForTemplateOpening()
	...
	If FileType = "mxl" Then
		ApplicationNameForOpeningTemplate = NStr("en='1C:Enterprise - File operations';ru='1С:Предприятие - Работа с файлами';vi='1C:DOANH NGHIỆP - Làm việc với các tệp'");
		ApplicationAddressForTemplateOpening = "http://1c-dn.com/developer_tools/fileworkshop/";
	ElsIf FileType = "doc" Then
		ApplicationNameForOpeningTemplate = NStr("en='Microsoft Word';ru='Microsoft Word';vi='Microsoft Word'");
		ApplicationAddressForTemplateOpening = "http://office.microsoft.com/en-us/word";
	ElsIf FileType = "odt" Then
		ApplicationNameForOpeningTemplate = NStr("en='OpenOffice Writer';ru='OpenOffice Writer';vi='OpenOffice Writer'");
	...
	InfoForFilling.Insert("ActionsDetails", ?(Parameters.OnlyOpening, NStr("en='openings';ru='открытия';vi='mở'"), NStr("en='make changes';ru='внесения изменений';vi='bổ sung thay đổi'")));
