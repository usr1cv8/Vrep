...
Procedure UpdateOwnerParents(RightSettingsOwner, HasChanges, UpdateHierarchy = False, ObjectsWithChanges = Undefined) Export
	...
	ErrorTitle =
		NStr("en='An error occurred when updating right owner hierarchy by access values.';ru='Ошибка при обновлении иерархии владельцев прав по значениям доступа.';vi='Lỗi khi cập nhật phân nhánh đối tượng chủ của quyền theo giá trị truy cập.'")
		+ Chars.LF
		+ Chars.LF;
...
"типа ""%1"" не настроено использование настроек прав объектов.';vi='Đối với"
"kiểu ""%1"" chưa thiết lập sử dụng tùy chỉnh quyền đối tượng.'"),
			String(OwnerType));
	EndIf;
