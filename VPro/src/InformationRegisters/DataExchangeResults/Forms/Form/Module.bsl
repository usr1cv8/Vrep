...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	If CommonUseReUse.DataSeparationEnabled() AND UseVersioning Then
		...
		Items.CollisionsOtherVersionAuthor.Title = NStr("en='Version is received from the application';ru='Версия получена из приложения';vi='Đã nhận phiên bản từ ứng dụng'");
...
"Версия в этой программе была замененена на версию из другой программы.';vi='Cho phép tự động giải quyết xung đột theo hướng có lợi cho chương trình ""%1""."
"Đã thay thế phiên bản trong chương trình này sang phiên bản từ chương trình khác.'");
...
"Версия в этой программе была сохранена, версия из другой программы была отклонена.';vi='Xung đột được tự động giải quyết theo hướng có lợi cho chương trình này."
"Phiên bản của chương trình này đã được lưu, phiên bản từ chương trình khác bị loại bỏ.'");
...
Procedure AcceptVersionUnaccepted(Command)
	...
	ShowQueryBox(NOTifyDescription, NStr("en='Accept version despite import prohibition?';ru='Принять версию, несмотря на запрет загрузки?';vi='Tiếp nhận dữ liệu dù đã cấm kết nhập?'"), QuestionDialogMode.YesNo, , DialogReturnCode.No);
...
Procedure ConflictResultChange(Command)
	...
	If Items.Collisions.CurrentData <> Undefined Then
		...
		If Items.Collisions.CurrentData.OtherVersionAccepted Then
			...
			QuestionText = NStr("en='Replace the version received from another application with the version of this application?';ru='Заменить версию, полученную из другой программы, на версию из этой программы?';vi='Thay thế phiên bản đã nhận từ chương trình khác sang phiên bản của chương trình này?'");
			
		Else
			
			QuestionText = NStr("en='Replace a version of this application with the version received from another application?';ru='Заменить версию этой программы на версию, полученную из другой программы?';vi='Thay thế phiên bản của chương trình này sang phiên bản đã nhận từ chương trình khác?'");
...
Procedure FillNodeList()
	...
	If ContextOpen AND ExchangeByRulesAbsent Then
		Title = NStr("en='Conflicts during data synchronization';ru='Конфликты при синхронизации данных';vi='Xung đột khi đồng bộ hóa dữ liệu'");
...
Procedure ShowDifference(Item)
	...
	If ComparedVersions.Count() <> 2 Then
		...
		CommonUseClientServer.MessageToUser(NStr("en='There is no version to compare.';ru='Нет версии для сравнения.';vi='Không có phiên bản để so sánh.'"));
		Return;
...
Procedure PageHeaderRefresh()
	...
	SetPageHeader(Items.UnpostedDocumentsPage, NStr("en='Unposted documents';ru='Непроведенные документы';vi='Chứng từ chưa kết chuyển'"), UnpostedDocumentsCount());
	SetPageHeader(Items.BlankAttributesPage, NStr("en='Blank attributes';ru='Незаполненные реквизиты';vi='Mục tin chưa điền'"), CountBlankAttributes());
	
	If UseVersioning Then
		SetPageHeader(Items.CollisionsPage, NStr("en='Conflicts';ru='Конфликты';vi='Xung đột'"), NumberOfColission());
		SetPageHeader(Items.PageUnacceptedByProhibitionDate, NStr("en='Not accepted by closing date';ru='Непринятые по дате запрета';vi='Chưa được tiếp nhận theo ngày cấm'"), NumberOfUnaccepted());
	EndIf;
