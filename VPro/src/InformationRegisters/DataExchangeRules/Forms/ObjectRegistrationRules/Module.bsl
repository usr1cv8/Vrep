...
Procedure ImportRules(Command)
	...
	DialogueParameters.Insert("Title", NStr("en='Specify a file to import the rules from';ru='Укажите, из какого файла загрузить правила';vi='Hãy chỉ ra, quy tắc được kết nhập từ tệp nào'"));
	DialogueParameters.Insert("Filter",
		  NStr("en='Registration rule files (*.xml)';ru='Файлы правил регистрации (*.xml)';vi='Tệp quy tắc ghi nhận (*.xml)'") + "|*.xml|"
		+ NStr("en='ZIP archives (*.zip)';ru='Архивы ZIP (*.zip)';vi='Lưu trữ ZIP (*.zip)'")   + "|*.zip"
	);
...
Procedure UnloadRules(Command)
	...
	NameFilter = NStr("en='Rule files (*.xml)';ru='Файлы правил (*.xml)';vi='Tệp quy tắc (*.xml)'") + "|*.xml";
	...
	If IsBlankString(NameParts.BaseName) Then
		FullFileName = NStr("en='Registration Rules';ru='Правила регистрации';vi='Quy tắc ghi nhận'");
	Else
...
Procedure ImportRulesEnd(Val FilesPlacingResult, Val AdditionalParameters) Export
	...
	If IsBlankString(ErrorText) AND IsBlankString(PlacedFileAddress) Then
		ErrorText = NStr("en='An error occurred when transferring the file to the server';ru='Ошибка передачи файла на сервер';vi='Lỗi truyền tệp đến Server'");
	EndIf;
