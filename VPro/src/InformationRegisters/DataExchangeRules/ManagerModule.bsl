	...
	If IsArchive Then
		...
		If DataExchangeServer.UnpackZIPFile(TemporaryArchiveName, TempFolderName) Then
			...
			If UnpackedFileList.Count() = 0 Then
				NString = NStr("en='When unzipping archive, file with rules was not found.';ru='При распаковке архива не найден файл с правилами.';vi='Khi giải nén lưu trữ không tìm thấy tệp có quy tắc.'");
...
"CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента.';vi='Tên tệp trong phần lưu trữ không tương ứng với tên cần có. Cần có tệp:"
"ExchangeRules.xml - quy tắc chuyển đổi đối với chương trình hiện tại;"
"CorrespondentExchangeRules.xml - quy tắc chuyển đổi đối với chương trình đối ứng.'");
						DataExchangeServer.ShowMessageAboutError(NString, Cancel);
				...
				ElsIf UnpackedFileList.Count() = 1 Then
					NString = NStr("en='Archive contains one file with conversion rules. Expected number of files in the archive is two. Expected files: ExchangeRules.xml - rules for the current application conversion; CorrespondentExchangeRules.xml - rules for the correspondent application conversion.';ru='В архиве найден один файл правил конвертации. Ожидаемое количество файлов в архиве - два. Ожидаются файлы: ExchangeRules.xml - правила конвертации для текущей программы; CorrespondentExchangeRules.xml - правила конвертации для программы-корреспондента.';vi='Trong phần lưu trữ chưa tìm thấy tệp quy tắc chuyển đổi. Số lượng tệp trong lưu trữ là hai. Dự tính tệp: ExchangeRules.xml - quy tắc chuyển đổi cho chương trình hiện tại; CorrespondentExchangeRules.xml - quy tắc chuyển đổi đối với chương trình đối ứng.'");
					DataExchangeServer.ShowMessageAboutError(NString, Cancel);
				// There are several files in the archive but there should be one - deny importing.
				ElsIf UnpackedFileList.Count() > 1 Then
					NString = NStr("en='When unzipping archive, multiple files were found. There should be only one file with rules.';ru='При распаковке архива найдено несколько файлов. Должен быть только один файл с правилами.';vi='Khi giải nén lưu trữ tìm thấy nhiều tệp. Chỉ một tệp có quy tắc.'");
					DataExchangeServer.ShowMessageAboutError(NString, Cancel);
				EndIf;
		...
		Else // If you failed to unpack file - deny importing.
			NString = NStr("en='Cannot unpack an archive with rules.';ru='Не удалось распаковать архив с правилами.';vi='Không thể giải nén lưu trữ có quy tắc.'");
			DataExchangeServer.ShowMessageAboutError(NString, Cancel);
		EndIf;
...
"RegistrationRules.xml - правила регистрации для текущей программы.';vi='Tên tệp trong phần lưu trữ không tương ứng với tên cần có. Cần có tệp:"
"ExchangeRules.xml - quy tắc chuyển đổi cho chương trình hiện tại;"
"CorrespondentExchangeRules.xml - quy tắc chuyển đổi cho chương trình đối ứng;"
"RegistrationRules.xml - quy tắc đăng ký đối với chương trình hiện tại.'");
...
"RegistrationRules.xml - quy tắc đăng ký đối với chương trình hiện tại.'");
	...
	Else
		// If you failed to unpack file - deny importing.
		Raise NStr("en='Cannot unpack an archive with rules.';ru='Не удалось распаковать архив с правилами.';vi='Không thể giải nén lưu trữ có quy tắc.'");
	EndIf;
	...
	If ImportRecordRules.ErrorFlag Then
		Raise NStr("en='An error occurred when importing registration rules.';ru='Ошибка при загрузке правил регистрации.';vi='Lỗi khi kết nhập quy tắc ghi nhận.'");
	EndIf;
...
Procedure DeleteProvidedRules(ExchangePlanName) Export
	...
	For Each RuleKind IN Enums.DataExchangeRuleKinds Do
		...
		If HasErrors Then
			Raise NStr("en='An error occurred when importing rules from configuration.';ru='Ошибка при загрузке правил из конфигурации.';vi='Lỗi khi kết nhập quy tắc từ cấu hình.'");
		Else
			RecordManager.Write();
		EndIf;
...
Function GetBinaryDataFromConfigurationTemplate(Cancel, ExchangePlanName, TemplateName)
	...
	Except
		...
		MessageString = NStr("en='An error occurred when receiving a template of configuration %1 for exchange plan %2';ru='Ошибка получения макета конфигурации %1 для плана обмена %2';vi='Lỗi nhận khuôn mẫu cấu hình %1 đối với sơ đồ trao đổi %2'");
...
Procedure RunFieldsFillCheckup(Cancel, Record)
	...
	If IsBlankString(Record.ExchangePlanName) Then
		...
		NString = NStr("en='Specify exchange plan.';ru='Укажите план обмена.';vi='Hãy chỉ ra sơ đồ trao đổi.'");
	...
	ElsIf Record.RulesSource = Enums.RuleSourcesForDataExchange.ConfigurationTemplate
		...
		NString = NStr("en='Specify standard rules.';ru='Укажите типовые правила.';vi='Hãy chỉ ra quy tắc chung.'");
...
"Возможно выбран не тот файл, либо XML-файл имеет некорректную структуру. Выберите корректный файл.';vi='Không thể xác định dạng quy tắc do lỗi trong việc phân tích tệp XML [FileName]. "
"Có thể đã chọn không phải tệp này hoặc tệp XML có cấu trúc sai. Hãy chọn tệp đúng.'");
...
Function RulesFromFileInformation(RulesFilename)
	...
	If Not ((ExchangeRules.LocalName = "ExchangeRules") AND (ExchangeRules.NodeType = XMLNodeType.StartElement)) Then
		Raise NStr("en='Error in exchange rule format';ru='Ошибка формата правил обмена';vi='Lỗi định dạng quy tắc trao đổi'");
	EndIf;
