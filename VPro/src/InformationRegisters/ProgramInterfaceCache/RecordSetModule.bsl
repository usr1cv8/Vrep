...
Procedure PrepareDataForWrite() Export
	...
	If Not AdditionalProperties.Property("ReceivingParameters", ReceivingParameters) Then
		Raise NStr("en='Data receipt parameters are not specified';ru='Не определены параметры получения данных';vi='Chưa xác định tham số nhận dữ liệu'");
	EndIf;
...
"внутри транзакции записи из сеанса с включенным разделением!';vi='Không cho phép thay đổi nguồn Dữ liệu bản ghi của biểu ghi thông tin ProgramInterfaceCache bên trong giao tác ghi trong số phiên làm việc có bật đường chia tách!'");
