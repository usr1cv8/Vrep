...
Procedure OnOpen(Cancel)
	...
	If Object.ReminderTimeSettingVariant = PredefinedValue("Enum.ReminderTimeSettingVariants.RelativelyToCurrentTime") Then
		ReminderTimeSettingVariant = NStr("en='every';ru='через';vi='qua'") + " " + UserRemindersClientServer.TimePresentation(Object.ReminderInterval);
	Else
...
Procedure ReminderTimeSettingVariantOnChange(Item)
	...
	If TimeInterval > 0 Then
		...
		ReminderTimeSettingVariant = NStr("en='every';ru='через';vi='qua'") + " " + TimeIntervalAsString;
	Else
		If Items.ReminderTimeSettingVariant.ChoiceList.FindByValue(ReminderTimeSettingVariant) = Undefined Then
			CommonUseClientServer.MessageToUser(NStr("en='Time interval not specified.';ru='Интервал времени не определен.';vi='Chưa xác định khoảng thời gian.'"), , "ReminderTimeSettingVariant");
		EndIf;
	EndIf;
...
Procedure TimeIntervalOnChange(Item)
	...
	If Object.ReminderInterval > 0 Then
		TimeIntervalAsString = UserRemindersClientServer.TimePresentation(Object.ReminderInterval);
	Else
		CommonUseClientServer.MessageToUser(NStr("en='Time interval not specified';ru='Интервал времени не определен';vi='Chưa xác định khoảng thời gian'"), , "TimeIntervalAsString");
	EndIf;
EndProcedure
...
Procedure Delete(Command)
	...
	DialogButtons.Add(DialogReturnCode.Yes, NStr("en='Delete';ru='Удалить';vi='Xóa bỏ'"));
	DialogButtons.Add(DialogReturnCode.Cancel, NStr("en=""Don't delete"";ru='Не удалять';vi='Không xóa'"));
	
	NotifyDescription = New NotifyDescription("DeleteReminder", ThisObject);
	ShowQueryBox(NOTifyDescription, NStr("en='Dismiss the reminder?';ru='Удалить напоминание?';vi='Xóa nhắc việc?'"), DialogButtons);
...
Function GetPredefinedWaysNotifications()
	Result = New Map;
	Result.Insert(NStr("en='relative to subject';ru='относительно предмета';vi='theo chủ đề'"), PredefinedValue("Enum.ReminderTimeSettingVariants.RelativelyToSubjectTime"));
	Result.Insert(NStr("en='at a specified time';ru='в указанное время';vi='trong thời gian đã chỉ ra'"), PredefinedValue("Enum.ReminderTimeSettingVariants.InSpecifiedTime"));
	Result.Insert(NStr("en='periodic';ru='периодически';vi='định kỳ'"), PredefinedValue("Enum.ReminderTimeSettingVariants.Periodically"));
	Return Result;
EndFunction
...
Procedure FillOptionsAlerts()
	...
	For Each Interval IN IntervalsTime Do
		OptionsAlerts.Add(NStr("en='every';ru='через';vi='qua'") + " " + Interval);
...
Procedure FillInOptionsForFrequency()
	...
	EndDo;
	Items.FrequencyVariant.ChoiceList.Add("", NStr("en='on the specified schedule...';ru='по заданному расписанию...';vi='theo lịch biểu đã đặt...'"));	
EndProcedure
...
Procedure OpenDialogScheduleSettingsEnd(SelectedSchedule, AdditionalParameters) Export
	...
	If Not ScheduleMeetsRequirements(Schedule) Then 
		ShowMessageBox(, NStr("en='Daily frequency is not supported. Corresponding settings cleared.';ru='Периодичность в течение дня не поддерживается, соответствующие настройки очищены.';vi='Tính định kỳ trong ngày không được hỗ trợ theo tùy chỉnh đã bị xóa.'"));
	EndIf;
