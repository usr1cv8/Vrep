...
""
"Указаны неверные параметры.';vi='Lỗi"
"trong thủ tục UpdateUsersGroups của mô-đun điều khiển biểu ghi thông tin Nhóm giá trị truy cập."
""
"Đã chỉ ra tham số sai.'");
	EndIf;
...
""
"';vi='Lỗi khi cập nhật nhóm giá trị truy cập.'");
...
"типа ""%1"" не настроено использование групп значений доступа.';vi='Đối với"
"dạng ""%1"" chưa thiết lập tùy chỉnh sử dụng nhóm giá trị truy cập.'"),
...
"attribute is not created in the access values type.""%1""';ru='У типа значений доступа ""%1"" не создана специальная табличная часть ГруппыДоступа со специальным реквизитом ГруппаДоступа.';vi='Trong kiểu giá trị truy cập ""%1"" chưa tạo phần bảng đặc biệt AccessGroups với mục tin đặc biệt AccessGroups.'"),
					String(AccessValueType));
			Else
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					ErrorTitle +
...
"с реквизитом ГруппаДоступа по ошибке: %3';vi='Trong giá trị"
"truy cập"
"""%1"" của kiểu ""%2"" không thể"
"đọc phần bảng AccessGroup"
"có mục tin AccessGroup theo lỗi: %3'"),
					String(AccessValue),
					String(AccessValueType),
...
"значений доступа ""%1"" не создан специальный реквизит ГруппаДоступа.';vi='Trong kiểu"
"giá trị truy cập ""%1"" chưa tạo mục tin đặc biệt AccessGroup.'"),
					String(AccessValueType));
			Else
				Raise StringFunctionsClientServer.SubstituteParametersInString(
					ErrorTitle +
...
"реквизит ГруппаДоступа по ошибке: %3';vi='Trong giá trị"
"truy cập"
"""%1"" của kiểu ""%2"" không thể đọc"
"mục tin AccessGroup theo lỗi: %3'"),
					String(AccessValue),
					String(AccessValueType),
					BriefErrorDescription(Error));
			EndIf;
...
""
"Указаны неверные параметры.';vi='Lỗi"
"trong thủ tục UpdatePerformersGroup của mô đun điều khiển biểu ghi thông tin AccessValueGroups."
""
"Đã chỉ ra tham số sai.'");
	EndIf;
