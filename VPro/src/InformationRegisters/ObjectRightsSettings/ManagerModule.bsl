...
"Rights are not set for the %2 table objects.';ru='Ошибка в процедуре РегистрыСведений.НастройкиПравОбъектов.Прочитать() Неверное значение параметра СсылкаНаОбъект ""%1"". Для объектов таблицы ""%2"" права не настраиваются.';vi='Lỗi trong thủ tục InformationRegisters procedure.ObjectRightsSettings.Read() Giá trị tham số sai RefToObject %1."
"Đối với đối tượng của bảng ""%2"" chưa tùy chỉnh quyền.'"),
...
"Требуется исправить данные регистра.';vi='Lỗi trong thủ tục InformationRegisters procedure.ObjectRightsSettings.Read()"
""
"Đối với đối tượng của bảng ""%1"""
"chưa tùy chỉnh quyền ""%2"", tuy nhiên đã ghi lại quyền này"
"trong biểu ghi thông tin ObjectsRightsSettings cho"
"đối t ượng ""%3""."
""
"Có thể, việc cập nhật cơ sở thông tin"
"chưa được thực hiện hoặc đã thực hiện có lỗi."
"Cần chỉnh sửa dữ liệu của biểu ghi.'"),
				ObjectReference.Metadata().FullName(),
				String.Right,
				String(ObjectReference));
		EndIf;
...
""
"Тип владельцев прав ""%1"" не указан в измерении Объект.';vi='Lỗi trong thủ tục UpdateSubordinateRegisterData"
"của mô-đun bộ quản lý biểu ghi thông tin ObjectsRightsSettings."
""
"Chưa chỉ ra kiểu đối tượng chủ của quyền ""%1"" theo chiều đo Đối tượng.'"),
				TypeOwnerRight);
		EndIf;
...
"в процедуре ПриЗаполненииВозможныхПравДляНастройкиПравОбъектов общего модуля УправлениеДоступомПереопределяемый.';vi='Lỗi"
"trong thủ tục OnFillingPossibleRightsForSettingObjectRights của mô-đun chung AccessManagementPredefined.'")
		+ Chars.LF
		+ Chars.LF;
	...
	For Each PossibleRight IN PossibleRights Do
		...
		If MetadataObjectOwner = Undefined Then
			Raise StringFunctionsClientServer.SubstituteParametersInString(
				ErrorTitle + NStr("en='Owner of rights ""%1"" is not found.';ru='Не найден владелец прав ""%1"".';vi='Không tìm thấy đối tượng chủ của quyền ""%1"".'"),
...
"владельца прав ""%1"" не указан в определяемом типе ""Владелец настроек прав"".';vi='Kiểu"
"đối tượng chủ của quyền ""%1"" chưa được chỉ ra trong kiểu xác định ""Đối tượng chủ tùy chỉnh quyền"".'"),
...
"для корректного заполнения регистра НаборыЗначенийДоступа.';vi='Kiểu đối tượng chủ của quyền ""%1"""
"chưa được chỉ ra trong kiểu xác định ""Giá trị truy cập"","
"nhưng được sử dụng để điền tập hợp giá trị truy cập,"
"bởi vì đã chỉ ra trong một lần đăng ký sự kiện:"
"- ЗаписатьЗависимыеНаборыЗначенийДоступа*,"
"- ЗаписатьНаборыЗначенийДоступа*."
"Cần chỉ ra kiểu trong kiểu xác định ""Giá trị truy cập"""
"để điền đúng biểu ghi НаборыЗначенийДоступа."
"'"),
					String(ReferenceType));
			EndIf;
...
"владельца прав ""%1"" не указан в подписке на событие ""Обновить группы значений доступа"".';vi='Kiểu"
"đối tượng chủ của quyền ""%1"" chưa được chỉ ra trong đăng ký sự kiện ""Cập nhật nhóm giá trị truy cập"".'"),
					String(ReferenceType),
...
"как тип групп значений доступа, но обнаружен в описании вида доступа ""%2"".';vi='Kiểu"
"đối tượng chủ của quyền ""%1"" không thể được sử dụng"
"như kiểu nhóm giá trị truy cập, nhưng đã tìm thấy trong mô tả"
"dạng truy cập ""%2""."
"'"),
					String(ReferenceType),
					AccessKindsProperties.ByValuesTypes.Get(ReferenceType).Name);
...
"прав ""%1"" повторно определено право ""%2"".';vi='Đối với đối tượng chủ"
"quyền ""%1"" đã xác định lại quyền ""%2""'"),
				PossibleRight.RightsOwner,
				PossibleRight.Name);
		EndIf;
...
"В этом случае отдельных таблиц указывать не нужно.';vi='Đối với đối tượng chủ"
"của quyền ""%1"" đối với quyền ""%2"" trong các bảng, để đọc đã chỉ ra ký tự ""*""."
"Trong trường hợp này, chưa cần chỉ ra các bảng riêng biệt.'")
				Else
					ErrorDescription = NStr("en='Character ""*"" is"
...
"В этом случае отдельных таблиц указывать не нужно.';vi='Đối với đối tượng chủ"
"của quyền ""%1"" cho quyền ""%2"" trong các bảng, để thay đổi đã chỉ ra ký tự ""*""."
"Trong trường hợp này, không cần chỉ ra các bảng cụ thể.'")
				EndIf;
...
"прав ""%1"" для права ""%2"" в таблицах для чтения указан символ ""*""."
"Однако символ ""*"" уже указан в таблицах для чтения для права ""%3"".';vi='Đối với đối tượng chủ"
"của quyền ""%1"" cho quyền ""%2"" trong các bảng, để đọc đã chỉ ra ký tự ""*""."
"Tuy nhiên, ký tự ""*"" đã chỉ ra trong các bảng để đọc cho quyền ""%3"".'")
				Else
					ErrorDescription = NStr("en='Character ""*"" is"
"specified for the %1 rights owner for %2 right in tables for changing."
"However, the * character is already specified in the tables for changing for %3 right.';ru='Для владельца"
"прав ""%1"" для права ""%2"" в таблицах для изменения указан символ ""*""."
"Однако символ ""*"" уже указан в таблицах для изменения для права ""%3"".';vi='Đối với đối tượng chủ"
"quyền ""%1"" đối với quyền ""%2"" trong các bảng, để thay đổi đã chỉ ra ký tự  ""*""."
"Tuy nhiên, ký tự  ""*"" đã được chỉ ra trong các bảng để thay đổi cho quyền ""%3"".'")
				EndIf;
...
"Имеет смысл использовать только символ ""*"".';vi='Đối tượng chủ của quyền ""%1"""
"đối với quyền ""%2"" đã chỉ ra bảng cụ thể để đọc ""%3""."
"Tuy nhiên, điều này không có ý nghĩa, bởi vì quyền Đọc có thể phụ thuộc chỉ vào quyền Đọc."
"Có ý nghĩa sử dụng chỉ ký tự ""*"".'");
...
"прав ""%1"" для права ""%2"" не найдена таблица для чтения ""%3"".';vi='Đối với đối tượng chủ"
"của quyền ""%1"" cho quyền ""%2"" chưa tìm thấy bảng để đọc ""%3"".'")
			Else
				ErrorDescription = NStr("en='For the %1"
"rights owner for %2 right the table for changing %3 is not found.';ru='Для владельца"
"прав ""%1"" для права ""%2"" не найдена таблица для изменения ""%3"".';vi='Đối với đối tượng chủ"
"quyền ""%1"" cho quyền ""%2"" chưa tìm thấy bảng để thay đổi ""%3"".'")
			EndIf;
