	...
	Except
		...
		If RateCalculationErrorByFormula[Currency] = Undefined Then
			...
			ErrorText = StringFunctionsClientServer.SubstituteParametersInString(
				NStr("en='Calculation of exchange rate ""%1"" using formula ""%2"" is not executed:';ru='Расчет курса валюты ""%1"" по формуле ""%2"" не выполнен:';vi='Tính tỷ giá ngoại tệ ""%1"" theo công thức ""%2"" chưa thực hiện:'", CommonUseClientServer.MainLanguageCode()), Currency, Formula);
			...
			Else
				WriteLogEvent(NStr("en='Currency.Exchange rates import';ru='Валюты.Загрузка курсов валют';vi='Tiền tệ.Kết nhập tỷ giá hối đoái'", CommonUseClientServer.MainLanguageCode()),
