	...
	If Parameters.FillingValues.Property("Application")
	   AND ValueIsFilled(Parameters.FillingValues.Application) Then
		...
		Title = StringFunctionsClientServer.SubstituteParametersInString(
			NStr("en='Path to application %1 on Linux server';ru='Путь к программе %1 на сервере Linux';vi='Đường dẫn đến chương trình %1 trên Server Linux'"), Parameters.FillingValues.Application);
