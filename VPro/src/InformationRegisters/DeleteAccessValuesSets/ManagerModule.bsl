...
Function MoveDataToNewTable() Export
	...
	If Selection.Count() < 10000 Then
		...
		WriteLogEvent(
			...
			,
			,
			NStr("en='Data transfer from the DeleteAccessValuesSets register is complete.';ru='Завершен перенос данных из регистра УдалитьНаборыЗначенийДоступа.';vi='Đã hoàn tất chuyển dữ liệu từ biểu ghi УдалитьНаборыЗначенийДоступа.'"),
	...
	Else
		WriteLogEvent(
			...
			,
			,
			NStr("en='Step of data transfer from register DeleteAccessValuesSets is executed.';ru='Выполнен шаг переноса данных из регистра DeleteAccessValuesSets.';vi='Đã thực hiện bước chuyển dữ liệu từ biểu ghi DeleteAccessValuesSets.'"),
			EventLogEntryTransactionMode.Transactional);
	EndIf;
