...
Procedure OnCreateAtServer(Cancel, StandardProcessing)
	...
	Title = "Products" + " " + NStr("en='for';ru='за';vi='trong'") + " " + Device;
...
Procedure FilterOnChange(Item)
	...
	Status(NStr("en='Goods table is being updated...';ru='Выполняется обновление таблицы товаров...';vi='Đang cập nhật bảng hàng hóa...'"));
...
Procedure RecordChanges(Command)
	...
	If CodesArray.Count() > 0 Then
		...
		Else
			ShowMessageBox(Undefined, NStr("en='During the change registration an error occurred:';ru='В процессе регистрации изменений произошла ошибка:';vi='Trong quá trinh ghi nhận thay đổi, đã xảy ra lỗi:'") + " " + ErrorDescription);
		EndIf;
	Else
		ShowMessageBox(Undefined, NStr("en='Rows for change registration are not selected';ru='Не выбраны строки для регистрации изменений';vi='Chưa chọn dòng để ghi nhận thay đổi'"));
	EndIf;
...
Procedure DeleteChangesRegistrationForSelectedStrings(Command)
	...
	If CodesArray.Count() > 0 Then
		...
		If Result Then
			...
			Notify("Record_CodesOfGoodsPeripheral", New Structure, Undefined);
		Else
			ShowMessageBox(Undefined, NStr("en='An error occurred during the change registration deletion:';ru='В процессе удаления регистрации изменений произошла ошибка:';vi='Trong quá trình xóa ghi nhận thay đổi, đã xảy ra lỗi:'") + " " + ErrorDescription);
		EndIf;
	Else
		ShowMessageBox(Undefined, NStr("en='Rows for change registration deletion are not selected';ru='Не выбраны строки для удаления регистрации изменений';vi='Chưa chọn dòng để xóa ghi nhận thay đổi'"));
	EndIf;
