...
Procedure FillCheckProcessingAtServer(Cancel, CheckedAttributes)
	...
	If Selection.Next() // Barcode is already written in the database
		...
		ErrorDescription = StrReplace(ErrorDescription, "%ProductsAndServices%", """" + Selection.ProductsAndServicesPresentation + """"
						...
						+ ?(ValueIsFilled(Selection.Batch), " " + NStr("en='with the batch';ru='с партией';vi='với lô'") + " """ + Selection.BatchPresentation + """", "")
						+ ?(ValueIsFilled(Selection.MeasurementUnit), " " + NStr("en='UOM';ru='Единицы измерения';vi='Đơn vị tính'") + " """ + Selection.MeasurementUnitPresentation + """", ""));
